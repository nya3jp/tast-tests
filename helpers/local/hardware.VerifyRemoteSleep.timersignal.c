...
static void handler(int sig) {
    ...
    if (gettimeofday(&tv, NULL)) {
        send_msg("gettimeofdayERR: ping\n");
    } else {
        char msg[256];

        /* WARNING ctime and localtime are thread unsafe,
           do not add concurrent calls to this prog */
        struct tm *lt = localtime(&tv.tv_sec);
        snprintf(msg, sizeof(msg) - 1, "%02d:%02d:%02d.%03lu ping\n",
...
int main(int argc, char* argv[]) {
    ...
    msecs = safe_atoll(argv[1]);
    iterations = safe_atoll(argv[2]);
