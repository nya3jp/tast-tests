...
var (
	...
	nameDetectNVMe = regexp.MustCompile(`\s*Serial Number:\s+(?P<param>\S+).*`)
	// nameDetectSATA detects the name of a SATA-based device using a regex.
	nameDetectSATA = regexp.MustCompile(`\s*Serial Number:\s+(?P<param>\S+).*`)
...
func parseDeviceNameSATA(outLines []string) string {
	for _, line := range outLines {
		match := nameDetectSATA.FindStringSubmatch(line)
		if match != nil {
			return match[1]
		}
	}
