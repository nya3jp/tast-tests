...
// NOTE: While AES, SHA256 and PBKDF2 is used here, but it is not for security,
// and is only used for speedy generation of test data that can be reproduced.

// AppendFile appends to the the file at path, for a size of length32*32 bytes.
...
func AppendFile(ctx context.Context, runner hwsec.CmdRunner, path string, key string, length32 int) error {
