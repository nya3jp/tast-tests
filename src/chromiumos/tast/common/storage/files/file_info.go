...
// NewFileInfo create a new FileInfo.
func NewFileInfo(ctx context.Context, path string, runner hwsec.CmdRunner) (*FileInfo, error) {
	...
	if err := ResetFile(ctx, runner, path); err != nil {
		return nil, errors.Wrapf(err, "failed to reset file %q", path)
	}
...
func (f *FileInfo) Step(ctx context.Context) error {
	...
	UpdateHashForIteration(f.hash, k, l)
...
// The returned int is the number of KiB to write.
