...
// getTestfileTemplates returns the test file templates.
// Golang doesn't have constant array so we've this function.
func getTestfileTemplates() []string {
	return []string{testfile1, testfile2, testfile3, testfile4, testfile5}
}

// getTestfiles returns the array of test file paths, given the user's
// obfuscated username.
func getTestfiles(sanitizedUsername string) []string {
	t := getTestfileTemplates()
	var result []string
	for _, s := range t {
