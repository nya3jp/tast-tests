...
func createDockerClient(ctx context.Context) (*client.Client, error) {
	...
	if _, err := os.Stat("/var/run/docker.sock"); err != nil {
		if !errors.Is(err, os.ErrNotExist) {
			testing.ContextLogf(ctx, "createDockerClient: os.Stat /var/run/docker.sock")
			return nil, err
		}
		testing.ContextLogf(ctx, "createDockerClient:/var/run/docker.sock not found, creating TCP connect")
		return client.NewClientWithOpts(client.WithHost("tcp://192.168.231.1:2375"), client.WithAPIVersionNegotiation())
	}
	testing.ContextLogf(ctx, "createDockerClient:/var/run/docker.sock found, creating Sock connect")
...
func NewProxy(ctx context.Context, servoHostPort, keyFile, keyDir string) (newProxy *Proxy, retErr error) {
	...
	if strings.Contains(host, "docker_servod") {
		testing.ContextLogf(ctx, "NewProxy:Creating Docker Client object")
		pxy.dcl, err = createDockerClient(ctx)
		if err != nil {
			testing.ContextLogf(ctx, "NewProxy:Failed to create docker client")
			return nil, err
		}
		pxy.sdc = host
	}
