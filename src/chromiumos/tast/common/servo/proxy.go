...
func createDockerClient() (*client.Client, error) {
	...
	if _, err := os.Stat("/var/run/docker.sock"); err != nil {
		...
		testing.ContextLogf(ctx, "Creating Docker Client over the TCP.")
...
func (p *Proxy) dockerExec(ctx context.Context, stdin io.Reader, name string, args ...string) ([]byte, error) {
	...
	if p.sdc == "" {
		err := errors.New("dockerExec:Servod is not running inside a docker container")
		return nil, err
	}
	...
	if err != nil {
		testing.ContextLog(ctx, "dockerExec: Could not create exec configuration to run an exec proces.")
		return nil, err
	}
	...
	go func(dcl *client.Client) {
		...
		for {
			...
			if err != nil {
				testing.ContextLog(ctx, "dockerExec: Running exec process timed out.")
				break
			}
		}
	}(p.dcl)
