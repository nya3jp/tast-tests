...
func (p *Proxy) dockerExec(ctx context.Context, stdin io.Reader, name string, args ...string) ([]byte, error) {
	...
	testing.ContextLog(ctx, "ClientVersion.......", p.dcl.ClientVersion())

	serv, err := p.dcl.ServerVersion(ctx)
	if err != nil {
	...
	containers, err := p.dcl.ContainerList(ctx, types.ContainerListOptions{})
	if err != nil {
	...
	go func(dcl *client.Client) {
		defer wg.Done()
		for {
			iRes, err := p.dcl.ContainerExecInspect(ctx, r.ID)
