...
// NetCertStore is a struct that contains the information related to using the TPM to store certificates/keys during the test. Strictly speaking, this struct holds the information required to access a chaps slot/token.
// Note that NetCertStore is currently a singleton because users of this struct (network tests) only need one NetCertStore at a moment and handling multiple NetCertStore at a time adds unnecessary complexity.
...
// CleanupNetCertStore resets the environment (chaps keystore and cryptohome vault) back to the state before the NetCertStore instance is created.
