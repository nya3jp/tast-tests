...
// ImportPEMKeyAndCertBySlot create a key by importing it from existing PEM format private key specified by privKey, and the associated certificate in PEM format certificate specified by cert. The key will be inserted into the token specified by slot. The object will have an ID of objID. It is OK for either privKey or cert to be empty if they are not needed.
func (p *Chaps) ImportPEMKeyAndCertBySlot(ctx context.Context, scratchpadPath, privKey, cert, objID string, slot int) (*KeyInfo, error) {
	...
	if privKey != "" {
		...
		keyPemPath := filepath.Join(scratchpadPath, uuid.New().String()+".der")
	...
	if cert != "" {
		// Generate temp file path for the PEM to DER conversion.
		certPemPath := filepath.Join(scratchpadPath, uuid.New().String()+".der")
		...
		// Convert the PEM file to DER file.
		if msg, err := p.runner.Run(ctx, "openssl", "x509", "-in", certPemPath, "-out", certDerPath, "-inform", "PEM", "-outform", "DER"); err != nil {
