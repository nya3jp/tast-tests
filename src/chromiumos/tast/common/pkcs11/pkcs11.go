...
func (p *Util) CreateRsaSoftwareKey(ctx context.Context, utility utilityToCryptohome, username string, keyname string, objID string, forceSoftwareBacked bool, checkSoftwareBacked bool) (*KeyInfo, error) {
	...
	if checkSoftwareBacked {
		isSoftwareBacked, err := result.IsSoftwareBacked(ctx)
		if err != nil {
...
func (key *KeyInfo) GetObjectAttribute(ctx context.Context, objType string, attributeName string) (string, string, error) {
	// Execute the command and convert its output.
	binaryMsg, err := key.util.runner.Run(ctx, "p11_replay", "--get_attribute", "--slot="+strconv.Itoa(key.slot), "--id="+key.objID, "--attribute="+attributeName, "--type="+objType)
	var msg string
	if binaryMsg != nil {
		msg = string(binaryMsg)
	}
...
func (key *KeyInfo) SetObjectAttribute(ctx context.Context, objType string, attributeName string, attributeValue string) (string, error) {
	// Execute the command and convert its output.
	binaryMsg, err := key.util.runner.Run(ctx, "p11_replay", "--set_attribute", "--slot="+strconv.Itoa(key.slot), "--id="+key.objID, "--attribute="+attributeName, "--data="+attributeValue, "--type="+objType)
	var msg string
	if binaryMsg != nil {
		msg = string(binaryMsg)
	}
...
func (key *KeyInfo) IsSoftwareBacked(ctx context.Context) (bool, error) {
	isSoftwareBackedStr, msg, err := key.GetObjectAttribute(ctx, "privkey", "kKeyInSoftware")
	if err != nil {
