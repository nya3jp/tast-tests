...
// bytes is the amount of data sent over the wire.
//
	...
	for len(srcFilesAndDirsQueue) > 0 {
		...
		if srcFileInfo.IsDir() {
			...
			for _, dirFileInfo := range dirFiles {
				dirFile, err := filepath.Abs(dirFileInfo.Name())
				if err != nil {
				...
				if _, ok := absoluteFiles[dirFile]; !ok {
					absoluteFiles[dirFile] = filepath.Join(dirDst, filepath.Base(dirFile))
