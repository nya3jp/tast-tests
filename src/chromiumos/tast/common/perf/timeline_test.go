...
type fakeClock struct {
	...
	// sem is semaphore whose value represents the amount of Advance()-ed fake time in milliseconds. A sleeping goroutine will wait for fake time to advance by Aquire()-ing from this semaphore.
...
// NewSleepSemaphore returns a new semaphore for fakeClock. The value of the semaphore is 0 in beginning.
func NewSleepSemaphore() *semaphore.Weighted {
	s := semaphore.NewWeighted(math.MaxInt64)
	s.Acquire(context.Background(), math.MaxInt64)
	return s
}
