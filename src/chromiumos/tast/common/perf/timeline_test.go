...
// makeTimeout creates a channel that is closed after a given time duration.
func makeTimeout(duration time.Duration) chan interface{} {
...
func (d *testTimelineDatasource) WaitForSnapshot() {
	...
	case <-makeTimeout(5 * time.Second):
...
func NewFakeClock() *fakeClock {
	return &fakeClock{clock: time.Unix(0, 0), sleepChannel: make(chan interface{})}
}
