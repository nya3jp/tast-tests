...
// marshalUserProto iterates over all the user policies and set their corresponding proto message
// then appends the base64 encoded value to the policies array.
...
// set their corresponding proto messages, then appends the base64 encoded value to the policies array.
...
// marshalExtensionPM marshals the raw json associated with each extension id, then
// appends the base64 encoded value to the externalPolicies array.
...
// unmarshalUserProto sets the userProto in the blob for the unmarshalled policies array.
// There is only one entry with PolicyTypeUser in the policies array.
func (pb *Blob) unmarshalUserProto(policies []entry) {
	for _, policy := range policies {
		if policy.PolicyType == PolicyTypeUser {
			b, _ := base64.StdEncoding.DecodeString(policy.Value)
			userProto := &empb.CloudPolicySettings{}
			proto.Unmarshal(b, userProto)
...
// unmarshalDeviceProto sets the DeviceProto in the blob for the unmarshalled policies array.
// There is only one entry with PolicyTypeDevice in the policies array.
func (pb *Blob) unmarshalDeviceProto(policies []entry) {
	for _, policy := range policies {
		if policy.PolicyType == PolicyTypeDevice {
			b, _ := base64.StdEncoding.DecodeString(policy.Value)
			deviceProto := &empb.ChromeDeviceSettingsProto{}
			proto.Unmarshal(b, deviceProto)
...
// unmarshalPublicAccountsMapProto sets the publicAccountsMapProto in the blob for the unmarshalled policies array.
func (pb *Blob) unmarshalPublicAccountsMapProto(policies []entry) {
	publicAccountsMapProto := make(map[string]*empb.CloudPolicySettings)
	for _, policy := range policies {
		if policy.PolicyType == PolicyTypePublicAccount {
			b, _ := base64.StdEncoding.DecodeString(policy.Value)
			publicAccountProto := &empb.CloudPolicySettings{}
			proto.Unmarshal(b, publicAccountProto)
...
// unmarshalExtensionPM sets the ExtensionPM in the blob for the unmarshalled external_policies array.
func (pb *Blob) unmarshalExtensionPM(extPolicies []entry) {
...
func (pb *Blob) UnmarshalJSON(jsonData []byte) error {
	...
	// Extract the policies array and unmarshal user, device and public accounts policies protos.
	if policiesRawJSON, ok := objmap["policies"]; ok {
		var policies []entry
		json.Unmarshal(*policiesRawJSON, &policies)
		pb.unmarshalUserProto(policies)
		pb.unmarshalDeviceProto(policies)
		pb.unmarshalPublicAccountsMapProto(policies)
	}
	...
	if extPoliciesRawJSON, ok := objmap["external_policies"]; ok {
		...
		json.Unmarshal(*extPoliciesRawJSON, &extPolicies)
		pb.unmarshalExtensionPM(extPolicies)
	}
	return nil
}
