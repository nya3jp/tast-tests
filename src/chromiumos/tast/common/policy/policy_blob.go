...
func (pb *Blob) MarshalJSON() ([]byte, error) {
	...
	// For each public account id, create an empty CloudPolicy proto or use an
	// existing proto, then iterate over all the user policies associated with
	...
	// Add public account protos to the policies array.
...
// setUserProto sets the userProto in the blob for the unmarshalled policies array.
// There is only one entry with PolicyTypeUser in the policies array.
func (pb *Blob) setUserProto(policies []entry) {
...
// setDeviceProto sets the DeviceProto in the blob for the unmarshalled policies array.
// There is only one entry with PolicyTypeDevice in the policies array.
func (pb *Blob) setDeviceProto(policies []entry) {
	for _, policy := range policies {
...
// setPublicAccountsMapProto sets the publicAccountsMapProto in the blob for the unmarshalled policies array.
func (pb *Blob) setPublicAccountsMapProto(policies []entry) {
	publicAccountsMapProto := make(map[string]*empb.CloudPolicySettings)
	for _, policy := range policies {
...
// setExtensionPM sets the ExtensionPM in the blob for the unmarshalled external_policies array.
func (pb *Blob) setExtensionPM(extPolicies []entry) {
...
func (pb *Blob) UnmarshalJSON(jsonData []byte) error {
	...
	if policiesRawJSON, ok := objmap["policies"]; ok {
		...
		pb.setUserProto(policies)
		pb.setDeviceProto(policies)
		pb.setPublicAccountsMapProto(policies)
	}
