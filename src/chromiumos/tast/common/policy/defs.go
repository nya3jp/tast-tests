...
type KerberosAccountsValueOmit struct {
	Password         string `json:"password"`
	Principal        string `json:"principal"`
	RememberPassword bool   `json:"remember_password"`
}

func (v *KerberosAccountsValueOmit) SetPassword(password string) { v.Password = password }
...
func (p *KerberosAccounts) UnmarshalAs(m json.RawMessage) (interface{}, error) {
	var v []*KerberosAccountsValueIf
	if err := json.Unmarshal(m, &v); err != nil {
		return nil, errors.Wrapf(err, "could not read %s as []*KerberosAccountsValueIf", m)
	}
	return v, nil
}
