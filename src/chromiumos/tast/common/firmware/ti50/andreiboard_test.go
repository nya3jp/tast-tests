...
func createDut(ctrl *gomock.Controller, bufLen int) (*andreiBoard, *mocks.MockSerialPort) {
	p := mocks.NewMockSerialPort(ctrl)
...
func TestSerial(t *testing.T) {
	expectRead := func(port *mocks.MockSerialPort, inLen int, fill []byte) *gomock.Call {
		err := error(nil)
		if fill == nil {
			err = errors.New("EOF")
		}
		return port.EXPECT().Read(nil, gomock.Any()).Return(len(fill), err).Do(func(ctx context.Context, b []byte) {
			if len(b) != inLen {
	...
	var dut *andreiBoard
	var port *mocks.MockSerialPort

        t.Log("Read error should result in same error.")
	dut, port = createDut(ctrl, 5)
	expectRead(port, 5, nil)
	checkMatch(errors.New("EOF"), "")(dut.ReadSerialSubmatch(nil, regexp.MustCompile("abc")))

        t.Log("Matched string should be returned.")
	dut, port = createDut(ctrl, 5)
	expectRead(port, 5, []byte("abc"))
	checkMatch(nil, "abc")(dut.ReadSerialSubmatch(nil, regexp.MustCompile("abc")))
	...
	checkMatch(nil, "ab")(dut.ReadSerialSubmatch(nil, regexp.MustCompile("ab")))
	checkMatch(nil, "c")(dut.ReadSerialSubmatch(nil, regexp.MustCompile("c")))
	checkMatch(errors.New("Could not find match"), "")(dut.ReadSerialSubmatch(nil, regexp.MustCompile("c")))
	...
	t.Log("Buffer full condition should result it error")
	dut, port = createDut(ctrl, 5)
	gomock.InOrder(
		expectRead(port, 5, []byte("abcde")),
	)
	checkMatch(errors.New("Buffer is full"), "")(dut.ReadSerialSubmatch(nil, regexp.MustCompile("f")))
	...
	checkMatch(nil, "cde")(dut.ReadSerialSubmatch(nil, regexp.MustCompile("cde")))
	...
	checkMatch(nil, "abcde", "ab", "cde")(dut.ReadSerialSubmatch(nil, regexp.MustCompile("(..)(...)")))
	...
	checkMatch(nil, "ab\ncd", "b\ncd")(dut.ReadSerialSubmatch(nil, regexp.MustCompile("(?s)a(.*)")))
