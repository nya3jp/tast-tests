...
func CreateDutPtyPair(ctx context.Context, dut *dut.DUT) (s1, s2 string, cancelf func(), done <-chan error, err error) {
...
// their names, along with cancel and done to manage their lifetimes.
func CreateHostPtyPair(ctx context.Context) (s1, s2 string, cancelf func(), done <-chan error, err error) {
...
func createPtyPair(ctx context.Context, cmd iCmd) (s1, s2 string, cancelf func(), done <-chan error, err error) {
	...
	return s1, s2, cmd.Abort, doneC, nil
}

// DoTestRead tests Port.Read.
func DoTestRead(ctx context.Context, logf func(...interface{}), o1, o2 PortOpener) error {
...
func DoTestWrite(ctx context.Context, logf func(...interface{}), o1, o2 PortOpener) error {
	logf("Open should work")
	p1, err := o1.OpenPort(ctx)
	if err != nil {
		return errors.Wrap(err, "open 1 errored")
...
func DoTestFlush(ctx context.Context, logf func(...interface{}), o1, o2 PortOpener) error {
	logf("Open should work")
	p1, err := o1.OpenPort(ctx)
	if err != nil {
		return errors.Wrap(err, "open 1 errored")
	...
	if err == nil || n != 0 {
		return errors.Errorf("read1 should have failed: %v n: %d buf: %s", err, n, string(buf[:n]))
	}
	logf("Read errored as expected: " + err.Error())
	...
	if err == nil {
		return errors.Wrap(err, "read2 should have failed")
	}
	logf("Read errored as expected: " + err.Error())
	testing.Sleep(ctx, 50*time.Millisecond)
	err = p2.Flush(ctx)
