...
func findMaxAPWPRange(ctx context.Context) (string, error) {
	...
	for i := 0; i < 3; i++ {
		...
		// Match output for largest range. Output looks like: `start=0x00000000 length=0x01000000 (all)`
		maxRange := regexp.MustCompile(`start=(0[xX][0-9a-fA-F]+)\s*length=(0[xX][0-9a-fA-F]+)\s*\(([^\r\n]+)\)`)
		match := maxRange.FindAllStringSubmatch(string(out), -1)
		if match != nil && match[len(match)-1][3] == "all" {
