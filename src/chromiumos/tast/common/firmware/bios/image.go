...
// WriteImageToFile writes image data to a tmp file to use for flashrom command.
func (i *Image) WriteImageToFile(ctx context.Context, sec ImageSection) (string, error) {
	dataRange, ok := i.Sections[sec]
	if !ok {
		return "", errors.Errorf("section %q is not recognized", string(sec))
	}

	imgFile, err := ioutil.TempFile("/var/tmp", "")
	if err != nil {
		return "", errors.Wrap(err, "creating tmpfile for image contents")
	}
