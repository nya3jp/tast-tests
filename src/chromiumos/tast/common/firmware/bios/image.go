...
func EnableAPSoftwareWriteProtect(ctx context.Context) error {
	...
	if err := testexec.CommandContext(ctx, "flashrom", "-p", "host", "-r", "/tmp/testfile.bin").Run(testexec.DumpLogOnError); err != nil {
		return errors.Wrap(err, "failed to read the bios file")
	}

	out, err := testexec.CommandContext(ctx, "fmap_decode", "/tmp/testfile.bin").Output(testexec.DumpLogOnError)
	if err != nil {
