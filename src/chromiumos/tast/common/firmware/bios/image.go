...
func SetAPSoftwareWriteProtect(ctx context.Context, enable bool, args *WPArgs) error {
	...
	if args != nil && args.WPRangeStart != -1 && args.WPRangeLength != -1 {
		...
		// TODO(tij@): There is an ongoing issue with --wp-region argument resulting
		// in segfaults, refer to bug b/242010682.
		...
		if err := testexec.CommandContext(ctx, "flashrom", "-p", "host", "-r", "-i", fmt.Sprintf("FMAP:%s", tmpFile.Name())).Run(testexec.DumpLogOnError); err != nil {
			return errors.Wrap(err, "failed to read the file")
		}

		fmap, err := testexec.CommandContext(ctx, "dump_fmap", "-p", tmpFile.Name()).Output(testexec.DumpLogOnError)
		if err != nil {
			return errors.Wrap(err, "could not dump_fmap on firmware host image")
		}
		info, err := ParseSections(string(fmap))
		if err != nil {
			return errors.Wrap(err, "could not parse dump_fmap output")
		}
		sec := info[args.WPSection]
		rangeStr = fmt.Sprintf("--wp-range=%x,%x", sec.Start, sec.Length)
		if out, err := testexec.CommandContext(ctx, "flashrom", wpCmd...).Output(testexec.DumpLogOnError); err != nil {
			return errors.Wrapf(err, "unable set write protection to %s with flashrom: %s", expState, string(out))
		}
		wpCmd = []string{"-p", "host", rangeStr}
		// return errors.Errorf("flashrom: %s", strings.Join(wpCmd, " "))

		// out, err := testexec.CommandContext(ctx, "fmap_decode", tmpFile.Name()).Output(testexec.DumpLogOnError)
		// if err != nil {
		// 	return errors.Wrapf(err, "failed to decode the bios file: %v", string(out))
		// }
		...
		// // First set wp, then run again with specific region
		// if out, err := testexec.CommandContext(ctx, "flashrom", wpCmd...).Output(testexec.DumpLogOnError); err != nil {
		// 	return errors.Wrapf(err, "unable set write protection to %s with flashrom: %s", expState, string(out))
		// }
		// wpCmd = []string{"-p", "host", fmt.Sprintf("--wp-region=%s", regionName)}
		// wpCmd = append(wpCmd, fmt.Sprintf("--wp-region=%s", regionName))
	}
