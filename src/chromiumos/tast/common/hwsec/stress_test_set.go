...
type attestationFlow interface {
	...
	CreateEnrollRequest(ctx context.Context, pcaType int) (string, error)
	// Finishes the enroll with |resp| from pca server of |pcaType|. Returns any
	// encountered error during the operation.
	FinishEnroll(ctx context.Context, pcaType int, resp string) error
...
func (r *StressTaskEnroll) RunTask(ctx context.Context) error {
	...
	req, err := r.af.CreateEnrollRequest(ctx, a9n.DefaultACAType)
	if err != nil {
	...
	err = r.af.FinishEnroll(ctx, a9n.DefaultACAType, resp)
	if err != nil {
