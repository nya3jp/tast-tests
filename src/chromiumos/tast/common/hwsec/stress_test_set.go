...
func (r *StressTaskEnroll) RunTask(ctx context.Context) error {
	...
	resp, err := SendPostRequestTo(ctx, req, "https://chromeos-ca.gstatic.com/enroll")
	if err != nil {
...
func (r *StressTaskCert) RunTask(ctx context.Context) error {
	...
	resp, err := SendPostRequestTo(ctx, req, "https://chromeos-ca.gstatic.com/sign")
	if err != nil {
		return errors.Wrap(err, "failed to send request to CA")
	}
	if len(resp) == 0 {
...
type vaultController interface {
	...
	MountVault(ctx context.Context, username string, password string, label string, create bool) error

	// Checks the vault for |username|.
	CheckVault(ctx context.Context, username string, password string, label string) (bool, error)
...
func (r *stressTaskMount) RunTask(ctx context.Context) error {
	username := r.username
	passwd := r.passwd
	err := r.vc.MountVault(ctx, username, passwd, "dontcare", true)
	if err != nil {
...
func (r *stressTaskCheckKey) RunTask(ctx context.Context) error {
	result, err := r.vc.CheckVault(ctx, r.username, r.passwd, "dontcare")
	if err != nil {
