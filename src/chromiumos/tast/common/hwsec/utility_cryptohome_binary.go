...
func (u *UtilityCryptohomeBinary) GetSanitizedUsername(ctx context.Context, username string, useDBus bool) (string, error) {
	...
	if err != nil {
		return "", errors.Wrap(err, "invalid regular expression in GetSanitizedUsername")
	}
	if !matched {
		return "", errors.Errorf("invalid sanitized username %q", output)
	}
	return output, nil
}
...
func (u *UtilityCryptohomeBinary) GetSystemSalt(ctx context.Context, useDBus bool) (string, error) {
	...
	if err != nil {
		return "", errors.Wrap(err, "invalid regular expression in GetSystemSalt")
	}
	if !matched {
