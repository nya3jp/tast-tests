...
func (u *UtilityCryptohomeBinary) MountVault(ctx context.Context, username string, password string, label string, create bool) error {
	_, err := u.binary.MountEx(ctx, username, password, create, label)
	if err != nil {
...
func (u *UtilityCryptohomeBinary) GetFirmwareManagementParameters(ctx context.Context) (string, string, string, string, error) {
	...
	if err != nil {
		...
		if haveError {
			return "", "", msg, errorCode, errors.Wrapf(err, "failed to call GetFirmwareManagementParameters command, error %q", errorCode)
		}
		return "", "", msg, "", errors.Wrap(err, "failed to call GetFirmwareManagementParameters with unknown error")
	}

	// return the hash and flags if they exist, and return error otherwise.
	flags, haveFlags := params[flagsPrefix]
	hash, haveHash := params[hashPrefix]
	if !haveFlags {
		return "", "", msg, "", errors.New("no flags in GetFirmwareManagementParameters output")
	}
	if !haveHash {
		return "", "", msg, "", errors.New("no hash in GetFirmwareManagementParameters output")
	}

	return flags, hash, msg, "", nil
}
...
// FirmwareManagementParametersInfo contains the informations regarding FWMP, so that it can be backed up and restored.
...
func (u *UtilityCryptohomeBinary) BackupFWMP(ctx context.Context) (*FirmwareManagementParametersInfo, error) {
	flags, hash, _, errorCode, err := u.GetFirmwareManagementParameters(ctx)
	if err != nil {
		if errorCode != "CRYPTOHOME_ERROR_FIRMWARE_MANAGEMENT_PARAMETERS_INVALID" {
...
// RestoreFWMP restores the FWMP from fwmp in parameter, and return nil iff the operation is sucessful.
