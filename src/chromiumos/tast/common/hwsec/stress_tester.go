...
type StressTestStatus struct {
	...
	Count int
	// Errors records the error occuring during stress test (#iteration -> error)
...
func (s *StressTaskSequence) RunTask() error {
	for _, subTask := range s.tasks {
		if err := subTask.RunTask(); err != nil {
			panic(err)
			return err
		}
	}
...
// NewStressTaskSequence creats a new StressTaskSequence of |tasks|.
...
func (st *StressTester) Run(wg *sync.WaitGroup) {
	defer func() {
		if wg != nil {
			wg.Done()
		}
	...
	for st.c.ShallRun() {
		if err := st.runner.RunTask(); err != nil {
...
// RunTask implements the one of StressTaskRunner.
func (s *StressTaskRunnerConcurrentOnce) RunTask() error {
