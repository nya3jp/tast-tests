...
func parseStringMap(ctx context.Context, msg string, checkMatch bool,
	prefixes []string) (map[string]string, error) {
			...
			if strings.HasPrefix(line, prefix) {
				if _, found := parsed[prefix]; found {
					testing.ContextLogf(ctx,
						"Command have duplicate prefix, message %q", msg)
					return map[string]string{},
						errors.Errorf("duplicate prefix %q found", prefix)
				}
	...
	if checkMatch && len(parsed) != len(prefixes) {
		return map[string]string{},
			errors.Errorf("missing attribute/prefix, message %q", msg)
	}
	return parsed, nil
}
...
func parseNonsensitiveStatusInfo(ctx context.Context, checkStatus bool, msg string) (info *NonsensitiveStatusInfo, returnedError error) {
	...
	if err != nil {
		return &NonsensitiveStatusInfo{},
			errors.Wrap(err, "failed to parse string map")
	}
	...
	if checkStatus {
		...
		if parsed[StatusPrefix] != tpmManagerStatusSuccessMessage {
			return &NonsensitiveStatusInfo{},
				errors.Errorf(
					"incorrect status %q from NonsensitiveStatusInfo",
					parsed[StatusPrefix])
		}
	}
	...
	if _, err := fmt.Sscanf(parsed[IsEnablePrefix], "%t", &isEnabled); err != nil {
		return &NonsensitiveStatusInfo{},
			errors.Wrapf(err,
				"isEnabled doesn't start with a valid boolean %q",
				parsed[IsEnablePrefix])
	}
	isOwned := false
	if _, err := fmt.Sscanf(parsed[IsOwnedPrefix], "%t", &isOwned); err != nil {
		return &NonsensitiveStatusInfo{},
			errors.Wrapf(err,
				"isOwned doesn't start with a valid boolean %q",
				parsed[IsOwnedPrefix])
	}
	ownerPass := false
	if _, err := fmt.Sscanf(parsed[IsOwnerPasswordPresentPrefix], "%t", &ownerPass); err != nil {
		return &NonsensitiveStatusInfo{},
			errors.Wrapf(err,
				"ownerPass doesn't start with a valid boolean %q",
				parsed[IsOwnerPasswordPresentPrefix])
	}
	lockPerm := false
	if _, err := fmt.Sscanf(parsed[HasResetLockPermissionsPrefix], "%t", &lockPerm); err != nil {
		return &NonsensitiveStatusInfo{},
			errors.Wrapf(err,
				"lockPerm doesn't start with a valid boolean %q",
				parsed[HasResetLockPermissionsPrefix])
	}
...
func (u *UtilityTpmManagerBinary) DropResetLockPermissions(ctx context.Context) (func() error, error) {
	...
	if err := proto.Unmarshal(d, &data); err != nil {
