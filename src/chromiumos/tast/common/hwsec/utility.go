...
func EnsureTpmIsReset(ctx context.Context, h *Helper, utility Utility) error {
...
func NewUtility(ctx context.Context, h *Helper, clientType ClientType) (Utility, error) {
	switch clientType {
	case CryptohomeBinaryType:
		proxy, err := NewCryptohomeBinary(ctx, h)
		if err != nil {
			return nil, err
		}
