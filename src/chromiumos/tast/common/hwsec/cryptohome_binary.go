...
func (c *cryptohomeBinary) authenticateSmartCardAuthFactor(ctx context.Context, authSessionID, label, algorithm string) ([]byte, error) {
	args := []string{"--action=authenticate_auth_factor", "--auth_session_id=" + authSessionID, "--key_label=" + label, "--challenge_response_algo=" + algo}
	return c.call(ctx, args...)
}
...
func (c *cryptohomeBinary) addSmartCardAuthFactor(ctx context.Context, authSessionID, label, algo string) ([]byte, error) {
	args := []string{"--action=add_auth_factor", "--auth_session_id=" + authSessionID, "--key_label=" + label, "--challnge_response_algorithm=" + algo}
	return c.call(ctx, args...)
}
