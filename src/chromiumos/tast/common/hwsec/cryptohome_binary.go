...
// authenticateAuthSessionWithChallengeCredential calls "cryptohome --action=authenticate_auth_session".
// with additional flags for challenge credentials.
func (c *cryptohomeBinary) authenticateAuthSessionWithChallengeCredential(ctx context.Context, authSessionID string, publicMount bool, extraFlags []string) ([]byte, error) {
	args := []string{"--action=authenticate_auth_session", "--auth_session_id=" + authSessionID}
	if publicMount {
		args = append(args, "--public_mount", "--key_label=public_mount")
	} else {
		args = append(args, "--key_label=fake_label")
	}
	args = append(args, extraFlags...)
	return c.call(ctx, args...)
}
...
// addCredentialsAuthSessionWithChallengeCredential calls "cryptohome --action=add_credentials".
// with additional flags for challenge credentials.
func (c *cryptohomeBinary) addCredentialsAuthSessionWithChallengeCredential(ctx context.Context, user, authSessionID string, publicMount bool, extraFlags []string) ([]byte, error) {
	args := []string{"--action=add_credentials", "--auth_session_id=" + authSessionID}
	if publicMount {
		args = append(args, "--public_mount", "--key_label=public_mount")
	} else {
