...
// 'cryptohome' execuable. For more detail, please check //src/platform2/cryptohome/cryptohome.cc.
type CryptohomeBinary struct {
	ctx            context.Context
	doesCallRemote bool
}

func VATypeIntToString(VAType int) string {
	if VAType == 0 {
		return "default"
	}
	if VAType == 1 {
...
func NewCryptohomeBinary(ctx context.Context) (*CryptohomeBinary, error) {
	out, err := testexec.CommandContext(ctx, "which", "cryptohome").Output()
	return &CryptohomeBinary{ctx, err != nil || len(out) == 0}, nil
}
...
// TpmStatus calls "cryptohome --action=tpm_attestation_status".
...
func (c *CryptohomeBinary) TpmTakeOwnership() error {
	out, err := c.callCryptohome("--action=tpm_take_ownership")
	if err != nil {
...
func (c *CryptohomeBinary) TpmWaitOwnership() error {
	out, err := c.callCryptohome("--action=tpm_wait_ownership")
	if err != nil {
		return errors.Wrap(err, "failed to call cryptohome")
	}
	if len(out) > 0 {
...
// TpmAttestationCreateEnrollRequest calls "cryptohome --action=enroll_request".
...
func (c *CryptohomeBinary) TpmAttestationEnterpriseVaChallenge(
	...
	VATypeString := VATypeIntToString(VAType)
	out, err := c.callCryptohome(
