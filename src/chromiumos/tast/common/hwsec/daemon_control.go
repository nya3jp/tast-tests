...
func waitForDBusService(ctx context.Context, h *Helper, name string) error {
...
func StartCryptohome(ctx context.Context, h *Helper) error {
	...
	return waitForDBusService(ctx, h, "\"org.chromium.Cryptohome\"")
}
...
func StartAttestation(ctx context.Context, h *Helper) error {
	if _, err := h.Run(ctx, "start", "attestationd"); err != nil {
		return errors.Wrap(err, "failed to start attestation")
	}
	return waitForDBusService(ctx, h, "attestation")
}

// StopAttestation stops attestationd.
func StopAttestation(ctx context.Context, h *Helper) error {
	if _, err := h.Run(ctx, "stop", "attestationd"); err != nil {
...
func RestartAttestation(ctx context.Context, h *Helper) error {
	if _, err := h.Run(ctx, "restart", "attestationd"); err != nil {
