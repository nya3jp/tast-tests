...
func (u *CryptohomeClient) MountChallengeRespVault(ctx context.Context, username, label, keyDelegateName, keyDelegatePath string, publicSPKI []byte, challengeAlg cpb.ChallengeSignatureAlgorithm, create bool, config *VaultConfig) error {
	extraFlags := vaultConfigToExtraFlags(config)
	extraFlags = append(extraFlags, "--challenge_alg="+challengeAlg.String())
	extraFlags = append(extraFlags, "--public_spki="+base64.StdEncoding.EncodeToString(publicSPKI))
...
func (u *CryptohomeClient) CheckChallengeRespVault(ctx context.Context, username, label, keyDelegateName, keyDelegatePath string, publicSPKI []byte, challengeAlg cpb.ChallengeSignatureAlgorithm) (bool, error) {
	var extraFlags []string
	extraFlags = append(extraFlags, "--challenge_alg="+challengeAlg.String())
	extraFlags = append(extraFlags, "--public_spki="+base64.StdEncoding.EncodeToString(publicSPKI))
	extraFlags = append(extraFlags, "--key_delegate_name="+keyDelegateName)
