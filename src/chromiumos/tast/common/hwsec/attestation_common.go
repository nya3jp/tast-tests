...
func getCAServerURL(pcaType PCAType) string {
	if pcaType == DefaultPCA {
...
func getEnrollURL(pcaType PCAType) string {
	return getCAServerURL(pcaType) + "/" + "enroll"
}

func getCertURL(pcaType PCAType) string {
	return getCAServerURL(pcaType) + "/" + "sign"
}
...
// Enroll creates the enroll reqeust, sends it to the corresponding PCA server, and finishes the request with the received response.
func (at *AttestationTest) Enroll(ctx context.Context) error {
	...
	resp, err := SendPostRequestTo(ctx, req, getEnrollURL(at.pcaType))
	if err != nil {
...
// GetCertificate creates the cert reqeust, sends it to the corresponding PCA server, and finishes the request with the received response.
func (at *AttestationTest) GetCertificate(ctx context.Context, username string, label string) error {
	...
	resp, err := SendPostRequestTo(ctx, req, getCertURL(at.pcaType))
	if err != nil {
		return errors.Wrap(err, "failed to send request to CA")
	}
	if len(resp) == 0 {
