...
func Pkcs11ClearObject(ctx context.Context, helper *Helper, slot int, objID string, objType string) error {
	...
	for i := 0; i < 20; i++ {
		_, err := helper.RunShell(ctx, fmt.Sprintf("pkcs11-tool --module=%s --slot=0 --delete-object --type %s --id %s", chapsPath, objType, objID))
		if err != nil {
...
func Pkcs11CreateRsaSoftwareKey(ctx context.Context, helper *Helper, utility Utility, username string, keyname string, objID string) (Pkcs11KeyInfo, error) {
	...
	_, err = helper.RunShell(ctx, fmt.Sprintf("openssl req -nodes -x509 -sha1 -newkey rsa:2048 -keyout /tmp/%s-priv.key -out /tmp/%s-cert.crt -days 365 -subj /C=US/ST=CA/L=MTV/O=ChromiumOS/CN=chromiumos.example.com", keyname, keyname))
	if err != nil {
		return Pkcs11KeyInfo{}, errors.Wrap(err, "failed to create key with openssl")
	}

	// Extract the public key from the private key.
	_, err = helper.RunShell(ctx, fmt.Sprintf("openssl rsa -in /tmp/%s-priv.key -pubout -out /tmp/%s-pub.key", keyname, keyname))
	if err != nil {
	...
	_, err = helper.RunShell(ctx, fmt.Sprintf("openssl pkcs8 -inform pem -outform der -in /tmp/%s-priv.key -out %s -nocrypt", keyname, result.privKeyPath))
	if err != nil {
	...
	_, err = helper.RunShell(ctx, fmt.Sprintf("openssl rsa -pubin -inform pem -outform der -in /tmp/%s-pub.key -out %s", keyname, result.pubKeyPath))
	if err != nil {
		return Pkcs11KeyInfo{}, errors.Wrap(err, "failed to convert private key to DER format with OpenSSL")
	}

	// Convert the cert to DER format.
	_, err = helper.RunShell(ctx, fmt.Sprintf("openssl x509 -in /tmp/%s-cert.crt -outform der -out %s", keyname, result.certPath))
	if err != nil {
		return Pkcs11KeyInfo{}, errors.Wrap(err, "failed to convert cert to DER format with openssl")
	}

	// Import the private key into chaps
	_, err = helper.RunShell(ctx, fmt.Sprintf("p11_replay --import --path=%s --type=privkey --id=%s", result.privKeyPath, result.objID))
	if err != nil {
		return Pkcs11KeyInfo{}, errors.Wrap(err, "failed to import private key into chaps")
	}

	// Import the certificate into chaps
	_, err = helper.RunShell(ctx, fmt.Sprintf("p11_replay --import --path=%s --type=cert --id=%s", result.certPath, result.objID))
	if err != nil {
...
type Pkcs11MechanismInfo struct {
	...
	toolSignInputFileProcessor func(ctx context.Context, helper *Helper, input string) string
...
func Pkcs11NoOpFileProcessor(ctx context.Context, helper *Helper, input string) string {
	return input
}
...
func Pkcs11Sign(ctx context.Context, helper *Helper, key Pkcs11KeyInfo, input string, output string, mechanism Pkcs11MechanismInfo) error {
	chapsPath := GetChapsCryptokiModule()
	if chapsPath == "" {
		return errors.New("unable to find chaps module")
	}
	...
	_, err := helper.RunShell(ctx, fmt.Sprintf("rm -f '%s'", output))
	if err != nil {
		return errors.New("failed to remove the output before signing")
	}

	cmd := fmt.Sprintf("pkcs11-tool --module=%s --slot=%d --id=%s --sign -m %s %s -i %s -o %s", chapsPath, key.slot, key.objID, mechanism.toolMParam, mechanism.toolExtraParam, mechanism.toolSignInputFileProcessor(ctx, helper, input), output)
	_, err = helper.RunShell(ctx, cmd)
	if err != nil {
...
func Pkcs11VerifyWithOpenSSL(ctx context.Context, helper *Helper, key Pkcs11KeyInfo, input string, signaturePath string, mechanism Pkcs11MechanismInfo) error {
	// Verify with OpenSSL
	...
	binaryMsg, err := helper.RunShell(ctx, cmd)
	if err != nil {
...
func Pkcs11SignVerify(ctx context.Context, helper *Helper, key Pkcs11KeyInfo, input string, altInput string, mechanism Pkcs11MechanismInfo) error {
	// Test signing.
	if err := Pkcs11Sign(ctx, helper, key, input, input+".sig", mechanism); err != nil {
		return err
	}
	// Test verification of signed message.
	if err := Pkcs11VerifyWithOpenSSL(ctx, helper, key, input, input+".sig", mechanism); err != nil {
		return err
	}
	// Test verification of another message (should fail).
	err := Pkcs11VerifyWithOpenSSL(ctx, helper, key, altInput, input+".sig", mechanism)
	if err == nil {
		// Should not happen
