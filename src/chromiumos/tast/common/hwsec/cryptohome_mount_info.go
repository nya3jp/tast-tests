...
func (c *CryptohomeMountInfo) findMounterPID(ctx context.Context, mounter string) (int32, error) {
	...
	if err != nil {
		// Return -1 means we couldn't find the process.
		return -1, nil
	}
...
func (c *CryptohomeMountInfo) findMountsForPID(ctx context.Context, pid int32) ([]disk.PartitionStat, error) {
	...
	for _, mount := range mounts {
		...
		if len(fields) < 4 {
			return nil, errors.Errorf("mount information doesn't have 4 fields: %q", mount)
		}
...
func (c *CryptohomeMountInfo) validatePermanentPartition(p *disk.PartitionStat) error {
	...
	case "ext4":
		if !devRegexp.MatchString(p.Device) || devLoopRegexp.MatchString(p.Device) {
			return errors.Errorf("ext4 device %q does not match %q excluding %q", p.Device, devRegexp, devLoopRegexp)
		}
	case "ecryptfs":
		if !shadowRegexp.MatchString(p.Device) {
			return errors.Errorf("ecryptfs device %q does not match %q", p.Device, shadowRegexp)
		}
	default:
...
func (c *CryptohomeMountInfo) validateGuestPartition(p *disk.PartitionStat) error {
	...
	case "ext4":
		if !devLoopRegexp.MatchString(p.Device) {
			return errors.Errorf("ext4 device %q does not match %q", p.Device, devLoopRegexp)
		}
	case "tmpfs":
		if p.Device != "guestfs" {
