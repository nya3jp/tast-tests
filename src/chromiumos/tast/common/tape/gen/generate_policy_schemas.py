...
\tout, err {assignment} json.Marshal(p.{value})
\t\tif err != nil {{
...
class PolicySchema:
  ...
  def generate_code(self):
    ...
    values2String = VALUE_TO_JSON_TEMPLATE.format(assignment=":=", value=self.types[0])
    value_names = "\t\t\t\t\t\"" + self.types[0] + "\":%s"
    path_str = "\"" + self.types[0] + "\""
    for value_type in self.types[1:]:
      values2String += VALUE_TO_JSON_TEMPLATE.format(assignment="=", value=value_type)
      value_names += ",\n\t\t\t\t\t\"" + value_type + "\":%s"
      path_str += ", \"" + value_type + "\""
