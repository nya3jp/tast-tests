...
# Basic structs all policy schemas are using.
COMMON_STRUCTS = """type requests struct {
...
def parse_enum(field, enums):
  ...
  for enum in enums:
    ...
    if field['typeName'] == enum['name']:
      enum_str += enum['name'] + ' int\n\n const (\n'
      count = 0
      for enum_entry in enum['value']:
        if 'name' not in enum_entry:
          continue
        if count == 0:
          enum_str += ('\t' + enum_entry['name'] + ' ' + enum['name'] +
            ' = iota // ' + str(count) + '\n')
        else:
          enum_str += '\t' + enum_entry['name'] + ' // ' + str(count) + '\n'
        count+=1
      enum_str += ')\n\n'
  return enum_str

def parse_values(definition, messages, types, additional_structs,
  duplication_set):
  ...
  for message in messages:
    ...
    # struct_str will hold the code of a single struct representing message.
    struct_str = 'type ' + message['name'] + ' struct {\n'
    ...
    for field in message['field']:
      ...
      # Add the field to our struct with a capital letter so it get's exported.
      struct_str += '\t' + field['name'][:1].upper() + field['name'][1:] + ' '
      ...
      struct_str += type_name + '`json:"' + field['name'] + '"`\n'
    first_message = False
    struct_str += '}\n'
...
def parse_schema(policy_schema, duplication_set):
  ...
  link = policy_schema['name'].split('/')[-1]
  ...
  types = [] # Holds the types of the fields in the policy.
  additional_structs = [] # Holds all structs we need for the policy.
  parse_values(definition, messages, types, additional_structs,
    duplication_set)
...
def main():
  ...
  policy_schemas = json.loads(json_str)
