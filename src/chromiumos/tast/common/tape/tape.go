...
type newTapeClientOption struct {
	credsJSON                []byte
	localAndRemoteAuthConfig *localAndRemoteAuth
}

type localAndRemoteAuth struct {
	localRefreshToken        string
	authenticationConfigJSON []byte
}
...
// WithLocalAndRemoteAuthentication specifies authentication configuration parameters
// that can be used for local testing, as well as server testing. Note that
// a local refresh token must be generated by:
// 1. Authentication with TAPE IAP by following the instructions in the README.
// 2. Adding --var tape.local_refresh_token=<VALUE> when running a test.
func WithLocalAndRemoteAuthentication(localRefreshToken string, authenticationConfigJSON []byte) NewTapeClientOption {
	return func(opt *newTapeClientOption) {
		opt.localAndRemoteAuthConfig = &localAndRemoteAuth{
			localRefreshToken:        localRefreshToken,
			authenticationConfigJSON: authenticationConfigJSON,
		}
	}
}

func tokenSourceFromLocalAndRemoteAuthConfig(ctx context.Context, conf *localAndRemoteAuth) (oauth2.TokenSource, error) {
	// Parse out the authentication config.
	var authenticationConfig AuthenticationConfig
	if err := json.Unmarshal(conf.authenticationConfigJSON, &authenticationConfig); err != nil {
		return nil, errors.Wrap(err, "failed to unmarshal configuration")
	}

	// Check the service account paths on the DUT and use the first one that is found.
	for _, path := range authenticationConfig.DroneServiceAccountLocations {
		// If the file is found, use it directly.
		pathOnDUT := serviceAccountPathOnDUT(path)
		credsJSON, err := ioutil.ReadFile(pathOnDUT)
		if err != nil {
			testing.ContextLogf(ctx, "service account not found at: %s", pathOnDUT)
			continue
		}

		testing.ContextLogf(ctx, "using service account at: %s", pathOnDUT)
		return createTokenSource(ctx, credsJSON)
	}

	// At this point, it's assumed the service is running locally so use the
	// provided refresh token.
	if len(conf.localRefreshToken) <= 0 {
		return nil, errors.New("invalid refresh token provided")
	}

	testing.ContextLog(ctx, "using local refresh token for authentication")
	return oauth2.ReuseTokenSource(nil, &jwtToken{
		conf: &oauth2.Config{
		...
		refresh: &oauth2.Token{
			AccessToken:  "",
			TokenType:    "Bearer",
			RefreshToken: conf.localRefreshToken,
			Expiry:       time.Time{},
		},
	}), nil
...
func NewTapeClient(ctx context.Context, opts ...NewTapeClientOption) (*http.Client, error) {
	...
	var err error
	if option.localAndRemoteAuthConfig != nil {
		ts, err = tokenSourceFromLocalAndRemoteAuthConfig(ctx, option.localAndRemoteAuthConfig)
		if err != nil {
			return nil, errors.Wrap(err, "failed to create token from local and remote auth")
		}
	} else if len(option.credsJSON) > 0 {
		ts, err = createTokenSource(ctx, option.credsJSON)
		if err != nil {
			return nil, errors.Wrap(err, "failed to create token from credsJSON")
		}
	} else {
		return nil, errors.New("invalid options defined")
	}
