...
const serviceAccountSkylab = "/creds/service_accounts/skylab-drone.json"
const serviceAccountVM = "/creds/service_accounts/service-account-chromeos.json"
...
// NewTapeClient creates a http client which provides the neccessary tokens to connect to the TAPE
// GCP. All functions in the tape package should be passed this http client.
func NewTapeClient(ctx context.Context) (*http.Client, error) {
	data, err := ioutil.ReadFile(serviceAccountSkylab)
	if err != nil {
		data, err = ioutil.ReadFile(serviceAccountVM)
		if err != nil {
	...
	customClaims["target_audience"] = "770216225211-ihjn20dlehf94m9l4l5h0b0iilvd1vhc.apps.googleusercontent.com"
