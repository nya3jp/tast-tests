...
// NewTapeClient creates a http client which provides the neccessary tokens to connect to the TAPE GCP.
// All functions in the tape package should be passed this http client.
func NewTapeClient(ctx context) (*http.Client, error) {
	data, err := ioutil.ReadFile("/creds/service_accounts/skylab-drone.json")
	if err != nil {
		data, err = ioutil.ReadFile("/creds/service_accounts/service-account-chromeos.json")
		if err != nil {
