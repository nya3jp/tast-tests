// Copyright 2020 The Chromium OS Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Package certificate contains test certificates for use in public key cryptography. These can be used in both remote
// and local tests.
package certificate

// Certificate holds certificate information, including the private certificate, key, and CA certificate (for use by an
// authenticator), as well as a corresponding client certificate, in PEM format.

// CertSet holds a set of credentials which share the same CA certificate.
type CertSet struct {
	CACert     string
	ServerCred Credential
	ClientCred Credential

	expiredServerCred *Credential
}

// ExpiredServerCred returns the expired server credential in "comma ok" idiom.
func (c *CertSet) ExpiredServerCred() (Credential, bool) {
	if c.expiredServerCred == nil {
		return Credential{}, false
	}
	return *c.expiredServerCred, true
}

// Credential holds a pair of certificate and private key.
type Credential struct {
	Cert       string
	PrivateKey string
}

// Test certificate borrowed from Autotest (client/common_lib/cros/site_eap_certs.py). These are only for test usage.
// TODO(crbug.com/1047146): upgrade from MD5 crypto.
var certSet = CertSet{
	CACert: `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`,
	ServerCred: Credential{
		Cert: `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`,
		PrivateKey: `-----BEGIN RSA PRIVATE KEY-----

eOiTwkwmWD7oFjqWUYO1Pw7ipktE1uFPd4KUtwcMbid9
-----END RSA PRIVATE KEY-----
`,
	},
	ClientCred: Credential{
		Cert: `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`,
		PrivateKey: `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
`,
	},
	expiredServerCred: &Credential{
		Cert: `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`,
		PrivateKey: `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
`,
	},
}
var certSet2 = CertSet{
	CACert: `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`,
	ServerCred: Credential{
		Cert: `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`,
		PrivateKey: `-----BEGIN RSA PRIVATE KEY-----

opbkSIzq136j+PzIJX9RLZ0VXiIwM7MtOEvxZCBjAo8M
-----END RSA PRIVATE KEY-----
`,
	},
	ClientCred: Credential{
		Cert: `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`,
		PrivateKey: `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
`,
	},
}

// TestCertificate retrieves a well-known set of certificates, for use by tests that want to establish an
// authenticator and client (e.g., for network EAP tests). This certificate does not provide any real security or
// privacy guarantees, as the certificate and private key are well-known (i.e., published in public repositories).
// This CertSet contains expired server credential.
func TestCertificate() CertSet {
	return certSet
}

// TestCertificate2 provides the certs generated with a different CA from TestCertificate.
// Both the private key and the Common Name of the CA are different.
func TestCertificate2() CertSet {
	return certSet2
}
