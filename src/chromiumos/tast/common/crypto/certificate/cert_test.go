...
func TestCertificates(t *testing.T) {
	...
	for testi, testcase := range []CertSet{TestCertificate(), TestCertificate2()} {
		...
		if expiredServerCred, ok := testcase.ExpiredServerCred(); ok {
			if err := testCred(expiredServerCred, true); err != nil {
...
func TestCADifference(t *testing.T) {
	// Check that TestCertificate and TestCertificate2 are using different CAs.
	if TestCertificate().CACert == TestCertificate2().CACert {
		t.Error("TestCertificate and TestCertificate2 are using the same CA")
	}
}
