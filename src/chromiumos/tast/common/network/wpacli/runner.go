...
func (r *Runner) P2PGetGOPassphrase(ctx context.Context, iface string) (string, error) {
	...
	testing.ContextLog(ctx, lines)
	testing.ContextLog(ctx, lines[0])
	testing.ContextLog(ctx, lines[1])
...
func (r *Runner) addNetwork(ctx context.Context) (int, error) {
	...
	if err != nil {
		return 0, errors.Wrapf(err, "failed running wpa_cli add_network")
	}
	lines := strings.Split(string(cmdOut), "\n")
...
func (r *Runner) P2PWaitForConnected(ctx context.Context, ssid, iface string) error{
	...
	configurationTimeout := 60 * time.Second
	...
	err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if ! isAssoc {
			return errors.Errorf("Failed status is not assciated")
		}
		return err
	...
	err = testing.Poll(ctx, func(ctx context.Context) error {
		...
		if ! isConn {
			return errors.Errorf("Failed status is not connected")
		}
		return err
...
func (r *Runner) P2PScanNetwork(ctx context.Context, ssid string) error{
	...
	err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		for _, network:= range results {
			testing.ContextLog(ctx, network["SSID"])
			testing.ContextLog(ctx, ssid)
