...
// run runs a specific TDLS command and checks for expected response.
func (r *Runner) run(ctx context.Context, expected string, opts ...string) error {
	...
	if !strings.Contains(string(cmdOut), expected) {
		return errors.Errorf("failed to get %q in wpa_cli %s output: %s", expected, opts[0], string(cmdOut))
	}
	return nil
}
