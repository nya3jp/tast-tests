...
// abortScan requests ongoing scan to be aborted.
func (r *Runner) abortScan(ctx context.Context) error {
	cmdOut, err := r.cmd.Output(ctx, "sudo", sudoWPACLI("abort_scan")...)
	if err != nil {
		return errors.Wrap(err, "failed running wpa_cli abort_scan")
	}
	if !strings.Contains(string(cmdOut), "OK") {
		return errors.Errorf("failed to detect 'OK' in the output of 'wpa_cli abort_scan', output: %s", string(cmdOut))
	}
	return nil
}
...
func (r *Runner) P2PGroupAddPersistent(ctx context.Context) error {
	// persistent=0: Specify a restart of a persistent group.
...
func (r *Runner) ScanNetwork(ctx context.Context, dutConn *ssh.Conn, ssid string) error {
	...
	if err := r.scan(ctx); err != nil {
		// Sometimes the scan command fails due to an ongoing scan.
		// Abort the current scan and trigger a new scan.
		if err := r.abortScan(ctx); err != nil {
			return errors.Wrap(err, "failed to abort ongoing scan")
		}
		if err := r.scan(ctx); err != nil {
