...
func TestMayOutputToFile(t *testing.T) {
	baseName := "runnerTest"
	filename := "test.log"
	outDir, err := ioutil.TempDir("", baseName)
	if err != nil {
		t.Fatal("Unable to get a temporary directory: ", err)
	}
	...
	testcases := []struct {
		...
		{
			cmdOut: []byte("single line"),
			expect: "single line",
		},
		{
			cmdOut: []byte("line 1\nline2\n"),
			expect: fmt.Sprintf("output file: /tmp/tast/results/latest/tests/%s/%s", baseName, filename),
		},
	}

	for _, tc := range testcases {
		out := MayOutputToFile(tc.cmdOut, outDir, filename)
		if out != tc.expect {
			t.Errorf("Unexpected MayOutputToFile() returns: got %s, want %s", out, tc.expect)
		}
		// Verify file content.
		if strings.HasPrefix(out, "output file:") {
			filepath := path.Join(outDir, filename)
			content, err := ioutil.ReadFile(filepath)
			if err != nil {
				t.Errorf("Unable to read output file %s: %s", filepath, err)
			}
			if !bytes.Equal(content, tc.cmdOut) {
				t.Errorf("Unexpected output file content: got %s, want %s", content, tc.cmdOut)
			}
		}
	}
}
