...
// mockCmdRunner is a simple mock of CmdRunner which always returns the given content
// as command output. This is useful for testing some simple parsing that is not
// extracted as a independent function.
type mockCmdRunner struct {
	out []byte
}

// Run is a noop mock which always return nil.
func (r *mockCmdRunner) Run(ctx context.Context, cmd string, args ...string) error {
	return nil
}

// Output is a mock which pretends the command is executed successfully and prints
// the pre-assigned output.
func (r *mockCmdRunner) Output(ctx context.Context, cmd string, args ...string) ([]byte, error) {
	return r.out, nil
}
...
`,
	...
	for i, tc := range testcases {
		...
		if err != nil {
			t.Errorf("case#%d, unexpected error in GetRegulatoryDomain: %s", i, err.Error())
		...
		if err != nil {
			t.Errorf("case#%d, unexpected error in IsRegulatorySelfManaged: %x", i, err.Error())
