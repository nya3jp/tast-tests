...
func parseMaxSTAs(contents string) (int, error) {
	m, err := extractMatch(`\s*Maximum associated stations in AP mode: (\d+)`, contents)
	if err != nil {
		return 0, nil
	}
...
func parseHT(attrs *sectionAttributes, sectionName, contents string) {
...
func parseVHT(attrs *sectionAttributes, sectionName, contents string) {
...
func parseHE(attrs *sectionAttributes, sectionName, contents string) {
...
var throughputParsers = []struct {
	prefix string
	parse  func(attrs *sectionAttributes, sectionName, contents string)
}{
	{
		prefix: "Capabilities",
		parse:  parseHT,
...
func parseThroughput(attrs *sectionAttributes, sectionName, contents string) error {
	...
	sections, err := parseSection(`(?m)^\t\t(\w.*):.*$`, contents)
	if err != nil {
