...
func (r *Runner) start(fullCtx context.Context) (err error) {
	...
	// This statement solves that in the local test environment, once the parent process is terminated,
	// the tcpdump process will automatically become a zombie process. Therefore,
	// this statement will only have an impact on the local test environment.
	err = r.cmd.ReleaseLocalCmd()
	if err != nil {
