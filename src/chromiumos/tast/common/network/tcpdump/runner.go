...
func (r *Runner) start(fullCtx context.Context) (err error) {
	...
	testing.ContextLogf(ctx, "Starting tcpdump on %s", config.iface)

	args := []string{"-U", "-i", config.iface, "-w", config.packetPath}
	if config.snaplen != 0 {
		args = append(args, "-s", strconv.FormatUint(config.snaplen, 10))
	}

	r.cmd.Create(ctx, tcpdumpCmd, args...)

	r.cmd.SetStdOut(config.stdoutFile)
	...
	go func() {
		...
		multiWriter := io.MultiWriter(config.stderrFile, readyWriter)
		io.Copy(multiWriter, stderrPipe)
	}()
	if err := r.cmd.StartExistedCmd(); err != nil {
...
func (r *Runner) close(ctx context.Context) error {
	...
	if r.CheckCmdExisted() {
		...
		r.cmd.Run(ctx, "pkill", "-f", fmt.Sprintf("^%s.*%s", tcpdumpCmd, config.packetPath))
		r.cmd.WaitExistedCmd()
	}
	config.wg.Wait()
	if config.stderrFile != nil {
		config.stderrFile.Close()
	}
	if config.stdoutFile != nil {
		config.stdoutFile.Close()
	}
	return err
}
...
func (r *Runner) OutputTCP(ctx context.Context) ([]byte, error) {
	if r.CheckCmdExisted() {
		out, err := r.cmd.Output(ctx, "tcpdump", "tcp", "-r", config.packetPath)
		if err != nil {
			return nil, err
		}
		if err := r.cmd.Run(ctx, "rm", config.packetPath); err != nil {
			return nil, errors.Wrapf(err, "failed to clean up tcpdump file %s: ", config.packetPath)
		}
		return out, nil
	}
	return nil, nil
...
// CheckCmdExisted confirms whether the tcpdump command has been successfully started.
func (r *Runner) CheckCmdExisted() bool {
	return r.cmd.CheckCmdExisted()
}
