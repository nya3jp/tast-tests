// Copyright 2020 The Chromium OS Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Package eapcerts provides some Certs and Keys for testing.
package eapcerts

import "encoding/json"

// These certificate trees are for testing only in sealed containers
// so it is okay that we have them checked into a GIT repository.
// Nobody will ever use this information on the open air.

// ServerCert3AltSubjectMatch returns the subject alternative names in ServerCert3.
func ServerCert3AltSubjectMatch() (string, error) {
	b, err := json.Marshal([]struct {
		Type  string `json:"Type"`
		Value string `json:"Value"`
	}{
		{Type: "DNS", Value: "www.example.com"},
		{Type: "DNS", Value: "mail.example.com"},
		{Type: "EMAIL", Value: "example@domain.com"},
	})
	return string(b), err
}

// A good set of Certs and Keys for testing.
const (
	CACert1 = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
	ServerCert1 = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
	ServerPrivateKey1 = `-----BEGIN RSA PRIVATE KEY-----

eOiTwkwmWD7oFjqWUYO1Pw7ipktE1uFPd4KUtwcMbid9
-----END RSA PRIVATE KEY-----
`
	ClientCert1 = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
	ClientPrivateKey1 = `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
`
)

// The following certs are generated with the same CA as above but are expired.
const (
	ServerExpiredCert = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
	ServerExpiredKey = `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
`
)

// The following certs are generated with a different CA (both the private key and the Common Name of the CA are different).
const (
	CACert2 = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
	ServerCert2 = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
	ServerPrivateKey2 = `-----BEGIN RSA PRIVATE KEY-----

opbkSIzq136j+PzIJX9RLZ0VXiIwM7MtOEvxZCBjAo8M
-----END RSA PRIVATE KEY-----
`
	ClientCert2 = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
	ClientPrivateKey2 = `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
`
	DH1024PEMKey1 = `-----BEGIN DH PARAMETERS-----
MIGHAoGBAL/YrUzFuA5cPGzhXVqTvDugmPi9CpbWZx2+TCTKxZSjNiVJxcICSnql
uZtkR3sOAiWn384E4ZQTBrYPUguOuFfbMTRooADhezaG9SXtrE9oeVy9avIO7xQK
emZydO0bAsRV+eL0XkjGhSyhKoOvSIXaCbJUn7duEsfkICPRLWCrAgEC
-----END DH PARAMETERS-----
`
)

// This cert is used for authenticating via ONC.
const (
	ClientCert1PKCS12 = `

`
	CACert3 = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
)

// The following certificate has been issued by CACert3.
const (
	ServerCert3 = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`
	ServerPrivateKey3 = `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
`
)
