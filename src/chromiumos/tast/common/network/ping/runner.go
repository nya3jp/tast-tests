...
import (
	...
	"chromiumos/tast/common/network/ping/regexps"
...
func parseOutput(out string) (*Result, error) {
	m := regexps.SentRE.FindStringSubmatch(out)
	if len(m) != 2 {
	...
	m = regexps.ReceivedRE.FindStringSubmatch(out)
	if len(m) != 2 {
	...
	m = regexps.LossRE.FindStringSubmatch(out)
	if len(m) != 2 {
	...
	m = regexps.StatRE.FindStringSubmatch(out)
	if len(m) != 5 {
