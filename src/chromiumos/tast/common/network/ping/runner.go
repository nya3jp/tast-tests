...
// ExitCode is used to save the exitcode of the ping/su command.
var ExitCode int
...
func (r *Runner) Ping(ctx context.Context, targetIP string, options ...Option) (*Result, error) {
	...
	if cfg.User != "" {
		command = "su"
		userCmd := pingCmd
		for _, pingOpt := range args {
			userCmd = userCmd + " " + pingOpt
		}
