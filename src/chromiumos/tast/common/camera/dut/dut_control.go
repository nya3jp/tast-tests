...
func CCADimBacklight(ctx context.Context) (string, error) {
	return DoDimBacklight(ctx, func(name string, arg ...string) (string, error) {
		originalVal, err := exec.CommandContext(ctx, name, arg...).Output()
		if err != nil {
			return "", err
		}
...
func DUTDimBacklight(ctx context.Context, d *dut.DUT) (string, error) {
	return DoDimBacklight(ctx, func(name string, arg ...string) (string, error) {
		originalVal, err := d.Conn().CommandContext(ctx, name, arg...).Output()
		if err != nil {
			return "", err
		}
		brightnessVal := string(originalVal)
...
func DoRestoreBacklight(ctx context.Context, OriginalBrightness string, executeCommand func(string, ...string) error) error {
	...
	brightnessArg := fmt.Sprintf("--set_brightness_percent=%d", OriginalBrightness)
...
func CCARestoreBacklight(ctx context.Context, OriginalBrightness string) error {
	return DoRestoreBacklight(ctx, OriginalBrightness, func(name string, arg ...string) error {
		err := exec.CommandContext(ctx, name, arg...).Run()
		if err != nil {
			return err
		}
		return nil
...
func DUTRestoreBacklight(ctx context.Context, d *dut.DUT, OriginalBrightness string) error {
	return DoRestoreBacklight(ctx, OriginalBrightness, func(name string, arg ...string) error {
		err := d.Conn().CommandContext(ctx, name, arg...).Run()
		if err != nil {
			return err
		}
		return nil
