...
func NewHelper(ctx context.Context, d *dut.DUT, altHostname, chartPath, outDir string, displayLevel float32) (_ *Helper, retErr error) {
...
func (h *Helper) SetDisplayLevel(displayLevel float32) error {
	if h.chart != nil {
		err := h.chart.Close(h.ctx, h.outDir)
		if err != nil {
			return err
		}
		h.chart = nil
