...
func (c *CommonChameleond) GetConnectorType(ctx context.Context, portID PortID) (PortType, error) {
	...
	if err != nil {
		portType, isMapped := chameleonV2PortIDToPortTypeMap[portID]
		if isMapped {
...
func (c *CommonChameleond) FetchSupportedPortIDsByType(ctx context.Context) (map[PortType][]PortID, error) {
	...
	for _, portID := range supportedPortIDs {
		...
		if portIDs, ok := supportedPortIDsByType[portType]; !ok {
			supportedPortIDsByType[portType] = []PortID{portID}
		} else {
			supportedPortIDsByType[portType] = append(portIDs, portID)
		}
	}
...
func (c *CommonChameleond) collectSupportedPortTypes(supportedPortIDsByType map[PortType][]PortID) []PortType {
	supportedPortTypes := make([]PortType, len(supportedPortIDsByType))
	i := 0
	for portType := range supportedPortIDsByType {
		supportedPortTypes[i] = portType
		i++
	}
...
func (c *CommonChameleond) FetchSupportedPortIDByType(ctx context.Context, portType PortType, index int) (PortID, error) {
	...
	for i := 0; matchingPortType == "" && i < len(supportedPortTypes); i++ {
		if strings.EqualFold(supportedPortTypes[i].String(), portType.String()) {
			matchingPortType = supportedPortTypes[i]
		}
	}
	if matchingPortType == "" {
