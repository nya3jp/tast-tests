...
func TestMapToAudioDataFormat(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			input: map[string]interface{}{
				"file_type": AudioFileTypeWav.ToString(),
			},
			expected: &AudioDataFormat{
		...
		},
		{
			input: map[string]interface{}{
				"file_type":     AudioFileTypeWav.ToString(),
				"sample_format": AudioSampleFormatS8.ToString(),
				"channel":       1,
				"rate":          2,
			},
			expected: &AudioDataFormat{
...
func TestAudioDataFormatToMap(t *testing.T) {
	for _, tc := range []struct {
		...
		if !matches {
			t.Errorf("AudioDataFormat{%v}.ToMap() = %v; want %v", tc.input, actual, tc.expected)
		}
	}
}
