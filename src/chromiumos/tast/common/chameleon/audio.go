...
func (ch *Chameleon) AudioBoardConnect(ctx context.Context, busNumber int, audioBusEndpoint AudioBusEndpoint) error {
	return ch.xmlrpc.Run(ctx, xmlrpc.NewCall("AudioBoardConnect", busNumber, audioBusEndpoint))
}

// AudioBoardDisconnect disconnects an endpoint to an audio bus.
// busNumber: 1 or 2 for audio bus 1 or bus 2.
// audioBusEndpoint defined by type AudioBusEndpoint
func (ch *Chameleon) AudioBoardDisconnect(ctx context.Context, busNumber int, audioBusEndpoint AudioBusEndpoint) error {
	return ch.xmlrpc.Run(ctx, xmlrpc.NewCall("AudioBoardDisconnect", busNumber, audioBusEndpoint))
}
...
//	  audioDataFormat: A map representation of AudioDataFormat.
//	    Currently Chameleon only accepts the following data format:
...
//	    }
//
// User should do the format conversion to minimize work load on Chameleon board.
func (ch *Chameleon) StartPlayingAudio(ctx context.Context, portID PortID, path string, audioDataFormat map[string]interface{}) error {
	return ch.xmlrpc.Run(ctx, xmlrpc.NewCall("StartPlayingAudio", portID, path, audioDataFormat))
}
...
func (ch *Chameleon) StopPlayingAudio(ctx context.Context, portID PortID) error {
	return ch.xmlrpc.Run(ctx, xmlrpc.NewCall("StopPlayingAudio", portID))
}
...
func (ch *Chameleon) AudioBoardGetRoutes(ctx context.Context, busNumber int) ([][]string, error) {
	var value [][]string
...
func (ch *Chameleon) AudioBoardClearRoutes(ctx context.Context, busNumber int) error {
	return ch.xmlrpc.Run(ctx, xmlrpc.NewCall("AudioBoardClearRoutes", busNumber))
}

// Reset resets Chameleon board
func (ch *Chameleon) Reset(ctx context.Context) error {
	return ch.xmlrpc.Run(ctx, xmlrpc.NewCall("Reset"))
}
