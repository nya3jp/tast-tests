...
var ErrNotFound = errors.New("cannot to found server information")
...
func Server(serverType ServerType, role string) (string, error) {
	serverTypeNames := []string{"servo", "provision server", "DUT server", "cache server"}
	allServer, err := allServerHosts()
	if err != nil {
		return "", errors.Wrapf(
			ErrNotFound, "cannot find %s for role %s", serverTypeNames[serverType], role)
	}
...
func allServerHosts() (res map[ServerType](map[string]string), returnErr error) {
	once.Do(func() {
		servoHosts, err := parseServerVarValues(servoServers.Value())
		if err != nil {
			returnErr = errors.Wrap(err, "failed to parse servo information")
			return
		}
		provisionServerHosts, err := parseServerVarValues(provisionServers.Value())
		if err != nil {
			returnErr = errors.Wrap(err, "failed to parse provision server information")
			return
		}
		dutServerHosts, err := parseServerVarValues(dutServers.Value())
		if err != nil {
			returnErr = errors.Wrap(err, "failed to parse DUT server information")
			return
		}
		cacheServerHosts, err := parseServerVarValues(cacheServers.Value())
		if err != nil {
			returnErr = errors.Wrap(err, "failed to parse cache server information")
			return
		}
		allServers = map[ServerType](map[string]string){
			Servo:     servoHosts,
			...
			DUT:       dutServerHosts,
			Cache:     cacheServerHosts,
		}
	})
	return allServers, returnErr
}

// parseServerVarValues parse the values of server related run time variables
// and return a role to server host map.
// Example input: "primary:addr1:22,cd1:addr2:2222"
// Example output: { "primary": "addr1:22", "cd1": "addr2:2222" }
