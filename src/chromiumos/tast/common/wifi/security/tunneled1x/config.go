...
func NewConfigFactory(serverCACert, serverCert, serverKey, clientCACert, identity, serverPassword string, ops ...Option) *ConfigFactory {
	fac := &ConfigFactory{
...
func (f *ConfigFactory) Gen() (security.Config, error) {
	wpaeapConf, err := wpaeap.NewConfigFactory(f.serverCACert, f.serverCert, f.serverKey, f.wpaeapOps...).Gen()
	if err != nil {
