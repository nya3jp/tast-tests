...
func (c *Config) InstallClientCredentials(ctx context.Context, netCertStore *netcertstore.Store) error {
	if !c.hasClientCred() {
		return nil
	}

	c.Pin = netCertStore.Pin()
	c.ClientSlotID = netCertStore.Slot()
	netCertID, err := netCertStore.InstallCertKeyPair(ctx, c.clientCred.PrivateKey, c.clientCred.Cert)
	if err != nil {
		return err
	}
