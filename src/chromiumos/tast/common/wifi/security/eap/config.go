...
func (c *Config) ShillServiceProperties() (map[string]interface{}, error) {
	...
	if c.NeedsTPMStore() && (c.Pin == "" || c.ClientSlotID == 0) {
...
func (c *Config) NeedsTPMStore() bool {
...
func (c *Config) InstallClientCredentials(ctx context.Context, tpm *wifi.TPMStore) error {
	...
	return tpm.InstallKeyAndCertificate(ctx, c.clientCred.PrivateKey, c.clientCred.Cert, c.tpmID)
}
