...
type Config struct {
	// fileSuffix will become the file name suffix of the cert/key files which are installed onto the router.
	fileSuffix string
...
func (c *Config) ShillServiceProperties() (map[string]interface{}, error) {
	// For c.ClientSlotID, 0 is a system slot but not a user slot so we can treat 0 as an acceptable zero value,
...
func (c *Config) InstallClientCredentials(ctx context.Context, tpm *wifi.TPMStore) error {
	...
	return tpm.InstallKeyAndCertificate(ctx, c.clientKey, c.clientCert, c.clientCertID)
}
