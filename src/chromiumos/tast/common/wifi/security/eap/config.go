...
func (c *Config) InstallRouterCredentials(ctx context.Context, host *ssh.Conn, workDir string) error {
	...
	c.serverCACertFile = workDir + "hostapd_ca_cert_file." + c.fileSuffix
	c.serverCertFile = workDir + "hostapd_cert_file." + c.fileSuffix
	c.serverKeyFile = workDir + "hostapd_key_file." + c.fileSuffix
	c.serverEAPUsersFile = workDir + "hostapd_eap_user_cert_file." + c.fileSuffix
