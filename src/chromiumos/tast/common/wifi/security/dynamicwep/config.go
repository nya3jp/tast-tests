...
func (f *ConfigFactory) Gen() (security.Config, error) {
	...
	// The reason why we stores a partially initialized object, blueprint, in ConfigFactory is that
	// we can use it to carry fields that don't raise error when being assigned.
	// For those fields may raise error during initialization, postpone it to here.
	conf := f.blueprint
