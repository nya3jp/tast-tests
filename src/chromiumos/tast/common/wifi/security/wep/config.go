...
func (f *Factory) Gen() (security.Config, error) {
	...
	// To prevent sharing the same array between the different Configs that
	// are Gen-ed by the same Factory, copy the array in each calling of Gen.
...
func NewFactory(keys []string, ops ...Option) *Factory {
	// Default config.
	fac := &Factory{
