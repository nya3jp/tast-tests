...
func NewTPMStore(ctx context.Context, cryptohomeUtil *hwsec.UtilityCryptohomeBinary, runner hwsec.CmdRunner) (result *TPMStore, retErr error) {
	...
	singletonTPMStore = &TPMStore{runner, slot, label, pin}, nil
	return singletonTPMStore
}
...
func ResetTPMStore(ctx context.Context, s *TPMStore, cryptohomeUtil *hwsec.UtilityCryptohomeBinary) error {
	if s != singletonTPMStore {
		panic("instance of TPMStore passed to ResetTPMStore is not the singleton")
	}
	s.slot = -1
	s.runner = nil
