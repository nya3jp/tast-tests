...
func InitializeAllowedEntries() []AllowedEntry {
	return []AllowedEntry{
		...
		{"shill", "eap_listener.cc", ".*Could not bind socket to interface: No such device.*", 0},                                 // Test HW only
		{"shill", "eap_listener.cc", ".*Could not open EAP listener socket.*", 0},                                                 // Test HW only
		{"shill", "ethernet.cc", ".*cannot disable \\[18\\] tx-tcp-ecn-segmentation.*", 0},                                        // Test HW only
		{"shill", "ethernet.cc", ".*OnSetInterfaceMacResponse received response with error Cannot assign requested address.*", 0}, // Test HW only
