...
// RunCaseParams contains the parmaters for c-ats case run.
...
func GetNodeClientAddress(deviceHostName, requestURL string) (string, *mtbferrors.MTBFError) {
	deviceInfo, err := getDeviceInfo(deviceHostName, requestURL)
	if err != nil {
...
func getDeviceInfo(deviceHostName string, requestURL string) (*DUTInfo, *mtbferrors.MTBFError) {
	deviceInfo, err := QueryDeviceInfoByDutID(deviceHostName, requestURL)
	if err != nil {
...
func runCase(ctx context.Context, params *RunCaseParams, createTask createTaskClosure) (interface{}, *mtbferrors.MTBFError) {
	...
	if params.query {
		deviceInfo, mtbfErr = getDeviceInfo(params.HostName, params.RequestURL)
		if mtbfErr != nil {
			return nil, mtbfErr
		}
	}
...
func DetachCaseRun(ctx context.Context, params *RunCaseParams, main, cleanUp sdk.Handler) (interface{}, error) {
	...
	//androidTest, err := sdk.New("10.10.11.224:6603")
	if err != nil {
	...
	caseRpt, result, err := androidTest.RunDelegate(ctx, sdk.CaseDescription{
		...
		Description: params.CaseName,
		ReportPath:  path.Join("home", "logs", params.TaskName, params.DeviceID, params.CaseName),
