...
// QuitJoinedCall quits joined call.
func (dev *Device) QuitJoinedCall(ctx context.Context) {
	defer func() {
		dev.Client.ExecCommand(dev.DeviceID, "shell cmd statusbar collapse").Do(ctx)
		dev.Client.StartMainActivity(
			dev.DeviceID,
			".SigningInActivity",
			"com.google.android.talk").Do(ctx)
		dev.PressCancelButton(ctx, 2)
	}()

	dev.Client.ExecCommand(dev.DeviceID, "shell cmd statusbar expand-notifications").Do(ctx)

	joined, _ := dev.Client.VerifyListItem(dev.DeviceID,
		ui.UiNodePropDesc{Text: "text=Hangouts"}).Reference(
		ui.UiNodePropDesc{Text: "Ongoing video call"}).Parent(
		ui.UiNodePropDesc{Class: "android.widget.FrameLayout"}).Do(ctx)

	if !joined {
		return
	}

	exit, _ := dev.Client.VerifyListItem(dev.DeviceID,
		ui.UiNodePropDesc{Text: "text=Hangouts"}).Reference(
		ui.UiNodePropDesc{Text: "Ongoing video call"}).Parent(
		ui.UiNodePropDesc{ResourceID: "android:id/notification_action_list_margin_target"}).Do(ctx)

	if exit {
		dev.Client.UIAClickListItem(dev.DeviceID, "text=Exit").ParentSelector(
			"text=Ongoing video call").ParentSelector(
			"ID=android:id/notification_action_list_margin_target").ScrollSelector(
			"ID=com.android.systemui:id/notification_stack_scroller").Do(ctx)

		return
	}

	collapsed, _ := dev.Client.VerifyListItem(dev.DeviceID,
		ui.UiNodePropDesc{ResourceID: "android:id/expand_button::description=Expanded"}).Reference(
		ui.UiNodePropDesc{Text: "Ongoing video call"}).Parent(
		ui.UiNodePropDesc{Class: "android.widget.FrameLayout"}).Do(ctx)
	...
	dev.Client.ClickListItem(dev.DeviceID,
		ui.UiNodePropDesc{ResourceID: "android:id/expand_button"}).Reference(
		ui.UiNodePropDesc{Text: "Ongoing video call"}).Parent(
		ui.UiNodePropDesc{Class: "android.widget.FrameLayout"}).Do(ctx)
}
...
func (dev *Device) SendMessageToChromeOS(ctx context.Context, message string) error {
	...
	dev.Client.InputText(dev.DeviceID, message).Do(ctx)
...
func (dev *Device) OpenGoogleMusicAndPlay(ctx context.Context) error {
	...
	ClickSelector(ctx, dev, "text=NO THANKS")
	ClickSelector(ctx, dev, "textstartwith=SKIP")
	ClickSelector(ctx, dev, "descstartwith=SKIP")
	ClickSelector(ctx, dev, "text=GOT IT")
	...
	if !found {
		ClickSelector(ctx, dev, "desc=Show navigation drawer")
		ClickSelector(ctx, dev, "text=Music library")
	}
	ClickSelector(ctx, dev, "text=ALBUMS")
...
func (dev *Device) VerifyAndSkipAd(ctx context.Context) {
	...
	if ad {
		...
		ClickSelector(ctx, dev, "id=com.google.android.youtube:id/skip_ad_button_text::textcontains=Skip ad")
	}
...
func (dev *Device) OpenVLCAndEnterToDownload(ctx context.Context) error {
	...
	if isNext {
		...
		if isNext {
			...
			ClickSelector(ctx, dev, "ID=org.videolan.vlc:id/design_menu_item_text::text=Directories")
		}
	}
	ClickSelector(ctx, dev, "textstartwith=Got it")
	...
	ClickSelector(ctx, dev, "ID=org.videolan.vlc:id/title::text=Download")
...
func (dev *Device) OpenSpotifyAndPlay(ctx context.Context) error {
	...
	//dev.Client.UIAClick(dev.DeviceID).Selector("descstartwith=NO"), UIAClickServiceOptions(service.Suppress()).Do(ctx)
	ClickSelector(ctx, dev, "descstartwith=NO")
	//dev.Client.UIAClick(dev.DeviceID).Selector("textstartwith=NO"), UIAClickServiceOptions(service.Suppress()).Do(ctx)
	ClickSelector(ctx, dev, "textstartwith=NO")
	//dev.Client.UIAClick(dev.DeviceID).Selector("text=CLOSE"), UIAClickServiceOptions(service.Suppress()).Do(ctx)
	ClickSelector(ctx, dev, "text=CLOSE")
	//dev.Client.UIAClick(dev.DeviceID).Selector("desc=CLOSE"), UIAClickServiceOptions(service.Suppress()).Do(ctx)
	ClickSelector(ctx, dev, "desc=CLOSE")

	init, _ := dev.Client.UIAObjEventWait(dev.DeviceID, "ID=com.spotify.music:id/play_pause_button", 6000, ui.ObjEventTypeAppear).Do(ctx)
	if !init {
		dev.Client.UIAClick(dev.DeviceID).Selector("ID=com.spotify.music:id/bottom_navigation_item_icon::desc=Search").Do(ctx)
		dev.Client.UIAClick(dev.DeviceID).Selector("ID=com.spotify.music:id/find_search_field").Do(ctx)
		dev.Client.InputText(dev.DeviceID, "Taylor Swift").Do(ctx)
		dev.Client.UIAClickListItem(dev.DeviceID,
			"ID=android:id/text1::text=Taylor Swift").ReferenceSelector(
			"ID=android:id/text2::text=Artist").ScrollSelector(
			"class=androidx.recyclerview.widget.RecyclerView").Do(ctx)

		dev.Client.UIAClick(dev.DeviceID).Selector("text=SHUFFLE PLAY").Do(ctx)
	} else {
		play, _ := dev.Client.UIAObjEventWait(dev.DeviceID, "ID=com.spotify.music:id/play_pause_button::desc=Pause", 6000, ui.ObjEventTypeAppear).Do(ctx)
		if !play {
			dev.Client.UIAClick(dev.DeviceID).Selector("ID=com.spotify.music:id/play_pause_button").Do(ctx)
		}
	}

	play, _ := dev.Client.UIAObjEventWait(dev.DeviceID, "ID=com.spotify.music:id/play_pause_button::desc=Pause", 6000, ui.ObjEventTypeAppear).Do(ctx)
	if !play {
		play, _ := dev.Client.UIAObjEventWait(dev.DeviceID, "ID=com.spotify.music:id/play_pause_button::desc=Pause", 60000, ui.ObjEventTypeAppear).Do(ctx)
		if !play {
...
func (dev *Device) OpenYoutubeMusicAndPlay(ctx context.Context) error {
	...
	ClickSelector(ctx, dev, "text=DISMISS")
