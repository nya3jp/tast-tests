...
func (dev *Device) QuitJoinedCall(ctx context.Context) {
	...
	joined, _ := dev.Client.VerifyListItem(dev.DeviceID,
		ui.NewUiNodePropDesc().Text("Hangouts")).Reference(
		ui.NewUiNodePropDesc().Text("Ongoing video call")).Parent(
		ui.NewUiNodePropDesc().Class("android.widget.FrameLayout")).Do(ctx)

	if joined {
		exit, _ := dev.Client.VerifyListItem(dev.DeviceID,
			ui.NewUiNodePropDesc().Text("Exit")).Reference(
			ui.NewUiNodePropDesc().Text("Ongoing video call")).Parent(
			ui.NewUiNodePropDesc().ResourceID("android:id/notification_action_list_margin_target")).Do(ctx)

		if !exit {
			collapsed, _ := dev.Client.VerifyListItem(dev.DeviceID,
				ui.NewUiNodePropDesc().ResourceID("android:id/expand_button").Description("Expanded")).Reference(
				ui.NewUiNodePropDesc().Text("Ongoing video call")).Parent(
				ui.NewUiNodePropDesc().Class("android.widget.FrameLayout")).Do(ctx)

			if collapsed {
				dev.Client.UIAClickListItem(dev.DeviceID, "ID=android:id/expand_button").
					ReferenceSelector("text=Ongoing video call").
					ParentSelector("class=android.widget.FrameLayout").
					ScrollSelector("ID=com.android.systemui:id/notification_stack_scroller").Do(ctx)
			}
		}

		dev.Client.UIAClickListItem(dev.DeviceID, "text=Exit").ReferenceSelector(
	...
	dev.Client.StartMainActivity(dev.DeviceID, ".SigningInActivity", "com.google.android.talk").Do(ctx)
