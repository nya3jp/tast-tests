...
func (p *testFixturePreImpl) Prepare(ctx context.Context, s *testing.PreState) interface{} {
	...
	if p.features&TFFeaturesRouters != 0 {
		if routers, ok := s.Var("routers"); ok && routers != "" {
			...
			s.Log("Using default router names")
			router1 := companionName(s, dut.CompanionSuffixRouter)
			router2 := companionName(s, dut.CompanionSuffixPcap)
			s.Logf("routers: %s, %s", router1, router2)
			ops = append(ops, TFRouter(router1, router2))
		}
	} else {
