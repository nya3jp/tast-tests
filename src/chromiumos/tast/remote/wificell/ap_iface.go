...
import (
	...
	"chromiumos/tast/remote/wificell/httpserver"
...
func StartAPIface(ctx context.Context, r router.Base, name string, conf *hostapd.Config, enableDNS, enableHTTP bool) (_ *APIface, retErr error) {
	...
	// Confirm 1: what is the resolved address that DNS server should provide to DHCP client? should be interface IP address?
	// h.ServerIP() is set to the interface h.iface in StartDHCP() -> AddIP()
	if enableDNS {
	...
	if enableHTTP {
		// Confirm 2: port 80 binding is permission denied
		h.httpd = httpserver.New("3000")
		if err := h.httpd.StartOnRouter(ctx); err != nil {
			return nil, err
		}
	}
...
func (h *APIface) Stop(ctx context.Context) error {
	...
	// Stop HTTP server
	if h.httpd != nil {
		if err := h.httpd.Stop(ctx); err != nil {
