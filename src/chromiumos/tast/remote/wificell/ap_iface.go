...
func StartAPIface(ctx context.Context, r router.Base, name string, conf *hostapd.Config) (_ *APIface, retErr error) {
	...
	if err := support.Validate(r, []string{
		"Hostapd",
		"DHCP",
	}); err != nil {
		return nil, errors.Wrap(err, "router type must support Hostapd and DHCP")
	}
	if rWithSupport, ok := r.(supportedRouter); ok {
		h.router = rWithSupport
	} else {
		return nil, errors.New("router type not supported")
	}
