...
// NewClinetInterface creates a ClinetIface that contains the queriable address properties of a network device.
func (tf *TestFixture) NewClinetInterface(ctx context.Context) (*ClientIface, error) {
	netIf, err := tf.wifiClient.WifiInterface(ctx, &empty.Empty{})
	if err != nil {
		return nil, errors.Wrap(err, "failed to get the Wifi interface name")
	}
	return &ClientIface{name: netIf.Iface, testFixture: tf}, nil
}
