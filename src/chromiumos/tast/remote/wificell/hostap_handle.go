...
// HostAPHandle is the handle object of an instance of hostap service managed by Router.
...
func (h *HostAPHandle) Config() hostapd.Config {
	return *h.config
}
...
func (h *HostAPHandle) start(ctx context.Context) (retErr error) {
	defer func() {
		if retErr == nil {
			return
		}
...
func (h *HostAPHandle) stop(ctx context.Context) error {
	var retErr error
	if h.dhcpd != nil {
		if err := h.dhcpd.Stop(ctx); err != nil {
			retErr = errors.Wrapf(retErr, "failed to stop dhcp server, err=%s", err.Error())
		}
	}
	if h.hostapd != nil {
		if err := h.hostapd.Stop(ctx); err != nil {
...
// setupIface which we're providing services on.
func (h *HostAPHandle) setupIface(ctx context.Context) error {
	var retErr error
...
// teardownIface which we provided services on.
func (h *HostAPHandle) teardownIface(ctx context.Context) error {
	var retErr error
...
// flushIP on h.iface.
func (h *HostAPHandle) flushIP(ctx context.Context) error {
...
func (h *HostAPHandle) configureIP(ctx context.Context) error {
	if err := h.flushIP(ctx); err != nil {
		return err
	}
