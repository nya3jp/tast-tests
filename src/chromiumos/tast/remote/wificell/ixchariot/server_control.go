...
func main() {
	ixChariotServer := "https://localhost:8080"
	userame := "admin"
	password := "admin"
	apiKey := "c57d632337bc4d04a82fc1b5c6985f3c"
	ws := new(restful.WebService)
	testOptions := ws.Route(ws.GET("config/ixchariot/testOptions"))
	restful.Add(ws)
	log.Fatal(http.ListenAndServe(":8080", nil))
}

func serverAuth(username, password, apiKey, ws) {
	sessionKeyURL := ws.GET("auth/session/key")
	ws.post(sessionKeyURL, WebService("username"+username, "password"+password), false)
}

//From IxChariot's python library:
//
//#!/usr/bin/env python
//# -*- coding: utf-8 -*-
