...
func (s *Sender) Send(ctx context.Context, t Type, ch int, ops ...Option) error {
	...
	if err := cmd.Run(ctx); err != nil {
		return errors.Wrapf(err, "send_management_frame failed with stderr=%q", string(buf.Bytes()))
	}
	return nil
}
...
func (s *Sender) configToArgs(ctx context.Context, c *config) ([]string, error) {
	...
	if len(c.footer) != 0 {
		p := path.Join(s.workDir, "probe_response_footer_XXX")
		f, err := fileutil.WriteTmp(ctx, s.host, p, c.footer)
		if err != nil {
