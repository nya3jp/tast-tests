...
// WpaPsk returns a WpaOption which sets psk in WPA config.
func WpaPsk(psk string) WpaOption {
	return func(c *WpaConfig) {
		c.Psk = psk
	}
}

// WpaMode returns a WpaOption which sets WPA mode in WPA config.
...
// WpaCiphers returns a WpaOption which sets the used WPA cipher in WPA config.
...
// Wpa2Ciphers returns a WpaOption which sets the used WPA2 cipher in WPA config.
...
// WpaPtkRekeyPeriod returns a WpaOption which sets maximum lifetime for PTK in WPA config.
...
// WpaGtkRekeyPeriod returns a WpaOption which sets time interval for rekeying GTK in WPA config.
...
// WpaGmkRekeyPeriod returns a WpaOption which sets time interval for rekeying GMK in WPA config.
...
// WpaUseStrictRekey returns a WpaOption which sets strict rekey in WPA config.
func WpaUseStrictRekey(use bool) WpaOption {
	return func(c *WpaConfig) {
		c.UseStrictRekey = use
	}
}

// WpaFTMode returns a WpaOption which sets fast transition mode in WPA config.
func WpaFTMode(ft FTModeEnum) WpaOption {
	return func(c *WpaConfig) {
		c.FTMode = ft
	}
}
...
// WpaConfig is the security config used to start hostapd and set properties of DUT.
...
// Gen simply calls NewWpaConfig but returns as interface Config.
...
func (c *WpaConfig) validate() error {
	...
	if c.WpaMode&WpaPure > 0 && c.WpaMode&Wpa2Pure == 0 && len(c.Wpa2Ciphers) > 0 {
...
func (c *WpaConfig) validatePsk() error {
	if len(c.Psk) == 0 {
