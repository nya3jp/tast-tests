...
// NasIdentifier returns an Option which sets nas identifier in hostapd config.
func NasIdentifier(nasID string) Option {
	return func(c *Config) {
		c.NasIdentifier = nasID
	}
}
...
// R0KHs returns an Option which sets R0KHs in hostapd config.
...
// R1KHs returns an Option which sets R1KHs in hostapd config.
...
func (c *Config) validate() error {
	...
	for i, khs := range append(c.R0KHs, c.R1KHs...) {
		...
		if len(fields) != 3 {
			return errors.New("key holder should have exactly three fields")
		}
