...
// Hidden returns an Option which sets if it is a hidden network in hostapd config.
...
func NewConfig(ops ...Option) (*Config, error) {
	...
	// We can not set the default value of HTCaps above since it would pollute the user config.
	if conf.HTCaps == 0 && (conf.is80211n() || conf.is80211ac()) {
		conf.HTCaps = HTCapHT20
	}
