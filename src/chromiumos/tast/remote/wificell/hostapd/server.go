...
func (s *Server) StartChannelSwitch(ctx context.Context, csaCount, csaChannel int, options ...CSOption) error {
	csaFreq, err := ChannelToFrequency(csaChannel)
	if err != nil {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if chConfig.Number == csaChannel {
			// Update hostapd channel.
			s.conf.Channel = csaChannel
			return nil
		}
		return errors.Errorf("failed to switch to the alternate channel %s", csaChannel)
	}, &testing.PollOptions{
