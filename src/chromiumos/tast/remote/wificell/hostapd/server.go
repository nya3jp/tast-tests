...
func (s *Server) start(fullCtx context.Context) (retErr error) {
	...
	if err := readyWriter.Wait(ctx); err != nil {
		if firstReadyErr != nil {
			return errors.Wrapf(err, "failed to wait for hostapd to be ready and ready error found: %v", firstReadyErr)
		}
		return errors.Wrap(err, "failed to wait for hostapd to be ready")
	}
	if firstReadyErr != nil {
		return errors.Wrap(err, "error raised while waiting for hostapd to be ready")
	}
