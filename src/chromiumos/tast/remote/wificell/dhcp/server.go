...
func (d *Server) start(fullCtx context.Context) (err error) {
	...
	port := 0
	confVals := make(map[string]string)
	...
	confVals["port"] = strconv.Itoa(port)
	confVals["iface"] = d.iface
	confVals["ipStart"] = d.ipStart.String()
	confVals["ipEnd"] = d.ipEnd.String()
	confVals["leasefile"] = d.leasePath()
