...
func StartServer(ctx context.Context, host *ssh.Conn, name, iface, workDir string, ipStart, ipEnd net.IP, opt DNSOption) (*Server, error) {
...
func (d *Server) start(fullCtx context.Context) (err error) {
	...
	confVals := map[string]string{
		"port":      strconv.Itoa(d.dnsOpt.Port),
		"iface":     d.iface,
		"ipStart":   d.ipStart.String(),
		"ipEnd":     d.ipEnd.String(),
		"leasefile": d.leasePath(),
		"address":   fmt.Sprintf("/%s/%s", d.dnsOpt.ResolvedHost, resolvedIP),
	}
	...
	testing.ContextLogf(ctx, "Debug: dnsmasq configuration is %s", b.String())
