...
func NewRouter(ctx, daemonCtx context.Context, host *ssh.Conn, name string) (*Router, error) {
	...
	if err := common.StartLogCollectors(daemonCtx, r.host, r.logCollectors, logsToCollect, true); err != nil {
		r.Close(shortCtx)
...
func (r *Router) startHostapdOnIface(ctx context.Context, iface, name string, conf *hostapd.Config) (_ *hostapd.Server, retErr error) {
	...
	conf.EnvironmentVars["OPENSSL_CONF"] = "/etc/ssl/openssl.cnf.compat"
	conf.EnvironmentVars["OPENSSL_CHROMIUM_SKIP_TRUSTED_PURPOSE_CHECK"] = "1"
