...
func (r *Router) StartHTTP(ctx context.Context, name, iface, redirectAddr, httpScriptPath string, port, statusCode int) (_ *http.Server, retErr error) {
	httpServer, err := http.StartServer(ctx, r.host, name, iface, r.workDir(), redirectAddr, httpScriptPath, port, statusCode)
	if err != nil {
