...
func NewLegacyRouter(ctx, daemonCtx context.Context, host *ssh.Conn, name string) (*legacyRouterStruct, error) {
	r := &legacyRouterStruct{
		BaseRouterStruct: sharedrouter.BaseRouterStruct{
			Host:  host,
			Name:  name,
			Rtype: sharedrouter.LegacyT,
		},
	...
	shortCtx, cancel := sharedrouter.ReserveForRouterClose(ctx)
	defer cancel()
	...
	if err := r.Host.CommandContext(shortCtx, "sh", "-c", strings.Join([]string{"rm", "-rf", sharedrouter.AutotestWorkdirGlob}, " ")).Run(); err != nil {
		r.Close(shortCtx)
		return nil, errors.Wrapf(err, "failed to remove workdir %q", sharedrouter.AutotestWorkdirGlob)
	}
	...
	if err := r.removeDevicesWithPrefix(shortCtx, sharedrouter.BridgePrefix); err != nil {
		r.Close(shortCtx)
		return nil, err
	}
	// Note that we only need to remove one side of each veth pair.
	if err := r.removeDevicesWithPrefix(shortCtx, sharedrouter.VethPrefix); err != nil {
		r.Close(shortCtx)
		return nil, err
	}
...
func (r *legacyRouterStruct) RouterType() sharedrouter.Type {
	return r.Rtype
}
...
func (r *legacyRouterStruct) Close(ctx context.Context) error {
	...
	if err := r.removeDevicesWithPrefix(ctx, sharedrouter.BridgePrefix); err != nil {
		return err
	}
	// Note that we only need to remove one side of each veth pair.
	if err := r.removeDevicesWithPrefix(ctx, sharedrouter.VethPrefix); err != nil {
		return err
	}
...
func (r *legacyRouterStruct) workDir() string {
	return sharedrouter.WorkingDir
}
...
func (r *legacyRouterStruct) NewBridge(ctx context.Context) (_ string, retErr error) {
	br := fmt.Sprintf("%s%d", sharedrouter.BridgePrefix, r.bridgeID)
	r.bridgeID++
...
func (r *legacyRouterStruct) NewVethPair(ctx context.Context) (_, _ string, retErr error) {
	veth := fmt.Sprintf("%s%d", sharedrouter.VethPrefix, r.vethID)
	vethPeer := fmt.Sprintf("%s%d", sharedrouter.VethPeerPrefix, r.vethID)
	r.vethID++
...
func (r *legacyRouterStruct) ReleaseVethPair(ctx context.Context, veth string) error {
	...
	if strings.HasPrefix(veth, sharedrouter.VethPeerPrefix) {
		veth = sharedrouter.VethPrefix + veth[len(sharedrouter.VethPeerPrefix):]
	}
	vethPeer := sharedrouter.VethPeerPrefix + veth[len(sharedrouter.VethPrefix):]
