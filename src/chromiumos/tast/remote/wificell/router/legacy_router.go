...
import (
	...
	"chromiumos/tast/remote/wificell/util"
	"chromiumos/tast/ssh"
...
func (r *legacyRouterStruct) Close(ctx context.Context) error {
	...
	for _, nd := range r.availIfaces {
		if err := r.removeWifiIface(ctx, nd.IfName); err != nil {
			util.CollectFirstErr(ctx, &firstErr, errors.Wrap(err, "failed to remove interfaces"))
		}
	}
	...
	if err := r.collectLogs(ctx, ".close"); err != nil {
		util.CollectFirstErr(ctx, &firstErr, errors.Wrap(err, "failed to collect logs"))
	}
	if err := r.stopLogCollectors(ctx); err != nil {
		util.CollectFirstErr(ctx, &firstErr, errors.Wrap(err, "failed to stop loggers"))
	}
	if err := r.host.Command("rm", "-rf", r.workDir()).Run(ctx); err != nil {
		util.CollectFirstErr(ctx, &firstErr, errors.Wrap(err, "failed to remove working dir"))
	}
	return firstErr
}
...
func (r *legacyRouterStruct) StopHostapd(ctx context.Context, hs *hostapd.Server) error {
	...
	iface := hs.Interface()
	if err := hs.Close(ctx); err != nil {
		util.CollectFirstErr(ctx, &firstErr, errors.Wrap(err, "failed to stop hostapd"))
	}
	util.CollectFirstErr(ctx, &firstErr, r.ipr.SetLinkDown(ctx, iface))
	r.freeIface(iface)
	return firstErr
}
...
func (r *legacyRouterStruct) StopDHCP(ctx context.Context, ds *dhcp.Server) error {
	...
	if err := ds.Close(ctx); err != nil {
		util.CollectFirstErr(ctx, &firstErr, errors.Wrap(err, "failed to stop dhcpd"))
	}
	util.CollectFirstErr(ctx, &firstErr, r.ipr.FlushIP(ctx, iface))
	return firstErr
}
...
func (r *legacyRouterStruct) StopCapture(ctx context.Context, capturer *pcap.Capturer) error {
	...
	if err := capturer.Close(ctx); err != nil {
		util.CollectFirstErr(ctx, &firstErr, errors.Wrap(err, "failed to stop capturer"))
	}
	if err := r.ipr.SetLinkDown(ctx, iface); err != nil {
		util.CollectFirstErr(ctx, &firstErr, err)
	}
	r.freeIface(iface)
	return firstErr
}
...
func (r *legacyRouterStruct) ReleaseBridge(ctx context.Context, br string) error {
	var firstErr error
	util.CollectFirstErr(ctx, &firstErr, r.ipr.FlushIP(ctx, br))
	util.CollectFirstErr(ctx, &firstErr, r.ipr.SetLinkDown(ctx, br))
	util.CollectFirstErr(ctx, &firstErr, r.ipr.DeleteLink(ctx, br))
	return firstErr
}
...
func (r *legacyRouterStruct) ReleaseVethPair(ctx context.Context, veth string) error {
	...
	var firstErr error
	util.CollectFirstErr(ctx, &firstErr, r.ipr.FlushIP(ctx, veth))
	util.CollectFirstErr(ctx, &firstErr, r.ipr.SetLinkDown(ctx, veth))
	util.CollectFirstErr(ctx, &firstErr, r.ipr.FlushIP(ctx, vethPeer))
	util.CollectFirstErr(ctx, &firstErr, r.ipr.SetLinkDown(ctx, vethPeer))
	// Note that we only need to delete one side.
	util.CollectFirstErr(ctx, &firstErr, r.ipr.DeleteLink(ctx, veth))
	return firstErr
}
