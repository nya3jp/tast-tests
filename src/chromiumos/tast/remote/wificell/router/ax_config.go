...
const (
	...
	// KeyObssCap is a boolean (0,1) dictating the router's 160Mhz capabilites on GT0AX11000 devices
	KeyObssCap NvramKeyEnum = "obss_coex"
	// KeyBw160 is a boolean (0,1) dictating the router's 160Mhz capabilites on AX6100 devices only
	...
	// KeyBssOpModeCapReqd is an int dictating the required capabilites required for BSS Operation (for Ax6100 only)
	...
	return func(c *Config) {
		...
		case Mode80211ac:
			if c.Type == GtAx11000 {
				...
				c.RouterConfigParams = append(c.RouterConfigParams, AxRouterConfigParam{Wl2, Key11Ax, "0"}, AxRouterConfigParam{Wl2, KeyBssOpModeCapReqd, "0"}, AxRouterConfigParam{Wl2, KeyNModeX, "0"})
		...
		case Mode80211ax:
			if c.Type == GtAx11000 {
				...
				c.RouterConfigParams = append(c.RouterConfigParams, AxRouterConfigParam{Wl2, Key11Ax, "1"}, AxRouterConfigParam{Wl2, KeyBssOpModeCapReqd, "4"}, AxRouterConfigParam{Wl2, KeyNModeX, "9"})
			}
...
func ChanBandwidth(ch int, bw ChanBandwidthEnum) Option {
	return func(c *Config) {
		...
		if c.Type == GtAx11000 {
			c.RouterConfigParams = append(c.RouterConfigParams, AxRouterConfigParam{c.Band, KeyBw, strconv.Itoa(bandw)}, AxRouterConfigParam{c.Band, KeyBwCap, strconv.Itoa(bwCap)}, AxRouterConfigParam{c.Band, KeyObssCap, strconv.Itoa(bw160)})
		} else if c.Type == Ax6100 {
			c.RouterConfigParams = append(c.RouterConfigParams, AxRouterConfigParam{c.Band, KeyBw, strconv.Itoa(bandw)}, AxRouterConfigParam{c.Band, KeyBwCap, strconv.Itoa(bwCap)}, AxRouterConfigParam{c.Band, KeyBw160, strconv.Itoa(bw160)})
		}
		c.RouterConfigParams = append(c.RouterConfigParams, AxRouterConfigParam{c.Band, KeyChanspec, fmt.Sprintf("%d%s", ch, suffix)})
	}
}
