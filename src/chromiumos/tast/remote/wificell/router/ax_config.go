...
func Channel(channel int) Option {
	return func(c *Config) {
		c.routerConfigParams = append(c.routerConfigParams, AxRouterConfigParam{c.Band, KeyChanspec, fmt.Sprintf("%dl", channel)})
	}
}
	...
	return func(c *Config) {
		...
		case Mode80211ac:
			for _, band := range []BandEnum{Wl0, Wl1, Wl2} {
				c.routerConfigParams = append(c.routerConfigParams, AxRouterConfigParam{band, KeyTxBfBfeCap, "1"}, AxRouterConfigParam{band, KeyTxBfBfrCap, "1"})
			}
		case Mode80211ax:
			for _, band := range []BandEnum{Wl0, Wl1, Wl2} {
				c.routerConfigParams = append(c.routerConfigParams, AxRouterConfigParam{band, KeyTxBfBfeCap, "5"}, AxRouterConfigParam{band, KeyTxBfBfrCap, "5"})
			}
		}
	}
}
