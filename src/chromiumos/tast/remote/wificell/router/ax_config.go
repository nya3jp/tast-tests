...
type Config struct {
	...
	Ssid               string
	NvramOut           *string
	RouterRecoveryMap  map[string]AxRouterConfigParam
...
func Radio(enabled bool) Option {
	return func(c *Config) {
		if enabled {
			c.RouterConfigParams = append(c.RouterConfigParams, AxRouterConfigParam{c.Band, KeyRadio, "1"})
		} else {
			c.RouterConfigParams = append(c.RouterConfigParams, AxRouterConfigParam{c.Band, KeyRadio, "0"})
		}
	}
}
