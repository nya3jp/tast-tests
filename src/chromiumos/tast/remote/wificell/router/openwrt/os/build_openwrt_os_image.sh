...
FILES_DIR="${BASE_TMP_DIR}/included_files"
if [ -d "${FILES_DIR}" ]; then
  rm -rf "${FILES_DIR}"
fi
mkdir "${FILES_DIR}"
...
# Disable services by default TODO
DISABLED_SERVICES=(
  "wpad"
  "dhcp"
)
...
EXTRA_IMAGE_NAME="cros"
echo "Building image..."
cd "${BUILDER_DIR}"  && umask 022 && make image PROFILE="${BUILD_PROFILE}" PACKAGES="${PACKAGES_STR}" FILES="${FILES_DIR}" BIN_DIR="${BIN_DIR}" EXTRA_IMAGE_NAME="${EXTRA_IMAGE_NAME}"
