...
func (r *Router) closeActiveServices(ctx context.Context) error {
	...
	for len(r.activeServices.http) != 0 {
		if err := r.StopHTTP(ctx, r.activeServices.http[0]); err != nil {
			wifiutil.CollectFirstErr(ctx, &firstError, err)
		}
	}
...
func (r *Router) StartHTTP(ctx context.Context, name, iface, port, statusCode, redirectAddr string) (_ *http.Server, retErr error) {
	...
	r.activeServices.http = append(r.activeServices.http, httpServer)
...
func (r *Router) StopHTTP(ctx context.Context, httpServer *http.Server) error {
	...
	// Remove from active services.
	for i, service := range r.activeServices.http {
		if httpServer == service {
			active := make([]*hostapd.Server, 0)
			active = append(active, r.activeServices.hostapd[:i]...)
			active = append(active, r.activeServices.hostapd[i+1:]...)
			r.activeServices.hostapd = active
