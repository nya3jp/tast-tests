...
type Router struct {
	...
	iwr            *remote_iw.Runner
	ipr            *remote_ip.Runner
...
func NewRouter(ctx, daemonCtx context.Context, host *ssh.Conn, name string) (*Router, error) {
	...
	shortCtx, cancel := common.ReserveForRouterClose(ctx)
	defer cancel()
	...
	if err := common.StartLogCollectors(daemonCtx, r.host, r.logCollectors, logsToCollect); err != nil {
	...
	if err := r.iwr.SetRegulatoryDomain(shortCtx, "US"); err != nil {
		err = errors.Wrap(err, "failed to set regulatory domain to US")
		closeBeforeErrorReturn(err)
		return nil, err
	}
...
func (r *Router) setupWifiPhys(ctx context.Context) error {
	...
	for _, p := range phys {
		...
		// TODO confirm this cat manually or if path is different
