...
// uci CLI flags, as documented by https://openwrt.org/docs/guide-user/base-system/uci#usage.
const (
	flagConfigPath                 string = "-c" // <path>; set the search path for config files (default: /etc/config)
	FlagDelimiter                         = "-d" // set the delimiter for list values in uci show
	flagFileInput                         = "-f" // <file>; use <file> as input instead of stdin
	FlagMerge                             = "-m" // when importing, merge data into an existing package
	FlagNameUnnamedSections               = "-n" // name unnamed sections on export (default)
	FlagDoNotNameUnnamedSections          = "-N" // don't name unnamed sections
	flagSearchPath                        = "-p" // <path>; add a search path for config change files
	flagSearchPathDefault                 = "-P" // <path>; add a search path for config change files and use as default
	FlagQuietMode                         = "-q" // quiet mode (don't print error messages)
	FlagForceStrictMode                   = "-s" // force strict mode (stop on parser errors, default)
	FlagDisableStrictMode                 = "-S" // disable strict mode
	FlagDoNotUseShowExtendedSyntax        = "-X" // do not use extended syntax on 'show'
)

// FlagConfigPath builds the parameterized CLI flag to set the search path for
// config files (default: /etc/config).
func FlagConfigPath(path string) string {
	return fmt.Sprintf("%s %q", flagConfigPath, path)
}

// FlagFileInput builds the parameterized CLI flag to use <file> as input
// instead of stdin.
func FlagFileInput(file string) string {
	return fmt.Sprintf("%s %q", flagFileInput, file)
}

// FlagSearchPath builds the parameterized CLI flag to add a search path for
// config change files.
func FlagSearchPath(path string) string {
	return fmt.Sprintf("%s %q", flagSearchPath, path)
}

// FlagSearchPathDefault builds the parameterized CLI flag to add a search path
// for config change files and use as default.
func FlagSearchPathDefault(path string) string {
	return fmt.Sprintf("%s %q", flagSearchPathDefault, path)
}
