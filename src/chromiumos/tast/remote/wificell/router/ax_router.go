...
func (r *axRouterStruct) getDefaultValue(setting AxRouterConfigParam, contents string) (AxRouterConfigParam, error) {
	...
	for _, m := range matches {
		for i, tag := range re.SubexpNames() {
			if tag == "value" {
				return AxRouterConfigParam{setting.Band, setting.Key, m[i]}, nil
			}
		}
	}
	return AxRouterConfigParam{}, errors.New("could not find default value")
}
...
func (r *axRouterStruct) RestoreConfiguration(ctx context.Context, recoveryMap map[string]AxRouterConfigParam) error {
	defer r.restartWirelessService(ctx)
	paramList := make([]AxRouterConfigParam, 0, len(recoveryMap))
