...
// selectPhy finds an suitable phy for the given channel and target interface type t.
// The selected phy index is returned.
func (r *Router) selectPhy(ctx context.Context, channel int, t iw.IfType) (int, error) {
...
func (r *Router) selectInterface(ctx context.Context, channel int, t iw.IfType) (*iw.NetDev, error) {
	phyID, err := r.selectPhy(ctx, channel, t)
	if err != nil {
		return nil, err
	}
...
func (r *Router) StartAPIface(ctx context.Context, name string, conf *hostapd.Config) (*APIface, error) {
	...
	nd, err := r.selectInterface(ctx, conf.Channel, iw.IfTypeManaged)
	if err != nil {
		return nil, err
	}
	iface := nd.IfName
...
func (r *Router) StartCapture(ctx context.Context, name string, ch int, freqOps []iw.SetFreqOption, pcapOps ...pcap.Option) (ret *pcap.Capturer, retErr error) {
	...
	nd, err := r.selectInterface(ctx, ch, iw.IfTypeMonitor)
	if err != nil {
		return nil, err
	}
	iface := nd.IfName
	...
	if !shared {
		// The interface is not shared, set up frequence and bandwidth.
		...
		testing.ContextLogf(ctx, "Skip configuring of shared interface %s", iface)
	}
...
// StopCapture stops the packet capturer and release related resources.
func (r *Router) StopCapture(ctx context.Context, capturer *pcap.Capturer) error {
	var err error
	iface := capturer.Interface()
	if err2 := capturer.Close(ctx); err2 != nil {
		err = errors.Wrapf(err, "failed to stop capturer, err=%s", err2.Error())
	}
	if err2 := r.host.Command("ip", "link", "set", iface, "down").Run(ctx); err2 != nil {
		err = errors.Wrapf(err, "failed to set %s down, err=%s", iface, err2.Error())
	}
	r.freeIface(iface)
	return err
}
