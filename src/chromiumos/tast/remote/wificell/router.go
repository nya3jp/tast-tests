...
// service is returned.
func (r *Router) StartHostAP(ctx context.Context, conf *hostapd.Config) (ret *HostAPHandle, retErr error) {
	name := r.getUniqueID()
...
func (r *Router) startHostapd(ctx context.Context, name string, conf *hostapd.Config) (*hostapd.Server, error) {
	...
	server, err := hostapd.NewServer(ctx, r.host, name, iface, r.workDir(), conf)
	if err != nil {
...
func (r *Router) stopHostapd(ctx context.Context, server *hostapd.Server) error {
	var err error
	iface := server.Interface()

	if err2 := server.Stop(ctx); err2 != nil {
