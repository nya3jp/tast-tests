...
func (r *Router) StopHostapd(ctx context.Context, hs *hostapd.Server, iface string) error {
	var firstErr error
	if err := hs.Close(ctx); err != nil {
...
func (r *Router) StartDHCP(ctx context.Context, name, iface string, ipStart, ipEnd, serverIP, broadcastIP net.IP, mask net.IPMask) (_ *dhcp.Server, retErr error) {
	...
	defer cancel()
	return dhcp.StartServer(ctx, r.host, name, iface, r.workDir(), ipStart, ipEnd)
}

// StopDHCP stops the DHCP server and flushes the interface.
func (r *Router) StopDHCP(ctx context.Context, ds *dhcp.Server, iface string) error {
	var firstErr error
