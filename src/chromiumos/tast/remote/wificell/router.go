...
func (r *Router) waitBridgeState(ctx context.Context, br string, expectedState ip.LinkState) error {
	...
	return testing.Poll(ctx, func(ctx context.Context) error {
		state, _, err := r.ipr.LinkStatus(ctx, br)
		if err != nil {
...
// TODO: Find a better way to make sure that shill has already enabled/disabled the bridge.
...
func (r *Router) cloneMAC(ctx context.Context, dst, src string) error {
	_, mac, err := r.ipr.LinkStatus(ctx, src)
	if err != nil {
		return err
	}
...
func (r *Router) MAC(ctx context.Context, iface string) (net.HardwareAddr, error) {
	_, mac, err := r.ipr.LinkStatus(ctx, iface)
	return mac, err
}
