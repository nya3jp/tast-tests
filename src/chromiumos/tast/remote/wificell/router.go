...
func (r *Router) removeWifiIfaces(ctx context.Context) error {
	...
	for _, w := range wdevs {
		...
		if out, err := r.host.Command("iw", "dev", w.IfName, "del").CombinedOutput(ctx); err != nil {
			return errors.Wrapf(err, "failed to delete wdev %s: %q", w.IfName, string(out))
		}
	}
	return nil
}
...
func (r *Router) selectPhy(ctx context.Context, channel int) (int, error) {
	...
	better := func(id1 int, p1 *iw.Phy, id2 int, p2 *iw.Phy) bool {
		if p1 == nil {
			return false
		}
		if p2 == nil {
		...
		// Prefer free phy. If both are free, prefer p2.
		return r.busyPhy[id1] == 0 && r.busyPhy[id2] > 0
	}
	...
	for id, phy := range r.phys {
		...
		if better(id, phy, selectedID, selected) {
			selectedID = id
			selected = phy
		}
	}
...
// getUniqueHandle returns an unique ID string for services on this router. Useful for giving names to daemons/services.
func (r *Router) getUniqueHandle() string {
	id := strconv.Itoa(r.handleID)
	r.handleID++
	return id
}

// StartInterface starts a hostapd service which includes hostapd and dhcpd. It will select a suitable
// phy and interface automatically. The handle object for the service is returned.
func (r *Router) StartInterface(ctx context.Context, conf *hostapd.Config) (*InterfaceHandle, error) {
	// Reserve required resources.
	name := r.getUniqueHandle()
...
// StopInterface stops the InterfaceHandle and free related resources.
func (r *Router) StopInterface(ctx context.Context, h *InterfaceHandle) error {
	err := h.stop(ctx)
