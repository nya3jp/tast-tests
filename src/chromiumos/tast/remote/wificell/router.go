...
func (r *Router) phy(ctx context.Context, channel int, t iw.IfType) (int, error) {
	...
	// Try to find any phy which is suitable, even an idle one
	for id, phy := range r.phys {
		if phySupportsFrequency(phy, freq) {
			return id, nil
		}
...
// ensureUniqueMAC reconfigures an interface's MAC address if needed to satisfy uniqueness requirements.
//
// The Linux kernel does not allow multiple APs with the same BSSID on the same PHY (at least, with some
// drivers). Hence, we want to ensure that the DEVs for a PHY have unique MAC addresses.
//
// Note that we do not attempt to make the MACs unique across PHYs, because some tests deliberately create such scenarios.
func (r *Router) ensureUniqueMAC(ctx context.Context, nd *iw.NetDev) error {
	...
	unique := true
	for _, sibling := range r.busyIfaces {
		if sibling.PhyNum != nd.PhyNum || sibling.IfName == nd.IfName || sibling.IfType == "monitor" {
			continue
		}
		sibMAC, err := r.ipr.MAC(ctx, sibling.IfName)
		if err != nil {
