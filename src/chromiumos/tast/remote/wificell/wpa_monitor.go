...
var eventDefs = []eventDef{
	{
		...
		func(matches []string) (parsedEvent SupplicantEvent, firstError error) {
...
func (w *WPAMonitor) waitReady(ctx context.Context) (err error) {
...
// atoi parses string to inte, writes error to *parseErr only if it's nil
func atoi(str string, parseErr *error) (value int) {
...
// and firstErr contains the first parsing error that occured.
