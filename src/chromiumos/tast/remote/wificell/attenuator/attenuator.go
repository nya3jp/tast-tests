...
func Open(ctx context.Context, host string, proxyConn *ssh.Conn) (att *Attenuator, errRet error) {
	...
	var maxAtten, maxFreq string
	unpack(retSlice, &a.model, &maxFreq, &maxAtten)
	if a.model == "RC4DAT" {
		a.channels = 4
	} else {
		a.channels = 1
	...
	a.maxFreq, err = strconv.ParseFloat(maxFreq, 64)
	if err != nil {
		return nil, errors.Wrapf(err,
			"Unable to parse frequency from model number %s", ret)
	}
...
func (a *Attenuator) MinTotalAttenuation(channel int) float64 {
	return a.minTotalAttenuation[channel]
}
