...
const (
	maxAttenDb = 95 // Maximum for RC4DAT
)
...
func (a *Attenuator) sendCmd(ctx context.Context, cmd string) (string, error) {
	res, err := a.proxyConn.Command("wget", "-q", "-O", "-",
...
func (a *Attenuator) SetAtten(ctx context.Context, channel uint, val float64) error {
	if channel > a.channels {
		return errors.Errorf("invalid channel %d, attenuator model %s", channel, a.model)
	}
	...
	if ret != "1" {
		return errors.Errorf("failed to set given attenuation, ret: %s", ret)
	}
	return nil
}
...
func (a *Attenuator) SetTotalAttenuation(ctx context.Context, channel uint, attenDb float64, frequencyMhz uint) error {
	...
	return a.SetAtten(ctx, channel, variableAttenDb)
}
