...
// isP2PIface checks if the interface type is P2P.
func isP2PIface(ctx context.Context, iface string, host *ssh.Conn) (bool, error) {
	iwr := remoteiw.NewRemoteRunner(host)
	dev, err := iwr.InterfaceAttributes(ctx, iface)
	if err != nil {
		return false, err
	}
...
func (tf *TestFixture) p2pIface(ctx context.Context, host *ssh.Conn) (string, error) {
	...
	for _, iface := range ifaces {
		isP2P, err := isP2PIface(ctx, iface, host)
		if err != nil {
			return "", err
		}
		if isP2P {
...
func (tf *TestFixture) P2PConfigureGO(ctx context.Context) error {
	...
	tf.p2pGOIface, err = tf.p2pIface(ctx, tf.duts[DefaultDUT].dut.Conn())
	if err != nil {
		return err
	}
...
func (tf *TestFixture) P2PConnect(ctx context.Context) error {
	...
	tf.p2pClientIface, err = tf.p2pIface(ctx, tf.duts[PeerDUT].dut.Conn())
	if err != nil {
		return err
	}

	if tf.p2pClientIface == "" {
		return errors.New("failed to find the p2p interface")
	}
	...
	if !connected {
		for {
			...
			if _, connectionSuccess := event.(*wpacli.ConnectedEvent); connectionSuccess {
				break
			}
		}
	}
