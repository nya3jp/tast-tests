...
func (tf *TestFixture) ArpingFromDUT(ctx context.Context, serverIP string, ops ...arping.Option) error {
	...
	testing.ContextLogf(ctx, "arping from DUT: sent=%d received=%d loss=%g avgLatency=%.3f", res.Sent, res.Received, res.Loss, res.AvgLatency)
...
func (tf *TestFixture) ArpingFromServer(ctx context.Context, serverIface string, ops ...arping.Option) error {
	...
	res, err := runner.Arping(ctx, clientIP.String(), serverIface, ops...)
	if err != nil {
		return errors.Wrap(err, "arping failed")
	}
	testing.ContextLogf(ctx, "arping from server: sent=%d received=%d loss=%g avgLatency=%.3f", res.Sent, res.Received, res.Loss, res.AvgLatency)
