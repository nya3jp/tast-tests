...
import (
	...
	ciw "chromiumos/tast/common/network/iw"
...
func (tf *TestFixture) VerifyConnection(ctx context.Context, ap *APIface) error {
	...
	// Check frequency.
	clientFreq, err := iw.NewRemoteRunner(tf.dut.Conn()).LinkValue(ctx, iface, ciw.LinkKeyFrequency)
	if err != nil {
		return errors.Wrap(err, "failed to get client frequency: ")
	}
	...
	if clientFreq != strconv.Itoa(serverFreq) {
		return errors.Errorf("frequency does not match, got %s want %d", clientFreq, serverFreq)
	}
