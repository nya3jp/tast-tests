...
func (tf *TestFixture) ReserveForDeconfigAP(ctx context.Context, ap *APIface) (context.Context, context.CancelFunc) {
	...
	if capturer := tf.capturers[ap]; capturer != nil {
		// Shorten more if capturer available, drop CancelFunc as the parent's is enough.
...
// If no Capturer is spawned, nil is returned.
func (tf *TestFixture) Capturer(ap *APIface) *pcap.Capturer {
	return tf.capturers[ap]
}
