...
func NewTestFixture(fullCtx, daemonCtx context.Context, d *dut.DUT, rpcHint *testing.RPCHint, ops ...TFOption) (ret *TestFixture, retErr error) {
	...
	if tf.ciscoCtrlTarget != "" {
		...
		splitUserPasswd := strings.Split(splitTarget[0], ":")
		if len(splitTarget) != 2 || len(splitUserPasswd) != 2 {
			return nil, errors.New("invalid ciscoctrl parameter format, should be user:passwd@host")
		}

		testing.ContextLog(ctx, "Initializing Cisco controller: ", splitTarget[2])
...
func (tf *TestFixture) Close(ctx context.Context) error {
	...
	if tf.ciscoCtrl != nil {
		if err := tf.ciscoCtrl.Close(ctx); err != nil {
			collectFirstErr(ctx, &firstErr, errors.Wrap(err, "failed to close Cisco controller"))
		}
		tf.ciscoCtrl = nil
	}
...
func (tf *TestFixture) configureCiscoAPOnRouterID(ctx context.Context, idx int, ops []hostapd.Option, fac security.ConfigFactory) (ret APIface, retErr error) {
	...
	ap, err := cisco.StartCiscoAP(ctx, tf.ciscoCtrl, config)
	if err != nil {
