...
func (tf *TestFixture) SuspendAndAssertConnect(ctx context.Context, wakeUpTimeout int) (time.Duration, error) {
	...
	resp, err := tf.wifiClient.SuspendAndAssertConnect(ctx, &network.SuspendAndAssertConnectRequest{
		WakeUpTimeout: int32(wakeUpTimeout),
		ServicePath:   service.ServicePath,
	})
	if err != nil {
		return 0, errors.Wrap(err, "failed to suspend and assert connection")
	}
	return time.Duration(resp.ReconnectTime) * time.Nanosecond, nil
}
