...
func (tf *TestFixture) P2PConfigureGO(ctx context.Context) error {
	...
	tf.p2pGOSSID, err = wpa.GetNetworkSSID(ctx, tf.p2pGOIface)
	if err != nil {
		return err
	}
	tf.p2pGOPassphrase, err = wpa.P2PGetGOPasspharase(ctx, 0, tf.p2pGOIface)
	if err != nil {
		return err
	}
...
func (tf *TestFixture) P2PConnect(ctx context.Context) error {
	...
	if err = wpa.P2PWaitForConnected(ctx, tf.p2pGOSSID, tf.p2pGOIface); err != nil {
		return err
	}
...
func (tf *TestFixture) P2PAddIPRoute(ctx context.Context) error {
	...
	if err := iprDUT.RouteIP(ctx, tf.p2pGOIface, net.ParseIP(p2pGOIPAddress)); err != nil {
		return err
	}
	if err := iprPeer.RouteIP(ctx, tf.p2pClientIface, net.ParseIP(p2pClientIPAddress)); err != nil {
		return err
	}
...
func (tf *TestFixture) P2PDeleteIPRoute(ctx context.Context) error {
	...
	if err := iprDUT.DelIP(ctx, tf.p2pClientIface, net.ParseIP(p2pGOIPAddress), 24); err != nil {
		return err
	}
...
func (tf *TestFixture) P2PAssertPingFromGO(ctx context.Context, opts ...ping.Option) error {
	...
	defer st.End()
	testing.Conextlog(ctx, "Pinging GO --> Client")
...
func (tf *TestFixture) P2PAssertPingFromClient(ctx context.Context, opts ...ping.Option) error {
	...
	defer st.End()
	testing.Conextlog(ctx, "Pinging Client --> GO")
...
func (tf *TestFixture) P2PDeconfigureGO(ctx context.Context) error {
	wpa_DUT := wpacli.NewRunner(&cmd.RemoteCmdRunner{Host: tf.dut.Conn()})

	if err := wpa_DUT.P2PGroupRemove(ctx, tf.p2pGOIface); err != nil {
		return err
	}
	if err := wpa_DUT.P2PRemoveAllNetwroks(ctx); err != nil {
		return err
	}
	if err := wpa_DUT.P2PFlush(ctx); err != nil {
		return err
	}
	testing.Conextlog(ctx, "P2P GO deconfigured")
...
func (tf *TestFixture) P2PDeconfigureClient(ctx context.Context) error {
	wpa_Peer := wpacli.NewRunner(&cmd.RemoteCmdRunner{Host: tf.peer.Conn()})

	if err := wpa_Peer.P2PGroupRemove(ctx, tf.p2pClientIface); err != nil {
		return err
	}
	if err := wpa_Peer.P2PRemoveAllNetwroks(ctx); err != nil {
		return err
	}
	if err := wpa_Peer.P2PFlush(ctx); err != nil {
		return err
	}
	testing.Conextlog(ctx, "P2P Client deconfigured")
