...
func TFRouterType(rtype sharedrouter.Type) TFOption {
	return func(tf *TestFixture) {
		tf.routerType = rtype
	}
}
...
func TFPcapType(rtype sharedrouter.Type) TFOption {
	return func(tf *TestFixture) {
		tf.pcapType = rtype
	}
}
...
type TestFixture struct {
	...
	routers    []routerData
	routerType sharedrouter.Type
	pcapType   sharedrouter.Type
...
func (tf *TestFixture) connectCompanion(ctx context.Context, hostname string, retryDNSNotFound bool) (*ssh.Conn, error) {
	...
	if tf.routerType == sharedrouter.AxT {
		sopt.User = "admin"
	}
...
func NewTestFixture(fullCtx, daemonCtx context.Context, d *dut.DUT, rpcHint *testing.RPCHint, ops ...TFOption) (ret *TestFixture, retErr error) {
	...
	tf := &TestFixture{
		...
		routerType: sharedrouter.LegacyT,
		// Set the pcap capture device's default router type.
		pcapType: sharedrouter.LegacyT,
...
func newRouter(ctx, daemonCtx context.Context, host *ssh.Conn, name string, rtype sharedrouter.Type) (router.Base, error) {
	ctx, st := timing.Start(ctx, "NewRouter")
	defer st.End()
	switch rtype {
	case sharedrouter.LegacyT:
		return legacyrouter.NewLegacyRouter(ctx, daemonCtx, host, name)
	case sharedrouter.AxT:
