...
func TFRouterType(rtype RouterType) TFOption {
	return func(tf *TestFixture) {
		tf.rtype = rtype
	}
}
...
func TFPcapType(rtype RouterType) TFOption {
	return func(tf *TestFixture) {
		tf.ptype = rtype
	}
}
...
func NewTestFixture(fullCtx, daemonCtx context.Context, d *dut.DUT, rpcHint *testing.RPCHint, ops ...TFOption) (ret *TestFixture, retErr error) {
	...
	for i := range tf.routers {
		...
		routerObj, err := NewRouter(ctx, daemonCtx, router.host,
			strings.ReplaceAll(router.target, ":", "_"), tf.rtype)
		if err != nil {
	...
	if tf.pcapHost == nil && tf.pcapTarget != "" {
		...
		if err != nil {
			...
			tf.pcap, err = NewRouter(ctx, daemonCtx, tf.pcapHost, "pcap", tf.ptype)
			if err != nil {
