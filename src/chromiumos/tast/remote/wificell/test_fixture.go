...
func (tf *TestFixture) SetAPInterfaceDown(ctx context.Context, ap *APIface) error {
	if err := tf.router.SetAPIfaceDown(ctx, ap); err != nil {
		return errors.Wrap(err, "failed to set the AP interface down")
	}
	return nil
}
...
// AssureDisconnectService assures that the WiFi service with the SSID has disconnected.
func (tf *TestFixture) AssureDisconnectService(ctx context.Context, ssid string) error {
