...
func (tf *TestFixture) rebootRouter(ctx context.Context, rd *routerData) error {
	...
	defer t.End()
	routerMsgName := fmt.Sprintf("%s router %q", rd.object.RouterType().String(), rd.object.RouterName())
	...
	var err error
	if rd.host, err = tf.connectCompanion(ctx, rd.target, true); err != nil {
	...
	if rd.object, err = newRouter(ctx, ctx, rd.host, rd.object.RouterName(), rd.object.RouterType()); err != nil {
