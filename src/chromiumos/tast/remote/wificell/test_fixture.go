...
func NewTestFixture(fullCtx, daemonCtx context.Context, d *dut.DUT, rpcHint *testing.RPCHint, ops ...TFOption) (ret *TestFixture, retErr error) {
	...
	// Wificell precondiotion always supplant us with router name, but we need
	// to handle case when the fixture is created from outside precondition.
	if len(tf.routers) == 0 {
	...
	for _, router := range tf.routers {
		...
		// If hostnames are supplied manually, it is responsibility of the person running
		// the test to make sure devices with different names are unique.
