...
func (tf *TestFixture) Close(ctx context.Context) error {
	...
	if err := tf.resetNetCertStore(ctx); err != nil {
		util.CollectFirstErr(ctx, &firstErr, errors.Wrap(err, "failed to reset the NetCertStore"))
	}
	...
	if tf.pcap != nil {
		if err := tf.pcap.Close(ctx); err != nil {
			util.CollectFirstErr(ctx, &firstErr, errors.Wrap(err, "failed to close pcap"))
		}
		if err := tf.pcapHost.Close(ctx); err != nil {
			util.CollectFirstErr(ctx, &firstErr, errors.Wrap(err, "failed to close pcap ssh"))
		}
		tf.pcap = nil
	}
	...
	for i := range tf.routers {
		...
		if router.object != nil {
			if err := router.object.Close(ctx); err != nil {
				util.CollectFirstErr(ctx, &firstErr, errors.Wrapf(err, "failed to close rotuer %s", router.target))
			}
		}
		router.object = nil
		if router.host != nil {
			if err := router.host.Close(ctx); err != nil {
				util.CollectFirstErr(ctx, &firstErr, errors.Wrapf(err, "failed to close router %s ssh", router.target))
			}
		}
		router.host = nil
	}
	if tf.wifiClient != nil {
		if tf.setLogging {
			if err := tf.setLoggingConfig(ctx, tf.originalLogLevel, tf.originalLogTags); err != nil {
				util.CollectFirstErr(ctx, &firstErr, errors.Wrap(err, "failed to tear down test state"))
			}
		}
...
func (tf *TestFixture) DeconfigAP(ctx context.Context, ap *APIface) error {
	...
	if err := ap.Stop(ctx); err != nil {
		util.CollectFirstErr(ctx, &firstErr, errors.Wrap(err, "failed to stop APIface"))
	}
	if capturer != nil {
...
func (tf *TestFixture) DeconfigAllAPs(ctx context.Context) error {
	...
	for ap := range tf.aps {
		if err := tf.DeconfigAP(ctx, ap); err != nil {
			util.CollectFirstErr(ctx, &firstErr, errors.Wrap(err, "failed to deconfig AP"))
		}
	}
	return firstErr
}
...
func (tf *TestFixture) GetLegacyRouter() router.Legacy {
	return tf.RouterByID(0).(router.Legacy)
}
...
func (tf *TestFixture) GetLegacyPcap() router.Legacy {
	return tf.pcap.(router.Legacy)
}
