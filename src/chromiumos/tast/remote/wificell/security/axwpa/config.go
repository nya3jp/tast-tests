...
func (f *AxConfigFactory) Gen() (security.AxConfig, error) {
	...
	if f.cipher == router.AES {
		routerConfigParams = append(routerConfigParams, router.AxRouterConfigParam{f.band, router.KeyCrypto, "aes"})
	} else {
		routerConfigParams = append(routerConfigParams, router.AxRouterConfigParam{f.band, router.KeyCrypto, "tkip+aes"})
	}
