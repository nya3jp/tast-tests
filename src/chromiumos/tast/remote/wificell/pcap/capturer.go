...
func (c *Capturer) start(ctx context.Context) (err error) {
	...
	runner = remotetcpdump.NewRemoteRunner(c.host)
	if c.snaplen != 0 {
		runner.Snaplen(c.snaplen)
	}
	_, err = runner.StartTcpdump(ctx, c.iface, c.packetPathOnRemote(), c.stdoutFile, c.stderrFile)
...
func (c *Capturer) ReserveForClose(ctx context.Context) (context.Context, context.CancelFunc) {
	return runner.ReserveForClose(ctx)
}
