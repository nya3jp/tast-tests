...
func (h *HelperRemote) EnsureTPMIsReset(ctx context.Context) error {
	...
	if err := WaitForTPMReady(ctx, h.ti); err != nil {
		return errors.Wrap(err, "cryptohome not ready")
	}

	return nil
}

// WaitForTPMReady waits until cryptohome is ready.
...
func WaitForTPMReady(ctx context.Context, utility hwsec.TPMInitializer) error {
...
func EnsureOwnerIsReset(ctx context.Context, d *dut.DUT) error {
	...
	if err := WaitForTPMReady(ctx, utility); err != nil {
		return errors.Wrap(err, "cryptohome not ready")
	}
