...
// FullHelperRemote is the full version of all kinds of helper
type FullHelperRemote struct {
	CmdHelperRemote
	AttestationHelperRemote
}
...
func NewHelper(r hwsec.CmdRunner, d *dut.DUT) (*CmdHelperRemote, error) {
	...
	return &CmdHelperRemote{*helper, d}, nil
}
...
func NewFullHelper(r hwsec.CmdRunner, d *dut.DUT, h *testing.RPCHint) (*FullHelperRemote, error) {
	cmdHelperRemote, err := NewHelper(r, d)
	if err != nil {
		return nil, errors.Wrap(err, "failed to create CmdHelperRemote")
	}
	attestationHelperRemote, err := NewAttestationHelper(d, h)
	if err != nil {
		return nil, errors.Wrap(err, "failed to create AttestationRemote")
	}
	return &FullHelperRemote{*cmdHelperRemote, *attestationHelperRemote}, nil
}
...
func (h *CmdHelperRemote) ensureTPMIsReset(ctx context.Context, removeFiles bool) error {
...
func (h *CmdHelperRemote) Reboot(ctx context.Context) error {
...
// EnsureTPMIsReset ensures the TPM is reset when the function returns nil.
// Otherwise, returns any encountered error.
func (h *CmdHelperRemote) EnsureTPMIsReset(ctx context.Context) error {
	return h.ensureTPMIsReset(ctx, false)
}

// EnsureTPMIsResetAndPowerwash ensures the TPM is reset and simulates a Powerwash.
func (h *CmdHelperRemote) EnsureTPMIsResetAndPowerwash(ctx context.Context) error {
