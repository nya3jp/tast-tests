...
func ArcSnapshot(ctx context.Context, s *testing.State) {
	...
	accHelper, acc, err := tape.NewOwnedTestAccountManagerFromClient(ctx, tapeClient, false, tape.WithTimeout(int32(arcSnapshotTestTimeout.Seconds())), tape.WithPoolID(tape.ArcSnapshot))
	if err != nil {
