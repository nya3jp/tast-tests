...
func ArcSnapshot(ctx context.Context, s *testing.State) {
	...
	accManager, acc, err := tape.NewOwnedTestAccountManagerFromClient(ctx, tapeClient, false, tape.WithTimeout(int32(arcSnapshotTestTimeout.Seconds())), tape.WithPoolID(tape.ArcSnapshot))
	if err != nil {
