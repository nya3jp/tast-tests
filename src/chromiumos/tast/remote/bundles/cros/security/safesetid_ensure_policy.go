...
func SafesetidEnsurePolicy(ctx context.Context, s *testing.State) {
	...
	*/
	...
	for _, tc := range []struct {
		...
		{"ipsec", "root", true, false},
	} {
		err := transitionSetUID(tc.parent, tc.child, tc.capSETUID, tc.expectSuccess)
		if err != nil {
			s.Logf(" %v unable to setuid to %v", tc.parent, tc.child)
			s.Fatalf("%v", err)
		}
		//Need to check for kernel version 5.8, as safesetid relies on functionality from that version
		if checkKernelVersion(ver, 5, 8) {
...
func transitionSetUID(parent, child string, giveCapSetUID, expectSuccess bool) error {
	var caps string
	if giveCapSetUID {
		caps = "0xc0"
	} else {
...
func checkUsersExist() error {
	...
	userslist := []user.User{
		...
		{Name: "ipsec", Uid: "212", Gid: "212", HomeDir: "/dev/null"},
		{Name: "openvpn", Uid: "217", Gid: "217", HomeDir: "/dev/null"},
	...
	for _, val := range userslist {
		...
		if err != nil {
			...
			newerr := createUser(val)
			if newerr != nil {
				return errors.Wrap(newerr, val.Name)
			}
			//}
		}
	}
...
func listExistingUsers(s *testing.State) {
	// See which UID's are available for debug output
...
func createUser(newuser user.User) error {
	cmd := exec.Command("/usr/sbin/useradd", "-u", newuser.Uid, "-d", newuser.HomeDir, newuser.Name)
	...
	if err != nil {
		return errors.Wrapf(err, "createUser: %v", stderr.String())
	}
	return nil
}
