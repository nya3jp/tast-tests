...
func login(ctx context.Context, s *testing.State, cl *rpc.Client) (*empty.Empty, error) {
	cr := example.NewChromeServiceClient(cl.Conn)
	if _, err := cr.New(ctx, &empty.Empty{}); err != nil {
		s.Fatal("Failed to start Chrome: ", err)
		return nil, err
	}
	return cr.Close(ctx, &empty.Empty{})
}
...
func BootLockbox(ctx context.Context, s *testing.State) {
	...
	// this makes boot lockbox readonly
	if _, err := login(ctx, s, cl); err != nil {
