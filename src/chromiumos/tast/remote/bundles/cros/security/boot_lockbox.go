...
func login(ctx context.Context, s *testing.State, client security.BootLockboxServiceClient) (*empty.Empty, error) {
	if _, err := client.New(ctx, &empty.Empty{}); err != nil {
		s.Fatal("Failed to start Chrome: ", err)
		return nil, err
	}
	return client.Close(ctx, &empty.Empty{})
}
...
func BootLockbox(ctx context.Context, s *testing.State) {
	...
	// This makes boot lockbox readonly as it starts a new chrome instance remotely
	if _, err := login(ctx, s, client); err != nil {
		s.Error("Unable to login to chrome: ", err)
	}
