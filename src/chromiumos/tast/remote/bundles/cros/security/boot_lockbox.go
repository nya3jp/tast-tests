...
func restoreTestKeyValue(ctx context.Context, s *testing.State, client security.BootLockboxServiceClient) {
	s.Log("Trying to restore the Test Key value to 0")

	nullByteValue := []byte("0")
	_, err := client.Store(ctx, &security.StoreBootLockboxRequest{Key: testKey, Value: nullByteValue})
	if err != nil {
		s.Error("Failed to store to boot lockbox: ", err)
	}
	response, err := client.Read(ctx, &security.ReadBootLockboxRequest{Key: testKey})
	...
	if bytes.Compare(response.Value, nullByteValue) != 0 {
		s.Error("Expected retrieved value should be 0, but got = ", hex.EncodeToString(response.Value))
	}
	s.Log("Test Key Value successfully restored to 0")
}

func rebootAndReconnect(ctx context.Context, s *testing.State) *rpc.Client {
...
func BootLockbox(ctx context.Context, s *testing.State) {
	...
	client = security.NewBootLockboxServiceClient(cl.Conn)
	restoreTestKeyValue(ctx, s, client)
}
