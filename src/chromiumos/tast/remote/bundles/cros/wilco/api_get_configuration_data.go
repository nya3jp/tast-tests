...
func APIGetConfigurationData(ctx context.Context, s *testing.State) {
	if err := hwsec.EnsureTPMIsReset(ctx, s.DUT()); err != nil {
	...
	res, err := pc.StartURLPolicyServer(ctx, &ps.StartURLPolicyServerRequest{
		Contents: []byte(configurationData),
	})
	if err != nil {
		s.Fatal("Failed to start a URLPolicyServer: ", err)
	}
	defer pc.StopURLPolicyServer(ctx, &ps.StopURLPolicyServerRequest{
		Url: res.Url,
	})
