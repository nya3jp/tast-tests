...
func APIGetConfigurationData(ctx context.Context, s *testing.State) {
	...
	const newConfigurationData = `{"testb": 2}`
	...
	if _, err := pc.EnrollUsingChrome(ctx, &ps.EnrollUsingChromeRequest{
		PolicyJson: createPolicyBlob(configurationData),
	}); err != nil {
	...
	if _, err := pc.UpdatePolicies(ctx, &ps.UpdatePoliciesRequest{
		PolicyJson: createPolicyBlob(newConfigurationData),
	}); err != nil {
	...
	if data.JsonConfigurationData != newConfigurationData {
		s.Errorf("Unexpected policy value: got %s, want %s", data.JsonConfigurationData, newConfigurationData)
	}
