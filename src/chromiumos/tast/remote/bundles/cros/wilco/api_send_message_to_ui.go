...
func APISendMessageToUI(ctx context.Context, s *testing.State) { // NOLINT
	...
	extensionMsg := testMsg{
		Test: 8,
	}

	if err := nm.AddReply(ctx, &extensionMsg); err != nil {
	...
	var replyMsg testMsg
	if err := json.Unmarshal([]byte(reply.ResponseJsonMessage), &replyMsg); err != nil {
		s.Fatal("Failed to unamrshal message: ", err)
	}

	if extensionMsg != replyMsg {
		s.Errorf("Unexpected reply received: got %v; want %v", replyMsg, extensionMsg)
	}
	...
	if sendMsg != recvMsg {
		s.Errorf("Unexpected request received: got %v; want %v", recvMsg, sendMsg)
	}
}
