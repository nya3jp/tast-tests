...
func APIHandleMessageFromUI(ctx context.Context, s *testing.State) { // NOLINT
	...
	if err := nm.SendMessageAndGetReply(sendMessageCtx, &uiMessage, &uiResponse); err != nil {
	...
	if uiMessage != vmRequest {
		s.Errorf("Unexpected message received: got %v; want %v", vmRequest, uiMessage)
	}
}
