...
func DrallionTabletPower(ctx context.Context, s *testing.State) {
	...
	initLidAngle := re.FindAllString(string(out), -1)
	s.Logf("Initial settings: lid_angle=%s hys=%s", initLidAngle[0], initLidAngle[1])
	...
	// Use EC tool to set tablet_mode_angle to 0 degrees (Device is always in tablet mode).
	...
	// Use EC tool to hold down power button for < 1.5 sec
	...
	_, err = powerMenuService.CloseChrome(ctx, &empty.Empty{})
	if err != nil {
		s.Log("Failed to close chrome: ", err)
	}

	// Use EC tool to hold down power button for > 8 sec
	...
	// Restore tablet_mode_angle setting.
	out, err = d.Command("ectool", "--name=cros_ish", "motionsense", "tablet_mode_angle", initLidAngle[0], initLidAngle[1]).Output(ctx)
	if err != nil {
		s.Fatal("Failed to restore tablet_mode_angle settings: ", err)
	}
	s.Logf("Reset initial tablet mode settings: %s", out)
}
