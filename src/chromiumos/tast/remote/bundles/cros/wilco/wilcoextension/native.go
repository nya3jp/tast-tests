...
func (n *NativeMessaging) WaitForMessage(ctx context.Context, message interface{}) error {
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			return testing.PollBreak(errors.Wrap(err, "failed to get number of messages"))
		}

		var length int
		if err := json.Unmarshal(res.Result, &length); err != nil {
			return testing.PollBreak(errors.Wrap(err, "failed to unmarshal message"))
		}

		if length == 0 {
			return errors.New("no messages present")
		}
