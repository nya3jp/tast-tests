...
func init() {
	testing.AddTest(&testing.Test{
		...
		ServiceDeps:  []string{"tast.cros.power.EthernetService"},
		Fixture:      fixture.NormalMode,
		Vars:         []string{"power.iteration"},
...
func UsbEthernetBehaviourColdreboot(ctx context.Context, s *testing.State) {
	...
	wantType := s.Param().(*regexp.Regexp)

	if err := checkUSBType(ctx, wantType); err != nil {
	...
	iteration := 1
	if val, ok := s.Var("power.iteration"); ok {
		i, err := strconv.Atoi(val)
		if err != nil {
			s.Fatal("Failed to convert var to integer: ", err)
		}
		iteration = i
	}
	...
	browseOverEthernet := func() {
		...
		client := power.NewEthernetServiceClient(cl.Conn)
		...
		request := power.WifiRequest{Enabled: false}
		if _, err := client.SetWifi(ctx, &request); err != nil {
			s.Fatal("Failed to disable Wi-Fi: ", err)
		}

		urlrequest := power.BrowseRequest{Url: "https://www.google.com"}
...
// checkUSBType checks whether usb is 3.0 or 2.0.
