...
func init() {
	testing.AddTest(&testing.Test{
		Func:         DPColdboot,
		Desc:         "Verifies DP native port functionality with cold boot operation",
...
func DPColdboot(ctx context.Context, s *testing.State) {
	...
	defer func(ctx context.Context) {
		if !dut.Connected(ctx) {
			if err := wakeOnDut(ctx, pxy, dut); err != nil {
	...
	loginChrome := func(ctx context.Context) {
		...
		if err := externalDisplayDetection(ctx, dut, 1); err != nil {
	...
	for i := 1; i <= iter; i++ {
		...
		if err := performColdboot(ctx, dut); err != nil {
			s.Fatal("Failed to power off DUT")
		}
		...
		if count != 5 {
			s.Fatal("Previous sleep state must be 5")
		}
	}
}

func externalDisplayDetection(ctx context.Context, dut *dut.DUT, numberOfDisplays int) error {
	...
	var (
		...
		connectedRe     = regexp.MustCompile(`\[CONNECTOR:\d+:DP.*status: connected`)
...
func performColdboot(ctx context.Context, dut *dut.DUT) error {
	testing.ContextLog(ctx, "Performing coldboot")
...
func wakeOnDut(ctx context.Context, pxy *servo.Proxy, dut *dut.DUT) error {
