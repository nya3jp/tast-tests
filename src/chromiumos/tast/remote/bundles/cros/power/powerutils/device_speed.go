...
func ListDevicesInfo(ctx context.Context, dut *dut.DUT) ([]USB, error) {
...
// USBDeviceSpeed returns speed for all USB devices with given devClassName.
// If failed to get devices speed returns error.
func USBDeviceSpeed(ctx context.Context, dut *dut.DUT, devClassName string, numberOfConnectedDevice int) ([]string, error) {
	res, err := ListDevicesInfo(ctx, dut)
	if err != nil {
	...
	want := numberOfConnectedDevice
	if got := len(speedSlice); want != got {
		return nil, errors.Errorf("unexpected number of connected devices = got %d, want %d", got, want)
	}
	return speedSlice, nil
}

// USBDeviceDetection verifies USB device is connected or not.
func USBDeviceDetection(ctx context.Context, dut *dut.DUT, usbDeviceClassName, usbSpeed string, numberOfConnectedDevice int) error {
	speedOut, err := USBDeviceSpeed(ctx, dut, usbDeviceClassName, numberOfConnectedDevice)
	if err != nil {
		return errors.Wrap(err, "failed to check for USB speed")
	}
	speedFound := false
	...
	if !speedFound {
		return errors.Errorf("unexpected USB device speed: want %q, got %v", usbSpeed, speedOut)
	}
	return nil
}
