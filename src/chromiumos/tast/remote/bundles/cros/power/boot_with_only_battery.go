...
func verifyEctoolBattery(ctx context.Context, dut *dut.DUT) error {
	...
	if !dischargeFlagRe.MatchString(string(out)) {
		return errors.New("unexpected battery flag: want discharging, got charging")
	}
	return nil
}
...
func verifyPowerSupplyInfo(ctx context.Context, dut *dut.DUT) error {
	...
	if !dischargeStateRe.MatchString(string(out)) {
		return errors.New("unexpected power_supply_info state: want discharging, got charging")
	}
	return nil
}
...
func verifyEctoolChargeState(ctx context.Context, dut *dut.DUT) error {
	...
	if !batteryACPowerRe.MatchString(string(out)) {
		return errors.New("unexpected AC flag in chargestate info: want 0, got 1")
	}
	return nil
}
...
func verifyECPowerinfo(ctx context.Context, h *firmware.Helper) error {
	...
	if want := "S0"; got != want {
		return errors.Errorf("unexpected EC power state: want %s, got %s", want, got)
	}
	return nil
}
...
func verifyECBattery(ctx context.Context, h *firmware.Helper) error {
	...
	if !strings.Contains(actualOut, expectedOut) {
		return errors.Errorf("unexpected EC battery info: want %s; got %s", expectedOut, actualOut)
	}
	return nil
}
