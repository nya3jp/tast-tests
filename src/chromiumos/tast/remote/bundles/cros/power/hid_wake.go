...
func HidWake(ctx context.Context, s *testing.State) {
	...
	go attemptSuspendAndWake(ctx, d, firstSuspendDuration, otherWakeSourceRegex, suspendErr)
	...
	// Wait for the first suspend to complete
	if err := <-suspendErr; err != nil {
		s.Fatal("Error during first suspend: ", err)
	}
	...
	// Suspend the DUT
	go attemptSuspendAndWake(ctx, d, secondSuspendDuration, inputWakeSourceRegex, suspendErr)
	...
	// Wait for the second suspend to complete
	if err := <-suspendErr; err != nil {
		s.Fatal("Error during second suspend: ", err)
	}
...
// attemptSuspendAndWake suspends the DUT and checks the output of powerd_dbus_suspend for a particular wake source.
// Errors thrown in attemptSuspendAndWake or caused by a regex mismatch are returned over the suspendErr channel.
func attemptSuspendAndWake(ctx context.Context, d *dut.DUT, suspendDuration int, wakeSourceRegex string, suspendErr chan error) {
