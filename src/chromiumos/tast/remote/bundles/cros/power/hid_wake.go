...
func HidWake(ctx context.Context, s *testing.State) {
	...
	if err := setServoKeyboardRemoteWakeup(ctx, d, s, dir, "disabled\n"); err != nil {
	...
	if err := setServoKeyboardRemoteWakeup(ctx, d, s, dir, "enabled\n"); err != nil {
		s.Fatal("Failed to disable servo keyboard wake capability: ", err)
	}
	...
	// Turn off the servo keyboard emulator
...
func getServoKeyboardWakeCount(ctx context.Context, d *dut.DUT, s *testing.State, servoKeyboardDir string) (int, error) {
	cl, err := rpc.Dial(ctx, d, s.RPCHint())
	if err != nil {
...
func setServoKeyboardRemoteWakeup(ctx context.Context, d *dut.DUT, s *testing.State, servoKeyboardDir, remoteWake string) error {
	cl, err := rpc.Dial(ctx, d, s.RPCHint())
	if err != nil {
		return errors.Wrap(err, "failed to connect to the RPC service on the DUT")
	}
	fs := dutfs.NewClient(cl.Conn)
	if err := fs.WriteFile(ctx, servoKeyboardDir+"/power/wakeup", []byte(remoteWake), 0644); err != nil {
