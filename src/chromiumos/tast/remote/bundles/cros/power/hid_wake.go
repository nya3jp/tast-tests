...
func HidWake(ctx context.Context, s *testing.State) {
	...
	go func(ctx context.Context) {
		...
		if match, err := regexp.MatchString(`Wakeup\s+type\:\s+other`, string(out)); err != nil {
			s.Fatal("Unable to check wakeup source for first suspend: ", err)
		} else if !match {
			s.Fatal("Incorrect wakeup source for first suspend")
		}
	}(ctx)
	...
	count1, err := getServoKeyboardWakeCount(ctx, d, dir)
	if err != nil {
	...
	count2, err := getServoKeyboardWakeCount(ctx, d, dir)
	if err != nil {
		s.Fatal("Failed to get wake count for servo keyboard emulator: ", err)
	}
...
func getServoKeyboardDir(ctx context.Context, d *dut.DUT, s *testing.State) (string, error) {
	...
	for _, device := range usbDevices {
		vid, err := d.Conn().CommandContext(ctx, "cat", usbDeviceDir+device.Name()+"/idVendor").Output()
		if err != nil {
			continue
		}
		...
		pid, err := d.Conn().CommandContext(ctx, "cat", usbDeviceDir+device.Name()+"/idProduct").Output()
		if err != nil {
			continue
		}
...
func getServoKeyboardWakeCount(ctx context.Context, d *dut.DUT, servoKeyboardDir string) (int64, error) {
	out, err := d.Conn().CommandContext(ctx, "cat", servoKeyboardDir+"/power/wakeup_count").Output()
	if err != nil {
