...
func USBDeviceFunctionality(ctx context.Context, s *testing.State) {
	...
	for i := 1; i <= iter; i++ {
		...
		// Check for USB device(s) detection before coldboot.
		usbDevicesList, err := powerutils.ListDevicesInfo(ctx, dut)
		if err != nil {
			s.Fatal("Failed to get USB devices list: ", err)
		}

		got := powerutils.NumberOfUSBDevicesConnected(ctx, dut, usbDevicesList, testParam.usbDeviceClassName, testParam.usbSpeed)
		if want := testParam.noOfConnectedDevice; got != want {
			s.Fatalf("Unexpected number of USB devices connected: got %d, want %d", got, want)
		}

		// Check USB storage device is shown in filesapp before coldboot.
		...
		// Performing chrome login after powering on DUT from coldboot.
		cl, err = loginChrome()
		if err != nil {
			s.Fatal("Failed to login Chrome: ", err)
		}

		// Check for USB device(s) detection after coldboot.
		usbDevicesList, err = powerutils.ListDevicesInfo(ctx, dut)
		if err != nil {
			s.Fatal("Failed to get USB devices list: ", err)
		}
		...
		got = powerutils.NumberOfUSBDevicesConnected(ctx, dut, usbDevicesList, testParam.usbDeviceClassName, testParam.usbSpeed)
		if want := testParam.noOfConnectedDevice; got != want {
			s.Fatalf("Unexpected number of USB devices connected after coldboot: got %d, want %d", got, want)
		}

		// Check USB storage device is shown in filesapp after coldboot.
		if testParam.usbDeviceClassName == "Mass Storage" {
			audioService := ui.NewAudioServiceClient(cl.Conn)
			dirName := &ui.AudioServiceRequest{DirectoryName: usbStorageName, FileName: ""}
			if _, err := audioService.OpenDirectoryAndFile(ctx, dirName); err != nil {
				s.Fatalf("Failed to open %q directory after coldboot: %v", usbStorageName, err)
			}
		}
