...
func init() {
	testing.AddTest(&testing.Test{
		Func:         HDMIColdboot,
		Desc:         "Verifies HDMI native port functionality with cold boot operation",
...
func HDMIColdboot(ctx context.Context, s *testing.State) {
	...
	loginChrome := func(ctx context.Context) {
		...
		if err := externalMonitorDetection(ctx, dut, 1); err != nil {
	...
	for i := 1; i <= iter; i++ {
		...
		if err := performsShutdown(ctx, dut); err != nil {
			s.Fatal("Failed to perform coldboot: ", err)
		}
		if err := eCStateCheck(ctx, pxy); err != nil {
...
func externalMonitorDetection(ctx context.Context, dut *dut.DUT, numberOfDisplays int) error {
...
func performsShutdown(ctx context.Context, dut *dut.DUT) error {
...
func eCStateCheck(ctx context.Context, pxy *servo.Proxy) error {
	ecStateToCheck := "S5"
	if err := testing.Poll(ctx, func(ctx context.Context) error {
