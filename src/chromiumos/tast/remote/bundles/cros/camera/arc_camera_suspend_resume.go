...
func ArcCameraSuspendResume(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Failed to connect to the RPC service: ", err)
	}
	defer cl.Close(ctx)
	...
	// TODO(lnishan): Iterations
	if _, err := sr.ClearResult(ctx, &empty.Empty{}); err != nil {
		s.Fatal("Failed to clear result: ", err)
	}

	if err := s.DUT().Suspend(ctx); err != nil {
	...
	s.Log(cl.Conn.GetState()) // This line outputs ready.
	sr = pb.NewSuspendResumeClient(cl.Conn)

	res, err := sr.GetResult(ctx, &empty.Empty{})
	if err != nil {
		s.Fatal("Failed to get result: ", err)
	}
	s.Logf("Result: %s", res.GetResult())

	resLog, err := sr.GetResultLog(ctx, &empty.Empty{})
	if err != nil {
		s.Fatal("Failed to get result log: ", err)
	}
	s.Logf("Result log: %s", resLog.GetResultLog())
	// TODO(lnishan): End iterations

	if _, err := sr.TearDown(ctx, &empty.Empty{}); err != nil {
		s.Fatal("Failed to tear down the environment: ", err)
	}
