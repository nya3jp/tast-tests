...
func calibrateDisplayLevel(ctx context.Context, ch *chart.Helper, d *dut.DUT, facing pb.Facing, outDir string, targetLumaMin, targetLumaMax int64) (float32, error) {
	...
	for {
		...
		// Sill try to test ROI for display level 100.
		if luma >= targetLumaMin && luma <= targetLumaMax || displayLevel == 100 {
		...
		if luma > targetLumaMax {
			// try low display level between 0~adjustStep
			if displayLevel <= adjustStep {
				displayLevel -= adjustStep / 5
			} else {
				displayLevel -= adjustStep
			}
		}
		if _, ok := seen[displayLevel]; ok {
			return 0.0, errors.Errorf("tested same display level %v twice; all tested: %v", displayLevel, seen)
		}
		seen[displayLevel] = struct{}{}
	}
...
func FaceAutoExposure(ctx context.Context, s *testing.State) {
	...
	luma, err := getFaceLumaValue(ctx, d, facing, true, outDir, displayLevel)
	if err != nil {
