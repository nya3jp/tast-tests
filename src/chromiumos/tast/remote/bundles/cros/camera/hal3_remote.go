...
func HAL3Remote(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Error("Remote call RunTest() failed: ", err)
	}

	// Check test result.
	if response == nil {
		s.Error("Remote call RunTest() on DUT return nil response")
		return
	}
	...
	// Collect logs.
	if err := linuxssh.GetFile(cleanupCtx, d.Conn(), response.OutPath, s.OutDir()); err != nil {
	...
	if err := d.Conn().Command("rm", "-r", response.OutPath).Start(cleanupCtx); err != nil {
		s.Fatalf("Failed to cleanup remote output directory %q from DUT: %v", response.OutPath, err)
	}
}
