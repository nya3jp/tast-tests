	...
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{
			...
			testing.Param{
				...
				ExtraData:         append([]string{pre.CtsVerifierX86.DataPath()}, pre.CtsVerifierX86.PatchesDataPaths()...),
...
func ITS(ctx context.Context, s *testing.State) {
	...
	if _, err := os.Stat(chartPath); err != nil {
		if os.IsNotExist(err) {
			testing.ContextLogf(ctx, "No chart %v, skipping display chart for scene%d", chartPath, param.Scene)
		} else {
