...
func cleanup(ctx context.Context, conn *ssh.Conn, dir string, pid string, outDir string) error {
	cleanupDisplayProcess := func() (retErr error) {
		if len(pid) == 0 {
			return nil
		}
		defer func() {
	...
	for _, cleanup := range [](func() error){
		...
		if err := cleanup(); err != nil {
			if firstErr == nil {
				firstErr = errors.Wrapf(err, "failed to run cleanup process")
			} else {
