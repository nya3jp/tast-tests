...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:         []string{"preview.html", "preview.css", "preview.js", "align.svg"},
...
func CameraboxAlign(ctx context.Context, s *testing.State) {
	...
	if _, err := linuxssh.PutFiles(
		ctx, d.Conn(), map[string]string{
			s.DataPath("preview.html"): filepath.Join(dataPath, "preview.html"),
			s.DataPath("preview.css"):  filepath.Join(dataPath, "preview.css"),
			s.DataPath("preview.js"):   filepath.Join(dataPath, "preview.js"),
		},
		linuxssh.DereferenceSymlinks); err != nil {
		s.Fatalf("Failed to send data to remote data path %v: %v", dataPath, err)
	}
	defer func() {
		if err := linuxssh.GetFile(ctx, d.Conn(),
			filepath.Join(dataPath, "jslog.txt"),
			filepath.Join(s.OutDir(), "jslog.txt"),
		); err != nil {
			s.Error("Failed to get remote jslog.txt: ", err)
		}
		d.Conn().Command("rm", "-r", dataPath).Output(ctx)
	}()

	acl := pb.NewAlignmentServiceClient(cl.Conn)
	if _, err := acl.ManualAlign(ctx, &pb.ManualAlignRequest{DataPath: dataPath, Username: user, Password: pass, Facing: facing}); err != nil {
