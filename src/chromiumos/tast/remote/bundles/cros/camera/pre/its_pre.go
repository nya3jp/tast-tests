...
const CTSVerifier = "android-cts-verifier-9.0_r13-linux_x86-x86.zip"
...
// ITSPre is the test precondition to run Android ITS test.
var ITSPre = &ITSPreImpl{}

func (p *ITSPreImpl) String() string         { return "its_precondition" }
...
func (p *ITSPreImpl) Prepare(ctx context.Context, s *testing.PreState) interface{} {
	...
	// Unpack CTSVerifier.
	if err := testexec.CommandContext(ctx, "unzip", s.DataPath(CTSVerifier), "-d", p.dir).Run(testexec.DumpLogOnError); err != nil {
	...
	if pyPath, err := binPath(ctx, "python"); err != nil {
		...
		s.Fatal("Failed to hijack python path from %v to %v", pyPath, tempPyPath)
	}
...
func (p *ITSPreImpl) TestCmd(ctx context.Context, scene int, camera int) *testexec.Cmd {
	...
	python %s device=%s scenes=%d camera=%d`,
		p.itsRoot(), setupPath, scriptPath, p.hostname, scene, camera)
...
func (p *ITSPreImpl) Chart(scene int) string {
	s := "scene" + strconv.Itoa(scene)
	return path.Join(p.itsRoot(), "tests", s, s+".pdf")
}

// CameraId returns corresponding camera id of camera facing on DUT.
func (p *ITSPreImpl) CameraId(ctx context.Context, facing pb.Facing) (int, error) {
	out, err := p.adb.Command(ctx, "shell", "pm", "list", "features").Output(testexec.DumpLogOnError)
	if err != nil {
