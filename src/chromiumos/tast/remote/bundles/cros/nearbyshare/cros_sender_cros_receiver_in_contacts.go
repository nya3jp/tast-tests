...
func CrosSenderCrosReceiverInContacts(ctx context.Context, s *testing.State) {
	...
	sender, err := enableNearbyShareGAIA(ctx, s, cl1, senderDisplayName, s.RequiredVar("nearbyshare.cros_username"), s.RequiredVar("nearbyshare.cros_password"))
	if err != nil {
	...
	receiver, err := enableNearbyShareGAIA(ctx, s, cl2, receiverDisplayName, s.RequiredVar("nearbyshare.android_username"), s.RequiredVar("nearbyshare.android_password"))
	if err != nil {
	...
	if err != nil {
		s.Log("Failed to select share target on DUT1 (Sender). Possibly due to contact certs not synced. Trying again")
		_, err = sender.StartSend(ctx, sendReq)
		if err != nil {
			s.Fatal("Failed to start send on DUT1 (Sender): ", err)
		}
