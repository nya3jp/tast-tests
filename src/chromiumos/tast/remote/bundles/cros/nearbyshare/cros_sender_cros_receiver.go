...
func CrosSenderCrosReceiver(ctx context.Context, s *testing.State) {
	...
	defer d2.Conn().Command("rm", "-r", filepath.Join(filesapp.DownloadPath, "*")).Run(ctx)
	...
	senderShareToken, err := ns1.SelectShareTarget(ctx, targetReq)
	if err != nil {
	...
	receiverShareToken, err := ns2.WaitForSenderAndAcceptShare(ctx, receiveReq)
	if err != nil {
