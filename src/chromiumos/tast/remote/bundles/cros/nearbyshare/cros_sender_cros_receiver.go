...
func CrosSenderCrosReceiver(ctx context.Context, s *testing.State) {
	...
	fileReq := &nearbyservice.CrOSSendFilesRequest{FileName: remoteFilePath}
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		dut1FileRes, err := ns1.GetFilesHashes(ctx, dut1FileReq)
		if err != nil {
			return errors.Wrap(err, "failed to get file hashes on DUT1")
		}
		dut2FileReq := &nearbyservice.CrOSFileHashRequest{FileNames: file_names.FileNames, FileDir: filesapp.DownloadPath}
		dut2FileRes, err := ns2.GetFilesHashes(ctx, dut2FileReq)
		if err != nil {
			return errors.Wrap(err, "failed to get file hashes on DUT2")
		}
		if len(dut1_file_res.Hashes) != len(dut2FileRes.Hashes) {
			return errors.Wrap(err, "length of hashes don't match")
		}
		for i := range dut1_file_res.Hashes {
			if dut1_file_res.Hashes[i] != dut2FileRes.Hashes[i] {
