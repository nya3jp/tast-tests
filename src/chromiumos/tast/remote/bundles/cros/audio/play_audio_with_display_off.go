...
func PlayAudioWithDisplayOff(ctx context.Context, s *testing.State) {
	...
	performTest := func() {
		...
		if curBrightness != 0 {
			s.Fatal("Failed to go to idle state: ", err)
		}
		...
		if curBrightness == 0 {
			s.Fatal("Failed: DUT display still in idle state: ", err)
		}
...
func setSystemBrightness(ctx context.Context, dut *dut.DUT, brightness int) error {
	return dut.Conn().CommandContext(ctx, "backlight_tool", fmt.Sprintf("--set_brightness=%d", brightness)).Run()
}
