...
func LaunchFeedbackFromPowerButton(ctx context.Context, s *testing.State) {
	...
	if _, err := uiautoSvc.WaitUntilExists(ctx, &ui.WaitUntilExistsRequest{Finder: feedbackButtonFinder}); err != nil {
		s.Fatal("Failed to find feedback button on DUT UI: ", err)
	}
	if _, err := uiautoSvc.LeftClick(ctx, &ui.LeftClickRequest{Finder: feedbackButtonFinder}); err != nil {
	...
	if _, err := uiautoSvc.WaitUntilExists(ctx, &ui.WaitUntilExistsRequest{Finder: issueDescriptionInputFinder}); err != nil {
	...
	for i := 0; i < 5; i++ {
		...
		if _, err := uiautoSvc.WaitUntilExists(ctx, &ui.WaitUntilExistsRequest{Finder: helpLinkFinder}); err != nil {
