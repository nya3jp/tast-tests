...
func SDCardPlugUnplugDuringSleep(ctx context.Context, s *testing.State) {
	...
	defer func(ctx context.Context) {
		...
		if err := plugUnplugSDCardViaServo(ctx, pxy, false); err != nil {
	...
	if err := plugUnplugSDCardViaServo(ctx, pxy, true); err != nil {
	...
	isPlug := s.Param().(bool)
	s.Log("Plugging/Unplugging microSD after suspend")
	if err := plugUnplugSDCardViaServo(ctx, pxy, isPlug); err != nil {
		s.Fatal("Failed to plug/unplug microSD storage device to DUT: ", err)
	}
...
// plugUnplugSDCardViaServo perform microSD plug/unplug via servo.
func plugUnplugSDCardViaServo(ctx context.Context, pxy *servo.Proxy, isPlug bool) error {
	if isPlug {
