...
func DUTBehaviorOnECCommands(ctx context.Context, s *testing.State) {
	...
	dutSupportsEC := isECSupported(ctx, dut)
...
// isECSupported returns EC supported status of DUT.
func isECSupported(ctx context.Context, dut *dut.DUT) bool {
...
func performServoPowerLongPress(ctx context.Context, h *firmware.Helper, dut *dut.DUT) error {
	...
	defer cancel()
	if err := h.Servo.KeypressWithDuration(powerOffCtx, servo.PowerKey, servo.DurLongPress); err != nil && !errors.Is(err, context.DeadlineExceeded) {
