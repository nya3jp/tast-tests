...
func RecModeTime(ctx context.Context, s *testing.State) {
	...
	fwBootTime, err := getFirmwareTimestampBootTime(ctx, h)
	if err != nil {
		s.Fatal("Failed to read firmware boot time: ", err)
	}

	var expectedBootTime float64 = 5
	if fwBootTime > expectedBootTime {
		s.Fatalf("Failed to verify firmware boot time, got: %f, want: %f", fwBootTime, expectedBootTime)
	}

}

// getFirmwareTimestampBootTime reads firmware startup time from /tmp/firmware-boot-time.
func getFirmwareTimestampBootTime(ctx context.Context, h *firmware.Helper) (float64, error) {
