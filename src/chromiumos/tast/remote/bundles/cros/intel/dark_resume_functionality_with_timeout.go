...
func DarkResumeFunctionalityWithTimeout(ctx context.Context, s *testing.State) {
	...
	if err := performDarkResumeSuspend(ctx, h, dut, !isPowerPress); err != nil {
...
func performDarkResumeSuspend(ctx context.Context, h *firmware.Helper, dut *dut.DUT, isPowerPress bool) error {
	...
	if !isPowerPress {
		...
		if err := wakeDUTWithEnterKeyPress(ctx, h, dut); err != nil {
...
func wakeDUTWithEnterKeyPress(ctx context.Context, h *firmware.Helper, dut *dut.DUT) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
