...
func init() {
	testing.AddTest(&testing.Test{
		...
		VarDeps:      []string{"servo", "intel.iterations"},
		HardwareDeps: hwdep.D(hwdep.ChromeEC(), hwdep.X86()),
		Timeout:      48*time.Minute + 3*time.Minute, // Overall timeout for 500 iterations + shorten timeout.
	})
}
...
func LidCloseOpenSuspendStress(ctx context.Context, s *testing.State) {
	...
	iter, err := strconv.Atoi(s.RequiredVar("intel.iterations"))
	if err != nil {
