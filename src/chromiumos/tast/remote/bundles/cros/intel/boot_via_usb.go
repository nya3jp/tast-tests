...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "DUT can boot from USB type-C pen drive",
...
func BootViaUSB(ctx context.Context, s *testing.State) {
	...
	if err := checkUSBPDMuxInfo(ctx, h.DUT, "USB=1"); err != nil {
		s.Fatal("Failed to usb type-c: ", err)
	}
...
// checkUSBPDMuxInfo verifies whether USB=1 or not.
func checkUSBPDMuxInfo(ctx context.Context, dut *dut.DUT, deviceStr string) error {
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		return errors.Wrap(err, "failed to check usb=1")
	}
	return nil
}
