...
func PowerSave(fullCtx context.Context, s *testing.State) {
	...
	ap, err := tf.ConfigureAP(tfCtx, []hostapd.Option{
		...
		hostapd.HTCaps(hostapd.HTCapHT20), hostapd.DTIMPeriod(5)}, secConfFac)
	if err != nil {
	...
	iw := remote_iw.NewRunner(s.DUT().Conn())

	if err := iw.SetPowersaveMode(ctx, iface, true); err != nil {
		s.Fatal("DUT: failed to set the powersave mode: ", err)
	}

	if err := checkPowersave(ctx, iw, iface, true); err != nil {
	...
	if err := iw.SetPowersaveMode(ctx, iface, false); err != nil {
		s.Fatal("DUT: failed to set the powersave mode: ", err)
	}

	if err := checkPowersave(ctx, iw, iface, false); err != nil {
		s.Fatal("DUT: failed checking the powersave mode: ", err)
	}
...
func checkPowersave(ctx context.Context, iw *iw.Runner, intf string, expected bool) error {

	psMode, err := iw.PowersaveOn(ctx, intf)
	if err != nil {
