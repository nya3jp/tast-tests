...
func init() {
	testing.AddTest(&testing.Test{
		Func:        SuspendStress,
		Desc:        "Test suspend and resume with powerd_dbus_suspend command and assert wifi connectivity to router",
...
func SuspendStress(ctx context.Context, s *testing.State) {
	...
	testOnce := func(ctx context.Context, s *testing.State, suspendCount int, apOps []hostapd.Option, secConfFac security.ConfigFactory) {
		...
		s.Log("Start suspend stress")
		var connectTimes []float64
		for i := 0; i < suspendCount; i++ {
			connectTime, err := tf.SuspendAndAssertConnect(ctx, suspendTime)
			if err != nil {
				s.Error("Failed to suspend: ", err)
			} else {
