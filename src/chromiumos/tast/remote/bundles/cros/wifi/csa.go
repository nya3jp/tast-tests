...
func init() {
	testing.AddTest(&testing.Test{
		Func:        CSA,
		Desc:        "Verifies that DUT will move off-channel when sent a Spectrum Management action frame that contains a Channel Move element",
...
func CSA(ctx context.Context, s *testing.State) {
	...
	// to disconnect from the AP which is what we verify in this test.
	...
	if err != nil {
		s.Fatal("Failed to create frame sender, err: ", err)
	}
	defer func(dCtx context.Context) {
		if err := tf.Router().CloseFrameSender(dCtx, sender); err != nil {
			s.Error("Failed to close frame sender, err: ", err)
		}
	}(ctx)
	...
	for i := 0; i < maxRetry; i++ {
		s.Logf("Try send channel switch frame %d", i)
		if err := sender.Send(ctx, framesender.TypeChannelSwitch, alterChannel); err != nil {
			s.Fatal("Failed to send channel switch frame, err: ", err)
		}
		...
		if err != nil {
			s.Fatal("Failed to ping server, err: ", err)
		}
