...
func init() {
	testing.AddTest(&testing.Test{
		Func:         ConnectScan,
		Desc:         "The test verifies that the 802.11 probe frames with expected SSIDs are seen over-the-air when connecting to WiFi",
...
func ConnectScan(ctx context.Context, s *testing.State) {
	...
	for _, p := range packets {
		...
		if err != nil {
			...
			// avoid packets with invalid FCS, so we don't expect
			// malformed packets here.
