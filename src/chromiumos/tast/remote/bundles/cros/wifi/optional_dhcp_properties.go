...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:        "Verifies that optional DHCP properties set on the DUT are used as parameters in DHCP Request messages",
...
packetLoop:
	for _, p := range packets {
		...
		if layer == nil {
			s.Fatal("Non DHCPv4 packet passed type filter: ", p)
		}
		...
		if opt := optMap[layers.DHCPOptMessageType]; opt == nil {
			...
			continue
		} else if !bytes.Equal(opt.Data, []byte{3}) {
			continue
		}
