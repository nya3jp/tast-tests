...
func Nl80211StationInformation(ctx context.Context, s *testing.State) {
	...
	// Check if the difference between signal, signal avg and beracon signal avg
	// is too large
	for _, attribute := range []string{"signal", "signal avg"} {
		if math.Abs(float64(attributeValues[attribute]-attributeValues["beacon signal avg"])) > signalError {
			s.Fatalf("The difference between %s %d and beacon signal avg %d is larger than the threshold %d", attribute, attributeValues[attribute], attributeValues["signal avg"], signalError)
		}
	}
}
