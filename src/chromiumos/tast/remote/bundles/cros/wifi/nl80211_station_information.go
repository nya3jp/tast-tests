...
func Nl80211StationInformation(ctx context.Context, s *testing.State) {
	const (
		// The difference between NL80211_STA_INFO_SIGNAL_AVG and
		// NL80211_STA_INFO_SIGNAL. This value is subject to changes based on
		// test results.
		signalError      = 5
	...
	// represented by u64.
	nl80211Attributes := []nl80211Attribute{{"rx bytes", 1, math.MaxInt64}, {"tx bytes", 1, math.MaxInt64}, {"rx packets", 1, math.MaxUint32}, {"tx packets", 1, math.MaxUint32}, {"rx drop misc", 0, math.MaxInt64}, {"tx retries", 0, math.MaxUint32}, {"tx failed", 0, math.MaxUint32}, {"beacon signal avg", signalLowerBound, signalUpperBound}, {"signal avg", signalLowerBound, signalUpperBound}, {"signal", signalLowerBound, signalUpperBound}}
	...
	if missingAttributes != nil {
		s.Fatalf("%d nl80211_sta_info attributes are missing: %#v", len(missingAttributes), missingAttributes)
	}
	...
	// Check if the difference between signal and signal avg is too large
	if math.Abs(float64(attributeValues["signal"]-attributeValues["signal avg"])) > signalError {
		s.Fatalf("The difference between signal %s and signal avg %d is larger than the threshold %d", attributeValues["signal"], attributeValues["signal avg"], signalError)
	}
}
