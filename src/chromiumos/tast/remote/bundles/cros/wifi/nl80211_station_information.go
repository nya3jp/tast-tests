...
func Nl80211StationInformation(ctx context.Context, s *testing.State) {
	...
	for _, attribute := range []string{"signal", "signal avg"} {
		signal, ok1 := attributeValues[attribute]
		beaconSignal, ok2 := attributeValues["beacon signal avg"]
		if ok1 && ok2 && math.Abs(float64(signal-beaconSignal)) > signalError {
			s.Errorf("The difference between %q %d and \"beacon signal avg\" %d is larger than the threshold %d", attribute, attributeValues[attribute], attributeValues["beacon signal avg"], signalError)
		}
	}
}
