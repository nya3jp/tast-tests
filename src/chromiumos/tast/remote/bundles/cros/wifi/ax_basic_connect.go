...
func AxBasicConnect(ctx context.Context, s *testing.State) {
	axConnect(ctx, s, "Velop-5G", "chromeos")
	axConnect(ctx, s, "Rapture-5G-1", "chromeos")
	axConnect(ctx, s, "Juplink-RX4-1500", "chromeos")
	axConnect(ctx, s, "NETGEAR69-5G", "chromeos")
}

func axConnect(ctx context.Context, s *testing.State, ssid, passphrase string) {
	defer func(ctx context.Context) {
		rpc, _ := rpc.Dial(ctx, s.DUT(), s.RPCHint(), "cros")
		wificlient := network.NewWifiServiceClient(rpc.Conn)
		disconnectWifi(ctx, wificlient, false)
	}(ctx)
	s.Log("Connecting to Ax Router ", ssid)
	rpc, err := rpc.Dial(ctx, s.DUT(), s.RPCHint(), "cros")
	if err != nil {
