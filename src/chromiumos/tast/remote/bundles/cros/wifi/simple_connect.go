...
type simpleConnectTestcase struct {
	...
	securityConfigFactory security.Factory
}
		...
		Params: []testing.Param{
			{
				...
				Val: []simpleConnectTestcase{
					{
						apOptions: []hostapd.Option{hostapd.Mode(hostapd.Mode80211g), hostapd.Channel(1)},
						securityConfigFactory: wpa.NewFactory(
				...
				Val: []simpleConnectTestcase{
					{
						apOptions: []hostapd.Option{hostapd.Mode(hostapd.Mode80211g), hostapd.Channel(1)},
						securityConfigFactory: wep.NewFactory(
...
func SimpleConnect(ctx context.Context, s *testing.State) {
	...
	for i, tc := range testcases {
		subtest := func(ctx context.Context, s *testing.State) {
			testOnce(ctx, s, tc.apOptions, tc.securityConfigFactory)
		}
