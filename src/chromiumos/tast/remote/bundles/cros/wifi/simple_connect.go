...
func SimpleConnect(ctx context.Context, s *testing.State) {
	...
	testOnce := func(ctx context.Context, s *testing.State, options []ap.Option, fac security.ConfigFactory, pingOps []ping.Option, expectedFailure bool) {
		...
		// TODO move this type of checking to test fixture initialization with hardware deps
		if err := support.Validate(apIface.Router(), []string{
			"logs",
			"hostapd",
			"dhcp",
			"capture",
		}); err != nil {
			s.Fatal("Router failed type validation: ", err)
		}
