		...
		Params: []testing.Param{
			{
				...
				Val: []simpleConnectTestcase{
					...
					{
						// Should success if we specify that we have no CACert.
						apOpts: []ap.Option{ap.Mode(ap.Mode80211g), ap.Channel(1)},
						secConfFac: wpaeap.NewConfigFactory(
							eapCert1.CACert, eapCert1.ServerCred,
							wpaeap.ClientCred(eapCert1.ClientCred),
				...
				Name:      "8021xpeap",
				ExtraAttr: []string{"wificell_unstable"},
				// This test may take up to 8 minutes on some machines in labs.
				...
				Name:      "8021xttls",
				ExtraAttr: []string{"wificell_unstable"},
				// This test may take up to 13 minutes on some machines in labs.
...
// peapTestParam generate the tests of 8021xpeap. 4 bad tests and 3*5 good tests so totally 19 test cases.
...
// ttlsTestParam generate the tests of 8021xttls. 4 bad tests and 6*5 good tests so totally 34 test cases.
