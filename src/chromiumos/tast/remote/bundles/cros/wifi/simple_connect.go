		...
		Params: []testing.Param{
			{
				...
				// This tests verifies that we can connect to an AP broadcasting a WPA2 network using AES based CCMP.
				...
				Val: []simpleConnectTestcase{
					...
					{apOpts: simpleConnectCommonCase(ap.SSID(byteSequenceStr(1, 32) + "\x00"))},
					{apOpts: simpleConnectCommonCase(ap.SSID(byteSequenceStr(32, 64)))},
					{apOpts: simpleConnectCommonCase(ap.SSID(byteSequenceStr(64, 96)))},
					{apOpts: simpleConnectCommonCase(ap.SSID(byteSequenceStr(96, 128)))},
					{apOpts: simpleConnectCommonCase(ap.SSID(byteSequenceStr(128, 160)))},
					{apOpts: simpleConnectCommonCase(ap.SSID(byteSequenceStr(160, 192)))},
					{apOpts: simpleConnectCommonCase(ap.SSID(byteSequenceStr(192, 224)))},
					{apOpts: simpleConnectCommonCase(ap.SSID(byteSequenceStr(224, 256)))},
					// Valid Unicode characters.
					{apOpts: simpleConnectCommonCase(ap.SSID("\xe4\xb8\xad\xe5\x9b\xbd"))},
					// Single extended ASCII character (a-grave).
					{apOpts: simpleConnectCommonCase(ap.SSID("\xe0"))},
					// Mix of ASCII and Unicode characters as SSID.
					{apOpts: simpleConnectCommonCase(ap.SSID("Chrome\xe7\xac\x94\xe8\xae\xb0\xe6\x9c\xac"))},
				},
			},
		},
	})
