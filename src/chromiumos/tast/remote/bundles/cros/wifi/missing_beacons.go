		...
		Params: []testing.Param{
			{
				...
				Name: "scan",
				// Run a scan before taking down the AP interface.
				Val: true,
			},
		},
	})
...
func MissingBeacons(fullCtx context.Context, s *testing.State) {
	...
	if err := tf.Router().SetAPInterfaceDown(ctx, ap); err != nil {
	...
	if elapsed.Seconds() > float64(maxDisconnectTimeSeconds) {
		s.Errorf("DUT: timeout while disconnecting from the SSID, it took %f seconds", elapsed.Seconds())
	}
