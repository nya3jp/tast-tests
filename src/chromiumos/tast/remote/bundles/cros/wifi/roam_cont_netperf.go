...
func RoamContNetperf(ctx context.Context, s *testing.State) {
	...
	// Allow override
	var rounds int
	roundsStr, _ := s.Var("wifi.RoamContNetperf.rounds")
	if roundsStr == "" {
		rounds = param.Rounds
	} else {
		var err error
	...
	session, err := netperf.NewContinuousSession(ctx,
		s.DUT().Conn(), addrs[0].String(), tf.Router().Conn(), wifiutil.ServerIP(),
		netperf.Config{
	...
	targetThorughput := res[0].Measurements[netperf.CategoryThroughput] * param.Param.(netperfParam).threshold
