...
func RoamContNetperf(ctx context.Context, s *testing.State) {
	...
	if serverConn != ct.Router().Conn() {
		...
		ctx, pcapFinish := wifiutil.SetupPcapRouterConnection(ctx, s, ct.Router().Conn(), serverConn)
		defer pcapFinish()
		ctx, cancel = ctxutil.Shorten(ctx, 5*time.Second)
		defer cancel()

		// Make sure we have a pingable server as well.
		if err := tf.PingFromDUT(ctx, "192.168.1.50", ping.Count(5), ping.Interval(0.2)); err != nil {
			s.Fatal("Failed to ping ap from the DUT: ", err)
		}
		if err := tf.PingFromDUT(ctx, serverIP, ping.Count(5), ping.Interval(0.2)); err != nil {
			// Pinging pcap on internal address requires a cable to be connected between LAN ports
			// of both Gales. Failing ping means no such connection exists yet, fall back to 1-router
			// test then.
			s.Log("Failed to ping pcap from the DUT, falling back to a one-router setup. Err: ", err)
			serverConn = ct.Router().Conn()
			serverIP = wifiutil.ServerIP()
		}
	} else {
		s.Log("Separate pcap router not detected, falling back to a one-router setup")
		serverIP = wifiutil.ServerIP()
	}
