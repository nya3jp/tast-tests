...
func BSSTMRequest(ctx context.Context, s *testing.State) {
	...
	runTest := func(ctx context.Context, waitForScan bool) {
		...
		// When enabled for the current test, scan until the second AP is discovered again
		if waitForScan {
			s.Logf("Scanning for %s", roamBSSID)
		...
		// Wait for the DUT to roam to the second AP
		// TODO: Should we check for disconnects like roam_dbus does?
		s.Log("Waiting for roam")
	...
	// the second AP.
	s.Log("Testing BSS Transition Management With Prior Scan")
	runTest(ctx, true)
	...
	// transition request.
	s.Log("Testing BSS Transition Management Without Prior Scan")
	runTest(ctx, false)
}
