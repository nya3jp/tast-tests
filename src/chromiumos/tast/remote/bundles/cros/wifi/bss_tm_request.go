...
/*
	This test vefifies a device's behavior when it recieves a BSS Transition
	...
	6- Assert that the Shill property RoamState transitions from configuration
		-> ready -> idle, which indicates a roam has occured.
    ...
    9- If "disassoc_imminent" is enabled, send another BSSTMRequest from AP1,
		but this time assert that the roam fails due to the dissaciation.
    10- Send a BSSTMRequest from AP1 without dissasoc imminent.
...
func BSSTMRequest(ctx context.Context, s *testing.State) {
	...
	runTest := func(ctx context.Context, s *testing.State, waitForScan bool) {
		...
		// Before sending the BSSTM request, add the current BSSID into into the
		...
		// from the but it should prevent it from roaming back.
