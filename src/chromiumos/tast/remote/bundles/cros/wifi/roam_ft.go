...
type roamFTparam struct {
	...
	mixedSecConfFac security.ConfigFactory
	mixed           bool
}
	...
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			Val: roamFTparam{
				...
				mixedSecConfFac: wpa.NewConfigFactory("chromeos", wpa.Mode(wpa.ModePureWPA2), wpa.Ciphers2(wpa.CipherCCMP), wpa.FTMode(wpa.FTModeMixed)),
				mixed:           true,
			},
		}, {
			Name:      "eap",
			...
			Val: roamFTparam{
				...
				// TODO(crbug.com/977722): We shouldn't need different certs. Remove mixedSecConfFac field and use secConfFac once the bug is resolved.
				mixedSecConfFac: wpaeap.NewConfigFactory(
					roamFTCert2.CACert, roamFTCert2.ServerCred,
					wpaeap.ClientCACert(roamFTCert2.CACert), wpaeap.ClientCred(roamFTCert2.ClientCred),
					wpaeap.Mode(wpa.ModePureWPA2), wpaeap.FTMode(wpa.FTModeMixed),
				),
				mixed: true,
			},
...
func RoamFT(ctx context.Context, s *testing.State) {
	/*
		...
		to bridge the two interfaces either (as soon as managed0 is binded
	...
	*/
	...
	runOnce := func(ctx context.Context, secConfFac security.ConfigFactory) {
		var err error
		var br [2]string
		for i := 0; i < 2; i++ {
	...
	s.Log("Turning off the background scan")
	bgscanResp, err := tf.WifiClient().GetBgscanConfig(ctx, &empty.Empty{})
	if err != nil {
		s.Fatal("Failed to get background scan config: ", err)
	}
	oldBgConfig := bgscanResp.Config
	bgConfig := *bgscanResp.Config
	bgConfig.Method = shillconst.DeviceBgscanMethodNone
	if _, err := tf.WifiClient().SetBgscanConfig(ctx, &network.SetBgscanConfigRequest{Config: &bgConfig}); err != nil {
		s.Fatal("Failed to set background scan config: ", err)
	}
	defer func(ctx context.Context) {
		s.Log("Restoring bgscan config: ", oldBgConfig)
		if _, err := tf.WifiClient().SetBgscanConfig(ctx, &network.SetBgscanConfigRequest{Config: oldBgConfig}); err != nil {
			s.Error("Failed to set background scan config: ", err)
		}
	}(ctx)
	...
	runOnce(ctx, param.secConfFac)
	if param.mixed {
		// Run the test without global FT. If should pass if we configured the AP in mixed mode.
		if _, err := tf.WifiClient().SetGlobalFTProperty(ctx, &network.SetGlobalFTPropertyRequest{Enabled: false}); err != nil {
			s.Fatal("Failed to turn on the global FT property: ", err)
		}
		runOnce(ctx, param.mixedSecConfFac)
	}
}
