...
func RoamFT(ctx context.Context, s *testing.State) {
	...
	*/
	...
	runOnce := func(ctx context.Context, secConfFac security.ConfigFactory, expectedFailure bool) {
		...
		var br [2]string
		for i := 0; i < 2; i++ {
			br[i], err = tf.GetLegacyRouter().NewBridge(ctx)
			if err != nil {
				s.Fatal("Failed to get a bridge: ", err)
			}
			defer func(ctx context.Context, b string) {
				if err := tf.GetLegacyRouter().ReleaseBridge(ctx, b); err != nil {
		...
		var veth [2]string
		veth[0], veth[1], err = tf.GetLegacyRouter().NewVethPair(ctx)
		if err != nil {
			s.Fatal("Failed to get a veth pair: ", err)
		}
		defer func(ctx context.Context) {
			if err := tf.GetLegacyRouter().ReleaseVethPair(ctx, veth[0]); err != nil {
		...
		for i := 0; i < 2; i++ {
			if err := tf.GetLegacyRouter().BindVethToBridge(ctx, veth[i], br[i]); err != nil {
				s.Fatalf("Failed to bind the veth %q to bridge %q: %v", veth[i], br[i], err)
			}
			defer func(ctx context.Context, ve string) {
				if err := tf.GetLegacyRouter().UnbindVeth(ctx, ve); err != nil {
		...
		ap0, err := tf.GetLegacyRouter().StartHostapd(ctx, ap0Name, ap0Conf)
		if err != nil {
			s.Fatal("Failed to start the hostapd server on the first AP: ", err)
		}
		defer func(ctx context.Context) {
			if err := tf.GetLegacyRouter().StopHostapd(ctx, ap0); err != nil {
		...
		ds, err := tf.GetLegacyRouter().StartDHCP(ctx, ap0Name, br[0], startIP, endIP, serverIP, broadcastIP, mask)
		if err != nil {
			s.Fatal("Failed to start the DHCP server: ", err)
		}
		defer func(ctx context.Context) {
			if err := tf.GetLegacyRouter().StopDHCP(ctx, ds); err != nil {
		...
		ap1, err := tf.GetLegacyRouter().StartHostapd(ctx, ap1Name, ap1Conf)
		if err != nil {
			s.Fatal("Failed to start the hostapd server on the second AP: ", err)
		}
		defer func(ctx context.Context) {
			if err := tf.GetLegacyRouter().StopHostapd(ctx, ap1); err != nil {
