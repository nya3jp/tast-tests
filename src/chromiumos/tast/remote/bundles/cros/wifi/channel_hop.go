...
func ChannelHop(ctx context.Context, s *testing.State) {
	...
	runOnce := func(ctx context.Context, apOps []hostapd.Option) (retErr error) {
		...
		// TODO(b/172785860): We have longer timeout than the Autotest version
		// here because fast scan after disconnection might not be available
		// in some cases . It can take DUT up to 60s (default scan interval)
