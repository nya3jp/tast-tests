...
func AxSimpleConnect(ctx context.Context, s *testing.State) {
	...
	tfOps = append(tfOps, wificell.TFRouterType(sharedrouter.AxT))
	// Assert WiFi is up.
	...
	for i, tc := range testcases {
		...
		subtest := func(ctx context.Context, s *testing.State) {
			testOnce(ctx, s, tc.band, append(tc.apOpts, defaultOpts...), tc.secConfFac, tc.pingOps, tc.expectedFailure)
		}
