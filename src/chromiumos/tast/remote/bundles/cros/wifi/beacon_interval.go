...
func BeaconInterval(testCtx context.Context, s *testing.State) {
	...
	val, err := iwr.LinkValue(testCtx, iface, "beacon int")
	if err != nil {
		s.Fatal("Failed to get interface beacon interval value: ", err)
	}
	bi, err := strconv.Atoi(val)
	if err != nil {
		s.Fatalf("Beacon interval %q not a number: %v", val, err)
	}
	if bi != expectBeaconInt {
		s.Fatalf("Unmatched beacon interval, got %d, want %d", bi, expectBeaconInt)
	}
	// Check connectivity.
	...
	s.Log("Verification done, tearing down")
}
