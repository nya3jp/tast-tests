...
func CSADisconnect(ctx context.Context, s *testing.State) {
	...
	csaDisconnectCore := func(ctx context.Context, tf *wificell.TestFixture, clientInit bool, primaryChannel, alternateChannel, i int) {
		...
		s.Logf("Connected. Sending channel switch frame (try %d)", i)
	...
	for i := 0; i < maxRetry; i++ {
		csaDisconnectCore(ctx, tf, s.Param().(bool), primaryChannel, alternateChannel, i)
	}
}
