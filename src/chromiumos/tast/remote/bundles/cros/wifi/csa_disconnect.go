...
func CSADisconnect(ctx context.Context, s *testing.State) {
	...
	// Run it multiple times since the DUT's WiFi module might be in power
	// saving mode, we are not guaranteed it will hear this message the first
	// time around. Alternate the AP channel with the CSA announced channel to
	// work around with drivers (Marvell 8897) that disallow reconnecting
	// immediately to the same AP on the same channel after CSA to a different
	// channel.
