...
func init() {
	testing.AddTest(&testing.Test{
		Func:        CSAReconnect,
		Desc:        "Verifies that DUT will switch to the new channel after the AP sends a CSA action frame",
		...
		HardwareDeps: hwdep.D(hwdep.Platform("kukui", "jacuzzi", "grunt")),
	})
}
...
func CSAReconnect(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < maxRetry; i++ {
		...
		// Router send CSA.
		if err := ap.SendCSA(ctx, alterChannel); err != nil {
