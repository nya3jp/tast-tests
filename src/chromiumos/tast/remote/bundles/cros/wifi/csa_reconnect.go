...
func CSAReconnect(ctx context.Context, s *testing.State) {
	...
	csaFreq, err := hostapd.ChannelToFrequency(alterChannel)
	if err != nil {
		s.Fatal("Failed to convert channel to frequency: ", err)
	}

	// Router starts ChannelSwitching.
	if err := ap.StartChannelSwitch(ctx, hostapd.CSAMode("ht"), hostapd.CSAFrequency(csaFreq), hostapd.CSACount(8)); err != nil {
