...
func CSAReconnect(ctx context.Context, s *testing.State) {
	...
	props := []*wificell.ShillProperty{
		&wificell.ShillProperty{
			...
			Method:         network.ExpectShillPropertyRequest_CHECK_POLL,
		},
	}
	...
	/*
		if err := tf.WaitForFrequency(ctx, servicePath, alterChannel); err != nil {
			s.Error("Failed to wait for frequency change: ", err)
		}
	*/
