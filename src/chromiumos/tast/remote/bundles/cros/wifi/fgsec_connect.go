...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{
			...
			{
				// And now something completely different ;) AP1 is WpaWpa2 mixed
				// mode and we add AP2 with Wpa3.  This should result in Wpa2Wpa3
				// (with Wpa being dropped as we do not support mode with all three
				// Wpa modes).
				Name: "dropwpa",
				Val: []fgsecTestcase{{
					ap1Opts: []ap.Option{ap.Mode(ap.Mode80211nPure), ap.Channel(1), ap.HTCaps(ap.HTCapHT20)},
...
func FgsecConnect(ctx context.Context, s *testing.State) {
	...
	singleTest := func(ctx context.Context, s *testing.State, tc *fgsecTestcase) {
		...
		// Make sure DUT sees APs - for tests with single AP this is not
		// needed but for tests with 2 APs since we only connect to AP1
		// this explicit scan request (with blocking wait for report)
		// should ensure that both APs have been seen by the DUT.
		if err := wifiutil.RequestScanAndWaitForReport(ctx, s); err != nil {
			s.Fatal("Failed to request a scan: ", err)
		}
		_, err = tf.ConnectWifiAP(ctx, ap1)
		if err != nil {
	...
	testcases := s.Param().([]fgsecTestcase)
