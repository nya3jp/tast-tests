	...
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{
			...
			},
			{
				...
				// meeting wpa1+wpa2+wpa3 we tranition to wpa-all security.
				Name: "wpaall",
				Val: fgsecTestcase{
					ap1Opts: []ap.Option{ap.Mode(ap.Mode80211nPure), ap.Channel(1), ap.HTCaps(ap.HTCapHT20)},
					sec1Conf: wpa.NewConfigFactory("chromeos",
...
func FgsecConnect(ctx context.Context, s *testing.State) {
	...
	connectAPs := func(ctx context.Context, s *testing.State, tc *fgsecTestcase) {
	...
	testcase := s.Param().(fgsecTestcase)
	connectAPs(ctx, s, &testcase)
