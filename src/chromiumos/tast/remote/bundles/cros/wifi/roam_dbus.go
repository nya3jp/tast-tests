...
func RoamDbus(ctx context.Context, s *testing.State) {
	...
	if err := tf.RequestRoam(ctx, iface, ap2BSSID, ap2, 30*time.Second); err != nil {
		s.Errorf("DUT: failed to roam from current BSSID = %s to the BSSID = %s: %v", ap1BSSID, ap2BSSID, err)
	}
