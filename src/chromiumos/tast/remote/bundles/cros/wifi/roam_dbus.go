...
func RoamDbus(fullCtx context.Context, s *testing.State) {
	...
	// shill. After that shill will send a dbD-Bus roam command to wpa_supplicant.
	...
	const ap2Channel = 1
	optionsAP2 := []hostapd.Option{hostapd.Mode(hostapd.Mode80211nPure), hostapd.Channel(ap2Channel), hostapd.HTCaps(hostapd.HTCapHT20), hostapd.SSID(ap1.Config().SSID), hostapd.BSSID(ap2BSSID)}
