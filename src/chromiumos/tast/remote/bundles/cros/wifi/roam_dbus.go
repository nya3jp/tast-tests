...
func init() {
	testing.AddTest(&testing.Test{
		...
		HardwareDeps: hwdep.D(hwdep.SkipOnPlatform("veyron_jerry"), hwdep.SkipOnPlatform("veyron_mickey"), hwdep.SkipOnPlatform("veyron_speedy")),
...
func RoamDbus(ctx context.Context, s *testing.State) {
	...
	req := &network.ExpectShillPropertyValuesRequest{
		...
		Props: []*network.ExpectShillPropertyValuesRequest_Pair{

			&network.ExpectShillPropertyValuesRequest_Pair{
				Key:             shillconst.ServicePropertyState,
				ExpectedVals:    []*network.ShillVal{config},
				NotExpectedVals: []*network.ShillVal{idle},
				Check:           false},

			&network.ExpectShillPropertyValuesRequest_Pair{
				Key:             shillconst.ServicePropertyState,
				ExpectedVals:    connectedStates,
				NotExpectedVals: []*network.ShillVal{idle},
				Check:           false},

			&network.ExpectShillPropertyValuesRequest_Pair{
				Key:             shillconst.ServicePropertyWiFiBSSID,
				ExpectedVals:    []*network.ShillVal{roamToBSSID},
				NotExpectedVals: []*network.ShillVal{idle},
				Check:           true}},
	}
