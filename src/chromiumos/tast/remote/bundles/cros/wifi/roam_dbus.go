...
func RoamDbus(fullCtx context.Context, s *testing.State) {
	...
	defer func() {
		...
		if _, err := tf.WifiClient().DeleteEntriesForSSID(apCtx, &network.DeleteEntriesForSSIDRequest{Ssid: []byte(ap1BSSID)}); err != nil {
	...
	optionsAP2 := []hostapd.Option{hostapd.Mode(hostapd.Mode80211nPure), hostapd.Channel(channels[1]), hostapd.HTCaps(hostapd.HTCapHT20), hostapd.SSID(ap1.Config().SSID), hostapd.BSSID(ap2BSSID)}
	...
	// Check which AP we are currently connected. This is to include the case
	// that wpa_supplicant automatically roam to AP2 during the scan.
	...
	testing.Sleep(apCtx, 10*time.Second)
