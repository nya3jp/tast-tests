...
func RoamDbus(ctx context.Context, s *testing.State) {
	...
	defer cancel()
	monitorProps := []string{shillconst.ServicePropertyState, shillconst.ServicePropertyWiFiFrequencyList, shillconst.ServicePropertyWiFiFrequency, shillconst.ServicePropertyWiFiBSSID}
	...
	for _, ph := range monitorResult {
		if ph.Name == shillconst.ServicePropertyState {
			if err := shillconst.IsConnectedState(ph.Name, ph.Value); err != nil {
				s.Fatal("DUT: failed to stay connected during the roaming process, err: ", err)
			}
		}
	}
