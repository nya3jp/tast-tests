...
func MultiAuth(ctx context.Context, s *testing.State) {
	...
	testcases := []multiAuthParam{
		...
		{
			// In WPA-PSK, all stations share the same passphrase, whose leakage
			// may result in attack. WPA-EAP reduces the risk by adopting a
			// RADIUS server to authenticate the clients.
			name0: "WPA",
