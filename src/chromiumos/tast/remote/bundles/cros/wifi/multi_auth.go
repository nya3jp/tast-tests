...
func MultiAuth(ctx context.Context, s *testing.State) {
	testOnce := func(ctx context.Context, s *testing.State, param multiAuthParam) {
		...
		s.Logf("Configuring AP 0 (%s)", param.name0)
		ap0, err := tf.ConfigureAP(ctx, apOpts, param.cfg0)
		if err != nil {
		...
		s.Logf("Configuring AP 1 (%s)", param.name1)
		ap1, err := tf.ConfigureAP(ctx, apOpts, param.cfg1)
		if err != nil {
