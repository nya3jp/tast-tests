...
func tdlsSetupTeardown(ctx context.Context, tc *tdlsTestcase, peer1, peer2 tdlsPeer) error {
	...
	pcapPath, err := wifiutil.CollectPcapForAction(ctx, tc.pcap, tc.name, tc.ap.Config().Channel, freqOpts,
		func(ctx context.Context) (ret error) {
			...
			if result.Received < 8 {
				// Some packets may be lost in theory, we don't want too much noise from tests.
				return errors.New("too many lost packets")
			}
			if result.Sent != result.Received {
