...
func init() {
	testing.AddTest(&testing.Test{
		...
		Attr:         []string{"group:wificell_dual"},
...
func tdlsDiscover(ctx context.Context, tc *tdlsTestcase, peer1, peer2 tdlsPeer) error {
	...
	err := r.TDLSCmd(ctx, "tdls_discover", peer2.mac.String())
	if err != nil {
...
func tdlsSetupTeardown(ctx context.Context, tc *tdlsTestcase, peer1, peer2 tdlsPeer) error {
	...
	testing.ContextLogf(ctx, "Peer 2 MAC: %s", peer2.mac.String())
	testing.ContextLogf(ctx, "Peer 2 IP: %s", peer2.ip.String())
	freqOpts, err := tc.ap.Config().PcapFreqOptions()
	if err != nil {
		return errors.Wrap(err, "failed to get Freq Opts")
	}

	result, err := tc.tf.PingFromSpecificDUT(ctx, peer1.dutIdx, peer2.ip.String())
	if err != nil {
		return err
	}
	...
	pcapPath, err := wifiutil.CollectPcapForAction(ctx, tc.pcap, wifiutil.UniqueAPName(), tc.ap.Config().Channel, freqOpts,
		func(ctx context.Context) error {
			// Setup TDLS tunnel
			r := wpacli.NewRunner(&cmd.RemoteCmdRunner{Host: peer1.conn})
			err := r.TDLSCmd(ctx, "tdls_setup", peer2.mac.String())
			if err != nil {
			...
			// Teardown the tunnel.
			err = r.TDLSCmd(ctx, "tdls_teardown", peer2.mac.String())
			if err != nil {
				return errors.Wrap(err, "failed TDLS Teardown")
			}
...
func TDLS(ctx context.Context, s *testing.State) {
	...
	// 3. Start capture.
	// 4. Run control scans from Peer DUT (using both iw and wpa_cli).
	// 5. Run subtests
	...
	defer cancel()
	ok, err = wifiutil.RunAndCheckOutput(runnerCtx, tf.DUTConn(1).CommandContext(runnerCtx, "iwlist", tdlsPeers[1].ifName, "scan"), apIface.Config().SSID)
	if err != nil {
		s.Fatal("Failed to call iwlist scan: ", err)
	}
	...
	if err != nil {
		s.Fatal("Failed to run wpa_cli scan: ", err)
	}
