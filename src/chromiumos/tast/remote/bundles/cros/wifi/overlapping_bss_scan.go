...
func OverlappingBSSScan(ctx context.Context, s *testing.State) {
	...
	// and collect ping statistics. The Capturer object is also returned
	// so the caller can verify the OBSS scan setting is working properly.
	...
	if statsBgscan.MaxLatency > statsNoBgscan.MaxLatency+latencyMargin {
		s.Errorf("Significant difference in rtt due to OBSS scan: %f ms > %f + %f ms",
	...
	if len(packets) == 0 {
		s.Fatal("No coexistence management found in pcap")
	}
}
