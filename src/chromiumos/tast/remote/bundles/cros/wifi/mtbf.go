...
func MTBF(ctx context.Context, s *testing.State) {
	...
	runOnce := func(ctx context.Context) (succeeded bool) {
		...
		s.Log("Simple connect to AP1")
		ap1Opts, ap1SecConfFac := ap1Config()
		ap1, err := tf.ConfigureAP(ctx, ap1Opts, ap1SecConfFac)
		if err != nil {
			s.Fatal("Failed to start AP1: ", err)
		}
		...
		s.Log("Connected; Start pinging for ", pingDuration)
		if err := tf.PingFromDUT(ctx, ap1.ServerIP().String(), pingOpts...); err != nil {
			s.Log("Failed to ping from the DUT: ", err)
			return false
		}

		s.Log("Suspend and reconnect to AP1")
		reconnectDuration, err := tf.SuspendAssertConnect(ctx, suspendDuration)
		if err != nil {
			s.Log("Failed to suspend and reconnect to AP1: ", err)
			return false
		}
		s.Log("Reconnect duration after suspend: ", reconnectDuration)
		if err := tf.PingFromDUT(ctx, ap1.ServerIP().String(), pingOpts...); err != nil {
			s.Log("Failed to ping from the DUT: ", err)
			return false
		}

		// We run suspend in a go routine and configure APs simultaneously, so a channel for reporting error is needed.
		suspendErr := make(chan error)
		defer close(suspendErr)

		s.Log("Suspend and move to AP2")
		...
		go func() { suspendErr <- tf.Suspend(ctx, suspendDuration) }()
		// Wait for the DUT to fall asleep.
		testing.Sleep(ctx, time.Second*10)

		deconfigAP(ctx, &ap1)
		ap2Opts, ap2SecConfFac := ap2Config()
		ap2, err := tf.ConfigureAP(ctx, ap2Opts, ap2SecConfFac)
		if err != nil {
			s.Fatal("Failed to start AP2: ", err)
		}
		...
		s.Log("Connected; Start pinging for ", pingDuration)
		if err := tf.PingFromDUT(ctx, ap2.ServerIP().String(), pingOpts...); err != nil {
			s.Log("Failed to ping from the DUT: ", err)
			return false
		}

		s.Log("Suspend and move back to AP1")
