...
func MTBF(ctx context.Context, s *testing.State) {
	...
	step := 1
	runStep := func(ctx context.Context, desc string, f func(ctx context.Context) error) (succeeded bool) {
		defer func() { step++ }()
		s.Logf("Step %d: %v", step, desc)
		ctx, st := timing.Start(ctx, desc)
		defer st.End()
		if err := f(ctx); err != nil {
			s.Logf("Failed at step %d: %v, err=%v", step, desc, err)
			return false
		}
		return true
	}
	...
	runOnce := func(ctx context.Context) (succeeded bool) {
		...
		stepCtx, cancel := context.WithTimeout(ctx, simpleConnectTimeout)
		defer cancel()
		if !runStep(stepCtx, "Connect to AP1 and perform ping test", func(ctx context.Context) error {
		...
		stepCtx, cancel = context.WithTimeout(ctx, suspendReconnectTimeout)
		defer cancel()
		if !runStep(stepCtx, fmt.Sprintf("Suspend for %v and reconnect to AP1", suspendDuration), func(ctx context.Context) error {
		...
		stepCtx, cancel = context.WithTimeout(ctx, suspendMoveTimeout)
		defer cancel()
		if !runStep(stepCtx, fmt.Sprintf("Suspend for %v and move to AP2", suspendDuration), func(ctx context.Context) error {
		...
		stepCtx, cancel = context.WithTimeout(ctx, suspendMoveBackTimeout)
		defer cancel()
		return runStep(stepCtx, fmt.Sprintf("Suspend for %v and move back to AP1", suspendDuration), func(ctx context.Context) error {
