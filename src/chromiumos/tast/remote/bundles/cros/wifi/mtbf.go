...
func MTBF(ctx context.Context, s *testing.State) {
	...
	runOnce := func(ctx context.Context) (succeeded bool) {
		...
		s.Log("Step: Connect to AP1 and perform ping test")
		...
		s.Logf("Step: Suspend for %v and reconnect to AP1", suspendDuration)
		...
		s.Logf("Step: Suspend for %v and move to AP2", suspendDuration)
		...
		s.Logf("Step: Suspend for %v and move back to AP1", suspendDuration)
