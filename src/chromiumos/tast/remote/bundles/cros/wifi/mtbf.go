...
func MTBF(ctx context.Context, s *testing.State) {
	...
	runOnce := func(ctx context.Context) (succeeded bool) {
		...
		if !runStep(ctx, "Connect to AP1 and perform ping test", func(ctx context.Context) error {
			ctx, cancel = context.WithTimeout(ctx, simpleConnectTimeout)
			defer cancel()
		...
		if !runStep(ctx, fmt.Sprintf("Suspend for %v and reconnect to AP1", suspendDuration), func(ctx context.Context) error {
			ctx, cancel = context.WithTimeout(ctx, suspendReconnectTimeout)
			defer cancel()
		...
		if !runStep(ctx, fmt.Sprintf("Suspend for %v and move to AP2", suspendDuration), func(ctx context.Context) error {
			ctx, cancel = context.WithTimeout(ctx, suspendMoveTimeout)
			defer cancel()
		...
		return runStep(ctx, fmt.Sprintf("Suspend for %v and move back to AP1", suspendDuration), func(ctx context.Context) error {
			ctx, cancel = context.WithTimeout(ctx, suspendMoveBackTimeout)
			defer cancel()
