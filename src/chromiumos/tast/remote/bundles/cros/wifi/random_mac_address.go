	...
	testOnce := func(fullCtx context.Context, s *testing.State, name string, enabled bool) {
		...
		defer func(restore bool) {
			if _, err := tf.WifiClient().SetMACRandomize(fullCtx, &network.SetMACRandomizeRequest{Enable: restore}); err != nil {
				s.Errorf("Failed to restart MAC randomization setting to %t: %v", restore, err)
			}
		}(resp.OldSetting)
		...
		// before our setting.
		if _, err := tf.WifiClient().WaitScanDone(ctx, &empty.Empty{}); err != nil {
...
// file is returned.
func scanAndCollectPcap(fullCtx context.Context, tf *wificell.TestFixture, name string, ch int) (ret string, retErr error) {
	...
	// Return the path where capturer will save the pcap.
	return capturer.PacketPath(ctx)
}
