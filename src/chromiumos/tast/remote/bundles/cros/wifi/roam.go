	...
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{
			{ // Verifies that DUT can roam between two APs in full view of it.
				Name: "none",
				Val: apConfiguration{
					apOpts1:    []hostapd.Option{hostapd.Mode(hostapd.Mode80211nPure), hostapd.Channel(1), hostapd.HTCaps(hostapd.HTCapHT20), hostapd.SSID(bssSSID)},
					apOpts2:    []hostapd.Option{hostapd.Mode(hostapd.Mode80211nPure), hostapd.Channel(48), hostapd.HTCaps(hostapd.HTCapHT20), hostapd.SSID(bssSSID)},
					secConfFac: nil,
				},
			}, {
				...
				Name: "wep",
				Val: apConfiguration{
					apOpts1:    []hostapd.Option{hostapd.Mode(hostapd.Mode80211b), hostapd.Channel(1), hostapd.SSID(bssSSID)},
					apOpts2:    []hostapd.Option{hostapd.Mode(hostapd.Mode80211a), hostapd.Channel(48), hostapd.SSID(bssSSID)},
			...
			// TODO: Add a test case for EAP-TLS configuration to verify that DUT can roam between two
			// 802.1x EAP-TLS APs in full view of it. In the meantime, the EAP-TLS is blocked by hwsec's
...
func Roam(fullCtx context.Context, s *testing.State) {
	...
	ap1, err := tf.ConfigureAP(ap1Ctx, s.Param().(apConfiguration).apOpts1, s.Param().(apConfiguration).secConfFac)
	if err != nil {
	...
	ap2, err := tf.ConfigureAP(ap2Ctx, s.Param().(apConfiguration).apOpts2, s.Param().(apConfiguration).secConfFac)
	if err != nil {
		s.Fatal("Failed to configure ap, err: ", err)
	}
	defer func() {
