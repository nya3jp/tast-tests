...
func Roam(fullCtx context.Context, s *testing.State) {
	...
	defer func() {
		...
		req := &network.DeleteEntriesForSSIDRequest{Ssid: []byte(ap1SSID)}
		if _, err := tf.WifiClient().DeleteEntriesForSSID(ap2Ctx, req); err != nil {
			s.Errorf("Failed to remove entries for ssid=%s, err: %v", ap1SSID, err)
		}
	}()
	...
	ops := append(param.apOpts2, hostapd.SSID(ap1SSID))
