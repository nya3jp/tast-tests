...
func LinkMonitorFailure(ctx context.Context, s *testing.State) {
	...
	startTime, err := tf.CurrentClientTime(ctx)
	if err != nil {
	...
	reassoc, err := reassociateStartTime(el)
	if err != nil {
		s.Fatal("Failed to get reassociate start time: ", err)
	}
	linkFailureTime := reassoc.Sub(startTime)
	...
	conn, err := connectedTime(el)
	if err != nil {
		s.Fatal("Failed to get connected time: ", err)
	}
	reassociateTime := conn.Sub(reassoc)
...
func reassociateStartTime(el *iw.EventLogger) (time.Time, error) {
...
func connectedTime(el *iw.EventLogger) (time.Time, error) {
