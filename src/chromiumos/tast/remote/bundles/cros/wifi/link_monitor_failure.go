...
func LinkMonitorFailure(ctx context.Context, s *testing.State) {
	...
	if err := testing.Poll(ctx, func(context.Context) error {
		_, err := el.LinkFailureDetectedTime()
		return err
	}, &testing.PollOptions{
	...
	linkFailureDetectedTime, err := el.LinkFailureDetectedTime()
	if err != nil {
	...
	if reassociateDuration < 0 {
		s.Errorf("Unexpected reassociate duration: %d is negtive", reassociateDuration)
	}
