...
func LinkMonitorFailure(ctx context.Context, s *testing.State) {
	...
	linkFailureTS, err := tf.CurrentClientTime(ctx) // TS stands for timestamp
	if err != nil {
	...
	if err := testing.Poll(ctx, func(context.Context) error {
		_, err := findConnected(el)
		return err
	}, &testing.PollOptions{
		Timeout:  linkFailureTimeout + reassociateTimeout,
		Interval: time.Second * 3,
	}); err != nil {
		s.Error("Failed to wait for link failure and reassociation to complete: ", err)
	}

	// Calculate duration for sensing the link failure.
	linkFailureDetectedTS, err := findLinkFailure(el)
	if err != nil {
		s.Fatal("Failed to get link failure detection time: ", err)
	}
	linkFailureTime := linkFailureDetectedTS.Sub(linkFailureTS)
	...
	connectedTS, err := findConnected(el)
	if err != nil {
		s.Fatal("Failed to get connected time: ", err)
	}
	reassociateTime := connectedTS.Sub(linkFailureDetectedTS)
	if reassociateTime > reassociateTimeout {
		s.Error("Failed to reassociate within given timeout")
	}
	s.Logf("Reassociate time: %.2f seconds", reassociateTime.Seconds())
}

func findLinkFailure(el *iw.EventLogger) (time.Time, error) {
	var reassoc time.Time

	if startScanEvs := el.EventsByType(iw.EventTypeScanStart); len(startScanEvs) != 0 {
		reassoc = startScanEvs[0].Timestamp
	}
	...
	if disconnectEvs := el.EventsByType(iw.EventTypeDisconnect); len(disconnectEvs) != 0 {
		if reassoc.IsZero() || reassoc.After(disconnectEvs[0].Timestamp) {
			reassoc = disconnectEvs[0].Timestamp
		}
	}

	if reassoc.IsZero() {
		return time.Time{}, errors.New("start scan event and disconnect event not found")
	}
	return reassoc, nil
}

func findConnected(el *iw.EventLogger) (time.Time, error) {
