...
func init() {
	// ANQP test aims to verify a DUT is able to fetch ANQP data from an access point.
...
func ANQP(ctx context.Context, s *testing.State) {
	...
	monitor, monitorStop, sCtx, err := tf.StartWPAMonitor(ctx, wificell.DefaultDUT)
	if err != nil {
...
func fetchANQPAndWait(ctx context.Context, r *wpacli.Runner, m *wificell.WPAMonitor, bssid net.HardwareAddr) error {
	...
	for {
		...
		if e, ok := event.(*wificell.ANQPQueryDoneEvent); ok {
			...
			return errors.Errorf("ANQP query failed (result=%q)", e.Result)
		}
	}
}
