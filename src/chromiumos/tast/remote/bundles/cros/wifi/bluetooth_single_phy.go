...
func BluetoothSinglePhy(ctx context.Context, s *testing.State) {
	// Cleanup on exit.
	...
	for _, ch := range channels {
		if err := togglePhys(ctx, ch, btClient, tf, wifiClient, credKey, true); err != nil {
			s.Fatalf("Failed to run WiFi without Bluetooth path on channel %d: %v", ch, err)
		}
		if err := togglePhys(ctx, ch, btClient, tf, wifiClient, credKey, false); err != nil {
			s.Fatalf("Failed to run Bluetooth without WiFi path on channel %d: %v", ch, err)
		}
	}
}

func togglePhys(ctx context.Context, channel int, btClient network.BluetoothServiceClient, tf *wificell.TestFixture, wifiClient network.WifiServiceClient, credKey string, enableWifiFirst bool) error {
	// Disable WiFi.
	...
	if enableWifiFirst {
		...
		// Validate Bluetooth is functional.
		if _, err := btClient.ValidateBluetoothFunctional(ctx, &empty.Empty{}); err != nil {
