...
func ConnectRandomizeMAC(ctx context.Context, s *testing.State) {
	...
	if bytes.Equal(hwMac, connMac) {
		s.Fatal("Failed to randomize MAC during connection: ", connMac)
	}
	...
	if !bytes.Equal(reconnMac, connMac) {
		s.Fatalf("Failed to keep the MAC during re-connection: got %s, want %s", reconnMac, connMac)
	}
	...
	if !bytes.Equal(connMac, connMac1) {
		s.Fatalf("MAC used for AP1 changed after switching AP: got %s, want %s", connMac1, connMac)
	}
