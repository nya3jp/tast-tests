...
func DisconnectClearsIP(ctx context.Context, s *testing.State) {
	...
	tf.DefaultWiFiOptions = []hostapd.Option{hostapd.Mode(hostapd.Mode80211nPure), hostapd.Channel(48), hostapd.HTCaps(hostapd.HTCapHT20)}

	ap, err := tf.ConfigureAP(ctx, tf.DefaultWiFiOptions...)
	if err != nil {
	...
	addr, err := iface.Addresses(ctx)
	if err != nil {
		s.Fatal("DUT: failed to get the IP addresses info: ", err)
	}
	...
	waitIPGone := func() {
		...
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			...
			if addr.IPv4 != "" {
				return errors.Errorf("DUT: expect no IPv4 address, got: %s", addr.IPv4)
			}
			return nil
		}, &testing.PollOptions{Timeout: 10 * time.Second, Interval: 200 * time.Millisecond}); err != nil {
			s.Fatal("Somthing failed: ", err)
		}
	}
