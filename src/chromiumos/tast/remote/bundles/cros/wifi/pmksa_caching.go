...
func PMKSACaching(ctx context.Context, s *testing.State) {
	...
	defer cancel()
	waitForRoam, err := tf.ExpectShillProperty(roamCtx, connResp.ServicePath, roamProps(ap1BSSID))
	if err != nil {
	...
	roamCtx, cancel = context.WithTimeout(ctx, roamTimeout)
	defer cancel()
	waitForRoam, err = tf.ExpectShillProperty(roamCtx, connResp.ServicePath, roamProps(ap0BSSID))
	if err != nil {
		s.Fatal("Failed to create a property watcher on DUT: ", err)
	}
	skippedRecver, err = tf.EAPAuthSkipped(roamCtx)
	...
	// Shill may keep IsConnected during the whole roaming process if wpa_supplicant didn't explicitly tell that the DUT have disconnected.
