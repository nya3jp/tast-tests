...
func PMKSACaching(ctx context.Context, s *testing.State) {
	...
	func(ctx context.Context) {
		ctx, cancel := context.WithTimeout(ctx, roamTimeout)
		defer cancel()
		...
		waitForRoam, err := tf.ExpectShillProperty(ctx, connResp.ServicePath, roamToAp1Props)
		if err != nil {
		...
		if err := tf.DiscoverBSSID(ctx, ap1BSSID, iface, []byte(ssid)); err != nil {
		...
		if err := tf.RequestRoam(ctx, iface, ap1BSSID, 5*time.Second); err != nil {
	...
	func(ctx context.Context) {
		...
		waitForRoam, err := tf.ExpectShillProperty(ctx, connResp.ServicePath, roamToAp0Props)
		if err != nil {
			s.Fatal("Failed to create a property watcher on DUT: ", err)
		}
