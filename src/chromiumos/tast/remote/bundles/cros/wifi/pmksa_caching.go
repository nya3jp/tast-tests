...
func PMKSACaching(ctx context.Context, s *testing.State) {
	...
	// idleStateHandler indicates how should the roaming handle idle state.
	type idleStateHandler int
	const (
		ignoreIdleState = iota
		disallowIdleState
	)
	roamProps := func(bssid string, ih idleStateHandler) []*wificell.ShillProperty {
		...
		case ignoreIdleState:
			// The props has already met this case.
		}
		return props
	}
	...
	roamToAP1Props := roamProps(ap1BSSID, disallowIdleState)
	...
	roamToAP0Props := roamProps(ap0BSSID, ignoreIdleState)
	roamCtx, cancel = context.WithTimeout(ctx, roamTimeout)
	defer cancel()
