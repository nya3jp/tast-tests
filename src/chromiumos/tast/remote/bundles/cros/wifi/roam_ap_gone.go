...
func RoamAPGone(ctx context.Context, s *testing.State) {
	...
	// Assure that the DUT has disconnected.
	if err := tf.WaitForDisconnection(ctx, 20*time.Second); err != nil {
		s.Fatal("DUT: failed to disconnect from the initial AP: ", err)
	}
	s.Log("Disconnected AP1")
