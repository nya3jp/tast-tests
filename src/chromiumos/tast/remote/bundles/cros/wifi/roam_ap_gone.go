		...
		Params: []testing.Param{
			{
				...
				Val: roamTestcase{
					apOpts1:    []hostapd.Option{hostapd.Mode(hostapd.Mode80211b), hostapd.Channel(1), hostapd.BSSID(ap1BSSID)},
					apOpts2:    []hostapd.Option{hostapd.Mode(hostapd.Mode80211a), hostapd.Channel(48), hostapd.BSSID(ap2BSSID)},
					secConfFac: wpaeap.NewConfigFactory(certificate.TestCert1().CACert, certificate.TestCert1().ServerCred, wpaeap.ClientCACert(certificate.TestCert1().CACert), wpaeap.ClientCred(certificate.TestCert1().ClientCred)),
				},
			},
		},
	})
...
func RoamAPGone(ctx context.Context, s *testing.State) {
	...
	waitForProps, err := tf.ExpectShillProperty(ctx, servicePath, props)
	if err != nil {
