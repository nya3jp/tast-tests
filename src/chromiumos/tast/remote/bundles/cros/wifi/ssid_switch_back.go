...
func SSIDSwitchBack(ctx context.Context, s *testing.State) {
	...
	tryConnect := func(ctx context.Context, ops []hostapd.Option) (retPath string, retErr error) {
		...
		defer func(ctx context.Context) {
			...
			if err := wifiutil.WaitServiceIdle(ctx, tf, servicePath); err != nil {
				collectErr(errors.Wrap(err, "failed to wait for DUT leaving initial AP"))
			}
		}(ctx)
		...
		defer func(ctx context.Context) {
			if err := tf.DeconfigAP(ctx, ap); err != nil {
				collectErr(errors.Wrap(err, "failed to deconfig the initial AP"))
			}
		}(ctx)
	...
	defer func(ctx context.Context) {
		if err := tf.DeconfigAP(ctx, ap); err != nil {
			s.Error("Failed to deconfig the initial AP: ", err)
		}
	}(ctx)
	ctx, cancel = tf.ReserveForDeconfigAP(ctx, ap)
	defer cancel()
	...
	if err := tf.VerifyConnection(ctx, ap); err != nil {
		s.Fatal("Failed to verify connection to the inital AP: ", err)
	}
}
