...
func BgscanBackoff(ctx context.Context, s *testing.State) {
	...
	setupAndPing := func(ctx context.Context, bgscan bool) (ret *ping.Result, retErr error) {
		...
		if _, err := tf.WifiClient().SetBgscan(ctx, &network.SetBgscanRequest{Config: &config}); err != nil {
			return nil, errors.Wrap(err, "failed to set background scan config")
		}
		defer func(ctx context.Context) {
			s.Log("Restoring bgscan config: ", bgscanResp.Config)
			req := &network.SetBgscanRequest{
				Config: bgscanResp.Config,
			}
			if _, err := tf.WifiClient().SetBgscan(ctx, req); err != nil {
		...
		if err != nil {
			s.Fatal("Failed to ping router with bgscan: ", err)
		}
		...
		defer func(ctx context.Context) {
			...
			if err := tf.DeconfigAP(ctx, ap2); err != nil {
				s.Error("Failed to deconfig AP2: ", err)
			}
		}(ctx)
		...
		if err != nil {
			s.Fatalf("Failed to get MAC address of %s on router: %v", ap2.Interface(), err)
		}
	...
	// The lantecy thresholds in ms to match the unit of ping.Result.
	const (
		latencyBaseline = 100
	...
	if statsBgscan.MaxLatency > statsNoBgscan.MaxLatency+latencyMargin {
		s.Fatalf("Significant difference in rtt due to bgscan: %f ms > %f + %f ms",
