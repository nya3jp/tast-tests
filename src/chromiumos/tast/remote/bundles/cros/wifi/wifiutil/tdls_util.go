...
// VerifyTDLSDiscoveryPackets finds TDLS Discovery packets that deviate from standard.
func VerifyTDLSDiscoveryPackets(pcapPath string, addrs []net.HardwareAddr) ([]gopacket.Packet, error) {
	if len(addrs) != 2 {
		return nil, errors.New("Needs exactly two addresses")
	}
	// Discovery request.
