...
func DecodeRoamingConsortiums(s string) ([]string, error) {
	...
	for i := 0; i < len(b); {
		...
		i++
		if i+rcLen > len(b) {
			return nil, decodeError(nil, s, "roaming consortium length exceed buffer size")
		}
...
func DecodeVenueGroupTypeName(s string) (info hostapd.VenueInfo, names []string, err error) {
	b, err := hex.DecodeString(s)
	if err != nil {
		return info, names, decodeError(err, "failed to decode venue name hex string", s)
	}
	// The buffer should at least contain 3 bytes: the group, the type and the size of the name.
	if len(b) <= 3 {
		return info, names, decodeError(nil, s, "buffer too short for group, type and size (length=%d)", len(b))
	}
	info.Group = b[0]
	info.Type = b[1]
	...
	for i := 2; i < len(b); {
		...
		if size <= 3 {
			return info, names, decodeError(nil, s, "venue name size too short to contain a valid name")
		}
		if i+size > len(b) {
			return info, names, decodeError(nil, s, "venue name size exceeds buffer length")
		}
...
func DecodeDomainNames(s string) ([]string, error) {
	...
	for i := 0; i < len(b); {
		...
		i++
		if i+size > len(b) {
			return nil, decodeError(err, s, "domain name size exceeds buffer length")
		}
