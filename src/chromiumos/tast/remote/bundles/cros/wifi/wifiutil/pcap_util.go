...
func ScanAndCollectPcap(fullCtx context.Context, tf *wificell.TestFixture, name string, scanCount, ch int) (string, error) {
	...
	return CollectPcapForAction(fullCtx, tf, name, ch, action, false)
}
...
// returned.
func CollectPcapForAction(fullCtx context.Context, tf *wificell.TestFixture, name string, ch int, action func(ctx context.Context) error, useRouter bool) (string, error) {
	var router *wificell.Router
	if useRouter {
		router = tf.Router()
	} else {
		router = tf.Pcap()
