...
type ContTest struct {
	...
	lastScan    time.Time
	servicePath string
	ds          *destructorStack // stack for initial setup
	dsFinal     *destructorStack // stack for finalization
}
...
func ContinuityTestInitialSetup(ctx context.Context, s *testing.State, tf *wificell.TestFixture) (context.Context, *ContTest, func()) {
	...
	*/
	ct := &ContTest{tf: tf}
	ds, destroy := newDestructorStack()
	defer destroy()

	var err error
	ctx, ct.restoreBg, err = tf.TurnOffBgscan(ctx)
	if err != nil {
	...
	ds.push(func() {
		if _, err := ct.tf.WifiClient().SetGlobalFTProperty(ctx, &network.SetGlobalFTPropertyRequest{Enabled: ct.ftEnabled}); err != nil {
			s.Errorf("Failed to set global FT property back to %v: %v", ct.ftEnabled, err)
		}
	})

	ct.veth[0], ct.veth[1], err = tf.Router().NewVethPair(ctx)
	if err != nil {
		s.Fatal("Failed to get a veth pair: ", err)
	}
	ds.push(func() {
		if err := ct.tf.Router().ReleaseVethPair(ctx, ct.veth[0]); err != nil {
			s.Error("Failed to release veth: ", err)
		}
	})
	// Bind the two ends of the veth to the two bridges.
	for i := 0; i < 2; i++ {
		ct.br[i], err = tf.Router().NewBridge(ctx)
		if err != nil {
			s.Fatal("Failed to get a bridge: ", err)
		}
		if err := tf.Router().BindVethToBridge(ctx, ct.veth[i], ct.br[i]); err != nil {
	...
	ds.push(func() {
		if err := ct.tf.Router().ReleaseBridge(ctx, ct.br[0]); err != nil {
			s.Error("Failed to release bridge: ", err)
		}
		if err := ct.tf.Router().ReleaseBridge(ctx, ct.br[1]); err != nil {
			s.Error("Failed to release bridge: ", err)
		}
	})
	...
	ct.dserv, err = tf.Router().StartDHCP(ctx, ap0Name, ct.br[0], startIP, endIP, serverIP, broadcastIP, mask)
	if err != nil {
		s.Fatal("Failed to start the DHCP server: ", err)
	}
	ds.push(func() {
		if err := ct.tf.Router().StopDHCP(ctx, ct.dserv); err != nil {
...
func (ct *ContTest) ContinuityTestSetupFinalize(ctx context.Context, s *testing.State) (context.Context, func()) {
	...
	ct.ap[1], err = ct.tf.Router().StartHostapd(ctx, ap1Name, ap1Conf)
	if err != nil {
		s.Fatal("Failed to start the hostapd server on the second AP: ", err)
	}
	ds.push(func() {
		if err := ct.tf.Router().StopHostapd(ctx, ct.ap[1]); err != nil {
	...
	ct.dsFinal = ds.export()
	return ctx, ct.dsFinal.destroy
}
...
func (ct *ContTest) ContinuityRound(ctx context.Context, s *testing.State, round int) {
	...
	// A scan would be valid for 30 seconds at least, so as we want to test roaming
	// continuity, not scanning continuity, then it's better not to waste time for
	// scanning before every roaming attempt.
	if time.Since(ct.lastScan) > 30*time.Second {
		ct.lastScan = time.Now()
		discCtx, cancel := context.WithTimeout(ctx, 15*time.Second)
		defer cancel()
		if err := ct.tf.DiscoverBSSID(discCtx, ct.mac[(round+1)%2].String(), ct.iface,
			[]byte(ct.ap[(round+1)%2].Config().SSID)); err != nil {
			s.Fatalf("Failed to discover the BSSID %s: %v", ct.mac[(round+1)%2].String(), err)
		}
	}
	...
	if err := ct.tf.RequestRoam(ctx, ct.iface, ct.mac[(round+1)%2].String(), 15*time.Second); err != nil {
		s.Fatalf("Failed to roam from %s to %s: %v", ct.mac[round%2], ct.mac[(round+1)%2], err)
	}
