...
func init() {
	testing.AddTest(&testing.Test{
		...
		ServiceDeps: []string{"tast.cros.network.WifiService",
			"tast.cros.network.BluetoothService"},
		Vars: []string{"router",
			"wifi.signinProfileTestExtensionManifestKey"},
	})
}
...
func PersistenceBluetoothSansWifi(ctx context.Context, s *testing.State) {
	...
	btClient = network.NewBluetoothServiceClient(r.Conn)
	// Disable wifi
	if _, err := wifiClient.SetWifiStatus(ctx, &network.TechnologyToggleRequest{Status: false}); err != nil {
