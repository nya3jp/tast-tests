...
func Prefer5Ghz(fullCtx context.Context, s *testing.State) {
	...
	configureAP := func(ctx context.Context, channel int) (context.Context, int, func(context.Context), error) {
	...
	ctx, freq2g, deconfig2g, err := configureAP(ctx, 1)
	if err != nil {
		s.Fatal("Failed to set up AP: ", err)
	}
	defer deconfig2g(fullCtx)

	ctx, freq5g, deconfig5g, err := configureAP(ctx, 48)
	if err != nil {
		s.Fatal("Failed to set up AP: ", err)
	}
	defer deconfig5g(fullCtx)
	s.Log("AP setup done. Expecting the DUT sees the SSID on both 2.4GHz and 5GHz channels")
...
func wifiSignal(ctx context.Context, tf *wificell.TestFixture, dut *dut.DUT, ssid string) (map[int]float64, error) {
	...
	scanResult, err := iw.NewRunner(dut.Conn()).ScanDump(ctx, iface)
	if err != nil {
