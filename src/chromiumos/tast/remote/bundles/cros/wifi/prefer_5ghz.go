...
func Prefer5Ghz(ctx context.Context, s *testing.State) {
	...
	configureAP := func(ctx context.Context, ssid string, channel int) (*wificell.APIface, int, func(ctx context.Context), error) {
	...
	ap2g, freq2g, deconfig2g, err := configureAP(ctx, ssid, channel2g)
	if err != nil {
		s.Fatal("Failed to set up AP: ", err)
	}
	defer deconfig2g(ctx)
	ctx, cancel = tf.ReserveForDeconfigAP(ctx, ap2g)
	defer cancel()

	ap5g, freq5g, deconfig5g, err := configureAP(ctx, ssid, channel5g)
	if err != nil {
		s.Fatal("Failed to set up AP: ", err)
	}
	defer deconfig5g(ctx)
	ctx, cancel = tf.ReserveForDeconfigAP(ctx, ap5g)
	defer cancel()
