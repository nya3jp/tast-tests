...
func DuplicateBSSID(fullCtx context.Context, s *testing.State) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		scanData, err = remoteiw.NewRemoteRunner(s.DUT().Conn()).TimedScan(ctx, iface, freqs, ssids)
		if err != nil {
			return err
		}
		return nil
	...
	for _, ssid := range ssids {
		foundSSID = false
		for _, data := range scanData.BSSList {
			if ssid == data.SSID {
				foundSSID = true
				break
			}
		...
		if !foundSSID {
			s.Errorf("DUT: failed to find the ssid=%v in the scan", ssid)
		}
	}
