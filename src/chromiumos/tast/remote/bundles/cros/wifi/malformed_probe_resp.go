...
func MalformedProbeResp(ctx context.Context, s *testing.State) {
	// The test uses framesender to periodicly send malformed probe
...
// This seems to only work for Marvell card reset and searching the function name
...
func getCardResetCount(ctx context.Context, d *dut.DUT) (uint64, error) {
	...
	if err != nil {
		// When no match found, grep will return 1.
		var sshErr *ssh.ExitError
		if errors.As(err, &sshErr) && sshErr.ExitStatus() == 1 {
			return 0, nil
		}
