...
func MalformedProbeResp(ctx context.Context, s *testing.State) {
	// The test use framesender to periodicly send malformed probe responses
	// and trigger background scans on the same channel. Then verifies that
	// the SSID of malformed response is found and no disconnection during
	// the period.
	...
	// Get the reset count before test.
	resetCountPre, err := getCardResetCount(ctx, s.DUT())
	if err != nil {
		s.Fatal("Failed to get reset count before test: ", err)
	}
	...
	// Check if there's resets during the test.
	resetCount, err := getCardResetCount(ctx, s.DUT())
	if err != nil {
		s.Fatal("Failed to get reset count after test: ", err)
	}
	if resetCount >= resetCountPre {
		resetCount -= resetCountPre
	} else {
		// Log rotated, resetCount will be the estimation.
		s.Log("Log rotated")
	}
	if resetCount > 0 {
		s.Fatalf("Client card reset detected, count=%d", resetCount)
	}
}

// getCardResetCount returns the count of card resets until now.
// TODO(b/171763917): Ported from the Autotest version added in crbug.com/498018.
// This seems to only work for marvel card reset and searching the function name
// in log of WARN_ON is not very accurate. We might need a more general way to
// detect the reset.
func getCardResetCount(ctx context.Context, d *dut.DUT) (uint64, error) {
	resetMsg := "mwifiex_sdio_card_reset"
	out, err := d.Command("grep", "-c", resetMsg, "/var/log/messages").Output(ctx)
	if err != nil {
		return 0, errors.Wrap(err, "failed to get reset count")
	}
	count, err := strconv.ParseUint(strings.TrimSpace(string(out)), 10, 64)
	if err != nil {
		return 0, errors.Wrap(err, "failed to parse reset count")
	}
	return count, nil
}
