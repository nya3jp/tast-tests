...
func NonPrefChan(ctx context.Context, s *testing.State) {
	...
	*/
	...
	for tc, chans := range [][]wpacli.NonPrefChan{
		...
		if len(actionPackets) == 0 {
			s.Fatal("No action packets found")
		}
		expectedChanMap := make(map[uint8]wpacli.NonPrefChan)
		for _, ch := range chans {
			expectedChanMap[ch.Channel] = ch
		...
		for _, p := range actionPackets {
			...
			for i := 4; i < len(action.Contents); {
				...
				for j := 0; j < tagLen-7; j++ {
					foundChan := action.Contents[i+5+j]
					if _, chanExists := actualNonPrefChans[foundChan]; chanExists {
						s.Fatalf("Malformed non-preferred channel report. Channel %d reported multiple times", foundChan)
					}
					actualNonPrefChans[foundChan] = wpacli.NonPrefChan{
						OpClass: opClass,
						Channel: foundChan,
						Pref:    pref,
						Reason:  reason,
