...
func NonPrefChan(ctx context.Context, s *testing.State) {
	...
	*/
	...
	pcapPath, err := wifiutil.CollectPcapForAction(ctx, tf.Router(), "connect", channel, []iw.SetFreqOption{iw.SetFreqChWidth(iw.ChWidthHT20)}, connect)
	...
	for tc, chans := range [][]wpacli.NonPrefChan{
		...
		pcapPath, err = wifiutil.CollectPcapForAction(ctx, tf.Router(), fmt.Sprintf("setNonPrefChans%d", tc), channel, []iw.SetFreqOption{iw.SetFreqChWidth(iw.ChWidthHT20)}, setNonPrefChans(chans...))
		if err != nil {
