...
func NonPrefChan(ctx context.Context, s *testing.State) {
	...
	*/
	...
	pcapPath, err := wifiutil.CollectPcapForAction(ctx, tf.Router(), "connect", channel, freqOpts, connect)
		...
		for _, l := range p.Layers() {
			...
			if element.ID == layers.Dot11InformationElementIDSuppOperatingClass {
				...
				for i := 1; i < int(element.Length); i++ {
					if element.Info[i] == OpClass2GHz {
						supports2GHz = true
					} else if element.Info[i] == OpClass5GHz {
						supports5GHz = true
					}
				}
				if !supports2GHz {
			...
			if element.ID == layers.Dot11InformationElementIDVendor {
				if int(element.Length) < 19 ||
					!bytes.Equal(element.OUI[:3], ieee80211.WFAOUI) ||
					element.OUI[3] != OUITypeMBO {
					continue
				}
				containsMBO = true
				...
				for i := 0; i < len(element.Info); {
					...
					if attrID == ChanReportSubelem && attrLen == 4 {
	...
	for tc, chans := range [][]wpacli.NonPrefChan{
		...
		pcapPath, err = wifiutil.CollectPcapForAction(ctx, tf.Router(), fmt.Sprintf("setNonPrefChans%d", tc), channel, freqOpts, setNonPrefChans(chans...))
		if err != nil {
		...
		for _, p := range actionPackets {
			...
			if action.Contents[0] != WNMCategoryCode {
				continue
			}
			foundWNM = true
			...
			for i := 4; i < len(action.Contents); {
				...
				if tagNum != TagNumVendor || !bytes.Equal(action.Contents[i:i+3], ieee80211.WFAOUI) || action.Contents[i+3] != OUITypeNonPrefChanReport {
