...
const rnDebugEnabled = true

type roamNaturalTestcase struct {
	apOpts1    []hostapd.Option
	apOpts2    []hostapd.Option
	apOpts3    []hostapd.Option
	secConfFac security.ConfigFactory
}

const (
	naturalAP1BSSID = "00:11:22:33:44:55"
	naturalAP2BSSID = "00:11:22:33:44:56"
	naturalAP3BSSID = "00:11:22:33:44:57"
)
...
var roamNaturalSSID = hostapd.RandomSSID("TAST_TEST_")
		...
		Params: []testing.Param{
			{
				Val: roamNaturalTestcase{
					apOpts1:    []hostapd.Option{hostapd.Mode(hostapd.Mode80211nPure), hostapd.Channel(1), hostapd.HTCaps(hostapd.HTCapHT20), hostapd.BSSID(naturalAP1BSSID), hostapd.SSID(roamNaturalSSID)},
					apOpts2:    []hostapd.Option{hostapd.Mode(hostapd.Mode80211nPure), hostapd.Channel(2), hostapd.HTCaps(hostapd.HTCapHT20), hostapd.BSSID(naturalAP2BSSID), hostapd.SSID(roamNaturalSSID)},
					apOpts3:    []hostapd.Option{hostapd.Mode(hostapd.Mode80211nPure), hostapd.Channel(36), hostapd.HTCaps(hostapd.HTCapHT20), hostapd.BSSID(naturalAP3BSSID), hostapd.SSID(roamNaturalSSID)},
					secConfFac: nil,
				},
			},
		},
...
func simulateDutMove(ctx context.Context, s *testing.State, offsetRange [3]int, center int,
	...
	for {
		...
		attenuator.SetTotalAttenuation(ctx, 0, math.Max(atten0, minAtten), freq0)
		minAtten, err = attenuator.MinTotalAttenuation(1)
		if err != nil {
			s.Fatal("Failed to get minimal attenuation")
		}
		attenuator.SetTotalAttenuation(ctx, 1, math.Max(atten0, minAtten), freq0)
		minAtten, err = attenuator.MinTotalAttenuation(2)
		if err != nil {
			s.Fatal("Failed to get minimal attenuation")
		}
		attenuator.SetTotalAttenuation(ctx, 2, math.Max(atten1, minAtten), freq1)
		minAtten, err = attenuator.MinTotalAttenuation(3)
		if err != nil {
			s.Fatal("Failed to get minimal attenuation")
		}
		attenuator.SetTotalAttenuation(ctx, 3, math.Max(atten1, minAtten), freq1)
	...
	for center := roamNaturalMinCenter; center < roamNaturalMaxCenter; center += 2 * roamNaturalAttenStep {
		...
		for roundPass := 0; roundPass < roamNaturalRoundPassCount; roundPass++ {
			...
			for offsetRangeIdx, offsetRange := range offsetRanges {
				...
				attenuator.SetAttenuation(ctx, 0, 0)

				// bring back the AP that was stopped earlier
				var ap *wificell.APIface
				if offsetRangeIdx == 0 {
					rnDebug(s, "bringing back AP 0")
					ap0, freq0, _ = wifiutil.ConfigureAP(ctx, s, apAllParams[0], 0, secConfFac)
					ap = ap0
				} else if offsetRangeIdx == 2 {
					rnDebug(s, "bringing back AP 1")
					ap1, freq1, _ = wifiutil.ConfigureAP(ctx, s, apAllParams[1], 1, secConfFac)
					ap = ap1
				}

				rnDebug(s, "discovering AP")
				tf.DiscoverBSSID(ctx, ap.Config().BSSID, iface, []byte(ap.Config().SSID))
			}
		}
	}
}
...
func RoamNatural(ctx context.Context, s *testing.State) {
	...
	param := s.Param().(roamNaturalTestcase)
	...
	executeRoamNaturalTest(ctx, s, [][]hostapd.Option{param.apOpts1, param.apOpts2}, 0, param.secConfFac, &roamStats, &failureStats)
	executeRoamNaturalTest(ctx, s, [][]hostapd.Option{param.apOpts1, param.apOpts3}, 1, param.secConfFac, &roamStats, &failureStats)
