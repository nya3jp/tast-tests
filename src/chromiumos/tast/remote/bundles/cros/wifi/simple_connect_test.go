...
func tunneled1xPEAPAndTTLS() []params {
...
func TestSimpleConnect(t *testing.T) {
	...
	ps = append(ps, hidden())
	ps = append(ps, wep40And104()...)
	ps = append(ps, wepHidden())

	// TODO: Merge with the other params below. We are doing these because we don't want to break the order.
	var ps8021x []params
	ps8021x = append(ps8021x, tunneled1xPEAPAndTTLS()...)
	...
	para8021x := genparams.Template(t, template, ps8021x)
	para := genparams.Template(t, template+`{
	...
	genparams.Ensure(t, "simple_connect.go", para+para8021x)
}
