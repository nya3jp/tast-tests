...
type params struct {
	// Customized test. If set, all fields below are ignored.
	Customized string
...
func paramWEP() []params {
	mkOps := func(keyLen int) []paramsVal {
		var p []paramsVal
...
func paramNonASCIISSID() params {
	return params{
		Customized: `// This test case verifies that the DUT accepts ascii and non-ascii type characters as the SSID.
			Name:                 "non_ascii_ssid",
...
func paramTunneled1x() []params {
	mkFailOps := func(outer, inner string) []paramsVal {
		var p []paramsVal
	...
	mkOps := func(outer, inner string) []paramsVal {
		var p []paramsVal
		p = append(p, paramsVal{
			ApOpts: commonSecApOpts,
