...
func open80211abg() []params {
	...
	for _, tc := range []struct {
		...
		meta: params{
			Name:      "80211a",
			Doc:       "// Verifies that DUT can connect to an open 802.11a network on channels 48, 64.",
			ExtraAttr: []string{"wificell_unstable"},
		},
		valTemplate: "{apOpts: []ap.Option{ap.Mode(ap.Mode80211a), ap.Channel(%d)}},\n",
		valArgs:     []int{48, 64},
	}, {
		meta: params{
			Name: "80211b",
			Doc:  "// Verifies that DUT can connect to an open 802.11b network on channels 1, 6, 11.",
		},
		valTemplate: "{apOpts: []ap.Option{ap.Mode(ap.Mode80211b), ap.Channel(%d)}},\n",
		valArgs:     []int{1, 6, 11},
	}, {
		meta: params{
			Name: "80211g",
			Doc:  "// Verifies that DUT can connect to an open 802.11g network on channels 1, 6, 11.",
		},
		valTemplate: "{apOpts: []ap.Option{ap.Mode(ap.Mode80211g), ap.Channel(%d)}},\n",
		valArgs:     []int{1, 6, 11},
	}} {
		for _, ch := range tc.valArgs {
			tc.meta.Val += fmt.Sprintf(tc.valTemplate, ch)
		}
		ps = append(ps, tc.meta)
	}
	return ps
}
...
func open80211n() []params {
	mkOps := func(htCaps string, channels ...int) string {
		...
		for _, ch := range channels {
			ret += fmt.Sprintf("{apOpts: []ap.Option{ap.Mode(ap.Mode80211nPure), ap.Channel(%d), ap.HTCaps(%s)}},\n", ch, htCaps)
		}
		return ret
	}
	return []params{{
		Name:      "80211n24ht20",
		Doc:       "// Verifies that DUT can connect to an open 802.11n network on 2.4GHz channels 1, 6, 11 with a channel width of 20MHz.",
		ExtraAttr: []string{"wificell_unstable"},
		Val:       mkOps("ap.HTCapHT20", 1, 6, 11),
	}, {
		Name:      "80211n24ht40",
		Doc:       "// Verifies that DUT can connect to an open 802.11n network on 2.4GHz channel 6 with a channel width of 40MHz.",
		ExtraAttr: []string{"wificell_unstable"},
		Val:       mkOps("ap.HTCapHT40", 6),
	}, {
		Name: "80211n5ht20",
		Doc:  "// Verifies that DUT can connect to an open 802.11n network on 5GHz channel 48 with a channel width of 20MHz.",
		Val:  mkOps("ap.HTCapHT20", 48),
	}, {
		Name: "80211n5ht40",
		Doc: `// Verifies that DUT can connect to an open 802.11n network on 5GHz channel 48
...
func open80211nsgi() []params {
	return []params{{
		Name: "80211nsgi",
		Doc:  "// This test verifies that DUT can connect to an open 802.11n network on 5 GHz channel with short guard intervals enabled (both 20/40 Mhz).",
...
func open80211ac() []params {
	mkOps := func(mode string, channel int, htCaps, extra string) string {
		return fmt.Sprintf("{apOpts: []ap.Option{\nap.Mode(%s), ap.Channel(%d), ap.HTCaps(%s),\n%s\n}},", mode, channel, htCaps, extra)
	}
	return []params{{
		Name:              "80211acvht20",
		Doc:               "// Verifies that DUT can connect to an open 802.11ac network on channel 60 with a channel width of 20MHz.",
		Val:               mkOps("ap.Mode80211acPure", 60, "ap.HTCapHT20", "ap.VHTChWidth(ap.VHTChWidth20Or40),"),
		ExtraHardwareDeps: "hwdep.D(hwdep.Wifi80211ac())",
	}, {
		Name:              "80211acvht40",
		Doc:               "// Verifies that DUT can connect to an open 802.11ac network on channel 48 with a channel width of 40MHz.",
		Val:               mkOps("ap.Mode80211acPure", 48, "ap.HTCapHT40", "ap.VHTChWidth(ap.VHTChWidth20Or40),"),
		ExtraHardwareDeps: "hwdep.D(hwdep.Wifi80211ac())",
	}, {
		Name: "80211acvht80mixed",
		Doc:  "// Verifies that DUT can connect to an open 802.11ac network on 5GHz channel 36 with center channel of 42 and channel width of 80MHz.",
		Val:  mkOps("ap.Mode80211acMixed", 36, "ap.HTCapHT40Plus", "ap.VHTCaps(ap.VHTCapSGI80), ap.VHTCenterChannel(42), ap.VHTChWidth(ap.VHTChWidth80),"),
	}, {
		Name: "80211acvht80pure",
		Doc: `// Verifies that DUT can connect to an open 802.11ac network on channel 157 with center channel of 155 and channel width of 80MHz.
		      // The router is forced to use 80 MHz wide rates only.`,
		Val:               mkOps("ap.Mode80211acPure", 157, "ap.HTCapHT40Plus", "ap.VHTCaps(ap.VHTCapSGI80), ap.VHTCenterChannel(155), ap.VHTChWidth(ap.VHTChWidth80),"),
		ExtraHardwareDeps: "hwdep.D(hwdep.Wifi80211ac())",
	}}
}
