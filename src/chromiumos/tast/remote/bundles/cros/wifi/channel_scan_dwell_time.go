...
/* The DUT performs an active scan on each channel. The AP keeps sending beacon
frames, so that the DUT stays on the channel until maxChannelTime, and the actual
dwellTime = maxChannelTime. The actual dwellTime is calculated based on the
timestamps of the probe request and SSIDs discovered in the active scan. The
purpose of this test is to verify that minDwellTime <= dwellTime (maxChannelTime)
<= maxDwellTime. */
