...
type csdtTestcase struct {
	...
	maxChannelTimeLowerBound time.Duration
	maxChannelTimeUpperBound time.Duration
}
...
func ChannelScanDwellTime(ctx context.Context, s *testing.State) {
	...
	// The AP keeps sending beacon frames, so that the DUT stays at the frequency colleting beacon frames untile maxChannelTime.
	// The actual dwell time equals to maxChannelTime. This test verifies maxChannelTimeLowerBound <= maxChannelTime <= maxChannelTimeLowerBound.
	...
	testcases := []csdtTestcase{
		{
			...
			maxChannelTimeLowerBound: 5 * time.Millisecond,
			maxChannelTimeUpperBound: 250 * time.Millisecond,
		},
		{
			apChannel:                9,
			apOpts:                   []hostapd.Option{hostapd.Mode(hostapd.Mode80211nMixed), hostapd.HTCaps(hostapd.HTCapHT40)},
	...
	testOnce := func(ctx context.Context, s *testing.State, tc csdtTestcase) {
		// Probe requests from the DUT may not be captured in the pcap file as the AP keeps sending beacon frames.
		// To prevent this from failing the test, repeat the channel scan until the probe request is captured in the
		// pcap file, or it reached the max number of retries.
		for testCount := 1; testCount < maxRetries; testCount++ {
			...
			if err != nil {
				if testCount < maxRetries {
					s.Logf("Failed to read beacons from packet capture, testCount %d < maxRetries %d, run test again", testCount, maxRetries)
					continue
				}
				s.Fatal("Failed to read beacons from packet capture and testCount reached maxRetries : ", err)
			}
			beaconFirst := ""
			...
			if (dwellTime < tc.maxChannelTimeLowerBound) || (dwellTime > tc.maxChannelTimeUpperBound) {
				s.Fatalf("Dwell time %v is not within range [%v, %v]", dwellTime, tc.maxChannelTimeLowerBound, tc.maxChannelTimeUpperBound)
			}
