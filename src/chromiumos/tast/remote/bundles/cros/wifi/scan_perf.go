...
func ScanPerf(ctx context.Context, s *testing.State) {
	...
	*/
	...
	// Temporarily set to sufficiently long value to make those tests always pass.
	wifi6eChipSet := map[wlan.DeviceID]struct{}{
	...
	} else if _, ok := wifi6eChipSet[devID]; ok {
	...
	for i := 1; i <= scanTimes; i++ {
		if duration, err := pollTimedScan(ctx, nil, fgFullScanTimeout, pollTimeout, ssid, iface, iwr); err != nil {
			s.Fatalf("Failed to perform full channel scan at frequency %d: %v", freq, err)
		} else {
