...
func ScanPerf(fullCtx context.Context, s *testing.State) {
	...
	pollTimedScan := func(ctx context.Context, freqs []int) (time.Duration, error) {
	...
	// Single channel scan.
	if duration, err := pollTimedScan(ctx, []int{freq}); err != nil {
	...
	if duration, err := pollTimedScan(ctx, nil); err != nil {
