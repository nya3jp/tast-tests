...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Verifies that the DUT can reconnect to a autoconnectable AP during dark resume",
		Contacts: []string{
		...
		HardwareDeps: hwdep.D(hwdep.Platform("volteer")),
...
func ReconnectInDarkResume(ctx context.Context, s *testing.State) {
	// This test sets up two autoconnectable services on DUT and put the DUT
	// into suspend. Then, deconfigure and reconfigure the AP, and make sure
	// the DUT can properly reconnect to the autoconnect services in dark
	...
	deconfigAP := func(ctx context.Context) error {
		if ap == nil {
			return nil
		}
	...
	if err != nil {
		s.Fatal("Failed to configure AP to reconnect: ", err)
	}
	...
	// Assert that the DUT doesn't fully wake up.
	if err := dut.WaitUnreachable(ctx); err != nil {
		s.Fatal("Failed to wait for DUT to become unreachable after reconnect: ", err)
	}
