...
func init() {
	testing.AddTest(&testing.Test{
		...
		ServiceDeps:  []string{"tast.cros.browser.ChromeService", "tast.cros.shimlessrma.Service"},
...
func DisableHWWPWithBatteryDisconnection(ctx context.Context, s *testing.State) {
	...
	client := pb.NewServiceClient(cl.Conn)
	if _, err := client.NewShimlessRMA(ctx, request, grpc.WaitForReady(true)); err != nil {
		s.Fatal("Failed to start Chrome: ", err)
	}
	s.Log("New ShimessRMA successfully")
	defer client.CloseShimlessRMA(ctx, &empty.Empty{})
