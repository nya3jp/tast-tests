...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Verifies connected cable type is passive cable or active cable",
...
func isDeviceEnumerated(ctx context.Context, dut *dut.DUT, device, port string) (bool, error) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		out, err := dut.Conn().CommandContext(ctx, "cat", deviceNameFile).Output()
		if err != nil {
			return errors.Wrapf(err, "failed to execute 'cat %s' command", deviceNameFile)
		}
