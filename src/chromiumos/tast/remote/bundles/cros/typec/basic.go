...
// DP alternate mode support with the selected pin configuration setting.
...
func checkForDPAltMode(ctx context.Context, d *dut.DUT, s *testing.State, pinAssign string) error {
	...
	for _, device := range strings.Split(string(out), "\n") {
		...
		if out, err := d.Command("cat", svidPath).Output(ctx); err != nil {
			return errors.Wrap(err, "couldn't read alt mode regex")
		...
		if pinAssign == "c" {
			if vdoVal&pinCBitMask != 0 {
				return nil
			}
		} else if pinAssign == "d" {
			if vdoVal&pinDBitMask != 0 {
				return nil
			}
		}
	}
