...
func HidWake(ctx context.Context, s *testing.State) {
	...
	pxy, _ := servo.NewProxy(ctx, servoSpec, d.KeyFile(), d.KeyDir())
	cl, _ := rpc.Dial(ctx, d, s.RPCHint())
	fs := dutfs.NewClient(cl.Conn)
	svo := pxy.Servo()

	// Turn on the servo keyboard emulator and wait for the kernel to register it
	s.Log("Turning on servo keyboard emulator")
	if err := svo.SetOnOff(ctx, servo.USBKeyboard, servo.On); err != nil {
		s.Fatal("Failed to enable the servo keybpoard emulator: ", err)
	}

	if err := testing.Sleep(ctx, 2*time.Second); err != nil {
		s.Fatal("Failed to wait for servo keyboard emulator to turn on: ", err)
	}
	...
	if !d.Connected(ctx) {
		s.Fatal("DUT is not connected after first suspend")
	} else {
		s.Log("DUT reconnected after first suspend")
	}
	...
	if !d.Connected(ctx) {
		s.Fatal("DUT is not connected after second suspend")
	} else {
		s.Log("DUT reconnected after second suspend")
	}
	...
	// Compare initial and updated wake count for the servo
	if count2 <= count1 {
		s.Fatal("Wake count did not increase during test")
	} else {
		s.Log("Wake count successfully incremented")
	}
