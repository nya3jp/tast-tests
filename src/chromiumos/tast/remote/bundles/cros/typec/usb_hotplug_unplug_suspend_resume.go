...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			// Timeout deadline extended due to multiple SSH disconnect/connect
			// 1 hour = ((2 * 5 sec sleep + 15 sec poll + about 30 sec ssh reconnect + 5 sec buffer) * 4 per iteration + 1 min buffer) * 10 iterations + 10 min buffer
			Timeout: 1 * time.Hour,
		}},
	})
}
...
func UsbHotplugUnplugSuspendResume(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Failed to get USB mux state: ", err)
	}
	...
	// TODO(b/226235556): Skip test if usb connect/disconnect cannot be simulated. (not pass)
	// Once hardware/software dependency take servo into account, move this check to a dependency to skip the test when changes in usb mux state does not connect/disconnect any USB devices.
	if len(usbDevices) == 0 {
...
func testSuspend(ctx context.Context,
	...
	}, &testing.PollOptions{Timeout: longWaitDelay * time.Second})
}
