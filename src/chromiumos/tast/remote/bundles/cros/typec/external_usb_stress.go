...
func ExternalUSBStress(ctx context.Context, s *testing.State) {
	...
	if !checkUSBDisconnectAndConnect(logsBefore, logsAfter, s) {
		s.Fatal("Failed detect USB device disconnect/connect")
	}
...
func testSuspend(ctx context.Context,
	...
	if !checkUSBDisconnectAndConnect(logsBefore, logsAfter, s) &&
		...
		s.Fatal("Failed to detect USB device disconnect/connect")
	}
}
...
// checkUSBDisconnectAndConnect compares two logs and return true if there
// exists USB device disconnect and connect log.
func checkUSBDisconnectAndConnect(logsBefore, logsAfter []string, s *testing.State) bool {
