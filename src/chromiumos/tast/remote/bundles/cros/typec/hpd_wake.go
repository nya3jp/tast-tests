...
func HpdWake(ctx context.Context, s *testing.State) {
	...
	go func(ctx context.Context) {
		out, _ := d.Conn().CommandContext(ctx, "powerd_dbus_suspend", "--timeout=120").CombinedOutput()
	...
	if wakesAfter != wakesBefore+1 {
		s.Fatalf("Didn't register wakeup event, before: %d, after: %d", wakesBefore, wakesAfter)
	}
