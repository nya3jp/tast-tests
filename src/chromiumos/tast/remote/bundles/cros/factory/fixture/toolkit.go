...
const (
	...
	// FIXME(b/205779346): disk_space_hacks.sh removes data still need in lab test
...
func installFactoryToolKit(ctx context.Context, conn *ssh.Conn) (version string, err error) {
	...
	// FIXME(b/205779346): workaround to prevent disable disk_space_hacks.sh
	// from deleting directories needed by other test
