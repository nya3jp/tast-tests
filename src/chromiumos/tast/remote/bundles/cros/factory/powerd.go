...
func Powerd(ctx context.Context, s *testing.State) {
	...
	checkPowerdInFactoryMode(ctx, s, conn.Conn)
}
...
func checkPowerdInFactoryMode(ctx context.Context, s *testing.State, conn *grpc.ClientConn) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		s.Fatal("powerd not in factory mode: ", err)
	}
}
