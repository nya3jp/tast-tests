...
func init() {
	testing.AddTest(&testing.Test{
		...
		ServiceDeps: []string{"tast.cros.meta.OutputService"},
	})
}
...
func ServiceOutput(ctx context.Context, s *testing.State) {
	...
	oc := meta.NewOutputServiceClient(cl.Conn)
	if _, err := oc.SaveOutputs(ctx, &meta.SaveOutputsRequest{Files: want}); err != nil {
