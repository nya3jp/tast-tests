...
func coldbootStartupPerfOnce(ctx context.Context, s *testing.State, i, iterations int) {
	...
	s.Run(ctx, "BrowserLaunch", func(ctx context.Context, s *testing.State) {
		...
		flags := []string{
			...
			"-var=lacros.StartupPerf.username=" + username,
			"-var=lacros.StartupPerf.password=" + password,
			"-var=skipInitialLogin=true",
		}

		variantName := strings.Split(s.TestName(), ".")[2]
		_, _, err := tastrun.Exec(ctx, s, "run", flags, []string{"lacros.StartupPerf." + variantName})
		if err != nil {
