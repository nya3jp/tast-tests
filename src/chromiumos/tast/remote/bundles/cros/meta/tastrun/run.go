...
// SkipPolicy says whether it's okay for tests to get skipped.
type SkipPolicy string

// The values of type SkipPolicy.
const (
	SkipPolicyAllowSkipping    = "allowSkippingTests"
	SkipPolicyDisallowSkipping = "disallowSkippingTests"
)

// RunAndEvaluate runs tests and checks the results, propagating any test failure. It returns (the names of) any skipped tests.
func RunAndEvaluate(ctx context.Context, s *testing.State, flags, patterns []string, resultsDir string, policy SkipPolicy) []string {
	// Run tests.
	...
	if len(skippedTests) > 0 && policy == SkipPolicyDisallowSkipping {
