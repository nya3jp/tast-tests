...
func ServerVarsRemote(ctx context.Context, s *testing.State) {
	...
	cacheServerHosts, err := verifyServerVars(servers.Cache)
	if err != nil {
		s.Error("Failed to verify cache server variable: ", err)
	}
	testing.ContextLog(ctx, "Cache server hosts Info: ", cacheServerHosts)
}
...
func verifyServerVars(serverType servers.ServerType) (map[string]string, error) {
	...
	for role, host := range hosts {
		h, _, err := servers.Server(serverType, role)
		if err != nil {
			return nil, errors.Wrapf(err, "failed to get host with role %s", role)
		}
		if h != host {
			return nil, errors.Wrapf(err, "got host %s for role %s; wanted %s", h, role, host)
		}
	}
	return hosts, nil
}
