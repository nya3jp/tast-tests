...
type testCase struct {
	...
	isValidSkew bool // true if it's a valid supporting skew
}
...
func UpdateStatefulToRootfs(ctx context.Context, s *testing.State) {
	...
	isValidSkew := s.Param().(testCase).isValidSkew
	...
	} else if isValidSkew != statefulLacrosVersion.IsSkewValid(ashVersion) {
		s.Fatalf("Invalid Stateful Lacros version: %v, not expected skew to Ash: %v, should be a valid skew? %v", statefulLacrosVersion, ashVersion, isValidSkew)
	}
