...
func init() {
	testing.AddTest(&testing.Test{
		...
		// TODO(hyungtaekim): Enable the test in CQ/CI by uncommenting Attr when ready.
...
func UpdateRootfsToStateful(ctx context.Context, s *testing.State) {
	...
	utsClient := lacros.NewUpdateTestServiceClient(conn.Conn)
...
func getRootfsLacrosVersion(ctx context.Context, dut *dut.DUT, utsClient lacros.UpdateTestServiceClient) (version.Version, error) {
...
func clearLacrosUpdate(ctx context.Context, utsClient lacros.UpdateTestServiceClient) error {
	if _, err := utsClient.ClearUpdate(ctx, &lacros.ClearUpdateRequest{}); err != nil {
