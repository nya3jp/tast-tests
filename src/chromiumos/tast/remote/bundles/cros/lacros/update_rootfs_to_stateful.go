...
func UpdateRootfsToStateful(ctx context.Context, s *testing.State) {
	...
	if !statefulLacrosVersion.IsValid() || !statefulLacrosVersion.IsNewerThan(rootfsLacrosVersion) {
...
func getRootfsLacrosVersion(ctx context.Context, dut *dut.DUT, utsClient lacrosservice.UpdateTestServiceClient) (version.Version, error) {
	...
	rootfsLacrosVersion := version.Parse(res.Versions[0])
	if !rootfsLacrosVersion.IsValid() {
		return version.Version{}, errors.Wrap(err, "invalid Rootfs Lacros version")
	}
	return rootfsLacrosVersion, nil
}
