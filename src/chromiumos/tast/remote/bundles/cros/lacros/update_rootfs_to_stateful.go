...
func UpdateRootfsToStateful(ctx context.Context, s *testing.State) {
	...
	if err := provisionLacrosFromRootfsLacrosImagePath(ctx, provision.TLSAddrVar.Value(), dut, statefulLacrosVersion.GetString(), common.UpdateComponentVar.Value()); err != nil {
	...
	if err := verifyLacrosUpdate(ctx, statefulLacrosVersion.GetString(), common.UpdateComponentVar.Value(), utsClient); err != nil {
...
func provisionLacrosFromRootfsLacrosImagePath(ctx context.Context, tlsAddr string, dut *dut.DUT, overrideVersion, overrideComponent string) error {
	...
	return tlsClient.ProvisionLacrosFromDeviceFilePath(
		ctx, dutName,
		common.RootfsLacrosImageFileURL, overrideVersion, filepath.Join(common.LacrosRootComponentPath, overrideComponent))
}
...
func verifyLacrosUpdate(ctx context.Context, overrideVersion, overrideComponent string, utsClient lacros.UpdateTestServiceClient) error {
	...
	ashCtx := &lacrosservice.BrowserContext{
		...
		Opts: []string{
			...
			"--component-updater=url-source=" + common.BogusComponentUpdaterURL, // Block Component Updater.
		},
	}
