...
func CameraboxLoLOff(ctx context.Context, s *testing.State) {
	...
	powercycleDirPath, powercycleFilePath, err := utils.CreateTmpDir(ctx, d.Conn())
	if err != nil {
		s.Fatal("Tmp dir creation failed on DUT")
	}
	filePaths, err := utils.SendImageTar(ctx, d.Conn(), archive, powercycleDirPath, powercycleFilePath)
	...
	req := &pb.NewHpsSettingRequest{
		Setting: "Lock on Leave",
		Enable:  false,
	}
	client := pb.NewHpsServiceClient(cl.Conn)
	if _, err := client.ChangeSettings(hctx.Ctx, req, grpc.WaitForReady(true)); err != nil {
