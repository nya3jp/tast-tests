...
func (f *hpsdCustomFirmwareFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	// Use -n in case the .orig file already exists (perhaps left over from a previous failed cleanup).
	// In that case we will leave it as is and use it in TearDown().
	cmd := dut.Conn().CommandContext(ctx, "cp", "-p", "-n", overrideFilename, overrideOrigFilename)
