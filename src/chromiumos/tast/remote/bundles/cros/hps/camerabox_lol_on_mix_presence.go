...
func CameraboxLoLOnMixPresence(ctx context.Context, s *testing.State) {
	...
	testing.ContextLog(ctx, "initialBrightness: ", initialBrightness)
	testing.ContextLog(ctx, "dimmedBrightness: ", dimmedBrightness)
	...
	err = expectBrightnessChange(ctx, dimmedBrightness, initialBrightness, startTime, 1*time.Second, dut.Conn())
	if err != nil {
	...
	err = expectBrightnessChange(ctx, initialBrightness, dimmedBrightness, undimTime, quickDimMetrics.DimDelay.AsDuration(), dut.Conn())
	if err != nil {
...
func expectBrightnessChange(ctx context.Context, initialBrightness, expectedBrightness float64, startTime time.Time, maxDuration time.Duration, conn *ssh.Conn) error {
	err := utils.PollForBrightnessChange(ctx, initialBrightness, maxDuration, conn)
	if err != nil {
		return err
	}
	endTime := time.Now()
