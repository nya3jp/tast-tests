...
func CameraboxLoLOnMixPresence(ctx context.Context, s *testing.State) {
	...
	testing.Sleep(ctx, quickDimMetrics.ScreenOffDelay.AsDuration()+utils.ExtraDelay())
	newBrightness, err = utils.GetBrightness(hctx.Ctx, d.Conn())
	if err != nil {
		s.Fatal("Error when getting brightness: ", err)
	}
	if newBrightness != 0 {
		s.Fatal("Sceren not off")
	}

	testing.Sleep(ctx, quickDimMetrics.LockDelay.AsDuration()+utils.ExtraDelay())
