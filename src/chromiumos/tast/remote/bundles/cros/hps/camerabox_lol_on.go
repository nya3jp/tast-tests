...
func CameraboxLoLOn(ctx context.Context, s *testing.State) {
	...
	dimDelay, screenOffDelay, lockDelay := getDelayForPresence(presenceNo.numOfPerson, quickDimMetrics)
	if err != nil {
	...
	testing.Sleep(ctx, screenOffDelay+utils.ExtraDelay())
	...
	testing.Sleep(ctx, lockDelay+utils.ExtraDelay())
...
// getDelayForPresence returns the expected dim time depending on the human presence.
func getDelayForPresence(numPresence int, dimSettings *pb.RetrieveDimMetricsResponse) (time.Duration, time.Duration, time.Duration) {
	if numPresence == 0 {
