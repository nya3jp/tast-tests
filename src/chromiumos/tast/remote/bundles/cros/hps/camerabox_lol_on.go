...
func CameraboxLoLOn(ctx context.Context, s *testing.State) {
	...
	quickDimEnabledReq := &wrappers.BoolValue{
		Value: presenceNo.numOfPerson == utils.ZeroPresence,
	}
	quickDimMetrics, err := client.RetrieveDimMetrics(hctx.Ctx, quickDimEnabledReq)
...
func delayForPresence(numPresence string, dimSettings *pb.RetrieveDimMetricsResponse) (time.Duration, time.Duration) {
	...
	// When a human is present, the screen off delay is 30s from dim.
	return dimSettings.DimDelay.AsDuration() * 2, 30 * time.Second
}
