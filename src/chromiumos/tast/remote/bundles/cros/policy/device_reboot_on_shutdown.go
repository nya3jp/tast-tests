...
import (
	...
	"chromiumos/tast/remote/bundles/cros/policy/power"
...
func DeviceRebootOnShutdown(ctx context.Context, s *testing.State) {
	...
	defer func(ctx context.Context) {
		if err := power.EnsureDUTisOn(ctx, d, pxy.Servo()); err != nil {
	...
	for _, tc := range []struct {
		...
		s.Run(ctx, tc.name, func(ctx context.Context, s *testing.State) {
			...
			err = dututils.EnsureDisconnected(ctx, d, 2*time.Minute)

			if err == nil && tc.wantRestart {
				s.Error("Device didn't restarted while it should be")
			}
			if err != nil && !tc.wantRestart {
				s.Error("Device restarted while it should not be: ", err)
			}
		})
	}
}
