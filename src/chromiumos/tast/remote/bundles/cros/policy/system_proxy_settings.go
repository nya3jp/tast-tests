...
func init() {
	testing.AddTest(&testing.Test{
		...
		Contacts: []string{"acostinas@google.com",
			"pmarko@chromium.org",
			"omorsi@chromium.org", "hugobenichi@chromium.org",
		},
...
func SystemProxySettings(ctx context.Context, s *testing.State) {
	...
	activeProcessCount := func(path string) int {
		...
		if err != nil {
			// pidof() return status 1 when no pid if found for path.
			return 0
		}
		outMain := string(out)
		if len(out) == 0 {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if param.p.Val.SystemProxyEnabled == true {
				// Expect two worker processes: one which tunnels system traffic and one for ARC++ traffic.
				if workerProcessCount != 2 {
					s.Errorf("Unexpected number of worker processes running: %t", workerProcessCount)
				}
			} else {
