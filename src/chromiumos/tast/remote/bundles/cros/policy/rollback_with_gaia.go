...
// to enable the DUT to recieve updates.
...
func signBoardName(ctx context.Context, client aupb.UpdateServiceClient) ([]byte, error) {
	// Get board name from /etc/lsbJSON-release.
	response, err := client.LSBReleaseContent(ctx, &empty.Empty{})
	if err != nil {
		return []byte(nil), errors.Wrap(err, "failed to check for updates")
	}

	var lsb map[string]string
	if err := json.Unmarshal(response.ContentJson, &lsb); err != nil {
		return []byte(nil), errors.Wrap(err, "failed to unmarshal lsb-relese content")
	}

	board, ok := lsb[lsbrelease.Board]
	if !ok {
		return []byte(nil), errors.New("failed to determine DUT board")
	}
	...
	if err := json.Unmarshal(response.ContentJson, &statefulLsb); err != nil {
		return []byte(nil), errors.Wrap(err, "failed to unmarshal stateful lsb-relese content")
	}
	statefulLsb[lsbrelease.Board] = signedBoardName

	newStatefulLsbJSON, err := json.Marshal(statefulLsb)
	if err != nil {
		return []byte(nil), errors.Wrap(err, "failed to serialize stateful lsb-release content")
	}

	if _, err := client.OverwriteStatefulLSBRelease(ctx, &aupb.LSBRelease{ContentJson: newStatefulLsbJSON}); err != nil {
		return []byte(nil), errors.Wrap(err, "failed to overwrite lsb-release in the stateful partition")
	}
