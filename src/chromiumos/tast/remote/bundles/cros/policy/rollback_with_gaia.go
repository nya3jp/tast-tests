...
func init() {
	testing.AddTest(&testing.Test{
		...
		ServiceDeps:  []string{"tast.cros.policy.PolicyService", "tast.cros.policy.UpdateService"},
...
func RollbackWithGaia(ctx context.Context, s *testing.State) {
	...
	func(ctx context.Context) {
		...
		// Enable the DUT to recieve updates from Gaia.
		updateClient := ppb.NewUpdateServiceClient(cl.Conn)
		...
		if lsbOverwrite.Board != signedBoardName {
			...
			if _, err := updateClient.OverwriteLSBRelease(ctx, &ppb.LSBRelease{Board: signedBoardName}); err != nil {
		...
		// Update DUT.
		if _, err := updateClient.CheckForUpdate(ctx, &ppb.UpdateRequest{}); err != nil {
			s.Fatal("Failed to check for updates: ", err)
		}
