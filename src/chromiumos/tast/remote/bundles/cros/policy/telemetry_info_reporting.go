...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{
			{
				Name: "telemetry_info_reporting_enabled",
				...
				Name: "telemetry_info_reporting_disabled",
				Val: telemetryInfoReportingParameters{
...
type VerifyEvent func(event reportingutil.InputEvent) (b bool)
type TestType int
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		for _, internalParam := range []struct {
			...
			s.Run(ctx, internalParam.name, func(ctx context.Context, s *testing.State) {
				...
				if !param.reportingEnabled && len(prunedEvents) > 0 {
					s.Errorf("events found when reporting is disabled for %t", internalParam.name)
				}
				if param.reportingEnabled && len(prunedEvents) > 1 {
					s.Errorf("more than one event reporting at test %t", internalParam.name)
				}
				if param.reportingEnabled && len(prunedEvents) == 0 {
					s.Errorf("no events found while reporting enabled at test %t", internalParam.name)
				}
