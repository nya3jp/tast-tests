...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Notifications of DeviceMinimumVersion policy when device has reached end-of-life",
		Contacts: []string{
...
func DeviceMinimumVersionNotifications(ctx context.Context, s *testing.State) {
	...
	// Notification Id hardcoded in Chrome
	...
	if _, err = pc.EvalExpressionInChromeURL(ctx, &ps.EvalExpressionInChromeUrlRequest{
		...
		s.Error("Failed to verify end-of-life banner on management settings: ", err)
	}
}
