...
func TestSomething(ctx context.Context, s *testing.State) {
	...
	if v := os.Getenv("HOME"); v != "" {
		fileName = filepath.Join(v, ".config", "gcloud", f)
	}
	// Else, fall back to user.Current:
	if u, err := user.Current(); err == nil {
		fileName = filepath.Join(u.HomeDir, ".config", "gcloud", f)
	}
	...
	if err != nil {
		s.Fatal("Failed to find default credentials: ", err, " ", fileName, " ", filename)
	}
	s.Log(creds)
