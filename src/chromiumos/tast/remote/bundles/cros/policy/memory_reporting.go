...
func pruneEvents(ctx context.Context, events []reportingutil.InputEvent, clientID string, testStartTime time.Time) ([]reportingutil.InputEvent, error) {
	var prunedEvents []reportingutil.InputEvent
	for _, event := range events {
		...
		enqueueTime := time.Unix(us/(int64(time.Second)/int64(time.Microsecond)), 0)
...
func MemoryReporting(ctx context.Context, s *testing.State) {
	...
	if err = testing.Sleep(ctx, 60*time.Second); err != nil {
		s.Fatal("Failed to wait for reporting to reach server: ", err)
	}
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err = validateTMEInfo(ctx, *tmeInfo(events[0]), param.vProSpecific); err != nil {
		...
		s.Errorf("Failed to validate heartbeat event: %v:", errors.Wrap(err, "error when polling the reporting API"))
	}
}
