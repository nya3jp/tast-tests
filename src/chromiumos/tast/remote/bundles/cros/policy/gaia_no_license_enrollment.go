...
func GAIANoLicenseEnrollment(ctx context.Context, s *testing.State) {
	...
	if _, err := pc.StartNewChromeReader(ctx, &ps.StartNewChromeReaderRequest{}); err != nil {
	...
	checkChromeLog := func() {
		syslogErr <- func() error {
			if _, err := pc.WaitForEnrollmentError(ctx, &ps.WaitForEnrollmentErrorRequest{}); err != nil {
