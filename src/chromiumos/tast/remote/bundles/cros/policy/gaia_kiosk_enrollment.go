...
func GAIAKioskEnrollment(ctx context.Context, s *testing.State) {
	...
	accManager, acc, err := tape.NewOwnedTestAccountManagerFromClient(ctx, tapeClient, false, tape.WithTimeout(int32(gaiaKioskEnrollmentTimeout.Seconds())), tape.WithPoolID(poolID))
	if err != nil {
