...
func init() {
	testing.AddTest(&testing.Test{
		...
		Attr:         []string{"group:enrollment", "group:mainline", "informational"},
...
func SystemTimezone(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		p        policy.SystemTimezone // p is the policy we test.
		timezone string                // timezone is a short string of the timezone set by the policy.
	}{
		{
			name:     "Berlin",
			p:        policy.SystemTimezone{Val: "Europe/Berlin"},
			timezone: "CEST",
		},
		{
			name:     "Los Angeles",
			p:        policy.SystemTimezone{Val: "America/Los_Angeles"},
			timezone: "PDT",
		},
	} {
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if ret.Timezone != param.timezone {
				s.Errorf("Unexpected timezone %s: got %q; want %q", param.name, ret.Timezone, param.timezone)
			}
			s.Logf("timezone %s: %s", param.name, ret.Timezone)
		})
	}
}
