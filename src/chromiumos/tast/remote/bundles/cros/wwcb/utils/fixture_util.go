...
func InitFixture(ctx context.Context) error {
	...
	if err != nil {
		return error.wrap(err, "failed to get port list")
	}

	if len(ports) == 0 {
		return error.wrap("No serial ports found!")
	}
	...
	for _, port := range ports {
		...
		usb_port, err := serial.Open(port, mode)
		if err != nil {
			return error.wrap(err)
		}

		var t time.Duration = 3 * time.Second
		usb_port.SetReadTimeout(t)

		_, err = usb_port.Write([]byte("i"))
		if err != nil {
			return error.wrap(err)
		}
		...
		for {
			...
			for _, s := range res {
				if len(s) > 22 && strings.Count(s[0:15], "_") == 2 {
					serial := s[0:15]
					fixture_id := fmt.Sprintf("test fixture id: %s => port: %s \n", serial, port)
					testing.ContextLog(ctx, fixture_id)
					...
					if found {
						...
						s := fmt.Sprintf("serial %s is not in fixture list.", serial)
...
func ControlFixture(ctx context.Context, uid string, cmd string) error {
	...
	_, err = usb_port.Write([]byte(cmdNum))
	if err != nil {
		return error.wrap(err)
	}
