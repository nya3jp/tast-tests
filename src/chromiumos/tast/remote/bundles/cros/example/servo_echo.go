...
func ServoEcho(ctx context.Context, s *testing.State) {
	...
	// TODO(CL): Just hard-coding this for now
	servodBotAddress := "chromeos15-row14a-rack6-host5-servo.cros"
	localPort := 1236
	remotePort := 9999
	localServodTunnelAddr := "127.0.0.1:1236"

	opts := host.SSHOptions{Hostname: servodBotAddress, Port: 22, User:"root", KeyFile:"/home/jeffcarp/.ssh/testing_rsa"}
	ssh, err := host.NewSSH(ctx, &opts)
	if err != nil {
		s.Fatal("error setting up SSH for servod proxy: ", err)
	}

	proxy, err := servo.NewServodProxy(ctx, localPort, remotePort, ssh, s)
	if err != nil {
