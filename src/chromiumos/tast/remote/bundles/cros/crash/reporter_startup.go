...
func ReporterStartup(ctx context.Context, s *testing.State) {
	...
	fixture := crash_service.NewFixtureServiceClient(cl.Conn)
	_, err = fixture.DisableCrashFilter(ctx, &empty.Empty{})
	if err != nil {
	...
	expectedCorePattern := fmt.Sprintf("|%s --user=%%P:%%s:%%u:%%g:%%e", commoncrash.CrashReporterPath)
	...
	if err != nil || !os.FileMode(flagInfo.Mode).IsRegular() {
		s.Error("Crash reporter enabled file flag is not present at ", commoncrash.CrashReporterEnabledPath)
	}
