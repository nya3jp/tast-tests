...
func Kexec(ctx context.Context, s *testing.State) {
	...
	if err := d.Conn().CommandContext(ctx, "kexec", "-s", "-l", "/boot/vmlinuz", "--command-line="+string(cmdline)+KexecTest).Run(); err != nil {
	...
	if !kexecRegexp.Match(f) {
		s.Fatal("Boot was not triggered by kexec, ramoops")
	}
	...
	if !cmdlineRegexp.Match(cmdline) {
		s.Fatal("Boot was not triggered by kexec, cmdline")
	}
