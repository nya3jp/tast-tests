...
func Kexec(ctx context.Context, s *testing.State) {
	...
	kimage := "/boot/vmlinuz"
	output, err := d.Conn().CommandContext(ctx, "sh", "-c", "ls /boot/Image*").CombinedOutput()
	if err != nil {
		s.Log(string(output))
	} else {
		kimage = strings.TrimSpace(string(output))
	}

	// Load the same kernel, and kexec()
	if output, err = d.Conn().CommandContext(ctx, "kexec-lite", "--reboot", "--kernel", kimage, "--cmdline="+string(cmdline)+KexecTest).CombinedOutput(); err != nil {
		s.Log(string(output))
		if !strings.Contains(err.Error(), "remote command exited without exit status") {
	...
	if !cmdlineRegexp.Match(cmdline) {
		s.Log(string(cmdline))
		s.Fatal("Boot was not triggered by kexec, checking cmdline")
	}
}
