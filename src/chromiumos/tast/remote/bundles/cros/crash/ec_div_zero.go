...
// EcDivZero verifies that crash files are generated when the EC crashes.
func EcDivZero(ctx context.Context, s *testing.State) {
	...
	if out, err := d.Command("logger", "Running EcDivZero").CombinedOutput(ctx); err != nil {
	...
	s.Log("Setting ec regexp")
	if err := pxy.Servo().SetEcRegexp(ctx, "['Divide by 0']"); err != nil {
		s.Fatal("Failed to set ec regexp")
	}
