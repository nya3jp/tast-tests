...
func getMatchingFiles(ctx context.Context, d *dut.DUT, glob string) (map[string]bool, error) {
	...
	args := []string{systemCrashDir, "-mindepth", "1", "-maxdepth", "1", "-size", "+0", "-name", glob, "-print0"}
	out, err := d.Command("find", args...).CombinedOutput(ctx)
	if err != nil {
...
func KernelCrash(ctx context.Context, s *testing.State) {
	...
	if out, err := d.Command("sh", "-c", cmd).CombinedOutput(ctx); err != nil {
		s.Fatalf("Failed to panic DUT: %v: %s", err, out)
	}
