...
// checkFileExists checks if the given file exists on the device under test.
func checkFileExists(ctx context.Context, d *dut.DUT, filename string) bool {
	cmd := fmt.Sprintf("test -e %s", shutil.Escape(filename))
...
// 'crash-test-in-progress' is a tmpfs file that gets checked by crash_reporter for enabling features under test and gets cleared after reboot.
func initCrashReporter(ctx context.Context, d *dut.DUT) bool {
	cmd := `touch /run/crash_reporter/crash-test-in-progress; /sbin/crash_reporter --init`
...
// reboot reboots the device under test, waits for it to boot up and reconnect once the device is up and running.
