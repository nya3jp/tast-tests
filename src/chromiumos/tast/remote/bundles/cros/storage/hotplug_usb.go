...
func HotplugUSB(ctx context.Context, s *testing.State) {
	...
	if err := svo.SwitchUSBKeyPower(ctx, "off"); err != nil {
		s.Fatalf("Failed to turn off usb key: %v", err)
	}
	time.Sleep(2 * time.Second) // Added 2 secs delay between SwitchUSBKeyPower and SwitchUSBKey
	...
	s.Logf("USB devices before plug: %v", devsBeforePlug)
	s.Log("Hot plug usb device")
	if err := svo.SwitchUSBKey(ctx, "dut"); err != nil {
		s.Fatalf("Failed to switch usb key: %v", err)
	}
	devsAfterPlug, err := getUSBDevices()
	if err != nil {
		s.Fatal("Failed to get usb devices:", err)
	}
	s.Logf("USB devices after plug: %v", devsAfterPlug)
