...
func ECHibernate(ctx context.Context, s *testing.State) {
	...
	ok, err := pxy.Servo().GetBool(ctx, "charger_attached")
	if err != nil {
	...
	var regexEC = []string{"."}
	_, errEC := pxy.Servo().RunECCommandGetOutput(waitECCtx, "version", regexEC)
	if _, isFault := errEC.(xmlrpc.FaultError); isFault {
	...
	out, err := pxy.Servo().RunECCommandGetOutput(waitECCtx, "chgstate", regexChgstate)
	if err != nil {
		s.Fatal("Failed to retrieve charge-state info: ", err)
	}
	if len(out) < 1 {
		s.Fatal("Failed to get output from EC command")
	}
	if len(out[0].([]interface{})) < 2 {
		s.Fatal("Failed to get output from EC command")
	}
	result := out[0].([]interface{})[1]
	if result != "1" {
		s.Fatal("Battery is not charging")
	}
	s.Logf("Battery is charging. Value of ac: %q", result)
