...
func CheckServoKeyPresses(ctx context.Context, s *testing.State) {
	...
	go func(wg *sync.WaitGroup) {
		...
		path := filepath.Join(s.OutDir(), "raw_evdev_events.txt")
		if err := ioutil.WriteFile(path, []byte(res.Keys), 0644); err != nil {
			s.Error("Failed to save the keyboard output: ", err)
		}
	}(&wg)

	// TODO(kmshelton): Make utils.ReadServoKeyboard a streaming RPC, so it
	// can tell the test when it has started listening for keys, to avoid a
	// race condition here of the keys being sent before the utils service
	// is listening.
