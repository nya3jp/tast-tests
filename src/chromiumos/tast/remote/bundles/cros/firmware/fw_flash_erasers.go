...
func prepareJunkImage(ctx context.Context, conn *ssh.Conn,
	biosImagePath, junkImagePath, ddTemplate string, sectionSize int) error {
...
func FwFlashErasers(ctx context.Context, s *testing.State) {
	...
	if err := flashromRead(ctx, s.DUT().Conn(), string(biosImagePath)); err != nil {
		s.Fatalf("Flashrom read into file %s failed with error %v", string(biosImagePath), err)
	}
	...
	for _, sectionSize := range sizes {
		...
		if err := prepareJunkImage(ctx, s.DUT().Conn(), biosImagePath, junkImagePath, ddTemplate, sectionSize); err != nil {
			s.Fatalf("%v", err)
		}
		...
		if err := flashromWrite(ctx, s.DUT().Conn(), junkImagePath, biosImagePath, true); err != nil {
			s.Fatalf("Flashrom write from file %s failed with error %v", junkImagePath, err)
		}
		...
		if err := flashromWrite(ctx, s.DUT().Conn(), biosImagePath, junkImagePath, false); err != nil {
			s.Fatalf("Flashrom write from file %s failed with error %v", biosImagePath, err)
		}
