...
func getSectionAttributes(ctx context.Context, s *testing.State, imagePath, section string) (uint64, uint64) {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		if tokens[0] == "area_name:" && tokens[1] == section {
			...
			sizeAttr := attributes["area_offset:"]
			offset, _ := strconv.ParseUint(strings.ReplaceAll(offsetAttr, "0x", ""), 16, 64)
			size, _ := strconv.ParseUint(strings.ReplaceAll(sizeAttr, "0x", ""), 16, 64)
			return offset, size
		}
		if len(tokens) > 1 {
