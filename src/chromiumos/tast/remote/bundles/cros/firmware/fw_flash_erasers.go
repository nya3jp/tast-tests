...
func flashromWrite(ctx context.Context, s *testing.State, imagePath, originalImagePath string, noVerifyAll bool) {
	...
	//if noVerifyAll {
	//	commandLineArgs = []string{"/usr/sbin/flashrom", "-w", imagePath, "--flash-contents", originalImagePath, "--noverify-all"}
	//} else {
	//	commandLineArgs = []string{"/usr/sbin/flashrom", "-w", imagePath, "--flash-contents", originalImagePath}
	//}

	args := []string{"/usr/sbin/flashrom", "-w", imagePath, "--flash-contents", originalImagePath}
	if noVerifyAll {
...
func createBlobOnDut(ctx context.Context, s *testing.State, byteValue byte, dutBlobFilePath string) {
	var testBlobData [maxSectionSize]byte
	for i := range testBlobData {
		testBlobData[i] = byteValue
	}
...
func getSectionAttributes(ctx context.Context, s *testing.State, imagePath, section string) (uint64, uint64) {
	out, err := runCommandLine(ctx, s, []string{"/usr/bin/dump_fmap", imagePath})
	if err != nil {
	...
	for _, line := range strings.Split(string(out), "\n") {
		...
		if tokens[0] == "area_name:" && tokens[1] == section {
			s.Logf("Found section %s in bios file %s with recorded offset %s and size %s", section, imagePath, attributes["area_offset:"], attributes["area_size:"])
...
func FwFlashErasers(ctx context.Context, s *testing.State) {
	...
	sectionSizes := []int{4096, 4096 * 2, 4096 * 4, 4096 * 8, 4096 * 16}
