...
func getSectionAttributes(ctx context.Context, s *testing.State, biosImagePath, section string) (uint64, uint64) {
	// dump fmap
	commandLine := []string{"/usr/bin/dump_fmap", biosImagePath}
	cmd := s.DUT().Conn().CommandContext(ctx, commandLine[0], commandLine[1:]...)
...
func FwFlashErasers(ctx context.Context, s *testing.State) {
	...
	*/ // === end of temp code
	...
	commandLine = []string{"/usr/sbin/flashrom", "-r", string(biosImagePath)}
	cmd = d.Conn().CommandContext(ctx, commandLine[0], commandLine[1:]...)
	out, err = cmd.CombinedOutput()
	...
	for _, testSize := range testSizes {
		...
		commandLine = []string{"cp", string(biosImagePath), string(junkImagePath)}
		cmd = d.Conn().CommandContext(ctx, commandLine[0], commandLine[1:]...)
		out, err = cmd.CombinedOutput()
		...
		cmd = d.Conn().CommandContext(ctx, ddCmd[0], ddCmd[1:]...)
		out, err = cmd.CombinedOutput()
		...
		commandLine = []string{"/usr/sbin/flashrom", "-w", junkImagePath, "--flash-contents", biosImagePath, "--noverify-all"}
		cmd = d.Conn().CommandContext(ctx, commandLine[0], commandLine[1:]...)
		out, err = cmd.CombinedOutput()
		...
		commandLine = []string{"/usr/sbin/flashrom", "-w", biosImagePath, "--flash-contents", junkImagePath}
		cmd = d.Conn().CommandContext(ctx, commandLine[0], commandLine[1:]...)
		out, err = cmd.CombinedOutput()
