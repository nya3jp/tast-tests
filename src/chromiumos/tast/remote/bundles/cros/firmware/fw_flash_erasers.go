...
// If logOutput is true, all the output from command line run is logged into the test output file, and the latter
// is created in outDir directory.
// If logOutput is false, outDir is ignored, and output from command line run is not logged anywhere.
// Returns: stdout from command line run or error.
func runCommandLine(ctx context.Context, conn *ssh.Conn, args []string, logOutput bool, outDir string) ([]byte, error) {
	...
	if cmd.Stdout != nil {
		return nil, errors.New("Stdout was already set")
	}
	if cmd.Stderr != nil {
		return nil, errors.New("Stderr was already set")
	}
	...
	err := cmd.Wait()
	if err != nil {
		err = errors.Wrapf(err, "command %q returned non-zero error code", strings.Join(cmd.Args, " "))
	}

	if logOutput {
		f, err1 := os.OpenFile(path.Join(outDir, outputFileName), os.O_APPEND|os.O_CREATE|os.O_RDWR, 0644)
		if err1 != nil {
			testing.ContextLog(ctx, "Open output file fails: ", err1)
			if err != nil {
				return nil, err
			}
			return nil, err1
		...
		if _, err1 := fmt.Fprintf(f, " ==== Running command line with arguments: %v ==== \n", args); err1 != nil {
			testing.ContextLog(ctx, "Write output file fails: ", err1)
			if err != nil {
				return nil, err
			}
			return nil, err1
		}
		if _, err1 := f.Write([]byte("\nstdout\n")); err1 != nil {
			testing.ContextLog(ctx, "Write output file fails: ", err1)
			if err != nil {
				return nil, err
			}
		...
		if _, err1 := f.Write(outbuf.Bytes()); err1 != nil {
			testing.ContextLog(ctx, "Write output file fails: ", err1)
			if err != nil {
				return nil, err
			}
		...
		if _, err1 := f.Write(errbuf.Bytes()); err1 != nil {
			testing.ContextLog(ctx, "Write output file fails: ", err1)
			if err != nil {
				return nil, err
			}
...
func sectionAttributes(ctx context.Context, conn *ssh.Conn, imagePath, section string) (offset, size uint64, err error) {
	...
	// Example of dump_fmap output: https://paste.googleplex.com/6564209966972928
	// Group capturing:
