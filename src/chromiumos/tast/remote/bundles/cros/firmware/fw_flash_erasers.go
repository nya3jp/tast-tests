...
func getSectionSizes() []int {
	return []int{maxSectionSize / 16, maxSectionSize / 8, maxSectionSize / 4, maxSectionSize / 2, maxSectionSize}
}

func runCommandLine(ctx context.Context, s *testing.State, commandLineArgs []string, logOutput bool) ([]byte, error) {
...
func flashromRead(ctx context.Context, s *testing.State, imagePath string) {
	...
	_, err := runCommandLine(ctx, s, []string{dutFlashromPath, "-r", string(imagePath)}, true)
...
func prepareJunkImage(ctx context.Context, s *testing.State,
	...
	_, err := runCommandLine(ctx, s, []string{dutCpPath, string(biosImagePath), string(junkImagePath)}, false)
	...
	_, err = runCommandLine(ctx, s, ddCmd, true)
