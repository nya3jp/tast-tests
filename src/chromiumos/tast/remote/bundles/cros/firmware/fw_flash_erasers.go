...
func flashromRead(ctx context.Context, s *testing.State, imagePath string) {
	...
	out, err := runCommandLine(ctx, s, []string{"/usr/sbin/flashrom", "-r", string(imagePath)})
...
func flashromWrite(ctx context.Context, s *testing.State, imageToWrite, imageFlashContents string, noVerifyAll bool) {
	s.Logf("Writing image from file %s, flash contents %s", imageToWrite, imageFlashContents)

	var commandLineArgs []string
	if noVerifyAll {
		commandLineArgs = []string{"/usr/sbin/flashrom", "-w", imageToWrite, "--flash-contents", imageFlashContents, "--noverify-all"}
	} else {
		commandLineArgs = []string{"/usr/sbin/flashrom", "-w", imageToWrite, "--flash-contents", imageFlashContents}
	}

	out, err := runCommandLine(ctx, s, commandLineArgs)

	testing.ContextLog(ctx, "Flashrom write output:", "\n", string(out))

	if err != nil {
		s.Fatalf("Writing from file %s failed", imageToWrite)
	}

	s.Logf("Successfully write from file %s, flash contents %s", imageToWrite, imageFlashContents)
}

func createBlobOnDut(ctx context.Context, s *testing.State, blobByteContent byte, dutBlobFilePath string) {
	var testBlobData [4096 * 16]byte // max of test sizes
	for i := range testBlobData {
		testBlobData[i] = blobByteContent
	}
	...
	_, err = linuxssh.PutFiles(ctx, s.DUT().Conn(),
...
func FwFlashErasers(ctx context.Context, s *testing.State) {
	...
	for _, testSize := range testSizes {
		...
		s.Logf("Step 1 done: successfully copied good bios image from %s to %s", string(biosImagePath), string(junkImagePath))
		...
		s.Log("Step 2 done: successfully set section of the junk image with all 1s. Junk image is ready")
