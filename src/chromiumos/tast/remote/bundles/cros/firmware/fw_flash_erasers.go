...
func runCommandLine(ctx context.Context, s *testing.State, args []string, logOutput bool) ([]byte, error) {
	s.Log("Running command line with arguments: ", args)
	cmd := s.DUT().Conn().CommandContext(ctx, args[0], args[1:]...)
...
func flashromRead(ctx context.Context, s *testing.State, imagePath string) error {
	s.Logf("Reading image into file %s", imagePath)

	_, err := runCommandLine(ctx, s, []string{dutFlashromPath, "-r", imagePath}, true)
	return err
}

func flashromWrite(ctx context.Context, s *testing.State, imagePath, originalImagePath string, noVerifyAll bool) error {
	...
	_, err := runCommandLine(ctx, s, args, true)
	return err
}
...
func sectionAttributes(ctx context.Context, s *testing.State, imagePath, section string) (uint64, uint64) {
	out, err := runCommandLine(ctx, s, []string{dutDumpFmapPath, imagePath}, false)
	if err != nil {
...
func inactiveSection(ctx context.Context, s *testing.State) string {
	out, err := runCommandLine(ctx, s, []string{dutCrossystemPath, "mainfw_act"}, false)
	if err != nil {
...
func prepareJunkImage(ctx context.Context, s *testing.State,
	...
	if _, err := runCommandLine(ctx, s, []string{dutCpPath, biosImagePath, junkImagePath}, false); err != nil {
	...
	if _, err := runCommandLine(ctx, s, ddCmd, true); err != nil {
...
func FwFlashErasers(ctx context.Context, s *testing.State) {
	...
	if err := flashromRead(ctx, s, string(biosImagePath)); err != nil {
	...
	for _, sectionSize := range sizes {
		...
		if err := flashromWrite(ctx, s, junkImagePath, biosImagePath, true); err != nil {
		...
		if err := flashromWrite(ctx, s, biosImagePath, junkImagePath, false); err != nil {
