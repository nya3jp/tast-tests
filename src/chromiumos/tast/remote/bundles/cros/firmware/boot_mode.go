...
func BootMode(ctx context.Context, s *testing.State) {
	...
	defer cancel()
	if err := dut.WaitUnreachable(offCtx); err != nil {
		s.Fatal("Error during dut.WaitUnreachable: ", err)
	}
	...
	defer cancel()
	if err := dut.WaitConnect(onCtx); err != nil {
		s.Fatal("Error during dut.WaitConnect: ", err)
	}
}
