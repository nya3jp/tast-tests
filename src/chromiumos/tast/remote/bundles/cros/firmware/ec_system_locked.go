...
var mapFWWriteProtectStates = map[string]bool{
	string(servo.FWWPStateOff): false,
	string(servo.FWWPStateOn):  true,
}
...
func setFWWriteProtectStateAndReboot(ctx context.Context, h *firmware.Helper, s *testing.State, FWWriteProtectState bool) {
	...
	stateAfterReboot, err := h.Servo.GetString(ctx, servo.FWWPState)
	if err != nil {
