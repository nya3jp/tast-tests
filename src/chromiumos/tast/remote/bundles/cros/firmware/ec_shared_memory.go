...
func ECSharedMemory(ctx context.Context, s *testing.State) {
	...
	if err := checkSharedMemory(ctx, h); err != nil {
		s.Fatal("Failed to check shared memmory: ", err)
	}
	...
	if err := checkSharedMemory(ctx, h); err != nil {
		s.Fatal("Failed to check shared memmory after crash: ", err)
	}
	...
	if err := checkSharedMemory(ctx, h); err != nil {
		s.Fatal("Failed to check shared memmory after sysjump: ", err)
	}
}
...
func checkSharedMemory(ctx context.Context, h *firmware.Helper) error {
	...
	if err != nil {
		return errors.Wrapf(err, "failed to parse EC shared memory (%s) as int: %s",
			ecShmemStr)
	}
