...
func getUSBPorts(ctx context.Context, h *firmware.Helper) ([]firmware.USBEnablePin, error) {
	...
	if portsToCheck <= 0 {
		// If the value is -1 (unkown) or 0 (default), we want to check a bunch of numbers manually.
		portsToCheck = 11
	}
...
func checkUSBAPortEnabled(ctx context.Context, h *firmware.Helper, enablePins []firmware.USBEnablePin, expectedStatusInt int) error {
	...
	for _, pin := range enablePins {
		...
		// } else {
		// 	testing.ContextLogf(ctx, "Checking status of gpio pin name: %q", pin.Name)
		// 	gpioName := firmware.GpioName(pin.Name)
		// 	status, err := ec.FindBaseGpio(ctx, []firmware.GpioName{gpioName})
		// 	if err != nil {
		// 		return errors.Wrapf(err, "failed to parse ectool output for gpiopin %q", pin.Name)
		// 	} else {
		// 		if status[gpioName] != expectedStatus {
		// 			unexpectedStatus[pin.Name] = status[gpioName]
		// 		}
		// 	}
		// }
	}
