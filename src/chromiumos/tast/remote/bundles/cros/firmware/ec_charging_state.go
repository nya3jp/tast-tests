...
func ECChargingState(ctx context.Context, s *testing.State) {
	...
	// Verify that DUT is not charging.
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		ok, err := h.Servo.GetChargerAttached(ctx)
		if err != nil {
			return errors.Wrap(err, "error checking whether charger is attached")
		} else if ok {
			return errors.New("DUT is charging after disconnecting the power supply")
		}
		return nil
	}, &testing.PollOptions{Timeout: 10 * time.Second, Interval: 1 * time.Second}); err != nil {
		s.Fatal("Failed to check for charger after disconnecting power: ", err)
	}
