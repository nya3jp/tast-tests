...
func ECChargingState(ctx context.Context, s *testing.State) {
	...
	if ac, err := h.Servo.RunECCommandGetOutput(ctx, "chgstate", []string{`ac\s*=\s*(0|1)\s*`}); err != nil {
		s.Fatal("Failed to run chgstate ec command: ", err)
	} else if ac[0][1] != "0" {
		s.Fatal("Expected AC to not be plugged in but was was")
	}
	...
	if ac, err := h.Servo.RunECCommandGetOutput(ctx, "chgstate", []string{`ac\s*=\s*(0|1)\s*`}); err != nil {
		...
		s.Fatal("Expected AC to be plugged but was not")
	}
