...
func FlagsPreservation(ctx context.Context, s *testing.State) {
	...
	originalCrossystemMap := createCsMap(ctx, csOriginal)
	defer func() {
	...
	for _, tc := range []struct {
		...
		beforeCrossystemMap := createCsMap(ctx, csBefore)
		...
		case "powerCycleByRemovingBattery":
			...
			s.Log("Power on DUT by reconnecting AC")
		...
		afterCrossystemMap := createCsMap(ctx, csAfter)
...
func createCsMap(ctx context.Context, cs *crossystemValues) map[string]string {
