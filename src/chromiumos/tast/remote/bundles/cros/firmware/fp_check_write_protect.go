...
func init() {
	testing.AddTest(&testing.Test{
		...
		ServiceDeps:  []string{"tast.cros.platform.DbusService", "tast.cros.platform.UpstartService", "tast.cros.baserpc.FileSystem"},
		Vars:         []string{"servo"},
	})
}
...
func FpCheckWriteProtect(ctx context.Context, s *testing.State) {
	...
	t, err := fingerprint.NewFirmwareTest(ctx, d, servoSpec, s.OutDir(), fingerprint.NewMPFirmwareFile(ctx, d), false, false)
	if err != nil {
