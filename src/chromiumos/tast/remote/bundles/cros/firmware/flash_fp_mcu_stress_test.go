...
func flashFpFirmware(ctx context.Context, d *dut.DUT, fpFirmwarePath string) error {
	...
	testing.ContextLog(ctx, "Entropy initialized, now rebooting to rebind cros-ec-uart on zork")
...
func FlashFpMcuStressTest(ctx context.Context, s *testing.State) {
	d := s.DUT()
	...
	for (i := 0; i < 10; i++) {
		if err := flashFpFirmware(ctx, d, fpFirmwarePath); err != nil {
			s.Fatalf("Failed to flash FP firmware: ", err)
		}
