...
func FWTries(ctx context.Context, s *testing.State) {
	...
	if err := firmware.CheckFWTries(ctx, d, "A", 0); err != nil {
		s.Error("Error with DUT state at start of test: ", err)
	}
	if err := firmware.SetFWTries(ctx, d, "B", 2); err != nil {
		s.Fatal("Setting FWTries to B/2: ", err)
	}
	if err := firmware.CheckFWTries(ctx, d, "A", 2); err != nil {
	...
	if err := firmware.CheckFWTries(ctx, d, "B", 1); err != nil {
	...
	if err := firmware.CheckFWTries(ctx, d, "B", 0); err != nil {
