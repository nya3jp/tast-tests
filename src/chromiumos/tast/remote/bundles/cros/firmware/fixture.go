...
func Fixture(ctx context.Context, s *testing.State) {
	...
	if v.BootMode != mode {
		s.Fatalf("Fixture boot mode unexpected, got %v, want %v", v.BootMode, mode)
	}
	...
	if err != nil {
		...
		s.Logf("Successfully verified that fixture set boot mode to %q", v.BootMode)
	}
	...
	if err := checker.GBBFlags(ctx, v.GBBFlags); err != nil {
		...
		s.Logf("Successfully verified that fixture set GBB flags to %q", v.GBBFlags.Set)
	}
}
