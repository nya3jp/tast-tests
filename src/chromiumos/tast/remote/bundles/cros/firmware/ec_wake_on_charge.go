...
func ECWakeOnCharge(ctx context.Context, s *testing.State) {
	...
	checkCCDTestlab := func(ctx context.Context) error {
		...
		if testlabStatus != string(servo.On) {
			s.Log("Checking if enabling test lab is possible")
	...
	openCCD := func(ctx context.Context) error {
		if hasCCD, err := h.Servo.HasCCD(ctx); err != nil {
			...
			// Running 'ccd testlab open' wouldn't work if it was not enabled first.
			// But, if CCD was locked, testlab mode couldn't seem to be enabled, and
			// the console output would print 'access denied'. For debugging purposes,
