...
func ECWakeOnCharge(ctx context.Context, s *testing.State) {
	...
	for _, tc := range []struct {
		lidOpen             string
		deviceHasHibernated bool
	}{
		{string(servo.LidOpenYes), false},
		{string(servo.LidOpenNo), false},
	} {
		...
		s.Log("Stop AC Power")
		if err := setPowerSupply(ctx, false, tc.deviceHasHibernated); err != nil {
		...
		if err := setPowerSupply(ctx, true, tc.deviceHasHibernated); err != nil {
