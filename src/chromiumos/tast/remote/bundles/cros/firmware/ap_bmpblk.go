...
func init() {
	testing.AddTest(&testing.Test{
		...
		// TODO(kmshelton): Move to firmware_unstable, then firmware_bios
...
//APBmpblk inspects coreboot contents.  See https://chromium.googlesource.com/chromiumos/platform/bmpblk for more context.
func APBmpblk(ctx context.Context, s *testing.State) {
	...
	if !notScaledRe.MatchString(outs) {
		s.Fatalf("Failed inspection for generic configuration.  This build was configured for a generic "+
			...
			"(saved output to %s).", notScaledRe, filepath.Base(path))
	}
}
