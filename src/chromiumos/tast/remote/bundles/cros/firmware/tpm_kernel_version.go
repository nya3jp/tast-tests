...
func TPMKernelVersion(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Failed to parse firmware version stored in TPM as string: ", err)
	}
	...
	if kernVersion == 0xFFFFFFFF {
		s.Fatalf("Invalid kernel version found in TPM!")
	}

	if fwVersion == 0xFFFFFFFF {
		s.Fatalf("Invalid firmware version found in TPM!")
	}
}
