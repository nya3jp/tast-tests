...
type testRollbackParams struct {
	...
	expectedFingerprintTaskStatus bool
...
func testFlashingFirmwareRollback(ctx context.Context, d *rpcdut.RPCDUT, params *testRollbackParams) error {
	...
	_, err := d.Conn().CommandContext(ctx, cmd[0], cmd[1:]...).Output(ssh.DumpLogOnError)
	testing.ContextLog(ctx, "Josie, err != nil", (err != nil))
	testing.ContextLog(ctx, "Josie, expectedFpTaskStatus", params.expectedFingerprintTaskStatus)
	if (err == nil) != params.expectedFingerprintTaskStatus {
		return errors.Wrap(err, "unexpected error checking fingerprint task")
	}
	// taskRunningBool := string(out) != ""
	// if taskRunningBool != params.expectedFingerprintTaskStatus {
...
func FpObeysRollback(ctx context.Context, s *testing.State) {
	...
	folderpath := "/mnt/stateful_partition/fptast"
	...
	// Clean up fptast directory.
	defer func() {
		s.Log("Delete fptast directory and contained files from DUT")
		if _, err := d.Conn().CommandContext(ctx, "rm", "-rf", folderpath).Output(ssh.DumpLogOnError); err != nil {
	...
	// }
	// if fpUpdaterEnabled {
	// 	if err := fingerprint.DisableFPUpdater(ctx, d); err != nil {
	// 		s.Fatal("Failed to disable FP updater: ", err)
	// 	}
	// }
	// defer func() {
	// 	// If FP updater disabled, enable it.
	...
	if err := testFlashingFirmwareRollback(ctx, d,
		&testRollbackParams{
			...
			expectedFingerprintTaskStatus: true,
	...
	if err := testFlashingFirmwareRollback(ctx, d,
		&testRollbackParams{
			...
			// Fingerprint task should be running.
			expectedFingerprintTaskStatus: false,
			// Expected rollback state.
			expectedRollbackState: fingerprint.RollbackState{
