...
func testFlashingFirmwareRollback(ctx context.Context, d *rpcdut.RPCDUT, params *testRollbackParams) error {
	...
	cmd := d.Conn().CommandContext(ctx, "ectool", "--name=cros_fp", "fpinfo")
	err := cmd.Run()
	if !errors.Is(err, params.expectedFingerprintTaskStatusErr) {
		if err.Error() != params.expectedFingerprintTaskStatusErr.Error() {
			testing.ContextLog(ctx, "got:", err, "|expected:", params.expectedFingerprintTaskStatusErr, "|")
...
func FpObeysRollback(ctx context.Context, s *testing.State) {
	...
	defer func() {
		...
		cmd := d.Conn().CommandContext(cleanupCtx, "rm", "-rf", folderpath)
