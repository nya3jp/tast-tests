...
func SoftwareSync(ctx context.Context, s *testing.State) {
	...
	// TODO: old test disables EC WP here
	...
	if common.GBBFlagsContains(old, pb.GBBFlag_DISABLE_EC_SOFTWARE_SYNC) {
		...
		// TODO: old test does warm reset, seems unneeded
	}
	...
	// TODO: Old test checks that fw-a section does not have preamble flag PREAMBLE_USE_RO_NORMAL. this really needed?
	// TODO: Old test unlocks CCD, is this needed?
	...
	ecHashBefore, err := h.DUT.Conn().CommandContext(ctx, "sh", "-c", "ectool echash | grep hash: | sed \"s/hash:\\s\\+//\"").
		Output()
	if err != nil {
	...
	_, err = bs.CorruptECSection(ctx, &pb.CorruptSection{Section: ecSection})
	...
	ecHashAfter, err := h.DUT.Conn().CommandContext(ctx, "sh", "-c", "ectool echash | grep hash: | sed \"s/hash:\\s\\+//\"").
		Output()
	if err != nil {
		s.Fatal("Failed to get ec hash: ", err)
	}
