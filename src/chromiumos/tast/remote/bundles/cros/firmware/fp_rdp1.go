...
func FpRDP1(ctx context.Context, s *testing.State) {
	...
	if err := testRDP1(ctx, s, d, t.BuildFwFile(), false /*preserve RDP level*/, t.NeedsRebootAfterFlashing()); err != nil {
	...
	if err := testRDP1(ctx, s, d, t.BuildFwFile(), true /*remove Flash Read Protect*/, t.NeedsRebootAfterFlashing()); err != nil {
...
func testRDP1(ctx context.Context, s *testing.State, d *rpcdut.RPCDUT, buildFwFile string, removeFlashReadProtect, needsReboot bool) (e error) {
	...
	flashout := filepath.Join(s.OutDir(), strconv.FormatBool(removeFlashReadProtect)+"flash_fp_mcu_output.txt")
	testing.ContextLogf(ctx, "flash_fp_mcu output written to %q", flashout)
	errfp := ioutil.WriteFile(flashout, out, 0644)
	if errfp != nil {
		return errors.Wrapf(errfp, "failed to write flash_fp_mcu output to %q", flashout)
	}
	if err == nil {
		return errors.New("should not be able to flash without error")
	}
	...
	if removeFlashReadProtect {
		...
		if err != nil {
			return errors.Wrap(err, "failed to get info of test file")
		}
		buildinfo, err := fs.Stat(ctx, buildFwFile)
		if err != nil {
			return errors.Wrap(err, "failed to get info of build file")
		}
