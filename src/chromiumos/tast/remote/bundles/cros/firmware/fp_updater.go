...
func init() {
	testing.AddTest(&testing.Test{
		...
		ServiceDeps:  []string{"tast.cros.firmware.FpUpdaterService", "tast.cros.platform.DbusService", "tast.cros.platform.UpstartService", dutfs.ServiceName},
		Vars:         []string{"servo"},
...
func FpUpdater(ctx context.Context, s *testing.State) {
	...
	t, err := fingerprint.NewFirmwareTest(ctx, d, servoSpec, s.OutDir(), fingerprint.NewMPFirmwareFile(ctx, d), true /*HW protect*/, true /*SW protect*/)
	if err != nil {
