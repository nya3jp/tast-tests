...
func Ti50SystemTestImage(ctx context.Context, s *testing.State) {
	...
	board, rpcClient, err := remoteTi50.GetTi50TestBoard(ctx, s.DUT(), s.RPCHint(), mode, spiflash, 10000, 30*time.Second)
	...
	if failCount > 0 {
		s.Fatalf("%s test failures", failCount)
	}
}
...
func checkTestResults(ctx context.Context, s *testing.State, board ti50.DevBoard, sectionName string) int {
	...
	for {
		...
		m, err := board.ReadSerialSubmatch(ctx, regexp.MustCompile("("+endMarker+"|##TEST (SKIP|START) (\\S+)\\s)"))
		if err != nil {
