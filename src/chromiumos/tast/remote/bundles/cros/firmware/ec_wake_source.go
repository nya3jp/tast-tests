...
func testWakeWithLid(ctx context.Context, h *firmware.Helper) error {
	...
	// Command never completes, so start it then let it run in the bg.
	go h.DUT.Conn().CommandContext(ctx, "powerd_dbus_suspend", fmt.Sprintf("--delay=%d", int(powerdSuspendDelay.Seconds()))).Start()
...
func testSuspendAndWakeWithInternalKeypress(ctx context.Context, h *firmware.Helper) error {
	...
	if match == nil {
		...
		// Wether or not it wakes on keypress depends on keyboard being enabled/disabled.
