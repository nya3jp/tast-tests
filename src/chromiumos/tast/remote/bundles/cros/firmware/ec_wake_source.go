...
func ECWakeSource(ctx context.Context, s *testing.State) {
	...
	case wakeByKeyboard:
		...
		if match := tabletModeNotAvailable.FindStringSubmatch(out[0].([]interface{})[0].(string)); match != nil {
			...
			s.Log("Getting initial tablet mode")
			out, err := h.Servo.RunECCommandGetOutput(ctx, "tabletmode reset", []string{reGetTabletMode})
			if err != nil {
				s.Fatal("Failed to get output for tabletmode command: ", err)
			}
			// initialTabletMode is either "enabled" or "disabled".
			initialTabletMode := out[0].([]interface{})[1].(string)
			s.Log("Initial tablet mode: ", initialTabletMode)
			...
			// Reset tablet mode to initial value
			var resetTabletMode string
			if initialTabletMode == "disabled" {
				resetTabletMode = "tabletmode off"
			} else {
				resetTabletMode = "tabletmode on"
			}

			s.Logf("Resetting tablet mode to initial value using %q cmd", resetTabletMode)
			out, err = h.Servo.RunECCommandGetOutput(ctx, resetTabletMode, []string{reGetTabletMode})
			if err != nil {
