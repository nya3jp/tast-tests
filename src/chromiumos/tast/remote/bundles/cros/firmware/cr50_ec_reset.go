...
func init() {
	testing.AddTest(&testing.Test{
		...
		SoftwareDeps: []string{"gsc", "gsc_can_wake_ec_with_reset"},
	})
}
...
func CR50ECReset(ctx context.Context, s *testing.State) {
	...
	if !h.Config.GSCCanWakeECWithReset {
		s.Fatal("This DUT has a hardware limitation that prevents cr50 from waking the EC with EC_RST_L. Exclude the Model from this test by editing a SW Dep")
	}
