...
func DeepSleep(ctx context.Context, s *testing.State) {
	...
	// g3Timeout is the time to wait for the DUT to reach G3 after power off
	const g3Timeout = 30 * time.Second
	// postWakeRetryTime is the time to wait for the battery after waking up from hibernate
	const postWakeRetryTime = 5 * time.Second
	// getChargerTimeout is the time to retry the GetChargerAttached command. Unexpected EC uart logging can make it fail.
	const getChargerTimeout = 2 * time.Second
	...
	// DUT is probably still booting, wait for it to finish.
