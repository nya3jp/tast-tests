...
func init() {
	testing.AddTest(&testing.Test{
		...
		Contacts:     []string{"hc.tsai@cienet.com"},
...
func DeepSleep(ctx context.Context, s *testing.State) {
	...
	s.Log("Stoping power supply from servo")
	...
	if err = testing.Poll(ctx, func(c context.Context) error {
		state, err := pxy.Servo().GetString(ctx, "ec_system_powerstate")
		if err != nil {
	...
	max, err := pxy.Servo().GetInt(ctx, "battery_full_charge_mah")
	if err != nil {
		s.Fatal("Failed to get full charge mAh: ", err)
	}
	s.Logf("Battery max capacity: %dmAh", max)

	mahStart, err := pxy.Servo().GetInt(ctx, "battery_charge_mah")
	if err != nil {
	...
	mahEnd, err := pxy.Servo().GetInt(ctx, "battery_charge_mah")
	if err != nil {
		s.Fatal("Failed to get charge mAh: ", err)
	}
