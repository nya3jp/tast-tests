...
func adjustKBBacklight(ctx context.Context, h *firmware.Helper, d *dut.DUT, extremeValue int, dur time.Duration, actionKey, action string) error {
	...
	for shouldContinue(kbLight, extremeValue, action) {
		...
		if timeNow.After(endTime) {
			...
			finalPwm, err := saveKbLightPwm(ctx, d, "final", pwms)
			if err != nil {
				testing.ContextLog(ctx, "checking final kb light pwm failed: ", err)
			}
			hwdepResults := checkKBLightDependency(ctx, h)
			return &timeoutError{E: errors.Errorf("timeout in adjusting kb backlight. Got kb light initial pwm val: %s, final pwm val: %s, and hwdep val: %q", initPwm, finalPwm, hwdepResults)}
		}
...
func saveKbLightPwm(ctx context.Context, dut *dut.DUT, stage string, pwms kbLightPwmVals) (string, error) {
