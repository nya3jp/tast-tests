...
func adjustKBBacklight(ctx context.Context, h *firmware.Helper, d *dut.DUT, extremeValue int, dur time.Duration, actionKey, action string) error {
	// Save the pwm value before adjusting kb light if it exists.
	pwms := kbLightPwmVals{}
	initPwm, err := saveKbLightPwm(ctx, d, "initial", pwms)
	if err != nil {
	...
	for shouldContinue(kbLight, extremeValue, action) {
		...
		if timeNow.After(endTime) {
			// At timeout, save the final pwm value for kb light if it exists.
			finalPwm, err := saveKbLightPwm(ctx, d, "final", *initPwm)
			if err != nil {
				testing.ContextLog(ctx, "checking final kb light pwm failed: ", err)
			}
			hwdepResults := checkKBLightDependency(ctx, h)
			return &timeoutError{E: errors.Errorf("timeout in adjusting kb backlight. Got kb light initial pwm val: %s, final pwm val: %s, and hwdep val: %q", finalPwm.initialVal, finalPwm.endVal, hwdepResults)}
		}
...
// saveKbLightPwm saves the pwm value before/after adjusting keyboard backlight.
func saveKbLightPwm(ctx context.Context, dut *dut.DUT, stage string, pwms kbLightPwmVals) (*kbLightPwmVals, error) {
	val, err := checkKbLightPwm(ctx, dut)
	if err != nil {
	...
	switch stage {
	case "initial":
		pwmValue = &pwms.initialVal
	case "final":
		pwmValue = &pwms.endVal
	default:
		return nil, errors.New("unknown query")
	}
	*pwmValue = val
	testing.ContextLogf(ctx, "PWM %s value: %s", stage, *pwmValue)
	return &pwms, nil
}
