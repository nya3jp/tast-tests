...
func ECCbiEeprom(ctx context.Context, s *testing.State) {
	...
	pageSize := 8
	maxBytes := 64
	testData1 := strings.TrimRight(strings.Repeat("0xaa ", 8), " ")
	testData2 := strings.TrimRight(strings.Repeat("0x55 ", 8), " ")
	...
	if err := h.Servo.SetOnOff(ctx, servo.I2CMuxEn, servo.Off); err != nil {
		s.Log("i2c_mux_en does not exist, ignoring")
	} else {
		s.Log("i2c_mux_en present and reset")
	}
