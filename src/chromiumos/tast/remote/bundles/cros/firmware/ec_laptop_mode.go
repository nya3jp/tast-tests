...
func ECLaptopMode(ctx context.Context, s *testing.State) {
	...
	checkLaptopMode := func(ctx context.Context) error {
		...
		if len(m) != 2 {
			return errors.New("fail to get lid angle")
		}
		lidAngle, err := strconv.Atoi(string(m[1][:]))
		if err != nil {
			return errors.Wrap(err, "fail to parse lid angle")
		}
		...
		m = re.FindSubmatch([]byte(data))
		if len(m) != 2 {
			errors.New("fail to get tablet mode angle")
		}
		tabletModeAngle, err := strconv.Atoi(string(m[1][:]))
		if err != nil {
			return errors.Wrap(err, "fail to parse tablet mode angle")
		}
	...
	if err := h.Servo.SetPowerState(ctx, servo.PowerStateOff); err != nil {
		s.Fatal("Failed to power off DUT by servo: ", err)
	}
	...
	s.Log("Set power off")
	if err := h.Servo.SetPowerState(ctx, servo.PowerStateOff); err != nil {
		s.Fatal("Failed to turn off DUT using servo: ", err)
	}
