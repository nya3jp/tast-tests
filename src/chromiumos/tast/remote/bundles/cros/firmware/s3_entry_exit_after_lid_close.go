...
func S3EntryExitAfterLidClose(ctx context.Context, s *testing.State) {
	...
	defer func(ctx context.Context) {
		...
		cleanupCtx, cancel := ctxutil.Shorten(ctx, 1*time.Minute)
		defer cancel()
		if err := h.Servo.SetString(cleanupCtx, "lid_open", "yes"); err != nil {
	...
	if actualValue != expectedValue {
		s.Fatalf("Failed to enter S3 state; expected powerdConfig value %s; got %s", expectedValue, actualValue)
	}
	...
	s.Log("Checking DUT in S3 state after suspend")
