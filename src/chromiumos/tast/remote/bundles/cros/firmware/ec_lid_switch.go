...
func suspendAndWakeWithLid(ctx context.Context, h *firmware.Helper, delay time.Duration) (reterr error) {
...
func checkKeyPressesWithLidClosed(ctx context.Context, h *firmware.Helper) (reterr error) {
	...
	if err != nil {
		testing.ContextLog(ctx, "No keyboard found, skipping keyboard tests: ", err)
		return nil
	}
	device := res.Path
