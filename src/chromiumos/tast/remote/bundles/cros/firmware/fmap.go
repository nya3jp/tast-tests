...
func Fmap(ctx context.Context, s *testing.State) {
	...
	/*
		Expected ec Structure:
			...
			EC_RW
				|--> RW_FWID
	*/
	// This is basically an adjacency list format, each expected region is added
	// If the region has subregions, add as elements in the value list (subregions should also be in the map as keys).
	...
	*/
	...
	case pb.Programmer_BIOSProgrammer:
		...
		// If actual fmap has ME_RW_A (implies also has ME_RW_B) add those to expected structure under RW_SECTION_A/B respectively.
...
func checkStructure(ctx context.Context, fmap map[pb.ImageSection]*pb.Range, target map[pb.ImageSection][]pb.ImageSection) error {
	// Verify child region is entirely contained within parent region.
	...
	for parent, children := range target {
		...
		// Sort children ranges by start so not every pair of children have to be checked for overlap.
