...
func RemoteSerialPort(ctx context.Context, s *testing.State) {
	...
	s.Log("Opening port should work")
	...
	err = commonSerial.DoTestRead(ctx, s.Log, p1, p2)
	if err != nil {
		s.Fatal("TestRead failed: ", err)
	}

	err = commonSerial.DoTestWrite(ctx, s.Log, p1, p2)
	if err != nil {
		s.Fatal("TestWrite failed: ", err)
	}

	err = commonSerial.DoTestFlush(ctx, s.Log, p1, p2)
	if err != nil {
