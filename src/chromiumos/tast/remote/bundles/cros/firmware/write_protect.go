...
var wpTargetToRegion = map[wpTarget]pb.ImageSection{
	targetBIOS: pb.ImageSection_ROFRIDImageSection,
...
func testReadWrite(ctx context.Context, h *firmware.Helper, target wpTarget) error {
	...
	// Read current section again in case reboot from setting write protect updated image.
	testing.ContextLog(ctx, "Read current fw image")
	roBefore, err = h.BiosServiceClient.BackupImageSection(ctx, &pb.FWSectionInfo{
		Section:    wpTargetToRegion[target],
		Programmer: wpTargetToProg[target],
	...
	if out, err := h.DUT.Conn().CommandContext(ctx, "cmp", roTest.Path, roAfter.Path).CombinedOutput(ssh.DumpLogOnError); err != nil {
		...
		if errCode, ok := testexec.ExitCode(err); !ok || errCode == 2 {
			...
			return errors.Wrapf(err, "files %q and %q were not identical, so either write protect or read failed: %s", roTest.Path, roAfter.Path, string(out))
		}
	}
