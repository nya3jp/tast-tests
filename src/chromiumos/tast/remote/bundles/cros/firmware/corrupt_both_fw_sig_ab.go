...
func CorruptBothFWSigAB(ctx context.Context, s *testing.State) {
	...
	defer func() {
		if _, err := h.DUT.Conn().CommandContext(ctx, "rm", FWSignAPath.Path).Output(ssh.DumpLogOnError); err != nil {
	...
	defer func() {
		if _, err := h.DUT.Conn().CommandContext(ctx, "rm", FWSignBPath.Path).Output(ssh.DumpLogOnError); err != nil {
	...
	defer func() {
		...
		if _, err := h.DUT.Conn().CommandContext(ctx, "rm", FWSignAPath.Path, FWSignBPath.Path).Output(ssh.DumpLogOnError); err != nil {
			s.Fatal("Failed to delete FW Signs backup: ", err)
		}
		...
		if mainFWType, err := h.Reporter.CrossystemParam(ctx, reporters.CrossystemParamMainfwType); err != nil {
			...
			s.Fatalf("Expected mainfw_type to be %s, got %q", bootModeName, mainFWType)
		}
