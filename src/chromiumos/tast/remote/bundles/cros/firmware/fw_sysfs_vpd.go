...
// via rading from sysfs values and logs them on test output
func FWSysfsVPD(ctx context.Context, s *testing.State) {
	...
	if fwidMajor <= 8846 {
		s.Fatal("Firmware version is below 8846, cannot conitnue: ", fwidMajor)
	}

	// Back up firmware prior to modifcations
	...
	// Generate a random string for validation
	...
	if err != nil {
		s.Fatal("Failed reading new RO_VPD value after a reboot: ", err)
	}

	if string(newVPDROout) != roSectionString {
		s.Fatalf("RO_VPD section mismtch! Got: %s, wanted: %s\n", newVPDROout, roSectionString)
	}

	// Verify RW section of VPD
	...
	if err != nil {
		s.Fatal("Failed reading new RW_VPD value after a reboot: ", err)
	}

	if string(newVPDRWout) != rwSectionString {
		s.Fatalf("RO_VPD section mismtch! Got: %s, wanted: %s\n", newVPDRWout, roSectionString)
	}
