...
func FWSysfsVPD(ctx context.Context, s *testing.State) {
	...
	// TODO: Centralize the version checking logic
	...
	if string(newVPDROout) != roSectionString {
		s.Fatalf("RO_VPD section mismatch! (expected: %s, got: %s)\n", roSectionString, newVPDROout)
	}
	...
	if string(newVPDRWout) != rwSectionString {
		s.Fatalf("RO_VPD section mismtch! (expected: %s, got: %s)\n", rwSectionString, newVPDRWout)
	}
