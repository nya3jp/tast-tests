...
func Wrapper(ctx context.Context, s *testing.State) {
	...
	enabledSyscalls, err := ioutil.ReadFile(s.DataPath("enabled_syscalls.txt"))
	if err != nil {
		s.Fatalf("Unable to read enabled syscalls: %v", err)
	}
	s.Logf("Enabled syscalls: %v", strings.Split(string(enabledSyscalls), "\n"))
	...
	config := syzkallerConfig{
		...
		EnableSyscalls: strings.FieldsFunc(string(enabledSyscalls), func(c rune) bool { return c == '\n' }),
	}
