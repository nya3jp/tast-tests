...
func loadEnabledSyscalls(fpath, board string) (drivers, enabledSyscalls []string, scriptContents string, err error) {
	...
	for _, config := range feconfig {
		...
		// Enable syscalls of a driver if |Boards| is empty, and the DUT board is not present in |ExcludedBoards|.
		// Also, enable syscalls of a driver if |Boards| contains the DUT board.
