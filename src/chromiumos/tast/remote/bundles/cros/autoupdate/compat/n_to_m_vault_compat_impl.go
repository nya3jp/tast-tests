...
func NToMVaultCompatImpl(ctx context.Context, s *testing.State, vtype VaultType) {
	...
	defer utility.RemoveVault(ctxForCleanUp, userName)
	{
		...
		if err := hwsec.WriteUserTestContent(ctx, utility, cmdRunner, userName, testFile1, testFileContent); err != nil {
	...
	// Create vault
	{
		...
		if content, err := hwsec.ReadUserTestContent(ctx, utility, cmdRunner, userName, testFile1); err != nil {
