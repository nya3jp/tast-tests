...
func NToMVaultCompatImpl(ctx context.Context, s *testing.State, vtype VaultType) {
	...
	defer utility.RemoveVault(ctxForCleanUp, userName)
	{
		...
		if _, err := cmdRunner.Run(ctx, "sh", "-c", fmt.Sprintf("echo -n %q > %q", testFileContent, encstatefulFile)); err != nil {
	...
	// Create vault
	{
		...
		if content, err := cmdRunner.Run(ctx, "cat", encstatefulFile); err != nil {
