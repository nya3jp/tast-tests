...
const (
	...
	TotalTestTime = initTimeoutN2M + preUpdateTimeoutN2M + postUpdateTimeoutN2M + updateutil.UpdateTimeout
)

// Ops contains operations performed at various points of update sequnce
type Ops struct {
...
// NoOp is a function that defines a stub no-op function for a part of test.
func NoOp(ctx context.Context, s *testing.State) {
	s.Log("NO_OP")
}

// NToMTest drives autoupdate and calls to client code providing callbacks.
func NToMTest(ctx context.Context, s *testing.State, ops *Ops, deltaM int) {
