...
func EnterpriseRollbackPreviousVersion(ctx context.Context, s *testing.State) {
	...
	defer func(ctx context.Context) {
		...
		// Check the image version. Roll back if it's not the original one.
		version, err := updateutil.ImageVersion(ctx, s.DUT(), s.RPCHint())
		if err != nil {
			s.Fatal("Failed to read DUT image version: ", err)
		}
	...
	// Policy client will be closed before reboot to rollback.
	...
	// Clean up the rpc and policy clients.
	client.Close(ctx)
	...
	// Stopping ui early to prevent accidental reboots during TPM clear.
	...
	// Ineffective reset is expected because rollback initiates TPM ownership instantely.
