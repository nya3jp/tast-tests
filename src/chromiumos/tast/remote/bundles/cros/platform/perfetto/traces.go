...
func RunPerfetto(ctx context.Context, pc platform.PerfettoTraceBasedMetricsServiceClient, traceConfigPath string) (string, error) {
	...
	if _, err := tempFile.Write(response.Result); err != nil {
		os.Remove(tempFile.Name())
		return "", errors.Wrap(err, "failed to write the result to temp file")
	}
...
func RunMetrics(ctx context.Context, traceProcessorPath, outputPath string, metrics []string) (*tppb.TraceMetrics, error) {
