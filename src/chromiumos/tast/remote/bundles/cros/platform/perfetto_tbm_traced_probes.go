...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:        []string{traceConfigFileCPU, traceConfigFileMEM, traceProcessor},
...
func RunPerfettoAndRunMetric(ctx context.Context, pc *platform.PerfettoTraceBasedMetricsServiceClient, traceConfigFile string, metric string, s *testing.State) *tppb.TraceMetrics {
...
func PerfettoTBMTracedProbes(fullCtx context.Context, s *testing.State) {
	...
	metrics := RunPerfettoAndRunMetric(ctx, &pc, traceConfigFileCPU, traceMetricCPU, s)

	// We may also send the result to crosbolt for the regression check.
	ProcessCPUMetric(metrics.GetAndroidCpu(), s)

	metrics = RunPerfettoAndRunMetric(ctx, &pc, traceConfigFileMEM, traceMetricMEM, s)
