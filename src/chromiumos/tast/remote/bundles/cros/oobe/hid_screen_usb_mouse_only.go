...
import (
	...
	util "chromiumos/tast/remote/bundles/cros/oobe/servoutil"
...
func HidScreenUsbMouseOnly(ctx context.Context, s *testing.State) {
	...
	mouseSvc := inputspb.NewMouseServiceClient(fv.DUTRPCClient.Conn)
	...
	if err := crui.CheckNodeWithNameExists(ctx, uiautoSvc, oobeui.ContinueButtonEnabledNodeName); err == nil {
	...
	if err := crui.CheckNodeWithNameExists(ctx, uiautoSvc, oobeui.FoundUSBMouseNodeName); err != nil {
	...
	if err := crui.CheckNodeWithNameExists(ctx, uiautoSvc, oobeui.SearchingForMouseNodeName); err != nil {
		s.Fatal("Failed to find node: ", err)
	}
