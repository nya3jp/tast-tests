...
func perfBootOnce(ctx context.Context, s *testing.State, i, iterations int, pvMultiple *perf.Values, singleMetrics map[perf.Metric][]float64) {
	d := s.DUT()
...
func PerfBoot(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < iterations; i++ {
		...
		perfBootOnce(ctx, s, i, iterations, pv, singleMetrics)
	}
