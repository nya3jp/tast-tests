...
func DataCollector(ctx context.Context, s *testing.State) {
	...
	genUreadaheadPack := func() {
		...
		if needUploadPacks {
			if err = upload(
				...
				ureadAheadPacks,
				v,
				filepath.Join(s.OutDir(), initialPack)); err != nil {
		...
		if needUploadPacks {
			if err = upload(
				...
				ureadAheadPacks,
				v,
				filepath.Join(s.OutDir(), provisionedPack)); err != nil {
	...
	genGmsCoreCache := func() {
		...
		needUpload := func() bool {
			archives := []string{response.GmsCoreCacheName, response.GsfCacheName}
			for _, archive := range archives {
				gsURL := remoteURL(gmsCoreCaches, v, archive)
				if err := exec.Command("gsutil", "stat", gsURL).Run(); err != nil {
					return true
				}
			}
		...
		if needUpload() {
			if err = upload(
				shortCtx,
				d,
				filepath.Join(response.TargetDir, response.GmsCoreCacheName),
				gmsCoreCaches,
				v,
				filepath.Join(s.OutDir(), response.GmsCoreCacheName)); err != nil {
				s.Fatal("Failed to upload GMS Core caches: ", err)
			}
			if err = upload(
				shortCtx,
				d,
				filepath.Join(response.TargetDir, response.GsfCacheName),
				gmsCoreCaches,
				v,
				filepath.Join(s.OutDir(), response.GsfCacheName)); err != nil {
