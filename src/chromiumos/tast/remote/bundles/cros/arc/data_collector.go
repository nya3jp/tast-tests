...
func DataCollector(ctx context.Context, s *testing.State) {
	...
	genUreadaheadPack := func() (retErr error) {
		...
		// the already provisioned account. However nowdays this pack is no longer used but
		// is still referenced from the ChromeOS build system. Recently we switched to the
		// pool of accounts and this introduces an inconsistency when the already
		// provisined pass exececuted with a random account, which in most cases does not
