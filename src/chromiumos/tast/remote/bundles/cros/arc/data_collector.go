...
const (
	...
	// Path to ARC ureadahead pack on the devcie.
...
func uploadPack(ctx context.Context, s *testing.State, dut *dut.DUT, version string, dst string) error {
	...
	s.Logf("Uploaded pack to the server: %q", gsURL)
	return nil
}
...
func DataCollector(ctx context.Context, s *testing.State) {
	...
	if err = uploadPack(ctx, s, d, v, initialPack); err != nil {
	...
	if err = uploadPack(ctx, s, d, v, provisionedPack); err != nil {
