...
func AdbSideload(ctx context.Context, s *testing.State) {
	...
	// Performing the test twice - for confirm and cancel buttons oh the UI
...
func doAdbSideloadAction(ctx context.Context, s *testing.State, requestAction string) {
	...
	// Storing the Enable ADB Sideloding flag to generate the UI
	...
	// Clicking the button on the Warning UI to confirm or cancel ADB sideloading
	service = arc.NewAdbSideloadServiceClient(cl.Conn)
	...
	expectedByte := []byte{49}
	if requestAction == "cancel" {
		expectedByte = []byte{48}
	}
	s.Logf("Boot Lock Box value after button click: %s", string(response.Value))
	...
	// Resetting the value of the bootlock box to 0 again to restore original state
	client = security.NewBootLockboxServiceClient(cl.Conn)
	client.Store(ctx, &security.StoreBootLockboxRequest{Key: adbSideloadingBootLockboxKey, Value: []byte("0")})
}
