...
// resourceInfo describes attribut of file resource used for layout verification
...
func (r1 resourceInfo) Matches(r2 resourceInfo) bool {
	...
	if r1.blockSize != r2.blockSize {
		...
		// they may sligtly change in size even on the same maching and the same build.
		// We do allow this difference. Small change usually is not reflected but may cross
		// 4K page size and in last case this may turn to 8 block size (4K = 512*8)
		// difference.
		...
		if filepath.Ext(r1.name) != ".odex" || d != 8 {
			return false
		}
	}
...
// diffLayouts reads two layout files and verifis they match. In case of match, empty diff is returned.
