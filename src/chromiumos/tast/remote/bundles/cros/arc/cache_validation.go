...
func CacheValidation(ctx context.Context, s *testing.State) {
	...
	// Also returns the temp directory for removal.
	getPackagesCache := func(packagesCacheEnabled, gmsCoreEnabled bool) (string, string, string) {
	...
	withCache, genCache, withCacheDir := getPackagesCache(true, true)
	defer os.RemoveAll(withCacheDir)
	withoutCache, genCache, withoutCacheDir := getPackagesCache(false, false)
