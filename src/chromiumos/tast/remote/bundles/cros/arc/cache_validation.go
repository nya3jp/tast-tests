...
func CacheValidation(ctx context.Context, s *testing.State) {
	...
	withoutCacheTTSCacheFile, pregenTTSCacheFile := getTTSCache(ctx, s, cl, tempDir, false)
	...
	if pregenTTSCacheFile == "" {
		...
		if err := saveOutput(filepath.Join(s.OutDir(), "without_cache_tts_cache_diff.txt"),
			...
			s.Error("Error validating TTS cache with no cache setup: ", err)
		}

		withCacheTTSCacheFile, pregenTTSCacheFile := getTTSCache(ctx, s, cl, tempDir, true)
		...
		if err := saveOutput(filepath.Join(s.OutDir(), "with_cache_tts_cache_diff.txt"),
			...
			s.Error("Error validating TTS cache with cache setup: ", err)
		}
	}
}

func getTTSCache(ctx context.Context, s *testing.State, cl *rpc.Client, tempDir string, cacheEnabled bool) (string, string) {
