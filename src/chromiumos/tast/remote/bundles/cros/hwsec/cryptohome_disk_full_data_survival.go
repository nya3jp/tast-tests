...
const (
	...
	// maxAvailSize is the threshold for available space in user's home after filling it up.
	// If the amount of available space is greater than this amount, then the test will fail.
	maxAvailSize = 300 // 300MiB
)
...
func getHomeAvailableSpace(ctx context.Context, utility *hwsec.CryptohomeClient, r hwsec.CmdRunner, username string) (int, error) {
	...
	if len(arr) < 2 {
		return -1, errors.Errorf("df output is lesser than 2 line: %q", out)
	}
...
func fillVaultAndClear(ctx context.Context, utility *hwsec.CryptohomeClient, r hwsec.CmdRunner, username string) error {
	...
	for i := 0; i < 10; i++ {
		fillerPath := filepath.Join(homedir, fmt.Sprintf("%s%02d", testSpaceFillerFilenamePrefix, i))
