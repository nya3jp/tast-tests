...
// NOTE: This test is largely similar to hwsec.DictionaryAttackLockoutResetTPM2 (a local test), if change is made to one, it is likely that the other have to be changed as well.
...
func DictionaryAttackLockoutResetTPM1(ctx context.Context, s *testing.State) {
	...
	helper, err := hwsecremote.NewHelper(cryptohomeUtil, cmdRunner, s.DUT())
	if err != nil {
