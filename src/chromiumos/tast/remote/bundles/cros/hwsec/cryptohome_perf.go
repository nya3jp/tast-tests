...
func parseBootstat(ctx context.Context, remote *hwsecremote.CmdRunnerRemote, fileName string, fieldNum int) ([]float64, error) {
	...
	for _, line := range lines {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "malformed bootstat content: %q", line)
		}
...
func CryptohomePerf(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Failed to parse chaps uptime: ", err)
	}
	...
	startUpTime := cryptohomeUptime - math.Max(tpmManagerUptime, chapsUptime)
	startUpTimeInternal := cryptohomeInternalUptime - math.Max(tpmManagerUptime, chapsUptime)
