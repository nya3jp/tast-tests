...
func PinWeaverWithAuthFactor(ctx context.Context, s *testing.State) {
	...
	if out, err := cmdRunner.Run(ctx, "mkdir", "-p", "/var/lib/cryptohome"); err != nil {
		s.Fatal("Failed to create USS flag dir: ", out, err)
	}
	if out, err := cmdRunner.Run(ctx, "touch", ussFlagFile); err != nil {
		s.Fatal("Failed to create USS flag file: ", out, err)
	}
