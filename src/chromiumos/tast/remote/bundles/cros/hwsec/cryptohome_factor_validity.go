...
func unmountVaultAndTest(ctx context.Context, client *hwsec.CryptohomeClient, hf *files.HomedirFiles) error {
	...
	if err := testMountState(ctx, client, hf, false); err != nil {
...
// existing files is still available and correct. It will also update the file
// contents so that we so that subsequent checks and see that modifications to
// the content of the mount are not lost.
func testMountState(ctx context.Context, client *hwsec.CryptohomeClient, hf *files.HomedirFiles, expected bool) error {
...
func testAddRemoveFactor(ctx context.Context, client *hwsec.CryptohomeClient, hf *files.HomedirFiles) error {
	// Add the new factor.
	err := func() error {
		authSessionID, err := client.StartAuthSession(ctx, util.FirstUsername /*ephemeral=*/, false, uda.AuthIntent_AUTH_INTENT_DECRYPT)
		if err != nil {
	...
	if err := checkFactors(ctx, client,
		...
		return errors.Wrap(err, "list of factor is incorrect after adding factor")
	}
	...
	if err := testMountState(ctx, client, hf, true); err != nil {
	...
	if err != nil {
		return errors.Wrap(err, "was able to mount user with removed factor")
	}
	if err := testMountState(ctx, client, hf, false); err != nil {
...
func testUpdateFactor(ctx context.Context, client *hwsec.CryptohomeClient, hf *files.HomedirFiles) error {
	...
	err := func() error {
		...
		if err := client.UpdatePasswordAuthFactor(ctx, authSessionID, util.Password1Label, util.Password1Label, util.FirstChangedPassword); err != nil {
	...
	// The old password should no longer be able to mount.
	err = func() error {
		authSessionID, err := client.StartAuthSession(ctx, util.FirstUsername /*ephemeral=*/, false, uda.AuthIntent_AUTH_INTENT_DECRYPT)
		if err != nil {
			return errors.Wrap(err, "failed to start auth session")
	...
	if err != nil {
		return errors.Wrap(err, "was able to mount user with old factor after update")
	}
	if err := testMountState(ctx, client, hf, false); err != nil {
		return errors.Wrap(err, "vault mounted after failed mount")
	}
	if err := client.MountVault(ctx, util.Password1Label, hwsec.NewPassAuthConfig(util.FirstUsername, util.FirstPassword1), false, hwsec.NewVaultConfig()); err == nil {
		return errors.New("still can mount vault with old password")
	}

	// Mount with new password and change the password back.
	err = func() error {
		authSessionID, err := client.StartAuthSession(ctx, util.FirstUsername /*ephemeral=*/, false, uda.AuthIntent_AUTH_INTENT_DECRYPT)
		if err != nil {
			return errors.Wrap(err, "failed to start auth session")
	...
	if err != nil {
		return errors.Wrap(err, "failed to mount user with new factor after update")
	}
	if err := testMountState(ctx, client, hf, true); err != nil {
	...
	err = func() error {
		...
		if err := client.UpdatePasswordAuthFactor(ctx, authSessionID, util.Password1Label, util.Password1Label, util.FirstPassword1); err != nil {
			return errors.Wrap(err, "failed to update auth factor")
		}
		return nil
	}()
	if err != nil {
		return errors.Wrap(err, "failed to restore the original auth factor while mounted")
	}
	if err := testMountState(ctx, client, hf, true); err != nil {
		return errors.Wrap(err, "vault not mounted after updating factor while mounted")
	}

	// Authentiating with the new secret should be effective immediately without a remount.
...
// CryptohomeFactorValidity exercises and tests the correctness of adding and
// removing auth factors when the DUT goes through various states (ownership
// not taken, ownership taken, after reboot).
func CryptohomeFactorValidity(ctx context.Context, s *testing.State) {
