...
func create2VaultsForTesting(ctx context.Context, utility *hwsec.CryptohomeClient, keyInfo1, keyInfo2 []keyInfo) (returnedErr error) {
	...
	if err := utility.MountVault(ctx, keyInfo1[0].username, keyInfo1[0].password, keyInfo1[0].keyLabel, true, hwsec.NewVaultConfig()); err != nil {
		return errors.Wrap(err, "failed to create first user")
	}

	if err := utility.MountVault(ctx, keyInfo2[0].username, keyInfo2[0].password, keyInfo2[0].keyLabel, true, hwsec.NewVaultConfig()); err != nil {
...
func checkVaultWorks(ctx context.Context, utility *hwsec.CryptohomeClient, keyInfos []keyInfo) error {
	for _, info := range keyInfos {
		if err := utility.MountVault(ctx, info.username, info.password, info.keyLabel, false, hwsec.NewVaultConfig()); err != nil {
			return errors.Wrapf(err, "failed to mount with %s", info.keyLabel)
		}
		if err := utility.UnmountAll(ctx); err != nil {
			return errors.Wrap(err, "failed to unmount vault")
		}
		if result, _ := utility.CheckVault(ctx, info.username, info.password, info.keyLabel); !result {
...
func checkOthersAreBlocked(ctx context.Context, utility *hwsec.CryptohomeClient, otherKeyInfo []keyInfo) error {
	...
	if err := utility.MountVault(ctx, util.ThirdUsername, util.ThirdPassword, util.Password1Label, true, hwsec.NewVaultConfig()); err == nil {
