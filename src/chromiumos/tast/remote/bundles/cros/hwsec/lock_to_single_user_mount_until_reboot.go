...
func create2VaultsForTesting(ctx context.Context, utility *hwsec.UtilityCryptohomeBinary) (returnedErr error) {
	...
	if err := utility.MountVault(ctx, util.FirstUsername, util.FirstPassword, util.PasswordLabel, true, []string{} /* extraFlags */); err != nil {
		return errors.Wrap(err, "failed to create first user")
	}
	if err := utility.MountVault(ctx, util.SecondUsername, util.SecondPassword, util.PasswordLabel, true, []string{} /* extraFlags */); err != nil {
...
func checkVaultWorks(ctx context.Context, utility *hwsec.UtilityCryptohomeBinary, username, password, pin string) error {
	if err := utility.MountVault(ctx, username, password, util.PasswordLabel, false, []string{} /* extraFlags */); err != nil {
		return errors.Wrap(err, "failed to mount with password")
	}
	if err := utility.UnmountAll(ctx); err != nil {
		return errors.Wrap(err, "failed to unmount vault")
	}
	if err := utility.MountVault(ctx, username, pin, util.PinLabel, false, []string{} /* extraFlags */); err != nil {
		return errors.Wrap(err, "failed to mount with password")
	}
	if err := utility.UnmountAll(ctx); err != nil {
		return errors.Wrap(err, "failed to unmount vault")
...
func checkBothVaultsAreOperational(ctx context.Context, utility *hwsec.UtilityCryptohomeBinary) error {
...
func checkOthersAreBlocked(ctx context.Context, utility *hwsec.UtilityCryptohomeBinary) error {
	// Mount should fail.
	if err := utility.MountVault(ctx, util.SecondUsername, util.SecondPassword, util.PasswordLabel, false, []string{} /* extraFlags */); err == nil {
		return errors.Wrap(err, "second user is mountable with password after locking to single user")
	}
	if err := utility.MountVault(ctx, util.SecondUsername, util.SecondPin, util.PinLabel, false, []string{} /* extraFlags */); err == nil {
	...
	if err := utility.MountVault(ctx, util.ThirdUsername, util.ThirdPassword, util.PasswordLabel, true, []string{} /* extraFlags */); err == nil {
...
func LockToSingleUserMountUntilReboot(ctx context.Context, s *testing.State) {
	...
	if err := checkBothVaultsAreOperational(ctx, utility); err != nil {
		s.Fatal("Initially created vaults doesn't work: ", err)
	}

	// Now mount the first user's vault and lock to single user mount.
	if err := utility.MountVault(ctx, util.FirstUsername, util.FirstPin, util.PinLabel, false, []string{} /* extraFlags */); err != nil {
