...
func create2VaultsForTesting(ctx context.Context, utility *hwsec.UtilityCryptohomeBinary) error {
	...
	if err := utility.MountVault(ctx, hwsec.SecondUsername, hwsec.SecondPassword, hwsec.PasswordLabel, true); err != nil {
	...
	if err := utility.AddVaultKey(ctx, hwsec.FirstUsername, hwsec.FirstPassword, hwsec.PasswordLabel, hwsec.FirstPin, hwsec.PinLabel, false); err != nil {
		return errors.Wrap(err, "failed to add key to vault for first user")
	}
	if err := utility.AddVaultKey(ctx, hwsec.SecondUsername, hwsec.SecondPassword, hwsec.PasswordLabel, hwsec.SecondPin, hwsec.PinLabel, false); err != nil {
...
func cleanupVault(ctx context.Context, utility *hwsec.UtilityCryptohomeBinary) error {
	...
	if _, err := utility.RemoveVault(ctx, hwsec.SecondUsername); err != nil {
...
func checkVaultWorks(ctx context.Context, utility *hwsec.UtilityCryptohomeBinary, username, password, pin string) error {
	if err := utility.MountVault(ctx, username, password, hwsec.PasswordLabel, false); err != nil {
		return errors.Wrap(err, "failed to mount with password")
	}
	if err := utility.UnmountAll(ctx); err != nil {
		return errors.Wrap(err, "failed to unmount vault")
	}
	if err := utility.MountVault(ctx, username, pin, hwsec.PinLabel, false); err != nil {
		return errors.Wrap(err, "failed to mount with password")
	}
	if err := utility.UnmountAll(ctx); err != nil {
		return errors.Wrap(err, "failed to unmount vault")
	...
	if result, _ := utility.CheckVault(ctx, username, password, hwsec.PasswordLabel); !result {
		return errors.New("failed to check key with password")
	}

	if result, _ := utility.CheckVault(ctx, username, pin, hwsec.PinLabel); !result {
...
func checkBothVaultIsOperational(ctx context.Context, utility *hwsec.UtilityCryptohomeBinary) error {
	...
	if err := checkVaultWorks(ctx, utility, hwsec.FirstUsername, hwsec.FirstPassword, hwsec.FirstPin); err != nil {
		return errors.Wrap(err, "first user's vault doesn't work")
	}
	if err := checkVaultWorks(ctx, utility, hwsec.SecondUsername, hwsec.SecondPassword, hwsec.SecondPin); err != nil {
...
// checkOthersAreBlocked will check that users other than hwsec.FirstUsername is blocked from mounting and CheckKeyEx (unlock).
func checkOthersAreBlocked(ctx context.Context, utility *hwsec.UtilityCryptohomeBinary) error {
	// Mount should fail.
	if err := utility.MountVault(ctx, hwsec.SecondUsername, hwsec.SecondPassword, hwsec.PasswordLabel, false); err == nil {
		return errors.Wrap(err, "second user is mountable with password after locking to single user")
	}
	if err := utility.MountVault(ctx, hwsec.SecondUsername, hwsec.SecondPin, hwsec.PinLabel, false); err == nil {
		return errors.Wrap(err, "second user is mountable with pin after locking to single user")
	}

	// CheckKeyEx should fail too.
	if result, _ := utility.CheckVault(ctx, hwsec.SecondUsername, hwsec.SecondPassword, hwsec.PasswordLabel); result {
		return errors.New("check key succeeded after locking to single user")
	}
	if result, _ := utility.CheckVault(ctx, hwsec.SecondUsername, hwsec.SecondPin, hwsec.PinLabel); result {
		return errors.New("check key succeeded with pin after locking to single user")
	}

	// Shouldn't be able to create new users.
	if err := utility.MountVault(ctx, hwsec.ThirdUsername, hwsec.ThirdPassword, hwsec.PasswordLabel, true); err == nil {
...
func LockToSingleUserMountUntilReboot(ctx context.Context, s *testing.State) {
	...
	if err := utility.LockToSingleUserMountUntilReboot(ctx, hwsec.FirstUsername); err != nil {
