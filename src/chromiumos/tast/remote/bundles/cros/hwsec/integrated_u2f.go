...
func setU2fdFlags(ctx context.Context, helper *hwsecremote.FullHelperRemote, u2f, g2f, userKeys bool) (retErr error) {
	...
	// Remove flags.
	if _, err := cmd.Run(ctx, "rm", "-f", "/var/lib/u2f/force/*.force"); err != nil {
...
func u2fDevicePath(ctx context.Context, cmd *hwsecremote.CmdRunnerRemote) (string, error) {
	...
	path := fmt.Sprintf("/sys/bus/hid/devices/*:%s:%s.*/hidraw", VID, PID)
	var dev string
	err := testing.Poll(ctx, func(context.Context) error {
		data, err := cmd.Run(ctx, "ls", path)
		if err != nil {
...
func runU2Test(ctx context.Context, dut *dut.DUT, device string, svo *servo.Servo) error {
	...
	cmd := dut.Command(u2fTestPath, device)
	...
	for true {
		...
		if err != nil {
			return errors.Wrap(err, "failed to read stdout while looking for the new line")
		}
		if strings.Contains(line, "Touch device and hit enter..") {
