...
func RetakeOwnershipLatePreparation(ctx context.Context, s *testing.State) {
	...
	helper, err := libhwsecremote.NewHelper(utility, s.DUT())
	if err != nil {
		s.Fatal("Helper creation error: ", err)
	}
	s.Log("Start resetting TPM if needed")
	if err := helper.EnsureTPMIsReset(ctx, r); err != nil {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err := hwsec.RestartCryptohome(ctx, r); err != nil {
			return err
		}
		if passwd, err := utility.GetOwnerPassword(ctx); err != nil {
			return err
