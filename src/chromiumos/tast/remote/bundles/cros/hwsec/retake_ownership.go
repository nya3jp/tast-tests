...
func RetakeOwnership(ctx context.Context, s *testing.State) {
	...
	if err := helper.EnsureTPMIsReady(ctx, utility, libhwsec.DefaultTakingOwnershipTimeout); err != nil {
	...
	if err := helper.EnsureIsPreparedForEnrollment(ctx,
		utility, libhwsec.DefaultPreparationForEnrolmentTimeout); err != nil {
	...
	checksumOutput, err := r.Run(ctx, "sha256sum", libhwsec.AttestationDBPath)
	if err != nil {
	...
	if checksumOutput2, err := r.Run(ctx, "sha256sum", libhwsec.AttestationDBPath); err != nil {
		s.Fatal("Error calculating checksum for attestation database")
	} else if !bytes.Equal(checksumOutput, checksumOutput2) {
		s.Fatal("Inconsistent checksum after reboot")
	}
	if passwd2, err := utility.GetOwnerPassword(); err != nil {
		s.Fatal("Failed to get owner password")
	...
	lastTime, _ := r.RunShell(ctx, "stat -c %y /var/lib/tpm_manager/local_tpm_data")
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		newTime, _ := r.RunShell(ctx, "stat -c %y /var/lib/tpm_manager/local_tpm_data")
		...
		if err := libhwsec.RestartCryptohome(ctx, r); err != nil {
			return err
		}
