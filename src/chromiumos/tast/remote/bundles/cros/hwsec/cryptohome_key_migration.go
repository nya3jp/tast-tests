...
func CryptohomeKeyMigration(ctx context.Context, s *testing.State) {
	...
	if err := utility.MountVault(ctx, username, passwd, "dummylabel", true); err != nil {
	...
	if result {
		s.Fatal("Vault key set is tpm wrapped")
	}
	...
	result, err = utility.CheckVault(ctx, username, passwd, "dummylabel")
	if err != nil {
