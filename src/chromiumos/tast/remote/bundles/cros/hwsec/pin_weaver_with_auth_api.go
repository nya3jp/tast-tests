...
func PINWeaverWithAuthAPI(ctx context.Context, s *testing.State) {
	...
	if err = authenticateWithCorrectPIN(ctx, ctxForCleanUp, testUser1, cmdRunner, helper, userParam /*shouldAuthenticate=*/, true); err != nil {
	...
	if err = authenticateWithCorrectPIN(ctx, ctxForCleanUp, testUser2, cmdRunner, helper, userParam /*shouldAuthenticate=*/, true); err != nil {
		s.Fatal("Failed to run authenticateWithCorrectPIN with error: ", err)
	}
	...
	if err = attemptWrongPIN(ctx, ctxForCleanUp, testUser1, cmdRunner, helper, userParam /*attempts=*/, 4); err != nil {
	...
	if err = attemptWrongPIN(ctx, ctxForCleanUp, testUser1, cmdRunner, helper, userParam /*attempts=*/, 2); err != nil {
		s.Fatal("Failed to run attemptWrongPIN with error: ", err)
	}

	// After the PIN lock out we should not be able to authenticate with correct PIN.
	if err = authenticateWithCorrectPIN(ctx, ctxForCleanUp, testUser1, cmdRunner, helper, userParam /*shouldAuthenticate=*/, false); err != nil {
...
func setupUserWithPIN(ctx, ctxForCleanUp context.Context, userName string, cmdRunner *hwsecremote.CmdRunnerRemote, helper *hwsecremote.CmdHelperRemote, userParam pinWeaverWithAuthAPIParam) error {
	...
	if userParam.useUserSecretStash {
		...
		err = cryptohomeHelper.AddCredentialsWithAuthSession(ctx, passwordAuthFactorLabel, passwordAuthFactorSecret, authSessionID /*kiosk=*/, false)
	}
...
func authenticateWithCorrectPassword(ctx, ctxForCleanUp context.Context, testUser string, r *hwsecremote.CmdRunnerRemote, helper *hwsecremote.CmdHelperRemote, userParam pinWeaverWithAuthAPIParam) error {
	...
	if userParam.useUserSecretStash {
		...
		err = cryptohomeHelper.AuthenticateAuthSession(ctxForCleanUp, passwordAuthFactorSecret, authSessionID /*kiosk_mount=*/, false)
	}
	if err != nil {
