...
func runTest(ctx context.Context, user1, user2 string, preReboot bool, r *hwsecremote.CmdRunnerRemote, helper *hwsecremote.HelperRemote, s *testing.State) {
	...
	if preReboot {
		if err := helper.DaemonController().Stop(ctx, hwsec.CryptohomeDaemon); err != nil {
			s.Fatal("Failed to stop cryptohome")
		}
		...
		if err := helper.DaemonController().Start(ctx, hwsec.CryptohomeDaemon); err != nil {
			s.Fatal("Failed to start cryptohome")
		}
		...
		if err := utility.MountVault(ctx, user1, testPassword, "default", true, hwsec.NewVaultConfig()); err != nil {
			s.Fatal("Failed to create initial user: ", err)
		}
		if err := utility.AddVaultKey(ctx, user1, testPassword, "default", goodPin, keyLabel1, true); err != nil {
	...
	if err := utility.MountVault(ctx, user1, goodPin, keyLabel1, false, hwsec.NewVaultConfig()); err != nil {
	...
	for i := 0; i < 5; i++ {
		if err := utility.MountVault(ctx, user1, badPin, keyLabel1, false, hwsec.NewVaultConfig()); err == nil {
			s.Fatal("Mount succeeded but should have failed")
		}
	}

	if err := utility.MountVault(ctx, user1, testPassword, "default", false, hwsec.NewVaultConfig()); err != nil {
	...
	if err := utility.MountVault(ctx, user2, testPassword, "default", true, hwsec.NewVaultConfig()); err != nil {
		s.Fatal("Failed to create user2: ", err)
	}

	leCredsBeforeAdd := getLECredsFromDisk(ctx, r, s)

	if err := utility.AddVaultKey(ctx, user2, testPassword, "default", goodPin, keyLabel1, true); err != nil {
		s.Fatal("Failed to add le credential: ", err)
	}
	if err := utility.AddVaultKey(ctx, user2, testPassword, "default", goodPin, keyLabel2, true); err != nil {
		s.Fatal("Failed to add le credential: ", err)
	}
	if err := utility.UnmountAll(ctx); err != nil {
		s.Fatal("Failed to unmountAll: ", err)
	...
	if _, err := utility.RemoveVault(ctx, user2); err != nil {
