...
func CryptohomeMountPerf(ctx context.Context, s *testing.State) {
	...
	normalMountIterations := int64(10)
	if val, ok := s.Var("hwsec.CryptohomeMountPerf.normalMountIterations"); ok {
		var err error
		normalMountIterations, err = strconv.ParseInt(val, 10, 64)
		if err != nil {
			s.Fatal("Unparsable normalMountIterations variable: ", err)
		}
	}

	rebootMountIterations := int64(3)
	if val, ok := s.Var("hwsec.CryptohomeMountPerf.rebootMountIterations"); ok {
		var err error
		normalMountIterations, err = strconv.ParseInt(val, 10, 64)
		if err != nil {
			s.Fatal("Unparsable rebootMountIterations variable: ", err)
		}
	}
	...
	for i := int64(0); i < normalMountIterations; i++ {
		startTs := time.Now()
	...
	for i := int64(0); i < rebootMountIterations; i++ {
