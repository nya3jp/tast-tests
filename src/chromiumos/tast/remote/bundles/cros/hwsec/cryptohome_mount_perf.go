...
func CryptohomeMountPerf(ctx context.Context, s *testing.State) {
	...
	startTs := time.Now()
	if err := cryptohome.MountVault(ctx, util.FirstUsername, util.FirstPassword1, util.Password1Label, true, hwsec.NewVaultConfig()); err != nil {
	...
	for i := 0; i < normalMountIterations; i++ {
		startTs := time.Now()
		err := cryptohome.MountVault(ctx, util.FirstUsername, util.FirstPassword1, util.Password1Label, false, hwsec.NewVaultConfig())
