...
func SimpleConnect(ctx context.Context, s *testing.State) {
	...
	// Steal dut's key to connect to router...
	...
	// Setup AP.
	ssid := wifi.RandomSSID("AAA_TEST_POC_")
	...
	testing.ContextLog(ctx, "AP setup done")

	// This is a PoC, so... let's sleep to wait scan.
	testing.Sleep(ctx, 5*time.Second)

	testing.ContextLog(ctx, "Try to connect")
