...
func SystemProxyForArc(ctx context.Context, s *testing.State) {
	...
	if _, err := pc.CreateArcInstance(ctx, &empty.Empty{}); err != nil {
		s.Fatal("Failure to start ARC: ", err)
	}
	...
	if err != nil {
		s.Fatal("Failure to test ARC app connectivity: ", err)
	}
	...
	if rez.Response != successCode {
		s.Fatalf("Reponse code %s; want %s", rez.Response, successCode)
	}
}
