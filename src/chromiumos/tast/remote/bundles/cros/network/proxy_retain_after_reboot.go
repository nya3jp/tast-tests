...
func ProxyRetainAfterReboot(ctx context.Context, s *testing.State) {
	...
	proxyValues := &network.ProxyConfigs{
		HttpHost:  "localhost",
		HttpPort:  "123",
		HttpsHost: "localhost",
		HttpsPort: "456",
	...
	func() {
		...
		if _, err := proxyRebootClient.Setup(ctx, proxyValues); err != nil {
	...
	returnedValues, err := proxyRebootClient.FetchConfigurations(ctx, &empty.Empty{})
	if err != nil {
		s.Fatal("Failed to setup proxy: ", err)
	}

	if diff := cmp.Diff(returnedValues, proxyValues, protocmp.Transform()); diff != "" {
