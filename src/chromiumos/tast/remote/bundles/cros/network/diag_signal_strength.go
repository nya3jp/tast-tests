...
func DiagSignalStrength(ctx context.Context, s *testing.State) {
	...
	pollAndValidateRoutine := func(expectedResult *diag.RoutineResult, attenuate bool) error {
		req := &network.RunRoutineRequest{
			Routine: diag.RoutineSignalStrength,
		}
		// Starting attenuation value.
		weakConnectionAttenuationDb := 50.0

		if err := testing.Poll(ctx, func(ctx context.Context) error {
			// If we attenuate the signal, start at the initial attenuation value
			// and slowly increase it until the desired routine result is
			// achieved.
			if attenuate {
				defer func() {
					...
					weakConnectionAttenuationDb = weakConnectionAttenuationDb + 1.0
				}()
			}
	...
	// Do not attenuate the signal. The routine is expected to pass with no issues.
	expectedResult := &diag.RoutineResult{
		Verdict:  diag.VerdictNoProblem,
		Problems: []uint32{},
	}
	if err := pollAndValidateRoutine(expectedResult, false); err != nil {
	...
	// tests.
	// TODO(http://b/188068031): remove when handeled by the fixture.
	defer func() {
		if err := setAttenuation(0); err != nil {
	...
	if err := pollAndValidateRoutine(expectedResult, true); err != nil {
