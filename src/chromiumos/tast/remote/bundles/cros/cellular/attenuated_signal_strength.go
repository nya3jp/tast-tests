...
// rxPower fetches the current signal power (dBm) received at the dut.
...
func AttenuatedSignalStrength(ctx context.Context, s *testing.State) {
	...
	// calculate initial calibration offset
	offset := pReq - pMeas
	...
	for i := 0; i < tc.stepCount; i++ {
		pReq -= stepSize
		req := &manager.ConfigureRxPowerRequestBody{Power: manager.NewRxPower(pReq + offset)}
		...
		strength = serviceResp.Strength
		s.Logf("Power: %f, strength: %d %%, offset: %f dBm", pMeas, strength, offset)
...
func (s signalStrengthTest) waitForPower(ctx context.Context, client cellular.RemoteCellularServiceClient, pTarget float64) (float64, error) {
	...
	if err = testing.Poll(ctx, func(ctx context.Context) error {
		pMeas, err = s.fetchPower(ctx, client)
		if err != nil {
			return errors.Wrap(err, "failed to get DUT Rx signal properties")
		}

		if math.Abs(pTarget-pMeas) > rsrpMargin {
			return errors.Errorf("failed to wait for DUT Rx power to reach %f, got %f", pTarget, pMeas)
		}
