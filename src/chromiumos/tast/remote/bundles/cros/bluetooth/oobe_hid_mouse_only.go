...
func OobeHidMouseOnly(ctx context.Context, s *testing.State) {
	...
	if result, err := fv.BTPeers[0].BluetoothMouseDevice().AdapterPowerOn(ctx); err != nil || !result {
	...
	if result, err := fv.BTPeers[0].BluetoothMouseDevice().AdapterPowerOff(ctx); err != nil || !result {
	...
	// after test ends.
	if err := fv.BTPeers[0].BluetoothMouseDevice().Reset(ctx); err != nil {
		s.Fatal("Failed to reset device: ", err)
	}

	if result, err := fv.BTPeers[0].BluetoothMouseDevice().AdapterPowerOff(ctx); err != nil || !result {
		s.Fatal("Failed to power off btPeer adapter: ", err)
	}
}
