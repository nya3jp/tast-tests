...
func DiscoverEmulatedBTPeerDevices(ctx context.Context, s *testing.State) {
	...
	devices, err := fv.BluetoothService.Devices(ctx, &emptypb.Empty{})
	if err != nil {
		s.Fatal("Failed to get devices: ", err)
	}

	// Check that both the keyboard and the mouse are discoverable.
	if !deviceWithAddressExists(keyboardDevice.LocalBluetoothAddress(), devices) {
		s.Fatal("Failed to discover the keyboard")
	}
	if !deviceWithAddressExists(mouseDevice.LocalBluetoothAddress(), devices) {
