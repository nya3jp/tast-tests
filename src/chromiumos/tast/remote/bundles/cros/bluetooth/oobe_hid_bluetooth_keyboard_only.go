...
// OobeHidKeyboardOnly tests that a single Blueooth mouse is connected to during OOBE.
func OobeHidKeyboardOnly(ctx context.Context, s *testing.State) {
	...
	if err := checkNodeWithNameExists(ctx, uiautoSvc, s, UsbKeyboardConnectedNode); err == nil {
	...
	if err := checkNodeWithNameExists(ctx, uiautoSvc, s, SearchingForKeyboardNodeName); err != nil {
	...
	if err := checkNodeWithNameExists(ctx, uiautoSvc, s, FoundKeyboardNodeName); err != nil {
		s.Fatal("Failed to find node: ", err)
	}
