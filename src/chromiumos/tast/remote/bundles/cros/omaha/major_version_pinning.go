...
func MajorVersionPinning(ctx context.Context, s *testing.State) {
	...
	for vpChromeVersion, vpChromeOSVersion := range state.Config.ChromeOSVersionFromMilestone {
		if vpChromeVersion > state.Config.CurrentStableChrome {
			s.Logf("Not testing version %d higher than stable %d", vpChromeVersion, state.Config.CurrentStableChrome)
			continue
		}
