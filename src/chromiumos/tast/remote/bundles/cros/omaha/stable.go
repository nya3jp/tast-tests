...
func Stable(ctx context.Context, s *testing.State) {
	...
	prevVersion, err := state.Config.PreviousMilestoneOSVersion(state.Config.StableChrome)
	if err != nil {
	...
	prevMilestone, err := state.Config.PreviousMilestone(state.Config.StableChrome)
	if err != nil {
		s.Fatal("Failed to get previous version: ", err)
	}
	...
	} else if !request.MatchOneOfVersions(chromeVersion, state.Config.StableChrome, prevMilestone) {
		s.Errorf("Chrome Version %q does not match the current %d or previous %d milestone", chromeVersion, state.Config.StableChrome, prevMilestone)
	}

	currentStableChromeOS := state.Config.ChromeOSVersionFromMilestone[state.Config.StableChrome]
