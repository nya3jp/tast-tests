...
func Stable(ctx context.Context, s *testing.State) {
	...
	req.Apps = append(req.Apps, request.GenerateRequestApp(params.Device, params.Config.OldVersion, request.Stable))
	...
	} else if !request.MatchOneOfVersions(chromeVersion, params.Config.CurrentStableChrome, params.Config.NextStableChrome) {
		s.Errorf("Chrome Version %q does not match the current %d or next %d milestone", chromeVersion, params.Config.CurrentStableChrome, params.Config.NextStableChrome)
	}

	currentStableChromeOS := params.Config.ChromeOSVersionFromMilestone[params.Config.CurrentStableChrome]
	nextStableChromeOS := params.Config.ChromeOSVersionFromMilestone[params.Config.NextStableChrome]
