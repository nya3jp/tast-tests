...
func Stable(ctx context.Context, s *testing.State) {
	...
	currentChromeOSStable := state.Config.CurrentChromeOSStable()

	prevVersion, err := state.Config.PreviousMilestoneOSVersion(currentChromeOSStable)
	if err != nil {
	...
	prevMilestone, err := state.Config.PreviousMilestone(currentChromeOSStable)
	if err != nil {
		s.Fatal("Failed to get previous version: ", err)
	}
	...
	} else if !request.MatchOneOfVersions(chromeVersion, currentChromeOSStable, prevMilestone) {
		s.Errorf("Chrome Version %q does not match the current %d or previous %d milestone", chromeVersion, currentChromeOSStable, prevMilestone)
	}

	currentStableChromeOS := state.Config.ChromeOSVersionFromMilestone[currentChromeOSStable]
