...
func (d *Configuration) PreviousMilestone(milestone int) (int, error) {
	// M95 does not exist, we still need previous to work for us to test M96 LTS rollback.
	// Missing milestones are not a common occurrence and the function should fail if one is missing.
	if milestone == 96 {
		if _, ok := d.ChromeOSVersionFromMilestone[94]; ok {
			return 94, nil
		}
	}
