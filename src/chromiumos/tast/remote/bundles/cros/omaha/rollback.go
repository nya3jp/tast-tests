...
func Rollback(ctx context.Context, s *testing.State) {
	...
	chromeOSStableInt := state.Config.ChromeOSVersionFromMilestone[state.Config.StableChrome]
	...
	for vpMilestone, vpChromeOSVersion := range state.Config.ChromeOSVersionFromMilestone {
		if vpMilestone > state.Config.StableChrome {
			s.Logf("Not testing version %d higher than stable %d", vpMilestone, state.Config.StableChrome)
			continue
		}
		...
		if vpMilestone < state.Config.StableChrome-4 {
