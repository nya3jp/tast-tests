...
func drive039GCompanionPhone(ctx context.Context, dut *utils.Device, contact, hangoutsURL string) error {
	defer func() {
	...
	if !sent.True {
		dut.SendMessageToChromeOS(ctx, "Hi")
	}
...
func drive039GDUT(ctx context.Context, s *testing.State) error {
...
func cleanup039GCompanionPhone(ctx context.Context, dut *utils.Device, deviceName string) {
...
func MTBF039GBTHangoutMain(ctx context.Context, s *testing.State) {
	...
	report, _, err := androidTest.RunDelegate(ctx, sdk.CaseDescription{
		...
		if err := drive039GCompanionPhone(ctx, compDev, contact, hangoutsURL); err != nil {
			utils.FailCase(ctx, client, err)
		}

		if err := drive039GDUT(ctx, s); err != nil {
			utils.FailCase(ctx, client, err)
		}
		return nil, nil
	}, func(ctx context.Context, client sdk.DelegateClient) (interface{}, error) {
		...
		cleanup039GCompanionPhone(ctx, compDev, deviceHostname)
		return nil, nil
	})
