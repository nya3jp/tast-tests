...
func init() {
	testing.AddTest(&testing.Test{
		Func:     MTBF004GFacebookVideoPlaying,
...
func MTBF004GFacebookVideoPlaying(ctx context.Context, s *testing.State) {
	...
	report, _, mtbferr := androidTest.RunDelegate(ctx, sdk.CaseDescription{
		...
		if err := verifyFacebookVideoPlaying(ctx, s, dutDev); err != nil {
...
func verifyFacebookVideoPlaying(ctx context.Context, s *testing.State, dut *utils.Device) error {
	...
	dut.Client.UIAClick(dut.DeviceID).Coordinate(coord).Do(ctx, service.Sleep(0))
