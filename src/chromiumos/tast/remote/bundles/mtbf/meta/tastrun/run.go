...
// RunTestWithRelogin run test with relogin
func RunTestWithRelogin(ctx context.Context, s *testing.State, flags []string, testCases []string, ErrList []string) error {
	reloginTestName := "mtbfutil.Relogin"
	for _, testName := range testCases {
		if err := RunTestWithFlags(ctx, s, flags, testName); err != nil {
			isRelogin := false
			for _, errorStr := range ErrList {
			...
			if isRelogin {
				...
				if err := RunTestWithFlags(ctx, s, flags, reloginTestName); err != nil {
					return err
				}
			}
			return err
...
func getFileNum(path string, prefix string) (int, error) {
	i := 0
