...
func EnsureBatteryPercentage(ctx context.Context, c *rpc.Client, s *servo.Servo, minLevel, maxLevel float64) error {
	...
	// If a range is provided, that means it is intended for some sort of tesing. So putting the battery into avg makes sense
	// even if power source gets disconnected/reconnected to avoid flakiness
	avgLevel := minLevel + (maxLevel-minLevel)/2.0
