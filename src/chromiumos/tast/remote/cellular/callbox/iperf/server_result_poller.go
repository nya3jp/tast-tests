...
// serverPoller is responsible for starting/stopping callbox server results polling sessions in the background.
//
// Note: while resultsPoller runs the results in the background, the resultsPoller struct itself is not thread safe i.e. multiple start/stop calls cannot be made in parallel.
...
// pollSession is a single session of results polling.
...
func (s *serverResultPoller) start(ctx context.Context, cfg *iperf.Config) {
	s.stop(ctx)
...
// stop stops the current polling session (if any is active).
	...
	for time.Now().Before(s.endTime) {
		...
		if err != nil {
			if !errors.Is(err, context.Canceled) {
				testing.ContextLog(ctx, "Failed to fetch results from callbox, err: ")
			}
			return
		}
...
func (s *serverPollSession) fetchResult(ctx context.Context) (*iperf.Result, error) {
	...
	// make sure any polling routines are stopped
	s.stop(ctx)
