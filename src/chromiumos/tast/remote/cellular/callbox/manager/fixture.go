...
func (tf *TestFixture) TearDown(ctx context.Context, s *testing.FixtState) {
	...
	if tf.fcm != nil {
		if err := tf.fcm.Close(ctx); err != nil {
			allErrors = errors.Wrapf(allErrors, "failed to close tunnel to CallboxManager: %v", err) // NOLINT
		}
	}

	if _, err := tf.ChromeServiceClient.Close(ctx, &empty.Empty{}); err != nil {
		allErrors = errors.Wrapf(allErrors, "failed to close Chrome: %v", err) // NOLINT
	}

	// Ignore the error of rpc.Close as aborting rpc daemon will always have error.
	tf.crRPCClient.Close(ctx)

	if allErrors != nil {
		s.Fatal("Failed during teardown: ", allErrors)
	}
}
