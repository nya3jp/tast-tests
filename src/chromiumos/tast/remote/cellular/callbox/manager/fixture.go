...
func (tf *TestFixture) TearDown(ctx context.Context, s *testing.FixtState) {
	if tf.fcm != nil {
		if err := tf.fcm.Close(ctx); err != nil {
			s.Error("Failed to close tunnel to CallboxManager during tear down: ", err)
		}
	}

	if _, err := tf.ChromeServiceClient.Close(ctx, &empty.Empty{}); err != nil {
		s.Error("Failed to close Chrome during tear down: ", err)
	}

	if err := tf.crRPCClient.Close(ctx); err != nil {
		s.Error("Failed to close Chrome RPC client during tear down: ", err)
	}
}
