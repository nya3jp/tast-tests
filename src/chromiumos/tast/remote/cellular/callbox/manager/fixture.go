...
func (tf *TestFixture) ConnectToCallbox(ctx context.Context, s *testing.State, dutConn *ssh.Conn, configureRequestBody *ConfigureCallboxRequestBody, cellularInterface string) interface{} {
	...
	dbusSendArgs := []string{
		"--system", "--fixed", "--print-reply",
		"--dest=org.chromium.flimflam", "/",
		"org.chromium.flimflam.Manager.DisableTechnology", "string:cellular",
	}
        if err := dutConn.CommandContext(ctx, "dbus-send", dbusSendArgs).Run(exec.DumpLogOnError); err != nil {
	...
	dbusSendArgs = []string{
		"--system", "--fixed", "--print-reply",
		"--dest=org.chromium.flimflam", "/",
		"org.chromium.flimflam.Manager.EnableTechnology", "string:cellular",
	}
        if err := dutConn.CommandContext(ctx, "dbus-send", dbusSendArgs).Run(exec.DumpLogOnError); err != nil {
