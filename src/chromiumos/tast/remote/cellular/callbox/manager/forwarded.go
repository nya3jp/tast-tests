...
func newForwardedCallboxManager(ctx context.Context, config *forwardedCallboxManagerConfig) (*forwardedCallboxManager, error) {
	...
	sshOptions := &ssh.Options{
		...
		User:           config.sshUser,
		Hostname:       fmt.Sprintf("%s:%d", config.sshHostname, 2300),
	}
	...
	fcm.localPortForwarder, err = conn.ForwardLocalToRemote("tcp", "localhost:5000", config.remoteCallboxManagerAddress, onFwdError)
	if err != nil {
