...
// TestContext sets up the context for a basic wifi test.
...
// NewTestContext connects to rotuer and gRPC on DUT and then returns a TestContext.
// TODO: kind of awkward that tast-lint complains about passing testing.State,
// and we'll always have to extract all the information outside.
func NewTestContext(ctx context.Context, dut *dut.DUT, rpcHint *testing.RPCHint, routerHostname string) (ret *TestContext, retErr error) {
	tc := &TestContext{}
	defer func() {
		if retErr != nil {
			tc.Close(ctx)
...
// Close the connections created by TestContext.
...
func (tc *TestContext) ConfigureAP(ctx context.Context, ops ...hostap.Option) (*HostAPHandle, error) {
	apConf := hostap.NewConfig(ops...)
	return tc.router.StartHostAP(ctx, apConf)
}

// DeconfigAP stops the wifi service on router.
func (tc *TestContext) DeconfigAP(ctx context.Context, ap *HostAPHandle) error {
	return tc.router.StopHostAP(ctx, ap)
}

// ConnectWifi asks DUT to connect to the given wifi service.
func (tc *TestContext) ConnectWifi(ctx context.Context, ap *HostAPHandle) error {
...
func (tc *TestContext) DisconnectWifi(ctx context.Context) error {
	var err error
	wc := network.NewWifiClient(tc.rpc.Conn)
...
func (tc *TestContext) PingFromDUT(ctx context.Context) error {
	if tc.curHostAP == nil {
