...
func (ap *Server) confPath() string {
	return path.Join(ap.workDir, ap.filename("suffix"))
}
...
func (ap *Server) spawnStdoutParser(ctx context.Context, stdout io.Reader) chan error {
	...
	go func() {
		...
		if err := fileutils.ReadToOutDir(ctx, ap.stdoutFile(), pr, nil); err != nil {
...
func (ap *Server) start(ctx context.Context) (err error) {
	...
	// Collect stderr.
	if err := fileutils.ReadToOutDir(ctx, ap.stderrFile(), stderr, nil); err != nil {
