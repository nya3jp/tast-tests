...
// TODO(yenlinlai): Extract common daemon logic (from dhcp and hostap).
...
func NewServer(ctx context.Context, host *host.SSH, name, iface, workDir string, config *Config) (*Server, error) {
	server := &Server{
		host:    host,
		name:    name,
		conf:    config,
...
func (ap *Server) confPath() string {
	return path.Join(ap.workDir, fmt.Sprintf("hostapd-%s-%s.conf", ap.name, ap.iface))
}

func (ap *Server) ctrlPath() string {
	return path.Join(ap.workDir, fmt.Sprintf("hostapd-%s-%s.ctrl", ap.name, ap.iface))
}

func (ap *Server) stdoutFile() string {
	return fmt.Sprintf("hostapd-%s-%s.stdout", ap.name, ap.iface)
}

func (ap *Server) stderrFile() string {
	return fmt.Sprintf("hostapd-%s-%s.stderr", ap.name, ap.iface)
}
...
func (ap *Server) start(ctx context.Context) (err error) {
	...
	go func() {
		...
		if err := utils.ReadToOutDir(ctx, ap.stdoutFile(), pr, nil); err != nil {
		...
		// Service ready.
		close(done)
		// TODO(yenlinlai): detect events from log (e.g. deauth, disconnect...)
		io.Copy(pw, stdout)
	}()
