...
// Server is the object to control the hostapd on router.
type Server struct {
...
// NewServer creates a new Server object and runs hostapd on the ssh host.
func NewServer(ctx context.Context, host *host.SSH, iface, workDir string, c *Config) (*Server, error) {
	server := &Server{
		host:    host,
		conf:    c,
		iface:   iface,
