...
// freqToChannelMap ported from Autotest.
// TODO: Not sure if we really want to keep this map as iw already
// prints channel num along with supported frequency. I guess the
// main difference is that the table here exclude channels cannot
// be used in US.
...
func writeToDUT(ctx context.Context, dut *dut.DUT, path string, data []byte) error {
	...
	// It is possible to write to a local temp file and upload it, but the putfile
	// function is not yet exported from dut.DUT, to prevent spreading PoC in
	// multiple repos, use this as a working placeholder for now.
