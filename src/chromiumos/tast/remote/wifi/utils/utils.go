...
// WriteToHostDirect writes content to a remote file on given host with tee.
// This is a alternative of "echo $content > $path".
// TODO(yenlinlai): This is a temporary work-around. host.SSH.PutFiles will try to compress
// and untar on remote which for now has some problems when operating on procfs.
func WriteToHostDirect(ctx context.Context, hst *host.SSH, path string, content []byte) error {
	tee := hst.Command("tee", path)
...
func SyncTime(ctx context.Context, hst utils.Commander) error {
	const base = 1e9
	t := time.Now().UnixNano()
	timeStr := fmt.Sprintf("%d.%09d", t/base, t%base)
	if hst.Command("date", "-u", fmt.Sprintf("--set=@%s", timeStr)).Run(ctx) == nil {
		return nil
	}
	// Retry with Busybox format "%Y%m%d%H%M.%S".
	const format = "200601021504.05"
	bbTime := time.Now().Format(format)
	return hst.Command("data", "-u", bbTime).Run(ctx)
}
