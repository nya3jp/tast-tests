...
// Config is the config to setup dhcp server.
type Config struct {
	IPIndex byte
}

// NewConfig creates config to spawn DHCPServer.
...
// Server is the object to contorl DHCP server on AP router.
type Server struct {
...
// NewServer creates and runs a DHCP server on a test AP.
...
// IPIndex returns the ip index used to configure this dhcp server.
func (d *Server) IPIndex() byte {
	return d.conf.IPIndex
}
