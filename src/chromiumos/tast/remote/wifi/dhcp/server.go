...
import (
	...
	"chromiumos/tast/remote/wifi/fileutils"
...
type Config struct {
	// SubnetIdx is used as part of the subnet 192.168.{SubnetIdx}.0/24 managed by the DHCP server.
	SubnetIdx byte
}

// NewConfig creates a Config to spawn DHCPServer.
func NewConfig(subnetIdx byte) *Config {
	return &Config{
		SubnetIdx: subnetIdx,
	}
}
...
func NewServer(ctx context.Context, host *host.SSH, name, iface, workDir string, conf *Config) (*Server, error) {
	server := &Server{
		host:    host,
		name:    name,
		iface:   iface,
...
// confPath is the location on host of dnsmasq.conf for this instance.
func (d *Server) confPath() string {
	return path.Join(d.workDir, d.filename("conf"))
}

// leasePath is the localtion on host of dhcp lease file.
func (d *Server) leasePath() string {
	return path.Join(d.workDir, dhcpLeaseFile)
}

// stdoutFile is the filename under OutDir to store stdout of this daemon.
func (d *Server) stdoutFile() string {
	return d.filename("stdout")
}

// stderrFile is the filename under OutDir to store stderr of this daemon.
...
func (d *Server) start(ctx context.Context) (err error) {
	...
	if err := fileutils.ReadToOutDir(ctx, d.stdoutFile(), stdout, nil); err != nil {
		return errors.Wrap(err, "failed to spawn reader for stdout")
	}
	if err := fileutils.ReadToOutDir(ctx, d.stderrFile(), stderr, nil); err != nil {
...
func (d *Server) getSubnetIP(suffix byte) net.IP {
	return net.IPv4(192, 168, d.conf.SubnetIdx, suffix)
}
