...
func (d *Server) confPath() string {
	return path.Join(d.workDir, fmt.Sprintf("dnsmasq-%s-%s.conf", d.name, d.iface))
}
...
func (d *Server) stdoutFile() string {
	return fmt.Sprintf("dnsmasq-%s-%s.stdout", d.name, d.iface)
}

func (d *Server) stderrFile() string {
	return fmt.Sprintf("dnsmasq-%s-%s.stderr", d.name, d.iface)
}
