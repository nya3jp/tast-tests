...
// NewNearbyShareFixture creates a fixture for neabry share tests in different configurations.
func NewNearbyShareFixture(dataUsage nearbysetup.DataUsage, visibility nearbysetup.Visibility, gaiaLogin bool) testing.FixtureImpl {
...
func init() {
	testing.AddFixture(&testing.Fixture{
		Name: "nearbyShareRemoteDataUsageOfflineAllContactsTestUser",
		Desc: "Fixture for Nearby Share's CB -> CB tests. Configured with 'Data Usage' set to 'Offline' and 'Visibility' set to 'All Contacts'.",
...
func (f *nearbyShareFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	// TODO(crbug/1127165): Remove after data is supported in fixture.
	// Workaround to read data files downloaded in other tests.
	const (
		prebuiltLocalDataPath = "/usr/local/share/tast/data/chromiumos/tast/remote/bundles/cros/nearbyshare/data"
		builtLocalDataPath    = "/home/dhaddock/trunk/src/platform/tast-tests/src/chromiumos/tast/remote/bundles/cros/nearbyshare/data/"
	)
	s.Log("Moving data files to DUT1 (Sender)")
	for _, data := range f.testFiles {
		remoteFilePath := filepath.Join(remoteDir, data)
		if _, err := linuxssh.PutFiles(ctx, d1.Conn(), map[string]string{filepath.Join(builtLocalDataPath, data): remoteFilePath}, linuxssh.DereferenceSymlinks); err != nil {
	...
	var senderUsername, senderPassword string
	if f.gaiaLogin {
	...
	var receiverUsername, receiverPassword string
	if f.gaiaLogin {
...
func (f *nearbyShareFixture) TearDown(ctx context.Context, s *testing.FixtState) {
	// Delete the files from the sender.
