...
func GetDeviceInfo(ctx context.Context, dut *dut.DUT, rpcHint *testing.RPCHint) (DeviceInfo, error) {
	...
	if err != nil {
		return deviceInfo, errors.Wrap(err, "failed to get all the required information from lsb-release")
	}

	deviceInfo.Board = lsbContent[lsbrelease.Board]
	deviceInfo.Version = lsbContent[lsbrelease.Version]
	deviceInfo.Milestone, err = strconv.Atoi(lsbContent[lsbrelease.Milestone])
	if err != nil {
		return deviceInfo, errors.Wrapf(err, "failed to convert milestone to integer %s", lsbContent[lsbrelease.Milestone])
	}
...
// during rollback. Returns sensitive data to check it is not accidently logged
// anywhere during rollback.
...
// VerifyRollbackData ensures that sensitive data was not accidently logged
