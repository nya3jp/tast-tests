...
func init() {
	testing.AddFixture(&testing.Fixture{
		...
		Desc: "Fixture cleaning enrollment related state. Use when your local test performs REAL enrollment and proper clean up is necessary",
		Contacts: []string{
...
func (co *cleanOwner) Reset(ctx context.Context) error {
	if err := EnsureTPMAndSystemStateAreReset(ctx, co.dut); err != nil {
