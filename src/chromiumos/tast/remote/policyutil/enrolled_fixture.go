...
func (e *enrolledFixt) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		// This helps differntiate real issues from timeout hitting different components.
		...
		defer func() {
			...
			chromeDir := path.Join(s.OutDir(), "Chrome"+triesStr)
			...
			fdmsDir := path.Join(s.OutDir(), "FakeDMS"+triesStr)
