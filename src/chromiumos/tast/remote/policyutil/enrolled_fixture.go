...
func getVPDContent(ctx context.Context, d *dut.DUT) ([]byte, error) {
...
func checkVPDState(ctx context.Context, d *dut.DUT) error {
	...
	if out, err := getVPDContent(ctx, d); err != nil {
		return err
		...
		// Try reading the VPD again to make sure it's really invalild.
		out, err := getVPDContent(ctx, d)
		if err != nil {
