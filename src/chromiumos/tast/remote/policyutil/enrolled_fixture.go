...
func enroll(ctx context.Context, attemptDir string, dut *dut.DUT, rpcHint *testing.RPCHint, fdmsDir string, vpdOK bool) (retErr error) {
	...
	defer func(ctx context.Context, ok bool) {
		// No need for a screenshot is the enrollment was seccessful.
		if ok {
			return
		}

		screenshotService := graphics.NewScreenshotServiceClient(cl.Conn)
		prefix := "enrollment" + path.Base(attemptDir)
		if _, err := screenshotService.CaptureScreenshot(ctx,
			&graphics.CaptureScreenshotRequest{FilePrefix: prefix},
			// &graphics.CaptureScreenshotRequest{FilePrefix: "enrolledError"},
		); err != nil {
