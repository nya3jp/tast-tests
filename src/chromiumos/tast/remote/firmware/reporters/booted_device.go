...
const (
	mainfwKey        string = "mainfw_type"
	kernvfyKey       string = "kernkey_vfy"
...
func (r *reporter) BootedDevice(ctx context.Context) (BootedDeviceType, error) {
	if cs, err := r.Crossystem(ctx, mainfwKey, kernvfyKey); err != nil {
		return "", err
	...
	} else if bootedDevice, found := verificationMap[lookupTuple{cs[mainfwKey], removable, cs[kernvfyKey]}]; !found {
		return "", errors.Errorf("unrecognized combination of %v=%v, %v=%v, %v=%v", mainfwKey, cs[mainfwKey], "removable", removable, kernvfyKey, cs[kernvfyKey])
	} else {
...
func isRemovableDevice(ctx context.Context, r *reporter, device string) (bool, error) {
	...
	baseDev := stripPart(strings.Split(device, "/")[2])
...
// stripPart removes the partition portion of the device
func stripPart(devWithPart string) string {
	return rDevNameStripper.ReplaceAllString(devWithPart, "")
}
