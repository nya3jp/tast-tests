...
// ConfigDatafiles returns the names of all datafiles contained in fw-testing-configs, as well as the directory itself.
// It is intended to be used in the Data field of a testing.Test declaration.
func ConfigDatafiles() []string {
	var dfs = make([]string, len(configPlatforms)+1)
	for i, platform := range configPlatforms {
		dfs[i] = fmt.Sprintf("%s/%s.json", ConfigDir, platform)
	}
	dfs[len(dfs)-1] = ConfigDir
	return dfs
}
