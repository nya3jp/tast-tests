...
func parentFromBytes(b []byte) (string, error) {
	...
	if cfg.Parent != "" {
		...
		// No parent specified, so inherit from the default config.
...
func NewConfig(configDataDir, platform string) (*Config, error) {
	// Load JSON config bytes for platform and its (grand*)parents, and for DEFAULTS, in an array ordered from least significant (DEFAULTS) to most significant (platform).
	var bInheritance [][]byte
	var err error
	for ; platform != ""; platform, err = parentFromBytes(bInheritance[0]) {
		if err != nil {
			return nil, errors.Wrapf(err, "determining parent platform from bytes %s", bInheritance[0])
		}
		b, err := loadBytes(configDataDir, platform)
		if err != nil {
			return nil, errors.Wrapf(err, "loading config bytes for platform %s", platform)
		}
		bInheritance = append([][]byte{b}, bInheritance...) // prepend b to bInheritance
	}

	// Unmarshal bytes to a Config object, starting with least significant configs (DEFAULTS) and overwriting with more significant configs.
	var cfg *Config
	for _, b := range bInheritance {
		err = json.Unmarshal(b, &cfg)
		if err != nil {
			return nil, errors.Wrapf(err, "unmarshaling json bytes %s onto config %+v", b, cfg)
		}
	}
	return cfg, nil
}
