...
// RecMode boots to Receover Mode.
...
var normalMode = &impl{&Value{common.BootModeNormal, nil}, nil, prepareTimeout}
var devMode = &impl{&Value{common.BootModeDev, nil}, nil, prepareTimeout}
var recMode = &impl{&Value{common.BootModeRecovery, nil}, nil, prepareTimeout}
...
func (i *impl) Prepare(ctx context.Context, s *testing.PreState) interface{} {
	...
	if mode != i.v.BootMode {
		if err := i.setAndCheckBootMode(ctx, i.v.BootMode); err != nil {
			...
			s.Logf("Successuflly prepared DUT to be in boot mode %q", i.v.BootMode)
		}
	} else {
		s.Logf("DUT is already in boot mode %q", i.v.BootMode)
	}
...
func (i *impl) Close(ctx context.Context, s *testing.PreState) {
	...
	// Restore the boot mode if known.
	if i.origBootMode != nil {
		if mode, err := i.v.Helper.Reporter.CurrentBootMode(ctx); err != nil {
			s.Error("Could not get boot mode: ", err)
		} else if mode != *i.origBootMode {
...
// setAndCheckBootMode reboots to the specified mode and checks to ensure it worked, it assumes the helper is present.
func (i *impl) setAndCheckBootMode(ctx context.Context, mode common.BootMode) error {
	...
	if curr, err := i.v.Helper.Reporter.CurrentBootMode(ctx); err != nil {
		return err
	} else if mode != curr {
		return errors.Errorf("Boot mode is unexpected, want %s, got %s", mode, curr)
	}
	return nil
}
