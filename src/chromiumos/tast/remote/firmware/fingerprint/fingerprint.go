...
func NewMPFirmwareFile(ctx context.Context, d *rpcdut.RPCDUT) FirmwareFile {
	...
	if err != nil {
		testing.ContextLog(ctx, "failed to get expected RW version", err)
		return FirmwareFile{}
	}
	ROVersion, err := getExpectedFwInfo(fpBoard, mpFirmwarePath, fwInfoTypeRoVersion)
	if err != nil {
		testing.ContextLog(ctx, "failed to get expected RO version", err)
		return FirmwareFile{}
	}
	return FirmwareFile{filePath: mpFirmwarePath, KeyType: KeyTypeMp, ROVersion: ROVersion, RWVersion: RWVersion}
}
