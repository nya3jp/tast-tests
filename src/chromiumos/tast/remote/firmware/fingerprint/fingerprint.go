...
func NewMPFirmwareFile(ctx context.Context, d *rpcdut.RPCDUT) FirmwareFile {
	fpBoard, err := Board(ctx, d)
	if err != nil {
		testing.ContextLog(ctx, "Failed to get fingerprint board: ", err)
		return FirmwareFile{}
	}
	mpFirmwarePath, err := FirmwarePath(ctx, d, fpBoard)
	if err != nil {
		testing.ContextLog(ctx, "Failed to get build firmware file path: ", err)
		return FirmwareFile{}
	}
	return FirmwareFile{filePath: mpFirmwarePath, KeyType: KeyTypeMp}
}
...
func CheckValidFlashState(ctx context.Context, d *rpcdut.RPCDUT, fpBoard FPBoardName, firmwareFile FirmwareFile) error {
	...
	expectedRWVersion, err := GetBuildRWFirmwareVersion(ctx, d, firmwareFile.filePath)
	if err != nil {
