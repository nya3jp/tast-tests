...
// KeyType necessary to define firmwareFile.
type KeyType string
...
func NewFirmwareFile(file string, key KeyType) FirmwareFile {
	return FirmwareFile{filePath: file, KeyType: key}
}
...
func InitializeKnownState(ctx context.Context, d *rpcdut.RPCDUT, outdir string, pxy *servo.Proxy, fpBoard FPBoardName, buildFWFile string, firmwareFile FirmwareFile, needsRebootAfterFlashing bool) error {
	...
	if err := CheckValidFlashState(ctx, d, fpBoard, buildFWFile, firmwareFile); err != nil {
...
func CheckValidFlashState(ctx context.Context, d *rpcdut.RPCDUT, fpBoard FPBoardName, buildFWFile string, firmwareFile FirmwareFile) error {
	...
	if (firmwareFile != FirmwareFile{}) {
		byteString := make([]byte, versionStringLenBytes)
		copy(byteString, expectedROVersion)
		newVersion, err := createVersionStringWithSuffix(("." + string(firmwareFile.KeyType)), byteString)
		if err != nil {
			return errors.Wrap(err, "failed to modify version string")
		}
		// get rid of terminator suffix
		newVersion = newVersion[:len(newVersion)-1]
		expectedROVersion = string(newVersion)
	}
