...
func FlashFirmware(ctx context.Context, d *rpcdut.RPCDUT, needsRebootAfterFlashing bool) error {
	...
	cmd := d.Conn().Command(flashCmd[0], flashCmd[1:]...)
...
func InitializeKnownState(ctx context.Context, d *rpcdut.RPCDUT, outdir string, pxy *servo.Proxy, fpBoard FPBoardName, buildFWFile string, needsRebootAfterFlashing bool) error {
	...
	if err := CheckValidState(ctx, d, fpBoard, buildFWFile); err != nil {
...
func CheckValidState(ctx context.Context, d *rpcdut.RPCDUT, fpBoard FPBoardName, buildFWFile string) error {
	...
	// state is entropy is not set. The reason we are doing this is so that
