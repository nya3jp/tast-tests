...
func (rd *RPCDUT) Reboot(ctx context.Context) error {
	...
	//   then we connected to DUT during update.
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		// We are not using UpstartService here because it requies RPC
		...
		return errors.Wrap(err, "failed to wait for DUT boot")
	}
	if err := rd.RPCDial(ctx); err != nil {
