...
func TestFlashProtectIsSet(t *testing.T) {
	...
	if !flags.IsSet(FlashProtectGpioAsserted) {
		t.Fatal("Flag FlashProtectGpioAsserted was reporrted as not set.")
	}

	if !flags.IsSet(FlashProtectRoAtBoot) {
		t.Fatal("Flag FlashProtectRoAtBoot was reporrted as not set.")
	}

	if !flags.IsSet(FlashProtectRoNow) {
		t.Fatal("Flag FlashProtectRoNow was reporrted as not set.")
	}
