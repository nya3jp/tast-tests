...
func (t *FirmwareTest) Close(ctx context.Context) error {
	...
	// The test can fail when DUT is disconnected (e.g. context timeout while reconnecting to DUT).
	// In this case we should attempt to connect to the DUT. When connecting fails, we shouldn't
	// proceed because without healthy connection there is nothing we can do.
	if !t.d.Connected(ctx) || t.d.RPC() == nil {
