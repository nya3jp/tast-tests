...
func fmapSectionInfo(ctx context.Context, firmwareFilePath string, section FMAPSection) (*fmapSection, error) {
	...
	output, err := hostCommand(ctx, cmd).Output()
	if err != nil {
...
func readFileAtOffset(fileName string, b []byte, offset int64) error {
	...
	if _, err := f.ReadAt(b, offset); err != nil {
		return errors.Wrapf(err, "failed to read from offset: %q", offset)
	}

	return nil
}

func writeFileAtOffset(fileName string, offset int64, data []byte) error {
	...
	if _, err := f.WriteAt(data, offset); err != nil {
		return errors.Wrapf(err, "failed to write data: %q to offset: %q", data, offset)
	}
...
func createVersionStringWithSuffix(suffix string, version []byte) ([]byte, error) {
	if len(version) != versionStringLenBytes {
		return nil, errors.Errorf("incorrect version size, expected: %d, actual: %d", versionStringLenBytes, len(version))
	}
...
func addSuffixToVersionString(filePath, suffix string, version *fmapSectionValue) error {
	...
	if err := writeFileAtOffset(filePath, int64(version.Section.Offset), newVersion); err != nil {
...
func createRollbackBytes(newRollbackValue uint32) []byte {
	...
	binary.LittleEndian.PutUint32(rollbackBytes[0:], newRollbackValue)
	return rollbackBytes
}

func modifyRollbackValue(filePath string, newRollbackValue uint32, rollback *fmapSectionValue) error {
	if rollback.Section.Size != rollbackSizeBytes {
		return errors.Errorf("incorrect version size, expected: %q, actual: %q", rollbackSizeBytes, rollback.Section.Size)
	}

	rollbackBytes := createRollbackBytes(newRollbackValue)

	if err := writeFileAtOffset(filePath, int64(rollback.Section.Offset), rollbackBytes); err != nil {
...
func generateRollbackImage(ctx context.Context, devKeyPair *keyPair, origFirmwareFilePath string, rwVersion, roVersion *fmapSectionValue, rollback *fmapSectionValue, newRollbackValue uint32) (string, error) {
	...
	if err := modifyRollbackValue(rollbackFilePath, newRollbackValue, rollback); err != nil {
...
func generateCorruptFirstByteImage(ctx context.Context, origFirmwareFilePath string) (string, error) {
	...
	// write single byte to offset
	if err := writeFileAtOffset(corruptFilePath, int64(rwSection.Offset) + 100, randByte); err != nil {
...
func generateCorruptLastByteImage(ctx context.Context, origFirmwareFilePath string) (string, error) {
	...
	if err := writeFileAtOffset(corruptFilePath, int64(rwSection.Offset) - 100, []byte{0}); err != nil {
		return "", errors.Wrap(err, "failed to write corrupt first byte")
	}
