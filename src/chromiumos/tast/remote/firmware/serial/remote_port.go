...
// RemotePort allows communication with a serial port on a remote machine.
...
// Flush unread data on the port.
func (p *RemotePort) Flush(ctx context.Context) error {
	_, err := p.client.Flush(ctx, &empty.Empty{})
	return err
}

// Close the connection.  The port can not be used afterwards.
