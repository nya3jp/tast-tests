...
func (s *Context) SuspendDUT(state State, args SuspendArgs) error {
	...
	if err := s.h.WaitForPowerStates(s.ctx, firmware.PowerStateInterval, firmware.PowerStateTimeout, state.String()); err != nil {
...
// IsSupported determines if the DUT supports a given state.
...
func (s *Context) IsSupported(state State) (bool, error) {
