...
The Helper now has all the information it needs to create a Reporter, Servo, RPC connection, Config, and Servo. Additionally, `h.Close` will close any firmware structures that it initialized.
...
If you prefer, you can use `h.Reporter` directly, rather than binding to a new variable:
...
If you try to run your code, the compiler will throw unused-import errors, due to our removed code. You can go ahead and delete any unused imports (`reporters`, `fwService`, `rpc`).
...
[`RequireBiosServiceClient`]: https://source.chromium.org/chromiumos/chromiumos/codesearch/+/main:src/platform/tast-tests/src/chromiumos/tast/remote/firmware/helper.go;l=81?q=func.*RequireBiosServiceClient&sq=
...
We don't need to verify the DUT's boot mode after rebooting; `ms.RebootToMode` does that, and if the DUT doesn't end up in the expected boot mode, it will return an error. If we did want to verify the boot mode, `h.Reporter.CurrentBootMode(ctx)` would tell us.
...
As it stands, this test does something rude: it leaves the DUT in recovery mode. When the next test starts, the DUT will still be in recovery mode, which could cause unexpected behavior. This is bad! You could clean up manually by rebooting back to normal mode. But in the next section, we'll explore an alternative.
