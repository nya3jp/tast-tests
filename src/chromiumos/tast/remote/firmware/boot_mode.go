...
func (ms ModeSwitcher) RebootToMode(ctx context.Context, toMode fwCommon.BootMode) error {
	...
	case fwCommon.BootModeNormal:
		...
		// 5. If booting from Dev Mode, deactivate dev screen.
		...
		if fromMode == fwCommon.BootModeDev {
			if err := ms.devScreenToNormalMode(ctx); err != nil {
				return errors.Wrap(err, "moving from dev screen to normal mode")
			}
		}
	...
	case fwCommon.BootModeDev:
		...
		// Past there, behavior varies based on the DUT's ModeSwitcherType.
		if err := ms.enableRecMode(ctx, servo.USBMuxHost); err != nil {
			return err
		}
		switch h.Config.ModeSwitcherType {
		case KeyboardDevSwitcher:
			// 2. Wait until the firmware screen appears.
			// 3. Press Ctrl-D to move to the confirm screen.
			// 4. Wait until the confirm screen appears.
			// 5. Push some button depending on the DUT config (toggle the rec button, press power, or press enter).
...
// devScreenToNormalMode moves the DUT from the "developer mode" bootup screen to Normal mode.
// This should be called immediately after powering on.
// The actual behvior depends on the ModeSwitcherType.
func (ms *ModeSwitcher) devScreenToNormalMode(ctx context.Context) error {
	h := ms.Helper
	if err := h.RequireServo(ctx); err != nil {
		return errors.Wrap(err, "requiring servo")
	}
	...
	default:
		return errors.Errorf("unsupported ModeSwitcherType %s for devScreenToNormalMode", h.Config.ModeSwitcherType)
	}
	return nil
}
