...
func (ms ModeSwitcher) RebootToMode(ctx context.Context, toMode fwCommon.BootMode, opts ...ModeSwitchOption) error {
	...
	case fwCommon.BootModeNormal:
		...
		if fromMode != fwCommon.BootModeNormal {
			hasSerialAP, err = ms.hasSerialAPFirmware(ctx)
			if err != nil {
				testing.ContextLogf(ctx, "Could not determine is AP fw has serial port enabled: %s", err)
				hasSerialAP = false
			}
		}
...
// This should be called immediately after powering on.
// The actual behavior depends on the ModeSwitcherType.
func (ms *ModeSwitcher) fwScreenToDevMode(ctx context.Context, hasSerialAP bool) error {
	h := ms.Helper
	if err := h.RequireServo(ctx); err != nil {
		return errors.Wrap(err, "requiring servo")
	}
...
func (ms *ModeSwitcher) hasSerialAPFirmware(ctx context.Context) (bool, error) {
