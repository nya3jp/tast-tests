...
func (ms *ModeSwitcher) ModeAwareReboot(ctx context.Context, resetType ResetType) error {
	...
	powerState := map[ResetType]servo.PowerStateValue{
	...
	// Verify successful reboot.
	expectMode := map[fwCommon.BootMode]fwCommon.BootMode{
		fwCommon.BootModeNormal:   fwCommon.BootModeNormal,
		fwCommon.BootModeDev:      fwCommon.BootModeDev,
		fwCommon.BootModeRecovery: fwCommon.BootModeNormal,
	}[fromMode]
	if curr, err := h.Reporter.CurrentBootMode(ctx); err != nil {
