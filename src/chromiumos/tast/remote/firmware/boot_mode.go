...
func cyclePowerState(ctx context.Context, d *dut.DUT, svo *servo.Servo, cfg *Config, ps servo.PowerStateValue) error {
	...
	testing.ContextLogf(ctx, "Reestablishing connection to DUT (timeout=%d seconds)", cfg.DelayRebootToPing)
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		// Set a short timeout to the iteration in case of any SSH operations blocking for a long time.
