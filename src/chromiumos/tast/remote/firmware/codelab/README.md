...
Now, let's use servo to find out DUT's `ec_board` exactly the same as we did in remote but this time as a local test. We will use the same `GetString` method.
