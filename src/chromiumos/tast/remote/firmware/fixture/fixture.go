...
func (i *impl) PreTest(ctx context.Context, s *testing.FixtTestState) {
	...
	if common.GBBFlagsStatesEqual(i.value.GBBFlags, curr) {
		...
		// As reported in b/236026413, SetGBBFlags was unsuccessful for some DUTs and failed in erase/write.
		// From manual tests, clearing host/ec write protection ranges would resolve this error.
		clearRange := "0x00" + "," + "0x00"
		for _, programmer := range []string{"ec", "host"} {
			if err := i.value.Helper.DUT.Conn().CommandContext(
				ctx, "flashrom", "-p", programmer, "--wp-disable", "--wp-range", clearRange).Run(exec.DumpLogOnError); err != nil {
