...
func (h *Helper) RequireRPM(ctx context.Context) error {
	...
	rpmHost := rpm.LocalRPMHost
	if _, err := net.ResolveIPAddr("ip", rpmHost); err != nil {
		testing.ContextLogf(ctx, "Could not resolve %q: %s", rpmHost, err)
		rpmHost = rpm.RemoteRPMHost
	}
	// When running outside of the lab, the rpm server is not accessable, so proxy through the servo host.
	if h.ServoProxy.Proxied() {
		fwd, err := h.ServoProxy.NewForwarder(ctx, fmt.Sprintf("%s:%d", rpmHost, rpm.DefaultRPMPort))
		if err != nil {
			return errors.Wrap(err, "forwarding rpm port")
		}
		host, portstr, err := net.SplitHostPort(fwd.ListenAddr().String())
		if err != nil {
	...
	rpm, err := rpm.New(ctx, rpmHost, rpm.DefaultRPMPort)
	if err != nil {
		return errors.Wrap(err, "new rpm client")
	}
	h.RPM = rpm
