...
type Helper struct {
	...
	// ModeSwitcher allows rebooting the DUT into different firmware boot modes (normal, dev, recovery).
...
// RequireModeSwitcher creates a ModeSwitcher, unless one already exists.
func (h *Helper) RequireModeSwitcher() {
	if h.ModeSwitcher == nil {
		h.ModeSwitcher = NewModeSwitcher(h)
	}
}

// CheckBootMode shells out a call to h.ModeSwitcher.CheckBootMode.
func (h *Helper) CheckBootMode(ctx context.Context, mode fwCommon.BootMode) (bool, error) {
	h.RequireModeSwitcher()
	return h.ModeSwitcher.CheckBootMode(ctx, mode)
}

// RebootToMode shells out a call to h.ModeSwitcher.RebootToMode.
func (h *Helper) RebootToMode(ctx context.Context, mode fwCommon.BootMode) error {
	h.RequireModeSwitcher()
	return h.ModeSwitcher.RebootToMode(ctx, mode)
}
