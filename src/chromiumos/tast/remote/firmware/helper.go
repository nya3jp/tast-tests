...
func (h *Helper) SetupUSBKey(ctx context.Context, cloudStorage *testing.CloudStorage) (retErr error) {
	...
	defer func(ctx context.Context) {
		...
		if err = h.ServoProxy.RunCommand(ctx, true, "blockdev", "--rereadpt", usbdev); err != nil {
	...
	if err = h.ServoProxy.InputCommand(ctx, true, reader, "sh", "-c", fmt.Sprintf("tar -JxOf - | dd of=%q bs=1M iflag=fullblock oflag=dsync", usbdev)); err != nil {
