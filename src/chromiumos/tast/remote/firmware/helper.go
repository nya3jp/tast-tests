...
const (
	// Filepaths on the DUT for local Tast files
	...
	// Relative paths within a tempdir on the server to copies of DUT Tast files
	tmpLocalRunner    = "local-runner"
	tmpLocalBundleDir = "local-bundle-dir"
	tmpLocalDataDir   = "local-data-dir"

	// Basenames for zipped-up Tast files
...
func (h *Helper) CopyTastFilesFromDUT(ctx context.Context) error {
	...
	tmpDir, err := ioutil.TempDir("/tmp/gredelston", "dut-tast-files-copy")
	if err != nil {
		return err
	}
	...
	// Record the last modified time of the DUT's Tast
...
func (h *Helper) CopyTastFilesToDUT(ctx context.Context) error {
	...
	// Unzip Tast bundles and data files to their expected locations
