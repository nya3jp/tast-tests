...
import (
	...
	"chromiumos/tast/rpc"
	bluetoothUiService "chromiumos/tast/services/cros/bluetooth"
...
const (
	bluetoothUIServiceName = "tast.cros.bluetooth.BluetoothUIService"
	chromeServiceName      = "tast.cros.browser.ChromeService"
)
...
func init() {
	testing.AddFixture(&testing.Fixture{
		...
		ServiceDeps:     []string{bluetoothUIServiceName, chromeServiceName},
	})
	testing.AddFixture(&testing.Fixture{
...
type FixtValue struct {
	...
	// BluetoothUIService is a client of the BluetoothUIService that uses the DUTRPCClient connection.
	BluetoothUIService bluetoothUiService.BluetoothUIServiceClient
...
func (f *fixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	f.fv.BluetoothUIService = bluetoothUiService.NewBluetoothUIServiceClient(f.fv.DUTRPCClient.Conn)

	if _, err := f.fv.BluetoothUIService.Initialize(ctx, &bluetoothUiService.InitializeRequest{
		Floss: false,
	}); err != nil {
	...
	if _, err := f.fv.BluetoothUIService.Enable(ctx, &emptypb.Empty{}); err != nil {
...
func (f *fixture) Reset(ctx context.Context) error {
	if _, err := f.fv.BluetoothUIService.Reset(ctx, &emptypb.Empty{}); err != nil {
		errors.Wrap(err, "failed to reset the Bluetooth state on the DUT")
	}
	if err := f.resetBTPeers(ctx); err != nil {
