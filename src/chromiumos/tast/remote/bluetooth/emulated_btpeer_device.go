...
func (d *EmulatedBTPeerDevice) evaluateCapabilities(ctx context.Context) error {
	...
	// Evaluate hasPinCode.
	if hasPinCode, ok := d.cache.capabilities[cbt.DeviceCapabilityHasPin.String()]; ok {
		if hasPinCodeBool, ok := hasPinCode.(bool); ok {
	...
	if supportsInitConnect, ok := d.cache.capabilities[cbt.DeviceCapabilityInitConnect.String()]; ok {
		if supportsInitConnectBool, ok := supportsInitConnect.(bool); ok {
	...
	if supportedTransportMethods, ok := d.cache.capabilities[cbt.DeviceCapabilityTransports.String()]; ok {
		if supportedTransportMethodsSlice, ok := supportedTransportMethods.([]interface{}); ok {
...
// ResetDevice power-cycles and re-initializes the device and local data cache.
func (d *EmulatedBTPeerDevice) ResetDevice(ctx context.Context) error {
	// TODO
	return nil
}
