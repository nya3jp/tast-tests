...
func fromResults(ctx context.Context, testType TestType, results string, duration time.Duration) *Result {
	...
	case TestTypeUDPMaerts:
		...
		ret.measurements[CategoryThroughput], _ = strconv.ParseFloat(tokens[5], 64)
		ret.measurements[CategoryErrors], _ = strconv.ParseFloat(tokens[4], 64)
		//break
...
func getStats(samples []*Result, category Category) (mean, deviation float64) {
	var sum float64
	numSamples := 0
	for _, sample := range samples {
	...
	// Assert we have something to calculate.
	if numSamples == 0 {
		panic("No valid samples delivered to calculate.")
	}
...
func FromSamples(ctx context.Context, samples []*Result) (*Result, error) {
	...
	if hasCategory(samples, CategoryThroughput) {
		ret.measurements[CategoryThroughput], ret.measurements[CategoryThroughputDev] =
			getStats(samples, CategoryThroughput)
	}
	if hasCategory(samples, CategoryTransactionRate) {
		ret.measurements[CategoryTransactionRate], ret.measurements[CategoryTransactionRateDev] =
			getStats(samples, CategoryTransactionRate)
	}
	if hasCategory(samples, CategoryErrors) {
		ret.measurements[CategoryErrors], ret.measurements[CategoryErrorsDev] =
			getStats(samples, CategoryErrors)
	}
...
func (r *Result) AllDeviationsLessThanFraction(fraction float64) bool {
	measurements := []Category{
		CategoryThroughput, CategoryErrors, CategoryTransactionRate}
	deviations := []Category{
		CategoryThroughputDev, CategoryErrorsDev, CategoryTransactionRateDev}
	for i := range measurements {
		value, ok := r.measurements[measurements[i]]
		if !ok {
			continue
		}
		dev, ok := r.measurements[deviations[i]]
		if !ok {
			continue
		}
		if value == 0 && dev == 0 {
