...
func getStats(samples []*Result, category Category) (mean, deviation float64, validSamples int) {
	var sum float64
	numSamples := 0
	for _, sample := range samples {
...
func FromSamples(ctx context.Context, samples []*Result) (*Result, error) {
	...
	if hasCategory(samples, CategoryThroughput) {
		ret.Measurements[CategoryThroughput],
			...
			validSamples = getStats(samples, CategoryThroughput)
	...
	if hasCategory(samples, CategoryTransactionRate) {
		ret.Measurements[CategoryTransactionRate],
			...
			validSamples = getStats(samples, CategoryTransactionRate)
		// The result of the same typ consists of either throuput or transaction rate,
