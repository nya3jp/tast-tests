...
func prepareServer(
	...
	netserverPath, err := cmd.GetCmdPath(ctx, npr.server.conn, "netserver")
	if err != nil {
		return errors.Wrap(err, "failed to find command netserver")
	}
	npr.netserverPath = netserverPath
	return npr.stopNetserv(ctx)
}
...
func (r *runner) restartNetserv(ctx context.Context) error {
	if err := r.stopNetserv(ctx); err != nil {
		testing.ContextLog(ctx, "Problems while stopping netserver, err: ", err)
	}
	if err := r.startNetserv(ctx); err != nil {
		return errors.Wrap(err, "failed to start netserver")
	}
	return nil
}
