...
// ReleaseLocalCmd detaches the command process from the parent process and keeps it alive
// after the parent process dies.
// The function only works for local test environment.
func (r *RemoteCmdRunner) ReleaseLocalCmd() error {
	return nil
}

// ResetCmd will set the command to null.
