...
// AddrInfo contains the addresses of a network interface.
...
// NewRunner creates an ip command Runner on the given host.
func NewRunner(host commander.Commander) *Runner {
	return &Runner{host: host}
}

// IP runs an ip command and returns it's output.
