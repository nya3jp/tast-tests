...
// AddInfo contains the addresses of the client.
type AddInfo struct {
	MAC  string
	IPv4 string
	IPv6 string
...
func (r *Runner) Addresses(ctx context.Context, iface string) (*AddInfo, error) {
	addressInfoBytes, err := r.IP(ctx, "addr", "show", iface)
	if err != nil {
		return nil, errors.Wrap(err, "failed to get the addresses")
	}
	return parseAddsOutput(ctx, string(addressInfoBytes))
}

// parseAddsOutput parses the output of `ip addr show  "iface"` command into a AddInfo struct.
func parseAddsOutput(ctx context.Context, addressInfo string) (*AddInfo, error) {
	/*
	...
	*/
	...
	for _, addressLine := range strings.Split(addressInfo, "\n") {
		...
		case addressTypeMAC:
			clientAdds.MAC = addressParts[1]
		case addressTypeIPV4:
			clientAdds.IPv4 = addressParts[1]
		case addressTypeIPV6:
			clientAdds.IPv6 = addressParts[1]
		default:
			testing.ContextLogf(ctx, "Found unexpected address type: got %s, want %v", addressParts[0], addressTypes)
		}
	}

	return &clientAdds, nil
}
