...
// Option is a function object type used to specify options of NewEventWatcher.
type Option func(*config)

// EventsBufferSize returns a Option that sets the buffer size of the events channel.
func EventsBufferSize(size int) Option {
	return func(c *config) { c.eventsBufferSize = size }
}

// NewEventWatcher creates and starts a new EventWatcher.
func NewEventWatcher(ctx context.Context, dut *dut.DUT, ops ...Option) (*EventWatcher, error) {
	conf := &config{eventsBufferSize: 10}
	for _, op := range ops {
		op(conf)
	}
...
// IsChannelClosedError checks that err is channelClosedError or not.
func IsChannelClosedError(err error) bool {
	e := &channelClosedError{}
	return errors.As(err, &e)
}

// Wait waits for the next event.
// If there is no more event (i.e., channel has been closed), Wait returns
// a channelClosedError and can be checked by IsChannelClosedError.
func (e *EventWatcher) Wait(ctx context.Context) (*Event, error) {
	select {
	case ev, ok := <-e.events:
		if !ok {
			return nil, &channelClosedError{E: errors.New("event channel has been closed")}
		}
		return ev, nil
	case <-ctx.Done():
		return nil, ctx.Err()
