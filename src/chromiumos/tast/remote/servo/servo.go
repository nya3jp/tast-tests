...
func New(ctx context.Context, connSpec string) (*Servo, error) {
	sHost, sPort, err := parseConnSpec(connSpec)
	if err != nil {
		return nil, err
	}
	s := &Servo{sHost, sPort, nil}

	// Set up SSH TCP tunneling if host is a different machine.
	if s.host != "localhost" {
		opts := host.SSHOptions{Hostname: s.host, Port: s.port}
		sshConn, err := host.NewSSH(ctx, &opts)
		if err != nil {
			return nil, err
		}

		// TODO(CL): resolve hostname (is this necessary?)
		// ----------
		ip := net.IPv4(127, 0, 0, 1)
		// ----------
		tunnelLoc := &net.TCPAddr{IP: ip, Port: s.port}
		tcpConn, err := sshConn.DialTCP(tunnelLoc)
		if err != nil {
			return nil, err
		}
		s.tunnel = tcpConn
