...
func hwsecGetDACounter(ctx context.Context, s *testing.TestHookState) (int, error) {
	...
	tpmManagerUtil, err := hwsec.NewUtilityTPMManagerClient(cmdRunner)
	if err != nil {
		return 0, errors.Wrap(err, "failed to create UtilityTPMManagerClient")
	}
...
func hwsecGetTPMStatus(ctx context.Context, s *testing.TestHookState) (*hwsec.NonsensitiveStatusInfo, error) {
	...
	tpmManagerUtil, err := hwsec.NewUtilityTPMManagerClient(cmdRunner)
	if err != nil {
		return nil, errors.Wrap(err, "failed to create UtilityTPMManagerClient")
	}
