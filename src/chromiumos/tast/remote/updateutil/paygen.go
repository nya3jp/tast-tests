...
// Delta desribes an update delta in the paygen.json file.
type Delta struct {
...
func FindLatestStable(ctx context.Context, board string) (*Delta, error) {
	...
	lookup := paygen.FilterBoardChannelDeltaType(board, channel, deltaType)

	return findLatest(lookup)
}

// LoadPaygenFromGS download the paygen.json file, parses it, and return with the Paygen obejct.
...
// FilterBoardChannelDeltaType filters the Paygen obejct based on the board, channel and delta type.
...
// FilterMilestone filters the Paygen obejct based on the milestone.
...
func findLatest(paygen *Paygen) (*Delta, error) {
	if len(paygen.Deltas) == 0 {
	...
	for _, delta := range paygen.Deltas {
		newer := false
...
func version(chromeOSVersion string) (int, int, int, error) {
	...
	if err != nil {
		return 0, 0, 0, errors.Errorf("minor version is not an integer %q", versionSlice[0])
	}

	patch, err := strconv.Atoi(versionSlice[2])
	if err != nil {
		return 0, 0, 0, errors.Errorf("patch number is not an integer %q", versionSlice[0])
	}
