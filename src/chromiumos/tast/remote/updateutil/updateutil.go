...
func UpdateFromGS(ctx context.Context, dut *dut.DUT, outdir string, rpcHint *testing.RPCHint, builderPath string) (retErr error) {
	...
	metadataFilename := metadataPath[strings.LastIndex(metadataPath, "/")+1:]
	metadataURL := url + "/" + metadataFilename

	args := []string{
		"--tries=1",
		"--connect-timeout=20",
		"-P", tempDir.Path, // download folder
		metadataURL, // payload metadata address
	}

	// Download the payload metadata from the caching server.
	testing.ContextLogf(preparationCtx, "Downloading payload metadata from %q", metadataURL)
