...
func newPSISystemStats(statBlob []byte) (*PSIOneSystemStats, error) {
	...
	if len(lines) != len(blocks) {
		return nil, errors.Errorf("PSI metrics file should have %d lines, found %d", len(blocks), len(lines))
	}
...
func NewPSIStats(ctx context.Context, a *arc.ARC) (*PSIStats, error) {
	...
	} // else it is a host that does not support PSI

	if a != nil {
		out, err := a.Command(ctx, "cat", psiFilename).Output(testexec.DumpLogOnError)
		if err != nil {
			return nil, errors.Wrap(err, "could not retrieve PSI metrics on guest")
		}
