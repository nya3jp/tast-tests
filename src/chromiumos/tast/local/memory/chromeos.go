...
// Parameter size is the size of the allocatoin in bytes.
...
func (c *ChromeOSAllocator) AllocateUntil(
	...
	for attempt := 0; attempt < attempts; {
		...
		if available >= margin {
			bufferSize := max((available-margin)/10, 1) * bytesPerMiB
			...
			// so free anything extra we may have allocated.
			for toFree := (margin - available) * bytesPerMiB; toFree > 0 && c.Size() > 0; {
