...
func (t *ArcLifecycleTask) Run(ctx context.Context, testEnv *TestEnv) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		distance, err := t.limit.Distance(ctx)
		if err != nil {
			return err
		}
		if distance <= 0 {
...
func NewArcLifecycleTask(id int, allocateBytes int64, limit memory.Limit) *ArcLifecycleTask {
	return &ArcLifecycleTask{id, allocateBytes, limit}
}
