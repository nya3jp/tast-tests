...
func (t *ArcLifecycleTask) Run(ctx context.Context, testEnv *TestEnv) error {
	...
	startOut, err := testEnv.arc.Command(ctx, "am", "start", "-W", t.mainActivity()).Output(testexec.DumpLogOnError)
	if err != nil {
	...
	// TODO: Figure out why the app fails to respond to the broadcast unless we
	// put this sleep here, we wait for the app to start, which should mean the
	// Activity is up, which means the broadcast receiver is registered.
	// But this is somehow needed to ensure that the app responds to the
	// broadcast.
	if err := testing.Sleep(ctx, time.Second); err != nil {
		return errors.Wrap(err, "failed to sleep betwerrn start and alloc")
	}
