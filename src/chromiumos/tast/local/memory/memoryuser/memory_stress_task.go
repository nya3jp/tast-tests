...
type MemoryStressTask struct {
	url    string
	conn   *chrome.Conn
	memLow memory.MemLow
}

// MemeoryStressTask is a MemoryTest.
var _ MemoryTask = (*MemoryStressTask)(nil)

var errMemLow = errors.New("memory still low")

// Run creates a Chrome tab that allocates memory. If a MemLow has been
// provided, we wait until memory is no longer low.
func (st *MemoryStressTask) Run(ctx context.Context, testEnv *TestEnv) error {
	...
	// MemLow has been provided, wait until memory is not low.
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		distance, err := st.memLow.Distance(ctx)
		if err != nil {
			return err
		}
		if distance <= 0 {
		...
		return errors.Wrap(err, "failed to wait for ChromeOS memory to not be low")
	}
	return nil
}
...
// allocMB - The amount of memory the tab will allocate.
// ratio   - How compressable the allocated memory will be.
// memLow  - (optional) wait until memory is not low after creating the tab.
func (s *MemoryStressServer) NewMemoryStressTask(allocMb int, ratio float32, memLow memory.MemLow) *MemoryStressTask {
