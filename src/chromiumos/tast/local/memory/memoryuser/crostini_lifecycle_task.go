...
func (t *CrostiniLifecycleTask) Run(ctx context.Context, testEnv *TestEnv) error {
	...
	alloc, err := memory.NewCrostiniAnonAlloc(ctx, t.cont, t.ratio)
	if err != nil {
...
func (t *CrostiniLifecycleTask) String() string {
	return "Crostini Lifecycle Task"
}
...
//Crostini.
func NewCrostiniLifecycleTask(cont *vm.Container, bytes int64, ratio float64, limit memory.Limit) *CrostiniLifecycleTask {
