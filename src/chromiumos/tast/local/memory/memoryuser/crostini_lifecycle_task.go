...
func crostiniHomeDir(ctx context.Context, cont *vm.Container) (string, error) {
...
func (t *CrostiniLifecycleTask) Run(ctx context.Context, testEnv *TestEnv) error {
	...
	// Call Wait so that t.cmd.ProcessState is set as soon as it terminates.
	go func() {
		t.cmd.Wait()
	}()
	...
	if doneString, err := stdout.ReadString('\n'); err != nil {
		...
		return errors.Errorf("failed to read done line, exptected \"done\", got %q", doneString)
	}
...
const crostiniLifecycleName = "multivm.Lifecycle.crostini"

// TODO: Should be under /usr/local/libexec?
const crostiniLifecyclePath = "/usr/libexec/tast/helpers/local/cros/" + crostiniLifecycleName
...
func InstallCrostiniLifecycle(ctx context.Context, cont *vm.Container) error {
	...
	if err := cont.PushFile(ctx, crostiniLifecyclePath, containerPath); err != nil {
		return errors.Wrap(err, "failed to push executable to Crostini")
	}
	if err := cont.Command(ctx, "chmod", "755", containerPath).Run(); err != nil {
		return errors.Wrap(err, "failed to make programs executable")
	}
	return nil
}

// UninstallCrostiniLifecycle deletes the script and executable used to run
