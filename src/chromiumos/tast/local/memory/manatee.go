...
func ManaTEEMetrics(ctx context.Context, p *perf.Values, outdir, suffix string) error {
	manateePath, err := exec.LookPath("manatee")
	if err != nil {
		// If the manatee binary doesn't exist, jkkk
		return nil
	}

	pgrepCmd := testexec.CommandContext(ctx, manateePath, "-a", "shell-notty", "--", "-c", "pgrep crosvm")
	...
	for _, pid := range pids {
		manateeCmd := fmt.Sprintf("cat /proc/%d/smaps", pid)

		smapsCmd := testexec.CommandContext(ctx, manateePath, "-a", "shell-notty", "--", "-c", manateeCmd)
	...
	meminfoCmd := testexec.CommandContext(ctx, manateePath, "-a", "shell-notty", "--", "-c", "cat /proc/meminfo")
