...
func LogMemoryStats(ctx context.Context, base *BaseMemoryStats, a *arc.ARC, p *perf.Values, outdir, suffix string) error {
	...
	zramSummary, err := memory.ZramMmStatMetrics(ctx, p, outdir, suffix)
	if err != nil {
	...
	if a != nil {
		...
		for i := 0; i < dumpsysRetries; i++ {
			...
			if err != nil {
				...
				if vmSummary != nil && hostSummary != nil && zramSummary != nil && p != nil {
					memory.LogSummaryMetrics(ctx, vmEnabled, hostSummary, vmSummary, zramSummary, p, suffix)
				}
