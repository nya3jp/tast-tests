...
func compareDeviceStatelist(ctx context.Context, s *testing.State, orgDeviceState, curDeviceState []netconfig.DeviceStateProperties, enabled bool) {
	for _, cfg := range curDeviceState {
		...
		if deviceType == netconfig.Cellular {
			if enabled {
				if deviceState != netconfig.EnabledDST {
					s.Fatal("Unexpected Cellular state expected: enabled got: ", deviceState)
				}
			} else {
				if deviceState != netconfig.DisabledDST {
					s.Fatal("Unexpected Cellular state expected: disabled got:", deviceState)
				}
			}
		}
		...
		for _, cfg := range orgDeviceState {
			if cfg.Type == deviceType && cfg.DeviceState != deviceState {
				s.Fatalf("Unexpected state for device: %d Expected: %d got: %d", cfg.Type, cfg.DeviceState, deviceState)
			}
		}
	}
}

// MojoCellularToggleCompare enables/distable cellular device using Mojo and confirms using mojo that WiFi/Ethernet are not affected
func MojoCellularToggleCompare(ctx context.Context, s *testing.State) {
	...
	defer wifiManager.Enable(ctx, false)
	...
	for i := 0; i < iterations; i++ {
		...
		compareDeviceStatelist(ctx, s, orgDeviceState, curDeviceState, enabled)
	}
}
