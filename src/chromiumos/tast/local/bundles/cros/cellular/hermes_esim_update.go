...
func HermesESIMUpdate(ctx context.Context, s *testing.State) {
	...
	if err := performFWUpdate(ctx, s, f.Name(), false); err != nil {
		s.Fatal("Could not test EOS update with invalid file: ", err)
	}
	if err := performFWUpdate(ctx, s, s.DataPath("third_party/Test_multiscript_chrome_v1.xml"), true); err != nil {
		s.Fatal("EOS update failed: ", err)
	}
}

func performFWUpdate(ctx context.Context, s *testing.State, fwPath string, isValidUpdate bool) error {
	s.Log("Updating ESIM FW using ", fwPath)
	if err := os.Chmod(fwPath, 0644); err != nil {
		return errors.Wrap(err, "unable to change permissions of ESIM FW")
	}
