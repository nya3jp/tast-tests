...
func Smoke(ctx context.Context, s *testing.State) {
	...
	verifyNetworkConnectivity := func(ctx context.Context) error {
		...
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			googStatus := "could not reach google.com and,"
			const googURL = "http://google.com"
			gResp, err := http.Get(googURL)
			defer gResp.Body.Close()
			if err != nil {
				googStatus = "reached google.com but,"
			}
			...
			if err != nil {
				return errors.Wrapf(err, "%s error fetching data from URL %q", googStatus, fetchURL)
			}
