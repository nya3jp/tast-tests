...
func ModemFWManifestVerification(ctx context.Context, s *testing.State) {
	...
	for _, device := range manifest.Device {
		...
		if device.GetDlcId() != "" {
			dlcCounter++
			// Only the variant that matches the device's variant will contain a DLC that is already installed by modemfwd.
			// Manually install the DLCs for other variants. This won't work on non test images since the DLCs are purged.
			// It works on test images because the modem DLCs have the DLC_PRELOAD flag in their ebuilds.
			...
			// Until DLC is not fully proven to work 100% for modmefwd, we keep a copy of all
