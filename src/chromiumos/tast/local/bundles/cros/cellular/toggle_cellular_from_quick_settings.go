...
func ToggleCellularFromQuickSettings(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine("Wait until cellular is enabled in UI and not inhibited",
		ui.WaitUntilCheckedState(quicksettings.ToggleMobileData, true),
		ui.WaitUntilEnabled(quicksettings.ToggleMobileData),
	)(ctx); err != nil {
		s.Fatal("Failed: ", err)
	}
	...
	for i := 0; i < iterations; i++ {
		...
		if err := ui.LeftClick(quicksettings.ToggleMobileData)(ctx); err != nil {
		...
		if err := uiauto.Combine("Wait until cellular is in expected state in UI and not inhibited",
			ui.WaitUntilCheckedState(quicksettings.ToggleMobileData, state),
			ui.WaitUntilEnabled(quicksettings.ToggleMobileData),
		)(ctx); err != nil {
			s.Fatal("Failed: ", err)
		}
