		...
		if deviceType == netconfig.Cellular {
			if enabled {
				if deviceState != netconfig.EnabledDST {
					return errors.Errorf("unexpected Cellular state expected: enabled got: %s", deviceState)
				}
			} else {
				if deviceState != netconfig.DisabledDST {
					return errors.Errorf("unexpected Cellular state expected: disabled got:", deviceState)
				}
			}
		}
...
// MojoEnsureNetworkStateAfterCellularToggle enables/distable cellular device using Mojo and confirms using mojo that WiFi/Ethernet are not affected.
func MojoEnsureNetworkStateAfterCellularToggle(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < iterations; i++ {
		...
		if err := compareDeviceStatelist(orgDeviceState, curDeviceState, enabled); err != nil {
			s.Fatal("Device state comparsion failed: ", err)
		}
	}
}
