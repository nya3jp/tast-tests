...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			Name: "hermes_only",
			Val:  hermes.HermesOnly,
		}, {
			Name: "hermes_and_mm",
			Val:  hermes.HermesAndMM,
		}},
	})
}
...
func HermesTestCI(ctx context.Context, s *testing.State) {
	...
	// Emulate Chrome's call that occurs after powerwash and check that it doesn't change slots
	if testMode != hermes.HermesOnly {
...
func switchSlotIfMMTest(ctx context.Context, s *testing.State, testMode string) {
	if testMode == hermes.HermesAndMM {
