...
func MojoEthernetPreferredOverCellular(ctx context.Context, s *testing.State) {
	...
	// Autoconnecting iss causing following connection code to fail
	if _, err := cellularHelper.SetServiceAutoConnect(ctx, false); err != nil {
	...
	// Disable Ethernet WiFi if present and defer re-enabling.
