...
func MojoEthernetPreferredOverCellular(ctx context.Context, s *testing.State) {
	...
	if status, err := netconfig.NetworkStateIsConnectedOrOnline(networkStates[0]); err != nil {
		s.Fatal("Error checking Ethernet state: ", err)
	} else if !status {
		s.Fatal("Ethernet not Connected or Online")
	}
	...
	if status, err := netconfig.NetworkStateIsConnectedOrOnline(networkStates[1]); err != nil {
		s.Fatal("Error checking Cellular network state: ", err)
	} else if !status {
		s.Fatal("Cellular not Online or Connected")
	}
}
