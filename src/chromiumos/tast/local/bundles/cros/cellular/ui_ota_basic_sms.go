...
func UIOtaBasicSms(ctx context.Context, s *testing.State) {
	...
	*/
	...
	var phoneNumbers [2]string
	phoneNumbers[0] = "918317507407"
	if value != nil {
		phoneNumber, ok := value.([]string)
		if !ok {
			s.Fatal("Empty ownnumbers property: ", err)
		}
		phoneNumbers[0] = phoneNumber[0]
	}
	...
	_, err = uiHelper.ValidateMessage(ctx, messageToSend, phoneNumbers[0])
	if err != nil {
		faillog.DumpUITreeOnError(ctx, s.OutDir(), s.HasError, uiHelper.Tconn)
