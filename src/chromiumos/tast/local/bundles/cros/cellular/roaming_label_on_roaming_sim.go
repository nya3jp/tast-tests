...
func init() {
	testing.AddTest(&testing.Test{
		Func:         RoamingLabelOnRoamingSIM,
		LacrosStatus: testing.LacrosVariantUnneeded,
		Desc:         "Checks the roaming label status on a roaming SIM",
		Contacts: []string{
...
func RoamingLabelOnRoamingSIM(ctx context.Context, s *testing.State) {
	...
	err = cellular.ConfigureRoamingNetwork(ctx, tconn, cr)
	if err != nil {
		s.Fatal("Failed to set roaming property: ", err)
	}

	networkName, err := cellular.GetCellularNetwork(ctx, tconn, cr)
	if err != nil {
	...
	if roamingSubLabel != "Currently roaming" {
		s.Fatal("Roaming sub label is not 'Currently roaming'")
	}
}
