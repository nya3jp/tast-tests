...
func checkCannotRead(ctx context.Context, s *testing.State, k *pkcs11.KeyInfo, attributeName string) {
	res, _, err := k.GetObjectAttribute(ctx, "privkey", attributeName)
	if err == nil {
...
func checkCannotWrite(ctx context.Context, s *testing.State, k *pkcs11.KeyInfo, attributeName string) {
	_, err := k.SetObjectAttribute(ctx, "privkey", attributeName, "01")
	if err == nil {
...
func checkKey(ctx context.Context, s *testing.State, pkcs11Util *pkcs11.Util, k *pkcs11.KeyInfo) {
	...
	if _, err := k.SetObjectAttribute(ctx, "privkey", "CKA_ISSUER", IssuerTestValue); err != nil {
		s.Fatal("Unable to set CKA_ISSUER attribute: ", err)
	}

	// Read it back to check that it's set correctly.
	res, _, err := k.GetObjectAttribute(ctx, "privkey", "CKA_ISSUER")
	if err != nil {
	...
	for _, attributeName := range []string{"CKA_PRIME_1", "CKA_VALUE", "kKeyBlobAttribute"} {
		checkCannotRead(ctx, s, k, attributeName)
	}
	...
	for _, attributeName := range []string{"CKA_ALWAYS_SENSITIVE", "CKA_NEVER_EXTRACTABLE", "CKA_MODULUS", "CKA_EC_PARAMS", "kKeyBlobAttribute"} {
		checkCannotWrite(ctx, s, k, attributeName)
	}
...
func ChapsAttributePolicy(ctx context.Context, s *testing.State) {
	...
	importedKey, err := pkcs11Util.CreateRsaSoftwareKey(ctx, utility, "", "testkey1", "999999", false, true)
	if err != nil {
	...
	softwareKey, err := pkcs11Util.CreateRsaSoftwareKey(ctx, utility, "", "testkey2", "888888", true, true)
	if err != nil {
		s.Fatal("Failed to create software key: ", err)
	}
	defer func() {
