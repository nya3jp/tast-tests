...
// createKeysForTestingByUser create all the possible keys that we should test that belongs to the given username (reside in the slot that is associated with the user).
// Specify empty string for username to specify system token slot.
func createKeysForTestingByUser(ctx context.Context, username string, pkcs11Util *pkcs11.Chaps, scratchpadPath string) (keys []*pkcs11.KeyInfo, retErr error) {
	defer func() {
		if retErr != nil {
			// Function failed, we need to cleanup all created keys.
			for _, k := range keys {
				if err := pkcs11Util.DestroyKey(ctx, k); err != nil {
					testing.ContextLogf(ctx, "Failed to destroy key %s during cleanup when createKeysForTestingByUser failed: %q", pkcs11Util.DumpKeyInfo(k), err)
				}
			}
			keys = []*pkcs11.KeyInfo{}
		}
...
// CreateKeysForTesting will create the set of keys that we want to cover in our tests.
func CreateKeysForTesting(ctx context.Context, r hwsec.CmdRunner, pkcs11Util *pkcs11.Chaps, cryptohomeUtil *hwsec.UtilityCryptohomeBinary, scratchpadPath string) (keys []*pkcs11.KeyInfo, retErr error) {
	...
	if _, err := r.Run(ctx, "mkdir", "-p", userScratchpadPath); err != nil {
		return keys, errors.Wrap(err, "failed to create scratchpad for user keys")
	}
	retKeys, err := createKeysForTestingByUser(ctx, FirstUsername, pkcs11Util, userScratchpadPath)
	if err != nil {
	...
	if _, err := r.Run(ctx, "mkdir", "-p", systemScratchpadPath); err != nil {
		return keys, errors.Wrap(err, "failed to create scratchpad for system keys")
	}
	retKeys, err = createKeysForTestingByUser(ctx, "", pkcs11Util, scratchpadPath)
	if err != nil {
