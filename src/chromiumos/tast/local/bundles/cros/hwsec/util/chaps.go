...
// createKeysForTestingForUser create all the possible keys that we should test that belongs to the given username (reside in the slot that is associated with the user).
...
func CreateKeysForTesting(ctx context.Context, r hwsec.CmdRunner, pkcs11Util *pkcs11.Chaps, cryptohomeUtil *hwsec.UtilityCryptohomeBinary, scratchpadPath string) (keys []*pkcs11.KeyInfo, retErr error) {
	...
	if err := os.Mkdir(userScratchpadPath, 0755); err != nil {
	...
	if err := os.Mkdir(systemScratchpadPath, 0755); err != nil {
