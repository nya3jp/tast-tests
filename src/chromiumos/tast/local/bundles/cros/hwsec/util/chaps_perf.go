...
func ImportKeysAndMeasure(ctx context.Context, pkcs11Util *pkcs11.Chaps, privKeyPath string, slot int, username, prefix string, times int, softwareBacked bool) (importedKeys []*pkcs11.KeyInfo, importElapsed time.Duration, retErr error) {
	...
	if _, err := pkcs11Util.ImportPrivateKeyBySlot(ctx, pkcs11.ImportPrivateKeyOptions{
		PrivKeyPath:         privKeyPath,
		Slot:                slot,
		Username:            username,
		ObjID:               fmt.Sprintf("%sABCD", prefix),
		ForceSoftwareBacked: softwareBacked}); err != nil {
	...
	for i := 0; i < times; i++ {
		...
		key, err := pkcs11Util.ImportPrivateKeyBySlot(ctx, pkcs11.ImportPrivateKeyOptions{
			PrivKeyPath:         privKeyPath,
			Slot:                slot,
			Username:            username,
			ObjID:               objID,
