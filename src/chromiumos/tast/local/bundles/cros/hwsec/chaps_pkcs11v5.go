...
func ChapsPKCS11V5(ctx context.Context, s *testing.State) {
	...
	// Create the key.
	_, err = helper.Run(ctx, "openssl", "req", "-nodes", "-x509", "-sha1", "-newkey", "rsa:2048", "-keyout", "/tmp/testkey1.key", "-out", "/tmp/testkey1.crt", "-days", "365", "-subj", "/C=US/ST=CA/L=MTV/O=ChromiumOS/CN=chromiumos.example.com")
	if err != nil {
		s.Fatal("Failed to create testkey1 with OpenSSL: ", err)
	}

	// Convert the cert to DER format.
	_, err = helper.Run(ctx, "openssl", "x509", "-in", "/tmp/testkey1.crt", "-outform", "der", "-out", "/tmp/testkey1-cert.der")
	if err != nil {
		s.Fatal("Failed to convert testkey1 cert to DER format with OpenSSL: ", err)
	}

	// Extract the public key from the private key.
	_, err = helper.Run(ctx, "openssl", "rsa", "-in", "/tmp/testkey1.key", "-pubout", "-out", "/tmp/testkey1-pub.key")
	if err != nil {
		s.Fatal("Failed to extract testkey1 public key from private key with OpenSSL: ", err)
	}

	// Convert the private key to DER format.
	_, err = helper.Run(ctx, "openssl", "pkcs8", "-inform", "pem", "-outform", "der", "-in", "/tmp/testkey1.key", "-out", "/tmp/testkey1-priv.der", "-nocrypt")
	if err != nil {
	...
	for i := 0; i < 20; i++ {
		_, err = helper.Run(ctx, "pkcs11-tool", "--module="+chapsPath, "--slot=0", "--delete-object", "--type", "privkey", "--id", "aaaaaa")
		if err != nil {
	...
	for i := 0; i < 20; i++ {
		_, err = helper.Run(ctx, "pkcs11-tool", "--module="+chapsPath, "--slot=0", "--delete-object", "--type", "cert", "--id", "aaaaaa")
		if err != nil {
			// If we fail to delete that object, then it's already gone, so we are done.
			break
		}
	...
	_, err = helper.Run(ctx, "p11_replay", "--import", "--path=/tmp/testkey1-priv.der", "--type=privkey", "--id=aaaaaa")
	if err != nil {
		s.Fatal("Failed to import testkey1 private key into chaps: ", err)
	}

	// Import the certificate into chaps
	_, err = helper.Run(ctx, "p11_replay", "--import", "--path=/tmp/testkey1-cert.der", "--type=cert", "--id=aaaaaa")
	if err != nil {
	...
	_, err = helper.Run(ctx, "pkcs11-tool", "--module="+chapsPath, "--slot=0", "--id=aaaaaa", "--sign", "-m", "SHA1-RSA-PKCS", "-i", "/tmp/testfile1.txt", "-o", "/tmp/testfile1.sha1.sig")
	if err != nil {
		s.Fatal("Failed to sign with SHA1 PKCS1 v1.5: ", err)
	}

	// Verify with OpenSSL
	binaryMsg, err := helper.Run(ctx, "openssl", "dgst", "-sha1", "-verify", "/tmp/testkey1-pub.key", "-signature", "/tmp/testfile1.sha1.sig", "/tmp/testfile1.txt")
	if err != nil {
	...
	_, err = helper.Run(ctx, "openssl", "dgst", "-sha1", "-verify", "/tmp/testkey1-pub.key", "-signature", "/tmp/testfile1.sha1.sig", "/tmp/testfile2.txt")
	if err == nil {
		s.Fatal("OpenSSL verifies invalid signature of SHA1 PKCS1 v1.5")
	}

	// Test PKCS#1 v1.5 Signing with SHA256
	_, err = helper.Run(ctx, "pkcs11-tool", "--module="+chapsPath, "--slot=0", "--id=aaaaaa", "--sign", "-m", "SHA256-RSA-PKCS", "-i", "/tmp/testfile1.txt", "-o", "/tmp/testfile1.sha256.sig")
	if err != nil {
		s.Fatal("Failed to sign with SHA256 PKCS1 v1.5: ", err)
	}

	// Verify with OpenSSL
	binaryMsg, err = helper.Run(ctx, "openssl", "dgst", "-sha256", "-verify", "/tmp/testkey1-pub.key", "-signature", "/tmp/testfile1.sha256.sig", "/tmp/testfile1.txt")
	if err != nil {
	...
	// Check that if we verify against a different file, it'll fail.
	_, err = helper.Run(ctx, "openssl", "dgst", "-sha256", "-verify", "/tmp/testkey1-pub.key", "-signature", "/tmp/testfile1.sha256.sig", "/tmp/testfile2.txt")
	if err == nil {
