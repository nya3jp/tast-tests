...
func ChapsRSAPSS(ctx context.Context, s *testing.State) {
	...
	testFile1Path, testFile2Path, err := pkcs11Util.PrepareScratchpadAndTestFiles(ctx, "/tmp/ChapsRSAPSSTest")
	if err != nil {
	...
	importedKey, err := pkcs11Util.CreateRsaSoftwareKey(ctx, utility, "", "testkey1", "111111", false, true)
	if err != nil {
	...
	softwareKey, err := pkcs11Util.CreateRsaSoftwareKey(ctx, utility, "", "testkey2", "222222", true, true)
	if err != nil {
		s.Fatal("Failed to create software key: ", err)
	}
	defer func() {
	...
	for _, k := range keys {
		...
		for _, m := range []pkcs11.MechanismInfo{pkcs11.GetMechanism("SHA1-RSA-PKCS-PSS"), pkcs11.GetMechanism("SHA256-RSA-PKCS-PSS"), pkcs11.GetMechanism("RSA-PKCS-PSS+SHA1"), pkcs11.GetMechanism("RSA-PKCS-PSS+SHA256")} {
