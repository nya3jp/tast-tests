...
// checkOwnership check if the ownership of dirName is chaps:chronos-access
func checkOwnership(dirName string) (bool, error) {
	var err error
	correct := false
	if info, err := os.Stat(dirName); err == nil {
		stat := info.Sys().(*syscall.Stat_t)
		if usr, err := user.LookupId(strconv.FormatUint(uint64(stat.Uid), 10)); err == nil {
			if grp, err := user.LookupGroupId(strconv.FormatUint(uint64(stat.Gid), 10)); err == nil {
				if usr.Username == "chaps" && grp.Name == "chronos-access" {
					correct = true
				}
			}
		}
...
func Pkcs11InitOnLogin(ctx context.Context, s *testing.State) {
	...
	// Test the token is available.
	if _, err := r.Run(ctx, "cryptohome", "--action=pkcs11_is_user_token_ok"); err != nil {
		s.Error("The PKCS #11 token is not available.: ", err)
	}
	...
	if correct, err := checkOwnership(userChapsPath); err != nil || !correct {
		s.Error("User chaps path ownership setting is incorrect")
	}
	if correct, err := checkOwnership(userChapsPath + "/database"); err != nil || !correct {
		s.Error("User chaps database path ownership setting is incorrect")
	}
	...
	value.Set(perf.Metric{
		...
		Multiple:  false,
	}, timeElapsed.Seconds()/float64(util.ImportHWTimes))
	value.Save(s.OutDir())
}
