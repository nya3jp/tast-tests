...
func Attestation(ctx context.Context, s *testing.State) {
	...
	resp, err = a9n.SendGetRequestTo("https://test-dvproxy-server.sandbox.google.com/dvproxy/getchallenge")
	//resp, err = a9n.SendGetRequestTo("https://qa-dvproxy-server-gws.sandbox.google.com/dvproxy/getchallenge")
	if err != nil {
		s.Error("Failed to send request to VA: ", err)
	}
	challenge, err := a9n.DecodeBase64String(resp)
	if err != nil {
	...
	signedChallenge, err := utility.SignEnterpriseVAChallenge(
		...
		true,
		challenge)
	b64SignedChallenge := a9n.EncodeToBase64String([]byte(signedChallenge))
	if err != nil {
	...
	urlForVerification := "https://test-dvproxy-server.sandbox.google.com/dvproxy/verifychallengeresponse?signeddata=" + a9n.EscapeURL(b64SignedChallenge)
	s.Log(urlForVerification)
	resp, err = a9n.SendGetRequestTo(urlForVerification)
	if err != nil {
