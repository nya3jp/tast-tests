...
func init() {
	testing.AddTest(&testing.Test{
		...
		SoftwareDeps: []string{"chrome", "tmp"},
	})
}
...
func Attestation(ctx context.Context, s *testing.State) {
	...
	resp, err = attestation.SendGetRequestTo("https://test-dvproxy-server.sandbox.google.com/dvproxy/getchallenge")
	if err != nil {
		s.Fatal("Failed to send request to VA: ", err)
	}
	challenge, err := attestation.DecodeBase64String(resp)
	if err != nil {
	...
	signedChallenge, err := utility.SignEnterpriseVAChallenge(
		...
		true,
		challenge)
	b64SignedChallenge := attestation.EncodeToBase64String([]byte(signedChallenge))
	if err != nil {
	...
	urlForVerification := "https://test-dvproxy-server.sandbox.google.com/dvproxy/verifychallengeresponse?signeddata=" + attestation.EscapeURL(b64SignedChallenge)
	s.Log(urlForVerification)
	resp, err = attestation.SendGetRequestTo(urlForVerification)
	if err != nil {
