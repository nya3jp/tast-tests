...
// prepareKeyWithOpenSSL generate an RSA keypair in DER format and store in on the disk with OpenSSL.
...
// importKeysAndMeasure import the key specified by privKeyPath into token held by slot slot in chaps and import it times times. prefix should be a unique hex prefix between calls. It'll return the KeyInfo to the imported keys, the total duration and if an error occured.
func importKeysAndMeasure(ctx context.Context, pkcs11Util *pkcs11.Chaps, privKeyPath string, slot int, prefix string, times int, softwareBacked bool) (importedKeys []*pkcs11.KeyInfo, importElapsed time.Duration, retErr error) {
	...
	if _, err := pkcs11Util.ImportPrivateKeyBySlot(ctx, privKeyPath, slot, fmt.Sprintf("%sABCD", prefix), softwareBacked); err != nil {
		return []*pkcs11.KeyInfo{}, importElapsed, errors.Wrap(err, "warmup for import failed")
	}

	// Time the import operation.
	// We run import 16 times because there's a large variance in import run time, and we want to reduce that variance.
	importStart := time.Now()
	for i := 0; i < times; i++ {
		objID := fmt.Sprintf("%s%04X", prefix, i)
		key, err := pkcs11Util.ImportPrivateKeyBySlot(ctx, privKeyPath, slot, objID, softwareBacked)
		if err != nil {
			return []*pkcs11.KeyInfo{}, importElapsed, errors.Wrap(err, "failed to import keys")
		}
...
// signAndMeasure will sign the content pointed by tmpFile1 with mechanism mechanism and write the signature into tmpFile2 for times times. It'll return the total duration and if an error occured..
