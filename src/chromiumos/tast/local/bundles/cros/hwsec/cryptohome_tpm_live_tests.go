...
func CryptohomeTpmLiveTests(ctx context.Context, s *testing.State) {
	if errs := hwsec.ResetTpmAndSystemStates(ctx); len(errs) > 0 {
		s.Fatal("Failed to reset TPM or system states: ", errs)
	}

	if err := ready.Wait(ctx, func(msg string) { s.Log(msg) }); err != nil {
		s.Fatal("Failed to wait for system to be ready: ", err)
	}
