...
func ChapsPKCS1V15(ctx context.Context, s *testing.State) {
	...
	importedKey, err := pkcs11Util.CreateRSASoftwareKey(ctx, utility, scratchpadPath, "", "testkey1", "aaaaaa", false, true)
	if err != nil {
	...
	softwareKey, err := pkcs11Util.CreateRSASoftwareKey(ctx, utility, scratchpadPath, "", "testkey2", "bbbbbb", true, true)
	if err != nil {
		s.Fatal("Failed to create software key: ", err)
	}
	defer func() {
	...
	generatedKey, err := pkcs11Util.CreateRsaGeneratedKey(ctx, utility, scratchpadPath, "", "testkey3", "cccccc")
	if err != nil {
	...
	for _, k := range keys {
		...
		for _, n := range []string{"SHA1-RSA-PKCS", "SHA256-RSA-PKCS"} {
