...
func checkFWMPCleared(ctx context.Context, utility *hwsec.UtilityCryptohomeBinary) error {
	_, _, _, errorCode, err := utility.GetFirmwareManagementParameters(ctx)
	if err == nil {
		return errors.New("call to GetFirmwareManagementParameters succeeded when FWMP is cleared")
	}

	if errorCode != fwmpRemovedErrorCode {
		return errors.Errorf("call to GetFirmwareManagementParameters failed with an incorrect error code, got %q, want %q", errorCode, fwmpRemovedErrorCode)
	}

	return nil
}

// clearAndCheckFWMP clears FWMP and checks that it's cleared correctly. It return nil iff FWMP is successfully cleared.
func clearAndCheckFWMP(ctx context.Context, utility *hwsec.UtilityCryptohomeBinary) error {
...
func checkFWMPSet(ctx context.Context, utility *hwsec.UtilityCryptohomeBinary, expectedFlags, expectedHash string) error {
	flags, hash, _, _, err := utility.GetFirmwareManagementParameters(ctx)
	if err != nil {
...
func setAndCheckFWMP(ctx context.Context, utility *hwsec.UtilityCryptohomeBinary, flags, hash string) error {
...
func FirmwareManagementParameters(ctx context.Context, s *testing.State) {
	...
	if err := clearAndCheckFWMP(ctx, utility); err != nil {
	...
	if err := setAndCheckFWMP(ctx, utility, testFlags1, testHash1); err != nil {
	...
	if err := setAndCheckFWMP(ctx, utility, testFlags2, testHash2); err != nil {
