...
func newCryptohomeKeyDelegate(
	lf func(string, ...interface{}), dbusConn *dbus.Conn, testUser string,
	keyAlg cpb.ChallengeSignatureAlgorithm, rsaKey *rsa.PrivateKey,
	pubKeySPKIDER []byte) (keyDelegate *cryptohomeKeyDelegate, err error) {
	const (
		dbusPath  = "/org/chromium/CryptohomeKeyDelegate"
		dbusIface = "org.chromium.CryptohomeKeyDelegateInterface"
	)
	localKeyDelegate := cryptohomeKeyDelegate{
		lf, dbusConn, dbusPath, dbusIface, testUser, keyAlg, rsaKey, pubKeySPKIDER,
		0 /* challengeCallCnt */}
	if localErr := dbusConn.Export(&localKeyDelegate, dbusPath, dbusIface); localErr != nil {
		return nil, localErr
	}
	return &localKeyDelegate, nil
}
...
func handleChallengeKey(
	...
	if accountID.AccountId == nil || *accountID.AccountId != testUser {
		return nil, errors.Errorf(
			"wrong account_id: expected %s, got %v", testUser, accountID.AccountId)
	}
