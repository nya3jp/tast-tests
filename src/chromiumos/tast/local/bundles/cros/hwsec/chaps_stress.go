...
type chapsStressState struct {
	...
	// usernames above.
	passwords []string
	// mounted records whether a user is mounted. It corresponds to the usernames abov.
	mounted []bool
...
// getNextKeyID generates the label and keyID for the key that we're going to creat.
...
func ChapsStress(ctx context.Context, s *testing.State) {
	...
	// Seed the random with a deterministic seed for reproducable run.
