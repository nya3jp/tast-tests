...
func doCreateKeyTurn(ctx context.Context, state *chapsStressState, pkcs11Util *pkcs11.Chaps, scratchpadPath string, round roundType) (retErr error) {
	...
	createKeyFunc := func(label, keyID string) (*pkcs11.KeyInfo, error) {
		panic("Unused create key function called")
	}
	if round == createImportedKeyRound {
		...
		panic("Invalid round in doCreateKeyTurn")
	}
...
// - Remove a key
// - Sign with a key
func runOneTurn(ctx context.Context, state *chapsStressState, cryptohome *hwsec.CryptohomeClient, round roundType, pkcs11Util *pkcs11.Chaps, scratchpadPath, f1, f2 string) (retErr error) {
	if round == mountRound {
