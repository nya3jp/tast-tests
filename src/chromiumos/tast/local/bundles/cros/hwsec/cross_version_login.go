...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			// amd64-generic. However it could not be used on "reven-vmtest", since
			// its TPM is bound to different PCR. So, we use the hwdep.Model to
			// ensure the device could handle the data.
			// Note that If the data could not used on amd64-generic in the future,
			// we would need to split them to test set.
			ExtraHardwareDeps: hwdep.D(hwdep.Model("betty", "amd64-generic")),
			Val: []string{
