...
// testListKeys tests that ListVaultKeys works as expected
func testListKeys(ctx context.Context, cryptohome *hwsec.CryptohomeClient, username string, target []string) error {
...
// testGetKeyData tests that GetKeyData works as expected
func testGetKeyData(ctx context.Context, cryptohome *hwsec.CryptohomeClient, username string, targetLabels []string) error {
...
func testConfig(ctx context.Context, lf util.LogFunc, cryptohome *hwsec.CryptohomeClient, config *util.CrossVersionLoginConfig) error {
	...
	if authConfig.AuthType == hwsec.ChallengeAuth {
		...
		if err := testListKeys(ctx, cryptohome, username, targetLabels); err != nil {
			return errors.Wrap(err, "failed to properly list keys")
		}
		if err := testGetKeyData(ctx, cryptohome, username, targetLabels); err != nil {
			return errors.Wrap(err, "failed to properly get key data")
		}
