...
func MountCombinations(ctx context.Context, s *testing.State) {
	...
	for _, c := range TestCombinations {
		...
		if err := utility.MountVault(ctx, util.FirstUsername, c.password1, c.label1, false, []string{} /* extraFlags */); err != nil {
			s.Fatal("Failed to mount first user vault: ", err)
		}

		// Mount the second user.
		if err := utility.MountVault(ctx, util.SecondUsername, c.password2, c.label2, false, []string{} /* extraFlags */); err != nil {
