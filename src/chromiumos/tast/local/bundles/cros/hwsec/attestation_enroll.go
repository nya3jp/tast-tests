...
func AttestationEnroll(ctx context.Context, s *testing.State) {
	...
	if passwd, err := utility.GetOwnerPassword(ctx); err != nil || len(passwd) != 0 {
		...
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			// Presumably, all owner dependency should be automatically triggered after TPM ownership is taken.
	...
	signedChallenge, err := utility.SignSimpleChallenge(
		ctx,
		username,
		hwsec.DefaultCertLabel,
		[]byte{})
	if err != nil {
		s.Fatal("Failed to sign simple challenge")
	}
	publicKeyHex, err := utility.GetPublicKey(ctx, username, hwsec.DefaultCertLabel)
	if err != nil {
	...
	publicKeyHex2, err := utility.GetPublicKey(ctx, username, hwsec.DefaultCertLabel)
	if err != nil {
		s.Fatal("Failed to get public key from service: ", err)
	}
	...
	publicKeyDer, err := hwsec.HexDecode([]byte(publicKeyHex))
	if err != nil {
	...
	for _, sc := range []string{signedChallenge, signedChallenge2} {
		signedChallengeProto, err := hwsec.UnmarshalSignedData([]byte(sc))
