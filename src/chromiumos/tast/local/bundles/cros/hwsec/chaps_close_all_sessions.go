...
func ChapsCloseAllSessions(ctx context.Context, s *testing.State) {
	...
	// --use_sessions_loop will create a session, then notify through the ipc file that it's got a working session, then it'll continuously verify that the session is still working, until it've received communication from the ipc file that we're done, then it'll exit.
	// --check_close_all_sessions will wait for --use_sessions_loop to be ready through ipc file, then it'll create a session, call C_CloseAllSessions() to verify that it is closed, then signal to --use_sessions_loop through ipc file that it's done.
