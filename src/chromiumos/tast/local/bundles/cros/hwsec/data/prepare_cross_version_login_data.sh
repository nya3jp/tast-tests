...
# Uploads the data to the Google Cloud Storage and generate the external data file for tast-tests.
upload_data() {
...
prepare_data() (
  ...
  if ! tast run -failfortests "${HOST}:${PORT}" "${test_name}" ; then
...
rm -rf "${TMP_DIR}"
ls ${DATA_DIR} | sed 's/\(.*\).external/"cross_version_login\/\1",/g' | sort
