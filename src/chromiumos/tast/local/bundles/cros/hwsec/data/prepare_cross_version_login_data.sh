...
usage() {
  die "Usage: $(basename "$0") <board> <version> [<ssh_identity_file>]"
}
...
REMOTE_TMP_DIR="/tmp/cross_version_login"
TMP_DIR="$(mktemp -d)" || die "failed to mkdir ${TMP_DIR}"
IMAGE="${TMP_DIR}/chromiumos_test_image.bin"

BOARD="$1"
VERSION="$2"
KEY_FILE_SRC="${3-"${HOME}/trunk/chromite/ssh_keys/testing_rsa"}"
KEY_FILE="${TMP_DIR}/testing_rsa"

test -f "${KEY_FILE_SRC}" || die "keyfile '${KEY_FILE_SRC}' does not exist"
test -n "${BOARD}" || usage
test -n "${VERSION}" || usage


setup_keyfile() {
  if ! cp "${KEY_FILE_SRC}" "${KEY_FILE}" ; then
    error "failed to cp '${KEY_FILE_SRC}' to '${KEY_FILE}'"
    return 1
  fi
  if ! chmod 400 "${KEY_FILE}" ; then
    error "failed to chmod keyfile '${KEY_FILE}'"
    return 1
  fi
  return 0
}
...
upload_data() {
  ...
  local remote_data_path="${REMOTE_TMP_DIR}/data.tar.xz"

  local config_name="${prefix}_config.json"
  local config_path="${DATA_DIR}/${config_name}"
  local remote_config_path="${REMOTE_TMP_DIR}/config.json"
  ...
  if ! scp -o StrictHostKeyChecking=no -o GlobalKnownHostsFile=/dev/null \
      -o UserKnownHostsFile=/dev/null -o LogLevel=quiet -i "${KEY_FILE}" \
      -P "${PORT}" "root@${HOST}:${remote_data_path}" "${data_path}"
  then
  ...
  if ! scp -o StrictHostKeyChecking=no -o GlobalKnownHostsFile=/dev/null \
      -o UserKnownHostsFile=/dev/null -o LogLevel=quiet -i "${KEY_FILE}" \
      -P "${PORT}" "root@${HOST}:${remote_config_path}" "${config_path}"
  then
...
if setup_keyfile && fetch_image ; then
