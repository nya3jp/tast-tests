	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			clobberFunc: func(ctx context.Context) error {
				...
				if err := os.Chown(userPath, 0, 0); err != nil {
					return errors.Wrap(err, "failed to chown user home")
				}
				return nil
			},
			cleanupFunc: func() {
		...
		},
		{
			...
			clobberFunc: func(ctx context.Context) error {
				if err := os.Mkdir(systemPath, 0755); err != nil {
					return errors.Wrap(err, "failed to create user root")
				}
				if err := os.Chown(systemPath, 1, 1); err != nil {
					return errors.Wrap(err, "failed to chown user root")
				}
				return nil
			},
			cleanupFunc: func() {
		...
		},
		{
			...
			clobberFunc: func(ctx context.Context) error {
				if err := os.Mkdir(userPath, 0777); err != nil {
					return errors.Wrap(err, "failed to create user home")
				}
				if err := os.Chown(userPath, 1, 1); err != nil {
					return errors.Wrap(err, "failed to chown user home")
				}
				return nil
			},
