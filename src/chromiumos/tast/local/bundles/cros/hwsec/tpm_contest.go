...
func writeFailedCmdOutput(s *testing.State, output []byte) {
	file := "failed_cmd_output.txt"
	...
	if err := ioutil.WriteFile(path, output, 0644); err != nil {
		...
		s.Error("Path on DUT to command output: ", path)
	}
}
...
func TPMContest(ctx context.Context, s *testing.State) {
	...
	// Therefore, in the worst case len(commands)*2 results could be written to the channel
	...
	for _, cmd := range commands {
		...
		go func() {
			...
			if r.err == nil {
				...
				// Send default result as an error already occured
	...
	for i := 0; i < len(commands)*2; i++ {
		...
		if result.err != nil {
			...
			if result.output != nil {
				writeFailedCmdOutput(s, result.output)
			}
		}
		// Cancel workers if:
		// 1. An error occurs
