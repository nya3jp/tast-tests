...
// It would use |testPassword| to change the password, and |changePasswordShouldFail| represents the action of changing password should fail or not.
...
func migrateNonexistUserTest(ctx context.Context, s *testing.State, cryptohome *hwsec.CryptohomeClient) {
	...
	if err := cryptohome.ChangeVaultPassword(ctx, user, oldPassword, util.PasswordLabel, newPassword); err == nil {
		s.Fatal("Cryptohome was successfully changed the password for non-existent user; want: should have failed")
	}
}
