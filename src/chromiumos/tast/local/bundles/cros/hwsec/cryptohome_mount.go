...
// checkUserMountInfo checks the mount point information for user and checks the mount status with shouldMounted.
func checkUserMountInfo(ctx context.Context, mountInfo *hwsec.CryptohomeMountInfo, user string, shouldMounted bool) error {
	mounted, err := mountInfo.IsMounted(ctx, user)
	if shouldMounted && err != nil {
		return errors.Wrap(err, "failed to get mount info")
	}
	if shouldMounted && !mounted {
		return errors.New("user mount point not found")
	}
	if !shouldMounted && mounted {
		return errors.New("unexpected user mount point found")
	}
	return nil
}
...
func CryptohomeMount(ctx context.Context, s *testing.State) {
	...
	if err := cryptohome.MountVault(ctx, user, badPassword, util.PasswordLabel, false, hwsec.NewVaultConfig()); err == nil {
	...
	// Check the mount point information.
	if err := checkUserMountInfo(ctx, mountInfo, user, true); err != nil {
		s.Fatal("User mount point check failed after mount with good password: ", err)
	}
}
