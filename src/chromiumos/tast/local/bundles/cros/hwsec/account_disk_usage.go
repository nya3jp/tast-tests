...
import (
	...
	"chromiumos/tast/local/bundles/cros/hwsec/ownership"
...
func setupVaultAndUserAsOwner(ctx context.Context, s *testing.State, username, password, label string, utility *hwsec.UtilityCryptohomeBinary) error {
	// We need the policy/ownership related stuff becasue we want to set the owner, so that we can create ephemeral mount.
	...
	if err := utility.MountVault(ctx, username, password, label, true, []string{} /* extraFlags */); err != nil {
	...
	if err = sm.StartSession(ctx, username, ""); err != nil {
		s.Fatalf("Failed to start new session for %s: %v", username, err)
	}
...
func setupEphemeralVaultAndUser(ctx context.Context, username, password, label string, utility *hwsec.UtilityCryptohomeBinary) error {
	if err := utility.MountVault(ctx, username, password, label, true, []string{hwsec.MountFlagEphemeral} /* extraFlags */); err != nil {
...
func setupEcryptfsVaultAndUser(ctx context.Context, username, password, label string, utility *hwsec.UtilityCryptohomeBinary) error {
	if err := utility.MountVault(ctx, username, password, label, true, []string{hwsec.MountFlagEcryptfs} /* extraFlags */); err != nil {
		return errors.Wrap(err, "failed to create user vault for testing")
	}
...
func AccountDiskUsage(ctx context.Context, s *testing.State) {
	...
	// Setup the second user as ephemeral mount and test the ephemeral mount.
