...
// getDAInfo is a simple utility function that calls GetDAInfo from both tpm_manager and cryptohome, and see if they match. If both suceeded and the results agree with each other, then err is nil.
...
func DictionaryAttackLockoutReset(ctx context.Context, s *testing.State) {
	...
	if _, err := tpmManagerUtil.WriteSpaceFromFile(ctx, testNVRAMIndex, testFilePath, testIncorrectPassword); err == nil {
		// It suceeded when it shouldn't.
		s.Fatal("Write NVRAM Space suceeded with incorrect password")
	}
