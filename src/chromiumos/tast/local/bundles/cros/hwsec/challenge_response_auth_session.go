...
func ChallengeResponseAuthSession(ctx context.Context, s *testing.State) {
	...
	if err := cryptohome.CreateUserAuthSessionWithChallengeCredential(ctx, testUser, hwsec.NewChallengeAuthConfig(testUser, dbusName, keyDelegate.DBusPath, pubKeySPKIDER, keyAlg)); err != nil {
		s.Fatal("Failed to create the user: ", err)
	}

	_, err = cryptohome.AuthenticateAuthSessionWithChallengeCredential(ctx, testUser, false /*isEphemeral*/, hwsec.NewChallengeAuthConfig(testUser, dbusName, keyDelegate.DBusPath, pubKeySPKIDER, keyAlg))
	if err != nil {
