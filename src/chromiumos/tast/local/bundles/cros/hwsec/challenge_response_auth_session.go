...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Various tests with Smart Card backed login. Includes tests for creation, remounting and Lock Screen verification for both both persistent and ephemeral users",
		Contacts: []string{
...
// ChallengeResponseAuthSession initalizes the key delegate and dbus connections for the card and calls the respective tests.
func ChallengeResponseAuthSession(ctx context.Context, s *testing.State) {
	...
	for _, tc := range []struct {
		...
		persistentCreationAndRemount(ctx, s, tc.expectRemountSuccess, keyDelegate, pubKeySPKIDER)
	}
	...
	for _, tc := range []struct {
		...
		lockScreenLogin(ctx, s, tc.expectAuthSuccess, tc.isEphemeral, keyDelegate, pubKeySPKIDER)
	}
}

func persistentCreationAndRemount(ctx context.Context, s *testing.State, expectRemountSuccess bool, keyDelegate *util.CryptohomeKeyDelegate, pubKeySPKIDER []byte) {
	cmdRunner := hwseclocal.NewCmdRunner()
	...
	//Explicity invalidate AuthSession after use.
	if err := client.InvalidateAuthSession(ctx, authSessionID); err != nil {
		s.Fatal("Failed to invalidate the AuthSession: ", err)
	}
	...
	if expectRemountSuccess { // Remount should succeed.
		...
		authSessionID, err = cryptohome.AuthenticateAuthSessionWithChallengeCredential(ctx, fakeTestUser, false /*isEphemeral*/, hwsec.NewChallengeAuthConfig(testUser, dbusName, keyDelegate.DBusPath, pubKeySPKIDER, keyAlg))
		if err == nil {
...
func lockScreenLogin(ctx context.Context, s *testing.State, expectAuthSuccess, isEphemeral bool, keyDelegate *util.CryptohomeKeyDelegate, pubKeySPKIDER []byte) {
	const (
	...
	if isEphemeral {
		...
		if err := hwseclocal.SetUpVaultAndUserAsOwner(ctx, s.DataPath("testcert.p12"), ownerUser, "whatever", "whatever", utility); err != nil {
