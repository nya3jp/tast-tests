...
func CryptohomeNonDirs(ctx context.Context, s *testing.State) {
	...
	cryptohome, err := hwsec.NewUtilityCryptohomeClient(cmdRunner)
	if err != nil {
		s.Fatal("Failed to create cryptohome client: ", err)
	}
	...
	for _, tc := range []struct {
		...
		s.Run(ctx, tc.name, func(ctx context.Context, s *testing.State) {
			...
			if err := cryptohome.MountVault(ctx, user, pass, label, true, hwsec.NewVaultConfig()); err == nil {
				s.Fatal("Mount unexpectedly succeeded: ", err)
			}
		})
	}
}
