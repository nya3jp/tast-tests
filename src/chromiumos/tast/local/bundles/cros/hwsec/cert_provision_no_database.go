...
func CertProvisionNoDatabase(ctx context.Context, s *testing.State) {
	...
	if err := func() (retErr error) {
		if err := dCtrl.Stop(ctx, hwsec.AttestationDaemonInfo); err != nil {
			return errors.Wrap(err, "failed to stop attestationd")
		}
		defer func() {
			if err := dCtrl.Start(ctx, hwsec.AttestationDaemonInfo); err != nil && retErr == nil {
	...
	defer func() {
		...
		if err := dCtrl.Stop(ctx, hwsec.AttestationDaemonInfo); err != nil {
			...
			defer func() {
				if err := dCtrl.Start(ctx, hwsec.AttestationDaemonInfo); err != nil {
