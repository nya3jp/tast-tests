...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Verifies cert provision availability after clearing attestation database",
...
func CertProvisionNoDatabase(ctx context.Context, s *testing.State) {
	...
	libhwsec.StopAttestation(ctx, helper)
	if _, err := helper.Run(ctx, "mv", a9n.DBPath, a9n.DBPath+".bak"); err != nil {
		s.Fatal("Failed to rename attestation database")
	}
	libhwsec.StartAttestation(ctx, helper)
	// Restarts crytpohome so attestatoind can get system salt.
	...
	s.Log("Restoring database")
	if _, err := helper.Run(ctx, "mv", a9n.DBPath+".bak", a9n.DBPath); err != nil {
		s.Fatal("Failed to rename attestation database")
	}
