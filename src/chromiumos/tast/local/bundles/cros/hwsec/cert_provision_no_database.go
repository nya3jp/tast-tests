...
func CertProvisionNoDatabase(ctx context.Context, s *testing.State) {
	...
	libhwsec.StopAttestation(ctx, r)
	if _, err := r.Run(ctx, "mv", a9n.DBPath, a9n.DBPath+".bak"); err != nil {
		s.Fatal("Failed to rename attestation database")
	}
	libhwsec.StartAttestation(ctx, r)
	...
	if _, err := r.Run(ctx, "mv", a9n.DBPath+".bak", a9n.DBPath); err != nil {
		s.Fatal("Failed to rename attestation database")
	}
