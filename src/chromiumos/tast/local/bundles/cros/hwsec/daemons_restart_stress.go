...
func DaemonsRestartStress(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < 10; i++ {
		func() {
			if err := daemonController.Stop(ctx, hwsec.CryptohomeDaemonInfo); err != nil {
				s.Fatal("Failed to stop cryptohomed: ", err)
			}
			defer func() {
				if err := daemonController.Start(ctx, hwsec.CryptohomeDaemonInfo); err != nil {
					s.Fatal("Failed to start cryptohomed: ", err)
				}
			}()

			if err := daemonController.Stop(ctx, hwsec.AttestationDaemonInfo); err != nil {
				s.Fatal("Failed to stop attestationd: ", err)
			}
			defer func() {
				if err := daemonController.Start(ctx, hwsec.AttestationDaemonInfo); err != nil {
					s.Fatal("Failed to start attestationd: ", err)
				}
			}()

			if err := daemonController.Stop(ctx, hwsec.TPMManagerDaemonInfo); err != nil {
				s.Fatal("Failed to stop tpm_managerd: ", err)
			}
			defer func() {
				if err := daemonController.Start(ctx, hwsec.TPMManagerDaemonInfo); err != nil {
			...
			switch tpmVer {
			case "1.2":
				if daemonController.Restart(ctx, hwsec.TcsdDaemonInfo); err != nil {
					s.Fatal("Failed to restart tcsd: ", err)
				}
			case "2.0":
				if daemonController.Restart(ctx, hwsec.TrunksDaemonInfo); err != nil {
