...
func DaemonsRestartStress(ctx context.Context, s *testing.State) {
	...
	var restorePermCall func(ctx context.Context) error

	// Drop the reset lock permission.
	func() {
		resumeCallback, err := hwseclocal.StopTPMDaemons(ctx, false, false)
	...
	// The counter shouldn't constantly increase when we restart deamons multiple time, but it's acceptable to increase a little bit.
