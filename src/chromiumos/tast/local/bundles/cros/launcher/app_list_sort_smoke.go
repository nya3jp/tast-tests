...
func init() {
	testing.AddTest(&testing.Test{
		...
		Fixture:      "chromeLoggedInWith100FakeAppsInNameReverseOrderLauncherSort",
	})
}
...
const (
	increasing indexOrder = "increasing"
	decreasing indexOrder = "decreasing"
)
...
func AppListSortSmoke(ctx context.Context, s *testing.State) {
	...
	if !verifyGridIndicesFollowOrder(decreasing, gridIndices) {
	...
	if !verifyGridIndicesFollowOrder(increasing, gridIndices) {
	...
	if err := uiauto.Combine("redo temporary sorting",
...
func verifyGridIndicesFollowOrder(order indexOrder, gridIndices []launcher.GridIndex) bool {
