...
func CreateAndRenameFolder(ctx context.Context, s *testing.State) {
	...
	if originallyEnabled && !tabletMode {
		...
		if err := ui.EnsureGoneFor(launcherNode, 10*time.Second)(ctx); err != nil {
...
// createFolder is a helper function to create a folder by dragging an icon on top of the other icon.
func createFolder(ctx context.Context, tconn *chrome.TestConn, ui *uiauto.Context) error {
	...
	srcInfo, err := ui.Info(ctx, srcIcon)
	if err != nil {
		return errors.Wrap(err, "failed get informaton for the source icon")
	}

	targetInfo, err := ui.Info(ctx, targetIcon)
	if err != nil {
		return errors.Wrap(err, "failed get informaton for the target icon")
	}

	start, err := ui.Location(ctx, srcIcon)
	if err != nil {
		return errors.Wrapf(err, "failed to get locaton for icon %v", srcInfo.Name)
	}
	end, err := ui.Location(ctx, targetIcon)
	if err != nil {
		return errors.Wrapf(err, "failed to get locaton for icon %v", targetInfo.Name)
	}

	folder := nodewith.Name("Folder Unnamed").ClassName(launcher.ExpandedItemsClass)

	return ui.Retry(3, uiauto.Combine(fmt.Sprintf("createFolder(%s, %s)", srcInfo.Name, targetInfo.Name),
