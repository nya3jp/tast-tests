...
func NewlyInstalledApps(ctx context.Context, s *testing.State) {
	...
	isNewInstall, err := isInNewInstallState(ctx, cr, tconn, view)
	if err != nil {
		s.Fatal("Unable to compute new install state state: ", err)
	} else if !isNewInstall {
		s.Fatal("Unexpected new install state before launching the app")
	}
	...
	if err != nil {
		...
		s.Fatal("Unexpected new install state after launching the app")
	}
}
