...
func OpenTabSearch(ctx context.Context, s *testing.State) {
	...
	for _, expectedTabTitle := range expectedTabTitles {
		s.Run(ctx, expectedTabTitle, func(ctx context.Context, s *testing.State) {
			defer faillog.DumpUITreeWithScreenshotOnError(cleanupCtx, s.OutDir(), s.HasError, cr, "ui_dump_subtest_"+expectedTabTitle)
