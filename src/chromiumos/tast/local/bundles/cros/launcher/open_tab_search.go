...
func setupBrowser(ctx context.Context, s *testing.State, tconn *chrome.TestConn, ui *uiauto.Context, cr *chrome.Chrome, config browserConfig) (*browser.Conn, error) {
	...
	// Verify that the opened tabs have the expected titles before potentially minimizing the window.
	verifyTabs(ctx, s, ui, config)
	if config.minimized {
		ash.SetWindowStateAndWait(ctx, tconn, window.ID, ash.WindowStateMinimized)
	}
	return conn, nil
}

// verifyTabs checks that there exist tabs with expected titles as per config.
// Note that this assumes the tabs are in a browser window that has not been minimized.
func verifyTabs(ctx context.Context, s *testing.State, ui *uiauto.Context, config browserConfig) {
	for _, expectedTabTitle := range config.tabTitles {
