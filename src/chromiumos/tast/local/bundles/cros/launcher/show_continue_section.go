...
func ShowContinueSection(ctx context.Context, s *testing.State) {
	...
	if sortNudgeFound, err := ui.IsNodeFound(ctx, sortNudge); err != nil {
		s.Fatal("Failed to search for continue section: ", err)
	} else if sortNudgeFound {
	...
	for i := 0; i < numFiles; i++ {
		fileName := fmt.Sprintf("fake-file-%d-%d.html", time.Now().UnixNano(), rand.Intn(10000))
	...
	// If privacy notice is shown, click on the button to confirm it.
	privacyNoticeButton := nodewith.Ancestor(continueSection).ClassName("PillButton")
	if privacyNoticeFound, err := ui.IsNodeFound(ctx, privacyNoticeButton); err != nil {
		s.Fatal("Failed to search for privacy toast: ", err)
	} else if privacyNoticeFound {
	...
	for i, filePath := range testDocFileNames {
		...
		if continueSectionFound, err := ui.IsNodeFound(ctx, continueSection); err != nil {
			s.Fatal("Failed to search for continue section: ", err)
		} else if !continueSectionFound {
			if err := openLauncher(ctx, tconn, tabletMode); err != nil {
				s.Fatal("Failed to open launcher: ", err)
			}
		}
		...
		if err := ash.WaitForApp(ctx, tconn, chromeApp.ID, 10*time.Second); err != nil {
			s.Fatalf("File task %d - %s never opened: %v", i, filePath, err)
		}

		fileContentNode := nodewith.Name(fmt.Sprintf("Test file %d", i)).First()
		if err := ui.WaitUntilExists(fileContentNode)(ctx); err != nil {
			s.Fatal("Failed to find file contents node: ", err)
		}
