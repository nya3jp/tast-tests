...
func CreateAndFillFolder(ctx context.Context, s *testing.State) {
	...
	if err := ui.Retry(3, launcher.CreateFolder(ctx, tconn, ui))(ctx); err != nil {
		s.Fatal("Failed to create colder app: ", err)
	}

	// Add remaining 18 app items on this app list page to the folder.
	for i := 0; i < 18; i++ {
		launcher.DragIconToIcon(tconn, ui, 1, 0)(ctx)
	}
	...
	var size int
	size, err = launcher.GetFolderSize(ctx, tconn, folder)
	if err != nil {
