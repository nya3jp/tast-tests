...
func UninstallApp(ctx context.Context, s *testing.State) {
	...
	appItem := nodewith.Name(app.name).HasClass(launcher.ExpandedItemsClass).Ancestor(nodewith.NameContaining("Recent Apps").HasClass("RecentAppsView"))
	...
	if err := ui.EnsureGoneFor(nodewith.Name(app.name).HasClass(launcher.ExpandedItemsClass), 10*time.Second)(ctx); err != nil {
		s.Fatalf("The app %s still remains in launcher: %v", app.name, err)
	}
}

// installApp installs the app and returns an error indicating if the app has been successfully installed.
func installApp(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, a *arc.ARC, app *appInfo) error {
	cleanupCtx := ctx
	...
	case fromPlayStore:
		...
		if err := playstore.InstallOrUpdateAppAndClose(ctx, tconn, a, d, app.arcAppPkgName, &playstore.Options{TryLimit: -1, InstallationTimeout: installationTimeout}); err != nil {
			return errors.Wrapf(err, "failed to install %s", app.name)
		}
	default:
