...
func AppDragAndDrops(ctx context.Context, s *testing.State) {
	...
	if err := dragandDrops(ctx, tconn, cr, false); err != nil {
		s.Fatal("Failed to drag and drop on app icons by mouse: ", err)
	}
	if err := dragandDrops(ctx, tconn, cr, true); err != nil {
		s.Fatal("Failed to drag and drop on app icons by touch: ", err)
	}
}

// dragandDrops tests the functionality of dragging and dropping on app icons in different ways.
func dragandDrops(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome, isTouch bool) error {
	cleanupCtx := ctx
	ctx, cancel := ctxutil.Shorten(ctx, 10*time.Second)
	defer cancel()
	...
	targetApp := nodewith.HasClass("AppListItemView").First()
...
func (m *dragAndDropByMouse) dragToNextPage(ctx context.Context, src *nodewith.Finder) error {
	...
	end, err := m.ui.Location(ctx, nodewith.HasClass("AppsGridView"))
	if err != nil {
		return errors.Wrap(err, "failed to get location for AppsGridView")
	}
	endPoint := coords.NewPoint(end.CenterPoint().X, end.Bottom())
