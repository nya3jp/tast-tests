...
func BubbleSmoke(ctx context.Context, s *testing.State) {
	...
	// Wait until the bubble bounds become stable.
	if err := ui.WaitForLocation(bubble)(ctx); err != nil {
		s.Fatal("Failed to wait for bubble location changes: ", err)
	}

	// Ensure that system apps show by scrolling to the end through focus traversal when the bubble is in overflow.
	if err := kb.TypeKey(ctx, input.KEY_UP); err != nil {
		s.Fatalf("Failed to send the key up %d: %v", input.KEY_UP, err)
	}
	...
	if err := ui.WaitForLocation(settingButton)(ctx); err != nil {
		s.Fatal("Failed to wait for the setting button location changes: ", err)
	}
