...
// BubbleSmoke runs basic smoke tests for the bubble launcher.
func BubbleSmoke(ctx context.Context, s *testing.State) {
	...
	ui := uiauto.New(tconn)
	bubble := nodewith.ClassName("AppListBubbleView")
	...
	if err := uiauto.Combine("close bubble by launching Settings app",
		ui.LeftClick(nodewith.Role(role.Button).Name("Settings").Ancestor(bubble)),
		ui.WaitUntilGone(bubble),
	)(ctx); err != nil {
		s.Fatal("Could not close bubble by clicking in screen corner: ", err)
	}
