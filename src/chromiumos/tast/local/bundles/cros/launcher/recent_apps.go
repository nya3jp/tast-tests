...
func RecentApps(ctx context.Context, s *testing.State) {
	...
	if err := hideLauncherWithOverview(ctx, tconn, tabletMode); err != nil {
...
// hideLauncherWithOverview hides the launcher and waits until the launcher is hidden or its state becomes "hidden".
// Unlike the regular flow that hides the launcher on tablet mode by opening an app, this method enters
// overview mode with the swipe gesture that only works on tablet mode.
// When tabletMode is false, the regular flow to close the bubble launcher is used.
func hideLauncherWithOverview(ctx context.Context, tconn *chrome.TestConn, tabletMode bool) error {
	if tabletMode {
