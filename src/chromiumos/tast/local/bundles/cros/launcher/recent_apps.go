...
func RecentApps(ctx context.Context, s *testing.State) {
	...
	if err := verifyRecentAppsOrder(ctx, tconn, expectedOrderedAppNames); err != nil {
...
func uninstallAppViaSettings(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, name, id string) error {
	...
	testing.ContextLogf(ctx, "Uninstall CWS app: %q", name)
...
// verifyRecentAppsOrder confirms that apps in the recent app section appear in the same order as orderedAppsNames
func verifyRecentAppsOrder(ctx context.Context, tconn *chrome.TestConn, orderedAppsNames []string) error {
	ui := uiauto.New(tconn)
	recentApps := nodewith.ClassName("RecentAppsView")
	if err := ui.Exists(recentApps)(ctx); err != nil {
