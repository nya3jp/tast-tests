...
type settingsTestParams struct {
	...
	query        string
	featureFlag  string
	waitForApp   peripheraltypes.UIDriver
	settingsPage string
}
...
func LaunchAppFromSettings(ctx context.Context, s *testing.State) {
	...
	if !ok {
		crWithFeature, err := chrome.New(ctx, chrome.EnableFeatures(params.featureFlag))
		if err != nil {
	...
	// Condition to satify verification of LaunchAtPageURL
