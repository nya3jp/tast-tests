...
type cmdVars struct {
	chromad    chrome.ChromadSettings
	user, pass string
	reset      bool
	extraArgs  []string
}
...
func getVars(s *testing.State) cmdVars {
	var c chrome.ChromadSettings
	c.EnrollUser = optionalVar(s, "enrollUser", "admin@chromad-muc-tier15.deviceadmin.goog")
	c.EnrollPass = optionalVar(s, "enrollPass", "test765!")
	c.DomainUser = optionalVar(s, "domainUser", "feiling@chromad.cros-kir.com")
	c.DomainPass = optionalVar(s, "domainPass", "ad3pky001!")
...
func CollectLogs(ctx context.Context, s *testing.State) {
	...
	opts = append(opts, chrome.EnableChromad(vars.chromad))
