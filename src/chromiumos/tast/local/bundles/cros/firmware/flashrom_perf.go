...
func FlashromPerf(ctx context.Context, s *testing.State) {
	...
	const lowerBound = 250 // sub-process execution should take some time.

	if err := perf.Save(s.OutDir()); err != nil {
		s.Error("Failed saving perf data: ", err)
	}
	if duration <= lowerBound || duration >= deadline {
		s.Errorf("Flashrom execute outside time-bounds, %v < expected < %v ms, got = %v ms", lowerBound, p.deadline, duration)
	}
}
