...
func FwupdPowerdUpdateCheck(ctx context.Context, s *testing.State) {
	...
	if discharge {
		...
		// This sleep timer allows the powerd plugin to adjust to the discharging state created by SetBatteryDischarge.
		testing.Sleep(ctx, 10*time.Second)

		// This sleep timer allows the powerd plugin to adjust to the charging state created by SetBatteryNormal
