...
const (
	flashromCmdRegMst = "^-p realtek_mst_i2c_spi:bus=8,reset-mcu=1,enter-isp=1 --flash-size[\n]+" +
		"-p realtek_mst_i2c_spi:bus=8,reset-mcu=0,enter-isp=1 --layout /tmp/flashrom-i2c-8-" +
		"[a-zA-Z0-9]{6}/layout --image PARTITION[0-9]:/tmp/flashrom-i2c-8-[a-zA-Z0-9]{6}/" +
		"rtd2142-V99.99.bin -w[\n]+-p realtek_mst_i2c_spi:bus=8,reset-mcu=1,enter-isp=0" +
		" --layout /tmp/flashrom-i2c-8-[a-zA-Z0-9]{6}/layout --image" +
		" FLAG[0-9]:/tmp/flashrom-i2c-8-[a-zA-Z0-9]{6}/flag[0-9].bin -w[\n]*$"
)
...
func FwupdInstallRtd2142(ctx context.Context, s *testing.State) {
	...
	if err := verifyFlashromCmdMst(filepath.Join(s.OutDir(), "fwupd_flashrom_call_parameters.txt")); err != nil {
