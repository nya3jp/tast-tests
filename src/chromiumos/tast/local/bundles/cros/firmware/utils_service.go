...
func (*UtilsService) ReadServoKeyboard(ctx context.Context, req *empty.Empty) (*fwpb.ReadServoKeyboardResponse, error) {
	// The servo's keyboard emulator device node has a symlink, captured here as a constant, that will always
	// link to the actual node (which can get assigned different values, transparently since we use the
	// symlink).
	...
	// completion of events.  For now, this constant sets the number of bytes to read from the servo's keyboard
	// emulator, somewhat arbitrarily (128 is observed through experimentation to be enough to capture at least
	// two keyboard events.
