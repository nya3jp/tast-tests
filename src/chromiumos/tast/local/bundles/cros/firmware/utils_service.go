...
func (us *UtilsService) ReadServoKeyboard(ctx context.Context, req *fwpb.ReadServoKeyboardRequest) (*fwpb.ReadServoKeyboardResponse, error) {
	...
	// TODO(kmshelton): Migrate to using a library if the usecase of using evdev bindings is considered strong enough
	// to deal with the drawbacks that come with enabling cgo in tast (b:187786098).
	...
	_ = cmd.Wait()
	// An occurance of "value 0" in evtest output corresponds to a press of a key, whereas "value 1" is a release.
	re := regexp.MustCompile(`\(([A-Z_]*)\), value 0`)
	...
	us.s.Log(keys)
	return &fwpb.ReadServoKeyboardResponse{Key: keys}, nil
}
