...
func checkForPowerdDisabled(output []byte, s *testing.State) error {
	...
	if err := json.Unmarshal(output, &w); err != nil {
		s.Error("Failed parsing command output: ", err)
	}

	for i := range w.Plugins {
		if w.Plugins[i].Name == "powerd" {
			for j := range w.Plugins[i].Flags {
				if w.Plugins[i].Flags[j] == "disabled" {
					return nil
				}
			}
		}
...
func FwupdPowerdDisabled(ctx context.Context, s *testing.State) {
	...
	if err := startupPowerd(ctx); err != nil {
		s.Fatal("Failed to restart the power daemon after testing: ", err)
	}
}
