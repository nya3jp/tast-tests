...
func WaitForStart(ctx context.Context, s *testing.State, finished chan bool) {
        conn, err := dbus.SessionBus()
        if err != nil {
                s.Log(os.Stderr, "Failed to connect to session bus:", err)
                os.Exit(1)
        }
...
func FwupdInhibitSuspend(ctx context.Context, s *testing.State) {
	...
	go WaitForStart(ctx, s, updateStarted)
	<-updateStarted
