...
func FwupdInhibitSuspend(ctx context.Context, s *testing.State) {
        if _, err := os.Stat("/run/lock/power_override/fwupd.lock"); err == nil {
                return errors.New("System cannot suspend but no update has started")
        }

        val := RunUpdate()
        if _, err := os.Stat("/run/lock/power_override/fwupd.lock"); os.IsNotExist(err) {
                return errors.New("System can suspend but update is in progress")
        }

        if _, err := os.Stat("/run/lock/power_override/fwupd.lock"); err == nil {
                return errors.New("System cannot suspend but update has finished")
        }
}
