...
func extractBinaryToFlash(ctx context.Context, tarballPath string) string {
	...
	return "bloonchipper/test-utils.bin"
}
...
func flashUnittestBinary(ctx context.Context, s *testing.State) {
	binaryToFlash := extractBinaryToFlash(ctx, s.DataPath("fpmcu_unittests.tar.bz2"))
