...
func ACVP(ctx context.Context, s *testing.State) {
	...
	if newFormat {
		acvBytes, err := json.Marshal(a)
		if err != nil {
			s.Fatal("Can't marshal acv version to bytes")
		}
		outfileBytes := append([]byte("["), acvBytes...)
		outfileBytes = append(outfileBytes, byte(','))
		...
		outResultBytes, err := json.MarshalIndent(outResult, "", "    ")
		if err != nil {
			s.Fatal("Can't marshal out results interface to bytes")
		}
		outfileBytes = append(outfileBytes, outResultBytes...)
		outfileBytes = append(outfileBytes, byte(']'))
		if err := ioutil.WriteFile(filepath.Join(s.OutDir(), outFile),
			outfileBytes, 0644); err != nil {
