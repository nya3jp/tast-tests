...
func (e *ecdsa) setCurve(curvehex string) error {
...
func (e *ecdsa) setHash(hashBytes string) error {
	switch e.op {
	case tpmSigVer, tpmSigGen:
		hash, err := hex.DecodeString(hashBytes)
		if err != nil {
			return errors.Errorf("unable to decode hash hex value: %q", hashBytes)
		}
...
func (e *ecdsa) setDigest(msg string) error {
	var digestSlice = make([]byte, 32)
	msgBytes, err := hex.DecodeString(msg)
	if err != nil {
		return err
	}
	msgDigest := sha256.Sum256(msgBytes)
	digestSlice = msgDigest[:]
	e.digest = hex.EncodeToString(digestSlice)
	return nil
}
...
func newECDSA(curve, r, s, d, qx, qy, msg, hash, op string) (*ecdsa, error) {
	e := &ecdsa{r: r, s: s, d: d, qx: qx, qy: qy}
	if err := e.setOp(op); err != nil {
		return e, err
	}
	if err := e.setCurve(curve); err != nil {
		err = errors.Errorf("ECDSA params: %s, %s, %s, %s, %s, %s, %s, %s, %s", curve, r, s, d, qx, qy, msg, hash, op)
		return e, err
	}
	...
	if e.op == tpmSigGen || e.op == tpmSigVer {
		if err := e.setDigest(msg); err != nil {
			return e, err
		}
	}
	return e, nil
...
func (w *cr50IO) populateOutBuf(b []byte, n uint8) error {
	...
	if n == 1 {
		...
		// Skip first byte as it is hardcoded to 1
		b = b[1:]
		for i := uint8(0); i < n; i++ {
...
// Returns command, type of operation, and possible error value.
func getTrunksCmds(b []byte) ([]string, uint8, error) {
	...
	case "ECDSA":
		...
		case 8: //sigver
			...
			s = algArgs[7]
		default:
			return nil, 0, errors.Errorf("incorrect number of args for ECDSA operation: got %d, want 1, 3, 4, or 6", argLen)
		}
		e, err := newECDSA(curve, r, s, d, qx, qy, msg, hash, op)
		if err != nil {
			//err = errors.Errorf("ECDSA params: %s, %s, %s, %s, %s, %s, %s, %s, %s", curve, r, s, d, qx, qy, msg, hash, op)
			return nil, 0, err
		}
