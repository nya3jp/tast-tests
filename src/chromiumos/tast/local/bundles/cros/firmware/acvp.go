...
const (
	...
	aesECBTestFile     = "aes-ecb-test.json"
)
...
// Converts contents of inBuf into a trunks command.
...
// Takes a cr50 command response and
...
// Constructs a trunks AES command
...
// Takes a JSON generated by the ACVP server and runs the test cases in it.
func ACVP(ctx context.Context, s *testing.State) {
	var vectors acvp.Vectors
	vectorsBytes, err := ioutil.ReadFile(s.DataPath(aesECBTestFile))
	if err != nil {
		s.Fatal("Failed reading internal data file: ", err)
	} else {
		s.Log("Read internal data file: ", aesECBTestFile)
	}
