...
func (w *cr50IO) populateOutBuf(b []byte, n uint8) error {
	if len(b)%int(n) != 0 {
		return errors.Errorf("unexpected response length: %d for %d response values", len(b), n)
	}
	respSize := uint32(len(b)) / uint32(n)
	...
	for i := uint8(0); i < n; i++ {
		w.outBuf.Write(respSizeBytes) // assumption: each response value is same size (is true now, but could change in the future)
	}
	w.outBuf.Write(b)
	return nil
}
