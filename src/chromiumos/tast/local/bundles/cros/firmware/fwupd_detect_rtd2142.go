...
func verifyRTD2142Detected(output []byte, s *testing.State) error {
	matched, err := regexp.Match(rtd2142InfoPattern, output)
	if err != nil {
		return err
	}
	if !matched {
		if err := ioutil.WriteFile(filepath.Join(s.OutDir(), "fwupd_output.txt"),
			output, 0644); err != nil {
			s.Error("Failed to write fwupd output to file: ", err)
		}
...
func FwupdDetectRTD2142(ctx context.Context, s *testing.State) {
	...
	if err := verifyRTD2142Detected(output, s); err != nil {
