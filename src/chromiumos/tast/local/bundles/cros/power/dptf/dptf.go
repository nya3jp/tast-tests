// Copyright 2020 The Chromium OS Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// Package dptf implements the test scenario for the power.dptf* test.
package dptf

import (
	"context"
	"os"
	"strings"

	"chromiumos/tast/errors"
	"chromiumos/tast/local/testexec"
)

// GetProfileFromOverrideScript checks existence of dptf_override.sh and calls
// the script get expected DPTF profile.
func GetProfileFromOverrideScript(ctx context.Context) (string, error) {
	if _, err := os.Stat("/etc/dptf/dptf_override.sh"); err != nil {
		if os.IsNotExist(err) {
			return "", errors.Wrap("Can't find DPTF override script")
		}
		return "", errors.Wrap("Unexpected os.Stat error: ", err)
	}

	out, err := testexec.CommandContext(ctx, "sh", "-c", ". /etc/dptf/dptf_override.sh; dptf_get_override").Output()
	if err != nil {
		return "", errors.Wrap("Error in DPTF override script: ", err)
	}
	if len(out) == 0 {
		return "", errors.Wrap("Can't find DPTF profile from override script")
	}
	return "/etc/dptf/" + strings.TrimSpace(string(out)), nil
}

// GetProfileFromCrosConfig calls cros_config to get expected DPTF profile.
func GetProfileFromCrosConfig(ctx context.Context) (string, error) {
	out, err = testexec.CommandContext(ctx, "cros_config", "/thermal", "dptf-dv").Output()
	if err != nil {
		// cros_config won't return 0 when expecting no DPTF profile.
		return "", nil
	}
	if len(out) == 0 {
		return "", errors.Wrap("Can't find DPTF profile via cros_config")
	}
	return "/etc/dptf/" + strings.TrimSpace(string(out)), nil
}

// GetProfileFromPgrep uses pgrep to get DPTF profile currently in use.
func GetProfileFromPgrep(ctx context.Context) (string, error) {
	out, err = testexec.CommandContext(ctx, "pgrep", "-a", "esif_ufd").Output()
	if err != nil {
		return "", errors.Wrap("Search for DPTF process failed: ", err)
	}
	if len(out) == 0 {
		return "", errors.Wrap("Can't find DPTF process")
	}
	outSlice := strings.Fields(string(out))
	last := outSlice[len(outSlice)-1]
	if strings.HasPrefix(last, "/etc/dptf/") {
		return last, nil
	}
	return "", nil
}
