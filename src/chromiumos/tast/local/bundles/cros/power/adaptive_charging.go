...
func AdaptiveCharging(ctx context.Context, s *testing.State) {
	...
	if err := testexec.CommandContext(ctx, "stop", "powerd").Run(); err != nil {
		s.Fatal("Failed to stop powerd: ", err)
	}
	defer testexec.CommandContext(ctx, "restart", "powerd").Run()
	...
	if err := testexec.CommandContext(ctx, "start", "powerd").Run(); err != nil {
	...
	go mlservice.FakeAdaptiveChargingMLService(ctx)
...
func testChargeNow(ctx context.Context, s *testing.State, cr *chrome.Chrome) {
	...
	if err := ui.WaitUntilExists(chargeNowButton)(ctx); err != nil {
	...
	if !isBatterySustaining(ctx, s) {
		s.Fatal("Battery sustainer is not on")
	}
...
func pollUntilBatterySustainingState(ctx context.Context, s *testing.State, sustaining bool) {
	if err := testing.Poll(ctx, func(c context.Context) error {
		...
		if sustaining {
			s.Fatal("Timeout waiting for Adaptive Charging to start: ", err)
		} else {
			s.Fatal("Timeout waiting for Adaptive Charging to stop: ", err)
		}
	}
}
