...
func adapterDetection(ctx context.Context) error {
	LsusbCmd := "lsusb -t"
	// USBDetectionPattern := regexp.MustCompile(`Class=Hub.*(480M|5000M|10G|20G)`)
	USBDetectionPattern := regexp.MustCompile(`Class=,.*12M`)
	out, err := testexec.CommandContext(ctx, "sh", "-c", LsusbCmd).Output()
	if err != nil {
		return errors.Wrapf(err, "failed to execute %s command: ", LsusbCmd)
	}
	if !(USBDetectionPattern).MatchString(string(out)) {
