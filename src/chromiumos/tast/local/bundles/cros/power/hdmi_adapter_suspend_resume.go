...
func assertSLPCounter(ctx context.Context, slpOpSetPre string) error {
	...
	if string(slpOpSetPost) == "0" {
		return errors.Error("failed SLP counter value must be non-zero, noted is: ", slpOpSetPost)
	}
	return nil
}
...
func assertPackageCState(ctx context.Context, pkgOpSetPre string) error {
	...
	if matchSetPost == nil {
		return errors.Error("failed to match post PkgCstate value: ", pkgOpSetPostOutput)
	}
