...
func WilcoPeakShift(ctx context.Context, s *testing.State) {
	...
	verifyACUsage := func(expected bool) {
		pollPowerStatus := func(pollCtx context.Context) error {
			status := wilco.GetPowerStatus(pollCtx, s)
	...
	defer wilco.WriteFileStrict(s, peakShiftEnablePath, wilco.ReadFileStrict(s, peakShiftEnablePath))
	defer wilco.WriteFileStrict(s, peakShiftBatteryThresholdPath, wilco.ReadFileStrict(s, peakShiftBatteryThresholdPath))
	defer wilco.WriteFileStrict(s, peakShiftSchedulePath, wilco.ReadFileStrict(s, peakShiftSchedulePath))
	...
	wilco.WriteFileStrict(s, peakShiftEnablePath, "1")
	wilco.WriteFileStrict(s, peakShiftBatteryThresholdPath, "15")

	s.Log("Setting schedule to disable Peak Shift, waiting for the DUT to use AC")
	wilco.WriteFileStrict(s, peakShiftSchedulePath, "01:00 02:00 03:00")
	verifyACUsage(true)

	s.Log("Setting schedule to use full Peak Shift, waiting for the DUT to not use AC")
	wilco.WriteFileStrict(s, peakShiftSchedulePath, "01:00 22:00 23:00")
	verifyACUsage(false)
