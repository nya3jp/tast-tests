...
func TabletModePowerOffMenu(ctx context.Context, s *testing.State) {
	...
	// ui = uiauto.New(tconn)
	// if err := ui.WaitUntilExists(nodewith.Name("Sign in")); err != nil {
	// 	s.Fatal("Failed to wait for 'Sign in' button: ", err)
	// }
	...
	// if err := uiauto.Combine("press 'Shut down' button", ui.WaitUntilExists(shutdownButton), ui.LeftClick(shutdownButton))(ctx); err != nil {
	// 	s.Fatal("Failed to find and press 'Shut down' button: ", err)
	// }
...
func runSignOutTest(ctx context.Context, s *testing.State, p *power.PowerManagerEmitter) {
	cleanupCtx := ctx
	ctx, cancel := ctxutil.Shorten(ctx, 10*time.Second)
	defer cancel()
