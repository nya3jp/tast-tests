...
func TimeTakenSuspendResume(ctx context.Context, s *testing.State) {
	...
	if sd > suspendTime || rd > resumeTime {
		s.Fatalf("Failed : Suspend or Resume duration is greater than expected; got suspend time %v, want suspend time %v, got resume time %v, want resume time %v", sd, suspendTime, rd, resumeTime)
	}
}
