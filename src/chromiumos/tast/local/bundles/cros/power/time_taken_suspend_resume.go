...
func TimeTakenSuspendResume(ctx context.Context, s *testing.State) {
	...
	configValue, err := testexec.CommandContext(ctx, "bash", "-c", powerdConfigCmd).Output(ssh.DumpLogOnError)
	if err != nil {
	...
	if sd > time.Second && rd > time.Second {
		s.Fatalf("Failed : Suspend or Resume duration is greater than expected < 1.0, got %f, %f", suspendDuration, resumeDuration)
	}
}
...
// readSuspendResumeDuration reads and calculates the wakeup duration from last_resume_timings file.
...
func readSuspendResumeDuration(ctx context.Context, earliestModTime time.Time) (float64, float64, error) {
	const (
	...
	if err := testing.Poll(ctx, func(c context.Context) error {
		...
		return 0.0, 0.0, errors.Wrapf(err, "failed to check existence of a new last_resume_timings file within %v", pollOpts.Timeout)
	}
	...
	lines := []*regexp.Regexp{
		...
		regexp.MustCompile(`start_suspend_time\s*=\s*(\d+\.\d+)`),
		regexp.MustCompile(`end_suspend_time\s*=\s*(\d+\.\d+)`),
	}
	timestamps := []float64{0.0, 0.0, 0.0, 0.0} // start and end timestamp extracted from the file.
	for i, line := range lines {
		if ss := line.FindStringSubmatch(string(b)); ss != nil {
			timestamp, err := strconv.ParseFloat(ss[1], 64)
			if err != nil {
				return 0.0, 0.0, errors.Wrapf(err, "failed to get timestamp for %v", line)
			}
	...
	if len(timestamps) != 4 {
		return 0.0, 0.0, errors.New("failed to find timestamps")
	}

	secondsSystemSuspend := timestamps[3] - timestamps[2]
	secondSystemResume := timestamps[1] - timestamps[0]
