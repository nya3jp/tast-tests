...
func Logout(ctx context.Context, s *testing.State) {
	...
	go monitorSessionState(ctx, loggedOut)
	...
	case logoutClickOnButton:
		if err := logoutByClickOnButton(ctx, tconn); err != nil {
			s.Fatal("Failed to logout: ", err)
		}
	}
	...
	case _, ok := <-loggedOut:
		if !ok {
			s.Fatal("Failed for DUT to logged out")
		}
		s.Log("Logged out successfully")
	case <-ctx.Done():
		s.Fatal("Failed to wait for DUT logged out")
	}
...
func monitorSessionState(ctx context.Context, loggedOut chan bool) error {
	...
	select {
	case <-sw.Signals:
		testing.ContextLog(ctx, "SessionStateChanged: stopped")
	case <-ctx.Done():
