...
func checkPanels(ctx context.Context, res *ubertrayLayoutTestResources) error {
	panels := []string{
		"network",
		"Bluetooth",
		"notification",
	...
	for _, name := range panels {
		...
		if err := uiauto.Combine("check panel",
			searchPanelInQuickSettings(res, targetBtn),
...
func searchPanelInQuickSettings(res *ubertrayLayoutTestResources, targetBtn *nodewith.Finder) uiauto.Action {
