...
func MediaControls(ctx context.Context, s *testing.State) {
	...
	if err := verifyMediaControlsNotExists(ctx, resource); err != nil {
...
// verifyMediaControlsNotExists verifies media controls icon is not in the shelf and the quick settings panel.
func verifyMediaControlsNotExists(ctx context.Context, res *testResource) error {
