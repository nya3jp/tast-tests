...
func NewWilcoFixture(state bool) *wilcoDTCFixture {
...
type wilcoDTCFixture struct {
	...
	// testMode, if enabled, relaunches VM & supportd in test configuration.
	testMode bool
...
func (w *wilcoDTCFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	// Performing DTC policy enrollment.
	...
	if w.testMode {
		...
		if err := wilco.StartVM(ctx, &wilco.VMConfig{
			...
			TestDBusConfig: false, // test config.
		}); err != nil {
			s.Fatal("Failed to restart the Wilco DTC VM: ", err)
		}
	}
