...
func SludgeDTC(ctx context.Context, s *testing.State) {
	...
	if err := wilco.StartWilcoVM(startCtx, config); err != nil {
		s.Fatal("unable to start WilcoVM VM: ", err)
	}
	defer wilco.StopWilcoVM(cleanupCtx)

	if err := wilco.StartWilcoDtcSupportdDaemon(startCtx); err != nil {
		s.Fatal("unable to start wilco_dtc_suportd: ", err)
	}
	defer wilco.StopWilcoDtcSupportdDaemon(cleanupCtx)
