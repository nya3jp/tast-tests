...
type getProcDataParam struct {
	...
	// resultFile is the expected file relative to expectedPrefix
	expectedFile string
}
...
func APIGetProcData(ctx context.Context, s *testing.State) {
	...
	if param.expectedFile != "" {
		expectedFile := path.Join(param.expectedPrefix, param.expectedFile)

		if len(response.FileDump) != 1 {
			s.Log(response.String())
			s.Fatalf("Only expected %s as the result", expectedFile)
		}

		if response.FileDump[0].Path != expectedFile {
			s.Log(response.String())
			s.Fatalf("Expected %s, but got %s", expectedFile, response.FileDump[0].Path)
		}
	}
