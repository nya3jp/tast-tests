...
func SludgeGRPC(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		...
		if err := parseDPSLUtilMsg(out, &osRes); err != nil {
			s.Error("Unable to parse get OS version response: ", err)
		}
	...
	if err != nil {
		...
		if err := parseDPSLUtilMsg(out, &ecRes); err != nil {
			s.Error("Unable to parse EC telemetry response: ", err)
		}
