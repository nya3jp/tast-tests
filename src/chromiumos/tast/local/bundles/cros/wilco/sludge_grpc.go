...
func SludgeGRPC(ctx context.Context, s *testing.State) {
	...
	defer wvm.StopWilcoSupportDaemon(ctx)
	...
	if err != nil {
		s.Error("Unable to parse JSON body from EC Telemetry: ", err)
	}
	...
	if ecRes.Status != dtc.GetEcTelemetryResponse_STATUS_OK {
		s.Errorf("Unexpected EC telemetry response status %s, want GetEcTelemetryResponse_STATUS_OK", ecRes.Status)
	}
}
...
// command, and any error. See https://crrev.com/c/1767044 for a description of
// the diagnostics_dpsl_test_requester.
