...
func (c *WilcoService) SetRTC(ctx context.Context, ts *timestamp.Timestamp) (*empty.Empty, error) {
	...
	// arg := fmt.Sprintf(" --date '1/1/01 12:00:00'")//, time.Unix(ts.Seconds, int64(ts.Nanos)).UTC().Format(time.RFC3339))
	if out, err := testexec.CommandContext(ctx, "/sbin/hwclock", "--set", "--date", time.Unix(ts.Seconds, int64(ts.Nanos)).UTC().Format(time.RFC3339), "--localtime", "--rtc=/dev/rtc1", "--noadjfile").CombinedOutput(); err != nil {
		testing.ContextLog(ctx, "Failed: ", string(out))
