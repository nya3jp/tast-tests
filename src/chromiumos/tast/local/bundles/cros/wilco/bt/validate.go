...
// Powered returns an option that configures expected powered value
// of Bluetooth adapter for ValidateBluetoothData function.
func Powered(powered bool) option {
...
func ValidateBluetoothData(ctx context.Context, msg *dtcpb.HandleBluetoothDataChangedRequest, opts ...option) error {
	...
	if msgAdapter.CarrierStatus != want {
		return errors.Errorf("unexpected carrier status; got %s, want %s", msgAdapter.CarrierStatus, want)
	}
