...
func APIRoutine(ctx context.Context, s *testing.State) {
	...
	// checks the result against expectedStatus. Routine would be cancelled,
	// while.expectedStatus is specified as ROUTINE_STATUS_CANCELLED.
	executeRoutine := func(ctx context.Context,
		...
		// Some routines would not get back to service right away, so shortening
		// test time by cancelling them then check the result if as expected.
