...
// CallRunRoutine executes a RunRoutine call and does sanity checks on the result
func CallRunRoutine(ctx context.Context, request dtcpb.RunRoutineRequest, response *dtcpb.RunRoutineResponse) error {
	...
	if response.Status == dtcpb.DiagnosticRoutineStatus_ROUTINE_STATUS_FAILED_TO_START {
		return errors.Errorf("routine %s failed to start", request.Routine)
	}

	if response.Status == dtcpb.DiagnosticRoutineStatus_ROUTINE_STATUS_CANCELLING {
		return errors.Errorf("routine %s cancelling", request.Routine)
	}

	if response.Status == dtcpb.DiagnosticRoutineStatus_ROUTINE_STATUS_CANCELLED {
		return errors.Errorf("routine %s cancelled", request.Routine)
	}

	if response.Status == dtcpb.DiagnosticRoutineStatus_ROUTINE_STATUS_REMOVED {
		return errors.Errorf("routine %s removed", request.Routine)
	}
...
func WaitUntilRoutineChangesState(ctx context.Context, uuid int32,
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if response.Status == state {
			return errors.Errorf("current State is %s", response.Status)
		}
