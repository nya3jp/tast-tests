...
func isBluetoothEnabled(ctx context.Context) (bool, error) {
	...
	if v, err := dbusutil.Property(ctx, obj, btProperty); err != nil {
		return false, err
	} else if powered, ok := v.(bool); !ok {
...
func enableBluetooth(ctx context.Context, enable bool) error {
	...
	if err := dbusutil.SetProperty(ctx, obj, btProperty, enable); err != nil {
		return err
	}
	return nil
}
...
func APIHandleBluetoothDataChanged(ctx context.Context, s *testing.State) {
	btEnable, err := isBluetoothEnabled(ctx)
	if err != nil {
