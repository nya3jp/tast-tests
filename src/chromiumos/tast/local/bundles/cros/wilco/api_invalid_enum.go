...
func APIInvalidEnum(ctx context.Context, s *testing.State) {
...

	{
		...
		if response.Status != dtcpb.GetDriveSystemDataResponse_STATUS_ERROR_REQUEST_TYPE_UNKNOWN {
			s.Fatalf("Unexpected GetDriveSystemData response status; got %s, want STATUS_ERROR_REQUEST_TYPE_UNKNOWN", response.Status)
		}
	}

	{
		...
		if response.Status != dtcpb.DiagnosticRoutineStatus_ROUTINE_STATUS_FAILED_TO_START {
			s.Fatalf("Unexpected RunRoutine response status; got %s, want ROUTINE_STATUS_FAILED_TO_START", response.Status)
		}
	}

	{
		...
		if response.Status != dtcpb.DiagnosticRoutineStatus_ROUTINE_STATUS_ERROR {
			s.Fatalf("Unexpected GetRoutineUpdate response status; got %s, want ROUTINE_STATUS_ERROR", response.Status)
		}
	}

	{
		...
		if response.Status != dtcpb.PerformWebRequestResponse_STATUS_ERROR_REQUIRED_FIELD_MISSING {
			s.Fatalf("Unexpected GetRoutineUpdate response status; got %s, want STATUS_ERROR_REQUIRED_FIELD_MISSING", response.Status)
		}
	}
