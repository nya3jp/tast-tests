...
func APIGetVPDField(ctx context.Context, s *testing.State) {
	getVpdField := func(vpd_field string) (string, error) {
		content, err := ioutil.ReadFile(path.Join("/sys/firmware/vpd/ro/", vpd_field))
		if err == nil {
			return string(content), nil
		}
		content, err = ioutil.ReadFile(path.Join("/sys/firmware/vpd/rw/", vpd_field))
		if err == nil {
			return string(content), nil
		}
	...
	for _, tc := range []struct {
		...
		if err != nil {
			s.Logf("Unable to get %s VPD field: %v", tc.vpdField, err)
			continue
		}
