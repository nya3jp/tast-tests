...
func shouldRunTest(sn *iio.Sensor) bool {
	return sn.Name != iio.Light
}
...
func SensorIioserviceHard(ctx context.Context, s *testing.State) {
	...
	hasSensorsToTest := false
	for _, sn := range sensors {
		if shouldRunTest(sn) {
			hasSensorsToTest = true
			break
		}
	}
	...
	if err != nil {
		s.Fatal("Failed to start the tracing session: ", err)
	}
