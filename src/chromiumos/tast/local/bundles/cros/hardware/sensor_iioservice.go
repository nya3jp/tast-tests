...
func processLatency(latency, pattern, strOut string, sensor *iio.Sensor, pv *perf.Values, s *testing.State) error {
	...
	metricName := "SensorIioservice." + string(sensor.Name) + "." + string(sensor.Location) + ".latency." + latency
	pv.Set(perf.Metric{
		Name:      metricName,
		Unit:      "second",
...
func SensorIioservice(ctx context.Context, s *testing.State) {
	...
	for _, sn := range sensors {
		...
		for latency, pattern := range latencies {
			err := processLatency(latency, pattern, strOut, sn, pv, s)
			if err != nil {
