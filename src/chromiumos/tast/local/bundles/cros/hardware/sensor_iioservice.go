...
func SensorIioservice(ctx context.Context, s *testing.State) {
	...
	for _, sn := range sensors {
		...
		if err := util.ProcessCPUMetric(metrics.GetAndroidCpu(), "SensorIioservice", util.IioserviceProcessName, pv); err != nil {
			s.Fatal("Failed to process CPU metric: ", err)
		}
		if err := util.ProcessMemMetric(metrics.GetAndroidMem(), "SensorIioservice", util.IioserviceProcessName, pv); err != nil {
