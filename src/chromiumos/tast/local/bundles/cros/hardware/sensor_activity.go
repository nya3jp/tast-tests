...
func SensorActivity(ctx context.Context, s *testing.State) {
	...
	if strings.Contains(string(output), "enabled") {
		originSpoofEnable = 1
		originSpoofState, err = sensor.ReadAttr("in_proximity_raw")
		if err != nil {
			s.Fatal("Failed to read originSpoofState: ", err)
		}
	} else {
	...
	if originSpoofEnable == 0 {
		...
		// ectool motionsense spoof ActivitySenserID activity BodyDetectionID Enable originSpoofState
		_, err = testexec.CommandContext(ctx, "ectool", "motionsense", "spoof", id, "activity", "4", "1", originSpoofState).Output()
	}
	if err != nil {
