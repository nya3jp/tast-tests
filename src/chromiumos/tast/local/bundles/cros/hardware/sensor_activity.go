...
func SensorActivity(ctx context.Context, s *testing.State) {
	...
	if sensor == nil {
		s.Log("No activity sensor found")
		return
	}
	...
	err = testOnBodyDetection(ctx, sensor, reader, 1)
	if err != nil {
		s.Fatal("Failed to switch to on-body: ", err)
	}
	err = testOnBodyDetection(ctx, sensor, reader, 0)
	if err != nil {
		s.Fatal("Failed to switch to off-body: ", err)
	}

	// Recover spoofing state.
	// ectool motionsense spoof ActivitySenserID activity BodyDetectionID OriginSpoofEnable
	_, err = testexec.CommandContext(ctx, "ectool", "motionsense", "spoof", ID, "activity", "4", OriginSpoofEnable).Output()
	if err != nil {
