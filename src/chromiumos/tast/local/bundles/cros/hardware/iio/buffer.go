...
func (s *Sensor) parseChannel(fn string) (ChannelSpec, error) {
	...
	if matches == nil {
		return ret, errors.New("not channel type")
	}
	...
	if err != nil {
		return ret, errors.Wrap(err, "error reading channel type")
	}
	...
	if matches == nil {
		return ret, errors.Errorf("bad type decriptor %q", t)
	}
	...
	if err != nil {
		return ret, errors.Wrapf(err, "error parsing number %q", matches[3])
	}
	ret.RealBits = uint(val)

	val, err = strconv.Atoi(matches[4])
	if err != nil {
		return ret, errors.Wrapf(err, "error parsing number %q", matches[4])
	}
	...
	if err != nil {
		return ret, errors.Wrapf(err, "error parsing number %q", matches[5])
	}
	ret.Shift = uint(val)

	ix, err := s.ReadAttr("scan_elements/" + ret.Name + "_index")
	if err != nil {
		return ret, errors.Wrapf(err, "error reading channel %v index", ret.Name)
	}

	val, err = strconv.Atoi(ix)
	if err != nil {
		return ret, errors.Wrapf(err, "error parsing number %q", ix)
	}
	ret.Index = val
