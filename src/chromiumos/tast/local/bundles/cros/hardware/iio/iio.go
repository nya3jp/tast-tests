...
func GetSensors() ([]Sensor, error) {
	...
	for _, file := range files {
		sensor, err := parseSensor(file, iioPath)
		if err == nil {
...
func parseSensor(file os.FileInfo, iioPath string) (Sensor, error) {
	var sensor Sensor
...
func (sensor *Sensor) Reading() (SensorReading, error) {
	...
	rawReading := func(axis string) (float64, error) {
		r, err := ioutil.ReadFile(path.Join(sensorPath,
			strings.Join([]string{"in", r, axis, "raw"}, "_")))
		if err != nil {
			return 0, err
		}
