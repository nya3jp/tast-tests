...
func parseTrigger(devName string) (*Trigger, error) {
	...
	re := regexp.MustCompile(`^trigger[0-9]+$`)
	if !re.MatchString(devName) {
		return nil, errors.New("not a trigger")
	}

	trigger.Path = devName

	rawName, err := trigger.ReadAttr("name")
	if err != nil {
		return nil, errors.Wrap(err, "trigger has no name")
	}
	trigger.Name = rawName

	re = regexp.MustCompile("^(.*)trig.*$")
	rawType := re.FindStringSubmatch(rawName)
	if rawType == nil {
