...
// cpu metric.
func ProcessCPUMetric(ctx context.Context, cpuMetric *perfetto_proto.AndroidCpuMetric, metricNamePrefix, targetProcessName string, pv *perf.Values) error {
	for _, processInfo := range cpuMetric.GetProcessInfo() {
		if processInfo.GetName() == targetProcessName {
			metric := processInfo.GetMetrics()
			testing.ContextLog(ctx, "megacycles: ", metric.GetMcycles())
			testing.ContextLog(ctx, "runtime in nanosecond: ", metric.GetRuntimeNs())
			testing.ContextLog(ctx, "min_freq in kHz: ", metric.GetMinFreqKhz())
			testing.ContextLog(ctx, "max_freq in kHz: ", metric.GetMaxFreqKhz())
			testing.ContextLog(ctx, "avg_freq in kHz: ", metric.GetAvgFreqKhz())
...
// memory metric.
func ProcessMemMetric(ctx context.Context, memMetric *perfetto_proto.AndroidMemoryMetric, metricNamePrefix, targetProcessName string, pv *perf.Values) error {
	for _, processMetric := range memMetric.GetProcessMetrics() {
		if processMetric.GetProcessName() == targetProcessName {
			...
			testing.ContextLog(ctx, "anon_avg in rss: ", counters.GetAnonRss().GetAvg())
			testing.ContextLog(ctx, "file_avg in rss: ", counters.GetFileRss().GetAvg())
			testing.ContextLog(ctx, "swap_avg in rss: ", counters.GetSwap().GetAvg())
