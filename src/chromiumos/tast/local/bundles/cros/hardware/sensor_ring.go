		...
		func() {
			...
			for d := false; !d; {
				...
				case <-timeout:
					d = true
				case <-ctx.Done():
					s.Fatalf("Context closed while reading from sensor %v %v",
						sn.Sensor.Location, sn.Sensor.Name)
				}
			}
			end := boottime()
...
func validate(rs []*iio.SensorReading, start, end int64, sn *iio.Sensor, collectTime time.Duration, s *testing.State) {
