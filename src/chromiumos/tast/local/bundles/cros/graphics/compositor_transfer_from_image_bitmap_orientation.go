...
const delayToScreenshot = 1 * time.Second
...
func CompositorTransferFromImageBitmapOrientation(ctx context.Context, s *testing.State) {
	...
	screenshotCanvas := func(id string) (image.Image, error) {
		...
		// Wait for fullscreen transition to complete before taking screenshot.
	...
	for _, sample := range samples {
		...
		if resultColor == paddingColor {
			s.Errorf("Unexpected occurrence of padding color %v at (%d, %d)",
				paddingColor, sample.X, sample.Y)
		}
	}
}
