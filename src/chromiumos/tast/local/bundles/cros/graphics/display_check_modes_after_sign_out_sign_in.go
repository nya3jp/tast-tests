...
func DisplayCheckModesAfterSignOutSignIn(ctx context.Context, s *testing.State) {
	...
	supPorts, err := cham.GetSupportedPorts(ctx)
	if err != nil {
	...
	for _, port := range supPorts {
		...
		usePort, err := shouldUsePort(ctx, cham, port)
		if err != nil {
			s.Fatal("Failed to determine if plug should be used: ", err)
		}
		...
		s.Log("Port: ", port, " usePort: ", usePort)
