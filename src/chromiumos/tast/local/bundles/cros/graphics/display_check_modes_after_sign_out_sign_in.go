...
func DisplayCheckModesAfterSignOutSignIn(ctx context.Context, s *testing.State) {
	...
	chamPort := ""
	if chameleonPort != nil {
		chamPort = chameleonPort.Value()
	}
	connSpec := chamIP + ":" + chamPort
	...
	for _, port := range supPorts {
		isPhysPlug, err := ch.IsPhysicalPlugged(ctx, port)
		if err != nil {
			s.Fatal("Failed to check if port is physically plugged: ", err)
		}

		// Plug in for physically plugged ports
		if isPhysPlug {
		...
		s.Log("Port: ", port, " isPlug: ", isPlug, " isPhysPlug: ", isPhysPlug)
		if isPlug {
			...
			if res1[0] != res2[0] && res1[1] != res2[1] {
				s.Fatal("Failed as resolution has changed")
			}
			...
			if res2[0] != res3[0] && res2[1] != res3[1] {
				s.Error("Failed as resolution has changed")
			}
		}
	}
}
