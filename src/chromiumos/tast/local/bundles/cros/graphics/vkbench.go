...
// VKBench benchs the vulkan performance.
func VKBench(ctx context.Context, s *testing.State) {
	...
	args := []string{"--spirv_dir", vkbenchShaderPath}
	if testConfig.hasty {
	...
	if err != nil {
		s.Fatal("Failure during analyzing the result: ", err)
	}

	if !testConfig.hasty {
		if err := pv.Save(s.OutDir()); err != nil {
			s.Error("Faield to save perf data: ", err)
		}
	}
}
