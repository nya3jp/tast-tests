...
type resultSummary struct {
	passed  int // passed is the number of passed subtests
	failed  int // failed is the number of failed subtests
	skipped int // skipped is the number of skipped subtests
}
...
func IGT(ctx context.Context, s *testing.State) {
	...
	if err == nil || isExitError {
		...
		if isExitError {
			s.Errorf("Failed to run %s (%v): %s", exePath, exitError, summary)
		} else {
			s.Log(summary)
		}
	} else {
