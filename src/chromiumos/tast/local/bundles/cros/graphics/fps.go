...
func init() {
	testing.AddTest(&testing.Test{
		...
		HardwareDeps: hwdep.D(hwdep.InternalDisplay(),
			// TODO(b/172225622): broken on veyron.
...
func parseTrace(tracePath string, s *testing.State, i int64) ([]float64, error) {
	// Line format:
	// <proc> [000] d.h1 87154.652132: drm_vblank_event: crtc=0, seq=49720
	// TODO(b/172225622): Do we need to care about seq?
	re := regexp.MustCompile(`^.* ([0-9\.]+): drm_vblank_event: crtc=([0-9]+),.*$`)
	...
	for scanner.Scan() {
		...
		if matches := re.FindStringSubmatch(line); matches != nil {
			...
			crtc, err := strconv.ParseInt(matches[2], 10, 64)
			if err != nil {
...
func FPS(ctx context.Context, s *testing.State) {
	const (
		...
		trimPercent = 10
		// Accept up to 0.3 fps margin over the reference fps.
		// TODO(b/172225622): Restore to 0.2 after oak switches to 4.19
		margin = 0.3
	...
	for i, info := range infos {
		// Create map of modes with unique refresh rates.
		...
		for _, mode := range rates {
			if err := display.SetDisplayProperties(
				ctx, tconn, info.ID, display.DisplayProperties{DisplayMode: mode}); err != nil {
				s.Log("Failed to set display properties: ", err)
				continue
			}
			...
			// Acknowledge the configuration change or it will automatically revert after some time.
			if err := kb.Accel(ctx, "Enter"); err != nil {
				s.Fatal("Failed to send keyboard action to confirm display configuration: ", err)
			}
			...
			fpsData, err := parseTrace(outputPath, s, int64(i))
			if err != nil {
		...
		if err := kb.Accel(ctx, "Search+Alt+M"); err != nil {
			s.Fatal("Failed to send keybord action to move the active window between displays: ", err)
		}
	}
}
