	...
	for i, info := range infos {
		...
		for _, mode := range rates {
			...
			if err := kb.Accel(ctx, "Enter"); err != nil {
				s.Fatal("Failed to send keyboard action to confirm display "+
					"configuration: ", err)
			}
			...
			if mean > targetFPS+margin || mean < targetFPS-margin {
				s.Fatalf("Mean FPS %f out of expected range %f +/- %f", mean,
					targetFPS, margin)
			}

			// TODO(b/172225622): re-enable stddev check if we can find meaningful
			// bound.
			if stddev > maxStddev {
		...
		if err := kb.Accel(ctx, "Search+Alt+M"); err != nil {
			s.Fatal("Failed to send keybord action to move the active window between "+
				"displays: ", err)
		}
	}
}
