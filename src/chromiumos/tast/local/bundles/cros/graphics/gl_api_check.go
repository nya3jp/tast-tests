...
func extractEGLVersion(s *testing.State, matched [][]string) (major,
	...
	if err != nil {
		return 0, 0, errors.Errorf("could not convert extracted egl major regex value: %s , to float", matched[0][1])
	}
	eglVerMinor, err := strconv.Atoi(matched[0][2])
	if err != nil {
		return 0, 0, errors.Errorf("could not convert extracted egl minor regex value: %s , to float", matched[0][2])
	}
