...
func Idle(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Failed to start new chrome: ", err)
	}
	defer cr.Close(ctx)
...
// gemIdle check that we can get all gem objects to become idle (i.e. the i915_gem_active list or i915_gem_objects client/process gem object counts need to go to 0).
