...
func checkVulkan(ctx context.Context, cmd *testexec.Cmd, vulkanEnabled bool) (*version, error) {
	...
	infoFile := filepath.Join(testing.ContextOutDir(ctx), "vulkaninfo.log")
	...
	// Write file to vulkaninfo.html
	htmlFile := filepath.Join(testing.ContextOutDir(ctx), "vulkan.html")
