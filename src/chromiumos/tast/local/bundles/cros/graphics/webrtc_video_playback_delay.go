...
func WebRTCVideoPlaybackDelay(ctx context.Context, s *testing.State) {
	...
	for _, bucket := range histogramDiff.Buckets {
		bucketAverage := float64(bucket.Max + bucket.Min) / 2.0
		...
		sum += bucketAverage * float64(bucket.Count)
	}

	s.Logf("%s histogram: %v; average: %f", histogramName, laterHistogram.String(), sum / float64(numSamples))
