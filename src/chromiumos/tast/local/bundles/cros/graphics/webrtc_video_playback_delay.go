...
func WebRTCVideoPlaybackDelay(ctx context.Context, s *testing.State) {
	...
	testing.ContextLogf(shortCtx, "Initial %s histogram: %v", histogramName, initHistogram.Buckets)


	if err = conn.Eval(shortCtx, "start()", nil); err != nil {
		s.Fatalf("start failed %v", err)		
	}
	s.Log("start worked")
	if err = conn.WaitForExpr(shortCtx, "callButton.disabled == false"); err != nil {
  ...
  for _, bucket := range histogramDiff.Buckets {
	...
	// TODO(mcasas): consider using here Append() and passing the value of
	// (bucket.Max + bucket.Min) /2, bucket.Count times, per bucket in
	// histogramDiff.Buckets
	perfValues.Set(WebRTCVideoPlaybackDelayMetric, averageSum / float64(numSamples))
