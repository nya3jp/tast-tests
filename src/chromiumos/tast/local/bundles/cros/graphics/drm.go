...
func init() {
	allTestOpts := map[string]option{
		"atomic_test":        {command: "atomictest -a -t all", timeout: 5 * time.Minute, runDeps: []string{"display_backlight", "drm_atomic"}},
		"drm_cursor_test":    {command: "drm_cursor_test", timeout: 20 * time.Second, runDeps: []string{"display_backlight"}},
		"mmap_test":          {command: "mmap_test", timeout: 5 * time.Minute, runDeps: []string{"display_backlight"}},
		"null_platform_test": {command: "null_platform_test", timeout: 20 * time.Second, runDeps: []string{"display_backlight"}},
		"linear_bo_test":     {command: "linear_bo_test", timeout: 20 * time.Second, runDeps: []string{"display_backlight"}},
		"swrast_test":        {command: "swrast_test", timeout: 20 * time.Second, runDeps: []string{}},
		"vgem_test":          {command: "vgem_test", timeout: 20 * time.Second, runDeps: []string{"display_backlight"}},
		"vk_glow":            {command: "vk_glow", timeout: 20 * time.Second, runDeps: []string{"display_backlight", "vulkan"}},
	}
	...
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			ExtraSoftwareDeps: allTestOpts["atomic_test"].runDeps,
			ExtraAttr:         []string{"disabled"},
		}, {
			Name:              "drm_cursor_test",
			Val:               []option{allTestOpts["drm_cursor_test"]},
			ExtraSoftwareDeps: allTestOpts["drm_cursor_test"].runDeps,
			ExtraAttr:         []string{"disabled"},
		}, {
			Name:              "mmap_test",
			Val:               []option{allTestOpts["mmap_test"]},
			ExtraSoftwareDeps: allTestOpts["mmap_test"].runDeps,
			ExtraAttr:         []string{"disabled"},
		}, {
			Name:              "null_platform_test",
			Val:               []option{allTestOpts["null_platform_test"]},
			ExtraSoftwareDeps: allTestOpts["null_platform_test"].runDeps,
			ExtraAttr:         []string{"disabled"},
		}, {
			Name:              "linear_bo_test",
			Val:               []option{allTestOpts["linear_bo_test"]},
			ExtraSoftwareDeps: allTestOpts["linear_bo_test"].runDeps,
			ExtraAttr:         []string{"disabled"},
		}, {
			Name:              "swrast_test",
			Val:               []option{allTestOpts["swrast_test"]},
			ExtraSoftwareDeps: allTestOpts["swrast_test"].runDeps,
			ExtraAttr:         []string{"disabled"},
		}, {
			Name:              "vgem_test",
			Val:               []option{allTestOpts["vgem_test"]},
			ExtraSoftwareDeps: allTestOpts["vgem_test"].runDeps,
			ExtraAttr:         []string{"disabled"},
		}, {
			Name:              "vk_glow",
			Val:               []option{allTestOpts["vk_glow"]},
			ExtraSoftwareDeps: allTestOpts["vk_glow"].runDeps,
			ExtraAttr:         []string{"disabled"},
		}},
...
// checkRunAttr checks the runDeps config to decide should we run the test.
...
// runTest runs the exe binary test. Before running, SetUp() must be called.
