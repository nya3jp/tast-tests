...
func testGeneratedScreenshot(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatalf("Failed to open %v: %s", url, err)
		return
	}
	defer conn.Close()
	...
	if err := ew.Type(ctx, "f"); err != nil {
		s.Error("Failed to inject the 'f' key: ", err)
	}
	...
	testOneScreenshot := func(name string, key string) error {
		...
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			...
			if err := testexec.CommandContext(ctx, "perceptualdiff", "-verbose", reference, resized).Run(testexec.DumpLogOnError); err != nil {
				return errors.Wrapf(err, "Unexpected diff from reference for %s", name)
			}

			return nil
		}, &testing.PollOptions{Timeout: 5 * time.Second, Interval: time.Second}); err != nil {
			s.Error("Screenshot did not match: ", err)
			return err
		}
