...
func RunTest(ctx context.Context, s *testing.State, traceNameMap map[string]string) {
	...
	s.Logf("Logging container gfx environment to ", envFile)
	...
	for traceFile, traceName := range traceNameMap {
		...
		perfValue, err := runTrace(shortCtx, cont, s.DataPath(traceFile), outputFile)
		if err != nil {
...
func runTrace(ctx context.Context, cont *vm.Container, traceFile, outputFile string) (*perf.Values, error) {
	...
	perfValue, err := parseResult(filepath.Base(outputFile), string(traceOut))
	if err != nil {
		return nil, errors.Wrap(err, "failed to parse the result")
	}
	return perfValue, nil
}

// parseResult parses the output of apitrace and return the perfs.
func parseResult(testName string, output string) (*perf.Values, error) {
