...
//RunTest starts a VM and runs all traces in trace, which maps from filenames (passed to s.DataPath) to a human-readable name for the trace.
...
func runTrace(ctx context.Context, cont *vm.Container, traceFile, outputFile string) (*perf.Values, error) {
	...
	return parseResult(filepath.Base(outputFile), string(traceOut))
}
