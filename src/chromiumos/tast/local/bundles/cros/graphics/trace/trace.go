...
// decompressTrace trys to unzip the trace into trace format if possible. If the input is uncompressed, this function will do nothing.
...
func decompressTrace(ctx context.Context, cont *vm.Container, traceFile string) (string, error) {
	if filepath.Ext(traceFile) == ".bz2" {
		...
		if err != nil {
			return "", errors.Wrapf(err, "failed to decompress bz2 %f", output)
		}
