...
// be larger than twice the DEQP watchdog timeout at 30s. However, that's
// because in the original Autotest code, we have the option of using the hasty
// mode for running batches of tests with one command. Since we're running each
// test individually here, we don't have to set the deadline so large.
const deqpCmdTimeout = 40 * time.Second
...
func testNameToAPI(test string) (graphics.APIType, error) {
	...
	return graphics.UNKNOWN, errors.Errorf("%q is not a valid test name", test)
}
...
// "dEQP-GLES2.info.vendor". env lists the environment variables to set when
// running the test, e.g., "SHELL=/bin/bash". The test's log is written to a
// file named <test>.log within logDir.
//
// This function returns the outcome of the test determined by the result of
// parsing the test's log file. If an irrecoverable parsing error occurs,
...
func runSingleTest(ctx context.Context, s *testing.State, test string, env []string, logDir string) string {
	...
	if cmdCtx.Err() == context.DeadlineExceeded {
		...
		cmd.DumpLog(ctx)
		s.Fatal("Command timeout! Please check the deqpCmdTimeout setting")
	}
...
func logOutcomeMap(ctx context.Context, outcomes outcomeMap) {
	...
	testing.ContextLog(ctx, "=== DEQP OUTCOME SUMMARY:")
	for _, o := range sortedOutcomes {
		testing.ContextLog(ctx, strings.ToUpper(o), ":")
...
func DEQP(ctx context.Context, s *testing.State) {
	...
	for i, t := range deqpTests {
		...
		o := runSingleTest(ctx, s, t, env, logDir)
		s.Logf("Result for %s: %s", t, strings.ToUpper(o))
		outcomes[o] = append(outcomes[o], t)
	}
	logOutcomeMap(ctx, outcomes)
}
