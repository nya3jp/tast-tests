...
func mapGet(dataMap map[string]string, key string) bool {
...
func KernelConfig(ctx context.Context, s *testing.State) {
	...
	for _, configKey := range isBuiltin {
		if !mapGet(kernelConfigMap, configKey) || kernelConfigMap[configKey] != "y" {
	...
	for _, configKey := range isDisabled {
		if mapGet(kernelConfigMap, configKey) && kernelConfigMap[configKey] != "n" {
	...
	for _, configKey := range isModule {
		if !mapGet(kernelConfigMap, configKey) || kernelConfigMap[configKey] != "m" {
