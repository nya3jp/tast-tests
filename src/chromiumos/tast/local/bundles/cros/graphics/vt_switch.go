...
func VTSwitch(ctx context.Context, s *testing.State) {
	...
	if err := testing.Sleep(ctx, waitTime); err != nil {
		s.Fatal("Error while waiting for Chrome to start: ", err)
	}
	...
	s.Logf("percent_VT1_screenshot_max_difference: %d", maxVT1DifferencePercent)
	s.Logf("percent_VT2_screenshot_max_difference: %d", maxVT2DifferencePercent)
	s.Logf("num_identical_vt1_screenshots: %d", numIdenticalVT1Screenshots)
	s.Logf("num_identical_vt2_screenshots: %d", numIdenticalVT2Screenshots)

	if numErrors > 0 {
		s.Fatalf("Failed to switch between VT1 and VT2 %d time(s)", numErrors)
	}
}
