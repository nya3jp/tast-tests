...
func isPerceptualDiff(ctx context.Context, file1, file2 string) (bool, int, error) {
	...
	// If some error occured and it was not due to images being different.
	if (err != nil && len(matched) == 0) || len(string(stderr)) > 0 {

		return true, numPix, errors.Wrap(err, "error occured while running perceptual diff")
	}
...
func VTSwitch(ctx context.Context, s *testing.State) {
	...
	captureAndCompare := func(vt, id int, original string) {
		...
		// In this case the images are the same as no error occured and the isSame is true as well.
		if isSame && err == nil {
		...
		// In this perceptualdiff found the two images to be different as no error occured and isSame is false.
		if err == nil && !isSame {
