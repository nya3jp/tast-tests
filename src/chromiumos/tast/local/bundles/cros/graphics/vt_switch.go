...
// cmdTest opens a terminal and runs perceptualdiff between two images
func cmdTest(ctx context.Context, s *testing.State, file1, file2 string) bool {
	...
	testing.ContextLog(ctx, errs.String())
	cmd.Run()
	return (strings.Contains(out.String(), "PASS"))
}
...
func VTSwitch(ctx context.Context, s *testing.State) {
	...
	if cmdTest(ctx, s, vt1Screenshot, vt2Screenshot) {
		numErrors++
	...
	for i := 0; i < iterations; i++ {
		...
		if !cmdTest(ctx, s, vt1Screenshot, currentVT1Screenshot) {
		...
		if !cmdTest(ctx, s, vt2Screenshot, currentVT2Screenshot) {
