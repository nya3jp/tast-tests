...
func VTSwitch(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Error("Failed to decode file: ", err)
	}
	...
	var maxDifferenceArrPercent [2]int
	...
	captureAndCompare := func(vt, id int, original string) {
		...
		if ok != nil && numPix > 0 {
			...
			maxDifferenceArrPercent[vt-1] = max(maxDifferenceArrPercent[vt-1], pixelPercentage)
			return
		}
	...
	savePerf(maxDifferenceArrPercent[0], "percent_VT1_screenshot_max_difference", "percent", pv)
	savePerf(maxDifferenceArrPercent[1], "percent_VT2_screenshot_max_difference", "percent", pv)
