...
func VTSwitch(ctx context.Context, s *testing.State) {
	...
	captureAndCompare := func(vt, id int, original string) {
		...
		// In this case the images are the same as no error occurred and the isSame is true as well.
		if isSame && err == nil {
		...
		// In this perceptualdiff found the two images to be different as no error occurred and isSame is false.
		if err == nil && !isSame {
