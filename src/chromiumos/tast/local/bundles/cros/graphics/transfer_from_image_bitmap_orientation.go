...
// TransferFromImageBitmapOrientation draws to offscreen canvases and transfers their contents to
// bitmap renderers, and compares the results against reference canvases to validate that the
// results are oriented correctly.
func TransferFromImageBitmapOrientation(ctx context.Context, s *testing.State) {
	...
	for _, scale := range samples {
		...
		if expectedColor != resultColor {
			s.Errorf("Mismatched colors at (%d, %d): Expected %v, Actual %v", x, y,
				expectedColor, resultColor)
		}
	}
