...
func expectWifiEnabled(ctx context.Context, expectEnabled bool) error {
	...
	for {
		...
		technologies, err := prop.GetStrings(shill.ManagerPropertyEnabledTechnologies)
		if err != nil {
		...
		if _, err := watcher.WaitAll(ctx, shill.ManagerPropertyEnabledTechnologies); err != nil {
