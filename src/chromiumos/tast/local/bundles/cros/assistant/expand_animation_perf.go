...
func init() {
	testing.AddTest(&testing.Test{
		Func:         ExpandAnimationPerf,
		Desc:         "Measures the smoothness of expanding the assistant from the search bar",
...
// ExpandAnimationPerf measures the animation performance of
// showing and hiding the assistant while the launcher is open.
func ExpandAnimationPerf(ctx context.Context, s *testing.State) {
	...
	for nWindows := 0; nWindows < 3; nWindows++ {
		...
		processHistogram(histograms, pv, nWindows)
	}
...
func processHistogram(histograms []*metrics.Histogram, pv *perf.Values, nWindows int) error {
...
func toggleLauncher(ctx context.Context, tconn *chrome.TestConn, expectedState ash.LauncherState) error {
...
func showAndHideAssistant(ctx context.Context, tconn *chrome.TestConn, keyboard *input.KeyboardEventWriter) error {
	...
	if err := keyboard.Accel(ctx, "escape"); err != nil {
