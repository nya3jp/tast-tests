...
func recordAssistantToggleSmoothness(ctx context.Context, tconn *chrome.TestConn, pv *perf.Values, postfix string) error {
	...
	if err != nil {
		return errors.Wrap(err, "failed to collect open histogram")
	}

	if err := recordPerfData(histograms, pv, fmt.Sprintf("%s.open", postfix)); err != nil {
		return errors.Wrap(err, "failed to record perf data for open")
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to collect close histogram")
	}

	if err := recordPerfData(histograms, pv, fmt.Sprintf("%s.close", postfix)); err != nil {
		return errors.Wrap(err, "failed to record perf data for close")
	}

	return nil
}

func recordPerfData(histograms []*metrics.Histogram, pv *perf.Values, postfix string) error {
