...
func DiffLib(ctx context.Context, s *testing.State) {
	...
	if err := d.Diff("nomatches", nodewith.ClassName("MissingClassName"))(ctx); err == nil {
		s.Fatal("Expected no matches")
	}
	if err := d.Diff("multiplematches", nodewith.ClassName("FrameCaptionButton"))(ctx); err == nil {
	...
	screenshot.DiffPerConfigOrDie(ctx, s, []screenshot.Config{
		...
		ui := uiauto.New(tconn).WithTimeout(time.Second * 3)
