...
func takeScreenshots(ctx context.Context, d screenshot.Differ) error {
	...
	if err := uiauto.Combine("take screenshots of files app",
		...
		d.DiffWindow("filesApp"))(ctx); err != nil {
		return err
	}
...
func ScreenDiff(ctx context.Context, s *testing.State) {
	...
	if err := expectError(
		d.DiffWithOptions("nowindowopen", nodewith.ClassName("FrameCaptionButton"), screenshot.Options{Timeout: 500 * time.Millisecond})(ctx),
