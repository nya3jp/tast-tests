...
func ScreenDiff(ctx context.Context, s *testing.State) {
	...
	defer faillog.DumpUITreeOnError(ctx, s.OutDir(), s.HasError, d.Tconn())
	...
	if err := ash.SetTabletModeEnabled(ctx, d.Tconn(), true); err != nil {
		s.Fatal("Failed to enable tablet mode: ", err)
	}
	defer func() {
		if err := ash.SetTabletModeEnabled(ctx, d.Tconn(), false); err != nil {
			s.Log("Failed to revert to clamshell mode: ", err)
		}
	}()
