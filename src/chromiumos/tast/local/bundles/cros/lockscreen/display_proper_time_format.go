...
func DisplayProperTimeFormat(ctx context.Context, s *testing.State) {
	...
	ui := uiauto.New(tconn)
	if err := uiauto.Combine(fmt.Sprintf("toggle setting: Use 24-hour clock"),
		ui.WaitUntilExists(nodewith.Name("Use 24-hour clock").Role(role.ToggleButton)),
		osSettings.SetToggleOption(cr, "Use 24-hour clock", use24HourFormat),
	)(ctx); err != nil {
	...
	s.Log(info.Name)
	if is24HourFormat(info.Name) != use24HourFormat {
