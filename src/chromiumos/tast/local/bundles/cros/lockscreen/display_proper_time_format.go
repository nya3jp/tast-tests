	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			const (
				...
				password             = "good"
				dateTImeSettingPage  = "dateTime"
				timeFormatToggleName = "Use 24-hour clock"
			)
			...
			ui := uiauto.New(tconn)
			osSettings, err := ossettings.LaunchAtPageURL(ctx, tconn, cr, dateTImeSettingPage, ui.Exists(nodewith.Name(timeFormatToggleName).Role(role.ToggleButton)))
			if err != nil {
				s.Fatal("Failed to open Date and Time page: ", err)
			}
			defer osSettings.Close(cleanupCtx)
			defer faillog.DumpUITreeWithScreenshotOnError(cleanupCtx, s.OutDir(), s.HasError, cr, "ui_dump")

			// Toggle time format setting.
			if err := uiauto.Combine(fmt.Sprintf("toggle setting: %s", timeFormatToggleName),
				ui.WaitUntilExists(nodewith.Name(timeFormatToggleName).Role(role.ToggleButton)),
				osSettings.SetToggleOption(cr, timeFormatToggleName, param.use24HourFormat),
			)(ctx); err != nil {
