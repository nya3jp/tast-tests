...
func BluetoothToggleUsingMojo(ctx context.Context, s *testing.State) {
	...
	if err := crconn.Call(ctx, &bluetoothMojo, BTConfigJS); err != nil {
		s.Fatal(errors.Wrap(err, "failed to create Bluetooth mojo JS"))
	}

	if err := bluetoothMojo.Call(ctx, nil, `function dummy(){ this.initSysPropObs()}`); err != nil {
		s.Fatal(errors.Wrap(err, "failed to initailize the observer"))
	}

	const iterations = 5
	state := false
	var exp BluetoothSystemState = KDisabled

	for i := 0; i < iterations; i++ {
		s.Logf("(iteration %d of %d)", i+1, iterations)
		s.Logf("Toggling Bluetooth state to %t", state)

		if err := SetBluetoothEnabledState(ctx, s, bluetoothMojo, state); err != nil {
		...
		if err = PollForBluetoothSystemState(ctx, s, bluetoothMojo, exp); err != nil {
			s.Fatal("Failed to get SystemProperties: ", err)

		}

		//		s.Logf("system state is %d", sysprop.SystemState)
		//		s.Logf("system properities is %+v\n", sysprop)
