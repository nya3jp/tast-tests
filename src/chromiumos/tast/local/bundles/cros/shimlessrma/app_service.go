...
// GetTextByPrefix returns the text with prefix.
func (shimlessRMA *AppService) GetTextByPrefix(ctx context.Context,
	req *pb.GetTextByPrefixRequest) (*pb.GetTextByPrefixResponse, error) {
	node, err := shimlessRMA.app.GetTextByPrefix(ctx, req.Prefix)
	if err != nil {
		return nil, errors.Wrapf(err, "failed to find info with prefix: %s", req.Prefix)
	}

	return &pb.GetTextByPrefixResponse{Value: node.Name}, nil
}
