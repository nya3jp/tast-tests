...
// checkSimulcastEncImpl checks that the encoders used for simulcast is expected.
...
func checkSimulcastEncImpl(implName string, isImplHWInAdapter []bool) error {
	...
	// The implementation name is like SinulcastEncoderAdapter(libvpx, VaapiVideoEncodeAccelerator, VaapiVideoEncodeAccelerator).
	...
	if len(implNames) != isImplHWInAdapter {
		return errors.Errorf("the number of simulcast streams mismatches: got (%s), expected %d", inStr, len(isImplHWInAdapter))
	}
