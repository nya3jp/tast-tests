...
const (
	// streamWidth and streamHeight utilized around this test.
	streamWidth  = 1280
	streamHeight = 720

	// Before taking any measurements, we need to wait for the RTCPeerConnection
	// to ramp up the CPU adaptation; until then, the transmitted resolution is
...
func waitForPeerConnectionStabilized(ctx context.Context, conn *chrome.Conn, parseTxStatsJS string) error {
	...
	for i := 0; i < maxStreamWarmUpSeconds; i++ {
	...
	if txMeasurement.FrameHeight < streamHeight || txMeasurement.FrameWidth < streamWidth {
		return errors.Errorf("Timeout waiting for tx resolution to reach %dx%d, last: %.0fx%.0f", streamWidth, streamHeight,
