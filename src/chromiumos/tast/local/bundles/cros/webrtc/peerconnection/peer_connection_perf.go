...
func measureGPUCounters(ctx context.Context, prefix string, p *perf.Values) error {
	...
	if err != nil {
		return errors.Wrap(err, "error correcting graphics performance counters")
	}
	if counters["total"].Milliseconds() == 0 {
		return errors.New("total elapsed time counter is zero")
	}

	if counters["rcs"].Seconds() != 0 {
		rcsUsage := 100 * counters["rcs"].Seconds() / counters["total"].Seconds()
	...
	if counters["vcs"].Seconds() != 0 {
		vcsUsage := 100 * counters["vcs"].Seconds() / counters["total"].Seconds()
	...
	if counters["vecs"].Seconds() != 0 {
		vecsUsage := 100 * counters["vecs"].Seconds() / counters["total"].Seconds()
