...
// RunGetUserMedia run a test in /webrtc/data/getusermedia.html.
...
func RunGetUserMedia(ctx context.Context, s *testing.State, cr *chrome.Chrome,
	duration time.Duration, verbose utils.VerboseLoggingMode) CameraResults {
	if verbose == utils.VerboseLogging {
...
func RunPeerConn(ctx context.Context, s *testing.State, cr *chrome.Chrome,
	codec videotype.Codec, duration time.Duration, verbose utils.VerboseLoggingMode) PeerConnResult {
	if verbose == utils.VerboseLogging {
		vl, err := logging.NewVideoLogger()
		if err != nil {
			s.Fatal("Failed to set values for verbose logging")
