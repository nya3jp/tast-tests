...
// Package camera provides common code for webrtc.* tests related to camera.
package camera
...
import (
	...
	"chromiumos/tast/local/bundles/cros/webrtc/lib/rtcchrome"
...
// runTest checks if the given camera tests work correctly.
...
// CameraResults is a type for decoding JSON objects obtained from /webrtc/data/getusermedia.html.
...
// RunGetUserMedia runs a test in /webrtc/data/getusermedia.html.
...
func RunGetUserMedia(ctx context.Context, s *testing.State, cr *chrome.Chrome,
	duration time.Duration, verbose rtcchrome.VerboseLoggingMode) CameraResults {
	if verbose == rtcchrome.VerboseLogging {
...
// peerConnStats is a struct used in PeerConnResult for FPS data.
...
// setPerf stores performance data of peerConnStats into p.
// suffix is a string that will be used as a sufix in metric names.
func (s *peerConnStats) setPerf(p *perf.Values, suffix string) {
...
// PeerConnResult is a struct for decoding JSON objects obtained from /camera/data/loopback_camera.html.
type PeerConnResult struct {
	CameraType    string        `json:"cameraType"`
	PeerConnStats peerConnStats `json:"peerConnStats"`
	FrameStats    frameStats    `json:"frameStats"`
	Errors        []string      `json:"errors"`
}

// SetPerf stores performance data of PeerConnResult into p.
// codec is a video codec exercised in testing.
func (r *PeerConnResult) SetPerf(p *perf.Values, codec videotype.Codec) {
	r.FrameStats.setPerf(p, string(codec))
	r.PeerConnStats.setPerf(p, string(codec))
}

// RunPeerConn run a test in /webrtc/data/loopback_camera.html.
// codec is a video codec to exercise in testing.
// duration specifies how long video capturing will run for each resolution.
// If verbose is true, video drivers' verbose messages will be enabled.
// verbose must be false for performance tests.
func RunPeerConn(ctx context.Context, s *testing.State, cr *chrome.Chrome,
	codec videotype.Codec, duration time.Duration, verbose rtcchrome.VerboseLoggingMode) PeerConnResult {
	if verbose == rtcchrome.VerboseLogging {
		vl, err := logging.NewVideoLogger()
		if err != nil {
			s.Fatal("Failed to set values for verbose logging")
