...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			Val:               rtcPerfTest{enableHWAccel: true, profile: "VP9", svc: "L1T2"},
			ExtraSoftwareDeps: []string{caps.HWEncodeVP9},
			...
			Val:               rtcPerfTest{enableHWAccel: true, profile: "VP9", svc: "L1T3"},
			ExtraSoftwareDeps: []string{caps.HWEncodeVP9},
			Fixture:           "chromeVideoWithFakeWebcamAndSVCEnabled",
		}, {
...
func RTCPeerConnectionPerf(ctx context.Context, s *testing.State) {
	...
	if err := peerconnection.RunDecodePerf(ctx, s.FixtValue().(*chrome.Chrome), s.DataFileSystem(), s.OutDir(), testOpt.profile, testOpt.enableHWAccel, testOpt.videoGridDimension, testOpt.videoGridFile, testOpt.svc); err != nil {
