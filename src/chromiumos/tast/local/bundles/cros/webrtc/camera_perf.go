...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:         append(common.DataFiles(), "getusermedia.html"),
	})
}

// CameraPerf is the full version of WebRTC. It renders the camera's media
...
func CameraPerf(ctx context.Context, s *testing.State) {
	// Run tests for 20 seconds per resolution.
	results := common.RunWebRTC(ctx, s, s.PreValue().(*chrome.Chrome), 20*time.Second,
		common.NoVerboseLogging)
