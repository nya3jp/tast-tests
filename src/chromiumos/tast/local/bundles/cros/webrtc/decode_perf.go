...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:         append(rtcchrome.LoopbackDataFiles, "crowd720_25frames.y4m", rtcchrome.AddStatsJSFile),
...
func DecodePerf(ctx context.Context, s *testing.State) {
	addStatsJS, err := ioutil.ReadFile(s.DataPath(rtcchrome.AddStatsJSFile))
	if err != nil {
