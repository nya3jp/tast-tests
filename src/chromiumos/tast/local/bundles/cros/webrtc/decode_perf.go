...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:         append(common.LoopbackDataFiles(), "crowd720_25frames.y4m", common.AddStatsJSFile),
...
func DecodePerf(ctx context.Context, s *testing.State) {
	addStatsJS, err := ioutil.ReadFile(s.DataPath(common.AddStatsJSFile))
	if err != nil {
		s.Fatal("Failed to read JS for gathering decode time: ", err)
	}
	common.RunWebRTCDecodePerf(ctx, s, "crowd720_25frames.y4m", common.MeasureConfig{
