...
// 'bla' is the SDP id for |profile| and 'foo' is the |targetBitrate| in Kbps;
// this statement is needed to prevent RTCPeerConnections from dropping
// resolution to keep a by-default low start bitrate.
function appendStartBitrateToSDP(sdp, profile, targetBitrate) {
  const codec_id = findRtpmapId(splitSdpLines(sdp), profile);
  if (codec_id) {
    const targetBitrateKbpsAsInt = Math.trunc(targetBitrate / 1000);
    sdp += `a=fmtp:${codec_id} ` +
