...
func checkForCodecImplementation(ctx context.Context, s *testing.State, conn *chrome.Conn, codecType CodecType) error {
	// Let the RTCPeerConnections roll for a second to allow for the stats to be updated.
	...
	// "unknown") means it is using software encoding.
	...
	if implementation != expectedImplementation {
		return errors.Errorf("expected implementation to be %s and got %s", expectedImplementation, implementation)
	}
	return nil
}
