...
type fioSettings struct {
	// |fileSize|, |blockSize|: fio size description, e.g., "1G" or "4m"
	// |runTime|: fio time description, e.g., "10s", "1m"
	fileSize, blockSize, runTime string
}
...
func runFIOJob(ctx context.Context, guestEnv, hostEnv runEnv, job fioJob, perfValues perf.Values) {
	...
	for _, settings := range []fioSettings{
		...
		for i := 1; i <= numTries; i++ {
			...
			if err != nil {
				testing.ContextLogf(ctx, "%v with block size %v failed: %v", job.name, settings.blockSize, err)
				continue
			}
