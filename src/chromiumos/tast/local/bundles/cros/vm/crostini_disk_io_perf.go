...
type fioSettings struct {
	fileSize, blockSize string // fio size description, e.g. "1G" or "4m"
	loopNum             int
}
...
func reportMetric(ctx context.Context, metricName string, guestValue, hostValue float64, perfValues perf.Values) {
	...
	testing.ContextLogf(ctx, "Reporting metric %.2f: %.2f %.2f %.2f", metricName, guestValue, hostValue, ratio)
...
func runFIOJob(ctx context.Context, guestEnv, hostEnv runEnv, job fioJob, perfValues perf.Values) {
	...
	for _, setting := range []fioSettings{
		...
		for i := 1; i <= numTries; i++ {
			...
			guestRead, guestWrite, err := runFIO(ctx, guestEnv, job.fileName, setting)
			if err != nil {
			...
			hostRead, hostWrite, err := runFIO(ctx, hostEnv, job.fileName, setting)
			if err != nil {
				testing.ContextLogf(ctx, "%v with block size %v failed: %v", job.name, setting.blockSize, err)
				continue
			}
