...
type hostEnv struct {
	// Passed in a filename, returns the complete data path.
	dataPathFunc func(fileName string) (absPath string)
}
...
func runFIO(ctx context.Context, re runEnv, jobFile string, setting fioSettings) (avgRead, avgWrite float64, err error) {
	envArgs := []string{
	...
	if err != nil {
		...
		firstNewLineIdx := bytes.IndexByte(out, byte('\n'))
		if firstNewLineIdx != -1 {
			out = out[:firstNewLineIdx]
		}
...
func runFIOJob(ctx context.Context, guestRE, hostRE runEnv, job fioJob, perfValues perf.Values) {
	...
	for _, setting := range []fioSettings{
		{"300m", "4k", 1},
		{"500m", "64k", 1},
		{"500m", "1m", 2},
		{"500m", "16m", 2},
	} {
		baseMetricName := fmt.Sprintf("%v_bs_%v", job.name, setting.blockSize)

		const numTries = 3

		for i := 1; i <= numTries; i++ {
			testing.ContextLogf(ctx, "Running %v with block size %v in container (%v/%v)", job.name, setting.blockSize, i, numTries)
			guestRead, guestWrite, err := runFIO(ctx, guestRE, job.fileName, setting)
			if err != nil {
				testing.ContextLogf(ctx, "%v with block size %v failed: %v", job.name, setting.blockSize, err)
				continue
			}

			testing.ContextLogf(ctx, "Running %v with bs %v on host (%v/%v)", job.name, setting.blockSize, i, numTries)
			hostRead, hostWrite, err := runFIO(ctx, hostRE, job.fileName, setting)
			if err != nil {
				testing.ContextLogf(ctx, "%v with block size %v failed: %v", job.name, setting.blockSize, err)
				continue
			}
...
// CrostiniDiskIOPerf Run disk IO performance test by running the tool "fio".
func CrostiniDiskIOPerf(ctx context.Context, s *testing.State) {
	...
	for _, job := range fioJobs {
		...
		if err := cont.PushFile(ctx, s.DataPath(job.fileName), containerJobFilePath); err != nil {
			s.Errorf("Failed to push %v to container: %v", job.fileName, err)
		}
	}

	guestRE := guestEnv{cont}
	hostRE := hostEnv{s.DataPath}
	perfValues := perf.Values{}
	for _, job := range fioJobs {
		runFIOJob(ctx, guestRE, hostRE, job, perfValues)
	}
