...
type runEnv struct {
	...
	fioCmd func(ctx context.Context, jobFile string, envArgs []string, cmdArgs []string) *testexec.Cmd
}
...
func runFIOJob(ctx context.Context, guestRE, hostRE runEnv, job fioJob, perfValues perf.Values) {
	...
	defer func() {
		hostDataFile := hostRE.testDataPath
...
func CrostiniDiskIOPerf(ctx context.Context, s *testing.State) {
	...
	for _, job := range fioJobs {
		runFIOJob(ctx, guestEnv, hostEnv, job, perfValues)
	}
