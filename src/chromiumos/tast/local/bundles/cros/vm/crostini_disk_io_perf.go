...
func runJobFile(ctx context.Context, guestRE, hostRE runEnv, jobFile string, reportFlag reportFlags, perfValues perf.Values) {
	...
	for _, setting := range []fioSettings{
		...
		for i := 1; i <= numTries; i++ {
			testing.ContextLogf(ctx, "Running %v with block size %v in container (%v/%v)", jobFile, setting.blockSize, i, numTries)
			guestRead, guestWrite, err := runFIO(ctx, guestRE, jobFile, setting)
			if err != nil {
				testing.ContextLogf(ctx, "%v with block size %v failed: %v", jobFile, setting.blockSize, err)
				continue
			}
...
func CrostiniDiskIOPerf(ctx context.Context, s *testing.State) {
	...
	for _, jobFile := range jobFiles {
		...
		if err := cont.PushFile(ctx, s.DataPath(jobFile.name), containerJobFilePath); err != nil {
			s.Errorf("Failed to push %v to container: %v", jobFile.name, err)
		}
	}
