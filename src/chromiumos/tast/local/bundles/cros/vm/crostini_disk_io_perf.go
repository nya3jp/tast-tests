...
type errorLogWriter func(title string, content string) error
...
func runFIO(ctx context.Context, re runEnv, jobFile string, settings fioSettings, logWriter errorLogWriter) (avgRead, avgWrite float64, err error) {
	...
	if err != nil {
		cmd.DumpLog(ctx)
		if err := logWriter("Run fio failure", string(out)); err != nil {
	...
	if err := json.Unmarshal(out, &result); err != nil {
		if err := logWriter("Parse fio failure", string(out)); err != nil {
...
func CrostiniDiskIOPerf(ctx context.Context, s *testing.State) {
	...
	defer errFile.Close()
	logWriter := func(title string, content string) error {
