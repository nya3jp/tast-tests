...
func CrostiniInputLatency(ctx context.Context, s *testing.State) {
	...
	s.Logf("%v out of %v pings are successful", numSucPings, numPings)
	if float64(numSucPings) < 0.9*float64(numPings) {
		s.Errorf("Too much failed pings (%v/%v)", numPings-numSucPings, numPings)
	}
	if numSucPings == 0 {
		s.Fatal("All pings are failed. Abort testing")
	}
	avgRTT := time.Duration(int64(sumRTT) / int64(numSucPings))
	...
	for i := 0; i < numTries; i++ {
		...
		s.Logf("Key input latency (%v/%v): %v", i+1, numTries, latency.Round(time.Microsecond))
	...
	s.Logf("%v out of %v keyboard latency measurements are successful", numSucTries, numTries)
	if float64(numSucTries) < 0.9*float64(numTries) {
		s.Errorf("Too much failed keyboard latency measurements (%v/%v)", numTries-numSucTries, numTries)
	}
	if numSucTries != 0 {
		avgKeyLatency := time.Duration(int64(sumKeyLatency) / int64(numSucTries))
...
func startInputLatencyServer(ctx context.Context, cont *vm.Container, socketServerFile, socketServerLog fileMapping,
	...
	cleanup = func() error {
		if err := socketServerCmd.Wait(); err != nil {
			return errors.Wrap(err, "socket server exits with error")
		}
