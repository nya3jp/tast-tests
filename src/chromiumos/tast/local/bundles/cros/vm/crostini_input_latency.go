...
func CrostiniInputLatency(ctx context.Context, s *testing.State) {
	...
	// cleanupFunc wraps a cleanup function and logs the error it throws.
	...
	defer runCleanup(evtestCleanup, "Failed to start evtest: ")
	if err != nil {
	...
	defer runCleanup(serverCleanup, "Input latency server failed: ")
	if err != nil {
	...
	defer runCleanup(socketListenerCleanup, "Socket listener failed: ")
	if err != nil {
...
// a channel. The cleanupFunc returned, if not nil, must be run unconditionally by the caller.
