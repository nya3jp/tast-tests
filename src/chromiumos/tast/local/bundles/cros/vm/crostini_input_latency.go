...
func getTimeFromTimestamp(s string) (t time.Time, err error) {
	...
	if err != nil {
		return time.Time{}, errors.Wrapf(err, "failed to parse %q", s)
	}
	sec, nsec := math.Modf(ts)
	return time.Unix(int64(sec), int64(nsec*float64(time.Second))), nil
}
...
// Instead, we first try to calibrate the clock by sending pings to a socket server in container.
// The socket server always returns the local (guest) timestamp. After getting the timestamp, we
// estimate the clock difference between host and guest by taking socket RTT time into
// consideration.
// After that, we emulate a key press and let the socket server returns key received time to host.
// The host can then calculate input latency using a calibrated key received time.
func CrostiniInputLatency(ctx context.Context, s *testing.State) {
	...
	// We need to make sure the evtest gorutine starts before sending a key, otherwise the key
	// event may be missed.
	...
	// Need some time for the server to be ready, otherwise would get a "connection refused" error
	...
	err = testing.Poll(ctx, func(ctx context.Context) error {
		var err error
		conn, err = net.Dial("tcp", containerIP+":"+socketServerPort)
		return err
	}, &testing.PollOptions{Timeout: 3 * time.Second})
	if err != nil {
