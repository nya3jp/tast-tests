...
func init() {
	testing.AddTest(&testing.Test{
		Func:         CrostiniGfxTrace,
		Desc:         "Tests Termina VM graphics performance with virgl enabled",
		Contacts:     []string{"chromeos-gfx@google.com"},
		Attr:         []string{"informational"},
		Data:         []string{"crostini_gfx_trace_glxgears.trace"},
		Timeout:      10 * time.Minute,
		SoftwareDeps: []string{"chrome_login", "vm_hosts"},
	})
}

func CrostiniGfxTrace(ctx context.Context, s *testing.State) {
	...
	const traceFileName = "crostini_gfx_trace_glxgears.trace"
