...
func parseIPV4(ips []byte) (string, error) {
	for _, v := range bytes.Split(bytes.TrimSpace(ips), []byte{' '}) {
	...
	for _, line := range bytes.Split(bytes.TrimSpace(text), []byte{'\n'}) {
		...
		if matched != nil {
			...
			if err != nil {
				...
				return nil, math.NaN(), errors.Errorf("failed to parse time %q in ping output", matched[1])
			}
		...
		if matched != nil {
			...
			if err != nil {
				return nil, math.NaN(), errors.Errorf("failed to parse num packets transmitted %q", matched[1])
			}
			received, err := strconv.Atoi(string(matched[2]))
			if err != nil {
				return nil, math.NaN(), errors.Errorf("failed to parse num packets received %q", matched[2])
			}
			if all != 0 {
...
func (dir direction) metricName(name string) string {
	var prefix string
...
func CrostiniNetworkPerf(ctx context.Context, s *testing.State) {
	...
	// TODO(cylee): remove the installing code once CL:1389056 is submitted.
	// Install needed packages.
	packages := []string{
		"iperf3",
		"iputils-ping",
	}
	s.Log("Installing ", strings.Join(packages, ","))
	installCmdArgs := append([]string{"sudo", "apt-get", "-y", "install"}, packages...)
	if _, err := runCmd(cont.Command(ctx, installCmdArgs...)); err != nil {
		s.Fatalf("Failed to install needed packages %v: %v", strings.Join(packages, ","), err)
	}
	...
	var measurePing = func(dir direction) error {
		pingArgs := []string{
			"ping",
		...
		s.Logf("rtts: %v, loss rate: %.2f", rtts, lossRate)
		perfValues.Append(
			perf.Metric{
	...
	measureBandwidthFunc := func(dir direction) func(context.Context) error {
