...
func CrostiniNetworkPerf(ctx context.Context, s *testing.State) {
	...
	runCmd := func(cmd *testexec.Cmd) (out []byte, err error) {
		...
		if err != nil {
			...
			// Ouptut complete stdout to a log file.
			if e := writeError(cmdString, out); e != nil {
				s.Log("Failed to write cmd running error to log file: ", e)
			}
	...
	type direction int
	const (
		hostToContainer = 1 << iota
		containerToHost
	)
	measureBandwidth := func(dir direction) (result iperfMetrics) {
		// -J for json output.
		// -c for running an iperf3 client instead of server.
		args := []string{"-J", "-c", containerIP}
		...
		if err = json.Unmarshal(out, &result); err != nil {
			if e := writeError("parsing iperf3 result", out); e != nil {
				s.Log("Failed to write cmd running error to log file: ", e)
			}
			s.Error("Failed to parse iperf3 output: ", err)
		}
		s.Logf("Finished in %.2f seconds, bits per seconds %v",
			result.End.SumSent.Seconds, result.End.SumSent.BitsPerSecond)
		return result
	}
