...
func testBackupRestore(ctx context.Context, s *testing.State, tconn *chrome.Conn, ownerID string) {
	...
	// Delete a lot of other files to make the image size smaller.
	// This makes a big speed difference on slow devices.
	for _, path := range []string{
		...
		cmd := vm.DefaultContainerCommand(ctx, ownerID, "sudo", "rm", "-rf", path)
		if err := cmd.Run(); err != nil {
			s.Logf("Warning, failed to delete path %v in container: %v", path, err)
		}
	}
