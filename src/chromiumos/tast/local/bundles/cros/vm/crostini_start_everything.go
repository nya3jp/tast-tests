...
func CrostiniStartEverything(s *testing.State) {
	...
	// terminal commands. We need to base64 encode it since we are using the vsh
	// command underneath which is a terminal and binary control characters may
	...
	cmd = cont.Command(ctx, "sh", "-c",
		"echo '"+base64Deb+"' | base64 --decode >~/"+debianFilename)
	if err = cmd.Run(); err != nil {
		cmd.DumpLog(ctx)
