...
func CrostiniAudioSanity(ctx context.Context, s *testing.State) {
	...
	if err = cont.Command(ctx, playbackArgs...).Run(testexec.DumpLogOnError); err != nil {
	...
	if err = cont.Command(ctx, captureArgs...).Run(testexec.DumpLogOnError); err != nil {
