...
// Executes a test wayland application directly from the command line in the
// terminal and verifies that it draws to the screen successfully.
func WaylandAppFromTerminal(s *testing.State, cont *vm.Container) {
	s.Log("Executing wayland app test from terminal launch")

	// Launch the wayland test app which will maximize itself and then use the
	// argument as a solid color to fill as its background.
	cmd := cont.Command(s.Context(), "/opt/google/cros-containers/bin/wayland_demo", "0x3388dd")
	if err := cmd.Start(); err != nil {
		cmd.DumpLog(s.Context())
		s.Error("Failed launching the wayland_demo application")
		return
	}

	const screenshotName = "screenshot_wayland_terminal.png"
	...
	expectedColor := screenshot.Color{0x3333, 0x8888, 0xdddd}
	...
	// Terminate the wayland app now so that if there's a failure in the
	...
	if err != nil {
		cmd.DumpLog(s.Context())
		s.Error("Failure in screenshot comparison for wayland app from terminal: ", err)
	}
}
