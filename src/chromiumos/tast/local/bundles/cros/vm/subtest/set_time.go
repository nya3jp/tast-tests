...
func getTime(ctx context.Context, s *testing.State, cont *vm.Container) (time.Duration, error) {
	...
	secs, err := strconv.ParseInt(outStr)
	if err != nil {
...
func SyncTime(ctx context.Context, s *testing.State, cont *vm.Container) {
	...
	const PastTime = 10000
	// Set the time with maitred_client.
	cmd := exec.CommandContext(ctx, "maitred_client", fmt.Sprintf("--cid=%d", cont.VM.Cid), "--port=8888", fmt.Sprintf("--set_time_sec=%d", PastTime))
	...
	secs := vmTime.Seconds()
	if diff := PastTime - secs; math.Abs(diff) > 60 {
	...
	if diff := actualTime - secs; math.Abs(diff) > 60 {
		s.Errorf("Failed to correct time; got %d, want %d", secs, actualTime)
	}
}
