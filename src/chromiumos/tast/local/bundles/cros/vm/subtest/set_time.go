...
func SyncTime(ctx context.Context, s *testing.State, cont *vm.Container) {
	...
	cmd := exec.CommandContext(ctx, "maitred_client", fmt.Sprintf("--cid=%d", cont.VM.ContextID), "--port=8888", fmt.Sprintf("--set_time_sec=%d", pastTime.Unix()))
	...
	secs := vmTime
	if diff := pastTime.Sub(secs); diff < -time.Minute || diff > time.Minute {
		s.Errorf("Maitred failed to set time; got %d, want %d", secs, pastTime)
	}
	...
	secs = vmTime
	actualTime := time.Now()
	if diff := actualTime.Sub(secs); diff < -time.Minute || diff > time.Minute {
		s.Errorf("Failed to correct time; got %v, want %v", secs, actualTime)
	}
}
