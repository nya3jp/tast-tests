...
func VerifyAppFromTerminal(s *testing.State, cont *vm.Container, name string,
	command string, rgbColor uint32) {
	...
	commandColor := fmt.Sprintf("0x%02x%02x%02x", red, green, blue)
	cmd := cont.Command(ctx, command, commandColor)
	if err := cmd.Start(); err != nil {
		defer cmd.DumpLog(ctx)
		s.Error("Failed launching the  application: %q", command)
		return
	}
	...
	expectedColor := screenshot.Color{uint16(red | (red << 8)), uint16(green | (green << 8)),
		uint16(blue | (blue << 8))}
	...
	if err != nil {
		defer cmd.DumpLog(ctx)
		s.Error("Failure in screenshot comparison for "+name+" from terminal: ", err)
	}
}
