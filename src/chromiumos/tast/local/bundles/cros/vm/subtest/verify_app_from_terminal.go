...
func VerifyAppFromTerminal(ctx context.Context, s *testing.State, cont *vm.Container,
	name, command string, expectedColor color.Color) {
	...
	commandColor := fmt.Sprintf("0x%02x%02x%02x", r>>8, g>>8, b>>8)
	...
	err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		return fmt.Errorf("screenshot did not have matching dominant color, expected %v but got %v at ratio %v",
