...
// toTimeUnit returns time.Duration int unit |unit| as float64 numbers
...
func CrostiniCPUPerf(ctx context.Context, s *testing.State) {
	...
	var parseSyscallBenchOutput = func(out string) (t time.Duration, err error) {
		...
		if matched != nil {
			...
			if err != nil {
				return 0.0, errors.Wrapf(err, "failed to parse time %q in lmbench output", matched[1])
			}
	...
	var measureSyscallTime = func(args ...string) error {
		...
		// Currently the lmbench installed on Cros installs individual benchmarks in /usr/local/bin so
	...
	// lmbench reads /dev/zero and writes to /dev/null. "null" calls getpid().
	...
	// The three command operates on a file.
