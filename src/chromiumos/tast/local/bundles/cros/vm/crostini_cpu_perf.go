...
import (
	...
	"chromiumos/tast/local/bundles/cros/vm/perfutils"
...
func CrostiniCPUPerf(ctx context.Context, s *testing.State) {
	...
	defer errFile.Close()
	writeError := perfutils.WriteErrorFunc(ctx, errFile)
	runCmd := perfutils.RunCmdFunc(ctx, writeError)
	...
	sysBenchRunner := perfutils.HostBinaryRunner{
	...
	var parseSyscallBenchOutput = func(out string) (time.Duration, error) {
	...
	var measureSyscallTime = func(args ...string) error {
		...
		perfValues.Set(
			...
			perfutils.ToTimeUnit(time.Microsecond, hostTime)...)
		perfValues.Set(
			perf.Metric{
				Name:      "crostini_cpu",
				Variant:   metricName("guest"),
			...
			perfutils.ToTimeUnit(time.Microsecond, guestTime)...)
		perfValues.Set(
			perf.Metric{
				Name:      "crostini_cpu",
				Variant:   metricName("ratio"),
