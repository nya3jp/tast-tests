...
func Blogbench(ctx context.Context, s *testing.State) {
	...
	if config.externalDisk {
		td = "/media/removable/SSD/tast.vm.Blogbench"
	...
	*/
	...
	if kind == "block" || kind == "block_btrfs" {
		...
		// Assume /dev/sda is mounted at usbDir.
		usbDir := "/media/removable/SSD/"
		// We unmount /dev/sda in the host and pass it to crosvm.
		if err := syscall.Unmount(usbDir, 0); err != nil {
			s.Fatalf("Failed to unmount %s: %v", usbDir, err)
		}
		defer func() {
			if err := syscall.Mount("/dev/sda", usbDir, "ext4", 0, ""); err != nil {
