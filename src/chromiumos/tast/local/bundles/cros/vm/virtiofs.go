...
func Virtiofs(ctx context.Context, s *testing.State) {
	...
	tar := testexec.CommandContext(ctx, "tar", "xaf", s.DataPath("rootfs_x86_64.tar.xz"), "-C", td)
	...
	args := []string{
		"run",
		"-p", "root=/dev/root rootfstype=virtiofs systemd.unit=run-pjdfstest.service",
		"-c", "1",
		"-m", "1024",
		"-s", td,
		"--serial", fmt.Sprintf("type=file,num=1,console=true,path=%s", logFile),
		"--shared-dir", fmt.Sprintf("%s:/dev/root:type=fs", filepath.Join(td, "rootfs")),
		"--disable-sandbox",
