...
func CrostiniStartTime(ctx context.Context, s *testing.State) {
	...
	if err := vm.StartContainer(ctx, cont); err != nil {
	...
	var vm_start_times, container_start_times, total_times []float64
	for i := 0; i < sampleNum; i++ {
		...
		s.Log("Shutting down VM")
		if err := vmInstance.Close(ctx); err != nil {
			s.Fatal("Falsed to close VM: ", err)
		}
	}
	for i := 0; i < sampleNum; i++ {
	...
	value.Set(perf.Metric{
		...
		Multiple:  true}, vm_start_times...)
	value.Set(perf.Metric{
		Name:      "crostini_start_time",
		Variant:   "container_start_time",
		Unit:      "s",
		Direction: perf.SmallerIsBetter,
		Multiple:  true}, container_start_times...)
	value.Set(perf.Metric{
		Name:      "crostini_start_time",
		Variant:   "total_time",
		Unit:      "s",
		Direction: perf.SmallerIsBetter,
		Multiple:  true}, total_times...)
	value.Save(s.OutDir())
}
