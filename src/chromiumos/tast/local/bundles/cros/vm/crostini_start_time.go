...
func CrostiniStartTime(ctx context.Context, s *testing.State) {
	...
	// Hold the container instance across multiple runs.
	...
	measure := func() *measurements {
		...
		// Create default container for the first call of this function.
		if cont == nil {
		...
		startTime = time.Now()
		if err := vmInstance.OpenExistingContainer(ctx, cont); err != nil {
