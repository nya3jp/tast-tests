...
func openTapDevice(device *pp.NetworkDevice) (int, error) {
	...
	if err != nil {
		return -1, errors.Wrapf(err, "failed to open Tap device: %v", device.Ifname)
	}
...
func VhostUserNet(ctx context.Context, s *testing.State) {
	...
	s.Log("Start vhost-user-net-device")
	...
	clientOut, err := os.Create(filepath.Join(s.OutDir(), "crosvm2.log"))
	if err != nil {
		s.Fatal("Failed to create crosvm log file: ", err)
	}
