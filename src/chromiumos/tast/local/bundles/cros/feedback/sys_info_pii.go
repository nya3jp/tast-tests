...
func SysInfoPII(ctx context.Context, s *testing.State) {
	...
	if err := conn.Eval(ctx, "document.title", &title); err != nil {
		s.Fatal("Failed to get page title: ", err)
	}
		...
		if info.Key == "mem_usage_with_title" {
			if !strings.Contains(info.Value, title) {
				...
				if err := saveLog(s.OutDir(), info.Key, info.Value); err != nil {
					s.Error("Also, failed to save log contents")
				}
			}
		} else {
			if strings.Contains(info.Value, title) {
				s.Errorf("Log %q unexpectedly contained tab title", info.Key)
				if err := saveLog(s.OutDir(), info.Key, info.Value); err != nil {
					s.Error("Also, failed to save log contents")
				}
			} else if strings.Contains(info.Value, searchQuery) {
				s.Errorf("Log %q unexpectedly contained search query", info.Key)
				if err := saveLog(s.OutDir(), info.Key, info.Value); err != nil {
					s.Error("Also, failed to save log contents")
				}
			}

			if strings.Contains(info.Value, sensitiveURLEnd) {
