...
func ReportContainsEmail(ctx context.Context, s *testing.State) {
	...
	cr, err := chrome.New(ctx, chrome.EnableFeatures("OsFeedback", "OsFeedbackSaveLocal"))
	if err != nil {
	...
	var localState localStateParam
	...
	for _, email := range localState.Emails {
		if err := ui.WaitUntilExists(nodewith.NameContaining(email).First())(ctx); err == nil {
			selectedEmail = email
		}
	}

	// Choose not to include email if needed.
	if !s.Param().(testParam).includeEmail {
		for i := 0; i < 4; i++ {
