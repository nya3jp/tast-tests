...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Verify the screenshot is taken when user navigates to share data page",
		Contacts: []string{
...
func ScreenshotIsTaken(ctx context.Context, s *testing.State) {
	...
	screenshotLabel := nodewith.Name("Screenshot").Role(role.StaticText).Ancestor(feedbackRootNode)
	screenshotImg := nodewith.Name("$i18n{screenshotA11y}").Role(role.Image).Ancestor(feedbackRootNode)
	...
	if err := uiauto.Combine("Verify screenshot exist",
