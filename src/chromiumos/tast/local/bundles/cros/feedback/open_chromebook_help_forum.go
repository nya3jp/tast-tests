...
func OpenChromebookHelpForum(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatalf("Failed to connect to active browser for %v: %v", chromebookHelpForumAddress, err)
	}
	defer brCleanUp(ctx)
	c, err := br.NewConnForTarget(ctx, chrome.MatchTargetURL(chromebookHelpForumAddress))
	if err != nil {
		s.Fatalf("Failed to find browser window for %v: %v", chromebookHelpForumAddress, err)
	}
	defer c.Close()
