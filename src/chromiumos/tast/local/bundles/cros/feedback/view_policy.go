...
func ViewPolicy(ctx context.Context, s *testing.State) {
	...
	// Find link and click.
	forumLink := nodewith.NameContaining(s.Param().(testParam).linkName).Role(
		role.Link).Ancestor(feedbackRootNode)
	if err := ui.DoDefault(forumLink)(ctx); err != nil {
		s.Fatal("Failed to find chromebook help forum link: ", err)
	}
	...
	if err := nodewith.NameContaining(s.Param().(testParam).linkAddress).Role(
		...
		s.Fatal("Failed to open chromebook help forum: ", err)
	}
}
