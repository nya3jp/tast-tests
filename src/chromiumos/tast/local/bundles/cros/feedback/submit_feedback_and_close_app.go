...
func SubmitFeedbackAndCloseApp(ctx context.Context, s *testing.State) {
	...
	title := nodewith.Name("Thanks for your feedback").Role(role.StaticText).Ancestor(feedbackRootNode)
	...
	newReportButton := nodewith.Name("Send new report").Role(role.Button).Ancestor(feedbackRootNode)
	if err := ui.WithTimeout(20 * time.Second).WaitUntilExists(newReportButton)(ctx); err != nil {
		s.Fatal("Failed to find new report button: ", err)
	}

	s.Log("Verifying explore app link exists in the confirmation page")
	exploreAppLink := nodewith.NameContaining("Explore app").Role(role.Link).Ancestor(feedbackRootNode)
	if err := ui.WithTimeout(20 * time.Second).WaitUntilExists(exploreAppLink)(ctx); err != nil {
		s.Fatal("Failed to find explore app link: ", err)
	}
	s.Log("Verifying diagnostics app link exists in the confirmation page")
	diagnosticsAppLink := nodewith.NameContaining("Diagnostics app").Role(role.Link).Ancestor(feedbackRootNode)
	if err := ui.WithTimeout(20 * time.Second).WaitUntilExists(diagnosticsAppLink)(ctx); err != nil {
