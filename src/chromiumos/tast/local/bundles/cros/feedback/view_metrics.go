...
func ViewMetrics(ctx context.Context, s *testing.State) {
	...
	if err := ui.WaitUntilExists(metrics)(ctx); err != nil {
		s.Fatal("Failed to find metrics: ", err)
	}
	if err := uiauto.Combine("Verify metrics link exist and click it",
		ui.WaitUntilExists(metrics),
		ui.DoDefault(metrics),
	)(ctx); err != nil {
		s.Fatal("Failed to find metrics link and click it: ", err)
	}

	// Verify user can view metrics.
	histograms := nodewith.Name("Histograms").First()
	if err := ui.WaitUntilExists(histograms)(ctx); err != nil {
		s.Fatal("Failed to view metrics: ", err)
	}
}
