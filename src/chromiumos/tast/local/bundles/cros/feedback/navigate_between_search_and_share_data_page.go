...
func NavigateBetweenSearchAndShareDataPage(ctx context.Context, s *testing.State) {
	...
	s.Log("Launching feedback app and go to share data page")
	...
	attachfilesTitle := nodewith.Name("Attach files").Role(role.StaticText).Ancestor(feedbackRootNode)
	if err := ui.WithTimeout(20 * time.Second).WaitUntilExists(attachfilesTitle)(ctx); err != nil {
	...
	shareDiagnosticDataTitle := nodewith.Name("Share diagnostic data").Role(role.StaticText).Ancestor(
		feedbackRootNode)
	if err := ui.WithTimeout(20 * time.Second).WaitUntilExists(shareDiagnosticDataTitle)(ctx); err != nil {
	...
	if err := ui.WithTimeout(20 * time.Second).WaitUntilExists(issueDescription)(ctx); err != nil {
