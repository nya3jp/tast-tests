...
func AttachFile(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine("Open Downloads dir and select PNG file",
		...
		ui.LeftClick(nodewith.NameContaining(".png").Role(role.StaticText).Ancestor(
	...
	pngFileFinder := nodewith.NameContaining(".png").Role(
	...
	if err := uiauto.Combine("Open Downloads dir and select pdf file",
		ui.LeftClick(nodewith.Name("Downloads").Role(role.TreeItem)),
		ui.LeftClick(nodewith.NameContaining(".pdf").Role(role.StaticText).Ancestor(
			nodewith.Role(role.ListBox)).First()),
		ui.LeftClick(nodewith.Name("Open").Role(role.Button)),
	)(ctx); err != nil {
	...
	newFile := nodewith.NameContaining(".pdf").Role(role.StaticText).Ancestor(feedbackRootNode)
