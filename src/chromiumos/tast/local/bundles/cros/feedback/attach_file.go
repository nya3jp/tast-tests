...
func AttachFile(ctx context.Context, s *testing.State) {
	...
	// Taking a screenshot and generating a png file for uploading purpose.
	...
	// Looking for add file button and click.
	addFileButton := nodewith.NameContaining("Add file").Role(role.Button).Ancestor(feedbackRootNode)
	if err := ui.DoDefault(addFileButton)(ctx); err != nil {
		s.Fatal("Failed to click add file button: ", err)
	}

	// Opening Downloads dir and selecting the screenshot file to upload.
	...
	// Verifying the uploaded screenshot exists.
	screenshot := nodewith.NameContaining(".png").Role(role.StaticText).Ancestor(feedbackRootNode)
	if err := ui.WaitUntilExists(screenshot)(ctx); err != nil {
		s.Fatal("Failed to find screenshot: ", err)
	}

	// Looking for replace button and click.
	replaceButton := nodewith.NameContaining("Replace").Role(role.Button).Ancestor(feedbackRootNode)
