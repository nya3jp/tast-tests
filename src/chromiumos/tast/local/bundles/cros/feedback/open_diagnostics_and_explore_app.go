...
func OpenDiagnosticsAndExploreApp(ctx context.Context, s *testing.State) {
	...
	appLink := nodewith.NameContaining(
		s.Param().(testParam).appLinkName).Role(role.Link).Ancestor(feedbackRootNode).First()
	if err := ui.DoDefault(appLink)(ctx); err != nil {
		s.Fatal("Failed to find app link: ", err)
	}

	// Verify app is opened.
	if err = ash.WaitForApp(ctx, tconn, s.Param().(testParam).app.ID, time.Minute); err != nil {
