...
import (
	...
	"chromiumos/tast/local/chrome/uiauto/wm"
...
func FullScreenshot(ctx context.Context, s *testing.State) {
	...
	if err := wm.CaptureScreenshot(ctx, tconn, wm.FullScreen); err != nil {
	...
	// Check the existence and the size of the screenshot.
...
// And then verifys its size is the same as the size of the full screen by decoding the screenshot.
