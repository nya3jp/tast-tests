...
func APIBatteryCapacityRoutine(ctx context.Context, s *testing.State) {
	...
	// Assert the status is "passed" or "failed". We allow a failing routine (note: no "error", "cancelled", etc.)
	// because we can't make assumptions about the DUT's battery state. "passed" and "failed" signal
	// that the routine was executed successfully.
	if resp.Status != "passed" && resp.Status != "failed" {
		s.Errorf(`Unexpected routine status: got %q; want "passed" or "failed"`, resp.Status)
	}
}
