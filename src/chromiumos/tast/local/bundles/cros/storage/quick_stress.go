...
func setup(ctx context.Context, s *testing.State) {
	...
	resultWriter.RunSequential(ctx, s, []stress.BlockFunc{benchmarkMain, benchmarkSlc})
}
...
func testBlock(ctx context.Context, s *testing.State) {
	...
	resultWriter.RunParallel(ctx, s, []stress.BlockFunc{testBlockMain, testBlockSlc})
	stress.Suspend(ctx)

	resultWriter.RunSequential(ctx, s, []stress.BlockFunc{verifyBlockMain, verifyBlockSlc})
}
