...
type lshwInfo struct {
	// Children contains 'lshw -json' ouput info.
	// E.g.: (some fields are skipped)
	// ...
	// "children" : [{
	//   "id" : "core",
	//   "class" : "bus",
	...
	//   ...
	//   },
	Children []outerChildrenInfo `json:"children"`
}

type outerChildrenInfo struct {
	// Children contains outer children info of lshwInfo.
	// E.g.: (some fields are skipped)
	// ...
	// "children" : [{
	//   ...
	...
	//     "claimed" : true,
	// ...
	Children []innerChildrenInfo `json:"children"`
}

type innerChildrenInfo struct {
	// Children contains inner children info of outerChildrenInfo.
	// E.g.: (some fields are skipped)
	// ...
	// "children" : [{
	//   ...
	...
	//         "claimed" : true,
	// ...
	Children []expectedChildrenInfo `json:"children"`
}

type expectedChildrenInfo struct {
...
func SATAStorage(ctx context.Context, s *testing.State) {
	...
	for _, outerChildrenInfo := range info.Children {
		for _, innerChildrenInfo := range outerChildrenInfo.Children {
			for _, expectedChildrenInfo := range innerChildrenInfo.Children {
				gotID = expectedChildrenInfo.ID
				if gotID == sataStorageID {
					isIDFound = true
				}
				gotDescriptionName = expectedChildrenInfo.Description
				if gotDescriptionName == descriptionName {
					isDescriptNameFound = true
				}
				gotVendorName = expectedChildrenInfo.Vendor
				if gotVendorName == vendorName {
					isVendorNameFound = true
				}
				gotProductName = expectedChildrenInfo.Product
