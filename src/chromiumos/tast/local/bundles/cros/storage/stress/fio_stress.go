...
func runFIO(ctx context.Context, testDataPath, jobFile string, verifyOnly bool) (*fioResult, error) {
	...
	cmd := testexec.CommandContext(ctx, "fio", []string{jobFile, "--output-format=json", "--end_fsync=1"}...)
	cmd.Env = []string{
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "failed to run fio: "+string(out))
	}
...
func getStorageInfo(ctx context.Context) (info *storage.Info, err error) {
...
// validateJSONName replaces all invalid characters that might be present in the device name to appear
