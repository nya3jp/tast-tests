...
func RunFioStressForBootDev(ctx context.Context, s *testing.State, testConfig TestConfig) {
	config := testConfig.WithPath(BootDevicePath).WithJobFile(s.DataPath(testConfig.Job))
...
// returns immediately
// If fio returns an error, this function will fail the Tast test.
func RunFioStressForSlcDev(ctx context.Context, s *testing.State, testConfig TestConfig) {
	if !IsDualQual(ctx, s) {
		return
	}
