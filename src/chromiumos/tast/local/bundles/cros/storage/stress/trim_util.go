...
func WriteRandomData(ctx context.Context, filename string, chunkCount uint64) error {
	...
	bufLength := chunkCount * TrimChunkSize
	testing.ContextLog(ctx, "LENGTH: ", bufLength)
	buf := make([]byte, bufLength)
	if n, err := rand.Read(buf); err != nil || uint64(n) != bufLength {
		return errors.Wrapf(err, " error generating random buffer of length: %d", bufLength)
	}

	if n, err := f.Write(buf); err != nil || uint64(n) != bufLength {
