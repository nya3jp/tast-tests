...
// qualConfig is the configuration of dual-qual functionality.
...
type subTestFunc func(context.Context, *testing.State, *stress.FioResultWriter, qualConfig)
...
func setupBenchmarks(ctx context.Context, s *testing.State, rw *stress.FioResultWriter, testConfig qualConfig) {
	...
	if testConfig.isSlcEnabled {
		// Run tests to collect metrics for Slc device.
		runFioStress(ctx, s, testConfig.WithPath(testConfig.slcDevice).WithJob("4k_write"))
		runFioStress(ctx, s, testConfig.WithPath(testConfig.slcDevice).WithJob("4k_read"))
	}
}

// soakTestBlock runs long, write-intensive storage stresses.
func soakTestBlock(ctx context.Context, s *testing.State, rw *stress.FioResultWriter, testConfig qualConfig) {
	testConfigNoVerify := &stress.TestConfig{
		Duration: testConfig.soakBlockTimeout / 2,
	}
	...
	if testConfig.isSlcEnabled {
		stressTasks = append(stressTasks,
			func(ctx context.Context) {
				runFioStress(ctx, s, testConfigNoVerify.WithPath(testConfig.slcDevice).WithJob("4k_write"))
				runFioStress(ctx, s, testConfigVerify.WithPath(testConfig.slcDevice).WithJob("4k_write"))
			})
	}
...
func retentionTestBlock(ctx context.Context, s *testing.State, rw *stress.FioResultWriter, testConfig qualConfig) {
	writeConfig := stress.TestConfig{
		Job:      "8k_async_randwrite",
		Duration: testConfig.retentionBlockTimeout,
	}
	...
	if testConfig.isSlcEnabled {
		writeTasks = append(writeTasks,
			func(ctx context.Context) {
				runFioStress(ctx, s, writeConfig.WithPath(testConfig.slcDevice))
			})
		verifyTasks = append(verifyTasks,
			func(ctx context.Context) {
				runFioStress(ctx, s, verifyConfig.WithPath(testConfig.slcDevice))
			})
	}
	...
	pollOptions := &testing.PollOptions{
		Timeout:  testConfig.retentionBlockTimeout,
...
func suspendTestBlock(ctx context.Context, s *testing.State, rw *stress.FioResultWriter, testConfig qualConfig) {
	if deadline, _ := ctx.Deadline(); time.Until(deadline) < testConfig.suspendBlockTimeout {
	...
	if testConfig.isSlcEnabled {
		tasks = append(tasks,
			func(ctx context.Context) {
				runContinuousStorageStress(ctx, "4k_write", s.DataPath("4k_write"), rw, testConfig.slcDevice)
			})
	}

	runTasksInParallel(ctx, testConfig.suspendBlockTimeout, tasks)
}

// trimTestBlock is a dispatcher function to start trim test on the boot device
// and on the slc.
func trimTestBlock(ctx context.Context, s *testing.State, rw *stress.FioResultWriter, testConfig qualConfig) {
	...
	if testConfig.isSlcEnabled {
		trimTestBlockImpl(ctx, s, testConfig.slcDevice, rw)
	}
}
...
func stressRunner(ctx context.Context, s *testing.State, rw *stress.FioResultWriter, testConfig qualConfig) {
	for _, tc := range []struct {
		...
		for retries := 0; retries < maxSubtestRetry; retries++ {
			...
			passed := s.Run(ctx, tc.name, func(ctx context.Context, s *testing.State) {
				tc.function(ctx, s, rw, testConfig)
			})
			if passed {
				break
			}
...
// functionalRunner excercises only the functional part of the block.
// It is intended to be used in the lab on bringup devices.
func functionalRunner(ctx context.Context, s *testing.State, rw *stress.FioResultWriter, testConfig qualConfig) {
	for _, tc := range []struct {
		name     string
		function subTestFunc
	}{
...
func miniSoakRunner(ctx context.Context, s *testing.State, rw *stress.FioResultWriter, testConfig qualConfig) {
	soakTestBlock(ctx, s, rw, testConfig)
}
...
func FullQualificationStress(ctx context.Context, s *testing.State) {
	subtest := s.Param().(func(context.Context, *testing.State, *stress.FioResultWriter, qualConfig))
	start := time.Now()

	testConfig := qualConfig{}
	if val, ok := s.Var("tast_storage_slc_qual"); ok {
		var err error
		if testConfig.isSlcEnabled, err = strconv.ParseBool(val); err != nil {
			s.Fatal("Cannot parse argumet 'storage.QuickStress.slcQual' of type bool: ", err)
		}
		if testConfig.isSlcEnabled {
			// Run tests to collect metrics for Slc device.
			if testConfig.slcDevice, err = slcDevice(ctx); err != nil {
	...
	testConfig.soakBlockTimeout = defaultSoakBlockTimeout
	testConfig.retentionBlockTimeout = defaultRetentionBlockTimeout
	testConfig.suspendBlockTimeout = defaultSuspendBlockTimeout

	if val, ok := s.Var("tast_suspend_block_timeout"); ok {
		var err error
		if testConfig.suspendBlockTimeout, err = time.ParseDuration(val); err != nil {
	...
	if passed {
		passed = s.Run(ctx, "storage_subtest", func(ctx context.Context, s *testing.State) {
			resultWriter := &stress.FioResultWriter{}
			defer resultWriter.Save(ctx, s.OutDir(), true)
			subtest(ctx, s, resultWriter, testConfig)
		})
	}
