...
func trimTestBlock(ctx context.Context, s *testing.State, rw *stress.FioResultWriter, slcConfig slcQualConfig) {
	...
	trimTestBlockImpl(ctx, s, bootDevPartition, stress.BootDeviceFioPath, rw)

	if slcConfig.enabled {
		trimTestBlockImpl(ctx, s, slcConfig.device, slcConfig.device, rw)
	}
}
...
func trimTestBlockImpl(ctx context.Context, s *testing.State, trimPath, stressPath string, rw *stress.FioResultWriter) {
	...
	testConfig := &stress.TestConfig{ResultWriter: resultWriter, Path: stressPath}
