...
func runPeriodicPowerSuspend(ctx context.Context) {
	...
	for {
		...
		if err := testing.Sleep(ctx, sleepDuration); errors.As(err, context.DeadlineExceeded) {
			return
		}
...
func runTasksInParallel(ctx context.Context, timeout time.Duration, tasks []func(ctx context.Context)) bool {
	...
	ctx.ContextLog("Starting parallel tasks at: ", time.Now())

	var wg sync.WaitGroup
	for _, task := range tasks {
		wg.Add(1)
		go func(task func(ctx context.Context)) {
			s.Log("Starting parallel subtest: ", task)
			task(ctx)
			s.Log("Finished parallel subtest: ", task)
			wg.Done()
		}(task)
	}
	wg.Wait()
	ctx.ContextLog("Finished parallel tasks at: ", time.Now())
