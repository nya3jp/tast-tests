...
func setupBenchmarks(ctx context.Context, s *testing.State, rw *stress.FioResultWriter) {
	...
	stress.RunFioStressForBootDev(ctx, s, testConfig.WithJob("seq_write"))
	stress.RunFioStressForBootDev(ctx, s, testConfig.WithJob("seq_read"))
	stress.RunFioStressForBootDev(ctx, s, testConfig.WithJob("4k_write"))
	stress.RunFioStressForBootDev(ctx, s, testConfig.WithJob("4k_read"))
	stress.RunFioStressForBootDev(ctx, s, testConfig.WithJob("16k_write"))
	stress.RunFioStressForBootDev(ctx, s, testConfig.WithJob("16k_read"))

	// Run tests to collect metrics for Slc device.
	stress.RunFioStressForSlcDev(ctx, s, testConfig.WithJob("4k_write"))
	stress.RunFioStressForSlcDev(ctx, s, testConfig.WithJob("4k_read"))
}
...
func soakTestBlock(ctx context.Context, s *testing.State, rw *stress.FioResultWriter) {
	...
	stressTasks := []func(context.Context){
		func(ctx context.Context) {
			stress.RunFioStressForBootDev(ctx, s, testConfigNoVerify.WithJob("64k_stress"))
			stress.RunFioStressForBootDev(ctx, s, testConfigVerify.WithJob("surfing"))
		},
		func(ctx context.Context) {
			stress.RunFioStressForSlcDev(ctx, s, testConfigNoVerify.WithJob("4k_write"))
			stress.RunFioStressForSlcDev(ctx, s, testConfigVerify.WithJob("4k_write"))
		},
	}

	stress.RunTasksInParallel(ctx, soakBlockTimeout, writeTasks)
}
...
func retentionTestBlock(ctx context.Context, s *testing.State, rw *stress.FioResultWriter) {
	...
	writeTasks := []func(context.Context){
		func(ctx context.Context) {
			stress.RunFioStressForBootDev(ctx, s, writeConfig)
		},
		func(ctx context.Context) {
			stress.RunFioStressForSlcDev(ctx, s, writeConfig)
		},
	}
	...
	verifyTasks := []func(context.Context){
		func(ctx context.Context) {
			stress.RunFioStressForBootDev(ctx, s, verifyConfig)
		},
		func(ctx context.Context) {
			stress.RunFioStressForSlcDev(ctx, s, verifyConfig)
		},
	}

	stress.RunTasksInParallel(ctx, retentionBlockTimeout, writeTasks)
	...
	stress.RunTasksInParallel(ctx, retentionBlockTimeout*0.5, verifyTasks)
}
...
func runContinuousStorageStress(ctx context.Context, job, jobFile string, rw *stress.FioResultWriter) {
	...
	for {
		if err := stress.RunFioStressForBootDev(ctx, testConfig); errors.Is(err, context.DeadlineExceeded) {
...
func runContinuousSlcStorageStress(ctx context.Context, job string, rw *stress.FioResultWriter) {
	if !stress.IsDualQual(ctx, s) {
		return
	}
	testConfig := stress.TestConfig{
	...
	// Running write stress continuously, until timeout.
	for {
		if err := stress.RunFioStressForSlcDev(ctx, testConfig); errors.Is(err, context.DeadlineExceeded) {
			return // Timeout exceeded.
		}
	}
}
...
func suspendTestBlock(ctx context.Context, s *testing.State, rw *stress.FioResultWriter) {
	...
	tasks := []func(context.Context){
		func(ctx context.Context) {
			runContinuousStorageStress(ctx, "write_stress", rw)
		},
		func(ctx context.Context) {
			runContinuousSlcStorageStress(ctx, "write_4k", rw)
		},
		runPeriodicPowerSuspend,
	}

	stress.RunTasksInParallel(ctx, suspendBlockTimeout, tasks)
}
...
func FullQualificationStress(ctx context.Context, s *testing.State) {
	...
	if err := stress.WriteTestStatusFile(ctx, s, passed, start); err != nil {
