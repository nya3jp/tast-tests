...
func setupBenchmarks(ctx context.Context, s *testing.State, rw *stress.FioResultWriter) {
	...
	runFioStress(ctx, s, testConfig.WithJob("seq_write"), true)
	runFioStress(ctx, s, testConfig.WithJob("seq_read"), true)
	runFioStress(ctx, s, testConfig.WithJob("4k_write"), true)
	runFioStress(ctx, s, testConfig.WithJob("4k_read"), true)
	runFioStress(ctx, s, testConfig.WithJob("16k_write"), true)
	runFioStress(ctx, s, testConfig.WithJob("16k_read"), true)
}
...
func soakTestBlock(ctx context.Context, s *testing.State, rw *stress.FioResultWriter) {
	...
	runFioStress(ctx, s,
		testConfigNoVerify.WithJob("64k_stress").WithDuration(soakBlockTimeout/2), true)
	runFioStress(ctx, s,
		testConfigVerify.WithJob("surfing").WithDuration(soakBlockTimeout/2), true)
}
...
func retentionTestBlock(ctx context.Context, s *testing.State, rw *stress.FioResultWriter) {
	...
	runFioStress(ctx, s, testConfig.WithJob("8k_async_randwrite").WithDuration(retentionBlockTimeout), true)
	...
	}, pollOptions); !errors.As(err, &context.DeadlineExceeded) {
		s.Fatal("Failed running retention block: ", err)
	}

	// Verify disk consistency written by the initial FIO test.
	runFioStress(ctx, s, testConfig.WithJob("8k_async_randwrite").WithVerifyOnly(true), true)
}

// suspendStorageStress is a storage stress that is periodically interrupted by a power suspend.
func suspendStorageStress(ctx context.Context, s *testing.State, rw *stress.FioResultWriter) {
	...
	for {
		if err := runFioStress(ctx, s, testConfig, false); errors.Is(err, context.DeadlineExceeded) {
...
// Exit only when context deadline is exceeded.
...
func runParallelSubTests(ctx context.Context, s *testing.State, rw *stress.FioResultWriter,
	...
	s.Log("Starting parallel test at: ", time.Now())

	var wg sync.WaitGroup
	for _, subTest := range subTests {
		wg.Add(1)
		go func(subTest subTestFunc) {
			s.Log("Starting parallel subtest: ", subTest)
			subTest(ctx, s, rw)
			s.Log("Finished parallel subtest: ", subTest)
			wg.Done()
		}(subTest)
	}
	wg.Wait()
	s.Log("Finished parallel test at: ", time.Now())
...
// If fio returns an error, this function will fail the Tast test if failOnError is true.
func runFioStress(ctx context.Context, s *testing.State, testConfig stress.TestConfig, failOnError bool) error {
	if err := stress.RunFioStress(ctx, testConfig.WithJobFile(s.DataPath(testConfig.Job))); err != nil {
		if failOnError {
			...
			s.Log("Deadline exceeded: ", err)
			return err
		} else {
			s.Log("Non-critical error running FIO stress: ", err)
		}
	}
	return nil
}
