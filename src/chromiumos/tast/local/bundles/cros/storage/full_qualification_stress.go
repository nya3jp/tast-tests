...
// requestedDiskSizeGb returned the disk size as passed to the test
// by an external variable from Moblab UI through an Autotest wrapper.
func requestedDiskSizeGb(s *testing.State) int {
...
func setupChecks(ctx context.Context, s *testing.State) {
	...
	err = info.CheckMainDeviceSize(16 * 1024 * 1024 * 1024)
	if err != nil {
	...
	requestedSizeGb := requestedDiskSizeGb(s)
...
func retentionTestBlock(ctx context.Context, s *testing.State, rw *stress.FioResultWriter) {
	...
	// Rounding start time is needed to strip monitonic clock, which is skewed by sleeps.
...
func stressRunner(ctx context.Context, s *testing.State, rw *stress.FioResultWriter) {
	for _, tc := range []struct {
		...
		for passed, retries := false, 0; !(passed || retries >= maxSubtestRetry); retries++ {
