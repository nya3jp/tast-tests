...
func soakTestBlock(ctx context.Context, s *testing.State, rw *stress.FioResultWriter) {
	stressTasks := []func(context.Context){
		...
		func(ctx context.Context) {
			...
			runFioStressIfSlc(ctx, s,
				testConfigNoVerify.WithJob("4k_write").WithDuration(soakBlockTimeout/2))
			runFioStressIfSlc(ctx, s,
				testConfigVerify.WithJob("4k_write").WithDuration(soakBlockTimeout/2))
		},
	}
