...
func CrosConfig(ctx context.Context, s *testing.State) {
	...
	if !fp.BoardName(board).IsValid() {
		s.Fatalf("Config /fingerprint board %q is invalid", board)
	}
	...
	if crosconfig.IsNotFound(err) {
		s.Log("Not using ", fp.SensorTypeStandAlone, " for fingerprint-sensor-type")
		return
	}
	if !fp.SensorType(sensorType).IsValid() {
		s.Fatalf("Config /fingerprint fingerprint-sensor-type %q is invalid",
			sensorType)
	}
	...
	if fp.SensorLoc(sensorLocation) == fp.SensorLocPowerButtonTopLeft {
		if fp.SensorType(sensorType) != fp.SensorTypeOnPowerButton {
			s.Fatalf("Sensor location is on power button, but type %q doesn't match", sensorType)
		}
	}
