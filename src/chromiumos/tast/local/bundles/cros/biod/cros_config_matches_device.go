...
func captureDmesg(ctx context.Context, dir string) error {
	testing.ContextLog(ctx, "Please check the captured dmesg.txt for cros-ec binding issues")
	...
	err = os.WriteFile(filepath.Join(dir, "dmesg.txt"), dmesg, 0644)
	if err != nil {
...
func CrosConfigMatchesDevice(ctx context.Context, s *testing.State) {
	...
	if cfgFpSupported && fpDevNotExists {
		...
		if err := captureDmesg(ctx, s.OutDir()); err != nil {
			s.Error("Failed to capture dmesg: ", err)
		}
		return
	}
