...
func ParentalControlsLink(ctx context.Context, s *testing.State) {
	...
	defer func(ctx context.Context) {
		faillog.DumpUITreeWithScreenshotOnError(ctx, s.OutDir(), s.HasError, cr, "ui_dump_after_open_settings")
	...
	if err := uiauto.Combine("open parental controls link",
		...
		s.Fatal("Failed to click parental controls link and wait families page exists: ", err)
	}
