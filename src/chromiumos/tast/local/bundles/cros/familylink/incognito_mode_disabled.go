...
func IncognitoModeDisabled(ctx context.Context, s *testing.State) {
	...
	ui := uiauto.New(tconn)
	s.Log("Right clicking the Chromium shelf app button")
	if err := uiauto.Combine("Right clicking the Chromium shelf app button",
		ui.RightClick(nodewith.Name("Chromium").Role(role.Button)),
		ui.WaitUntilExists(nodewith.Name("New window").Role(role.MenuItem)))(ctx); err != nil {
		s.Fatal("Failed to right click the Chromium shelf app button: ", err)
	}
