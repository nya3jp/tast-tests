...
func NonEducoexistenceInsession(ctx context.Context, s *testing.State) {
	...
	if err := familylink.AddEduSecondaryAccountForFamilyLink(ctx, cr, tconn, ui, unicornParentFirstName, unicornParentLastName, unicornParentUser, unicornParentPass, gellerParentUser, gellerParentPass); err != nil {
	...
	s.Log("Clicking close button on modal dialog to reset state for future tests")
	// There are two close buttons, one on the parent settings page and one on the Edu Coexistence flow modal dialog.
	// We want the second close button.
