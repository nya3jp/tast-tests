...
func waitForBoolPrefValue(ctx context.Context, c *chrome.TestConn, prefName string, expectedValue bool, timeout time.Duration) error {
	falseErr := errors.Errorf("%q is not the right value", prefName)
...
func UnicornExtensions(ctx context.Context, s *testing.State) {
	...
	if err := waitForBoolPrefValueFromAshOrLacros(ctx, tconn, isLacros, "profile.managed.extensions_may_request_permissions", true, 10*time.Minute); err != nil {
