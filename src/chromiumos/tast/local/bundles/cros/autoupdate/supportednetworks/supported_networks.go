...
// details. Details are for debug purposes and informational logs.
...
var SupportedNetworks = []SupportedConfiguration{
	{Config: pskConfig, Details: "PSK"},
	{Config: peapWifiConfig, Details: "wifi PEAP"},
}
...
// appropiate verification methods are called for each of them.
func VerifyNetwork(ctx context.Context, nwID ConfigID, nwSet *nc.ManagedProperties) (bool, error) {
	...
	case Psk:
		...
		testing.ContextLogf(ctx, "Verifying the preservation of the %s network", pskExp.Details)
	...
	case PeapWifi:
		...
		testing.ContextLogf(ctx, "Verifying the preservation of the %s network", peapWifiExp.Details)
