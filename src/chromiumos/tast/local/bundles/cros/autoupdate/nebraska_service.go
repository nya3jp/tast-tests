...
type NebraskaService struct { // NOLINT
...
func (nebraska *NebraskaService) CreateTempDir(ctx context.Context, req *empty.Empty) (*aupb.CreateTempDirResponse, error) {
	dir, err := ioutil.TempDir("/tmp", "nebraska")
	if err != nil {
		return &aupb.CreateTempDirResponse{}, errors.Wrap(err, "failed to create temp dir")
	}
	nebraska.root = dir
...
func (nebraska *NebraskaService) Start(ctx context.Context, req *aupb.StartRequest) (*aupb.StartResponse, error) {
	...
	if err := nebraska.cmd.Start(); err != nil {
		return &aupb.StartResponse{}, errors.Wrap(err, "failed to start Nebraska service")
	}
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		return &aupb.StartResponse{}, errors.Wrap(err, "failed to find the port file")
	}

	// Get and check the port number.
	port, err := ioutil.ReadFile(filepath.Join(root, "port"))
	if err != nil {
		return &aupb.StartResponse{}, errors.Wrap(err, "failed to read the Nebraska's port file")
	} else if req.Port != "" && req.Port != string(port) {
		return &aupb.StartResponse{}, errors.Errorf("Nebraska started with wrong port; want %s, got %s", req.Port, string(port))
	}
...
func (nebraska *NebraskaService) Stop(ctx context.Context, req *empty.Empty) (*empty.Empty, error) {
	...
	if !ok {
		testing.ContextLog(ctx, "Failed to wait until the process stopped")
	}
