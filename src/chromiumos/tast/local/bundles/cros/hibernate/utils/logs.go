...
// DetectLog reads the string channel until the expected string matches or context expires.
func DetectLog(ctx context.Context, s *testing.State, ch <-chan string, expMsg string) error {
	for {
		select {
		case msg := <-ch:
		...
		case <-ctx.Done():
			return errors.Errorf("didn't see %q in channel: %v", expMsg, ctx.Err())
		}
	}
	return nil
}
