...
func ARCPlaybackPerf(ctx context.Context, s *testing.State) {
	runTest(ctx, s, s.PreValue().(arc.PreData).ARC, s.Param().(params).videoName, s.Param().(params).videoDesc)
}
...
func runTest(ctx context.Context, s *testing.State, a *arc.ARC, videoName, videoDesc string) {
	...
	if duration < lowerBound || duration > upperBound {
		s.Fatalf("Measured duration %d in msec should lay into [%d, %d]; video is not well-played", duration, lowerBound, upperBound)
	}
