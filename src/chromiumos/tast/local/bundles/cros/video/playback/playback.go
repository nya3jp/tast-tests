...
import (
	...
	"chromiumos/tast/local/bundles/cros/video/lib/cpuutil"
...
const (
	...
	WaitIdleCpuTimeoutSec = 30 * time.Second
...
func RunTest(ctx context.Context, s *testing.State, videoName string, videoDesc string) {
	...
	// TODO(crbug.com/890733): Check if video is downloaded correctlly.
...
func playback(ctx context.Context, fileSystem http.FileSystem, videoName string, gatherPerfFunc metricsFunc, kvs []keyVal, disableHWAcc bool) error {
	...
	cpuutil.WaitForIdleCpu(ctx, WaitIdleCpuTimeoutSec, IdleCpuUsagePercent)
	...
	conn.Exec(ctx, VideoElement+".loop=true")
	if err != nil {
		testing.ContextLog(ctx, "Failed to settle video looping: ", err)
		return err
	}
	...
	// Stop video.
	conn.Exec(ctx, VideoElement+".pause()")
	if err != nil {
		testing.ContextLog(ctx, "Failed to stop video: ", err)
		return err
	}
