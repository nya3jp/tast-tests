...
func RunTest(ctx context.Context, s *testing.State, videoName, videoDesc string) {
	...
	if err := measure(ctx, s.DataFileSystem(), videoName, getDroppedFrames, perfData); err != nil {
	...
	if err := savePerfResults(ctx, mds, perfData, videoDesc, s.OutDir()); err != nil {
		s.Fatal("Failed to save perf data: ", perfData)
	}
}

// measure collects video playback performance by gatherPerfFunc, playing a video with SW decoder and
// also with HW decoder if available.
func measure(ctx context.Context, fileSystem http.FileSystem, videoName string, gatherPerfFunc metricsFunc, perfData collectedPerfData) error {
	// Try Software playback.
	if err := measureWithConfig(ctx, fileSystem, videoName, gatherPerfFunc, perfData, hwAccelDisabled); err != nil {
		return err
	}
	...
	if err := measureWithConfig(ctx, fileSystem, videoName, gatherPerfFunc, perfData, hwAccelEnabled); err != nil {
		return err
	}
	return nil
}

// measureWithConfig plays video one time and measures performance values by executing gatherPerfFunc().
...
func measureWithConfig(ctx context.Context, fileSystem http.FileSystem, videoName string, gatherPerfFunc metricsFunc, perfData collectedPerfData, hwState hwAccelState) error {
	...
	testing.ContextLogf(ctx, "Initial %s histograms: %v", constants.MediaGVDInitStatus, initHistogram.Buckets)
...
func recordMetrics(ctx context.Context, vs map[metricDesc]metricValue, perfData collectedPerfData, cr *chrome.Chrome, initHistogram *metrics.Histogram, hwState hwAccelState) error {
	hwAccelUsed, err := isHWAccelUsed(ctx, cr, initHistogram)
	if err != nil {
...
func isHWAccelUsed(ctx context.Context, cr *chrome.Chrome, initHistogram *metrics.Histogram) (bool, error) {
...
func savePerfResults(ctx context.Context, mds []metricDef, perfData collectedPerfData, videoDesc, outDir string) error {
	// TODO(hiroh): Remove tastSuffix after removing video_PlaybackPerf in autotest.
	p := &perf.Values{}
	const tastPrefix = "tast_"
	for _, pType := range [2]playbackType{playbackWithHWAccel, playbackWithoutHWAccel} {
		keyval, find := perfData[pType]
		if !find {
