...
func measurePerformance(ctx context.Context, s *testing.State, cs ash.ConnSource, cr *chrome.Chrome, fileSystem http.FileSystem, videoName string,
	...
	var roughness, switches, switchesDuration, decThreadsSwitches, decThreadsSwitchesDuration float64
	...
	if perfTracing {
		...
		go func() {
			defer wg.Done()
			switches, switchesDuration, decThreadsSwitches, decThreadsSwitchesDuration, traceErr = measureSchedEvents(ctx, s)
		}()
	}
	...
	if perfTracing {
		p.Set(perf.Metric{
			Name:      "context_switches_in_gpu_process",
			Unit:      "count",
			Direction: perf.SmallerIsBetter,
		}, switches)
		p.Set(perf.Metric{
			Name:      "context_switches_average_duration_in_gpu_process",
			Unit:      "ms",
			Direction: perf.SmallerIsBetter,
		}, switchesDuration)

		p.Set(perf.Metric{
			Name:      "decoder_thread_context_switches_in_gpu_process",
			Unit:      "count",
			Direction: perf.SmallerIsBetter,
		}, decThreadsSwitches)
		p.Set(perf.Metric{
			Name:      "decoder_thread_context_switches_average_duration_in_gpu_process",
			Unit:      "ms",
			Direction: perf.SmallerIsBetter,
		}, decThreadsSwitchesDuration)
	}
...
// measureSchedEvents measure the number of context switches in GPU process and its average waiting duration.
func measureSchedEvents(ctx context.Context, s *testing.State) (float64, float64, float64, float64, error) {
	if err := testing.Sleep(ctx, stabilizationDuration); err != nil {
		return 0, 0, 0, 0, err
	}

	testing.ContextLog(ctx, "Tracing scheduler events")
	// Record system events for |measuremenDuration|.
	sess, err := tracing.StartSession(ctx, s.DataPath(TraceConfigFile))
	if err != nil {
		return 0, 0, 0, 0, errors.Wrap(err, "failed to start tracing")
	}
	if err := testing.Sleep(ctx, measurementDuration); err != nil {
		return 0, 0, 0, 0, errors.Wrap(err, "failed to sleep to wait for the tracing session")
	}
	if err := sess.Stop(); err != nil {
		return 0, 0, 0, 0, errors.Wrap(err, "failed to stop tracing")
	}
	...
	if err != nil {
		return 0, 0, 0, 0, errors.Wrap(err, "failed in querying")
	}
	...
	for _, res := range results[1:] { // Skip, the first line, "ts","dur","state","tid","name".
		...
		case "R": // Runnable
			...
			if err != nil {
				return 0, 0, 0, 0, errors.Wrapf(err, "failed to convert to integer, %s", res[1])
			}
			if dur == -1 {
	...
	switchesDuration := float64(sumRunnableDur) / float64(runnableCnt) * 1000
	decThreadsSwitchesDuration := float64(decThreadsSumRunnableDur) / float64(decThreadsRunnableCnt) * 1000
	return float64(switches), switchesDuration, float64(decThreadsSwitches), decThreadsSwitchesDuration, nil
}
