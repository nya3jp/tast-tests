...
func RunTest(ctx context.Context, s *testing.State, videoName string, videoDesc string) {
	...
	if err := measurePlaybackPerf(ctx, videoName, getDroppedFrames, kvs); err != nil {
		s.Fatalf("Fail in collecting performance values: ", err)
		return
	}
...
func playback(ctx context.Context, videoName string, gatherPerfFunc metricsFunc, kvs []keyVal, disableHWAcc bool) error {
	...
	server := httptest.NewServer(http.FileServer(nil))
	defer server.Close()
...
func measurePlaybackPerf(ctx context.Context, videoName string, gatherPerfFunc metricsFunc, kvs []keyVal) error {
	// Try Software playback.
	// TODO(hiroh): enable this after a histogram issue is resolved.
	// if err := playback(s, ctx, video, gatherPerfFunc, kvs, true); err != nil {
	// 	return err
	// }
	// Try without disabling HW Acceleration.
	if err := playback(ctx, videoName, gatherPerfFunc, kvs, false); err != nil {
		return err
	}
	return nil
}
