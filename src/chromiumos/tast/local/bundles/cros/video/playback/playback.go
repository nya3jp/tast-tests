...
func measureContextSwitch(ctx context.Context, s *testing.State) (gpu, gpuDec contextSwitchStat, err error) {
	...
	const microsToMs = 0.0001
	gpu.count = switches
	gpu.avgDurationInMs = float64(sumRunnableDur) / float64(runnableCnt) * microsToMs
	gpuDec.count = decThreadsSwitches
	gpuDec.avgDurationInMs = float64(decThreadsSumRunnableDur) / float64(decThreadsRunnableCnt) * microsToMs
