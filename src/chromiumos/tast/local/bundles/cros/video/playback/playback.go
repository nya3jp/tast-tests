...
func startPlayback(s *testing.State, ctx context.Context, video string, gatherResultFunc metricsFunc, keyvals *map[string]map[string]float64, disableHWAcc bool) {
	...
	histogramStart, err := metrics.GetHistogram(ctx, cr, common.MediaGVDInitStatus)
	if err != nil {
		s.Fatal("Failed to get histogramStart: ", err)
		return
	}
	testing.ContextLogf(ctx, "begin histograms/%s: %v", common.MediaGVDInitStatus, histogramStart.Buckets)
	...
	// 1. No hitogram is updated. This is the case HW Acceleration is disabled due to Chrome flag, --disable-accelerated-video-decode.
	...
	shortTimeContext, _ = context.WithTimeout(shortTimeContext, time.Duration(12)*time.Second)
	histogramEnd, err := metrics.WaitForHistogramUpdate(ctx, cr, common.MediaGVDInitStatus, histogramStart, time.Duration(10)*time.Second)
	HWAccelerationUsed := false
	if err == nil {
		testing.ContextLogf(ctx, "end histograms/%s: %v", common.MediaGVDInitStatus, histogramEnd.Buckets)
		histogramDiff, err := histogramEnd.Diff(histogramStart)
		s.Logf("histogramDiff Buckets length: %v", len(histogramDiff.Buckets))
		if err != nil {
			s.Fatal("Failed to compute histogram difference: ", err)
		}
		if len(histogramDiff.Buckets) > 1 {
			s.Fatal("Unexpected Histogram Difference: ", histogramDiff.Buckets)
			return
		}
		s.Logf("histogramDiff Buckets: %v", histogramDiff.Buckets)

		// If HW acceleration is used, the single bucket is {0, 1, X}.
		diff := histogramDiff.Buckets[0]
		if diff.Min == common.MediaGVDBucket && diff.Max == common.MediaGVDBucket+1 {
			HWAccelerationUsed = true
		}
	} else {
		s.Error("Histogram is not updated: ", err)
	}
