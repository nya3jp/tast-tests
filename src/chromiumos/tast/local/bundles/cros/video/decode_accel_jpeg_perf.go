...
func DecodeAccelJPEGPerf(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatalf("Failed to disable CPU frequency scaling: %v", err)
	}
	defer restoreCPUFrequencyScaling()

	restoreThermalThrottling, err := cpu.DisableThermalThrottling(shortCtx)
	if err != nil {
		s.Fatalf("Failed to disable Thermal throttling: %v", err)
	}
	defer restoreThermalThrottling()

	if err := cpu.WaitForIdle(shortCtx, waitIdleCPUTimeout, idleCPUUsagePercent); err != nil {
		s.Fatalf("Failed waiting for CPU to become idle: %v", err)
	}

	testing.ContextLogf(ctx, "Measuring SW jpeg decode performance")
	cpuUsageSW := runBenchmark(shortCtx, s, tempDir, swFilter)
	testing.ContextLogf(ctx, "Measuring HW jpeg decode performance")
	cpuUsageHW := runBenchmark(shortCtx, s, tempDir, hwFilter)

	// TODO(dstaessens@): Remove "tast_" prefix after removing video_JDAPerf in autotest.
	var cpuUsageSWMetric = perf.Metric{Name: "tast_sw_jpeg_decode_cpu", Unit: "percent", Direction: perf.SmallerIsBetter}
	var cpuUsageHWMetric = perf.Metric{Name: "tast_hw_jpeg_decode_cpu", Unit: "percent", Direction: perf.SmallerIsBetter}
	p := &perf.Values{}
...
// unittest binary.
func runBenchmark(ctx context.Context, s *testing.State, tempDir string, filter string) float64 {
	args := []string{
	...
	cmd, err := bintest.RunAsync(ctx, exec, args, s.OutDir())
	if err != nil {
	...
	select {
	case <-ctx.Done():
		s.Fatalf("failed waiting for CPU usage to stabilize %v", err)
	case <-time.After(stabilizationDuration):
	}

	testing.ContextLogf(ctx, "Sleeping %v to measure CPU usage", measurementDuration.Round(time.Second))
	cpuUsage, err := cpu.MeasureUsage(ctx, measurementDuration)
	if err != nil {
		s.Fatalf("failed to measure CPU usage %v", err)
	}
