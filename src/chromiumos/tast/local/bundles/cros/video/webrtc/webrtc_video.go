...
// The test fails if bucketValue on histogramName is not count up.
...
// openWebRTCPageAndCheckBucket opens video/data/loopback.html and communicate via webRTC in a fake way. The stream on WebRTC is streamFile.
// It checks bucketValue on histogramName is count up in the end of the test.
