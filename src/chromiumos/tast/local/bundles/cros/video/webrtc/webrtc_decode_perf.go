...
func (a timeDurations) Len() int           { return len(a) }
func (a timeDurations) Swap(i, j int)      { a[i], a[j] = a[j], a[i] }
func (a timeDurations) Less(i, j int) bool { return a[i] < a[j] }
...
func getMedian(s timeDurations) time.Duration {
	size := len(s)
	if size == 0 {
...
// If input is empty, returns 0.
func getMax(s timeDurations) time.Duration {
	max := time.Duration(0)
	for _, n := range s {
		if n > max {
			max = n
...
func measureDecodeTime(ctx context.Context, webRTCInternals *chrome.Conn) (max, median time.Duration, err error) {
	...
	var decodeTimes timeDurations
	// Maximum observed frame decode time.
	var maxDecodeTimes timeDurations

	err = testing.Poll(ctx,
		func(ctx context.Context) error {
			if e := webRTCInternals.Eval(ctx, "googMaxDecodeMs", &maxDecodeTimes); e != nil {
			...
			if e := webRTCInternals.Eval(ctx, "googDecodeMs", &decodeTimes); e != nil {
	...
	for i, v := range maxDecodeTimes {
		maxDecodeTimes[i] = v * time.Millisecond
	}
	for i, v := range decodeTimes {
		decodeTimes[i] = v * time.Millisecond
	}
...
// readDataFile reads data file and returns its content as string.
func readDataFile(s *testing.State, fn string) (string, error) {
	path := s.DataPath(fn)
...
func RunWebRTCDecodePerf(ctx context.Context, s *testing.State, streamName string) {
	...
	addStatsJS, err := readDataFile(s, AddStatsJSFile)
	if err != nil {
