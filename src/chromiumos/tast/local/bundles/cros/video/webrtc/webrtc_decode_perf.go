...
// MeasureParams is a set of parameters for measureFunc to reference.
...
type measureFunc func(context.Context, *testing.State, *chrome.Chrome, *perf.Values, MeasureParams) error
...
func measureCPU(ctx context.Context, s *testing.State, cr *chrome.Chrome, p *perf.Values, params MeasureParams) error {
	s.Logf("Sleeping %v to wait for CPU usage to stabilize", params.CPUStabilize)
	if err := testing.Sleep(ctx, params.CPUStabilize); err != nil {
		return err
	}
	s.Log("Measuring CPU usage for ", params.CPUMeasure)
	cpuUsage, err := cpu.MeasureUsage(ctx, params.CPUMeasure)
	if err != nil {
		return err
	}
	s.Logf("CPU usage: %f%%", cpuUsage)
	p.Set(perf.Metric{
		Name:      params.NamePrefix + "video_cpu_usage",
		Unit:      "percent",
...
func measureDecodeTime(ctx context.Context, s *testing.State, cr *chrome.Chrome, p *perf.Values, params MeasureParams) error {
	...
	err = testing.Poll(ctx,
		func(ctx context.Context) error {
			...
			if len(maxTimesMs) < params.DecodeTimeSamples {
			...
			if len(timesMs) < params.DecodeTimeSamples {
				return errors.New("insufficient samples")
			}
			...
			return nil
		}, &testing.PollOptions{Interval: time.Second, Timeout: params.DecodeTimeTimeout})
	if err != nil {
		return err
	}
	if len(maxDecodeTimes) < params.DecodeTimeSamples {
		return errors.Errorf("got %d max decode time sample(s); want %d", len(maxDecodeTimes), params.DecodeTimeSamples)
	}
	if len(decodeTimes) < params.DecodeTimeSamples {
		return errors.Errorf("got %d decode time sample(s); want %d", len(decodeTimes), params.DecodeTimeSamples)
	}
	...
	p.Set(perf.Metric{
		Name:      params.NamePrefix + "decode_time.percentile_0.50",
		Unit:      "milliseconds",
		Direction: perf.SmallerIsBetter},
		float64(median)/float64(time.Millisecond))
	p.Set(perf.Metric{
		Name:      params.NamePrefix + "decode_time.max",
		Unit:      "milliseconds",
		Direction: perf.SmallerIsBetter},
...
func measureCPUDecodeTime(ctx context.Context, s *testing.State, cr *chrome.Chrome, p *perf.Values, params MeasureParams) error {
	if err := measureCPU(ctx, s, cr, p, params); err != nil {
		return err
	}
	return measureDecodeTime(ctx, s, cr, p, params)
}
...
func webRTCDecodePerf(ctx context.Context, s *testing.State, streamFile, loopbackURL string, measure measureFunc,
	disableHWAccel bool, p *perf.Values, params MeasureParams) (hwAccelUsed bool) {
	...
	params.NamePrefix = "tast_" + prefix
	if err := measure(shortCtx, s, cr, p, params); err != nil {
...
func RunWebRTCDecodePerf(ctx context.Context, s *testing.State, streamName string, params MeasureParams) {
	...
	hwAccelUsed := webRTCDecodePerf(shortCtx, s, streamFilePath, loopbackURL, measureCPUDecodeTime, false, p, params)
	if hwAccelUsed {
		webRTCDecodePerf(shortCtx, s, streamFilePath, loopbackURL, measureCPUDecodeTime, true, p, params)
	}
	p.Save(s.OutDir())
}
