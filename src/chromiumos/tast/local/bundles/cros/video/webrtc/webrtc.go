...
// duration specifies how many seconds video capturing will run in for each resolution.
...
func RunWebRTCCamera(ctx context.Context, s *testing.State, cr *chrome.Chrome,
	duration time.Duration, verbose bool) CameraResults {
	if verbose {
...
func RunWebRTCPeerConnCamera(ctx context.Context, s *testing.State, cr *chrome.Chrome,
	codec videotype.Codec, duration time.Duration, verbose bool) PeerConnCameraResult {
	if verbose {
		vl, err := logging.NewVideoLogger()
		if err != nil {
			s.Fatal("Failed to set values for verbose logging")
