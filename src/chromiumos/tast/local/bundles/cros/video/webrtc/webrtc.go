...
// SetPerf records performance data in webRTCPeerCameraResult to perf.Values.
// p is a pointer for perf.Values where data will be stored.
func (result *webRTCCameraResult) SetPerf(p *perf.Values) {
...
func RunWebRTCCamera(ctx context.Context, s *testing.State, duration int) []webRTCCameraResult {
	var results []webRTCCameraResult
...
// webRTCCameraResult is a struct used in webRTCPeerConnectionWithCameraResult.
...
func RunWebRTCPeerConnectionWithCamera(
	...
	if result.TestStatus != "ok-done" {
		s.Fatal("Failed: ", result.TestStatus)
	}
