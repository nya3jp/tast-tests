...
func loadExpectations(ctx context.Context, s *testing.State) (platformDecodingExpectations, expectations.FileType) {
	...
	f, fileType, err := expectations.OpenTestExpectations(ctx, s)
	if err == nil {
		err = yaml.NewDecoder(f).Decode(expectationsFileContents)
		if err != nil {
...
func PlatformDecoding(ctx context.Context, s *testing.State) {
	...
	for _, filename := range testOpt.filenames {
		expectation, hasExpectation := testExpectations[filename]

		if hasExpectation && expectation.Expectation == expectations.ExpectSkip {
			testing.ContextLogf(ctx, "Skipping %s due to test expectations file", filename)
			continue
		}

		expectError := hasExpectation && expectation.Expectation == expectations.ExpectFailure
		if expectError {
