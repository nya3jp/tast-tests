...
// --var=video.PlatformDecoding.expectationsFileType=model|board|platform
//
...
func PlatformDecoding(ctx context.Context, s *testing.State) {
	...
	testExpectations := expectationsFileContents[testOpt.decoder]
	...
	for _, filename := range testOpt.filenames {
		expectation, hasExpectation := testExpectations[filepath.Base(filename)]
			...
			if updateExpectations {
				...
				if hadError {
					...
					if testExpectations == nil {
						...
						expectationsFileContents[testOpt.decoder] = testExpectations
					}
					testExpectations[filepath.Base(filename)] = expectations.Expectation{Expectation: expectations.ExpectFailure, Ticket: ticket}
				} else {
					testing.ContextLogf(ctx, "Removing unnecessary expecation for %s", filename)
					delete(testExpectations, filepath.Base(filename))
				}
			} else {
