...
type failExpectedFn func(stdout, stderr []byte, err error) bool
	...
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			Val: platformDecodingParams{
				...
				failExpected: func(stdout, stderr []byte, err error) bool {
...
func PlatformDecoding(ctx context.Context, s *testing.State) {
	...
	if stdout, stderr, err := testexec.CommandContext(
		ctx,
		filepath.Join(chrome.BinTestDir, exec),
		"--video="+s.DataPath(testOpt.filename),
	).SeparatedOutput(testexec.DumpLogOnError); err != nil {
		if testOpt.failExpected != nil && testOpt.failExpected(stdout, stderr, err) {
			testing.ContextLog(ctx, "libva decode binary failed as expected")
			return
		}

		s.Fatalf("%v failed: %v", exec, errors.Wrap(err, string(stdout)))
	}
}
