...
func WebRTCMediaRecorder(ctx context.Context, s *testing.State) {
	chromeArgs := []string{
		...
		"--use-fake-device-for-media-stream", // Feeds fake stream with specified fps to getUserMedia()
	...
	runTest := func(js string) error {
		...
		if err := conn.WaitForExpr(ctx, "testDone"); err != nil {
			return errors.Wrap(err, "failed to wait for test completion")
		}
		...
		if result != "PASS" {
			return errors.Errorf("result was %q; want %q", result, "PASS")
		}
		return nil
	}
