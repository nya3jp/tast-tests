...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			Name:      "video_encoder",
			Val:       v4l2Test{command: []string{"v4l2-compliance", "-d", "/dev/video-enc"}, timeout: 20 * time.Second},
			Attr:      []string{"group:graphics", "graphics_video", "graphics_nightly"},
			ExtraAttr: []string{"informational"},
		}, {
			Name:      "video_decoder",
			Val:       v4l2Test{command: []string{"v4l2-compliance", "-d", "/dev/video-dec0"}, timeout: 20 * time.Second},
			Attr:      []string{"group:graphics", "graphics_video", "graphics_nightly"},
			ExtraAttr: []string{"informational"},
		}, {
			Name:      "v4l2_ctl",
