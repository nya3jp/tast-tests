...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Measures video playback performance on ARC++ for VP8 1080p@30fps video",
		Contacts:     []string{"johnylin@chromium.org", "chromeos-video-eng@google.com"},
		Attr:         []string{"group:crosbolt", "crosbolt_perbuild"},
		SoftwareDeps: []string{"android", "chrome"},
		Data:         []string{arcplayback.APKName, "1080p_30fps_600frames.vp8.webm"},
		Pre:          arc.Booted(),
	})
}
...
func ARCPlaybackPerfVP81080P30FPS(ctx context.Context, s *testing.State) {
	arcplayback.RunTest(ctx, s, s.PreValue().(arc.PreData).ARC, "1080p_30fps_600frames.vp8.webm", "vp8_1080p_30fps")
}
