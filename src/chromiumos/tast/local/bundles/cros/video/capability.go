...
func Capability(ctx context.Context, s *testing.State) {
	...
	testing.ContextLog(ctx, "autocaps package result: ", staticCaps)
	...
	testing.ContextLog(ctx, "avtest_label_detect result: ", string(avOut))
	...
	for _, c := range avtestLabelToCapability {
		c = stripPrefix(c)
		_, detectedCapsHas := detectedCaps[c]
		if _, ok := staticCaps[c]; !ok {
			s.Errorf("static setting doesn't have entry for %s", c)
			continue
		}
...
func stripPrefix(cap string) string {
	if !strings.HasPrefix(cap, caps.Prefix) {
		panic(fmt.Sprintf("%s doesn't start with %s", cap, caps.Prefix))
	}
