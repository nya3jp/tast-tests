...
var libyuvUnitTestBin = "libyuv_unittest"
...
// PlatformLibYUVUnittest runs a libyuv unit test with and without SIMD
...
func PlatformLibYUVUnittest(ctx context.Context, s *testing.State) {
	...
	err := runLIBYUVUnittest(ctx, logFileOpt, s.Param().(string), env)
	if err != nil {
	...
	errNoOpt := runLIBYUVUnittest(ctx, logFileNoOpt, s.Param().(string), env)
	if errNoOpt != nil {
		s.Fatal("Failed to run test binary: ", err)
	}
