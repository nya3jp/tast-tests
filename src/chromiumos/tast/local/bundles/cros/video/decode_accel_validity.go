...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			// The "vp9_validity" SoftwareDeps is an allowlist used to filter out devices that are
			...
			ExtraSoftwareDeps: []string{"vp9_validity"},
...
// DecodeAccelValidity runs the FlushAtEndOfStream test in the video_decode_accelerator_tests. The
...
func DecodeAccelValidity(ctx context.Context, s *testing.State) {
	decode.RunAccelVideoValidityTest(ctx, s, s.Param().(string))
}
