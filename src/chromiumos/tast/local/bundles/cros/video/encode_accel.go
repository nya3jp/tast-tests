...
const (
	...
	eaCacheExtractedVideo = false
)

func init() {
	testing.AddTest(&testing.Test{
		Func: EncodeAccel, LacrosStatus: testing.LacrosVariantUnknown, Desc: "Verifies hardware encode acceleration by running the video_encode_accelerator_tests binary",
...
func EncodeAccel(ctx context.Context, s *testing.State) {
	encode.RunAccelVideoTest(ctx, s, s.Param().(encode.TestOptions), eaCacheExtractedVideo)
}
