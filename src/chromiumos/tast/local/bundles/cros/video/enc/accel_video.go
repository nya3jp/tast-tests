...
// Package enc provides common code to run Chrome binary tests for encoding
package enc
...
func RunAccelVideoTest(ctx context.Context, s *testing.State, profile videotype.CodecProfile, params StreamParams) {
	...
	encodeOutFile := chrometest.NewWritableFile(s.OutDir(), fmt.Sprintf("%s.out", params.Name))
	defer func() {
		if err := encodeOutFile.Move(); err != nil {
