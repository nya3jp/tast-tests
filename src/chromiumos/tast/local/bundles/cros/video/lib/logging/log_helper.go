...
func getLogSpecs() ([]logSpec, error) {
	...
	for _, l := range []struct {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "failed to match: %v", l.glob)
		}
		if len(files) > 0 {
...
func NewVideoLogger() (*VideoLogger, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to get logSepcs")
	}
	...
	for _, l := range vl.specs {
		for _, f := range l.files {
			if err := ioutil.WriteFile(f, l.enableValue, 0644); err != nil {
				return nil, errors.Wrap(err, "failed to set log level failed")
			}
		}
	}
	return &vl, nil
	...
	for _, l := range vl.specs {
		for _, f := range l.files {
			if err := ioutil.WriteFile(f, l.disableValue, 0644); err != nil {
				return errors.Wrap(err, "failed to set log level failed")
			}
		}
	}
	return nil
