...
// WaitUntilIdle waits until CPU is idle, for a maximum of 60s. The CPU is
...
func WaitUntilIdle(ctx context.Context) error {
	const (
		...
		// times we wait for CPU to become idle, max usage is increased each time.
		idleCPUSteps = 5
	)
...
// waitUntilIdle waits until CPU is idle, or the specified timeout is elapsed.
// The CPU is considered idle if the CPU average CPU usage over all cores is
// less than maxUsage, which is a percentage in the range [0.0, 100.0].
