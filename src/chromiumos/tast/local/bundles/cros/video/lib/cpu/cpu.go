...
func MeasureProcessCPU(ctx context.Context, runCmdAsync StartProcFunc, duration time.Duration) (float64, error) {
	...
	// cleanupProcess will be trigger upon exiting this function to kill the process.
	cleanupProcess := func() {
...
// tearDown function should be scheduled by the caller if err is non-nil.
func SetUpBenchmark(ctx context.Context) (tearDown func(ctx context.Context), err error) {
	const (
	...
	// Run the tearDown function automatically if we encounter an error.
	cleanup := tearDown
	defer func() {
		if cleanup != nil {
			cleanup(ctx)
	...
	// Disarm running the tearDown function now that we expect the caller to do it.
	cleanup = nil
	return tearDown, nil
}
...
// caller can restore the original CPU frequency scaling configuration. The
// caller should always run the function with a longer context, even on failure.
