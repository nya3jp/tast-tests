...
func MeasureProcessCPU(ctx context.Context, runCmdAsync StartProcFunc, duration time.Duration) (float64, error) {
	...
	cleanupProcess := func() {
		...
		if err := cmd.Wait(); err != nil {
			...
			if !ok || !ws.Signaled() || ws.Signal() != syscall.SIGKILL {
				testing.ContextLog(ctx, "failed waiting the command to exit: ", err)
			}
		}
	}
	...
	testing.ContextLogf(shortCtx, "Sleeping %v to measure CPU usage", duration.Round(time.Second))
	cpuUsage, err := MeasureUsage(shortCtx, duration)
	if err != nil {
...
func SetUpBenchmark(ctx context.Context) (undo func(ctx context.Context), err error) {
	const (
	...
	if restoreScaling, err = disableCPUFrequencyScaling(shortCtx); err != nil {
