...
func SetUpBenchmark(ctx context.Context) (shortCtx context.Context, undo func(), err error) {
	...
	// CPU idle threshold.
	idleCPUUsagePercentIncrease := (idleCPUUsagePercentMax - idleCPUUsagePercent) / (idleCPUSteps - 1)
	for i := 0; i < idleCPUSteps; i++ {
		currentIdlePercent := idleCPUUsagePercent + (idleCPUUsagePercentIncrease * float64(i))
		if err = WaitForIdle(shortCtx, waitIdleCPUTimeout/idleCPUSteps, currentIdlePercent); err == nil {
			break
		} else if i == idleCPUSteps-1 {
