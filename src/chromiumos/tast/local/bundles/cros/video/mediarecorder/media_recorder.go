...
// MediaRecorder.
func VerifyEncodeAccelUsed(ctx context.Context, s *testing.State, codec string) {
	chromeArgs := []string{
	...
	startRecordJs := "startRecording('" + codec + "')"
	if err := conn.EvalPromise(ctx, startRecordJs, nil); err != nil {
		s.Fatal("Failed to evaluate startRecording(): ", err)
	}
	...
	if bucket.Min != bucketValue || bucket.Max != bucketValue+1 || bucket.Count != 1 {
		s.Error("unexpected histogram update: ", bucket)
	}
}
