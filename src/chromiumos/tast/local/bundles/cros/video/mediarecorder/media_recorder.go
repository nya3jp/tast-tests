...
func reportMetric(name, unit, outDir string, value float64, direction perf.Direction) error {
	p := perf.NewValues()
	p.Set(perf.Metric{
...
func getMetricName(name string, hwAccelUsed bool) (fullName string) {
	if hwAccelUsed {
		return "hw_" + name
	}
	return "sw_" + name
...
func MeasurePerf(ctx context.Context, fileSystem http.FileSystem, outDir string, codec videotype.Codec, streamFile string, fps int) error {
	hwAccelUsed, err := measureAndReport(ctx, fileSystem, outDir, codec, streamFile, fps, true)
	if err != nil {
		return err
	}
	...
	hwAccelUsed, err = measureAndReport(ctx, fileSystem, outDir, codec, streamFile, fps, false)
	if err != nil {
		return err
	}
	return nil
...
func measureAndReport(ctx context.Context, fileSystem http.FileSystem, outDir string, codec videotype.Codec, streamFile string, fps int, hwAccelEnabled bool) (hwAccelUsed bool, err error) {
	...
	err = reportPerf(processingTime, cpuUsage, hwAccelUsed, outDir)
	if err != nil {
		return hwAccelUsed, errors.Wrapf(err, "failed to report perf. HWAccel requested = %v used = %v", hwAccelEnabled, hwAccelUsed)
	}
...
func reportPerf(processingTime time.Duration, cpuUsage float64, hwAccelUsed bool, outDir string) error {
	metricName := getMetricName("frame_processing_time", hwAccelUsed)
	if err := reportMetric(metricName, "millisecond", outDir, float64(processingTime.Nanoseconds()*1000000), perf.SmallerIsBetter); err != nil {
		return errors.Wrapf(err, "failed to report perf result %v", metricName)
	}
	metricName = getMetricName("cpu_usage", hwAccelUsed)
	if err := reportMetric(metricName, "percent", outDir, cpuUsage, perf.SmallerIsBetter); err != nil {
		return errors.Wrapf(err, "failed to report perf result %v", metricName)
	}
	return nil
}
...
func doMeasurePerf(ctx context.Context, fileSystem http.FileSystem, outDir string, codec videotype.Codec, disableHWAccel bool, streamFile string, fps int) (processingTime time.Duration, cpuUsage float64, hwAccelUsed bool, err error) {
	cr, err := chrome.New(ctx, chrome.ExtraArgs(getChromeArgs(fps, streamFile, disableHWAccel)...))
	if err != nil {
		return time.Duration(0), 0, false, errors.Wrap(err, "failed to connect to Chrome")
	}
	defer cr.Close(ctx)
	...
	if err != nil {
		return time.Duration(0), 0, false, errors.Wrap(err, "failed to set up benchmark")
	}
	...
	if err != nil {
		return time.Duration(0), 0, false, errors.Wrap(err, "failed to get initial histogram")
	}

	conn, err := cr.NewConn(shortCtx, server.URL+"/loopback_media_recorder.html")
	if err != nil {
		return time.Duration(0), 0, false, errors.Wrap(err, "failed to open recorder page")
	}
	defer conn.Close()

	if err := conn.WaitForExpr(shortCtx, "pageLoaded"); err != nil {
		return time.Duration(0), 0, false, errors.Wrap(err, "Timed out waiting for page loading")
	}
	...
	if err := conn.EvalPromise(shortCtx, startRecordJS, nil); err != nil {
		return time.Duration(0), 0, false, errors.Wrapf(err, "failed to evaluate %v", startRecordJS)
	}

	// While the video recording is in progress, measure CPU usage.
	cpuUsage = 0.0
	if cpuUsage, err = measureCPUUsage(shortCtx, conn); err != nil {
		return time.Duration(0), 0, false, errors.Wrap(err, "failed to measure CPU")
	}

	// Recorded video will be saved in |videoBuffer| in base64 format.
	videoBuffer := ""
	if err := conn.EvalPromise(shortCtx, "stopRecording()", &videoBuffer); err != nil {
		return time.Duration(0), 0, false, errors.Wrap(err, "failed to stop recording")
	}

	hwUsed, err := histogram.WasHWAccelUsed(shortCtx, cr, initHistogram, constants.MediaRecorderVEAUsed, int64(constants.MediaRecorderVEAUsedSuccess))
	if err != nil {
		// In case of errors, assume that HW acceleration was not used.
		hwUsed = false
	}

	elapsedTimeMs := 0
	if err := conn.Eval(shortCtx, "elapsedTime", &elapsedTimeMs); err != nil {
		return time.Duration(0), 0, false, errors.Wrap(err, "failed to evaluate elapsedTime")
	}

	videoBytes, err := base64.StdEncoding.DecodeString(videoBuffer)
	if err != nil {
		return time.Duration(0), 0, false, errors.Wrap(err, "failed to decode base64 string into byte array")
	}

	frames := 0
	if frames, err = computeNumFrames(videoBytes, outDir); err != nil {
		return time.Duration(0), 0, false, errors.Wrap(err, "failed to compute number of frames")
	}

	processingTime = time.Duration(elapsedTimeMs / frames * 1000000)
