...
func MeasurePerf(ctx context.Context, s *testing.State, chromeArgs []string, codec videotype.Codec, processingTime *int, cpuUsage *float64) error {
	...
	videoBufferLen := 0
	if err := conn.Eval(shortCtx, "videoBufferSize", &videoBufferLen); err != nil {
		return errors.Wrap(err, "Failed to evaluate videoBufferLength")
	}
	elapsedTime := 0
	...
	//s.Errorf("video buffer: elapsedTime=%v videoBufferLength=%v %v %v", elapsedTime, videoBufferLen, len(videoBuffer), *cpuUsage)
	...
	frames := 0
	if err := computeFrameNum(videoBytes, s.OutDir(), &frames, s); err != nil {
		return errors.Wrap(err, "Failed to compute number of frames")
	}

	s.Errorf("video frames=%v elapsedTime=%v cpu=%v ", frames, elapsedTime, *cpuUsage)

	*processingTime = elapsedTime / frames
	return nil
}

func computeFrameNum(videoBytes []byte, tmpDir string, frameNum *int, s *testing.State) error {
