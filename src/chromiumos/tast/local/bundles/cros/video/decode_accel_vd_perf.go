...
func DecodeAccelVDPerf(ctx context.Context, s *testing.State) {
	...
	if err := decoding.RunAccelVideoPerfTest(ctx, s.OutDir(), s.DataPath(param.dataPath), decoding.TestParams{
		DecoderType:            decoding.VD,
		DisableGlobalVaapiLock: param.disableGlobalVaapiLock,
		RunConcurrentDecoders:  param.runConcurrentDecoders || param.disableGlobalVaapiLock,
	}); err != nil {
