...
// RunTest runs video_encode_accelerator_unittest with profile and params.
// It fails if video_encode_accelerator_unittest fails.
func RunTest(ctx context.Context, s *testing.State, profile videotype.CodecProfile, params StreamParams) {
	...
	var encodedOutFile string
	if encodedOutFile, err = chrometest.CreateWritableFile(s.OutDir(), fmt.Sprintf("%s.out", params.Name)); err != nil {
		s.Fatalf("Failed to create a writable file in OutDir")
	}

	// This is debug
	x := exec.Command("ls", "-al", encodedOutFile)
	...
	// debug end
	// This is debug
	x = exec.Command("ls", "-al", filepath.Join(s.OutDir()))
	x.Stdout = &out
	x.Run()
	s.Log(x)
	// debug end
	...
	testParamList := []string{
		...
		createStreamDataArg(params, profile, streamPath, encodedOutFile),
		"--ozone-platform=gbm",
		"-v=2"}
