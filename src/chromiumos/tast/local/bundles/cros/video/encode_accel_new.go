...
// encodeTestData returns the files used in video.EncodeAccelNew, the webm file and the json file returned by encode.JSONFileName().
func encodeTestData(webmFileName string) []string {
	return []string{webmFileName, encode.JSONFileName(webmFileName)}
}
