...
func DecodeEncodeAccelPerf(ctx context.Context, s *testing.State) {
	...
	cpuUsage, err := cpu.MeasureProcessCPU(ctx,
		[]cpu.StartProcFunc{runEncodeTestAsync, runDecodeTestAsync}, measureDuration)
	if err != nil {
...
// startTest returns a function that starts the specified test binary with the
// provided arguments.
func startTest(ctx context.Context, exec, filter, outDir string,
	args []string) cpu.StartProcFunc {
	return func() (*testexec.Cmd, error) {
		return gtest.New(
