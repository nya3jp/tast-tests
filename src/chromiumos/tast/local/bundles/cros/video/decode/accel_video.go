...
func DataFiles(profile videotype.CodecProfile, mode VDABufferMode) []string {
	...
	// TODO(dstaessens@) Only add json file when the old VDA tests have been deprecated.
...
// TODO(dstaessens@) Rename this function once the video_decode_accelerator_unittest
...
func RunAccelVideoTestNew(ctx context.Context, s *testing.State, filename string) {
	...
	args := []string{s.DataPath(filename)}
