...
// RunAccelVideoSanityTest runs the FlushAtEndOfStream test in the video_decode_accelerator_tests.
// The test only fails if the video decoder's kernel driver crashes.
...
func RunAccelVideoSanityTest(ctx context.Context, s *testing.State, filename string) {
	...
	// Run the FlushAtEndOfStream test, but ignore errors. The test only fails
	// if the kernel crashes.
	args := []string{s.DataPath(filename), s.DataPath(filename + ".json"),
		"--output_folder=" + s.OutDir(), "--disable_validator"}
	const exec = "video_decode_accelerator_tests"
	_, _ = gtest.New(
		filepath.Join(chrome.BinTestDir, exec),
		gtest.Logfile(filepath.Join(s.OutDir(), exec+".log")),
