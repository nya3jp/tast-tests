...
func RunVAAPIBinary(ctx context.Context, s *testing.State, filename string, expectedFail bool) {
	...
	defer upstart.EnsureJobRunning(ctxForCleanup, "ui")
	...
	if output, err := testexec.CommandContext(
		...
		errMsg := errors.Wrap(err, string(output))
		if !expectedFail {
			s.Fatalf("%v failed: %v", exec, errMsg)
		}
		if errors.Is(err, context.DeadlineExceeded) {
			s.Fatalf("%v exceeded deadline to complete: %v", exec, errMsg)
		}

		testing.ContextLog(ctx, "libva decode binary expected to fail and did")
		return
	}
}
