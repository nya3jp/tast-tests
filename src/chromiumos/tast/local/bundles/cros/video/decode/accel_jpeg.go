...
func RunAccelJpegTest(ctx context.Context, s *testing.State, name string, testFiles []string) {
	...
	defer func() {
		err := chrometest.RemoveDir(tempDir)
		if err != nil {
			s.Fatalf("Failed to delete temp dir %s: %v", tempDir, err)
		}
	}()
	...
	const jdabinTest = "jpeg_decode_accelerator_unittest"
	if err := chrometest.Run(ctx, s.OutDir(), jdabinTest, testParamList); err != nil {
		s.Fatal(err)
	}
}
