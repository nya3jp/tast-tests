...
const logFileName = "gtest_logs.txt"
const x86ApkName = "c2_e2e_test.apk"
const armApkName = "c2_e2e_test_arm.apk"
...
func waitForFinish(ctx context.Context, a *arc.ARC) error {
...
func runARCVideoTest(ctx context.Context, s *testing.State, md decodeMetadata, cfg arcTestConfig) {
	...
	if err := waitForFinish(ctx, s.PreValue().(arc.PreData).ARC); err != nil {
...
func runARCVideoPerfTest(ctx context.Context, s *testing.State, md decodeMetadata, cfg arcTestConfig) (perf map[string]float64) {
	...
	outLogFile := pullLogsAndCheckPassing(shortCtx, s, cfg.logPrefix)
