...
func init() {
	testing.AddTest(&testing.Test{
		...
		// Although the ability to android is unrelated to this test ability, we would like to run this test to ARC++ enabled boards.
		// TODO(hiroh): Remove "android" deps once Chrome VEAs and Chrome OS supports DMABUF-backed video frame on all boards.
		SoftwareDeps: []string{"android", caps.HWEncodeH264},
...
func EncodeAccelH264NV12Dmabuf(ctx context.Context, s *testing.State) {
	encode.RunAccelVideoTest(ctx, s, encode.TestOptions{Profile: videotype.H264Prof, Params: encode.Bear192P, PixelFormat: videotype.NV12, ExtraArgs: []string{"--native_input"}})
}
