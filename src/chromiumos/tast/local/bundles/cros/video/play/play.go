...
// It starts a new Chrome session for a test and logs out after the test.
// filename is the name of the video file being played.
// videotype represents the type of the given video. If it is MSEVideo, filename is the name of an MPD file.
...
func testPlay(ctx context.Context, s *testing.State, cr *chrome.Chrome,
	filename string, videotype VideoType, mode HistogramMode) {
	ctx, st := timing.Start(ctx, "test_play")
	defer st.End()
...
// It starts a new Chrome session for a test, plays the video and seeks randomly into it 100 times,
// and logs out after the test.
// filename is the name of the video file being played.
