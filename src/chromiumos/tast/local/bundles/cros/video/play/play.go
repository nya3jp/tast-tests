...
// seekVideo plays the video referenced by videoFile while repeatedly and
// randomly seeking into it. It will return an error if seeking did not succeed
// for some reason.
func seekVideo(ctx context.Context, conn *chrome.Conn, videoFile string) error {
	const numSeeks = 100
	const numFastSeeks = 16
