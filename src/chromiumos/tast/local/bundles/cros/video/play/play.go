...
const (
	...
	MSEVideo
	// DRMVideo represents a video requireing Digital Rights Management (DRM).
	DRMVideo
)
...
// mpdFile is the name of MPD file for the video stream.
// url is the URL of the shaka player webpage.
func playDRMVideo(ctx context.Context, s *testing.State, cr *chrome.Chrome, mpdFile, url string) (isHwDrmPipeline bool, err error) {
	...
	if err := screenshot.CaptureChrome(ctx, cr, sshotPath); err != nil {
		return false, errors.Wrap(err, "failed taking screenshot")
	}
	f, err := os.Open(sshotPath)
	if err != nil {
		return false, errors.Wrap(err, "failed opening screenshot image")
	}
	defer f.Close()
	im, err := png.Decode(f)
	if err != nil {
		return false, errors.Wrap(err, "failed decoding screenshot")
	}
	...
	if ratio < 0.95 || !colorcmp.ColorsMatch(color, colorcmp.RGB(0, 0, 0), 0) {
		return false, errors.Errorf("screenshot did not have solid black, instead got %v at ratio %0.2f",
			colorcmp.ColorStr(color), ratio)
	}

	isHwDrmPipeline, err = devtools.CheckHwDrmPipeline(ctx, observer, url)

	return isHwDrmPipeline, err
}
...
func TestPlay(ctx context.Context, s *testing.State, cr *chrome.Chrome,
	...
	var playErr error
	var usesPlatformVideoDecoder = false
	var isHwDrmPipeline = false
	var url string
	switch videotype {
	case NormalVideo:
