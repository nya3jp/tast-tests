...
func isBlack(c color.Color) bool {
	// Note that the RGBA method returns components in the range [0, 65535]
	// (https://blog.golang.org/image).
	cR, cG, cB, cA := c.RGBA()
	return cR == 0 && cG == 0 && cB == 0 && cA == 65535
}
...
func TestPlayAndScreenshot(ctx context.Context, s *testing.State, cr *chrome.Chrome, filename, refFilename string) error {
	...
	for ; top < img.Bounds().Dy(); top++ {
		if c := img.At(xMiddle, top); !isBlack(c) {
			break
		}
	}
	bottom := img.Bounds().Dy() - 1
	for ; bottom >= 0; bottom-- {
		if c := img.At(xMiddle, bottom); !isBlack(c) {
			break
		}
	}
	if bottom <= top {
	...
	for ; left < img.Bounds().Dx(); left++ {
		if c := img.At(left, yMiddle); !isBlack(c) {
			break
		}
	}
	right := img.Bounds().Dx() - 1
	for ; right >= 0; right-- {
		if c := img.At(right, yMiddle); !isBlack(c) {
			break
		}
	}
	if right <= left {
