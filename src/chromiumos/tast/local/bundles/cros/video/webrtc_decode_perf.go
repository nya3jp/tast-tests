...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Measures WebRTC decode performance in terms of CPU usage and decode time with and without hardware acceleration to the performance dashboard",
		Contacts:     []string{"deanliao@chromium.org", "chromeos-video-eng@google.com"},
		Attr:         []string{"group:crosbolt", "crosbolt_perbuild"},
		SoftwareDeps: []string{"chrome_login"},
		Data:         append(webrtc.LoopbackDataFiles(), streamFile, webrtc.AddStatsJSFile),
	})
}
...
func WebRTCDecodePerf(ctx context.Context, s *testing.State) {
	webrtc.RunWebRTCDecodePerf(ctx, s, streamFile)
}
