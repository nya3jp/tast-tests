...
func computeBitstreamQuality(ctx context.Context, yuvFile, outDir string, bitstreams [][]byte,
	...
	if tid == numTemporalLayers-1 {
		bitstreamFile, err = saveBitstream(bitstreams, codec, w, h, framerate, outDir)
		if err != nil {
			return psnr, ssim, errors.Wrap(err, "failed preparing bitstream")
		}
		defer os.Remove(bitstreamFile)
	} else {
		yuvFile, err = prepareYUVFileForTL(ctx, yuvFile, w, h, tid, tids)
		if err != nil {
			return psnr, ssim, errors.Wrap(err, "failed preparing yuv")
		}
		defer os.Remove(yuvFile)

		bitstreamFile, err = saveBitstreamForTL(bitstreams, codec, w, h, framerate, outDir, tid, tids)
		if err != nil {
			return psnr, ssim, errors.Wrap(err, "failed preparing bitstream")
		}
		defer os.Remove(bitstreamFile)
...
// prepareYUVFileForTL creates a file that contains YUV frames whose temporal layer id is not more than tid.
...
func prepareYUVFileForTL(ctx context.Context, yuvFilePath string, w, h, tid int, tids []int) (string, error) {
	...
	planeLen := w*h + 2*((w+1)/2)*((h+1)/2)
	if stat, err := yuvFile.Stat(); err != nil {
		return "", errors.Wrap(err, "failed to getting a YUV file size")
	} else if stat.Size() != int64(planeLen*len(tids)) {
		return "", errors.Errorf("unexpected file size: expected=%d, actual=%d", planeLen*len(tids), stat.Size())
	}
