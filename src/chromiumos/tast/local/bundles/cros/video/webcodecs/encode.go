...
func RunEncodeTest(ctx context.Context, cr *chrome.Chrome, fileSystem http.FileSystem, testArgs TestArgs, videoFile, outDir string) error {
	...
	if err := conn.WaitForExpr(ctx, "TEST.complete()"); err != nil {
		return outputJSLogAndError(cleanupCtx, conn, errors.Wrap(err, "failed executing TEST.complete()"))
	}

	var success bool
	if err := conn.Eval(ctx, "TEST.success()", &success); err != nil {
		return outputJSLogAndError(cleanupCtx, conn, errors.Wrap(err, "failed executing TEST.success()"))
	}
	...
	SSIM, PSNR, err := computeBitstreamQuality(ctx, videoFile, bitstreamFile, outDir, testArgs.Codec, config.width, config.height)
	if err != nil {
