...
func RunEncodeTest(ctx context.Context, cs ash.ConnSource, fileSystem http.FileSystem, testArgs TestEncodeArgs, videoFile, outDir string) error {
	...
	if testArgs.OutOfProcessEnabled {
		...
		if err := multipleEncoders(ctx, cs, fileSystem, testArgs, videoFile, outDir); err != nil {
...
// findVideoEncoderUtilityProcess verifies the utility process that houses
// out-of-process video encoding was created.
...
// multipleEncoders verifies that opening multiple encoders doesn't create
// multiple utility processes for the same renderer.
func multipleEncoders(ctx context.Context, cs ash.ConnSource, fileSystem http.FileSystem, testArgs TestEncodeArgs, videoFile, outDir string) error {
	var crowd720pVideoConfig = videoConfig{width: 1280, height: 720, numFrames: 30, framerate: 30}

	cleanupCtx, server, conn, _, deferFunc, err := prepareWebCodecsTest(ctx, cs, fileSystem, encodeHTML)
	if err != nil {
		return err
	}
	defer deferFunc()
