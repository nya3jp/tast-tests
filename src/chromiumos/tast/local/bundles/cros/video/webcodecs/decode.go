...
func validateMD5s(ctx context.Context, gotMD5s []string, jsonFilePath string) error {
	...
	stdout, stderr, err := testexec.CommandContext(
		...
		validatePath,
		"--exec=cat",
		fmt.Sprintf("--args=%s", md5File.Name()),
		fmt.Sprintf("--metadata=%s", jsonFilePath)).SeparatedOutput(testexec.DumpLogOnError)
	if err != nil {
		output := append(stdout, stderr...)
		testing.ContextLogf(ctx, "md5 validation fails: %s", string(output))
...
func RunDecodeTest(ctx context.Context, cr *chrome.Chrome, fileSystem http.FileSystem, testArgs TestDecodeArgs, jsonFilePath string) error {
	...
	return validateMD5s(ctx, gotMD5s, jsonFilePath)
}
