...
func RunDecodeTest(ctx context.Context, cr *chrome.Chrome, fileSystem http.FileSystem, testArgs TestDecodeArgs, jsonFilePath, outDir string) error {
	...
	defer decodedFrames.Release(ctx)
	...
	decodedFramesLen := 0
	if decodedFrames.Call(ctx, &decodedFramesLen, `function() {return this.length(); }`); err != nil {
		return outputJSLogAndError(cleanupCtx, conn, errors.Wrap(err, "failed getting the number of decoded frames"))
	}
	if decodedFramesLen != numFrames {
		return outputJSLogAndError(cleanupCtx, conn, errors.Errorf("unexpected number of decoded frames, %d", decodedFramesLen))
	}
