...
func saveBitstream(bitstreams [][]byte, codec videotype.Codec, width, height, framerate int, dir string) (string, error) {
	return saveBitstreamForTL(bitstreams, codec, width, height, framerate, dir, 0, []int{})
}

// saveBitstreamForTL saves bitstreams in dir. The saved format is H.264 Annex B format if codec is H264 and IVF file format otherwise.
// If tids is non empty, the saved bitstream represents frames that are in temporal layers up to tid.
func saveBitstreamForTL(bitstreams [][]byte, codec videotype.Codec, width, height, framerate int, dir string, temporalID int, temporalIDs []int) (string, error) {
	...
	for i, b := range bitstreams {
		if len(temporalIDs) > 0 && temporalIDs[i] > temporalID {
			// Skip bitstream that stands for a frame in upper temporal layers than temporalId.
			continue
		}
