...
func ImageProcessor(ctx context.Context, s *testing.State) {
	...
	// Change directory to the data file path before running the test.
	// TODO(nhebert): The file path limitation should be fixed in the
	// image_processor_test binary.
	wd, err := os.Getwd()
	if err != nil {
		s.Fatal("Failed to get working directory: ", err)
	}
	dp := filepath.Dir(s.DataPath("images/bear_320x192.rgba"))
	testing.ContextLogf(
		ctx,
