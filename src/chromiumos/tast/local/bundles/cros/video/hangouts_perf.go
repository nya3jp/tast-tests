...
func init() {
	testing.AddTest(&testing.Test{
		Func:         HangoutsPerf,
		Desc:         "Simulates hangouts performance by simultaneously decoding and encoding a 30fps 1080p video",
		Contacts:     []string{"dstaessens@chromium.org", "chromeos-video-eng@google.com"},
		Attr:         []string{"group:crosbolt", "crosbolt_perbuild"},
		SoftwareDeps: []string{"chrome", caps.HWDecodeVP8},
		Data:         []string{"1080p_30fps_300frames.h264", "1080p_30fps_300frames.h264.json", encode.Crowd1080P.Name},
	})
}

func HangoutsPerf(ctx context.Context, s *testing.State) {
	const (
		...
		decodeFilename = "1080p_30fps_300frames.h264"
		// Pixelformat of video that will be encoded.
		encodePixelFormat = videotype.I420
		// Profile used to encode video.
	...
	// Start the encoder performance test asynchronously.
	encoderArgs := []string{
		encode.CreateStreamDataArg(encodeParams, encodeProfile, encodePixelFormat, streamPath, "/dev/null"),
		"--run_at_fps",
		"--num_frames_to_encode=100000000",
		"--ozone-platform=gbm",
		"--test-launcher-timeout=3600000",
		"--single-process-tests",
	}
	stopEncoderTest, err := startTest(ctx, s,
		"video_encode_accelerator_unittest", "SimpleEncode/*/0", encoderArgs)
	if err != nil {
		s.Fatal("Failed to start video encoder test: ", err)
	}
	defer stopEncoderTest(ctx, s)
	...
	stopDecoderTest, err := startTest(ctx, s,
		"video_decode_accelerator_perf_tests", "*MeasureCappedPerformance", decoderArgs)
	if err != nil {
		s.Fatal("Failed to start video decoder test: ", err)
	}
	defer stopDecoderTest(ctx, s)
	...
	testing.ContextLog(ctx, "CPU usage: ", cpuUsage)
...
func startTest(ctx context.Context, s *testing.State, exec string,
	filter string, args []string) (func(context.Context, *testing.State), error) {
	cmd, err := gtest.New(
