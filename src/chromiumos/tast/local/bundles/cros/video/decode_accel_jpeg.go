...
// runAccelJPEGTest runs the jpeg_decode_accelerator_unittest binary.
// jpegTestFiles should contain the list of all files required by the test
// binary.
func runAccelJPEGTest(ctx context.Context, s *testing.State, name string, jpegTestFiles []string) {
...
func DecodeAccelJPEG(ctx context.Context, s *testing.State) {
	runAccelJPEGTest(ctx, s, "DecodeAccelJpeg", jpegTestFiles)
}
