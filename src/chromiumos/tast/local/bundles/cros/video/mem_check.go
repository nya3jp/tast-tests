...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			ExtraSoftwareDeps: []string{"amd64_video_overlays", caps.HWDecodeH264, "chrome_internal"}, // "chrome_internal" is needed because H.264 is a proprietary codec.
			...
			ExtraSoftwareDeps: []string{"amd64_video_overlays", caps.HWDecodeVP8},
			Pre:               pre.ChromeVideoWithGuestLogin(),
			Timeout:           10 * time.Minute,
		}, {
			Name:              "vp9_hw",
			...
			ExtraSoftwareDeps: []string{"amd64_video_overlays", caps.HWDecodeVP9},
			Pre:               pre.ChromeVideoWithGuestLogin(),
			Timeout:           10 * time.Minute,
		}},
	})
