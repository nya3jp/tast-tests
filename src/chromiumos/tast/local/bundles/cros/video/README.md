...
- The test videos should consist of a single still image so that it's not
important the exact moment at which we capture its pixels.
- We should have cases where the video's visible size is the same as its coded
size (e.g., a 1280x720 H.264) and cases where it's not (e.g., a 640x360 H.264).
- In cases where the visible size is not the same as the coded size, the
non-visible area should contain a color that's unexpected by the test so that
certain cases of incorrect cropping can be detected.
- We should have a case where the visible rectangle does not start at (0, 0).
These videos are not expected to be common in the wild, but the H.264
specification allows such exotic crops.
- The videos should be long enough that we don't have to worry about the
possibility of capturing an empty frame when the video loops.

For the curious, the following script was used to generate the test videos:
...
# Given an image input_file, generates an H.264 video of size width x height
# which lasts 30 seconds, removes the EXIF metadata, and saves it as
# output_file.
generate_video() {
  width=$1
  height=$2
...
# Similar to generate_video(), but we get to specify the H.264 crop rectangle.
...
generate_image 640 360 640 360 0 0 still-colors-360p.bmp
generate_cropped_video 640 368 0 0 8 0 still-colors-360p.bmp \
  still-colors-360p.h264.mp4
overwrite_image_height still-colors-360p.h264.mp4 360

generate_image 854 480 854 480 0 0 still-colors-480p.bmp
generate_cropped_video 864 480 0 10 0 0 still-colors-480p.bmp \
  still-colors-480p.h264.mp4
overwrite_image_width still-colors-480p.h264.mp4 854

generate_image 1280 720 1280 720 0 0 still-colors-720p.bmp
generate_video 1280 720 still-colors-720p.bmp \
  still-colors-720p.h264.mp4

generate_image 1920 1080 1920 1080 0 0 still-colors-1080p.bmp
generate_cropped_video 1920 1088 0 0 8 0 still-colors-1080p.bmp \
  still-colors-1080p.h264.mp4
...
# 640x360. Chrome should render this just like the still-colors-360p.h264.mp4
# generated above.

generate_image 720 480 640 360 20 30 still-colors-720x480.bmp
generate_cropped_video 720 480 30 60 90 20 still-colors-360p.bmp \
