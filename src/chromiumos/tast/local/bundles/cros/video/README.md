...
superseding the older [`video_encode_accelerator_unittest`]).
...
## Video encoder integration tests (`video.EncodeAccelNew`)
...
### Video encoder performance tests (`video.EncodeAccelNewPerf`)

Similarly, `video.EncodeAccelNewPerf` measure Chrome's video encode stack
