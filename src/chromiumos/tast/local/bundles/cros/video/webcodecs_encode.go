...
const mp4DemuxerJS = "third_party/mp4/mp4_demuxer.js"
const mp4BoxJS = "third_party/mp4/mp4box.all.min.js"
const mp4BoxJSMap = "third_party/mp4/mp4box.all.min.js.map"
...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:         []string{encodeHTML, commonJS, mp4DemuxerJS, mp4BoxJS, mp4BoxJSMap, crowd720pMP4},
...
func RunEncodeTest(ctx context.Context, cr *chrome.Chrome, fileSystem http.FileSystem, args testArgs, videoFile, outDir string) error {
	...
	if err := conn.Call(ctx, nil, "EncodeAndSave", codec, args.acceleration, config.width, config.height, bitrate, config.framerate, config.numFrames, server.URL+"/"+crowd720pMP4); err != nil {
		var logs string
