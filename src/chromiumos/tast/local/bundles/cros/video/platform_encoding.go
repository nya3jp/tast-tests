...
// regExpFPSH264is the regexp to find the FPS output from the H.264 binary log.
...
func vp8args(exe, yuvFile string, size coords.Size) (command []string, ivfFile string) {
	...
	// From Chromecast, corresponds to a bit more than 0.1 BPP.
...
// h264args constructs the command line for the VP9 encoding binary exe.
func h264args(exe, yuvFile string, size coords.Size) (command []string, h264File string) {
	...
	command = append(command, "--initialqp", "26" /* Quality Parameter */)
