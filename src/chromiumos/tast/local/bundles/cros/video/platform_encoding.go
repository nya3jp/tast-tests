...
func argsOpenh264enc(ctx context.Context, testName, exe, yuvFile string, size coords.Size, fps int) (command []string, h264File string, bitrate int, _ error) {
	...
	// "-org" for origin and "-bf" for bistream (output) files.
	...
	command = append(command, "-minqp", "24", "-maxqp", "37" /* Quality Parameter */)

	// RC_BITRATE_MODE (1) would not work with openh264enc when feeding the
	// input file at maximum speed and not a the frame rate. Use
	...
	//command = append(command, "-maxbrTotal", strconv.Itoa(int(1.5 * float64(bitrate))) /* bps */)
