...
// vpxencArgs constructs the command line for vpxenc.
func vpxencArgs(ctx context.Context, testName, exe, yuvFile string, size coords.Size, fps int) (command []string, ivfFile string, bitrate int, _ error) {
	command = append(command, exe, "-w", strconv.Itoa(size.Width), "-h", strconv.Itoa(size.Height))
	...
	if strings.Contains(testName, strings.ToLower(videotype.VP8)) {
		command = append(command, "--codec=vp8")
		command = append(command, "--undershoot-pct=100", "--overshoot-pct=15")
	} else if strings.Contains(testName, strings.ToLower(videotype.VP9)) {
		command = append(command, "--codec=vp9")
		command = append(command, "--undershoot-pct=50", "--overshoot-pct=50")
	} else {
		return nil, "", 0, errors.Wrap(err, "This function only supports VP8 and VP9")
	}
