...
func PlatformEncoding(ctx context.Context, s *testing.State) {
	...
	keyFrames, err := countKeyFrames(ctx, encodedFile)
	if err != nil {
...
func argsAomenc(ctx context.Context, testName, exe, yuvFile string, size coords.Size, fps int) (command []string, ivfFile string, bitrate int, err error) {
	...
	rtcOptions := []string{
		...
		"--min-q=2", "--max-q=52", // Min and max quantizer value [0..63].
	...
	bitrate = int(0.07 * 0.07 /* BPP */ * float64(fps) * float64(size.Width) * float64(size.Height))
...
func countKeyFrames(ctx context.Context, file string) (count int, err error) {
	cmd := []string{"ffprobe", "-show_frames", "-show_entries", "frame=pict_type", file}
	testing.ContextLogf(ctx, "Running: %s", shutil.EscapeSlice(cmd))

	ffprobeOut, err := testexec.CommandContext(ctx, cmd[0], cmd[1:]...).Output(testexec.DumpLogOnError)
	if err != nil {
		return 0, errors.Wrap(err, "failed to run ffprobe")
	}

	cmd = []string{"grep", "-c", "pict_type=I"}
	command := testexec.CommandContext(ctx, cmd[0], cmd[1:]...)
	command.Stdin = strings.NewReader(string(ffprobeOut[:]))

	grepOut, err := command.Output(testexec.DumpLogOnError)
	if err != nil {
