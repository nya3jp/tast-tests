...
//
// Note that "num_fragments" in the json file is not probably correct for H264
// streams. The value needs to be found by running video_decode_accelerator_tests.
package main
...
func parseStream(file string) (width, height, bitDepth, frameRate int, prof Profile, err error) {
	...
	if len(jsonData.Streams) != 1 {
		err = errors.New("ffprobe detects multiple or no video streams")
		return
	}
	...
	fpsNum, err := strconv.ParseInt(fpsFrac[0], 10, 32)
	if err != nil {
		return
	}

	fpsDenom, err := strconv.ParseInt(fpsFrac[1], 10, 32)
	if err != nil {
		return
	}

	frameRate = int(math.Round(float64(fpsNum) / float64(fpsDenom)))
	width = stream.Width
...
func computeFileCheckSum(file string) (string, error) {
	...
	if _, err := io.Copy(hash, f); err != nil {
		return "", errors.Wrap(err, "computing md5")
	}
...
func genJSONInfo(file string) (JSONInfo, error) {
	...
	info.File = file
	info.FileChecksum, err = computeFileCheckSum(file)
	if err != nil {
		return info, err
	}
...
func main() {
	for _, file := range os.Args[1:] {
		...
		if ext != ".ivf" && ext != ".h264" {
			fmt.Fprintf(os.Stderr, "The file's extension must be ivf or h264:", file)
			continue
		}
