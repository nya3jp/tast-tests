...
const (
	// Duration of the interval during which CPU usage will be measured in
...
// - Capped performance: the specified test video is encoded for 300 frames by the hardware encoder in 30 fps rate.
...
func RunNewAccelVideoPerfTest(ctxForDefer context.Context, s *testing.State, opts TestOptionsNew) {
	...
	// Test 1: Measure maximum FPS and the qualities of the encoded bitstream.
	testArgs := []string{
		fmt.Sprintf("--codec=%s", codec),
