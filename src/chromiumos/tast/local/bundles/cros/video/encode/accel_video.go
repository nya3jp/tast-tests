...
func RunARCVideoTest(ctx context.Context, s *testing.State, opts TestOptions) {
	...
	for _, abi := range []string{"amd64", "x86", "arm"} {
		...
		out, err := a.Command(ctx, "ls", exec).Output()
		if err == nil && strings.Compare(strings.TrimSpace(string(out)), exec) == 0 {
			execs = append(execs, exec)
		}
	}
	if len(execs) == 0 {
		s.Fatal("Test binary is not found.")
	}

	// Execute binary at ARC.
	for _, exec := range execs {
		s.Log("Running: ", exec, strings.Join(args, " "))
