...
type TestOptions struct {
	...
	// If true, nv12 test cases runs (e.g. FlushAtEndOfStream_NV12DmabufScaling) that are disabled by default.
	// TODO(hiroh): Remove this and runs nv12 test cases by default if the dashboard is green.
	RunNV12Test bool
}

// MakeTestOptions creates TestOptions from webMName and profile.
// TemporalLayers is set to 1 and RunNV12Test is set to false.
...
// TemporalLayers is set to 1 and RunNV12Test is set to true.
...
func RunAccelVideoTest(ctxForDefer context.Context, s *testing.State, opts TestOptions) {
	...
	if opts.RunNV12Test {
		// Run NV12 test cases only.
		testArgs = append(testArgs, fmt.Sprintf("--run_nv12_tests"))
	...
	t := gtest.New(exec, gtest.Logfile(logfile), gtestFilter,
