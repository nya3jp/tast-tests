...
func runARCVideoTest(ctx context.Context, s *testing.State, opts TestOptions, ba binArgs) {
	...
	for _, exec := range execs {
		...
		out, err := os.Create(filepath.Join(s.OutDir(), filepath.Base(exec)+".log"))
		if err != nil {
			s.Errorf("Failed to create output log file: ", err)
			continue
		}
		...
		if err := out.Close(); err != nil {
			s.Errorf("Failed to close output log file: ", err)
			continue
		}
		...
		if err != nil {
			s.Errorf("Failed to read file %v: %v", xmlOutPath, err)
			continue
		}
