...
func runARCBinary(ctx context.Context, a *arc.ARC, exec string, args []string, outDir string) (funcErr error) {
	...
	defer func() {
		if err := out.Close(); err != nil && funcErr == nil {
			funcErr = errors.Wrap(err, "failed to close output log file")
		}
	}()

	testing.ContextLogf(ctx, "Running %v %v", exec, strings.Join(execArgs, " "))
