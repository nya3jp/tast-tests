...
func runAccelVideoTest(ctx context.Context, s *testing.State, mode testMode, opts TestOptions, bas ...binArgs) {
	...
	for _, ba := range bas {
		...
		if ba.measureUsage {
			...
			if ok {
				// TODO(akahuang): Don't write power consumption to disk, as this can increase test flakiness.
				powerLogPath := filepath.Join(s.OutDir(), powerLog)
				str := fmt.Sprintf("%f", powerConsumption)
				if err := ioutil.WriteFile(powerLogPath, []byte(str), 0644); err != nil {
...
func RunAccelVideoPerfTest(ctx context.Context, s *testing.State, opts TestOptions) {
	const (
		...
		// duration of the interval during which CPU usage and power consumption ill be measured.
