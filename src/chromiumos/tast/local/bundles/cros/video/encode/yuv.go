...
func PrepareYUV(ctx context.Context, webMFile string, pixelFormat videotype.PixelFormat, w, h int) (string, error) {
	...
	if pixelFormat == videotype.NV12 {
		nv12File, err := createNV12FileFromI420File(yuvFile, w, h)
		if err != nil {
			os.Remove(yuvFile)
...
func convertI420ToNV12(i420 []byte, w, h int, numFrames int) []byte {
	yLeng := w * h
	uvLeng := w * h / 2
	...
	for i := 0; i < numFrames; i++ {
		...
		// Write Y Plane as-is.
		copy(nv12[offset:offset+yLeng], i420[offset:offset+yLeng])
...
func createNV12FileFromI420File(i420File string, w, h int) (string, error) {
	...
	n, err := nv12File.Write(convertI420ToNV12(i420, w, h, numFrames))
	if err != nil {
		os.Remove(nv12File.Name())
		return "", err
	}
