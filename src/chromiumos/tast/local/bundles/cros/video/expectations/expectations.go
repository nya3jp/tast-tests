...
const (
	...
	// BuildFile files have the format "build-<DUT build>.yml"
	BuildFile FileType = "build"
...
// BuildStringToBoard returns the board string from a build string.
...
// | trogdor64          | trogdor      |
func BuildStringToBoard(variant string) string {
...
func OpenTestExpectationsWithDirectory(ctx context.Context, s *testing.State, testExpectationsDirectory string) (*os.File, FileType, error) {
	...
	// base_directory/build-<build>.yml
	// base_directory/board-<board>.yml
	// base_directory/platform-<platform>.yml
	for _, fileType := range []FileType{ModelFile, BuildFile, BoardFile, PlatformFile} {
