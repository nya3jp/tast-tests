...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Ensures that cryptohome correctly creates, authenticate and mount with an Auth session",
		Contacts: []string{
...
// AuthSessionMount ensures that an AuthSession can be used for mounting purposes.
...
func AuthSessionMount(ctx context.Context, s *testing.State) {
	const (
		testUser = "cryptohome_autha_session_test@chromium.org"
		testPass = "testme"
	)
	...
	if err != nil {
		s.Error("Failed to add credentials with Auth session: ", err)
		return
	}
