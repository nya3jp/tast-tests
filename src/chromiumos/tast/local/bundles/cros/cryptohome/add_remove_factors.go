...
func compareReplyToExpectations(when string, reply *uda.ListAuthFactorsReply, expectedConfigured []expectedConfiguredFactor, expectedSupported []uda.AuthFactorType, s *testing.State) {
	// Compare the configured and expected configured factors. We do this by reducing the configured factors in the reply to a
	// list of expectedConfiguredFactor instances, so that we can do a direct diff of the two lists (actual vs expected). Order
	// does not matter.
	...
	actualConfigured := make([]expectedConfiguredFactor, len(reply.ConfiguredAuthFactors))
	for i, configured := range reply.ConfiguredAuthFactors {
		actualConfigured[i].Type = configured.Type
		actualConfigured[i].Label = configured.Label
	}
