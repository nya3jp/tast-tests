...
func RemoveCredentialAuthSession(ctx context.Context, s *testing.State) {
	...
	// Step 8: Unmount everything. We will remount with the updated password credentials.
	if err := client.UnmountAll(ctx); err != nil {
		s.Fatal("Failed to unmount vaults for re-mounting: ", err)
	}

	// Step 9: Check we can use login with pin. This also checks if wrong pin attempt fails.
	...
	// Step 11: Check key should fail after remove, even when AuthSession was started with pin keyset.
	...
	// Step 12: Remove pin key, and test everything is alright after removing the key.
	if err := client.UnmountAll(ctx); err != nil {
		s.Fatal("Failed to unmount vaults for re-mounting: ", err)
	}

	// Step 13: Check we can use login with pin. This should fail.
	authSessionID, err = client.StartAuthSession(ctx, userName /*isEphemeral*/, false)
	if err != nil {
		s.Fatal("Failed to start Auth session: ", err)
	}
	...
	err = client.AuthenticateAuthSession(ctx, userPassword, keyLabel, authSessionID /*kiosk_mount=*/, false)
