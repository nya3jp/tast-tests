...
func init() {
	testing.AddTest(&testing.Test{
		Func:         Login,
		Desc:         "Verify the cryptohome is mounted only after login.",
...
func Login(ctx context.Context, s *testing.State) {
	...
	if err := cryptohome.CreateVault(ctx, testUser, testPass); err != nil {
		s.Fatalf("Failed to create a vault for the test user: ", err)
	}
	...
	func() {
		...
		if err != nil {
			s.Fatal("Failed to log in by Chrome: %v", err)
		}
		defer cr.Close(ctx)
	...
	if err := upstart.RestartJob(ctx, "ui"); err != nil {
		s.Fatalf("Failed to log out: ", err)
	}
