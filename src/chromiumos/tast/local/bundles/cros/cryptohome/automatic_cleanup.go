...
func AutomaticCleanup(ctx context.Context, s *testing.State) {
	...
	createCacheDirWithContent := func(ctx context.Context, user, pass, dir string, size uint64) error {
	...
	if err := createCacheDirWithContent(ctx, user1, password, "Cache", homedirSize); err != nil {
		s.Fatal("Failed to create user with content: ", err)
	}
	defer cryptohome.RemoveVault(ctx, user1)

	if err := createCacheDirWithContent(ctx, user2, password, "Cache", homedirSize); err != nil {
		s.Fatal("Failed to create user with content: ", err)
	}
