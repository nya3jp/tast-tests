...
func AutomaticCleanupManyUsers(ctx context.Context, s *testing.State) {
	...
	// Free space at the beginning of test.
	startingFreeSpace := disk.MinimalFreeSpace - uint64(userCount)*homedirSize
	...
	for i := 1; i <= userCount; i++ {
		...
		fillFile, err := disk.CreateFilledUserHomedir(userCreationCtx, user, password, "Cache", homedirSize)
		if err != nil {
	...
	if err := disk.RunAutomaticCleanup(automaticCleanupCtx); err != nil {
