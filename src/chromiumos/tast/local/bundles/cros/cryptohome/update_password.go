...
func UpdatePassword(ctx context.Context, s *testing.State) {
	...
	if !errors.As(err, &authExitErr) {
		s.Fatalf("Unexpected error for authentication with password: got %q; want *hwsec.CmdExitError", err)
	}
	if authExitErr.ExitCode != cryptohomeAuthorizationKeyFailed {
		s.Fatalf("Unexpected exit code for authentication with password: got %d; want %d", authExitErr.ExitCode, cryptohomeAuthorizationKeyFailed)
	}
