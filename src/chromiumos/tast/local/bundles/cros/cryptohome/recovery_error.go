...
func RecoveryError(ctx context.Context, s *testing.State) {
	...
	if exitErr.ExitCode != (int)(uda.CryptohomeErrorCode_CRYPTOHOME_ERROR_RECOVERY_TRANSIENT) {
		s.Fatalf("Unexpected exit code in authentication: got %d; want %d",
			exitErr.ExitCode, uda.CryptohomeErrorCode_CRYPTOHOME_ERROR_RECOVERY_TRANSIENT)
	}
	...
	if exitErr.ExitCode != (int)(uda.CryptohomeErrorCode_CRYPTOHOME_ERROR_RECOVERY_FATAL) {
		s.Fatalf("Unexpected exit code in authentication: got %d; want %d",
			exitErr.ExitCode, uda.CryptohomeErrorCode_CRYPTOHOME_ERROR_RECOVERY_FATAL)
	}
}
