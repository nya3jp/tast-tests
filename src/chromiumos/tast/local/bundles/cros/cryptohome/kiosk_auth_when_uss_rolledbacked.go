...
func init() {
	testing.AddTest(&testing.Test{
		Func: KioskAuthWhenUSSRolledbacked,
		Desc: "Ensures that cryptohome correctly mounts kiosk sessions through AuthFactor APIs if UserSecretStash is rolledbacked",
		Contacts: []string{
			"betuls@google.com",
...
// KioskAuthWhenUSSRolledbacked tests the following case:
// User Created with AuthFactor API and USS:
// Ensure that the user can login with AuthFactor APIs after USS disabled.
func KioskAuthWhenUSSRolledbacked(ctx context.Context, s *testing.State) {
	const (
	...
	// Authenticate a new AuthSession, create the user, mount the vault
	// and add kiosk factor.
	...
	if err := helper.DisableUserSecretStash(ctx); err != nil {
		s.Fatal("Failed to diable the UserSecretStash experiment: ", err)
	}

	// Authenticate a new auth session should succeed with backup legacy keysets.
