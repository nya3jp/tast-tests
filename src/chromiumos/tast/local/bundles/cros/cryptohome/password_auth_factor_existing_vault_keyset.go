...
// PasswordAuthFactorExistingVaultKeyset tests the AuthFactor API flows when
...
func PasswordAuthFactorExistingVaultKeyset(ctx context.Context, s *testing.State) {
	...
	// 2. Enable the UserSecretStash experiment for the duration of the test by
	...
	// 3. Add a PIN credential to the existing user after UserSecretStash is enabled.
	// PIN VaultKeyset should be added and UserSecretStash shouldn't be created.
	//
	// Start an Auth session and get an authSessionID.
	authSessionID, err = client.StartAuthSession(ctx, userName, false /*ephemeral*/, uda.AuthIntent_AUTH_INTENT_DECRYPT)
	if err != nil {
	...
	// 4. Update and then remove VaultKeyset backed PIN credential when UserSecretStash is enabled.
	// Both update and remove operations should operate with the PIN VaultKeyset, and UserSecretStash shouldn't be created.
	//
	// Start an Auth session and get an authSessionID.
	authSessionID, err = client.StartAuthSession(ctx, userName, false /*ephemeral*/, uda.AuthIntent_AUTH_INTENT_DECRYPT)
	if err != nil {
	...
	if err := client.InvalidateAuthSession(ctx, authSessionID); err != nil {
		s.Fatal("Failed to invaldiate AuthSession: ", err)
	}
	...
	if _, err := client.AuthenticatePinAuthFactor(ctx, authSessionID, pinLabel, userPin); err == nil {
		s.Fatal("Authenticating with the old PIN suceeded but should have failed: ", err)
	}
	...
	// Authenticate with PIN should now failed.
	if _, err := client.AuthenticatePinAuthFactor(ctx, authSessionID, pinLabel, userPinNew); err == nil {
		s.Fatal("Authenticating with PIN should have failed after removal, but suceeded: ", err)
	}
}
