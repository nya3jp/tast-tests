...
func PasswordAuthFactorExistingVaultKeyset(ctx context.Context, s *testing.State) {
	...
	// 4. Update AuthFactor and then remove AuthFactor VaultKeyset backed credential when UserSecretStash is enabled.
	// Both update and remove operations should operate with the VaultKeyset, and UserSecretStash shouldn't be created.
	//
	// Start an Auth session and get an authSessionID.
	_, authSessionID, err = client.StartAuthSession(ctx, userName, false /*ephemeral*/, uda.AuthIntent_AUTH_INTENT_DECRYPT)
	if err != nil {
