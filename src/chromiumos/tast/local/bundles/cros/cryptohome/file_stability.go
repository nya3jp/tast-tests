...
// keepGoingError is a fake error type used to keep testing.Poll from returing
...
func FileStability(ctx context.Context, s *testing.State) {
	...
	fileContents := []string{
		...
		"This is a long file ", // More text is added below
		"\000control characters\001",
	}

	// Make one file reasonably long
	for i := 0; i < 10000; i++ {
		fileContents[2] += strconv.Itoa(i)
	}
	...
	// chrome.New should promise we only have 1 active session.
	if len(sessions) != 1 {
