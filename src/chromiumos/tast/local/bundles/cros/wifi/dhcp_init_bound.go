...
func DHCPInitBound(ctx context.Context, s *testing.State) {
	...
	rule1 := dhcp.NewRespondToDiscovery(
		intendedIP.String(), gatewayIP.String(),
		dhcp.GetBaseOptionMap(subnet), dhcp.FieldMap{}, true /*shouldRespond*/)
	rule2 := dhcp.NewRespondToRequest(
		intendedIP.String(), gatewayIP.String(),
		dhcp.GetBaseOptionMap(subnet), dhcp.FieldMap{},
		true /*shouldRespond*/, gatewayIP.String(), intendedIP.String(), true /*expSvrIPSet*/)
	rule2.SetIsFinalHandler(true)

	verifyFunc, err := dhcp.StartTestServerInEnv(ctx, wifi.Router, subnet, gatewayIP, []dhcp.HandlingRule{*rule1, *rule2})
	if err != nil {
		s.Fatal("Failed to start DHCP tests server: ", err)
	}
	...
	if err := wifi.Service.WaitForConnectedOrError(ctx); err != nil {
		s.Error("Failed to wait for WiFi service be connected: ", err)
	}

	// testing.Sleep(ctx, 15*time.Minute)

	if err := verifyFunc(); err != nil {
		s.Fatal("Failed to stop and verify the DHCP test server: ", err)
	}
	...
	verifyFunc, err = dhcp.StartTestServerInEnv(ctx, wifi.Router, subnet, gatewayIP, []dhcp.HandlingRule{*rule1, *rule2})
	if err != nil {
		s.Fatal("Failed to start DHCP server: ", err)
	}
