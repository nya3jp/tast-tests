		...
		Params: []testing.Param{
			...
			},
			{
				...
				Val: wifiInShillByCmdTestCase{
					...
					props: map[string]string{
						...
						shillconst.IPConfigPropertyDomainName:  "test1.com,test2.com",
					},
				},
			},
			{
			...
			},
			{
				...
				Val: wifiInShillByCmdTestCase{
					...
					props: map[string]string{
						shillconst.ServicePropertyAutoConnect:               "false",
						shillconst.IPConfigPropertyManualProxyConfiguration: "true",
					},
				},
			},
		},
...
func CreateWifiInShillByCmd(ctx context.Context, s *testing.State) {
	...
	reg := regexp.MustCompile(`.*Save failed.*`)
	if out != nil && reg.Match(out) {
		...
		for prop := range tc.props {
			...
			case shillconst.ServicePropertyAutoConnect:
				...
				if strconv.FormatBool(b) != tc.props[prop] {
					s.Errorf("autoreconnect = %t, want: %t", strconv.FormatBool(b), tc.props[prop])
				}
			...
			case shillconst.IPConfigPropertyManualProxyConfiguration:
				proxy, err := p.Get("ProxyConfig")
				if err != nil {
					s.Error("Failed to get proxy config property from service: ", err)
				}
			...
			case shillconst.IPConfigPropertyDomainName:
				...
				if !reflect.DeepEqual(expected, searchDomains.([]string)) {
					s.Errorf("searchDomains is: %+v, however %+v is expected", searchDomains.([]string), expected)
				}
			}
		}
	}
