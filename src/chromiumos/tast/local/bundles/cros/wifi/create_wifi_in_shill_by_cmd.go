		...
		Params: []testing.Param{
			...
			},
			{
				...
				Val: wifiInShillByCmdTestCase{
					...
					props: map[string]interface{}{
						shillconst.ServicePropertyAutoConnect:  false,
						shillconst.IPConfigPropertyProxyConfig: true,
					},
				},
			},
		},
	...
	for _, service := range services {
		...
		for prop := range tc.props {
			...
			case shillconst.ServicePropertyAutoConnect:
				...
				if b != tc.props[prop].(bool) {
					s.Errorf("autoreconnect = %v, want: %v", b, tc.props[prop].(bool))
				}
			case shillconst.IPConfigPropertyWebProxyAutoDiscoveryURL:
				proxy, err := p.Get(shillconst.IPConfigPropertyProxyConfig)
				if err != nil {
			...
			case shillconst.IPConfigPropertyProxyConfig:
				proxy, err := p.Get(shillconst.IPConfigPropertyProxyConfig)
				if err != nil {
					s.Error("Failed to get proxy config property from service: ", err)
				}
