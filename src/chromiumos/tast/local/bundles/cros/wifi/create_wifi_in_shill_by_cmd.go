...
// runAddNetworkTestCases expects a wifi with given configs are created correctly through shell cmd.
func runAddNetworkTestCases(ctx context.Context, s *testing.State, m *shill.Manager, a *arc.ARC, tc createWifiInShillByCmdTestCase) (retErr error) {
	...
	for _, service := range services {
		...
		decodeSSID, _ := hex.DecodeString(hexSSID)
		curSSID := string(decodeSSID)

		// If current service if not newly added wifi network, then continue
		if curSSID != tc.SSID {
			continue
		}
