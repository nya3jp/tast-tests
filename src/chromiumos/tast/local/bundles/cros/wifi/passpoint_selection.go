...
func PasspointSelection(ctx context.Context, s *testing.State) {
	...
	profile, err := passpoint.CreateFakeUserProfile(ctx, tc.manager, profileName)
	if err != nil {
		s.Fatal("Failed to create test profile: ", err)
	}
	defer func(ctx context.Context) {
		// Remove the test profile
		if err := passpoint.RemoveFakeUserProfile(ctx, tc.manager, profileName); err != nil {
	...
	defer passpoint.SetInterworkingSelectEnabled(cleanupCtx, tc.manager, tc.clientIface, false)
