...
func (pc *Credentials) ToProperties() map[string]interface{} {
	...
	if len(pc.HomeOIs) > 0 {
		...
		for _, oi := range pc.HomeOIs {
			ois = append(ois, fmt.Sprintf("%d", oi))
		}
...
func CreateFakeUserProfile(ctx context.Context, m *shill.Manager, name string) (path dbus.ObjectPath, err error) {
	...
	// Remove the current user profile if any
	err = m.PopAllUserProfiles(ctx)
	if err != nil {
	...
	defer func() {
		if err != nil {
			if err2 := os.RemoveAll(profilePath); err2 != nil {
				testing.ContextLogf(ctx, "Failed to remove %s: %v", profilePath, err2)
			}
		}
	}()
	err = os.Chown(fakeProfilesDir, int(uid), int(gid))
	if err != nil {
		return "", errors.Wrap(err, "failed to chown")
	}
	err = os.Chown(profilePath, int(uid), int(gid))
	if err != nil {
		return "", errors.Wrap(err, "failed to chown")
	}
	...
	defer func() {
		if err != nil {
			if err2 := m.RemoveProfile(ctx, profileName); err2 != nil {
				testing.ContextLogf(ctx, "Failed to remove profile %s: %v", profileName, err2)
			}
		}
	}()
