...
type Credentials struct {
	// Domain represents the domains of the compatible service providers.
	Domains []string
...
%s        </Node>`, homeOiNodes)
	...
	if len(pc.OtherHomePartners()) > 0 {
		var otherHomePartnersNodes string
		for i, partner := range pc.OtherHomePartners() {
			otherHomePartnersNodes += fmt.Sprintf(`          <Node>
            <NodeName>x%d</NodeName>
            <Node>
