...
const driverName = "mac80211_hwsim"

// load loads the driver on the device and asks for 'radios' interfaces.
// Returns the list of network interfaces that belong to the driver.
func load(ctx context.Context, radios int) ([]net.Interface, error) {
	var ifaces []net.Interface

	// Load the driver
	cmd := testexec.CommandContext(ctx, "modprobe", driverName, fmt.Sprintf("radios=%d", radios))
...
func unload(ctx context.Context) error {
	cmd := testexec.CommandContext(ctx, "modprobe", "-r", driverName)
	if err := cmd.Run(); err != nil {
...
func isLoaded() (bool, error) {
	path := filepath.Join("/sys/module", driverName)
	var err error
...
func belongsToDriver(iface string) bool {
	...
	return strings.Contains(module, driverName)
}
