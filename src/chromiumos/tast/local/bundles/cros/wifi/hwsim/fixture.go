...
func (f *fixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	var shillIfaces []string
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		ifaces, err := wm.Interfaces(ctx)
		if err != nil {
			return errors.Wrap(err, "failed to list Wi-Fi interfaces from Shill")
		}
		if len(ifaces) == 0 {
			return errors.New("Shill has no Wi-Fi interfaces")
		}
		// wlan1 and wlan2 might already been blocked by Shill startup
		// configuration thus we can't expect the exact number of simulated
		// interfaces.
		if len(ifaces) < ifaceCount-2 {
			return errors.New("Shill has not enough Wi-Fi interfaces")
		}
		shillIfaces = ifaces
		return nil
	}, &testing.PollOptions{Timeout: shillIfaceTimeout}); err != nil {
	...
	for _, iface := range shillIfaces {
		testing.ContextLogf(ctx, "Shill interface: %s", iface)
	}
