...
// return a nil table and and an invalid version number alongside the error itself.
...
func verifySARTableDomainType(values []int64, name string) error {
	domainType := int(values[0])
	if domainType != 0x07 {
		// Domain Type must be 0x07 (WiFi Core).
		return errors.Errorf("table %v: invalid Domain Type 0x%02X", name, domainType)
	}
	return nil
}
...
func getGeoSARTablesFromASL(data []byte) ([]geoSARTable, error) {
	...
	if err := verifySARTableDomainType(values, "WGDS"); err != nil {
		return nil, err
	}
	...
	for i := 0; i < 3; i++ {
		...
		if version == 2 {
			t := &currentTable
			t.max6g = values[start+6]
			t.chainAOffset6g = values[start+7]
			t.chainBOffset6g = values[start+8]
		}
...
func getSARTableFromASL(data []byte, tableType sarTableType) ([]int64, error) {
	...
	if err := verifySARTableDomainType(values, tableName); err != nil {
		return nil, err
	}
	if version == 2 {
		switch tableType {
