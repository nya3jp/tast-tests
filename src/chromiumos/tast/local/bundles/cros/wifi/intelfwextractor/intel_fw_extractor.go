...
// The following are the mandaotry files that must exists in the fw dump
...
func ValidateFWDump(ctx context.Context, file string) error {
	...
	if fwDumpHeader.Magic == yoyoMagic {
		...
		if err := validateNonYoYoMagicFWDump(fwDumpBuffer); err != nil {
			return err
		}
	} else {
	...
	if !csr || !monitor || !dccmLmac {
		return errors.Errorf("One of the mandatory files id messing: got {csr: %t, monitor: %t, dccm_lmac: %t}, want {csr: True, monitor: True, dccm_lmac: True}", csr, monitor, dccmLmac)
	}
...
func validateNonYoYoMagicFWDump(fwDumpBuffer *bytes.Buffer) error {
