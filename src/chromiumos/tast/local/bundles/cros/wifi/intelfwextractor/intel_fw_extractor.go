...
func ValidateFWDump(ctx context.Context, file string) error {
	...
	if fwDumpHeader.Magic == yoyoMagic {
		...
		if err := validateYoYoMagicFWDump(ctx, fwDumpBuffer); err != nil {
			return err
		}
...
func validateYoYoMagicFWDump(ctx context.Context, fwDumpBuffer *bytes.Buffer) error {
	// Parse the fw dump.
	tlHeader := TLHeader{}
	...
	for fwDumpBuffer.Len() > 0 {
		...
		if tlHeader.Length > 0 {
			...
			if tlHeader.Type == dumpInfoType {
				// If the header is dumpInfoType then it shoud have the Region IDs for the
