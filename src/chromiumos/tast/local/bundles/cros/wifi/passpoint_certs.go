...
func init() {
	// Test to reproduce Passpoint provisioning interaction of certificates
	// and keys from ARC. It is expected for certificates and keys to be
	// added to the TPM once the Passpoint credentials is provisioned.
...
func PasspointCerts(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < 2; i++ {
		...
		// Ensure that a certificate and a private key is added. It can
		// be racy to wait for the certificates and keys without a poll.
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			...
			if len(aObjs) != 2 {
				return errors.Errorf("len(aObjs) = %d, want 2", len(aObjs))
			}
			...
			s.Fatal("Failed to ensure correct certificate and key added: ", err)
		}
		...
		if err := a.Command(ctx, "cmd", "wifi", "remove-passpoint-config", fqdn).Run(testexec.DumpLogOnError); err != nil {
			s.Fatal("Failed to remove Passpoint config from ARC: ", err)
		}
...
// getAddedPKCS11Objects get all added PKCS#11 objects from the user slot (slot 1)
// by comparing the current available objects and the parameter objs.
