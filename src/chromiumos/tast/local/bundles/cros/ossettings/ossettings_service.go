...
func computeNetworkConfigNetworkType(networkType pb.OpenNetworkDetailPageRequest_NetworkType) netconfig.NetworkType {
	if networkType == pb.OpenNetworkDetailPageRequest_NetworkType_CELLULAR {
		return netconfig.Cellular
	}

	// Default to returning WiFi to simplify logic and error handling.
...
func (s *Service) OpenNetworkDetailPage(ctx context.Context, req *pb.OpenNetworkDetailPageRequest) (*empty.Empty, error) {
	...
	networkType := computeNetworkConfigNetworkType(req.NetworkType)
	if _, err := ossettings.OpenNetworkDetailPage(ctx, tconn, cr, req.NetworkName, networkType); err != nil {
