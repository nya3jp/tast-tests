...
func init() {
	testing.AddTest(&testing.Test{
		...
		Contacts: []string{
			"TBD other name here?",     // Kernel team list
			"kathrelkeld@chromium.org", // Test port author
		},
...
func HighResTimers(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Error("failed to open timer list: ", err)
	}
	defer f.Close()
	...
	for scanner.Scan() {
		if matches := re.FindStringSubmatch(scanner.Text()); matches != nil {
			...
			if res != 1 {
				s.Errorf("timer resoultion %d != 1 ns", res)
			}
		}
	}
