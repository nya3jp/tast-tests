...
func testPeakshiftBattThresh(s *testing.State) {
	...
	for _, arg := range good {
		...
		if exp := arg + "\n"; res != exp {
			s.Fatalf("Read %q from %s; wanted %q",
				res, path, arg+"\\n")
		}
	}
...
func WilcoSysfsEC(ctx context.Context, s *testing.State) {
	...
	testBoolSetting(s, filepath.Join(wilcoECSysfsDir, "properties/wireless_sw_wlan"), wilcoECW)
	testBoolSetting(s, filepath.Join(wilcoECSysfsDir, "stealth_mode"), wilcoECW)
