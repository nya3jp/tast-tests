...
func PerfCallgraph(ctx context.Context, s *testing.State) {
	...
	for _, ln := range strings.Split(string(out), "\n") {
		// Blank line separating samples.
		if ln == "" {
			chainLength = 0
			continue
		}

		// "dso:" line at the end of a sample describing where it came from.
		...
		// "chain:" line near the beginning of a sample containing the length.
