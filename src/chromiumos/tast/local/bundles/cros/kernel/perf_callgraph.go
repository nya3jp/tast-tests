...
func init() {
	testing.AddTest(&testing.Test{
		...
		SoftwareDeps: []string{"amd64"}, // callchains are only reported with length 2 on ARM
...
func PerfCallgraph(ctx context.Context, s *testing.State) {
	...
	for _, ln := range strings.Split(string(out), "\n") {
		if strings.Contains(ln, "PERF_RECORD_SAMPLE") || ln == "" {
			chainLength = 0
		} else if ms := chainRegexp.FindStringSubmatch(ln); ms != nil {
			if chainLength, err = strconv.Atoi(ms[1]); err != nil {
				s.Fatalf("Failed to parse callchain length %q: %v", ms[1], err)
			}
		} else if strings.Contains(ln, "dso: "+exe) && chainLength >= wantedChainLength {
