...
func ConfigVerify(ctx context.Context, s *testing.State) {
	...
	newKernelConfigCheck(&kernelVersion{major: major, minor: minor}, u.Machine).test(m, s)
}
...
func readKernelConfig(ctx context.Context) (map[string]string, error) {
	...
	// Add configs module to generate /proc/config.gz.
