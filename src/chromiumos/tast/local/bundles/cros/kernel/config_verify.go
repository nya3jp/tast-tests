...
func newKernelConfigCheck(ver *sysutil.KernelVersion, arch string, osVer map[string]string) *kernelConfigCheck {
	...
	// The following configs are Chromium kernel specific.  Given that ChromiumOS could be with
	// non-Chromium kernel.  Only enforce the checks on ChromeOS.
	if osVer[osrelease.Name] == "Chrome OS" {
