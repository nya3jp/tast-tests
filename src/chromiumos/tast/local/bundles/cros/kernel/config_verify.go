...
func ConfigVerify(ctx context.Context, s *testing.State) {
	...
	log := func(l string) { s.Log(l) }
	conf, err := readKernelConfig(ctx, log)
	if err != nil {
...
func kernelVersionAndArch() (*kernelVersion, string, error) {
	...
	if err != nil {
		return nil, "", errors.Wrapf(err, "wrong release format %s", u.Release)
	}
...
func readKernelConfig(ctx context.Context, log func(string)) (map[string]string, error) {
	configs, err := readKernelConfigBytes(ctx, log)
	if err != nil {
		return nil, err
	}
	...
	for _, line := range strings.Split(string(configs), "\n") {
		...
		if !strings.HasPrefix(kv[0], configPrefix) {
			return nil, errors.Errorf("config %s doesn't start with %s unexpectedly", kv[0], configPrefix)
		}
...
func readKernelConfigBytes(ctx context.Context, log func(string)) ([]byte, error) {
	...
	if err != nil {
		log(fmt.Sprintf("Falling back: failed to open %s: %v", filename, err))
		u, err := sysutil.Uname()
		if err != nil {
			return nil, errors.Wrap(err, "failed to get uname")
		}
