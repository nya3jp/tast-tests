...
func ConfigVerify(ctx context.Context, s *testing.State) {
	...
	// versionIsOrNewer returns whether the kernel is newer or the same to the given base version.
	versionIsOrNewer := func(base string) bool {
		t := strings.Split(base, ".")
		if len(t) != 2 {
			s.Fatalf("Wrong base format %s", base)
		}
...
// readKernelConfig reads kernel config key value pairs trimming CONFIG_ prefix from the keys.
