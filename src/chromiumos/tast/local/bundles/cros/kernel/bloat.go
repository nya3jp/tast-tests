...
func findUnusedDriversAndDevices(ctx context.Context, s *testing.State, pv *perf.Values) {
	...
	for _, bus := range busses {
		...
		for _, device := range devices {
			...
			if busName == "iio" {
				// The IIO bus has triggers that never have drivers attached
				if strings.HasPrefix(deviceName, "trigger") || strings.HasSuffix(deviceName, "trigger") {
					continue
				}
			}
...
func measureKernelText(ctx context.Context, s *testing.State, pv *perf.Values) {
	...
	for scanner.Scan() {
		...
		symbol := tokens[2]
		if symbol == "_stext" {
			_stext, err = strconv.ParseUint("0x"+addr, 0, 64)
			if err != nil {
				s.Fatal("Can't convert _stext to integer: ", err)
			}
		} else if symbol == "_etext" {
			_etext, err = strconv.ParseUint("0x"+addr, 0, 64)
			if err != nil {
	...
	if _stext == 0 && _etext == 0 {
		s.Error("Couldn't find _stext/_etext to measure kernel size")
	} else {
		pv.Set(
			perf.Metric{
				Name:      "kernel_text",
