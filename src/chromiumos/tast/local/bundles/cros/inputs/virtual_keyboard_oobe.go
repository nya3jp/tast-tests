...
func init() {
	testing.AddTest(&testing.Test{
		...
		HardwareDeps: hwdep.D(hwdep.Model(pre.InputsTabletModels...)),
		Vars:         []string{"inputs.signinProfileTestExtensionManifestKey"},
	})
}

func VirtualKeyboardOOBE(ctx context.Context, s *testing.State) {
	cr, err := chrome.New(ctx, chrome.NoLogin(), chrome.LoadSigninProfileExtension(s.RequiredVar("inputs.signinProfileTestExtensionManifestKey")))
	if err != nil {
