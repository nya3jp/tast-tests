...
func InputMethodManagement(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine("test input method management",
		...
		func(ctx context.Context) error {
			return ime.WaitForInputMethodInstalled(ctx, tconn, inputMethodCode, 30*time.Second)
		},
		settings.RemoveInputMethod(inputMethodName),
		func(ctx context.Context) error {
			return ime.WaitForInputMethodRemoved(ctx, tconn, inputMethodCode, 30*time.Second)
		},
	)(ctx); err != nil {
