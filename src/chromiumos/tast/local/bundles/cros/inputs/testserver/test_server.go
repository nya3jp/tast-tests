...
// ClearAction is the same as Clear, but generates
...
func (its *InputsTestServer) ClearAction(inputField InputField) uiauto.Action {
	return uiauto.NamedAction(
		fmt.Sprintf("testserver.InputsTestServer).ClearAction(inputField=%v)", inputField),
		func(ctx context.Context) error { return its.Clear(ctx, inputField) })
}

// WaitForFieldToBeActiveAction is the same as WaitForFieldToBeActive, but generates
// an action suitable for use with the uiauto library.
func (its *InputsTestServer) WaitForFieldToBeActiveAction(inputField InputField) uiauto.Action {
	return uiauto.NamedAction(
		fmt.Sprintf("testserver.InputsTestServer).WaitForFieldToBeActiveAction(inputField=%v)", inputField),
		func(ctx context.Context) error { return its.WaitForFieldToBeActive(ctx, inputField) })
}

// ClickFieldAndWaitForActiveAction is the same as ClickFieldAndWaitForActive, but generates
// an action suitable for use with the uiauto library.
func (its *InputsTestServer) ClickFieldAndWaitForActiveAction(tconn *chrome.TestConn, inputField InputField) uiauto.Action {
	return uiauto.NamedAction(
		fmt.Sprintf("testserver.InputsTestServer).ClickFieldAndWaitForActiveAction(*chrome.TestConn, inputField=%v)", inputField),
...
func (inputField InputField) ClickAction(tconn *chrome.TestConn) uiauto.Action {
	return uiauto.NamedAction(
		"testserver.InputField).ClickAction(*chrome.TestConn)",
...
func (inputField InputField) ClickUntilVKShownAction(tconn *chrome.TestConn) uiauto.Action {
	return uiauto.NamedAction(
		"testserver.InputField).ClickUntilVKShownAction(*chrome.TestConn)",
		func(ctx context.Context) error { return inputField.ClickUntilVKShown(ctx, tconn) })
}

// WaitForValueToBeAction is the same as WaitForValueToBe, but generates
// an action suitable for use with the uiauto library.
func (inputField InputField) WaitForValueToBeAction(tconn *chrome.TestConn, expectedValue string) uiauto.Action {
	return uiauto.NamedAction(
		fmt.Sprintf("testserver.InputField).WaitForValueToBeAction(*chrome.TestConn, expectedValue=%v)", expectedValue),
