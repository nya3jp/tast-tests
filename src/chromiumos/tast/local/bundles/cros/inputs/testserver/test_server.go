...
// LaunchServer launches a local web server to serve inputs testing on different type of input fields.
func LaunchServer(ctx context.Context, cr *chrome.Chrome, fileSystem http.FileSystem) (*InputsTestServer, error) {
...
func (inputField InputField) GetValue(ctx context.Context, tconn *chrome.TestConn) (string, error) {
	...
	return out, inputField.action(ctx, tconn, actionFunc)
}
...
func (inputField InputField) WaitForValueToBe(ctx context.Context, tconn *chrome.TestConn, expectedValue string) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			return errors.Wrap(err, "failed to get input value")
		}
