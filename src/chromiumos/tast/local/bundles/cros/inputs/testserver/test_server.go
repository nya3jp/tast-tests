	...
	return uiauto.Combine("validate vk input function on field "+string(inputField),
		...
		func(ctx context.Context) error {
			...
			if testData.SubmitFromSuggestion {
				if err := vkbCtx.SelectFromSuggestion(testData.ExpectedText)(ctx); err != nil {
					return errors.Wrapf(err, "failed to select %s from suggestion bar", testData.ExpectedText)
				}
			}
			return nil
		},
