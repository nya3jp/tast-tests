...
func PhysicalKeyboardEmojiSuggestion(ctx context.Context, s *testing.State) {
	...
	validateEmojiSuggestion := func(shouldSuggest bool) error {
	...
	s.Run(ctx, testName, func(ctx context.Context, s *testing.State) {
		...
		if err := validateEmojiSuggestion(true); err != nil {
	...
	s.Run(ctx, testName, func(ctx context.Context, s *testing.State) {
		...
		if err := uiauto.Combine("toggle off emoji suggestion and close settings app",
			...
			s.Fatal("Failed to toggle off emoji suggestion: ", err)
		}

		if err := validateEmojiSuggestion(false); err != nil {
			s.Fatal("Failed to validate disabing emoji suggestion: ", err)
		}
	})
	...
	s.Run(ctx, testName, func(ctx context.Context, s *testing.State) {
		...
		if err := uiauto.Combine("toggle on emoji suggestion and close settings app",
			imeSettings.ToggleEmojiSuggestions(tconn),
			imeSettings.WaitUntilEmojiSuggestion(cr, tconn, true),
			imeSettings.Close(),
		)(ctx); err != nil {
			s.Fatal("Failed to toggle on emoji suggestion: ", err)
		}

		if err := validateEmojiSuggestion(true); err != nil {
			s.Fatal("Failed to validate re-enabled emoji suggestion: ", err)
		}
	})
}
