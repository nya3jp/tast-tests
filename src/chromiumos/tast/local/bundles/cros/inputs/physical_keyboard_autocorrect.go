...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{
			{
				Name: "en_us_1",
				Val: util.AutocorrectTestCase{
					InputMethodID: string(ime.INPUTMETHOD_XKB_US_ENG),
					MisspeltWord:  "helol",
					CorrectWord:   "hello",
					UndoMethod:    util.AutocorrectUndoViaPopupUsingPK,
				},
			}, {
				Name: "en_us_2",
				Val: util.AutocorrectTestCase{
					...
					MisspeltWord:  "wrold",
					CorrectWord:   "world",
					UndoMethod:    util.AutocorrectUndoViaPopupUsingMouse,
				},
			},
...
func PhysicalKeyboardAutocorrect(ctx context.Context, s *testing.State) {
	testCase := s.Param().(util.AutocorrectTestCase)
	...
	case util.AutocorrectUndoViaBackspace:
		// Not applicable for PK
		break
	case util.AutocorrectUndoViaPopupUsingPK:
	...
	case util.AutocorrectUndoViaPopupUsingMouse:
