...
// Default IME code used for new Chrome instance.
...
// VKEnabledTablet creates a new precondition for testing virtual keybaord in tablet mode.
...
type preImpl struct {
	...
	dm      deviceMode     // device ui mode to test in
}
...
func (p *preImpl) Prepare(ctx context.Context, s *testing.PreState) interface{} {
	...
	if p.cr != nil {
		err := func() error {
			...
			if err := ResetIMEStatus(ctx, p.cr); err != nil {
	...
	opts := []chrome.Option{chrome.ExtraArgs("--enable-features=LanguageSettingsUpdate")}
...
func ResetIMEStatus(ctx context.Context, cr *chrome.Chrome) error {
	...
	testing.ContextLog(ctx, "Reset IME settings")
	if err := tconn.Call(ctx, nil, `tast.promisify(chrome.inputMethodPrivate.reset)`); err != nil {
		return errors.Wrap(err, "failed to reset ime settings")
	}
	...
	for _, inputMethod := range installedInputMethods {
		if inputMethod.ID != defaultIMECode {
			...
			if err := ime.RemoveInputMethod(ctx, tconn, inputMethod.ID); err != nil {
				return errors.Wrapf(err, "failed to remove input method: %q", inputMethod.ID)
			}
		}
	}
