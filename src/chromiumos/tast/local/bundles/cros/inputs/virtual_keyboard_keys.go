...
// VirtualKeyboardKeys tests virtual keyboard features for different languages.
func VirtualKeyboardKeys(ctx context.Context, s *testing.State) {
	...
	for _, im := range []ime.InputMethod{
		...
		f := func(ctx context.Context, s *testing.State) {
			...
			if err := im.InstallAndActivate(tconn)(ctx); err != nil {
				s.Fatalf("Faield to install and activate input method %q: %v", im.Name, err)
			}
