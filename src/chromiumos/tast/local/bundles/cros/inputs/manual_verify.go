...
const helpStr = `
	...
	Supported variables: 
		profile: 'guest' or 'enterprise'. Login params will be ignored if profile is provided.
		...
		imeCode: Input method to be installed and activated for testing. Default IME is xkb:us::eng.
		...
		rdp: 'true' or 'false'. Whether install RDP extension and initializing remote desktop session.
	...
	* Login guest user with virtual keyboard enabled	
		tast run -var=profile=guest -var=vkEnabled=true <dut ip> inputs.ManualVerify
	* To test a specific input method on an android app.
		tast run -var=arc=true -var=imeCode=nacl_mozc_us <dut ip> inputs.ManualVerify
`
...
func getVars(s *testing.State) *envSettings {
	...
	rdp, err := strconv.ParseBool(rdpStr)
	if err != nil {
		s.Fatal("Failed to parse the variable `rdp`: ", err)
	}
	settings.rdp = rdp
