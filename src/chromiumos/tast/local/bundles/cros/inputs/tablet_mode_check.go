...
var re = regexp.MustCompile(`tablet_mode_angle=(\d+) hys=(\d+)`)
...
// setTabletModeUsingEC sets tabletModeAngle for provided degree of lidAngle, hysAngle.
...
func modeValues(ctx context.Context) (lidAngle, hysAngle int, err error) {
...
func ensureTabletModeEnabled(ctx context.Context) (func(ctx context.Context) error, error) {
	...
	initLidAngle, initHysAngle, err := modeValues(ctx)
	if err != nil {
	...
	const tabletModeLidAngle, tabletModeHysAngle = 0, 0

	if initLidAngle != tabletModeLidAngle || initHysAngle != tabletModeHysAngle {
		if err := setTabletModeUsingEC(ctx, tabletModeLidAngle, tabletModeHysAngle); err != nil {
	...
	return func(ctx context.Context) error {
		return setTabletModeUsingEC(ctx, initLidAngle, initHysAngle)
	}, nil
}
...
func evtestEvent(ctx context.Context, eventReString, expectedDetectionStatus string) error {
	out, err := exec.Command("evtest").CombinedOutput()
	if err != nil {
		if !strings.Contains(err.Error(), "exit status 1") {
			return errors.Wrap(err, "failed to execute evtest command")
		}
	}
