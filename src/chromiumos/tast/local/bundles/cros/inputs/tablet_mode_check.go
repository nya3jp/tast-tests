...
func TabletModeCheck(ctx context.Context, s *testing.State) {
	...
	isOnboardKeyboard, keyboardDevPath, err := input.FindPhysicalKeyboard(ctx)
	if err != nil {
		s.Fatal("Failed to create keyboard event: ", err)
	}

	if !isOnboardKeyboard {
		s.Fatal("Failed to find keyboard input device")
	}

	isTouchpad, touchpadDevPath, err := input.FindPhysicalTrackpad(ctx)
	if err != nil {
	...
	onboardKeyboardEventPath := strings.Trim(keyboardDevPath, trimString)
	...
	touchpadEventPath := strings.Trim(touchpadDevPath, trimString)
