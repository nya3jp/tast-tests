...
func ColdLaunchPerf(ctx context.Context, s *testing.State) {
	...
	// Wait for init XKB extension is killed.
	...
	subtests := []struct {
		...
		{
			Name: "Cal a number",
			Action: uiauto.Combine("calculate number of 'a' in the string",
				ui.Retry(2, func(ctx context.Context) error {
					nodeInfo, err := ui.Info(ctx, inputField.Finder())
					if err != nil {
						return err
					}
					rawACnt := strings.Count(nodeInfo.Value, "a")
					if rawACnt > 0 {
						return errors.Errorf("failed to decode first %d key events, Got: %s", rawACnt, nodeInfo.Value)
					}
					return nil
				},
				),
