...
func VirtualKeyboardAccent(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine("Input accent letter with virtual keyboard",
		...
		ui.WaitUntilExists(vkb.NodeFinder.Name(languageLabel).First()),
		ui.MouseMoveTo(vkb.KeyFinder.Name(keyName), 500*time.Millisecond),
		...
		ui.WaitUntilExists(accentKeyFinder),
