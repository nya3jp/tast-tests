...
func InputMethodShelf(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine("toggle show input options in shelf",
		...
		settings.WaitUntilToggleOption(cr, tconn, string(imesettings.ShowInputOptionsInShelf), false),
		...
		settings.WaitUntilToggleOption(cr, tconn, string(imesettings.ShowInputOptionsInShelf), true),
		...
		settings.ToggleShowInputOptionsInShelf(cr, tconn, false),
		ui.WaitUntilGone(imeMenuTrayButtonFinder),
	)(ctx); err != nil {
