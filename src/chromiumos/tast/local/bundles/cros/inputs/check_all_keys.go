...
func CheckAllKeys(ctx context.Context, s *testing.State) {
	...
	for test, testDetail := range tests {
		f := func(ctx context.Context, s *testing.State) {
			cleanupSubtestsCtx := ctx
			ctx, cancel := ctxutil.Shorten(ctx, 5*time.Second)
			defer cancel()

			testing.ContextLog(ctx, "Switching input method to ", testDetail.ime.Name)
			if err := testDetail.ime.InstallAndActivate(tconn)(ctx); err != nil {
				s.Fatalf("Failed to switch to input mehtod [%s]: %v", testDetail.ime.Name, err)
			}

			if err := setupJpKeyboardFormat(ctx, cr, tconn, vkbCtx, test); err != nil {
			...
			for _, detail := range testDetail.details {
				...
				if detail.checkSuggestions {
					s.Log("Checking input suggestions")
		...
		if !s.Run(ctx, fmt.Sprintf("verify virtual keyboard subcase %q", test), f) {
			s.Errorf("Failed to complete test of verifying virtual keyboard %q", test)
		}
	}
}
