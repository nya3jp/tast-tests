...
const html = `<title>E14s test page</title>
<link href='https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.6/codemirror.css' rel='stylesheet'>
<script src='https://cdnjs.cloudflare.com/ajax/libs/codemirror/5.65.6/codemirror.js'></script>
<div id="editor"></div>
<script>
...
func init() {
	testing.AddTest(&testing.Test{
		Func:         AppCompatibilityCodeMirror,
...
func AppCompatibilityCodeMirror(ctx context.Context, s *testing.State) {
	...
	its, err := testserver.LaunchBrowserWithHTML(ctx, s.FixtValue().(fixture.FixtData).BrowserType, cr, tconn,
		html)
	if err != nil {
	...
	if err := uiauto.UserAction(
		...
		uiauto.Combine("click the CodeMirror editor, edit the text inside it, and validate the result",
			...
			ui.LeftClick(existingTextFinder),
