...
func VirtualKeyboardOverscroll(ctx context.Context, s *testing.State) {
	...
	waitForOverscrollCompletionAction := uiauto.Combine(
		...
		// Allow 500 milliseconds for the overscroll to be performed.
		...
		uiauto.RetrySilently(3, func(ctx context.Context) error {
			// Get the VK bounds.
			keyboardBounds, err := vkbCtx.Location(ctx)
			if err != nil {
			...
			if keyboardBounds.Width == 0 || keyboardBounds.Height == 0 {
				return errors.Errorf(
					...
					keyboardBounds.Width, keyboardBounds.Height)
			}
			...
			// Difference between the bottom of the input field and top of the
			// keyboard which can be negative in the case of any overlap.
			yDiff := keyboardBounds.Top -
				(inputFieldBounds.Top + inputFieldBounds.Height)

			// Validate that overscroll is performed by checking that the
			// the input field is in view port and its top is above the VK but
			// not very far above it.
			if inputFieldBounds.Top < 0 ||
				inputFieldBounds.Top > keyboardBounds.Top ||
				yDiff > keyboardBounds.Height {
				return errors.Errorf(
					"the input field is overlaid by VK, "+
						"keyboardTop=%d,  inputFieldTop=%d, "+
						"keyboardHeight=%d, inputFieldHeight=%d",
					keyboardBounds.Top, inputFieldBounds.Top,
					keyboardBounds.Height, inputFieldBounds.Height)
			}
	...
	if err := uiauto.UserAction(
		...
		&useractions.UserActionCfg{
			Attributes: map[string]string{
				...
				useractions.AttributeFeature:      useractions.FeatureVKOverscroll,
			},
		},
	)(ctx); err != nil {
	...
	if err := uiauto.UserAction(
		...
		uiauto.Combine(
			...
			uiauto.Sleep(200*time.Millisecond),
			// Type something in the input field that is now hidden.
