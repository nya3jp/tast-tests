...
func VirtualKeyboardOverscroll(ctx context.Context, s *testing.State) {
	...
	waitForOverscrollCompletionAction := uiauto.Combine(
		...
		uiauto.RetrySilently(3, func(ctx context.Context) error {
			...
			// overlap.
			yDiff := vkBounds.Top - (inputFieldBounds.Top + inputFieldBounds.Height)

			// Validate that after overscroll, the input field is not too far
			// above the VK.
			if yDiff > vkBounds.Height {
				return errors.Errorf("the input field is too far above VK, keyboardTop=%d,  inputFieldTop=%d, keyboardHeight=%d, inputFieldHeight=%d", vkBounds.Top, inputFieldBounds.Top, vkBounds.Height, inputFieldBounds.Height)
			}
