...
func VirtualKeyboardOverscroll(ctx context.Context, s *testing.State) {
	...
	validatedInputFieldVisibility := func(ctx context.Context, expectedVisibility bool) error {
		// Get the VK bounds.
	...
	waitForInputFieldVisibility := func(expectedVisibility bool) uiauto.Action {
		return uiauto.RetrySilently(3, func(ctx context.Context) error {
			return validatedInputFieldVisibility(ctx, expectedVisibility)
		})
	}

	// Validate the assumption that the input field is initially offscreen
	// or its location is somewhere that can be occluded by VK.
	// Whenever finding this assumption is frequently incorrect (e.g. high
	// resolution or large screens), the test can be extended by setting the
	// input field's top position somewhere outside the view using JS. The
	// severity of this validation can be set to Log, if such scenarios are
	// rare.
	if err := uiauto.Combine("validate the input field is initially offscreen or occluded",
		// Show VK.
		vkbCtx.ShowVirtualKeyboard(),
		// Scroll to top while VK is visible.
		its.ScrollTo(0, 0),
		// Wait for the view to be stable.
		uiauto.Sleep(200*time.Millisecond),
		// Check that the input field is offscreen or occluded.
		waitForInputFieldVisibility(false),
		// Hide VK to cleanup the state.
		vkbCtx.HideVirtualKeyboard(),
	)(ctx); err != nil { // Error must not be nil.
		s.Fatal("Failed to ensure input field is initially occluded or offscreen")
	}
