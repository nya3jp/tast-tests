...
func init() {
	testing.AddTest(&testing.Test{
		...
		HardwareDeps: hwdep.D(hwdep.TouchScreen(), pre.InputsStableModels),
...
func VirtualKeyboardGlideTyping(ctx context.Context, s *testing.State) {
	...
	validateGlideTyping := func(isGlideTypingEnabled bool) uiauto.Action {
		return uiauto.Combine("validate glide typing",
			...
			func(ctx context.Context) error {
				if !isGlideTypingEnabled {
					...
					return util.WaitForFieldTextToBeIgnoringCase(tconn, inputField.Finder(), keySeq[len(keySeq)-1])(ctx)
				}
				if err := util.WaitForFieldTextToBeIgnoringCase(tconn, inputField.Finder(), glideTypingWord.ExpectedText)(ctx); err != nil {
					...
					return uiauto.Combine("selecting candidate from suggestion bar",
						...
						util.WaitForFieldTextToBeIgnoringCase(tconn, inputField.Finder(), glideTypingWord.ExpectedText),
					)(ctx)
				}
				return nil
			},
	...
	s.Run(ctx, testName, func(ctx context.Context, s *testing.State) {
		...
		if err := validateGlideTyping(true)(ctx); err != nil {
