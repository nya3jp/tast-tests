...
func VirtualKeyboardHandwriting(ctx context.Context, s *testing.State) {
	...
	for _, inputMethod := range hwTestIMEs {
		for _, message := range hwTestMessages {
			...
			s.Run(ctx, testName, func(ctx context.Context, subS *testing.State) {
				...
				defer func() {
					// Cleanup states.
					if err := vkbCtx.SwitchFromHandwritingToTyping()(ctx); err != nil {
				...
				swtichAction := vkbCtx.SwitchToFullMode()
				if isFloat {
				...
				checkEngineReady := uiauto.Combine("Wait for handwriting engine to be ready",
					...
					vkbCtx.ClearInputField(inputField.Finder()))
