...
import (
	...
	"chromiumos/tast/local/bundles/cros/inputs/tablet"
...
func TouchpadFunctionality(ctx context.Context, s *testing.State) {
	...
	if testOpt.tabletMode {
		...
		cleanUp, err := tablet.EnsureTabletModeEnabled(ctx, tabletModeAngle, tabletHysAngle)
		if err != nil {
	...
	if len(result) > 0 {
		...
		s.Fatal("Failed touchpad not found in evtest command output")
	}
