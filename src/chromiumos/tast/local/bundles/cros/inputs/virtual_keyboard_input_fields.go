...
func VirtualKeyboardInputFields(ctx context.Context, s *testing.State) {
	...
	subtest := func(testName string, inputData data.InputData, inputField testserver.InputField) func(ctx context.Context, s *testing.State) {
	...
	for _, inputMethod := range inputFieldTestIMEs {
		// Setup input method
		s.Logf("Set current input method to: %s", inputMethod)
		...
		for inputField, message := range inputFieldToMessage {
			...
			s.Run(ctx, testName, subtest(testName, inputData, inputField))
		}
	}
}
