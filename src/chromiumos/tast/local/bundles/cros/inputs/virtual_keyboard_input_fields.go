		...
		for inputField, message := range inputFieldToMessage {
			...
			s.Run(ctx, testName, func(ctx context.Context, s *testing.State) {
				...
				if err := vkbCtx.TapKeysIgnoringCase(inputData.CharacterKeySeq)(ctx); err != nil {
					s.Fatalf("Failed to tap keys %v: %v", inputData.CharacterKeySeq, err)
				}
				...
				if inputField == testserver.PasswordInputField {
					if err := util.WaitForFieldTextToBe(tconn, inputField.Finder(), strings.Repeat("•", len(inputData.CharacterKeySeq)))(ctx); err != nil {
