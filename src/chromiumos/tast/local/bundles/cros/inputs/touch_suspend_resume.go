...
func TouchSuspendResume(ctx context.Context, s *testing.State) {
	...
	if err := launchCanvas(ctx, cr, srv.URL); err != nil {
...
func launchCanvas(ctx context.Context, cr *chrome.Chrome, url string) error {
	conn, err := cr.NewConn(ctx, url+"/canvas.html")
	if err != nil {
