...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Test voice input from Optin menu",
...
func InputMethodShelfInputs(ctx context.Context, s *testing.State) {
	...
	// Start of voice input sub test.
	testName := "voice_input"
	testDescription := "verify voice input triggered from opt-in menu"
	...
	verifyAudioInputAction := uiauto.Combine(testDescription,
		...
		if err := useractions.NewUserAction("Voice input",
			...
			&useractions.UserActionCfg{
				Attributes: map[string]string{
					...
					useractions.AttributeTestScenario: "Validate voice input triggered from opt-in menu",
				},
	...
	// End of voice input sub test.

	// Start of handwriting input sub test.
	testName = "handwriting"
	testDescription = "verify handwriting input triggered from opt-in menu"
	...
	verifyHandWritingInputAction := uiauto.Combine(testDescription,
		its.Clear(inputField),
		uiauto.New(tconn).Sleep(time.Second),
		its.ClickFieldAndWaitForActive(inputField),
		ui.LeftClick(imeMenuTrayButtonFinder),
		...
		func(ctx context.Context) error {
			...
			return uiauto.Combine("handwriting input",
	...
	s.Run(ctx, testName, func(ctx context.Context, s *testing.State) {
		defer faillog.DumpUITreeWithScreenshotOnError(ctx, s.OutDir(), s.HasError, cr, "ui_tree_"+testName)
		if err := useractions.NewUserAction("VK handwriting",
			verifyHandWritingInputAction,
			uc,
			&useractions.UserActionCfg{
				Attributes: map[string]string{
					useractions.AttributeInputField:   string(inputField),
					useractions.AttributeTestScenario: "Validate handwriting input triggered from opt-in menu",
				},
				Tags: []useractions.ActionTag{
	...
	// End of handwriting input sub test.

	// Start of Emoji input sub test.
	testName = "emoji"
	testDescription = "verify emoji input triggered from opt-in menu"
	inputEmoji := "😄"
	...
	verifyEmojiInputAction := uiauto.Combine(testDescription,
		its.Clear(inputField),
		uiauto.New(tconn).Sleep(time.Second),
		its.ClickFieldAndWaitForActive(inputField),
		ui.LeftClick(imeMenuTrayButtonFinder),
	...
	s.Run(ctx, testName, func(ctx context.Context, s *testing.State) {
		...
		if err := useractions.NewUserAction(
			...
			uc,
			&useractions.UserActionCfg{
				Attributes: map[string]string{useractions.AttributeInputField: string(inputField), useractions.AttributeTestScenario: "Input emoji by emoji picker via IME tray"},
				Tags: []useractions.ActionTag{
			...
			s.Fatal("Failed to validate emoji input: ", err)
		}
	})
	// End of Emoji input sub test.
}
