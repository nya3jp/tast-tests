...
func PhysicalKeyboardPerformance(ctx context.Context, s *testing.State) {
	...
	percentFast, err := percentSamplesBelow(latencyHistogram, 5)
	if err != nil {
	...
	// Record percentage of 'acceptable' key events. We define 'acceptable' as < 20ms.
	percentAcceptable, err := percentSamplesBelow(latencyHistogram, 20)
	if err != nil {
