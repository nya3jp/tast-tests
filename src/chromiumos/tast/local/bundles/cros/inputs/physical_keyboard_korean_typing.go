...
func PhysicalKeyboardKoreanTyping(ctx context.Context, s *testing.State) {
	...
	var subtests = []struct {
		testName       string
		keyboardLayout koreanKeyboardLayout // input Mode should match the name in IME setting.
	...
	for _, subtest := range subtests {
		s.Run(ctx, subtest.testName, func(ctx context.Context, s *testing.State) {
			...
			currentLayout, ok := uc.Attributes()["useractions.AttributeKeyboardLayout"]
			if !ok || currentLayout != string(subtest.keyboardLayout) {
	...
	s.Run(ctx, testName, func(ctx context.Context, s *testing.State) {
		...
		// Change layout in IME settings only if required.
		currentLayout, ok := uc.Attributes()["useractions.AttributeKeyboardLayout"]
		if !ok || currentLayout != string(koreanInputType2Set) {
