...
func VirtualKeyboardEditing(ctx context.Context, s *testing.State) {
	...
	validateAction := uiauto.Combine("edit text using virtual keyboard",
		...
		vkbCtx.WaitForKeysExist([]string{"backspace"}),
		vkbCtx.TapKey("backspace"),
		ui.LeftClick(TextFieldHeaderFinder),
		func(ctx context.Context) error {
			textBounds, err := ui.BoundsForRange(ctx, textFinder, 0, 2)
			if err != nil {
				return errors.Wrap(err, "failed to get text location")
			}
			return ui.MouseClickAtLocation(0, textBounds.RightCenter())(ctx)
		},
		vkbCtx.WaitForKeysExist([]string{"space"}),
		vkbCtx.TapKey("space"),

		// Edit text while focused in text field.
		func(ctx context.Context) error {
			textBounds, err := ui.BoundsForRange(ctx, textFinder, 0, 8)
			if err != nil {
				return errors.Wrap(err, "failed to get text location")
			}
			return ui.MouseClickAtLocation(0, textBounds.RightCenter())(ctx)
		},
		vkbCtx.WaitForKeysExist([]string{"i", "h"}),
		vkbCtx.TapKeys([]string{"i", "h"}),
		func(ctx context.Context) error {
			textBounds, err := ui.BoundsForRange(ctx, textFinder, 0, 9)
			if err != nil {
				return errors.Wrap(err, "failed to get text location")
			}
			return ui.MouseClickAtLocation(0, textBounds.RightCenter())(ctx)
		...
		vkbCtx.TapKey("backspace"),
		func(ctx context.Context) error {
			textBounds, err := ui.BoundsForRange(ctx, textFinder, 0, 5)
			if err != nil {
				return errors.Wrap(err, "failed to get text location")
			}
			return ui.MouseClickAtLocation(0, textBounds.RightCenter())(ctx)
