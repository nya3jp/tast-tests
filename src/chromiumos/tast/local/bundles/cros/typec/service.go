...
func (c *Service) NewChromeLoginWithPeripheralDataAccess(ctx context.Context, req *typec.KeyPath) (*empty.Empty, error) {
	...
	cr, err := chrome.New(ctx,
		chrome.DeferLogin())
	if err != nil {
		return &empty.Empty{}, errors.Wrap(err, "failed to start Chrome at login screen")
	}

	// Enable the setting.
	if err := typecutils.EnablePeripheralDataAccess(ctx, req.Path); err != nil {
		return &empty.Empty{}, errors.Wrap(err, "failed to enable peripheral data access setting")
	}

	if err := cr.ContinueLogin(ctx); err != nil {
		return &empty.Empty{}, errors.Wrap(err, "failed to login")
	}
