...
func FindConnectedDisplay(ctx context.Context, displayName string, totalDisplays int) error {
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		connectors, err := graphics.ModetestConnectors(ctx)
		testing.ContextLog(ctx, connectors)
		if err != nil {
			return errors.Wrap(err, "failed to get connectors")
		}
		displayCount := 0
		re := strings.Replace(displayName, "|", "|^", -1)
		for _, connector := range connectors {
			matched, err := regexp.MatchString("^"+re, connector.Name)
			if err != nil {
...
func CheckDisplayInfo(ctx context.Context, typecHdmi bool) error {
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		var DisplayInfoPtrn = regexp.MustCompile(`.*pipe\s+[BCD]\]:\n.*active=yes, mode=.[0-9]+x[0-9]+.: [0-9]+.*\s+[hw: active=yes]+`)
		found := (DisplayInfoPtrn).MatchString(string(out))
		if !found {
