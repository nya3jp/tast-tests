...
func TbtDataTransferAfterHotplug(ctx context.Context, s *testing.State) {
	...
	devicePath := getTBTMountPath(dirsAfterPlug, dirsBeforePlug)[0]
...
func getTBTMountPath(dirsAfterPlug, dirsbeforePlug []string) []string {
	mount := make(map[string]struct{}, len(dirsbeforePlug))
	for _, x := range dirsbeforePlug {
		mount[x] = struct{}{}
	}
	var mountPath []string
	for _, x := range dirsAfterPlug {
		if _, found := mount[x]; !found {
			mountPath = append(mountPath, x)
		}
	}
	return mountPath
}
