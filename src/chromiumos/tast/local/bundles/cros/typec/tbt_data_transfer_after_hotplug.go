...
func TbtDataTransferAfterHotplug(ctx context.Context, s *testing.State) {
	...
	getRemovables := func() []string {
		out, err := testexec.CommandContext(ctx, "sh", "-c", fmt.Sprintf("ls %s", tbtMount)).Output()
		if err != nil {
			s.Fatal("Failed to get removable devices: ", err)
		}
		return strings.Fields(string(out))
	}
	beforePlug := getRemovables()
	...
	sourcePath, err := ioutil.TempDir(downloadsPath, tempDir)
	if err != nil {
	...
	afterPlug := getRemovables()
	devicePath := getTBTMountPath(beforePlug, afterPlug)[0]
...
func getTBTMountPath(slice1 []string, slice2 []string) []string {
