...
// Since we can't pass the TestConn reference in the Poll functions, we stash it
// a global variable.
var testConn *chrome.TestConn
...
func ModeSwitch(ctx context.Context, s *testing.State) {
	...
	cr, err := chrome.New(ctx, chrome.DeferLogin(), chrome.LoadSigninProfileExtension(s.RequiredVar("ui.signinProfileTestExtensionManifestKey")))
	if err != nil {
...
func findConnectedDPMonitor(ctx context.Context) error {
	...
	infos, err := display.GetInfo(ctx, testConn)
	if err != nil {
