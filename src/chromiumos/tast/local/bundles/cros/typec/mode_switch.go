...
func ModeSwitch(ctx context.Context, s *testing.State) {
	...
	err = testing.Poll(ctx, func(ctx context.Context) error {
		...
		return nil
	}, &testing.PollOptions{Interval: 100 * time.Millisecond, Timeout: 5 * time.Second})
	if err != nil {
	...
	err = testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			errors.Wrap(err, "failed to get connectors")
		}

		for _, connector := range connectors {
			if err := checkIsConnectorDP(connector, s); err == nil {
				return nil
			}
		}

		return errors.New("no valid DP monitor found")
...
func checkIsConnectorDP(connector *graphics.Connector, s *testing.State) error {
	...
	if !matched {
		return errors.Errorf("Connector:%s is not DP", connector.Name)
	}

	if !connector.Connected {
		return errors.Errorf("Connector:%s is not connected", connector.Name)
	}
	s.Logf("Found connector:%s", connector.Name)
