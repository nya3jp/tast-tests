...
type usbTestParams struct {
	...
	iter int
	// tablet is boolean true for tabletmode test.
	tablet bool
	// reString is string value for regex.
	reString string
}
	...
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			Val: usbTestParams{
				...
				deviceType: "Keyboard",
				iter:       1,
				reString:   `(?i)/dev/input/event([0-9]+):.*USB.*Keyboard.*`,
			},
		}, {
			...
			Val: usbTestParams{
				...
				deviceType: "Keyboard",
				iter:       1,
				reString:   `(?i)/dev/input/event([0-9]+):.*C-Type.*`,
			},
		}, {
...
func USBTypeAFunctionalityCheck(ctx context.Context, s *testing.State) {
	...
	if testParms.deviceType == "storage" {
		dirsBeforePlug, err := removableDirsList(mediaRemovable)
		if err != nil {
		...
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			dirsAfterPlug, err = removableDirsList(mediaRemovable)
			if err != nil {
...
func removableDirsList(mountPath string) ([]string, error) {
...
// newMountPath returns the latest removable device.
...
// verifyUSBHIDDevice returns connected status of USB Human Interface Devices
// like USB Keyboard, Mouse, etc.
...
func usbKeyboardEventNumber(ctx context.Context, reString string) (int, error) {
