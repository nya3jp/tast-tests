...
func ModeCrash(ctx context.Context, s *testing.State) {
	...
	oldPID, err := checkTypecdRunning(ctx)
	if err != nil {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		newPID, err := checkTypecdRunning(ctx)
		if err != nil {
			return errors.Wrap(err, "failed to get new typecd PID")
		}

		if newPID == oldPID {
			errors.Errorf("typecd still has old PID: %d", newPID)
		}
...
// is running. Returns:
// - PID of the process, or 0 on error.
// - Relevant error messsage (if applicable).
func checkTypecdRunning(ctx context.Context) (int, error) {
