...
func checkTypecPortDir(typecPortPath string) error {
	...
	var usbPortDir []string
	for _, file := range ls {
		if matched, err := regexp.MatchString(`^usb\d+(-|_)port\d+`, file); err != nil {
			return errors.Wrapf(err, "could not match regex with %s", file)
		} else if matched {
			usbPortDir = append(usbPortDir, file)
		}
	}
	if len(usbPortDir) == 0 {
	...
	for _, usbPort := range usbPortDir {
		usbPortPath := filepath.Join(typecPortPath, usbPort.Name())
		if err := checkUsbPortDir(usbPort, typecPortAbsPath); err != nil {
			return err
		}
	}
