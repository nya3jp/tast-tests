...
func init() {
	testing.AddTest(&testing.Test{
		...
		SoftwareDeps: []string{"usb_typec_link"},
	})
}
...
func checkUsbPortDir(usbPortPath string, isUsb4 bool) error {
	...
	if matched, err := regexp.MatchString(absolutePathRegex, string(absolutePath)); err != nil {
...
func checkConnectorDir(connectorPath string) error {
	...
	if matched, err := regexp.MatchString(typecConnAbsolutePathRegex, string(absolutePath)); err != nil {
		return errors.Wrapf(err, "could not match regex with %s", string(absolutePath))
	} else if !matched {
