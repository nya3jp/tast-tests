...
func SymlinkToUSB(ctx context.Context, s *testing.State) {
	...
	for _, port := range ports {
		...
		} else if err := checkForUsbPortDir(ctx, filepath.Join(typecPath, port.Name())); err != nil {
...
func checkForUsbPortDir(ctx context.Context, typecPortPath string) error {
	...
	for _, usbPort := range ls {
		...
		if err == nil && matched {
			if !isSymlink(ctx, filepath.Join(typecPortPath, usbPort.Name()), `^.*\/usb\d+\/\d+-\d+:1\.0\/.*`) {
				return errors.Errorf("usb port directory, %s not linked to usb port device", usbPort.Name())
			}
			foundUsbPortDir = true
			if err := checkForConnectorDir(ctx, filepath.Join(typecPath, usbPort.Name())); err != nil {
				return err
			}
		...
		if err == nil && matched {
			if !isSymlink(ctx, filepath.Join(typecPortPath, usbPort.Name()), `^.*\/domain\d+\/\d+-\d+\/.*`) {
...
func checkForConnectorDir(ctx context.Context, usbPortPath string) error {
	...
	for _, connector := range ls {
		if connector.Name() == "connector" {
			if isSymlink(ctx, filepath.Join(usbPortPath, connector.Name()), `^.*\/typec/.*`) {
...
func isSymlink(ctx context.Context, path, absolutePathRegex string) bool {
	absolutePath, err := testexec.CommandContext(ctx, "readlink", "-f", path).Output()
