...
func init() {
	testing.AddTest(&testing.Test{
		...
		SoftwareDeps: []string{"typec_location"},
	})
}
...
func checkForPhysicalLocationDir(portPath string) error {
	...
	for _, file := range files {
		...
		if d == "unknown" {
			return errors.Error("unknown physical location value in file %s", filePath)
		}
		if file.Name() == "lid" && d == "yes" {
			return errors.Error("not located on the lid, but lid set as yes")
		}
	}
