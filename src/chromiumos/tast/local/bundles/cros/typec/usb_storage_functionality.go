...
type usbFileInfo struct {
	// Holds USB pendrive speed.
	usbSpeed string
	// Holds file size in bytes to create a file.
	fileSize int
	// Name of the file.
	fileName string
	// Type of device like TBT, USB4.
	deviceType string
}
...
func USBStorageFunctionality(ctx context.Context, s *testing.State) {
	...
	speedOut, err := typecutils.DeviceSpeed(ctx)
	if err != nil {
	...
	if !speedFound {
		s.Fatalf("Unexpcted USB device speed: want %q, got %q", testParms.usbSpeed, speed)
	}
	...
	if err := testexec.CommandContext(ctx, "sh", "-c", fmt.Sprintf("cp -rf %s %s", sourceFilePath, destinationFilePath)).Run(); err != nil {
	...
	if err := testexec.CommandContext(ctx, "sh", "-c", fmt.Sprintf("cp -rf %s %s", destinationFilePath, sourceFilePath)).Run(); err != nil {
		s.Fatalf("Failed to copy file to %s path: %v", sourceFilePath, err)
	}
...
func removableDirs(ctx context.Context, mountPath string) ([]string, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to list media removable files")
	}
