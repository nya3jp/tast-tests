	...
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			Val: usbFileInfo{
				...
				usbSpeed:   regexp.MustCompile("(?i)Mass Storage.*5000M"),
				deviceType: "TBT",
			},
			...
			Val: usbFileInfo{
				...
				usbSpeed:   regexp.MustCompile("(?i)Mass Storage.*480M"),
				deviceType: "TBT",
			},
...
func USBStorageFunctionality(ctx context.Context, s *testing.State) {
	...
	var mediaRemovable = "/media/removable/"
	...
	dirsBeforePlug, err := removableDirs(ctx, mediaRemovable)
	if err != nil {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		dirsAfterPlug, err = removableDirs(ctx, mediaRemovable)
		if err != nil {
	...
	output, err := testexec.CommandContext(ctx, "lsusb", "-t").Output(testexec.DumpLogOnError)
	if err != nil {
		s.Fatal("Failed to run lsusb: ", err)
	}
	if !testParms.usbSpeed.MatchString(string(output)) {
		s.Fatalf("USB pendrive speed %s is not matched", testParms.usbSpeed)
	}
	...
	localHash, err := calculateChecksumOfFile(sourceFilePath)
	if err != nil {
	...
	destHash, err := calculateChecksumOfFile(destinationFilePath)
	if err != nil {
	...
	s.Logf("Transferring file from %s to %s", destinationFilePath, sourceFilePath)
	copyErr = testexec.CommandContext(ctx, "sh", "-c", fmt.Sprintf("cp -rf %s %s", destinationFilePath, sourceFilePath)).Run()
	if copyErr != nil {
	...
	isConnected, err := cswitch.IsDeviceEnumerated(ctx, deviceVal["device_name"].(string), tbtPort)
	if isConnected {
		s.Fatal("Failed to disconnect the TBT device: ", err)
	}
}

// calculateChecksumOfFile checks the checksum for the input file.
func calculateChecksumOfFile(path string) ([]byte, error) {
...
func removableDirs(ctx context.Context, mountPath string) ([]string, error) {
