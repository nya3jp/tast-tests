...
func USBStorageFunctionality(ctx context.Context, s *testing.State) {
	...
	if !speedFound {
		s.Fatalf("Unexpected USB device speed: want %q, got %q", testParms.usbSpeed, speedOut)
	}
	...
	if err := typecutils.CopyFile(ctx, sourceFilePath, destinationFilePath); err != nil {
	...
	if err := typecutils.CopyFile(ctx, destinationFilePath, sourceFilePath); err != nil {
		s.Fatal("Failed to copy file: ", err)
	}
