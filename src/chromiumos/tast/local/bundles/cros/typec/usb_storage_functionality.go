...
func USBStorageFunctionality(ctx context.Context, s *testing.State) {
	...
	var usbType, devicePath string
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		removableDevicesList, err := sysutil.GetAllRemovableDevices()
		if err != nil {
		...
		for _, usbInfo := range removableDevicesList.RemovableDevices {
			usbType = usbInfo.UsbType
			devicePath = usbInfo.Mountpoint
		}
