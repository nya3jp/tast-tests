...
func init() {
	testing.AddTest(&testing.Test{
		...
		Timeout: time.Hour,
		VarDeps: []string{"ui.gaiaPoolDefault", "utils.WWCB", "1stPeripheralID"},
...
func Printscan1Printer(ctx context.Context, s *testing.State) {
	...
	// 1. Login to a ChromeOS device and connect USB printer.
	if err := printscan1PrinterStep1(ctx, tconn, peripheralID1); err != nil {
	...
	// ---- Verify printer produces the printed page successfully(limited to reasonable acceptance)
	// ---- Verify print job status notification is present
	if err := printscan1PrinterStep2To4(ctx, cr, tconn); err != nil {
	...
	// 6. Confirm PDF and PNG files are printed successfully
	if err := printscan1PrinterStep6(ctx, s, tconn, cr); err != nil {
...
func printscan1PrinterStep1(ctx context.Context, tconn *chrome.TestConn, peripheralID1 string) error {

	testing.ContextLog(ctx, "Step 1 - Login to a ChromeOS device and connect USB printer")

	// connect usb printer
	if err := utils.SwitchFixture(ctx, peripheralID1, "on", "0"); err != nil {
		return errors.Wrap(err, "failed to connect usb printer")
	}
...
func printscan1PrinterStep2To4(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn) error {

	testing.ContextLog(ctx, "Step 2 - Open google search page and trigger print dialog on the current chrome page by pressing Ctrl + p")
	...
	testing.ContextLog(ctx, "Step 3, 4 - Select printer and proceed to printing with this printer")

	if err := selectPrinter(ctx, tconn); err != nil {
	...
	if err := waitForPrintCompleted(ctx, tconn); err != nil {
...
func printscan1PrinterStep5(ctx context.Context) error {

	testing.ContextLog(ctx, "Step 5 - Start a print job and unplug the USB connection in the middle of the print job")
...
func printscan1PrinterStep6(ctx context.Context, s *testing.State, tconn *chrome.TestConn, cr *chrome.Chrome) error {

	testing.ContextLog(ctx, "Step 6 - Confirm PDF and PNG files are printed successfully")
	...
	pdfFileLocation := filepath.Join(downloadsPath, printFilePDF)
	...
	pngFileLocation := filepath.Join(downloadsPath, printFilePNG)
	...
	for _, testParams := range []struct {
		...
		testing.ContextLogf(ctx, "Confirm %s are printed successfully", testParams.file)
		...
		// verify print file
		if err := verifyPrintFile(ctx, s, tconn, cr, testParams.key); err != nil {
...
func printscan1PrinterStep7(ctx context.Context, s *testing.State, cr *chrome.Chrome) error {
	...
	testing.ContextLog(ctx, "Step 7 - Confirm print job can be done from ARC++ / Android app")
	...
	if err := downloadTextView.WaitForExists(ctx, defaultTimeout); err != nil {
		testing.ContextLog(ctx, "downloadTextView doesn't exists: ")
	...
	if err := fileTextView.WaitForExists(ctx, defaultTimeout); err != nil {
		testing.ContextLog(ctx, "fileTextView doesn't exists: ")
	...
	if err := printButton.WaitForExists(ctx, defaultTimeout); err != nil {
		testing.ContextLog(ctx, "printButton doesn't exists: ")
	...
	testing.ContextLog(ctx, "Waiting for print preview to load")
	printpreview.WaitForPrintPreview(tconn)(ctx)
	...
	// verify print file
	if err := verifyPrintFile(ctx, s, tconn, cr, comparePrinterPDF); err != nil {
		return errors.Wrap(err, "failed to verify print file")
	}
...
func printscan1PrinterStep8(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn) error {
	...
	testing.ContextLog(ctx, "Step 8 - Confirm printing with changes to 'More settings' controls works as intended")
...
func setDropdown(ctx context.Context, s *testing.State, tconn *chrome.TestConn, dropdownName dropdownName, dropdownOption string) error {
	...
	testing.ContextLogf(ctx, "Setting dropdown %s to %s ", dropdownName, dropdownOption)
...
func setOption(ctx context.Context, s *testing.State, tconn *chrome.TestConn, optionName printOption, checked checked.Checked) error {
	...
	testing.ContextLogf(ctx, "Setting option %s to %s ", string(optionName), string(checked))
...
func selectPrinter(ctx context.Context, tconn *chrome.TestConn) error {
...
func waitForPrintCompleted(ctx context.Context, tconn *chrome.TestConn) error {
	...
	// ---- Verify printer produces the printed page successfully(limited to reasonable acceptance)
	testing.ContextLog(ctx, "Waiting for print job to complete")
...
func verifyPrintFile(ctx context.Context, s *testing.State, tconn *chrome.TestConn, cr *chrome.Chrome, key comparePrinterKey) error {

	testing.ContextLog(ctx, "verfiy print file")
	...
	fs, err := utils.WaitForFileSaved(ctx, downloadsPath, pat, startTime)
	if err != nil {
		return errors.Wrap(err, "failed to wait for file saved")
	}

	// copy file from chromebook to wwcb server
	filepath := filepath.Join(downloadsPath, fs.Name())
	...
	if err := comparePrinterPic(ctx, s, key, uploadPath); err != nil {
...
func comparePrinterPic(ctx context.Context, s *testing.State, key comparePrinterKey, filepath string) error {
	...
	testing.ContextLog(ctx, "request: ", URL)
	...
	testing.ContextLog(ctx, "response: ", data)
