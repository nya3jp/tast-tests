...
func Dock3UsbCharging(ctx context.Context, s *testing.State) {
	...
	if err := dock3UsbChargingStep2(ctx, extDispID1); err != nil {
		s.Fatal("Failed to execute step2: ", err)
	}

	// Step 3 - Connect docking station to chromebook.
	if err := dock3UsbChargingStep3(ctx, dockingID); err != nil {
		s.Fatal("Failed to execute step3: ", err)
	}

	// Step 4 - Verify chromebook is charging or not.
	if err := dock3UsbChargingStep4(ctx); err != nil {
		s.Fatal("Failed to execute step4: ", err)
	}
}

func dock3UsbChargingStep2(ctx context.Context, extDispID1 string) error {
...
func dock3UsbChargingStep3(ctx context.Context, dockingID string) error {
...
func dock3UsbChargingStep4(ctx context.Context) error {
	testing.ContextLog(ctx, "Step 4 - Check chromebook is charging or not")
