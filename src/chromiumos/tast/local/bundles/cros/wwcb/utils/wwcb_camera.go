...
func CameraGetColor(ctx context.Context, s *testing.State, dispSeq DisplaySequence) (string, error) {

	dispType, dispIndex := getDispTypeAndIndex(dispSeq)

	want, err := GetPiColor(ctx, s, dispType, dispIndex, "0")
	if err != nil {
...
func CameraCheckColor(ctx context.Context, s *testing.State, dispSeq DisplaySequence, expectColor string) error {

	dispType, dispIndex := getDispTypeAndIndex(dispSeq)

	color, err := GetPiColor(ctx, s, dispType, dispIndex, "0")
	if err != nil {
		return errors.Wrap(err, "failed to execute GetPiColor")
	}
...
func CameraCheckColorLater(ctx context.Context, s *testing.State, dispSeq DisplaySequence) error {

	dispType, dispIndex := getDispTypeAndIndex(dispSeq)

	_, err := GetPiColor(ctx, s, dispType, dispIndex, "5")
	if err != nil {
		return errors.Wrap(err, "failed to execute GetPiColor")
	}
...
func CameraCheckColorResult(ctx context.Context, s *testing.State, expectColor string) error {

	color, err := GetPiColorResult(ctx, s)
	if err != nil {
...
func CameraCheckPlayback(ctx context.Context, s *testing.State, dispSeq DisplaySequence) error {
	...
	videoPath, err := VideoRecord(ctx, s, "60", folderPath, dispType, dispIndex)
	if err != nil {
	...
	if err := GoldenPredict(ctx, s, videoPath, dispType, dispIndex, false); err != nil {
