...
// explain parameters
// displayID repesent the id of display fixture
// interval means how much time to delay that wwcb server execute function after it accept api
func GetPiColor(ctx context.Context, displayID, interval string) (string, error) {
	api := fmt.Sprintf("api/getpicolor?id=%s&Interval=%s", displayID, interval)
...
// filepath means video storage path
// displayID means the id of display fixture
func VideoRecord(ctx context.Context, durations, filepath, displayID string) (string, error) {
	api := fmt.Sprintf("api/VideoRecord?Durations=%s&Output=%s&id=%s&Width=1280&Height=720", durations, filepath, displayID)
...
func GoldenPredict(ctx context.Context, videoPath, displayID string, audio bool) error {
	api := fmt.Sprintf("api/goldenpredict?Input=%s&id=%s&Audio=%t", videoPath, displayID, audio)
	_, err := HTTPGetRequest(ctx, api)
	if err != nil {
		return err
	}
