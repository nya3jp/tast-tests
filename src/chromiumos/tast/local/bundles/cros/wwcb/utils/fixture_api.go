...
// getWebURL retrieve fixture web api URL from input var
...
func CompareScannerPic(s *testing.State, color, size, resolution, filepath string) error {
	...
	// construct URL
	URL := fmt.Sprintf("%s/api/compare_scan_pic?color=%s&size=%s&resolution=%s&filepath=%s",
		getWebURL(s),
...
// FixtureServerNotice pop up msg on tast server
...
// GetServerFile get file from fixture server
