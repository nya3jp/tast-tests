...
func getPort(action ActionState, sType string) string {
	...
	// hdmi
	if strings.Contains(sType, SwitchHDMI) {
		return getHdmiPort(action)
	}

	// type a
	if strings.Contains(sType, SwitchTYPEA) {
		return getTypeAPort(action)
	}

	// type c
	if strings.Contains(sType, SwitchTYPEC) {
		return getTypeCPort(action)
	}

	// dp
	if strings.Contains(sType, SwitchDP) {
		return getDPPort(action)
	}

	// ethernet
	if strings.Contains(sType, SwitchETHERNET) {
