...
func Dock16UnifiedDesktop(ctx context.Context, s *testing.State) {
	...
	// testing.ContextLogf(ctx,"Step 1 - Boot-up and Sign-In to the device")
...
func dock16UnifiedDesktopStep2(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn) error {
	testing.ContextLog(ctx, "Step 2 - Open Chrome browser in the maximized window")
...
func dock16UnifiedDesktopStep3(ctx context.Context, s *testing.State, tconn *chrome.TestConn, dockingID, extDispID1 string) error {
	testing.ContextLog(ctx, "Step 3.1 - Connect ext-display to station")
	if err := utils.SwitchFixture(ctx, extDispID1, "on", "0"); err != nil {
		return errors.Wrap(err, "failed to connect external display")
	}
	testing.ContextLog(ctx, "Step 3.2 - Connect docking station to chromebook")
...
func dock16UnifiedDesktopStep4(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn, fdms *fakedms.FakeDMS) error {
	...
	testing.ContextLog(ctx, "Step 4 - Turn off and on unified desktop option")
...
func dock16UnifiedDesktopStep5(ctx context.Context, s *testing.State, tconn *chrome.TestConn) error {
	...
	testing.ContextLog(ctx, "Step 5 - Arrange monitor order")
...
func dock16UnifiedDesktopStep6(ctx context.Context, s *testing.State, tconn *chrome.TestConn) error {
	...
	testing.ContextLog(ctx, "Step 6 - Change resolution")
...
func dock16UnifiedDesktopStep7(ctx context.Context, s *testing.State) error {
	...
	testing.ContextLog(ctx, "Step 7 - Enter dock mode by closing/open lid")
...
func dock16UnifiedDesktopStep8(ctx context.Context, s *testing.State, cr *chrome.Chrome) error {

	testing.ContextLog(ctx, "Step 8 - Suspend and resume the device by running 'powerd_dbus_suspend'")
...
func dock16UnifiedDesktopStep9(ctx context.Context, s *testing.State, tconn *chrome.TestConn) error {
	...
	testing.ContextLog(ctx, "Step 9 - Press Ctrl-F4 to switch to mirror mode, verify mirror mode works, and back to Unified view")
	...
	testing.ContextLogf(ctx, "Primary info is %s", string(obj))
...
func dock16UnifiedDesktopStep10(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn, fdms *fakedms.FakeDMS, dockingID, extDispID1, extDispID2 string) error {

	testing.ContextLog(ctx, "Step 10 - Add one more external display (2+ monitors) and repeat 3-8")
	...
	// repeat 3-8
	// step 3 - connect ext-display to station, connect station to chromebook
	if err := dock16UnifiedDesktopStep3(ctx, s, tconn, dockingID, extDispID1); err != nil {
	...
	// step 8 - suspend & wake up chromebook
	if err := dock16UnifiedDesktopStep8(ctx, s, cr); err != nil {
...
func dock16UnifiedDesktopStep11(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn, fdms *fakedms.FakeDMS, dockingID, extDispID1 string) error {

	testing.ContextLog(ctx, "Step 11 - Repeat 3-8 in Tablet mode")
	// into tablet mode
