...
func dock89DonglePowerStep3(ctx context.Context, tconn *chrome.TestConn, uc *utils.UsbController, extDispID1, ethernetID string, usbsID []string) error {
	testing.ContextLog(ctx, "Step 3 - Plug peripherals one by one, then check")
	// plug peripherals one by one
	if err := utils.ControlPeripherals(ctx, uc, "on", "0", extDispID1, ethernetID, usbsID); err != nil {
...
func dock89DonglePowerStep8(ctx context.Context, cr *chrome.Chrome, uc *utils.UsbController) error {
