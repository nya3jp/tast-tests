...
func Dock89DonglePower(ctx context.Context, s *testing.State) {
	...
	// build usb recorder
	uc, err := utils.NewUsbController(ctx, dockingID)
	if err != nil {
	...
	if err := dock89DonglePowerStep10(ctx, s, cr, uc, dockingID); err != nil {
	...
	if err := dock89DonglePowerStep11(ctx, s, cr, uc, dockingID); err != nil {
		s.Fatal("Failed to execute step 11: ", err)
	}
	// Step 12 - into tablet, then repeat above steps
	if err := dock89DonglePowerStep12(ctx, s, cr, uc, dockingID, extDispID1, ethernetID, peripheralID1, peripheralID2, peripheralID3, peripheralID4, peripheralID5); err != nil {
...
func dock89DonglePowerStep3(ctx context.Context, s *testing.State, tconn *chrome.TestConn, uc *utils.UsbController, extDispID1, ethernetID, peripheralID1, peripheralID2, peripheralID3, peripheralID4, peripheralID5 string) error {
	testing.ContextLog(ctx, "Step 3 - Plug peripherals one by one, then check")
	// plug peripherals one by one
	if err := utils.ControlPeripherals(ctx, s, uc, "on", "0", extDispID1, ethernetID, peripheralID1, peripheralID2, peripheralID3, peripheralID4, peripheralID5); err != nil {
...
func dock89DonglePowerStep4(ctx context.Context, s *testing.State, tconn *chrome.TestConn, uc *utils.UsbController, dockingID string) error {
...
func dock89DonglePowerStep5(ctx context.Context, s *testing.State, tconn *chrome.TestConn, uc *utils.UsbController, dockingID string) error {
	...
	// plug in dongle
	if err := utils.SwitchFixture(ctx, dockingID, "on", "0"); err != nil {
		return errors.Wrap(err, "failed to plug dongle")
	}
	// check peripherals
	if err := utils.VerifyPeripherals(ctx, s, tconn, uc, utils.IsConnect); err != nil {
		return errors.Wrap(err, "failed to check peripherals on dongle")
...
func dock89DonglePowerStep6(ctx context.Context, s *testing.State, intoTablet bool, uc *utils.UsbController) error {
...
func dock89DonglePowerStep7(ctx context.Context, s *testing.State, intoTablet bool, uc *utils.UsbController) error {
...
func dock89DonglePowerStep8(ctx context.Context, s *testing.State, cr *chrome.Chrome, uc *utils.UsbController) error {
...
func dock89DonglePowerStep9(ctx context.Context, s *testing.State, cr *chrome.Chrome, uc *utils.UsbController, dockingID string) error {
	testing.ContextLog(ctx, "Step 9 - Plug in dongle, sleep chromebook & wake up it, then check peripherals") // plug in
	if err := utils.SwitchFixture(ctx, dockingID, "on", "0"); err != nil {
		return errors.Wrap(err, "failed to plug dongle")
	}
...
func dock89DonglePowerStep10(ctx context.Context, s *testing.State, cr *chrome.Chrome, uc *utils.UsbController, dockingID string) error {
	testing.ContextLog(ctx, "Step 10 - Plug in dongle, suspend chromebook, unplug dongle, wake up it, plug dongle, then check peripherls")
	// plug in dongle
	if err := utils.SwitchFixture(ctx, dockingID, "on", "0"); err != nil {
		return errors.Wrap(err, "failed to plug dongle")
	}
...
func dock89DonglePowerStep11(ctx context.Context, s *testing.State, cr *chrome.Chrome, uc *utils.UsbController, dockingID string) error {
	testing.ContextLog(ctx, "Step 11 - Unplug dongle, suspend chromebook, plug in dongle, wake up it, then check peripherals")
	// unplug dongle
	if err := utils.SwitchFixture(ctx, dockingID, "off", "0"); err != nil {
		return errors.Wrap(err, "failed to unplug dongle")
	}
...
func dock89DonglePowerStep12(ctx context.Context, s *testing.State, cr *chrome.Chrome, uc *utils.UsbController, dockingID, extDispID1, ethernetID, peripheralID1, peripheralID2, peripheralID3, peripheralID4, peripheralID5 string) error {
	...
	// Step 6 - reboot then check peripherals
	if err := dock89DonglePowerStep6(ctx, s, true, uc); err != nil {
		s.Fatal("Failed to execute step 6: ", err)
	}
	// Step 7 - power off dongle & reboot, then check peripherals
	if err := dock89DonglePowerStep7(ctx, s, true, uc); err != nil {
		s.Fatal("Failed to execute step 7: ", err)
	}
	// Step 8 - check peripherals when power up / down couple times
	if err := dock89DonglePowerStep8(ctx, s, cr, uc); err != nil {
