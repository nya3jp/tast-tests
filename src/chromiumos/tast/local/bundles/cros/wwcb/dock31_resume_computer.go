...
func dock31ResumeComputerStep7To9(ctx context.Context, s *testing.State, cr *chrome.Chrome) error {
	...
	// call before suspend
	if err := utils.CameraCheckColorLater(ctx, s, utils.InternalDisplay); err != nil {
	...
	if err := utils.CameraCheckColorResult(ctx, s, "black"); err != nil {
...
func dock31ResumeComputerStep10To12(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn) error {
	...
	if err := utils.CameraCheckColor(ctx, s, utils.InternalDisplay, "white"); err != nil {
		return errors.Wrap(err, "Fail to check chromebook monitor is white")
	}

	if err := utils.CameraCheckColor(ctx, s, utils.ExternalDisplay1, "white"); err != nil {
