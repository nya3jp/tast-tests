...
func Dock31ResumeComputer(ctx context.Context, s *testing.State) { // chrome.LoggedIn()
	...
	if err := dock31ResumeComputerStep3(ctx, extDispID); err != nil {
		s.Fatal("Failed to execute step 3: ", err)
	}

	// step 4 - connect docking station to chromebook
	if err := dock31ResumeComputerStep4(ctx, dockingID); err != nil {
	...
	if err := dock31ResumeComputerStep5To6(ctx, tconn); err != nil {
...
func dock31ResumeComputerStep3(ctx context.Context, extDispID string) error {
...
func dock31ResumeComputerStep4(ctx context.Context, dockingID string) error {
...
func dock31ResumeComputerStep5To6(ctx context.Context, tconn *chrome.TestConn) error {
...
func dock31ResumeComputerStep7To9(ctx context.Context, s *testing.State, cr *chrome.Chrome) error {
	...
	_, err := utils.SuspendChromebook(ctx, cr)
	if err != nil {
...
func dock31ResumeComputerStep10To12(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn) error {
	testing.ContextLog(ctx, "Step 10 - Wake up chromebook")
	testing.ContextLog(ctx, "Step 11 - Check the external monitor display properly by test fixture")
	testing.ContextLog(ctx, "Step 12 - Check the chromebook display properly by test fixture")
