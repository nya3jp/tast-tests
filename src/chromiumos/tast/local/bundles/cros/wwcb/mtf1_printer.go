...
func Mtf1Printer(ctx context.Context, s *testing.State) {
	...
	// 1. Login to a ChromeOS device and connect USB printer.
	if err := mtf1PrinterStep1(ctx, s, tconn); err != nil {
	...
	// ---- Verify printer produces the printed page successfully(limited to reasonable acceptance)
	// ---- Verify print job status notification is present
	if err := mtf1PrinterStep2To4(ctx, s, cr, tconn); err != nil {
	...
	// 6. Confirm PDF and PNG files are printed successfully
	if err := mtf1PrinterStep6(ctx, s, tconn); err != nil {
	...
	// ---- no extra steps in the printing steps sequence
	// ---- no impact to system stability or quality of the printed page
	if err := mtf1PrinterStep7(ctx, s, cr); err != nil {
	...
	// - Quality
	// - Options: Headers and footers; Two-sided; Background graphics   "
	if err := mtf1PrinterStep8(ctx, s, cr, tconn); err != nil {
		s.Fatal("Failed to execute step 8: ", err)
	}
}

func mtf1PrinterStep1(ctx context.Context, s *testing.State, tconn *chrome.TestConn) error {
...
func mtf1PrinterStep2To4(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn) error {
...
func mtf1PrinterStep5(ctx context.Context, s *testing.State) error {
...
func mtf1PrinterStep6(ctx context.Context, s *testing.State, tconn *chrome.TestConn) error {
...
func mtf1PrinterStep7(ctx context.Context, s *testing.State, cr *chrome.Chrome) error {
...
func mtf1PrinterStep8(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn) error {
	...
	for _, testParams := range []struct {
		...
		case comparePrinterPapersize: // paper size: letter -> a4
			...
			if err := utils.WebNotification(s, "Please change printer page size to A4"); err != nil {
				return err
			}
		...
		case comparePrinterQuality: // quality: normal -> high
			...
			if err := utils.WebNotification(s, "Please change printer page size to Letter"); err != nil {
				return err
			}
...
func verifyPrintFile(ctx context.Context, s *testing.State, tconn *chrome.TestConn, key comparePrinterKey) error {
	...
	if err := utils.WebNotification(s, msg); err != nil {
		return err
	}
	...
	uploadPath, err := utils.UploadFile(s, filepath)
	if err != nil {
