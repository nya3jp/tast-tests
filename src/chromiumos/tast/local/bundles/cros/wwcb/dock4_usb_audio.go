...
/***
Test case #4 Audio Test over USB via a Dock Station
Pre-Condition:
...
Verification:
1)  By default under (Audio settings) menu "Output/Input - USB Audio/Mic should be [checked] associate with Docking
2)  Make sure video/audio playback without any issue
3)  Make sure video/audio playback without any issue
***/
...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Test headphone, microphone, Chrome OS speaker via a Dock",
...
func Dock4UsbAudio(ctx context.Context, s *testing.State) {
	...
	testVideoLocation := filepath.Join(downloadsPath, testVideo)
	if err := fsutil.CopyFile(s.DataPath(testVideo), testVideoLocation); err != nil {
		s.Fatal("Failed to copy the file to Downloads: ", err)
	}
	...
	if err := dock4UsbAudioStep4(ctx, tconn, kb, downloadsPath); err != nil {
...
func dock4UsbAudioStep3(ctx context.Context, tconn *chrome.TestConn, kb *input.KeyboardEventWriter) error {
	...
	// Connect external audio in a pre-setting state.
	// When docking station is connected,
	// input & output channel would change to the dock channel automatically.
...
func dock4UsbAudioStep4(ctx context.Context, tconn *chrome.TestConn, kb *input.KeyboardEventWriter, downloadsPath string) error {
	testing.ContextLog(ctx, "Step 4 - Play the file and record, then detect audio")

	files, err := filesapp.Launch(ctx, tconn)
	if err != nil {
		return errors.Wrap(err, "failed to launch the filesapp")
	}
