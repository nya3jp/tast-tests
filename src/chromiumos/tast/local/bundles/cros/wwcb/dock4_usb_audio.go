...
func Dock4UsbAudio(ctx context.Context, s *testing.State) {
	...
	testingFileLocation := filepath.Join(filesapp.DownloadPath, testingFile)
	if err := fsutil.CopyFile(s.DataPath(testingFile), testingFileLocation); err != nil {
		s.Fatal("Failed to copy the testing file to downloads: ", err)
	}
	defer os.Remove(testingFileLocation)
	...
	// Step 4 - Connect (Headphone/Microphone/External Speaker) onto docking station.
	...
	// Step 6 - Detect audio.
	if err := dock4UsbAudioStep5and6(ctx, cr, tconn, kb, testingFile); err != nil {
...
func dock4UsbAudioStep5and6(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, kb *input.KeyboardEventWriter, testAudio string) error {
	...
	// Upload file to wwcb server then detect audio.
