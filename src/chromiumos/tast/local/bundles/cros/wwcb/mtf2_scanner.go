...
func Mtf2Scanner(ctx context.Context, s *testing.State) {
	...
	// -WiFi
	// -USB
	if err := mtf2ScannerStep1(ctx, s, tconn); err != nil {
	...
	// ----WiFi / Settings / DocFeeder, Downloads / JPG / Grayscale, A4, 600dpi
	// ----USB / Settings / DocFeeder, Downloads / PDF / Color, A4, 75dpi"
	if err := mtf2ScannerStep2To6(ctx, s, cr, tconn, usb, wifi); err != nil {
		s.Fatal("Failed to execute step2, 3, 4, 5, 6: ", err)
	}

}

func mtf2ScannerStep1(ctx context.Context, s *testing.State, tconn *chrome.TestConn) error {
...
func mtf2ScannerStep2To6(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn, scannerUsb, scannerWifi string) error {
...
func verifyScanFile(ctx context.Context, s *testing.State, startTime time.Time, ts *testingStruct) error {
	...
	filepath, err := utils.UploadFile(s, scanfile)
	if err != nil {
