...
func Printscan2Scanner(ctx context.Context, s *testing.State) {
	...
	// -WiFi
	// -USB
	if err := printscan2ScannerStep1(ctx, tconn, peripheralID1); err != nil {
...
func printscan2ScannerStep1(ctx context.Context, tconn *chrome.TestConn, peripheralID1 string) error {
	testing.ContextLog(ctx, "Step 1 - Connect a scanner device through either")
	testing.ContextLog(ctx, "Wifi - connect maunauly in advanced")
	testing.ContextLog(ctx, "USB - plug in usb")
...
func printscan2ScannerStep2To6(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn, scannerUsb, scannerWifi string) error {
	...
	testing.ContextLog(ctx, "Step 2 - Open ‘Scan’ Utility app or go to Settings-> Print and Scan-> Scan")

	testing.ContextLog(ctx, "Step 3, 4 - Observe the available selection options and choose")

	testing.ContextLog(ctx, "Step 5, 6 - Repeat above steps with different combinations ")
...
func verifyScanFile(ctx context.Context, s *testing.State, startTime time.Time, ts *testingStruct) error {
	...
	if err := compareScannerPic(ctx, s,
		string(ts.ColorMode),
		string(ts.PageSize),
...
func getScannerFromUI(ctx context.Context, s *testing.State, tconn *chrome.TestConn) (string, string, error) {
	...
	testing.ContextLog(ctx, "Getting scanner from ui ")
...
func confirmDetails(ctx context.Context, s *testing.State, scannerInfo map[string]string, whatKind scanapp.DropdownName, whatThing string) error {
	...
	case scanapp.DropdownNameColorMode:
		testing.ContextLog(ctx, scannerInfo["cs"])
	//  file type: jpg, png, pdf
	// 	"pdl": "image/jpeg,application/pdf",
	case scanapp.DropdownNameFileType:
		testing.ContextLog(ctx, scannerInfo["pdl"])
	// source:
	// 	"duplex": "F",
	// 	"is": "platen,adf",
	case scanapp.DropdownNameSource:
		testing.ContextLog(ctx, scannerInfo["is"])
		testing.ContextLog(ctx, scannerInfo["duplex"])
	default:
		testing.ContextLog(ctx, scannerInfo["123"])
	}
...
func compareScannerPic(ctx context.Context, s *testing.State, color, size, resolution, filepath string) error {
