...
func init() {
	testing.AddTest(&testing.Test{
		...
		Fixture:      "ccaLaunched",
		Vars:         []string{"utils.WWCB", "1stPeripheralID"},
	})
}
...
func Webcam1ExternalCam(ctx context.Context, s *testing.State) {
	...
	// 1. Launch ChromeOS camera app
	// 	   -- Verify default camera is the front camera when the device have multiple cameras
	if err := webcam1ExternalCamStep1(ctx, app); err != nil {
	...
	// 2. Plug the external Camera and select the plugged external camera using the camera switch button.
	// 	   -- Verify camera app preview should switch to USB camera video feed.
	if err := webcam1ExternalCamStep2(ctx, app, peripheralID1); err != nil {
	...
	// 	-- Verify camera app should automatically switch to the default front camera.
	// 	--  Playback the video and verify it plays as expected"
	if err := webcam1ExternalCamStep4(ctx, app, peripheralID1); err != nil {
		s.Fatal("Failed to execute step4: ", err)
	}
}

func webcam1ExternalCamStep1(ctx context.Context, app *cca.App) error {

	testing.ContextLog(ctx, "Step 1 - Launch ChromeOS camera app")

	testing.ContextLog(ctx, "Verify default camera is the front camera when the device have multiple cameras")
...
func webcam1ExternalCamStep2(ctx context.Context, app *cca.App, peripheralID1 string) error {

	testing.ContextLog(ctx, "Step 2 - Plug the external Camera and select the plugged external camera using the camera switch button")

	// plug in external camera
	if err := utils.SwitchFixture(ctx, peripheralID1, "on", "0"); err != nil {
		return errors.Wrap(err, "failed to connect external camera")
	}
...
func webcam1ExternalCamStep3(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn, app *cca.App) error {
	...
	testing.ContextLog(ctx, "Step 3 - Verify the photo and video taken with external camera are clear and looks good")
...
func webcam1ExternalCamStep4(ctx context.Context, app *cca.App, peripheralID1 string) error {

	testing.ContextLog(ctx, "Step 4 - Unplug the external camera")
	...
	testing.ContextLog(ctx, "Verify camera app should automatically switch to the default front camera")
...
func checkPhotoLooksGood(ctx context.Context, s *testing.State, app *cca.App) error {
	...
	if err := compareWebcamPic(ctx, s, uploadPath); err != nil {
...
func checkVideoLooksGood(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn, app *cca.App) error {
	...
	if err := compareWebcamVideo(ctx, s, uploadPath); err != nil {
...
func compareWebcamPic(ctx context.Context, s *testing.State, filepath string) error {
...
func compareWebcamVideo(ctx context.Context, s *testing.State, filepath string) error {
