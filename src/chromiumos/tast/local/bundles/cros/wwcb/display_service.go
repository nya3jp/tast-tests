...
const systemResponseTimeout = 5 * time.Second
...
// GetDisplayCount returns numbers of display that Chromebook system detected.
func (ds *DisplayService) GetDisplayCount(ctx context.Context, req *empty.Empty) (*wwcb.QueryResponse, error) {
	...
	return &wwcb.QueryResponse{DisplayCount: int32(len(infos))}, nil
}
...
func (ds *DisplayService) SetMirrorDisplay(ctx context.Context, req *wwcb.QueryRequest) (*empty.Empty, error) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		return nil
	}, &testing.PollOptions{Timeout: systemResponseTimeout}); err != nil {
...
func (ds *DisplayService) SwitchWindowToDisplay(ctx context.Context, req *wwcb.QueryRequest) (*empty.Empty, error) {
	...
	w, err := ash.FindWindow(ctx, tconn, func(w *ash.Window) bool {
		return strings.Contains(w.Title, string(req.WindowTitle))
	})
	if err != nil {
	...
	if err := ui.Exists(expectedWindow)(ctx); err != nil {
		...
		return &empty.Empty{}, uiauto.Combine("switch window to "+displayName,
			...
			ui.WithTimeout(30*time.Second).WaitUntilExists(expectedWindow),
		)(ctx)
	}
...
func (ds *DisplayService) VerifyWindowOnDisplay(ctx context.Context, req *wwcb.QueryRequest) (*empty.Empty, error) {
	...
	infos, err := display.GetInfo(ctx, tconn)
	if err != nil {
		return &emptypb.Empty{}, errors.Wrap(err, "failed to get display info")
	}
	...
	displayID := infos[int(req.DisplayIndex)].ID
	if w.DisplayID != displayID {
		return &empty.Empty{}, errors.Errorf("unexpected display id: got %s, want %s", w.DisplayID, displayID)
	}
