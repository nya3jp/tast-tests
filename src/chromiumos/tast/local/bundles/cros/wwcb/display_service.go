...
// DisplayTimeout is the default timeout for Chromebook system detect and catch display information.
var displayTimeout = testing.RegisterVarString(
	"wwcb.DisplayTimeout",
	"30",
	"This timeout is the default timeout for Chromebook system to detect and catch display information",
)
...
func (ds *DisplayService) VerifyWindowOnDisplay(ctx context.Context, req *wwcb.QueryRequest) (*empty.Empty, error) {
	...
	duration, err := strconv.Atoi(displayTimeout.Value())
	if err != nil {
		return nil, errors.Wrapf(err, "unable to parse timeout %q to an int", displayTimeout.Value())
	}
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		return nil
	}, &testing.PollOptions{Timeout: time.Duration(duration) * time.Second}); err != nil {
		return &empty.Empty{}, err
	}
