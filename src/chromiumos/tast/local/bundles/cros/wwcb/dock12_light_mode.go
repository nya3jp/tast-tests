...
func Dock12LightMode(ctx context.Context, s *testing.State) {
	...
	if err := dock12LightModeStep1(ctx, extDispID1); err != nil {
		s.Fatal("Failed to execute step 1: ", err)
	}

	// Step 2 - Connect docking station to Chromebook.
	if err := dock12LightModeStep2(ctx, tconn, dockingID); err != nil {
		s.Fatal("Failed to execute step 2: ", err)
	}

	// Step 3 - Enable night light and adjust the color temperature.
	if err := dock12LightModeStep3(ctx, tconn, kb, extDispID1); err != nil {
		s.Fatal("Failed to execute step 3: ", err)
	}
}

func dock12LightModeStep1(ctx context.Context, extDispID1 string) error {
	testing.ContextLog(ctx, "Step 1 - Plug in external monitor to the docking station")
...
func dock12LightModeStep2(ctx context.Context, tconn *chrome.TestConn, dockingID string) error {
	testing.ContextLog(ctx, "Step 2 - Connect docking station to the chromebook")
...
func dock12LightModeStep3(ctx context.Context, tconn *chrome.TestConn, kb *input.KeyboardEventWriter, extDispID string) error {
	testing.ContextLog(ctx, "Step 3 - Enable night light and adjust the color temperature")
