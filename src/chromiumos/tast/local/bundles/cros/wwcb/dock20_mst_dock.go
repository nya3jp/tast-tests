...
func Dock20MstDock(ctx context.Context, s *testing.State) {
	...
	if err := dock20MstDockStep3(ctx, dockingID); err != nil {
		s.Fatal("Failed to execute step 3: ", err)
	}

	// step 4 - connect 1st ext-display to docking station via DP
	if err := dock20MstDockStep4(ctx, extDispID); err != nil {
		s.Fatal("Failed to execute step4 : ", err)
	}

	testing.ContextLog(ctx, "Step 5 - Connect the 2nd external monitor to the 1st external monitor via DP cable. (Manual)")

	// step 6, 7, 8 - play youtube on ext-display1
	if err := dock20MstDockStep6To8(ctx, cr, tconn, kb); err != nil {
		s.Fatal("Failed to execute step 6, 7, 8: ", err)
	}

	// step 9 - verification
	if err := dock20MstDockStep9(ctx, tconn, extDispID); err != nil {
		s.Fatal("Failed to execute step 9: ", err)
	}
}

func dock20MstDockStep3(ctx context.Context, dockingID string) error {
...
func dock20MstDockStep4(ctx context.Context, extDispID string) error {
	testing.ContextLog(ctx, "Step 4 - Connect the 1st external monitor to the docking station via DP cable")
...
func dock20MstDockStep6To8(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, kb *input.KeyboardEventWriter) error {
	testing.ContextLog(ctx, "Step 6, 7, 8 - Play youtube on ext-display")
...
func dock20MstDockStep9(ctx context.Context, tconn *chrome.TestConn, extDispID string) error {
