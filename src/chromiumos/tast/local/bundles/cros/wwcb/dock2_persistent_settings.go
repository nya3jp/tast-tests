...
func Dock2PersistentSettings(ctx context.Context, s *testing.State) {
	...
	// Step 2 - Connect two external display to docking.
	...
	// Step 5 - Unplug and plug-in, then check window bounds on which display.
	...
	// Step 7 - Unplug and plug-in, then check window bounds on display.
...
func dock2PersistentSettingsStep4(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome, kb *input.KeyboardEventWriter) error {
	...
	// Switch windows to 1st external display.
...
func dock2PersistentSettingsStep6(ctx context.Context, tconn *chrome.TestConn, kb *input.KeyboardEventWriter) error {
	...
	// Ensure 1st external display to be primary.
	...
	// Verfiy windows on 1st external display.
...
func dock2PersistentSettingsStep8(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		for _, info := range infos {
			if intDispInfo.ID != info.MirroringSourceID {
				return errors.Errorf("unexcepted mirror source ID: got %d, want %d", info.MirroringSourceID, intDispInfo.ID)
			}
		}
		return nil
	...
	testing.ContextLog(ctx, "Exit mirror mode, then verify display state and windows is on internal display")
	// Exit mirror mode.
