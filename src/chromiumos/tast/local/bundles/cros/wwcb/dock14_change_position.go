...
const (
	...
	// second app
	testappPkg = "org.chromium.arc.testapp.windowmanager24"
	testappAct = "org.chromium.arc.testapp.windowmanager.ResizeableLandscapeActivity"
	testappApk = "ArcWMTestApp_24.apk"
)
...
func dock14ChangePositionStep4To5(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC) error {
	...
	// install testing app
	if err := a.Install(ctx, arc.APKPath(testappApk)); err != nil {
	...
	settingsAct, err := arc.NewActivity(a, settingsPkg, settingsAct)
	if err != nil {
		return err
	}
	...
	if err := ash.WaitForVisible(ctx, tconn, settingsPkg); err != nil {
		return err
	}

	// Start wm Activity on external display.
	wmAct, err := arc.NewActivityOnDisplay(a, testappPkg, testappAct, externalARCDisplayID)
	if err != nil {
		return err
	}
	defer wmAct.Close()
	...
	if err := ash.WaitForVisible(ctx, tconn, testappPkg); err != nil {
		return err
	}
	...
	for _, test := range []struct {
		...
		utils.RunOrFatal(ctx, s, test.name, func(ctx context.Context, s *testing.State) error {
			if err := utils.EnsureSetWindowState(ctx, tconn, settingsPkg, test.windowState); err != nil {
				return err
			}
			settingsWindowInfo, err := ash.GetARCAppWindowInfo(ctx, tconn, settingsPkg)
			if err != nil {
				return err
			}

			if err := utils.EnsureSetWindowState(ctx, tconn, testappPkg, test.windowState); err != nil {
				return err
			}
			wmWindowInfo, err := ash.GetARCAppWindowInfo(ctx, tconn, testappPkg)
			if err != nil {
				return err
			}
			...
			for _, relayout := range []struct {
				...
				utils.RunOrFatal(ctx, s, relayout.name, func(ctx context.Context, s *testing.State) error {
					...
					if err := utils.EnsureWindowStable(ctx, tconn, settingsPkg, settingsWindowInfo); err != nil {
						return err
					}
					return utils.EnsureWindowStable(ctx, tconn, testappPkg, wmWindowInfo)
				})
			}
			return nil
		})
...
func dock14ChangePositionStep6(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, a *arc.ARC) error {
	...
	act, err := arc.NewActivityOnDisplay(a, settingsPkg, settingsAct, 0)
	if err != nil {
	...
	win, err := ash.GetARCAppWindowInfo(ctx, tconn, settingsPkg)
	if err != nil {
