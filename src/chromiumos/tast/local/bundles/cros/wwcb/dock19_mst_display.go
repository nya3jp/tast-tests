...
func Dock19MstDisplay(ctx context.Context, s *testing.State) {
	...
	if err := dock19MstDisplayStep3(ctx, extDispID); err != nil {
		s.Fatal("Failed to execute step3: ", err)
	}

	// step 4 - connect ext-display2 manually
	if err := dock19MstDisplayStep4(ctx); err != nil {
		s.Fatal("Failed to execute step4: ", err)
	}

	// step 5 - connect station to chromebook
	if err := dock19MstDisplayStep5(ctx, dockingID); err != nil {
		s.Fatal("Failed to execute step5: ", err)
	}

	// step 6, 7, 8 - play youtube on ext-display1
	if err := dock19MstDisplayStep6To8(ctx, cr, tconn, kb); err != nil {
		s.Fatal("Failed to execute step6, 7, 8: ", err)
	}

	// step 9 - verification
	if err := dock19MstDisplayStep9(ctx, tconn, extDispID); err != nil {
		s.Fatal("Failed to execute step9: ", err)
	}
}

func dock19MstDisplayStep3(ctx context.Context, extDispID string) error {
	testing.ContextLog(ctx, "Step 3 - Connect the 1st external monitor to the chromebook via DP cable. (switch Type-C & HDMI fixture)")
...
func dock19MstDisplayStep4(ctx context.Context) error {
	testing.ContextLog(ctx, "Step 4 - Connect the 2nd external monitor to the 1st external monitor via DP cable. (Manually)")
	return nil
}

func dock19MstDisplayStep5(ctx context.Context, dockingID string) error {
	testing.ContextLog(ctx, "Step 5 - Connect docking station to chromebook")
...
func dock19MstDisplayStep6To8(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, kb *input.KeyboardEventWriter) error {
	testing.ContextLog(ctx, "Step 6, 7, 8 - Play youtube")
...
func dock19MstDisplayStep9(ctx context.Context, tconn *chrome.TestConn, extDispID string) error {

	testing.ContextLog(ctx, "Step 8 - Run verification")
