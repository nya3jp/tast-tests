...
func Dock1PersistentSettings(ctx context.Context, s *testing.State) {
	...
	defer utils.ResetAllFixtures(ctx, s)
...
func dock1PersistentSettingsStep2(ctx context.Context, s *testing.State) error {
	...
	if err := utils.ControlFixture(ctx, s, utils.ExtDisp1Type, utils.ExtDisp1Index, utils.ActionPlugin, false); err != nil {
		return errors.Wrap(err, "failed to plug ext-display into docking station")
	}
...
func dock1PersistentSettingsStep3(ctx context.Context, s *testing.State, tconn *chrome.TestConn) error {
	...
	if err := utils.ControlFixture(ctx, s, utils.StationType, utils.StationIndex, utils.ActionPlugin, false); err != nil {
		return errors.Wrap(err, "failed to plug docking station into Chromebook")
	}
...
func dock1PersistentSettingsStep5(ctx context.Context, s *testing.State, tconn *chrome.TestConn) error {
	...
	for _, param := range []struct {
		...
		{utils.ActionUnplug, 0}, // internal
		{utils.ActionPlugin, 1}, // external
	} {
		// // control ext-display to plug in or unplug
		if err := utils.ControlFixture(ctx, s, utils.ExtDisp1Type, utils.ExtDisp1Index, param.action, false); err != nil {
		...
		for _, pkgName := range []string{
			...
			s.Logf("Checking %s window is on %d display", pkgName, param.dispIndex)
...
func dock1PersistentSettingsStep6(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC) error {
	...
	testing.ContextLog(ctx, "Let ext-display 1 become primary ")
...
func dock1PersistentSettingsStep8(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC) error {
	...
	testing.ContextLog(ctx, "Exit mirror mode, then check display attributes and windows in on internal display")
