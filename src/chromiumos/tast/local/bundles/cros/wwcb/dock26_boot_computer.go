...
func Dock26BootComputer(ctx context.Context, s *testing.State) { // chrome.LoggedIn()
	...
	testing.ContextLog(ctx, "Step 1 - Connect external moniter to docking station manually in advance")
	testing.ContextLog(ctx, "Step 2 - connect docking station to chromebook manually in advance")
	testing.ContextLog(ctx, "Step 3 - Power the Chrombook On")
	testing.ContextLog(ctx, "Step 4 - Sign-in account")

	// only verify
	// step 5, 6 - check display properly
	if err := dock26BootComputerStep5To6(ctx, tconn); err != nil {
		s.Fatal("Failed to execute 5, 6: ", err)
	}
}

func dock26BootComputerStep5To6(ctx context.Context, tconn *chrome.TestConn) error {
	testing.ContextLog(ctx, "Step 5, 6 - Check external display info")
