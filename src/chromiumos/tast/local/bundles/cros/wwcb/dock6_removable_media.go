...
func Dock6RemovableMedia(ctx context.Context, s *testing.State) {
	...
	testPngLocation := filepath.Join(downloadsPath, testPng)
	...
	if err := dock6RemovableMediaStep5(ctx, tconn, testPng, downloadsPath); err != nil {
...
func dock6RemovableMediaStep5(ctx context.Context, tconn *chrome.TestConn, previewImageFile, downloadsPath string) error {
	...
	if err := testexec.CommandContext(ctx, "cp", filepath.Join(downloadsPath, previewImageFile), usbPath).Run(testexec.DumpLogOnError); err != nil {
