...
func Dock6RemovableMedia(ctx context.Context, s *testing.State) {
	...
	if err := dock6RemovableMediaStep4(ctx, peripheralsID); err != nil {
	...
	// step 6 - disconnect peripherals and check chromebook is ok
	if err := dock6RemovableMediaStep6(ctx, tconn, kb, peripheralsID, testVideo); err != nil {
...
func dock6RemovableMediaStep4(ctx context.Context, peripheralsID []string) error {
	testing.ContextLog(ctx, "Step 4 - Connect any of this (Flash Drive /Mouse /Keyboard /Webcam) onto Dock station")
	if err := controlPeripheralsThenCheck(ctx, peripheralsID, "on"); err != nil {
		return errors.Wrap(err, "failed to connect peripherals then check")
	}
	return nil
}
...
func dock6RemovableMediaStep6(ctx context.Context, tconn *chrome.TestConn, kb *input.KeyboardEventWriter, peripheralsID []string, testVideo string) error {
	testing.ContextLog(ctx, "Step 6 - Disconnect each peripheral then check chromebook is ok")
	if err := controlPeripheralsThenCheck(ctx, peripheralsID, "off"); err != nil {
		return errors.Wrap(err, "failed to disconnect peripherals then check")
	}
...
// controlPeripheralsThenCheck control peripherals' fixture to connect or disconnect, then verify usb count change
func controlPeripheralsThenCheck(ctx context.Context, peripheralsID []string, action string) error {
	before, err := testexec.CommandContext(ctx, "lsusb").Output()
	if err != nil {
		return err
	}
	for _, pid := range peripheralsID {
		if err := utils.SwitchFixture(ctx, pid, action, "0"); err != nil {
			return errors.Wrapf(err, "failed to control peripheral %s", pid)
		}
	}
	...
	if int(math.Abs(float64(beforeUsbCount-afterUsbCount))) != len(peripheralsID) {
		return errors.Errorf("usb count is not as expected, before is %d , after is %d, want %d", beforeUsbCount, afterUsbCount, len(peripheralsID))
	}
	return nil
}
