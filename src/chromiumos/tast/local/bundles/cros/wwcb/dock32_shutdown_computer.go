...
func Dock32ShutdownComputer(ctx context.Context, s *testing.State) { // chrome.LoggedIn()
	...
	if err := dock32ShutdownComputerStep3(ctx, extDispID); err != nil {
		s.Fatal("Failed to execute step 3: ", err)
	}
	// step 4 - connect station to chromebook
	if err := dock32ShutdownComputerStep4(ctx, dockingID); err != nil {
	...
	if err := dock32ShutdownComputerStep5To6(ctx, tconn); err != nil {
		s.Fatal("Failed to execute step 5, 6: ", err)
	}

	// step 7, 8 - power off chromebook
	if err := dock32ShutdownComputerStep7To8(ctx); err != nil {
...
func dock32ShutdownComputerStep3(ctx context.Context, extDispID string) error {
...
func dock32ShutdownComputerStep4(ctx context.Context, dockingID string) error {
...
func dock32ShutdownComputerStep5To6(ctx context.Context, tconn *chrome.TestConn) error {
...
func dock32ShutdownComputerStep7To8(ctx context.Context) error {
	testing.ContextLog(ctx, "Step 7, 8 - Power off chromebook")
	if err := utils.PoweroffChromebook(ctx); err != nil {
...
func dock32ShutdownComputerStep9To10(ctx context.Context, s *testing.State, tconn *chrome.TestConn) error {
	testing.ContextLog(ctx, "Step 9 -  Check the external monitor become dark by test fixture")
	testing.ContextLog(ctx, "Step 10 - Check the chromebook display become dark by test fixture")
