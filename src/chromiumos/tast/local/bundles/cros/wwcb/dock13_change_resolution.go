...
func dock13ChangeResolutionStep4(ctx context.Context, s *testing.State, tconn *chrome.TestConn) error {
	...
	s.Log("external info : ", prettyPrint(extDispInfo))
	...
	for _, param := range []struct {
		...
		s.Log("Setting display properties: mode = ", prettyPrint(mode))
...
func switchFixture(s *testing.State, whatType, index, cmd, interval string) error {
	...
	// construct URL
	URL := fmt.Sprintf("http://%s/api/switchfixture?Type=%s&Index=%s&cmd=%s&Interval=%s",
		WWCBURL,
		whatType,
		index,
		cmd,
