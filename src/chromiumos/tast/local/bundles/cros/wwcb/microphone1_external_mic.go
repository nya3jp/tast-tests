...
func init() {
	testing.AddTest(&testing.Test{
		...
		VarDeps:      []string{"ui.gaiaPoolDefault", utils.WWCBServerURL},
		Data:         []string{"apple.mp3"},
	})
}
...
func Microphone1ExternalMic(ctx context.Context, s *testing.State) {
	...
	if err := fsutil.CopyFile(s.DataPath(audioFile), audioFileLocation); err != nil {
		s.Fatalf("Failed to copy the test audio file to %s: %s", audioFileLocation, err)
	}
...
func microphone1ExternalMicStep1(ctx context.Context, s *testing.State) error {
	...
	s.Log("Step 1 - Plug in external mic")

	// there is no fixture to control yet
	if err := utils.WebNotification(s, "Please plug in external mic"); err != nil {
		return errors.Wrap(err, "failed to plug in external mic")
	}
...
func microphone1ExternalMicStep6To8(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn) error {
	...
	// uplug external mic
	if err := utils.WebNotification(s, "Please unplug external mic"); err != nil {
		return errors.Wrap(err, "failed to unplug external mic")
	}
...
func playAudioFile(ctx context.Context, s *testing.State, tconn *chrome.TestConn) error {
	...
	if err := files.OpenFile(audioFile)(ctx); err != nil {
		s.Fatalf("Failed to open the audio file %q: %v", audioFile, err)
	}
