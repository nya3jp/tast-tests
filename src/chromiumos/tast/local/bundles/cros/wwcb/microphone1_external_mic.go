...
func Microphone1ExternalMic(ctx context.Context, s *testing.State) {
	...
	googleAudioFileLocation := filepath.Join(downloadsPath, googleAudioFile)
	...
	goldenAudioFileLocation := filepath.Join(downloadsPath, goldenAudioFile)
	...
	if err := selectAudioOption(ctx, tconn, audioOptions[0]); err != nil {
...
func microphone1ExternalMicStep2(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn, audioOptions []string) error {
	...
	// select external mic
	if err := selectAudioOption(ctx, tconn, audioOptions[3]); err != nil {
...
func microphone1ExternalMicStep4(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn, audioOptions []string) error {
	...
	// select internal mic
	if err := selectAudioOption(ctx, tconn, audioOptions[2]); err != nil {
...
func selectAudioOption(ctx context.Context, tconn *chrome.TestConn, deviceName string) error {
