...
func RunAppSettingsTests(ctx context.Context, s *testing.State, cr *chrome.Chrome, testParams []TestingStruct, scannerParams ScannerStruct) {
	...
	s.Logf("Started virtual printer: %s", printer.PrintableName)
	...
	if _, err := ash.WaitForNotification(ctx, tconn, 30*time.Second, ash.WaitMessageContains(printer.PrintableName)); err != nil {
	...
	for _, test := range testParams {
		...
		settings.Scanner = printer.PrintableName
