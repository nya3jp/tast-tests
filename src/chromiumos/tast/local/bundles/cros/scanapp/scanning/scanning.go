...
const (
	// Test each combination of color mode, page size and resolution.
	HardwareTestRunAllCombinations HardwareTestMode = iota
	// Test the minimal number of combinations necessary to test each color
	// mode, page size and resolution at least once. Combinations will be
	// randomized each run.
...
// TODO: Comment.
func adjustScanDimensions(mode HardwareTestMode, colorModes []scanapp.ColorMode, pageSizes []scanapp.PageSize, resolutions []scanapp.Resolution) ([]scanapp.ColorMode, []scanapp.PageSize, []scanapp.Resolution, error) {
	switch mode {
...
// TODO: Comment.
func getNextScanCombination(numScan int, colorModes []scanapp.ColorMode, pageSizes []scanapp.PageSize, resolutions []scanapp.Resolution, mode HardwareTestMode) (colorMode scanapp.ColorMode, pageSize scanapp.PageSize, resolution scanapp.Resolution, err error) {
	switch mode {
	case HardwareTestRunAllCombinations:
...
func RunHardwareTests(ctx context.Context, s *testing.State, cr *chrome.Chrome, scanner ScannerDescriptor, mode HardwareTestMode) {
	...
	for _, source := range scanner.SupportedSources {
		...
		colorModes, pageSizes, resolutions, err := adjustScanDimensions(mode, source.SupportedColorModes, source.SupportedPageSizes, source.SupportedResolutions)
		if err != nil {
		...
		for i := 0; i < numScans; i++ {
			colorMode, pageSize, resolution, err := getNextScanCombination(i, colorModes, pageSizes, resolutions, mode)
			if err != nil {
