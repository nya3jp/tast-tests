...
func Scan(ctx context.Context, s *testing.State) {
	...
	// supports them.
	for _, settings := range []scanSettings{{
		...
		for _, dropdown := range []struct {
			...
			{"scannerSelect", settings.scanner},
			{"sourceSelect", settings.source},
			{"fileTypeSelect", settings.fileType},
			{"colorModeSelect", settings.colorMode},
			{"pageSizeSelect", settings.pageSize},
