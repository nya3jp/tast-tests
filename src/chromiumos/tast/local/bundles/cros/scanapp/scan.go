...
func Scan(ctx context.Context, s *testing.State) {
	...
	for _, settings := range []scanapp.ScanSettings{{
		...
		for _, dropdown := range []struct {
			id     scanapp.DropdownID
			option string
		}{
			{scanapp.DropdownIDScanner, settings.Scanner},
			{scanapp.DropdownIDSource, settings.Source},
			{scanapp.DropdownIDFileType, settings.FileType},
			{scanapp.DropdownIDColorMode, settings.ColorMode},
			{scanapp.DropdownIDPageSize, settings.PageSize},
			{scanapp.DropdownIDResolution, settings.Resolution},
		} {
			if err := app.SelectOption(ctx, dropdown.id, dropdown.option); err != nil {
				s.Fatalf("Failed to select %v from %v: %v", dropdown.option, dropdown.id, err)
			}
		}
