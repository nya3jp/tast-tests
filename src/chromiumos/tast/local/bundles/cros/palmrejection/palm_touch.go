...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data: []string{"palm_touch_index.html", "palm_touch_script.mjs", "palm_touch_kohaku_palmedgeinmiddle.csv", "palm_touch_coachz_palmedgeinmiddle.csv"},
	})
}
...
func PalmTouch(ctx context.Context, s *testing.State) {
	...
	for _, dataFile := range paramVal.dataFiles {
		evtest := readEvtestFile(s, "palm_touch_"+dataFile)
