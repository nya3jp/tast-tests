...
func ModemHelperManifestVerification(ctx context.Context, s *testing.State) {
	...
	for _, helper := range helperManifest.Helper {
		...
		for _, variant := range helper.Variant {
			if variants[variant] {
				s.Fatal("The variant: ", variant, " is present in multiple helpers")
			}
	...
	for _, device := range fwManifest.Device {
		...
		if !variants[device.Variant] {
			s.Fatal("The variant `", device.Variant, "` has no modem helper. The variant is missing in ", modemHelperPath)
		}
	}
}
