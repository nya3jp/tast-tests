...
func BasicProfileProperties(ctx context.Context, s *testing.State) {
	...
	if profPropsEntries, err := profProps.Get(shill.ProfilePropertyEntries); err != nil {
		s.Fatal("Failed getting profile entries: ", err)
	}

	if profPropsEntriesArray, err := profProps.GetStrings(shill.ProfilePropertyEntries); err != nil {
	...
	// Log the Profile Properties Entries, when Ethernet entry is missing.
	s.Log("Found Profile Properties Entries: ", profPropsEntries)
	s.Error("Missing Ethernet entry from profile: ", ethernetEntryKey)
}
