...
func New(subnet *net.IPNet, dns []string, forceAddressToIP string) *dnsmasq {
	return &dnsmasq{subnet: subnet, dns: dns, forceAddressToIP: forceAddressToIP}
}
...
func (d *dnsmasq) Start(ctx context.Context, env *env.Env) error {
	...
	if len(d.dns) > 0 {
		...
		confVals["port"] = dnsPort // enable DNS
	}
	if d.forceAddressToIP != "" {
		confVals["address"] = fmt.Sprintf("/%v/%v", d.forceAddressToIP, gateway.String())
