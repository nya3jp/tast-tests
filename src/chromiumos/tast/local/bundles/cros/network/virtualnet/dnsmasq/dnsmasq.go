...
// New creates a new dnsmasq object. Currently dnsmasq with only be used as a
...
func (d *dnsmasq) Start(ctx context.Context, env *env.Env) error {
	...
	poolStart := net.IPv4(ip[0], ip[1], ip[2], 100)
	poolEnd := net.IPv4(ip[0], ip[1], ip[2], 200)

	// d.subnet.Mask is in type Mask and thus cannot be stringify as an IP.
