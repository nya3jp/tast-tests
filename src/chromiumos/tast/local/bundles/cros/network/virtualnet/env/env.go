...
func (e *Env) GetVethInAddrs(ctx context.Context) (retAddrs *IfaceAddrs, retErr error) {
	...
	defer func() {
		if tempErr := cleanup(); tempErr != nil {
			testing.ContextLogf(ctx, "Failed to back to the original netns from netns %s: %v", e.NetNSName, tempErr)
			if retErr == nil {
				retErr = tempErr
			}
		}
...
// the thread going back to the root netns. Notes that this function calls
...
func (e *Env) EnterNetNS(ctx context.Context) (func() error, error) {
	...
	return func() error {
		...
		if tid != tidNow {
			return errors.Errorf("cleanup func does not run on the same thread with the one enters the netns %s", e.NetNSName)
		}
		...
		if err := unix.Setns(int(currentNSFile.Fd()), unix.CLONE_NEWNET); err != nil {
			return errors.Wrap(err, "failed to back to the original netns")
		}
		return nil
	}, nil
}
