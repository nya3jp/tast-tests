...
type Env struct {
	...
	// VethInStaticIPv4Addr is the static IPv4 address configured directly on
	// VethInName.
	VethInStaticIPv4Addr net.IP
	// VethInStaticIPv6Addr is the non-link-local IPv6 address configured directly
	// on VethInName. Note that this does not include the IPv6 address received by
	// RA packets.
	VethInStaticIPv6Addr net.IP
...
func (e *Env) GetVethInAddrs(ctx context.Context) (*IfaceAddrs, error) {
	...
	defer func() {
		if err := cleanup(); err != nil {
			testing.ContextLogf(ctx, "Failed to back to the original netns")
		}
	}()
...
func (e *Env) EnterNetNS(ctx context.Context) (func() error, error) {
	if err := setNetNS(ctx, "/run/netns/"+e.NetNSName); err != nil {
		return nil, errors.Wrapf(err, "failed to enter associated netns")
	}
	return func() error {
		if err := setNetNS(ctx, "/proc/1/ns/net"); err != nil {
			return errors.Wrapf(err, "failed to enter associated netns")
		}
		return nil
	}, nil
}
...
func setNetNS(ctx context.Context, path string) error {
	fd, err := os.Open(path)
	if err != nil {
		return errors.Wrapf(err, "failed to open netns fd")
	}
	defer func() {
...
func (e *Env) ConnectToRouter(ctx context.Context, router *Env, pool *subnet.Pool) error {
	...
	if err := e.InstallIPAndRoute(ctx, e.VethInName, selfIPv4Addr, ipv4Subnet); err != nil {
		return errors.Wrapf(err, "failed to configure IPv4 on %s", e.VethInName)
	}
	if err := router.InstallIPAndRoute(ctx, e.VethOutName, routerIPv4Addr, ipv4Subnet); err != nil {
	...
	e.VethInStaticIPv4Addr = selfIPv4Addr
	...
	routerIPv6Addr[15] = 1
	if err := e.InstallIPAndRoute(ctx, e.VethInName, selfIPv6Addr, ipv6Subnet); err != nil {
		return errors.Wrapf(err, "failed to configure IPv6 on %s", e.VethInName)
	}
	if err := router.InstallIPAndRoute(ctx, e.VethOutName, routerIPv6Addr, ipv6Subnet); err != nil {
	...
	if err := e.RunWithoutChroot(ctx, "ip", "route", "add", "default", "via", routerIPv6Addr.String()); err != nil {
		return errors.Wrap(err, "failed to add IPv6 defualt route")
	}
	e.VethInStaticIPv6Addr = selfIPv6Addr

	return nil
}

// InstallIPAndRoute configures |addr| on |ifname|, and adds a route to point
// |subnet| to this interface.
func (e *Env) InstallIPAndRoute(ctx context.Context, ifname string, addr net.IP, subnet *net.IPNet) error {
	if err := e.RunWithoutChroot(ctx, "ip", "addr", "add", addr.String(), "dev", ifname); err != nil {
		return errors.Wrap(err, "failed to install address")
	}
