...
// starts).
func (e *Env) GetVethInAddrs(ctx context.Context) (*ifaceaddrs.IfaceAddrs, error) {
	...
	addrs, err := ifaceaddrs.ReadFromInterface(e.VethInName)
	if err != nil {
		return nil, errors.Wrapf(err, "failed to read iface addrs inside %s", e.NetNSName)
	}
	return addrs, nil
}
