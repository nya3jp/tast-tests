...
// Package env provides the basic building block in a vritualnet.
package env
...
// Cleanup removes all the modications that this object does on the DUT. The
...
func (e *Env) Cleanup(ctx context.Context) error {
	...
	if err != nil {
		lastErr = errors.Wrapf(err, "failed to open file for logging in %s", e.name)
	...
	for serverName, server := range e.servers {
		...
		if f != nil {
			if _, err := f.WriteString("\n\n>>>>> " + serverName + "\n"); err != nil {
				lastErr = errors.Wrapf(err, "failed to write header lines in log file for server %s in %s", serverName, e.name)
				testing.ContextLog(ctx, "Cleanup failed: ", lastErr)
			}
			if err := server.WriteLogs(ctx, f); err != nil {
				lastErr = errors.Wrapf(err, "failed to write logs for server %s in %s", serverName, e.name)
				testing.ContextLog(ctx, "Cleanup failed: ", lastErr)
			}
		}
	}
	...
	if e.netnsCreated {
		if err := testexec.CommandContext(ctx, "ip", "netns", "del", e.NetNSName).Run(); err != nil {
			lastErr = errors.Wrapf(err, "failed to delete the netns %s", e.NetNSName)
			testing.ContextLog(ctx, "Cleanup failed: ", lastErr)
		}
	}
	...
	if e.vethPair != nil {
		if err := e.vethPair.Delete(ctx); err != nil {
			lastErr = errors.Wrap(err, "failed to delete remove veth pair")
			testing.ContextLog(ctx, "Cleanup failed: ", lastErr)
		}
	}
	...
	if _, err := testexec.CommandContext(ctx, "rm", "-rf", "--one-file-system", e.chrootDir).Output(); err != nil {
		lastErr = errors.Wrap(err, "failed removing chroot filesystem in which the VPN server was running")
		testing.ContextLog(ctx, "Cleanup failed: ", lastErr)
	}
