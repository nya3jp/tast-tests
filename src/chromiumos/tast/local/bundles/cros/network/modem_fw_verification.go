...
func init() {
	testing.AddTest(&testing.Test{
		Func:     ModemFWVerification,
		Desc:     "Verifies that all modem FWs compatible with a device are installed",
...
// ModemFWVerification Test
func ModemFWVerification(ctx context.Context, s *testing.State) {
	const modemFirmwarePath = "/opt/google/modemfwd-firmware/"
	modemFirmwareProtoPath := filepath.Join(modemFirmwarePath, "firmware_manifest.prototxt")
	...
	if err != nil {
		// TODO:Is there a way to skip the test instead of succeed?
		s.Fatal("Failed to access the firmware manifest: ", err)
		return
	}
...
// checkIfFirmwareExists Return true if directory exist, false otherwise
