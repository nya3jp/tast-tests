...
// Prepare is a helper to format and write out the config files, certificates,
// etc., needed by hostapd.
// Returns the path to which hostapd.conf was written.
func (c EAPConf) Prepare(ctx context.Context, dir, ctrlPath string) (string, error) {
...
func (s *Server) ExpectSTAStatus(ctx context.Context, staAddr, key, val string) error {
	...
	stdout, stderr, err = s.Server.CliCmd(ctx, "sta", staAddr)
	out := stdout + "\nstderr:\n" + stderr
	if err != nil {
	...
	path := filepath.Join(s.Server.OutDir, fmt.Sprintf("hostapd_auth_%d.txt", s.logIndex))
...
func (s *Server) StartEAPOL(ctx context.Context) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		stdout, stderr, err := s.Server.CliCmd(ctx, "new_sta", nearestMAC)
		out = stdout + "\nstderr:\n" + stderr
		return err
