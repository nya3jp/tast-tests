...
func HostToARCVPN(ctx context.Context, s *testing.State) {
	...
	// Host traffic gets routed to the ARC VPN correctly. 192.168.2.2 comes from the address being set in
	// //platform/tast-tests/android/ArcVpnTest/src/org/chromium/arc/testapp/arcvpn/ArcTestVpnService.java
	if err := routing.ExpectPingSuccessWithTimeout(ctx, "192.168.2.2", "chronos", 10*time.Second); err != nil {
		s.Fatal("Failed to ping 192.168.2.2 from host: ", err)
	}
}
