...
func testLoginMultiProfile(ctx context.Context, env *shillscript.TestEnv) error {
	...
	for i := 0; i < 5; i++ {
		...
		if len(files) != 1 {
			return errors.Errorf("found unexpected number of profiles in the directorey %s: got %v, want 1", shillscript.ShillUserProfilesDir, len(files))
		}
		if files[0].Name() != "chronos" {
			return errors.Errorf("found unexpected profile link in the directorey %s: got %v, want chronos", shillscript.ShillUserProfilesDir, files[0].Name())
		}
