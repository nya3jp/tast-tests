...
// On success, the caller is responsible to cleanup the environment through the returned |Cleanup| function.
...
func CreateDNSServerEnv(ctx context.Context, envName string, ipv4Subnet, ipv6Subnet *net.IPNet, routerEnv *env.Env, httpsCerts *certs.Certs) (*env.Env, error) {
	success := false
	...
	// Start a DNS server.
	if err := server.StartServer(ctx, "dnsmasq", dnsmasq.New(false /*enableDHCP=*/, true /*enableDNS=*/, nil /*subnet=*/, []string{} /*dnsServers=*/, "" /*resolvedHost=*/, addr)); err != nil {
