...
func dohResponder(addr string) func(http.ResponseWriter, *http.Request) {
...
func SetupDNSEnv(ctx context.Context, pool *subnet.Pool) (cleanup func(context.Context), routerEnv, serverEnv *env.Env, httpsCerts *certs.Certs, err error) {
	success := false

	// Shill-related setup.
	m, err := shill.NewManager(ctx)
	if err != nil {
		return nil, nil, nil, nil, errors.Wrap(err, "failed to create manager proxy")
	}
	...
	if err := m.SetProperty(ctx, shillconst.ProfilePropertyCheckPortalList, "wifi,cellular"); err != nil {
		return nil, nil, nil, nil, errors.Wrap(err, "failed to disable portal detection on ethernet")
	}
	defer func() {
		if success {
			return
	...
	if err != nil {
		cleanupCerts(ctx)
		return nil, nil, nil, nil, errors.Wrap(err, "failed to setup certificates")
	}
	defer func() {
		if success {
			return
	...
	if err := restartDNSProxy(ctx); err != nil {
		return nil, nil, nil, nil, errors.Wrap(err, "failed to restart DNS proxy")
	}

	// Allocate subnet for DNS server.
	serverIPv4Subnet, err := pool.AllocNextIPv4Subnet()
	if err != nil {
		return nil, nil, nil, nil, errors.Wrap(err, "failed to allocate v4 subnet")
	}
	serverIPv6Subnet, err := pool.AllocNextIPv6Subnet()
	if err != nil {
		return nil, nil, nil, nil, errors.Wrap(err, "failed to allocate v6 subnet")
	}
	...
	if err != nil {
		return nil, nil, nil, nil, errors.Wrap(err, "failed to set up router env")
	}
	defer func() {
		if success {
			return
	...
	if err := svc.WaitForProperty(ctx, shillconst.ServicePropertyState, shillconst.ServiceStateOnline, 10*time.Second); err != nil {
		return nil, nil, nil, nil, errors.Wrap(err, "failed to wait for base service online")
	}

	server, err := CreateDNSServerEnv(ctx, "server", serverIPv4Subnet, serverIPv6Subnet, router, httpsCerts)
	if err != nil {
		return nil, nil, nil, nil, errors.Wrap(err, "failed to set up server env")
	}
...
func CreateDNSServerEnv(ctx context.Context, envName string, ipv4Subnet, ipv6Subnet *net.IPNet, routerEnv *env.Env, httpsCerts *certs.Certs) (serverEnv *env.Env, err error) {
	...
	// Start a DoH server.
	httpsserver := httpserver.New("443", dohResponder(addr.String()), httpsCerts)
