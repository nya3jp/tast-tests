...
func VPNPolicy(ctx context.Context, s *testing.State) {
	...
	if err := policyutil.ResetChrome(ctx, fdms, cr); err != nil {
		s.Fatal("Failed to clean up Chrome: ", err)
	}
	...
	// 	AutoConnect: false,
	// 	Host:        "strongswan.crostestnetworks.com",
	// 	Type:        "L2TP-IPsec",
	// 	L2TP: &policy.ONCL2TP{
	// 		Username: "chromeos",
	// 		Password: "vpn-test",
	...
	//TODO: remove
	testing.ContextLog(ctx, "before refreshing... ")
	testing.Sleep(ctx, 300*time.Second)

	// Update device policy and verify shill.
	if err := policyutil.ServeAndRefresh(ctx, fdms, cr, []policy.Policy{deviceNetPolicy}); err != nil {
		s.Fatal("Failed to update policies: ", err)
	}

	//TODO: remove
	testing.ContextLog(ctx, "refreshed... ")
	testing.Sleep(ctx, 300*time.Second)
	testing.ContextLog(ctx, "continue... ")

	if err := verifyVPNProfile(ctx, m, deviceProfileServiceGUID); err != nil {
		s.Fatal("Verifying device profile defined VPN failed: ", err)
	}

	// Update user policy and verify shill.
	if err := policyutil.ServeAndRefresh(ctx, fdms, cr, []policy.Policy{userNetPolicy}); err != nil {
		s.Fatal("Failed to update policies: ", err)
	}

	if err := verifyVPNProfile(ctx, m, userProfileServiceGUID); err != nil {
		s.Fatal("Verifying user profile defined VPN failed: ", err)
	}
}
...
func verifyVPNProfile(ctx context.Context, m *shill.Manager, serviceGUID string) error {
	...
	timeoutCtx, cancel := context.WithTimeout(ctx, 350*time.Second)
	defer cancel()
