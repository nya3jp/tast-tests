...
func getGeoSARTablesFromASL(data []byte) ([]geoSARTable, error) {
	...
	if len(values) != 19 {
		return nil, errors.New("invalid ASL format for GEO SAR table" + strconv.Itoa(len(values)))
	}
	...
	for i := 0; i < 3; i++ {
		...
		currentTable.max2g = values[start]
		currentTable.chainAOffset2g = values[start+1]
		currentTable.chainBOffset2g = values[start+2]
		currentTable.max5g = values[start+3]
		currentTable.chainAOffset5g = values[start+4]
		currentTable.chainBOffset5g = values[start+5]
...
func getSARTableFromASL(data []byte, tableType sarTableType) ([]int64, error) {
	...
	values, err := getRawSARValues(data, tableKey)
	if values == nil {
		if err == nil {
			// A missing dynamic SAR table should cause failure.
			return nil, errors.New(tableName + " SAR table not found")
		}
		return nil, err
	}

	// tableIndicies[1] should be the length of  array.
	if len(values) != tableLength {
		return nil, errors.New("incorrectly formatted " + tableName + " table, expected length: " +
			strconv.Itoa(tableLength) + ", actual length: " + strconv.Itoa(len(values)))
	}
