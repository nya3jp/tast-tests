...
func ModemmanagerEnableAndConnect(ctx context.Context, s *testing.State) {
	...
	simpleConnectProps := map[string]interface{}{"allow-roaming": false, "apn": apn}
	simpleModem, err := modem.GetSimpleModem(ctx)
	if err != nil {
		s.Fatal("Could not get simplemodem object: ", err)
	}

	// check reigstration state and give a wait of 60sec, modem should do scan
	// networks and register. TODO: push to register as if any modem needs it
	if err := modemmanager.EnsureRegistered(ctx, modem, simpleModem); err != nil {
		// Do graceful exit for network timeout errors
		if strings.Contains(err.Error(), "Network timeout") {
			s.Log("Could not register to any network - Network timeout: ", err)
			return
		}
		s.Fatal("Modem not registered: ", err)
	}

	// sleep to allow registration props to loaded to try connect
	...
	if err := simpleModem.Call(ctx, mmconst.ModemConnect, simpleConnectProps).Err; err != nil {
		// Do graceful exit for network timeout errors
		if strings.Contains(err.Error(), "Network timeout") {
			s.Log("Could not connect - Network timeout: ", err)
			return
		}
