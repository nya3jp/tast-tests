...
func ModemmanagerEnableAndConnect(ctx context.Context, s *testing.State) {
	...
	// Check reigstration state and wait 60 seconds, modem scans for networks
	...
	if err := modemmanager.EnsureRegistered(ctx, modem, simpleModem); err != nil {
		...
		if strings.Contains(err.Error(), "Network timeout") {
			s.Error("Could not register to any network - Network timeout: ", err)
		}
		s.Fatal("Modem not registered: ", err)
	}

	// Sleep to allow registration properties to load to try to connect.
	...
	if err := simpleModem.Call(ctx, mmconst.ModemConnect, simpleConnectProps).Err; err != nil {
		...
		if strings.Contains(err.Error(), "Network timeout") {
			s.Error("Could not connect - Network timeout: ", err)
		}
