...
func (n *NetworkHealth) GetNetworkList(ctx context.Context, s *testing.State) ([]Network, error) {
	...
	if err := n.mojoRemote.Call(ctx, &result,
		"async function() { console.log('Printing message inside GetNetworkList'); var res = await this.getNetworkList(); console.log(res.networks); return res.networks;}"); err != nil {
	...
	//s.Log("Value of result : %v", result)
	//s.Logf("Value of result.Result %v", result.Result)
