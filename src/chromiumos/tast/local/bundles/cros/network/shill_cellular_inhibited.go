...
func verifyNotConnected(ctx context.Context, helper *cellular.Helper) error {
	...
	if s, err := helper.Manager.WaitForServiceProperties(ctx, props, 5*time.Second); err == nil {
...
func ShillCellularInhibited(ctx context.Context, s *testing.State) {
	...
	const setTimeout = 60 * time.Second
	s.Log("Inhibit Cellular Modem")
	if err := helper.SetDeviceProperty(ctx, shillconst.DevicePropertyInhibited, true, setTimeout); err != nil {
	...
	if err := helper.SetDeviceProperty(ctx, shillconst.DevicePropertyInhibited, false, setTimeout); err != nil {
		s.Fatal("Unable to set Device.Inhibited to false: ", err)
	}

	// Wait for Scanning to be false.
	if err := helper.Device.WaitForShillProperty(ctx, shillconst.DevicePropertyScanning, false, 30*time.Second); err != nil {
	...
	s.Log("Verify Cellular Service and Connect (this may take up to 60 seconds)")
	if service, err := helper.FindServiceForDeviceWithTimeout(ctx, 60*time.Second); err != nil {
