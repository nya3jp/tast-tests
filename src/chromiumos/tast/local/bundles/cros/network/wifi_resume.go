...
func WifiResume(ctx context.Context, s *testing.State) {
	...
	defer func() {
		if !flag {
			testexec.CommandContext(ctx, "start", "shill").Output()
			s.Log("starting shill")
			testexec.CommandContext(ctx, "start", "wpasupplicant").Output()
	...
	for i := 0; i < 5; i++ {
		...
		testexec.CommandContext(ctx, "stop", "shill").Output()
		s.Log("stopping shill")
		testexec.CommandContext(ctx, "stop", "wpasupplicant").Output()
		s.Log("stopping wpa_supplicant")

		testexec.CommandContext(ctx, "ip", "link", "set", "wlan0", "up").Output()
