...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Verifies that profile switching succeeds",
		Contacts: []string{
			"pholla@google.com",
...
func HermesMultiProfile(ctx context.Context, s *testing.State) {
	if uninhibit, err := modemmanager.InhibitModem(ctx); err != nil {
...
func checkProfileState(ctx context.Context, s *testing.State, p *hermes.Profile, expected int32) {
	...
	if actual != expected {
		s.Fatalf("Unexpected profile state, actual: %d, expected: %d", actual, expected)
	}
}
