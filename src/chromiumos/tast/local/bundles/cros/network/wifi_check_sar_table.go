...
// defined by Intel WiFi. It could be extended to provide support for multiple
// ewrd tables or for wgds.
type sarTableType int

const (
	wrds sarTableType = iota
	ewrd
...
func getSARTableFromASL(data []byte, tableType sarTableType) ([]int64, error) {
	...
	switch tableType {
	case wrds:
		keyIndex = strings.Index(dataString, "Name(WRDS,Package")
		tableIndices = []int{2, 12}
	case ewrd:
		keyIndex = strings.Index(dataString, "Name(EWRD,Package")
		tableIndices = []int{3, 13}
	}
	if keyIndex == -1 {
		return nil, errors.New("could not find SAR tables in SSDT")
	}
...
// against a set of SAR limits. These limits serve as a sanitiy check for the SAR
...
func verifyTable(decodedSSDT []byte, tableType sarTableType, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Unable to find SAR values, does your device support SAR: ", err)
	}
	tableName := ""
	switch tableType {
	case wrds:
...
func WifiCheckSARTable(ctx context.Context, s *testing.State) {
	...
	// WRDS stores "worst case" SAR table.
	verifyTable(decodedSSDT, wrds, s)
	// EWRD stores "dynamic" SAR tables.
	verifyTable(decodedSSDT, ewrd, s)
}
