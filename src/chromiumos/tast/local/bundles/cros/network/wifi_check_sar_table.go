...
// defined by Intel WiFi. It could be extended to provide support for wgds or multiple
// sets within ewrd.
...
func getSARTableFromASL(data []byte, tableType sarTableType) ([]int64, error) {
	...
	keyIndex = strings.Index(dataString, "Name("+tableName+",Package")
	if keyIndex == -1 {
		return nil, errors.New("could not find " + tableName + " SAR table in SSDT")
	}
