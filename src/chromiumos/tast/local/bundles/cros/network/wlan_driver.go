...
const marvell88w8797Sdio = "Marvell 88W8797 SDIO"
const marvell88w8887Sdio = "Marvell 88W8887 SDIO"
const marvell88w8897Sdio = "Marvell 88W8897 SDIO"
const marvell88w8897Pcie = "Marvell 88W8897 PCIE"
const marvell88w8997Pcie = "Marvell 88W8997 PCIE"
const atherosAr9280 = "Atheros AR9280"
const atherosAr9382 = "Atheros AR9382"
const atherosAr9462 = "Atheros AR9462"
const qualcommAtherosQca6174 = "Qualcomm Atheros QCA6174"
const qualcommAtherosQca6174Sdio = "Qualcomm Atheros QCA6174 SDIO"
const qualcommWcn3990 = "Qualcomm WCN3990"
const intel7260 = "Intel 7260"
const intel7265 = "Intel 7265"
const intel9000 = "Intel 9000"
const intel9260 = "Intel 9260"
const intel22260 = "Intel 22260"
const broadcomBcm4354Sdio = "Broadcom BCM4354 SDIO"
const broadcomBcm4356Pcie = "Broadcom BCM4356 PCIE"
const broadcomBcm4371Pcie = "Broadcom BCM4371 PCIE"
...
var deviceLookup = map[deviceInfo]string{
	deviceInfo{vendor: "0x02df", device: "0x9129"}: marvell88w8797Sdio,
	deviceInfo{vendor: "0x02df", device: "0x912d"}: marvell88w8897Sdio,
	deviceInfo{vendor: "0x02df", device: "0x9135"}: marvell88w8887Sdio,
	deviceInfo{vendor: "0x11ab", device: "0x2b38"}: marvell88w8897Pcie,
	deviceInfo{vendor: "0x1b4b", device: "0x2b42"}: marvell88w8997Pcie,
	deviceInfo{vendor: "0x168c", device: "0x002a"}: atherosAr9280,
	deviceInfo{vendor: "0x168c", device: "0x0030"}: atherosAr9382,
	deviceInfo{vendor: "0x168c", device: "0x0034"}: atherosAr9462,
	deviceInfo{vendor: "0x168c", device: "0x003e"}: qualcommAtherosQca6174,
	deviceInfo{vendor: "0x105b", device: "0xe09d"}: qualcommAtherosQca6174,
	deviceInfo{vendor: "0x0271", device: "0x050a"}: qualcommAtherosQca6174Sdio,
	deviceInfo{vendor: "0x8086", device: "0x08b1"}: intel7260,
	deviceInfo{vendor: "0x8086", device: "0x08b2"}: intel7260,
	deviceInfo{vendor: "0x8086", device: "0x095a"}: intel7265,
	deviceInfo{vendor: "0x8086", device: "0x095b"}: intel7265,
	deviceInfo{vendor: "0x8086", device: "0x9df0"}: intel9000,
	deviceInfo{vendor: "0x8086", device: "0x31dc"}: intel9000,
	deviceInfo{vendor: "0x8086", device: "0x2526"}: intel9260,
	deviceInfo{vendor: "0x8086", device: "0x2723"}: intel22260,
	deviceInfo{vendor: "0x02d0", device: "0x4354"}: broadcomBcm4354Sdio,
	deviceInfo{vendor: "0x14e4", device: "0x43ec"}: broadcomBcm4356Pcie,
	deviceInfo{vendor: "0x14e4", device: "0x440d"}: broadcomBcm4371Pcie,
	deviceInfo{compatible: "qcom,wcn3990-wifi"}:    qualcommWcn3990,
}

var expectedDriver = map[string]map[string]string{
	atherosAr9280: {
...
func WlanDriver(ctx context.Context, s *testing.State) {
	netIf, err := getWifiInterface(ctx)
	if err != nil {
		s.Fatal("Failed to get network interface name: ", err)
	}
	deviceName, err := getDeviceName(ctx, netIf)
	if err != nil {
		s.Fatal("Failed to get device name: ", err)
	}
	if _, ok := expectedDriver[deviceName]; !ok {
		s.Fatal("Unexpected device ", deviceName)
	}

	kernelReleaseBytes, err := testexec.CommandContext(ctx, "uname", "-r").Output()
	if err != nil {
		s.Fatal("Failed to get kernel release: ", err)
	}
	kernelRelease := string(kernelReleaseBytes)
	baseRevision := strings.Join(strings.Split(kernelRelease, ".")[:2], ".")

	expectedPath, ok := expectedDriver[deviceName][baseRevision]
	if !ok {
		s.Fatalf("Unexpected base revision %v for device %v", baseRevision, deviceName)
	}

	netDriversRoot := filepath.Join("/lib/modules", strings.TrimSpace(kernelRelease), "kernel/drivers/net")
	...
	if want, got := filepath.Base(expectedPath), moduleName+".ko"; want != got {
		s.Error("Expected module %s, got %s", want, got)
	}
}
...
func getWifiInterface(ctx context.Context) (string, error) {
	...
	isWifiDevice := func(name string) (bool, error) {
		cmd := testexec.CommandContext(ctx, "iw", "dev", strings.TrimSpace(name), "info")
		if err := cmd.Run(); err == nil {
			return true, nil
		} else if err, ok := err.(*exec.ExitError); ok {
			return false, nil
		} else {
			return false, errors.Wrapf(err, "failed to get wifi status for %s", name)
		}
	}

	for _, nic := range nics {
		if ok, err := isWifiDevice(nic.Name()); err != nil {
			return "", err
		} else if ok {
			return nic.Name(), nil
		}
...
func getDeviceName(ctx context.Context, netIf string) (string, error) {
	...
	devices := []deviceInfo{deviceInfo{vendor: vendorID, device: productID}}
