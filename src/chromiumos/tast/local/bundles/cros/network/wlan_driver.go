...
func getWLANDeviceName(ctx context.Context, netIf string) (string, error) {
	...
	var devices []wlanDeviceInfo
	if err := func() error {
		vendorID, err := readInfo("vendor")
		if os.IsNotExist(err) {
			testing.ContextLog(ctx, "Vendor ID not found for ", netIf)
			return nil
		}
		if err != nil {
		...
		if os.IsNotExist(err) {
			testing.ContextLog(ctx, "Product ID not found for ", netIf)
			return nil
		}
		if err != nil {
			return errors.Wrapf(err, "get device %s: failed to get product ID", netIf)
		}
		devices = append(devices, wlanDeviceInfo{vendor: vendorID, device: productID})
		return nil
	}(); err != nil {
		return "", err
	}
