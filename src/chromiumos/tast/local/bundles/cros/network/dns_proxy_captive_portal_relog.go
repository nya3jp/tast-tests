...
func DNSProxyCaptivePortalRelog(ctx context.Context, s *testing.State) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		return digMatch(ctx, dns.DigProxyIPRE, true)
	}, &testing.PollOptions{Timeout: 30 * time.Second}); err != nil {
		s.Fatal("Failed to verify system proxy: ", err)
	}

	// Currently, there is no simple way to do plain-txt DNS from ARC.
	...
	defer func() {
		if unblock {
			network.UnblockPortalDetector(cleanupCtx)
		}
	}()
	if err := network.BlockPortalDetector(ctx); err != nil {
	...
	unblock = false
	if err := network.UnblockPortalDetector(ctx); err != nil {
