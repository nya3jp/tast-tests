...
func DHCPInitBound(ctx context.Context, s *testing.State) {
	...
	// Get the WiFi device in shill and disable MAR.
	d, err := m.DeviceByType(ctx, shillconst.TypeWifi)
	if err != nil {
		s.Fatal("Failed to get the test WiFi device in shill: ", err)
	}
	if err := d.SetProperty(ctx, shillconst.DevicePropertyMACAddrRandomEnabled, false); err != nil {
		s.Fatal("Failed to disable MAR on the test WiFi device: ", err)
	}
