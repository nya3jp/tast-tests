...
func findActiveMatchingNetwork(networks []health.Network, sType, guid string) (*health.Network, error) {
	defaultNetworkType, err := health.NetworkTypeFromShillType(sType)
	if err != nil {
		return nil, err
	}
	for _, n := range networks {
		if defaultNetworkType != n.Type {
			continue
		}
...
func HealthGetNetworkList(ctx context.Context, s *testing.State) {
	...
	network, err := findActiveMatchingNetwork(networks, sType, guid)
	if err != nil {
		s.Fatalf("No matching active network found for %v: %v", name, err)
	}
