...
func HealthGetNetworkList(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatalf("Failed to check whether service %v is connected", name)
	}
	if !connected {
		s.Fatalf("Service %v is not connected", name)
	}
	...
	if networks[0].State != health.OnlineNS && networks[0].State != health.ConnectedNS {
		s.Fatalf("Network with GUID %v is not connected: got %v", networks[0].GUID, networks[0].State)
	}
}
