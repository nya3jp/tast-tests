...
func ModemFWManifestVerification(ctx context.Context, s *testing.State) {
	if _, err := os.Stat(cellular.GetModemFirmwareManifestPath()); err != nil {
		s.Fatal("Cannot find firmware_manifest.prototxt")
	}

	modemFirmwarePath := cellular.GetModemFirmwarePath()
	manifest, err := cellular.ParseModemFWManifest(ctx, s)
	if err != nil {
	...
	if len(missingFiles) > 0 {
		s.Fatal("The following firmware files are specified in the manifest, but don't exist: '", strings.Join(missingFiles, ","))
	}
}
