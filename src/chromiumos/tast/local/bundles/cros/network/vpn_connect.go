...
func VPNConnect(ctx context.Context, s *testing.State) {
	for _, vpnType := range vpnTypes {
		...
		if err := ping(ctx, vpn.Xl2tpdServerIPAddress, 10, 3, 60, ""); err != nil {
...
func ping(ctx context.Context, host string, deadline int, tries int, timeout int, user string) error {
	...
	*/
	...
	//testing.ContextLogf(ctx, "command result, stdout: %v, stderr: %v, exitCode: %v, command: %v", lines.String(), stderr.String(), fmt.Sprintf("%d", exitCode), args)
