...
func VPNConnect(ctx context.Context, s *testing.State) {
	...
	} else if params.vpnType == wireguard {
		server, err = vpn.StartWireGuardServer(ctx)
	} else {
	...
	// testing.Sleep(ctx, 50000*time.Millisecond)
...
func getVpnClientProperties(ctx context.Context, params vpnTestParams, serverAddress string) (map[string]interface{}, error) {
	if (params.vpnType == l2tpIPsec) && (params.authType == pskAuth) {
		...
		peers := []map[string]string{{
			"PublicKey":    vpn.WGServerPublicKey,
			"PresharedKey": vpn.WGPresharedKey,
			"EndPoint":     serverAddress + ":" + vpn.WGServerListenPort,
		...
		properties := map[string]interface{}{
			...
			"Type":                 "vpn",
			"Wireguard.PrivateKey": vpn.WGClientPrivateKey,
