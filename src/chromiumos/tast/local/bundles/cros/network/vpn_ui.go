...
func init() {
	testing.AddTest(&testing.Test{
		...
		Contacts: []string{
			...
			"cros-networking@chromium.org",
		},
		Attr:         []string{"group:mainline", "informational"},
		SoftwareDeps: []string{"chrome"},
		// Do not use shillReset here since manipulating profiles when Chrome is
		// running may cause races.
...
func VPNUI(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Failed to create VPN connection")
	}
	defer vpnConn.Cleanup(ctx)
	if err := vpnConn.SetUpWithoutService(ctx); err != nil {
		s.Fatal("Failed to setup VPN server")
	}
	...
	if err := uiauto.Combine("Disconnect VPN",
		...
		s.Fatal("Failed to disconnect VPN")
	}
