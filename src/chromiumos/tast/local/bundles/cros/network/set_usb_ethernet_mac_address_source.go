...
func SetUSBEthernetMACAddressSource(ctx context.Context, s *testing.State) {
	...
	eth, name := func() (*shill.Device, string) {
		...
		for _, devicePath := range devices {
			...
			busType, has, err := device.Properties().GetString(shill.DevicePropertyEthernetBusType)
			if !has {
				continue
			}
			if err != nil {
			...
			iface, has, err := device.Properties().GetString(shill.DevicePropertyInterface)
			if !has {
				s.Fatalf("Failed to get required device interface name: %+v", device.Properties())
			}
			if err != nil {
	...
	verifyProperty := func(prop string, expectedValue string) {
		value, has, err := eth.Properties().GetString(prop)
		if err != nil {
			s.Fatalf("Failed to get %s property: %v", prop, err)
		}
		if !has {
			s.Fatal("Device doesn't have required property: ", prop)
		}
