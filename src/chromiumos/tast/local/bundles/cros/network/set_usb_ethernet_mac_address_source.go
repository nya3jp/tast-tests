...
func SetUSBEthernetMACAddressSource(ctx context.Context, s *testing.State) {
	...
	eth, name := func() (*shill.Device, string) {
		...
		for _, devicePath := range devices {
			...
			deviceProps, err := device.Properties(ctx)
			if err != nil {
	...
	verifyProperty := func(prop string, gotValue interface{}, expectedValue string) {
		if str, ok := gotValue.(string); !ok || str != expectedValue {
			s.Fatalf("Property %s changed unexpectedly: got %v, want %v", prop, gotValue, expectedValue)
		}
	}
