...
func SetUSBEthernetMACAddressSource(ctx context.Context, s *testing.State) {
	...
	eth, name := func() (*shill.Device, string) {
		...
		for _, devicePath := range devices {
			...
			busType, ok := props[shill.DevicePropertyEthernetBusType].(string)
			if !ok {
				continue
			}
	...
	setMACSource := func(source string) {
		...
		props, err := eth.GetProperties(ctx)
		if err != nil {
			s.Fatal("Failed to load device properties: ", err)
		}

		newSource, ok := props[shill.DevicePropertyEthernetMACSource].(string)
		if !ok {
			s.Fatalf("Failed to get USB Ethernet MAC address source property: %+v", props)
		}
	...
	for _, tc := range []struct {
		...
		swPropertyChanged, err := eth.CreatePropertyChangedWatcher(ctx)
		if err != nil {
		...
		expectedChanges := map[shill.DeviceProperty]interface{}{
			shill.DevicePropertyAddress:           strings.Replace(tc.expectedMAC, ":", "", -1),
			shill.DevicePropertyEthernetMACSource: tc.source,
		}
		if err = swPropertyChanged.WaitForExpectedChanges(ctx, expectedChanges); err != nil {
