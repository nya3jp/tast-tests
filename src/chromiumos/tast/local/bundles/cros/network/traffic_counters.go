...
// for network traffic generated by certain user IDs and when connected to a
// host VPN. It does not currently verify traffic flows originating from guests.
...
func TrafficCounters(ctx context.Context, s *testing.State) {
	...
	// Generates traffic for each source on each server and returns the counters
	// obtained from patchpanel.
	getCounters := func(svrs []*server, users userSources) map[string]*counters {
	...
	test := func(svrs []*server, users userSources) {
		base := getCounters(svrs, users)
	...
	if param.vpn {
		// Connect the VPN on UDP4 router.
		conn, err := connectVPN(ctx, svrs[0])
		if err != nil {
...
func connectVPN(ctx context.Context, s *server) (*vpn.Connection, error) {
	...
	if _, err = conn.Connect(ctx); err != nil {
		return nil, errors.Wrap(err, "failed to connect to vpn")
	}
	return conn, err
}
