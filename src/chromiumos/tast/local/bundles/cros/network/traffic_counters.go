...
func TrafficCounters(ctx context.Context, s *testing.State) {
	...
	if !param.vpn {
		// IPv6 VPN not supported currently.
	...
	// Returns a slice of counter keys for the given traffic sources per device
	// and IP family.
	...
	if param.vpn {
		...
		// Inject the counter we expect to see for chrome traffic attributed to the
		// vpn user.
