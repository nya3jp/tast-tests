...
func TrafficCounters(ctx context.Context, s *testing.State) {
	...
	cfg, err := setup(ctx, mgr, pool, udpserver.UDP4)
	if err != nil {
	...
	if cfg, err = setup(ctx, mgr, pool, udpserver.UDP6); err != nil {
		s.Fatal("Failed to setup router: ", err)
	}
	cfgs = append(cfgs, cfg)
	devices = append(devices, cfg.rt.VethOutName)
	...
	test := func() map[string]*counters {
		...
		for _, cfg := range cfgs {
			for src, u := range users {
				...
				if cfg.fam == udpserver.UDP6 {
					addr = fmt.Sprintf("[%v]", addr)
					fam = pp.TrafficCounter_IPV6
				}
				dst := fmt.Sprintf("%v:%v", addr, cfg.port)
				if err := txrx(ctx, msg, u, cfg.fam.String(), dst, cfg.port+1000); err != nil {
...
func setup(ctx context.Context, mgr *shill.Manager, pool *subnet.Pool, fam udpserver.Family) (*config, error) {
