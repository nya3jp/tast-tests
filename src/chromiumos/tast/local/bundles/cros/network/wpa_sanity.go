...
func WPASanity(ctx context.Context, s *testing.State) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		cmdOut, err := testexec.CommandContext(
			ctx, "sudo", "-u", "wpa", "-g",
			"wpa", "wpa_cli", "-i", iface, "ping").CombinedOutput()
		output = string(cmdOut)
		return err
	}, &testing.PollOptions{Timeout: 5 * time.Second}); err != nil {
		s.Fatal("Failed to ping wpa_supplicant. wpa_cli output: ", output)
	}

	if !strings.Contains(output, "PONG") {
		s.Fatal("Failed to see expected PONG reply. wpa_cli output: ", output)
	}
}
