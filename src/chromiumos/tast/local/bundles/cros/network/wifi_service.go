...
func (s *WifiService) ResetTest(ctx context.Context, req *network.ResetTestRequest) (*empty.Empty, error) {
	...
	mwifiexResetPath := func(iface string) (string, error) {
	...
	mwifiexReset := func(ctx context.Context, resetPath string) error {
		...
		return testing.Poll(ctx, func(ctx context.Context) error {
			...
			// We have shortened the ctx in the beginning of the function.
			Interval: mwifiexInterval,
		})
	}
	ath10kResetPath := func(iface string) (string, error) {
		phy, err := shill.WifiPhyName(iface)
		if err != nil {
			return "", errors.Wrap(err, "failed to get the WiFi phy name")
		}
	...
	iwlwifiResetPath := func(iface string) (string, error) {
		par, err := shill.WifiParentDeviceName(iface)
		if err != nil {
			return "", errors.Wrap(err, "failed to get the WiFi parent device name")
		}
	...
	testing.ContextLogf(ctx, "Starting %d resets before each suspend; Totally %d suspends; Reset path=%s", resetNum, suspendNum, resetPath)
	...
	for i := 0; i < suspendNum; i++ {
		...
		// Suspend and wait for the service enters Idle state and IsConnected in order.
		if err := func(ctx context.Context) error {
			...
			ctx, cancel := context.WithTimeout(ctx, idleConnectTimeout)
			defer cancel()
			if err := pw.Expect(ctx, shillconst.ServicePropertyState, shillconst.ServiceStateIdle); err != nil {
				return errors.Wrap(err, "failed to wait for the service enters Idle state")
			}
