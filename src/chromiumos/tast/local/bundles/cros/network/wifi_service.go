	...
	go func(ctx context.Context) {
		defer close(done)
			...
			checkSig := func(sig *dbus.Signal) error {
				...
				bssSSID, ok := bss.Props["SSID"]
				if !ok {
					return errors.Errorf("got body %v, want string", bss.Props["SSID"])
				}
				bssSSIDs := string(bssSSID.([]byte))

				bssBSSID, ok := bss.Props["BSSID"]
				if !ok {
					return errors.Errorf("got body %v, want string", bss.Props["BSSID"])
				}
				bssBSSIDs := net.HardwareAddr(bssBSSID.([]byte)).String()
