...
// ExpectShillServiceProperty is a streaming gRPC, takes a shill service path and expects a list of property
...
// This is the implementation of network.Wifi/ExpectShillServiceProperty gRPC.
func (s *WifiService) ExpectShillServiceProperty(req *network.ExpectShillServicePropertyRequest, sender network.WifiService_ExpectShillServicePropertyServer) error {
	ctx := sender.Context()
	...
	if err := sender.Send(&network.ExpectShillServicePropertyResponse{}); err != nil {
	...
	for _, p := range req.Props {
		...
		if p.Method != network.ExpectShillServicePropertyRequest_ON_CHANGE {
			props, err := service.GetProperties(ctx)
			if err != nil {
				return err
			}
			...
			if foundIn(val, expectedVals) {
				...
				if err := sender.Send(&network.ExpectShillServicePropertyResponse{Key: p.Key, Val: shillVal}); err != nil {
			...
			if p.Method == network.ExpectShillServicePropertyRequest_CHECK_ONLY {
				return errors.Errorf("unexpected property %q: got %s, want any of %v", p.Key, val, expectedVals)
			}
		}
