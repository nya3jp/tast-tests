...
// criteria, and takes a list of shill properties to monitor. When a property's value is expected,
// it responds the property's (key, value) pair. The method sends an empty response as the property
// watcher is set. A property matching criterion consists of a property name, a list of expected
// values, a list of excluded values, and a "CheckType". We say a criterion is met iff the property
// value is in one of the expected values and not in any of the excluded values. If the property
// value is one of the excluded values, the method fails immediately. The call monitors the specified
// shill properties and returns the monitor results as a chronological list of pairs (changed property,
