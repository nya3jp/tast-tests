...
// values, and a "check" flag. We say a criterion is met iff the property value is in one
// of the expected values and not in any of the excluded values. If the property value is
// one of the excluded values, the method fails immediately. If the "check" flag is set,
// the method checks if the criterion are met without waiting for property change. So if
// none of the expected value is matched, the method fails.
