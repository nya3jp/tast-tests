...
func (s *WifiService) wait(ctx context.Context, svc *shill.Service,
	...
	for {
		vals, err := pw.WaitAll(ctx, key)
		if err != nil {
			return errors.Wrap(err, "failed to wait service state change")
		}
		if reflect.DeepEqual(value, vals[0]) {
			return nil
		}
	}
}
...
func (s *WifiService) Connect(ctx context.Context, config *network.Config) (*network.Service, error) {
	...
	// Wait until connection established with timeout 30s.
	timeoutCtx, cancel := context.WithTimeout(30 * time.Second)
	defer cancel()
	if err := s.wait(timeoutCtx, service, pw, shill.ServicePropertyIsConnected, true); err != nil {
		return nil, err
	}
...
func (s *WifiService) DeleteEntriesForSSID(ctx context.Context, ssid *network.SSID) (*empty.Empty, error) {
	...
	for _, profile := range profiles {
		...
		v, err := props.Get(shill.ProfilePropertyEntries)
		if err != nil {
