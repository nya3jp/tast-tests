...
// InitTestState properly initializes the DUT for WiFi tests.
func (s *WifiService) InitTestState(ctx context.Context, _ *empty.Empty) (*empty.Empty, error) {
	...
	if _, err := m.CreateProfile(ctx, wifiTestProfileName); err != nil {
		return nil, errors.Wrap(err, "failed to create the test profile")
	}
	// Push the test profile.
	if _, err := m.PushProfile(ctx, wifiTestProfileName); err != nil {
		return nil, errors.Wrap(err, "failed to push the test profile")
	}
	return &empty.Empty{}, nil
}

// TearDown reverts the settings made by InitTestState.
func (s *WifiService) TearDown(ctx context.Context, _ *empty.Empty) (*empty.Empty, error) {
	...
	if err := upstart.StartJob(ctx, "ui"); err != nil {
		testing.ContextLog(ctx, "Failed to start ui, err: ", err)
	}
	if retErr != nil {
		return nil, retErr
	}
