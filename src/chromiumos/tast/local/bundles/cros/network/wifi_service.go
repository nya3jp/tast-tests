...
// ExpectWifiFrequency checks if the device discovers the given SSID on the specific frequencies.
func (s *WifiService) ExpectWifiFrequency(ctx context.Context, req *network.ExpectWifiFrequencyRequest) (*empty.Empty, error) {
	...
	for {
		...
		freqs := uint16sToUint32s(fs)
		if reflect.DeepEqual(req.Frequencies, freqs) {
			break
		}
