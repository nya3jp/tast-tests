...
func (*WifiService) expectServiceProperty(ctx context.Context, service *shill.Service, prop string, val interface{}, f func() error) (dbus.Sequence, error) {
	pw, err := service.CreateWatcher(ctx)
	if err != nil {
		return 0, errors.Wrap(err, "failed to create properties watcher")
	}
	defer pw.Close(ctx)
	if err := f(); err != nil {
		return 0, err
...
// ProfileBasicTest creates, pushes, and pops the profiles and asserts the connection states between those operations.
// This is the main body of the ProfileBasic test.
func (s *WifiService) ProfileBasicTest(ctx context.Context, req *network.ProfileBasicTestRequest) (_ *empty.Empty, retErr error) {
	...
	connectAndPing := func(ctx context.Context, service *shill.Service, ap *network.ProfileBasicTestRequest_Config) error {
		...
		for k, v := range shillProps {
			if err = service.SetProperty(ctx, k, v); err != nil {
				return errors.Wrapf(err, "failed to set properties %s to %v", k, v)
			}
		}
	...
	if _, err := m.PushProfile(ctx, profileBottomName); err != nil {
		return nil, errors.Wrapf(err, "filed to push profile %q", profileBottomName)
	}
	...
	if idleSeq > isConnectedSeq {
		return nil, errors.New("expected to get the Idle signal of AP0 before the IsConnected of AP1 but got an inverse order")
	}
	...
	// Verify that popping a profile that does not affect the service should also clear the service's "explicitly disconnected" flag.
	if _, err := expectIsConnected(ctx, service0, func() error {
		return m.PopProfile(ctx, profileTopName)
	}); err != nil {
