	...
	go func(ctx context.Context) {
		defer close(done)
			...
			checkSig := func(sig *dbus.Signal) error {
				...
				bss, err := iface.ParseBSSAddedSignal(ctx, sig, []string{wpasupplicant.BSSAddedSignalPropSSID, wpasupplicant.BSSAddedSignalPropBSSID})
				if err != nil {
					return errors.Wrap(err, "failed to parse the BSSAdded signal")
				}

				bssSSID := string(bss.Props[wpasupplicant.BSSAddedSignalPropSSID].([]byte))
				bssBSSID := net.HardwareAddr(bss.Props[wpasupplicant.BSSAddedSignalPropBSSID].([]byte)).String()

				if bssSSID != request.Ssid || bssBSSID != request.Bssid {
					return errors.Errorf("unexpected SSID and BSSID values: got %s, %s, Want %s, %s", bssSSID, bssBSSID, request.Ssid, request.Bssid)
				}
