...
func (s *WifiService) RequestRoam(ctx context.Context, req *network.RequestRoamRequest) (*empty.Empty, error) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if uint32(frequency) != req.Frequency {
			return errors.Wrapf(err, "unexpected frequency: got %d, want %d", uint32(frequency), uint32(2402))
		}
	...
	if currBSSID != req.Bssid {
		return nil, errors.Wrapf(err, "unexpected BSSID: got %s, want %s", currBSSID, req.Bssid)
	}
