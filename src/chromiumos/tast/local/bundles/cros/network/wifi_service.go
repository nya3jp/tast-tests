...
func (s *WifiService) DiscoverBSSID(ctx context.Context, request *network.DiscoverBSSIDRequest) (*network.DiscoverBSSIDResponse, error) {
	...
	for _, bss := range bsses {
		...
		} else if request.Ssid == string(ssid) {
			bssid, err := bss.BSSID(ctx)
			if err != nil {
				testing.ContextLog(ctx, "Failed to get SSID for bss: ", err)
			} else if request.Bssid == net.HardwareAddr(bssid).String() {
				testing.ContextLogf(ctx, "The BSSID %s is discovered", net.HardwareAddr(bssid).String())
				return nil, nil
			}
		}
	}
	...
	go func(ctx context.Context) {
		defer close(done)
			...
			checkSig := func(sig *dbus.Signal) error {
				// Checks if the BSS is found.
				if len(sig.Body) != 2 {
					return errors.Errorf("got body length=%d, want 2", len(sig.Body))
				}

				bssProps, ok := sig.Body[1].(map[string]dbus.Variant)
				if !ok {
					return errors.Errorf("got body %v, want map[string]dbus.Variant", sig.Body[1])
				}

				bssSSID, ok := bssProps["SSID"]
				if !ok {
					return errors.Errorf("got body %v, want string", bssProps["SSID"])
				}
				bssSSIDs := string(bssSSID.Value().([]byte))

				bssBSSID, ok := bssProps["BSSID"]
				if !ok {
					return errors.Errorf("got body %v, want string", bssProps["BSSID"])
				}
				bssBSSIDs := net.HardwareAddr(bssBSSID.Value().([]byte)).String()

				if bssSSIDs != request.Ssid || bssBSSIDs != request.Bssid {
					return errors.Errorf("unexpected SSID and BSSID values: got %s, %s, Want %s, %s", bssSSIDs, bssBSSIDs, request.Ssid, request.Bssid)
				}
