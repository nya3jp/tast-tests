...
func suspend(ctx context.Context, wakeUpTimeout time.Duration) error {
	...
	readFileAndTrimSpace := func(filePath string) (string, error) {
	...
	rtcTimeSeconds := func() (int, error) {
		s, err := readFileAndTrimSpace(rtcPath)
		if err != nil {
			return 0, err
		}
	...
	if wakeUpTimeout < 2*time.Second {
		...
		return errors.Errorf("unexpected wake up timeout: got %s, want >= 2 seconds", wakeUpTimeout.String())
	}

	wc, err := readFileAndTrimSpace(wakeUpCountPath)
	if err != nil {
		return err
	}

	suspendStartTimeSeconds, err := rtcTimeSeconds()
	if err != nil {
		return err
	}

	wakeUpTimeoutSecond := int(wakeUpTimeout.Truncate(time.Second).Seconds())
	...
	suspendFinishTimeSeconds, err := rtcTimeSeconds()
	if err != nil {
		return err
	}

	testing.ContextLogf(ctx, "RTC suspend time: %d", suspendFinishTimeSeconds-suspendStartTimeSeconds)

	if suspendedInterval := suspendFinishTimeSeconds - suspendStartTimeSeconds; suspendedInterval < wakeUpTimeoutSecond {
...
func (s *WifiService) SuspendAndAssertConnect(ctx context.Context, req *network.SuspendAndAssertConnectRequest) (*network.SuspendAndAssertConnectResponse, error) {
	...
	if err := suspend(ctx, time.Duration(req.WakeUpTimeout)*time.Nanosecond); err != nil {
