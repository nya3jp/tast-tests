...
func waitIfaceRemoval(ctx context.Context, pw *shill.PropertiesWatcher, iface string) error {
	for {
		...
		for _, dPath := range devicePaths {
			...
			if err != nil {
				// This is possible if the interface is gone before we query or if timeout.
				// We'll check timeout on exit so regard it as removed here.
	...
	// This select ensures that the interface is not skipped due to deadline.
	select {
	case <-ctx.Done():
		return ctx.Err()
	default:
...
func removeIfaceAndWait(ctx context.Context, iface string) error {
	manager, err := shill.NewManager(ctx)
	if err != nil {
		return err
	}
	pw, err := manager.Properties().CreateWatcher(ctx)
	if err != nil {
		return err
	}
	defer pw.Close(ctx)
...
func IwlwifiPCIRescan(ctx context.Context, s *testing.State) {
	...
	// Remove interface and wait until the interface is gone from shill.
	testing.ContextLog(ctx, "Remove the interface and wait shill update")
	err = removeIfaceAndWait(ctx, iface)
	if err != nil {
		s.Fatal("Failed to remove interface: ", err)
	}

	// Check the interface does recover.
