...
// This test mainly verifies that, when a new network with higher priority
...
func RoutingHighPriority(ctx context.Context, s *testing.State) {
	...
	// when the service becomes Online, it may not be avaiable in both families
	...
	for _, user := range []string{"root", "chronos"} {
		...
		for _, ip := range baseRouterAddrs.All() {
