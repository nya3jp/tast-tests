...
func ShillCaptivePortalDNS(ctx context.Context, s *testing.State) {
	...
	clean := clean{cleanupCtx: ctx}
	ctx, cancel := ctxutil.Shorten(ctx, 5*time.Second)
	defer cancel()
	defer router.Cleanup(clean.cleanupCtx)
	...
	if err := captureHTTPProbe(ctx, clean, svc, router.VethOutName, runner); err != nil {
...
func waitForShillNameServer(ctx context.Context, svc *shill.Service) (string, error) {
	...
	for _, config := range configs {
		...
		if ip != nil && ip.To4() != nil {
			...
			// By setting the dnsmasq configuration, the dnsmasq service tells the DHCP client
			// that the DNS server is set to the address of the machine running dnsmasq. This
			// test return the first nameserver to check if it is the machine running dnsmasq.
...
func captureHTTPProbe(ctx context.Context, clean clean, svc *shill.Service, ifName string, runner *tcpdump.Runner) error {
