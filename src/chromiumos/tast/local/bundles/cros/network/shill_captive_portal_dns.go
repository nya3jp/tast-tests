...
func ShillCaptivePortalDNS(ctx context.Context, s *testing.State) {
	...
	if out == nil {
		s.Errorf("Unexpected tcpdump result: packets don't match %s", strings.Join(args, " "))
	}
}
...
func waitForShillNameServer(ctx context.Context, svc *shill.Service) (string, error) {
	...
	for _, ipConfig := range ipConfigs {
		p, err := ipConfig.GetProperties(ctx)
		if err != nil {
		...
		if net.ParseIP(ipAddr) != nil && net.ParseIP(ipAddr).To4() != nil {
...
func captureHTTPProbe(ctx context.Context, svc *shill.Service, ifName string, r *tcpdump.Runner) error {
	...
	if err := setCheckPortal(ctx, svc, "false"); err != nil {
		return errors.Wrap(err, "failed to disable CheckPortal")
	}
	if err := setCheckPortal(ctx, svc, "true"); err != nil {
		return errors.Wrap(err, "failed to enable CheckPortal")
	}

	return nil
}

// setCheckPortal sets the CheckPortal property in the service and
// waits for the service state to become online.
func setCheckPortal(ctx context.Context, svc *shill.Service, state string) error {
	if err := svc.SetProperty(ctx, shillconst.ServicePropertyCheckPortal, state); err != nil {
		return errors.Wrap(err, "failed to set CheckPortal")
	}
	if err := svc.WaitForProperty(
		ctx,
		shillconst.ServicePropertyState,
