...
func testParseScanResults(t *testing.T) {
	...
	l, err := iwr.parseScanResults(testStr)
	if err != nil {
		t.Error(err.Error())
	}
	cmpBss := bssData{BSS: "00:11:22:33:44:55", Frequency: 2447, SSID: "my_open_network", Security: "RSN", HT: "HT20", Signal: -46}
	if len(l) > 1 {
...
func testParseScanTime(t *testing.T) {
	...
	iwr := NewRunner(nil, nil)
	cmpFloat := 3.01
	res, err := iwr.parseScanTime(testStr)
	if err != nil {
		t.Error(err.Error())
	}
	if res != cmpFloat {
