...
// TestParsers runs unit test all parsing primitives in iw_runner.go.
...
func testParseScanResults(t *testing.T) {
	...
	iwr := NewRunner(nil, nil)
	var l []*bssData = iwr.parseScanResults(testStr)
	var cmpBss bssData = bssData{BSS: "00:11:22:33:44:55", Frequency: 2447, SSID: "my_open_network", Security: "open", HT: "HT20", Signal: -46}
	if len(l) > 1 {
...
func testParseScanTime(t *testing.T) {
	...
	iwr := NewRunner(nil, nil)
	cmpFloat := 3.01
	var res float64 = iwr.parseScanTime(testStr)
	if res != cmpFloat {
