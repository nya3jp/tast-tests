...
func TestExtractBSSID(t *testing.T) {
	const testStr1 = `Connected to 04:f0:21:03:7d:bb (on wlan0)`
	const testStr2 = `Station 04:f0:21:03:7d:bb (on mesh-5000mhz)`
	const cmpStr = `04:f0:21:03:7d:bb`
	res := extractBSSID(testStr1, "wlan0", false)
	if res != cmpStr {
		t.Errorf("extractBSSID(%q) = %q; want %q", testStr1, res, cmpStr)
	}
	res = extractBSSID(testStr2, "mesh-5000mhz", true)
	if res != cmpStr {

		t.Errorf("extractBSSID(%q) = %q; want %q", testStr2, res, cmpStr)
	}
}
