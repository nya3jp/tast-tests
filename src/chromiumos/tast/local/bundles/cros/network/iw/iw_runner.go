	...
	parsers := []struct {
	...
	}{
		{
			...
			parse: func(sectionName string, contents string) error {
				...
				for _, m := range ms {
					rateStr := strings.TrimSpace(strings.Split(m[1], ":")[1])
				...
				ms = regexp.MustCompile(`(\d+) MHz (\[\d+\])(?: \(([0-9.]+ dBm)\))?(?: \(([a-zA-Z, ]+)\))?`).FindAllStringSubmatch(contents, -1)
				for _, m := range ms {
					flags := strings.Split(string(m[4]), ",")
					if len(flags) > 0 && flags[0] != "" {
						freq, _ := strconv.Atoi(m[1])
						currentBand.FrequencyFlags[freq] = flags
					}
				}
