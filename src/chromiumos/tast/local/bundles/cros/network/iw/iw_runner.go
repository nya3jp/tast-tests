...
func extractBSSID(out string, iface string, stationDump bool) string {
	id := "Connected to"
	if stationDump {
		id = "Station"
	}
	re := regexp.MustCompile(fmt.Sprintf(`%s ([0-9a-fA-F:]{17}) \(on %s\)`,
		id, iface))
...
func parseScanResults(output string) ([]*BSSData, error) {
	var bssList []*BSSData
	re := regexp.MustCompile(`BSS ([0-9A-Fa-f]{2}([:]|)){6}`)
