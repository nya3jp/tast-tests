...
func GetInterfaceAttributes(ctx context.Context, iface string) (*NetDev, error) {
	...
	if len(matchIfs) > 1 {
		return nil, errors.Errorf("multiple interfaces match the name %s", iface)
	}
...
func parsePhyTxRx(contents string) (tx int, rx int, err error) {
...
func newPhy(phyMatch string, dataMatch string) (*Phy, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "unable to find \"max # scan SSIDs\"")
	}
	maxScanSSIDs, err := strconv.Atoi(m[0])
	if err != nil {
		return nil, errors.Wrapf(err, "unable to convert value of \"max # scan SSIDs\" to int: %s", m[0])
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to parse txAntenna/rxAntenna")
	}
	// Sections are defined as blocks of text that are delimited by level 1 indent lines.
...
func parseBand(attrs *sectionAttributes, sectionName string, contents string) error {
	...
	// Band rate handling.
	currentBand.MCSIndices, err = parseBandMCSIndices(contents)
	if err != nil {
		return errors.Wrap(err, "failed to parse band rates")
	}

	// Band channel info handling.
	currentBand.FrequencyFlags, err = parseFrequencyFlags(contents)
	if err != nil {
...
func parseSectionSpecificAttributes(output string) (*sectionAttributes, error) {
	...
	secHeads, secBodies, err := splitTextOnRegexMatches(`(?m)^\t(\w.*):\s*$`, output)
	if err != nil {
	...
	for i, m := range secHeads {
		m = strings.TrimSpace(m)
		for _, parser := range parsers {
			if !strings.HasPrefix(m, parser.prefix) {
				continue
			}
			if err := parser.parse(&attrs, m, secBodies[i]); err != nil {
				return nil, err
			}
		}
	}
...
// splitTextOnRegexMatches splits the text into sections by their header defined
// with regex. The first split which does not have a header is dropped to have
// returned sections paired with their headers.
