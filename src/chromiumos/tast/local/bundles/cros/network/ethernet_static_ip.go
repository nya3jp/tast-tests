...
func EthernetStaticIP(ctx context.Context, s *testing.State) {
	const (
		...
		testDefaultProfilePath = "/var/cache/shill/ethTestProfile.profile"
		testUserProfilePath    = "/var/cache/shill/ethTestProfile2.profile"
	...
	func() {
		...
		if err = os.Remove(testDefaultProfilePath); err != nil && !os.IsNotExist(err) {
			s.Fatal("Failed removing default profile: ", err)
		}
		if err = os.Remove(testUserProfilePath); err != nil && !os.IsNotExist(err) {
			s.Fatal("Failed removing default profile: ", err)
		}
	}()
	...
	defer func() {
		...
		os.Remove(testDefaultProfilePath)
		os.Remove(testUserProfilePath)
		shill.SafeStart(ctx)
	}()
