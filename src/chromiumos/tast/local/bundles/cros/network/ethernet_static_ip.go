...
func EthernetStaticIP(ctx context.Context, s *testing.State) {
	...
	service, err := func() (*shill.Service, error) {
		...
		return manager.WaitForServiceProperties(ctx, map[string]interface{}{shillconst.ServicePropertyType: "ethernet",
			shillconst.ServicePropertyIsConnected: true}, 8*time.Second)
	}()
	if err != nil {
