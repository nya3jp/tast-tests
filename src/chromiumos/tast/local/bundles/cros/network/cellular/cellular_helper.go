...
func (h *Helper) Enable(ctx context.Context) error {
	...
	if err := h.Device.WaitForShillProperty(ctx, shillconst.DevicePropertyScanning, false, defaultTimeout); err != nil {
...
func (h *Helper) Disable(ctx context.Context) error {
	...
	if err := h.Device.WaitForShillProperty(ctx, shillconst.DevicePropertyPowered, false, defaultTimeout); err != nil {
		return err
	}
	if err := h.Device.WaitForShillProperty(ctx, shillconst.DevicePropertyScanning, false, defaultTimeout); err != nil {
		return errors.Wrap(err, "expected scanning to become false, got true")
