...
func CrostiniConnectivity(ctx context.Context, s *testing.State) {
	...
	var ping4Addrs []net.IP
	ping4Addrs = append(ping4Addrs, routerAddrs.IPv4Addr)
	ping4Addrs = append(ping4Addrs, serverAddrs.IPv4Addr)
	for _, ip := range ping4Addrs {
		if err := cont.Command(ctx, "/bin/ping", "-c1", "-w1", ip.String()).Run(); err != nil {
			s.Fatalf("Failed to ping %s from Crostini: %v", ip.String(), err)
		}
