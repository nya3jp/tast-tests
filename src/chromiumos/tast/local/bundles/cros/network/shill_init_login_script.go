...
func ShillInitLoginScript(ctx context.Context, s *testing.State) {
	...
	if err := testLogin(ctx, &env); err != nil {
		s.Fatal("Failed running testLogin")
	}
}
...
func testLogin(ctx context.Context, env *shillscript.TestEnv) error {
	...
	if err := shillscript.AssureIsLinkTo("/run/shill/user_profiles/chronos", env.ShillUserProfileDir); err != nil {
		return err
	}
