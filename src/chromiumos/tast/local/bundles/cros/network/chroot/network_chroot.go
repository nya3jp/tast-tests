...
func NewNetworkChroot(serverInterfaceName, serverAddress string, networkPrefix int) *NetworkChroot {
	...
	localIPandPrefix := serverAddress + "/" + fmt.Sprintf("%d", networkPrefix)
...
func (n *NetworkChroot) Startup(ctx context.Context) error {
	...
	if err := testexec.CommandContext(ctx, "ip", "-n", netnsVPN, "addr", "add", n.netLocalIPandPrefix, "dev", n.netInterface).Run(); err != nil {
		return errors.Wrapf(err, "failed to add the address %d to the server", n.netLocalIPandPrefix)
	}
	...
	args := []string{"/bin/bash", filepath.Join("/", startup), "&"}
	tempArgs := []string{"netns", "exec", netnsVPN, "/sbin/minijail0", "-C", tempDir}
	temp := append(jailArgs, args...)
	tempArgs = append(tempArgs, temp...)
	if err := testexec.CommandContext(ctx, "ip", tempArgs...).Start(); err != nil {
...
func (n *NetworkChroot) Shutdown(ctx context.Context) error {
	...
	if _, err := testexec.CommandContext(ctx, "rm", "-rf", "--one-file-system", tempDir).Output(); err != nil {
...
func (n *NetworkChroot) RunChroot(ctx context.Context, args []string) error {
	...
	if err := testexec.CommandContext(ctx, "minijail0", tempArgs...).Start(); err != nil {
...
func (n *NetworkChroot) GetLogContents(ctx context.Context) (string, error) {
	if !assureExists(chrootPath("var/log/charon.log")) {
		testing.ContextLogf(ctx, "It does not exist %s", chrootPath("var/log/charon.log"))
	}

	if !assureExists(chrootPath("var/log/startup.log")) {
		testing.ContextLogf(ctx, "It does not exist %s", chrootPath("var/log/startup.log"))
	}
