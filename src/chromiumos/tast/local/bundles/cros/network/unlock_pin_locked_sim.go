...
func UnlockPinLockedSim(ctx context.Context, s *testing.State) {
	...
	settings, _ := ossettings.Launch(ctx, tconn)
	if err != nil {
	...
	s.Log("Waiting for 'Unlock' button to show on screen as cellular may be inhibitied if device is eSIM capable")
	if err := ui.WaitUntilExists(ossettings.UnlockButton.Focusable())(ctx); err != nil {
		s.Fatal("Failed to show a clickable unlock button")
	}
