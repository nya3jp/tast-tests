...
func restartInterface(ctx context.Context) error {
	...
	devPath := fmt.Sprintf("/sys/class/net/%s/device", iface.intf)
	driverPath := devPath + "/driver"

	pl, err := filepath.EvalSymlinks(devPath)
	if err != nil {
		return errors.Wrapf(err, "could not evaluate symlink on payload %s", devPath)
	}
	pth, err := filepath.EvalSymlinks(driverPath)
	if err != nil {
