...
func restartWifiInterface(ctx context.Context) error {
	...
	// Extract devices' paths belongs to the brcmfmac (Broadcom FullMAC) driver.
	// In general, only one device is associated with a driver. However, for brcmfmac driver,
	// it associates with two devices. We have to unbind/bind both.
	extractBrcmfmacDevicePaths := func(driverPath string) []string {
	...
	// Note that in older kernel, e.g. 3.14, driver name of Broadcom FullMAC is "brcmfmac_sdio";
	// however, in recent kernel, e.g. 4.19, it is named as "brcmfmac". So we use prefix match here.
	if strings.HasPrefix(filepath.Base(driverRealPath), "brcmfmac") {
		devPaths = extractBrcmfmacDevicePaths(driverPath)
		if len(devPaths) <= 1 {
			return errors.Errorf("expect brcmfmac driver has more than one devices, found %d", len(devPaths))
		}
