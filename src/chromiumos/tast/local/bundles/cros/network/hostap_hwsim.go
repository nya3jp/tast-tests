...
func HostapHwsim(ctx context.Context, s *testing.State) {
	...
	// The hostap hwsim tests normally expect to use the executables
	// compiled locally in their own source tree, but they can be convinced
	// to point at "alternate" copies (e.g., our system copies) found in
	// the log directory.
	for _, link := range []struct {
		src  string
		dest string
	}{
		{"/usr/sbin/wpa_supplicant", "alt-wpa_supplicant/wpa_supplicant/wpa_supplicant"},
		{"/usr/bin/wpa_cli", "alt-wpa_supplicant/wpa_supplicant/wpa_cli"},
		{"/usr/local/sbin/hostapd", "alt-hostapd/hostapd/hostapd"},
		{"/usr/local/bin/hostapd_cli", "alt-hostapd/hostapd/hostapd_cli"},
		{"/usr/local/sbin/hostapd", "alt-hostapd-as/hostapd/hostapd"},
		{"/usr/local/bin/hlr_auc_gw", "alt-hlr_auc_gw/hostapd/hlr_auc_gw"},
	} {
		dest := filepath.Join(td, link.dest)
		if err := os.MkdirAll(filepath.Dir(dest), 0755); err != nil {
			s.Fatalf("Failed to create directory for %q: %v", dest, err)
		}
		// Clean up symlinks from 'OutDir', so we don't bother copying
		// these into the test results.
		defer os.RemoveAll(filepath.Dir(dest))

		if err := os.Symlink(link.src, dest); err != nil {
			s.Fatalf("Failed to symlink %q -> %q: %v", link.src, dest, err)
		}
	}
	...
	cmd.Env = append(os.Environ(), "LOGDIR="+td)
	...
	if err := cmd.Wait(); err != nil {
		s.Fatal("Test wrapper failed: ", err)
	}
}
