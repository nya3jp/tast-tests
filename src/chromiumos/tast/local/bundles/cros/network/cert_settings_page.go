...
const caInvalidError = ".*ERR_CERT_AUTHORITY_INVALID.*"
...
// useWebsite attempts to open `webiste` in Chrome. If `expectCertPopup` is true,
...
func createAndUseWebsite(ctx context.Context, s *testing.State,
	...
	// It can take a bit of time for the imported certs to propogate everywhere and
	// start working. Therefore try several times until the expect result is found.
	for i := 0; i < 3; i++ {
...
func deleteClientCert(ctx context.Context, s *testing.State, ui *uiauto.Context) {
	if err := uiauto.Combine("delete CA cert",
		...
		ui.LeftClick(nodewith.Name("Show certificates for organization").Role(role.Button)),
		ui.LeftClick(nodewith.Name("More actions").Role(role.Button)),
		ui.LeftClick(nodewith.Name("Delete").Role(role.MenuItem)),
		ui.WaitUntilExists(nodewith.Name("OK").Role(role.Button)),
	)(ctx); err != nil {
		s.Fatal("Failed to delete CA cert: ", err)
	}
	// The OK button doesn't work sometimes, if the test clicks it too early.
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		return uiauto.Combine("delete CA cert",
			ui.LeftClick(nodewith.Name("OK").Role(role.Button)),
			ui.WaitUntilGone(nodewith.Name("org-TEST_CLIENT_ORG")))(ctx)
	}, nil); err != nil {
		s.Fatal("Failed to press OK: ", err)
	}
}
...
func deleteCACert(ctx context.Context, s *testing.State, ui *uiauto.Context) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		return uiauto.Combine("delete CA cert",
			...
			ui.WaitUntilGone(nodewith.Name("TEST_CA_ORG")))(ctx)
	}, nil); err != nil {
		s.Fatal("Failed to press OK: ", err)
	}
}
...
func CertSettingsPage(ctx context.Context, s *testing.State) {
	...
	createAndUseWebsite(ctx, s, browser, ui, false /*expectCertPopup*/, caInvalidError)
