...
// import it.
func importCACert(ctx context.Context, s *testing.State, ui *uiauto.Context, fileName string) {
	copyToDownloads(s, fileName)

	if err := ui.LeftClick(nodewith.Name("Authorities").Role(role.Tab))(ctx); err != nil {
		s.Fatal("Failed to click on Authorities: ", err)
	}
	ui.WaitUntilExists(nodewith.Name("Authorities").ClassName("tab selected"))(ctx)

	if err := ui.LeftClick(nodewith.Name("Import").Role(role.Button))(ctx); err != nil {
		s.Fatal("Failed to click on Import: ", err)
	}

	if err := ui.LeftClick(nodewith.Name(fileName).Role(role.StaticText))(ctx); err != nil {
		s.Fatalf("Failed to click on %s: %v", fileName, err)
	}

	if err := ui.LeftClick(nodewith.Name("Open").Role(role.Button))(ctx); err != nil {
		s.Fatal("Failed to click on Open: ", err)
	}

	const trustCheckboxText = "Trust this certificate for identifying websites"
	if err := ui.LeftClick(nodewith.Name(trustCheckboxText).Role(role.CheckBox))(ctx); err != nil {
		s.Fatal("Failed to choose trust settings: ", err)
	}

	if err := ui.LeftClick(nodewith.Name("OK").Role(role.Button))(ctx); err != nil {
		s.Fatal("Failed to click on OK: ", err)
	}
...
func importClientCert(ctx context.Context, s *testing.State, ui *uiauto.Context) {
	...
	if err := ui.LeftClick(nodewith.Name("Your certificates").Role(role.Tab))(ctx); err != nil {
		s.Fatal("Failed to click on Your certificates: ", err)
	}
	ui.WaitUntilExists(nodewith.Name("Your certificates").ClassName("tab selected"))(ctx)

	if err := ui.LeftClick(nodewith.Name("Import and Bind").Role(role.Button))(ctx); err != nil {
		s.Fatal("Failed to click on Import and Bind: ", err)
	}
	...
	if err := ui.LeftClick(passwordTextBox.Ancestor(passwordDialog))(ctx); err != nil {
		s.Fatal("Failed to click on Password Box: ", err)
	}
...
func createWebsite(s *testing.State) *httptest.Server {
	...
	testServer.TLS = &tls.Config{
		...
		// The certificate that the website present to the client, so the client can trust it.
...
func createAndUseWebsite(ctx context.Context, s *testing.State,
	...
	// The website request a client certificate and if Chrome has any, it will
...
func CertSettingsPage(ctx context.Context, s *testing.State) {
	...
	importCACert(ctx, s, ui, rootCertFileName)
