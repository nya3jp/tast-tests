...
// InstallCerts installs and writes certs for HTTPS server. In order to install certs
...
// and then mounts the tmp directory to |certDirectory|. After this is called, the
// default certs in |certDirectory| are hidden. uninstallCerts() will remove the mount
