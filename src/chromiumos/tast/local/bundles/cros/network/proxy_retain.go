...
// proxyValues defines the proxy field and its value.
...
func ProxyRetain(ctx context.Context, s *testing.State) {
	...
	proxyValues := proxyValues{
		...
		"https host": {ossettings.HTTPSHostTextField, "https://localhost"},
	...
	var (
		primaryUser   chrome.Creds
		secondaryUser chrome.Creds = chrome.Creds{User: "testuser2@gmail.com", Pass: "123456"}
	)

	s.Log("Creating user pod")
	cr, err := signIn(ctx, resources, false /* isNoLogin */, false /* isKeepState */)
	if err != nil {
		s.Fatal("Failed to create user pod: ", err)
	}
	primaryUser = cr.Creds()
	...
	var loginUsers []chrome.Option
	switch param.test.(type) {
	case *retainAfterLoginTest:
		loginUsers = []chrome.Option{
			chrome.FakeLogin(primaryUser),
		}
	case *retainAcrossUsersTest:
		loginUsers = []chrome.Option{
			chrome.FakeLogin(primaryUser),
	...
	for _, loginOpt := range loginUsers {
		func() {
			if cr, err = signIn(ctx, resources, false /* isNoLogin */, true /* isKeepState */, loginOpt); err != nil {
...
// signIn signs in user to ChromeOS with specified conditions and configures tconn and uiauto instances.
// This is an universal function across all sub-tests.
func signIn(ctx context.Context, res *proxyRetainResource, isNoLogin, isKeepState bool, loginOpt ...chrome.Option) (cr *chrome.Chrome, retErr error) {
	cleanupCtx := ctx
...
func (t *retainAfterLoginTest) preparationAtLoginScreen(ctx context.Context, res *proxyRetainResource, pv proxyValues) (retErr error) {
	...
	cr, err := signIn(ctx, res, true /* isNoLogin */, true /* isKeepState */)
	if err != nil {
...
func (t *retainAcrossUsersTest) preparationAfterLoggedIn(ctx context.Context, res *proxyRetainResource, pv proxyValues) (retErr error) {
	...
	cr, err := signIn(ctx, res, false /* isNoLogin */, true /* isKeepState */)
	if err != nil {
		return errors.Wrap(err, "failed to sign in")
	}
	defer cr.Close(cleanupCtx)
