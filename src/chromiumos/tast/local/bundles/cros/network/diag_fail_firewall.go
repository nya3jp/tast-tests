...
func DiagFailFirewall(ctx context.Context, s *testing.State) {
	...
	// When shill is reset, there is lots of activity using iptables and
	// additional commands can fail with RESOURCE_PROBLEM (exit code 4). Polling
	// makes the problem worse. This sleep ensures that the CreateFirewall call
	// will succeed.
	...
	if err := firewall.CreateFirewall(ctx, createFirewallParams); err != nil {
		s.Fatal("Unable to create firewall: ", err)
	}
