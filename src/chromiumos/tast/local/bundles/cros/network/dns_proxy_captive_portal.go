...
func DNSProxyCaptivePortal(ctx context.Context, s *testing.State) {
	...
	// So, first block these ports and the tell shill to rerun its portal detector, which should
	...
	// Unblock the HTTP/S ports which should cause shill's portal detector to succed, which will
...
// Runs dig to check name resoution works and verifies the expected server was used.
