...
// The package name and service name of the ARC-side fake VPN. These need to stay in sync with
// /vendor/google_arc/packages/system/ArcHostVpn
const arcVPNPackage = "org.chromium.arc.hostvpn"
const arcVPNService = "ArcHostVpnService"
...
func ARCVPNConnect(ctx context.Context, s *testing.State) {
	...
	// host VPN is still reachable from within ARC.
	...
	if err := expectHostPingSuccess(ctx, pr, conn.Server.OverlayIP); err != nil {
		s.Fatalf("Failed to ping from host %s: %v", conn.Server.OverlayIP, err)
	}
	if err := expectARCPingSuccess(ctx, a, "vpn", conn.Server.OverlayIP); err != nil {
	...
	// eth0 is hardcoded in ARC as the iface of our fake ethernet network that VPN traffic will
	// fall back to if there's an issue with the ARC VPN.
	if err := expectARCPingSuccess(ctx, a, "eth0", conn.Server.OverlayIP); err != nil {
		s.Fatalf("Failed to ping from ARC %s: %v", conn.Server.OverlayIP, err)
	}
}
...
// enableARCVPN currently doesn't persist any state. Flipping the flag only applies to the current
// ARC instance; the next ARC instance will start with the default value of the flag again so no
// cleanup of the flag state is needed.
...
func stopARCVPN(ctx context.Context, a *arc.ARC) error {
	...
	cmd := a.Command(ctx, "am", "force-stop", arcVPNPackage)
...
func expectHostPingSuccess(ctx context.Context, pr *ping.Runner, addr string) error {
	testing.ContextLogf(ctx, "Start to ping %s from the host", addr)
...
// expectARCPingSuccess checks if 'addr' is reachable over the 'network'. See
...
func expectARCPingSuccess(ctx context.Context, a *arc.ARC, network, addr string) error {
	testing.ContextLogf(ctx, "Start to ping %s from ARC over %s", addr, network)
