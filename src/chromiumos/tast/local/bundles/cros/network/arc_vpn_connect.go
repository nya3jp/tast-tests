...
// ARCVPNConnect differs from VPNConnect in that ARCVPNConnect focuses on testing the VPN
// that's started in ARC to mirror the host VPN. See b/147256449 for more details.
// Much of the testing around host VPN setup is left to VPNConnect to verify.
func ARCVPNConnect(ctx context.Context, s *testing.State) {
	...
	// For example, vpn.AuthTypeCert VPNs will log the user out while trying to prep the cert store.
	config := vpn.Config{
		Type:     vpn.TypeL2TPIPsecStroke,
		AuthType: vpn.AuthTypePSK,
	}

	// Start the VPN and purposely don't defer cleanup. We'll explicitly cleanup later
	// as part of the test
	...
	// Make sure ArcHostVpnService isn't running since we haven't enabled the flag yet
	if err := checkARCVPNState(ctx, a, false); err != nil {
		s.Fatal("ArcHostVpnService not supposed to be running: ", err)
	}

	// Disconnect VPN so we can enable the flag and check ARC VPN behavior next
	...
	// ArcHostVpnService is running as expected
	...
	// ARC VPN should also disconnect when the host VPN disconnects
	if err := conn.Cleanup(cleanupCtx); err != nil {
		s.Error("Failed to clean up connection: ", err)
	}
	if err := checkARCVPNState(ctx, a, false); err != nil {
	...
	// Connect the VPN again (can defer cleanup this time)
	connected, err = conn.Start(ctx)
	if err != nil || !connected {
		s.Fatal("Failed to connect to VPN server: ", err)
	}
	...
	// Force-stop ArcHostVpnService to simulate some sort of error
...
func enableARCVPN(ctx context.Context, a *arc.ARC) error {
	...
	o, err := a.Command(ctx, "dumpsys", "wifi", "set-cros-vpn-as-arc-vpn", "true").Output(testexec.DumpLogOnError)
	if err != nil {
...
// stopARCVPN force-stops the ArcHostVpnService to simulate some sort of unexpected stop (e.g. crash).
// This doesn't exercise normal ArcHostVpnService service disconnection flows.
func stopARCVPN(ctx context.Context, a *arc.ARC) error {
	testing.ContextLog(ctx, "Stopping ArcHostVpnService")
	if err := a.Command(ctx, "am", "force-stop", "org.chromium.arc.hostvpn").Run(testexec.DumpLogOnError); err != nil {
...
func expectPingSuccesses(ctx context.Context, pr *ping.Runner, a *arc.ARC, addr string) error {
	...
	// This polls for 5 seconds before it gives up on pinging from within ARC. We poll for a little bit
	// since the ARP table within ARC might not be populated yet - so give it some time before the ping
	// makes it through.
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		o, err := a.Command(ctx, "dumpsys", "wifi", "tools", "reach", addr).Output(testexec.DumpLogOnError)
		if err != nil {
