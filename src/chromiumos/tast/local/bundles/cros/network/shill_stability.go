...
func ShillStability(ctx context.Context, s *testing.State) {
	...
	// GetPID returns PID of shill. Raises an error if we don't find
	// exactly 1 shill process.
	getPID := func() int {
	...
	}, &testing.PollOptions{Timeout: stabilityDuration,
		Interval: stabilityInterval})
}
