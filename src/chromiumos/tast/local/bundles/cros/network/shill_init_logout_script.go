...
func ShillInitLogoutScript(ctx context.Context, s *testing.State) {
	...
	if err := testLogout(ctx, &env); err != nil {
		s.Fatal("Failed running testLogout")
	}
}
...
func testLogout(ctx context.Context, env *shillscript.TestEnv) error {
	...
	if err := shillscript.AssureNotExists("/run/shill/user_profiles"); err != nil {
	...
	if err := shillscript.AssureNotExists(shillscript.GuestShillUserProfileDir); err != nil {
		return errors.Wrapf(err, "failed guest shill user profile directory %s does exist", shillscript.GuestShillUserProfileDir)
	}

	if err := shillscript.AssureNotExists(shillscript.GuestShillUserLogDir); err != nil {
		return errors.Wrapf(err, "failed guest shill user log directory %s does exist", shillscript.GuestShillUserLogDir)
	}
