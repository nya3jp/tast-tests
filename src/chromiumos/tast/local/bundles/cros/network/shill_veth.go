...
func ShillVeth(ctx context.Context, s *testing.State) {
	...
	// Verify that Ethernet is connected.
	...
	if _, err := m.WaitForServiceProperties(ctx, ethProperties, 60*time.Second); err != nil {
		s.Fatal("Failed to get Ethernet Service: ", err)
	}
	...
	service, err := d.GetSelectedService(ctx)
	if err != nil {
