...
// HostVPNAlwaysOn Sets up an host VPN and check if this VPN can be configured
...
func HostVPNAlwaysOn(ctx context.Context, s *testing.State) {
	...
	if conn.SecondServer != nil {
		if err := routing.ExpectPingSuccessWithTimeout(ctx, conn.SecondServer.OverlayIP, "chronos", 10*time.Second); err != nil {
			s.Errorf("Failed to ping %s: %v", conn.SecondServer.OverlayIP, err)
		}
	}

	// Test a connection done by user applications is blocked by strict mode but not best effort mode.
	pingErr := testexec.CommandContext(ctx, "/bin/ping", "-c1", "-w1", "8.8.8.8").Run()
	switch vpnMode {
	case "strict":
		if pingErr == nil {
			s.Error("Still can ping 8.8.8.8 when strict mode VPN is disconnected: ")
		}
	case "best-effort":
		if pingErr != nil {
			s.Error("Cannot ping 8.8.8.8 when best effort mode VPN is disconnected: ", pingErr)
		}
	}
