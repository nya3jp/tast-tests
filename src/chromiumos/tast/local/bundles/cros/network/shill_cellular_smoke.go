...
func ShillCellularSmoke(ctx context.Context, s *testing.State) {
	...
	ethernetEnabled, err := helper.Manager.IsEnabled(ctx, shill.TechnologyEthernet)
	if err != nil {
	...
	wifiEnabled, err := helper.Manager.IsEnabled(ctx, shill.TechnologyWifi)
	if err != nil {
	...
	if ethernetEnabled || wifiEnabled {
		ctxForEnable := ctx
		newCtx, cancel := ctxutil.Shorten(ctx, 1*time.Second)
		ctx = newCtx
		defer cancel()
	...
	if err != nil {
		s.Fatal("Unable to find get IsConnected for Service: ", err)
	}
	if !isConnected {
	...
	// Unfortunately it is not documented anywhere in the chromeos source or google3.
	...
	// LookupHost periodically fails, so try multiple times to prevent flakiness.
	const lookupAttempts = 10
	var addrs []string
	...
	if err != nil || len(addrs) < 1 {
		s.Fatalf("Unable to lookup address for host: %s: after %d attempts: %s", hostName, lookupAttempts, err)
	}
	s.Log("IP: ", addrs[0])
	...
	if routeIface != deviceIface {
		s.Fatalf("Wrong route interface: got %s, Device interface: %s", routeIface, deviceIface)
	}
