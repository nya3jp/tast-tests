...
func ExpectPingSuccessWithTimeout(ctx context.Context, addr, user string, timeout time.Duration) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		o, err := testexec.CommandContext(ctx, "conntrack", "-D", "-f", "ipv4", "-p", "icmp").CombinedOutput()
		testing.ContextLog(ctx, string(o))
		o, err = testexec.CommandContext(ctx, "conntrack", "-D", "-f", "ipv6", "-p", "icmpv6").CombinedOutput()
		testing.ContextLog(ctx, string(o))
