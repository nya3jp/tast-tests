...
func Ping(ctx context.Context, s *testing.State, cfg pingConfig) (pingResult, error) {
	...
	if status, _ := testexec.GetWaitStatus(err); int(status) != 0 {
		return pingResult{}, errors.New(fmt.Sprintf("Ping timed out, can't parse output. Exit status: %d", status))
	}
	if out == "" {
		return pingResult{}, errors.New(fmt.Sprintf("No output from stdout. stderr is %s", err.Error()))
	}
...
func SimplePing(ctx context.Context, s *testing.State, hostName string) (bool, error) {
...
func NewFullPingConfig(ip string, ct int, size int, interval float64, iface string, qos string) pingConfig {
	return pingConfig{TargetIP: ip,
		Count:       ct,
		Size:        size,
		Interval:    interval,
...
// than NewPingResult.
func NewFullPingResult(snt int, rcv int, loss float64, minL float64, avgL float64, maxL float64, devL float64) pingResult {
	return pingResult{
		Sent:       snt,
		Received:   rcv,
		Loss:       loss,
...
func parseOutput(out string) (pingResult, error) {
	...
	if len(matchGroup) != 3 {
		return pingResult{}, errors.New(fmt.Sprintf("Parse error on lost packets. matched groups : %d", len(matchGroup)))
	}
