...
func NewRunner(contxt context.Context, state *testing.State, cfg pingConfig) runner {
	return runner{PingCommand: "ping",
		LossThreshold: 20,
		Config:        cfg,
		s:             state,
		ctx:           contxt}
...
func NewFullPingResult(snt int, rcv int, loss int, minL float64, avgL float64, maxL float64, devL float64) {
	return pingResult{
		Sent: snt,
		Received: rcv,
		Loss: loss,
...
func (pr runner) cfgToArgs() (string, error) {
	args := []string	
...
func (pr runner) parseOutput(out string) (pintResult, error){
