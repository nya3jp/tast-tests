...
func ARCVPNConfigs(ctx context.Context, s *testing.State) {
	...
	//
	// We specifically don't use a L2TP type because shill overrides the MTU value into a hardcoded
	// value. This eventually gets set properly again on the host-side, but Chrome passes the overridden
	// value to ARC so it won't get reflected properly in ARC. Note that the hardcoded value is a minimum
	// valid MTU size so it doesn't break any correctness.
	config1 := vpn.Config{
	...
	oStr := string(o)
	if err := checkMatch(oStr, `capabilities=.*`, `NOT_METERED`, true); err != nil {
		s.Fatal("Failed to verify capabilities on ARC VPN network: ", err)
	}
	if err := checkMatch(oStr, `mtu=.*`, `576`, true); err != nil {
		s.Fatal("Failed to verify MTU on ARC VPN network: ", err)
	}
	if err := checkMatch(oStr, `domains=.*`, `bar1 foo1`, true); err != nil {
	...
	// Connect with a different config and verify values. Use values that are different from the first
	// connection's config's values to ensure we didn't just get lucky with some default values.
	config2 := vpn.Config{
		Type:          vpn.TypeWireGuard,
		Metered:       true,
		MTU:           1280,
	...
	oStr = string(o)
	if err := checkMatch(oStr, `capabilities=.*`, `NOT_METERED`, false); err != nil {
		s.Fatal("Failed to verify capabilities on ARC VPN network: ", err)
	}
	if err := checkMatch(oStr, `mtu=.*`, `1280`, true); err != nil {
		s.Fatal("Failed to verify MTU on ARC VPN network: ", err)
	}
	if err := checkMatch(oStr, `domains=.*`, `bar2 foo2`, true); err != nil {
		s.Fatal("Failed to verify search domains on ARC VPN network: ", err)
	}
}

// checkMatch will check that there is some 'lineRegex' within 'input'. And within the submatch that matched the
// 'lineRegex', we should/shouldn't 'match' a 'value'.
//
// For example, given:
//
//	input:
...
//	        true
//
// In the example above, the 'lineRegex' would find the submatch 'bar=world jupiter'. And we expect to find 'world'
// somewhere within that submatch. The 'lineRegex' must be found within the given 'input'. Whether the 'value' is
// expected in the submatch is up to 'match'.
func checkMatch(input, lineRegex, value string, match bool) error {
