...
func DNSProxyOverVPN(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Failed to setup DNS virtualenv: ", err)
	}
	defer router.Cleanup(cleanupCtx)
	defer server.Cleanup(cleanupCtx)

	// Create and connect to a VPN server.
	vpnServer, err := createVPNServer(ctx, pool, router, httpsCerts)
	if err != nil {
...
// createVPNServer creates a VPN server and connects to it.
func createVPNServer(ctx context.Context, pool *subnet.Pool, router *env.Env, httpsCerts *certs.Certs) (serverEnv *env.Env, err error) {
	...
	conn, err := vpn.NewConnectionWithServer(ctx, config, server, nil)
	if err != nil {
