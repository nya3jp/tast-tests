...
func CellularPolicyConnection(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			managedNetworksWithBuildingIcon := nodewith.NameRegex(regexp.MustCompile(".*ManagedProfile,.*Managed by your Administrator.*")).Role(role.GenericContainer)
			infos, err := ui.NodesInfo(ctx, managedNetworksWithBuildingIcon)
			if err != nil {
				s.Fatal("Failed to get nodes info: ", err)
			}
			managedNetworkWithBuildingIcon := nodewith.NameRegex(regexp.MustCompile(".*ManagedProfile,.*Managed by your Administrator.*")).Role(role.GenericContainer).Nth(len(infos))
			...
			if err := ui.WaitUntilExists(ossettings.RoamingToggle)(ctx); err != nil {
				s.Log("Get back to network subpage")
