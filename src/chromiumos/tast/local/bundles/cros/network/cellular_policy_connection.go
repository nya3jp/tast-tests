...
func CellularPolicyConnection(ctx context.Context, s *testing.State) {
	...
	managedIccid := ""
	unmanagedCellularName := ""
	if managedIccid, unmanagedCellularName, err = enableEachCellularBeforeTest(ctx, tconn, cr); err != nil {
...
func enableEachCellularBeforeTest(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome) (string, string, error) {
	...
	if p == nil {
		return "", "", errors.Wrap(err, "failed to find the enabled profile")
	}

	// Disable all profiles before starting the test
	testing.ContextLogf(ctx, "Disabling profile %s", p.String())
	...
	testing.ContextLogf(ctx, "Getting iccid %s", managedIccid)
	...
	testing.ContextLogf(ctx, "Getting managed profile nickname %s", managedNickname)
	...
	for _, profile := range profiles {
		...
		if iccid != managedIccid && nickname != managedNickname {
	...
	testing.ContextLogf(ctx, "Getting unmanaged profile nickname %s", unmanagedNickname)
