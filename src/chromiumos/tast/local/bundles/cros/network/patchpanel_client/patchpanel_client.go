...
// TODO(crbug.com/1135106): Implements missing patchpanel D-Bus API methods
...
func (c *Client) ConnectNamespace(ctx context.Context, pid int32, outboundPhysicalDevice string,
	...
	bufRequest, err := proto.Marshal(request)
	if err != nil {
	...
	if err := c.obj.CallWithContext(ctx, connectNamespaceMethod, 0, bufRequest, remoteFd).Store(&bufResponse); err != nil {
		local.Close()
		return nil, nil, errors.Wrapf(err, "failed reading %s response", connectNamespaceMethod)
	}

	response := &pp.ConnectNamespaceResponse{}
	if err := proto.Unmarshal(bufResponse, response); err != nil {
		local.Close()
		return nil, nil, errors.Wrapf(err, "failed unmarshaling %s response", connectNamespaceMethod)
	}
