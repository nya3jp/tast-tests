...
func init() {
	testing.AddTest(&testing.Test{
		Func:         DHCPWebProxy,
		Desc:         "Verify that WPAD option got from DHCP is reflected in the IPConfig object exposed by shill",
...
func DHCPWebProxy(ctx context.Context, s *testing.State) {
	...
	if props.WebProxyAutoDiscoveryURL != wpad {
		s.Fatalf("Value for WPAD do not match: got %v, want %v", props.WebProxyAutoDiscoveryURL, wpad)
	}
}
