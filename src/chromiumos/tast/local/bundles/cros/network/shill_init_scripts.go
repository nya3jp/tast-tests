...
func dbusEventMonitor(ctx context.Context, chStartAction chan error) (func() ([]string, error), error) {
	...
	if err := testexec.CommandContext(ctx, "dbus-send", "--system", "/", "com.dummy.DummyStartSignal").Run(); err != nil {
		return stop, errors.Wrap(err, "failed sending dummy signal to start to start logging the D-Bus calls we want to observe")
	}
	...
	go func() {
		...
		case <-ctx.Done():
			// Timeout.
			chStartAction <- errors.New("failed due to timeout")
