...
func dbusEventMonitor(ctx context.Context, env *testEnv) chan error {
	...
	go func() {
		defer func() {
			...
			if err := cmd.Kill(); err != nil {
				ch <- errors.Wrap(err, "failed to kill dbus monitor")
			} else {
				ch <- nil
			}
			cmd.Wait()
