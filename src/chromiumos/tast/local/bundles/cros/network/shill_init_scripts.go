...
func dbusEventMonitor(ctx context.Context) (func() ([]string, error), error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to connect to private system bus")
	}
	err = conn.Auth(nil)
	if err != nil {
		conn.Close()
		return nil, errors.Wrap(err, "failed to authenticate the private system bus")
	}
	err = conn.Hello()
	if err != nil {
		conn.Close()
		return nil, errors.Wrap(err, "failed to send the Hello call to the private system bus")
	}
