...
type testEnv struct {
	// Root Cryptohome Directory.
	rootCryptohomeDir string
	// User Cryptohome Directory.
...
// convert types take an context.Context and return a error.
type convert func(ctx context.Context, env *testEnv) error
...
func ShillInitScripts(ctx context.Context, s *testing.State) {
	...
	defer func() {
		...
		if err := endTest(ctx, &env); err != nil {
			s.Fatal("Failed ending test: ", err)
		}
	}()

	if err := startTest(ctx, &env); err != nil {
		s.Fatal("Failed starting the test: ", err)
	}

	// Run test: startShill.
	if err := runTest(ctx, &env, "testStartShill", testStartShill); err != nil {
		s.Fatal("Failed running the test_start_shill: ", err)
	}

	// Run test: testStartLoggedIn.
	if err := runTest(ctx, &env, "testStartLoggedIn", testStartLoggedIn); err != nil {
		s.Fatal("Failed running the testStartLoggedIn: ", err)
	}
	...
	if err := runTest(ctx, &env, "testLogin", testLogin); err != nil {
		s.Fatal("Failed running the testLogin: ", err)
	}
	...
	if err := runTest(ctx, &env, "testLoginGuest", testLoginGuest); err != nil {
		s.Fatal("Failed running the testLoginGuest: ", err)
	}
	...
	if err := runTest(ctx, &env, "testLoginProfileExists", testLoginProfileExists); err != nil {
		s.Fatal("Failed running the testLoginProfileExists: ", err)
	}
	...
	if err := runTest(ctx, &env, "testLoginMultiProfile", testLoginMultiProfile); err != nil {
		s.Fatal("Failed running the testLoginMultiProfile: ", err)
	}
	...
	if err := runTest(ctx, &env, "testLogout", testLogout); err != nil {
		s.Fatal("Failed running the testLogout: ", err)
	}
}

// runTest runs the test, stops shill and erase the state.
func runTest(ctx context.Context, env *testEnv, name string, fn convert) error {
	if err := fn(ctx, env); err != nil {
		return errors.Wrapf(err, "failed running the test: %s", name)
	}
	if err := stopShill(ctx); err != nil {
...
func startTest(ctx context.Context, env *testEnv) error {
...
// endTest performs cleanup at the end of the test.
func endTest(ctx context.Context, env *testEnv) error {
	var errMsg error
	errMsg = nil
	if err := eraseState(ctx, env); err != nil {
...
func testLoginProfileExists(ctx context.Context, env *testEnv) error {
	...
	if err := os.Mkdir(env.shillUserProfileDir, 0777); err != nil {
		return errors.Wrapf(err, "failed creating the directory: %s", env.shillUserProfileDir)
	}
	if err := touch(env.shillUserProfile); err != nil {
		return err
	}
	if err := login(ctx, fakeUser); err != nil {
