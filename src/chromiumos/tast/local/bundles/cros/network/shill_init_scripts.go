...
type testFuncType func(ctx context.Context, done chan error, quit chan struct{}, env *testEnv) error
...
func ShillInitScripts(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < len(testName); i++ {
		if err := runTest(ctx, &env, testName[i], done, quit, testFunc[i]); err != nil {
...
func runTest(ctx context.Context, env *testEnv, name string, done chan error, quit chan struct{}, fn testFuncType) error {
	if err := fn(ctx, done, quit, env); err != nil {
		return err
	}
...
func testStartShill(ctx context.Context, done chan error, quit chan struct{}, env *testEnv) error {
...
func testStartLoggedIn(ctx context.Context, done chan error, quit chan struct{}, env *testEnv) error {
...
func testLogin(ctx context.Context, done chan error, quit chan struct{}, env *testEnv) error {
	if err := startShill(ctx); err != nil {
		return err
	}
	...
	if err := assureMethodCalls(expectedCalls, env); err != nil {
		return err
	}
...
func testLoginGuest(ctx context.Context, done chan error, quit chan struct{}, env *testEnv) error {
...
func testLoginProfileExists(ctx context.Context, done chan error, quit chan struct{}, env *testEnv) error {
	if err := startShill(ctx); err != nil {
		return err
	}
...
func dbusEventMonitor(ctx context.Context, done chan error, quit chan struct{}, env *testEnv) {
	...
	if err := cmd.Start(); err != nil {
		done <- errors.Wrap(err, "failed to spawn \"dbus monitor\"")
	}
	...
	go func() {
		defer func() {
			...
			if err := cmd.Kill(); err != nil {
				done <- errors.New("failed to kill dbus monitor")
			}
			cmd.Wait()
		}()
		env.calledMethods = nil
		...
		for scanner.Scan() {
			...
			default:
				...
				if strings.Contains(line, createUserProfile) {
					env.calledMethods = append(env.calledMethods, createUserProfile)
				}

				if strings.Contains(line, insertUserProfile) {
					env.calledMethods = append(env.calledMethods, insertUserProfile)
				}
			}
		}
		done <- nil
...
func assureMethodCalls(expectedMethodCalls []string, env *testEnv) error {
	if len(expectedMethodCalls) != len(env.calledMethods) {
		return errors.Errorf("found unexpected number of method calls: got %d, want %d ", len(env.calledMethods), len(expectedMethodCalls))
	}

	found := false
	for _, expected := range expectedMethodCalls {
		found = false
		for _, actual := range env.calledMethods {
			if expected == actual {
				found = true
				break
			}
		...
		if !found {
			return errors.Errorf("An expected method was not called: %s", expected)
		}
	}
...
func testLogout(ctx context.Context, done chan error, quit chan struct{}, env *testEnv) error {
	if err := startShill(ctx); err != nil {
		return err
	}
...
func testLoginMultiProfile(ctx context.Context, done chan error, quit chan struct{}, env *testEnv) error {
	if err := startShill(ctx); err != nil {
		return err
	}
