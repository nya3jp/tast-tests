...
// dbusEventMonitor monitors the system message bus for those D-Bus calls we want to observe (defined in isWhitelist()) and returns a stop function.
// The stop function stops the D-Bus monitor and return the called methods and/or error.
func dbusEventMonitor(ctx context.Context, chStartAction chan error) (func() ([]string, error), error) {
	...
	if err != nil {
		...
		return stop, errors.Wrap(err, "failed to get stdout reader")
	}

	if err := cmd.Start(); err != nil {
		ch <- errors.Wrap(err, "failed to spawn \"dbus monitor\"")
		return stop, errors.Wrap(err, "failed to spawn \"dbus monitor\"")
	}
	...
	go func() {
		...
		if err := testing.Sleep(ctx, 100*time.Millisecond); err != nil {
			chStartAction <- errors.Wrap(err, "failed to sleep")
			return
		}
		// Send a dummy dbus signal that is used to start logging the D-Bus calls we want to observe.
		if err := testexec.CommandContext(ctx, "dbus-send", "--system", "/", "com.dummy.DummyStartSignal").Run(); err != nil {
			chStartAction <- errors.Wrap(err, "failed sending dummy signal to start logging the D-Bus calls we want to observe")
			return
		}
		for scanner.Scan() {
			line := scanner.Text()
			...
			if isValid {
				if dbusCmd == dummyStartSignal {
					testing.ContextLog(ctx, "Found dummyStartSignal================================")
					chStartAction <- nil
					start = true
					continue
				}
				if start {
					if dbusCmd == dummyEndSignal {
						testing.ContextLog(ctx, "Found dummyEndSignal///////////////////////////////")
						ch <- nil
						return
					}
		...
		case <-ctx.Done():
			// Timeout.
			if start {
				ch <- errors.New("failed due to timeout couldn't find dummyEndSignal")
			} else {
				chStartAction <- errors.New("failed due to timeout couldn't find dummyStartSignal")
			}
		default:
			if start {
...
func testLogin(ctx context.Context, env *testEnv) error {
	...
	testing.ContextLog(ctx, "testLogin#########################")

	timeoutCtx, cancel := context.WithTimeout(ctx, dbusMonitorTimeout)
	defer cancel()

	ch := make(chan error, 1)
	stop, err := dbusEventMonitor(timeoutCtx, ch)
	if err != nil {
		return err
	}
...
func testLoginGuest(ctx context.Context, env *testEnv) error {
	...
	testing.ContextLog(ctx, "testLoginGuest#########################")
...
func testLoginProfileExists(ctx context.Context, env *testEnv) error {
	...
	testing.ContextLog(ctx, "testLofinProfileExists#########################")
...
func testLogout(ctx context.Context, env *testEnv) error {
	...
	testing.ContextLog(ctx, "testLogout#########################")
...
func testLoginMultiProfile(ctx context.Context, env *testEnv) error {
	...
	testing.ContextLog(ctx, "testLoginMultiProfile#########################")
