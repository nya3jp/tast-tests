...
func ShillInitScripts(ctx context.Context, s *testing.State) {
	...
	defer func() {
		if err := tearDownTest(ctx, &env); err != nil {
			s.Fatal("Failed ending test: ", err)
		}
	}()

	if err := setUpTest(ctx, &env); err != nil {
		s.Fatal("Failed starting the test: ", err)
	}

	var tests = []string{"testStartShill", "testStartLoggedIn", "testLogin", "testLoginGuest", "testLoginProfileExists", "testLoginMultiProfil", "testLogout"}

	// Run all the tests.
	for _, test := range tests {
		if err := runTest(ctx, &env, test, testStartShill); err != nil {
			s.Fatalf("Failed running the %s: %v", test, err)
		}
	}
}
...
// setUpTest setup the start of the test. Stop shill and create test harness.
func setUpTest(ctx context.Context, env *testEnv) error {
...
// tearDownTest performs cleanup at the end of the test.
func tearDownTest(ctx context.Context, env *testEnv) error {
	var errMsg error
	errMsg = nil
