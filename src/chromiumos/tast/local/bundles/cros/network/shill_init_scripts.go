...
func dbusEventMonitor(ctx context.Context) (func() ([]string, error), error) {
	...
	if err != nil {
		return stop, errors.Wrap(err, "failed to connect to private system bus")
	}
	err = conn.Auth(nil)
	if err != nil {
		conn.Close()
		return stop, errors.Wrap(err, "failed to authenticate the private system bus")
	}
	err = conn.Hello()
	if err != nil {
		conn.Close()
		return stop, errors.Wrap(err, "failed to send the Hello call to the private system bus")
	}

	var rules = []string{
		"type='method_call',member='InsertUserProfile',path='/',interface='org.chromium.flimflam.Manager'",
		"type='method_call',member='CreateProfile',path='/',interface='org.chromium.flimflam.Manager'",
		"type='method_call',member='PopAllUserProfiles',path='/',interface='org.chromium.flimflam.Manager'",
		"type='signal',member='DummyEndSignal',path='/',interface='com.dummy'",
	}
	var flag uint
	call := conn.BusObject().CallWithContext(ctx, "org.freedesktop.DBus.Monitoring.BecomeMonitor", 0, rules, flag)
	if call.Err != nil {
		return stop, errors.Wrap(call.Err, "failed to become monitor ")
	}
...
func dbusCallMember(dbusMessage *dbus.Message) (string, error) {
	...
	return "", errors.Errorf("failed found unexpected call: got %s, want %v", str, whitelistDbusCmd)
}
