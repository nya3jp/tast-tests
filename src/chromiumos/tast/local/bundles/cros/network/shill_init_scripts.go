...
// dbusEventMonitor monitors the system message bus for expectet dbus calls (dbus calls from Shill init Scripts).
	...
	go func() {
		...
		for scanner.Scan() {
			...
			if dbusCmd == dummySignal {
				if expectNoCalls || len(expectedCalls) == 0 {
					ch <- nil
				} else {
			...
			if !match {
				ch <- errors.Errorf("failed found unexpected call: %s", dbusCmd)
				return
			}
		}
		select {
		case <-ctx.Done():
			// Timeout.
			ch <- errors.Error("failed due to timeout")
		default:
