...
// dbusEventMonitor monitors the system message bus and updates env.calledMethods for those D-Bus calls we want to observe (defined in isWhitelist()).
// The monitor stops when it catches dummySignal. Callers can invoke the returned function to send a DummySignal on the system bus.
func dbusEventMonitor(ctx context.Context, env *testEnv) func() error {
	...
	stop := func() error {
		...
		if err := testexec.CommandContext(ctx, "dbus-send", "--system", "/", "com.dummy.DummySignal").Run(); err != nil {
			return errors.Wrap(err, "failed sending dummy signal to end the dbusEventMonitor")
		}
		if err := <-ch; err != nil {
			return err
		}
	...
	go func() {
		...
		for scanner.Scan() {
			...
			if isValid {
				if dbusCmd == dummySignal {
					ch <- nil
					return
				}
				env.calledMethods = append(env.calledMethods, dbusCmd)
			}
		}
		select {
		case <-ctx.Done():
			// Timeout.
			ch <- errors.New("failed due to timeout")
		default:
...
func inWhitelist(str string) (bool, string) {
	whitelistDbusCmd := []string{insertUserProfile, popAllUserProfiles, createUserProfile, dummySignal}
...
func assureMethodCalls(ctx context.Context, expectedMethodCalls []string, env *testEnv) error {
...
func testLogin(ctx context.Context, env *testEnv) error {
	...
	stop := dbusEventMonitor(timeoutCtx, env)
	...
	if err := assureMethodCalls(ctx, expectedCalls, env); err != nil {
		return err
	}
