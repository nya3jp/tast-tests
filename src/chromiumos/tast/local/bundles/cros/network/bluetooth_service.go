...
func (s *BluetoothService) SetBluetoothStatus(ctx context.Context, req *network.BluetoothStatusRequest) (*empty.Empty, error) {
	state := req.State
...
func (s *BluetoothService) GetBluetoothStatus(ctx context.Context, _ *empty.Empty) (*network.BluetoothStatusResponse, error) {
	status, err := bluetoothStatus(ctx)
	if err != nil {
		return nil, errors.Wrap(err, "error in querying bluetooth status")
	}
	return &network.BluetoothStatusResponse{Status: status}, nil
}
