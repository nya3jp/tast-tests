...
func (s *BluetoothService) SetBluetoothPowered(ctx context.Context, req *network.SetBluetoothPoweredRequest) (*empty.Empty, error) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		out, err := ioutil.ReadFile("/home/chronos/Local State")
		if err != nil {
			return err
		}
		if !strings.Contains(string(out), "\"ash\":{\"system\":{\"bluetooth\":{\"adapter_enabled\":false}}") {
			return errors.New("failed to change Local State Bluetooth setting")
		}
		return nil
	}, &testing.PollOptions{
