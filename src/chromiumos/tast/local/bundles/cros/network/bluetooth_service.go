...
func (s *BluetoothService) SetBluetoothPowered(ctx context.Context, req *network.SetBluetoothPoweredRequest) (*empty.Empty, error) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		runner := &cmd.LocalCmdRunner{}
		command := "jq"
		args := []string{
			".ash.system.bluetooth.adapter_enabled",
			"\"/tmp/home/chronos/Local State\"",
		}
		output, err := runner.Output(ctx, command, args...)
		if err != nil {
			return err
		} else if string(output) != "true" {
			return errors.Errorf("Local State value not set correctly: %v", string(output))
		}
		return nil
	}, &testing.PollOptions{
