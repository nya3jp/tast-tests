...
func ModemmanagerSlotSwitch(ctx context.Context, s *testing.State) {
	...
	// Exit if dut has only one slot
	if err == ErrorSingleSlot {
		s.Logf("Can not run this test as: ", err)
		return
	} else if err != nil {
	...
	if err = testing.Poll(ctx, func(ctx context.Context) error {
		...
		s.Logf("Modem paths are Old: %s ,New: %s ", modem.String(), modem1.String())
		return nil
...
// Do greaceful Exit from this test if DUT got single SIM slot error
var ErrorSingleSlot = errors.New("Single/No SIM slot DUT, Need two slots to run this test")
