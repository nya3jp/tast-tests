...
func ModemmanagerSlotSwitch(ctx context.Context, s *testing.State) {
	...
	err = setPrimarySimSlot(ctx, newslot)
	if err != nil {
	...
	err = setPrimarySimSlot(ctx, primary)
	if err != nil {
...
func pollModem(ctx context.Context, modem *modemmanager.Modem) error {
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		newModem, err := modemmanager.NewModem(ctx)
		if err != nil {
			return err
		}
		if modem.String() == newModem.String() {
			return errors.New("Old modem still exists")
		}
		testing.ContextLogf(ctx, "Modem paths are Old: %s, New: %s ", modem, newModem)
		return nil
