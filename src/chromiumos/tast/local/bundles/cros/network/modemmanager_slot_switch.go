...
func ModemmanagerSlotSwitch(ctx context.Context, s *testing.State) {
	...
	cleanupCtx := ctx
	ctx, cancel := ctxutil.Shorten(ctx, mmconst.SlotSwitchOperationTime)
	defer cancel()
	...
	if err = testing.Poll(ctx, func(ctx context.Context) error {
		...
		s.Logf("Modem paths are Old: %s, New: %s ", modem, newModem)
		return nil
	...
	defer newModem.Call(cleanupCtx, "SetPrimarySimSlot", primary)
