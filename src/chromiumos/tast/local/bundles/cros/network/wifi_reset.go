...
func selectAP(ctx context.Context, manager *shill.Manager) (map[string]interface{}, error) {
	var str uint8 = 0
	index := -1
	for i, ap := range aps {
		...
		if ok {
			return nil, errors.Wrapf(err, "parsing error on strength field %T", ap["Strength"])
		}
		if s >= str {
			str = s
			index = i
...
func WifiReset(ctx context.Context, s *testing.State) {
	...
	props, err := selectAP(ctx, manager)
	if err != nil {
		s.Fatal(err)
	}
	s.Logf("Wifi SSID: %s - Wifi Strength %d", props["Name"], props["Strength"])
	...
	for i := 0; i < numSuspends; i++ {
		...
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			_, err := network.FindWirelessInterface()
			return err
