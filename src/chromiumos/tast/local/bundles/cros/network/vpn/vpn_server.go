...
// Contants that used by the l2tpipsec_psk server.
const (
	ChapUser                           = "chapuser"
	...
	ipsecCaCertificate                 = "etc/ipsec.d/cacerts/ca.cert"
	ipsecServerCertificate             = "etc/ipsec.d/certs/server.cert"
	pppdPidFile                        = "run/ppp0.pid"
	XauthUser                          = "xauth_user"
	XauthPassword                      = "xauth_password"
	xauthSecondaryAuthenticationStanza = "rightauth2=xauth"
...
type l2tpipSecVpnServer struct {
	...
	performXauthAuthentication bool
}

// NewL2tpipSecVpnServer creates a new L2tpipSecVpnServer.
func NewL2tpipSecVpnServer(ctx context.Context, authType string, interfaceName string, address string, networkPrefix int, perfXauthAuth bool, localIPIsPublicIP bool) *l2tpipSecVpnServer {
	networkChroot := chroot.NewNetworkChroot(interfaceName, address, networkPrefix)

	return &l2tpipSecVpnServer{authenticationType: authType, netChroot: networkChroot, performXauthAuthentication: perfXauthAuth}
}
...
func (s *l2tpipSecVpnServer) StartServer(ctx context.Context) error {
	...
	if s.performXauthAuthentication {
		configValues["xauth_stanza"] = xauthSecondaryAuthenticationStanza
	}

	chro.AddConfigValues(configValues)
	chro.AddStartupCommand(fmt.Sprintf("%s start", ipsecCommand))

	tmpStr := fmt.Sprintf("%s -c /%s -C /tmp/l2tpd.control", xl2tpdCommand, xl2tpdConfigFile)
	chro.AddStartupCommand(tmpStr)
...
func (s *l2tpipSecVpnServer) Exit(ctx context.Context) error {
	...
	testing.ContextLogf(ctx, "Log contents: %s", content)
