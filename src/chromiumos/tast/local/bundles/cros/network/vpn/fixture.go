...
func (f *vpnFixture) Reset(ctx context.Context) error {
	...
	// We need to reset shill when the test failed, and thus it will invalid
	// the shill profile known to Chrome. Since there seems to be no reliable
	// way to check that Chrome gets new profile, let's do a full restart of
	// this fixture.
	if f.hasError {
		f.hasError = false
