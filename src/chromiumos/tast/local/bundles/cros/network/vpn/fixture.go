...
//     Note that a server CA certificate is stored as a raw string in shill, but Chrome
//     knows it only if it's installed in TPM.
//   - Start a new Chrome session.
//
// When a test failed, to ensure we have a clean setup, a full restart of this
// fixture will happen.
...
func (f *vpnFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	// Install CA cert to TPM, so that Chrome can know this CA cert.
