...
func (c *Connection) startServer(ctx context.Context) error {
	...
	case TypeWireGuard:
		c.Server, err = StartWireGuardServer(ctx, c.config.AuthType == AuthTypePSK, false /*isSecondServer*/)
		if err == nil && c.config.WGTwoPeers {
			// Always sets preshared key for the second peer.
			c.SecondServer, err = StartWireGuardServer(ctx, true /*usePSK*/, true /*isSecondServer*/)
		}
	default:
