...
type Config struct {
	...
	// the client side.
	WGAutoGenKey bool
	// OverlayAddressType defines the type of a list selected by the user for
	// the overlay IP address. If IPv4, the list contains one IPv4 address.
	// If IPv6, it contains multiple IPv6 addresses. If IPv4AndIPv6, it contains
	// one IPv4 address and multiple IPv6 addresses (one IPv4 address and
...
// OverlayAddressType defines IP address type of overlay IP address.
type OverlayAddressType int

const (
	// IPv4 address is used for overlay IP address.
	IPv4 OverlayAddressType = iota
	// IPv6 address is used for overlay IP address.
	IPv6
	// IPv4AndIPv6 is used for supporting the dual stack.
	IPv4AndIPv6
)
...
// NewConnectionWithEnvs creates a new connection similarly to NewConnection
// with additional parameter of already configured envs to be used by the VPN.
func NewConnectionWithEnvs(ctx context.Context, config Config, env, secondEnv *env.Env) (*Connection, error) {
	if env == nil {
	...
	conn.serverEnv = env
	conn.secondServerEnv = secondEnv
	return conn, nil
}
...
func (c *Connection) createWireGuardProperties() map[string]interface{} {
	...
	if c.Server != nil {
		...
		switch c.config.OverlayAddressType {
		case IPv4:
