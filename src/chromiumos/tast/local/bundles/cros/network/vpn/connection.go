...
type Config struct {
	...
	// Currently VPNs expect for WireGuard only supports IPv4.
	IPType IPType
...
func (c *Connection) Connect(ctx context.Context) (bool, error) {
	...
	testing.ContextLogf(ctx, "VPN connected, underlay_ip is %s", c.Server.UnderlayIP)
	if c.config.IPType == IPTypeIPv4 || c.config.IPType == IPTypeIPv4AndIPv6 {
		testing.ContextLogf(ctx, "overlay_ipv4 is %s", c.Server.OverlayIPv4)
	}
	if c.config.IPType == 6 || c.config.IPType == IPTypeIPv4AndIPv6 {
		testing.ContextLogf(ctx, "overlay_ipv6 is %s", c.Server.OverlayIPv6)
	}
	return true, nil
}
