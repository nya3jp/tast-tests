...
// and lets shill connects to the VPN server. Returns whether the connection is
// estabilished successfully.
...
func (c *Connection) prepareCertStore(ctx context.Context) error {
	...
	c.config.certSlot = fmt.Sprintf("%d", c.certStore.Slot())
	c.config.certPin = c.certStore.Pin()
	clientCred := certificate.TestCert1().ClientCred
	c.config.certID, err = c.certStore.InstallCertKeyPair(ctx, clientCred.PrivateKey, clientCred.Cert)
	if err != nil {
...
func (c *Connection) configureService(ctx context.Context) error {
	...
	if (c.config.Type == TypeL2TPIPsec) && (c.config.AuthType == AuthTypePSK) {
		...
		properties = map[string]interface{}{
			...
			"L2TPIPsec.ClientCertID":   c.config.certID,
			"L2TPIPsec.ClientCertSlot": c.config.certSlot,
			"L2TPIPsec.User":           chapUser,
			"L2TPIPsec.Password":       chapSecret,
			"L2TPIPsec.PIN":            c.config.certPin,
