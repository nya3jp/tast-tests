...
func (c *Connection) prepareCertStore(ctx context.Context) error {
	...
	c.config.certSlot = fmt.Sprintf("%d", certStore.Slot())
	c.config.certPin = certStore.Pin()
	clientCred := certificate.TestCert1().ClientCred
	c.config.certID, err = certStore.InstallCertKeyPair(ctx, clientCred.PrivateKey, clientCred.Cert)
	if err != nil {
