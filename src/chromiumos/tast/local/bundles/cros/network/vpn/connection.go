...
func (c *Connection) generateWireGuardKey(ctx context.Context) (string, error) {
	if err := c.configureService(ctx); err != nil {
		return "", errors.Wrap(err, "failed to initialize the WireGuard service")
	}
	properties, err := c.service.GetProperties(ctx)
	if err != nil {
		return "", errors.Wrap(err, "failed to get properties of the WireGuard service")
	}
	provider, err := properties.Get(shillconst.ServicePropertyProvider)
	if err != nil {
		return "", err
	}
	publicKey := provider.(map[string]interface{})["WireGuard.PublicKey"].(string)
