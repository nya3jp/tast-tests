...
func OobeNetworkConfigBaseline(ctx context.Context, s *testing.State) {
	...
	var networkProperties = nc.ConfigProperties{
	...
	guid, err := api.ConfigureNetwork(ctx, networkProperties, true)
	if err != nil {
	...
	if managedProperties.TypeProperties.Wifi.Security !=
		networkProperties.TypeConfig.Wifi.Security ||
		managedProperties.TypeProperties.Wifi.Ssid.ActiveValue !=
			networkProperties.TypeConfig.Wifi.Ssid {
		s.Errorf("Set and expected config of test network do not match. Got: %+v. Want: %+v", managedProperties.TypeProperties.Wifi, networkProperties.TypeConfig.Wifi)
	}
}
