...
func LanConnectivity(ctx context.Context, s *testing.State) {
	...
	// Set up an object to invoke networkDiagnosticsConnection functions.
	...
	//if err := networkDiagnosticsRoutines.Call(ctx, &res, `function() {return this.lanConnectivity()}`); err != nil {
	if err := networkDiagnosticsRoutines.Call(ctx, &res, `function() {return { verdict: "here" }}`); err != nil {
		s.Fatal("Failed to run lanConnectivity routine: ", err)
	}
	s.Log("verdict:", res.verdict) // prints nothing
}
