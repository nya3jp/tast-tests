...
func Ethernet8021X(ctx context.Context, s *testing.State) {
	...
	tctx, cancel := ctxutil.Shorten(ctx, 5*time.Second)
	defer cancel()

	tc, err := initializeTest(tctx, s)
	if err != nil {
		s.Fatal("Failed to initialize test: ", err)
	}
	defer func() {
		if err := cleanupTest(ctx, tc); err != nil {
	...
	if err := waitForDeviceProperty(tctx, tc.device, shill.DevicePropertyEapDetected, false); err != nil {
		s.Fatal("Failed to check initial EAP stat: ", err)
	}

	if err := startServer(tctx, tc); err != nil {
		s.Fatal("Failed to start server: ", err)
	}
	defer func() {
		if err := stopServer(ctx, tc); err != nil {
			s.Error("Failed to stop server: ", err)
		}
	}()

	if err := testAuthDetection(tctx, tc); err != nil {
		s.Fatal("Failed to initiate authentication: ", err)
	}

	if err := testAuthentication(tctx, tc); err != nil {
		s.Fatal("Failed to authenticate: ", err)
	}

	if err := testDeauthentication(tctx, tc); err != nil {
		s.Fatal("Failed to deauthenticate: ", err)
	}

	if err := testReauthentication(tctx, tc); err != nil {
	...
	defer func() {
		if err != nil {
			if err2 := cleanupProfile(ctx, m, testProfile); err2 != nil {
				err = errors.Wrapf(err, "failed to cleanup profile: %s", err2.Error())
			}
		}
	}()
	...
	defer func() {
		if err != nil {
			if err2 := vEth.Delete(ctx); err2 != nil {
				err = errors.Wrapf(err, "failed to cleanup veth: %s", err2.Error())
			}
		}
	}()
...
func cleanupTest(ctx context.Context, tc *testContext) error {
	...
	if err2 := cleanupProfile(ctx, tc.manager, testProfile); err2 != nil {
		return errors.Wrapf(err, "failed to clean up profile: %v", err2)
	}
	return err
}
...
func waitForAuth(ctx context.Context, tc *testContext, auth bool) error {
	...
	if auth {
		staStatusProperty = wiredhostapd.STAStatusAuthSucess
	} else {
	...
	if clientErr != nil {
		return errors.Wrapf(clientErr, "failed waiting for client auth==%b", auth)
	} else if serverErr != nil {
		return errors.Wrapf(serverErr, "server failed to reach auth==%b", auth)
	}
...
func testAuthentication(ctx context.Context, tc *testContext) error {
	...
	eap := map[string]interface{}{
		...
		shill.ServicePropertyEAPCACertPEM: []string{tc.cert.ClientCert},
