...
import (
	...
	"chromiumos/tast/local/bundles/cros/network/virtualethernet"
...
func prepareProfile(ctx context.Context, m *shill.Manager, profileName string) (dbus.ObjectPath, error) {
	...
	for _, path := range profiles {
		p, err := shill.NewProfile(ctx, path)
		if err != nil {
			return "", errors.Wrapf(err, "failed to get profile at path %q", path)
		}
...
func Ethernet8021X(ctx context.Context, s *testing.State) {
	...
	err = waitForDeviceProperty(ctx, d, shill.DevicePropertyEapCompleted, true)
	// Check in with hostapd in either failure or success.
	err1 := server.ExpectSTAStatus(ctx, vethMAC, "dot1xAuthAuthSuccessesWhileAuthenticating=1")
	if err != nil {
	...
	err = waitForDeviceProperty(ctx, d, shill.DevicePropertyEapCompleted, false)
	// Check in with hostapd in either failure or success.
	err1 = server.ExpectSTAStatus(ctx, vethMAC, "dot1xAuthAuthEapLogoffWhileAuthenticated=1")
	if err != nil {
