...
func ColdMinigameLaunch(ctx context.Context, s *testing.State) {
	...
	testutil.PerformTest(ctx, s, appPkgName, appActivity, func(params testutil.TestParams) error {
		...
		if err := uiauto.Combine("Log into Roblox",
			...
			uda.LeftClick(uidetection.Word("Log").Above(uidetection.Word("Terms"))),
			action.IfSuccessThen(
				uda.WaitUntilExists(uidetection.Word("Sign")),
				uda.LeftClick(uidetection.Word("In").Below(uidetection.Word("Up"))),
			),

			// Click the Username/Email/Phone field and type the username.
			uda.LeftClick(uidetection.Word("Username/Email/Phone")),
			action.Sleep(waitForActiveInputTime),
			kbd.TypeAction(account.Username),

			// Click the password field and type the password.
			uda.LeftClick(uidetection.Word("Password").First()),
			action.Sleep(waitForActiveInputTime),
			kbd.TypeAction(account.Password),

			// Click the log in button.
			uda.LeftClick(uidetection.TextBlock(strings.Split("Log In", " ")).First()),
		)(ctx); err != nil {
		...
		if err := uiauto.Combine("Load GPU Benchmark Minigame",
			...
			uda.LeftClick(uidetection.Word("Search").Above(uidetection.Word("Friends").First())),
			...
			uda.LeftClick(uidetection.CustomIcon(s.DataPath("roblox-search-benchmark-game-icon.png"))),
			...
			action.Combine("Start Game",
				...
				uda.LeftClick(uidetection.CustomIcon(s.DataPath("roblox-launch-game.png"))),
