...
func getProcCookie(p *process.Process) (int64, error) {
	pid := p.Pid
	path := fmt.Sprint("/proc/", pid, "/sched")
	...
	if err != nil {
		return 0, errors.New("failed to open proc/sched")
	}
	defer file.Close()
	...
	for scanner.Scan() {
		...
		f, err := strconv.ParseInt(res[0][1], 10, 64)
		if err != nil {
			return 0, errors.Wrap(err, "failed to parse cookie")
		}
		return f, nil
	}
...
func getThreadsFromProcess(p *process.Process) ([]process.Process, error) {
	path := fmt.Sprint("/proc/", p.Pid, "/task")
	...
	for _, fname := range fnames {
		...
		crosPtr, err := process.NewProcess(int32(pid))
		if err != nil {
			continue
		}

		ret = append(ret, *crosPtr)
	}
...
func verifyTags() error {
	ckMap := make(map[int64]int)

	p, _ := chrome.GetRendererProcesses()
	for _, pi := range p {
		parentCk, _ := getProcCookie(&pi)
		if parentCk == 0 {
			return errors.New("main thread of renderer not tagged")
		}

		if _, ok := ckMap[parentCk]; ok {
			return errors.New("duplicate renderer cookie or collision")
		}
		ckMap[parentCk] = 1

		c, _ := getThreadsFromProcess(&pi)
		for _, ci := range c {
			ck, _ := getProcCookie(&ci)
