...
func getThreadsFromProcess(p *process.Process) ([]*process.Process, error) {
	...
	for _, finfo := range finfos {
		...
		ptr, err := process.NewProcess(int32(pid))
		if err != nil {
			continue
		}
...
func verifyTags() error {
	...
	procs, _ := chrome.GetRendererProcesses()
...
func CoreSchedTag(ctx context.Context, s *testing.State) {
	...
	newsConn, err := cr.NewConn(ctx, "chrome://settings")
	if err != nil {
	...
	gmailConn, err := cr.NewConn(ctx, "chrome://version")
	if err != nil {
		s.Fatal("Failed to open page: ", err)
	}
