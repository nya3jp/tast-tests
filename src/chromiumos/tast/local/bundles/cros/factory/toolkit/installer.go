...
const (
	...
	// TastTestListName is the dedicated test list that is compatible to run
	// in the lab environment.
	TastTestListName = "generic_tast"

	factoryRootPath = "/usr/local/factory"
)

// Installer contains configurations to run the installation of factory toolkit.
// Currently, we are only allow to install toolkit from the one in the test
// image, and ImagePath is to be used to extract installer from a dedicated
// image from a image specificed by this path.
type Installer struct {
	// The path of install_factory_toolkit.run.
	InstallerPath string
	// TODO(b/150189948) The image path to retrieve installer, only needed
	// if install from factory image
	ImagePath string
	// test list to set after toolkit is installed
	TestList string
	NoEnable bool
}
...
func (i *Installer) InstallFactoryToolKitFromToolkitInstaller(ctx context.Context) (version string, err error) {
	if _, err := os.Stat(i.InstallerPath); err != nil {
		return "", errors.Wrapf(err, "failed to find factory toolkit installer: %s", i.InstallerPath)
	}
	...
	installCmd := testexec.CommandContext(ctx, i.InstallerPath, installArgs...)
...
func (i *Installer) configureToolkitWithLabEnvironment(ctx context.Context) error {
	...
	setTestListCmd := testexec.CommandContext(ctx, "factory", "test-list", i.TestList)
