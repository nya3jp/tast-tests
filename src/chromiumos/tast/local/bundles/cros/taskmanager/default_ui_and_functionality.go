...
func (f *endProcessButtonEnabledVerifier) verify(ctx context.Context) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if numberOfRows != len(processesInfo) {
			...
			return errors.New("task manager UI is still changing")
		}
		return nil
		// It may take some time for the task manager UI to be stable. Wait for 5 seconds before next polling.
	}, &testing.PollOptions{Interval: 5 * time.Second, Timeout: 30 * time.Second}); err != nil {
		return errors.Wrap(err, "failed to get the number of rows")
	}
