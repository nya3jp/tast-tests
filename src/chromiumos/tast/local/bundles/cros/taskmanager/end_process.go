...
func EndProcess(ctx context.Context, s *testing.State) {
	...
	for _, test := range []endProcessTest{
		...
		f := func(ctx context.Context, s *testing.State) {
			...
			if err := resources.taskManager.Open(ctx, tconn); err != nil {
...
func newNonPluginTest() *nonPluginTest {
	processes := []taskmanager.Process{
		taskmanager.NewChromeTabProcess("https://translate.google.com/?hl=en", "Google Translate"),
...
func newPluginTab(url, name, pluginName string, pluginNode *nodewith.Finder) *pluginTab {
	return &pluginTab{
...
func newPluginTest() *pluginTest {
	processes := []taskmanager.Process{
		newPluginTab("https://zoom.us/", "Video Conferencing",
			"https://ada.support/", nodewith.Name("Chat with bot").Role(role.Button),
		),
		newPluginTab("https://www.virtualspirits.com", "Chatbot Platform for Websites",
			"https://youtube.com/", nodewith.Name("YouTube").Role(role.RootWebArea),
		),
		newPluginTab("https://www.oreilly.com", "O'Reilly Media",
...
func newGroupedTabsTest() *groupedTabsTest {
	...
	for i := 0; i < groupedTabsAmount; i++ {
		tab := taskmanager.NewChromeTabProcess("chrome://newtab/", "New Tab")
