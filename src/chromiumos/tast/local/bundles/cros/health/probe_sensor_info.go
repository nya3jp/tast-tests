...
var queryLogRegexp = regexp.MustCompile(`.* INFO iioservice_query: .* GetAttributesCallback\(\): (.*)`)
...
func parseQueryLogs(logs []string) (*sensorAttributes, error) {
	...
	for _, line := range logs {
		...
		attrPair := ""
		if queryLogMatch := queryLogRegexp.FindStringSubmatch(line); queryLogMatch != nil {
			attrPair = queryLogMatch[1]
		}
		tks := strings.Split(attrPair, ":")
		if len(tks) != 2 {
		...
		if tks[0] == "Device id" {
			...
			return nil, errors.Errorf("unrecognized attribute field: %s, log: %s", tks[0], line)
		}
	}
	return &ret, nil
}
