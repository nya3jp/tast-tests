...
func parseQueryLogs(logs []string) (*sensorAttributes, error) {
	...
	for _, line := range logs {
		...
		if attrName == "Device id" {
			deviceID, err := strconv.ParseInt(strings.TrimSpace(attrValue), 10, 32)
			if err != nil {
				return nil, errors.Wrapf(err, "failed to parse sensor device ID, log: %s", line)
			}
			ret.DeviceID = int32(deviceID)
		} else if attrName == "Type" {
			sensorType := sensorType(strings.TrimSpace(attrValue))
			...
			name := strings.TrimSpace(attrValue)
			if name != "" {
				ret.Name = &name
			}
		} else if attrName == "location" {
			ret.Location = sensorLocation(strings.TrimSpace(attrValue))
		} else {
