...
// ErrorHolder keeps multiple errors and is able to convert them into a single
// error.
type ErrorHolder struct {
	msgs []string
}

// ToError converts the errors into a single error.
func (eh *ErrorHolder) ToError() error {
	if len(eh.msgs) == 0 {
		return nil
	}
	if len(eh.msgs) == 1 {
		return errors.New(eh.msgs[0])
	}
	return errors.Errorf("multiple errors: [{%s}]", strings.Join(eh.msgs, "}, {"))
}

// Hold appends an error into ErrorHolder.
func (eh *ErrorHolder) Hold(err error) {
	if err != nil {
		eh.msgs = append(eh.msgs, err.Error())
	}
}

// Handle helps user handle the return value from a function which returns
// (value, error). The error is holden and the value is returned.
func (eh *ErrorHolder) Handle(i interface{}, err error) interface{} {
	eh.Hold(err)
	return i
}
