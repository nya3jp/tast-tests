...
// CompareStringPtrWithFile compares pointer of string with a file. If the
// pointer is nil, the file must not exist.
func CompareStringPtrWithFile(fpath string, got *string) error {
	v, err := ReadFile(fpath)
	if err != nil {
		return err
	}
	return CompareStringPtr(v, got)
}
...
// CompareStringPtrWithCrosConfig compares pointer of string with cros config.
// If the pointer is nil, the cros config must not exist.
func CompareStringPtrWithCrosConfig(ctx context.Context, cpath string, got *string) error {
