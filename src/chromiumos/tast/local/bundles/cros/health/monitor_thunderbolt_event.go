...
func MonitorThunderboltEvent(ctx context.Context, s *testing.State) {
	...
	out, err := ioutil.TempFile("/tmp/", "tbt_logs.*.txt")
	if err != nil {
	...
	cmd := testexec.CommandContext(ctx, "sudo", "nohup", "cros-health-tool", "event", "--category=thunderbolt", "--length_seconds=600", "&")
	cmd.Stdout = out
	...
	// After plug the Thunderbolt device we will validate the output file if thunderbolt device is detected or not,
	// then we flush the output file so that it should not contain any old data for next iteration.
