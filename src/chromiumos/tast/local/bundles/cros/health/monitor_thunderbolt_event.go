...
func MonitorThunderboltEvent(ctx context.Context, s *testing.State) {
	...
	monitorCmd := testexec.CommandContext(ctx, "cros-health-tool", "event", "--category=thunderbolt", "--length_seconds=600")
	...
	addStdOut, err := udevadmToggle("add")
	if err != nil {
		s.Fatal("Failed to add device: ", err)
	}

	deviceAddedPattern := regexp.MustCompile("Device added")
	if !deviceAddedPattern.MatchString(addStdOut) {
		s.Fatal("Failed to detect thunderbolt event, event output: ")
	}

	removeStdOut, err := udevadmToggle("remove")
	if err != nil {
		s.Fatal("Failed to remove device: ", err)
	}

	deviceAddedPattern = regexp.MustCompile("Device removed")
	if !deviceAddedPattern.MatchString(removeStdOut) {
		s.Fatal("Failed to detect thunderbolt event, event output: ")
	}
