...
func ProbeNetworkInfo(ctx context.Context, s *testing.State) {
	...
	writeResultToFile := func(b []byte, attempt int) {
		if err := ioutil.WriteFile(filepath.Join(s.OutDir(), fmt.Sprintf("network_health_telem_%s.txt", attempt)),
			b, 0644); err != nil {
