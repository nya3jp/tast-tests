...
func init() {
	testing.AddTest(&testing.Test{
		Func: ProbeGraphicsInfo, LacrosStatus: testing.LacrosVariantUnknown, Desc: "Check that we can probe cros_healthd for graphics info",
		Contacts: []string{
			"kerker@google.com",
			"cros-tdm@google.com",
...
func ProbeGraphicsInfo(ctx context.Context, s *testing.State) {
	...
	if err := verifyGraphicsData(graphics); err != nil {
		s.Fatal("Failed to validate graphics data, err: ", err)
	}
}
