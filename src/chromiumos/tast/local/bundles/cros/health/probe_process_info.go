...
// compareValueWithPsOutput runs ps command and compares got with its output.
func compareValueWithPsOutput(ctx context.Context, got, command string) error {
...
func validateProcessInfo(ctx context.Context, info *processInfo) error {
	if err := compareValueWithPsOutput(ctx, info.Name, "comm="); err != nil {
		return errors.Wrap(err, "name")
	}
	ppidString, _ := json.Marshal(info.ParentProcessID)
	if err := compareValueWithPsOutput(ctx, string(ppidString), "ppid="); err != nil {
		return errors.Wrap(err, "parent_process_id")
	}
	pgidString, _ := json.Marshal(info.ProcessGroupID)
	if err := compareValueWithPsOutput(ctx, string(pgidString), "pgid="); err != nil {
		return errors.Wrap(err, "process_group_id")
	}
	if err := compareValueWithPsOutput(ctx, info.Command, "args="); err != nil {
