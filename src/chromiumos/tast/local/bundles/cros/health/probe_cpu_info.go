...
func verifyCPUVirtualization(CPUVirtualization *cpuVirtualizationInfo, cpuinfoContent string) error {
	if CPUVirtualization != nil {
		var isLocked bool
		var isEnabled bool

		if CPUVirtualization.Type == "VMX" {
			if !strings.Contains(cpuinfoContent, "vmx") {
		...
		if CPUVirtualization.Type == "SVM" {
			if !strings.Contains(cpuinfoContent, "svm") {
		...
		if isLocked != CPUVirtualization.IsLocked {
			return errors.Errorf("cpu virtualization locked status got: %t, want: %t",
				CPUVirtualization.IsLocked, isLocked)
		}
		if isEnabled != CPUVirtualization.IsEnabled {
			return errors.Errorf("cpu virtualization enabled status got: %t, want: %t",
				CPUVirtualization.IsEnabled, isEnabled)
		}
	}
...
func verifyPhysicalCPU(physicalCPU *physicalCPUInfo) error {
	...
	if err := verifyCPUVirtualization(physicalCPU.CPUVirtualization, string(cpuinfoContent)); err != nil {
		return err
	}
...
func validateVirtualization(virtualization *virtualizationInfo) error {
	...
	if virtualization == nil {
		return errors.New("no virtualization was reported")
	}
