...
func getExpectedCPUVirtualization(flags map[string]bool) (*cpuVirtualizationInfo, error) {
	...
	if _, vmxPresent := flags["vmx"]; vmxPresent {
		...
		val, err := readMsr(int64(vmxMsrReg), 0)
		if err != nil {
			return nil, err
		}
	...
	if _, svmPresent := flags["svm"]; svmPresent {
		...
		val, err := readMsr(int64(svmMsrReg), 0)
		if err != nil {
			return nil, err
		}
...
func validateVulnerabilities(vulnerabilities map[string]vulnerabilityInfo) error {
	...
	for _, vulnerabilityName := range vulnerabilityFiles {
		if _, ok := vulnerabilities[vulnerabilityName.Name()]; !ok {
			return errors.Errorf("vulnerability [%v] not reported", vulnerabilityName)
		}
	}
...
func validateCPUEquality() error {
	...
	for scanner.Scan() {
		...
		if keyValue[0] == "processor" {
			...
			if err != nil {
				return errors.Errorf("Processor ID cannot be turned into int: [%v]", keyValue[1])
			}
