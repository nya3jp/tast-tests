...
func expectedOsInfo(ctx context.Context) (osInfo, error) {
	...
	return osInfo{
		CodeName:      eh.Handle(utils.GetCrosConfig(ctx, cfgCodeName)).(string),
		MarketingName: eh.Handle(utils.GetOptionalCrosConfig(ctx, cfgMarketingName)).(*string),
		OsVersion:     eh.Handle(expectedOsVersion(ctx)).(osVersion),
		BootMode:      eh.Handle(expectedBootMode()).(string),
	}, eh.ToError()
}
