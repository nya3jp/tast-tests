...
func init() {
	testing.AddTest(&testing.Test{
		Func:         Max98390RecentReboot,
		Desc:         "Verifies sound_card_init max98390 skips boot time calibration when fast reboot",
...
// Max98390RecentReboot Verifies sound_card_init max98390 skips boot time calibration when fast reboot.
func Max98390RecentReboot(ctx context.Context, s *testing.State) {
	...
	if err := CreateBootTimeFile(ctx, now); err != nil {
		s.Fatalf("Failed to create %s: %v", BootTimeFile, err)
	}

	if err := CreateStopTimeFile(ctx, now+20); err != nil {
	...
	if err := VerifyCalibNotExist(ctx, ampCount); err != nil {
		s.Fatal("Boot time calibration should be skipped after the immediate reboot: ", err)
	}
}
