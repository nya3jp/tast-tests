...
// Copy audio file to Chromebook and play it through default audio player.
func LocalAudioPlayback(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatalf("Failed to create Cras object: ", err)
	}
	...
	setAudioNode := func() (string, string) {
		...
		if deviceType != expectedAudioNode {
			...
			if deviceType != expectedAudioNode {
				s.Fatalf("Failed to select the active device %s: %v", expectedAudioNode)
			}
		}
