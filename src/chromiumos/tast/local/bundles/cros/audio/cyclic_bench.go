...
func getNumberOfCPU(ctx context.Context, s *testing.State) int {
	lscpu := testexec.CommandContext(ctx, "lscpu")
	out, err := lscpu.Output()
	if err != nil {
		s.Fatal("Failed to run lscpu: ", err)
	}
	...
	for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
		...
		if err != nil {
			s.Fatal("Failed to parse number of cpus: ", line)
		}
		return ret
	}
	s.Fatal("Failed to get the number of cpus: ", string(out))
	return -1
}
