...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{
			...
			{
				Name: "rr12_1thread_10ms_stress_nice0_2workers_per_cpu",
				Val: cyclicTestParameters{
					SchedPolicy:    rrSched,
					Priority:       crasPriority,
					Threads:        1,
			...
			{
				// nice-20 will cause invalid tast test name.
				Name: "nice_20_1thread_10ms",
				Val: cyclicTestParameters{
					SchedPolicy:    otherSched,
					Priority:       -20,
					Threads:        1,
			...
			{
				Name: "nice0_1thread_10ms_stress_nice0_2workers_per_cpu",
				Val: cyclicTestParameters{
					SchedPolicy:    otherSched,
					Priority:       0,
					Threads:        1,
...
func getAffinityString(ctx context.Context, aff affinity) (string, error) {
	...
	switch aff {
	case defaultAff:
		return "0-" + strconv.Itoa(cpu-1), nil
	case smallCore:
		return "0", nil
	case bigCore:
		return strconv.Itoa(cpu - 1), nil
	}
...
func getStressContext(ctx context.Context, param cyclicTestParameters) (*testexec.Cmd, error) {
	...
	case rrSched:
		return testexec.CommandContext(ctx, "stress-ng",
			...
			"--sched="+strconv.Itoa(param.Priority),
