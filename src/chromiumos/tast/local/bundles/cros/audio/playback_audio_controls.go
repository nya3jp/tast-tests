...
func PlaybackAudioControls(ctx context.Context, s *testing.State) {
	...
	rawFilePath1 := filepath.Join(filesapp.DownloadPath, rawFileName1)
	wavFileName1 := "audioFile1.wav"
	wavFilePath1 := filepath.Join(filesapp.DownloadPath, wavFileName1)
	defer os.Remove(wavFilePath1)

	// Second audio file name and path variables.
	rawFileName2 := "audioFile2.raw"
	rawFilePath2 := filepath.Join(filesapp.DownloadPath, rawFileName2)
	wavFileName2 := "audioFile2.wav"
	wavFilePath2 := filepath.Join(filesapp.DownloadPath, wavFileName2)
