...
func PlayAndRecordAudio(ctx context.Context, s *testing.State) {
	...
	rawTempFile, err := ioutil.TempFile("", rawFileName)
	if err != nil {
	...
	if err := audio.ConvertRawToWav(ctx, rawTempFile.Name(), wavFile, audioRate, audioChannel); err != nil {
	...
	cmd := fmt.Sprintf("play -c %d -r %d %s & rec -r %d -c %d %s trim 0 30", audioChannel, audioRate, wavFile, audioRate, audioChannel, recWavFile)
