...
func init() {
	testing.AddTest(&testing.Test{
		...
		// TODO: We are not running this test on any bots intentionally.
...
func StressAudioPlaybackOnboardSpeaker(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < iterHour; i++ {
		...
		for {
			...
			testing.ContextLog(ctx, "Checking audio routing, test remaining time: ", remainingTime)
			if remainingTime >= 2*time.Minute {
				if err := testing.Sleep(ctx, 2*time.Minute); err != nil {
					s.Fatal("Failed to sleep: ", err)
				}
			} else {
				if err := testing.Sleep(ctx, remainingTime); err != nil {
					s.Fatal("Failed to sleep: ", err)
				}
			}
		}
