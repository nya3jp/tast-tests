...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Run alsa_conformance_test to test basic functions of ALSA.",
...
func AlsaConformance(ctx context.Context, s *testing.State) {
	...
	// If returns the number of failed tests and failed test suites.
	checkOutput := func(out []byte) (numFails int, failSuites []string) {
		...
		s.Logf("Test Result: %d passed %d failed", result.Pass, result.Fail)
