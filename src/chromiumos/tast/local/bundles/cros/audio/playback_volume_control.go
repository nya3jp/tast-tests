...
func PlaybackVolumeControl(ctx context.Context, s *testing.State) {
	...
	if err = kb.Accel(ctx, topRow.VolumeMute); err != nil {
		s.Fatal(`Failed to mute volume after press "VolumeMute": `, err)
	}
	if !isMuted() {
	...
	if err = kb.Accel(ctx, topRow.VolumeUp); err != nil {
		s.Fatal(`Failed to unmute volume after press "VolumeUp": `, err)
	}
	...
	for {
		...
		if err := vh.VerifyVolumeChanged(ctx, func() error {
			...
			s.Fatal(`Failed to change volume after press "VolumeDown": `, err)
		}
	}
	...
	for {
		...
		if err := vh.VerifyVolumeChanged(ctx, func() error {
			...
			s.Fatal(`Failed to change volume after press "VolumeUp": `, err)
		}
	}
}
