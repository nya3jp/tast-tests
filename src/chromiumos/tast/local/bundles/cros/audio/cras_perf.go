...
func CrasPerf(ctx context.Context, s *testing.State) {
	...
	if u, err := sysutil.Uname(); err == nil && u.Machine != "aarch64" {
		profs = append(profs, profiler.Perf(&profiler.PerfOpts{Type: profiler.PerfStatOnly, Pid: pid}))
	...
	playbackCommand := cras_test_utils.CRASPlaybackCommand(runCtx, duration, blocksize)
	// Capture function by CRAS.
	captureCommand := cras_test_utils.CRASCaptureCommand(runCtx, duration, blocksize)
