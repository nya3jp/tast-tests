...
func crasPerfOneIteration(ctx context.Context, s *testing.State, pid int, pv *perfpkg.Values) {
	...
	profs := []profiler.Profiler{
		...
		profiler.Perf(profiler.PerfStatOpts(pid)),
		profiler.Perf(profiler.PerfRecordOpts()),
	}

	s.Log("start audio")
	playbackCommand := crastestutils.CRASPlaybackCommand(runCtx, commandDuration.Milliseconds()/1000, blocksize)
	captureCommand := crastestutils.CRASCaptureCommand(runCtx, commandDuration.Milliseconds()/1000, blocksize)
