...
func setInputMute(ctx context.Context, tconn *chrome.TestConn, muted bool) error {
	if err := quicksettings.ToggleSetting(ctx, tconn, quicksettings.SettingPodMicMute, muted); err != nil {
		return errors.Wrap(err, "set failed")
	}

	// Check the input is really muted or unmuted.
	currMuted, err := quicksettings.SettingEnabled(ctx, tconn, quicksettings.SettingPodMicMute)
	if err != nil {
...
func testInputGain(ctx context.Context, s *testing.State, tconn *chrome.TestConn, kb *input.KeyboardEventWriter, input audio.TestRawData) {
	...
	// Set input unmute by UI quicksettings.
	if err := setInputMute(ctx, tconn, false); err != nil {
...
func testInputMute(ctx context.Context, s *testing.State, tconn *chrome.TestConn, input audio.TestRawData) {
	...
	// Set input mute by UI quicksettings, and unmute after test.
	if err := setInputMute(ctx, tconn, true); err != nil {
		s.Fatal("Failed to set mic muted: ", err)
	}
	defer setInputMute(ctx, tconn, false)
