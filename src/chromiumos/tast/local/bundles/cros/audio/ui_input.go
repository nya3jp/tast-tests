...
func testInputGain(ctx context.Context, s *testing.State, tconn *chrome.TestConn, kb *input.KeyboardEventWriter, input audio.TestRawData) {
	...
	for _, gainMin := range []int{lowSliderGainMin, highSliderGainMin} {
		...
		for {
			if currVal < gainMin {
				...
				currVal = increase
			} else if currVal < gainMin+sliderStepValue {
				// The current gain is what we want.
				break
			} else {
	...
	// Calcuate the expected linear gain between the representative gains in decibel for the low and high
