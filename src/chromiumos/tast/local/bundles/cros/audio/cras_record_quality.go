...
func CrasRecordQuality(ctx context.Context, s *testing.State) {
	...
	if err := audio.ConvertRawToWav(ctx, rawFile, wavFile); err != nil {
		s.Error("Failed to convert raw to wav: ", err)
	}

	// Remove first 0.5 seconds to avoid pop noise.
	if err := audio.TrimFileFrom(ctx, wavFile, clippedFile, 0.5); err != nil {
		s.Error("Failed to convert raw to wav: ", err)
	}

	if err := audio.CheckRecordingQuality(ctx, wavFile); err != nil {
