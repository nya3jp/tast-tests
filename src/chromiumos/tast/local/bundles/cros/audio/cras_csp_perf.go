...
func crasCspPerfOneIteration(ctx context.Context, s *testing.State, pid int, pv *perfpkg.Values) {
	...
	pipePlaybackCmd := testexec.CommandContext(
		runCtx,
		"cras_test_client",
		"--playback_file=/dev/stdin",
		fmt.Sprintf("--block_size=%d", blocksize),
	)
	playbackReader, captureWriter, err := os.Pipe()
	if err != nil {
		s.Fatal(err)
	}
	defer playbackReader.Close()
	defer captureWriter.Close()
	pipeCaptureCmd.Stdout = captureWriter
	...
	appCaptureCmd.Stdout = processedBuf
