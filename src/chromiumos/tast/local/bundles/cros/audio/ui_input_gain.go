...
func UIInputGain(ctx context.Context, s *testing.State) {
	...
	for _, gain := range []int{lowSliderGain, highSliderGain} {
		...
		// quicksettings library only supports coarse slider value adjustment (+-10 per step), it is
		// possible that we cannot reach the exact slider value as expected. The strategy here is to find
		// the value which is closest to but not smaller than the expected value. The range of that value
		// will be [expected_value, expected_value + 10).
		...
		for {
			...
			if diff < 0 {
				...
				s.Logf("Testing with actual mic gain slider value: %d", currVal)
				break
			}
		}
		...
		if err := testing.Sleep(shortCtx, 1*time.Second); err != nil {
			if killErr := playCmd.Kill(); killErr != nil {
				s.Log("Failed to kill playback command: ", err)
			}
			s.Fatal("Failed to wait for playback command: ", err)
		}
	...
	s.Logf("Calculated gain = %.2f", gain)
