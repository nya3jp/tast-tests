...
func ucmSequencesTestCard(ctx context.Context, s *testing.State, sectionDevice string, ucmName string) {
	...
	if err != nil {
		s.Errorf("Failed to read HiFi.conf: %v", ucmName, err)
	}

	if !bytes.Contains(b, []byte("SectionVerb")) {
		s.Logf("Skipping due to missing SectionVerb in HiFi.conf")
		return
	}
