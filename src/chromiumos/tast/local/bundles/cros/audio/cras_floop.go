...
func init() {
	testing.AddTest(&testing.Test{
		Func:     CrasFloop,
		Desc:     "Flexible loopback should record zero samples if there are no playback devices",
		...
		Params: []testing.Param{
			...
			},
			{
				Name: "rcbp",
				Val: &floop.Param{
					PlaybackTime:    floop.TI(3, 6),
					CaptureTime:     floop.TI(0, 8),
					RequestFloopSec: 0,
					FloopMask:       4,
					Checkers: []floop.Checker{
						floop.CheckZeroSample(1, 2),
