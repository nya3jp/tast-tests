...
// Given two audio signals, finds the first index where their overlaps differ,
// if it exists.
func findFirstDifference(a, b []int16) (differ bool, firstIndex int) {
...
// Given an audio signal, ensures that it is a continuous sine wave.
func checkSignalIsContinuous(s *testing.State, samples []int16) {
	for i := 0; i < len(samples)-1; i++ {
		...
		if delta > 3000 {
			...
			s.Fatal("Captured signal must be continuous")
		}
	}
}
...
func LoopbackRecord(ctx context.Context, s *testing.State) {
	...
	capture := testexec.CommandContext(ctx, "nice", "-20", "cras_tests", "capture", "-b512", "-c2", "-r48000", "-f", captureFile.Name())
	err = capture.Start()
	if err != nil {
		capture.DumpLog(ctx)
		s.Fatal("Failed: ", err)
	...
	// Play input file.
	playback := testexec.CommandContext(ctx, "nice", "-20", "cras_tests", "playback", "-b512", "-c2", "-r48000", "-f", playbackFile.Name())
	err = playback.Run()
	if err != nil {
	...
	checkSignalIsContinuous(s, trimAudioData(leftOut))
	checkSignalIsContinuous(s, trimAudioData(rightOut))
	...
	differ, index := findFirstDifference(leftIn, leftOut[leftOffset:])
	if differ {
		s.Logf("The left channel has its first error %.1f%% through the file",
			float64(index)/float64(len(leftIn))*100)
	}
	...
	differ, index = findFirstDifference(rightIn, rightOut[rightOffset:])
	if differ {
		s.Logf("The right channel has its first error %.1f%% through the file",
			float64(index)/float64(len(rightIn))*100)
	}

	s.Log("Best Offset: ", leftOffset, ", ", rightOffset)
	s.Logf("Lowest Distance: %.2f, %.2f", leftDistance, rightDistance)
	if leftDistance != 0.0 || rightDistance != 0.0 {
		s.Fatal("Signal was not accurately captured")
	}
}
