...
// e.g. chrome, nacl_helper, etc. They typically share paths, even though their
// binary names differ. The output is a new-line separated list of pids. It's
// returned as a bytes.Buffer to facilitate piping as input to another bash
// command.
// The current implementation always returns an extra pid, which is the pid for
// the grep command itself. This is typically okay because the pid won't be
// used by the time the next command consuming the pids is run. It is true that
// there is a race condition here where the pid gets reused, but that's true
// for all pids returned by this command. For example, it's possible that a
// chrome-related process is killed and the pid is reused.
func PidsFromPath(ctx context.Context, path string) *bytes.Buffer {
	// List all processes.
	psCmd := testexec.CommandContext(ctx, "ps", "aux")
	a, _ := psCmd.Output()

	// grep for processes matching the path
	grepCmd := testexec.CommandContext(ctx, "grep", path)
	grepCmd.Stdin = bytes.NewReader(a)
	b, _ := grepCmd.Output()

	// Use awk to extract the second column, which is pid.
	awkCmd := testexec.CommandContext(ctx, "awk", "{print $2}")
	awkCmd.Stdin = bytes.NewReader(b)
	c, _ := awkCmd.Output()
...
func LaunchLinuxChrome(ctx context.Context, p PreData) (*linuxChrome, error) {
	...
	killCmd.Cmd.Stdin = PidsFromPath(ctx, BinaryPath)
	killCmd.Run()
