...
func testURLRedirect(ctx context.Context, s *testing.State, params urlRedirectParams) {
	...
	case openInAshAsApplication:
		...
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			wins, title, err := determineNumberOfAshWindowsAndTitle(ctx, atconn)
			if err != nil {
				s.Fatal("Cannot determine number of Ash windows: ", err)
			}
	...
	case openInLacrosAsUnreachable:
		...
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			...
			if err != nil {
				s.Fatal("Cannot determine number of Lacros windows: ", err)
			}
			if wins != 1 {
	...
	case openInLacrosAsBlocked:
		...
		conn.Close()
		if err != nil {
			s.Fatal("Cannot close conn from finding blocked target: ", err)
		}
...
// of the app window.
func determineNumberOfAshWindowsAndTitle(ctx context.Context, tconn *chrome.TestConn) (int, string, error) {
...
func determineNumberOfLacrosWindowsAndTitle(ctx context.Context, tconn *chrome.TestConn) (int, string, error) {
	ws, err := ash.FindAllWindows(ctx, tconn, ash.BrowserTypeMatch(browser.TypeLacros))
