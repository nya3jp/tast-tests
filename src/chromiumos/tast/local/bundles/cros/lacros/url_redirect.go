...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			Val: urlRedirectParams{mode: openInAshAsApplication,
				url:           "os://flags/",
				appId:         "jaimifaeiicidiikhmjedcgdimealfbh",
				appTitleInAsh: "ChromeOS-URLs"},
...
func URLRedirect(ctx context.Context, s *testing.State) {
	...
	lacrosBrowser, err := lacros.Launch(ctx, atconn)
	if err != nil {
		s.Fatal("Failed to launch lacros-chrome: ", err)
	}
	defer lacrosBrowser.Close(cleanupCtx)
	...
	if err := navigateSingleTabToURLInLacros(ctx, params.url, lacrosBrowser, atconn, kb, s); err != nil {
	...
	if params.mode == openInAshAsApplication {
		...
		if err := ash.WaitForCondition(ctx, atconn, func(w *ash.Window) bool {
			...
			s.Fatal("Failed to wait for app to be visible for ID: %v with error %v", params.appId, err)
		}
		...
		wins, title := determineNumberOfAshWindowsAndTitle(ctx, atconn, s)
		...
		// Make sure that the navigation has succeeded - there was a delta in between
		// entering the search query and getting the updated title.
		testing.Sleep(ctx, time.Second/2)
		...
		if err == nil || conn != nil {
			wins, title := determineNumberOfLacrosWindowsAndTitle(ctx, atconn, s)
			if wins != 1 {
...
func navigateSingleTabToURLInLacros(ctx context.Context, url string, l *lacros.Lacros, tconn *chrome.TestConn, keyboard *input.KeyboardEventWriter, s *testing.State) error {
...
// determineNumberOfAshWindows counts all Ash windows (Name: "BrowserFrame) and get
// the title of the first window to see the title of the app window.
func determineNumberOfAshWindowsAndTitle(ctx context.Context, tconn *chrome.TestConn, s *testing.State) (int, string) {
...
// determineNumberOfLacrosWindows counts all Lacros windows (Name:"ExoShellSurface*")
// and get the Title of the first window to see what the navigation was doing.
func determineNumberOfLacrosWindowsAndTitle(ctx context.Context, tconn *chrome.TestConn, s *testing.State) (int, string) {
  ws, err := ash.FindAllWindows(ctx, tconn, func(w *ash.Window) bool {
