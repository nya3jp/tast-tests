...
func URLRedirect(ctx context.Context, s *testing.State) {
	...
	lacrosBrowser, err := lacros.Launch(ctx, tconnAsh)
	if err != nil {
	...
	if downloadsPath, err := cryptohome.DownloadsPath(ctx, cr.NormalizedUser()); err != nil {
		...
		if err := uiauto.StartRecordFromKB(ctx, tconnLacros, kb, downloadsPath); err != nil {
			s.Log("Failed to start recording: ", err)
		}
		defer uiauto.StopRecordFromKBAndSaveOnError(cleanupCtx, tconnLacros, s.HasError, s.OutDir(), downloadsPath)
	}
