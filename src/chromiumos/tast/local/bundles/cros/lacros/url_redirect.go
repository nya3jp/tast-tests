...
func URLRedirect(ctx context.Context, s *testing.State) {
	...
	// Close the lacros-chrome window and give it time to do so.
	ctxWithTimeout, cancel := context.WithTimeout(ctx, 30*time.Second)
	lacrosBrowser.Close(ctxWithTimeout)
	// Have seen that closing the browser window took sometimes slightly longer,
	// so adding some more time to wait here until it is done.
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		win, _ := ash.GetAllWindows(ctx, tconnLacros)
		if len(win) == 0 {
			return testing.PollBreak(nil)
		}
		return nil
	}, &testing.PollOptions{Timeout: 10 * time.Second}); err != nil {
		err = errors.Wrap(err, "Not all windows closed in time")
	}
}
