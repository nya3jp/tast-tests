...
func (uts *UpdateTestService) VerifyUpdate(ctx context.Context, req *proto.VerifyUpdateRequest) (*proto.VerifyUpdateResponse, error) {
	...
	// Verify without UI that the lacros is mounted in the expected versioned path.
	if _, err := os.Stat(expectedVersionedLacrosPath); os.IsNotExist(err) {
		return nil, errors.Wrap(err, "binary path does not exist yet")
	}
	...
	if actualVersionedLacrosPath != expectedVersionedLacrosPath {
		return &proto.VerifyUpdateResponse{
			Result: &proto.TestResult{
				...
				StatusDetails: fmt.Sprintf("VerifyUpdate: executable path expected: %v, actual: %v", expectedVersionedLacrosPath, actualVersionedLacrosPath),
			},
		}, nil
	}
	return &proto.VerifyUpdateResponse{
		Result: &proto.TestResult{
			...
			StatusDetails: fmt.Sprintf("VerifyUpdate: executable path: %v", actualVersionedLacrosPath),
		},
	}, nil
}

// ClearUpdate removes provisioned Lacros in the install path or browser data if needed.
...
func (uts *UpdateTestService) GetBrowserVersion(ctx context.Context, req *proto.GetBrowserVersionRequest) (*proto.GetBrowserVersionResponse, error) {
	...
	case proto.BrowserType_ASH:
		...
		if err != nil {
			return nil, errors.Wrap(err, "failed to get the Ash Chrome version")
		}
		version := versionRegexp.FindString(string(out))
		versions = append(versions, version)

	case proto.BrowserType_LACROS_ROOTFS:
		// Read /opt/google/lacros/metadata.json
		out, err := testexec.CommandContext(ctx, "cat", "/opt/google/lacros/metadata.json").Output(testexec.DumpLogOnError)
		if err != nil {
			return nil, errors.Wrap(err, "failed to get the Rootfs Lacros Chrome version")
		}
		metadataJSON := lacrosMetadata{}
		if err := json.Unmarshal([]byte(out), &metadataJSON); err != nil {
			return nil, errors.Wrap(err, "failed to parse Rootfs Lacros Chrome version")
		}
		versions = append(versions, metadataJSON.Content.Version)
