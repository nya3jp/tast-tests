...
import (
	...
	common "chromiumos/tast/common/cros/lacros"
...
func (uts *UpdateTestService) VerifyUpdate(ctx context.Context, req *lacrosservice.VerifyUpdateRequest) (*lacrosservice.VerifyUpdateResponse, error) {
	...
	cr, err := uts.setupChrome(ctx, opts)
	if err != nil {
...
func (uts *UpdateTestService) ClearUpdate(ctx context.Context, req *lacrosservice.ClearUpdateRequest) (*lacrosservice.ClearUpdateResponse, error) {
	...
	testexec.CommandContext(ctx, "touch", common.CorruptStatefulFilePath).Run()
	...
	lacrosComponentPathGlob := filepath.Join(common.LacrosRootComponentPath, "*")
	...
	if len(matches) == 0 {
		os.Remove(common.CorruptStatefulFilePath)
	}
...
func (uts *UpdateTestService) setupChrome(ctx context.Context, opts []chrome.Option) (*chrome.Chrome, error) {
...
// versionInfoFromUI returns the version detail for a given expression on the chrome://version page.
...
func (uts *UpdateTestService) versionInfoFromUI(ctx context.Context, l *launcher.LacrosChrome, expr string) (string, error) {
	lconn, err := l.NewConn(ctx, common.VersionURL)
	if err != nil {
		return "", errors.Wrapf(err, "failed to open: %v", common.VersionURL)
	}
	defer lconn.Close()
