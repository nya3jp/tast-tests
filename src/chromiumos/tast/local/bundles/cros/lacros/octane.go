...
func runOctaneTest(ctx context.Context, pd launcher.PreData, conn *chrome.Conn) (float64, error) {
	...
	if err := conn.WaitForExprFailOnErrWithTimeout(ctx, `window.testFinished`, time.Minute*10); err != nil {
		return 0.0, errors.Wrap(err, "octane tests did not finish")
	}

	var score float64
	if err := conn.Eval(ctx, `window.testScore`, &score); err != nil {
		return 0.0, errors.Wrap(err, "failed to fetch octane score")
	}
	return score, nil
}
...
func runCrosOctaneTest(ctx context.Context, pd launcher.PreData) (float64, error) {
	conn, cleanup, err := lacros.SetupCrosTestWithPage(ctx, pd, octaneURL)
	if err != nil {
