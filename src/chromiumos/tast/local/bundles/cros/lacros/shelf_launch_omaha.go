...
func chromeOSChannel() (string, error) {
	...
	if match == nil {
		return "", errors.Wrap(err, "failed to read channel in lsbrelease's releasetrack")
	}
	...
	osChannel := match[1]
	// if it is "dev" channel, further check if it is still on "canary" but not branched from trunk by checking the branch number.
...
// statefulLacrosChannels resolves the stateful-lacros channels from the given OS channel that is compatible within supported version skews.
...
func clearStatefulLacrosIf(ctx context.Context, hasError func() bool) {
	if hasError() {
		clearStatefulLacros(ctx)
	}
}

func clearStatefulLacros(ctx context.Context) {
	testing.ContextLog(ctx, "Clearing installed stateful-lacros")
...
func ShelfLaunchOmaha(ctx context.Context, s *testing.State) {
	...
	for _, lacrosChannel := range statefulLacrosChannels {
		s.Run(ctx, fmt.Sprintf("OS: %v, stateful-lacros: %v", osChannel, lacrosChannel), func(ctx context.Context, s *testing.State) {
			...
			// Remove the installed stateful-lacros if any error occurs.
			defer func() {
				clearStatefulLacrosIf(cleanupCtx, s.HasError)
			}()
