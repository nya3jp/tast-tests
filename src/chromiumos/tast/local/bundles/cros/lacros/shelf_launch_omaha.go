...
// Note that "canary" and "dev" channel are defined based on the timing of the first branch cut. "canary" if not yet branched, otherwise, "dev".
func chromeOSChannel() (string, error) {
	...
	osChannel := match[1]
	// if it is "dev" channel, further check if it is cut from the "canary" by checking the branch number used only by branches.
