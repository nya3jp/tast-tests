...
import (
	...
	"chromiumos/tast/local/chrome/versionutil"
...
// lacrosCompatibleChannels returns a map of lacros channels to the latest live versions that are compatible with Ash.
func lacrosCompatibleChannels(ctx context.Context, ashVersion string) (map[string]string, error) {
	// Call VersionHistory API to get the latest live versions of all channels.
	releases, err := versionutil.HTTPGetVersionHistory(ctx)
	if err != nil {
	...
	for _, release := range releases {
		...
		case "canary", "dev", "beta", "stable":
			...
			if ok, err := versionutil.Compatible(lacrosVersion, ashVersion); err != nil {
...
func ShelfLaunchOmaha(ctx context.Context, s *testing.State) {
	...
	osVersion, err := versionutil.ChromeOSVersion()
	if err != nil {
	...
	compatibleChannels, err := lacrosCompatibleChannels(ctx, ashVersion)
	if err != nil {
