...
// grepFilterSum greps for a particular statistic, filters for it, and then
// sums across all instances.
func grepFilterSum(ctx context.Context, stat string, input []byte) (int, error) {
	// Grep for the relevant memory stat.
	grepCmd := testexec.CommandContext(ctx, "grep", "-e", stat+":.*kB")
	grepCmd.Stdin = bytes.NewReader(input)
	b, _ := grepCmd.Output()

	// Filter for the second column, which happens to contain the actual memory number.
	awkCmd := testexec.CommandContext(ctx, "awk", "{print $2}")
	awkCmd.Stdin = bytes.NewReader(b)
	c, _ := awkCmd.Output()

	// Sum across all numbers.
	sumCmd := testexec.CommandContext(ctx, "awk", "{s+=$1}END{print s}")
	sumCmd.Stdin = bytes.NewReader(c)
	d, _ := sumCmd.Output()
...
// which may choose to spawn utility or renderer processes for its own
// purposes.
