...
func measureBothChrome(ctx context.Context, s *testing.State) (int, int) {
	...
	pmf, pss, err := measureProcesses(ctx, s.FixtValue().(launcher.PreData).LacrosPath)
	if err != nil {
...
func Memory(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < 10; i++ {
		...
		if params.mode == openTabMode {
			conns, err = openTabsChromeOS(ctx, s.FixtValue().(launcher.PreData).Chrome, params.numTabs)
			if err != nil {
				s.Fatal("Failed to open chromeos-chrome tabs: ", err)
			}
		} else {
			// Open a new tab to url.
			conn, err := s.FixtValue().(launcher.PreData).Chrome.NewConn(ctx, url)
			if err != nil {
		...
		err = s.FixtValue().(launcher.PreData).TestAPIConn.EvalPromise(ctx,
