...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			Name: "blank",
			Val:  testParams{openURLMode, "about:blank", 0},
		}, {
			Name: "docs",
			Val:  testParams{openURLMode, "https://docs.google.com/document/d/1_WmgE1F5WUrhwkPqJis3dWyOiUmQKvpXp5cd4w86TvA/edit", 0},
		}, {
			Name: "reddit",
			Val:  testParams{openURLMode, "https://old.reddit.com/", 0},
		}, {
			Name: "youtube",
			Val:  testParams{openURLMode, "https://www.youtube.com/watch?v=uS33jC2VYNU", 0},
		}, {
			Name: "twentytabs",
			Val:  testParams{openTabMode, "", 20},
		},
		},
	})
}
...
func Memory(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < 10; i++ {
		...
		if params.mode == openTabMode {
			err = openTabsLinux(ctx, l, params.numTabs)
			if err != nil {
				s.Fatal("Failed to oepn linux-chrome tabs: ", err)
			}
		} else {
			err = navigateSingleTabToURLLinux(ctx, url, l)
			if err != nil {
		...
		if params.mode == openTabMode {
			conns, err = openTabsChromeOS(ctx, s.PreValue().(launcher.PreData), params.numTabs)
			if err != nil {
...
func openTabsChromeOS(ctx context.Context, p launcher.PreData, numTabs int) ([]*chrome.Conn, error) {
	var conns []*chrome.Conn
	for i := 0; i < numTabs; i++ {
		conn, err := p.Chrome.NewConn(ctx, "about:blank")
		if err != nil {
			return nil, err
		}
		conns = append(conns, conn)
