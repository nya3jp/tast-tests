...
func injectSeedInLocalState(ctx context.Context, seed *variations.SeedData) error {
	// create a map to store the updated pref info
	prefMap := make(map[string]interface{})
	prefMap[variations.CompressedSeedPref] = seed.CompressedSeed
	prefMap[variations.SeedSignaturePref] = seed.SeedSignature
	if err := localstate.MarshalPref(browser.TypeLacros, prefMap); err != nil {
