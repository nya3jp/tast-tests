...
func DocsCUJ(ctx context.Context, s *testing.State) {
	...
	// Initializes perfutil runner as nil because it won't be needed to actually run the tests but
	// only for deriving metrics and save pv data.
		...
		for i := 0; i < iterationCount; i++ {
			...
			case browser.TypeLacros:
				if variantPv, err = runDocsPageLoad(ctx, docsURLToComment, func(ctx context.Context, url string) (*chrome.Chrome, *chrome.Conn, lacrosperf.CleanupCallback, error) {
					...
					r.Values().MergeWithSuffix(".lacros", variantPv.GetValues())
				}
			case browser.TypeAsh:
				if variantPv, err = runDocsPageLoad(ctx, docsURLToComment, func(ctx context.Context, url string) (*chrome.Chrome, *chrome.Conn, lacrosperf.CleanupCallback, error) {
					...
					r.Values().MergeWithSuffix(".ash", variantPv.GetValues())
				}
			}
		}
	}
