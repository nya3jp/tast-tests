...
func DocsCUJ(ctx context.Context, s *testing.State) {
	...
	cleanup, err := lacros.SetupPerfTest(ctx, tconn, "lacros.DocsCUJ")
	if err != nil {
	...
	if loadTime, visibleLoadTime, err := runDocsPageLoad(ctx, tconn, docsURLToComment, func(ctx context.Context, url string) (*chrome.Conn, lacros.CleanupCallback, error) {
