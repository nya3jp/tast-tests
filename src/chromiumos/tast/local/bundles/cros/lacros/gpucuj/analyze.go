...
// an average fps.
// TODO(crbug.com/1140906): Remove this code (and this file) when we figure out
// where the noise in perf data is coming from.
			...
			if fs.ta.annotationName(p, da) == "args" {
				...
				for i, v := range nv.DictValues {
					if nv.DictKeys[i] == "interval_us" {
						fs.curFlip.intervalNS = uint64(1000.0 * v.GetDoubleValue())
					}
				}
			}
		}
...
// align performs an alignment dynamic programming algorithm.
// Note that matchFn is used for backtracking and will be called from later values to earlier.
