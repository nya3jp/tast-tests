...
func (fs *frameStats) parseVizThreadInitial(ts uint64, trackName string, sd *seqData, p *trace.TracePacket, flowID uint64, newFlow bool) error {
...
func (fs *frameStats) computeProportionDroppedFrames() (*results, error) {
	err := fs.ta.parseTrackEvents(func(ts uint64, trackName string, sd *seqData, p *trace.TracePacket, flowID uint64, newFlow bool) error {
		return fs.parseVizThreadInitial(ts, trackName, sd, p, flowID, newFlow)
	})
	if err != nil {
		return nil, err
	}
