...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Measures Lacros login time",
...
func Login(ctx context.Context, s *testing.State) {
	func() {
		...
		// TODO(tvignatti): need to close lacros properly.
		// lacrosClosed := false
		// defer func() {
		// 	if !lacrosClosed {
		...
		// TODO(tvignatti): need to close lacros properly (see above).
		// if err := l.Close(ctx); err != nil {
		// 	s.Fatal("Failed to close lacros: ", err)
		// }
		// lacrosClosed = true
	...
	func() {
		...
		if err := lacros.WaitForLacrosWindow(ctx, tconn, "Alphabet"); err != nil {
