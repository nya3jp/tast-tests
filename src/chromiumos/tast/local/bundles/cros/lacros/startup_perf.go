...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			Name: "chrome",
			Val:  testParameters{browser.TypeAsh, nil},
		}},
...
func StartupPerf(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < iterationCount; i++ {
		...
		// metrics.
		sessionStartTime, browserRestoreTime, histogramLacrosStartTime, err := performRegularLogin(ctx, param.bt, param.cfg.WithVar(s), s.OutDir(), s.HasError, sm, server.URL, htmlPageTitle)
		if err != nil {
		...
		// Record the metrics.
		recordStartupPerf(pv, "session_start", sessionStartTime)
		recordStartupPerf(pv, "browser_restore", browserRestoreTime)
		if param.bt == browser.TypeLacros {
			recordStartupPerf(pv, "histogram.lacros_start", histogramLacrosStartTime)
		}
	}
...
func performRegularLogin(ctx context.Context, browserType browser.Type, cfg *browserfixt.LacrosConfig, outDir string, hasError func() bool, sm *session.SessionManager, url, expectedTitle string) (time.Duration, time.Duration, time.Duration, error) {
	...
	if err := waitForWindow(ctx, browserType, tconn, expectedTitle); err != nil {
		return 0, 0, 0, errors.Wrapf(err, "failed to restore to %v browser", browserType)
	}
