...
func runSpeedometerTest(ctx context.Context, pd launcher.PreData, conn *chrome.Conn) (float64, error) {
	...
	if err := conn.Eval(ctx, `
		...
		runner.runMultipleIterations(benchmarkClient.iterationCount);`, nil); err != nil {
		return 0.0, errors.Wrap(err, "speedometer tests did not run")
	}
	if err := conn.WaitForExprFailOnErrWithTimeout(ctx, `benchmarkClient.finished`, time.Minute*10); err != nil {
		return 0.0, errors.Wrap(err, "speedometer tests did not finish")
	}

	var score float64
	if err := conn.Eval(ctx, `benchmarkClient.totalScore / benchmarkClient.iterCount`, &score); err != nil {
		return 0.0, errors.Wrap(err, "failed to fetch speedometer score")
	}
	return score, nil
}
