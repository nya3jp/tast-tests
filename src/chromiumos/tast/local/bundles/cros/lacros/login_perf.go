...
var lacrosSideBySideConfig = lacrosfixt.LacrosConfig{
	...
	// TODO(tvignatti): lacrosfixt.LacrosOnly is a misleading name but it's okay to use it for now:
	// https://chromium-review.googlesource.com/c/chromiumos/platform/tast-tests/+/3499547/comment/9f82df17_60465e81
	Availability: lacrosfixt.LacrosOnly,
}
...
func LoginPerf(ctx context.Context, s *testing.State) {
	...
	// TODO(tvignatti): if I try to raise this say to '7', then I get "Failed to connect
	// session_manager: failed waiting for org.chromium.SessionManager service: context deadline
	// exceeded"
	const iterationCount = 3
	pv := perf.NewValues()
	for i := 0; i < iterationCount; i++ {
		sm, err := performInitialLogin(ctx, s, server.URL)
		if err != nil {
			s.Fatal("Failed to do initial login: ", err)
		}

		sessionStartTime, browserRestoreTime, umaLacrosStartTime, err := performRegularLogin(ctx, s, sm, server.URL, htmlPageTitle)
		if err != nil {
			s.Fatal("Failed to do regular login: ", err)
		}

		recordLoginPerf(pv, "sessionStart", sessionStartTime)
		recordLoginPerf(pv, "browserRestore", browserRestoreTime)
		recordLoginPerf(pv, "lacrosStartTime", umaLacrosStartTime)
...
// performInitialLogin TODO(tvignatti)
func performInitialLogin(ctx context.Context, s *testing.State, url string) (*session.SessionManager, error) {
	var bt = s.Param().(testParam).browserType
	var cfg lacrosfixt.LacrosConfig
	if s.Param().(testParam).lacrosConfig == (lacrosfixt.LacrosConfig{}) {
		cfg = lacrosfixt.LacrosConfig{}
	} else {
...
func performRegularLogin(ctx context.Context, s *testing.State, sm *session.SessionManager, url, expectedTitle string) (time.Duration, time.Duration, time.Duration, error) {
	var bt = s.Param().(testParam).browserType
	...
	umaLacrosStartTime, err := histogramToDuration(ctx, tconn, "ChromeOS.Lacros.StartTime")
	if err != nil {
		s.Fatal("Failed to get histogram: ", err)
	}
	testing.ContextLog(ctx, "LoginPerf: Got ChromeOS.Lacros.StartTime")

	return time.Duration(sessionStartTime), time.Duration(browserRestoreTime), umaLacrosStartTime, nil
}
