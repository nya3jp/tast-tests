...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			Val: testParam{
				browser.TypeLacros,
				// Due lacrosDeployedBinary var is required here, this variant will actually use
				// Lacros deployed by the user instead the rootfs from DefaultLacrosConfig.
				browserfixt.DefaultLacrosConfig,
				true,
			},
		}, {
		...
		VarDeps: []string{browserfixt.LacrosDeployedBinary},
	})
}
...
func LoginPerf(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < iterationCount; i++ {
		...
		if err := performInitialLogin(ctx, param, cfg, s.OutDir(), s.HasError, server.URL); err != nil {
			s.Fatal("Failed to do initial login: ", err)
		}

		// Start to collect data, restart UI, wait for browser window to be opened and get the
		// metrics.
		sessionStartTime, browserRestoreTime, histogramLacrosStartTime, err := performRegularLogin(ctx, param, cfg, s.OutDir(), s.HasError, sm, server.URL, htmlPageTitle)
		if err != nil {
		...
		if param.bt == browser.TypeLacros {
...
func performInitialLogin(ctx context.Context, param testParam, cfg *browserfixt.LacrosConfig, outDir string, hasError func() bool, url string) error {
	...
	cr, br, _, err := browserfixt.SetUpWithNewChrome(ctx, param.bt, cfg)
	if err != nil {
		return errors.Wrapf(err, "failed to connect to %v browser", param.bt)
	}
	defer cr.Close(ctx)
...
func performRegularLogin(ctx context.Context, param testParam, cfg *browserfixt.LacrosConfig, outDir string, hasError func() bool, sm *session.SessionManager, url, expectedTitle string) (time.Duration, time.Duration, time.Duration, error) {
	...
	if err != nil {
		return 0, 0, 0, errors.Wrap(err, "failed to watch for D-Bus signals")
	}
	defer sw.Close(ctx)
	...
	if err := waitForWindow(ctx, param.bt, tconn, expectedTitle); err != nil {
		return 0, 0, 0, errors.Wrapf(err, "failed to restore to %v browser", param.bt)
	}

	browserRestoreTime := time.Since(ashChromeStartTime)
	testing.ContextLog(ctx, "LoginPerf: Got browser restore time")

	if param.bt == browser.TypeAsh {
		return sessionStartTime, browserRestoreTime, 0, nil
	}

	histogramLacrosStartTime, err := histogramToDuration(ctx, tconn, "ChromeOS.Lacros.StartTime")
	if err != nil {
		return 0, 0, 0, errors.Wrap(err, "failed to get histogram")
	}
	testing.ContextLog(ctx, "LoginPerf: Got ChromeOS.Lacros.StartTime")

	return sessionStartTime, browserRestoreTime, histogramLacrosStartTime, nil
}
