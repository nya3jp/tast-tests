...
func LoginPerf(ctx context.Context, s *testing.State) {
	...
	bt, sm, err := performInitialLogin(ctx, s, server.URL)
	if err != nil {
		s.Fatal("Failed to do initial optin: ", err)
	}

	performRegularLogin(ctx, s, bt, sm, server.URL, htmlPageTitle)
}
...
func createHTMLServer(title string) *httptest.Server {
	// html page with a specified title.
...
func performInitialLogin(ctx context.Context, s *testing.State, url string) (browser.Type, *session.SessionManager, error) {
...
func performRegularLogin(ctx context.Context, s *testing.State, bt browser.Type, sm *session.SessionManager, url, expectedTitle string) error {
	...
	pv.Set(perf.Metric{
		Name:      "lacrosStartTime",
		Unit:      "seconds",
		Direction: perf.SmallerIsBetter,
	}, umaLacrosStartTime.Seconds())
