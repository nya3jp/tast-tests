...
const (
	// loginPerfRestoreURL is the URL used in Lacros at its first launch and then also the expected one to be running
	// in Lacros after the UI gets restarted.
	loginPerfRestoreURL = "https://abc.xyz"

	// loginPerfRestoreURLtitle is the URL title used for confirming in Lacros second launch, after UI is restarted.
	// It has to match with the content provided in loginPerfRestoreURL.
...
func LoginPerf(ctx context.Context, s *testing.State) {
	func() {
		const bt = browser.TypeLacros

		// Connect to a fresh ash-chrome instance to ensure the UI session first-run state, also get a browser
		// instance, in this case Lacros, for using browser functionality.
		...
		// Connect to Lacros and navigate to the URL that later is expected to be restored.
		...
		// According to the PRD of Full Restore go/chrome-os-full-restore-dd, it uses a throttle of 2.5s to save the
		// app launching and window statue information to the backend. Therefore, sleep 3 seconds here.
	...
	func() {
		...
		s.Log("Start measuring Lacros login time")
		start := time.Now()

		cr, err := chrome.New(ctx,
			// Set not to clear the notification after restore. By default, On startup is set to ask every time after
			// reboot and there is an alertdialog asking the user to select whether to restore or not.
		...
		s.Log("Stop measuring Lacros login time")
		...
		// After measurements are done, and for the sake of completeness, confirm that the Settings app is restored
		// also. This may be useful for situations where Lacros is not started, to quickly check whether other session
		// windows got restored.
