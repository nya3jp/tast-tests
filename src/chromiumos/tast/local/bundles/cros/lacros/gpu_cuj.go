...
func runTestChromeOS(ctx context.Context, pd launcher.PreData, pv *perf.Values, url string) error {
	...
	err = runHistogram(ctx, ctconn, pv, "cros")
	if err != nil {
		return err
	}
...
func GpuCUJ(ctx context.Context, s *testing.State) {
	...
	p := s.Param().(gpuCUJParams)

	pv := perf.NewValues()
	err = runTestLacros(ctx, s.PreValue().(launcher.PreData), pv, p.url)
	if err != nil {
		s.Fatal("Failed to run test: ", err)
	}

	err = runTestChromeOS(ctx, s.PreValue().(launcher.PreData), pv, p.url)
	if err != nil {
		s.Fatal("Failed to run test: ", err)
	}
