...
func GpuCUJ(ctx context.Context, s *testing.State) {
	...
	if err := ubertray.ToggleQuickSetting(ctx, tconn, ubertray.QuickSettingDoNotDisturb, true); err != nil {
		s.Fatal("Failed to disable notifications: ", err)
	}
	defer func() {
		if err := ubertray.ToggleQuickSetting(ctx, tconn, ubertray.QuickSettingDoNotDisturb, false); err != nil {
