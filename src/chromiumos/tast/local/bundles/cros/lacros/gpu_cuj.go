...
// tconn is a test connection to the current browser being used (either ChromeOS or Linux chrome). conn is
// a connection to a window to be used for the test.
func runTest(ctx context.Context, pd launcher.PreData, pv *perf.Values, p gpuCUJTestParams, lacros bool, tconn *chrome.TestConn, conn *chrome.Conn) error {
	...
	testName := string(p.testType)
	if lacros {
		testName = testName + ".lacros"
	} else {
		testName = testName + ".cros"
	}
...
func runLacrosTest(ctx context.Context, pd launcher.PreData, pv *perf.Values, p gpuCUJTestParams) error {
	...
	return runTest(ctx, pd, pv, p, true, ltconn, conn)
}
...
func runCrosTest(ctx context.Context, pd launcher.PreData, pv *perf.Values, p gpuCUJTestParams) error {
	...
	return runTest(ctx, pd, pv, p, false, ctconn, conn)
}
