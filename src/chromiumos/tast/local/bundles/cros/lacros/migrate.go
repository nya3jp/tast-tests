...
const (
	...
	insertIndexedDBDataJS = `async () => {
		async function insertIndexedDBData() {
			return new Promise((resolve) => {
				const req = window.indexedDB.open("someDataBase", 1 /* version */);
				req.onupgradeneeded = e => {
					const db = e.target.result;
					const objectStore = db.createObjectStore("users", { keyPath: "id" });
					objectStore.transaction.oncomplete = e => {
						const userObjectStore = db.transaction("users", 'readwrite').objectStore("users");
						userObjectStore.add({ id: 1, email: "test@gmail.com" });
						resolve(true);
					}
	...
	getIndexedDBDataJS = `async () => {
		async function getIndexedDBData() {
			return new Promise(resolve => {
				...
				req.onsuccess = e => {
					const db = e.target.result;
					const req = db.transaction("users").objectStore("users").get(1);
					req.onsuccess = () => resolve(req.result)
					req.onerror = resolve
				}
			})
		}
		const user = await getIndexedDBData();
		return user.email == 'test@gmail.com';
	}`
)
...
func prepareAshProfile(ctx context.Context, s *testing.State, kb *input.KeyboardEventWriter) {
	...
	var success bool
	if err := conn.Call(ctx, &success, insertIndexedDBDataJS); err != nil {
...
func verifyLacrosProfile(ctx context.Context, s *testing.State, kb *input.KeyboardEventWriter, cr *chrome.Chrome) {
	...
	func() {
		...
		var success bool
		if err := conn.Call(ctx, &success, getIndexedDBDataJS); err != nil {
