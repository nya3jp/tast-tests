...
func prepareAshProfile(ctx context.Context, s *testing.State, kb *input.KeyboardEventWriter) {
	...
	if err := conn.Call(ctx, nil, `(key, value) => localStorage.setItem(key, value)`, localStorageKey, storageValue); err != nil {
		s.Fatal("Failed to set localStorage localSalue: ", err)
	}
...
func verifyLacrosProfile(ctx context.Context, s *testing.State, kb *input.KeyboardEventWriter, cr *chrome.Chrome) {
	...
	func() {
		...
		if err := conn.Call(ctx, &contained,
			`(key, value) => { return localStorage.getItem(key) == value; }`, localStorageKey, storageValue); err != nil {
