...
func newLibfakedatetime() *timeSyscallOverrideLibrary {
	...
	return &timeSyscallOverrideLibrary{"libfakedatetime.so", leapFunc}
}
...
func testLibfakedatetime(ctx context.Context) error {
	testing.ContextLog(ctx, "testing the libfakedatetime")
	...
	if actualYear < expectedYear {
		return errors.Errorf("Assert libfakedatetime to simulate year (at least) %d, got: %d", expectedYear, actualYear)
	}
	testing.ContextLog(ctx, "libfakedatetime tests passed")
	return nil
}
...
func HAL3AUE(ctx context.Context, s *testing.State) {
	...
	if err := testLibfakedatetime(ctx); err != nil {
		s.Error("libfakedatetime tests failed: ", err)
		return
	}
