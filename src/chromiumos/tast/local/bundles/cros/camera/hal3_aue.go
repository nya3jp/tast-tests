...
// getLibraryPath will try to find the a file that matches the name
...
func createTestProcessCmd(ctx context.Context) (*testexec.Cmd, error) {
	...
	timeOffsetSeconds := int64(secondsInOneDay * daysInOneYear * aueYearOffset)
...
// and will also return cleanup function if success.
...
func testLibfakedatetime(ctx context.Context) error {
	...
	timeOffsetSeconds := int64(secondsInOneDay * (daysInOneYear + 5))
