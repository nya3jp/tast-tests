...
func GCAMultiCamera(ctx context.Context, s *testing.State) {
	gca.RunTest(ctx, s, func(ctx context.Context, a *arc.ARC, d *ui.Device) {
		...
		if err != nil {
			s.Fatal("Cannot get the default direction of camera: ", err)
		}
		...
		if facing, err := gca.GetFacing(ctx, d); err != nil {
			...
			s.Fatalf("GCA didn't open a camera with the default camera facing (default: %s, GCA opened: %s)", defaultFacing, facing)
		}
	})
}
