...
func CCAUIA11y(ctx context.Context, s *testing.State) {
	...
	// Wait until the shutter button is focused
	...
	// Because of busy resources, ChromeVox has a problem of not fully reading the first statement.
	// Sometimes, aria label of the first element or ChromeVox starting message is missing.
	// As a workaround, this test calls Consume() and skips to the next element without checking whether it is correct.
	if err = sm.Consume(ctx, []a11y.SpeechExpectation{a11y.NewRegexExpectation("ChromeVox spoken feedback is ready")}); err != nil {
		s.Log("Failed to match speeches: ", err)
	}
