...
func CCAUIA11y(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < len(expectedSpeeches1); i++ {
		if err := moveAroundByKeyboard(ctx, sm, s, app, []a11y.SpeechExpectation{expectedSpeeches1[i]}); err != nil {
	...
	for i := 0; i < len(expectedSpeeches2); i++ {
		if err := moveAroundByKeyboard(ctx, sm, s, app, []a11y.SpeechExpectation{expectedSpeeches2[i]}); err != nil {
			s.Fatal("Failed to check all functions: ", err)
		}
	}
}

func moveAroundByKeyboard(ctx context.Context, sm *a11y.SpeechMonitor, s *testing.State, app *cca.App, expectedSpeech []a11y.SpeechExpectation) error {
	tab := []string{"Tab"}
