...
func CCAUIA11y(ctx context.Context, s *testing.State) {
	...
	expectedSpeech := a11y.NewStringExpectation("Camera mode")
	if err := a11y.PressKeysAndCheckExpContain(ctx, sm, ctrlAltZ, expectedSpeech); err != nil {
		s.Log("Failed to enable Chromevox or the different mention is spoken: ", err)
	}

	tab := []string{"Tab"}
	expectedSpeech = a11y.NewStringExpectation("Button")
	if err := a11y.PressKeysAndCheckExpContain(ctx, sm, tab, expectedSpeech); err != nil {
	...
	defer func() {
		a11y.PressKeysAndCheckExpContain(ctx, sm, ctrlAltZ, a11y.NewStringExpectation(""))
	}()
}
...
func takePicture(ctx context.Context, sm *a11y.SpeechMonitor, s *testing.State, app *cca.App) error {
	...
	space := []string{"Space"}
	if err := a11y.PressKeysAndCheckExpContain(ctx, sm, space, a11y.NewStringExpectation("")); err != nil {
