...
func CCAUIExpert(ctx context.Context, s *testing.State) {
	...
	verifyExpertMode := func(on bool) {
		...
		} else if containsMetadata(fileInfos) != on {
			s.Error("Metadata is missing")
		}
	}
	...
	for _, action := range []struct {
		...
		{"switchCamera", switchCamera, false},
		...
		if err := action.Func(ctx, app); err != nil {
			...
			verifyExpertMode(action.Result)
		}
	}
}
