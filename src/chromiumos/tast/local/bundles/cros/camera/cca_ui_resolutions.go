	...
	if strings.HasSuffix(path, ".mkv") {
		...
		for _, b := range boxes {
			...
			if thkd.Width > 0 && thkd.Height > 0 {
				...
				// All possible rotation matrices in mp4 thkd produced from CCA.
				rotate0 := [9]int32{65536, 0, 0, 0, 65536, 0, 0, 0, 1073741824}
				rotat90 := [9]int32{0, 65536, 0, -2147418112, 0, 0, 0, 0, 1073741824}
				rotate180 := [9]int32{-2147418112, 0, 0, 0, -2147418112, 0, 0, 0, 1073741824}
				rotat270 := [9]int32{0, -2147418112, 0, 65536, 0, 0, 0, 0, 1073741824}
				switch thkd.Matrix {
				case rotate0:
				case rotate180:
				case rotat90:
