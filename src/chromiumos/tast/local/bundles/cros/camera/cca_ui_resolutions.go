...
// rtype is the type of resolution.
type rtype string

const (
	// photoResolution is the resolution type of photo resolution.
	photoResolution rtype = "photo"
	// videoResolution is the resolution type of video resolution.
	videoResolution rtype = "video"
)
...
func testPhotoResolution(ctx context.Context, app *cca.App, saveDir string) error {
	return cca.RunThruCameras(ctx, app, func(facing cca.Facing) error {
		...
		for i, r := range rs {
			testing.ContextLogf(ctx, "Switch to %dx%d resolution", r.Width, r.Height)
			// CCA UI will filter out resolutions of megapixels < 0.1 i.e. megapixels 0.0
...
func getVideoFileResolution(path string) (int, int, error) {
	doc, err := matroska.Decode(path)
	if err != nil {
		return -1, -1, errors.Wrapf(err, "failed to decode video file of path %v", path)
	}
	for _, track := range doc.Segment.Tracks {
		for _, ent := range track.Entries {
			if ent.Type == matroska.TrackTypeVideo {
				videoTrack := ent.Video
				return videoTrack.Width, videoTrack.Height, nil
			}
		}
	}
	return -1, -1, errors.New("no video track found in the file")
}
...
func testVideoResolution(ctx context.Context, app *cca.App, saveDir string) error {
	return cca.RunThruCameras(ctx, app, func(facing cca.Facing) error {
		...
		for i, r := range rs {
			...
			w, h, err := getVideoFileResolution(filepath.Join(saveDir, info.Name()))
			if err != nil {
				return err
			}
			if r.Width != w || r.Height != h {
				return errors.Wrapf(err, "incorrect captured resolution get %dx%d; want %dx%d", w, h, r.Width, r.Height)
			}
		}
		return nil
	})
...
func switchResolution(ctx context.Context, app *cca.App, t rtype, facing cca.Facing, index int) error {
	testing.ContextLogf(ctx, "Switch to %v th %v resolution", index, t)

	checkOpen := func(view string, openned bool) error {
		if active, err := app.GetState(ctx, view); err != nil {
			return errors.Wrap(err, "cannot get view open state")
		} else if active != openned {
			expect := "closed"
			if openned {
				expect = "openned"
			}
			return errors.Errorf("view %q is not %v", view, expect)
		}
		return nil
	}

	openSetting := func(name string, s string) error {
		testing.ContextLogf(ctx, "Open %q view", name)
		if err := app.ClickWithSelector(ctx, s); err != nil {
			return err
		}
		return checkOpen(name, true)
	}
	...
	view := fmt.Sprintf("%sresolutionsettings", t)
	if err := openSetting(view, fmt.Sprintf("#settings-%s-%sres", fname, t)); err != nil {
		return err
	}
