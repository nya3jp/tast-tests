...
func switchResolution(ctx context.Context, app *cca.App, rt resolutionType, facing cca.Facing, index int) error {
	...
	if toggled, err := app.IsCheckedWithIndex(ctx, optionUI, index); err != nil {
		return err
	} else if toggled {
		testing.ContextLogf(ctx, "%d th resolution option is already toggled", index)
	} else {
		if err := app.TriggerConfiguration(ctx, func() error {
			testing.ContextLogf(ctx, "Toggling %d th resolution option", index)
