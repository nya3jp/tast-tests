...
func videoTrackResolution(path string) (*cca.Resolution, error) {
	if strings.HasSuffix(path, ".mkv") {
		...
		boxes, err := mp4.ExtractBoxesWithPayload(file, nil, []mp4.BoxPath{{mp4.BoxTypeMoov(), mp4.BoxTypeTrak()}})
		if err != nil {
		...
		for _, b := range boxes {
			...
			if thkd.Width > 0 && thkd.Height > 0 {
				return &cca.Resolution{Width: int(thkd.Width), Height: int(thkd.Height)}, nil
			}
		}
	}
