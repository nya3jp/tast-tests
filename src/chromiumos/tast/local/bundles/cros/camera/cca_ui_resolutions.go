...
func testPhotoResolutionAndAspectRatio(ctx context.Context, app *cca.App) error {
	return app.RunThroughCameras(ctx, func(facing cca.Facing) error {
		...
		testing.ContextLog(ctx, "Checking amount of aspect ratio options")
		...
		for index := 0; index < numOptions; index++ {
			...
			if err := clickThroughAllPhotoResolutionOptions(ctx, app, facing, aspectRatio); err != nil {
...
func checkResolutionAspectRatio(ctx context.Context, resolution *cca.Resolution, aspectRatio float64) error {
	...
	if math.Abs(resolution.AspectRatio()-aspectRatio) > aspectRatioTolerance {
		return errors.Errorf("inconsistent aspect ratio. Image resolution: %d:%d; Want aspect ratio: %v", resolution.Width, resolution.Height, aspectRatio)
	}
	return nil
}
...
func testVideoResolutionAndFPS(ctx context.Context, app *cca.App) error {
	...
	return app.RunThroughCameras(ctx, func(facing cca.Facing) error {
		...
		for index := 0; index < numOptions; index++ {
			...
			if err := checkResolutionAspectRatio(ctx, &pr, r.AspectRatio()); err != nil {
				return err
			}
			...
			if vr.Width != r.Width || vr.Height != r.Height {
