...
func iterateResolutions(ctx context.Context, app *cca.App, rt cca.ResolutionType, facing cca.Facing, onSwitched func(r cca.Resolution) error) error {
	openInnerResolutionSetting := func(onOpened func() error) error {
	...
	countOptions := func() (int, error) {
		var optionNum int
		if err := openInnerResolutionSetting(func() error {
	...
	toggleOption := func(index int) (cca.Resolution, error) {
		...
		err := openInnerResolutionSetting(func() error {
	...
	for index := 0; index < optionNum; index++ {
		if r, err := toggleOption(index); err != nil {
			return err
