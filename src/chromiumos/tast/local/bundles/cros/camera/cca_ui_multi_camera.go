...
func CCAUIMultiCamera(ctx context.Context, s *testing.State) {
	...
	checkFacing := func() {
		...
		// or on device without facing configurations(Returns facing unknown
		// for every cameras). Try to query facing from every available cameras
		// to ensure its true failure.
