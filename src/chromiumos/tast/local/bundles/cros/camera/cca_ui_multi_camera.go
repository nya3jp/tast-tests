...
func CCAUIMultiCamera(ctx context.Context, s *testing.State) {
	...
	checkFacing := func() {
		...
		// tablet or clamshell mode on device without camera with that facing
		// or on device without facing configurations(Returns unknown for every
		// cameras). Try to query facing from every available cameras to ensure
		// the failure.
