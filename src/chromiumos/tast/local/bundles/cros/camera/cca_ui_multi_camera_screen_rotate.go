			...
			s.Run(ctx, tc.name, func(ctx context.Context, s *testing.State) {
				...
				if mode == cca.Video {
					if _, err := app.RecordVideo(ctx, cca.TimerOff, 1*time.Second); err != nil {
						s.Fatal("Failed to record video in %v subtest: %v", tc.name, err)
					}
				}

				if err := app.SaveScreenshot(ctx); err != nil {
					s.Error("Failed to save a screenshot in %v subtest: %v", tc.name, err)
				}

				orient, err := app.GetScreenOrientation(ctx)
				if err != nil {
					s.Fatal("Failed to get screen orientation in %v subtest: %v", tc.name, err)
				}
