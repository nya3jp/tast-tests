...
func CCAUIGalleryButton(ctx context.Context, s *testing.State) {
	...
	if thumbnail == newThumbnail {
		s.Error("Background image is not updated after capture")
	}
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if thumbnail == newThumbnail {
			return errors.New("Background image is not updated")
		}
		return nil
	}, &testing.PollOptions{Timeout: 10 * time.Second}); err != nil {
		s.Error("Failed to update background image of the gallery button after the file it points to is deleted: ", err)
	}
}
