...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Opens CCA and measures the CPU usage and power consumption",
...
// application and starts measuring system CPU usage and power consumption.
func CCAUIPreviewPerf(ctx context.Context, s *testing.State) {
	cr := s.PreValue().(*chrome.Chrome)

	err := cca.MeasurePerformance(ctx, cr, []string{s.DataPath("cca_ui.js")}, s.OutDir(), false)
	if err != nil {
