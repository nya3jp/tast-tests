...
func ArcCameraOrientation(ctx context.Context, s *testing.State) {
	...
	if err := a.Command(ctx, "pm", "grant", pkg, "android.permission.CAMERA").Run(testexec.DumpLogOnError); err != nil {
		s.Fatal("Railed granting camera permission to test app: ", err)
	}

	if err := a.Command(ctx, "am", "start", "-W", act).Run(); err != nil {
	...
	if err != nil {
		s.Fatal("Failed to read test result")
	}
	...
	if err != nil {
		s.Fatal("Failed to read test result log")
	}
