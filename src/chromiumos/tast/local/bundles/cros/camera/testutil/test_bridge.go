...
func NewTestBridge(ctx context.Context, cr *chrome.Chrome, useSWA bool) (*TestBridge, error) {
	pageConn, worker, bridge, err := setUpTestBridge(ctx, cr, useSWA)
	if err != nil {
		return nil, err
	}
	return &TestBridge{cr, pageConn, worker, bridge, useSWA}, nil
}

func setUpTestBridge(ctx context.Context, cr *chrome.Chrome, useSWA bool) (*chrome.Conn, *chrome.JSObject, *chrome.JSObject, error) {
	...
	if err != nil {
		return nil, nil, nil, errors.Wrap(err, "failed to construct bridge page connection")
	}

	if err := pageConn.Eval(ctx, "location.reload()", nil); err != nil {
		return nil, nil, nil, errors.Wrap(err, "failed to reload the test extension")
	}
	if err := pageConn.WaitForExpr(ctx, "document.readyState === 'complete'"); err != nil {
		return nil, nil, nil, errors.Wrap(err, "failed to wait for the ready state")
	}

	var worker chrome.JSObject
	var testBridge chrome.JSObject
	err = func() error {
		if err := pageConn.Call(ctx, &worker, `
			function() {
			  const workerPath = '/js/test_bridge.js';
			  return new SharedWorker(workerPath, {type: 'module'});
			}
		`); err != nil {
			return err
		}

		if err := worker.Call(ctx, &testBridge, `
			async function() {
			  const Comlink = await import('/js/lib/comlink.js');
			  return Comlink.wrap(this.port);
			}
		`); err != nil {
			if err2 := worker.Release(ctx); err2 != nil {
				testing.ContextLog(ctx, "Failed to release worker: ", err2)
			}
			return err
		}
		return nil
	...
	if err != nil {
		...
		return nil, nil, nil, errors.Wrap(err, "failed to get test bridge")
	}
	return pageConn, &worker, &testBridge, nil
}
...
func (t *TestBridge) Reset(ctx context.Context) error {
	...
	pageConn, worker, bridge, err := setUpTestBridge(ctx, t.cr, t.useSWA)
	if err != nil {
...
func (t *TestBridge) TearDown(ctx context.Context) error {
	...
	if t.worker != nil {
		if err := t.worker.Call(ctx, nil, `function() { this.close(); }`); err != nil {
			testing.ContextLog(ctx, "Failed to close worker: ", err)
		}
		if err := t.worker.Release(ctx); err != nil {
			testing.ContextLog(ctx, "Failed to release worker: ", err)
		}
		t.worker = nil
	}
	if t.pageConn != nil {
