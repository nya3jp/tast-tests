...
func (a *AppWindow) NotifyReady(ctx context.Context) error {
	return a.jsObj.Call(ctx, nil, "function() { this.notifyReadyOnTastSide(); }")
}
...
func (a *AppWindow) WaitUntilClosed(ctx context.Context) error {
	return a.jsObj.Call(ctx, nil, "function() { this.waitUntilClosed(); }")
}

// IsClosingItself checks if CCA intends to close itself.
func (a *AppWindow) IsClosingItself(ctx context.Context) (bool, error) {
	var isClosing bool
	if err := a.jsObj.Call(ctx, &isClosing, "function() { return this.isClosingItself(); }"); err != nil {
		return false, err
	}
	return isClosing, nil
}
...
func (a *AppWindow) Release(ctx context.Context) {
	a.jsObj.Release(ctx)
	a.jsObj = nil
}
