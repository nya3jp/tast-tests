...
var (
	...
	chromeWithPlatformApp = chrome.NewPrecondition("cca_platform_app", chrome.ExtraArgs("--disable-features=CameraSystemWebApp"))
...
// ChromeWithPlatformApp returns a precondition that Chrome is already logged in and CCA is installed as an platform app when a test is run.
func ChromeWithPlatformApp() testing.Precondition { return chromeWithPlatformApp }

// ARCWithPlatformApp returns a precondition that ARC Container has already booted and CCA is installed as an platform app when a test is run.
