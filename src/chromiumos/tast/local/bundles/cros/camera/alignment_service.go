...
func (a *AlignmentService) ManualAlign(ctx context.Context, req *pb.ManualAlignRequest) (*empty.Empty, error) {
	...
	cr, err := chrome.New(
		ctx, chrome.ARCDisabled(), chrome.Auth(req.Username, req.Password, ""),
		chrome.GAIALogin(), chrome.KeepState(), chrome.LogPath(req.DataPath))
	if err != nil {
	...
	conn, err := cr.NewConn(ctx, srv.URL+"/preview.html")
	if err != nil {
	...
	testing.ContextLog(ctx, "Wait for preview page loaded")
	if err := conn.WaitForExpr(ctx, "manualAlign !== undefined"); err != nil {
		return nil, errors.Wrap(err, "failed to wait for preview page loaded")
	}
	testing.ContextLog(ctx, "Preview page loaded")

	if err := conn.Call(ctx, nil, "manualAlign"); err != nil {
		return nil, errors.Wrap(err, "failed to call manualAlign on preview.html")
	}
