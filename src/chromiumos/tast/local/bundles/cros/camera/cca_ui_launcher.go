...
func CCAUILauncher(ctx context.Context, s *testing.State) {
	...
	if isMinimized, err := app.IsWindowMinimized(ctx); err != nil {
		...
		s.Fatal("App should be minimized after firing launch when the window is shown")
	}
	...
	if isMinimized, err := app.IsWindowMinimized(ctx); err != nil {
		...
		s.Fatal("App should be restored after firing launch when the window is minimized")
	}
}
