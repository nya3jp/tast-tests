...
func HDRnetPerf(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Errorf("Failed to connect to Chrome: %s", err)
	}
	defer cr.Close(ctx)
	...
	const (
		subTestTimeout  time.Duration = 5 * time.Minute
		stablDuration                 = 5 * time.Second
		...
		for _, t := range testCases {
			...
			s.Run(subTestCtx, name, func(cts context.Context, s *testing.State) {
				...
				if err := page.OpenWithConstraints(subTestCtx, cr, cst); err != nil {
					s.Fatalf("Failed to open camera test page: %s", err)
				}
				...
				pctx := camperf.Start(subTestCtx, stablDuration, measureDuration, name, subtestOutdir)
				...
				if err := page.Close(subTestCtx); err != nil {
					s.Errorf("Failed to close camera test page: %s", err)
				}
