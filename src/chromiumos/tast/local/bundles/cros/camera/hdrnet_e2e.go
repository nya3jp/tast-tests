...
func init() {
	testing.AddTest(&testing.Test{
		...
		SoftwareDeps: []string{"camera_feature_hdrnet", "camera_app", "chrome"},
...
func HdrnetE2E(ctx context.Context, s *testing.State) {
	...
	if err := createFile(hdrnetEnablePath); err != nil {
		s.Fatal("Failed to create HDRnet force enable file")
	}
	defer os.Remove(hdrnetEnablePath)
...
func testVideoRecording(ctx context.Context, app *cca.App, tconn *chrome.TestConn) error {
	...
	if _, err := histogramTests.wait(ctx, tconn, recorder); err != nil {
		return errors.Wrap(err, "failed to flush old histograms")
	}
