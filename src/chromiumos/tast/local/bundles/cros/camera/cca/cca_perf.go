...
func MeasurePerformance(ctx context.Context, cr *chrome.Chrome, scripts []string, options MeasurementOptions) (retErr error) {
	...
	defer (func() {
		if err := app.CheckErrorEvents(ctx); err != nil {
			if retErr != nil {
				testing.ContextLog(ctx, "Failed with javascript errors events: ", err)
			} else {
				retErr = errors.Wrap(err, "failed with javascript errors events")
			}
		}
	})()
...
func measureStreamingPerformance(ctx context.Context, cr *chrome.Chrome, app *App, perfValues *perf.Values, facing Facing, isRecording bool) error {
	...
	if isRecording {
		// Start the recording.
		var err error
		recordingStartTime, err = app.StartRecording(ctx, false)
		if err != nil {
