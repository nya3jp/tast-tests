...
func SetupTestEnvironment(ctx context.Context, config ChromeConfig) (*TestEnvironment, error) {
	...
	if err := testBridgeConn.WaitForExpr(ctx, "window['TestBridge'] !== undefined"); err != nil {
...
func launchApp(ctx context.Context, env *TestEnvironment, appLauncher AppLauncher, appWindow *chrome.JSObject) (*chrome.Conn, error) {
	if err := env.TestBridgeConn.Eval(ctx, "window['TestBridge'].registerUnboundWindow()", appWindow); err != nil {
...
func LaunchSWA(ctx context.Context, tconn *chrome.TestConn) error {
	return apps.LaunchSystemWebApp(ctx, tconn, "Camera",
		"chrome://camera-app/src/views/main.html?isTesting=true")
}
...
func (e *TestEnvironment) SetLaunchStartTime(ctx context.Context, isWarmLaunch bool) error {
	...
	codes := fmt.Sprintf("window['TestBridge'].setLaunchStartTime(%q)", launchEventName)
...
func (e *TestEnvironment) GetPerfEntries(ctx context.Context) ([]perfEntry, error) {
	...
	if err := e.TestBridgeConn.Eval(ctx, "window['TestBridge'].perfs", &entries); err != nil {
		return nil, err
	}
	if err := e.TestBridgeConn.Eval(ctx, "window['TestBridge'].perfs = []", nil); err != nil {
		return nil, err
	}
	return entries, nil
}
...
func (e *TestEnvironment) GetErrors(ctx context.Context) ([]errorInfo, error) {
	...
	if err := e.TestBridgeConn.Eval(ctx, "window['TestBridge'].errors", &errorInfos); err != nil {
		return nil, err
	}
	if err := e.TestBridgeConn.Eval(ctx, "window['TestBridge'].errors = []", nil); err != nil {
		return nil, err
	}
