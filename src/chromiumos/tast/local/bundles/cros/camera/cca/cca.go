...
func (a *App) WaitForState(ctx context.Context, state string, active bool) error {
	...
	if lState, hasLegacy := legacyStateMap[state]; hasLegacy {
		// Before/After migration only state with old/new name will be
		// flagged or unflagged, another new/old name state will always
		// be false. Thus, combining obtained two values with || is
		// what we need here.
