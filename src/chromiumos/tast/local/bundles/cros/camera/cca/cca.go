...
func (a *App) SwitchMode(ctx context.Context, mode Mode) error {
	...
	if err := a.conn.Call(ctx, nil, "(mode) => Tast.switchMode(mode)", mode); err != nil {
...
func (a *App) TriggerConfiguration(ctx context.Context, trigger func() error) error {
	if err := a.conn.Eval(ctx, "void(CCAConfigurationReady = Tast.waitNextConfiguration())", nil); err != nil {
		return errors.Wrap(err, "failed to set CCAConfigurationReady")
	}
	if err := trigger(); err != nil {
		return err
	}
	if err := a.conn.Eval(ctx, "CCAConfigurationReady", nil); err != nil {
		return errors.Wrap(err, "failed to evaluate CCAConfigurationReady")
	}
	return nil
}
