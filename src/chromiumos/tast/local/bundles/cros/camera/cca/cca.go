...
func Init(ctx context.Context, cr *chrome.Chrome, scriptPaths []string, outDir string, appLauncher AppLauncher) (*App, error) {
	...
	shouldMeasureCoverage := shouldMeasureCoverage(ctx)
	if shouldMeasureCoverage {
		addExecutionController := fmt.Sprintf(`
			...
			if (typeof startExecute === 'undefined') {
				var startExecute;
			}
			...
			startExecute = () => {
				executionControlPort.postMessage('READY_TO_START');
			}
	...
	if shouldMeasureCoverage {
		...
		if err := tconn.Exec(ctx, `startExecute`); err != nil {
			return nil, err
		}
	}
	...
	return &App{conn, cr, scriptPaths, outDir, bgConn}, nil
}
