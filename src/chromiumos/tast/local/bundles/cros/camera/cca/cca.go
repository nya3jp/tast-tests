...
func (a *App) TakeSinglePhoto(ctx context.Context, isPortrait bool, timerOn bool) ([]os.FileInfo, error) {
	if err := a.SetTimerOption(ctx, timerOn); err != nil {
		return nil, err
	}
	start := time.Now()
	...
	} else if elapsed := info.ModTime().Sub(start); timerOn && elapsed < TimerDelay {
