...
// RunThruCameras run specified function after switching to each available camera.
func RunThruCameras(ctx context.Context, app *App, f func()) error {
	...
	if len(devices) != numCameras {
		return errors.Errorf("failed to switch to some camera, run camera %v", devices)
	}
	return nil
}
