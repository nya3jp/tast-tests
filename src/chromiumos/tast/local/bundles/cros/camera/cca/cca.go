...
func Init(ctx context.Context, env *TestEnvironment, scriptPaths []string, outDir string, appLauncher AppLauncher) (*App, error) {
	...
	conn, appWindow, err := launchApp(ctx, env, appLauncher)
	if err != nil {
	...
	for _, scriptPath := range scriptPaths {
		...
		if err := conn.Call(ctx, nil, string(script), jsPath(env.Config.InstallSWA)); err != nil {
			return nil, err
		}
	}
...
func (a *App) Restart(ctx context.Context) error {
	if err := a.CheckJSError(ctx, a.outDir); err != nil {
