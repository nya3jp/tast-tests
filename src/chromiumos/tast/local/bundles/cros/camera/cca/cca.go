...
func Init(ctx context.Context, env *testutil.TestEnvironment, scriptPaths []string, outDir string, appLauncher testutil.AppLauncher) (*App, error) {
	cr := env.Chrome
	...
	conn, appWindow, err := testutil.LaunchApp(ctx, env, appLauncher)
	if err != nil {
	...
	for _, scriptPath := range scriptPaths {
		...
		if err := conn.Call(ctx, nil, string(script), testutil.JSPath(env.Config.InstallSWA)); err != nil {
			return nil, err
		}
	}
...
func New(ctx context.Context, env *testutil.TestEnvironment, scriptPaths []string, outDir string) (*App, error) {
	if env.Config.InstallSWA {
		return Init(ctx, env, scriptPaths, outDir, testutil.LaunchSWA)
	}
	return Init(ctx, env, scriptPaths, outDir, testutil.LaunchPlatformApp)
}
