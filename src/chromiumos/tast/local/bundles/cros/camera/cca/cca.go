...
// IsToggledWithIndex gets toggle state of nth ui.
func (a *App) IsToggledWithIndex(ctx context.Context, ui UIComponent, index int) (bool, error) {
	selector, err := a.resolveUISelector(ctx, ui)
	if err != nil {
		return false, err
	}
	...
	var toggled bool
	if err := a.conn.Eval(ctx, code, &toggled); err != nil {
		return false, errors.Wrapf(err, "failed to get toggled state on %v(th) %v", index, ui.Name)
	}
	return toggled, nil
}
...
// WaitForConfiguration triggers configuration by calling trigger() and waits for camera configuration finishing.
func (a *App) WaitForConfiguration(ctx context.Context, trigger func() error) error {
	var observerID int
	if err := a.conn.Eval(ctx, "Tast.ConfigureObserver.setObserver()", &observerID); err != nil {
		return err
	}
	if err := trigger(); err != nil {
		return err
	}
	return a.conn.EvalPromise(ctx, fmt.Sprintf("Tast.ConfigureObserver.waitUntilFinish(%d)", observerID), nil)
}
