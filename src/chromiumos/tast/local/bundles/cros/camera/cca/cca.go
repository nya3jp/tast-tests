...
// VerifyNoFileSaved verifies no captured file with specified filename pattern being saved after specified timestamp.
func (a *App) VerifyNoFileSaved(ctx context.Context, pat *regexp.Regexp, ts time.Time) error {
	path, err := a.getSavedDir(ctx)
	if err != nil {
		return err
	}
	...
	for _, file := range files {
		if file.ModTime().After(ts) && pat.MatchString(file.Name()) {
			return errors.Errorf("assert no captured file saved, found: %v", file.Name())
		}
	}
	return nil
}
...
func (a *App) SwitchMode(ctx context.Context, mode Mode) error {
	...
	// switching to specified mode. Verify the mode value again after switching.
	if active, err := a.GetState(ctx, string(mode)); err != nil {
		return errors.Wrapf(err, "failed to get mode state after switch to mode: %v", mode)
	} else if !active {
		return errors.Wrapf(err, "failed to switching to mode: %v", mode)
	}
	return nil
}
