...
// GetOption gets value of option from its classname.
func (a *App) GetOption(ctx context.Context, option string) (bool, error) {
	var result bool
	err := a.conn.Eval(ctx, fmt.Sprintf("cca.state.get('%s')", option), &result)
	if err != nil {
		err = errors.Wrapf(err, "failed to get option: %v", option)
	}
	return result, err
}
...
func (a *App) toggleOption(ctx context.Context, option string, toggleSelector string) (bool, error) {
	prev, err := a.GetOption(ctx, option)
	if err != nil {
		return false, err
	}
