...
const (
	...
	Expert = "expert"
	// Metadata is the state used to indicate save metadata.
...
func (a *App) TakeSinglePhoto(ctx context.Context, timerState TimerState) ([]os.FileInfo, error) {
	...
	isExpert, err := a.GetState(ctx, string(Expert))
	if err != nil {
		return nil, err
	}
	isMetadata, err := a.GetState(ctx, string(Metadata))
	if err != nil {
		return nil, err
	}
	// For metadata, check the extra metadata file.
	if isMetadata && isExpert {
