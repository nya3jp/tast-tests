...
func (sr *SuspendResume) SetUp(ctx context.Context, cfg *pb.SuspendResumeConfig) (*empty.Empty, error) {
	...
	// TODO(crbug/1111679): Use service-scoped context once it's supported.
	sr.a, err = arc.New(context.Background(), sr.td)
	if err != nil {
		return out, errors.Wrap(err, "failed to start ARC")
	}

	// TODO(crbug/1111679): Use service-scoped context once it's supported.
	sr.d, err = ui.NewDevice(context.Background(), sr.a)
	if err != nil {
...
func (sr *SuspendResume) ClearResult(ctx context.Context, e *empty.Empty) (*empty.Empty, error) {
	// HACK: UiAutomation process doesn't seem to persist.
	// sr.d, _ = ui.NewDevice(ctx, sr.a)
	out := new(empty.Empty)
