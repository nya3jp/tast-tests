...
func checkIntentBehavior(ctx context.Context, s *testing.State, cr *chrome.Chrome, a *arc.ARC, options intentOptions) error {
	...
	if err := checkCaptureResult(ctx, app, options.Mode, options.ShouldReturnResult, options.ShouldCheckResult, options.ResultInfo); err != nil {
		return err
	}
	// For intents which expects to receive result, CCA should auto close after
	// the capture is done.
	if err := checkAutoCloseBehavior(ctx, cr, options.ShouldReturnResult); err != nil {
		return err
	}
	return nil
}
