	...
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			Val: []testCase{{
				...
				Format: "launch camera on photo mode",
				IntentOptions: intentOptions{
				...
				Format: "launch camera on video mode",
				IntentOptions: intentOptions{
				...
				Format: "record video (has extras)",
				IntentOptions: intentOptions{
					Action:       recordVideoAction,
					URI:          testVideoURI,
					Mode:         cca.Video,
	...
	for _, tc := range s.Param().([]testCase) {
		s.Run(ctx, tc.Format, func(ctx context.Context, s *testing.State) {
			if err := checkIntentBehavior(ctx, s, cr, a, uiDevice, tc.IntentOptions); err != nil {
				s.Errorf("Failed for %s behavior test: %v", tc.Format, err)
			}
		})
	}
