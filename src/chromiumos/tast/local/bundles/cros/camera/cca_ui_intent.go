...
func CCAUIIntent(ctx context.Context, s *testing.State) {
	...
	for _, tc := range testCases {
		s.Run(ctx, tc.Name, func(ctx context.Context, s *testing.State) {
			if err := checkIntentBehavior(ctx, s, env, a, uiDevice, tc.IntentOptions); err != nil {
	...
	s.Run(ctx, "instances coexistanece test", func(ctx context.Context, s *testing.State) {
		if err := checkInstancesCoexistence(ctx, s, env, a); err != nil {
...
func launchIntent(ctx context.Context, env *testutil.TestEnvironment, a *arc.ARC, options intentOptions, scripts []string, outDir string) (*cca.App, error) {
	...
	return cca.Init(ctx, env, scripts, outDir, launchByIntent)
}
...
func checkIntentBehavior(ctx context.Context, s *testing.State, env *testutil.TestEnvironment, a *arc.ARC, uiDevice *ui.Device, options intentOptions) error {
	app, err := launchIntent(ctx, env, a, options, []string{s.DataPath("cca_ui.js")}, s.OutDir())
	if err != nil {
		return err
	}
	defer closeCCAAndTestApp(ctx, a, app, s.OutDir())
	...
	if options.TestBehavior.ShouldCloseDirectly {
		if err := closeCCA(ctx, app, s.OutDir()); err != nil {
			return err
		}
	} else {
		...
		if err := checkAutoCloseBehavior(ctx, env, shouldAppAutoClose); err != nil {
			return err
		}
	}
...
func checkAutoCloseBehavior(ctx context.Context, env *testutil.TestEnvironment, shouldClose bool) error {
	...
	if shouldClose {
		...
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			if isExist, err := cca.InstanceExists(ctx, cr, env.Config.InstallSWA); err != nil {
...
func checkInstancesCoexistence(ctx context.Context, s *testing.State, env *testutil.TestEnvironment, a *arc.ARC) error {
	// Launch regular CCA.
	regularApp, err := cca.New(ctx, env, []string{s.DataPath("cca_ui.js")}, s.OutDir())
	if err != nil {
		return errors.Wrap(err, "failed to launch CCA")
	}
	defer closeCCA(ctx, regularApp, s.OutDir())
	...
	intentApp, err := launchIntent(ctx, env, a, intentOptions{
		Action:       takePhotoAction,
		URI:          "",
		Mode:         cca.Photo,
		TestBehavior: captureConfirmAndDone,
	}, []string{s.DataPath("cca_ui.js")}, s.OutDir())
	if err != nil {
		return errors.Wrap(err, "failed to launch CCA by intent")
	}
	defer closeCCAAndTestApp(ctx, a, intentApp, s.OutDir())
	...
	if err := closeCCA(ctx, intentApp, s.OutDir()); err != nil {
