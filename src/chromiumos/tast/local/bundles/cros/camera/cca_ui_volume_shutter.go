...
func testAppInBackground(ctx context.Context, cr *chrome.Chrome, app *cca.App, kb *input.KeyboardEventWriter, vh *volumeHelper) (retErr error) {
	...
	defer (func(ctx context.Context) {
		if err := conn.CloseTarget(ctx); err != nil {
			if retErr != nil {
				testing.ContextLog(ctx, "Failed to close chrome setting: ", err)
			} else {
				retErr = errors.Wrap(err, "failed to close chrome setting")
			}
		}
		if err := conn.Close(); err != nil {
			if retErr != nil {
				...
				retErr = errors.Wrap(err, "failed to close connection to chrome settings")
			}
		}
