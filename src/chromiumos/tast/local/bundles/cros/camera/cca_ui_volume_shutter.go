...
func newVolumeTracker(ctx context.Context) (*volumeHelper, error) {
...
func (vt *volumeHelper) setVolume(ctx context.Context, volume int) error {
	return vt.cras.SetOutputNodeVolume(ctx, *vt.activeNode, volume)
}

func (vt *volumeHelper) currentVolume(ctx context.Context) (int, error) {
	node, err := getActiveCrasNode(ctx, vt.cras)
	if err != nil {
		return 0, err
	}
	if vt.activeNode.ID != node.ID {
		return 0, errors.Errorf("active node ID changed from %v to %v during the test", vt.activeNode.ID, node.ID)
	}
	vt.activeNode = node
...
func (vt *volumeHelper) verifyVolumeChanged(ctx context.Context, doChange func() error) error {
	prevVolume, err := vt.currentVolume(ctx)
	if err != nil {
		return err
	}
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		volume, err := vt.currentVolume(ctx)
		if err != nil {
			return err
		}
...
func CCAUIVolumeShutter(ctx context.Context, s *testing.State) {
	...
	originalVolume, err := vt.currentVolume(ctx)
...
func testSwitchDeviceMode(ctx context.Context, app *cca.App, kb *input.KeyboardEventWriter, vt *volumeHelper) error {
