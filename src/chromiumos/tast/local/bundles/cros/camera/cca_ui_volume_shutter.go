...
func (vh *volumeHelper) verifyVolumeChanged(ctx context.Context, doChange func() error) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			return errors.Wrap(err, "failed to get volume after doChange()")
		}
...
func testAppInBackground(ctx context.Context, cr *chrome.Chrome, app *cca.App, kb *input.KeyboardEventWriter, vh *volumeHelper) error {
	cleanupCtx := ctx
	ctx, cancel := ctxutil.Shorten(ctx, time.Second*5)
	defer cancel()
	...
	if err != nil {
		return errors.Wrap(err, "failed to open chrome settings")
	}
	defer (func(ctx context.Context) {
		if err := conn.CloseTarget(ctx); err != nil {
			testing.ContextLog(ctx, "Failed to close chrome settings: ", err)
		}
		if err := conn.Close(); err != nil {
			testing.ContextLog(ctx, "Failed to close connection to chrome settings: ", err)
		}
