...
func checkState(actual videoState, expecteds ...videoState) error {
	...
	if !found {
		return errors.Errorf("Assert video in %v state; get %v state", expecteds, actual)
	}
	return nil
}

// video tracks the expected video state across CCA's video manupulation.
type video struct {
...
// startRecordAndPause starts recording for 1 second, pauses the recording.
...
func testPauseResume(ctx context.Context, app *cca.App) error {
	...
	if err := testing.Sleep(ctx, time.Second); err != nil {
		return errors.Wrap(err, "failed to sleep 1 second in recording state")
	}
