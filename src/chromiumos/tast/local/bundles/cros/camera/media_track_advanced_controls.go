...
func verifyAdvancedControls(ctx context.Context, s *testing.State, conn *chrome.Conn) {
	parameters := [...]testParameter{
		{"exposureTime", []controlParameter{{"exposureMode", true, "manual", 0.0}}, 1.0, []float64{0.0}},
		{"focusDistance", []controlParameter{{"focusMode", true, "manual", 0.0}}, 0.01, []float64{0.0}},
		{"colorTemperature", []controlParameter{{"whiteBalanceMode", true, "manual", 0.0}}, 0.0, []float64{0.0}},
		{"exposureCompensation", []controlParameter{{"exposureMode", true, "continuous", 0.0}}, 0.0, []float64{0.0}},
		{"iso", []controlParameter{{"exposureMode", true, "manual", 0.0}, {"exposureTime", false, "", 333.0}}, 5.0, []float64{0.0}},
		{"brightness", []controlParameter{}, 0.0, []float64{0.0}},
		{"contrast", []controlParameter{}, 0.0, []float64{0.0}},
		{"saturation", []controlParameter{}, 0.0, []float64{0.0}},
		{"sharpness", []controlParameter{}, 0.0, []float64{0.0}},
		{"pan", []controlParameter{}, 0.0, []float64{0.0}},
		{"tilt", []controlParameter{}, 0.0, []float64{0.0}},
		{"zoom", []controlParameter{}, 0.0, []float64{0.0}},
	}
...
func setControlParameters(settings *mediaTrackSettings, parameters []controlParameter) {
	for i := 0; i < len(parameters); i++ {
		if parameters[i].isString {
			reflect.ValueOf(settings).Elem().FieldByName(strings.Title(parameters[i].control)).Set(reflect.ValueOf(&parameters[i].valueString))
		} else {
			reflect.ValueOf(settings).Elem().FieldByName(strings.Title(parameters[i].control)).Set(reflect.ValueOf(&parameters[i].valueFloat))
		}
	}
}
...
func verifyControl(ctx context.Context, s *testing.State, conn *chrome.Conn, parameter testParameter, isValid bool) {
	for _, value := range parameter.testValues {
		...
		setControlParameters(&constraints.Advanced[0], parameter.neededControls)
