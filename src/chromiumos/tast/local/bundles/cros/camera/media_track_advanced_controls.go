...
func getMediaTrackCapabilitiesField(capabilities *mediaTrackCapabilities, field string) uintptr {
	var ret uintptr
	switch field {
	case "whiteBalanceMode":
		ret = uintptr(unsafe.Pointer(&(*capabilities).WhiteBalanceMode))
	case "exposureMode":
		ret = uintptr(unsafe.Pointer(&(*capabilities).ExposureMode))
	case "focusMode":
		ret = uintptr(unsafe.Pointer(&(*capabilities).FocusMode))
	case "exposureCompensation":
		ret = uintptr(unsafe.Pointer(&(*capabilities).ExposureCompensation))
	case "exposureTime":
		ret = uintptr(unsafe.Pointer(&(*capabilities).ExposureTime))
	case "colorTemperature":
		ret = uintptr(unsafe.Pointer(&(*capabilities).ColorTemperature))
	case "iso":
		ret = uintptr(unsafe.Pointer(&(*capabilities).Iso))
	case "brightness":
		ret = uintptr(unsafe.Pointer(&(*capabilities).Brightness))
	case "contrast":
		ret = uintptr(unsafe.Pointer(&(*capabilities).Contrast))
	case "saturation":
		ret = uintptr(unsafe.Pointer(&(*capabilities).Saturation))
	case "sharpness":
		ret = uintptr(unsafe.Pointer(&(*capabilities).Sharpness))
	case "focusDistance":
		ret = uintptr(unsafe.Pointer(&(*capabilities).FocusDistance))
	case "pan":
		ret = uintptr(unsafe.Pointer(&(*capabilities).Pan))
	case "tilt":
		ret = uintptr(unsafe.Pointer(&(*capabilities).Tilt))
	case "zoom":
		ret = uintptr(unsafe.Pointer(&(*capabilities).Zoom))
	default:
		ret = uintptr(unsafe.Pointer(nil))
	}
	return ret
}

func getMediaTrackSettingsField(settings *mediaTrackSettings, field string) uintptr {
	var ret uintptr
	switch field {
	case "whiteBalanceMode":
		ret = uintptr(unsafe.Pointer(&(*settings).WhiteBalanceMode))
	case "exposureMode":
		ret = uintptr(unsafe.Pointer(&(*settings).ExposureMode))
	case "focusMode":
		ret = uintptr(unsafe.Pointer(&(*settings).FocusMode))
	case "exposureCompensation":
		ret = uintptr(unsafe.Pointer(&(*settings).ExposureCompensation))
	case "exposureTime":
		ret = uintptr(unsafe.Pointer(&(*settings).ExposureTime))
	case "colorTemperature":
		ret = uintptr(unsafe.Pointer(&(*settings).ColorTemperature))
	case "iso":
		ret = uintptr(unsafe.Pointer(&(*settings).Iso))
	case "brightness":
		ret = uintptr(unsafe.Pointer(&(*settings).Brightness))
	case "contrast":
		ret = uintptr(unsafe.Pointer(&(*settings).Contrast))
	case "saturation":
		ret = uintptr(unsafe.Pointer(&(*settings).Saturation))
	case "sharpness":
		ret = uintptr(unsafe.Pointer(&(*settings).Sharpness))
	case "focusDistance":
		ret = uintptr(unsafe.Pointer(&(*settings).FocusDistance))
	case "pan":
		ret = uintptr(unsafe.Pointer(&(*settings).Pan))
	case "tilt":
		ret = uintptr(unsafe.Pointer(&(*settings).Tilt))
	case "zoom":
		ret = uintptr(unsafe.Pointer(&(*settings).Zoom))
	default:
		ret = uintptr(unsafe.Pointer(nil))
	}
	return ret
...
func verifyAdvancedControls(ctx context.Context, s *testing.State, conn *chrome.Conn) {
	...
	for _, parameter := range parameters {
		ss := getMediaTrackCapabilitiesField(&capabilities, parameter.control)
		settingRangeAddr := *(**mediaSettingRange)(unsafe.Pointer(ss))
...
func setPrerequisite(settings, prerequisite *mediaTrackSettings) {
	...
	for _, field := range allStringFields {
		value := *(**string)(unsafe.Pointer(getMediaTrackSettingsField(prerequisite, field)))
		if value != nil {
			*(**string)(unsafe.Pointer(getMediaTrackSettingsField(settings, field))) = value
		}
	}
	for _, field := range allFloatFields {
		value := *(**float64)(unsafe.Pointer(getMediaTrackSettingsField(prerequisite, field)))
		if value != nil {
			*(**float64)(unsafe.Pointer(getMediaTrackSettingsField(settings, field))) = value
		}
	}
}
...
func verifyControl(ctx context.Context, s *testing.State, conn *chrome.Conn, parameter testParameter, isValid bool) {
	for _, value := range parameter.testValues {
		...
		origValue := **(**float64)(unsafe.Pointer(getMediaTrackSettingsField(&settings, parameter.control)))
		setPrerequisite(&constraints.Advanced[0], &parameter.prerequisite)
		*(**float64)(unsafe.Pointer(getMediaTrackSettingsField(&constraints.Advanced[0], parameter.control))) = &value
		...
		getValue := **(**float64)(unsafe.Pointer(getMediaTrackSettingsField(&settings, parameter.control)))
...
func verifyAllControls(ctx context.Context, s *testing.State, conn *chrome.Conn, parameters []testParameter) {
	...
	for _, parameter := range parameters {
		setPrerequisite(&constraints.Advanced[0], &parameter.prerequisite)
		*(**float64)(unsafe.Pointer(getMediaTrackSettingsField(&constraints.Advanced[0], parameter.control))) = &parameter.testValues[0]
	}
	err := applyMediaTrackConstraints(ctx, conn, constraints)
	if err != nil {
		s.Fatal("Can't apply constraints: ", err)
