...
func verifyControl(ctx context.Context, s *testing.State, conn *chrome.Conn, control mediaControl, isValid bool) {
	...
	for _, value := range testValues {
		...
		origValue := **control.getSettingFloatField(&settings)
		prerequisite := control.getPrerequisite()
		setPrerequisite(&constraints.Advanced[0], &prerequisite)
		*control.getSettingFloatField(&constraints.Advanced[0]) = &value
		...
		getValue := **control.getSettingFloatField(&settings)
