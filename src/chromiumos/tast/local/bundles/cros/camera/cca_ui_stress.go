...
func init() {
	testing.AddTest(&testing.Test{
		...
		// The timeout here is exceptional large for manual stress testing,
		// like new platform bring-up stability test. When running on CI
		// builders, the timeout would be shorten in the test according to the
...
func CCAUIStress(ctx context.Context, s *testing.State) {
	...
	realTimeout := time.Duration(iterations)*perIterationTimeout + cleanupTimeout
	ctx, cancel := context.WithTimeout(ctx, realTimeout)
	defer cancel()
