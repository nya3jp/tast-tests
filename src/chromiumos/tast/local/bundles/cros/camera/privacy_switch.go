...
func hasPrivacySwitchControl(ctx context.Context, s *testing.State) bool {
	...
	for _, devicePath := range usbCams {
		...
		if err == nil {
			s.Log("Device ", devicePath, " has a Privacy Control")
			return true
		}
...
func hasPrivacySwitchHardware(ctx context.Context, s *testing.State) bool {
	...
	out, err := exec.Command("cros_config", "/camera", "count").Output()
	if err != nil {
	...
	for i := 0; i < count; i++ {
		out, err = exec.Command("cros_config", fmt.Sprintf("/camera/devices/%v", i), "has-privacy-switch").Output()
		if err == nil && string(out) == "true" {
			s.Log("Camera ", i, " supports Privacy Switch")
			return true
		}
	}
