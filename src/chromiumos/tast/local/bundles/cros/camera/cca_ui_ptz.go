...
// testToggle tests toggling the control |c|.
func (c *ptzControl) testToggle(ctx context.Context, app *cca.App) error {
	pRect, err := findPattern(ctx, app)
	if err != nil {
		return errors.Wrapf(err, "failed to find pattern before clicking %v: %v", c.ui.Name, err)
	}
	if err := app.ClickPTZButton(ctx, *c.ui); err != nil {
		return errors.Wrapf(err, "failed to click: %v", err)
	}
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		rect, err := findPattern(ctx, app)
		if err != nil {
			return errors.Wrapf(err, "failed to find pattern after clicking %v: %v", c.ui.Name, err)
		}
		result, err := c.testFunc(pRect, rect)
		if err != nil {
		...
		return errors.Wrapf(err, "failed to run %v test func: %v", c.ui.Name, err)
	}
	return nil
}
