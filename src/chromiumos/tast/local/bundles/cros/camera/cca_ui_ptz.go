...
func rgbToYUV(c color.RGBA) (byte, byte, byte) {
	y := float64(c.R)*0.299000 + float64(c.G)*0.587000 + float64(c.B)*0.114000
	u := float64(c.R)*-0.168736 + float64(c.G)*-0.331264 + float64(c.B)*0.500000 + 128
	v := float64(c.R)*0.500000 + float64(c.G)*-0.418688 + float64(c.B)*-0.081312 + 128
...
// findPattern finds pattern region on preview.
...
func CCAUIPTZ(ctx context.Context, s *testing.State) {
	...
	// Do all comparison with 1px precision error introducing by fake file
	// VCD bilinear resizing implementation.
	const precision = 1
	abs := func(n int) int {
		if n < 0 {
			return -n
	...
	for _, tc := range []struct {
		...
		if err != nil {
			s.Fatalf("Failed to find pattern before click %v: %v", tc.ui.Name, err)
		}
		...
		if err != nil {
			s.Fatalf("Failed to find pattern after click %v: %v", tc.ui.Name, err)
		}
