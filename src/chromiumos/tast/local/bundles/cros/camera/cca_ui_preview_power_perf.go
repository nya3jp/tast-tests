...
func CCAUIPreviewPowerPerf(ctx context.Context, s *testing.State) {
	...
	app, err := cca.Init(ctx, cr, []string{s.DataPath("cca_ui.js")}, s.OutDir(), func(tconn *chrome.TestConn) error {
		launchApp := fmt.Sprintf(`tast.promisify(chrome.management.launchApp)(%q);`, cca.ID)
	...
	if err != nil {
		s.Fatal(errors.Wrap(err, "failed to open CCA"))
	}

	defer app.Close(ctx)

	if err := app.MaximizeWindow(ctx); err != nil {
		s.Fatal(errors.Wrap(err, "failed to maximize camera app"))
	}
