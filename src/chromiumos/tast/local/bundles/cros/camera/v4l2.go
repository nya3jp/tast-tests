...
func getTestList() (string, error) {
	const v1Path = "/usr/bin/arc_camera_service"
	const v3Path = "/usr/bin/cros_camera_service"
...
func isV4L2CaptureDevice(ctx context.Context, path string) bool {
	cmd := testexec.CommandContext(ctx, "media_v4l2_is_capture_device", path)
...
func getUSBInfo(path string) (string, error) {
	...
	info := strings.TrimSpace(string(vid)) + ":" + strings.TrimSpace(string(pid))
	return info, nil
}
