...
// config and writes logs to outDir.  The cros-camera service must be stopped
// before calling this function.
func runCrosCameraTest(ctx context.Context, outDir string, cfg crosCameraTestConfig) error {
...
// according to the given config and writes the logs to outDir.
func RunTest(ctx context.Context, outDir string, cfg TestConfig) (err error) {
	...
	defer func() {
		...
		if startErr := upstart.EnsureJobRunning(ctx, "cros-camera"); startErr != nil {
			err = errors.Wrap(startErr, "failed to start cros-camera")
		}
	}()
	...
	if len(cfg.CameraFacing) > 0 {
		...
		if err := runCrosCameraTest(shortCtx, outDir, cameraCfg); err != nil {
			return err
		}
