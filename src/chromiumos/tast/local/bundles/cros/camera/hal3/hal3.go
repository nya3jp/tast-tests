...
const (
	...
	mediaProfilePath           = "/mnt/stateful_partition/encrypted/var/cache/camera/media_profiles.xml"
)
...
func getRecordingParams(ctx context.Context) (string, error) {
	out, err := testexec.CommandContext(ctx, "generate_camera_profile").Output(testexec.DumpLogOnError)
	if err != nil {
		return "", err
	}
	cmd := shutil.EscapeSlice([]string{"cat", mediaProfilePath})
