...
func getRecordingParams(ctx context.Context) (string, error) {
	mediaProfilePath, err := getMediaProfilePath(ctx)
	if err != nil {
		return "", err
	}
...
// getMediaProfilePath gets the ARC media profile file path used on the device.
func getMediaProfilePath(ctx context.Context) (string, error) {
