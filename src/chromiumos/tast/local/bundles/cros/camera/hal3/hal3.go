...
// gtestResult is a helper struct for accessing gtest xml output file.
...
// getFailedTests returns failed test names from the gtest xml output file.
...
type TestConfig struct {
	...
	// won't fallback to SW encode when the HW encode failed.
	ForceJPEGHWEnc bool
	// ForceJPEGHWDec is the flag to enforce hardware decode for JPEG, so it
	// won't fallback to SW decode when the HW decode failed.
	ForceJPEGHWDec bool
}
...
func RunTest(ctx context.Context, s *testing.State, cfg TestConfig) {
	...
	if len(cfg.CameraFacing) > 0 {
		runCrosCameraTest(ctx, s, cameraCfg)
	} else {
