...
const (
	...
	downloadBase               = "http://commondatastorage.googleapis.com/chromiumos-test-assets-public/camera_HAL3/"
	jsonConfigPath             = "/var/cache/camera/test_config.json"
	mediaProfilePath           = "/vendor/etc/media_profiles.xml"
	testDataPath               = "/var/cache/camera/"
)
...
type crosCameraTestConfig struct {
	...
	skipCheckCrosCamera  bool   // skip checking the cros-camera service.
...
// downloadFile downloads from a url to a local file.
...
func RunTest(ctx context.Context, cfg TestConfig) (retErr error) {
	...
	if cfg.PortraitModeTestData != "" {
		if err := downloadFile(testDataPath+cfg.PortraitModeTestData, downloadBase+cfg.PortraitModeTestData); err != nil {
			return errors.Wrap(err, "failed to download portrait mode test data")
		}
		cameraCfg.portraitModeTestData = testDataPath + cfg.PortraitModeTestData
	}
	...
	if len(cfg.CameraFacing) > 0 || cfg.RunAdapterTest {
		cameraCfg.skipCheckCrosCamera = cfg.RunAdapterTest
