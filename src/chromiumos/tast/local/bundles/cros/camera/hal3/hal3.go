...
func runCrosCameraTest(ctx context.Context, cfg crosCameraTestConfig, stopCrosCameraService bool) error {
	if stopCrosCameraService {
...
func RunTest(ctx context.Context, cfg TestConfig) (retErr error) {
	...
	if !cfg.ConnectToCameraService {
		...
		if upstart.JobExists(ctx, "cros-camera") {
			...
			if err := upstart.EnsureJobRunning(ctx, "cros-camera"); err != nil {
				return errors.Wrap(err, "cros-camera service is not running")
			}
		}
	}
	...
	if len(cfg.CameraFacing) > 0 || cfg.ConnectToCameraService {
		...
		if err := runCrosCameraTest(shortCtx, cameraCfg, !cfg.ConnectToCameraService); err != nil {
			return err
		}
		...
		for i, path := range paths {
			...
			if err := runCrosCameraTest(shortCtx, cameraCfg, false); err != nil {
				return err
			}
			if err := updatePerfIfNeeded(); err != nil {
				return err
