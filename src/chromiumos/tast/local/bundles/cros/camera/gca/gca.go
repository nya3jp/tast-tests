...
func ClickShutterButtonAndVerifyFile(ctx context.Context, s *testing.State, d *ui.Device, pat string) {
	...
	if err != nil {
		s.Fatal("Failed to read the directory for saving media files")
	}
...
func RunTest(ctx context.Context, s *testing.State, f TestFunc) {
	...
	if err := a.Command(ctx, "pm", "grant", pkg, "android.permission.ACCESS_FINE_LOCATION").Run(testexec.DumpLogOnError); err != nil {
		s.Fatal("Failed to grant ACCESS_FINE_LOCATION permission to GCA: ", err)
	}
	if err := a.Command(ctx, "pm", "grant", pkg, "android.permission.ACCESS_COARSE_LOCATION").Run(testexec.DumpLogOnError); err != nil {
		s.Fatal("Failed to grant ACCESS_COARSE_LOCATION permission to GCA: ", err)
	}
