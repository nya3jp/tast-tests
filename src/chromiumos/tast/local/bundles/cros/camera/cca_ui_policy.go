...
func servePolicy(ctx context.Context, fdms *fakedms.FakeDMS, cr *chrome.Chrome, ps []policy.Policy, tb *testutil.TestBridge) (retErr error) {
...
func testBlockCCAExtension(ctx context.Context, fdms *fakedms.FakeDMS, cr *chrome.Chrome, tb *testutil.TestBridge) error {
	...
	if err := policyutil.ServeAndVerify(ctx, fdms, cr, []policy.Policy{&policy.ExtensionInstallBlacklist{Val: []string{cca.ID}}}); err != nil {
		return errors.Wrap(err, "failed to serve policy")
	}

	// We first need to tear down the previous connection to the background page
	// and then we can check if the background page is available.
