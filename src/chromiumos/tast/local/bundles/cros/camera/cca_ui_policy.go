...
func CCAUIPolicy(ctx context.Context, s *testing.State) {
	...
	if err := testNoPolicy(ctx, cr, scripts, outDir, tb, fakeDMS, isSWA); err != nil {
		s.Error("Failed to test with no policy: ", err)
	}

	if err := testBlockCCAExtension(ctx, cr, fakeDMS); err != nil {
		s.Error("Failed to block CCA extension: ", err)
	}

	if err := testBlockCameraFeature(ctx, cr, fakeDMS); err != nil {
		s.Error("Failed to block camera feature: ", err)
	}

	if err := testBlockVideoCapture(ctx, cr, scripts, outDir, tb, fakeDMS, isSWA); err != nil {
		s.Error("Failed to block video capture: ", err)
	}
}

func servePolicy(ctx context.Context, cr *chrome.Chrome, ps []policy.Policy, fakeDMS *fakedms.FakeDMS) error {
	if err := policyutil.ResetChrome(ctx, fakeDMS, cr); err != nil {
		return errors.Wrap(err, "failed to reset Chrome")
	}

	if err := policyutil.ServeAndVerify(ctx, fakeDMS, cr, ps); err != nil {
...
func testNoPolicy(ctx context.Context, cr *chrome.Chrome, scripts []string, outDir string, tb *testutil.TestBridge, fakeDMS *fakedms.FakeDMS, isSWA bool) error {
	if err := servePolicy(ctx, cr, []policy.Policy{}, fakeDMS); err != nil {
		return errors.Wrap(err, "failed to serve policy")
	}
...
func testBlockCCAExtension(ctx context.Context, cr *chrome.Chrome, fakeDMS *fakedms.FakeDMS) error {
	if err := servePolicy(ctx, cr, []policy.Policy{&policy.ExtensionInstallBlacklist{Val: []string{cca.ID}}}, fakeDMS); err != nil {
		return err
	}
...
func testBlockCameraFeature(ctx context.Context, cr *chrome.Chrome, fakeDMS *fakedms.FakeDMS) error {
	if err := servePolicy(ctx, cr, []policy.Policy{&policy.SystemFeaturesDisableList{Val: []string{"camera"}}}, fakeDMS); err != nil {
		return errors.Wrap(err, "failed to serve policy")
	}
...
func testBlockVideoCapture(ctx context.Context, cr *chrome.Chrome, scripts []string, outDir string, tb *testutil.TestBridge, fakeDMS *fakedms.FakeDMS, isSWA bool) error {
	if err := servePolicy(ctx, cr, []policy.Policy{&policy.VideoCaptureAllowed{Val: false}}, fakeDMS); err != nil {
		return errors.Wrap(err, "failed to serve policy")
	}
	// Since we have reset Chrome, we should reset the test bridge so that we
	// can communicate with CCA again in the following tests.
