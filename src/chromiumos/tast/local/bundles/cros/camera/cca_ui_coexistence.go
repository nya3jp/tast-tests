...
func testOpenCCAFirstAndCloseCCAFirst(ctx context.Context, cr *chrome.Chrome, openCCA func() (*cca.App, error), pageURL string) error {
	...
	defer cleanup(ctx, pageConn, trackState)
	...
	if pageConn, trackState, err = closeWebPage(ctx, pageConn, trackState); err != nil {
...
func closeWebPage(ctx context.Context, pageConn *chrome.Conn, trackState *chrome.JSObject) (*chrome.Conn, *chrome.JSObject, error) {
	var hasEnded bool
	var err error
	...
	return pageConn, trackState, err
}

func cleanup(ctx context.Context, pageConn *chrome.Conn, trackState *chrome.JSObject) {
	if pageConn != nil || trackState != nil {
		if _, _, err := closeWebPage(ctx, pageConn, trackState); err != nil {
