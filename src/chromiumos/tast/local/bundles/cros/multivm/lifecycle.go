...
func Lifecycle(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < numTasks/numTypes; i++ {
		if param.inHost {
			task := server.NewMemoryStressTask(int(stats.ExponentialInt64(taskAllocMiB, r)), compressRatio, hostLimit)
			tabsAliveTasks = append(tabsAliveTasks, task)
			tasks = append(tasks, task)
		}
		if param.inARC {
			task := memoryuser.NewArcLifecycleTask(len(appsAliveTasks), int64(stats.ExponentialInt64(taskAllocMiB, r))*memory.MiB, compressRatio, hostLimit)
