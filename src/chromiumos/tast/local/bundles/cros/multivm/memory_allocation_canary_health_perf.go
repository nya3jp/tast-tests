...
// updateOpenFileLimit updates the limit of the number of open files by the test process.
// We need this because this test opens one file (stdin) for one memory allocator.
func updateOpenFileLimit(ctx context.Context, limit int) error {
	pid := os.Getpid()
	testing.ContextLogf(ctx, "pid: %d", pid)
	cmd := testexec.CommandContext(ctx, "prlimit", fmt.Sprintf("--pid=%d", pid), fmt.Sprintf("--nofile=%d:%d", limit, limit))
	testing.ContextLogf(ctx, "The open file limit is increased to %d", limit)
...
func stressCanary(ctx context.Context, s *testing.State, param *canaryHealthPerfParam, cr *chrome.Chrome, a *arc.ARC) (int64, time.Duration, error) {
	...
	err := updateOpenFileLimit(ctx, openFileLimit)
	if err != nil {
	...
	for true {
		...
		if allocationManager.NumOfAllocators() >= openFileLimit/2 {
			openFileLimit *= 2
			updateOpenFileLimit(ctx, openFileLimit)
		}
