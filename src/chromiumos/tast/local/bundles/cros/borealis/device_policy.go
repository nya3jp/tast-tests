...
func DevicePolicy(ctx context.Context, s *testing.State) {
	...
	// Install the dlc now so we don't continually reinstall it in the below loop.
	...
	for _, param := range []struct {
		...
		},
		{
			name:               "User Disabled",
			shouldAllowInstall: false,
			policies:           []policy.Policy{&policy.UserBorealisAllowed{Val: false}},
		},
		{
			name:               "VMs Disabled",
			shouldAllowInstall: false,
			policies:           []policy.Policy{&policy.UserBorealisAllowed{Val: true}, &policy.VirtualMachinesAllowed{Val: false}},
		},
		// These cases are allowed.  Since we don't have a good way to uninstall in tast, success cases should be after failures.
		{
			name:               "User Allowed",
			shouldAllowInstall: true,
			policies:           []policy.Policy{&policy.UserBorealisAllowed{Val: true}},
		},
	} {
