...
func Basic(ctx context.Context, s *testing.State) {
	...
	// Ensure the UI updates to reflect the tethered network's status.
	...
	// when no networks are available, disconnect it here too so that notification will
	...
	// since the adapter still needs to be on to use tethering.
