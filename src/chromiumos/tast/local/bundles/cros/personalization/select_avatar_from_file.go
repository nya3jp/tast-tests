...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Test setting avatar from local files",
		Contacts: []string{
			"pzliu@google.com",
...
func selectFromGoogleDrive(ctx context.Context, ui *uiauto.Context) error {
	...
	if err := uiauto.Combine("choose a file from Google Drive",
		ui.WaitUntilExists(chooseFromFileButton),
		ui.LeftClick(chooseFromFileButton),
		ui.WaitUntilExists(googleDriveText),
		ui.LeftClick(googleDriveText),
		ui.WaitUntilExists(newAvatarIcon),
		ui.LeftClick(newAvatarIcon),
...
func selectProfileImage(ctx context.Context, ui *uiauto.Context) error {
	selectedAvatarOption := nodewith.Role(role.ListBoxOption).Name(googleProfilePhoto).HasClass("image-container")

	if err := uiauto.Combine("change avatar back to Google profile photo",
		ui.WaitUntilExists(selectedAvatarOption),
		ui.LeftClick(selectedAvatarOption),
	)(ctx); err != nil {
		return err
	}
...
func verifyAvatar(ctx context.Context, ui *uiauto.Context, name, className string) error {
	googleProfilePhotoContainer := nodewith.Role(role.ListBoxOption).NameContaining(name).HasClass(className)
	if err := ui.WaitUntilExists(googleProfilePhotoContainer)(ctx); err != nil {
		return err
	}
	return nil
}
...
func SelectAvatarFromFile(ctx context.Context, s *testing.State) {
	...
	// Verify that avatar is changed to the file from Google Drive
	if err := verifyAvatar(ctx, ui, "Existing photo from", "selected-last-external-image"); err != nil {
		s.Fatal("Verification failed: avatar from file is not selected: ", err)
	}
	...
	// Verify that avatar is changed back to Google profile image
	if err := verifyAvatar(ctx, ui, googleProfilePhoto, "selected-profile-image"); err != nil {
		s.Fatal("Verification failed: google profile avatar is not selected: ", err)
	}
}
