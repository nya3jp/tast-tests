...
func selectProfileImage(ctx context.Context, ui *uiauto.Context) error {
	...
	if err := uiauto.Combine("change avatar back to Google profile photo",
		...
		ui.LeftClick(googleProfilePhotoContainer),
	)(ctx); err != nil {
		return err
	}
...
func verifyAvatar(ctx context.Context, ui *uiauto.Context, name, className string) error {
	selectedAvatarOption := nodewith.Role(role.ListBoxOption).NameContaining(name).HasClass(className)
	if err := ui.WaitUntilExists(selectedAvatarOption)(ctx); err != nil {
		return err
	}
	return nil
}
