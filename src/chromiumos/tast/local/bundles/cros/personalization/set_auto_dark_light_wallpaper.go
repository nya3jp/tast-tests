...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{
			{
				Name: "dark",
				Val:  true, // dark mode enalbled
			},
			{
				Name: "light",
				Val:  false, // dark mode disabled
			},
		},
	})
}
...
func SetAutoDarkLightWallpaper(ctx context.Context, s *testing.State) {
	...
	if err := tconn.Call(ctx, nil, `tast.promisify(chrome.autotestPrivate.forceAutoThemeMode)`, darkModeEnabled); err != nil {
		s.Fatalf("Failed to force auto theme mode for testing, dark mode - %v: %v", darkModeEnabled, err)
	}
	...
	if err := uiauto.Combine(fmt.Sprintf("Select wallpaper from %v collection", constants.ElementCollection),
		...
		ui.WaitUntilExists(nodewith.Role(role.Link).NameContaining(constants.ElementCollection).HasClass("breadcrumb")),
	)(ctx); err != nil {
