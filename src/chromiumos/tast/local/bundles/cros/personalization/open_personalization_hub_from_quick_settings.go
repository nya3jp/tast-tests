...
func OpenPersonalizationHubFromQuickSettings(ctx context.Context, s *testing.State) {
	...
	if err := ui.WaitUntilExists(nodewith.Role(role.Window).NameContaining("Personalization").First())(ctx); err != nil {
...
func openPersonalizationHub(ctx context.Context, tconn *chrome.TestConn, ui *uiauto.Context) error {
	...
	// Although Dark theme pod label button is available in Quick Settings, we don't know the exact page it resides.
	// If we click on the Dark theme label button in a wrong page, it takes no effects.
	// So try to click on the Dark theme label button in all the pages.
	for _, page := range pages {
		...
		if err := uiauto.Combine("Open Personalization Hub",
			...
			ui.LeftClick(darkThemePodLabelButton))(ctx); err != nil {
			return errors.Wrap(err, "failed to open Dark theme settings")
		}
	}
	return nil
