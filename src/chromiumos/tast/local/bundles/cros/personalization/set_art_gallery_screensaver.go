// Copyright 2022 The Chromium OS Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

package personalization

import (
	"context"
	"strings"
	"time"

	"chromiumos/tast/ctxutil"
	"chromiumos/tast/errors"
	"chromiumos/tast/local/ambient"
	"chromiumos/tast/local/chrome"
	"chromiumos/tast/local/chrome/uiauto"
	"chromiumos/tast/local/chrome/uiauto/faillog"
	"chromiumos/tast/local/chrome/uiauto/nodewith"
	"chromiumos/tast/local/chrome/uiauto/role"
	"chromiumos/tast/testing"
)

func init() {
	testing.AddTest(&testing.Test{
		Func:         SetArtGalleryScreensaver,
		LacrosStatus: testing.LacrosVariantUnneeded,
		Desc:         "Test setting Google Photos screensaver in the personalization hub app",
		Contacts: []string{
			"thuongphan@google.com",
			"chromeos-sw-engprod@google.com",
			"assistive-eng@google.com",
		},
		Attr:         []string{"group:mainline", "informational"},
		VarDeps:      []string{"ambient.username", "ambient.password"},
		SoftwareDeps: []string{"chrome"},
		Timeout:      chrome.GAIALoginTimeout + time.Minute,
	})
}

func SetArtGalleryScreensaver(ctx context.Context, s *testing.State) {
	clearUpCtx := ctx
	ctx, cancel := ctxutil.Shorten(ctx, 5*time.Second)
	defer cancel()

	cr, tconn, err := ambient.SetUp(
		ctx,
		s.RequiredVar("ambient.username"),
		s.RequiredVar("ambient.password"),
	)
	if err != nil {
		s.Fatal("Failed to complete setup: ", err)
	}

	defer faillog.DumpUITreeOnError(clearUpCtx, s.OutDir(), s.HasError, tconn)

	// The test has a dependency of network speed, so we give uiauto.Context ample
	// time to wait for nodes to load.
	ui := uiauto.New(tconn).WithTimeout(30 * time.Second)

	if err := ambient.EnableAmbientMode(ui)(ctx); err != nil {
		s.Fatal("Failed to enable ambient mode: ", err)
	}

	if err := prepareArtGalleryScreensaver(ctx, tconn, ui); err != nil {
		s.Fatal("Failed to prepare Art Gallery screensaver: ", err)
	}

	if err := ambient.TestLockScreenIdle(ctx, cr, tconn, ui); err != nil {
		s.Fatal("Failed to start ambient mode: ", err)
	}

	if err := ambient.UnlockScreen(ctx, s.RequiredVar("ambient.password")); err != nil {
		s.Fatal("Failed to unlock screen")
	}
}

func prepareArtGalleryScreensaver(ctx context.Context, tconn *chrome.TestConn, ui *uiauto.Context) error {
	artGalleryContainer := nodewith.Role(role.GenericContainer).NameContaining(ambient.ArtGallery)
	artAlbumsFinder := nodewith.Role(role.GenericContainer).HasClass("album")

	if err := uiauto.Combine("Choose Art Gallery topic source",
		ui.FocusAndWait(artGalleryContainer),
		ui.LeftClick(artGalleryContainer),
		ui.WaitUntilExists(artAlbumsFinder.First()))(ctx); err != nil {
		errors.Wrap(err, "failed to select Art Gallery")
	}

	artAlbums, err := ui.NodesInfo(ctx, artAlbumsFinder)
	if err != nil {
		return errors.Wrap(err, "failed to find art gallery albums")
	}
	if len(artAlbums) < 2 {
		return errors.New("At least 2 art gallery albums expected")
	}

	// Turn off all but one art gallery album.
	for i, album := range artAlbums[1:] {
		if !strings.HasSuffix(album.Name, "selected") {
			return errors.Errorf("Art album %d should start selected", i)
		}
		albumNode := nodewith.HasClass("album").Name(album.Name)
		if err := ui.RetryUntil(uiauto.Combine("deselect Art Gallery album",
			ui.Exists(albumNode),
			ui.MouseClickAtLocation(0, album.Location.CenterPoint())),
			ui.Gone(albumNode))(ctx); err != nil {
			errors.Wrapf(err, "failed to deselect Art Gallery album %d", i)
		}
		// if err := ui.MouseClickAtLocation(0, album.Location.CenterPoint())(ctx); err != nil {
		// 	return errors.Wrapf(err, "failed to deselect art album %d", i)
		// }
	}

	if err := ambient.SetTimeouts(
		ctx,
		tconn,
		ambient.Timeouts{
			LockScreenIdle:       1 * time.Second,
			BackgroundLockScreen: 2 * time.Second,
			PhotoRefreshInterval: 1 * time.Second,
		},
	); err != nil {
		return errors.Wrap(err, "failed to configure ambient timeouts")
	}

	return nil
}
