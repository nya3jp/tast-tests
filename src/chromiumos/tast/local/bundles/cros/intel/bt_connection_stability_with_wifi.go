...
func BTConnectionStabilityWithWifi(ctx context.Context, s *testing.State) {
	...
	s.Log("Downloading iso file")
	if err := testexec.CommandContext(ctx, "cp", "-rf", s.DataPath("100,000 Files.iso"), downloadsPath).Run(); err != nil {
		s.Fatal("Failed to copy file to Downloads path: ", err)
	}
