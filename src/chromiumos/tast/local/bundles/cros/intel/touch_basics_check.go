...
func TouchBasicsCheck(ctx context.Context, s *testing.State) {
	...
	for _, node := range nodes {
		...
		if err := tapAndVerify(ctx, stw, scannerTouchscreen, x, y); err != nil {
	...
	coordinates := map[string]coords.Point{
		...
		"Top Centre":    {X: bounds.Left + bounds.Width/2, Y: bounds.Top},
		"Left Centre":   {X: bounds.Left, Y: bounds.Top + bounds.Height/2},
	}
