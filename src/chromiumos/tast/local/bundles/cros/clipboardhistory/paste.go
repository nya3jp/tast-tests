...
func Paste(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine("paste from clipboard history",
		...
		params.testFn(pasteActionParams{menu, ui, kbd}),

		// Make sure that once the menu is closed, the textfield contains text
...
// pasteOnClick pastes from `params.menu` by clicking on it.
func pasteOnClick(params pasteActionParams) uiauto.Action {
	return params.ui.LeftClick(params.menu)
}

// pasteOnEnter pastes from `params.menu` by pressing Enter.
func pasteOnEnter(params pasteActionParams) uiauto.Action {
	return params.kbd.AccelAction("Enter")
}

// pasteOnToggle pastes from `params.menu` by toggling the menu closed.
