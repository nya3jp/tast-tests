...
func pasteAndVerify(ctx context.Context, res *clipboardResource, inputFinder *nodewith.Finder) error {
...
func (b *browserTest) pasteAndVerify(ctx context.Context, res *clipboardResource) error {
	...
	return pasteAndVerify(ctx, res, searchbox)
}
...
func (g *gmailTest) pasteAndVerify(ctx context.Context, res *clipboardResource) error {
	...
	if err := uiauto.IfSuccessThen(res.ui.WithTimeout(10*time.Second).WaitUntilExists(getStartedBtn), clearPrompts)(ctx); err != nil {
...
func (s *settingsTest) pasteAndVerify(ctx context.Context, res *clipboardResource) error {
	return pasteAndVerify(ctx, res, ossettings.SearchBoxFinder)
}
