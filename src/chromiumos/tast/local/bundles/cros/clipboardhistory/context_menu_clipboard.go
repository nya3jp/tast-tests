...
func (g *gmailTest) pasteAndVerify(ctx context.Context, res *clipboardResource) error {
	var title string
	if err := g.conn.Eval(ctx, "document.title", &title); err != nil {
		return errors.Wrap(err, "failed to get the tab title")
	}

	rootView := nodewith.Role(role.Window).NameContaining(title).HasClass("BrowserFrame")
