...
func UserPolicyKeys(ctx context.Context, s *testing.State) {
	...
	keyFile := path.Join("/run/user_policy", userHash, "policy.pub")

	u, err := user.Lookup("chronos")
	if err != nil {
		s.Fatal("Failed to find chronos user: ", err)
	}
	chronosUID, err := strconv.ParseInt(u.Uid, 10, 64)
	if err != nil {
		s.Fatal("Failed to parse UID %q for chronos: %v", u.Uid, err)
	}
	g, err := user.LookupGroup("chronos")
	if err != nil {
		s.Fatal("Failed to find chronos group: ", err)
	}
	chronosGID, err := strconv.ParseInt(g.Gid, 10, 64)
	if err != nil {
		s.Fatal("Failed to parse GID %q for chronos: %v", g.Gid, err)
	}
	...
	verifyKeyFile := func(p string) error {
		...
		if ok, err := readable(fi, uint32(chronosUID), uint32(chronosGID)); err != nil {
		...
		for {
			...
			if ok, err := executable(di, uint32(chronosUID), uint32(chronosGID)); err != nil {
	...
	if err = cryptohome.RemoveVault(ctx, testUser); err != nil {
		s.Fatal("Failed to remove vaule: ", err)
	}
