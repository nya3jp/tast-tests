...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Verifies that Ownership API works, as an enterprise might, to set device policies",
		Contacts: []string{
...
func RemoteOwnership(ctx context.Context, s *testing.State) {
	...
	if retrieved, err := ownership.RetrieveSettings(ctx, sm); err != nil {
		...
		s.Fatal("Unexpected settings is retrieved. Diff is found in ", diffName)
	}
	...
	if retrieved, err := ownership.RetrieveSettings(ctx, sm); err != nil {
		...
		s.Fatal("Unexpected rekeyed settings is retrieved. Diff is found in ", diffName)
	}
	...
	if retrieved, err := ownership.RetrieveSettings(ctx, sm); err != nil {
		...
		s.Fatal("Unexpected user settings is retrieved. Diff is found in ", diffName)
	}
}
