...
func init() {
	testing.AddTest(&testing.Test{
		Func:         Twice,
		Desc:         "Ensures that the session_manager won't start the smae session twice",
		Attr:         []string{"informational"},
		SoftwareDeps: []string{"chrome_login"},
	})
}

func Twice(ctx context.Context, s *testing.State) {
	...
	// Second StartSession() should be failed with SessionExists.
	if err = sm.StartSession(ctx, user, ""); err == nil {
