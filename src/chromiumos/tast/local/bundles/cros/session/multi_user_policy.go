...
func MultiUserPolicy(ctx context.Context, s *testing.State) {
	...
	if ret, err := sm.RetrievePolicyForUser(ctx, user1); err != nil {
		...
		s.Fatalf("Unexpected policy is fetched for %s", user1)
	}
	...
	// Starts the second user's session, and verify that they has no
	// policy stored yet.
	...
	if ret, err := sm.RetrievePolicyForUser(ctx, user2); err != nil {
		...
		s.Fatalf("Unexpected policy is fetched for %s", user2)
	}
