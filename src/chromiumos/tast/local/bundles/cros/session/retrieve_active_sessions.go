...
func RetrieveActiveSessions(ctx context.Context, s *testing.State) {
	...
	// Restart session_manager at the end of the test to reset the state.
	...
	verify := func(as map[string]string, users ...string) {
		if len(as) != len(users) {
			s.Fatalf("Unexpected active sessions. got %v; want keys %v", as, users)
		}
		for _, u := range users {
			if _, ok := as[u]; !ok {
				s.Fatalf("Unexpected active sessions. got %v; want keys %v", as, users)
	...
	if err := cryptohome.CreateVault(ctx, user1, "a"); err != nil {
		s.Fatalf("Failed to create user dir for %s: %v", user1, err)
	}
	if err := cryptohome.CreateVault(ctx, user2, "b"); err != nil {
