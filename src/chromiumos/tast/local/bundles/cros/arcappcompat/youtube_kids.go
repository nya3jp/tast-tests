...
func launchAppForYoutubeKids(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	if err := checkForCorrectNumberPage.WaitForExists(ctx, testutil.DefaultUITimeout); err != nil {
	...
	// Select older ages.
	selectOlderAges := d.Object(ui.ClassName(frameworkLayoutClassName), ui.DescriptionMatches("(?i)"+olderAgesDes))
	if err := selectOlderAges.WaitForExists(ctx, testutil.DefaultUITimeout); err != nil {
		s.Log("selectOlderAges doesn't exist: ", err)
	} else if err := selectOlderAges.Click(ctx); err != nil {
		s.Fatal("Failed to click on selectOlderAges: ", err)
	}
...
func enterAge(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	clickOnOneButton := d.Object(ui.ClassName(textViewClassName), ui.DescriptionMatches("(?i)"+oneButtonDes))
	if err := clickOnOneButton.WaitForExists(ctx, testutil.DefaultUITimeout); err != nil {
		s.Log("clickOnOneButton doesn't exist: ", err)
	} else if err := clickOnOneButton.Click(ctx); err != nil {
		s.Fatal("Failed to click on clickOnOneButton: ", err)
	}

	clickOnNineButton := d.Object(ui.ClassName(textViewClassName), ui.DescriptionMatches("(?i)"+nineButtonDes))
	if err := clickOnNineButton.WaitForExists(ctx, testutil.DefaultUITimeout); err != nil {
		s.Log("clickOnNineButton doesn't exist: ", err)
	} else if err := clickOnNineButton.Click(ctx); err != nil {
		s.Fatal("Failed to click on clickOnNineButton: ", err)
	}
	...
	clickOnZeroButton := d.Object(ui.ClassName(textViewClassName), ui.DescriptionMatches("(?i)"+zeroButtonDes))
	if err := clickOnZeroButton.WaitForExists(ctx, testutil.DefaultUITimeout); err != nil {
		s.Log("clickOnZeroButton doesn't exist: ", err)
	} else if err := clickOnZeroButton.Click(ctx); err != nil {
		s.Fatal("Failed to click on clickOnZeroButton: ", err)
	}

	clickOnConfirmButton := d.Object(ui.ClassName(testutil.AndroidButtonClassName), ui.TextMatches("(?i)"+confirmBtnText))
	if err := clickOnConfirmButton.WaitForExists(ctx, testutil.DefaultUITimeout); err != nil {
		s.Log("clickOnConfirmButton doesn't exist: ", err)
	} else if err := clickOnConfirmButton.Click(ctx); err != nil {
		s.Fatal("Failed to click on clickOnConfirmButton: ", err)
	}
}
