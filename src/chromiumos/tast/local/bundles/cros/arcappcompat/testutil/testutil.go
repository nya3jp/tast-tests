...
func OrientationSize(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	//Todo: Remove ash.TabletModeEnabled(ctx, tconn) function until there is an exact way of finding a device in clamshell/ laptop mode using hardware/software dependencies.
	tabletModeEnabled, err := ash.TabletModeEnabled(ctx, tconn)
	if err != nil {
		s.Fatal("Failed to get tablet mode: ", err)
	}
