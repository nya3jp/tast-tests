...
func SetUpDevice(ctx context.Context, s *testing.State, appPkgName string, appActivity string) (*chrome.Chrome, *chrome.Conn, *arc.ARC, *ui.Device) {
...
func LaunchHelper(ctx context.Context, s *testing.State, a *arc.ARC, d *ui.Device, appPkgName string, appActivity string) {
...
func handleCrashOrANRDialog(ctx context.Context, s *testing.State, d *ui.Device) {
	...
	if err := openAppAgainButton.WaitForExists(ctx, DefaultUITimeout); err != nil {
		...
		if err := openAppAgainButton.Click(ctx); err != nil {
	...
	if err := closeButton.WaitForExists(ctx, DefaultUITimeout); err != nil {
		...
		if err := closeButton.Click(ctx); err != nil {
