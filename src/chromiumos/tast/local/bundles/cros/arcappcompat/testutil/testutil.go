...
func RunTestCases(ctx context.Context, s *testing.State, appPkgName, appActivity string, testCases TestParams) {
	...
	if err := power.TurnOnDisplay(ctx); err != nil {
		s.Log("Failed to ensure the display is on. Skipping the test: ", err)
		return
	}
	...
	if window, err := ash.GetARCAppWindowInfo(ctx, tconn, appPkgName); err != nil {
		s.Log("Failed to get window info. Skipping the test: ", err)
		return
	...
	for idx, test := range AllTests {
		...
		s.Run(testCaseCtx, test.Name, func(cleanupCtx context.Context, s *testing.State) {
			...
			// Launch the app.
			if err := act.Start(ctx, tconn); err != nil {
				s.Log("Failed to start app. Skipping the test: ", err)
				return
			}
			...
			if err != nil {
				s.Log("Failed initializing UI Automator. Skipping the test: ", err)
				return
			}
			defer d.Close(ctx)
			...
			if err != nil {
				s.Log("Failed to get current app package. Skipping the test: ", err)
				return
			}
...
func ClamshellFullscreenApp(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	t, ok := arc.Type()
	if !ok {
		s.Log("Unable to determine arc type. Skipping test")
		return
	}
	// If ARC-P.
	if t == arc.Container {
...
func MinimizeRestoreApp(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	if err != nil {
		s.Log("Failed to get the default window state. Skipping the test: ", err)
		return
	}
...
func ClamshellResizeWindow(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	if err != nil {
		s.Log("Failed to get tablet mode. Skipping the test: ", err)
		return
	}
	...
	t, ok := arc.Type()
	if !ok {
		s.Log("Unable to determine arc type. Skipping the test")
		return
	}
	// If ARC-P.
	if t == arc.Container {
...
func MouseScrollAction(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	if err := checkForScrollLayout.WaitForExists(ctx, DefaultUITimeout); err != nil {
		s.Log("ScrollLayout doesn't exist. Page is not scrollable. Skipping the test: ", err)
		return
	}
...
func OrientationSize(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	if t == arc.VM {
		...
		if err != nil {
			s.Log("Failed to get device display. Skipping the test: ", err)
		}
...
func ReOpenWindow(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	act, err := arc.NewActivity(a, appPkgName, appActivity)
	if err != nil {
		s.Log("Failed to create new app activity. Skipping the test: ", err)
		return
	}
	defer act.Close()
...
func SplitScreen(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	if err != nil {
		s.Log("Failed to get the internal display info. Skipping the test: ", err)
		return
	}
	...
	if err != nil {
		s.Log("Failed to obtain the orientation info. Skipping the test: ", err)
		return
	}
	...
	if err != nil {
		s.Log("Failed to access to the touch screen. Skipping the test: ", err)
		return
	}
	defer tew.Close()
	stw, err := tew.NewSingleTouchWriter()
	if err != nil {
		s.Log("Failed to create a single touch writer. Skipping the test: ", err)
		return
	}
	defer stw.Close()
