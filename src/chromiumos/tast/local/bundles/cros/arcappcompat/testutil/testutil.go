...
func ClamshellResizeWindow(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	if t == arc.VM {
		...
		button, err := chromeui.FindWithTimeout(ctx, tconn, chromeui.FindParams{ClassName: resizelockhelper.CenterButtonClassName}, 10*time.Second)
		if err != nil {
		...
		button, err = chromeui.FindWithTimeout(ctx, tconn, chromeui.FindParams{ClassName: resizelockhelper.CenterButtonClassName}, 10*time.Second)
		if err == nil {
			if button.Name == resizelockhelper.PhoneButtonName || button.Name == resizelockhelper.TabletButtonName {
...
func checkCompatModeButton(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) error {
	...
	} else if button.Name == resizelockhelper.PhoneButtonName || button.Name == resizelockhelper.TabletButtonName || button.Name == resizelockhelper.ResizableButtonName {
...
func selectResizeLockMode(ctx context.Context, tconn *chrome.TestConn, appPkgName string) error {
	...
	resizeLockModeButton, err := compatModeMenuDialog.DescendantWithTimeout(ctx, chromeui.FindParams{Name: resizelockhelper.ResizableButtonName}, 10*time.Second)
	if err != nil {
		return errors.Wrapf(err, "failed to find the %s button on the compat mode menu", resizelockhelper.ResizableButtonName)
	}
