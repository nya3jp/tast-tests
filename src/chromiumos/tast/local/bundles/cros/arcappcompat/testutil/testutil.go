...
// SplitScreen tests if app supports split screen and check if app perform split screen without crash or ANR.
func SplitScreen(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	s.Logf("App Resize info: %+v", windowInfo.CanResize)
	...
	// so that window snap on the left and right for landscape orientation
	// and Windows snap on the top and bottom for portrait orientation.
	...
	if orientation.Type == display.OrientationPortraitPrimary {
		...
		s.Logf("Get rotation of the app:%v", info.Rotation)
		s.Logf("Get orientation.Type of the app:%v", orientation.Type)
		s.Logf("Get orientation.Angle of the app:%v", orientation.Angle)
	...
	defer stw.Close()
	// Snap activities to left.
	s.Log("dragToSnapFirstOverviewWindow and snap left")
	...
	s.Log("wait until window state change to snap left")
	if err := ash.WaitForARCAppWindowState(ctx, tconn, appPkgName, ash.WindowStateLeftSnapped); err != nil {
		...
		s.Log("App performed split screen and snapped to left")
	}
	DetectAndHandleCloseCrashOrAppNotResponding(ctx, s, d)
}
