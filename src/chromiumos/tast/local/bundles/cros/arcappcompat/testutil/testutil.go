...
func getAppWindowInfo(ctx context.Context, s *testing.State, a *arc.ARC, d *ui.Device, appPkgName string) (string, error) {
	var (
		windowInfo                 string
		flagForWindowInfoCheckARCP bool
		flagForWindowInfoCheckARCR bool
	)
	...
	for splitLine := range splitOutput {
		if strings.Contains(splitOutput[splitLine], appPkgName) && strings.Contains(splitOutput[splitLine], TaskRecordPrefix) {
			...
			for appWindowInfo := range splitWindowInfo {
				if !flagForWindowInfoCheckARCP && strings.Contains(splitWindowInfo[appWindowInfo], windowStatePrefixForARCP) {
					...
					flagForWindowInfoCheckARCP = true
					break
				} else if !flagForWindowInfoCheckARCR && strings.Contains(splitWindowInfo[appWindowInfo], windowStatePrefixForARCR) {
					...
					flagForWindowInfoCheckARCR = true
					break
				}
			}
		}
