...
func RunTestCases(ctx context.Context, s *testing.State, appPkgName, appActivity string, testCases []TestCase) {
	...
	if err := ash.WaitForHidden(ctx, tconn, appPkgName); err != nil {
		s.Fatal("Failed to wait for the app to close before test cases: ", err)
	}
	...
	for idx, test := range testCases {
		...
		s.Run(ctx, test.Name, func(cleanupCtx context.Context, s *testing.State) {
			...
			defer func(ctx context.Context) {
				...
				if err := ash.WaitForHidden(ctx, tconn, appPkgName); err != nil {
					s.Fatal("Failed to wait for the app to close: ", err)
				}
			}(cleanupCtx)
