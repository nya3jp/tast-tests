...
func launchAppForFlipboard(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	const (
		...
		noneOFTheAboveButtonText = "None Of The Above|NONE OF THE ABOVE"
...
func signOutOfFlipboard(ctx context.Context, s *testing.State, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	const (
		...
		selectSiginOutText = "Sign Out|SIGN OUT"
	)
	...
	clickOnSelectSignOut := d.Object(ui.ID(selectSignOutID), ui.TextMatches("(?i)"+selectSiginOutText))
	...
	logOutOfFlipboard := d.Object(ui.ClassName(testutil.AndroidButtonClassName), ui.TextMatches("(?i)"+selectSiginOutText))
