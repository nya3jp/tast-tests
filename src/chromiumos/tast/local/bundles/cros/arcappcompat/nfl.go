...
// clamshellLaunchForNFL launches NFL in clamshell mode.
var clamshellLaunchForNFL = []testutil.TestCase{
	{Name: "Launch app in Clamshell", Fn: launchAppForNFL},
}

// touchviewLaunchForNFL launches NFL in tablet mode.
var touchviewLaunchForNFL = []testutil.TestCase{
	{Name: "Launch app in Touchview", Fn: launchAppForNFL},
}

func init() {
	testing.AddTest(&testing.Test{
		Func: NFL, LacrosStatus: testing.LacrosVariantUnknown, Desc: "Functional test for NFL that installs the app also verifies it is logged in and that the main page is open, checks NFL correctly changes the window state in both clamshell and touchview mode",
		...
		Params: []testing.Param{{
			...
			Val: testutil.TestParams{
				LaunchTests: clamshellLaunchForNFL,
			...
			Name: "tablet_mode",
			Val: testutil.TestParams{
				LaunchTests: touchviewLaunchForNFL,
