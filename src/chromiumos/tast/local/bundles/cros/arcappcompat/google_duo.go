...
var clamshellTestsForGoogleDuo = []testutil.TestSuite{
	{"Launch app in Clamshell", launchAppForGoogleDuo},
...
var touchviewTestsForGoogleDuo = []testutil.TestSuite{
	{"Launch app in Touchview", launchAppForGoogleDuo},
...
func init() {
	testing.AddTest(&testing.Test{
		Func:         GoogleDuo,
		Desc:         "Functional test for GoogleDuo that installs the app also verifies it is logged in and that the main page is open, checks Gmail correctly changes the window state in both clamshell and touchview mode",
...
func launchAppForGoogleDuo(ctx context.Context, s *testing.State, tconn *chrome.Conn, a *arc.ARC, d *ui.Device, appPkgName string, appActivity string) {
	...
	if err := giveAccessButton.WaitForExists(ctx, defaultUITimeout); err != nil {
		...
		testutil.FindTheError(s, giveAccessButton.Click(ctx))
	}
	...
	if err := allowButton.WaitForExists(ctx, defaultUITimeout); err != nil {
		...
		testutil.FindTheError(s, allowButton.Click(ctx))
	}
	...
	testutil.FindTheError(s, videoMessageButton.WaitForExists(ctx, testutil.LongUITimeout))
}
