...
var clamshellTestsForBandlab = []testutil.TestCase{
...
var touchviewTestsForBandlab = []testutil.TestCase{
...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			Val:               clamshellTestsForBandlab,
			ExtraSoftwareDeps: []string{"android_p"},
			Pre:               pre.AppCompatBooted,
		}, {
			Name:              "tablet_mode",
			Val:               touchviewTestsForBandlab,
...
func Bandlab(ctx context.Context, s *testing.State) {
	const (
		...
		appActivity = "com.bandlab.bandlab.core.activity.navigation.NavigationActivity"
	)
...
func launchAppForBandlab(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	if err := logIn_Btn.WaitForExists(ctx, testutil.DefaultUITimeout); err != nil {
		s.Error("LogIn button doesn't exist: ", err)
	} else if err := logIn_Btn.Click(ctx); err != nil {
