...
func launchAppForKhanacademyKids(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	// It will take few minutes for ok button to appear on the screen. So sleep for few minutes is needed.
	testing.Sleep(ctx, testutil.LongUITimeout)
	...
	if err := uiauto.Combine("Check for sign in button",
		...
		ud.WithScreenshotStrategy(uidetection.ImmediateScreenshot).LeftClick(uidetection.TextBlock([]string{"Sign", "in", "with", "Khan", "Kids", "account"})),
	)(ctx); err != nil {
