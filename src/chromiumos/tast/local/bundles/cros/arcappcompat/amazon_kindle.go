...
func init() {
	testing.AddTest(&testing.Test{
		Func:         AmazonKindle,
		Desc:         "Functional test for AmazonKindle that installs the app also verifies it reaches home page, also checks AmazonKindle correctly changes the window state in both clamshell and touchview mode",
...
// launchAppForAmazonKindle verifies reached main activity page of the app.
func launchAppForAmazonKindle(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	const (
		homePageID = "android:id/content"
	)

	// Check for home icon.
	homeIcon := d.Object(ui.ID(homePageID), ui.PackageName(appPkgName))
