...
var clamshellTestsForAmazonKindle = []testutil.TestCase{
	...
	{Name: "Clamshell: Signout app", Fn: signoutOfAmazonKindle},
}
...
var touchviewTestsForAmazonKindle = []testutil.TestCase{
	...
	{Name: "Touchview: Signout app", Fn: signoutOfAmazonKindle},
}
...
func launchAppForAmazonKindle(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	// Keep clicking emailid text field until the emailid text field is focused.
	...
	// Keep clicking signIn Button until not now button exist.
	...
	// Check for important message.
	checkForImportantMessage := d.Object(ui.TextStartsWith(importantMessageText))
	if err := checkForImportantMessage.WaitForExists(ctx, testutil.DefaultUITimeout); err != nil {
		s.Log("checkForImportantMessage doesn't exists: ", err)
	} else {
		s.Error("Failed to signed into the app")
	}
}
...
func signoutOfAmazonKindle(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	const (
	...
	signInWithAmazonButton := d.Object(ui.ClassName(textViewClassName), ui.Text(signInAmazonButtonText))
	if err := signInWithAmazonButton.WaitForExists(ctx, testutil.DefaultUITimeout); err != nil {
		s.Fatal("signInWithAmazonButton doesn't exists: ", err)
	} else {
