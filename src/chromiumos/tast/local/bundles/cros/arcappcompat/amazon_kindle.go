...
var clamshellTestsForAmazonKindle = []testutil.TestCase{
	...
	{Name: "Clamshell: Reopen app", Fn: reOpenWindowForAmazonKindleAndSignout},
}
...
var touchviewTestsForAmazonKindle = []testutil.TestCase{
	...
	{Name: "Touchview: Reopen app", Fn: reOpenWindowForAmazonKindleAndSignout},
}
...
func launchAppForAmazonKindle(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		if emailIDFocused, err := enterEmailAddress.IsFocused(ctx); err != nil {
			s.Log("Emailid text field is not focused: ", err)
	...
	myStuffIcon := d.Object(ui.ClassName(homeClassName), ui.DescriptionContains(myStuffDes))
...
// reOpenWindowForAmazonKindleAndSignout Test "close and relaunch the app" and signout from the app.
func reOpenWindowForAmazonKindleAndSignout(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
