...
var clamshellLaunchForShowtime = []testutil.TestCase{
	{Name: "Launch app in Clamshell", Fn: launchAppForShowtime},
}

// touchviewLaunchForShowtime launches Showtime in tablet mode.
var touchviewLaunchForShowtime = []testutil.TestCase{
	{Name: "Launch app in Touchview", Fn: launchAppForShowtime},
}

// clamshellAppSpecificTestsForShowtime are placed here.
var clamshellAppSpecificTestsForShowtime = []testutil.TestCase{
	{Name: "Clamshell: Video Playback", Fn: testutil.TouchAndPlayVideo},
}

// touchviewAppSpecificTestsForShowtime are placed here.
var touchviewAppSpecificTestsForShowtime = []testutil.TestCase{
	{Name: "Touchview: Video Playback", Fn: testutil.TouchAndPlayVideo},
}

func init() {
	testing.AddTest(&testing.Test{
		Func: Showtime, LacrosStatus: testing.LacrosVariantUnknown, Desc: "Functional test for Showtime that installs the app also verifies it is logged in and that the main page is open, checks Showtime correctly changes the window state in both clamshell and touchview mode",
		...
		Params: []testing.Param{{
			...
			Name: "tablet_mode",
			Val: testutil.TestParams{
				LaunchTests:      touchviewLaunchForShowtime,
				CommonTests:      testutil.TouchviewCommonTests,
				AppSpecificTests: touchviewAppSpecificTestsForShowtime,
			},
			ExtraSoftwareDeps: []string{"android_p"},
			// TODO(b/189704585): Remove hwdep.SkipOnModel once the solution is found.
