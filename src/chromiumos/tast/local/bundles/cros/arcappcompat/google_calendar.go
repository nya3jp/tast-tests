...
func init() {
	testing.AddTest(&testing.Test{
		Func:         GoogleCalendar,
		Desc:         "Tast Test for Google Calendar app to launch the app from playstore,verify app is logged in and reached main activity page of the app, also it uses library for opting into the playstore and installing app.",
...
// This test uses library for opting into the playstore and installing app
...
func GoogleCalendar(ctx context.Context, s *testing.State) {
	...
	// Setup developer options for test.
	...
	// Launch App.
	s.Log("Launching app")
	LaunchGoogleCalendar(ctx, s, a, d, appPkgName)

	// Verify user login and homescreen.
	s.Log("Verify user login and homescreen")
	VerifyLoginAndHomeScreen(ctx, s, a, d, appPkgName)
}

// Launch Google Calendar app.
func LaunchGoogleCalendar(ctx context.Context, s *testing.State, a *arc.ARC, d *ui.Device, pkgName string) {
	const (
	...
	openButton.WaitForExists(ctx, longUITimeout)

	if err := openButton.Exists(ctx); err != nil {
		s.Fatal("Open button doesn't exists: ", err)
	} else {
...
func VerifyLoginAndHomeScreen(ctx context.Context, s *testing.State, a *arc.ARC, d *ui.Device, pkgName string) {
	const (
	...
	if err := allowButton.Exists(ctx); err != nil {
		s.Log("Allow Button doesn't esists:", err)
	} else {
