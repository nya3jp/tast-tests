...
var clamshellLaunchForESPN = []testutil.TestCase{
	{Name: "Launch app in Clamshell", Fn: launchAppForESPN},
}

// touchviewLaunchForESPN launches ESPN in tablet mode.
var touchviewLaunchForESPN = []testutil.TestCase{
	{Name: "Launch app in Touchview", Fn: launchAppForESPN},
}

func init() {
	testing.AddTest(&testing.Test{
		Func: ESPN, LacrosStatus: testing.LacrosVariantUnknown, Desc: "Functional test for ESPN that installs the app also verifies it is logged in and that the main page is open, checks ESPN correctly changes the window state in both clamshell and touchview mode",
		...
		Params: []testing.Param{{
			...
			Val: testutil.TestParams{
				LaunchTests: clamshellLaunchForESPN,
			...
			Name: "tablet_mode",
			Val: testutil.TestParams{
				LaunchTests: touchviewLaunchForESPN,
