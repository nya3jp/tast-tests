...
var clamshellTestsForPbsKidsVideo = []testutil.TestCase{
...
var touchviewTestsForPbsKidsVideo = []testutil.TestCase{
...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			Val:               clamshellTestsForPbsKidsVideo,
			ExtraSoftwareDeps: []string{"android_p"},
			Pre:               pre.AppCompatBooted,
		}, {
			Name:              "tablet_mode",
			Val:               touchviewTestsForPbsKidsVideo,
...
func launchAppForPbsKidsVideo(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	const homeID = "org.pbskids.video:id/live_tv_container"
