...
func launchAppForTiktok(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	const (
		...
		homeID                         = "com.zhiliaoapp.musically:id/bnh"
	...
	loginWithGoogleButton := d.Object(ui.ClassName(loginWithGoogleButtonClassName), ui.Index(loginWithGoogleButtonIndex))
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		s.Fatal("emailAddress doesn't exist: ", err)
	}
