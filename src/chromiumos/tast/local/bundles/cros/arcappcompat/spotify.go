...
var clamshellTestsForSpotify = []testutil.TestSuite{
...
var touchviewTestsForSpotify = []testutil.TestSuite{
	...
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			Val: testutil.TestParams{
				TabletMode: false,
				Tests:      clamshellTestsForSpotify,
			},
			...
			Val: testutil.TestParams{
				TabletMode: true,
				Tests:      touchviewTestsForSpotify,
			},
		...
		Vars:    []string{"arcappcompat.username", "arcappcompat.password", "arcappcompat.Spotify.emailid", "arcappcompat.Spotify.password"},
	})
}
...
func launchAppForSpotify(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	s.Log("App is launched successfully in launchAppForSpotify")
	...
	// Enter email address.
	spotifyEmailID := s.RequiredVar("arcappcompat.Spotify.emailid")
