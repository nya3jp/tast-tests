...
func launchAppForAdobeIllustratorDraw(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	s.Log("App is launched successfully in launchAppForAdobeIllustratorDraw")
	...
	selectGmailAccount := d.Object(ui.ID(selectGmailAccountID), ui.Index(indexNum))
	if err := selectGmailAccount.WaitForExists(ctx, testutil.DefaultUITimeout); err != nil {
		s.Log("selectGmailAccount doesn't exists: ", err)
	} else if err := selectGmailAccount.Click(ctx); err != nil {
		s.Fatal("Failed to click on selectGmailAccount: ", err)
	}
