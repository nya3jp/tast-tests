...
var clamshellLaunchForArtrage = []testutil.TestCase{
	{Name: "Launch app in Clamshell", Fn: launchAppForArtrage},
}

// touchviewLaunchForArtrage launches Artrage in tablet mode.
var touchviewLaunchForArtrage = []testutil.TestCase{
	{Name: "Launch app in Touchview", Fn: launchAppForArtrage},
}

func init() {
	testing.AddTest(&testing.Test{
		Func: Artrage, LacrosStatus: testing.LacrosVariantUnknown, Desc: "Functional test for Artrage that installs the app also verifies it is logged in and that the main page is open, checks Artrage correctly changes the window state in both clamshell and touchview mode",
		...
		Params: []testing.Param{{
			...
			Val: testutil.TestParams{
				LaunchTests: clamshellLaunchForArtrage,
