...
import (
	...
	"chromiumos/tast/local/bundles/cros/arcappcompat/utils"
...
// clamshellTestFunc represents the "test" function.
type testFunc func(ctx context.Context, s *testing.State, tconn *chrome.Conn, a *arc.ARC, d *ui.Device, appPkgName string, appActivity string) error
...
var clamshellTestsForGmail = []testSuite{
	...
	{"Clamshell: Fullscreen app", utils.ClamshellFullscreenApp},
	{"Clamshell: Minimise and Restore", utils.MinimizeRestoreApp},
	{"Clamshell: Resize window", utils.ClamshellResizeWindow},
	{"Clamshell: Reopen app", utils.ReOpenWindow},
}

// TouchviewTests are placed here.
var touchviewTestsForGmail = []testSuite{
	{"Launch app in Touchview", launchAppForGmail},
	{"Touchview: Minimise and Restore", utils.MinimizeRestoreApp},
	{"Touchview: Reopen app", utils.ReOpenWindow},
}
...
func Gmail(ctx context.Context, s *testing.State) {
	...
	if err := ash.SetTabletModeEnabled(ctx, tconn, testSet.tabletMode); err != nil {
		s.Fatalf("Failed to set tablet mode enabled to %t: %v", testSet.tabletMode, err)
	}
	// Run the different test cases.
	for idx, test := range testSet.tests {
		s.Logf("Running test %q", test.name)

		if err := test.fn(ctx, s, tconn, a, d, appPkgName, appActivity); err != nil {
...
func launchAppForGmail(ctx context.Context, s *testing.State, tconn *chrome.Conn, a *arc.ARC, d *ui.Device, appPkgName string, appActivity string) error {
