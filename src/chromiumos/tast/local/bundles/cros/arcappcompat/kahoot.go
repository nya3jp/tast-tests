...
var clamshellLaunchForKahoot = []testutil.TestCase{
	{Name: "Launch app in Clamshell", Fn: launchAppForKahoot},
}

// touchviewLaunchForKahoot launches Kahoot in tablet mode.
var touchviewLaunchForKahoot = []testutil.TestCase{
	{Name: "Launch app in Touchview", Fn: launchAppForKahoot},
}

// clamshellAppSpecificTestsForKahoot are placed here.
var clamshellAppSpecificTestsForKahoot = []testutil.TestCase{
	{Name: "Clamshell: Signout app", Fn: signOutOfKahoot},
}

// touchviewAppSpecificTestsForKahoot are placed here.
var touchviewAppSpecificTestsForKahoot = []testutil.TestCase{
	{Name: "Touchview: Signout app", Fn: signOutOfKahoot},
}

func init() {
	testing.AddTest(&testing.Test{
		Func: Kahoot, LacrosStatus: testing.LacrosVariantUnknown, Desc: "Functional test for Kahoot that installs the app also verifies it is logged in and that the main page is open, checks Kahoot correctly changes the window state in both clamshell and touchview mode",
		...
		Params: []testing.Param{{
			...
			Val: testutil.TestParams{
				LaunchTests:      clamshellLaunchForKahoot,
				CommonTests:      testutil.ClamshellCommonTests,
				AppSpecificTests: clamshellAppSpecificTestsForKahoot,
			},
			...
			Name: "tablet_mode",
			Val: testutil.TestParams{
				LaunchTests:      touchviewLaunchForKahoot,
				CommonTests:      testutil.TouchviewCommonTests,
				AppSpecificTests: touchviewAppSpecificTestsForKahoot,
			},
			ExtraSoftwareDeps: []string{"android_p"},
			// TODO(b/189704585): Remove hwdep.SkipOnModel once the solution is found.
