...
func signOutOfMinecraft(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	if err := uiauto.Combine("Find switch accounts",
		...
		action.IfSuccessThen(
			ud.WithTimeout(time.Minute).WithScreenshotStrategy(uidetection.ImmediateScreenshot).WaitUntilExists(uidetection.TextBlock([]string{"Sign", "Out"})),
			uiauto.Combine("Click on Sign out of an app",
				ud.WithScreenshotStrategy(uidetection.ImmediateScreenshot).Tap(uidetection.TextBlock([]string{"Sign", "Out"})),
				action.Sleep(waitForActiveInputTime),
			),
		),
	)(ctx); err != nil {
