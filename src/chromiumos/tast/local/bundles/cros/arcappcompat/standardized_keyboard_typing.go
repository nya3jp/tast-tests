...
func runKeyboardTypingTests(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string, activity *arc.Activity) {
	...
	textKeyboardInputID := appPkgName + ":id/textKeyboardInput"
	if err := d.Object(ui.ID(textKeyboardInputID)).Exists(ctx); err != nil {
	...
	if err := d.Object(ui.ID(textKeyboardInputID), ui.Text(textForTest)).WaitForExists(ctx, testutil.ShortUITimeout); err != nil {
