...
func init() {
	testing.AddTest(&testing.Test{
		...
		//Data:         []string{"toontastic-plus-icon.png"},
...
func launchAppForToontastic(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	/* // Check for plus icon in the home page.
	plusIcon := uidetection.CustomIcon("toontastic-plus-icon.png")
	ud := uidetection.NewDefault(tconn).WithTimeout(time.Minute).WithScreenshotStrategy(uidetection.ImmediateScreenshot)
	if err := uiauto.Combine("Check for plus icon",
		ud.WaitUntilExists(plusIcon),
		action.Sleep(waitForActiveInputTime),
	)(ctx); err != nil {
		s.Fatal("Failed to find plus icon: ", err)
	} */
