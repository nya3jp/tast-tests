...
var clamshellLaunchForHouseparty = []testutil.TestCase{
	{Name: "Launch app in Clamshell", Fn: launchAppForHouseparty},
}

// touchviewLaunchForHouseparty launches Houseparty in tablet mode.
var touchviewLaunchForHouseparty = []testutil.TestCase{
	{Name: "Launch app in Touchview", Fn: launchAppForHouseparty},
}

func init() {
	testing.AddTest(&testing.Test{
		Func: Houseparty, LacrosStatus: testing.LacrosVariantUnknown, Desc: "Functional test for Houseparty that installs the app also verifies it is logged in and that the main page is open, checks Houseparty correctly changes the window state in both clamshell and touchview mode",
