...
var clamshellLaunchForCanva = []testutil.TestCase{
	{Name: "Launch app in Clamshell", Fn: launchAppForCanva},
}

// touchviewLaunchForCanva launches Canva in tablet mode.
var touchviewLaunchForCanva = []testutil.TestCase{
	{Name: "Launch app in Touchview", Fn: launchAppForCanva},
}

func init() {
	testing.AddTest(&testing.Test{
		Func: Canva, LacrosStatus: testing.LacrosVariantUnknown, Desc: "Functional test for Canva that installs the app also verifies it is logged in and that the main page is open, checks Canva correctly changes the window state in both clamshell and touchview mode",
		...
		Params: []testing.Param{{
			...
			Val: testutil.TestParams{
				LaunchTests: clamshellLaunchForCanva,
			...
			Name: "tablet_mode",
			Val: testutil.TestParams{
				LaunchTests: touchviewLaunchForCanva,
