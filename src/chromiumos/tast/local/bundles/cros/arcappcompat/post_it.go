...
var clamshellLaunchForPostIt = []testutil.TestCase{
	{Name: "Launch app in Clamshell", Fn: launchAppForPostIt},
}

// touchviewLaunchForPostIt launches PostIt in tablet mode.
var touchviewLaunchForPostIt = []testutil.TestCase{
	{Name: "Launch app in Touchview", Fn: launchAppForPostIt},
}

func init() {
	testing.AddTest(&testing.Test{
		Func: PostIt, LacrosStatus: testing.LacrosVariantUnknown, Desc: "Functional test for PostIt that installs the app also verifies it is logged in and that the main page is open, checks PostIt correctly changes the window state in both clamshell and touchview mode",
