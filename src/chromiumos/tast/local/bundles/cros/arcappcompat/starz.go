...
func init() {
	testing.AddTest(&testing.Test{
		...
		Vars:    []string{"arcappcompat.username", "arcappcompat.password", "arcappcompat.Starz.emailid", "arcappcompat.Starz.password"},
	})
}
...
func launchAppForStarz(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	// Click on enter email address.enterEmailAddress.IsFocused(ctx)
	...
	if err := dimissButton.WaitForExists(ctx, testutil.LongUITimeout); err != nil {
...
func signOutOfStarz(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, appPkgName, appActivity string) {
	...
	if err := logOutOfStarz.WaitForExists(ctx, testutil.DefaultUITimeout); err != nil {
