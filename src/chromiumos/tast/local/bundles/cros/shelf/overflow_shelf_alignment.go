...
func OverflowShelfAlignment(ctx context.Context, s *testing.State) {
	...
	if isUnderRTL && dispBounds.Right()-shelfInfo.LeftArrowBounds.Right() > dispHalfWidth {
		s.Fatalf("Failed to verify under RTL that the left arrow button is closer to the display right side than to the display "+
			"left side: the actual left arrow bounds %v; the actual display bounds %v", shelfInfo.LeftArrowBounds, dispBounds)
	}

	if !shelfInfo.RightArrowBounds.Size().Empty() {
		s.Fatal("Failed to verify the visibility of the right arrow button after pinning the Settings app to the bottom shelf: got true; expected false")
	}
