...
func OpenCloseSwitchApps(ctx context.Context, s *testing.State) {
	...
	if err := ui.WithPollOpts(opts).WaitUntilExists(chromeBtn)(ctx); err != nil {
		s.Fatal("Failed to find Chrome shelf button: ", err)
	}
	if err := ui.WithPollOpts(opts).WaitUntilExists(filesBtn)(ctx); err != nil {
	...
	for _, app := range checkApps {
		...
		if err := ui.WithTimeout(10 * time.Second).WaitUntilExists(closeBtn)(ctx); err != nil {
		...
		if err := ui.WithPollOpts(opts).LeftClickUntil(closeBtn, ui.Gone(closeBtn))(ctx); err != nil {
