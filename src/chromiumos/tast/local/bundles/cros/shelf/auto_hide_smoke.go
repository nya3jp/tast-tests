...
func AutoHideSmoke(ctx context.Context, s *testing.State) {
	...
	// Begin test in clamshell mode.
	{
		...
		mouse.Move(-2000, -2000)
		if err := ash.WaitForHotseatToEnterCorrectAutoHideState(ctx, tconn, true); err != nil {
		...
		mouse.Move(2000, 2000)
		if err := ash.WaitForHotseatToEnterCorrectAutoHideState(ctx, tconn, false); err != nil {
