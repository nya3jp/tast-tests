...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Tests changing the shelf alignment",
		Contacts: []string{
...
func VerifyShelfAlignment(ctx context.Context, s *testing.State) {
	...
	if err := ui.WaitForLocation(shelfInstance)(ctx); err != nil {
		s.Fatal("Failed to wait for the shelf to be idle: ", err)
	}
	appIcon := nodewith.ClassName(ash.ShelfIconClassName).Role(role.Button).Nth(0)
	if err := ui.WaitForLocation(appIcon)(ctx); err != nil {
		s.Fatal("Failed to wait for the first shelf app icon to be idle: ", err)
	}
	...
	if homeButtonBounds.Left-dispBounds.Left > gapUpperBound {
		s.Fatalf("Expected the distance between homeButtonBounds.Left and dispBounds.Left is not greater than %q; the actual gap is %q", gapUpperBound, homeButtonBounds.Left-dispBounds.Left)
	}
	...
	s.Logf("tray container bounds (%d, %d, %d, %d)", timeViewBounds.Left, timeViewBounds.Top, timeViewBounds.Width, timeViewBounds.Height)
	if dispBounds.Right()-timeViewBounds.Right() > gapUpperBound {
		s.Fatalf("Expected the distance between dispBounds.Right() and timeViewBounds.Right() is not greater than %q; the actual gap is %q", gapUpperBound, dispBounds.Right()-timeViewBounds.Right())
	}

	if err := ash.VerifyShelfAppAlignment(ctx, tconn, ash.ShelfAlignmentBottom); err != nil {
		s.Fatal("Failed to verify shelf app icon alignment: ", err)
	}

	// Pin an extra app then verify the shelf app icon alignment again.
	if err := ash.PinApps(ctx, tconn, []string{apps.Files.ID}); err != nil {
		s.Fatal("Failed to pin Files to shelf")
	}
	if err := ash.VerifyShelfAppAlignment(ctx, tconn, ash.ShelfAlignmentBottom); err != nil {
		s.Fatal("Failed to verify shelf app icon alignment after pinning the Files app: ", err)
	}

	// Unpin the Files app.
	if err := ash.UnpinApps(ctx, tconn, []string{apps.Files.ID}); err != nil {
		s.Fatal("Failed to unpin Files")
	}

	// Set the shelf with left alignment then wait until UI becomes idle.
	...
	if homeButtonBounds.Top-dispBounds.Top > gapUpperBound {
		s.Fatalf("Expected the distance between homeButtonBounds.Top and dispBounds.Top is not greater than %q when shelf is at the left; the actual gap is %q", gapUpperBound, homeButtonBounds.Top-dispBounds.Top)
	}

	timeViewBounds, err = ui.Location(ctx, timeView)
	if dispBounds.Bottom()-timeViewBounds.Bottom() > gapUpperBound {
		s.Fatalf("Expected the distance between dispBounds.Bottom() and timeViewBounds.Bottom() is not greater than %q when shelf is at the left; the actual gap is %q", gapUpperBound, dispBounds.Bottom()-timeViewBounds.Bottom())
	}

	if err := ash.VerifyShelfAppAlignment(ctx, tconn, ash.ShelfAlignmentLeft); err != nil {
		s.Fatal("Failed to verify shelf app icon alignment when shelf is at the left: ", err)
	}

	// Pin the Files app then verify the shelf app icon alignment again.
	if err := ash.PinApps(ctx, tconn, []string{apps.Files.ID}); err != nil {
		s.Fatal("Failed to pin Files to shelf when shelf is at the left")
	}
	if err := ash.VerifyShelfAppAlignment(ctx, tconn, ash.ShelfAlignmentLeft); err != nil {
		s.Fatal("Failed to verify shelf app icon alignment after pinning the Files app when shelf is at the left: ", err)
	}
	...
	if err := ui.WaitForLocation(shelfInstance)(ctx); err != nil {
		s.Fatalf("Failed to wait for the shelf to be idle after the alignment becomes %v: %v", ash.ShelfAlignmentRight, err)
	}
	if err := ui.WaitForLocation(appIcon)(ctx); err != nil {
		s.Fatalf("Failed to wait for the first shelf app icon to be idle after the alignment becomes %v: %v: ", ash.ShelfAlignmentRight, err)
	}

	homeButtonBounds, err = ui.Location(ctx, homeButton)
	if homeButtonBounds.Top-dispBounds.Top > gapUpperBound {
		s.Fatalf("Expected the distance between homeButtonBounds.Top and dispBounds.Top is not greater than %q; the actual gap is %q", gapUpperBound, homeButtonBounds.Top-dispBounds.Top)
	}

	timeViewBounds, err = ui.Location(ctx, timeView)
	if dispBounds.Bottom()-timeViewBounds.Bottom() > gapUpperBound {
		s.Fatalf("Expected the distance between dispBounds.Bottom() and timeViewBounds.Bottom() is not greater than %q; the actual gap is %q", gapUpperBound, dispBounds.Bottom()-timeViewBounds.Bottom())
	}

	if err := ash.VerifyShelfAppAlignment(ctx, tconn, ash.ShelfAlignmentRight); err != nil {
		s.Fatal("Failed to verify shelf app icon alignment when shelf is at the right: ", err)
	}

	// Pin the Files app then verify the shelf app icon alignment again.
	if err := ash.PinApps(ctx, tconn, []string{apps.Files.ID}); err != nil {
		s.Fatal("Failed to pin Files to shelf when shelf is at the right: ", err)
	}
	if err := ash.VerifyShelfAppAlignment(ctx, tconn, ash.ShelfAlignmentRight); err != nil {
		s.Fatal("Failed to verify shelf app icon alignment after pinning the Files app when shelf is at the right: ", err)
	}
