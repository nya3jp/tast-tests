...
func AppRearrangement(ctx context.Context, s *testing.State) {
	...
	shelfAppBounds, err := ash.GetShelfAppBoundsForNames(ctx, tconn, ui, defaultPinnedAppNamesInOrder)
	if err != nil {
	...
	if err := getStartDragAction(tconn, "start drag on the target app from the last slot", lastSlotCenter)(ctx); err != nil {
	...
	updatedMiddleAppBounds, err := ash.GetShelfAppBoundsForNames(ctx, tconn, ui, []string{middleAppName})
	if err != nil {
	...
	if err := getStartDragAction(tconn, "start drag on the target app from the first slot", firstSlotCenter)(ctx); err != nil {
