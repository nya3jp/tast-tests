...
func AppRearrangement(ctx context.Context, s *testing.State) {
	...
	if err := ash.VerifyShelfIconIndices(ctx, tconn, []string{apps.Chrome.ID, apps.Settings.ID, targetAppID}, []int{0, 1, 2}); err != nil {
	...
	if err := ash.VerifyShelfIconIndices(ctx, tconn, []string{apps.Chrome.ID, apps.Settings.ID, targetAppID}, []int{1, 2, 0}); err != nil {
		s.Fatal("Failed to verify shelf icon indices: ", err)
	}
