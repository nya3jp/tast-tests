...
func AppRearrangement(ctx context.Context, s *testing.State) {
	...
	cleanUp, err := ash.UnpinAppsAndReset(ctx, tconn, itemsToUnpin)
	if err != nil {
		s.Fatalf("Failed to unpin apps %v: %v", itemsToUnpin, err)
	}
	defer cleanUp(ctx)

	cleanUp, err = ash.PinAppsAndReset(ctx, tconn, []string{apps.Settings.ID})
	if err != nil {
		s.Fatal("Failed to pin Settings to shelf")
	}
	defer cleanUp(ctx)

	// The name and the ID of the app which is going to be dragged and dropped on the shelf.
	...
	cleanUp, err = ash.PinAppsAndReset(ctx, tconn, []string{targetAppID})
	if err != nil {
	...
	ac := uiauto.New(tconn)
	if err := uiauto.Combine("move the target app from the third slot to the first slot", getDragAndDropActionStream(ac, tconn, moveStartLocation, moveEndLocation)...)(ctx); err != nil {
	...
	if err := uiauto.Combine("move the target app from the first slot to the third slot", getDragAndDropActionStream(ac, tconn, moveEndLocation, moveStartLocation)...)(ctx); err != nil {
	...
	if err := uiauto.Combine("move the target app with the activated window from the third slot to the first slot", getDragAndDropActionStream(ac, tconn, moveStartLocation, moveEndLocation)...)(ctx); err != nil {
	...
	if err := uiauto.Combine("move the target app with the activated window from the first slot to the third slot", getDragAndDropActionStream(ac, tconn, moveEndLocation, moveStartLocation)...)(ctx); err != nil {
	...
	cleanUp, err = ash.UnpinAppsAndReset(ctx, tconn, []string{targetAppID})
	if err != nil {
	...
	if err := uiauto.Combine("move the unpinned app with the activated window from the third slot to the first slot", getDragAndDropActionStream(ac, tconn, moveStartLocation, moveEndLocation)...)(ctx); err != nil {
