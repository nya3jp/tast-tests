...
func AppRearrangement(ctx context.Context, s *testing.State) {
	...
	// The app ids following the pin order before any drag-and-drop operations. In other words, an app that is pinned earlier has a smaller array index.
	var defaultAppIDsInPinOrder []string

	// The updated app ids following the pin order after dragging the target app from the last slot to the first slot.
	...
	} else if isUnderRtl && updatedMiddleAppBounds[0].Right() >= middleSlotBounds.Left {
	...
	} else if isUnderRtl && updatedMiddleAppBounds[0].Left <= middleSlotBounds.Right() {
