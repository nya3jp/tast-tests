...
func HotseatScalable(ctx context.Context, s *testing.State) {
	...
	pinned, err := ash.AreAppsPinned(ctx, tconn, apps)
	if err != nil {
		s.Fatal("Failed to verify that the specified pinned app is displayed on the shelf: ", err)
	}
	...
	if err := ui.WaitUntilExists(nodewith.HasClass("UnifiedSystemTray").Role(role.Button))(ctx); err != nil {
		s.Fatal("Failed to verify status menu in laptop mode: ", err)
	}
}
