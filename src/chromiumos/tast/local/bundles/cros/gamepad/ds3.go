...
func handleGetReportDS3(ctx context.Context, d *uhid.Device, buf []byte) error {
	processRNum := func(rnum uhid.RNumType) ([]byte, error) {
		...
		case macAddressRequest:
			// This is a hardcoded array based on the uniq constant
			// dualshock.Uniq. the MAC address of the device is stored in
			// the bytes 4-9 rest has been dumped on the response of a real
			// Sixaxis controller.
