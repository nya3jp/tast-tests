...
func ChangeARCAvailability(ctx context.Context, s *testing.State) {
	...
	if err := accountmanager.CheckIsAccountPresentInArcAction(tconn, arcDevice, username, true)(ctx); err != nil {
	...
	if err := uiauto.Combine("Remove account from ARC",
		...
		accountmanager.CheckIsAccountPresentInArcAction(tconn, arcDevice, username, false),
	)(ctx); err != nil {
	...
	if err := uiauto.Combine("Add account to ARC",
		...
		// Check that account is present in ARC.
		accountmanager.CheckIsAccountPresentInArcAction(tconn, arcDevice, username, true),
	)(ctx); err != nil {
