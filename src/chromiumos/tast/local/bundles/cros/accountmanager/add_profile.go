...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Addition of a secondary lacros profile",
...
func AddProfile(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine("click on nextButton",
		ui.LeftClick(nodewith.Name("Continue without an account").Role(role.Button).Focusable().Ancestor(addProfileRoot)),
		ui.LeftClick(nodewith.NameStartingWith("Add a name").Role(role.TextField).Focusable().Required().Ancestor(customizeProfileRoot)),
		kb.TypeAction(newProfileName),
		ui.LeftClick(nodewith.Name("Done").Role(role.Button).Focusable().Ancestor(customizeProfileRoot)),
	)(ctx); err != nil {
