...
func SwitchOnlineWallpapers(ctx context.Context, s *testing.State) {
	...
	// Make sure yellow is last selected. We will be comparing the background wallpaper with the given rgba color.
...
func validateBackground(ctx context.Context, cr *chrome.Chrome, clr color.Color, expectedPercent int) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if percent < expectedPercent {
			return errors.Errorf("unexpected pixels percentage: got %d / %d = %d%%; want at least %d%%", clrPixels, totalPixels, percent, expectedPercent)
		}
		return nil
