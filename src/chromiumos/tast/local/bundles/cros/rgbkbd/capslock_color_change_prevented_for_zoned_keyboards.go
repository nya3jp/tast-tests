...
// CapslockColorChangePreventedForZonedKeyboards verifies that enabling or disabling Capslock updates
// the RGB backlight.
func CapslockColorChangePreventedForZonedKeyboards(ctx context.Context, s *testing.State) {
	...
	if string(initialContent) != string(contentAfterCapsLockKeyPress) {
		s.Fatal("Caps lock change logs written for zoned keyobard")
	}
}
