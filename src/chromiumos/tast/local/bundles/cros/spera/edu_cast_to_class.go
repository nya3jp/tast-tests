...
func EDUCastToClass(ctx context.Context, s *testing.State) {
	...
	defer uiauto.NamedAction("check the status of the casting", youtubeWeb.ResetCastStauts())(cleanupCastCtx)
	...
	if err = recorder.Run(ctx, func(ctx context.Context) error {
		...
		castYoutubeVideo := uiauto.NamedCombine("cast youtube video",
			...
			youtubeWeb.Cast(youtube.CastStart, accessCode),
		)
		...
		return uiauto.NamedCombine("cast to class",
			castYoutubeVideo,
			editSlide,
			youtubeWeb.Cast(youtube.CastStop, accessCode),
		)(ctx)
	}); err != nil {
