...
var (
	...
	iconInTaskManager = map[WindowsApp]string{
...
func NewCitrix(tconn *chrome.TestConn, kb *input.KeyboardEventWriter, dataPath func(string) string, desktopTitle string, testMode TestMode) *Citrix {
	return &Citrix{
		tconn:        tconn,
		ui:           uiauto.New(tconn),
...
func (c *Citrix) NewTab(url string, newWindow bool) action.Action {
	...
	return func(ctx context.Context) error {
		...
		if newWindow {
			openNewChromeWindow := func(ctx context.Context) error {
			...
			newTabAction = openNewChromeWindow
		}
		return uiauto.NamedCombine("open tab "+url,
			// First tab don't need to add a new tab.
			newTabAction,
