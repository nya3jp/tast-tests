...
func NewCitrix(tconn *chrome.TestConn, kb *input.KeyboardEventWriter, dataPath func(string) string, desktopTitle string) *Citrix {
	return &Citrix{
		tconn:        tconn,
		ui:           uiauto.New(tconn),
...
func (c *Citrix) UploadPhoto(filename string) action.Action {
	sampleFile := uidetection.Word(filename).First()
