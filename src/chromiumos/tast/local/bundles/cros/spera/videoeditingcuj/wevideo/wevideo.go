...
func (w *WeVideo) Create() action.Action {
	...
	createNewRe := regexp.MustCompile("(CREATE NEW)|(Create new)")
	...
	titleRe := regexp.MustCompile("(MY VIDEO)|(My Video)")
...
func (w *WeVideo) AddText(clipName, expectedTrack, text string) action.Action {
	...
	saveButtonRe := regexp.MustCompile("(SAVE CHANGES)|(Save changes)")
