...
func prepareAmbientMode(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if err := ui.StableFindAndClick(
		ctx,
		tconn,
		ui.FindParams{Name: "Screen saver", Role: ui.RoleTypeLink},
	...
	if err := testing.Sleep(ctx, 1*time.Second); err != nil {
		return errors.Wrap(err, "failed sleep")
	}

	artAlbums, err := ui.FindAll(
		ctx,
		tconn,
		ui.FindParams{
			Role: ui.RoleTypeCheckBox,
			Attributes: map[string]interface{}{
				"name": regexp.MustCompile("^Album.+"),
			},
		},
	)
	if err != nil {
...
func waitForAmbientStart(ctx context.Context, tconn *chrome.TestConn) error {
	if err := ambient.WaitForPhotoTransitions(ctx, tconn, 2, 6*time.Second); err != nil {
