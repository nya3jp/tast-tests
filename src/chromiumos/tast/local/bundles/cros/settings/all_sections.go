...
// checkSection checks section within the ossettings,
...
func ensureVisible(osSettings *ossettings.OSSettings, node *nodewith.Finder) uiauto.Action {
	return func(ctx context.Context) error {
		if found, err := osSettings.IsNodeFound(ctx, nodewith.Role(role.Navigation).First()); err != nil {
			...
			// The main menu might collapse depending on window size, expand the main menu to ensure the input node is able to be visible.
			if err = osSettings.LeftClick(ossettings.MainMenu)(ctx); err != nil {
				return errors.Wrap(err, "failed to click main menu")
			}
		}
