...
func AllSections(ctx context.Context, s *testing.State) {
	...
	testSamples := make([]*sectionTest, 0)
	for sectionName, sectionFinder := range sections {
		...
		if sectionFinder == ossettings.Network {
			sample.subSectionToExpand = "Add network connection"
		}
		if sectionFinder == ossettings.DateAndTime {
			sample.subSettingToToggle = "Use 24-hour clock"
		}

		testSamples = append(testSamples, &sample)
	}

	if err := checkSections(ctx, cr, osSettings, testSamples); err != nil {
...
// and verifies subsection or sub-setting is properly displayed
// by expands/toggles on it if the subsection or sub-setting is specified.
