...
func TermsLinkClickable(ctx context.Context, s *testing.State) {
	...
	ossettingsTest := func(ctx context.Context, s *testing.State) {
		...
		if err := uiauto.Combine("wait and click on `About Chrome OS page`",
			uiauto.New(tconn).FocusAndWait(ossettings.AboutChromeOS),
			settings.WaitUntilExists(ossettings.AboutChromeOS),
			settings.LeftClick(ossettings.AboutChromeOS),
		)(ctx); err != nil {
			s.Fatal("Failed to switch page: ", err)
		}
...
func verifyContent(ctx context.Context, cr *chrome.Chrome, outDir string) (err error) {
	...
	// Verify the content is within the terms page, not just on the screen.
