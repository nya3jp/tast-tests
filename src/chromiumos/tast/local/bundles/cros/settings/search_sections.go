...
func SearchSections(ctx context.Context, s *testing.State) {
	...
	for _, search := range detail {
		firstOption, err := searchAndCheck(ctx, osSettings, kb, search)
		if err != nil {
		...
		if search.subpageLabel != "" {
			if err := ui.WithTimeout(time.Minute).RetryUntil(
				mouse.Click(tconn, firstOption.Location.CenterPoint(), mouse.LeftButton),
...
func searchAndCheck(ctx context.Context, osSettings *ossettings.OSSettings, kb *input.KeyboardEventWriter,
	...
	if err != nil {
		return &uiauto.NodeInfo{}, err
	}

	// Verify search results count.
	if len(infos) == 0 {
		return &uiauto.NodeInfo{}, errors.New("no results found")
	} else if len(infos) > 5 || len(infos) < 1 {
		// The results should show a minimum of 1 or maximum of 5 results.
		return &uiauto.NodeInfo{}, errors.Errorf("unexpected result count, want: [1,5], got: %d", len(infos))
	}

	// Verify mismatch.
	if detail.expectedMismatch != mismatched {
		return &uiauto.NodeInfo{}, errors.Errorf("unexpected search result, want: [mismatch: %t], got: [mismatch: %t]", detail.expectedMismatch, mismatched)
	}
	...
	return &uiauto.NodeInfo{}, errors.New("no match results found")
}
