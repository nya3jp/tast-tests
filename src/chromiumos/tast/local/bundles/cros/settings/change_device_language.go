...
func ChangeDeviceLanguage(ctx context.Context, s *testing.State) {
	...
	if err := settings.ChangeDeviceLanguageAndRestart(ctx, tconn, "chinese", "Chinese (Simplified)"); err != nil {
	...
	if err := uiauto.New(tconn).WaitUntilExists(nodewith.Name("关机").First())(ctx); err != nil {
