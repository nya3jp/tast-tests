...
func calculateHashOfFile(path string) ([]byte, error) {
	f, err := os.Open(path)
	if err != nil {
		return []byte{}, errors.Wrap(err, "failed to open log files %q")
	}
	defer f.Close()

	h := sha256.New()
	if _, err := io.Copy(h, f); err != nil {
		return []byte{}, errors.Wrap(err, "failed to calculate the hash of the log file %q")
	}
...
func createLogFiles(s *testing.State) error {
	for i := 0; i < 7; i++ {
		...
		if err != nil {
			return errors.Wrap(err, "failed to create a log file %q")
		}
	}
	return nil
}
