...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Create a screencast and verifies it transitions from the uploading to the uploaded state",
...
func TransitionState(ctx context.Context, s *testing.State) {
	cleanupCtx := ctx
	// Reserve 2 mintes to delete screencasts via UI.
	...
	// Newly created screenast will always be the first.
	...
	s.Log("Wait for uploading to finish")
	if err := uiauto.Combine("Wait for uploading to finish",
		...
		// We don't verify the transition of transcoding to ready since it might
		// take a very long time.
	)(ctx); err != nil {
		s.Fatal("Failed to wait for uploading to finish: ", err)
	}
}
