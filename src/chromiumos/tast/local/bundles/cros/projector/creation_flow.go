...
func CreationFlow(ctx context.Context, s *testing.State) {
	...
	// Dismiss the onboarding dialog, if it exists. Since each user only sees the onboarding flow a
	// maximum of three times, the onboarding dialog may not appear.
	...
	if err := uiauto.Combine("launching the new screencast creation flow",
		...
		ui.LeftClick(annotatorButton),
		// This test saves the screencast to the downloads directory, and since the test device is
		// power-washed in between iterations, these screencasts should not take up disk space over time.
		// TODO(b/229631680): Figure out how to mock DriveFS and modify this test to go through the prod
		// creation flow, instead of saving to the downloads directory.
