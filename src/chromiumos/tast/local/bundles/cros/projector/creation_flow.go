...
func CreationFlow(ctx context.Context, s *testing.State) {
	...
	// We need to clean up any screencasts to prevent taking up Drive quota over time.
	s.Log("Deleting screencasts")
	if err := deleteScreencastItems(ctx, tconn); err != nil {
		s.Fatal("Failed to delete leftover screencast items: ", err)
	}
}
