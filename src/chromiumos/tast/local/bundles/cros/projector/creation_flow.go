...
func CreationFlow(ctx context.Context, s *testing.State) {
	...
	enabledNewScreencastButton := newScreencastButton.Focusable()
	speechRecognitionNotSupported := nodewith.Name("Speech recognition not supported").Role(role.GenericContainer)
	turnOnMicrophone := nodewith.Name("Turn on microphone").Role(role.GenericContainer)
	...
	if err := verifySodaInstalled(ctx); err != nil {
		...
		if err = uiauto.Combine("verify the new screencast button is disabled",
			...
			ui.Gone(enabledNewScreencastButton),
			ui.Exists(speechRecognitionNotSupported),
		)(ctx); err != nil {
			s.Fatal("SODA is not installed, but new screencast button is not disabled: ", err)
		}
	...
	if err := audio.WaitForDevice(ctx, audio.InputStream); err != nil {
		...
		if err = uiauto.Combine("verify the new screencast button is disabled",
			...
			// The new screencast button exists but it is not enabled.
			ui.Gone(enabledNewScreencastButton),
			ui.Exists(turnOnMicrophone),
		)(ctx); err != nil {
			s.Fatal("Microphone is unavailable, but new screencast button is not disabled: ", err)
		}
		// Pass the test and exit prematurely.
		return
	}
	...
	s.Log("SODA and microphone are enabled, launching the new screencast creation flow")
	if err := uiauto.Combine("launch the new screencast creation flow",
		ui.WithInterval(5*time.Second).RetryUntil(refreshApp, ui.Exists(enabledNewScreencastButton)),
		...
		// disappear.
		ui.LeftClickUntil(enabledNewScreencastButton, ui.Gone(enabledNewScreencastButton)),
...
func verifySodaInstalled(ctx context.Context) error {
	// Check if dlc libsoda and libsoda-model-en-us are installed.
