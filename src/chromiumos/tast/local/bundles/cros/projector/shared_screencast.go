...
func SharedScreencast(ctx context.Context, s *testing.State) {
	...
	// Open the screencast.
	conn.Eval(ctx, "window.location.href = '"+sharedScreencast+"';", nil)
	...
	ui := uiauto.New(tconn)
	if err := ui.Exists(nodewith.Name("Screencast for Tast (Do not modify)").Role(role.StaticText))(ctx); err != nil {
