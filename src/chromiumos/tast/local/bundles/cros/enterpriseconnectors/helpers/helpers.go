...
func GetCleanDconnSafebrowsing(ctx context.Context, br *browser.Browser, tconn *chrome.TestConn) (*browser.Conn, error) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err := dconnSafebrowsing.Eval(ctx, `document.getElementById("deep-scan-list").rows.length`, &numRows); err != nil {
			dconnSafebrowsing.Close()
			dconnSafebrowsing.CloseTarget(cleanupCtx)
		...
		// This dconn is good, so set it!
		retDconn = dconnSafebrowsing
		return nil
	...
	if retDconn == nil {
		return nil, errors.New("failed to create clean dconn to safe-browsing page")
	}
	return retDconn, nil
}
