...
func TestFileAttached(ctx context.Context, s *testing.State) {
	...
	testUploadForBrowser(ctx, s, testParams.BrowserType)
}
		...
		s.Run(ctx, params.TestName, func(ctx context.Context, s *testing.State) {
			...
			if !testParams.AllowsImmediateDelivery && testParams.ScansEnabled {
				...
				err = uiauto.Combine("show scanning dialog",
					...
					ui.WithTimeout(2*time.Second).WaitUntilExists(scanningLabelFinder),
			...
			if testParams.ScansEnabled && !params.IsUnscannable {
				...
				err := helpers.WaitForDeepScanningVerdict(ctx, dconnSafebrowsing, scanningTimeOut)
				if err != nil {
