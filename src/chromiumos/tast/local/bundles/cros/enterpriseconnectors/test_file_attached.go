...
func testUploadForBrowser(ctx context.Context, s *testing.State, browserType browser.Type) {
	...
	for _, param := range []struct {
		...
		ulBlockLabel    string
		ulIsBad         bool
		ulIsUnscannable bool
	}{
		{
			testName:        "Encrypted malware",
			ulFileName:      "unknown_malware_encrypted.zip",
			ulBlockLabel:    "ncrypted",
			ulIsBad:         true,
			ulIsUnscannable: true,
		},
		{
			testName:        "Unknown malware",
			ulFileName:      "unknown_malware.zip",
			ulBlockLabel:    "try again",
			ulIsBad:         true,
			ulIsUnscannable: false,
		},
		{
			testName:        "Known malware",
			ulFileName:      "content.exe",
			ulBlockLabel:    "try again",
			ulIsBad:         true,
			ulIsUnscannable: false,
		},
		...
		s.Run(ctx, param.testName, func(ctx context.Context, s *testing.State) {
			...
			files, err := filesapp.App(ctx, tconn, "picker")
			if err != nil {
			...
			if !policyParams.AllowsImmediateDelivery && policyParams.ScansEnabled {
				...
				if shouldBlockUpload {
					...
					blockedLabelTextFinder := nodewith.Role(role.StaticText).ClassName("Label").Ancestor(scanningDialogFinder).NameContaining(param.ulBlockLabel)
			...
			if policyParams.ScansEnabled && !param.ulIsUnscannable {
				...
				helpers.VerifyDeepScanningVerdict(ctx, s, dconnSafebrowsing, param.ulIsBad)
			}
		})
	}
}
