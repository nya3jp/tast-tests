...
func TestDownload(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		/*{
			test_name:          "Large file",
			dl_file_name:       "100MB.bin",
			dl_link_identifier: "Large file",
			dl_is_bad:          false,
			dl_is_unscannable:  true,
		},*/
		{
		...
		s.Run(ctx, param.testName, func(ctx context.Context, s *testing.State) {
			...
			if err := files.WithTimeout(10 * time.Second).WaitForFile(dlFileName)(ctx); err != nil {
