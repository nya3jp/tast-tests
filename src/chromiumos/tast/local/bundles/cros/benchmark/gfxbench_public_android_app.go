...
const (
	...
	benchmarkSyncTime    = 20 * time.Minute
...
func GFXBenchPublicAndroidApp(ctx context.Context, s *testing.State) {
	...
	if err := openGfxbench(ctx, s, tconn, device, ar); err != nil {
	...
	if err := startBenchmarkButton.Click(ctx); err != nil {
		s.Fatal("Failed to click start benchmark button: ", err)
	}
...
func openGfxbench(ctx context.Context, s *testing.State, tconn *chrome.TestConn, device *ui.Device, ar *arc.ARC) error {
		...
		if err := tabs.GetObject(ctx); err == nil {
			...
			if err := tabs.GetChild(ctx, homeViewTab); err != nil {
				...
				if err := homeViewTab.Click(ctx); err != nil {
					s.Log("Failed to ckilck home tab: ", err)
				}
			}
		}
