...
func startGfxbench(ctx context.Context, device *ui.Device, boundsInPx coords.Rect) error {
	...
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		return nil
	}, &testing.PollOptions{Timeout: 2 * time.Minute, Interval: time.Second})
}
...
func parseBenchmarkResultItem(ctx context.Context, device *ui.Device, item *ui.Object, logMessage string) (val string, err error) {
	...
	if err != nil {
		return "", errors.Wrapf(err, "cannot find result for %s", logMessage)
	}
...
func parseBenchmarkResult(ctx context.Context, device *ui.Device) (float64, float64, error) {
	...
	for i := 1; ; i++ {
		...
		if err := resultList.GetChild(ctx, listRow); err != nil {
			...
			// Stop further searching if we cannot get a row.
			break
		}
		...
		if val == "Aztec Ruins OpenGL (Normal Tier)" {
			...
			logMessage := "AztecRuinsOpenGLNormal(Aztec Ruins OpenGL (normal tier))" // Const string to match Tier result.
			...
			logMessage := "AztecRuinsOpenGLNormalOffscreen(1080p Aztec Ruins OpenGL (normal tier) Offscreen)" // Const string to match Offscreen result.
