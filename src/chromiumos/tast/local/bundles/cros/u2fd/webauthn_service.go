...
func (c *WebauthnService) New(ctx context.Context, req *hwsec.NewRequest) (*empty.Empty, error) {
	...
	lacrosCfg := browserfixt.DefaultLacrosConfig
	deployedPath := req.GetLacrosDeployedPath()
	if deployedPath != "" {
		lacrosCfg = lacrosCfg.WithDeployedPath(deployedPath)
	}
	...
	cr, br, closeBrowser, err := browserfixt.SetUpWithNewChrome(ctx, bt, lacrosCfg)
	if err != nil {
...
func (c *WebauthnService) SetupPin(ctx context.Context, req *hwsec.SetupPinRequest) (*empty.Empty, error) {
	_, err := util.SetUpUserPIN(ctx, c.cr, req.GetPin(), chrome.DefaultPass, false /*autosubmit*/)
	if err != nil {
		return nil, err
	}
