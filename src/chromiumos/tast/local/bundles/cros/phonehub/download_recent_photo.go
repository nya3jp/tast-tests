...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Exercises toggling the Recent Photos feature and and downloading a photo from a connected phone",
		Contacts: []string{
...
func DownloadRecentPhoto(ctx context.Context, s *testing.State) {
	...
	if err := androidDevice.TakePhoto(ctx); err != nil {
	...
	// Download the most recent photo to Tote.
	if err := phonehub.DownloadRecentPhoto(ctx, tconn); err != nil {
		s.Fatal("Failed to download recent photo: ", err)
	}
	if err := uiauto.Combine("view downloaded photo in the holding space tray",
		ui.LeftClick(holdingspace.FindTray()),
		ui.Exists(holdingspace.FindDownloadChip().NameContaining(".jpg").First()),
	)(ctx); err != nil {
		s.Fatal("Downloaded recent photo is not displayed in the holding space tray: ", err)
	}
