...
func ARCBlockedAppInstall(ctx context.Context, s *testing.State) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) (retErr error) {
		...
		if err := installButton.Exists(ctx); err == nil {
			return rl.Exit("verify install button should not exist", err)
		}

		installBlockedMessage := d.Object(ui.ClassName("android.widget.TextView"), ui.TextMatches("(?i)"+appUnavailableMessage))
		if err := installBlockedMessage.WaitForExists(ctx, defaultUITimeout); err != nil {
