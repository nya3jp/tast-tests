...
func dumpBugReport(ctx context.Context, a *arc.ARC, hasError func() bool, filePath string) {
	if !hasError() {
		return
	}
...
func ensurePackagesUninstallable(ctx context.Context, cr *chrome.Chrome, a *arc.ARC, outDir string, hasError func() bool, packages []string) error {
	...
	defer cancel()
	defer dumpBugReport(cleanupCtx, a, hasError, filepath.Join(outDir, "bugreport.zip"))
