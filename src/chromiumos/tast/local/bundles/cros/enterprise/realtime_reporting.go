...
func RealtimeReporting(ctx context.Context, s *testing.State) {
	...
	cr, err := chrome.New(
		...
		chrome.EncryptedReportingAddr(fmt.Sprintf("%v/record", reportingServerURL)),
	...
	if err := validateEventReception(events, deviceVirtualID, heartbeatEventDestination, testStartTime, &receivedNewEvents); err != nil {
		s.Fatal("Failed to validate event reception: ", err)
	}

	if !receivedNewEvents {
		s.Fatal("The Reporting Api Server did not receive any events")
	}
}
