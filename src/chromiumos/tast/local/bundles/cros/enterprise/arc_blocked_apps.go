...
// ARCBlockedApps force-installs an app and ensures it is removed if blocked by policy.
func ARCBlockedApps(ctx context.Context, s *testing.State) {
	const (
	...
	if err := testing.Poll(ctx, func(ctx context.Context) (retErr error) {
		...
		if err := arcent.EnableVerboseLogging(ctx, a); err != nil {
			return exit("enable verbose logging", err)
		}

		if err := arcent.IncreaseLogcatBufferSize(ctx, a); err != nil {
