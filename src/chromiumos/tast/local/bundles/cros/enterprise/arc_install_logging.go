...
func init() {
	testing.AddTest(&testing.Test{
		...
		Vars:         []string{"enterprise.ARCInstallLogging.user", "enterprise.ARCInstallLogging.password", "enterprise.ARCInstallLogging.gaiaId", "enterprise.ARCInstallLogging.testPackage"},
	})
}
...
func ARCInstallLogging(ctx context.Context, s *testing.State) {
	...
	gaiaID := s.RequiredVar("enterprise.ARCInstallLogging.gaiaId")
	testPackage := s.RequiredVar("enterprise.ARCInstallLogging.testPackage")
	...
	cr, err := chrome.New(
		ctx,
		chrome.Auth(user, password, gaiaID),
		chrome.GAIALogin(),
		chrome.ARCSupported(),
		chrome.ProdPolicy(),
	...
	if err := arc.WaitForPackages(ctx, a, []string{testPackage}); err != nil {
