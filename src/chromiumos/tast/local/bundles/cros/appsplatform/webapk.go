...
func WebAPK(ctx context.Context, s *testing.State) {
	...
	if err := installTestApps(ctx, cr, a, tconn, s.DataPath(generatedWebAPK)); err != nil {
		s.Fatal("Error installing test apps: ", err)
	}

	s.Log("Launching test app")
	device, activity, err := launchTestApp(ctx, a, tconn)
	if err != nil {
		s.Fatal("Error launching test app: ", err)
	}
	...
	if err := shareTextAndVerify(ctx, device, shareChan); err != nil {
		s.Fatal("Error sharing text from test app: ", err)
	}
}
...
func installTestApps(ctx context.Context, cr *chrome.Chrome, a *arc.ARC, tconn *chrome.TestConn, webAPKPath string) error {
	...
	if err := a.Install(ctx, webAPKPath); err != nil {
		return errors.Wrap(err, "failed installing WebAPK")
	}
	if err := a.Install(ctx, arc.APKPath(testAPK)); err != nil {
		return errors.Wrap(err, "failed installing test app")
	}
	return nil
...
func shareTextAndVerify(ctx context.Context, device *ui.Device, shareChan chan shareResult) error {
	...
	if receivedShare.title != expectedSharedTitle {
		return errors.Errorf("shared title did not match: got %q, want %q", receivedShare.title, expectedSharedTitle)
	}

	if receivedShare.text != expectedSharedText {
		return errors.Errorf("Shared title did not match: got %q, want %q", receivedShare.text, expectedSharedText)
	}
	return nil
}
