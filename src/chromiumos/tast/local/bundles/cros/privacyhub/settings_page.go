...
func checkAllToggles(cleanupCtx, ctx *context.Context, tconn *chrome.TestConn, cr *chrome.Chrome, timeout time.Duration) error {
	checkToggle := func(name string) error {
	...
	if err := checkToggle("Camera"); nil != err {
		return err
	}
	if err := checkToggle("Microphone"); nil != err {
		return err
	}
...
func SettingsPage(ctx context.Context, s *testing.State) {
	...
	if featureOn {
		...
		if err = checkAllToggles(&cleanupCtx, &ctx, tconn, cr, timeout); err != nil {
