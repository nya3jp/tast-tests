...
func checkAllToggles(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome, timeout time.Duration) error {
	checkToggle := func(ctx context.Context, name string) error {
		cleanupCtx := ctx
		ctx, cancel := ctxutil.Shorten(ctx, 5*time.Second)
		defer cancel()

		cameraLabel := nodewith.Name(name).Role(role.ToggleButton)
		settings, err := ossettings.LaunchAtPageURL(ctx, tconn, cr, "osPrivacy/privacyHub", uiauto.New(tconn).WithTimeout(timeout).WaitUntilExists(cameraLabel))
		if err != nil {
			return errors.Wrapf(err, "failed to find %s in Privacy Hub page", name)
		}
		defer settings.Close(cleanupCtx)
		return nil
	}

	if err := checkToggle(ctx, "Camera"); err != nil {
		return err
	}
	if err := checkToggle(ctx, "Microphone"); err != nil {
		return err
	}
...
func SettingsPage(ctx context.Context, s *testing.State) {
	...
	if featureOn {
		...
		if err := uiauto.New(tconn).WithTimeout(timeout).WaitUntilExists(privacyMenu)(ctx); err != nil {
			s.Fatal("Failed to find Privacy Hub in OS setting page: ", err)
		}
		// Check that the Privacy Hub section contains a button for camera.
		if err = checkAllToggles(ctx, tconn, cr, timeout); err != nil {
		...
		if err := uiauto.New(tconn).WithTimeout(timeout).WaitUntilExists(privacyMenu)(ctx); err == nil {
			s.Fatal("Found Privacy Hub in OS setting page even though the flag is not set: ", err)
		}
