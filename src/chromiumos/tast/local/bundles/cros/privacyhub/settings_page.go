...
func checkAllToggles(cleanupCtx, ctx *context.Context, tconn *chrome.TestConn, cr *chrome.Chrome) error {
	checkToggle := func(name string) error {
		...
		if err != nil {
			return errors.Errorf("failed to find %s in Privacy Hub page", name)
		}
...
func SettingsPage(ctx context.Context, s *testing.State) {
	...
	if featureOn {
		...
		if err = checkAllToggles(&cleanupCtx, &ctx, tconn, cr); err != nil {
