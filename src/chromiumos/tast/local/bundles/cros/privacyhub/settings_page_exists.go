...
func sectionsFromConnection(ctx context.Context, s *testing.State, conn *chrome.Conn) []string {
	var sections []string
	if err := webutil.EvalWithShadowPiercer(ctx, conn, `[...shadowPiercingQueryAll("h2.title")].map(x=>x.innerHTML)`, &sections); err != nil {
		s.Fatal("Failed to read the sections from Settings page: ", err)
	}
	return sections
}
...
func settingsSections(ctx context.Context, s *testing.State, sm *session.SessionManager) map[string]bool {
	cr := startChrome(ctx, s, sm)
	defer cr.Close(ctx)

	conn := openSettings(ctx, s, cr)
	defer conn.Close()

	sections := sectionsFromConnection(ctx, s, conn)
	s.Log("Received the following sections:", sections)

	//converting the list of section names into a map
...
func test(ctx context.Context, s *testing.State, sm *session.SessionManager) {
	...
	// TODO(janlanik): Testing of the wanted section falls out of scope of this test
	// However I find it a pitty to remove it, so I'll see if I can put it into a separate test.
	// This patch should not be merged with this comment - if you see it, please don't +1/+2.
	wantedSections := []string{"Network", "Bluetooth", "Connected devices", "Accounts", "Device", "Personalization", "Search and Assistant", "Security and Privacy", "Apps", "Accessibility"}

	sections := settingsSections(ctx, s, sm)
	for _, name := range wantedSections {
		if _, ok := sections[name]; !ok {
			s.Error("Missing a section in settings page:", name)
		}
	}
