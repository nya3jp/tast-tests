...
func Login(ctx context.Context, s *testing.State) {
	...
	settings, err := smartlock.OpenSmartLockSubpage(ctx, tconn, cr)
	if err != nil {
                s.Fatal("Failed to open Smart Lock subpage: ", err)
        }
	...
	if err := settings.EnableSmartLockLogin(cr, password)(ctx); err != nil {
		s.Fatal("Failed to enable smart lock login: ", err)
	}
	 if err := testing.Sleep(ctx, 5 * time.Second); err != nil {
                s.Fatal("Failed to sleep after clicking on the compat-mode button: ", err)
