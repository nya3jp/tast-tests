...
func Settings(ctx context.Context, s *testing.State) {
	...
	defer t.kb.Close()
	defer faillog.DumpUITreeOnError(t.ctx, t.s.OutDir(), t.s.HasError, t.tconn)
...
func goToLoginScreen(t *settingsTestData) error {
	...
	t.s.Log("Signed out after enabling Smart Lock sign-in")

	// TODO(b/217272610) Remove this second log in once this bug is resolved.
	t.s.Log("Logging in again with password before Smart Lock will work")
	if err = signIn(t); err != nil {
	...
	t.s.Log("Signed out after signing in with password")

	t.s.Log("Now trying to login with Smart Lock")
	t.cr, err = chrome.New(
		t.ctx,
		t.noLoginOpts...,
	)
