...
func getPersistentInteger(ctx context.Context, file string) (uint64, error) {
	...
	if _, err = f.Seek(4, 0); err != nil { // Skip version information.
		return 0, errors.Wrap(err, "Error while seeking persistent integer file")
	}

	if _, err := f.Read(numUnclean); err != nil {
		return 0, errors.Wrap(err, "Error while reading persistent integer value")
	}
	f.Close()
...
func UncleanShutdownCollector(ctx context.Context, s *testing.State) {
	...
	if err := testing.Poll(ctx, func(c context.Context) error {
		...
		if err != nil {
			s.Fatal("Could not get daily cycle count: ", err)
		}
		...
		return errors.Errorf("Did not see unclean shutdown. Got %d but expected %d (old cycle: %d, new cycle: %d)",
			newUnclean, oldUnclean+1, oldDailyCycle, newDailyCycle)
