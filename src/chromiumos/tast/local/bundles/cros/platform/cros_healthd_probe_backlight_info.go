...
func CrosHealthdProbeBacklightInfo(ctx context.Context, s *testing.State) {
	...
	if !hasBacklight {
		...
		// If there is no backlight, there are no records to verify.
		return
	}
	...
	// Verify each record contains valid values.
	for _, record := range records[1:] {
		if len(record) != 3 {
			s.Errorf("Wrong number of values: got %d, want 3", len(record))
			continue
		}
