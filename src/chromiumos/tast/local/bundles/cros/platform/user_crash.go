...
// testMaxEnqueuedCrash tests that maxCrashDirectorySize is enforced.
func testMaxEnqueuedCrash(ctx context.Context, s *testing.State) {
	...
	watcher, err := syslog.NewWatcher("/var/log/messages")
	if err != nil {
		s.Error("Failed to create watcher before queueing: ", err)
	}
	crashDir, err := crash.GetCrashDir(username)
	if err != nil {
		s.Error("Failed before queueing: ", err)
	}
	...
	if err != nil {
		s.Error("Failed to get crash dir size: ", crashDir)
	}
	...
	for i := 0; i < 10; i++ {
		...
		testing.ContextLogf(ctx, "New log messages: %s", "TODO: get logs")
		if !result.Crashed {
			s.Error("Failure after setting up queue: ", result.ReturnCode)
		}
		if !result.CrashReporterCaught {
