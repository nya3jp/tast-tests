...
func stashLeaveCore(ctx context.Context, cr *chrome.Chrome, s *testing.State) (e error) {
	...
	if _, err := os.Stat(leaveCorePath); err == nil {
		...
		return errors.Wrap(err, ".leave_core file did not disappear")
	}
	return nil
}
