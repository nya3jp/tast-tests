...
func getUptime() (time.Duration, error) {
	...
	out, err := ioutil.ReadFile(uptimeFile)
	if err != nil {
		return 0, errors.Wrapf(err, "failed to read file: %s", uptimeFile)
	}
	seconds, err := strconv.ParseFloat(strings.Split(string(out), " ")[0], 64)
	if err != nil {
		return 0, errors.Wrapf(err, "failed to parse content of uptime file: %s", out)
	}
	return time.Duration(seconds * 1e9), nil // in nanoseconds
}
...
func testReporterStartup(ctx context.Context, s *testing.State) {
	...
	if err != nil || !f.Mode().IsRegular() {
		s.Fatal("Crash reporter enabled file flag is not present at ", crashReporterEnabledPath)
	}
