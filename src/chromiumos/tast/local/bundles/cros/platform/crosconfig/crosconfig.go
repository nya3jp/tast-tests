...
// GoldenDBFile is a JSON struct for reading and writing the golden database file. A new
...
func DetermineDeviceToProcess(ctx context.Context, s *testing.State) (string, error) {
...
// device to be tested.
func BuildCommands(ctx context.Context, b []byte, deviceToProcess string,
	s *testing.State) []*testexec.Cmd {
	...
	if err != nil {
		s.Fatal("Failed to unmarshal json commands", err)
	}
...
func BuildCommand(ctx context.Context, binary string, line string) *testexec.Cmd {
	...
	return testexec.CommandContext(ctx, binary, arr...)
}
