...
func CompareGoldenOutput(
	...
	if err != nil {
		s.Fatal("Failed to unmarshal golden database", err)
	}
...
func DetermineDeviceToProcess(ctx context.Context, s *testing.State) string {
	...
	if err != nil {
		c.DumpLog(ctx)
		s.Fatal("Failed to determine the device name", err)
	}
...
// BuildCommands builds the CommandRec structs from the JSON input file for the
// device to be tested.
