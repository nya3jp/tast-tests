...
func CrosConfig(ctx context.Context, s *testing.State) {
	...
	devicesToProcess, deviceName = cros_config.DetermineDevicesToProcess(filepath.Join(cros_config.TastDir, "device_name.txt"), s)
	...
	cros_config.CompareGoldenOutput(goldenOutput, filepath.Join(cros_config.TastDir, deviceName+"_golden_db.json"), s)
}
