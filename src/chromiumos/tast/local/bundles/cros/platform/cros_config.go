...
func CrosConfig(ctx context.Context, s *testing.State) {
	deviceName, err := crosconfig.DetermineDeviceToProcess(ctx, s)
	if err != nil {
		s.Fatal("Failed to determine device", err)
	}
	...
	commandsToRun, err := crosconfig.BuildCommands(ctx, deviceName, s)
	if err != nil {
	...
	for _, c := range commandsToRun {
		...
		out, err := c.Output()
		if err != nil {
			s.Errorf("Failed to run %q %q err=%q", c.Cmd.Path, c.Cmd.Args, err)
			c.DumpLog(ctx)
		}
