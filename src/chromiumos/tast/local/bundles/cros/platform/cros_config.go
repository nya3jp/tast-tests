...
func CrosConfig(ctx context.Context, s *testing.State) {
	...
	for _, c := range commandsToRun {
		...
		recs = append(recs, goldenRecord{key, strings.TrimSpace(string(out))})
	}

	sort.Slice(recs, func(i, j int) bool { return recs[i].CommandKey < recs[j].CommandKey })
	goldenOutput := goldenFile{deviceName, recs}
...
func buildCommands(ctx context.Context, cmdPath string, deviceFilter string) ([]*testexec.Cmd, error) {
	...
	if err = json.Unmarshal(b, &commandRecs); err != nil {
		return nil, err
	}
...
func buildRecordsMap(recs []goldenRecord) (map[string]string, []string) {
...
func compareGoldenOutput(s *testing.State, output goldenFile, goldenFilename string) {
