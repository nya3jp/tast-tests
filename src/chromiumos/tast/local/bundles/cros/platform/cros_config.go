...
func CrosConfig(ctx context.Context, s *testing.State) {
	devicesToProcess, deviceName := crosconfig.DetermineDevicesToProcess(
		filepath.Join(crosconfig.TastDir, "device_name.txt"), s)
	s.Logf("Processing configs for devices: %v, this device %q", devicesToProcess, deviceName)

	// Get all of the commands from the cros_config_common_commands.yaml.
	b, err := ioutil.ReadFile(s.DataPath("cros_config_common_commands.yaml"))
	if err != nil {
		s.Fatal("Failed to read commands data file: ", err)
	}

	// Build a map of all commands to run on the DUT.
	commandsToRun := make(map[string]crosconfig.Command)

	var allCommands []crosconfig.Command = crosconfig.BuildCommands(b, devicesToProcess, s)
	...
	b, err = ioutil.ReadFile(
		filepath.Join(crosconfig.TastDir, "device_specific_commands.yaml"))
	if err != nil {
		s.Log("Failed to read device specific commands data file: ", err)
	}

	var deviceCommands []crosconfig.Command = crosconfig.BuildCommands(b, devicesToProcess, s)
	...
	var outputRecords []crosconfig.GoldenRec
	for k, c := range commandsToRun {
		s.Logf("Running command: %q -> %q", k, c)
		cmd := testexec.CommandContext(ctx, c.Binary, c.Args...)
		out, err := cmd.Output()
		if err != nil {
			s.Errorf("Failed to run %q: %v", strings.Join(cmd.Args, " "), err)
			cmd.DumpLog(ctx)
			// p.Set(testFailures, 1.0)
		}
		trimmedOutput := strings.Trim(string(out), "\n")
		s.Logf("Command output: %q", trimmedOutput)
		rec := crosconfig.GoldenRec{CommandKey: k, Value: trimmedOutput}
	...
	sort.Slice(outputRecords, func(i, j int) bool {
		return outputRecords[i].CommandKey < outputRecords[j].CommandKey
	})
	goldenOutput := crosconfig.GoldenDBFile{DeviceName: deviceName, Records: outputRecords}
	...
	crosconfig.CompareGoldenOutput(
		goldenOutput, filepath.Join(crosconfig.TastDir, deviceName+"_golden_db.json"), s)
}
