...
func CrosConfig(ctx context.Context, s *testing.State) {
	...
	var allCommands []cros_config.Command = cros_config.BuildCommands(
		bytes, devicesToProcess, s)
	...
	bytes, err = ioutil.ReadFile(
		filepath.Join(cros_config.TastDir, "device_specific_commands.yaml"))
	if err != nil {
		s.Log("Failed to read device specific commands data file: ", err)
	}

	var deviceCommands []cros_config.Command = cros_config.BuildCommands(
		bytes, devicesToProcess, s)
