...
func replaceCorePattern(crash_test bool) error {
	corePatternExpr := fmt.Sprintf("|%s --user=%%P:%%s:%%u:%%g:%%e",
		platform_crash.CrashReporterPath)

	if crash_test {
		corePatternExpr = corePatternExpr + " --crash_test"
	}

	if err := ioutil.WriteFile(platform_crash.CorePattern,
		[]byte(corePatternExpr), 0644); err != nil {
		return errors.Wrapf(err, "failed writing core pattern file %s",
			platform_crash.CorePattern)
	}
	return nil
}
...
func CrashReporterCrash(ctx context.Context, s *testing.State) {
	...
	if err := metrics.SetConsent(ctx, s.DataPath(platform_crash.TestCert),
		true); err != nil {
	...
	if err != nil || !f.Mode().IsRegular() {
		s.Error("Crash reporter enabled file flag is not present at ",
			crashReporterEnabledPath)
		return
	}
	...
	// Intentionally crash metrics_daeon so that (doomed) crash_reporter will
	// be called.
