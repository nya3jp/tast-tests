...
// EnsureTlsdatedStopped ensures that tlsdated is in a valid state and then
// stops tlsdated.
func (*BootPerfService) EnsureTlsdatedStopped(ctx context.Context, _ *empty.Empty) (*empty.Empty, error) {
	...
	// Before stopping tlsdated, ensure tlsdates and the sync-rtc bootstat files are in a valid state.
	if err := bootperf.EnsureTlsdatedStateBeforeReboot(ctx); err != nil {
		return nil, err
	}
