...
func logOutput(s *testing.State, logFilename, cmd, stdout, stderr string) {
	logf, err := os.OpenFile(filepath.Join(s.OutDir(), logFilename), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		s.Error("Failed to create logfile: ", err)
	}
	defer logf.Close()
