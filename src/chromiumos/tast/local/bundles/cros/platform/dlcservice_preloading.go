...
func DLCServicePreloading(ctx context.Context, s *testing.State) {
	...
	if err := dlc.Cleanup(ctx, dlc.IDPackage{dlc.TestID2, dlc.TestPackage}); err != nil {
	...
	// Place test DLC into the preload path.
	preloadIDPath := filepath.Join(dlc.PreloadDir, dlc.TestID2)
	preloadIDPackagePath := filepath.Join(preloadIDPath, dlc.TestPackage)
	if err := os.MkdirAll(preloadIDPackagePath, 0755); err != nil {
		s.Fatal("Failed to make preload directory: ", err)
	}
	defer os.RemoveAll(preloadIDPath)
	b, err := ioutil.ReadFile(filepath.Join(dlc.TestDir, dlc.TestID2, dlc.TestPackage, dlc.ImageFile))
	if err != nil {
		s.Fatal("Test image file is missing: ", err)
	}
	if err := ioutil.WriteFile(filepath.Join(preloadIDPackagePath, dlc.ImageFile), b, 0666); err != nil {
		s.Fatal("Writing to preload path failed: ", err)
	}
