...
func DLCServicePreloading(ctx context.Context, s *testing.State) {
	...
	convertInstalledToPreloaded := func(dlcID, dlcPackage string) {
		manifest, err := dlc.ReadImageloaderManifest(ctx, s, dlcID, dlcPackage)
		if err != nil {
	...
	path := filepath.Join(s.OutDir(), "list.log")
	dlcListOutputs := dlc.GetInstalled(ctx, s, path)
	...
	dlc.Install(ctx, s, dummyID, "")
	dlc.DumpAndVerifyInstalledDLCs(ctx, s, "install_preload_allowed", dummyID)
}
