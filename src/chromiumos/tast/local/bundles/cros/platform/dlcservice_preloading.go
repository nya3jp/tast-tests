...
import (
	...
	"chromiumos/tast/local/bundles/cros/platform/dlctest"
...
func DLCServicePreloading(ctx context.Context, s *testing.State) {
	...
	if err := dlctest.CopyFileAndChangePermissions(filepath.Join(dlctest.TestDir, dlctest.TestID2, dlctest.TestPackage, dlctest.ImageFile),
