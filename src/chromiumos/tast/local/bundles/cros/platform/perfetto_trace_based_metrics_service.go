...
func (*PerfettoTraceBasedMetricsService) GeneratePerfettoTrace(req *platform.GeneratePerfettoTraceRequest, sender platform.PerfettoTraceBasedMetricsService_GeneratePerfettoTraceServer) error {
	...
	index := 0
	recvSize := 100 * 1024 // Send |result| per 100 KB
	resultLen := len(result)
	for {
		if index+recvSize <= resultLen {
			return sender.Send(&platform.GeneratePerfettoTraceResponse{Result: result[index:]})
		}

		if err := sender.Send(&platform.GeneratePerfettoTraceResponse{Result: result[index : index+recvSize]}); err != nil {
			return err
		}

		index = index + recvSize
	}
}
