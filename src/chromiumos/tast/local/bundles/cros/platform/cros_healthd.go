...
func CrosHealthd(ctx context.Context, s *testing.State) {
	...
	if lineCount < 2 {
		...
		// The output of this command is just a CSV with a line of headers. So,
		// the theory here is that every board should have at least one
		// non-removable block device which should show up in this list, so the
		// number of lines should be at least 2.
