...
// CheckCrossystem checks that the "crossystem" command's basic functionality.
...
func CheckCrossystem(ctx context.Context, s *testing.State) {
	const (
		...
		hexNum        = `^0x[\da-fA-F]+$`
		bit           = `^[01]$`
		anything      = "" // match everything that isn't an error and empty
	)
	...
	checkOutput := func(pattern string, out []byte) bool {
		...
		return regexp.MustCompile(pattern).Match(out)
	}
