...
import (
	...
	"chromiumos/tast/local/bundles/cros/platform/osmeter"
...
// stddev returns the standard deviation of |values|.
...
func getActiveTabID(ctx context.Context, cr *chrome.Chrome) (int, error) {
	...
	var tabID int
	err := evalPromiseBodyInBrowser(ctx, cr, promiseBody, &tabID)
	if err != nil {
...
func addTab(ctx context.Context, cr *chrome.Chrome, url, isDormantExpr string) (*renderer, error) {
	...
	var isDormant bool
	err = r.conn.Eval(ctx, isDormantExpr, &isDormant)
	if err != nil {
		return nil, err
	}
...
func activateTab(ctx context.Context, cr *chrome.Chrome, tabID int, state *testState) error {
	...
	err := execPromiseBodyInBrowser(ctx, cr, code)
	if err != nil {
		return err
	}
	startTime := time.Now()
	r := state.renderers[tabID]
	err = execPromiseBody(ctx, r.conn, `
...
`)
	...
	state.perfValues.Append(state.tabSwitchMetric, switchTimeMs)
	state.tabSwitchTimesMs = append(state.tabSwitchTimesMs, switchTimeMs)
	return nil
}
...
func getValidTabIDs(ctx context.Context, cr *chrome.Chrome) ([]int, error) {
	var out []int
	err := evalPromiseBodyInBrowser(ctx, cr, `
...
func googleLogIn(ctx context.Context, cr *chrome.Chrome) error {
	...
	// Enter password.
	err = emulateTyping(ctx, cr, loginTab, "google.memory.chrome")
	if err != nil {
...
func wiggleTab(ctx context.Context, r *renderer) error {
	...
	for i := 0; i < scrollCount; i++ {
		if err := r.conn.Exec(ctx, scrollDownCode); err != nil {
			testing.ContextLog(ctx, "Scroll down failed: ", err)
			return err
		}
		lightSleep(ctx, scrollDelay)
	}
	if err := r.conn.Exec(ctx, scrollUpCode); err != nil {
		testing.ContextLog(ctx, "Scroll up failed: ", err)
		return err
	}
	lightSleep(ctx, scrollDelay)
	return nil
...
func initBrowser(ctx context.Context, useLiveSites bool, wprArchivePath string) (*chrome.Chrome, *testexec.Cmd, error) {
	...
	testing.ContextLogf(ctx, "WPR is up and running on %d", httpSocketName)
	...
	testing.ContextLogf(ctx, "WPR is up and running on %d", httpsSocketName)
	cr := tentativeCr
	tentativeCr = nil
	wpr := tentativeWPR
	tentativeWPR = nil
...
func MemoryPressure(ctx context.Context, s *testing.State) {
	...
	// Create and start the page fault meter.  This starts a goroutine
	// which ends upon ctx.Done().
	pfm := osmeter.New(ctx)
	go pfm.PageFaultMeterRun()
	...
	state.tabSwitchMetric = perf.Metric{
	...
	state.perfValues.Set(openedTabsMetric, float64(len(newRenderers)))
	lostTabs := len(newRenderers) + initialTabCount - len(validTabIDs)
	state.perfValues.Set(lostTabsMetric, float64(lostTabs))
	totalPageFaultCount, averagePageFaultRate, maxPageFaultRate := pfm.PageFaultMeterGetStats(ctx)
	state.perfValues.Set(totalPageFaultCount1Metric, float64(totalPageFaultCount))
	state.perfValues.Set(averagePageFaultRate1Metric, averagePageFaultRate)
	state.perfValues.Set(maxPageFaultRate1Metric, maxPageFaultRate)
	...
	state.perfValues.Set(totalPageFaultCount2Metric, float64(totalPageFaultCount))
	state.perfValues.Set(averagePageFaultRate2Metric, averagePageFaultRate)
	state.perfValues.Set(maxPageFaultRate2Metric, maxPageFaultRate)
