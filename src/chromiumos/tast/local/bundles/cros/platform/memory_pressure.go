...
func shrinkMemoryDownTo(ctx context.Context, snarferScript string,
	...
	if fillMB <= 0 {
		testing.ContextLogf(ctx, "Ignoring negative/zero fill %d MB", fillMB)
		return nil, nil
	}
	allocMB := int((float64(fillMB)) / ratio)
	...
	// sorry.)
	if allocMB > swapMB {
		allocMB = swapMB + (fillMB - int(float64(swapMB)*ratio))
	}
...
func MemoryPressure(ctx context.Context, s *testing.State) {
	const (
		...
		postShrinkGB         = 2.4           // try to shrink RAM down to this size
	...
	s.Log("Snarfing RAM")
	// Please see comments in data/snarfer.sh for details.
