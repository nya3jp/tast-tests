...
// evalPromiseBodyInBrowser performs as above, but execute the promise in the browser.
...
// execPromiseBody connects to Chrome and executes a JS promise
...
func addTabFromList(ctx context.Context, s *testing.State,
	...
	err = testing.Poll(ctx, func(ctx context.Context) error {
		hasQuiesced := false
		err := evalPromiseBody(ctx, s, tab.conn, quiescenceCode, &hasQuiesced)
		if err != nil {
			return err
		}
		if !hasQuiesced {
	...
	// Tolerate all timeout errors.
	if err != nil && errors.Cause(err) == errors.Cause(notQuiescedError) {
		s.Log("ignoring quiesce timeout")
		err = nil
	}
	if err != nil && errors.Cause(err) == errors.Cause(context.DeadlineExceeded) {
		s.Log(notQuiescedMsg)
		s.Log("ignoring timeout: ", err)
	}
	return tab, err
}
