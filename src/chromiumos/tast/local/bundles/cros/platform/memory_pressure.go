...
`)
	...
	s.Logf("tab switch time for tab %d: %v", tabID, time.Now().Sub(startTime))
	return nil
}
...
func wiggleTab(ctx context.Context, s *testing.State, r *renderer) {
	...
	for i := 0; i < scrollCount; i++ {
		if err := r.conn.Exec(ctx, "window.scrollBy(0, 100)"); err != nil {
...
// renderers maps a tab ID to its renderer struct.  The initial tab is not
// mapped here.
...
func MemoryPressure(ctx context.Context, s *testing.State) {
	...
	if useLiveSites {
		...
		// |newTabDelay| to a large number, like 1 minute.
	...
	s.Logf("Metrics: discarded tab count %v",
		len(openedTabs)+initialTabCount-len(validTabIDs))
