...
func MemoryPressure(ctx context.Context, s *testing.State) {
	...
	// Pin the initial tabs so that we can use them in a later phase
	// without worrying whether they get discarded.
	for _, id := range state.tabIDs {
		pinCode := fmt.Sprintf("chrome.tabs.update(%d, {pinned: true})", id)
		tconn, err := cr.TestAPIConn(ctx)
		if err != nil {
			s.Error("Cannot get test connection: ", err)
		}
		if err := tconn.Exec(ctx, pinCode); err != nil {
			s.Errorf("Cannot pin tab %d: %v", id, err)
		}
	}
	...
	stats, err := kernelMeter.PageFaultStats(ctx)
	if err != nil {
	...
	cycleTabs(ctx, cr, state, state.tabIDs[0:tabWorkingSetSize], false)
