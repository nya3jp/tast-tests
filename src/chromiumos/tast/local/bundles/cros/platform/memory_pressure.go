...
func wrapAsPromiseForTab(code string, tab int) string {
	body := fmt.Sprintf(`chrome.tabs.executeScript(%d, {code: "%s"}, resolve() => {})`,
		tab, code)
	return wrapAsPromiseBody(body)
}
...
func injectKeyboardHelper(ctx context.Context, s *testing.State, cr *chrome.Chrome, tab int) error {
	functionDefinition := `
function sendKbEvent(element, keyCode){
	...
	promiseBody := wrapAsPromiseForTab(functionDefinition, tab)
	return evalVoidPromiseBody(ctx, s, cr, promiseBody)
}
...
func emulateTyping(ctx context.Context, s *testing.State, cr *chrome.Chrome,
	...
	for i := 0; i < len(text); i++ {
		c := text[i]
		kc := keyCode(c)
		call := fmt.Sprintf("sendKbEvent(document.body, %d)", c, kc)
		code := wrapAsPromiseForTab(call, tabId)
		err := evalVoidPromiseBody(ctx, s, cr, code)
		if err != nil {
...
func googleLogIn(ctx context.Context, s *testing.State, cr *chrome.Chrome) error {
	...
	promiseBody := fmt.Sprintf(`chrome.tabs.executeScript(%d, {code: "%s"}, resolve() => {})`,
		loginTab, queryCode)
	...
	err = injectKeyboardHelper(ctx, s, cr, loginTab)
	if err != nil {
		return errors.Wrap(err, "cannot inject keyboard helper")
	}
