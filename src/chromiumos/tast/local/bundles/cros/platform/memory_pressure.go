...
func emulateTyping(ctx context.Context, s *testing.State, conn *chrome.Conn,
	text string) error {
		...
		code := fmt.Sprintf(`
{
    ...
    resolve();
		...
		err := execPromiseBody(ctx, s, conn, code)
		time.Sleep(50 * time.Millisecond)
		if err != nil {
			return errors.Wrapf(err, "cannot send key %d (keycode %d)", c, kc)
		}
	}
	return nil
}
...
func googleLogIn(ctx context.Context, s *testing.State, cr *chrome.Chrome) error {
	...
	// Enter email.
	err = emulateTyping(ctx, s, loginTab.conn, "wpr.memory.pressure.test@gmail.com\n")
	if err != nil {
	...
	// Enter password.
	err = emulateTyping(ctx, s, loginTab.conn, "google.memory.chrome")
	if err != nil {
		return errors.Wrap(err, "cannot enter password")
	}
	s.Log("password entered")
	time.Sleep(2 * time.Second)
	err = emulateTyping(ctx, s, loginTab.conn, "\n")
