...
// Sleep that can be interrupted by the test timeout.
...
// Remove all files in |dirname|.  Assume only simple files (i.e. no recursive
// directory removal).
...
func MemoryPressure(ctx context.Context, s *testing.State) {
	...
	err = removeAllFiles("/home/chronos/user/Cache")
	if err != nil {
