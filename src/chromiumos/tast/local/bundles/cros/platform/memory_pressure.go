...
func addTabFromList(ctx context.Context, s *testing.State,
	...
	// TODO(semenzato): only ignore timeouts, and report other errors.
	// Also we need an API that allows changing the timeout.
	if err := tab.conn.WaitForExpr(ctx, quiescenceCode); err != nil {
		s.Log("ignoring WaitForExpr error: ", err)
	}
	return tab, err
}
