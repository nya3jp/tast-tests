...
// availableTCPPort return a list of TCP ports on localhost that are not in
...
// ends (successfully or not).  If the returned error is not nil, the
...
func initBrowser(ctx context.Context, useLiveSites bool, wprArchivePath string) (*chrome.Chrome, *testexec.Cmd, error) {
	...
	// recommended) and the call to initBroser should be updated to reflect
	// that location.
