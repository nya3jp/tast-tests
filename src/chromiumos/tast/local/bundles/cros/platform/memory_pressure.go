...
// reunderer represents a Chrome renderer creaded by devtools.  Such renderer is
...
func MemoryPressure(ctx context.Context, s *testing.State) {
	...
	s.Logf("Metrics: Phase 2: mean tab switch time %v", mean(state.tabSwitchTimes))
	s.Logf("Metrics: Phase 2: stddev of tab switch times %v", stdDev(state.tabSwitchTimes))
