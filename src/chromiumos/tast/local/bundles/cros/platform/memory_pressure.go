...
const (
	wprArchiveName = "mixed_sites.wprgo"
)
...
func MemoryPressure(ctx context.Context, s *testing.State) {
	const (
		logIn             = false
		tabWorkingSetSize = 5
		newTabDelay       = 3 * time.Second
		tabCycleDelay     = 300 * time.Millisecond
	)

	// Start the Web Page Replay in replay mode.
	//
	// NOTE: this test can also be used to record a page set with WPR.  To
	// do that, change "replay" to "record" below, set |wprArchivePath| to
	// a file of your choice, and change |newTabDelay| to a much larger
	// number, like 1 minute.
	wprArchivePath := s.DataPath(wprArchiveName)
	s.Logf(wprArchivePath)
