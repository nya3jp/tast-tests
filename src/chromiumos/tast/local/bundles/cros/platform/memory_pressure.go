...
func runTabSwitches(ctx context.Context, cr *chrome.Chrome, data *testData,
	...
	testing.ContextLogf(ctx, "Metrics: %s: mean/stddev of switch times for all tabs: %v %v",
		label, mean(times).Seconds(), stdDev(times).Seconds())
	...
	for i, times := range tabTimes {
		testing.ContextLogf(ctx, "Metrics: %s: mean/stdev switch time for tab index %d: %f %f",
			label, i, mean(times).Seconds(), stdDev(times).Seconds())
	}
	return nil
}
...
func MemoryPressure(ctx context.Context, s *testing.State) {
	...
	s.Logf("Metrics: Phase 1: opened tab count %v", len(data.tabIDs))
