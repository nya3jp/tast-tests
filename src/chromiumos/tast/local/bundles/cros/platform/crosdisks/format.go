...
func testVolumeFormat(ctx context.Context, cd *crosdisks.CrosDisks, ld *crosdisks.LoopbackDevice, fsType string) error {
	...
	if actualLabel.Signature() != dbus.SignatureOfType(reflect.TypeOf("")) || actualLabel.Value() != label {
		return errors.Errorf("incorrect filesystem label: got %q; want %q", actualLabel.String(), label)
	}
	return nil
}
...
func RunFormatTests(ctx context.Context, s *testing.State) {
	...
	for _, fsType := range []string{"vfat", "exfat", "ntfs"} {
		s.Run(ctx, fsType, func(ctx context.Context, state *testing.State) {
