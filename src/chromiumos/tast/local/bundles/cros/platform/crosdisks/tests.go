...
func verifyProp(prop map[string]dbus.Variant) []error {
	...
	if df, ok := prop["DeviceFile"].Value().(string); !ok {
		return []error{errors.New("unexpected type for DeviceFile")}
	} else if df == "" {
	...
	if !ok {
		return []error{errors.New("unexpected type for DeviceMountPaths")}
	}

	mounted, ok := prop["DeviceIsMounted"].Value().(bool)
	if !ok {
		return []error{errors.New("unexpected type for DeviceIsMounted")}
	}
	if mounted {
		if len(paths) == 0 {
...
func testMountBootDeviceRejected(ctx context.Context, s *testing.State, cd *crosDisks) {
	...
	if m.status != mountErrorInvalidDevicePath {
		s.Errorf("Unexpected MountCompleted status code: got %u; want %u", m.status, mountErrorInvalidDevicePath)
	}
	if m.sourcePath != dev {
...
func testUnmountNonExistentDevice(ctx context.Context, s *testing.State, cd *crosDisks) {
	...
	if status != mountErrorPathNotMounted {
		s.Errorf("Unexpected Unmount status code: got %u; want %u", status, mountErrorPathNotMounted)
	}
}
