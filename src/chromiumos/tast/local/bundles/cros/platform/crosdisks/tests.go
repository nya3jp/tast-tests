...
func verifyProp(prop map[string]dbus.Variant) error {
	...
	for _, f := range []field{
		...
		if v, ok := prop[f.name]; !ok {
			errs = append(errs, fmt.Sprintf("%s not found in the property.", f.name))
		} else if v.Signature().String() != f.signature {
			errs = append(errs, fmt.Sprintf("unexpected signature for %s: got %s; want %s", f.name, v.Signature().String(), f.signature))
		}
	}
	if len(errs) > 0 {
	...
	if df, ok := prop["DeviceFile"].Value().(string); !ok {
		return errors.New("unexpected type for DeviceFile")
	} else if df == "" {
		return errors.New("deviceFile is empty")
	}

	// Check if the values of DeviceIsMounted and DeviceMountPaths are
	// consistent.
	paths, ok := prop["DeviceMountPaths"].Value().([]string)
	if !ok {
		return errors.New("unexpected type for DeviceMountPaths")
	}

	mounted, ok := prop["DeviceIsMounted"].Value().(bool)
	if !ok {
		return errors.New("unexpected type for DeviceIsMounted")
	}
	if mounted {
		if len(paths) == 0 {
			return errors.New("prop.DeviceMountPaths should not be empty if prop.DeviceIsMounted is true")
		}
	} else {
		if len(paths) != 0 {
			return errors.New("prop.DeviceMountPaths should be empty if prop.DeviceIsMounted is false")
		}
	}

	for _, path := range paths {
		if path == "" {
			return errors.New("prop.DeviceMountPaths should not contain any empty string")
		}
	}
...
func testEnumerateDevices(ctx context.Context, s *testing.State, cd *crosDisks) {
	...
	for _, d := range ds {
		...
		if err = verifyProp(prop); err != nil {
...
func testMountNonExistentDevice(ctx context.Context, s *testing.State, cd *crosDisks) {
	...
	if m.sourcePath != path {
		s.Errorf("Unexpected source_path: got %s; want %s", m.sourcePath, path)
	}
	if m.mountPath != "" {
		s.Errorf("Unexpected mount_path: got %s; want\"\"", m.mountPath)
	}
}
...
func testMountBootDeviceRejected(ctx context.Context, s *testing.State, cd *crosDisks) {
	...
	if m.status != mountErrorInvalidDevicePath {
		s.Errorf("Unexpected error code: got %u; want %u", m.status, mountErrorInvalidDevicePath)
	}
	if m.sourcePath != dev {
		s.Errorf("Unexpected source_path: got %s; want %s", m.sourcePath, dev)
	}
	if m.mountPath != "" {
		s.Errorf("Unexpected mount_path: got %s; want\"\"", m.mountPath)
	}
...
func testUnmountNonExistentDevice(ctx context.Context, s *testing.State, cd *crosDisks) {
	...
	if status != mountErrorPathNotMounted {
		s.Errorf("Unexpected status code: got %u; want %u", status, mountErrorPathNotMounted)
	}
}
