...
// withMountDo mounts the specified source and if it succeeds calls the provided function, cleaning up mount afterwards.
...
// listDirectoryRecursively list all files in a directory and subdirectories.
func listDirectoryRecursively(rootDir string) (items DirectoryContents, err error) {
	...
	for len(dirs) > 0 {
		dir := dirs[0]
		dirs = dirs[1:]
		entries, err := ioutil.ReadDir(path.Join(rootDir, dir))
		if err != nil {
			return nil, err
		}
		if len(entries) == 0 {
			...
			for _, entry := range entries {
				...
				if entry.IsDir() {
					...
					s, err := os.Stat(path.Join(rootDir, relPath))
					if err != nil {
						return nil, err
					}
...
func verifyThatKeysMatch(ctx context.Context, actual, expected DirectoryContents) (err error) {
	...
	if len(extra) > 0 || len(missing) > 0 {
		err = errors.Errorf("condition failed: %d extra and %d missing elements in map", len(extra), len(missing))
	}
	return
}
...
func verifyDirectoryContents(ctx context.Context, dir string, expectedContent DirectoryContents) error {
	...
	for k, v := range expectedContent {
		...
		if v.Data != nil {
			data, err := ioutil.ReadFile(path.Join(dir, k))
			if err != nil {
