...
func batteryComponents(ctx context.Context) ([]*rppb.Battery, error) {
	...
	userObj, err := user.Lookup(authedUser)
	if err != nil {
		return nil, err
	}

	authedUIDString := userObj.Uid
	authedUID, err := strconv.Atoi(authedUIDString)
	if err != nil {
		return nil, err
	}
	...
	if err := conn.Auth([]dbus.Auth{dbus.AuthExternal(authedUIDString)}); err != nil {
		return nil, err
	}

	if err := syscall.Setreuid(-1, rootUID); err != nil {
		return nil, err
	}
	...
	if err := dbusutil.WaitForService(ctx, conn, dbusName); err != nil {
		return nil, errors.Wrapf(err, "failed waiting for service %s", dbusName)
	}
