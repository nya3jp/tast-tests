...
func parseLSBRelease(path string) (appID string, relVersion string, err error) {
	...
	for _, val := range strings.Split(string(b), "\n") {
		if parts := strings.SplitN(val, "=", 2); len(parts) == 2 {
			if parts[0] == "CHROMEOS_RELEASE_APPID" {
				appID = parts[1]
			} else if parts[0] == "CHROMEOS_RELEASE_VERSION" {
				relVersion = parts[1]
			}
		}
	}
	return appID, relVersion, nil
}

// Response struct contains response parameters.
...
func New(ctx context.Context, dlcModuleID string) (*httptest.Server, error) {
	...
	response := Response{
		...
		DLCModuleID: dlcModuleID,
	}

	// Constructs response.
	t, err := template.New("resp").Parse(responseTmpl)
	if err != nil {
		return nil, err
	}
