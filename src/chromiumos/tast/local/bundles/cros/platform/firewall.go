...
func Firewall(ctx context.Context, s *testing.State) {
	...
	// Call PermissionBroker's method with its arguments, raise an error if the
	// method returns false.
	checkCall := func(method string, args ...interface{}) {
		if call(method, args...) == false {
	...
	// Call a no-side-effect method on PermissionBroker to make sure the lifeline fd clean-up happened.
	call("ReleaseTcpPort", uint16(0), "")
