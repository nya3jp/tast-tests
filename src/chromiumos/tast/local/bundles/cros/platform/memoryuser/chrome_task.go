...
func (chromeTask ChromeTask) RunMemoryTask(ctx context.Context, s *testing.State, cr *chrome.Chrome) {
	for i := 0; i < chromeTask.NumTabs; i++ {
		index := i % len(chromeTask.Urls)
		conn, err := cr.NewConn(ctx, chromeTask.Urls[index])
		if err != nil {
