...
func logCmd(ctx context.Context, outfile, cmd string, args ...string) {
	...
	if err != nil {
		testing.ContextLog(ctx, "Failed to create log file %s: %v", outfile, err)
	}
	defer file.Close()
	for {
		select {
		...
		case <-time.After(5 * time.Second):
			...
			if err != nil {
				testing.ContextLog(ctx, "Command %s failed: %v", cmd, err)
			}
...
func RunTest(ctx context.Context, s *testing.State, tasks []MemoryTask) {
	...
	for _, task := range tasks {
		go func(task MemoryTask) {
			if err := task.Run(ctx, testEnv); err != nil {
				s.Error("Failed to run memory task %s: %v", task.String(), err)
			}
			ch <- struct{}{}
		}(task)
