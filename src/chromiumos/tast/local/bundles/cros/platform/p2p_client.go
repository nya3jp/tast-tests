...
func newFakeP2PServer(port int, numConn int, files map[string]int) (*fakeP2PServer, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to create a service")
	}

	server, err := mdns.NewServer(&mdns.Config{Zone: service})
	if err != nil {
		return nil, errors.Wrap(err, "failed to start a server")
	}

	s := &fakeP2PServer{server, service, numConn, files}
	return s, nil
}
...
func P2PClient(fullCtx context.Context, s *testing.State) {
	...
	// Wait for all fake servers to be recognized by avahi.
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		for _, port := range []int{port1, port2, port3} {
			if !strings.Contains(s, fmt.Sprintf("port %d", port)) {
				return errors.Errorf("fake server at port %d is not recognized", port)
			}
		}
		return nil
	}, &testing.PollOptions{Interval: time.Second}); err != nil {
		s.Fatal("Failed to wait for fake servers to be recognized by avahi: ", err)
	}
	...
	if err := cmd.Run(); err == nil {
		cmd.DumpLog(ctx)
		s.Error("p2p-client succeeded to query only-b; expected to fail")
	}
