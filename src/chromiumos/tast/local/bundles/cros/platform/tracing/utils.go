...
func (sess *Session) RunMetrics(ctx context.Context, traceProcessorPath string, metrics []string) (*perfetto_protos.TraceMetrics, error) {
...
func StartTracing(ctx context.Context, configFile string) (*Session, error) {
...
// CollectTrace collects a system-wide trace using the perfetto
// command line tool.
// On success, returns the temporary file of the trace data. It's the
// caller's responsibility for removing it if it's no longer needed.
func CollectTrace(ctx context.Context, configFile string) (*Session, error) {
