...
func DLCService(ctx context.Context, s *testing.State) {
	...
	cmdRunChecker := func(cmd *testexec.Cmd, e expect, msg string) {
		if err := cmd.Run(); err != nil && e {
			s.Fatal("Failed to ", msg, err)
		} else if err == nil && !e {
			s.Fatal("Should have failed to ", msg, err)
		}
	}
	...
	dumpInstalledDLCModules("08_install_bad_dlc")
}
