...
func DLCService(ctx context.Context, s *testing.State) {
	...
	type dlcListOutput struct {
		DlcManifest      string `json:"dlc_manifest"`
		DlcRoot          string `json:"dlc_root"`
	...
	outputCmd := func(name string, args ...string) []byte {
		cmd := testexec.CommandContext(ctx, name, args...)
		output, err := cmd.Output(testexec.DumpLogOnError)
		if err != nil {
			s.Fatal("Failed run command ouput: ", err)
		}
		return output
	}

	dlcList := func() (output map[string]dlcListOutput) {
		bytes := outputCmd("sudo", "-u", "chronos", "dlcservice_util", "--list", "--json")
	...
	getRootMount := func(dlc string) string {
		if val, ok := dlcList()[dlc]; ok {
			return val.DlcRoot
		}
		return ""
	}

	verifyDlcContent := func(dlc string) {
	...
	s.Run(ctx, "Multi DLC combination tests", func(context.Context, *testing.State) {
		...
		verifyDlcContent(dlcID1)
		verifyDlcContent(dlcID2)
