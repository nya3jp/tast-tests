...
func DLCService(ctx context.Context, s *testing.State) {
	...
	defer func() {
		...
		if err != nil {
			...
			s.Error("Failed to clean up DLC cache directory: ", err)
		}
		for _, f := range d {
	...
	{
		// Only serves dlcID1.
		srv, err := updateserver.New(ctx, dlcID1)
		if err != nil {
	...
	{
		// Serves dlcID1 and dlcID2.
		srv, err := updateserver.New(ctx, dlcID1, dlcID2)
		if err != nil {
			s.Fatal("Failed to start update server: ", err)
		}
		defer srv.Close()
