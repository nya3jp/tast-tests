...
func DLCService(ctx context.Context, s *testing.State) {
	...
	for _, p := range []string{"rollback-version", "rollback-happened"} {
		prefsPath := filepath.Join(dlc.PowerwashSafeDir, updateEnginePowerwashSafePrefsSubDir, p)
	...
	if err := dlc.Cleanup(ctx, dlc.IDPackage{dlc.TestID1, dlc.TestPackage}); err != nil {
	...
	s.Run(ctx, "Mimic device reboot tests", func(ctx context.Context, s *testing.State) {
		...
		dlc.RestartUpstartJob(ctx, dlc.JobName, dlc.ServiceName)
