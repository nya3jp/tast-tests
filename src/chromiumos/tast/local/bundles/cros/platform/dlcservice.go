...
func DLCService(ctx context.Context, s *testing.State) {
	...
	stopNebraska := func(cmd *testexec.Cmd) {
		...
		if nebraskaLog, err := ioutil.ReadFile("/tmp/nebraska.log"); err != nil {
			s.Error("Nebraska log does not exist: ", err)
		} else {
			f, err := os.OpenFile(filepath.Join(s.OutDir(), "nebraska.log"), os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
			if err != nil {
				s.Error("Failed to create/open file: ", err)
			}
			defer f.Close()
			if _, err := f.Write(nebraskaLog); err != nil {
				s.Error("Failed to append to nebraska log: ", err)
			}
		}
	}
