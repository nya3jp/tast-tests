...
func DLCService(ctx context.Context, s *testing.State) {
	...
	s.Run(ctx, "Single DLC combination tests", func(context.Context, *testing.State) {
		func() {
			...
			// Uninstall single DLC before installing.
			uninstall(dlcID1, success)
			dumpAndVerifyInstalledDLCs("uninstall_dlc_before_installing")

			// Install single DLC from Nebraska/Omaha.
			install(dlcID1, url, success)
			dumpAndVerifyInstalledDLCs("install_single_from_url", dlcID1)
		}()
