...
func DevCoredump(ctx context.Context, s *testing.State) {
	...
	// This test calls RemoveInProgressCrashDir because it is designed to test device coredump
	// handling on developer images. The presence of the /run/crash_reporter directory causes
	// the DUT to behave as if it were running a base image and thus no .devcore files would
	// be created if we did not remove it.
	if err := localCrash.RemoveInProgressCrashDir(crashTestInProgressDir); err != nil {
		s.Fatal("RemoveInProgressCrashDir failed: ", err)
	}
