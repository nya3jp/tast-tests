...
// getRootMounts retrives the root mounts for the given id.
...
func checkSHA2Sum(hashPath string) error {
	...
	if actualSum != expectedSum {
		return errors.Errorf("SHA2 checksum do not match for %s. Actual=%s Expected=%s", path, actualSum, expectedSum)
	}
	return nil
}
...
func checkPerms(parmsPath string) error {
	...
	if actualPerm != expectedPerm {
		return errors.Errorf("Permissions do not match for %s. Actual=%s Expected=%s", path, actualPerm, expectedPerm)
	}
	return nil
}
