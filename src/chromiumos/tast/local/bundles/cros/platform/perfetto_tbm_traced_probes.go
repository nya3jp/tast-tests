...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:     []string{perfetto.TBMTracedProbesConfigFile, perfetto.TraceProcessorAmd64, perfetto.TraceProcessorArm, perfetto.TraceProcessorArm64},
...
func PerfettoTBMTracedProbes(ctx context.Context, s *testing.State) {
	...
	var traceProcessor string
	switch runtime.GOARCH {
	case "arm":
		traceProcessor = perfetto.TraceProcessorArm
	case "arm64":
		traceProcessor = perfetto.TraceProcessorArm64
	default:
		traceProcessor = perfetto.TraceProcessorAmd64
	}

	metrics, err := perfetto.RunMetrics(ctx, s.DataPath(traceProcessor), traceOutputFile.Name(), []string{traceMetricCPU, traceMetricMEM})
	if err != nil {
