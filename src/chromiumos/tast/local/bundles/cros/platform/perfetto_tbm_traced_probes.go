...
func PerfettoTBMTracedProbes(ctx context.Context, s *testing.State) {
	...
	PID, traceOutputPath, err := perfetto.StartTraceDataWithConfig(ctx, traceConfigPath)
	if err != nil {
	...
	metrics, err := perfetto.RunMetrics(ctx, traceOutputPath, []string{traceMetricCPU, traceMetricMEM})
	if err != nil {
