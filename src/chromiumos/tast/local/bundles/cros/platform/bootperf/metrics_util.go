...
func logRTCUser(ctx context.Context) {
	...
	// Log the cmdline and stat for the RTC device file user.
	cmdline, err := ioutil.ReadFile(filepath.Join("/proc", m[1], "cmdline"))
	if err != nil {
		testing.ContextLog(ctx, "Failed to get read the cmdline of the RTC user: ", err)
		return
	}
	...
	status, err := ioutil.ReadFile(filepath.Join("/proc", m[1], "stat"))
	if err != nil {
		testing.ContextLog(ctx, "Failed to get read the stat of the RTC user: ", err)
		return
	}

	testing.ContextLogf(ctx, "/dev/rtc user: cmdline: %s, stat: %s", string(cmdline), string(status))
}
