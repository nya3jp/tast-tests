...
//   - sync-rtc-tlsdated-stop is not present
func EnsureTlsdatedStateBeforeReboot(ctx context.Context) error {
	...
	if _, err := os.Stat(syncRtcTlsdatedStartFile); err != nil {
		testing.ContextLogf(ctx, "%s missing, restart tlsdated", syncRtcTlsdatedStartFile)
	}
	...
	// Restarting tlsdated generates syncRtcTlsdatedStop. Remove this one
	_ = os.Remove(filepath.Join(bootstatCurrentDir, syncRtcTlsdatedStop))
