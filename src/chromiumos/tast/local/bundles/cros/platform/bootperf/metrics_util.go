...
// Conversly, subtracting the offset from a measurement in clock source A will
...
func parseSyncRtc(rtcPath string) (float64, float64, int64, error) {
	...
	rtcStr := Ts[2] + " " + Ts[3]
	rtcTime, err := time.Parse(rtcFormat, rtcStr)
	if err != nil {
		return 0, 0, 0, errors.Wrapf(err, "failed in parsing RTC time %s", rtcStr)
	}
	return T0, T1, rtcTime.Unix(), nil
}
...
func GatherRebootMetrics(results *platform.GetBootPerfMetricsResponse) error {
	...
	//  - *RtcTime: seconds since epoch, but optained from an RTC source
