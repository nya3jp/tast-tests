...
func GatherRebootMetrics(results *platform.GetBootPerfMetricsResponse) error {
	...
	if err != nil {
		return errors.Wrap(err, "failed to list bootstat archive directories")
	}
...
func GatherMetricRawDataFiles(raw map[string][]byte) error {
	...
	for _, glob := range []string{uptimeFileGlob, diskFileGlob} {
		...
		if err != nil {
			return errors.Wrapf(err, "failed to list files from glob %s", glob)
		}
...
func GatherConsoleRamoops(raw map[string][]byte) error {
	list, err := filepath.Glob(ramOopsFileGlob)
	if err != nil {
		return errors.Wrap(err, "failed to list console-ramoops files")
	}
