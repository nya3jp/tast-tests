...
func Memd(ctx context.Context, s *testing.State) {
	...
	defer func() {
		// Stop memd if needed, so it will pick up the original margin
		// when it restarts.  (When this code executes, we don't know
		// if memd is running or not.)
		if err = upstart.StopJob(ctx, memdJob); err != nil {
			s.Error("Cannot stop memd: ", err)
		}
		...
		// Restart memd.  Note that upstart.Restart is not the same as
		// 'initcl restart' and tolerates a stopped job, which should
		// be the case here, because of the earlier StopJob.
		if err = upstart.RestartJob(ctx, memdJob); err != nil {
			s.Error("Cannot restart memd: ", err)
		}
	}()

	// Stop memd so that when it restarts it picks up the new margin.
	if err := upstart.StopJob(ctx, memdJob); err != nil {
		s.Fatal("Cannot stop memd: ", err)
	}
	...
	if err = upstart.EnsureJobRunning(ctx, memdJob); err != nil {
