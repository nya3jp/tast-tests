...
func emitDBusSignal(eventType metrics_event.Event_Type, member string) error {
...
func Memd(ctx context.Context, s *testing.State) {
	...
	err = emitDBusSignal(metrics_event.Event_TAB_DISCARD,
		"org.chromium.MetricsEventServiceInterface.ChromeEvent")
	if err != nil {
		s.Fatal("Cannot send fake tab discard signal: ", err)
	}
	// Send a fake OOM-kill notification.
	err = emitDBusSignal(metrics_event.Event_OOM_KILL_KERNEL,
		"org.chromium.AnomalyEventServiceInterface.AnomalyEvent")
	if err != nil {
