...
func sleep(ctx context.Context, s *testing.State, sleepDuration time.Duration, message string) {
	s.Log(message)
	select {
...
func Memd(ctx context.Context, s *testing.State) {
	...
	sleep(ctx, s, 3*time.Second, "waiting for memd to notice pressure")
	...
	sleep(ctx, s, 5100*time.Millisecond, "waiting for memd to dump clip")
