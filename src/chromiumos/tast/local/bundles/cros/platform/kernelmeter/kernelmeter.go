...
// counterToCounterData produces a vmCounterData from a vmCounter.  interval is
// the time delta for computing the average rate.
func (c *vmCounter) counterToCounterData(interval time.Duration) VMCounterData {
...
type VMStatsData struct {
	// PageFaultData reports major page fault count and rates
	PageFaultData VMCounterData
	// SwapInData reports swapin count and rates
	SwapInData VMCounterData
	// SwapOutData reports swapout count and rates
	SwapOutData VMCounterData
	// OomData reports out-of-memory kill count and rates
...
// newVMStatsMeter returns a vmStatsMeter instance, which must be reset before
// it can be used.
...
func (v *vmStatsMeter) stats() (*VMStatsData, error) {
	...
	v.updateCounts()
	return &VMStatsData{
		PageFaultData: v.counters["pgmajfault"].counterToCounterData(interval),
		SwapInData:    v.counters["pswpin"].counterToCounterData(interval),
		SwapOutData:   v.counters["pswpout"].counterToCounterData(interval),
		OomData:       v.counters["oom_kill"].counterToCounterData(interval),
	}, nil
}
