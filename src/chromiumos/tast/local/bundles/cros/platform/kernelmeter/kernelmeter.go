...
// a vmSample.fields vector.
var vmFieldIndices = map[string]int{
	"pgmajfault": pageFaultIndex,
	"pswpin":     swapInIndex,
...
func (v *vmStatsMeter) counterData(field int) VMCounterData {
...
func (v *vmStatsMeter) stats() (*VMStatsData, error) {
	...
	v.acquireSample()
	return &VMStatsData{
		PageFault: v.counterData(pageFaultIndex),
		SwapIn:    v.counterData(swapInIndex),
		SwapOut:   v.counterData(swapOutIndex),
		OOM:       v.counterData(oomKillIndex),
	}, nil
}
