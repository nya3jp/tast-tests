...
// KernelTotalReserve mimics the calculation of totalreserve_pages, which is
// not exported, in calculate_totalreserve_pages().  This number is a
// reasonable approximation (and an upper bound) of the minimum amount of RAM
// which the kernel tries to keep free by reclaiming.
func KernelTotalReserve() (amountMiB uint64, err error) {
	b, err := ioutil.ReadFile("/proc/zoneinfo")
	if err != nil {
		return 0, err
	}
	return kernelTotalReserve(string(b))
}

// kernelTotalReserve is the internal version of KernelTotalReserve, for unit testing.
// s is the content of /proc/zoneinfo.  Result is in MiB.
func kernelTotalReserve(s string) (amountMiB uint64, err error) {
	...
	totalReserve := 0       // total reserve, based on max per-zone reserve
	...
	const pageSize = 4096
	return uint64(totalReserve) * pageSize / (1024 * 1024), nil
}
...
func LogMemoryParameters(ctx context.Context, ratio float64) error {
	...
	// totalReserveMiB is used in the computation of "available" and we'd
	// like to track it.
	totalReserveMiB, err := KernelTotalReserve()
	if err != nil {
		testing.ContextLog(ctx, "Cannot compute total reserve: ", err)
	}
	...
	if marginMiB > totalReserveMiB {
		swapReductionMiB = (marginMiB - totalReserveMiB) * ramWeight
	...
	maxProcessMiB := totalMiB - totalReserveMiB + uint64(float64(usableSwapMiB)*(1-ratio))
	...
	testing.ContextLog(ctx, "Metrics: total reserve: ", totalReserveMiB)
