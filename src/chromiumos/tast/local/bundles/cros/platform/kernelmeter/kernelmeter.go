...
// Close stops the sampling goroutine.  May be called more than once.
func (m *Meter) Close(ctx context.Context) {
	...
	close(m.stop)
	// Wait for the goroutine to finish.  When m.stopped fires, the
	// collected stats are guaranteed to be race-free.
	select {
	case <-m.stopped:
		m.isStopped = true
	case <-ctx.Done():
...
func (m *Meter) PageFaultStats(ctx context.Context) (*PageFaultData, error) {
	if m.hasTimedOut {
		return nil, errors.New("Page fault stats not available on timeout")
	}
	if !m.isStopped {
		panic("Meter has not stopped")
	}
