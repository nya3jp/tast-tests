...
// the kernel tries to maintain.  This calculation mimicks the calculation of
// totalreserve_pages, which is not exported, in calculate_totalreserve_pages()
func KernelMinFree() (uint64, error) {
	...
	for _, line := range strings.Split(string(bytes), "\n") {
		...
		if groups != nil {
			if len(groups) != 2 {
				return 0, errors.New(fmt.Sprintf("unfathomable line in zoneinfo: %q", line))
			}
		...
		if groups != nil {
			if len(groups) != 2 {
				return 0, errors.New(fmt.Sprintf("unbelievable line in zoneinfo: %q", line))
			}
			if zoneMinFree == -1 {
...
// PreSwapMemory returns the approximate amount of memory in MiB that can be
// allocated in the current state before swap begins.
func PreSwapMemory() (preSwapMiB uint64, err error) {
	// Internal calculations are in KiB.
	meminfo, err := memInfo()
	if err != nil {
		return 0, err
	}
	minFile, err := readUint64FromFile("/proc/sys/vm/min_filelist_kbytes")
	if err != nil {
		return 0, err
	}
	freeMem := meminfo["MemFree"]
	// Ignore dirty file memory.
	fileMem := meminfo["Active(file)"] + meminfo["Inactive(file)"] - minFile
	minFree, err := KernelMinFree()
	if err != nil {
		return 0, err
	}
	return (freeMem+fileMem)/1024 - minFree, nil
}
