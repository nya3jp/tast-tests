...
// periodically according to pageFaultMeterSamplePeriod and tracks its max
// value.
...
// max page fault rate.  It will panic if: 1. the kernel meter has not run at
// all; 2. the kernel meter has not been stopped first; 3. the kernel meter has
// not run long enough.
func (km *KernelMeter) PageFaultStats() (faultCount int64, averageRate, maxRate float64) {
	if !km.hasStarted {
	...
	now := time.Now()
	readyTime := km.pfm.startTime.Add(pageFaultMeterSamplePeriod)
	if now.Before(readyTime) {
		panic(fmt.Sprintf("Kernel meter ran for less than %v", pageFaultMeterSamplePeriod))
	}
	// Use millisecond resolution.
	interval := float64(now.Sub(km.pfm.startTime)/time.Millisecond) / 1000.0
	faultCount = totalFaults() - km.pfm.startCount
