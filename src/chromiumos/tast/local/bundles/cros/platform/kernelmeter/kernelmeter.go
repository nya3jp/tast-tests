...
func kernelMinFree(s string) (amountMiB uint64, err error) {
	...
	for _, line := range strings.Split(s, "\n") {
		...
		if groups != nil {
			high, err := strconv.Atoi(groups[1])
			if err != nil {
		...
		if groups != nil {
			...
			totalMinFree += zoneMinFree + max
			zoneMinFree = 0
		}
	}

	// Result needs conversion from pages to MiB.
	const pageSize = 4096
...
func readMemInfo() (map[string]uint64, error) {
	...
	for _, line := range lines[:len(lines)-1] {
		...
		if len(fields) != 3 {
			return nil, errors.Errorf("unexpected line in /proc/meminfo: %q", line)
		}
