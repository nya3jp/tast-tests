...
func (vmsm *vmStatsMeter) stats() (*VMStatsData, error) {
	...
	return &VMStatsData{
		PageFaultData: counterToData("pgmajfault", interval),
		SwapInData:    counterToData("pswpin", interval),
		SwapOutData:   counterToData("pswpout", interval),
		OomData:       counterToData("oom_kill", interval),
	}, nil
}
