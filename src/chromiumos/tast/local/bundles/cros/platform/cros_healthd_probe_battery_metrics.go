...
func CrosHealthdProbeBatteryMetrics(ctx context.Context, s *testing.State) {
	...
	for _, key := range []string{"manufacture_date_smart"} {
		if value, ok := contentsMap[key]; !ok || (hasSmartInfo && (value == "NA" || value == "0000-00-00")) || (!hasSmartInfo && value != "NA") {
			s.Errorf("Failed to collect %s", key)
		}
	}
	for _, key := range []string{"temperature_smart"} {
		if value, ok := contentsMap[key]; !ok || (hasSmartInfo && (value == "NA" || value == "0")) || (!hasSmartInfo && value != "NA") {
			s.Errorf("Failed to collect %s", key)
		}
	}
}
