...
func CrosHealthdProbeBatteryMetrics(ctx context.Context, s *testing.State) {
	...
	if len(headerActual) != len(headerExpected) {
		s.Fatalf("Incorrect number of metrics: got=%d, want=%d", len(headerActualMap), len(headerExpected))
	}
	missing := []string{}
	for _, he := range headerExpected {
		ok := true
		if _, ok = headerActualMap[he]; ok {
			delete(headerActualMap, he)
		}
		if !ok {
