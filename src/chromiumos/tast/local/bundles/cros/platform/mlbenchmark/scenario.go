...
func processOutputFile(scenario, outDir, outputFilename string) error {
	...
	if err != nil {
		return errors.Wrap(err, "unable to open the results file from the ML Benchmark")
	}

	var results benchmarkResults
	if err := json.Unmarshal(outputJSON, &results); err != nil {
		return errors.Wrap(err, "failed to parse the results from the ML benchmark")
	}

	if results.Status != 0 {
		return errors.Wrapf(err, "the ML Benchmark returned an error, status message: %s, status code: %d",
