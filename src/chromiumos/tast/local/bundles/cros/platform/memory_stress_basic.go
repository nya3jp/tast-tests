...
func stressMain(ctx context.Context, localRand *rand.Rand, mbPerTab, minFilelistKB int, baseURL string, enableARC, useHugePages bool, perfValues *perf.Values) error {
	...
	if err := stress.ReportTestCaseResult(ctx, perfValues, result67, "67_percent_random"); err != nil {
		return errors.Wrap(err, "reporting 67_percent_random failed")
	}
	if err := stress.ReportTestCaseResult(ctx, perfValues, result33, "33_percent_random"); err != nil {
...
func stressTestCase(ctx context.Context, localRand *rand.Rand, mbPerTab, switchCount, minFilelistKB int, compressRatio float64, baseURL string, enableARC, useHugePages bool) (stress.TestCaseResult, error) {
	...
	if err != nil {
		return stress.TestCaseResult{}, errors.Wrap(err, "cannot start chrome")
	}
	...
	if minFilelistKB >= 0 {
		if err := ioutil.WriteFile("/proc/sys/vm/min_filelist_kbytes", []byte(strconv.Itoa(minFilelistKB)), 0644); err != nil {
			return stress.TestCaseResult{}, errors.Wrap(err, "could not write to /proc/sys/vm/min_filelist_kbytes")
		}
	}
...
func lacrosMain(ctx context.Context, s *testing.State, localRand *rand.Rand, mbPerTab int, baseURL string, perfValues *perf.Values) error {
	...
	result, err := stress.TestCase(ctx, lacros, localRand, mbPerTab, switchCount, compressRatio, baseURL)
	if err != nil {
		return errors.Wrap(err, "stress test case failed")
	}
