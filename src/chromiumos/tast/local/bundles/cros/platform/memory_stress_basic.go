...
func init() {
	testing.AddTest(&testing.Test{
		...
		Vars:         []string{"platform.MemoryStressBasic.minFilelistKB", "platform.MemoryStressBasic.noARC"},
	})
...
func MemoryStressBasic(ctx context.Context, s *testing.State) {
	...
	enableARC := true
	if val, ok := s.Var("platform.MemoryStressBasic.noARC"); ok {
		testing.ContextLog(ctx, "noARC: ", val)
		intVal, err := strconv.Atoi(val)
		if err != nil {
			s.Fatal("Cannot parse argument platform.MemoryStressBasic.noARC: ", err)
		}
		if intVal != 0 {
			enableARC = false
		}
	...
	result67, err := stressTestCase(ctx, perfValues, mbPerTab, switchCount, minFilelistKB, 0.67, baseURL, label67, enableARC)
	if err != nil {
		s.Fatal("67_percent_random test case failed: ", err)
	}
	label33 := "33_percent_random"
	result33, err := stressTestCase(ctx, perfValues, mbPerTab, switchCount, minFilelistKB, 0.33, baseURL, label33, enableARC)
	if err != nil {
...
func stressTestCase(ctx context.Context, perfValues *perf.Values, mbPerTab, switchCount, minFilelistKB int, compressRatio float64, baseURL, label string, enableARC bool) (testCaseResult, error) {
	...
	if err != nil {
		return result, errors.Wrap(err, "cannot start chrome")
	}

	waitCtx, cancel := context.WithTimeout(ctx, 60*time.Second)
	defer cancel()
	if err := cpu.WaitUntilIdle(waitCtx); err != nil {
		return result, errors.Wrap(err, "failed to wait for idle CPU")
	}
	...
	if minFilelistKB >= 0 {
		if err := ioutil.WriteFile("/proc/sys/vm/min_filelist_kbytes", []byte(strconv.Itoa(minFilelistKB)), 0644); err != nil {
			return result, errors.Wrap(err, "could not write to /proc/sys/vm/min_filelist_kbytes")
		}
	}

	vmstatsStart, err := kernelmeter.VMStats()
	if err != nil {
		return result, errors.Wrap(err, "failed to get vmstat")
	}

	createTabCount, err := openTabCount(mbPerTab)
	if err != nil {
		return result, errors.Wrap(err, "failed to get open tab count")
	}
	...
	if err := openTabs(ctx, cr, createTabCount, mbPerTab, compressRatio, baseURL); err != nil {
		return result, errors.Wrap(err, "failed to open tabs")
	}

	tconn, err := cr.TestAPIConn(ctx)
	if err != nil {
		return result, errors.Wrap(err, "failed to connect to test API")
	}
	...
	if err != nil {
		return result, errors.Wrap(err, "failed to get histograms")
	}

	result.reloadCount, err = switchTabs(ctx, cr, switchCount)
	if err != nil {
		return result, errors.Wrap(err, "failed to switch tabs")
	}
	...
	if err != nil {
		return result, errors.Wrap(err, "failed to diff histograms")
	}
	result.jankyCount = histograms[0]
	result.discardLatency = histograms[1]

	vmstatsEnd, err := kernelmeter.VMStats()
	if err != nil {
		return result, errors.Wrap(err, "failed to get vmstat")
	}
	result.oomCount = vmstatsEnd["oom_kill"] - vmstatsStart["oom_kill"]
...
func switchTabs(ctx context.Context, cr *chrome.Chrome, switchCount int) (int, error) {
