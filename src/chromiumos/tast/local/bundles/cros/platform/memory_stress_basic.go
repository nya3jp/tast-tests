...
func MemoryStressBasic(ctx context.Context, s *testing.State) {
	...
	result67, err := stressTestCase(ctx, localRand, mbPerTab, switchCount, minFilelistKB, 0.67, baseURL, label67, enableARC)
	if err != nil {
		s.Fatal("67_percent_random test case failed: ", err)
	}
	label33 := "33_percent_random"
	result33, err := stressTestCase(ctx, localRand, mbPerTab, switchCount, minFilelistKB, 0.33, baseURL, label33, enableARC)
	if err != nil {
...
func stressTestCase(ctx context.Context, localRand *rand.Rand, mbPerTab, switchCount, minFilelistKB int, compressRatio float64, baseURL, label string, enableARC bool) (testCaseResult, error) {
