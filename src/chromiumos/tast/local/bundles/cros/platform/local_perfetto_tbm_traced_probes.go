...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:     []string{tracing.TBMTracedProbesConfigFile, tracing.GetTraceProcessorByArch()},
...
func LocalPerfettoTBMTracedProbes(ctx context.Context, s *testing.State) {
	...
	sess, err := tracing.StartTracing(ctx, traceConfigPath)
	...
	metrics, err := sess.RunMetrics(ctx, s.DataPath(tracing.GetTraceProcessorByArch()), []string{traceMetricCPU, traceMetricMEM})
	if err != nil {
