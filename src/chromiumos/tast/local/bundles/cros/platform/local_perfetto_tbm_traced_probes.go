...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:     []string{tracing.TBMTracedProbesConfigFile, tracing.TraceProcessorByArch()},
...
func LocalPerfettoTBMTracedProbes(ctx context.Context, s *testing.State) {
	...
	metrics, err := sess.RunMetrics(ctx, s.DataPath(tracing.TraceProcessorByArch()), []string{traceMetricCPU, traceMetricMEM})
	if err != nil {
