...
func SelinuxViolation(ctx context.Context, s *testing.State) {
	...
	const logFileRegex = `selinux_violation\.\d{8}\.\d{6}\.0\.log`
	const metaFileRegex = `selinux_violation\.\d{8}\.\d{6}\.0\.meta`
	...
	if matchingFile != "" {
		...
		allFiles := append(append([]string{}, files[logFileRegex]...), files[metaFileRegex]...)
