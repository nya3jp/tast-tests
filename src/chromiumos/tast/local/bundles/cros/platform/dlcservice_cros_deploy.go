...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Verifies that DLC cros deploying works by setting mimicing the cros deploy flow",
...
func DLCServiceCrosDeploy(ctx context.Context, s *testing.State) {
	...
	idPackage := dlc.Info{ID: dlc.TestID1, Package: dlc.TestPackage}
	if err := dlc.Cleanup(ctx, idPackage); err != nil {
	...
	// Copy test DLC into dlcservice cache location. Let cleanup handle cache cleanup.
	...
	for _, slot := range []string{dlc.SlotA, dlc.SlotB} {
		...
		if err := os.MkdirAll(cacheSlotPath, 0755); err != nil {
		...
		if err := dlc.CopyFile(filepath.Join(dlc.TestDir, dlc.TestID1, dlc.TestPackage, dlc.ImageFile),
			filepath.Join(cacheSlotPath, dlc.ImageFile), 0644); err != nil {
