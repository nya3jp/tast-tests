...
func getNumFans(ctx context.Context) (int, error) {
	...
	if len(tokens) != 2 {
		return -1, errors.Wrapf(err, "failed to parse ectool output: %s", string(b))
	}

	numFans, err := strconv.Atoi(strings.TrimSpace(tokens[1]))
	if err != nil {
		return -1, errors.Wrap(err, "failed to convert string to integer")
	}
...
func CrosHealthdProbeFanInfo(ctx context.Context, s *testing.State) {
	...
	// If there are no values to verify, simply return.
	if len(lines) == 1 {
		return
	}
