...
func FuseboxFuseFrontend(ctx context.Context, s *testing.State) {
	...
	// Run command: set verbose true to debug the command output.
	const verbose = false
	run := func(command string) {
		command = strings.ReplaceAll(command, "<MP>", m.MountPath)
		args := strings.Fields(command)
		if output, err := testexec.CommandContext(ctx, args[0], args[1:]...).Output(testexec.DumpLogOnError); err != nil {
			s.Fatalf("%s failed: %v", command, err)
		} else if verbose {
			s.Logf("%s %s", command, string(output[:]))
		}
	}
