...
// Package pressurizer creates a realistic memory pressure situation and takes
...
func initBrowser(ctx context.Context, useLiveSites bool, recordPageSet bool, wprArchivePath string) (*chrome.Chrome, *testexec.Cmd, error) {
	if useLiveSites {
...
// RunParameters contains the configurable parameters for RunPressurizer.
type RunParameters struct {
	...
	// UseLogIn controls whether RunPressurizer should use GAIA login.
	// (This is not yet functional.)
	UseLogIn bool
	// UseLiveSites controls whether RunPressurizer should skip WPR and
	// load pages directly from the internet.
	UseLiveSites bool
	// RecordPageSet instructs RunPressurizer to run in record mode
	// vs. replay mode.
	RecordPageSet bool
}

// RunPressurizer creates a memory pressure situation by loading multiple tabs
...
func RunPressurizer(ctx context.Context, s *testing.State, p *RunParameters) {
	const (
		tabWorkingSetSize    = 5
	...
	if p.RecordPageSet {
		...
		if memInfo.Total < 4*1000*1000*1000 {
			s.Fatal("Not enough RAM to record page set")
		}
	}
