...
func pushed(key int) (string, string, error) {
	...
	if _, ok := f[key]; !ok {
		return "", "", errors.New(fmt.Sprintf("no file found for %d", key))
	}
	...
	if _, ok := p[key]; !ok {
		return "", "", errors.New(fmt.Sprintf("no path found for %d", key))
	}
...
func pushFilePath(key int) error {
	...
	return errors.New(fmt.Sprintf("'%s' not found", file))
}
...
func callSenderOneCrash(ctx context.Context, s *testing.State, req *crashRequest) error {
	...
	if req.report, err = prepareSenderOneCrash(ctx, s, req); err != nil {
		errors.Wrap(err, "failed to prepare sender one crash")
	}
	var parm string
	if req.ignorePause {
		parm = "--ignore_pause_file"
	}
	if out, err := testexec.CommandContext(ctx, crashSenderPath, parm).Output(testexec.DumpLogOnError); err != nil {
...
// Look at the system logs to determenine if crash_sender has ran.
...
// Validate that the sent crash is correct.
...
func initializeCrashReporter(ctx context.Context, lock bool) error {
	...
	if _, err := testexec.CommandContext(ctx, crashReporterPath, "--init").Output(testexec.DumpLogOnError); err != nil {
		return errors.Wrap(err, "failed initializing crash_reporter")
	}
