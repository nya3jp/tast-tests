...
func callCrashSender(ctx context.Context, req *crashRequest) error {
	logsCursor, err := logsCursor(ctx)
	if err != nil {
		return errors.Wrap(err, "failed getting the log current tip")
	}

	if req.report, err = prepareSenderCrash(ctx, req); err != nil {
		return errors.Wrap(err, "failed to prepare sender crash")
	}
	...
	if err = waitForSenderCompletion(ctx, logsCursor); err != nil {
...
func waitForSenderCompletion(ctx context.Context, logsCursor string) error {
	err := testing.Poll(ctx,
		func(ctx context.Context) error {
			return logsReaderFind(ctx, logsCursor)
		},
...
// logsReaderFind looks at the system logs to deternine if crash_sender has ran.
func logsReaderFind(ctx context.Context, logsCursor string) error {
	// TODO(chavey): need to implement a repeated a loop trying to
	//  find a given string. can merge with logs.
	//  logsFind could return the whole buffer for crash_sender
	//  i.e crashSenderFullLog() this is obtained by looking for crash_sender done.
	// \s+ crash_sender\[[0-9]\]: <string>
	_ = logsFind(ctx, logsCursor)
	return nil
}

// logsFind Look at the system logs to find the string.
func logsFind(ctx context.Context, logsCursor string) error {
	for {
		b, err := logsRead(ctx, logsCursor)
		if err != nil {
			break
		}
		r := bytes.NewReader(b)
		...
		for scanner.Scan() {
			testing.ContextLogf(ctx, "--LOG -- %s", scanner.Text())
		}
...
//   logsCursor should be set by an earlier call to logsCursor.
func logsRead(ctx context.Context, logsCursor string) ([]byte, error) {
	//if fm == JournaldExport {
	//	args = append(args, "-o", "export")
	//}
	// TODO(chavey) Should be explicit about using --system for logs.
	testing.ContextLogf(ctx, "journalctl --after-cursor=%q --no-pager", logsCursor)
	b, err := testexec.CommandContext(ctx, "journalctl",
		fmt.Sprintf("--after-cursor=%s", logsCursor), "--no-pager").Output(testexec.DumpLogOnError)
	if err != nil {
		return []byte(""), err
	}
...
func logsCursor(ctx context.Context) (string, error) {
	logsCursor, err := logs.GetJournaldCursor(ctx)
	if err != nil {
