...
func ValidateCSV(csv [][]string, rows int, columns ...column) error {
	...
	for i := 1; i < len(csv); i++ {
		for j, col := range columns {
			header := csv[0][j]
			if header != col.field {
				return errors.Errorf("incorrect header name: got %v, want %v", header, col.field)
			}
			if err := col.columnFunc(csv[i][j]); err != nil {
				return errors.Wrapf(err, "failed validation on %v (Column %v) with value %v", header, j, csv[i][j])
			}
		}
	}
	return nil
