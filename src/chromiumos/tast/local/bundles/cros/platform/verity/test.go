...
func createHash(ctx context.Context, dir, name, image string, nBlocks uint) (hash string, table string, err error) {
...
// setUpLoop sets up the loop back device for the given image, and returns
// its path.
...
// tearDownLoop tears down the loop back device created by setUpLoop.
func tearDownLoop(ctx context.Context, loop string) error {
	return releaseDevice(ctx, []string{"losetup", "-d", loop}, loop)
}

// createVerityDevice sets up the verity device for the given loop back
...
// verifiable walks completely on the device, and returns any error if found.
...
// preCheck returns whether the dm-verity related test can run on the current
// environment.
func preCheck() []error {
	var required = []string{
		"losetup",
...
func runCheck(ctx context.Context, name string, expect bool, modify func(string) error) error {
	...
	if fi, err := os.Stat(hash); err != nil {
		...
		return errors.Errorf("unexpected hash file size: got %d; want %d", fi.Size(), blockSize)
	}
