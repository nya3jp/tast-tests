...
// TabID retrieves a list of pids for all active tabs. It returns a promise
// that resolves with the list of pids.
async function TabIDs() {
  const tabs = await tabsQuery({});
  const active_tabs = tabs.filter(tab => tab.id)
  return Object.values(active_tabs).map(tab => tab.id);
}
