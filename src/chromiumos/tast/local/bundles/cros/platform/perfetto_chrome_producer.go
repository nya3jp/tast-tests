...
func waitForChromeProducer(ctx context.Context) error {
	...
	return testing.Poll(ctx, func(context.Context) error {
		...
		// Chrome producers retry the connection with delay on failure to connect. Poll using a 20 sec. timeout.
...
func PerfettoChromeProducer(ctx context.Context, s *testing.State) {
	...
	if err := upstart.EnsureJobRunning(shortCtx, tracedJob); err != nil {
		s.Fatal("Job traced isn't running: ", err)
	}
	defer func() {
		if err := upstart.StopJob(ctx, tracedJob); err != nil {
	...
	if err = waitForChromeProducer(shortCtx); err != nil {
