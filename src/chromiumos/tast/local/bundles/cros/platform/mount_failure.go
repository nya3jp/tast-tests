...
type mountFailureCrashLog struct {
	// path: location where the log is stored. Empty path implies that the log isn't generated by this test.
	path string
	// contents: fake contents to store in the log before the test.
	contents string
	// wantHeader: expected header that is present if the log collection was successful.
	wantHeader string
}
...
type mountFailureCrash struct {
	// name: crash name.
	name string
	// crashReporterArgs: command line arguments used to invoke crash-reporter.
	crashReporterArgs []string
	// wantLogCommands: expected log commands run by crash-reporter.
