...
func CrosHealthdProbeStatefulPartitionInfo(ctx context.Context, s *testing.State) {
	...
	// Verify each line of stateful partition info contains valid values.
	for _, line := range lines[1:] {
		if len(line) != 2 {
			s.Errorf("Wrong number of values: got %v, want 2", len(line))
			continue
		}

		if available, err := strconv.Atoi(line[0]); err != nil {
			s.Errorf("Failed to convert %q to integer: %v", want[0], err)
		} else if available < 0 {
			s.Errorf("Invalid %s: %v", want[0], available)
		}

		if total, err := strconv.Atoi(line[1]); err != nil {
			s.Errorf("Failed to convert %q to integer: %v", want[1], err)
		} else if total < 0 {
			s.Errorf("Invalid %s: %v", want[1], total)
		}
	}
}
