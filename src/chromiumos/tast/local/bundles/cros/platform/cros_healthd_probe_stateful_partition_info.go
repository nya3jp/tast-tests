...
func CrosHealthdProbeStatefulPartitionInfo(ctx context.Context, s *testing.State) {
	...
	high := realAvailable + margin
	var low uint64
	if realAvailable > margin {
		low = realAvailable - margin
	} else {
		low = 0
	}
	...
	if available, err := strconv.ParseUint(vals[0], 10, 64); err != nil {
		...
		s.Errorf("Invalid available_space: got %v; want %v +- 100000000", available, realAvailable)
	}
