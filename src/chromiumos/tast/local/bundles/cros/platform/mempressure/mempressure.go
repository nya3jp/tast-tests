...
// label.  Then it resets meter.
func recordAndResetStats(s *testing.State, meter *kernelmeter.Meter,
	values *perf.Values, label string) {
	defer meter.Reset()
	stats, err := meter.VMStats()
	if err != nil {
		s.Errorf("Cannot compute page fault stats (%s): %s", label, err)
		return
	}
...
func Run(ctx context.Context, s *testing.State, p *RunParameters) {
	...
	recordAndResetStats(s, fullMeter, perfValues, "initial")
	...
	recordAndResetStats(s, fullMeter, perfValues, "heavy")
