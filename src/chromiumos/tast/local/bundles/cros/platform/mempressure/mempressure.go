...
func memoryEqualizingAmount(ctx context.Context, workingSetMiB uint64, ratio float64) (allocMiB uint64, err error) {
	...
	//
	// where discardFreeSwap is the amount of free swap at discard time.
	// We derive
	//
	...
	//
	// We want to preallocate RAM so that when we start opening tabs we
	// have workingSetMiB worth of swap.
	//
