...
func Run(ctx context.Context, s *testing.State, p *RunParameters) {
	...
	if memInfo.Total > (5<<9) && memInfo.Total <= (9<<9) {
		...
		allocMiB = 3500 // empirically chosen for 15-20 additional tabs
		s.Logf("Adjust for large RAM (%v MiB): initial size %v, allocMiB %v",
			memInfo.Total/(2<<6), initialTabSetSize, allocMiB)
	}
	if memInfo.Total > (9 << 9) {
		// 16 GiB device
		initialTabSetSize = 100
		allocMiB = 15500 // empirically chosen for 15-20 additional tabs
		s.Logf("Adjust for large RAM (%v MiB): initial size %v, allocMiB %v",
			memInfo.Total/(2<<6), initialTabSetSize, allocMiB)
	}
