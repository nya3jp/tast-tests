...
func memoryEqualizingAmount(ctx context.Context, workingSetMiB uint64, ratio float64) (allocMiB uint64, err error) {
	...
	//
	//  available = minFree + discardSwap/ramWeight
	//
	// where "discardSwap" is the amount of swap ad discard.
	// We derive
	//
	//  discardFreeSwap = (margin - minFree) * ramWeight
	//
	// However, this only works if margin > minFree, which isn't always the
	// case.  If margin < minFree, then in theory the swap fills up
	// completely, then we start eating the remaining free RAM.  This
	// happens on the smaller systems, so the remaining free RAM is small
	// and we ignore it.
	//
	// We want to start the test with workingSetMiB worth of space.  At
	// discard time, minFree worth of this will be in RAM, the rest
	// in swap.  Thus the starting free swap should be
	//
	//  startFreeSwap = discarFreedSwap + (workingSet - minFree)
	//
	// To achieve that, first we fill up RAM with easy - minFree.  That's
	// the point at which the system starts swapping, and we need to
	// compute how much more to allocate.  Since using swap requires RAM,
	// for each incremental allocation A we will need to swap out A plus
	// some extra amount X to make room.  Let R be the compression ratio.
	// We have:
	//
	//  (before allocation)
	...
	//  X = AR / (1 - R)
	//
	// So allocating A costs us A + AR / (1 - R) = A/(1-R)
	// therefore if we want to use S swap space, we should allocate
	// an additional S*(1-R).
	...
	startFreeSwapMiB := discardFreeSwapMiB + workingSetMiB - minFreeMiB
	startUsedSwapMiB := swapTotalMiB - startFreeSwapMiB
	allocMiB = preSwapMiB + uint64(float64(startUsedSwapMiB)*(1-ratio))
...
func Run(ctx context.Context, s *testing.State, p *RunParameters) {
	const (
		...
		workingSetMiB        = 4000 // usable memory (RAM + swap) after preallocation
	)
