...
func Run(ctx context.Context, s *testing.State, p *RunParameters) {
	...
	if allocMiB > 0 && !p.RecordPageSet {
		...
		allocBytes := int(allocMiB * 1024 * 1024)
		...
		if runtime.GOARCH == "arm" || runtime.GOARCH == "386" {
			if uint64(allocBytes) >= math.MaxUint32 {
				s.Fatalf("Preallocation on 32bit architecture is too large: %d", allocBytes)
			}
		}

		data, err := syscall.Mmap(-1, 0, allocBytes, syscall.PROT_READ|syscall.PROT_WRITE, syscall.MAP_ANON|syscall.MAP_PRIVATE)
		if err != nil {
