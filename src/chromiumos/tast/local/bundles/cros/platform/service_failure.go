...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			Name: "pre_start",
			Val:  []string{"PRE_START_EXIT", "pre-start process"},
		}, {
			Name: "main",
			Val:  []string{"MAIN_EXIT", "main process"},
		}, {
			Name: "post_start",
			Val:  []string{"POST_START_EXIT", "post-start process"},
		}},
	})
}
...
func ServiceFailure(ctx context.Context, s *testing.State) {
	...
	if err := upstart.StartJob(ctx, failingServiceName, fmt.Sprintf("%s=1", params[0])); err != nil {
		// Ignore error; it's expected.
		// (upstart exits nonzero if a job fails in pre-start)
	}

	expectedLogMsg := fmt.Sprintf("%s %s", failingServiceName, params[1])
