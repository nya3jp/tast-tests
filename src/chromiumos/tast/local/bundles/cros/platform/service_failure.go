...
func ServiceFailure(ctx context.Context, s *testing.State) {
	...
	for i, env := range failureStages {
		...
		if err := upstart.StartJob(ctx, serviceName, fmt.Sprintf("%s=1", env)); err != nil {
			// Ignore error; it's probably expected.
		}

		expectedLogMsg := fmt.Sprintf("%s %s", serviceName, messages[i])
