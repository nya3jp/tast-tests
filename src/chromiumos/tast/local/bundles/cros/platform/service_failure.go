...
func ServiceFailure(ctx context.Context, s *testing.State) {
	...
	for _, tt := range testParams {
		s.Run(ctx, tt.name, func(sctx context.Context, ss *testing.State) {
			...
			logs := files[logRegex]
			if len(logs) != 1 {
				ss.Errorf("Multiple service failures found. Leaving for debugging: %s", strings.Join(logs, ", "))
				...
				if !strings.Contains(string(contents), expectedLogMsg) {
					ss.Errorf("Didn't find expected log contents: `%s`. Leaving %s for debugging", expectedLogMsg, logs[0])
