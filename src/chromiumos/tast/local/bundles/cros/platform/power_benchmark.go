...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{
			{Name: "full", Val: powerBenchmarkParams{numIterations: 10, minDuration: 1, maxDuration: 30, backlightLevels: []int{0, 50, 100, -1}}},
			{Name: "sanity", Val: powerBenchmarkParams{numIterations: 10, minDuration: 1, maxDuration: 1, backlightLevels: []int{0}}},
			{Name: "test_backlight", Val: powerBenchmarkParams{numIterations: 3, minDuration: 4, maxDuration: 4, backlightLevels: []int{-2}}},
		},
	})
}
...
func PowerBenchmark(ctx context.Context, s *testing.State) {
	...
	// TODO: implement a high-frequency background momentary battery power sampling
	//       test how it interferes with the power usage depending on the frequency
	// .     the less frequent the less impact it should have

loop:
	for _, backlight_percent := range p.backlightLevels {
		for test_duration := p.minDuration; test_duration <= p.maxDuration; test_duration += 1 {
			for i := 0; i < p.numIterations; i += 1 {
				...
				momentary_battery_w_start, err := power.ReadSystemMomentaryPower(batteryPaths)
				if err != nil {
				...
				test_name := ""
				if backlight_percent == -1 {
					test_name = "idle, backlight: 0%, 100%, 100%, 0%"
				...
				} else if backlight_percent == -2 {
					test_name = "idle, backlight: 0%, 50%, 100%, 0%"
					...
					test_name = fmt.Sprintf("idle, brightness %d%%", backlight_percent)

					setup.SetBacklightPercent(ctx, backlight_percent)
				...
				momentary_battery_w_end, err := power.ReadSystemMomentaryPower(batteryPaths)
				if err != nil {
					s.Fatal("failed to read battery momentary power: ", err)
				}
				...
				if battery_energy_start < battery_energy_end {
					s.Fatalf("battery_energy_start (", battery_energy_start, ") < battery_energy_end (", battery_energy_end, "): is charger not disabled?")
				}
				// convert Wh to W
	...
	csv := "timestamp,duration_sec,test_name,rapl_w,battery_w,momentary_battery_w_start,momentary_battery_w_end,battery_percentage_start,battery_percentage_end,t_start,t_end,la_start,la_end\n"
