...
func PowerBenchmark(ctx context.Context, s *testing.State) {
	...
	type Result struct {
		...
		battery_energy_wh_start   float64
		battery_energy_wh_end     float64
	...
	readMomentaryPowerW := func() float64 {
		...
		if err != nil {
			s.Error("failed to read battery momentary power: ", err)
			return 0
		}
		return result
	}
...
loop:
	for _, test := range p.tests {
		for test_duration := p.minDuration; test_duration <= p.maxDuration; test_duration += 1 {
			for i := 0; i < p.numIterations; i += 1 {
				...
				battery_energy_wh_start, err := power.ReadBatteryEnergy(batteryPaths)
				if err != nil {
					s.Fatal("failed to read battery energy ", err)
				}
				...
				if err != nil {
					s.Error("failed to read battery percentage: ", err)
					battery_percentage_start = 0
				}
				...
				battery_energy_wh_end, err := power.ReadBatteryEnergy(batteryPaths)
				if err != nil {
					s.Fatal("failed to read battery energy ", err)
				}
				...
				if err != nil {
					s.Error("failed to read battery percentage: ", err)
					battery_percentage_end = 0
				}

				if battery_energy_wh_start < battery_energy_wh_end {
					// mblsha: Happened to me on a poppy/ekko.
					s.Errorf("battery_energy_wh_start (", battery_energy_wh_start, ") < battery_energy_wh_end (", battery_energy_wh_end, "): is charger not disabled?")
					// don't want negative numbers poisoning the average.
					battery_energy_wh_end = battery_energy_wh_start
				}
				// convert Wh to W
				battery_watts := (battery_energy_wh_start - battery_energy_wh_end) / float64(test_duration) * 3600
				...
				r := Result{
					...
					battery_energy_wh_start:   battery_energy_wh_start,
					battery_energy_wh_end:     battery_energy_wh_end,
				...
				testing.ContextLog(newCtx, "battery_energy_wh_start: ", battery_energy_wh_start, "; battery_energy_wh_end: ", battery_energy_wh_end)
				testing.ContextLog(newCtx, r)
				results = append(results, r)

				// We want at least some of the results in case the DUT dies during the test.
				if battery_energy_wh_end < 2 {
					break loop
				}
			}
		}
	...
	csv := "timestamp,duration_sec,test_name,rapl_w,battery_w,sample_w," +
		"momentary_battery_w_start,momentary_battery_w_end," +
		"battery_percentage_start,battery_percentage_end," +
		"battery_energy_wh_start,battery_energy_wh_end," +
		"t_start,t_end,la_start,la_end\n"
	for _, r := range results {
		...
		// Could be used to calculate more precise power draw over a long period of time.
		csv += fmt.Sprintf(",%f", r.battery_energy_wh_start)
		csv += fmt.Sprintf(",%f", r.battery_energy_wh_end)
