...
// When name is an empty string, the flag will be removed.
...
func replaceCrashFilterString(oldPattern, name string) (string, error) {
	...
	for _, s := range e {
		...
		newargs = append(newargs, "--filter_in="+name)
		replaced = true
	}
	if len(name) != 0 && !replaced {
		newargs = append(newargs, "--filter_in="+name)
	}
...
func replaceCrashFilterIn(param string) error {
	...
	newPattern, err := replaceCrashFilterString(string(b), param)
	if err != nil {
		return err
	}
