...
func resetRateLimiting() error {
	if err := os.RemoveAll(crashSenderRateDir); err != nil && !os.IsNotExist(err) {
...
func RunCrashTests(ctx context.Context, s *testing.State, testFuncList []func(context.Context, *testing.State)) error {
	for _, f := range testFuncList {
		runCrashTest(ctx, s, f)
	}
