...
func RunCrasherProcessAndAnalyze(ctx context.Context, opts CrasherOptions) (CrasherResult, error) {
	...
	if err != nil {
		return result, errors.Wrapf(err, "failed to get crash directory for user [%s]", opts.Username)
	}
	if !opts.Consent {
	...
	if len(missingFileTypes) > 0 {
		return result, errors.Errorf("crash report is missing files: %v", addDotPrefix(missingFileTypes))
	}
	...
	if len(unknownFileTypes) > 0 {
		return result, errors.Errorf("crash report includes unkown files: %v", addDotPrefix(unknownFileTypes))
	}
