...
func runCrasherProcess(ctx context.Context, opts CrasherOptions) (*CrasherResult, error) {
	...
	select {
	case <-c.Done():
		// WaitForMessage timed out. This usually means |crashCaughtMessage| did not appear in the log.
		// However, there are two potential exceptional cases by race conditions.
		// 1. WaitForMessage returned non-timeout error (like I/O error) right before the context times out.
		// Ideally we should fail the test.
		...
		// 2. WaitForMessage successfully found target string right before context timed out. Covered here.
