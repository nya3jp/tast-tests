...
func callSenderOneCrash(ctx context.Context, opts senderOptions) (*SenderOutput, error) {
	...
	logReader, err := syslog.NewWatcher(messagesFile)
	if err != nil {
	...
	if code != 0 && !opts.ShouldFail {
		return nil, errors.Errorf("crash_sender returned an unexpected non-zero value (%s)", code)
	}

	if err := waitForSenderCompletion(ctx, w); err != nil {
		return nil, err
	}
	output, err := logReader.GetLogs()
	if err != nil {
