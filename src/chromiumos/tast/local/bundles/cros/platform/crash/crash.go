...
func cleanCrashSpoolDirs(ctx context.Context) error {
	...
	knownCrashNames := make(map[string]struct{})
	knownCrashNames["crash_log_test"] = struct{}{}
	knownCrashNames["crash_log_recursion_tast_test"] = struct{}{}
	knownCrashNames["platform_UserCrash_crasher"] = struct{}{}
	var firstErr error
	for _, f := range crashes {
		prog := strings.SplitN(filepath.Base(f), ".", 2)[0]
		if _, found := knownCrashNames[prog]; !found {
			continue
		}
