...
func checkCrashDirectoryPermissions(path string) error {
	...
	if _, found := permittedModes[mode]; !found {
		...
		return errors.Errorf("mode of %s got %v; want either of %v",
			path, mode, extractKeys(permittedModes))
	}
	return nil
}
...
// Those files can be restored by restoreCrashFiles later. Doesn't support recursive stashing.
func stashCrashFiles(userName string) error {
	path := getCrashDir(userName)
	if err := os.RemoveAll(backupCrashDir); err != nil && !os.IsNotExist(err) {
		return errors.Wrapf(err, "failed to remove %s, old backup of crash directory", path)
	}
	if err := os.Mkdir(backupCrashDir, os.FileMode(0770)); err != nil {
		return errors.Wrapf(err, "failed to create directory %s for crash directory backup", path)
	}
	files, err := ioutil.ReadDir(path)
	if err != nil {
	...
	if err := os.RemoveAll(path); err != nil {
		return errors.Wrapf(err, "failed to remove crash directory %s", path)
	}
	return nil
}
