...
//
// Args:
//  frameIndex: number of the stack frame (0 is innermost frame)
//  moduleName: name of the module (executable or dso)
//  functionName: name of the function in the stack
//  fileName: name of the file containing the function
//  lineNumber: line number
//  stack: text string of stack frame entries on separate lines.
//
// Returns:
//  bool indicating if an exact match is present.
//
// Note:
...
func verifyStack(ctx context.Context, stack []byte, basename string, fromCrashReporter bool) error {
	...
	testing.ContextLogf(ctx, "stack is %s", string(stack))
