...
func checkCrashDirectoryPermissions(path string) error {
	...
	if _, found := permittedModes[mode]; !found {
		return errors.Errorf("mode of %s got %v; want either of %v",
			path, mode, func(m map[os.FileMode]struct{}) []os.FileMode {
...
func stashCrashFiles(userName string) (func() error, error) {
	...
	backupDestination := filepath.Join(tempDir, "crash")
	if err := os.Rename(crashDir, backupDestination); err != nil {
		return nil, errors.Wrapf(err, "failed to rename crash directory from %s to %s", crashDir, backupDestination)
	}
	return func() error {
		...
		if err := os.Rename(backupDestination, crashDir); err != nil {
			return errors.Wrapf(err, "failed to restore crash directory from %s to %s", backupDestination, crashDir)
		}
...
func RunCrasherProcessAndAnalyze(ctx context.Context, opts CrasherOptions) (*CrasherResult, error) {
	...
	// Basename is used for entries in the file. Return the original name of the executable.
...
func isFrameInStack(ctx context.Context, frameIndex int, moduleName string, functionName string, fileName string,
...
func verifyStack(ctx context.Context, stack []byte, basename string, fromCrashReporter bool) error {
	...
	const crasherSource = `platform\.UserCrash\.crasher\.crasher\.cc`
...
func checkMinidumpStackwalk(ctx context.Context, minidumpPath string, basename string, fromCrashReporter bool) error {
...
func CheckCrashingProcess(ctx context.Context, opts CrasherOptions) error {
	...
	if err := checkMinidumpStackwalk(ctx, result.Minidump, result.Basename, true); err != nil {
		return errors.Wrap(err, "minidump")
	}
	// TODO(crbug.com/970930): Check if crash reporter announces minidump locatino to log "Stored minidump to "
