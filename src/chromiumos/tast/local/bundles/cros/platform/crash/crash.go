...
func clearSpooledCrashes() error {
	...
	for _, match := range matches {
		if err := os.Remove(match); err != nil && !os.IsNotExist(err) {
			return errors.Wrap(err, "failed cleaning a user crash dir")
		}
	}
	if err := os.Remove(fallbackUserCrashDir); err != nil && !os.IsNotExist(err) {
...
func Setup(ctx context.Context) error {
	...
	if err := testexec.CommandContext(ctx, "pkill", "-9", "-e", "crash_sennnnnnder").Run(); err != nil {
		return err
	}
...
func TearDown(ctx context.Context) error {
	if err := enableSystemSending(); err != nil {
		return err
	}
	return nil
}
