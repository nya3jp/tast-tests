...
func MLBenchmarkSoDA(ctx context.Context, s *testing.State) {
	const (
		...
		outputFilename = "soda_benchmark_results.json"
	...
	resultsPath := filepath.Join(workspacePath, outputFilename)
	...
	if err := json.Unmarshal([]byte(resultsFile), &results); err != nil {
	...
	if results.LatenciesUs.Percentile90 == 0 {
		s.Fatalf("No 90th percentile found in the results: %s", resultsFile)
	}
