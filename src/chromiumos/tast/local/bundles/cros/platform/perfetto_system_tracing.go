...
const (
	...
	traceConfigName = "perfetto/system_trace_cfg.pbtxt"
)
...
func ensureJobsStopped(ctx context.Context) error {
	...
	if err := upstart.StopJob(wctx, tracedJob); err != nil {
		return errors.Wrapf(err, "failed to stop job %s", tracedJob)
	}
	...
	if err := upstart.WaitForJobStatus(wctx, tracedProbesJob, upstart.StopGoal, upstart.WaitingState, upstart.RejectWrongGoal, waitDuration); err != nil {
		return errors.Wrapf(err, "job %s isn't stopped", tracedProbesJob)
	}
...
func collectTraceData(ctx context.Context, s *testing.State) error {
	...
	traceOutputPath := filepath.Join(s.OutDir(), "perfetto_trace.proto")
	traceConfigPath := s.DataPath(traceConfigName)
...
func getRunningJobStatus(ctx context.Context) (int, int, error) {
	...
	// Sleep briefly to give traced_probes time to start.
	if err := testing.Sleep(wctx, 3*time.Second); err != nil {
		return 0, 0, nil
	}

	// Ensure traced_probes is not in the zombie process state.
	if err := upstart.CheckJob(wctx, tracedProbesJob); err != nil {
		return 0, 0, err
	}
