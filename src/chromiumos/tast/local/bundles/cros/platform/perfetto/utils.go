...
func CollectTraceDataWithConfig(ctx context.Context, configFile string) (string, error) {
...
func StartTraceDataWithConfig(ctx context.Context, configFile string) (int, string, error) {
	tempFile, err := createTempFileForTrace()
	if err != nil {
		return -1, "", errors.Wrap(err, "failed to create temp file")
	}
	...
	out, err := cmd.Output()
	if err != nil {
		return -1, "", errors.Wrap(err, "failed to start the tracing session")
	}

	outInString := strings.TrimSuffix(string(out), "\n")
	PID, err := strconv.Atoi(outInString)
	if err != nil {
		return -1, "", errors.Wrap(err, "failed to parse PID")
	}

	return PID, tempFile.Name(), nil
}
