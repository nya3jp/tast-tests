...
// TracingSession stores the cmd and the result file of the trace.
// Remember to call TracingSession.RemoveTraceResultFile to clean up the
// temporary file.
...
// by StartTracing.
func (sess *TracingSession) Wait() error {
...
// On success, returns the temporary file of the trace data. It's the
// caller's responsibility for removing it if it's no longer needed.
func CollectTrace(ctx context.Context, configFile string) (*TracingSession, error) {
