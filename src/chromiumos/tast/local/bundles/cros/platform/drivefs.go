...
func Drivefs(ctx context.Context, s *testing.State) {
	...
	// some bug.
	err = waitForMountConnected(ctx, fuseIoTimeout, drivefsRoot)
	if err != nil {
	...
	if mounts, err = findMatchingMount(isDriveFs); err != nil {
		s.Fatal("Could not obtain mounts: ", err)
	}
	if len(mounts) != 1 {
		s.Fatal("Expected one drivefs mount to remain but found ", len(mounts), ". Found mounts ", mounts)
	}
