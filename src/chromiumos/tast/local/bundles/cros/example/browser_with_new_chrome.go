...
func BrowserWithNewChrome(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		{browser.TypeLacros, browserfixt.DefaultLacrosConfig},
		{browser.TypeLacros, browserfixt.DefaultLacrosConfig.WithVar(s)},                                    // To accept the external lacros-chrome specified with the var --lacrosDeployedBinary
		{browser.TypeLacros, browserfixt.NewLacrosConfig(browserfixt.Rootfs, browserfixt.LacrosSideBySide)}, // custom config
	} {
		bt := param.bt
		cfg := param.cfg
		s.Run(ctx, fmt.Sprintf("BrowserWithNewChrome browser: %v, cfg: %+v", bt, cfg), func(ctx context.Context, s *testing.State) {
			...
			_ /* conn */, err = br.NewConn(ctx, url, browser.WithNewWindow())
			if err != nil {
			...
			if bt == browser.TypeLacros {
				numNewWindows = 2 // Lacros opens an extra window in browserfixt.SetUp*.
			}
