...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Tests the browserfixt package. See http://go/lacros-tast-porting for the guidelines on how to use",
...
func BrowserFixt(ctx context.Context, s *testing.State) {
	...
	for _, url := range []string{
		...
		s.Run(ctx, fmt.Sprintf("browserfixt.CreateWindows browser: %v, url: %v", bt, url), func(ctx context.Context, s *testing.State) {
			...
			_ /* br */, closeBrowser, err := browserfixt.CreateWindows(ctx, s.FixtValue(), bt, tconn, url, numNewWindows)
			if err != nil {
