...
func Browser(ctx context.Context, s *testing.State) {
	...
	// TODO: Read vars to fill in LacrosConfig to be passed in to chrome.LacrosEnabled option.
	// TODO: Implement chrome.LacrosEnabled()
	bt := s.Param().(browser.Type)
	var opts []chrome.Option
	if bt == browser.TypeLacros {
		opts = append(opts, chrome.LacrosEnabled())
	}
	...
	if bt == browser.TypeLacros {
		l, err := lacros.LaunchFromShelf(ctx, tconn, cr.LacrosExecPath(ctx))
		if err != nil {
