...
// We can't just run "id" and "id chronos", since it include a security
// context that might differ.
func CommandContextUser(ctx context.Context, s *testing.State) {
	...
	for line := 0; line < 3; line++ {
		...
		if strings.Compare(expectedStr, testStr) != 0 {
			s.Fatalf("output line %d expected IDs don't match test IDs: %q != %q",
				line, expectedStr, testStr)
		}
	}
}
