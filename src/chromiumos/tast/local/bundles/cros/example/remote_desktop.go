...
func waitConnection(ctx context.Context, crd *chrome.Conn) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		// TODO(shik): Find a better way to click the "Share" button in the
		// popup confirmation window instead of keep pressing enter.  It's a
		// native UI widget so there is no page that we can connect to.
