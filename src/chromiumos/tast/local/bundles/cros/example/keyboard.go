...
func Keyboard(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine("Focus text box",
		...
		// TODO: ui.FocusAndWait doesn't seem to work on Lacros. Timed out waiting for event.Focus to occur.
		// Since the input element has 'autofocus' element commenting won't let the test fail.
