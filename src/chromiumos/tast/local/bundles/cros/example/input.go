...
func touchscreenExamples(ctx context.Context, s *testing.State, cr *chrome.Chrome) {
	...
	s.Logf("Internal display: %+v", info)
	s.Logf("Bounds: %+v", info.Bounds)

	// TODO(ricardoq): This page might change/break in the future. If so, a built-in
	// HTML page that accepts drawing should be used.
	...
	// use the Touchscreen.TouchEvent() type since it already has functions to manipulate
	// Touch events.
