...
func AccessVars(ctx context.Context, s *testing.State) {
	var boolVal bool
	if ok := testing.ContextVar(ctx, global.ExampleBoolVar, &boolVal); boolVal != true || !ok {
		s.Errorf("Got global variable value (%v, %v) from ContextVar want (%v, %v)", boolVal, ok, true, true)
	}
	var strVal string
	if ok := testing.ContextVar(ctx, global.ExampleStrVar, &strVal); strVal != "test" || !ok {
		s.Errorf("Got global variable value (%q, %v) from ContextVar want (%q, %v)", strVal, ok, "test", true)
	}
	var structVal global.ExampleStruct
	expected := global.ExampleStruct{Name: "t1", Value: 8}
	if ok := testing.ContextVar(ctx, global.ExampleStructVar, &structVal); !ok {
