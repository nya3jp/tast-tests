...
func CRXManifestUpdateURLIgnored(ctx context.Context, s *testing.State) {
	for _, tc := range []struct {
		...
		// update path in accordance with KioskCRXManifestUpdateURLIgnored policy.
		...
		// Update URL in crx pointss to V1 of the app. Version should correspond to updateURL in policy.
		{
			ignoreCrxURL:     true,
...
func launchKioskAndVerify(ctx context.Context, s *testing.State, ignoreCrxURL bool, originalAppTitle, updatedAppTitle, appID, updateURL string) {
	...
	// If not cleared, later tests are be affected by already installed extensions of different versions
	...
	if !openExtensionAndCheckVersion(ctx, s, cr, originalAppTitle, updatedAppTitle) {
		s.Fatal("Missmatch in Version")
	}
}

// openExtensionAndCheckVersion opens the kiosk app from login screen using the originalAppTitle and evaluates if the launched app has updatedAppTitle
func openExtensionAndCheckVersion(ctx context.Context, s *testing.State, cr *chrome.Chrome, originalAppTitle, updatedAppTitle string) bool {
