...
func StartAppFromSignInScreen(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine("open Kiosk application menu",
		...
		// elements. I was not able to find out how to overcome flakiness other
		// than multiple waits or using hard sleep before interacting with UI.
		ui.WaitUntilExists(localAccountsBtn),
		ui.WaitUntilExists(localAccountsBtn),
		ui.WaitUntilExists(localAccountsBtn),
		ui.LeftClick(localAccountsBtn), // Often, after that click App (local accounts) buttton freezes.
	...
	// test failed at finding the menu item.
	if err := uiauto.Combine("close and open Kiosk application menu then click on one menu item",
		ui.LeftClick(localAccountsBtn),                // Close previously opened menu.
		ui.LeftClick(localAccountsBtn),                // Open the menu again.
		ui.WaitUntilExists(kioskmode.KioskAppBtnNode), // Wait for the menu item to be visible.
