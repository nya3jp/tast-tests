...
func Autostart(ctx context.Context, s *testing.State) {
	...
	// '[...] kiosk_launch_controller.cc [...] Starting kiosk mode of type 2...'
	// '[...] kiosk_launch_controller.cc [...] Kiosk launch succeeded, wait for app window.'
	...
	const kioskStarting = "Starting kiosk mode"
	err = testing.Poll(ctx, func(ctx context.Context) error {
		return checkThePresenceOfKioskStatus(ctx, kioskStarting)
	}, &testing.PollOptions{Interval: time.Second, Timeout: time.Minute})
	if err != nil {
		s.Fatal("Failed to verify starting of Kiosk mode: ", err)
	}

	const kioskStartSucceeded = "Kiosk launch succeeded"
	err = testing.Poll(ctx, func(ctx context.Context) error {
		return checkThePresenceOfKioskStatus(ctx, kioskStartSucceeded)
	}, &testing.PollOptions{Interval: time.Second, Timeout: time.Minute})
	if err != nil {
...
// checkThePresenceOfKioskStatus checks the /var/log/messages for specific
// phrase indicating status of Kiosk mode.
func checkThePresenceOfKioskStatus(ctx context.Context, searchedPhrase string) error {
	const (
		command      = "grep"
		messagesFile = "/var/log/messages"
	)
	outputRaw, err := testexec.CommandContext(ctx,
		command,
		strings.Replace(searchedPhrase, " ", "\\ ", -1), // Replace spaces with double escaped space for BRE used in grep.
		messagesFile,
	).Output(testexec.DumpLogOnError)
	if err != nil {
		return errors.Wrapf(err, " failed to run command '%s %s %s'", command, searchedPhrase, messagesFile)
	}
	output := string(outputRaw)

	if res := strings.Contains(output, searchedPhrase); !res {
		errors.Errorf("failed to find indication of Kiosk mode phase in log file. Command [%s %s %s], result [%s]", command, searchedPhrase, messagesFile, output)
	}
	return nil
}
