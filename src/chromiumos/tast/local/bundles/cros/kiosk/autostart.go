...
func checkThePresenceOfKioskStatus(ctx context.Context, searchedPhrase string) error {
	...
	outputRaw, err := testexec.CommandContext(ctx, command, searchedPhrase, messagesFile).Output(testexec.DumpLogOnError)
	if err != nil {
		return errors.Wrapf(err, " failed to run command '%s %s %s'", command, searchedPhrase, messagesFile)
	}
	output := string(outputRaw)

	if res := strings.Contains(output, strings.Replace(searchedPhrase, "\\", "", -1)); !res {
