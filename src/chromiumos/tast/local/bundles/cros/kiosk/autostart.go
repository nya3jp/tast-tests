...
func Autostart(ctx context.Context, s *testing.State) {
	...
	// Checking the log for starting Kiosk mode.
	command := "grep"
	searchedPhrase := "Starting\\ kiosk\\ mode"
	...
	if err != nil {
		s.Errorf("Failed to run command %s %s %s", command, searchedPhrase, messagesFile)
	}

	output := string(outputRaw)

	if res := strings.Contains(output, strings.Replace(searchedPhrase, "\\", "", -1)); res == false {
		s.Fatalf("Failed to find indication of Kiosk mode in log file. Command [%s %s %s], result [%s]", command, searchedPhrase, messagesFile, output)
	}
}
