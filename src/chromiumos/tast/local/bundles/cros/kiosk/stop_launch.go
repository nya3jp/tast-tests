...
func StopLaunch(ctx context.Context, s *testing.State) {
	...
	// Restart Chrome without a signin profile test extension.
	if _, err := kiosk.RestartChromeWithOptions(
		ctx,
		chrome.NoLogin(),
		chrome.DMSPolicy(fdms.URL),
		chrome.KeepState()); err != nil {
		s.Fatal("Failed to restart Chrome for a fixture cleanup: ", err)
	}
}
