...
func FloatingAccessibilityMenuEnabledLacros(ctx context.Context, s *testing.State) {
	...
	cr, err := kioskmode.New(
		...
		// TODO(anqing): We need to parameterize the Chrome options to merge this test into another
		// existing one located at the `floating_accessibility_menu_enabled.go` file.
		kioskmode.ExtraChromeOptions(chrome.ExtraArgs("--enable-features=LacrosSupport,WebKioskEnableLacros", "--lacros-availability-ignore")),
