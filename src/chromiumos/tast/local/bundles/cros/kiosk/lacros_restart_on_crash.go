...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Checks that lacros properly restarts in webkiosk mode",
		Contacts: []string{
...
func LacrosRestartOnCrash(ctx context.Context, s *testing.State) {
	...
	// Wait until the splash screen disappeared and kiosk app is shown
	if _, err = reader.Wait(ctx, 60*time.Second, func(e *syslog.Entry) bool {
		return strings.Contains(e.Content, "closing splash screen")
	}); err != nil {
		s.Error("Failed to wait for log msg \"closing splash screen\": ", err)
	}
