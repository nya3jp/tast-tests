...
func SecondaryAppsInstalled(ctx context.Context, s *testing.State) {
	...
	if err := checkSecondaryAppAndExtension(ctx, cr, ui); err != nil {
...
func checkSecondaryAppAndExtension(ctx context.Context, cr *chrome.Chrome, ui *uiauto.Context) error {
	const (
	...
	testing.ContextLog(ctx, "Waiting for primary app to show up")
	testSecondaryAppBtn := nodewith.Name("Test secondary app").Focusable()
	if err := ui.WaitUntilExists(testSecondaryAppBtn)(ctx); err != nil {
		return errors.Wrap(err, "test secondary app button does not exist")
	}
	...
	testing.ContextLog(ctx, "Waiting for secondary app to show up")
	closeWindowButton := nodewith.Name("Close Window").Focusable()
	if err := ui.WaitUntilExists(closeWindowButton)(ctx); err != nil {
		return errors.Wrap(err, "close window button does not exist")
	}
