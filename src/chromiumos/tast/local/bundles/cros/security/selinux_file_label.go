...
func SELinuxFileLabel(s *testing.State) {
	system_cpu_filter := func(p string, fi os.FileInfo) bool {
		mode := fi.Mode()
	...
	for _, testArg := range []struct {
		...
		{"/sys/devices/system/cpu", "u:object_r:sysfs_devices_system_cpu:s0", true, system_cpu_filter},
		{"/sys/devices/system/cpu", "u:object_r:sysfs:s0", true, selinux.InvertFilter(system_cpu_filter)},
	} {
