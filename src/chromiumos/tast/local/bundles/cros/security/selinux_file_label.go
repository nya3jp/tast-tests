...
func SELinuxFileLabel(ctx context.Context, s *testing.State) {
	systemCPUFilter := func(p string, fi os.FileInfo) (bool, bool) {
		mode := fi.Mode()
	...
	testArgs := []struct {
		...
		{"/bin", "u:object_r:cros_coreutils_exec:s0", true, selinux.InvertFilterSkipThis(selinux.IsCoreutilsFile)},
		...
		{"/sys/devices/system/cpu", "u:object_r:sysfs:s0", true, selinux.InvertFilterSkipThis(systemCPUFilter)},
		...
		{"/sys/fs/selinux", "u:object_r:null_device:s0", true, selinux.InvertFilterSkipThis(selinux.IgnorePathItself("/sys/fs/selinux/null"))},
		...
		{"/usr/bin", "u:object_r:cros_coreutils_exec:s0", true, selinux.InvertFilterSkipThis(selinux.IsCoreutilsFile)},
