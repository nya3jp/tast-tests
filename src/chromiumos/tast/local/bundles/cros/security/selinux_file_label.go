...
func SELinuxFileLabel(s *testing.State) {
	for _, testArg := range []struct {
		...
		{"/sys/fs/selinux", "u:object_r:null_device:s0", true, selinux.FilterReverse(selinux.IgnorePath("/sys/fs/selinux/null"))},
		...
		{"/sys/kernel/debug/trace_marker", "u:object_r:debugfs_trace_marker:s0", false, selinux.SkipNonExist},
		{"/sys/devices/system/cpu", "u:object_r:sysfs_devices_system_cpu:s0", true, func(p string, f os.FileInfo) (bool, error) {
			mode := f.Mode()
		...
		if testArg.filter != nil {
			ret, err := testArg.filter(testArg.path, stat)
			if err != nil {
				s.Error(err)
			}
			filterResult = ret
		...
		if !filterResult {
			err := selinux.CheckFileContext(testArg.path, testArg.context)
			if err != nil {
				s.Error(err)
			}
		}
		if testArg.recursive {
			selinux.CheckFileContextRecursively(s, testArg.path, testArg.context, testArg.filter)
		}
	}
