...
// If SkipChidlren is included in a pattern , any matched directories' chidren are skipped.
// A map from absolute path names to strings describing problems is returned,
// along with the number of paths (not including ones skipped by SkipChidlren) that were inspected.
