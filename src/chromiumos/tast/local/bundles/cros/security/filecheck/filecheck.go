...
type Pattern struct {
	...
	skipContents bool         // should contents (if this is a dir) be skipped?
}
...
// SkipContents indicates that any child paths should not be checked.
// The directory itself will still be checked. This has no effect for non-directories.
func SkipContents() Option { return func(p *Pattern) { p.skipContents = true } }
...
// Tree returns a Matcher that matches both path and its contents.
...
// If SkipContents is included in a pattern , any matched directories' contents are skipped.
// A map from absolute path names to strings describing problems is returned,
// along with the number of paths (not including ones skipped by SkipContents) that were inspected.
