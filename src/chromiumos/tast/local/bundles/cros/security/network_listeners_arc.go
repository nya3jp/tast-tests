...
func NetworkListenersARC(ctx context.Context, s *testing.State) {
	...
	// arc-networkd runs an ADB proxy server on port 5555 whenever ARC is running. The proxy end listens only when ADB sideloading or ADB debugging on dev mode is enabled.
	ls["*:5555"] = "/usr/bin/arc-networkd"
