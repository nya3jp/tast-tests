...
func Mtab(ctx context.Context, s *testing.State) {
	...
	// a bit if the system appears to support Android.
	if upstart.JobExists(ctx, "arc-adbd") { // arbitrary Android-related job
		s.Logf("Waiting for Android mounts")
		const adbdMount = "/run/arc/adbd" // arbitrary Android-related mount
		notMountedErr := errors.Errorf("%v not mounted", adbdMount)
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			mounts, err := readMtab(liveMtab)
			if err != nil {
				return err
			}
			for _, m := range mounts {
				if m.mount == adbdMount {
					return nil
				}
			}
	...
	const defaultRW = "rw,nosuid,nodev,noexec"         // default options for read/write mounts
	...
	expMounts := map[string]mountSpec{
		...
		"/run/arc/media":                     {nil, "tmpfs", defaultRW + ",mode=755"},
		"/run/arc/obb":                       {nil, "tmpfs", defaultRW + ",mode=755"},
		"/run/arc/oem":                       {nil, "tmpfs", defaultRW + ",mode=755"},
		"/run/arc/sdcard":                    {nil, "tmpfs", defaultRW + ",mode=755"},
		...
		"/usr/share/oem":                     {nil, "ext4", "ro,nosuid,nodev,noexec"},
