...
func SELinuxProcessContext(ctx context.Context, s *testing.State) {
	...
	for _, testCase := range []struct {
		...
		case cmdline:
			p = selinux.FindProcessesByCmdline(ps, testCase.query)
		default:
			s.Errorf("%+v has invalid searchType %d", testCase, int(testCase.field))
			continue
		}
		if len(p) == 0 {
			s.Errorf("Cannot find processes for test field %v, query %q", testCase.field, testCase.query)
			continue
		}
		s.Logf("Found %d processes for test case %v: %v", len(p), testCase, p)
