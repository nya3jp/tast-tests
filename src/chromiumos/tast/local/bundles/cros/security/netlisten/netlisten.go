...
// DefaultListeners returns standard expected listeners (in the format expected by CheckPorts).
// The result may differ depending on whether the DUT supports ARC or not.
func DefaultListeners(ctx context.Context, cr *chrome.Chrome) map[string]string {
	listeners := map[string]string{
		cr.DebugAddrPort(): "/opt/google/chrome/chrome",
		// ADB may be running due to earlier ARC testing even if ARC isn't active currently.
	...
	if upstart.JobExists(ctx, "sslh") {
		listeners["*:2222"] = sshdExe
		listeners["*:22"] = sslhExe
	} else {
		listeners["*:22"] = sshdExe
	}
