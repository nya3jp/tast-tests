...
func ShellNoexec(ctx context.Context, s *testing.State) {
	...
	runShell := func(shell string, should_fail, is_exec_path bool, path string) {
		...
		if is_exec_path {
			...
			if string(out) != "out\n" {
				s.Errorf("%v [%v] output is incorrect: %s", shell, script, out)
				cmd.DumpLog(ctx)
				return
			}
		} else {
			...
			if should_fail {
				if err == nil {
					s.Errorf("%v [%v] incorrectly exited zero: %v", shell, script, out)
					cmd.DumpLog(ctx)
					return
				}
			} else {
			...
			// We patched the shells to display a warning message in this case.
			search := "noexec mount"
			if !strings.Contains(string(out), search) {
				s.Errorf("%v could not find %q in output: %s", shell, search, out)
				cmd.DumpLog(ctx)
				return
			}
		}
	...
	runTestCase := func(is_exec_path bool, path string) {
		runShell("/bin/sh", true, is_exec_path, path)
		runShell("/bin/dash", true, is_exec_path, path)
		runShell("/bin/bash", false, is_exec_path, path)
	}
