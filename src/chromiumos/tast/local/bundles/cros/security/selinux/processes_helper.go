...
// FindProcessesByCmdline returns processes from ps with Cmdline fields matching commRegex.
func FindProcessesByCmdline(ps []Process, commRegex string) []Process {
	var found []Process
	for _, proc := range ps {
		if matched, _ := regexp.MatchString(commRegex, proc.Cmdline); matched {
			found = append(found, proc)
		}
	}
	return found
