...
type FileLabelCheckFilter func(path string, fi os.FileInfo) (skipThis bool, skipSubdir bool)
...
func IgnorePathItself(pathToIgnore string) FileLabelCheckFilter {
	return func(p string, _ os.FileInfo) (bool, bool) { return p == pathToIgnore, false }
}

// CheckAll returns (false, nil) to let the test to check all files
func CheckAll(_ string, _ os.FileInfo) (bool, bool) { return false, false }
...
func InvertFilterSkipThis(filter FileLabelCheckFilter) FileLabelCheckFilter {
	return func(p string, fi os.FileInfo) (bool, bool) {
		skipThis, skipSubdir := filter(p, fi)
		return !skipThis, skipSubdir
	}
}
...
// subdirectory within path, unless the filter shows to skip subdir.
