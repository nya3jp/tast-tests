	...
	for _, sel := range testSelector {
		...
		case Stable:
			testCases = append(testCases, []testCaseType{
				...
				{cmdline, ".*(frecon|agetty|ping|recover_dts|udevadm|update_rw_vpd|mosys|vpd|flashrom|moblab|autotest|devserver|rotatelogs|apache2|envoy|containerd).*", notString, "chromeos", zeroProcs, domainIsolationErrorMessage},
