...
const (
	// StableTestCase to run test cases proven to be stable.
	StableTestCase SELinuxProcessTestCaseSelector = iota
	// NewOrFlakyTest to run newly introduced test cases or flaky cases.
	NewOrFlakyTest
)

// ProcessesTestInternal runs the test suite for SELinuxProcesses(Experimental)?
func ProcessesTestInternal(ctx context.Context, s *testing.State, testSelector []SELinuxProcessTestCaseSelector) {
