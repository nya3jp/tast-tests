...
func ProcessesTestInternal(ctx context.Context, s *testing.State, testSelector []ProcessTestCaseSelector) {
	...
	// noStr returns a regular expression to match all strings that are not equal to s.
	...
	for _, sel := range testSelector {
		...
		case Stable:
			testCases = append(testCases, []testCaseType{
				...
				{exe, "/usr/bin/chunneld", "cros_chunneld", zeroProcs, ""},
				...
				{exe, "/usr/bin/cups_proxy", "cros_cups_proxy", zeroProcs, ""},
				...
				{exe, "/usr/bin/seneschal", "cros_seneschal", zeroProcs, ""},
				...
				{exe, "/usr/bin/vm_cicerone", "cros_vm_cicerone", zeroProcs, ""},
				{exe, "/usr/bin/vm_concierge", "cros_vm_concierge", zeroProcs, ""},
				...
				{exe, "/usr/sbin/cdm-oemcrypto", "cros_cdm_oemcrypto", zeroProcs, ""},
				...
				{exe, "/usr/sbin/crosdns", "cros_crosdns", zeroProcs, ""},
				{exe, "/usr/sbin/cryptohomed", "cros_cryptohomed", zeroProcs, ""},
				{exe, "/usr/sbin/cryptohome-proxy", "cros_cryptohome_proxy", zeroProcs, ""},
		...
		case Unstable:
			testCases = append(testCases, []testCaseType{
				...
				{notExe, "/sbin/init", notStr("cros_init"), zeroProcs, domainIsolationErrorMessage},
				{notCmdline, ".*(ping|frecon|agetty|recover_duts).*", notStr("chromeos"), zeroProcs, domainIsolationErrorMessage},
