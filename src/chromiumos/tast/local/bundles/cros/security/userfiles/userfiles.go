...
func Check(ctx context.Context, s *testing.State, user string) {
	...
	checkPath(homeDir, []*chk.Pattern{
		chk.NewPattern(chk.Path("user"), chk.SkipContents()),
		chk.NewPattern(chk.Path("crash"), chk.SkipContents()),
		chk.NewPattern(chk.PathRegexp(`^u-`), chk.SkipContents()),
	...
	checkPath(userDir, []*chk.Pattern{
		chk.NewPattern(chk.Path("Downloads"), isChronosUID, isChronosAccessGID, chk.Mode(0710), chk.SkipContents()),
