...
func RootCA(ctx context.Context, s *testing.State) {
	...
	compareCerts := func(expected map[string]string, found map[string]string) {
		for fingerprint := range expected {
			if _, ok := found[fingerprint]; !ok {
				s.Error("Did not find expected cert with fingerprint ", fingerprint)
			}
		}
		for fingerprint := range found {
			if _, ok := expected[fingerprint]; !ok {
				s.Error("Found unexpected cert with fingerprint ", fingerprint)
			}
		}
	}
