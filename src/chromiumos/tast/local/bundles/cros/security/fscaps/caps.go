...
// Equal returns true if these capabilities exactly match o's capabilities.
func (c Caps) Equal(o Caps) bool {
	return c.Effective == o.Effective &&
		c.Inheritable == o.Inheritable &&
...
func (c Caps) String() string {
	...
	add := func(prefix string, n uint64) {
		...
		for i := 0; n != 0; i++ {
			if n&0x1 != 0 {
				names = append(names, capMaskToString(1<<uint32(i)))
			}
			n >>= 1
		}
		if len(names) > 0 {
