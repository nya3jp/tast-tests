...
func TestCapsEqual(t *testing.T) {
	caps := []Caps{
		Caps{},
		Caps{Effective: KILL},
		Caps{Effective: KILL | LEASE},
		Caps{Effective: KILL, Inheritable: LEASE},
		Caps{Effective: KILL, Inheritable: LEASE, Permitted: MKNOD},
	}

	for i := 0; i < len(caps); i++ {
		for j := 0; j < len(caps); j++ {
			self := i == j
			if eq := caps[i].Equal(caps[j]); eq != self {
				t.Errorf("%s.Equal(%s) = %v; want %v", caps[i], caps[j], eq, self)
			}
		}
	}
}
