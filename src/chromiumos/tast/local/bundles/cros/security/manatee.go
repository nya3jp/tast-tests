...
func Manatee(ctx context.Context, s *testing.State) {
	...
	// This isn't enough to notify the other end with EOF because fileOut is a duplicate of fileIn and it is a
	// bi-directional socket, so exit is explicitly called in the command.
	closeFileOut()
	...
	if response != expectedResponse {
		s.Errorf("Got %q; expected %q", response, expectedResponse)
	}
}
