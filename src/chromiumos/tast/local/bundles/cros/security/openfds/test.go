...
// compiledExpecatation array which matches to the given |path|.
func findExpectation(path string, es []compiledExpectation) (*compiledExpectation, error) {
	for _, e := range es {
...
func Expect(s *testing.State, ctx context.Context, asan bool, p *process.Process, es []Expectation) {
	...
	for _, f := range files {
		...
		if c, err := findExpectation(f.path, cs); err != nil {
			...
			// an error.
			s.Error(err)
		} else if !expectMode(f.lmode, c.modes) {
