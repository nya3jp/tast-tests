...
func PtraceProcess(ctx context.Context, s *testing.State) {
	const (
		sleeperPath = "/usr/local/libexec/security_tests/security.PtraceProcess.sleeper"
		sleepTime   = 120 * time.Second

		unprivUser = "chronos"
		// TODO(derat): Use sysutil constants once https://crrev.com/c/1519909 is in.
		unprivUID = 1000
		unprivGID = 1000
	)
	...
	startSleeper := func(tracerPID int, pidns bool) (*testexec.Cmd, error) {
		...
		go func() {
			b := make([]byte, 10)
			if n, err := stdout.Read(b); err != nil {
				ch <- err
			} else if msg := string(b[:n]); msg != "ready\n" {
				ch <- errors.Errorf("sleeper wrote %q", msg)
			} else {
				ch <- nil
			}
		}()
