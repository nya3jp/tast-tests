...
func StatefulFiles(ctx context.Context, s *testing.State) {
	...
	patterns := []*chk.Pattern{
		...
		chk.NewPattern(chk.Tree("encrypted/var/lib/oobe_config_restore"), users("oobe_config_restore"), groups("oobe_config_restore"), chk.Mode(700)),
		chk.NewPattern(chk.Tree("encrypted/var/lib/oobe_config_save"), users("oobe_config_save"), groups("oobe_config_save"), chk.Mode(700)),
