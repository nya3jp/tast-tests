...
func RunOCI(ctx context.Context, s *testing.State) {
	...
	defaultCfg := func() ociConfig {
		return ociConfig{
			...
			Mounts: []ociMount{
				ociMount{Destination: "/", Type: "bind", Source: "/", Options: []string{"rbind", "ro"}},
				ociMount{Destination: "/proc", Type: "proc", Source: "proc", Options: []string{"nodev", "noexec", "nosuid"}},
			},
			...
			Linux: ociLinux{
				Namespaces: []ociNamespace{
					ociNamespace{Type: "cgroup"},
					ociNamespace{Type: "pid"},
					ociNamespace{Type: "network"},
					ociNamespace{Type: "ipc"},
					ociNamespace{Type: "user"},
					ociNamespace{Type: "uts"},
					ociNamespace{Type: "mount"},
				},
				Resources: ociResources{
					Devices: []ociResourceDevice{
						ociResourceDevice{Allow: false, Access: "rwm"},
						ociResourceDevice{Allow: true, Type: "c", Major: 1, Minor: 5, Access: "r"},
					},
				},
				UIDMappings: []ociMapping{ociMapping{HostID: uid, ContainerID: 0, Size: 1}},
				GIDMappings: []ociMapping{ociMapping{HostID: gid, ContainerID: 0, Size: 1}},
			},
		}
	}
	...
	runTest := func(tc testCase) {
		// Create temp dir under /tmp to ensure that it's accessibly be the chronos user.
		...
		if err := os.Chown(td, uid, gid); err != nil {
			s.Fatal("Failed to chown temp dir: ", err)
		}
		...
		args = append(args, []string{"run", "-c", td, "test_container"}...)
