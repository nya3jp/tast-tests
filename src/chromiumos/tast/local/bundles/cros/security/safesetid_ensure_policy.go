...
func SafesetidEnsurePolicy(ctx context.Context, s *testing.State) {
	...
	for _, tc := range []struct {
		...
		if ver.IsOrLater(5, 10) {
			...
			s.Logf("Kernel version is too old for transitionSetGID, (%v), safesetid needs (5,10)", ver)
		}
	}
}
...
func checkUsersandGroupsExist() error {
	...
	for _, val := range usersList {
		_, errUser := user.Lookup(val.Name)
		if errUser != nil {
			return errors.Wrap(errUser, val.Name)
		}
		_, errGroup := user.LookupGroup(val.Name)
		if errGroup != nil {
			return errors.Wrap(errGroup, val.Name)
		}
	}
