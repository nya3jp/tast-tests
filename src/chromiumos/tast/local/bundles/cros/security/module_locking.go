...
func ModuleLocking(ctx context.Context, s *testing.State) {
	...
	modulePath := filepath.Join(moduleDir, "kernel/kernel", module+".ko")
	if _, err := os.Stat(modulePath); err != nil {
		s.Fatalf("Failed to find %q module: %v", module, err)
	}
	s.Log("Using module ", modulePath)

	// Unloads the module if it's currently loaded.
	unload := func() {
...
// Errors cause fatal test errors to be reported via s.
