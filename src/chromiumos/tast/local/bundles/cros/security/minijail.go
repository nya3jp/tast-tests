...
func Minijail(ctx context.Context, s *testing.State) {
	...
	// Landlock test cases where a nonzero exit code is expected.

	// Skip these tests if Landlock isn't supported.
	rawOutput, err := exec.Command("uname", "-r").Output()
	if err != nil {
		s.Log("something failed: ", err)
	}
	out := string(rawOutput)
	split := strings.Split(out, ".")
	splitStr := string(split[0] + "." + split[1])
	s.Log("The date is: ", splitStr)
	version, err := strconv.ParseFloat(splitStr, 64)
	if err != nil {
		s.Log("something failed: ", err)
	}
	s.Log("The date is: ", version)
