	...
	for _, tc := range []testCase{
		{
			...
			check: checkRegexp(`\bCapEff:\s*0000000000000002\b`),
		},
		...
		mountTestCase("chroot-tmp-ro-is-read-only", "if echo x > /tmp-ro/test-ro; then exit 1; fi", chrootArgs),
		{
		...
		},
		{
			...
			args64: []string{"-b", "/lib64,/lib64"},
			setup:  subdirSetup("c/bin", "c/lib64", "c/lib", "c/usr/lib", "c/usr/local"),
		},
		{
			name:  "pidns",
			cmd:   "echo $$",
		...
		mountTestCase("pivotroot-tmp-ro-is-read-only", "if echo x > /tmp-ro/test-ro; then exit 1; fi", pivotrootArgs),
		{
			name: "remount",
			cmd:  "if [ -w /proc/sys/kernel/printk ]; then exit 1; fi",
			args: []string{"-r"},
		},
		{
			name:  "rlimits",
