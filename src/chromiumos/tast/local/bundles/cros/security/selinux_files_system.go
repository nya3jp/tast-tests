...
func SELinuxFilesSystem(ctx context.Context, s *testing.State) {
	...
	for _, entryTree := range renderDs {
		...
		if err != nil {
			s.Fatal("unable to resolve absolute deviceReal: ", err)
		}
	...
	iioSensorFilter := func(p string, fi os.FileInfo) (skipFile, skipSubdir selinux.FilterResult) {
	...
	iioDevTrees, err := filepath.Glob("/sys/bus/iio/devices/iio:device*")
	for _, entryTree := range iioDevTrees {
		name, err := ioutil.ReadFile(filepath.Join(entryTree, "name"))
		if err != nil {
		...
		if strings.HasPrefix(string(name), "cros-ec") {
			testArgs = append(
				testArgs,
				[]selinux.FileTestCase{
					{Path: entryTree, Context: "cros_sensor_hal_sysfs", Recursive: true, Filter: iioSensorFilter, Log: false},
				}...)
		}
	}
