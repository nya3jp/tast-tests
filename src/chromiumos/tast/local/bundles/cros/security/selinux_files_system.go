...
func SELinuxFilesSystem(ctx context.Context, s *testing.State) {
	...
	testArgs := []selinux.FileTestCase{
		...
		{Path: "/run/metrics/external/crash-reporter", Context: "cros_run_metrics_external_crash", Recursive: false, Filter: selinux.IgnorePaths([]string{
			"/run/metrics/external/crash-reporter/unclean-shutdown-detected", "/run/metrics/external/crash-reporter/kernel-crash-detected"}), Log: false},
