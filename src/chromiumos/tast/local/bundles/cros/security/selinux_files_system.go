...
func SELinuxFilesSystem(ctx context.Context, s *testing.State) {
	testArgs := []selinux.FileTestCase{
		...
		{Path: "/run/metrics/external", Context: "cros_run_metrics_external", Recursive: false, Filter: selinux.IgnorePaths([]string{
			"/run/metrics/external/unclean-shutdown-detected", "/run/metrics/external/kernel-crash-detected"}), Log: false},
