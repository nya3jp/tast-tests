...
func SELinuxFilesSystem(ctx context.Context, s *testing.State) {
	...
	testArgs := []struct {
		...
		{"/etc/conntrackd", "cros_conntrackd_conf_file", true, nil},
		{"/etc/dhcpcd.conf", "cros_network_conf_file", false, nil},
		...
		{"/etc/modprobe.d", "cros_modprobe_conf_file", true, nil},
		...
		{"/etc/systemd/network", "cros_network_conf_file", true, nil},
		{"/etc/udev", "cros_udev_conf_file", true, nil},
