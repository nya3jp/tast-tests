...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Checks SELinux files contexts when the environment has ARC",
		Attr:         []string{"informational"},
		SoftwareDeps: []string{"android", "selinux"},
	})
}
...
func SELinuxFilesARC(ctx context.Context, s *testing.State) {
	...
	containerPidFileNames, err := filepath.Glob("/run/containers/android*/container.pid")
	if err != nil {
		s.Fatal("Failed to find container.pid file: %v", err)
	}
	if len(containerPidFileNames) != 1 {
		s.Fatal("Failed to locate current container.pid file")
	}
	containerPidFileName := containerPidFileNames[0]
	containerPidFile, err := os.Open(containerPidFileName)
	if err != nil {
		s.Fatalf("Failed to open container.pid file: %q; err: %v", containerPidFileName, err)
	}
	defer containerPidFile.Close()

	scanner := bufio.NewScanner(containerPidFile)
	if !scanner.Scan() {
		s.Fatalf("No content in container.pid file: %q", containerPidFileName)
	}
	containerPid := scanner.Text()
	androidRoot := fmt.Sprintf("/proc/%s/root", containerPid)
