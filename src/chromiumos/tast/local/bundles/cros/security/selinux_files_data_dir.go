...
func createSELinuxPolicyFile(ctx context.Context) error {
	...
	cmd := fmt.Sprintf("grep '%s' %s > %s", shutil.Escape("^/data"), shutil.Escape(androidFileContexts), shutil.Escape(dataDirFileContexts))
...
func verifyDirSELinuxContext(ctx context.Context, directoryPath, outDir string) error {
	...
	matchPathConCmd := fmt.Sprintf("find %s -exec matchpathcon -f %s -V {} \\;", directoryPath, dataDirFileContexts)
	...
	// All the files should be SELinux verified, else trigger an error.
	verifiedCount, err := testexec.CommandContext(ctx, "grep", "-c", "verified", matchPathConFileLocation).Output(testexec.DumpLogOnError)
...
func SELinuxFilesDataDir(ctx context.Context, s *testing.State) {
	...
	if !ok {
		s.Fatal("Failed to get the test output directory")
	}
	for _, dir := range dirList {
		// If the directory exists in the skip directory map, do not verify its SELinux context.
		if _, exists := skipDirMap[dir.Name()]; exists == true {
			continue
		}
		directoryPath := filepath.Join(dataDirPath, dir.Name())
		if err := verifyDirSELinuxContext(ctx, directoryPath, outDir); err != nil {
			s.Log("Look at the matchpathcon output here: ", filepath.Join(outDir, matchPathConFileName))
