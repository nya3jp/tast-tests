...
func createSELinuxPolicyFile(ctx context.Context) error {
	...
	err = ioutil.WriteFile(dataDirFileContexts, []byte(cmdOutput), 0644)
	if err != nil {
...
func verifyDirSELinuxContext(ctx context.Context, directoryPath, outDir string) error {
	...
	matchPathConOutput, err := testexec.CommandContext(ctx, "sh", "-c", matchPathConCmd).Output(testexec.DumpLogOnError)
	if err != nil {
		return err
	}
