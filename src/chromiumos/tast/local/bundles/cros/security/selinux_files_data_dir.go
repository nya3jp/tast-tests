...
func verifyDirSELinuxContext(ctx context.Context, directoryPath, outDir string) error {
	...
	for scanner.Scan() {
		...
		if !strings.Contains(line, "verified") {
			...
			selinuxRegexp := regexp.MustCompile(`\w:\w*:\w*:\w*(\S)*`)
			matches := selinuxRegexp.FindAllString(line, -1)
			...
			if len(matches) == 2 {
				...
				// Example:
				// Actual context - u:object_r:shared_relro_file:s0:c13,c260
				// Expected context - u:object_r:shared_relro_file:s0
