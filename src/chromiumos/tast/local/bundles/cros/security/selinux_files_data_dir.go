...
const (
	...
	matchPathConOutputFile = `/tmp/matchpath_con_output`
)
...
func verifyDirSELinuxContext(ctx context.Context, directoryPath string) error {
	...
	tempFile, err := os.Create(matchPathConOutputFile)
	if err != nil {
		return errors.Wrap(err, "error creating matchpathcon output file")
	}
	defer tempFile.Close()
	if _, err = tempFile.WriteString(string(matchPathConOutput)); err != nil {
		return err
	}

	verifiedCount, err := testexec.CommandContext(ctx, "grep", "-c", "verified", matchPathConOutputFile).Output(testexec.DumpLogOnError)
