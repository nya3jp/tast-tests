...
func PrivilegedFiles(ctx context.Context, s *testing.State) {
	// makeSet splits s on newlines, trims whitespace, and returns a set for fast lookup.
	// Lines beginning with "#" after trimming are ignored.
	makeSet := func(s string) map[string]struct{} {
	...
	walkErr := filepath.Walk("/", func(path string, fi os.FileInfo, err error) error {
		...
		} else if exp := capsBaseline[path]; !caps.Empty() && !caps.Equal(exp) {
