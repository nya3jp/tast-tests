...
func RunFiles(ctx context.Context, s *testing.State) {
	...
	patterns := []*chk.Pattern{
		...
		chk.NewPattern(chk.Path("arcvm/host_generated/oem/etc/media_profiles.xml"), chk.Users(s, "arc-camera"), chk.Groups(s, "arc-camera"), chk.Mode(0644)),
		chk.NewPattern(chk.Path("arcvm/host_generated/oem/etc/permissions/platform.xml"), chk.Users(s, "crosvm"), chk.Groups(s, "crosvm"), chk.Mode(0644)),
	}
