...
func USBGuard(ctx context.Context, s *testing.State) {
	...
	setScreenLocked := func(locked bool) error {
		...
		if locked {
			dbusMethod = ".LockScreen"
		} else {
			dbusMethod = ".HandleLockScreenDismissed"
		}

		_, obj, err := dbusutil.Connect(ctx, dbusName, dbus.ObjectPath(dbusPath))
		if err != nil {
			return errors.Wrap(err, "D-Bus connect failed while setting the lock-screen state")
		}

		if err = obj.CallWithContext(ctx, dbusInterface+dbusMethod, 0).Store(); err != nil {
			return errors.Wrapf(err, "failed to invoke %q", dbusInterface+dbusMethod)
		}
