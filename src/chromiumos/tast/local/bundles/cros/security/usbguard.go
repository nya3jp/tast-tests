...
func USBGuard(ctx context.Context, s *testing.State) {
	...
	setScreenLocked := func(locked bool) error {
		const (
		...
		var dbusMethod string
		if locked {
			dbusMethod = dbusInterface + ".LockScreen"
		} else {
			dbusMethod = dbusInterface + ".HandleLockScreenDismissed"
		}
		...
		if err = obj.CallWithContext(ctx, dbusMethod, 0).Store(); err != nil {
	...
	checkUsbguardRespawn := func() error {
		...
		c := testexec.CommandContext(ctx, "kill", strconv.Itoa(pid))
		_, err = c.Output()
		if err != nil {
			c.DumpLog(ctx)
			err = errors.Wrapf(err, "failed to kill %v(%i)", usbguardProcess, pid)
		}
		return err
	}
	...
	runTest := func(usbguardEnabled, usbbouncerEnabled bool) {
		...
		for name, val := range featureValues {
			if en, err := isFeatureEnabled(name); err != nil {
				s.Errorf("Failed checking if the %v feature is enabled: ", err)
				return
			} else if en != val {
		...
		if err = setScreenLocked(true); err != nil {
			s.Errorf("Failed to lock the screen: ", err)
			return
		}
		...
		if err = setScreenLocked(false); err != nil {
			s.Errorf("Failed to unlock the screen: ", err)
			return
		}
		if err = expectUsbguardRunning(false /*running*/, false /*onLockScreen*/); err != nil {
