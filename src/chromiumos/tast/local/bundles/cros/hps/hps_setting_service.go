...
// ChangeSettings changes the settings under the screen privacy page depending on the needs
func (hss *SettingService) ChangeSettings(ctx context.Context, req *pb.NewHpsSettingRequest) (*empty.Empty, error) {
	cr, err := chrome.New(ctx, chrome.ExtraArgs("--enable-feature=QuickDim"))
...
func (hss *SettingService) CheckForLockScreen(ctx context.Context, req *empty.Empty) (*empty.Empty, error) {
	...
	if tconn == nil {
		return nil, errors.New("no connection setup for chrome yet")
	}
