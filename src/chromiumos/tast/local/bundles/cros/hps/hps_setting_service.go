...
func (hss *SettingService) GetDelays(ctx context.Context, quickDimEnabled *wrappers.BoolValue) (*pb.DimMetrics, error) {
	...
	dimTime, err = getMetric(ctx, newestSettings, dimKey)
	if err != nil {
		return nil, errors.Wrap(err, "error getting dim delay")
	}
	screenOffTime, err = getMetric(ctx, newestSettings, screenOffKey)
	if err != nil {
...
func getMetric(ctx context.Context, settings []byte, key string) (time.Duration, error) {
