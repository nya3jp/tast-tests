...
func init() {
	testing.AddTest(&testing.Test{
		...
		Timeout: 6*chrome.LoginTimeout + 45*time.Second,
	})
}
...
func OfflineLoginWithUsernameAndPhotosDisabled(ctx context.Context, s *testing.State) {
	...
	creds := []chrome.Creds{
		...
		{User: "additionalUser2@gmail.com", Pass: "test pass 3"},
	}

	cleanUpCtx := ctx
	// 30 seconds should be enough for all clean up operations.
	ctx, cancel := ctxutil.Shorten(ctx, 400*time.Second)
	defer cancel()
	...
	loginOffline := func(ctx context.Context) error {
		...
		const uiTimeout = 5000 * time.Second
...
func clickLink(ctx context.Context, s *testing.State, ui *uiauto.Context) {
