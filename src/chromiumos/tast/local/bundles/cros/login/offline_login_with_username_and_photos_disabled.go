...
func (h *helper) disableAllNetworkInterfaces(ctx context.Context) error {
	...
	defer cancel()
	// Disable Ethernet and/or WiFi if present and maybe re-enabling.
	if h.enableEthernetFunc, err := h.Manager.DisableTechnologyForTesting(ctx, shill.TechnologyEthernet); err != nil {
		return errors.Wrap(err, "unable to disable Ethernet")
	}

	if h.enableWifiFunc, err := h.Manager.DisableTechnologyForTesting(ctx, shill.TechnologyWifi); err != nil {
		return errors.Wrap(err, "unable to disable Wifi")
	}

	if h.enableCellularFunc, err := h.Manager.DisableTechnologyForTesting(ctx, shill.shill.TechnologyCellular); err != nil {
