...
func RemoveExistingUser(ctx context.Context, s *testing.State) {
	// KnownUser has several fields stored in Local State, but in this test only email is used.
	type KnownUser struct {
		Email string `json:"email"`
	}

	// LocalState is a json like structure, from which we will need only KnownUsers field.
	type LocalState struct {
		KnownUsers []KnownUser `json:"KnownUsers"`
	}
	...
	createUser := func(creds chrome.Creds, keep_state bool) {
		opts := []chrome.Option{chrome.FakeLogin(creds)}
		if keep_state == true {
	...
	createUser(chrome.Creds{User: user1, Pass: password}, false)
	createUser(chrome.Creds{User: user2, Pass: password}, true)
	createUser(chrome.Creds{User: user3, Pass: password}, true)
	...
	if err := ui.WaitUntilGone(nodewith.Name("user3@gmail.com").Role(role.Button))(ctx); err != nil {
		s.Fatal("Removed user pod is still reachable: ", err)
	}
	// Check that there is no user3@gmail.com in known users list.
	...
	for _, knownUser := range localState.KnownUsers {
		knownEmails[knownUser.Email] = true
	}
	if knownEmails["user3@gmail.com"] {
	...
	if err := ui.WaitUntilExists(nodewith.Name("user3@gmail.com").Role(role.Button))(ctx); err == nil {
		s.Fatal("user3@gmail.com user pod is still reachable after restart")
	}
}
