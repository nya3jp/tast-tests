...
func manageUsersInSettings(ctx context.Context, s *testing.State, deviceOwner, ownersPassword, userToRemove string) {
	...
	settings, err := ossettings.LaunchAtPageURL(ctx, tconn, cr, "osPrivacy/accounts", func(context.Context) error { return nil })
	if err != nil {
	...
	// this also seems to work, but isn't nice
	// ossettings.LaunchAtPageURL(ctx, tconn, cr, "osAccessibility", func(context.Context) error { return nil })
	// ossettings.LaunchAtPageURL(ctx, tconn, cr, "osPrivacy/accounts", func(context.Context) error { return nil })

	if err := uiauto.Combine("show users",
		settings.WaitUntilToggleOption(cr, optionName, false),
		ui.WaitUntilExists(nodewith.Name(optionName).Role(role.ToggleButton)),
		settings.SetToggleOption(cr, optionName, true),
	)(ctx); err != nil {
	...
	if err := uiauto.Combine(removeButtonName,
		...
		settings.LeftClick(nodewith.Name(removeButtonName).Role(role.Button)),
	)(ctx); err != nil {
