...
func setupUsers(ctx context.Context, s *testing.State) {
	userutil.CreateUser(ctx, s, deviceOwner, commonPassword)

	userutil.CreateUser(ctx, s, additionalUser1, commonPassword, chrome.KeepState())
	userutil.CreateUser(ctx, s, additionalUser2, commonPassword, chrome.KeepState())
}
