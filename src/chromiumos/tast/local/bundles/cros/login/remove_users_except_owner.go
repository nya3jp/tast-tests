...
func manageUsersInSettings(ctx context.Context, s *testing.State, deviceOwner, ownersPassword, userToRemove string) {
	...
	if err := uiauto.Combine("open subsettings",
		ui.WaitUntilExists(nodewith.Name(subsettingsName)),
		ui.LeftClick(nodewith.Name(subsettingsName)),
	)(ctx); err != nil {
		s.Fatal(err, "failed to open subsettings to manage other users: ", err)
	}
	...
	removeButtonFound, err := ui.IsNodeFound(ctx, nodewith.Name("Remove "+getUsernameFromEmail(deviceOwner)).Role(role.Button))
	if err != nil {
