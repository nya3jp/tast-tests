...
func SignOutAll(ctx context.Context, s *testing.State) {
	...
	if err := signIn(ctx, tconn, creds); err != nil {
...
// signIn executes multiple sign-in via quick settings.
func signIn(ctx context.Context, tconn *chrome.TestConn, creds []chrome.Creds) error {
	var (
...
func signOutAllAndWait(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if err := uiauto.New(tconn).MouseMoveTo(nodewith.Name("Sign out all").HasClass("PillButton"), 0)(ctx); err != nil {
		return errors.Wrap(err, "failed to to move to the location of sign out button")
	}
