...
func handleSAMLLogin(ctx context.Context, tconn *chrome.TestConn, username, password string) error {
...
func ChromeSAML(ctx context.Context, s *testing.State) {
	...
	if err := handleSAMLLogin(ctx, tconn, username, password); err != nil {
		s.Fatal("Failed to handle SAML login: ", err)
	}
