...
func testChromeLogin(ctx context.Context, s *testing.State, sm *session.SessionManager, params *chromeTestParams, url, expected string) {
	...
	cr, _, closeBrowser, err := browserfixt.SetUpWithNewChrome(ctx, params.bt, lacrosfixt.NewConfig(), opts...)
	if err != nil {
	...
	conn, _, _, err := browserfixt.SetUpWithURL(ctx, cr, params.bt, url)
	if err != nil {
