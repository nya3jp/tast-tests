...
func findScanner(ctx context.Context, devInfo usbprinter.DevInfo) (bus, device string, err error) {
	outputBytes, err := testexec.CommandContext(ctx, "lsusb", "-d", fmt.Sprintf("%s:%s", devInfo.VID, devInfo.PID)).Output(testexec.DumpLogOnError)
	output := string(outputBytes)

	colonIndex := strings.Index(output, ":")
	if colonIndex == -1 {
		return "", "", errors.Wrap(err, "failed to find ':' in lsusb output")
	}

	tokens := strings.Split(output[:colonIndex], " ")
