...
func killIppUsbBridge(ctx context.Context, devInfo usbprinter.DevInfo) error {
	...
	for _, p := range ps {
		...
		if err := syscall.Kill(int(p.Pid), syscall.SIGINT); err != nil {
			// ippusb_bridge might have already timed out and exited.
			// Just log the error and continue instead of failing, since the
			// net effect is that ippusb_bridge is gone.
			testing.ContextLog(ctx, "Failed to kill ippusb_bridge: ", err)
		}
		...
		os.Remove(fmt.Sprintf("/run/ippusb/%s-%s.sock", devInfo.VID, devInfo.PID))
		os.Remove(fmt.Sprintf("/run/ippusb/%s-%s_keep_alive.sock", devInfo.VID, devInfo.PID))
	}
	return nil
}
...
func ScanESCLIPP(ctx context.Context, s *testing.State) {
	...
	if testOpt.Network {
		...
		defer killIppUsbBridge(ctx, devInfo)
	}
