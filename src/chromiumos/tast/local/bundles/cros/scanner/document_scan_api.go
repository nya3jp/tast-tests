...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Tests the DocumentScan API",
...
// DocumentScanAPI tests the chrome.documentScan API.
func DocumentScanAPI(ctx context.Context, s *testing.State) {
	...
	s.Log("Waiting for scan to complete")
	scanCompleteParams := ui.FindParams{Role: ui.RoleTypeTextField}
	scanCompleteNode, err := ui.FindWithTimeout(ctx, tconn, scanCompleteParams, 10*time.Second)
	if err != nil {
		s.Fatal("Failed to find scan complete node: ", err)
	}
	defer scanCompleteNode.Release(ctx)
	...
	imageParams := ui.FindParams{Role: ui.RoleTypeImage}
	imageNode, err := ui.FindWithTimeout(ctx, tconn, imageParams, 5*time.Second)
	if err != nil {
		s.Fatal("Failed to find scanned image: ", err)
	}
	defer imageNode.Release(ctx)

	s.Log("Getting image info")
	...
	_, err = scanFile.Write(imageData)
	if err != nil {
