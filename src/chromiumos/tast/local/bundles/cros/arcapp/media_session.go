...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:         []string{"media_session_test.apk", "media_session_60sec_test.ogg", "media_session_test.html"},
	})
}
...
func MediaSession(s *testing.State) {
	...
	apptest.RunWithChrome(s, cr, apk, pkg, cls, func(a *arc.ARC, d *ui.Device) {
		...
		// Wait for the entries to show that we have acquired audio focus.
		...
		// Check that Chrome has lost audio focus.
