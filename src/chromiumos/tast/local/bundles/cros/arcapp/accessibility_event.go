...
// cvoxExtConn returns connection to the ChromeVox extension's background page.
...
// Polls until UI element has been checked, otherwise returns error after 30 seconds.
...
func AccessibilityEvent(ctx context.Context, s *testing.State) {
	...
	if err := isElementChecked(ctx, chromeVoxConn, checkBox); err != nil {
		s.Fatalf("Failed to check toggled state: ", err)
	}
