...
func Sample(s *testing.State) {
	const (
		...
		apkName      = "todo-mvp.apk"
		packageName  = "com.example.android.architecture.blueprints.todomvp"
		activityName = "com.example.android.architecture.blueprints.todoapp.tasks.TasksActivity"
	...
	apptest.Run(s, apkName, packageName, activityName, func(a *arc.ARC, d *ui.Device) {
		// Wait for the default entries to show up.
		must(d.NewObject(ui.ID(titleID), ui.Text(defaultTitle1)).WaitForExists(ctx))
		must(d.NewObject(ui.ID(titleID), ui.Text(defaultTitle2)).WaitForExists(ctx))

		// Click the add button.
		must(d.NewObject(ui.ID(addButtonID)).Click(ctx))

		// Fill the form and click the done button.
		input := d.NewObject(ui.ID(titleInputID))
		must(input.WaitForExists(ctx))
		must(input.SetText(ctx, customTitle))
		must(d.NewObject(ui.ID(doneButtonID)).Click(ctx))

		// Wait for our new entry to show up.
		must(d.NewObject(ui.ID(titleID), ui.Text(customTitle)).WaitForExists(ctx))
	})
}
