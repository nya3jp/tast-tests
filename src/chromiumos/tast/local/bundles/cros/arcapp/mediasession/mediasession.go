...
const (
	...
	// CheckChromeIsPlaying is a JS expression that can be executed in Chrome to check if the test page is playing. The test page needs to have been opened previously.
	CheckChromeIsPlaying = "!audio.paused"

	// CheckChromeIsPaused is a JS expression that can be executed in Chrome to check if the test page is paused. The test page needs to have been opened previously.
...
func WaitForAndroidAudioFocusGain(ctx context.Context, d *ui.Device, focusType AudioFocusType) error {
	...
	if err := d.Object(ui.ID(currentFocusID), ui.Text(string(focusType))).WaitForExists(ctx); err != nil {
		return err
	}
