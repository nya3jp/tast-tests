	...
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			Val: screenshare.TestParams{
				...
				PolicyDLP:   screenshare.ScreenshareBlockPolicy,
			...
			Val: screenshare.TestParams{
				...
				PolicyDLP:   screenshare.ScreenshareWarnPolicy,
				BrowserType: browser.TypeAsh,
			},
		}, {
...
func DataLeakPreventionRulesListScreenshareWindow(ctx context.Context, s *testing.State) {
	...
	if conn, err = br.NewConn(ctx, screenshare.NonRestrictedSite); err != nil {
		s.Fatal("Failed to open page: ", err)
	}

	if err := webutil.WaitForQuiescence(ctx, conn, 10*time.Second); err != nil {
		s.Fatalf("Failed to wait for %q to achieve quiescence: %v", screenshare.NonRestrictedSite, err)
	}
	...
	if err := webutil.WaitForQuiescence(ctx, conn, 10*time.Second); err != nil {
		s.Fatalf("Failed to wait for %q to achieve quiescence: %v", params.URL, err)
	}
