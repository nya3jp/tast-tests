...
func DataLeakPreventionRulesListScreenshare(ctx context.Context, s *testing.State) {
	...
	// Checking the frame status immediately after sharing is resumed can fail, so we poll instead.
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err := checkFrameStatus(ctx, screenRecorder, params.restriction != restrictionlevel.WarnCancelled); err != nil {
			return errors.Wrap(err, "unexpected frame status")
		}

		return nil
	}, &testing.PollOptions{Timeout: 5 * time.Second, Interval: 500 * time.Millisecond}); err != nil {
		s.Fatal("Failed to wait for expected frame status: ", err)
	}
