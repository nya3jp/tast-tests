...
// GetClipboardWarnPolicy returns a clipboard dlp policy warning when clipboard content is copied and pasted from source to destination.
func GetClipboardWarnPolicy(source, destination string) []policy.Policy {
...
// GetClipboardBlockPolicy returns a clipboard dlp policy warning when clipboard content is copied and pasted from source to destination.
func GetClipboardBlockPolicy(source, destination string) []policy.Policy {
	return []policy.Policy{&policy.DataLeakPreventionRulesList{
		Val: []*policy.DataLeakPreventionRulesListValue{
			{
