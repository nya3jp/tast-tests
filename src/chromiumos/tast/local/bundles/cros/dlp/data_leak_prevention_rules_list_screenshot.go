...
// Type of DLP restriction enforced, potentially including the user's response to the warning dialog (proceed or cancel).
...
const captureNotAllowedTitle = "Can't capture confidential content"
const captureNotAllowedID = "screen_capture_dlp_blocked"
const captureTakenTitle = "Screenshot taken"
const captureTakenID = "capture_mode_notification"
...
func testScreenshot(ctx context.Context, tconn *chrome.TestConn, keyboard *input.KeyboardEventWriter, params screenshotTestParams) error {
	...
	if params.restriction == warnProceeded {
	...
	if params.restriction == warnCancelled {
		...
		wantScreenshotTaken := (params.restriction == allowed) || (params.restriction == warnProceeded)

		if takenScreenshot != wantScreenshotTaken {
			return errors.Wrapf(err, "unexpected screenshot allowed: got %q; want %q", takenScreenshot, wantScreenshotTaken)
		}
	}
