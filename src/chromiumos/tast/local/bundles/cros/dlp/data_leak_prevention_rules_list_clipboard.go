...
func DataLeakPreventionRulesListClipboard(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if !param.expectedAllowed && notification != nil {
			...
			if param.expectedAllowed && pastedError != nil {
				s.Fatal("Couldn't check for pasted content: ", pastedError)
			}

			if (!param.expectedAllowed && pastedError == nil) || (param.expectedAllowed && notification == nil) {
