...
func DataLeakPreventionRulesListClipboard(ctx context.Context, s *testing.State) {
	...
	pb := policy.NewBlob()
	if s.Param().(clipboardTestParams).restriction == restrictionlevel.Blocked {
		pb.AddPolicies(clipboard.GetClipboardBlockPolicy(sourceServer.URL+path, destServer.URL))
	} else if s.Param().(clipboardTestParams).restriction == restrictionlevel.WarnCancelled ||
		s.Param().(clipboardTestParams).restriction == restrictionlevel.WarnProceeded {
		pb.AddPolicies(clipboard.GetClipboardWarnPolicy(sourceServer.URL+path, destServer.URL))
	}
	...
	br, closeBrowser, err := browserfixt.SetUp(ctx, cr, s.Param().(clipboardTestParams).browserType)
	if err != nil {
	...
	defer faillog.DumpUITreeWithScreenshotOnError(cleanupCtx, s.OutDir(), s.HasError, cr, "ui_tree_"+s.Param().(clipboardTestParams).name)
	...
	switch s.Param().(clipboardTestParams).restriction {
	case restrictionlevel.Blocked:
		notifError := clipboard.CheckClipboardBubble(ctx, ui, parsedSourceURL.Hostname())
		if !s.Param().(clipboardTestParams).copyAllowed && notifError != nil {
	...
	case restrictionlevel.WarnCancelled:
		bubbleClass, notifError := clipboard.GetClipboardWarnBubble(ctx, ui, parsedSourceURL.Hostname())
		if notifError != nil {
			s.Error("Expected notification but found an error: ", notifError)
		}
	...
	if s.Param().(clipboardTestParams).copyAllowed && pastedError != nil {
