...
func DataLeakPreventionRulesListClipboard(ctx context.Context, s *testing.State) {
	...
	case restrictionlevel.Blocked:
		pb.AddPolicies(clipboard.BlockPolicy(sourceServer.URL+path, destServer.URL))
	case restrictionlevel.WarnCancelled, restrictionlevel.WarnProceeded:
		pb.AddPolicies(clipboard.WarnPolicy(sourceServer.URL+path, destServer.URL))
	}
	...
	if params.copyAllowed && pastedError != nil {
