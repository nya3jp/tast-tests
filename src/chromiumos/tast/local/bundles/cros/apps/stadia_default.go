...
func StadiaDefault(ctx context.Context, s *testing.State) {
	...
	// Check if Stadia is present in the launcher search results.
	_, err = launcher.WaitForAppResult(ctx, tconn, apps.Stadia.Name, time.Minute)
	if stadiaPreloaded && err != nil {
		s.Fatal("Stadia should exist in search results but is not present: ", err)
	}
	if err == nil && !stadiaPreloaded {
		s.Fatal("Stadia should not exist in search results but is present: ", err)
	}
}
