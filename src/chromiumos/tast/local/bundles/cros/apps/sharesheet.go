...
func Sharesheet(ctx context.Context, s *testing.State) {
	...
	appID, err := apps.InstallPWAForURL(ctx, cr, filepath.Join("http://localhost:8080/sharesheet_index.html"), 5*time.Second)
	if err != nil {
		s.Fatal("Failed installing PWA for URL: ", err)
	}

	if err := ash.WaitForChromeAppInstalled(ctx, tconn, appID, 10*time.Second); err != nil {
