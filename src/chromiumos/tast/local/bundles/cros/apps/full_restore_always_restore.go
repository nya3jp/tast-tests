...
func FullRestoreAlwaysRestore(ctx context.Context, s *testing.State) {
	func() {
		...
		_, err = ossettings.LaunchAtPage(ctx, tconn, nodewith.Name("Apps").Role(role.Link))
		if err != nil {
			s.Fatal("Failed to launch Apps Settings: ", err)
		}

		optionFinder := nodewith.Name("Restore apps on startup").Role(role.PopUpButton)
		if err := uiauto.New(tconn).LeftClick(optionFinder)(ctx); err != nil {
			s.Fatal("Failed to click the restore Setting: ", err)
		}

		settingFinder := nodewith.Name("Always restore").Role(role.ListBoxOption)
		if err := uiauto.New(tconn).LeftClick(settingFinder)(ctx); err != nil {
