...
func helpAppLaunchDuringOOBE(ctx context.Context, s *testing.State, isTabletMode bool) {
	...
	if isTabletMode {
		cr, err = chrome.New(ctx, chrome.Auth(username, password, ""), chrome.GAIALogin(), chrome.DontSkipOOBEAfterLogin(), chrome.ExtraArgs("--force-tablet-mode=touch_view"))
	} else {
		cr, err = chrome.New(ctx, chrome.Auth(username, password, ""), chrome.GAIALogin(), chrome.DontSkipOOBEAfterLogin(), chrome.ExtraArgs("--force-tablet-mode=clamshell"))
	}
	if err != nil {
...
func logRuntimeData(ctx context.Context, s *testing.State, cr *chrome.Chrome) error {
	const logFileName = "loadTimeData.json"
	const helpAppURL = "chrome-untrusted://help-app/app.html"
