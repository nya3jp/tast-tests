...
func WaitForApp(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if _, err := helpRootNode.DescendantWithTimeout(ctx, OverviewTab, 20*time.Second); err != nil {
...
func IsTabShown(ctx context.Context, tconn *chrome.TestConn, tabParams ui.FindParams) (bool, error) {
	helpRootNode, err := HelpRootNode(ctx, tconn)
	if err != nil {
	...
	return helpRootNode.DescendantExists(ctx, tabParams)
}
