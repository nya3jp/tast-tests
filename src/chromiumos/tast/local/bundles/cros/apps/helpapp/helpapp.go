...
func GetLoadTimeData(ctx context.Context, cr *chrome.Chrome) (*LoadTimeData, error) {
	...
	isManagedDevice, err := getLoadTimeBoolean(ctx, helpAppConn, "isManagedDevice")
	if err != nil {
		return nil, err
	}

	return &LoadTimeData{IsManagedDevice: isManagedDevice}, nil
}

func getLoadTimeBoolean(ctx context.Context, helpAppConn *chrome.Conn, key string) (bool, error) {
	val := false
	if err := helpAppConn.Eval(ctx, fmt.Sprintf("window.loadTimeData.getBoolean(%q)", key), &val); err != nil {
		return false, errors.Wrapf(err, "failed to evaluate window.loadTimeData.getBoolean(%q)", key)
	}
	return val, nil
}
