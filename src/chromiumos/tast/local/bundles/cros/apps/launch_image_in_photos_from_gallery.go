...
func LaunchImageInPhotosFromGallery(ctx context.Context, s *testing.State) {
	...
	imageElement := nodewith.Role(role.Image).Name(localFile).Ancestor(galleryapp.RootFinder)
	if err := ui.WaitUntilExists(imageElement)(ctx); err != nil {
		s.Fatal("Failed to render Gallery: ", err)
	}

	s.Log("Click annotation button")
	annotationButton := nodewith.Role(role.ToggleButton).Name(`Annotate`).Ancestor(galleryapp.RootFinder)
	if err := ui.WaitUntilExists(annotationButton)(ctx); err != nil {
	...
	if err := ui.WaitUntilExists(photosButton)(ctx); err != nil {
	...
	// todo: figure out how to get this to pass
	...
	// todo: ensure our image is opened in photos
}
