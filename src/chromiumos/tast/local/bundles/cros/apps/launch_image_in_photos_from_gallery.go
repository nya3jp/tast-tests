...
func LaunchImageInPhotosFromGallery(ctx context.Context, s *testing.State) {
	...
	maybeClearPrompt := func(finder *uidetection.Finder) uiauto.Action {
		return uiauto.IfSuccessThen(
			ud.WithTimeout(5*time.Second).WaitUntilExists(finder),
			uiauto.RetrySilently(3, uiauto.Combine("click button and waits its gone",
				ud.WithTimeout(1*time.Second).LeftClick(finder),
				ud.WithTimeout(3*time.Second).WaitUntilGone(finder),
			)),
		)
	}

	if err := uiauto.NamedCombine("reach main page of Photos app",
		maybeClearPrompt(allowButton),
		maybeClearPrompt(gotItButton),
