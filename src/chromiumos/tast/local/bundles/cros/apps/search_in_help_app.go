...
func init() {
	testing.AddTest(&testing.Test{
		...
		Vars:         []string{"apps.SearchInHelpApp.consumer_username", "apps.SearchInHelpApp.consumer_password"},
...
func SearchInHelpApp(ctx context.Context, s *testing.State) {
	username := s.RequiredVar("apps.SearchInHelpApp.consumer_username")
	password := s.RequiredVar("apps.SearchInHelpApp.consumer_password")

	cr, err := chrome.New(ctx, chrome.Auth(username, password, ""), chrome.GAIALogin(), chrome.EnableFeatures("HelpAppFirstRun", "HelpAppSearchServiceIntegration"))
	if err != nil {
