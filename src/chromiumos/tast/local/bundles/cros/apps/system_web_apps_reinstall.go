...
var testFileContent = []byte("testtest")
...
func SystemWebAppsReinstall(ctx context.Context, s *testing.State) {
	// Verify a set of system web apps that trigger different install code paths are installed.
	...
	err := func() error {
		...
		if err != nil {
			return errors.Wrap(err, "failed to create first chrome instance")
		}
		...
		testFilePath, err := getTestFilePath(ctx, cr)
		if err != nil {
		...
		return checkAppsInstalled(ctx, tconn, testAppInternalNames, "first-run", s)
	}()
	...
	err = func() error {
		...
		if err != nil {
			return errors.Wrap(err, "failed to create second chrome instance")
		}
		...
		return checkAppsInstalled(ctx, tconn, testAppInternalNames, "second-run", s)
	}()
...
// checkAppsInstalled performs assertions and verifies all apps whose internal names are in `testAppInternalNames`
// are installed by querying AppService.
//
// It returns `error` if something unexpected happened (e.g. failed to call Test API).
// If assertions fail, it fails the test and return nil.
//
// |when| is included in test failure message as informational text (to indicate this is the first time, or reinstall).
func checkAppsInstalled(ctx context.Context, tconn *chrome.TestConn, testAppInternalNames map[string]bool, when string, s *testing.State) error {
...
func getTestFilePath(ctx context.Context, cr *chrome.Chrome) (string, error) {
	downloadsPath, err := cryptohome.DownloadsPath(ctx, cr.NormalizedUser())
	if err != nil {
		return "", errors.Wrap(err, "failed to retrieve user's Downloads path")
	}

	return filepath.Join(downloadsPath, testFileName), nil
}
