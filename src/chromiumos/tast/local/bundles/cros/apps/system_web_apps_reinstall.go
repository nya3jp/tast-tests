...
func SystemWebAppsReinstall(ctx context.Context, s *testing.State) {
	...
	err := func() error {
		...
		return checkAppsInstalled(ctx, tconn, testAppInternalNames)
	}()
...
func checkAppsInstalled(ctx context.Context, tconn *chrome.TestConn, testAppInternalNames map[string]bool) error {
