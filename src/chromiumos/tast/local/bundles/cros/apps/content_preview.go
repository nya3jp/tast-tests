...
const (
	textFile  = "contentpreview_20210511.txt"
	zipFile   = "contentpreview_20210511.zip"
	videoFile = "contentpreview_20210511.webm"
	pngFile   = "contentpreview_20210511.png"
)
...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:         []string{textFile, zipFile, videoFile, pngFile},
...
func ContentPreview(ctx context.Context, s *testing.State) {
	...
	subTests := []subTestData{
		{
			name:        textFile,
			filePath:    filepath.Join(filesapp.DownloadPath, textFile),
			thumbnail:   "", // TODO (melzhang@google.com): to add functions to create thumbnail for the files.
			shareString: textFile,
		},
		{
			name:        zipFile,
			filePath:    filepath.Join(filesapp.DownloadPath, zipFile),
			thumbnail:   "",
			shareString: zipFile,
		},
		{
			name:        videoFile,
			filePath:    filepath.Join(filesapp.DownloadPath, videoFile),
			thumbnail:   "",
			shareString: videoFile,
		},
		{
			name:        pngFile,
			filePath:    filepath.Join(filesapp.DownloadPath, pngFile),
			thumbnail:   "",
			shareString: pngFile,
		},
	}
			...
			s.Run(ctx, des, func(ctx context.Context, s *testing.State) {
				...
				if err := fsutil.CopyFile(s.DataPath(data.name), data.filePath); err != nil {
					...
					if button {
						...
						if err := files.ClickContextMenuItem(data.name, share)(ctx); err != nil {
					...
					shareLabel := nodewith.Name(share).ClassName("Label").Role(role.StaticText).Ancestor(bubbleView)
