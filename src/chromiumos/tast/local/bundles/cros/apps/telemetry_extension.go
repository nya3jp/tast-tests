...
func TelemetryExtension(ctx context.Context, s *testing.State) {
	...
	if err := fsutil.CopyFile(s.DataPath(indexHTML), filepath.Join(dir, "index.html")); err != nil {
		s.Fatalf("Failed to copy %s file to %s: %v", indexHTML, dir, err)
	}
	if err := fsutil.CopyFile(s.DataPath(indexJS), filepath.Join(dir, "index.js")); err != nil {
		s.Fatalf("Failed to copy %s file to %s: %v", indexJS, dir, err)
	}
	...
	if err := ash.WaitForApp(ctx, tconn, apps.TelemetryExtension.ID); err != nil {
		s.Fatalf("Failed to wait for %s by app id %s: %v", apps.TelemetryExtension.Name, apps.TelemetryExtension.ID, err)
	}
	...
	if err := ioutil.WriteFile(filepath.Join(s.OutDir(), responseFile), []byte(apiResponse), 0666); err != nil {
		s.Error("Failed to save telemetry API response into: ", responseFile)
	}

	if apiStatus != "Success" {
		s.Error("Telemetry API failed, check reason in ", responseFile)
	}
}
