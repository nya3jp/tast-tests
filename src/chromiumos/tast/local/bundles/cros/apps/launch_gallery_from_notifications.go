...
func LaunchGalleryFromNotifications(ctx context.Context, s *testing.State) {
	...
	conn, err := cr.NewConn(ctx, path.Join(server.URL, "download_link.html"))
	if err != nil {
		s.Fatal("Failed navigating to image on local server: ", err)
	}

	// Wait for download link to appear from local test server.
	if err := conn.WaitForExprFailOnErrWithTimeout(ctx, "document.getElementById('download-gallery-image') != null", uiTimeout); err != nil {
		s.Fatal("Failed attempting to click download link: ", err)
	}

	if err := conn.Eval(ctx, "document.getElementById('download-gallery-image').click()", nil); err != nil {
		s.Fatal("Failed attempting to click download link: ", err)
	}
