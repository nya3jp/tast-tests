...
func createPWAs(ctx context.Context, folderPath string, totalApps int) error {
...
func testAvailableAppsListing(ctx context.Context, s *testing.State, files *filesapp.FilesApp, ew *input.KeyboardEventWriter, testCase, downloadsPath string, totalApps int) error {
	...
	// Setup the apps.
	if err := createPWAs(ctx, folderPath, totalApps); err != nil {
		return errors.Wrapf(err, "failed to create apps with name %q", folderPath)
	}
