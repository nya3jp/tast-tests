...
func ZipMount(ctx context.Context, s *testing.State) {
	...
	s.Run(ctx, "MountSingleZipFile", func(ctx context.Context, s *testing.State) {
		// Select ZIP file.
		if err := files.WaitForFile(ctx, simpleZipFile, 5*time.Second); err != nil {
			s.Fatal("Cannot wait for test ZIP file: ", err)
		}

		if err := files.SelectFile(ctx, simpleZipFile); err != nil {
		...
		checkAndUnmountZipFile(ctx, s, files, simpleZipFile)
	})

	s.Run(ctx, "CancelMultiplePasswordDialogs", func(ctx context.Context, s *testing.State) {
		// Open the Downloads folder.
		if err := files.OpenDownloads(ctx); err != nil {
			s.Fatal("Cannot open Downloads folder: ", err)
		}
	...
	s.Run(ctx, "MountMultipleZipFiles", func(ctx context.Context, s *testing.State) {
		// Open the Downloads folder.
		if err := files.OpenDownloads(ctx); err != nil {
			s.Fatal("Cannot open Downloads folder: ", err)
		}
