...
func DragDrop(ctx context.Context, s *testing.State) {
	...
	extID, err := setupDragDropExtension(ctx, s, extDir)
	if err != nil {
	...
	// Select the file first to ensure files source information is appended to drag.
	...
	fgURL := "chrome-extension://" + extID + "/window.html"
...
// tickCheckboxForFile ensures the file to drag is selected.
func tickCheckboxForFile(ctx context.Context, f *filesapp.FilesApp, fileName string) (coords.Point, error) {
	...
	srcPoint := file.Location.CenterPoint()
