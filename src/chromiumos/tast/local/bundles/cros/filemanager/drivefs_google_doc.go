...
func DrivefsGoogleDoc(ctx context.Context, s *testing.State) {
	...
	// Current refresh period for Drive is 2 minutes, giving buffer for update to propogate.
	const filesAppUITimeout = 2.5 * time.Minute
	testDocFileName := fmt.Sprintf("doc-drivefs-%d", time.Now().UnixNano())

	// Create a test file using Drive API.
	file, err := driveAPI.CreateBlankGoogleDoc(ctx, testDocFileName, []string{"root"})
	if err != nil {
		s.Fatal("Could not create blank google doc: ", err)
	}
	defer driveAPI.RemoveFileByID(ctx, file.Id)
