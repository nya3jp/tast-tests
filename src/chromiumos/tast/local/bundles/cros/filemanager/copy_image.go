...
func CopyImage(ctx context.Context, s *testing.State) {
	...
	testFolderLocation := filepath.Join(filesapp.DownloadPath, testFolderName)
	if err := files.CreateFolder(ctx, testFolderName); err != nil {
		s.Fatal("Failed to create folder: ", err)
	}
	defer os.RemoveAll(testFolderLocation)

	// Verify the create folder event actually occurred.
	if err := files.WaitForFile(ctx, testFolderName, 10*time.Second); err != nil {
		s.Fatal("Waiting for test file failed: ", err)
	}
