...
func CopyImage(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Failed createing temp extension directory: ", err)
	}
	defer os.RemoveAll(extDir)

	copyImageTargetExtID, err := setupCopyImageExtension(ctx, s, extDir)
	if err != nil {
		s.Fatal("Failed setup of copy image extension: ", err)
	}
	...
	// TODO: When opening a subfolder is fixed revisit this.
	// // Create a folder to paste into.
	// test_folder_name := "test_folder"
	// test_folder_location := filepath.Join(filesapp.DownloadPath, test_folder_name)
	// if err := files.CreateFolder(ctx, test_folder_name); err != nil {
	// 	s.Fatal("Failed to create folder: ", err)
	// }
	// defer os.RemoveAll(test_folder_location)

	// // Verify the create folder event actually occured.
	// if err := files.WaitForFile(ctx, test_folder_name, 10*time.Second); err != nil {
	// 	s.Fatal("Waiting for test file failed: ", err)
	// }
	...
	// 	s.Fatalf("Failed waiting for %q folder to be clickable: %v", test_folder_name, err)
	// }

	// if err := files.OpenFile(ctx, test_folder_name); err != nil {
	...
	// // Verify the paste event actually occured.
	// if err := files.WaitForFile(ctx, previewImageFile, 10*time.Second); err != nil {
	// 	s.Fatal("Waiting for test file failed: ", err)
	// }
	...
	mouse.Move(ctx, tconn, extWindow, time.Second)
	mouse.Click(ctx, tconn, extWindow, mouse.LeftButton)
	kb.Accel(ctx, "Ctrl+v")
...
func verifyPastedData(ctx context.Context, conn *chrome.Conn, filename string) error {
	...
	if err := conn.Eval(ctx, "window.document.title.replace('paste registered:', '')", &actualDroppedFileName); err != nil {
		return errors.Wrap(err, "failed retrieving the drag drop window title")
	}

	if filename != actualDroppedFileName {
		return errors.Errorf("failed pasted file doesnt match dragged file, got: %q; want: %q", actualDroppedFileName, filename)
	}
	...
	var fstag string
	if err := conn.Eval(ctx, "window.myData['fs/tag']", &fstag); err != nil {
		return errors.Wrap(err, "failed retrieving the custom window property for html")
	}

	if fstag != "filemanager-data" {
		return errors.Errorf("failed pasted file manager tag doesnt match dragged file, got: %q; want: %q", fstag, "filemanager-data")
	}
