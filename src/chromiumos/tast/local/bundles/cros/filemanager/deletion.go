...
func setupFilesAndFolders(ctx context.Context, tconn *chrome.TestConn, kb *input.KeyboardEventWriter,
	filesApp *filesapp.FilesApp, targets []string, nameMap map[deletionTestTarget]string, workingDirPath, dataPath string) (cleanUp func(context.Context), err error) {
	cleanTargets := make([]string, 0)
	cleanUp = func(ctx context.Context) {
		for _, target := range cleanTargets {
			if err := os.RemoveAll(target); err != nil {
				testing.ContextLogf(ctx, "Failed to remove target %q: %v", target, err)
			}
		}
	}
