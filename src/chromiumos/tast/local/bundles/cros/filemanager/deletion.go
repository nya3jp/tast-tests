	...
	for workingDir, workingDirPath := range dirPath {
		for _, test := range []struct {
			...
			subTest := func(ctx context.Context, s *testing.State) {
				...
				cleanUp, err := setupFilesAndFolders(ctx, res, targets)
...
func generateTestTargets(testTargets deletionTargetType, targetsName map[deletionTargetType]string,
	...
	if needToDelete(nonEmptyFolder) {
		...
		targets[nonEmptyFolder|file] = &deletionTargetDetail{
			...
			skipDeletion: true, // nonEmptyFolder is the target to delete, its contents are not.
		}
	}
...
func setupFilesAndFolders(ctx context.Context, res *deletionTestResource,
	targets *map[deletionTargetType]*deletionTargetDetail) (func(context.Context), error) {
	...
	for _, target := range *targets {
		if target.isFolder {
			...
			if err := os.Chown(target.path, int(sysutil.ChronosUID), int(sysutil.ChronosGID)); err != nil {
				return cleanUp, errors.Wrapf(err, "failed to chown of folder: %q", target.path)
			}
		}
	}
	// Setup files.
	for _, target := range *targets {
		if !target.isFolder {
			if err := fsutil.CopyFile(res.dataPath, target.path); err != nil {
				return cleanUp, errors.Wrapf(err, "failed to copy file: %q to: %q", res.dataPath, target.path)
			}
		}
	}
	return cleanUp, nil
