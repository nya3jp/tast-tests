...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			Name: "copy",
			Val:  smbFileOperationTest{Fn: testCopyOperation},
		}},
	})
}

type smbFileOperationTestFunc func(ctx context.Context, ew *input.KeyboardEventWriter, s *testing.State, files *filesapp.FilesApp)

type smbFileOperationTest struct {
	Fn smbFileOperationTestFunc
}

func SMBFileOperations(ctx context.Context, s *testing.State) {

	fixt := s.FixtValue().(smb.FixtureData)
	testFunc := s.Param().(smbFileOperationTest).Fn
...
func testCopyOperation(ctx context.Context, kb *input.KeyboardEventWriter, s *testing.State, files *filesapp.FilesApp) {
	...
	pasteFile := func(ctx context.Context) error {
		return kb.Accel(ctx, "Ctrl+V")
	}
