...
func testExtractingZipFile(ctx context.Context, s *testing.State, files *filesapp.FilesApp, ew *input.KeyboardEventWriter, zipBaseName, selectionLabel, copyLabel string) float64 {
	...
	completeNotification := nodewith.Name(completeLabel).Role(role.StaticText).Ancestor(copyNotification)
	...
	dismissButton := nodewith.Name(dismissButtonLabel).Role(role.Button).Ancestor(copyNotification)
...
func testZippingFiles(ctx context.Context, tconn *chrome.TestConn, s *testing.State, files *filesapp.FilesApp, ew *input.KeyboardEventWriter, zipBaseName, zipLabel string) float64 {
	...
	completeNotification := nodewith.Name(completeLabel).Role(role.StaticText).Ancestor(zippingNotification)
	...
	dismissButton := nodewith.Name(dismissButtonLabel).Role(role.Button).Ancestor(zippingNotification)
