...
func installExtension(ctx context.Context, s *testing.State, url string) error {
...
func DrivefsDssOffline(ctx context.Context, s *testing.State) {
	...
	if err := installExtension(ctx, s, "https://chrome.google.com/webstore/detail/application-launcher-for/lmjegmlicamnimmfhcmpkclmigmmcbeh"); err != nil {
		s.Fatal("Failed to install Application Launcher for Drive extension: ", err)
	}
	if err := installExtension(ctx, s, "https://chrome.google.com/webstore/detail/google-docs-offline/ghbmnnjooekpmoecnnnilnnbdlolhkhi"); err != nil {
