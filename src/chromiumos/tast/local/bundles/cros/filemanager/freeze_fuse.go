...
func FreezeFUSE(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < suspendAttempts; i++ {
		s.Run(ctx, fmt.Sprintf("FUSE Suspend Attempt %d", i), testMountZipAndSuspend)
	}
}
...
func testMountZipAndSuspend(ctx context.Context, s *testing.State) {
	...
	if err := ioutil.WriteFile(
		...
		0644); err != nil {
		s.Fatal("Freezer cgroup ui does not exist: ", err)
	}

	s.Log("Attempting suspend")
	if err := testexec.CommandContext(ctx, "powerd_dbus_suspend", "--wakeup_timeout=30").Run(); err != nil {
