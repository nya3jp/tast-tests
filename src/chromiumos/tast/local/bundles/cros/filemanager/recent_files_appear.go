...
func RecentFilesAppear(ctx context.Context, s *testing.State) {
	...
	for _, subtest := range []struct {
		...
		f := func(ctx context.Context, s *testing.State) {
			...
			if err := ui.RetryUntil(
				goToRecentImages(ctx, tconn, files, filtersInRecentsEnabled),
				files.FileExists(testImage),
			)(ctx); err != nil {
...
func goToRecentImages(ctx context.Context, tconn *chrome.TestConn, files *filesapp.FilesApp, isFlagOn bool) uiauto.Action {
	if isFlagOn {
		ui := uiauto.New(tconn)
		return uiauto.Combine("refresh recent images by filter button",
			files.OpenDir(filesapp.Recent, filesapp.FilesTitlePrefix+filesapp.Recent),
			ui.LeftClick(nodewith.Name("Images").Role(role.ToggleButton)),
		)
	}
	return uiauto.Combine("refresh recent images by filter menu",
		files.OpenDir(filesapp.Images, filesapp.FilesTitlePrefix+filesapp.Images),
	)
}
