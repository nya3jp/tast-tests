...
func DrivefsSearch(ctx context.Context, s *testing.State) {
	...
	if err := filesApp.WithPollOpts(testing.PollOptions{Timeout: 15 * time.Second, Interval: 2 * time.Second}).WaitUntilExists(nodes)(ctx); err != nil {
		s.Fatal("Failed waiting for search results: ", err)
	}

	resultsTextContainer, err := filesApp.NodesInfo(ctx, nodes)
	if err != nil {
		s.Fatal("Failed to find resultsTextContainerInfo: ", err)
	}

	// We expect 1 file to be returned given the filename length of the seed file.
	if len(resultsTextContainer) != 1 {
		s.Fatalf("Unexpected number of search results: got %d, want 1", len(resultsTextContainer))
	}
