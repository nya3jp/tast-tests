...
func waitUntilGDocsSynced(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, files *filesapp.FilesApp, filesAppConn *chrome.Conn, folderName string, filesName ...string) error {
	...
	waitUntilFileSynced := func(fileName string) error {
		return testing.Poll(ctx, func(ctx context.Context) error {
			...
			return files.WaitForFile(fileName)(ctx)
		}, &testing.PollOptions{Timeout: driveSyncTimeout})
	}

	if err := waitUntilFileSynced(folderName); err != nil {
	...
	for _, file := range filesName {
		if err := waitUntilFileSynced(file); err != nil {
