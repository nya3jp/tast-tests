...
func GoogleDriveFiles(ctx context.Context, s *testing.State) {
	...
	// apiClient.RemoveFileByID(ctx, folder.Id) removes the folder already, all descendants owned by the user shoule be deleted as well.
	...
	if err := waitUntilGDocsSynced(ctx, cr, tconn, filesApp, filesAppConn, folderName, files...); err != nil {
	...
	for _, testSample := range []struct {
		...
		{newGDocEditor(gDocFileName, res), "g doc test"},
		{newGSheetEditor(gSheetFileName, res), "g sheet test"},
		{newGSlides(gSlidesFileName, res), "g slide test"},
		{newImageEditor(jpegFile, res), "jpeg test"},
...
func reloadFilesappPage(ctx context.Context, conn *chrome.Conn) error {
	return conn.Eval(ctx, `location.reload()`, nil)
}

// waitUntilGDocsSynced waits until Google documents are all synced to filesapp.
func waitUntilGDocsSynced(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, files *filesapp.FilesApp, filesAppConn *chrome.Conn, folderName string, filesName ...string) error {
	if err := files.OpenDrive()(ctx); err != nil {
		return errors.Wrap(err, "failed to open Drive")
	}

	// reloadUntilSynced retries on reloading the filesapp page until the provided "synced" action is passed.
	reloadUntilSynced := func(synced uiauto.Action) error {
		return testing.Poll(ctx, func(ctx context.Context) error {
			// Check synced before retry action is preferred, therefore (*uiauto.Context).RetryUntil is not used here.
			if err := synced(ctx); err != nil {
				if err := reloadFilesappPage(ctx, filesAppConn); err != nil {
					return testing.PollBreak(errors.Wrap(err, "failed to reload filesapp page"))
				}
				return err
			}
			return nil
		}, &testing.PollOptions{Timeout: driveSyncTimeout, Interval: 3 * time.Second})
	}

	synced := files.WaitForFile(folderName)
	if err := reloadUntilSynced(synced); err != nil {
		return errors.Wrapf(err, "failed to wait for folder %q appears", folderName)
	}
	...
	var allFilesAppear []uiauto.Action
	for _, file := range filesName {
		allFilesAppear = append(allFilesAppear, files.WaitForFile(file))
	}

	synced = uiauto.Combine("wait all files appear", allFilesAppear...)
	return reloadUntilSynced(synced)
}
...
func (f *fileSample) verify(ctx context.Context) error {
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		if err := reloadFilesappPage(ctx, f.res.filesAppConn); err != nil {
			return errors.Wrap(err, "failed to reload filesapp page")
		}
