...
func checkAndUnmountZipFile(ctx context.Context, s *testing.State, files *filesapp.FilesApp, zipFile string) {
	...
	// First ensure that the Files App is displaying the content of the mounted ZIP file, and then unmount the ZIP file.
	if err := uiauto.Combine("find and click eject button - "+zipFile,
		...
		s.Fatalf("Cannot check and unmount the archive %q: %v", zipFile, err)
	}
}
...
func cancelMultiplePasswordDialogs(ctx context.Context, s *testing.State, files *filesapp.FilesApp, zipFiles []string) {
	...
	firstZipFileName, err := waitUntilPasswordDialogExists(ctx, files, processedZipFiles)
	if err != nil {
