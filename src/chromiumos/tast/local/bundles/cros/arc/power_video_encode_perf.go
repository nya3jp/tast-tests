...
const (
	encodeIterationCount    = 30
	encodeIterationDuration = 10 * time.Second
	encodeWarmupDuration    = 10 * time.Second
	encodeTestSlack         = 5 * time.Minute
	encodePowerTestDuration = encodeIterationCount*encodeIterationDuration +
		encodeWarmupDuration + encodeTestSlack
)
...
func init() {
	testing.AddTest(&testing.Test{
		...
		Timeout:      encodePowerTestDuration,
...
func PowerVideoEncodePerf(ctx context.Context, s *testing.State) {
	...
	params := opts.Params
	rawVideoPath, err := encoding.PrepareYUV(
		ctx, s.DataPath(params.Name), opts.PixelFormat, params.Size)
	if err != nil {
	...
	encodeOutFile := filepath.Join(
		arcFilePath, strings.TrimSuffix(params.Name, ".vp9.webm")+".h264")
	testArgs := []string{
		encoding.CreateStreamDataArg(params, opts.Profile, opts.PixelFormat,
			arcFilePath+"/"+filepath.Base(rawVideoPath), encodeOutFile),
	}
	...
	metrics, err := perf.NewTimeline(
		ctx, power.TestMetrics(), perf.Interval(encodeIterationDuration))
	if err != nil {
	...
	if err := testing.Sleep(ctx, encodeWarmupDuration); err != nil {
	...
	sleepDuration := encodeIterationCount * encodeIterationDuration
	testing.ContextLogf(ctx, "Sleeping for %v while measuring power drain", sleepDuration)
