...
func Notification(ctx context.Context, s *testing.State) {
	...
	findNotification := func() (*chrome.Notification, error) {
		ns, err := chrome.VisibleNotifications(ctx, tconn)
		if err != nil {
			return nil, err
		}
		for _, n := range ns {
	...
	if notif.Title != title {
		s.Fatal("Unexpected notification title: got %q; want %q", notif.Title, title)
	}
	if notif.Message != text {
		s.Fatal("Unexpected notification message: got %q; want %q", notif.Message, text)
	}
