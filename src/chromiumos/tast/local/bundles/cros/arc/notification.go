...
func Notification(ctx context.Context, s *testing.State) {
	...
	if err := runTest(ctx, func(ctx context.Context) error {
		d, err := a.NewUIDevice(ctx)
		if err != nil {
			s.Fatal("Failed to initialize UI Automator: ", err)
		}
...
func runTest(ctx context.Context, removeNotification func(context.Context) error, a *arc.ARC, tconn *chrome.TestConn) error {
