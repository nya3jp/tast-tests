...
const timelineDurationForPIPEnergyAndPower = time.Minute
...
func PIPEnergyAndPower(ctx context.Context, s *testing.State) {
	...
	if s.Param().(bool) { // Branch for arc.PIPEnergyAndPower.big.
		...
		resizeHandleBounds, err := d.Object(
			...
			ui.DescriptionMatches("(?!.+)"), // Look for an empty content description.
	...
	if err := testing.Sleep(ctx, timelineDurationForPIPEnergyAndPower); err != nil {
		s.Fatalf("Failed to wait %v: %v", timelineDurationForPIPEnergyAndPower, err)
	}
