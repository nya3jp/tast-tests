...
func PIPEnergyAndPower(ctx context.Context, s *testing.State) {
	...
	if s.Param().(bool) { // Branch for arc.PIPEnergyAndPower.big.
		// Resize the PIP window by dragging the corner because the
		// resize handle currently does not exist in Android R.
		// TODO(https://crbug.com/1093933): When the resize handle
		// has been added back, use it here.
