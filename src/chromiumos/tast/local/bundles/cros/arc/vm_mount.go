...
func init() {
	testing.AddTest(&testing.Test{
		// TODO: hopefully integrate into arc.StartStop once gets stable.
		Func:         VMMount,
...
func mountInfoForARCVM(ctx context.Context) ([]sysutil.MountInfo, error) {
	...
	cmd := arc.BootstrapCommand(ctx, "/system/bin/cat", "/proc/self/mountinfo")
	mi, err := cmd.Output()
	if err != nil {
	...
	for _, line := range strings.Split(string(mi), "\n") {
		...
		info, err := sysutil.ParseLine(line)
		if err != nil {
