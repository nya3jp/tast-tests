...
func MemoryAndroidPerf(ctx context.Context, s *testing.State) {
	...
	var a = memory.NewAndroidAllocator(s.PreValue().(arc.PreData).ARC)
	...
	epsilon := 5 // We want to be consistently under the critical margin, so make the target available just inside.
	if allocated, err := a.AllocateUntil(ctx, time.Second, 60, margin-epsilon); err != nil {
