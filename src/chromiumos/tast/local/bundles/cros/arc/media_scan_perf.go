...
func waitForPopulatedFilesAddedToMediaStore(ctx context.Context, a *arc.ARC) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		var tmpNumberOfScannedFiles int
		lines := bytes.Split(out, []byte("\n"))
		for _, line := range lines {
			if mediaStoreQueryLine := re.Find(bytes.TrimSpace(line)); mediaStoreQueryLine != nil {
				tmpNumberOfScannedFiles++
			}
		}
		testing.ContextLogf(ctx, "numberOfScannedFiles = %d", tmpNumberOfScannedFiles)
		if tmpNumberOfScannedFiles < numberOfCopies {
			return errors.Errorf("Not enough image file copies: want %d; got %d", numberOfCopies, tmpNumberOfScannedFiles)
		}
		if tmpNumberOfScannedFiles == numberOfScannedFiles {
			return errors.Errorf("MediaScan stops scanning image files: want %d; got %d", numberOfCopies, numberOfScannedFiles)
		}
