...
// createFileCopiesUnderTargetPath create numberOfCopies copies of an image file under the target directory.
func createFileCopiesUnderTargetPath(s *testing.State, targetDir string) {
...
// getElapsedTimeData gets the elapsed time during full-volume media scan
// from Android app UI.
...
// the elapsed time during a full-volume media scan, and starts Android app in DUT.
...
func MediaScanPerf(ctx context.Context, s *testing.State) {
	...
	defer func() {
		if err := os.RemoveAll(targetDir); err != nil {
			s.Fatalf("Failed to remove the target directory %q: %v", targetDir, err)
		}
	}()

	createFileCopiesUnderTargetPath(s, targetDir)
	...
	s.Logf("The elapsed time during a full volume media scan is %lf msec", time)
