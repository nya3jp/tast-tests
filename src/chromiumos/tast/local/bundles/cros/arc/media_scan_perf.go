...
const (
	...
	// we need to create a lot of files under the target directory.
...
func waitForPopulatedFilesAddedToMediaStore(ctx context.Context, a *arc.ARC) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		for _, line := range lines {
			if mediaStoreQueryLine := re.FindAllIndex(bytes.TrimSpace(line), -1); mediaStoreQueryLine != nil {
		...
		if numberOfScannedFiles == prevNumberOfScannedFiles {
			return testing.PollBreak(errors.Errorf("MediaScan stops scanning image files: want %d; got %d", numberOfCopies, prevNumberOfScannedFiles))
		}
	...
	}, &testing.PollOptions{Timeout: 1000 * time.Second, Interval: 3 * time.Second}); err != nil {
...
// getElapsedTimeData gets the data of elapsed time during full-volume media scan
// from Android app UI and converts the type of data from string into int.
func getElapsedTimeData(ctx context.Context, d *ui.Device) (float64, error) {
	...
	if err := view.WaitForExists(ctx, 300*time.Second); err != nil {
...
// startMeasureMediaScanPerfWithApp installs apk and starts Android app in DUT.
...
func MediaScanPerf(ctx context.Context, s *testing.State) {
	...
	// Unmount the target directory volume. This is ready for remounting to trigger a full-volume media scan.
	...
	// Start Android app to be ready for receiving media scan intent and measuring the elapsed time.
	...
	// Mount the target directory volume.  media scan will be triggered ane measuring will be start
	// just after mounting.
