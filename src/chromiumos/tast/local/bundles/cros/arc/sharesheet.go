...
func Sharesheet(ctx context.Context, s *testing.State) {
	...
	a, d, err := setUpARC(ctx, cr, s.OutDir())
	if err != nil {
	...
	if err := a.Install(ctx, arc.APKPath("ArcChromeSharesheetTest.apk")); err != nil {
	...
	fileContentField := d.Object(arcui.ID(fileContentsID), arcui.Text(expectedFileContents))
