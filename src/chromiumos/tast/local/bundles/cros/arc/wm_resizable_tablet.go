...
func wmRT22(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device) (retErr error) {
	...
	overActivity, err := arc.NewActivity(a, wm.ExtraPkgName, wm.ResizableUnspecifiedActivity)
	if err != nil {
		return err
	}
	...
	overActivityWInfo, err := ash.GetARCAppWindowInfo(ctx, tconn, wm.ExtraPkgName)
	if err != nil {
