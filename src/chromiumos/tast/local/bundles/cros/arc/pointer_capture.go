...
func enablePointerCapture(ctx context.Context, tconn *chrome.TestConn) error {
	...
	// actually has focus.
	w, err := ash.GetARCAppWindowInfo(ctx, tconn, motioninput.PKG)
	if err != nil {
...
func readPointerCaptureState(ctx context.Context, d *ui.Device) (*pointerCaptureState, error) {
	view := d.Object(ui.ID(motioninput.PKG + ":id/pointer_capture_state"))
...
func PointerCapture(ctx context.Context, s *testing.State) {
	...
	runSubtest := func(ctx context.Context, s *testing.State, subtestFunc pointerCaptureSubtestFunc) {
		...
		act, err := arc.NewActivity(a, motioninput.PKG, string(motioninput.PointerCaptureActivityCLS))
		if err != nil {
		...
		test.tester = motioninput.NewTester(a, d, motioninput.PointerCaptureActivityCLS, act)
...
func verifyPointerCaptureBounds(ctx context.Context, s *testing.State, t pointerCaptureSubtestState) {
	...
	for i := 0; i < 100; i++ {
		...
		pendingMatchers = append(pendingMatchers, motioninput.ActionSourceMatcher(motioninput.Move, motioninput.MouseRelative))

		// Sleep to ensure subsequent relative movements don't get aggregated.
...
func verifyPointerCaptureButtons(ctx context.Context, s *testing.State, t pointerCaptureSubtestState) {
	...
	matcher := func(a motioninput.Action, pressure float64) motioninput.Matcher {
		return motioninput.SinglePointerMatcher(a, motioninput.MouseRelative, coords.NewPoint(0, 0), pressure)
	}
	if err := t.tester.ExpectEventsAndClear(ctx, matcher(motioninput.Down, 1), matcher(motioninput.ButtonPress, 1), matcher(motioninput.ButtonRelease, 0), matcher(motioninput.Up, 0)); err != nil {
