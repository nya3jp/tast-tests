...
func PowerIdlePerf(ctx context.Context, s *testing.State) {
	...
	setup.Append(DisableService(ctx, "powerd"))
	setup.Append(DisableService(ctx, "update-engine"))
	setup.Append(DisableService(ctx, "vnc"))
	setup.Append(DisableService(ctx, "dptf"))
	setup.Append(SetBacklightLux(ctx, 150))
	setup.Append(SetKeyboardBrightness(ctx, 24))
	setup.Append(MuteAudio(ctx))
	var wifiInterfaceRe = regexp.MustCompile(".*wlan\\d+")
	setup.Append(DisableNetworkInterfaces(ctx, wifiInterfaceRe))
	setup.Append(SetBatteryDischarge(ctx, 2.0))
	// TODO: bluetooth
