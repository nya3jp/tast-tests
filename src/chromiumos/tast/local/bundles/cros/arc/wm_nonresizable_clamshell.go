...
func wmNC04(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		return wm.CheckMaximizeToFullscreenTogglePortrait(ctx, tconn, windowInfoMaximized.BoundsInRoot, windowInfoFullscreen.BoundsInRoot)
	}, &testing.PollOptions{Timeout: 10 * time.Second}); err != nil {
		return errors.Wrap(err, "failed to verify activity window state")
	}
