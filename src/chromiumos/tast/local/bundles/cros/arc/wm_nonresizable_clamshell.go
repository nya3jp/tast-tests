...
func wmNC17(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device) (err error) {
	...
	defer func() {
		if err := act.Stop(ctxForStopActivity, tconn); err != nil {
			err = errors.Wrap(err, "unable to stop activity")
		}
	}()
	...
	defer func() {
		if err := wm.EnsureARCFontScaleChanged(ctxForCleanup, a, 1); err != nil {
			err = errors.Wrap(err, "unable to clean up font scale changes")
		}
	}()
