...
func WindowAnimationJank(ctx context.Context, s *testing.State) {
	...
	a.Device().Root(ctx)
	//The path may depend on the kernel version. This is r version.
	if err := a.Device().Command(ctx, "shell", "echo 0 > /sys/kernel/tracing/tracing_on").Run(testexec.DumpLogOnError); err != nil {
	...
	// TODO(b/xxx): analyze and metrics output
}
