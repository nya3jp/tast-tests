...
func SetBounds(ctx context.Context, s *testing.State) {
	...
	if activityBounds.Height != initialHeight+captionHeight || activityBounds.Width != initialWidth {
		s.Fatalf("Initial window size is not expected one. expected = %d x %d, but actual = %d x %d", initialHeight+captionHeight, initialWidth, activityBounds.Width, activityBounds.Height)
	}

	clickButtonAndValidateBounds := func(buttonId string, expectedRect arc.Rect) {
		// Touch button.
		if err := d.Object(ui.ID(buttonId)).Click(ctx); err != nil {
			s.Fatalf("Could not click the button(%s)", buttonId)
		}
		...
		if activityBounds != expectedRect {
			s.Fatalf("Got wrong bounds: expected = %s, actual = %s", expectedRect.String(), activityBounds.String())
		}
	}
