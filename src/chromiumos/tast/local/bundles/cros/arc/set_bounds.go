	...
	for _, test := range []struct {
		...
		s.Run(ctx, test.act, func(ctx context.Context, s *testing.State) {
			...
			if actBounds.Height != initH || actBounds.Width != initW {
				// Even if initial bounds is wrong, we can proceed the test.
				s.Errorf("Unexpected initial bounds: got (%d, %d); want (%d, %d)", actBounds.Width, actBounds.Height, initH, initW)
			}
