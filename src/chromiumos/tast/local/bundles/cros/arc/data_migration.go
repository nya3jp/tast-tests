...
func checkSdkVersionsInPackagesXML(ctx context.Context, username string) error {
	...
	f, err := os.Open(filepath.Join(rootCryptDir, "android-data", packagesXMLPath))
	if err != nil {
	...
	if systemVersion <= 0 || dataVersion <= 0 {
		return errors.Wrap(err, "failed to get System SDK version or Data SDK version in packages.xml")
	}
