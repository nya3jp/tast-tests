...
func DataMigration(ctx context.Context, s *testing.State) {
	...
	// Ensure that user is logged out before executing mountVaultWithArchivedData().
	...
	if err := mountVaultWithArchivedData(ctx, homeDataPath, username, password); err != nil {
	...
	if err := checkVersionUpgradeInArcLog(ctx); err != nil {
		s.Fatal("Failed to check version upgrade in arc.log: ", err)
	}
...
func mountVaultWithArchivedData(ctx context.Context, homeDataPath, username, password string) error {
...
func checkVersionUpgradeInArcLog(ctx context.Context) error {
	const (
		arcLogPath = "/var/log/arc.log"
	)

	// SDK version of ARC running curently
	systemVersionRegexp := regexp.MustCompile(`System SDK version: (\d+)`)
	systemVersion := 0

	// SDK version of ARC on which /data was created
	dataVersionRegexp := regexp.MustCompile(`Data SDK version: (\d+)`)
	dataVersion := 0

	testing.ContextLogf(ctx, "Scanning %s to check SDK versions", arcLogPath)
	...
	if systemVersion <= 0 {
		return errors.Wrapf(err, "failed to get System SDK version in %s", arcLogPath)
	}
	if dataVersion <= 0 {
		return errors.Wrapf(err, "failed to get Data SDK version in %s", arcLogPath)
	}
