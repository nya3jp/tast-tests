...
func CopyCaches(ctx context.Context, a *arc.ARC, outputDir string) error {
	...
	if err != nil {
		return errors.Wrapf(err, "compression: %s failed", chimeraPath)
	}
	// Sanity check.
	if size := len(out); size < 100 {
		return errors.Errorf("GMS Core caches look too small with size %d did something go wrong?", size)
	}
	...
	// TODO: we need to dump stderr.
	out, err = arc.BootstrapCommand(
		ctx, "/system/bin/find", "-L", chimeraPath, "-exec", "stat", "-Lc", "%n:%a:%b", "{}", "+").Output()
	if err != nil {
