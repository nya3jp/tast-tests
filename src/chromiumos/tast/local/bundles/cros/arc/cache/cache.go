...
// BootARC boots ARC with caches turned on or off, depending on the mode parameter. In each case,
// resulting files (app_chimera tar archive, layout.txt) are copied to the specified output
// directory. On success, non-nil pointers are returned that must be closed by the calling
// function. However, if there is an error, both pointers will be nil.
func BootARC(ctx context.Context, mode PackagesCacheMode, outputDir string, extraArgs []string) (cr *chrome.Chrome, a *arc.ARC, retErr error) {
