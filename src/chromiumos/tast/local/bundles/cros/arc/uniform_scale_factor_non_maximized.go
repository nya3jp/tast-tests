...
// The returned value is used as the baseline to confirm that uniform scaling is applied correctly.
func defaultPixelCount(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, a *arc.ARC) (int, error) {
...
func UniformScaleFactorNonMaximized(ctx context.Context, s *testing.State) {
	...
	defaultPixelCount, err := defaultPixelCount(ctx, cr, tconn, a)
	if err != nil {
		s.Fatal("Failed to get default pixel count: ", err)
	}

	if err := perappdensity.VerifyPixelsWithUSFEnabled(ctx, cr, tconn, a, arc.WindowStateNormal, defaultPixelCount); err != nil {
