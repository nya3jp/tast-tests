...
func defaultPixelCount(ctx context.Context, a *arc.ARC, tconn *chrome.TestConn, cr *chrome.Chrome, act *arc.Activity) (int, error) {
	if err := perappdensity.StartDensityActivityWithWindowState(ctx, a, tconn, cr, act, arc.WindowStateNormal); err != nil {
...
func UniformScaleFactorNonMaximized(ctx context.Context, s *testing.State) {
	...
	if err := perappdensity.EnableUSFAndConfirmPixelState(ctx, a, tconn, cr, act, arc.WindowStateNormal, defaultPixelCount); err != nil {
