...
func MouseInput(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Failed to create motioninput.WmTestRunner: ", err)
	}
	...
	for _, params := range []motioninput.WmTestParams{
		...
		runner.RunTest(ctx, s, &params, "Verify Mouse", verifyMouse)
	}
}
...
func verifyMouse(ctx context.Context, s *testing.State, t *motioninput.WmTestState, tester *motioninput.Tester) {
