...
const playStroeAppID = "cnbgggchhmkkdmeppjobngjoejnihlei"
const arcSettingAppID = "mconboelelhjpkbdhhiijkgcimoangdj"
...
func launchAndroidSettings(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn) error {
	...
	if err := settingPage.Close(ctx); err != nil {
		return errors.Wrap(err, "failed to close ChromeOS settting page")
	}
	return nil
}
...
func GhostWindow(ctx context.Context, s *testing.State) {
	...
	// Test restore single PlayStore task.
	{
		...
		if err := verifyGhostWindow(ctx, s, cr, false, playStroeAppID); err != nil {
	...
	// Test restore PlayStore and Android Setting tasks.
	{
		...
		if err := verifyGhostWindow(ctx, s, cr, false, arcSettingAppID); err != nil {
			s.Fatal("Failed to launch ghost window: ", err)
		}
	}
}
