...
func verifyLog(ctx context.Context, cvconn *chrome.Conn, expectedLog axEventLog, checkOnlyLatest bool) error {
	...
	i := 0
	filteredLogs := make([]axEventLog, 0)
...
func runTestStep(ctx context.Context, cvconn *chrome.Conn, tconn *chrome.TestConn, ew *input.KeyboardEventWriter, test axEventTestpStep, isFirstStep bool) error {
...
// getEventTestSteps returns a slice of axEventTestpStep, which is read from the specific file.
func getEventTestSteps(filepath string) ([]axEventTestpStep, error) {
...
func AccessibilityEvent(ctx context.Context, s *testing.State) {
	...
	testFunc := func(ctx context.Context, cvconn *chrome.Conn, tconn *chrome.TestConn, currentActivity accessibility.TestActivity) error {
		...
		axEventTestpSteps, err := getEventTestSteps(s.DataPath(axEventFilePrefix + currentActivity.Name + ".json"))
		if err != nil {
			return errors.Wrap(err, "error reading from JSON")
		}
		for i, test := range axEventTestpSteps {
