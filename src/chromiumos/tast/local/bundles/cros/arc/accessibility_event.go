...
import (
	...
	"chromiumos/tast/local/arc/accessibility"
...
func AccessibilityEvent(ctx context.Context, s *testing.State) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		if res, err := accessibility.IsAccessibilityEnabled(ctx, a); err != nil {
	...
	// until here, wait for it to say chromevox spoken feedback is ready, before continuing with doing anything else.
