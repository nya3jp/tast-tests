...
func AccessibilityEvent(ctx context.Context, s *testing.State) {
	const (
		...
		nextKey = "Search+Space"
	)

	accessibility.RunTest(ctx, s, accessibility.MainActivity, func(ctx context.Context, a *arc.ARC, cvconn *chrome.Conn, tconn *chrome.TestConn, ew *input.KeyboardEventWriter) error {
