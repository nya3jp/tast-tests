...
// runTestStep is a function that:
// 1. Presses keys.
// 2. Waits for an event to fire on a node. The event and node are specified by
// the step parameter.
// 3. Waits for ChromeVox focus to change to the expected node.
func runTestStep(ctx context.Context, cvconn *a11y.ChromeVoxConn, tconn *chrome.TestConn, ew *input.KeyboardEventWriter, step axEventTestStep, isFirstStep bool) error {
	// Returns a function that presses keys.
	...
	// Returns a function that waits for ChromeVox to focus a node.
	...
	if err := ui.IfSuccessThen(ui.WithTimeout(10*time.Second).WaitForEvent(step.focus, step.event, sendKeyEvent()), waitForFocusedNode())(ctx); err != nil {
		return errors.Wrap(err, "failed to run a test step")
	}
