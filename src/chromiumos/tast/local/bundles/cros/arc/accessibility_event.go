...
func focusAndCheckElement(ctx context.Context, chromeVoxConn *chrome.Conn, elementClass string, expectedOutput []string, outputFilePath string) error {
	...
	if err := chromeVoxConn.Exec(ctx, "CommandHandler.onCommand('forward')"); err != nil {
		return errors.Wrap(err, "could not go to next element")
	}
  ...
  return returnLogs;
	...
	LogStore.instance.getLogsOfType(TextLog.LogType.EVENT).toString();`, &gotOutput); err != nil {
