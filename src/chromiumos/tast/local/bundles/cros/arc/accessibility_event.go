...
func runTestStep(ctx context.Context, cvconn *a11y.ChromeVoxConn, tconn *chrome.TestConn, ew *input.KeyboardEventWriter, step axEventTestStep, isFirstStep bool) error {
	...
	// Using cvconn instead of tconn because event from ARC++ are not sent to test connection.
	watcher, err := uiauto.NewRootWatcher(ctx, tconn, step.event.eventType)
	if err != nil {
	...
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		testing.ContextLogf(ctx, "events %+v", events)
		...
		return errors.New("expected event didn't occur")
...
func AccessibilityEvent(ctx context.Context, s *testing.State) {
	MainActivityTestSteps := []axEventTestStep{
		...
		axEventTestStep{
			...
			axEventParams{
				eventType: event.CheckedStateChanged,
			},
		},
		axEventTestStep{
			"Tab",
		...
		axEventTestStep{
			...
			axEventParams{
				eventType: event.RangeValueChanged,
			},
		},
		axEventTestStep{
			"Tab",
	...
	EditTextActivityTestSteps := []axEventTestStep{
		...
		axEventTestStep{
			...
			axEventParams{
				eventType: event.ValueInTextFieldChanged,
			},
		},
	}
