...
func verifyLogs(ctx context.Context, chromeVoxConn *chrome.Conn, expectedLogs []eventLog, checkOnlyLatest bool) error {
	...
	// Sometimes unrelated events are included after initialization (like focusing the entire applicatioin)
	// When checkOnlyLatest is true, check the latest logs only.
