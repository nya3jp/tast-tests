...
func verifyLog(ctx context.Context, chromeVoxConn *chrome.Conn, expectedLog []textLog) error {
	var log []textLog
	if err := chromeVoxConn.Eval(ctx, "LogStore.instance.getLogsOfType(TextLog.LogType.EVENT)", &log); err != nil {
		return errors.Wrap(err, "failed to get event log")
	}

	if !reflect.DeepEqual(log, expectedLog) {
		return errors.Errorf("event output is not as expected: got %q; want %q", log, expectedLog)
	}
	return nil
}
...
func focusAndIncrementElement(ctx context.Context, chromeVoxConn *chrome.Conn, elementClass string, expectedOutput []textLog, initialValue, expectedValue int) error {
	...
	if err := verifyLog(ctx, chromeVoxConn, expectedOutput); err != nil {
		return err
	}
	return nil
}
...
func AccessibilityEvent(ctx context.Context, s *testing.State) {
	...
	toggleButtonLog := []textLog{
		expectedTextLog("focus", "OFF", "undefined"),
		expectedTextLog("checkedStateChanged", "ON", "undefined"),
	}
	if err := focusAndCheckElement(ctx, chromeVoxConn, toggleButton, toggleButtonLog); err != nil {
	...
	if err := focusAndCheckElement(ctx, chromeVoxConn, checkBox, checkBoxLog); err != nil {
	...
	if err := focusAndIncrementElement(ctx, chromeVoxConn, seekBar, seekBarLog, seekBarInitialValue, seekBarExpectedValue); err != nil {
		s.Fatal("Failed focusing seekBar: ", err)
	}

	// Focus to and increment seekBarDiscrete element.
	seekBarDiscreteLog := []textLog{
		expectedTextLog("focus", "seekBarDiscrete", "undefined"),
		expectedTextLog("valueChanged", "seekBarDiscrete", "undefined"),
	}
	if err := focusAndIncrementElement(ctx, chromeVoxConn, seekBar, seekBarDiscreteLog, seekBarDiscreteInitialValue, seekBarDiscreteExpectedValue); err != nil {
