...
// TODO(hirokisato): Use event object for LogStr instead of dumped string.
type textLog struct {
...
func checkOutputLog(ctx context.Context, chromeVoxConn *chrome.Conn, expectedLogs []textLog) error {
	var logs []textLog
	if err := chromeVoxConn.Eval(ctx, "LogStore.instance.getLogsOfType(TextLog.LogType.EVENT)", &logs); err != nil {
...
func focusAndIncrementElement(ctx context.Context, chromeVoxConn *chrome.Conn, elementClass string, expectedOutput []textLog, initialValue, expectedValue int) error {
	...
	if err := checkOutputLog(ctx, chromeVoxConn, expectedOutput); err != nil {
		return err
	}
	return nil
}
...
// Returns an error indicating the success of both actions.
func focusAndCheckElement(ctx context.Context, chromeVoxConn *chrome.Conn, elementClass string, expectedOutput []textLog) error {
	ew, err := input.Keyboard(ctx)
	if err != nil {
		return errors.Wrap(err, "error with creating EventWriter from keyboard")
	}
...
func AccessibilityEvent(ctx context.Context, s *testing.State) {
	...
	if err := focusAndCheckElement(ctx, chromeVoxConn, toggleButton, toggleButtonOutput); err != nil {
	...
	if err := focusAndCheckElement(ctx, chromeVoxConn, checkBox, checkBoxOutput); err != nil {
	...
	if err := focusAndIncrementElement(ctx, chromeVoxConn, seekBar, seekBarOutput, seekBarInitialValue, seekBarExpectedValue); err != nil {
	...
	if err := focusAndIncrementElement(ctx, chromeVoxConn, seekBar, seekBarDiscreteOutput, seekBarDiscreteInitialValue, seekBarDiscreteExpectedValue); err != nil {
