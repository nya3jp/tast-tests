...
func waitForValueFocused(ctx context.Context, chromeVoxConn *chrome.Conn, className string, expectedValue int) error {
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if gotValue != expectedValue {
			return errors.Errorf("%s does not have expected value", className)
		}
		return nil
	}, &testing.PollOptions{Timeout: 30 * time.Second}); err != nil {
		return errors.Errorf("timed out waiting for %q with value %d to receive focus: %v", className, expectedValue, err)
	}
	return nil
}
