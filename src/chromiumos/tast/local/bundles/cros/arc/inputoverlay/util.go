...
// Package inputoverlay contains functions and structs used for testing the gaming input overlay.
...
func pollTappedCorrectly(ctx context.Context, params *TestParams, m mode, heuristic ButtonHeuristics) error {
	cmd := exec.Command("adb", "shell", "logcat", "-d", "|", "grep", "InputOverlayTest")
	out, err := cmd.Output()
	if err != nil {
		return errors.Wrapf(err, "failed to execute logcat command: %s", cmd.String())
	}
	...
	if len(lines) < int(m) {
		return errors.Errorf("only %v lines returned by logcat", len(lines))
	}
	...
	if err := appWindowBounds(ctx, params); err != nil {
...
// appWindowBounds saves the window bounds of the GIO test application in the provided
// TestParams pointer.
func appWindowBounds(ctx context.Context, params *TestParams) error {
	if params.appBounds != (coords.Point{}) {
		return nil
	}

	loc, err := params.Activity.WindowBounds(ctx)
	if err != nil {
	...
	params.appBounds = coords.Point{appWidth, appHeight}
...
func approximateLocation(point coords.Point, params *TestParams, heuristic ButtonHeuristics) error {
	x := int(float64(params.appBounds.X) * heuristic.xHeuristic)
	y := int(float64(params.appBounds.Y) * heuristic.yHeuristic)
