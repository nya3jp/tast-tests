...
func UreadaheadValidation(ctx context.Context, s *testing.State) {
	...
	if err = checkPackFileDump(ctx, logPath); err != nil {
		s.Fatalf("Failed to verify ureadahead pack file dump, please check %q", ureadaheadLogName)
	}
	...
	if vmEnabled {
		...
		if err = checkPackFileDump(ctx, vmLogPath); err != nil {
			s.Fatalf("Failed to verify ureadahead pack file dump, please check %q", ureadaheadVMLogName)
		}
	}
}
