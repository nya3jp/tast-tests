...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			// Please see arc.DataCollector for details.
...
func UreadaheadValidation(ctx context.Context, s *testing.State) {
	const (
		...
		// Guest ureadahead pack is smaller than host due to memory constraints in ARCVM.
		minAcceptableVMUreadaheadPackSizeKB = 100 * 1024
	)
	...
	if vmEnabled {
		...
		if err = checkPackFileDump(ctx, vmLogPath, minAcceptableVMUreadaheadPackSizeKB); err != nil {
