...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{
			...
			{
				Name:              "dataonline_allcontacts_small_file",
				ExtraSoftwareDeps: []string{"android_p"},
			...
			{
				Name:              "dataonline_allcontacts_small_file_vm",
				ExtraSoftwareDeps: []string{"android_vm"},
				Fixture:           "nearbyShareDataUsageOnlineNoOneARCEnabled",
				Val: nearbytestutils.TestData{
					Filename:        checkBoxSmallFileID,
			...
			{
				Name:              "dataonline_allcontacts_medium_file",
				ExtraSoftwareDeps: []string{"android_p"},
				Fixture:           "nearbyShareDataUsageOnlineNoOneARCEnabled",
				Val: nearbytestutils.TestData{
			...
			{
				Name:              "dataonline_allcontacts_large_file",
				ExtraSoftwareDeps: []string{"android_p"},
				Fixture:           "nearbyShareDataUsageOnlineNoOneARCEnabled",
				Val: nearbytestutils.TestData{
			...
			{
				Name:              "dataonline_allcontacts_large_file_vm",
				ExtraSoftwareDeps: []string{"android_vm"},
				Fixture:           "nearbyShareDataUsageOnlineNoOneARCEnabled",
				Val: nearbytestutils.TestData{
					Filename:        checkBoxLargeFileID,
			...
			{
				Name:              "dataonline_allcontacts_multiple_files",
				ExtraSoftwareDeps: []string{"android_p"},
				Fixture:           "nearbyShareDataUsageOnlineNoOneARCEnabled",
				Val: nearbytestutils.TestData{
			...
			{
				Name:              "dataonline_allcontacts_multiple_files_vm",
				ExtraSoftwareDeps: []string{"android_vm"},
				Fixture:           "nearbyShareDataUsageOnlineNoOneARCEnabled",
				Val: nearbytestutils.TestData{
					Filename:        checkBoxSmallFileID + "," + checkBoxMediumFileID,
...
func NearbyShareSend(ctx context.Context, s *testing.State) {
	const (
		...
		largeFileCheckboxTimeout = 30 * time.Second
	)
	...
	for _, uiID := range strings.Split(string(testData.Filename), ",") {
		...
		if uiID == checkBoxLargeFileID {
			// Need additional timeout for large file to generation when APK starts.
			timeout += largeFileCheckboxTimeout
		}

		// Make sure the check box is available.
		testCheckBoxItem := d.Object(ui.ClassName(checkBoxClassName), shareUIID(uiID))
		if err := testCheckBoxItem.WaitForExists(ctx, timeout); err != nil {
			s.Fatal("Failed to wait for check box to exist: ", err)
		}

		// Make sure the check box is enabled before clicking. Clicking a file check box
		// will only succeed if it's done after the file generation is completed and
		// check box is enabled.
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			enabled, err := testCheckBoxItem.IsEnabled(ctx)
			if err != nil {
				return testing.PollBreak(err)
			}
			if !enabled {
				return errors.New("check box is not enabled")
			}
			return nil
		}, &testing.PollOptions{Timeout: timeout}); err != nil {
			s.Fatal("Failed to verify check box is enabled: ", err)
		}
		...
		checked, err := testCheckBoxItem.IsChecked(ctx)
		if err != nil {
	...
	// Make sure the share button is available.
	testShareButtonItem := d.Object(ui.ClassName(buttonClassName), shareUIID(shareButtonID))
	if err := testShareButtonItem.WaitForExists(ctx, uiTimeout); err != nil {
		s.Fatal("Failed to wait for share button to exist: ", err)
	}
