...
const (
	...
	// Timeout for showing the Android Sharesheet.
	androidSharesheetVisibleTimeout = 40 * time.Second
	...
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{
			...
			},
			{
				...
				Timeout: baseArcTestTime + nearbycommon.ExtraLargeFileOnlineTransferTimeout + androidSharesheetVisibleTimeout + largeFileExtraBufferTime,
			},
			{
...
func NearbyShareSend(ctx context.Context, s *testing.State) {
	...
	if waitForAndroidSharesheet {
		if err = shareUIClickWithTimeout(ctx, d, androidSharesheetVisibleTimeout, ui.ClassName(buttonClassName), shareUIID(shareButtonID)); err != nil {
