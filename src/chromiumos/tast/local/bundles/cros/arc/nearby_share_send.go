...
func NearbyShareSend(ctx context.Context, s *testing.State) {
	...
	for _, uiID := range strings.Split(string(testData.Filename), ",") {
		...
		// Make sure the check box is clickable.
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			enabled, err := testCheckBoxItem.IsClickable(ctx)
			if err != nil {
				return testing.PollBreak(err)
			}
			if !enabled {
				return errors.New("check box not clickable")
			}
			return nil
		}, &testing.PollOptions{Timeout: timeout}); err != nil {
			s.Fatal("Failed to verify check box is clickable: ", err)
		}
	...
	testShareButtonItem := d.Object(ui.ClassName(checkBoxClassName), shareUIID(shareButtonID))
