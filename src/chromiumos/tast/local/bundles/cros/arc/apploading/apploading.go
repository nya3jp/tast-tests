...
func RunTest(ctx context.Context, s *testing.State, config TestConfig) (float64, error) {
	...
	/*
		...
		s.Log("Clearing swap space")
		if err := testexec.CommandContext(ctx, "swapoff -a && swapon -a").Run(testexec.DumpLogOnError); err != nil {
			return score, errors.New("failed to clear swap space")
		}
	*/

	s.Log("Starting setup")
	...
	sup.Add(setup.GrantAndroidPermission(ctx, a, packageName, "android.permission.INTERNET"))
	sup.Add(setup.GrantAndroidPermission(ctx, a, packageName, "android.permission.ACCESS_NETWORK_STATE"))
	...
	if err := power.WaitUntilCPUCoolDown(ctx, power.CoolDownPreserveUI); err != nil {
		return score, errors.Wrap(err, "failed to wait until CPI is cool down")
	}
	...
	s.Log("Finished writing to log: ", outputFile)
	...
	s.Logf("Finished test with result: %d", result)
