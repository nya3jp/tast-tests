...
// This provides longer timeout with higher temperature (46 to 60c) threshold than default
// due to numerous failures on low-end devices.
func coolDownConfig() cpu.CoolDownConfig {
	cdConfig := cpu.DefaultCoolDownConfig(cpu.CoolDownPreserveUI)
	cdConfig.PollTimeout = 5 * time.Minute
