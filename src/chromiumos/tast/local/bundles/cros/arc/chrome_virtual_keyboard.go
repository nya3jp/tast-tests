...
func editingTest(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, cr *chrome.Chrome, d *ui.Device, s *testing.State) {
	const (
...
func focusChangeTest(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, cr *chrome.Chrome, d *ui.Device, s *testing.State) {
	const (
		pkg          = "org.chromium.arc.testapp.keyboard"
	...
	if shown, err := vkb.IsShown(ctx, tconn); shown || err != nil {
		s.Fatal("The virtual keyboard doesn't hide: ", err)
	}
...
func ChromeVirtualKeyboard(ctx context.Context, s *testing.State) {
	...
	s.Run(ctx, "focusChange", func(ctx context.Context, s *testing.State) {
		focusChangeTest(ctx, tconn, a, cr, d, s)
	})
}
