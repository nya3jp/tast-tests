...
func waitForPrintPreviewToLoad(ctx context.Context, root *ui.Node) error {
	params := ui.FindParams{
		Name: "Loading preview",
	}
	loadingText, err := root.DescendantWithTimeout(ctx, params, 10*time.Second)
	if err != nil {
		return errors.Wrap(err, "failed to find loading text")
	}
	defer loadingText.Release(ctx)
...
func Print(ctx context.Context, s *testing.State) {
	...
	if err := waitForPrintPreviewToLoad(ctx, root); err != nil {
