...
func AshWindowState(ctx context.Context, s *testing.State) {
	...
	for _, test := range []struct {
		...
		state, err := ash.SetArcAppWindowState(ctx, tconn, pkg, test.wmEvent)
		if err != nil {
			s.Fatal("Failed to set window state for Settings app. ")
		}
		if state != test.expectedWindowState {
			s.Fatal("Unexpected window state: got %s, want %s", state, test.expectedWindowState)
		}
	}
}
