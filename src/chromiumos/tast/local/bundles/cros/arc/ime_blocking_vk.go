...
// waitVKUntilShownOrHidden waits until the virtual keyboard to be shown or hidden.
func waitVKUntilShownOrHidden(ctx context.Context, tconn *chrome.Conn, shown bool) error {
...
func ImeBlockingVK(ctx context.Context, s *testing.State) {
	...
	if err := waitVKUntilShownOrHidden(ctx, tconn, true); err != nil {
	...
	if err := waitVKUntilShownOrHidden(ctx, tconn, false); err != nil {
