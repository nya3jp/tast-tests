...
type Directory struct {
	...
	Path string
	// If set to true, create the test file at "Path". For directory where there is no actual file
	// path, set this to false and create the file before running the test (e.g. MTP).
	CreateTestFile bool
	// Optional: If set to true, wait for file type to appear before opening the file. Currently used
	// by DriveFS to ensure metadata has arrived.
	CheckFileType bool
}
