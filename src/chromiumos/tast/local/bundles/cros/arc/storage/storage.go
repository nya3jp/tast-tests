...
// SubDirectories, if specified, will open the sub-directories under "Name".
// Path is the actual path of the directory on the file system.
// CreateTestFile, if set to true, create the test file at "Path". For directory
// where there is no actual file path, set this to false and create the file before
// running the test (e.g. MTP).
// CheckFileType (Optional), if set to true, wait for file type to appear before
// opening the file. Currently used by DriveFS to ensure metadata has arrived.
