...
func DisableArc(ctx context.Context, s *testing.State) {
	...
	if err := ash.WaitForAppClosed(ctx, tconn, apps.PlayStore.ID); err != nil {
		s.Fatal("Failed to wait for Play Store to finish closing: ", err)
	}
...
func turnOffPlayStore(ctx context.Context, tconn *chrome.TestConn) error {
	...
	return uiauto.Run(ctx,
		ui.LeftClick(nodewith.Name("Apps").Role(role.Heading)),
		ui.FocusAndWait(nodewith.Name("Google Play Store").Role(role.Button)),
		ui.LeftClick(nodewith.Name("Google Play Store").Role(role.Button)),
