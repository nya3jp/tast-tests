...
func ResizeActivity(ctx context.Context, s *testing.State) {
	...
	if sw.TabletMode != nil && *sw.TabletMode == pmpb.SwitchStates_ON {
		s.Log("This test is not valid when the device is in Tablet mode. Skipping test")
		return
	}
	...
	if err := act.ResizeWindow(ctx, arc.BorderBottomRight, arc.Point{bounds.Left, bounds.Top}, 300*time.Millisecond); err != nil {
	...
	if err := act.MoveWindow(ctx, arc.Point{0, 0}, 1000*time.Millisecond); err != nil {
	...
	for idx, entry := range []struct {
		...
		{"right", arc.BorderRight, arc.Point{dispSize.W - marginForShelf, centerHeight}, 100 * time.Millisecond},
		{"bottom", arc.BorderBottom, arc.Point{bounds.Left, dispSize.H - marginForShelf}, 300 * time.Millisecond},
		{"bottom-right", arc.BorderBottomRight, arc.Point{dispSize.W - marginForShelf, dispSize.H - marginForShelf}, 100 * time.Millisecond},
	} {
		...
		act.ResizeWindow(ctx, entry.border, arc.Point{restoreBounds.Left, restoreBounds.Top}, 500*time.Millisecond)
	}
}

// countBlackPixels returns how many black pixels are contained in image.
func countBlackPixels(image image.Image) int {
	// TODO(ricardoq): At least on Eve, Nocturne, Caroline and Kevin, the color
	// that we are looking for is RGBA(0,0,0,255). But it might be possible that
	// on certain devices the color might be slightly different. In that case
	// we should adjust the colorMaxDiff.
