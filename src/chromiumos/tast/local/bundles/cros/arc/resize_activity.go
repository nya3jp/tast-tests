...
func init() {
	testing.AddTest(&testing.Test{
		Func:         ResizeActivity,
		Desc:         "Verifies that resizing ARC++ applications don't generate black content. See: http://b/122966727",
...
func ResizeActivity(ctx context.Context, s *testing.State) {
	...
	for idx, entry := range []struct {
		...
		bounds, err = ac.Bounds()
		if err != nil {
			s.Fatal("Failed to get activity bounds")
		}
		subImage := si.SubImage(
