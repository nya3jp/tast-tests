...
func offsetForBorder(border int, bounds rect) vertex {
	...
	dict := map[int]vertex{
		borderTop:         vertex{bounds.right + (bounds.right-bounds.left)/2, bounds.top - borderOffset},
		borderBottom:      vertex{bounds.right + (bounds.right-bounds.left)/2, bounds.bottom + borderOffset},
		borderLeft:        vertex{bounds.left - borderOffset, bounds.top + (bounds.bottom-bounds.top)/2},
		borderRight:       vertex{bounds.right + borderOffset, bounds.top + (bounds.bottom-bounds.top)/2},
		borderTopLeft:     vertex{bounds.left - borderOffset, bounds.top - borderOffset},
		borderTopRight:    vertex{bounds.right + borderOffset, bounds.top - borderOffset},
		borderBottomLeft:  vertex{bounds.left - borderOffset, bounds.bottom + borderOffset},
		borderBottomRight: vertex{bounds.right + borderOffset, bounds.bottom + borderOffset},
	}
	return dict[border]
}
func (ac *activity) resize(border int, toX, toY int, t time.Duration) error {
	bounds, _ := ac.bounds()
...
func ResizeActivity(ctx context.Context, s *testing.State) {
	...
	ac.resize(borderRight, 2300, bounds.top, 200*time.Millisecond)
	ac.resize(borderRight, 1000, bounds.top, 200*time.Millisecond)
	ac.resize(borderLeft, 30, bounds.top, 200*time.Millisecond)
	ac.resize(borderLeft, 1000, bounds.top, 200*time.Millisecond)
