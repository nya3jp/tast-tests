...
func CxxCrash(ctx context.Context, s *testing.State) {
	...
	// On ARC++, the Linux kernel is shared with ARC and Chrome OS. The kernel can directly
	// receive ARC's C++ binary crashes and no temporary files are created. On ARCVM, some
	// temporary files are created as part of the crash handling and we want to clean it up.
