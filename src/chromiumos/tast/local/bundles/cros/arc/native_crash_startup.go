...
func init() {
	testing.AddTest(&testing.Test{
		Func:         NativeCrashStartup,
		Desc:         "Test handling of a native binary crash which happens before ArcCrashCollector starts",
...
func NativeCrashStartup(ctx context.Context, s *testing.State) {
	...
	// We need to get BuildProperty before rebooting ARCVM because the `a` struct is invalidated by rebooting.
	s.Log("Getting BuildProperty")
	...
	// To make the situtation that there are some logs when ARCVM boots, stop ArcCrashCollector service, make crashes while it stops, and reboot ARCVM. We cannot kill the service because it restarts automatically.
	s.Log("Stop ArcCrashCollector service")
	if err := a.Command(ctx, "pkill", "-STOP", "-f", "org\\.chromium\\.arc\\.crash_collector").Run(); err != nil {
		s.Fatal("Failed to stop ArcCrashCollector service: ", err)
	}
