...
func init() {
	testing.AddTest(&testing.Test{
		Func: RobloxOBBPerf,
		Desc: "Measures Roblox OBB performance",
		Contacts: []string{
			"ricardo@chromium.org",
...
func RobloxOBBPerf(ctx context.Context, s *testing.State) {
	// Roblox itself measures the OBB performance and the result it logged in logcat.
	// This test just grabs that value from logcat and report it.

	// Roblox has the "OBB" file built-in. This means that this test doesn not
	...
	// Roblox might trigger an ANR while doing unzip OBB (see: http://b/169182810).
	// This test does not close the possible ANR dialog. The dialog will be closed
	// automatically during test cleanup when the APK is uninstalled.
	...
	// Reduce the scope of logcat entries.
	...
	if obbTimeMS == -1 {
		s.Fatal("Failed to get OBB time")
	}

	obbTimeMetric := perf.Metric{Name: "obb_time", Unit: "ms", Direction: perf.SmallerIsBetter}
	p := perf.NewValues()
