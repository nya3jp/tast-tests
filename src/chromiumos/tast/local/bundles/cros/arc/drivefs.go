...
func Drivefs(ctx context.Context, s *testing.State) {
	...
	expectations := []storage.Expectation{
		...
		{LabelID: storage.URIID, Value: constructURI(vmEnabled, drivefsRoot, config.FileName)},
...
// constructURI constructs a Drive FS URI.
func constructURI(vmEnabled bool, drivefsRoot, file string) string {
	if vmEnabled {
