...
const (
	phoneResizeLockModeName     = "Phone"
	tabletResizeLockModeName    = "Tablet"
	resizableResizeLockModeName = "Resizable"
)

func (mode resizeLockMode) String() string {
	switch mode {
	case phoneResizeLockMode:
		return phoneResizeLockModeName
	case tabletResizeLockMode:
		return tabletResizeLockModeName
	case resizableResizeLockMode:
		return resizableResizeLockModeName
	default:
		return ""
	}
}
...
func testResizeLockedAppCUJ(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, cr *chrome.Chrome) error {
	...
	for _, test := range []struct {
		...
		if err := toggleAppManagementSettingToggle(ctx, tconn, a, d, cr, activity, resizeLockAppName, test.currentMode, test.nextMode); err != nil {
			return errors.Wrapf(err, "failed to toggle the resizability state from %s to %s on the Chrome OS settings", getResizeLockModeString(test.currentMode), getResizeLockModeString(test.nextMode))
		}
	}
