...
func testResizeLockedAppCUJ(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, cr *chrome.Chrome) error {
	...
	if err := toggleResizeLockMode(ctx, tconn, a, d, cr, activity, resizableResizeLockMode, phoneResizeLockMode, dialogActionNoDialog); err != nil {
		return errors.Wrapf(err, "failed to change the resize lock mode of %s from resizable to phon", resizeLockMainActivityName)
	}
...
// checkBorder checks whether the splash screen for compatibility mode is shown or not.
...
// waitForCompatModeMenuToDisappear waits for the compat-mode button to disappear.
// After one of the resize lock mode buttons are selected, the compat mode button disappears after a few seconds of delay.
