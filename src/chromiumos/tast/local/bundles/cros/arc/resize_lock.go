...
func (mode resizeLockMode) String() string {
	...
	case phoneResizeLockMode:
		return wm.PhoneButtonName
	case tabletResizeLockMode:
		return wm.TabletButtonName
	case resizableResizeLockMode:
		return wm.ResizableButtonName
	default:
		return ""
	}
}
...
func checkResizeLockState(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, cr *chrome.Chrome, activity *arc.Activity, mode resizeLockMode, isSplashVisible bool) error {
	...
	if err := wm.CheckVisibility(ctx, tconn, wm.BubbleDialogClassName, isSplashVisible); err != nil {
...
func checkCompatModeButton(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, cr *chrome.Chrome, activity *arc.Activity, mode resizeLockMode) error {
	if mode == nonEligibleResizeLockMode {
		return wm.CheckVisibility(ctx, tconn, wm.CenterButtonClassName, false /* visible */)
	}

	return testing.Poll(ctx, func(ctx context.Context) error {
		button, err := chromeui.Find(ctx, tconn, chromeui.FindParams{ClassName: wm.CenterButtonClassName})
		if err != nil {
			return errors.Wrap(err, "failed to find the compat-mode button")
		}
		button.Release(ctx)
...
func showCompatModeMenuViaButtonClick(ctx context.Context, tconn *chrome.TestConn) error {
	...
	return wm.CheckVisibility(ctx, tconn, wm.BubbleDialogClassName, true /* visible */)
}
...
func showCompatModeMenuViaKeyboard(ctx context.Context, tconn *chrome.TestConn, keyboard *input.KeyboardEventWriter) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err := chromeui.WaitUntilExists(ctx, tconn, chromeui.FindParams{ClassName: wm.BubbleDialogClassName}, 2*time.Second); err != nil {
...
func waitForCompatModeMenuToDisappear(ctx context.Context, tconn *chrome.TestConn) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		dialog, err := chromeui.Find(ctx, tconn, chromeui.FindParams{ClassName: wm.BubbleDialogClassName})
		if err == nil && dialog != nil {
			defer dialog.Release(ctx)

			phoneButton, err := dialog.Descendant(ctx, chromeui.FindParams{Name: wm.PhoneButtonName})
...
func closeSplash(ctx context.Context, tconn *chrome.TestConn, method inputMethodType, keyboard *input.KeyboardEventWriter) error {
	splash, err := chromeui.Find(ctx, tconn, chromeui.FindParams{ClassName: wm.BubbleDialogClassName})
	if err != nil {
...
func closeSplashViaKeyboard(ctx context.Context, tconn *chrome.TestConn, splash *chromeui.Node, keyboard *input.KeyboardEventWriter) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		return wm.CheckVisibility(ctx, tconn, wm.BubbleDialogClassName, false /* visible */)
	}, &testing.PollOptions{Timeout: 10 * time.Second})
}
...
func closeSplashViaClick(ctx context.Context, tconn *chrome.TestConn, splash *chromeui.Node) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		button, err := splash.Descendant(ctx, chromeui.FindParams{Name: wm.SplashCloseButtonName})
		if err != nil {
...
func toggleResizeLockMode(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, cr *chrome.Chrome, activity *arc.Activity, currentMode, nextMode resizeLockMode, action confirmationDialogAction, method inputMethodType, keyboard *input.KeyboardEventWriter) error {
	...
	compatModeMenuDialog, err := chromeui.FindWithTimeout(ctx, tconn, chromeui.FindParams{ClassName: wm.BubbleDialogClassName}, 10*time.Second)
	if err != nil {
	...
	if action != dialogActionNoDialog {
		...
		confirmationDialog, err := chromeui.FindWithTimeout(ctx, tconn, chromeui.FindParams{ClassName: wm.OverlayDialogClassName}, 10*time.Second)
		if err != nil {
...
func handleConfirmationDialogViaKeyboard(ctx context.Context, tconn *chrome.TestConn, mode resizeLockMode, confirmationDialog *chromeui.Node, action confirmationDialogAction, keyboard *input.KeyboardEventWriter) error {
	if action == dialogActionCancel {
		return shiftViaTabAndEnter(ctx, tconn, confirmationDialog, chromeui.FindParams{Name: wm.CancelButtonName}, keyboard)
		...
		if action == dialogActionConfirmWithDoNotAskMeAgainChecked {
			if err := shiftViaTabAndEnter(ctx, tconn, confirmationDialog, chromeui.FindParams{ClassName: wm.CheckBoxClassName}, keyboard); err != nil {
				return errors.Wrap(err, "failed to select the checkbox of the resizability confirmation dialog via keyboard")
			}
		}
		return shiftViaTabAndEnter(ctx, tconn, confirmationDialog, chromeui.FindParams{Name: wm.ConfirmButtonName}, keyboard)
	}
	return nil
}
...
func handleConfirmationDialogViaClick(ctx context.Context, tconn *chrome.TestConn, mode resizeLockMode, confirmationDialog *chromeui.Node, action confirmationDialogAction) error {
	if action == dialogActionCancel {
		cancelButton, err := confirmationDialog.DescendantWithTimeout(ctx, chromeui.FindParams{Name: wm.CancelButtonName}, 10*time.Second)
		if err != nil {
		...
		if action == dialogActionConfirmWithDoNotAskMeAgainChecked {
			checkbox, err := chromeui.FindWithTimeout(ctx, tconn, chromeui.FindParams{ClassName: wm.CheckBoxClassName}, 10*time.Second)
			if err != nil {
		...
		confirmButton, err := confirmationDialog.DescendantWithTimeout(ctx, chromeui.FindParams{Name: wm.ConfirmButtonName}, 10*time.Second)
		if err != nil {
...
func toggleAppManagementSettingToggle(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, cr *chrome.Chrome, activity *arc.Activity, appName string, currentMode, nextMode resizeLockMode, method inputMethodType, keyboard *input.KeyboardEventWriter) error {
	...
	case inputMethodKeyEvent:
		if err := shiftViaTabAndEnter(ctx, tconn, nil, chromeui.FindParams{Name: wm.AppManagementSettingToggleName}, keyboard); err != nil {
...
func toggleAppManagementSettingToggleViaClick(ctx context.Context, tconn *chrome.TestConn) error {
	settingToggle, err := chromeui.FindWithTimeout(ctx, tconn, chromeui.FindParams{Name: wm.AppManagementSettingToggleName}, 10*time.Second)
	if err != nil {
...
func openAppManagementSetting(ctx context.Context, tconn *chrome.TestConn, appName string) error {
	...
	appInfoMenuItem, err := chromeui.FindWithTimeout(ctx, tconn, chromeui.FindParams{Name: wm.AppInfoMenuItemViewName, ClassName: wm.MenuItemViewClassName}, 10*time.Second)
	if err != nil {
...
func closeAppManagementSetting(ctx context.Context, tconn *chrome.TestConn) error {
	...
	closeMenuItem, err := chromeui.FindWithTimeout(ctx, tconn, chromeui.FindParams{Name: wm.CloseMenuItemViewName, ClassName: wm.MenuItemViewClassName}, 10*time.Second)
	if err != nil {
...
func checkAppManagementSettingToggleState(ctx context.Context, tconn *chrome.TestConn, mode resizeLockMode) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		settingToggle, err := chromeui.FindWithTimeout(ctx, tconn, chromeui.FindParams{Name: wm.AppManagementSettingToggleName}, 2*time.Second)
		if err != nil {
