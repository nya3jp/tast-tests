...
func runReboot(ctx context.Context, s *testing.State, a *arc.ARC, cr *chrome.Chrome) error {
	...
	s.Log("Waiting for old init process to be killed")
	if err := waitProcessDie(ctx, int(oldPID)); err != nil {
		return errors.Wrap(err, "failed to wait for old init process to be killed")
	}
...
// waitProcessDie waits for a process to die.
// It checks process state via the file "/proc/<pid>/stat".
func waitProcessDie(ctx context.Context, pid int) error {
	statPath := "/proc/" + strconv.Itoa(pid) + "/stat"
	return testing.Poll(ctx, func(ctx context.Context) error {
		b, err := ioutil.ReadFile(statPath)
		if err != nil {
			if os.IsNotExist(err) {
				return nil // The process is already dead.
			}
			// Read error. Should be reported.
			return errors.Wrapf(err, "failed to read %q", statPath)
		}

		stat := strings.Split(strings.TrimSpace(string(b)), " ")
		if stat[2] != "Z" {
			return errors.New("process is still alive")
		}
		return nil
...
func getVMCID(ctx context.Context, cr *chrome.Chrome) (int, error) {
	...
	if err != nil {
		...
		return -1, errors.New("VM is not enabled")
	}
