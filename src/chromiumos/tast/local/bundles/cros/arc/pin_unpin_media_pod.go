...
func PinUnpinMediaPod(ctx context.Context, s *testing.State) {
	...
	for appName, media := range map[string]*apputil.Media{
		youtubemusic.AppName: apputil.NewMedia("Beat It", "Michael Jackson • 4:19"),
	...
	if err := pinAndVerify(tconn, ytappVideo)(ctx); err != nil {
		s.Fatal("Failed to pin and verify: ", err)
	}

	if err := unpinAndVerify(tconn)(ctx); err != nil {
...
func unpinAndVerify(tconn *chrome.TestConn) uiauto.Action {
	return uiauto.Combine("unpin and find media pod in quick settings",
		...
		uiauto.New(tconn).WaitUntilExists(quicksettings.MediaControlsPod),
	)
}

// pinAndVerify pins media pod and verify it is disappeared in quick settings.
func pinAndVerify(tconn *chrome.TestConn, title string) uiauto.Action {
	return uiauto.Combine("pin media pod and verify it is disappeared in quick settings",
		...
		reopenQuickSettings(tconn),
		uiauto.New(tconn).WaitUntilGone(quicksettings.MediaControlsPod),
	)
}
