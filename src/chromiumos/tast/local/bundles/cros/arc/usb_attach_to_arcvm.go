...
func UsbAttachToArcvm(ctx context.Context, s *testing.State) {
	...
	// Sleep 1 second before check usb state to avoid race issue.
	// Similar issue when attaching the device to vm.
...
// cleanupVirtualUsbDevice, no op if fail to create virtual USB device at setup.
...
// checkUsbDeviceStatus which presnented by string |VID:PID| exsits in host
// and/or vm.
...
func checkUsbDeviceStatus(ctx context.Context, host, vm bool) error {
	...
	// If the specified device is not found, a non-zero exit code is returned by
	// lsusb and err will not be nil
	// |err == nil| indicates |lsusb -d XXXX:XXXX| finds the expected devices.
	if err := testexec.CommandContext(ctx, "lsusb", "-d",
		fmt.Sprintf("%s:%s", usbVID, usbPID)).Run(testexec.DumpLogOnError); (err == nil) != host {
	...
	if err := testexec.CommandContext(ctx, "/usr/sbin/android-sh", "-c",
		"lsusb | grep "+fmt.Sprintf("%s:%s", usbVID, usbPID)).Run(testexec.DumpLogOnError); (err == nil) != vm {
