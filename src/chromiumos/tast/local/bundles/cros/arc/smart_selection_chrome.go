...
func waitForMapOption(ctx context.Context, ui *uiauto.Context, address *nodewith.Finder) error {
	...
	return testing.Poll(ctx, func(ctx context.Context) error {
		if err := ui.RightClick(address)(ctx); err != nil {
			return errors.New("failed to right click on address")
		}
		err := testing.Poll(ctx, func(ctx context.Context) error {
			if err := ui.Exists(mapOption)(ctx); err != nil {
				return errors.New("did not find Map in context menu")
			}
			return nil
		...
		if err != nil {
			...
			if e := ui.RightClick(address)(ctx); e != nil {
				return errors.Wrap(err, "failed to close context menu")
			}
		} else {
