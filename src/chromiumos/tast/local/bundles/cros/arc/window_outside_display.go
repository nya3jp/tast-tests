...
func WindowOutsideDisplay(ctx context.Context, s *testing.State) {
	...
	initBounds.Width /= 2
	initBounds.Height /= 2
	if _, _, err := ash.SetWindowBounds(ctx, tconn, window.ID, initBounds, window.DisplayID); err != nil {
		s.Fatal("Failed to set window bounds: ", err)
	}

	if err := waitForWindowBounds(ctx, initBounds); err != nil {
		s.Fatal("Failed to wait for the activity to resize: ", err)
	}

	// Grab the center of the window caption bar.
	initDst := coords.Point{X: initBounds.Width / 2, Y: window.CaptionHeight / 2}
