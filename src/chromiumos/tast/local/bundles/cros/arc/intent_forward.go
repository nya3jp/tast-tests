...
func IntentForward(s *testing.State) {
	...
	test := func(action, data, url string) {
		...
		if err := cmd.Run(); err != nil {
			cmd.DumpLog(ctx)
			s.Error("Failed to send an intent: ", err)
			return
		}

		if _, err := cr.NewConnForTarget(ctx, func(t *chrome.Target) bool {
			return t.URL == url
		}); err != nil {
			s.Error(err)
		}
	...
	test(viewAction, localWebURL, localWebURL)
	test(viewDownloadsAction, "", filesAppURL)
	test(setWallpaperAction, "", wallpaperPickerURL)
}
