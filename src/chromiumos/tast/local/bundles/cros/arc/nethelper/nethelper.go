...
func listenForClients(ctx context.Context, listener net.Listener) {
	for {
		...
		testing.ContextLogf(ctx, "Connection is ready %s<->%s", conn.RemoteAddr().String(), conn.LocalAddr().String())

		// Use single bufio.Reader instance for each connection.
		go handleClient(ctx, conn, bufio.NewReader(conn))
	}

}

func handleClient(ctx context.Context, conn net.Conn, r *bufio.Reader) {
	defer conn.Close()
