...
func (c *Connection) Close(ctx context.Context) error {
	...
	for _, listener := range c.listeners {
		if err := listener.Close(); err != nil {
			errs = append(errs, errors.Wrap(err, "failed to close connection").Error())
		}
	}
	...
	for _, rule := range c.rules {
		for _, cmd := range cmds {
			...
			if err := testexec.CommandContext(ctx, cmd, args...).Run(testexec.DumpLogOnError); err != nil {
				errs = append(errs, errors.Wrapf(err, "failed to remove iptables rule: %s", rule).Error())
			}
			...
			if err := testexec.CommandContext(ctx, cmd, args...).Run(); err == nil {
				errs = append(errs, errors.Errorf("failed to verify removal of iptables rule: %s", rule).Error())
			}
		}
	}
