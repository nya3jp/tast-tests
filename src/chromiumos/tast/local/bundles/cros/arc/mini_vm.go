...
func MiniVM(ctx context.Context, s *testing.State) {
	...
	// Setup Chrome and login. mini-ARCVM should automatically start.
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		return miniVMCommand(ctx, cid, "pidof", "-s", "init").Run()
	}, nil); err != nil {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		if err := miniVMCommand(ctx, cid, "mountpoint", "-q", "/data").Run(); err == nil {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		return miniVMCommand(ctx, cid, "mountpoint", "-q", "/data").Run()
	}, nil); err != nil {
		s.Fatal("Failed to check /data mount: ", err)
	}
}

// miniVMCommand runs a command in the VM over vsh. We cannot use android-sh
// while mini-VM is running since it depends on having the cryptohome-ID
// set for the VM (the ID is not set until the VM is upgraded).
func miniVMCommand(ctx context.Context, cid int, command string, args ...string) *testexec.Cmd {
...
func waitForProp(ctx context.Context, cid int, prop, expected string) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		out, err := miniVMCommand(ctx, cid, "getprop", prop).Output()
		if err != nil {
			return err
		}
		val := strings.TrimSpace(string(out))
		if val != expected {
			return errors.Errorf("prop %s is %q; expected %q", prop, val, expected)
		}
		return nil
	}, nil)
}

// getMiniVMCID returns the CID of the currently running mini-ARCVM instance, if
// any.
