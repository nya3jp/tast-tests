...
func ShelfIcons(ctx context.Context, s *testing.State) {
	...
	for i, button := range buttons {
		...
		// Wait for animation to finish. It should take less than 150ms.
		if err := testing.Sleep(ctx, 150*time.Millisecond); err != nil {
			s.Fatal("Waiting for finished animation failed: ", err)
		}
		// Just in case the sleep wasn't long enough, we retry the screenshotting in case of mismatched colors.
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			...
			for j, menuItem := range menuItems {
				...
				iconColor := getIconColorFromUINode(menuItem, menuIconOffset, img, root)
				if !colorcmp.ColorsMatch(iconColor, expColor, colorMaxDiff) {
					return errors.Errorf("Waiting for animation to finish and color %v to be close to %v", iconColor, expColor)
				}
			}
			// Check icon on the shelf button. Failure here should not cause a retry
			// since the button is not part of the menu animation.
			buttonColor := getIconColorFromUINode(button, 0 /*iconOffset*/, img, root)
...
// getIconColorFromUINode takes a UI Node, and outputs the color of
// a pixel that should be close to the center of the item's icon.
func getIconColorFromUINode(item *ui.Node, iconOffset int, img image.Image, root *ui.Node) color.Color {
