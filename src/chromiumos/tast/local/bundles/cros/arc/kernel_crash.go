...
func KernelCrash(ctx context.Context, s *testing.State) {
	...
	// The android-sh returns the exit code of `echo` command when the command fails to crash ARCVM or the command stops before the crash of ARCVM actually started. The return code seems unstable, so we don't make errors.
