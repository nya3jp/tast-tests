...
func KernelCrash(ctx context.Context, s *testing.State) {
	...
	} else if cmd.ProcessState.ExitCode() != 123 {
		// The android-sh returns 0 if the command succeeds, and returns 1 in most errors which are not related to the kernel crash. It returns 123 when the kernel successfully crashes.
	...
	// Rebooting of ARCVM is slow and the timeout of `crash.WaitForCrashFiles` is relatively too short. So, you need to wait the new ARCVM process explicitly.
	...
	crashDir, err := crash.GetCrashDir(cr.User())
	if err != nil {
