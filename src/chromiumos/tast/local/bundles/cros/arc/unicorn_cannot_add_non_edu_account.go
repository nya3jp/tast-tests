...
func UnicornCannotAddNonEduAccount(ctx context.Context, s *testing.State) {
	...
	defer func() {
		...
		if err := uiauto.Combine("clicking close",
			ui.IfSuccessThen(ui.WithTimeout(10*time.Second).WaitUntilExists(closeButton), ui.LeftClick(closeButton)),
...
func addAndroidAccount(ctx context.Context, arcDevice *androidui.Device, tconn *chrome.TestConn, s *testing.State) error {
	...
	if err := familylink.NavigateEduCoexistenceFlow(ctx, tconn, parentPassword, gellerParentUser, gellerParentPass); err != nil {
		s.Fatal("Failed entering geller account details in add school acount flow: ", err)
	}
