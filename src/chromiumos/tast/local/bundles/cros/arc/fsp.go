...
func Fsp(ctx context.Context, s *testing.State) {
	...
	// Ensure the existence of Text app. This is because
	// TestOpenWithAndroidApp expects that there is at least one app (other
	// than ArcFileReaderTest, the Android app installed in
	// TestOpenWithAndroidApp) that can open a text file.
	if err := installTextAppIfNotInstalled(ctx, s, cr, tconn); err != nil {
		s.Fatal("Failed to prepare Text app: ", err)
	}
...
func installTextAppIfNotInstalled(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn) error {
	const (
		textAppName = "Text"
	...
	s.Log("Installing the missing Text app")
