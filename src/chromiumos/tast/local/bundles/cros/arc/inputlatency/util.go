...
type InputEvent struct {
	...
	// Time (in ns) that the event was generated (by Client kernel if it is ARCVM).
	...
	// Latency (in ms) that calculated in side of Android (not including host latency if it is ARCVM).
...
// mathCalculate calculates mean, median, std dev, max and min for the given float array.
func mathCalculate(values []float64) (mean, median, stdDev, max, min float64) {
...
func EvaluateLatency(ctx context.Context, s *testing.State, d *ui.Device, vmEnabled bool,
	...
	if vmEnabled {
		...
		diffMean, diffMedian, diffStdDev, diffMax, diffMin := mathCalculate(diff)
		vmAdjust = diffMean
...
// EvaluateLatencyReplay gets event data generated by replay, calculates the latency, and adds the result to performance metrics.
func EvaluateLatencyReplay(ctx context.Context, s *testing.State, d *ui.Device, vmEnabled bool,
	...
	for i := 0; i < len(events); i++ {
		if events[i].Action != *action {
			...
			// Equivalent to events[i].EventTimeNS = int64(adjust * 1000000.0) + events[i].ClientEventTime * 1000000
