...
func switchPlayStoreAccount(ctx context.Context, arcDevice *androidui.Device, tconn *chrome.TestConn, s *testing.State) error {

	if err := launcher.LaunchApp(tconn, apps.PlayStore.Name)(ctx); err != nil {
		return errors.Wrap(err, "failed to launch Play Store")
	}

	noThanksButton := arcDevice.Object(androidui.ClassName("android.widget.Button"), androidui.Text("NO THANKS"))
	if err := noThanksButton.WaitForExists(ctx, 10*time.Second); err != nil {
		...
		return errors.Wrap(err, "failed to click noThanksButton")
	}
	...
	if err := avatarIcon.Click(ctx); err != nil {
		return errors.Wrap(err, "failed to click avatarIcon")
	}

	accountNameButton := arcDevice.Object(androidui.ClassName("android.widget.TextView"), androidui.Text("Shireen Snow"))
	if err := accountNameButton.WaitForExists(ctx, 30*time.Second); err != nil {
		return errors.Wrap(err, "failed to find accountNameButton")
	} else if err := accountNameButton.Click(ctx); err != nil {
		return errors.Wrap(err, "failed to click accountNameButton")
	}
