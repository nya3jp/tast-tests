...
func HWOverlayPortrait(ctx context.Context, s *testing.State) {
	...
	for _, entry := range []struct {
		...
		if err := setTabletModeEnabled(ctx, cr, entry.tabletEnabled); err != nil {
		...
		if err := setRotation(ctx, cr, deviceID, entry.rotation); err != nil {
...
func setRotation(ctx context.Context, cr *chrome.Chrome, id string, rot int) error {
	tconn, err := cr.TestAPIConn(ctx)
	if err != nil {
		return err
	}

	rstr := strconv.Itoa(rot)

	if err = tconn.EvalPromise(ctx,
		`new Promise((resolve, reject) => {
			chrome.system.display.setDisplayProperties("`+id+`", {"rotation":`+rstr+`}, () => {
			  if (chrome.runtime.lastError === undefined) {
				resolve();
			  } else {
				reject(chrome.runtime.lastError.message);
