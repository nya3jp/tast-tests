...
func evaluateLatency(ctx context.Context, params gio.TestParams, eventTimes []int64, numLines int, perfName string, pv *perf.Values, match string) error {
	// Get event received RTC times.
	events, err := gio.PopulateReceivedTimes(ctx, params, numLines, match)
	if err != nil {
