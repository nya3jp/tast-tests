...
const (
	...
	numMoveEvents = 100
	waitMS        = 50
	moveDuration  = time.Millisecond * 1000
)
...
func InputOverlayPerf(ctx context.Context, s *testing.State) {
	gio.SetupTestApp(ctx, s, func(params gio.TestParams) error {
		...
		moveEventTimes := make([]int64, 0, numMoveEvents)
				...
				func(ctx context.Context) error {
					if i > 0 {
						...
						if err := testing.Sleep(ctx, moveDuration); err != nil {
							return errors.Wrap(err, "failed to sleep after release")
						}
					}
					return nil
				},
