...
func QuotaProjectID(ctx context.Context, s *testing.State) {
	...
	if projectID < projectIDExtDataStart || projectIDExtDataEnd < projectID {
		s.Errorf("Unexpected project ID: expected to be in range [%d %d], actual %d", projectIDExtDataStart, projectIDExtDataEnd, projectID)
	}
	...
	if projectID != projectIDExtMediaImage {
		s.Errorf("Unexpected project ID: expected %d, actual %d ", projectIDExtMediaImage, projectID)
	}
}
