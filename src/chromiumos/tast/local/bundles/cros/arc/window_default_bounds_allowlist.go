...
func wmAllowlistResizableUnspecified(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device) error {
	...
	if err != nil {
		return errors.Wrap(err, "failed to create the non-allowlisted activity")
	}

	if err := act.Start(ctx, tconn); err != nil {
		return errors.Wrap(err, "failed to start the non-allowlisted activity")
	}

	if err := wm.WaitUntilActivityIsReady(ctx, tconn, act, d); err != nil {
		return errors.Wrap(err, "failed to wait for the non-allowlisted activity to be ready")
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to get the default window state of the non-allowlisted activity")
	}
	if windowState != arc.WindowStateNormal {
		...
		if err := ash.WaitForARCAppWindowState(ctx, tconn, wm.Pkg24, ash.WindowStateMaximized); err != nil {
			return errors.Wrap(err, "failed to wait for the non-allowlisted activity to be restored")
		}
		if err := ash.WaitWindowFinishAnimating(ctx, tconn, winInfoBeforeRestore.ID); err != nil {
			return errors.Wrap(err, "failed to wait for the animation of the non-allowlisted activity to be finished")
		}
	}

	window, err := ash.GetARCAppWindowInfo(ctx, tconn, wm.Pkg24)
	if err != nil {
		return errors.Wrap(err, "failed to get the window info of the non-allowlisted activity")
	}
	launchBoundsThreshold = window.BoundsInRoot

	// This non-allowlisted app isn't necessary anymore.
	if err := act.Stop(); err != nil {
		return errors.Wrap(err, "failed to stop the non-allowlisted activity")
	}

	if err := act.Close(); err != nil {
		return errors.Wrap(err, "failed to close the non-allowlisted activity")
	}
