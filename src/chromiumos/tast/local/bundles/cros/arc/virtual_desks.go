...
func VirtualDesks(ctx context.Context, s *testing.State) {
	...
	if err := ki.Accel(ctx, "Ctrl+Search+="); err != nil {
	...
	if result, err := deskContainsWindow(ctx, tconn, "Desk_Container_B", "ExoShellSurface"); err != nil {
		s.Fatal("deskContainsWindow Failed: ", err)
	} else if !result {
		s.Error("Failed to find ExoShellSurface under Desk_Container_B")
	}

	if result, err := deskContainsWindow(ctx, tconn, "Desk_Container_A", "BrowserFrame"); err != nil {
		s.Fatal("deskContainsWindow Failed: ", err)
	} else if !result {
		s.Error("Failed to find BrowserFrame under Desk_Container_A")
	}

	if result, err := deskContainsWindow(ctx, tconn, "Desk_Container_A", "ExoShellSurface"); err != nil {
		s.Fatal("deskContainsWindow Failed: ", err)
	} else if result {
		s.Error("ExoShellSurface should not be under Desk_Container_A")
	}
}
