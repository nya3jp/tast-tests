...
func deskContainsWindow(ctx context.Context, tconn *chrome.Conn, s *testing.State, deskContainerName string, windowName string) bool {
	var found bool
	...
	if err != nil {
		s.Fatal("deskContainsWindow() failed: ", err)
	}
	return found
}
...
func VirtualDesks(ctx context.Context, s *testing.State) {
	...
	expectTrue := func(condition bool, msg string) {
		if !condition {
			s.Error(msg)
		}
	}

	expectFalse := func(condition bool, msg string) {
		if condition {
			s.Error(msg)
		}
	}
	...
	expectTrue(deskContainsWindow(ctx, tconn, s, "Desk_Container_B", "ExoShellSurface"), "Failed to find ExoShellSurface under Desk_Container_B")
	expectTrue(deskContainsWindow(ctx, tconn, s, "Desk_Container_A", "BrowserFrame"), "Failed to find BrowserFrame under Desk_Container_A")
	expectFalse(deskContainsWindow(ctx, tconn, s, "Desk_Container_A", "ExoShellSurface"), "ExoShellSurface should not be under Desk_Container_A")
}
