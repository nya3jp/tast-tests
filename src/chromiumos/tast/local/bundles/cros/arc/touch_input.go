...
func TouchInput(ctx context.Context, s *testing.State) {
	...
	runner, err := motioninput.NewWmTestRunner(ctx, tconn, a, d)
	if err != nil {
		s.Fatal("Failed to create motioninput.WmTestRunner")
	}
	...
	for _, params := range []motioninput.WmTestParams{
		...
		runner.RunTest(ctx, s, &params, "Verify Touch", verifyTouchscreen)
	}
}
...
// touch pointer.
func verifyTouchscreen(ctx context.Context, s *testing.State, t *motioninput.WmTestState, tester *motioninput.Tester) {
