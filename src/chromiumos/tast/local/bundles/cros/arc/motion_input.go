...
func singleTouchMatcher(a motioninput.Action, p coords.Point) motioninput.Matcher {
	return motioninput.SinglePointerMatcher(a, motioninput.Touchscreen, p, 1)
}
...
func mouseMatcher(a motioninput.Action, p coords.Point) motioninput.Matcher {
	pressure := 0.
	if a == motioninput.Move || a == motioninput.Down || a == motioninput.ButtonPress || a == motioninput.HoverExit {
		pressure = 1.
	}
	return motioninput.SinglePointerMatcher(a, motioninput.Mouse, p, pressure)
}
...
func MotionInput(ctx context.Context, s *testing.State) {
	...
	runSubtest := func(ctx context.Context, s *testing.State, params *motionInputSubtestParams, subtest motionInputSubtestFunc) {
		...
		if err := ash.WaitForVisible(ctx, tconn, motioninput.PKG); err != nil {
		...
		if params.WmEventToSend != nil {
			...
			if _, err := ash.SetARCAppWindowState(ctx, tconn, motioninput.PKG, event); err != nil {
		...
		if params.WmStateToVerify != nil {
			...
			if err := ash.WaitForARCAppWindowState(ctx, tconn, motioninput.PKG, windowState); err != nil {
		...
		test.w, err = ash.GetARCAppWindowInfo(ctx, tconn, motioninput.PKG)
		if err != nil {
...
func verifyTouchscreen(ctx context.Context, s *testing.State, test motionInputSubtestState) {
	...
	if err := test.tester.ExpectEventsAndClear(ctx, singleTouchMatcher(motioninput.Down, expected)); err != nil {
	...
	for i := 0; i < numMotionEventIterations; i++ {
		...
		if err := test.tester.ExpectEventsAndClear(ctx, singleTouchMatcher(motioninput.Move, expected)); err != nil {
			s.Fatal("Failed to expect events and clear: ", err)
		}
	}
	...
	if err := test.tester.ExpectEventsAndClear(ctx, singleTouchMatcher(motioninput.Up, expected)); err != nil {
		s.Fatal("Failed to expect events and clear: ", err)
	}
}
...
func verifyMouse(ctx context.Context, s *testing.State, t motionInputSubtestState) {
	...
	for i := 0; i < numMotionEventIterations; i++ {
		...
		if err := t.tester.ExpectEventsAndClear(ctx, mouseMatcher(motioninput.HoverMove, e)); err != nil {
			s.Fatal("Failed to expect events and clear: ", err)
		}
	}
	...
	if err := t.tester.ExpectEventsAndClear(ctx, mouseMatcher(motioninput.HoverExit, e), mouseMatcher(motioninput.Down, e), mouseMatcher(motioninput.ButtonPress, e)); err != nil {
		s.Fatal("Failed to expect events and clear: ", err)
	}
	...
	for i := 0; i < numMotionEventIterations; i++ {
		...
		if err := t.tester.ExpectEventsAndClear(ctx, mouseMatcher(motioninput.Move, e)); err != nil {
			s.Fatal("Failed to expect events and clear: ", err)
		}
	}
	...
	if err := t.tester.ExpectEventsAndClear(ctx, mouseMatcher(motioninput.ButtonRelease, e), mouseMatcher(motioninput.Up, e)); err != nil {
		s.Fatal("Failed to expect events and clear: ", err)
	}
	...
	if err := t.tester.ExpectEventsAndClear(ctx, mouseMatcher(motioninput.HoverEnter, e), mouseMatcher(motioninput.HoverMove, e)); err != nil {
		s.Fatal("Failed to expect events and clear: ", err)
	}
}
