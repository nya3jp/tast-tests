...
func verifyMouse(ctx context.Context, s *testing.State, t motionInputSubtestState) {
	...
	// TODO: Investigate why injecting only one initial move event (by setting the duration to 0)
	//  produces ACTION_HOVER_ENTER, ACTION_HOVER_MOVE, and ACTION_HOVER_EXIT, instead of the
	//  expected single event with action ACTION_HOVER_ENTER.
	if err := ash.MouseMove(ctx, t.tconn, p, 500*time.Millisecond); err != nil {
		s.Fatalf("Failed to inject move at %v: %v", e, err)
	}
	// TODO: Investigate why there are sometimes two ACTION_HOVER_ENTER events being sent. Once
	//  resolved, add expectation for ACTION_HOVER_ENTER and remove sleep.
	if err := testing.Sleep(ctx, 1*time.Second); err != nil {
		s.Log("Failed to sleep; ignoring: ", err)
	}
	...
	if err := ash.MousePress(ctx, t.tconn, ash.LeftButton); err != nil {
		s.Fatal("Failed to press button on mouse")
	}
	...
	if err := ash.MouseRelease(ctx, t.tconn, ash.LeftButton); err != nil {
		s.Fatal("Failed to release button on mouse")
	}
