...
// motionEventMatcher represents a matcher for motionEvent, which returns nil if the provided
// motionEvent matches, and an error otherwise.
...
func MotionInput(ctx context.Context, s *testing.State) {
	...
	runSubtest := func(ctx context.Context, s *testing.State, params *motionInputSubtestParams) {
		...
		// It can take some time for Android to handle the window state change, so wait for some
		// time to ensure the app is ready to process input.
	...
	for _, params := range []motionInputSubtestParams{
		...
		// TODO: Investigate why the first motion event received by the test app has incorrect
		//  coordinates in tablet mode, and add test params for tablet mode when resolved.
	} {
