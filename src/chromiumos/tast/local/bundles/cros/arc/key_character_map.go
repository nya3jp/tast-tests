...
func KeyCharacterMap(ctx context.Context, s *testing.State) {
	...
	checkMapping := func(ctx context.Context, input, output string) {
		...
		if err := d.Object(ui.ID(fieldID), ui.Text(output)).WaitForExists(ctx, 10*time.Second); err != nil {
			s.Errorf("Failed to find field %q after typing %q: %v", input, output, err)
		}
	}
