...
func PlayStoreOmnibox(ctx context.Context, s *testing.State) {
	...
	for _, tc := range []struct {
		...
		{"com.jshikaram.peanuttypes", "https://jeevan-shikaram.github.io"}, // TWA type
		{"com.twitter.android", "https://mobile.twitter.com"},              // WebAPK type
	} {
		s.Logf("Installing %s from %s", tc.pkgName, tc.url)
		...
		install, err := chromeui.FindWithTimeout(ctx, tconn, params, 30*time.Second)
		if err != nil {
		...
		if err := playstore.InstallApp(ctx, a, d, tc.pkgName); err != nil {
			s.Fatal("Failed to install app: ", err)
		}

		s.Logf("Successfully installed %s from %s", tc.pkgName, tc.url)
	}
}
