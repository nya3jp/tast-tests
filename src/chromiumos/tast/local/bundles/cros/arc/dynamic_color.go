...
func DynamicColor(ctx context.Context, s *testing.State) {
	...
	var ret bool
	if err := tconn.Call(ctx, &ret, "tast.promisify(chrome.autotestPrivate.sendOverlayColor)", 50); err != nil {
	...
	themeOverlay := result
	s.Log(themeOverlay["android.theme.customization.system_palette"].(float64), " ~~~ the palette!")
	s.Log(themeOverlay["android.theme.customization.theme_style"].(string), " ~~~ the style!")
