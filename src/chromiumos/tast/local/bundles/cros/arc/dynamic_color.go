...
func DynamicColor(ctx context.Context, s *testing.State) {
	...
	if err := tconn.Call(ctx, &ret, "tast.promisify(chrome.autotestPrivate.sendArcOverlayColor)", 50, testStyle); err != nil {
	...
	if palette := themeOverlay["android.theme.customization.system_palette"].(float64); palette != testPalette {
		s.Errorf("Invalid system_palette got: %d, want: %d", palette, testPalette)
	}
