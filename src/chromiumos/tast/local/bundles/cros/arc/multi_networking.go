...
func MultiNetworking(ctx context.Context, s *testing.State) {
	const (
		...
		pollTimeoutInitial  = 8 * time.Second // The time to wait for arc-networkd to set up virtual network after physical network changes.
		pollTimeoutInterval = 1 * time.Second // The time to wait for remaining configurations after detecting virtual network
	)
	...
	verifyDeviceAdded := func() {
		...
		testing.Poll(ctx, func(ctx context.Context) error {
			...
			return nil
		}, &testing.PollOptions{Timeout: pollTimeoutInitial})
		...
		testing.Poll(ctx, func(ctx context.Context) error {
			...
			return nil
		}, &testing.PollOptions{Timeout: pollTimeoutInterval})
