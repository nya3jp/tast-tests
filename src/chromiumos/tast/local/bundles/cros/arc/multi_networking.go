...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Verifies guess network setup upon physical interface change",
...
func MultiNetworking(ctx context.Context, s *testing.State) {
	...
	if err := testexec.CommandContext(ctx, "/bin/ip", "netns", "add", testNetnsName).
		...
		// Ignore failure here for potential netns already exists case. If it's a legitimate failure it will fail at net step.
	...
	// Verify IPv6 forwarding rule set up correctly
	if err := testexec.CommandContext(ctx, "/sbin/ip6tables", "-C", "FORWARD", "-i", ifname, "-o", brIfname, "-j", "ACCEPT").
		Run(testexec.DumpLogOnError); err != nil {
		s.Errorf("Cannot verify -A FORWARD -i %s -o %s -j ACCEPT rule: %s", ifname, brIfname, err)
	}
	if err := testexec.CommandContext(ctx, "/sbin/ip6tables", "-C", "FORWARD", "-i", brIfname, "-o", ifname, "-j", "ACCEPT").
		Run(testexec.DumpLogOnError); err != nil {
		s.Errorf("Cannot verify -A FORWARD -i %s -o %s -j ACCEPT rule: %s", brIfname, ifname, err)
	}
	...
	// Restart ARC
	doARC()
	// Wait for arc-networkd initialization
	...
	if err := arc.BootstrapCommand(ctx, "/system/bin/ip", "link", "show", ifname).Run(); err != nil {
		s.Errorf("Fail verifying interface %s in ARC (%s)", ifname, err)
		stopTest = true
	}
	...
	// Verify IPv6 forwarding rule removed correctly
	if err := testexec.CommandContext(ctx, "/sbin/ip6tables", "-C", "FORWARD", "-i", ifname, "-o", brIfname, "-j", "ACCEPT").
		Run(testexec.DumpLogOnError); err == nil {
		s.Errorf("ip6tables -A FORWARD -i %s -o %s -j ACCEPT rule is not removed", ifname, brIfname)
	} else if _, ok := err.(*exec.ExitError); !ok {
		s.Error("ip6tables -C returned unexpect error: ", err)
	}
	if err := testexec.CommandContext(ctx, "/sbin/ip6tables", "-C", "FORWARD", "-i", brIfname, "-o", ifname, "-j", "ACCEPT").
		Run(testexec.DumpLogOnError); err == nil {
