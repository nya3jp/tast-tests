...
func init() {
	testing.AddTest(&testing.Test{
		Func:         NullEditText,
		Desc:         "Checks TYPE_NULL EditText works on Android",
...
func NullEditText(ctx context.Context, s *testing.State) {
	...
	const (
		...
		cls = "org.chromium.arc.testapp.keyboard.NullEditTextActivity"
	)
	...
	if err := d.Object(ui.ID(fieldID), ui.Text("abc")).WaitForExists(ctx, 2*time.Minute); err != nil {
