...
type dragDropTestParams struct {
	stable bool
}
...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			Val:               &dragDropTestParams{stable: true},
		}, {
			Name:              "vm",
			...
			Name:              "unstable",
			ExtraSoftwareDeps: []string{"android_p"},
			Val:               &dragDropTestParams{stable: false},
		}, {
			Name:              "unstable_vm",
			ExtraAttr:         []string{"informational"},
			ExtraSoftwareDeps: []string{"android_vm"},
...
func DragDrop(ctx context.Context, s *testing.State) {
	...
	if s.Param().(*dragDropTestParams).stable {
	...
	s.Run(ctx, "AndroidToAndroid", func(ctx context.Context, s *testing.State) {
		...
		sourceBounds := coords.Rect{Left: 0, Top: 0, Width: 500, Height: 500}
		...
		targetBounds := coords.Rect{Left: 500, Top: 0, Width: 500, Height: 500}
