...
func maximizeVisibility(ctx context.Context, cr *chrome.Chrome, a *arc.ARC) error {
	...
	for _, test := range []struct {
		...
		if err := func() error {
			...
			if err := ensureNoBlackBkg(ctx, tconn, cr); err != nil {
				return err
			}
...
// ensureNoBlackBkg checks if there is black background.
func ensureNoBlackBkg(ctx context.Context, tconn *chrome.Conn, cr *chrome.Chrome) error {
	infos, err := display.GetInfo(ctx, tconn)
	if err != nil {
		return err
	}
