...
func MultiDisplay(ctx context.Context, s *testing.State) {
	...
	for idx, test := range []struct {
		...
		{"Ensure activity is visible on one display if another activity is maximized or fullscreen on another activity", maximizeVisibility},
	} {
...
// maximizeVisibility checkes if the window is visible on one display if another window is maximized on the other display.
func maximizeVisibility(ctx context.Context, cr *chrome.Chrome, a *arc.ARC) error {
	...
	for _, test := range []struct {
		...
		{"Maximize the activity on primary display and check the visibility of activity on external display", settingsAct, settingsPkgMD, wmAct, wmPkgMD, wmWinInfo},
		{"Maximize the activity on external display and check the visibility of activity on internal display", wmAct, wmPkgMD, settingsAct, settingsPkgMD, settingsWinInfo},
	} {
		if err := func() error {
			testing.ContextLogf(ctx, "Running subset %q", test.name)
			...
			if err := ensureNoBlackBgd(ctx, tconn, cr); err != nil {
				return err
			}
			return nil
		}(); err != nil {
		...
		// Reset maximized activity to normal
		if err := ensureSetWindowState(ctx, tconn, test.maxPkgName, test.maxAct, ash.WindowStateNormal); err != nil {
			return err
		}
	}
	return nil
...
// ensureSetWindowState checks if the window is in requested window state. If not, make sure to set window state into the requested window state.
func ensureSetWindowState(ctx context.Context, c *chrome.Conn, pkgName string, act *arc.Activity, expectedState ash.WindowStateType) error {
	...
	state, err := ash.SetARCAppWindowState(ctx, c, pkgName, windowEventMap[expectedState])
	if err != nil {
		return err
	}
	if err := act.WaitForIdle(ctx, 10*time.Second); err != nil {
...
// ensureWindowStable checkes if the window moves its position.
func ensureWindowStable(ctx context.Context, tconn *chrome.Conn, act *arc.Activity, pkgName string, expectedWindowInfo ash.ArcAppWindowInfo) error {
	...
	bounds := parseBoundsToString(windowInfo.Bounds)
	expectedBounds := parseBoundsToString(expectedWindowInfo.Bounds)
	if bounds != expectedBounds || windowInfo.DisplayID != expectedWindowInfo.DisplayID {
		return errors.Errorf("Window moves: got bounds %q (displayID %q); expected bounds %q (displayID %q)", bounds, windowInfo.DisplayID, expectedBounds, expectedWindowInfo.DisplayID)
	}
	return nil
}

// parseBoundsToString shows ash.Rect value as a string.
func parseBoundsToString(bounds ash.Rect) string {
	return fmt.Sprintf("(%d, %d - %d x %d)", bounds.Left, bounds.Top, bounds.Width, bounds.Height)
}

// ensureNoBlackBgd checks if there is black background.
func ensureNoBlackBgd(ctx context.Context, tconn *chrome.Conn, cr *chrome.Chrome) error {
	infos, err := display.GetInfo(ctx, tconn)
	if err != nil {
		return err
	}

	img, err := arcscreenshot.GrabScreenshot(ctx, cr)
	if err != nil {
		return err
	}
	arcscreenshot.CountPixels(img, color.RGBA{0, 0, 0, 255})

	for _, info := range infos {
		img, err := arcscreenshot.GrabScreenshotForDisplay(ctx, cr, info.ID)
		if err != nil {
			return err
		}
