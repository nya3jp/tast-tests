...
func removeAddDisplay(ctx context.Context, cr *chrome.Chrome, a *arc.ARC) error {
	...
	for _, removeAdd := range []struct {
		...
		if err := func() error {
			...
			// Wait for one display off.
			if err := waitForDisplayOff(ctx, tconn, 10*time.Second); err != nil {
				return err
			}
...
// waitForDisplayOff waits until one display is off.
func waitForDisplayOff(ctx context.Context, tconn *chrome.Conn, timeout time.Duration) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		countInfo, err := display.GetInfo(ctx, tconn)
		if err != nil {
			return err
		}
		if len(countInfo) != 1 {
			return errors.New("one display is still on")
		}
		return nil
	}, &testing.PollOptions{Timeout: timeout})
}
