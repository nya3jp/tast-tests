...
func PowerCameraPreviewPerf(ctx context.Context, s *testing.State) {
	...
	for _, targetFps := range []string{"30", "60"} {
		...
		numFramesMetric := perf.Metric{Name: "total_num_frames_" + targetFps + "fps", Unit: "frames", Direction: perf.BiggerIsBetter, Multiple: true}
		numDroppedFramesMetric := perf.Metric{Name: "num_dropped_frames_" + targetFps + "fps", Unit: "frames", Direction: perf.SmallerIsBetter, Multiple: true}
		frameDropRatioMetric := perf.Metric{Name: "frame_drop_ratio_" + targetFps + "fps", Unit: "ratio", Direction: perf.SmallerIsBetter, Multiple: true}
