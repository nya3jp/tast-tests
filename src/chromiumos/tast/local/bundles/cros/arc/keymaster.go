...
func Keymaster(ctx context.Context, s *testing.State) {
	...
	if statusText != "Status: OK" {
		s.Fatal("Run tests status: failed: ", statusText)
	}

	// Verify.
	err = verifyKeyGenerated(ctx, a, arcPrivateKeyPath, crosStorePath)
	if err != nil {
...
func verifyKeyGenerated(ctx context.Context, a *arc.ARC, arcPrivateKeyPath string, crosStorePath string) error {

	keyFilename, err := arcRetrieveFilenameFromReference(ctx, a, arcPrivateKeyPath)
	if err != nil {
...
func arcRetrieveFilenameFromReference(ctx context.Context, a *arc.ARC, referenceFilepath string) (string, error) {
	dataBytes, err := a.ReadFile(ctx, referenceFilepath)
	// dataBytes, err := a.Command(ctx, "/system/bin/cat", referenceFilepath).Output(testexec.DumpLogOnError)
	if err != nil {
		return "", errors.Wrap(err, "failed to get reference contents")
	}

	data := strings.TrimSpace(string(dataBytes))
	dataLengthActual := len(data)
	dataLengthExpected := keystoreHeaderLength + filenameLength
	if dataLengthActual != dataLengthExpected {
		return "", errors.Errorf("invalid length of reference content: actual %d, expected %d", dataLengthActual, dataLengthExpected)
	}
