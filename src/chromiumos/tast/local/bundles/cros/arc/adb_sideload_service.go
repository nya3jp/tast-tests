...
func (*AdbSideloadService) SetRequestAdbSideloadFlag(ctx context.Context, req *empty.Empty) (*empty.Empty, error) {
	...
	// Waiting for the Enable ADB Sideloading flag to be written to /home/chronos/Local State json
	// ImportantFileWriter writes Local State json to DUT's disk every 10 s
	testing.ContextLog(ctx, "Waiting for Enable ADB Sideloading flag to be written on DUT's Local State json")
	testing.Poll(ctx, func(ctx context.Context) error {
		err := testexec.CommandContext(ctx, "grep", "-c", "EnableAdbSideloadingRequested", "/home/chronos/Local State").Run()
		if err != nil {
			return err
		}
		return nil
...
func (*AdbSideloadService) ConfirmEnablingAdbSideloading(ctx context.Context, request *arcpb.AdbSideloadServiceRequest) (*empty.Empty, error) {
	...
	if request.Action == "cancel" {
		code = fmt.Sprintf("document.activeElement.shadowRoot.getElementById('enable-adb-sideloading-cancel-button').shadowRoot.getElementById('textButton').click()")
	}
