...
func (*AdbSideloadService) ConfirmEnablingAdbSideloading(ctx context.Context, request *arcpb.AdbSideloadServiceRequest) (*empty.Empty, error) {
	...
	prefix := "document.activeElement.shadowRoot.getElementById('"
	suffix := "').shadowRoot.getElementById('textButton').click()"

	code := ""
	if request.Action == "confirm" {
		code = prefix + "enable-adb-sideloading-ok-button" + suffix
	} else if request.Action == "cancel" {
		code = prefix + "enable-adb-sideloading-cancel-button" + suffix
	} else {
