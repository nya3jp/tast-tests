...
func (c *UreadaheadPackService) Generate(ctx context.Context, request *arcpb.UreadaheadPackRequest) (*arcpb.UreadaheadPackResponse, error) {
	...
	packPath, vmPackPath, err := generatePack(ctx, request, vmEnabled)
	if err != nil {
		return nil, errors.Wrap(err, "failed to generate ureadahead pack")
	}
...
// generatePack generates ureadahead initial pack for requested Chrome login mode for host and guest OS.
func generatePack(ctx context.Context, request *arcpb.UreadaheadPackRequest, vmEnabled bool) (string, string, error) {
	const (
	...
	if vmEnabled {
		...
		if err != nil {
			return "", "", errors.Wrap(err, "failed to obtain ureadahead pack from ARCVM guest OS")
		}
	}

	return packPath, vmPackPath, nil
}
