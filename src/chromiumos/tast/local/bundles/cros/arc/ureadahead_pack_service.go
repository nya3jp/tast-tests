...
func (c *UreadaheadPackService) Generate(ctx context.Context, request *arcpb.UreadaheadPackRequest) (*arcpb.UreadaheadPackResponse, error) {
	...
	if request.VmEnabled {
		if !request.InitialBoot {
			return nil, errors.New("failed to verify VM is performing initial boot")
		}
		...
		if err != nil {
			return nil, errors.Wrap(err, "failed to connect ARCVM")
		}
		defer a.Close()

		// Make sure ureadahead exited and stopped.
		if value, err := a.GetProp(ctx, "dev.arc.ureadahead.exit"); err != nil || value != "1" {
			return nil, errors.Wrap(err, "failed to verify ureadahead to exited")
		}
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			...
			return nil, errors.Wrap(err, "failed to wait for ureadahead to stop")
		}

		if err := a.PullFile(ctx, filepath.Join(ureadaheadDataDir, "pack"), packPath); err != nil {
			return nil, errors.Wrapf(err, "failed to pull %s from ARCVM: ", ureadaheadDataDir)
		}
	} else {
