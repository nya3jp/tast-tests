...
func (c *UreadaheadPackService) Generate(ctx context.Context, request *arcpb.UreadaheadPackRequest) (*arcpb.UreadaheadPackResponse, error) {
	...
	out, err := testexec.CommandContext(ctx, "lsof", "+D", root).CombinedOutput()
	if err != nil {
		// In case nobody holds file, lsof returns 1
		if exitError, ok := err.(*exec.ExitError); !ok || exitError.ExitCode() != 1 {
			return nil, errors.Wrap(err, "failed to verify root is not locked")
		}
	}
	outStr := string(out)
	if outStr != "" {
		return nil, errors.Errorf("found locks for %q: %q", root, outStr)
	}
	...
	sysOpenFilterContent := fmt.Sprintf("filename ~ \"%s/*\"", root)
	...
	// Reset filter on exit.
	defer func() {
		if err := ioutil.WriteFile(sysOpenFilterPath, []byte("0"), 0644); err != nil {
			testing.ContextLog(ctx, "Failed to reset sys open filter")
		}
	}()
