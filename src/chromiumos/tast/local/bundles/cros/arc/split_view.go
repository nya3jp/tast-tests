...
func SplitView(ctx context.Context, s *testing.State) {
	...
	if err := a.Install(ctx, s.DataPath("ArcKeyboardTest.apk")); err != nil {
	...
	if err := a.Command(ctx, "am", "start", "-W", "org.chromium.arc.testapp.keyboard/org.chromium.arc.testapp.keyboard.MainActivity").Run(); err != nil {
		s.Fatal("Failed starting app: ", err)
	}
	...
	if err := ash.SnapARCAppInSplitView(ctx, tconn, "org.chromium.arc.testapp.keyboard", ash.SnapPositionRight); err != nil {
		s.Fatal("Failed to snap app in split view: ", err)
	}
	...
	if state, err := ash.GetARCAppWindowState(ctx, tconn, "org.chromium.arc.testapp.keyboard"); err != nil {
		s.Fatal("Failed to get ash window state: ", err)
