...
func waitUntilStateChange(ctx context.Context, c *chrome.Conn, left *arc.Activity, right *arc.Activity) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		for _, test := range []struct {
			...
			if actual, err := ash.GetARCAppWindowState(ctx, c, test.act.PackageName()); err != nil {
				...
				return errors.Errorf("ash window state should be %v but got %v", test.ashState, actual)
			}
			...
			if actual, err := test.act.GetWindowState(ctx); err != nil {
				...
				return errors.Errorf("ARC window state should be %v but got %v", test.arcState, actual)
			}
		}
		return nil
	}, nil)
...
func showActivity(ctx context.Context, a *arc.ARC, pkgName, activityName string) (*arc.Activity, error) {
...
func SplitView(ctx context.Context, s *testing.State) {
	...
	// Show two activities. As the content of the activities doesn't matter, use two activities available by default.
	rightAct, err := showActivity(ctx, a, "com.android.storagemanager", ".deletionhelper.DeletionHelperActivity")
	if err != nil {
		s.Fatal("Failed to show an activity: ", err)
	}
	defer rightAct.Close()
	leftAct, err := showActivity(ctx, a, "com.android.settings", ".Settings")
	if err != nil {
		s.Fatal("Failed to show an activity: ", err)
	}
	defer leftAct.Close()
	...
	if err := ash.SnapARCAppInSplitView(ctx, tconn, leftAct.PackageName(), ash.SnapPositionLeft); err != nil {
		s.Fatal("Failed to snap app in split view: ", err)
	}
	if err := ash.SnapARCAppInSplitView(ctx, tconn, rightAct.PackageName(), ash.SnapPositionRight); err != nil {
		s.Fatal("Failed to snap app in split view: ", err)
	}
