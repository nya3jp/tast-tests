...
func SplitView(ctx context.Context, s *testing.State) {
	...
	// We put this "defer" statement after setting the defer statements for cleaning up the apps so
	// that we can capture the state *before* closing the apps when it fails.
