...
func RegularBoot(ctx context.Context, s *testing.State) {
	creds, err := performArcInitialBoot(ctx, s)
	if err != nil {
	...
	for i := 0; i < iterationCount; i++ {
		appLaunchDuration, enabledScreenDuration, err := performArcRegularBoot(ctx, s, creds)
		if err != nil {
...
func performArcInitialBoot(ctx context.Context, s *testing.State) (chrome.Creds, error) {
...
// actually shown. It returns time between the user session is created and and Play Store window is
// shown. Note, it is not actually possible to measure this time directly from test due to tast
...
// represents here the overhead from tast Chrome login implementation. In addition it returns
// duration to fully start Android system server.
// This also resets system caches befor login to simulate scenario when user uses Chromebook after
// reboot.
func performArcRegularBoot(ctx context.Context, s *testing.State, creds chrome.Creds) (time.Duration, time.Duration, error) {
...
func readFirstAppLaunchHistogram(ctx context.Context, tconn *chrome.TestConn, name string) (time.Duration, error) {
	...
	return time.Duration(timeMs*1000000.0) * time.Nanosecond, nil
}
