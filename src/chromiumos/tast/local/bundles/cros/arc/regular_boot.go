...
func init() {
	testing.AddTest(&testing.Test{
		Func: RegularBoot,
		Desc: "This exercises a scenario when use logs in Chrome where ARC is already provisioned and tries to use ARC app immediately. App launch delay is reported for this case",
		Contacts: []string{
...
func RegularBoot(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < iterationCount; i++ {
		time, err := performArcBoot(ctx, gaia, false /* initial */)
		if err != nil {
...
func performArcBoot(ctx context.Context, gaia chrome.Option, initial bool) (float64, error) {
	...
	const histogramTimeout = 20 * time.Second

	delay, err := metrics.WaitForHistogram(ctx, tconn, "Arc.FirstAppLaunchDelay.TimeDelta", histogramTimeout)
	if err != nil {
		return 0, errors.Wrap(err, "failed to get FirstAppLaunchDelay histogram")
	}

	request, err := metrics.WaitForHistogram(ctx, tconn, "Arc.FirstAppLaunchRequest.TimeDelta", histogramTimeout)
	if err != nil {
		return 0, errors.Wrap(err, "failed to get FirstAppLaunchRequest histogram")
	}

	requestTime, err := request.Mean()
	if err != nil {
		return 0, errors.Wrap(err, "failed to read FirstAppLaunchRequest histogram")
	}

	delayTime, err := delay.Mean()
	if err != nil {
		return 0, errors.Wrap(err, "failed to read FirstAppLaunchDelay histogram")
	}

	return (requestTime + delayTime) * 0.001, nil
}
