...
func TaskManager(ctx context.Context, s *testing.State) {
	...
	// This sleep is required for the ARC app to appear in first page to verify.
	// TODO(b/184323848): Remove this when there is reliable way to wait for ARC entry.
	...
	ui := uiauto.New(tconn)
	if err := uiauto.Combine("open Task Manager and look for ARC",
		// Press Search + Esc to launch task manager.
		kb.AccelAction("Search+Esc"),

		// Check the ARC app in task manager.
		ui.WaitUntilExists(arcEntry),

		// Exit task manager.
		kb.AccelAction("Ctrl+W"))(ctx); err != nil {
		s.Fatal("Failed to close Task Manager: ", err)
	}
