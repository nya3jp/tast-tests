...
func PowerAudioPlaybackPerf(ctx context.Context, s *testing.State) {
	const (
		...
		intentSetPerformanceMode = "org.chromium.arc.testapp.arcaudiotest.ACTION_SET_PERFORMANCE_MODE"
		intentStartPlayback      = "org.chromium.arc.testapp.arcaudiotest.ACTION_START_PLAYBACK"
		keyPerformanceMode       = "perf_mode"
		keyDuration              = "duration"
		playbackDurationSecond   = 90 // audioWarmupDuration + measureDuration  + 10 seconds
	)

	param := s.Param().(audio.TestParameters)
	s.Logf("Measures power consumption of audio playback with flag: %#x", param.PerformanceMode)
	intentExtras := []string{
		"--ei", keyPerformanceMode, strconv.FormatUint(uint64(param.PerformanceMode), 10),
		"--ei", keyDuration, strconv.FormatUint(playbackDurationSecond, 10),
	}
