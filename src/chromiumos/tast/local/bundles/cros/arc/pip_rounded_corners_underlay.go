...
func PIPRoundedCornersUnderlay(ctx context.Context, s *testing.State) {
	...
	logcat, err := os.Create(path.Join(s.OutDir(), "logcat.txt"))
	if err != nil {
		s.Fatal("Failed to create logcat.txt: ", err)
	}
	defer logcat.Close()

	if err := a.WaitForLogcat(ctx, func(ln string) bool {
		if _, err := logcat.WriteString(ln + "\n"); err != nil {
			s.Fatal("Failed to write line to logcat.txt: ", err)
			return true
		}
		return false
	}); err != nil {
		s.Fatal("WaitForLogcat reported error: ", err)
	}
