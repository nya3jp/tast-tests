...
func SurfaceInsets(ctx context.Context, s *testing.State) {
	...
	for _, op := range []string{"maximize", "restore", "maximize", "restore"} {
		...
		// Touch lower edge of the restore/maximize caption button to validate that any surface does not cover the caption button.
		...
		if initialState == newState {
			s.Fatalf("Pressing %q did not work. Tried to touch (%f, %f), state = %q", op, buttonCoordX, buttonCoordY, newState.String())
		}
	}
}
