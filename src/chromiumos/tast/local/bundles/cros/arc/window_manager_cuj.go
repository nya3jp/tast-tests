...
func WindowManagerCUJ(ctx context.Context, s *testing.State) {
	...
	type testFunc func(context.Context, *chrome.Chrome, *arc.ARC, *ui.Device) error
	for idx, test := range []struct {
		...
		if err := test.fn(ctx, cr, a, d); err != nil {
		...
		for _, orientation := range []struct {
			...
			if err := func() error {
				...
				if s, err := act.GetWindowState(ctx); err != nil {
					...
					// go/arc-wm-p only covers when the initial activity is Restored. Probably an oversight.
...
func wmPIP(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, d *ui.Device) error {
	...
	if ws, err := actPIP.GetWindowState(ctx); err != nil {
		return err
	} else if ws == notWanted {
		return errors.Errorf("invalid window state: got %q; want not %q", notWanted.String(), notWanted.String())
	}
	...
	if ws, err := actPIP.GetWindowState(ctx); err != nil {
		return err
	} else if ws != arc.WindowStatePIP {
		return errors.Errorf("invalid window state: got %q; want %q", ws.String(), arc.WindowStatePIP.String())
	}
...
func wmFreeformResize(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, d *ui.Device) error {
	...
	const pixelsToResize = 200
	to := arc.NewPoint(origBounds.Left+origBounds.Width+pixelsToResize,
		origBounds.Top+origBounds.Height/2)
...
func wmSnapping(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, d *ui.Device) error {
	...
	testing.ContextLogf(ctx, "Snapped bounds: %+v", snappedBounds)
	// 1-pixel margin error in case of using a odd screen width.
	if math.Abs(float64(snappedBounds.Width-halfWidth)) > 1 {
...
// go/arc-wm-p "Clamshell: display resolution change" (slide #28-#29)
func wmDisplayResolution(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, d *ui.Device) error {
	...
	testing.ContextLogf(ctx, "Bounds before changing zoom factor: %+v", oldBounds)

	testing.ContextLogf(ctx, "Display info: %+v", disp)
	for _, m := range disp.Modes {
		testing.ContextLogf(ctx, "Mode: %+v", *m)
	}
	...
	// New bounds should be: old bounds / newZoom. There shouldn't be any rounding errors.
	newBounds, err := act.WindowBounds(ctx)
	if err != nil {
		return err
	}
	expectedW := int(math.Round(float64(oldBounds.Width) / newZoom))
	if newBounds.Width != expectedW {
		return errors.Errorf("invalid width: got %d; want: %d", newBounds.Width, expectedW)
	}
	expectedH := int(math.Round(float64(oldBounds.Height) / newZoom))
	if newBounds.Height != expectedH {
		return errors.Errorf("invalid height: got %d; want: %d", newBounds.Height, expectedH)
	}
...
// go/arc-wm-p "Clamshell: Page/content zoom" (slide #30-#31)
func wmPageZoom(ctx context.Context, cr *chrome.Chrome, a *arc.ARC, d *ui.Device) error {
	tconn, err := cr.TestAPIConn(ctx)
	if err != nil {
		return err
	}
	...
	testing.ContextLogf(ctx, "Bounds: %+v", origBounds)
	...
	obj := d.Object(ui.PackageName(act.PackageName()),
	...
	r, err := obj.GetBounds(ctx)
	if err != nil {
		return err
	}
	testing.ContextLogf(ctx, "*** object rect : %+v", r)
	...
	// But the content should be adjusted to the new zoom factor. For that we count the black pixels in the android application.

	if err := testing.Sleep(ctx, 2*time.Second); err != nil {
		return err
	}

	if newBounds, err := act.WindowBounds(ctx); err != nil {
		return err
	} else if newBounds != origBounds {
		return errors.Errorf("invalid window bounds: got %v, want %v", newBounds, origBounds)
	}

	r2, err := obj.GetBounds(ctx)
	if err != nil {
		return err
	}
	testing.ContextLogf(ctx, "*** object rect : %+v", r2)
	return nil
}
...
// checkMaximizeNonResizeable checks that the window is both maximized and non-resizeable.
...
// checkPillarboxNonResizeable checks that the window is both in pillar-box mode and is non-resizeable.
...
func changeDisplayZoomFactor(ctx context.Context, tconn *chrome.Conn, dispID string, zoomFactor float64) error {
	...
	// TODO(ricardoq): Investigate what is a good delay, and get this constant directly from Chrome.
	const delayAfterZoomFactorChange = 500 * time.Millisecond
	return testing.Sleep(ctx, delayAfterZoomFactorChange)
}
