...
func wmDisplayResolution(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, d *ui.Device) error {
	...
	newZoom := disp.AvailableDisplayZoomFactors[0]
	if err := changeDisplayResolution(ctx, tconn, disp.ID, newZoom); err != nil {
		return err
	}
	...
	testing.ContextLogf(ctx, "Bounds after: %+v", bounds)
...
func changeDisplayResolution(ctx context.Context, tconn *chrome.Conn, dispID string, resolution float64) error {
	p := display.DisplayProperties{DisplayZoomFactor: &resolution}
