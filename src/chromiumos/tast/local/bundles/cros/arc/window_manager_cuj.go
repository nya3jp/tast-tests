...
func WindowManagerCUJ(ctx context.Context, s *testing.State) {
	...
	for idx, test := range []struct {
		...
		{"Lights out ignored N", wmLightsOutIgnored},
	} {
...
// wmLightsOutIgnored verifies that the activity cannot go from restored to fullscreen mode as defined in:
// go/arc-wm-p "Clamshell: lights out and fullscreen ignored" (slide #21)
func wmLightsOutIgnored(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, d *ui.Device) error {
	...
	for _, test := range []struct {
		...
		// Wanted initial state: Restored mode
		if err := act.SetWindowState(ctx, arc.WindowStateNormal); err != nil {
			return err
		}
		...
		// Wanted state: Restored mode
		// Wanted caption: visible
		ws, err := act.GetWindowState(ctx)
		if err != nil {
			return err
		}
		wantedWS := arc.WindowStateNormal
		if ws != wantedWS {
			return errors.Errorf("invalid window state %q; want %q", ws.String(), wantedWS.String())
		}
		...
		wantedC := wmVisible
		if s.CaptionVisibility != wantedC {
			return errors.Errorf("invalid caption visibility %q; want %q", s.CaptionVisibility, wantedC)
		}
