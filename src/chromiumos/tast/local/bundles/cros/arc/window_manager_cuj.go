	...
	for _, test := range []struct {
		...
		for _, orientation := range []struct {
			...
			if s, err := act.GetWindowState(ctx); err != nil {
				...
				return errors.Errorf("invalid window state %q; want %q", s.String(), arc.WindowStateNormal.String())
			}
...
func wmFreeformResize(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, d *ui.Device) error {
	...
	if bounds.Width <= origBounds.Width {
		...
		return errors.Errorf("invalid window width: %d; want %d > %d", bounds.Width, bounds.Width, origBounds.Width)
	}
	return nil
}
...
func wmSnapping(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, d *ui.Device) error {
	...
	if math.Abs(float64(snappedBounds.Width-halfWidth)) > 1 {
		return errors.Errorf("invalid window width: %d; want Abs(%d - %d) <= 1", snappedBounds.Width, snappedBounds.Width, halfWidth)
	}
	if snappedBounds.Left != 0 {
		return errors.Errorf("invalid window origin: %d; want 0", snappedBounds.Left)
	}
...
func wmDisplayResolution(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, d *ui.Device) error {
	...
	if newBounds.Width != expectedW {
		return errors.Errorf("invalid width: %d; want: %d", newBounds.Width, expectedW)
	}
	expectedH := int(math.Round(float64(oldBounds.Height) / newZoom))
	if newBounds.Height != expectedH {
		return errors.Errorf("invalid height: %d; want: %d", newBounds.Height, expectedH)
	}
