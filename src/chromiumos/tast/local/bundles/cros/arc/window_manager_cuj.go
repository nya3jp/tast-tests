...
// wmLightsOut verifies that child activities do not honor the root activity state as defined in:
// go/arc-wm-p "Clamshell: Springboard activities" (slide #18).
func wmLightsOut(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, d *ui.Device) error {
	...
	for _, test := range []struct {
		...
		{"Using Zoom Toggle key", func() error { return toggleFullscreen(ctx, tconn) }},
		{"Using Android API", func() error { return uiClickImmersive(ctx, act, d) }},
	} {
...
func parse(r []byte) (map[string]string, error) {
