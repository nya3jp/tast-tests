...
func InputOverlayDisplay(ctx context.Context, s *testing.State) {
	inputoverlay.SetupTestApp(ctx, s, func(params inputoverlay.TestParams) error {
		...
		if err := uiauto.Combine("Hide game overlay",
			...
			// Exit out of menu.
			uda.Tap(uidetection.CustomIcon(s.DataPath("input-overlay-menu-close.png")).Below(uidetection.CustomIcon(s.DataPath("input-overlay-menu-close.png")))),
		...
		if err := uiauto.Combine("Disable game overlay",
			...
			ui.Gone(nodewith.Name("m")),
			not(inputoverlay.TapOverlayButton(kb, "m", &params, inputoverlay.TopTap)),
			ui.Gone(nodewith.Name("w")),
			not(inputoverlay.MoveOverlayButton(kb, "w", &params)),
		)(ctx); err != nil {
