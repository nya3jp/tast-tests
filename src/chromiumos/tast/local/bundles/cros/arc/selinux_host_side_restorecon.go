...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Checks that host-side SELinux restorecon does not take effect on ARC",
...
func SELinuxHostSideRestorecon(ctx context.Context, s *testing.State) {
	...
	correctLabels := getSELinuxLabelsForPaths(ctx, s, cr, pathsComponents)
	...
	testSELinuxLabels(s, actualLabels, correctLabels)
...
// testSELinuxLabels checks that all SELinux labels in actualLabels match those
// in expectedLabels.
func testSELinuxLabels(s *testing.State, actualLabels, expectedLabels map[string]string) {
