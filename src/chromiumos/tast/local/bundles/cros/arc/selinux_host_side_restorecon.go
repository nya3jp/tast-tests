...
func invalidateSELinuxHashForPaths(ctx context.Context, cr *chrome.Chrome, pathsComponents [][]string) error {
	invalidateSELinuxHash := func(path string) error {
		...
		if err := cmd.Run(testexec.DumpLogOnError); err != nil {
			return errors.Wrapf(err, "failed to invalidate security.sehash xattr of %v", path)
		}
		return nil
	}
	...
	if err := invalidateSELinuxHash(filepath.Dir(vaultPath)); err != nil {
		return err
	}
	if err := invalidateSELinuxHash(vaultPath); err != nil {
		return err
	}
	if err := invalidateSELinuxHash(filepath.Join(vaultPath, "root")); err != nil {
		return err
	}
	if err := invalidateSELinuxHash(filepath.Join(vaultPath, "root", "android-data")); err != nil {
		return err
	}
	...
	for _, components := range pathsComponents {
		...
		for _, entry := range components {
			...
			if err := invalidateSELinuxHash(path); err != nil {
				return err
			}
		}
	}
...
func testPlayability(ctx context.Context, a *arc.ARC) error {
	...
	if err := playstore.InstallApp(ctx, a, d, pkgName, &playstore.Options{TryLimit: -1}); err != nil {
		return errors.Wrap(err, "failed to install Android app")
	}
