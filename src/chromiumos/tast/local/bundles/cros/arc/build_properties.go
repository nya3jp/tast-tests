...
func BuildProperties(ctx context.Context, s *testing.State) {
	...
	// Verify that the properties without a partition name still exist.
	properties := []string{"fingerprint", "id", "tags", "version.incremental"}
	for _, property := range properties {
		getProperty(fmt.Sprintf("ro.build.%s", property))
	}
}
