...
func PowerCameraRecordingPerf(ctx context.Context, s *testing.State, targetFps string) {
	const (
	...
	s.Logf("Recording to file: %s", outputFile)
	...
	// Check if video file was generated. `du -k` measures the size in 1024 byte blocks.
	duOutput, err := a.Command(ctx, "du", "-k", outputFile).Output()
	if err != nil {
		s.Fatal("Could not determine size of video recording file: ", err)
	}

	duSplit := strings.Fields(string(duOutput))
	if len(duSplit) != 2 {
		s.Fatalf("Tried to check file size of %q but received unexpected result: %q", outputFile, string(duOutput))
	}

	fileSize, err := strconv.Atoi(duSplit[0])
	if err != nil {
		s.Fatalf("Tried to check file size of %q but received unexpected result: %q", outputFile, string(duOutput))
	}

	if fileSize*1024 < minExpectedFileSize {
		s.Fatalf("Video recording file is smaller than expected: %d < %d", fileSize*1024, minExpectedFileSize)
	}
