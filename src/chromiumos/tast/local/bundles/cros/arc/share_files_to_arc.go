...
func ShareFilesToArc(ctx context.Context, s *testing.State) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		_, err := ioutil.ReadFile(crosPath)
		if err != nil {
			return errors.Wrap(err, "file doesn't exisit")
		}
		return nil
	...
	if actual, err := ioutil.ReadFile(crosPath); len(actual) == 0 {
		s.Fatal("The Downloaded file size is 0: ", err)
	}
