...
func AppPosition(ctx context.Context, s *testing.State) {
	...
	verifyLaunchBounds(ctx, s, d, act)
...
func verifyLaunchBounds(ctx context.Context, s *testing.State, d *ui.Device, act *arc.Activity) {
	const pkg = "org.chromium.arc.testapp.appposition"

	/*
		display_size, err := act.disp.Size()
		if err != nil {
	...
	*/
	launchBounds, err := act.WindowBounds(ctx)
	if err != nil {
		s.Fatal("Failed to get the size of the window of the activity: ", err)
	}

	frameSelector := d.Object(ui.PackageName(act.PackageName()),
