...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:         []string{apk},
		Pre:          arc.Booted(), // arc.Booted() starts with the clamshell mode. This test needs the device to be in clamshell mode.
	})
}
...
// getCaptionBounds gets caption bounds.
//
// The return value is accurate bounds if uiautomator can find caption; or a bounds
...
// getShadowBoundsList gets a list of shadow bounds.
//
...
//
// The return value is a list of bounds to check shadow colors.
...
// getContentBounds gets bounds of the internal content.
...
func verifyLaunchBounds(ctx context.Context, s *testing.State, d *ui.Device, ac *arc.Activity) {
	...
	launchBounds := coords.NewRectLTRB(
		int(float64(deviceBounds.Width)*mainActivityLaunchBoundsLeft),
		int(float64(deviceBounds.Height)*mainActivityLaunchBoundsTop),
		int(float64(deviceBounds.Width)*mainActivityLaunchBoundsRight),
		int(float64(deviceBounds.Height)*mainActivityLaunchBoundsBottom))
	...
	if launchBounds != frameBounds {
		s.Errorf("Unexpected launch bounds %v. Expecting: %v", frameBounds, launchBounds)
	}
}
