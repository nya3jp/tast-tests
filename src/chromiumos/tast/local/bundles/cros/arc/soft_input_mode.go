...
func SoftInputMode(ctx context.Context, s *testing.State) {
	...
	if _, err := ash.SetARCAppWindowState(ctx, tconn, act.PackageName(), ash.WMEventSnapRight); err != nil {
		s.Fatal("Failed to snap app in split view: ", err)
	}

	if _, err := ash.SetARCAppWindowState(ctx, tconn, settingsAct.PackageName(), ash.WMEventSnapLeft); err != nil {
		s.Fatal("Failed to snap app in split view: ", err)
	}
	...
	if err := act.Stop(ctx); err != nil {
		s.Fatal("Failed to stop the activity: ", err)
	}
	act.Close()
	...
	act, err = arc.NewActivity(a, pkg, ".AdjustResizeActivity")
	if err != nil {
		s.Fatal("Failed to create a new activity: ", err)
	}
