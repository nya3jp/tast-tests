...
func AndroidIMEInBrowser(ctx context.Context, s *testing.State) {
	...
	if err := tconn.Eval(ctx,
		fmt.Sprintf(`chrome.languageSettingsPrivate.addInputMethod("%s");`, id), nil); err != nil {
	...
	if err := tconn.Eval(ctx,
		fmt.Sprintf(`chrome.inputMethodPrivate.setCurrentInputMethod("%s");`, id), nil); err != nil {
	...
	for i := 0; i < len(expected); i++ {
		...
		if err := d.Click(ctx, 0, info.DisplayHeight-1); err != nil {
	...
	if err := conn.WaitForExpr(ctx,
		fmt.Sprintf(`document.getElementById('text').value === '%s'`, expected)); err != nil {
