...
func Suspend(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < numTrials; i++ {
		...
		if math.Abs(guestSuspendSeconds-hostSuspendSeconds) > boottimeDiffAllowanceSeconds {
			s.Fatalf("NG: Suspend time was not injected to the guest properly, got %f, want %f", guestSuspendSeconds, hostSuspendSeconds)
			numFailed++
		} else {
			s.Logf("OK: %f seconds of suspend time was injected to the guest", guestSuspendSeconds)
			numPassed++
		}
	}
	if numFailed != 0 {
		s.Fatalf("Failed for some runs, passed %d times, failed %d times", numPassed, numFailed)
	}
}
