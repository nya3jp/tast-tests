...
func NativeCrash(ctx context.Context, s *testing.State) {
	...
	// Use `sleep` command as a target to crash. This command doesn't nothing and is easy to send signals. The argument should not be so large for the cases when `pkill` failed to kill this command.
	...
	const pollingTimeout = 10 * time.Second // The time to wait dump files. Typically they appears in few seconds.
