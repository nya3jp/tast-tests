...
// toggleCompatModeMenu toggle the compat-mode menu via the given method and verifies the expected visibility.
func ToggleCompatModeMenu(ctx context.Context, tconn *chrome.TestConn, method InputMethodType, keyboard *input.KeyboardEventWriter, expected bool) error {
	switch method {
	case InputMethodClick:
		return toggleCompatModeMenuViaButtonClick(ctx, tconn, expected)
	case InputMethodKeyEvent:
		return toggleCompatModeMenuViaKeyboard(ctx, tconn, keyboard, expected)
	}
	return errors.Errorf("invalid InputMethodType is given: %s", method)
}

// toggleCompatModeMenuViaButtonClick clicks on the compat-mode button and verifies the expected visibility.
func toggleCompatModeMenuViaButtonClick(ctx context.Context, tconn *chrome.TestConn, expected bool) error {
	ui := uiauto.New(tconn)
	...
	return CheckVisibility(ctx, tconn, BubbleDialogClassName, expected /* visible */)
}

// toggleCompatModeMenuViaKeyboard injects the keyboard shortcut and verifies the expected visibility.
func toggleCompatModeMenuViaKeyboard(ctx context.Context, tconn *chrome.TestConn, keyboard *input.KeyboardEventWriter, expected bool) error {
	ui := uiauto.New(tconn)
...
func ToggleResizeLockMode(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device, cr *chrome.Chrome, activity *arc.Activity, currentMode, nextMode ResizeLockMode, action ConfirmationDialogAction, method InputMethodType, keyboard *input.KeyboardEventWriter) error {
	...
	if err := ToggleCompatModeMenu(ctx, tconn, method, keyboard, true); err != nil {
