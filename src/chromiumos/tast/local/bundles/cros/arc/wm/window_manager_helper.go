...
func SetupAndRunTestCases(ctx context.Context, s *testing.State, testCases []TestCase) (func() error, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to ensure if tablet mode is disabled")
	}
	...
	return func() error {
		if err := d.Close(); err != nil {
			return errors.Wrap(err, "failed to call close device")
		}
		if err := cleanup(ctx); err != nil {
			return errors.Wrap(err, "failed to call cleanup tablet mode")
		}
		return nil
	}, nil
}
