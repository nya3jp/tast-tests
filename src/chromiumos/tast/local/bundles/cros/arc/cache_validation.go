...
func CacheValidation(ctx context.Context, s *testing.State) {
	...
	if cr, a, err := cache.BootARCWithPackagesCacheMode(ctx, cache.SkipCopy, withoutCacheDir); err != nil {
	...
	cr, a, err := cache.BootARCWithPackagesCacheMode(ctx, cache.Copy, withCacheDir)
	if err != nil {
		s.Fatal("Booting ARC failed: ", err)
	}
	defer cr.Close(ctx)
	...
	// unpackGmsCoreCaches unpack GMS core caches which are returned by
	// cache.BootARCWithPackagesCacheMode packed in tar.
	unpackGmsCoreCaches := func(outputDirs []string) error {
		for _, outputDir := range outputDirs {
			...
			if _, err = testexec.CommandContext(
				ctx, "tar", "-xvpf", tarPath, "-C", outputDir).Output(); err != nil {
