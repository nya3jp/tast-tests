...
func CacheValidation(ctx context.Context, s *testing.State) {
	...
	if cr, a, err := cache.OpenSession(ctx, cache.PackagesSkipCopy, cache.GMSCoreDisabled, []string{}, withoutCacheDir); err != nil {
	...
	cr, a, err := cache.OpenSession(ctx, cache.PackagesCopy, cache.GMSCoreEnabled, []string{}, withCacheDir)
	if err != nil {
		s.Fatal("Booting ARC failed: ", err)
	}
	defer cr.Close(ctx)
