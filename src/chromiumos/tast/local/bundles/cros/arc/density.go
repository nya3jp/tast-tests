...
func checkRectBounds(ctx context.Context, cr *chrome.Chrome, bound int) error {
	const colorMaxDiff = 128
	shouldBeBlack := func(x, y int) bool {
		return x < int(bound) && y < int(bound)
	}
...
func Density(ctx context.Context, s *testing.State) {
	...
	if err := a.Install(ctx, s.DataPath(densityApk)); err != nil {
		s.Fatal("Failed to install app: ", 150)
	}
	...
	if err := checkRectBounds(ctx, cr, 150); err != nil {
