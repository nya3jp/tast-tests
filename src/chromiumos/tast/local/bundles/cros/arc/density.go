...
const (
	apkName      = "ArcDensityTest.apk"
	zoomSetting  = "persist.sys.enable_application_zoom"
	packageName  = "org.chromium.arc.testapp.densitytest"
	activityName = "org.chromium.arc.testapp.densitytest.DensityActivity"
	cls          = ".DensityActivity"
...
func performAndConfirmDensityChange(ctx context.Context, ew *input.KeyboardEventWriter, act *arc.Activity, test densityChange) error {
	...
	if gotScaleFactor, err := act.ScaleFactor(ctx); err != nil {
		...
		testing.ContextLog(ctx, test.scaleFactor)
		testing.ContextLog(ctx, gotScaleFactor)
		//return errors.Wrapf("scale factor incorrect, expected: %d; got: %d", test.scaleFactor, gotScaleFactor)
	}
	return nil
}
...
func Density(ctx context.Context, s *testing.State) {
	...
	if err := arc.BootstrapCommand(ctx, setprop, zoomSetting, "true").Run(testexec.DumpLogOnError); err != nil {
		s.Fatal("Failed to set developer option: ", err)
	}
	defer arc.BootstrapCommand(ctx, setprop, zoomSetting, "false").Run(testexec.DumpLogOnError)

	// Install and start the application
	testing.ContextLog(ctx, "Installing app")
	// Install ArcDensityTest.apk
	if err := a.Install(ctx, s.DataPath("app_sanity_hello_world.apk")); err != nil {
		s.Fatal("Failed to install app: ", "app sanity")
	}
	testing.ContextLog(ctx, "creating activity")
	act, err := arc.NewActivity(a, "org.chromium.arc.testapp.appsanitytast", ".MainActivity")
	if err != nil {
