...
func wmRC22(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device) (retErr error) {
	ctxLeftAct := ctx
	ctx, cancelLeft := ctxutil.Shorten(ctx, wm.TimeReservedForStop)
	defer cancelLeft()

	ctxRightAct := ctx
	ctx, cancelRight := ctxutil.Shorten(ctx, wm.TimeReservedForStop)
	defer cancelRight()
	...
	defer func() {
		if err := leftAct.Stop(ctxLeftAct, tconn); err != nil {
			if retErr == nil {
	...
	defer func() {
		if err := rightAct.Stop(ctxRightAct, tconn); err != nil {
			if retErr == nil {
