...
const (
	pkgMaximized  = "org.chromium.arc.testapp.windowmanager24.maximized"
	pkgPhoneSize  = "org.chromium.arc.testapp.windowmanager24.phonesize"
	pkgTabletSize = "org.chromium.arc.testapp.windowmanager24.tabletsize"
)
...
func wmRC01(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device) error {
	...
	for activityName, desiredOrientation := range map[string]string{
		...
		if err := func() error {
			...
			if orientationFromBounds != desiredOrientation {
				return errors.Errorf("invalid bounds orientation %v; want %v", orientationFromBounds, desiredOrientation)
			}
	...
	apkPath := map[string]string{
		pkgMaximized:  "ArcWMTestApp_24_Maximized.apk",
		pkgPhoneSize:  "ArcWMTestApp_24_PhoneSize.apk",
		pkgTabletSize: "ArcWMTestApp_24_TabletSize.apk",
	}
	for pkgName, verifyFunc := range map[string]func(*arc.Activity, *ash.Window) error{
		pkgPhoneSize: func(act *arc.Activity, window *ash.Window) error {
			...
			if orientation != wm.Portrait {
				return errors.Errorf("invalid orientation %v; want portrait", orientation)
			}
			orientationFromBounds := wm.OrientationFromBounds(window.BoundsInRoot)
			if orientationFromBounds != wm.Portrait {
				return errors.Errorf("invalid bounds orientation %v; want portrait", orientationFromBounds)
			}
		...
		pkgTabletSize: func(act *arc.Activity, window *ash.Window) error {
			...
			if orientation != wm.Landscape {
				return errors.Errorf("invalid orientation %v; want landscape", orientation)
			}
			orientationFromBounds := wm.OrientationFromBounds(window.BoundsInRoot)
			if orientationFromBounds != wm.Landscape {
				return errors.Errorf("invalid bounds orientation %v; want landscape", orientationFromBounds)
			}
