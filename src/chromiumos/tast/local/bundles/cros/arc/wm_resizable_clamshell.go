...
func wmRC22(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device) (retErr error) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if math.Abs(float64(rightWInfo.BoundsInRoot.Top-rWant.Top)) > margin ||
			math.Abs(float64(rightWInfo.BoundsInRoot.Bottom()-rWant.Bottom())) > margin ||
			math.Abs(float64(rightWInfo.BoundsInRoot.Left-rWant.Left)) > margin ||
			math.Abs(float64(rightWInfo.BoundsInRoot.Right()-rWant.Right())) > margin {
