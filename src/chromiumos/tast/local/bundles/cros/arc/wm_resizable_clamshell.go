...
const (
	...
	wmLActivity = "org.chromium.arc.testapp.windowmanager.ResizeableLandscapeActivity"
...
func wmRC13(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, d *ui.Device) error {
	act, err := arc.NewActivity(a, wmPkg, wmLActivity)
	if err != nil {
		return err
	}
	defer act.Close()
	...
	// R apps launch as default windowed.
	...
	right := info.DisplayWidth - 1
	testing.ContextLog(ctx, "Resizing app to right margin = ", right)
	to := arc.NewPoint(right, info.DisplayHeight/2)
