...
func wmRC14(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device) error {
	...
	if err := snapToHalfHelper(ctx, tconn, a, d, false, true); err != nil {
		return errors.Wrap(err, "snap to half by long pressing on the maximize caption button and drag to the leftfailed")
	}
...
// snapToHalfHelper runs snap to half test cases by either long pressing on the maximize caption button and drag to the left/right,
// or by dragging the activity to the top left/right corner of the display.
