...
type builtinBridgeConfig struct {
...
const (
	// Android built-in ARM executables expected to be runnable via built-in bridge.
...
func init() {
	testing.AddTest(&testing.Test{
		Func:         BuiltInBridge,
		Desc:         "Checks whether built-in bridge is properly set up for ARCVM",
		Contacts:     []string{"youkichihosoi@chromium.org", "arcvm-eng@google.com"},
		SoftwareDeps: []string{"android_vm", "chrome"},
		Attr:         []string{"group:mainline", "informational"},
		Params: []testing.Param{{
			Name: "houdini",
			Val: builtinBridgeConfig{
...
func BuiltInBridge(ctx context.Context, s *testing.State) {
	cfg := s.Param().(builtinBridgeConfig)
	...
	const builtinBridgeProp = "ro.boot.native_bridge"
	nb, err := a.GetProp(ctx, builtinBridgeProp)
	if err != nil {
		s.Fatalf("Failed to getprop %q: %v", builtinBridgeProp, err)
	}
	nbExpected := cfg.binaryTranslationLibrary
	if nb != nbExpected {
		s.Fatalf("Failure: the property %q is set to %q; expected to be %q", builtinBridgeProp, nb, nbExpected)
	}
