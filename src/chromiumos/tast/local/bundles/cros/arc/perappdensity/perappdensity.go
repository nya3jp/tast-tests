...
// WaitForBlackPixelCount polls until numberof black pixels is equal to wantPixelCount.
...
func StartDensityActivityWithWindowState(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, a *arc.ARC, windowState arc.WindowState) (*arc.Activity, error) {
...
// defaultPixelCount is the size of the drawn square, before USF is applied. This value is used to compute the expected of the
// drawn square.
func VerifyPixelsWithUSFEnabled(ctx context.Context, a *arc.ARC, tconn *chrome.TestConn, cr *chrome.Chrome, windowState arc.WindowState, defaultPixelCount int) error {
	const (
	...
	act, err := StartDensityActivityWithWindowState(ctx, cr, tconn, a, windowState)
	if err != nil {
	...
	wantPixelCount := (int)((float64)(defaultPixelCount) * uniformScaleFactor * uniformScaleFactor)
