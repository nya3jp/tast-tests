...
// Package perappdensity contains test logic to change density and confrim changes.
...
// DensityApk specifies the name of the APK and package that will be used in the test.
type DensityApk struct {
	Name    string
	Package string
}
...
// RunTest installs an apk (either perappdensity/ perappdensitysurface view), which contains a black square.
// The test then increase and decreases the screen density, confirming that the change occurred by verifying
// the size of the black square.
func RunTest(ctx context.Context, s *testing.State, densityApk DensityApk, f func(context.Context, *arc.ARC, *chrome.Conn) error) {
	const (
	...
	if err := a.Install(ctx, s.DataPath(densityApk.Name)); err != nil {
		s.Fatal("Failed to install app: ", err)
	}

	act, err := arc.NewActivity(a, densityApk.Package, mainActivity)
	if err != nil {
	...
	if err := ash.WaitForARCAppWindowState(ctx, tconn, densityApk.Package, ash.WindowStateFullscreen); err != nil {
