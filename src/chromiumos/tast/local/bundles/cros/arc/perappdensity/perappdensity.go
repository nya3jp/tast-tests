...
func StartDensityActivityWithWindowState(ctx context.Context, a *arc.ARC, tconn *chrome.TestConn, cr *chrome.Chrome, act *arc.Activity, windowState arc.WindowState) error {
	...
	if err := ash.WaitForARCAppWindowState(ctx, tconn, DensityPackageName, windowState.WindowStateType()); err != nil {
		return errors.Wrapf(err, "failed to wait for the activity to have required window state %q", windowState.String())
	}
...
func EnableUSFAndConfirmPixelState(ctx context.Context, a *arc.ARC, tconn *chrome.TestConn, cr *chrome.Chrome, act *arc.Activity, windowState arc.WindowState, defaultPixelCount int) error {
	const (
	...
	if err := StartDensityActivityWithWindowState(ctx, a, tconn, cr, act, arc.WindowStateNormal); err != nil {
