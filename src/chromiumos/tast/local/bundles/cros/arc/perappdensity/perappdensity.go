...
const (
	...
	// PackageName is the name of density activity.
...
// Change executes the density change, specified by KeySequence
// and confirms that the density was changed by validating the size of the square on the screen.
func (dc *Change) Change(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, ew *input.KeyboardEventWriter) error {
	...
	if err := WaitForBlackPixelCount(ctx, cr, tconn, int(dc.BlackPixelCount)); err != nil {
...
func CountBlackPixels(img image.Image) (int, error) {
...
func WaitForBlackPixelCount(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, wantPixelCount int) error {
	// Need to wait for relayout to complete, before grabbing new screenshot.
	if err := testing.Poll(ctx, func(ctx context.Context) error {

		winInfo, err := ash.GetARCAppWindowInfo(ctx, tconn, PackageName)
		if err != nil {
			return err
		}
		...
		// optionally crop is here?
		n, err := CountBlackPixels(img)
		if err != nil {
			testing.PollBreak(err)
		}
...
// TODO(sarakato): Make this function more robust, by allowing a view to be passed as an arg.
func StartViewActivityWithWindowState(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, windowState arc.WindowState) (*arc.Activity, error) {
	...
	if err := ash.SetShelfBehavior(ctx, tconn, dispInfo.ID, ash.ShelfBehaviorAlwaysAutoHide); err != nil {
		return nil, errors.Wrap(err, "failed to set shelf behavior to Always Auto Hide")
	}
...
func VerifyPixelsWithUSFEnabled(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, a *arc.ARC, windowState arc.WindowState, nonScaledPixelCount int) error {
	...
	if err := WaitForBlackPixelCount(ctx, cr, tconn, wantPixelCount); err != nil {
...
func RunTest(ctx context.Context, cr *chrome.Chrome, a *arc.ARC, packageName string, testSteps []Change, activity string, expectedInitialPixelCount float64) error {
	...
	if err := WaitForBlackPixelCount(ctx, cr, tconn, int(expectedInitialPixelCount)); err != nil {
		return errors.Wrap(err, "failed to check initial state: ")
	}

	for _, testStep := range testSteps {
		if err := testStep.Change(ctx, cr, tconn, ew); err != nil {
	...
	defer func() error {
		...
		if err := initialState.Change(ctx, cr, tconn, ew); err != nil {
