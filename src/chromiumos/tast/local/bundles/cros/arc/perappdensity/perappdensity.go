...
func WaitForBlackPixelCount(ctx context.Context, cr *chrome.Chrome, wantPixelCount int) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		// Allow a small epilson, to account for float computation.
		if diff > 0.01 {
...
// drawn square.
func VerifyPixelsWithUSFEnabled(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, a *arc.ARC, windowState arc.WindowState, defaultPixelCount int) error {
	const (
