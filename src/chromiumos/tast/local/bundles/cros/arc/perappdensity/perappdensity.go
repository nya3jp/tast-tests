...
// VerifyPixelsWithUSFEnabled enables Uniform Scale Factor(USF), which applies a scale factor of 1.25.
...
// drawn square.
func VerifyPixelsWithUSFEnabled(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, a *arc.ARC, windowState arc.WindowState, nonScaledPixelCount int, clr color.Color) (*arc.Activity, error) {
	const (
	...
	if err := arc.BootstrapCommand(ctx, Setprop, uniformScaleFactorSetting, "1").Run(testexec.DumpLogOnError); err != nil {
		return nil, errors.Wrap(err, "failed to set developer option")
	}
	...
	if err != nil {
		return act, errors.Wrap(err, "failed to start activity after enabling uniform scale factor")
	}

	return act, VerifyPixelCount(ctx, cr, a, clr, nonScaledPixelCount, act)
}
