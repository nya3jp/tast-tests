...
const (
	// Setprop is path for setprop command.
	Setprop = "/system/bin/setprop"
	// DensitySetting is the settings string for changing density.
...
// ExecuteChange changes the density of the activity,
...
func RunTest(ctx context.Context, s *testing.State, packageName string, testSteps []DensityChange, activities []string, expectedInitialPixelCount float64) error {
	...
	// Start each activity, and execute the changess density changes for each activity.
