...
func testWorkspaceInsets(ctx context.Context, _ *arc.ARC, _ *chrome.Chrome, tconn *chrome.TestConn, act *arc.Activity, d *ui.Device) error {
	...
	parseRectString := func(rectShortString string) (coords.Rect, error) {
		...
		displayWidthPX := int(float64(dispInfo.Bounds.Width) * dispMode.DeviceScaleFactor)
		displayHeightPX := int(float64(dispInfo.Bounds.Height) * dispMode.DeviceScaleFactor)
		return coords.NewRectLTRB(left, top, displayWidthPX-right, displayHeightPX-bottom), nil
	}
...
func testPopupWindow(ctx context.Context, a *arc.ARC, cr *chrome.Chrome, tconn *chrome.TestConn, act *arc.Activity, d *ui.Device) error {
	...
	setWindowBounds(ctx, d, coords.NewRect(0, 0, int(float64(dispInfo.Bounds.Width)*dispMode.DeviceScaleFactor), int(float64(dispInfo.Bounds.Height)*dispMode.DeviceScaleFactor)))
