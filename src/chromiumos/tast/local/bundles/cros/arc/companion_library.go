...
func testResizeWindow(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, act *arc.Activity, d *ui.Device, s *testing.State) error {
	...
	// Arc companion library demo provide a activity for resize test, there are four draggable bars in four sides.
	// Maximize the window by drag from four sides.
	...
	for _, test := range []struct {
		...
		{startX: bounds.Left + innerMargin, startY: middleY, endX: 0, endY: middleY},
		{startX: bounds.Left + bounds.Width - innerMargin, startY: middleY, endX: dispW - 1, endY: middleY},
		{startX: middleX, startY: bounds.Top + innerMargin + captionHeight, endX: middleX, endY: 0},
		{startX: middleX, startY: bounds.Top + bounds.Height - innerMargin, endX: middleX, endY: dispH - 1},
	} {
		...
		// Wait for application's UI ready, it will take a long time because of the window bounds changing.
