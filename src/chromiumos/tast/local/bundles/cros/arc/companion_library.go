...
func testResizeWindow(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, act *arc.Activity, d *ui.Device) error {
	...
	// Resize by companion library will take long time, and can't be detected by tast-test library, so use sleep for create safe a time gap.
