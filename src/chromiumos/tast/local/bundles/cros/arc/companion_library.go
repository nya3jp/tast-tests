...
func testWorkspaceInsets(ctx context.Context, tconn *chrome.Conn, act *arc.Activity, d *ui.Device, s *testing.State) error {
	...
	parseRectText := func(msg string, mode *display.DisplayMode) ash.Rect {
		...
		if n, err := fmt.Sscanf(msg, "Rect(%d,%d - %d,%d)", &left, &top, &right, &bottom); err != nil {
			s.Fatal("Error on parse Rect text: ", err)
		} else if n != 4 {
			s.Fatalf("The format of Rect text is not valid: %q", msg)
		}
		bound := ash.Rect{
			Left: left, Top: top, Width: mode.WidthInNativePixels - left - right, Height: mode.HeightInNativePixels - top - bottom,
		}
		return bound
	}

	parseWorkspaceMessage := func(msg string, mode *display.DisplayMode) ash.Rect {
		const messageHead = "Workspace Insets: "
		if !strings.HasPrefix(msg, messageHead) {
			s.Fatalf("The format of message text is not valid: %q", msg)
		}
		msg = strings.TrimPrefix(msg, messageHead)
		rect := parseRectText(msg, mode)
		return rect
	}
	...
	for _, test := range []struct {
		...
		// Read window insets size from Companion Library
		...
		parsedShelfRect := parseWorkspaceMessage(lines[len(lines)-1], dispMode)
