...
func testWindowShadow(ctx context.Context, _ *arc.ARC, cr *chrome.Chrome, tconn *chrome.TestConn, act *arc.Activity, d *ui.Device) error {
	...
	testing.ContextLogf(ctx, "WindowShadow: bound: %v; display: %v", bounds, dispBounds)

	if err != nil {
		return err
	}
	if bounds.Width >= dispBounds.Width || bounds.Height >= dispBounds.Height {
		return errors.New("activity is larger than screen so that shadow can't be visible")
	}
	if bounds.Left < shadowWidth || bounds.Left+bounds.Width+shadowWidth >= dispBounds.Width {
...
func testWorkspaceInsets(ctx context.Context, _ *arc.ARC, _ *chrome.Chrome, tconn *chrome.TestConn, act *arc.Activity, d *ui.Device) error {
	...
	parseRectString := func(rectShortString string) (*coords.Rect, error) {
		...
		if n, err := fmt.Sscanf(rectShortString, "[%d,%d][%d,%d]", &left, &top, &right, &bottom); err != nil {
			return nil, errors.Wrap(err, "error on parse Rect text")
		} else if n != 4 {
			return nil, errors.Errorf("the format of Rect text is not valid: %q", rectShortString)
		}
