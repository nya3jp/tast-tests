...
func BlackFlash(ctx context.Context, s *testing.State) {
	...
	if err = testing.Poll(ctx, func(ctx context.Context) error {
		...
		img, err := screenshot.GrabScreenshot(ctx, cr, fmt.Sprintf("%s/screenshot.png", s.OutDir()))
		if err != nil {
		...
		numScreenshot++
		s.Logf("Screenshot %d: black pixels = %d / %d (%d%%)", numScreenshot, blackPixels, totalPixels, percent)
