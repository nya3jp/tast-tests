...
func DataLeakPreventionRulesListArcClipboard(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			// Check Clipboard toast on opening ARC with copied content.
			toastError := checkDLPClipboardARCToast(ctx, tconn, param.url, param.copyAllowed)
			if toastError != nil {
			...
			if copiedString == "" && param.copyAllowed {
				s.Fatalf("String didn't copied: got %q, expected not empty", copiedString)
			}

			pageTitle := strings.Title(param.name)

			if !strings.Contains(copiedString, pageTitle) && param.copyAllowed {
				s.Fatalf("Failed to check title in copied string: got %q, want containing title: %q", copiedString, pageTitle)
			}
		})
	}
}

// checkDLPClipboardARCToast checks DLP Clipboard toast on opening of ARC with and without restricted content.
func checkDLPClipboardARCToast(ctx context.Context, tconn *chrome.TestConn, url string, copyAllowed bool) error {
	ui := uiauto.New(tconn)
	...
	if err != nil && !copyAllowed {
		return errors.Wrap(err, "failed to check for clipboard toast: ")
	}
	if err == nil && copyAllowed {
		return errors.New("clipboard toast found, want none")
	}
