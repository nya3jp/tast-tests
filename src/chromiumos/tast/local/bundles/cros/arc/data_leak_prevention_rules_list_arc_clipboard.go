...
func DataLeakPreventionRulesListArcClipboard(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			toastError := checkArcClipboardToast(ctx, tconn, param.url, param.wantAllowed)
			if toastError != nil {
...
func checkArcClipboardToast(ctx context.Context, tconn *chrome.TestConn, url string, wantAllowed bool) error {
