...
func init() {
	testing.AddTest(&testing.Test{
		Func: VideoDecodeAccelVDVDAPerf, LacrosStatus: testing.LacrosVariantUnknown, Desc: "Measures performance of hardware decode acceleration performance using media::VideoDecoder through the VDA interface, by running the video_decode_accelerator_perf_tests binary (see go/vd-migration)",
...
func VideoDecodeAccelVDVDAPerf(ctx context.Context, s *testing.State) {
	if err := decoding.RunAccelVideoPerfTest(ctx, s.OutDir(), s.DataPath(s.Param().(string)), decoding.VDVDA); err != nil {
