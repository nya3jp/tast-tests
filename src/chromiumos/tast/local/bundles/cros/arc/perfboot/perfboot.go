...
func GetPerfValues(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC) (map[string]time.Duration, error) {
	...
	adjustedArcStartTime := time.Duration(arcStartTimeMS*1000000.0) * time.Nanosecond
	...
	for scanner.Scan() {
		...
		p[eventTag] = time.Duration(eventTimeMs*1000000.0)*time.Nanosecond - adjustedArcStartTime
...
func clockDelta(ctx context.Context) (time.Duration, error) {
	...
	return time.Duration(hostClockNS-guestClockNS) * time.Nanosecond, nil
}
