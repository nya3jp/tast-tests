...
func Gralloc(ctx context.Context, s *testing.State) {
	...
	for _, arch := range []string{"amd64", "arm", "x86"} {
		...
		if err := a.PushFile(ctx, crosBinDir+archTest, tempDir); err != nil {
		...
		if testErr != nil {
			s.Fatalf("Failed to run %s: %v", archTest, testErr)
		} else {
			s.Logf("%s completed successfully", arcTest)
		}
		ranTest = true
	}
