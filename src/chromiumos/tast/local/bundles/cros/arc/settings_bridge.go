		...
		for _, enable := range []bool{true, false} {
			...
			if err := testing.Poll(ctx, func(ctx context.Context) error {
				if res, err := accessibility.IsEnabledAndroid(ctx, a); err != nil {
					...
					return errors.Errorf("accessibility is %t in Android", res)
				}
				if services, err := accessibility.EnabledAndroidAccessibilityServices(ctx, a); err != nil {
					return err
				} else if (enable && !(len(services) == 1 && services[0] == accessibility.ArcAccessibilityHelperService)) || (!enable && len(services) == 0) {
