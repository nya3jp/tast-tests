...
func setChromeFontScale(ctx context.Context, conn *chrome.Conn, size string) error {
	script := fmt.Sprintf(`
		chrome.fontSettings.setDefaultFontSize({pixelSize: %s}, function() {});
		chrome.fontSettings.setDefaultFixedFontSize({pixelSize: %s}, function() {});`, size, size)
	return conn.Exec(ctx, script)
}
...
// proxy is one of:
// global_http_proxy_host|globa_http_proxy_port|global_proxy_pac_url|global_http_proxy_exclusion_list.
...
func setChromeProxyFixedServers(ctx context.Context, conn *chrome.Conn, host, port, bypassList string) error {
	script := fmt.Sprintf(
		`new Promise((resolve) => {
			...
			function() {
				window.__proxy_set_complete = true;
				resolve();
			});
