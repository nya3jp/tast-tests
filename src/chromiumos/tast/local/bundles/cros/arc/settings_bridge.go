...
func testCaptionSettingsSync(ctx context.Context, tconn *chrome.Conn, a *arc.ARC) error {
	...
	if strings.TrimSpace(string(actualFontScale)) != expectedFontScale {
		return errors.Errorf("unexpected font scale: got %s; want %s", actualFontScale, expectedFontScale)
	}

	if strings.TrimSpace(string(actualBackgroundColor)) != expectedBackgroundColor {
		return errors.Errorf("unexpected background color: got %s; want %s", actualBackgroundColor, expectedBackgroundColor)
	}

	if strings.TrimSpace(string(actualCaptioningEnabled)) != expectedCaptioningEnabled {
		return errors.Errorf("unexpected captioning enabled: got %s; want %s", actualCaptioningEnabled, expectedCaptioningEnabled)
	}

	if strings.TrimSpace(string(actualEdgeType)) != expectedEdgeType {
		return errors.Errorf("unexpected edge type: got %s; want %s", actualEdgeType, expectedEdgeType)
	}

	if strings.TrimSpace(string(actualForegroundColor)) != expectedForegroundColor {
		return errors.Errorf("unexpected foreground color: got %s; want %s", actualForegroundColor, expectedForegroundColor)
	}
