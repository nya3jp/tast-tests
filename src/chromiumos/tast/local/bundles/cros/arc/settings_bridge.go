...
// setChromeProxy sets the network proxy for Chrome.
func setChromeProxy(ctx context.Context, tconn *chrome.TestConn, config chromeproxy.ProxyConfig) error {
	return chromeproxy.SetSettings(ctx, tconn, chromeproxy.ProxySettings{
		Value: config,
		Scope: chromeproxy.ScopeRegular,
	})
}
...
func runProxyTest(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, p proxySettingsTestCase) error {
	if err := setChromeProxy(ctx, tconn, p.config); err != nil {
