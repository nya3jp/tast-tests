...
func DemoMode(ctx context.Context, s *testing.State) {
	...
	// In Demo Mode, the session would restart if idling for 60 seconds, which
	// would distupt the test. However, some statement below may block for over
	// 60 seconds. To solve this problem, periodically move mouse on a separate
	// goroutine, to prevent idling.
