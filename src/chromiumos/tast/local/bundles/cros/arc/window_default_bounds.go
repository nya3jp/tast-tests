...
func checkPhoneSizeRestored(ctx context.Context, tconn *chrome.TestConn, act *arc.Activity) error {
	...
	_, workArea, err := getScreenSizeAndInternalWorkArea(ctx, tconn)
	if err != nil {
		return err
	}
...
func check80PercentRestored(ctx context.Context, tconn *chrome.TestConn, act *arc.Activity) error {
	...
	screen, workArea, err := getScreenSizeAndInternalWorkArea(ctx, tconn)
	if err != nil {
		return err
	}
	if err := checkCentered(bounds, *workArea); err != nil {
...
func getScreenSizeAndInternalWorkArea(ctx context.Context, tconn *chrome.TestConn) (*coords.Size, *coords.Rect, error) {
