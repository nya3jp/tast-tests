...
func getSdcardTargetDir(ctx context.Context, cr *chrome.Chrome) (string, error) {
	...
	return filepath.Join(androidDataDir, "android-data", "data", "media", "0", "Pictures", "capybaras"), nil
}
...
func startMediaScanTimeWithApp(ctx context.Context, s *testing.State, a *arc.ARC, tconn *chrome.TestConn, volumeURI string) (func(), error) {
	const (
...
func MediaScanTime(ctx context.Context, s *testing.State) {
	...
	volumeURIPrefix := "file:///storage/"
	volumeURI := volumeURIPrefix + param.volumeURISuffix

	closeApp, err := startMediaScanTimeWithApp(ctx, s, a, tconn, volumeURI)
	if err != nil {
		s.Fatal("Failed to start app: ", err)
	}
	defer closeApp()

	targetDir, err := param.getTargetDir(ctx, cr)
	if err != nil {
