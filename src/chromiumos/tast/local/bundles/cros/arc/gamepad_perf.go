...
func GamepadPerf(ctx context.Context, s *testing.State) {
	...
	if err := inputlatency.EvaluateLatency(ctx, s, d, repeat*2, eventTimes, diffs, "avgGamepadButtonLatency", pv); err != nil {
	...
	if err := inputlatency.EvaluateLatency(ctx, s, d, repeat*2, eventTimes, diffs, "avgGamepadStickLatency", pv); err != nil {
		s.Fatal("Failed to evaluate: ", err)
	}
	...
	if err := inputlatency.EvaluateLatency(ctx, s, d, repeat*4, eventTimes, diffs, "avgGamepadMixLatency", pv); err != nil {
		s.Fatal("Failed to evaluate: ", err)
	}
