		...
		s.Run(ctx, row.name, func(ctx context.Context, s *testing.State) {
			...
			err := testing.Poll(ctx, func(ctx context.Context) error {
				...
				if got == row.wantPastedData {
					s.Logf("Found expected paste data on attempt #%d, %0.3fs since subtest start, %0.3fs since copy returned", attempt, time.Since(start).Seconds(), time.Since(afterCopy).Seconds())
					return nil
				}
