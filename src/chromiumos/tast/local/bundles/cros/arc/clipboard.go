...
func testCopyImageFromChromeToAndroid(ctx context.Context, p *arc.PreData, tconn *chrome.TestConn, s *testing.State) {
	const (
	...
	if err != nil {
		s.Fatal("Failed to open clipboard_image.html: ", err)
	}
	defer conn.Close()
	...
	encodedImage := ""
	conn.Eval(ctx, "document.getElementsByTagName('img')[0].getAttribute('src')", &encodedImage)
	...
	re := regexp.MustCompile(`^<img src="(.+?)" style=".+?">$`)
...
func Clipboard(ctx context.Context, s *testing.State) {
	...
	s.Run(ctx, "CopyImageFromChromeToAndroid", func(ctx context.Context, s *testing.State) {
		testCopyImageFromChromeToAndroid(ctx, p, tconn, s)
	})
