...
func Clipboard(ctx context.Context, s *testing.State) {
	...
	s.Log("Writting uppacked extension to ", extDir)
	dir, _ := filepath.Split(s.DataPath("clipboard_extension/manifest.json"))
	localExtFiles, err := ioutil.ReadDir(dir)
	if err != nil {
		s.Fatal("Failed to read the files in extension folder")
	}
	for _, localExtFile := range localExtFiles {
		if err := fsutil.CopyFile(filepath.Join(dir, localExtFile.Name()), filepath.Join(extDir, localExtFile.Name())); err != nil {
			s.Fatalf("Failed to copy file %q: %v", localExtFile, err)
		}
	}
	...
	// TODO(ruanc): Copies big text (500Kb) is blocked by https://bugs.chromium.org/p/chromium/issues/detail?id=916882
}
