...
func grabPerfSamples(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, act *arc.Activity, dispID string) (samples map[perf.Metric][]float64, err error) {
	...
	for i := 0; i < samplesPerRotation; i++ {
		...
		for _, rot := range []int{90, 180, 270, 0} {
			...
			numFrames, err := strconv.Atoi(groups[numFramesGroupIdx])
			if err != nil {
				return nil, err
			}
			if numFrames == 0 {
			...
			for j := 1; j < len(groups); j = j + 2 {
				...
				key = key + keySuffix
				m := perf.Metric{
					Name:      nameForMetric(key),
					Unit:      unitForMetric(key),
					Direction: directionForMetric(key),
					Multiple:  true,
				}
