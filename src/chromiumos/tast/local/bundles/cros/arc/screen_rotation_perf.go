...
func ScreenRotationPerf(ctx context.Context, s *testing.State) {
	...
	// def _output_results(self, gfxinfo_dict):
	// for key, data in gfxinfo_dict.iteritems():
	// 	# Groups the results by rotation.
	// 	for i in xrange(2):
	// 		rotation_type = 'To Portrait' if i == 0 else 'To Landscape'
	// 		# Every other value corresponds to the same type of rotation.
	// 		subsequence = data[i::2]
	// 		average_value = sum(subsequence) / float(len(subsequence))
	// 		logging.info('%s (%s): %f', key, rotation_type, average_value)
	// 		self.output_perf_value(
	// 				description=rotation_type + '_' + key,
	// 				value=subsequence,
	// 				units=self._get_units(key),
	// 				higher_is_better=self._is_higher_better(key))

	values := perf.NewValues()
	for k, v := range samples {
		s.Logf("Processing: %q - %+v", k, v)
...
// directionForMetric returns whether higher values are better for a given metric key.
