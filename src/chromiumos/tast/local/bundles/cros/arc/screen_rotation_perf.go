	...
	for i := 0; i < samplesPerRotation; i++ {
		...
		for _, rot := range []int{90, 180, 270, 0} {
			...
			if err := testing.Poll(ctx, func(ctx context.Context) error {
				groups, err = gfxinfoDumpStats(ctx, a, pkgName)
				if err != nil {
					return err
				}
...
func gfxinfoDumpStats(ctx context.Context, a *arc.ARC, pkgName string) ([]string, error) {
	...
	ss := string(output)
	stats := re.FindStringSubmatch(ss)
	if stats == nil {
...
func waitUntilNoNewFramesAvailable(ctx context.Context, a *arc.ARC, pkgName string) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		const numFramesGroupIdx = 2
		if numFrames, err := strconv.Atoi(stats[numFramesGroupIdx]); err != nil {
			return testing.PollBreak(err)
		} else if numFrames != current {
			current = numFrames
