...
func OptInAfterInterruption(ctx context.Context, s *testing.State) {
	...
	for _, delay := range delays {
		...
		// continueAfterInterruptionFunc will return true if test should
		// continue after optin interruption or false if test should exit.
		continueAfterInterruptionFunc := attemptOptIn(ctx, s, &param, delaySeconds)
		if !continueAfterInterruptionFunc() {
			break
		}
...
func attemptOptIn(ctx context.Context, s *testing.State, p *optInTestParams, t time.Duration) func() bool {
	earlyCompleteHandler := func() bool {
		s.Logf("Completed test before ending iteration for %v delay", t)
		if t == p.delays[len(p.delays)-1] {
			s.Error("Test did not complete with the expected delay values")
		}
		return false
	}
