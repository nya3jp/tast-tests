...
func setupFakePolicyServer(ctx context.Context, outdir string) (*fakedms.FakeDMS, error) {
	...
	if err != nil {
		err = errors.Wrap(err, "failed to create fakedms")
		return nil, err
	}
	...
	if err := blob.AddPolicies(policy); err != nil {
		err = errors.Wrap(err, "failed to add policy to policy blob")
	}
	if err := fdms.WritePolicyBlob(blob); err != nil {
		err = errors.Wrap(err, "failed to write policy blob to fdms")
		return fdms, err
	}
	return fdms, nil
}
