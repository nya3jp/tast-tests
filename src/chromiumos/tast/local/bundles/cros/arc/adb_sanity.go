...
// testADBCommandStdin exercises that arc.ARC.Command's stdin does NOT convert
...
func testADBCommandStdin(ctx context.Context, s *testing.State, a *arc.ARC, tmpdir string, content []byte) {
	...
	if err != nil {
		s.Error("Failed to create stdin pipe: ", err)
		return
	}
	if _, err := stdin.Write(content); err != nil {
		s.Error("Failed to write the stdin: ", err)
		stdin.Close()
		return
	}
	if err := stdin.Close(); err != nil {
		s.Error("Failed to close the stdin: ", err)
		return
	}
...
// testADBCommandStdout exercises that arc.ARC.Command's output does NOT convert
// between LF and CRLF automatically.
