...
func getMidiPort(ctx context.Context, s *testing.State) (string, error) {
	const emptyPort = ""
	...
	return emptyPort, errors.New("Midi Through client not found")
}
...
func MIDIClient(ctx context.Context, s *testing.State) {
	...
	// Wait for 2 seconds for the app to start and arecordmidi to receive the MIDI message
	// before signalling an error.
	err = testing.Poll(ctx, func(ctx context.Context) error {
		fi, err := os.Stat(midiFile)
		if err != nil {
			return errors.Wrapf(err, "failed to stat file: %s", midiFile)
		}
		if fi.Size() == 0 {
	...
	if err != nil {
		s.Fatalf("arecordmidi failed to receive MIDI message: ", err)
	}
