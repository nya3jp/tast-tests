...
func PIP(ctx context.Context, s *testing.State) {
	...
	// TODO(ricardoq): replace precondition with "force shelf at bottom"
	if err := preconditionShelfAtBottom(ctx, tconn, dispMode); err != nil {
		s.Fatal("Precondition failed; shelf must be placed at the bottom: ", err)
	}
...
// preconditionShelfAtBottom verifies that Chrome OS shelf is placed at the bottom of the screen, as opposed to left / right placed.
func preconditionShelfAtBottom(ctx context.Context, tconn *chrome.Conn, dispMode *display.DisplayMode) error {
	r, err := getShelfRect(ctx, tconn)
	if err != nil {
		return errors.Wrap(err, "failed to get shelf rect")
	}
	// Testing Top as well in case placing the shelf at the top becomes valid.
	if r.Width != dispMode.Width || r.Top < dispMode.Height/2 {
		return errors.Errorf("invalid shelf position %+v", r)
	}
	return nil
}
