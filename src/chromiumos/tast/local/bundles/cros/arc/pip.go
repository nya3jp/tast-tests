	...
	for _, tabletMode := range tabletModes {
		...
		for _, multiActivityPIP := range enableMultiActivityPIP {
			...
			for idx, test := range []struct {
				...
				if isARCVM(s.SoftwareDeps()) && (test.name == "PIP ExpandPIP Shelf Icon" || test.name == "PIP ExpandPIP Menu Touch" || test.name == "PIP Toggle Tablet mode" || test.name == "PIP AutoPIP New Android Window") {
...
func testPIPResizeToMax(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, pipAct *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode) error {
	...
	if err := pipAct.ResizeWindowDirectly(ctx, arc.BorderTopLeft, coords.NewPoint(0, 0), time.Second, bounds, 5); err != nil {
