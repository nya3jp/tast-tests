...
type pipTestFunc func(context.Context, *chrome.TestConn, *arc.ARC, *chrome.Chrome, *arc.Activity, *ui.Device, *display.DisplayMode) error
	...
	for _, tabletMode := range tabletModes {
		...
		for _, multiActivityPIP := range enableMultiActivityPIP {
			...
			for idx, test := range s.Param().([]pipTestParams) {
				...
				if err := test.fn(ctx, tconn, a, cr, pipAct, dev, dispMode); err != nil {
...
func testPIPMove(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, cr *chrome.Chrome, pipAct *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode) error {
	const (
...
func testPIPResizeToMax(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, cr *chrome.Chrome, pipAct *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode) error {
...
func testPIPGravityQuickSettings(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, cr *chrome.Chrome, pipAct *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode) error {
...
func testPIPToggleTabletMode(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, cr *chrome.Chrome, act *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode) error {
	window, err := getPIPWindow(ctx, tconn)
	if err != nil {
		return errors.Wrap(err, "could not get PIP window")
	}
...
func testPIPAutoPIPMinimize(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, cr *chrome.Chrome, pipAct *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode) error {
...
func testPIPExpandViaMenuTouch(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, cr *chrome.Chrome, pipAct *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode) error {
	...
	if err := expandPIPViaMenuTouch(ctx, tconn, cr, pipAct, dev, dispMode, initialAshWindowState); err != nil {
...
func testPIPExpandViaShelfIcon(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, cr *chrome.Chrome, pipAct *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode) error {
	isTabletModeEnabled, err := ash.TabletModeEnabled(ctx, tconn)
	if err != nil {
		return errors.Wrap(err, "failed to get tablet mode")
	}
...
func testPIPAutoPIPNewAndroidWindow(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, cr *chrome.Chrome, pipAct *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode) error {
	const (
...
func testPIPAutoPIPNewChromeWindow(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, cr *chrome.Chrome, pipAct *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode) error {
...
func expandPIPViaMenuTouch(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome, act *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode, restoreWindowState ash.WindowStateType) error {
	sdkVer, err := arc.SDKVersion()
	if err != nil {
	...
	case arc.SDKR:
		return expandPIPViaMenuTouchR(ctx, tconn, cr, dispMode, restoreWindowState)
	default:
...
func expandPIPViaMenuTouchR(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome, dispMode *display.DisplayMode, restoreWindowState ash.WindowStateType) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		window, err := getPIPWindow(ctx, tconn)
		if err != nil {
		...
		return testing.Poll(ctx, func(ctx context.Context) error {
			...
			if err := waitForPIPMenu(ctx, cr, origBounds); err != nil {
				return errors.Wrap(err, "the PIP menu did not appear")
			}
...
func waitForPIPMenu(ctx context.Context, cr *chrome.Chrome, bounds coords.Rect) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		testing.ContextLogf(ctx, "pipBgPixels: %+v", pipBgPixels)
		testing.ContextLogf(ctx, "pipMenuPixels: %+v", pipMenuPixels)
