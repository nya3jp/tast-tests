...
const (
	...
	// Devices like Nocturne have some off-by-one pixel bugs. Apparently they are not PIP-specific.
	// TODO(ricardoq): Remove this once all off-by-one pixel bugs are fixed.
...
func testPIPMove(ctx context.Context, cr *chrome.Chrome, act *arc.Activity, dev *ui.Device) error {
	const (
...
func testPIPResize(ctx context.Context, cr *chrome.Chrome, act *arc.Activity, dev *ui.Device) error {
	tconn, err := cr.TestAPIConn(ctx)
	if err != nil {
	...
	// TODO(ricardoq): Get this value in runtime.
	...
	if pipMaxSizeH < pipMaxSizeW {
		if pipMaxSizeH-h > pipPositionErrorMarginPX {
			return errors.Wrapf(err, "invalid height; expected=%d, actual=%d (error margin=%d pixels)", pipMaxSizeH, h, pipPositionErrorMarginPX)
		}
	} else {
		if pipMaxSizeW-w > pipPositionErrorMarginPX {
			return errors.Wrapf(err, "invalid width; expected=%d, actual=%d (error margin=%d pixels)", pipMaxSizeW, w, pipPositionErrorMarginPX)
		}
	}
	return nil
}
...
// Assumes the shelf is placed at the bottom.
func testPIPFling(ctx context.Context, cr *chrome.Chrome, act *arc.Activity, dev *ui.Device) error {
	type borderType int
	const (
	...
	for _, dir := range []struct {
		...
		case bottom:
			...
			if bounds.Bottom >= shelfTopPX || shelfTopPX-bounds.Bottom > pipWorkAreaInsetsPX {
				return errors.Errorf("failed swipe to bottom; expected boundsBottom %d <= %d < %d",
...
func testPIPToggleTabletMode(ctx context.Context, cr *chrome.Chrome, act *arc.Activity, dev *ui.Device) error {
	tconn, err := cr.TestAPIConn(ctx)
	if err != nil {
		return errors.Wrap(err, "failed to get a Test API connection")
	}
