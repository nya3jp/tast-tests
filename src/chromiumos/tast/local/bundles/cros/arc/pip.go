...
func Pip(ctx context.Context, s *testing.State) {
	...
	for _, tabletMode := range []bool{false, true} {
		...
		if tm != tabletMode {
			if err := setTabletModeEnabled(ctx, tconn, tabletMode); err != nil {
				s.Fatalf("Failed to set tablet mode enabled to %t", tabletMode)
			}
		}
		...
		for idx, entry := range []struct {
			...
			{"PIP AvoidObstacle", testPIPAvoidObstacle},
	...
	if err := setTabletModeEnabled(ctx, tconn, tabletModeEnabled); err != nil {
		testing.ContextLogf(ctx, "Failed to restore tablet mode enabled = %t", tabletModeEnabled)
	}
}
...
func testPIPShelfAutoHide(ctx context.Context, cr *chrome.Chrome, tconn *chrome.Conn, act *arc.Activity, d *ui.Device) error {
	...
	if bounds.Top < origBounds.Top+pixelsToMove {
		testing.ContextLogf(ctx, "PiP failed to move %d pixels. Original position: %+v, final position: %+v", bounds.Top-origBounds.Top, origBounds, bounds)
		return errors.Errorf("PiP failed to move %d pixels; original position: %+v, final position: %+v", bounds.Top-origBounds.Top, origBounds, bounds)
	}
...
func testPIPMove(ctx context.Context, cr *chrome.Chrome, tconn *chrome.Conn, act *arc.Activity, d *ui.Device) error {
	...
	// The value varies depending on acceleration/speed of the gesture. 100 works for our purpose.
	const marginError = 100
	...
	for i := 0; i < 3; i++ {
		...
		if diff > marginError {
			return errors.Wrapf(err, "invalid PiP bounds: %+v; expected %d < %d", bounds, int(diff), marginError)
		}
		bounds = newBounds
	}
	...
	for _, dir := range []struct {
		...
		case left:
			if bounds.Left > borderOffset {
				testing.ContextLogf(ctx, "Failed swipe to left. Expected: left border < %d. Actual: %d", borderOffset, bounds.Left)
				return errors.Errorf("failed swipe to left; expected < %d, actual %d", borderOffset, bounds.Left)
			}
		case right:
			if bounds.Right < dispW-borderOffset {
				testing.ContextLogf(ctx, "Failed swipe to right. Expected: right border < %d. Actual: %d", dispW-borderOffset, bounds.Right)
				return errors.Errorf("failed swipe to right; expected < %d, actual %d", dispW-borderOffset, bounds.Right)
			}
		case top:
			if bounds.Top > borderOffset {
				testing.ContextLogf(ctx, "Failed swipe to top. Expected: top border <%d. Actual: %d", borderOffset, bounds.Top)
				return errors.Errorf("failed swipe to top; expected < %d, actual %d", borderOffset, bounds.Top)
			}
		case bottom:
...
func testPIPToggleTabletMode(ctx context.Context, cr *chrome.Chrome, tconn *chrome.Conn, act *arc.Activity, d *ui.Device) error {
	...
	if origBounds != bounds {
		testing.ContextLogf(ctx, "Wrong position after toggling tablet mode. Expected: %+v, actual: %+v", origBounds, bounds)
		return errors.Errorf("wrong position after toggling tablet mode; expected: %+v, actual: %+v", origBounds, bounds)
	}
...
func waitForPIPReady(ctx context.Context, timeout time.Duration) error {
	// FIXME(ricardoq): Find a robust way to wait for "PiP is ready" condition.
	// Calling WaitForIdle() or WaitForWindowUpdate() does not work since PiP windows are not the "current windows".
	select {
