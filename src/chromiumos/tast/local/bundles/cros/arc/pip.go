...
func testPIPMove(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, pipAct *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode) error {
	...
	for i := 0; i < totalMovements; i++ {
		...
		if err := movePIPWindow(ctx, tconn, movementDuration, newBounds, movedBounds, dispMode.DeviceScaleFactor); err != nil {
...
func movePIPWindow(ctx context.Context, tconn *chrome.TestConn, t time.Duration, toBounds, fromBounds coords.Rect, dsf float64) error {
...
func testPIPResizeToMax(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, pipAct *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode) error {
	...
	if err := resizePIPWindow(ctx, tconn, arc.BorderTopLeft, coords.NewPoint(0, 0), time.Second, bounds, 5, displaySize, dispMode.DeviceScaleFactor); err != nil {
...
func resizePIPWindow(ctx context.Context, tconn *chrome.TestConn, border arc.BorderType, to coords.Point, t time.Duration, bounds coords.Rect, borderOffset int, displaySize coords.Size, dsf float64) error {
