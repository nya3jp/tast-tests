	...
	for _, tabletMode := range []bool{false, true} {
		...
		for _, multiActivityPip := range []bool{false, true} {
			...
			for idx, test := range []struct {
				...
				{name: "PIP Toggle Tablet mode", fn: testPIPToggleTabletMode, initMethod: enterPip, pipType: all},
...
func testPIPToggleTabletMode(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, pipAct *arc.Activity, maPipBaseAct *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode) error {
	info, err := ash.GetARCAppWindowInfo(ctx, tconn, pkgName)
	if err != nil {
		return errors.Wrap(err, "could not get PIP window bounds")
	}
...
func testPIPAutoPIPNewAndroidWindow(ctx context.Context, tconn *chrome.Conn, a *arc.ARC, pipAct *arc.Activity, maPipBaseAct *arc.Activity, dev *ui.Device, dispMode *display.DisplayMode) error {
	...
	return testing.Poll(ctx, func(ctx context.Context) error {
		state, err := pipAct.GetWindowState(ctx)
		if err != nil {
