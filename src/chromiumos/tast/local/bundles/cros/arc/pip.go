...
func PIP(ctx context.Context, s *testing.State) {
	...
	enableMultiActivityPIP := []bool{false, true}
...
func dragWithPause(ctx context.Context, tconn *chrome.TestConn, from, to coords.Point, t time.Duration) error {
	...
	if err := testing.Sleep(ctx, time.Millisecond*300); err != nil {
		return err
	}
...
func minimizePIP(ctx context.Context, tconn *chrome.TestConn, pipAct *arc.Activity) error {
	if _, err := ash.SetARCAppWindowState(ctx, tconn, pipTestPkgName, ash.WMEventMinimize); err != nil {
