...
func MultiNetworkingConnectivity(ctx context.Context, s *testing.State) {
	...
	for _, device := range response.Devices {
		if device.GuestType != pp.NetworkDevice_ARCVM && device.GuestType != pp.NetworkDevice_ARCVM {
			continue
		}

		if ifup, err := interfaceUp(ctx, device.PhysIfname); err != nil {
			s.Errorf("Failed checking if %s is up: %s", device.PhysIfname, err)
			continue
		} else if !ifup {
			continue
		}
		...
		arcIP := make(net.IP, 4)
		arcIP[0] = byte(device.Ipv4Addr)
		arcIP[1] = byte(device.Ipv4Addr >> 8)
		arcIP[2] = byte(device.Ipv4Addr >> 16)
		arcIP[3] = byte(device.Ipv4Addr >> 24)
