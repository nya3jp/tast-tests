...
func parseReadClocksOutput(output []byte) (*arcpb.ClockValues, error) {
	...
	res.ClockMonotonicNs = clocks.Mono.Seconds*int64(time.Second) + clocks.Mono.NanoSeconds
	res.ClockBoottimeNs = clocks.Boot.Seconds*int64(time.Second) + clocks.Boot.NanoSeconds
	return res, nil
}
