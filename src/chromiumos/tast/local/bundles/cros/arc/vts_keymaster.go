...
func VTSKeymaster(ctx context.Context, s *testing.State) {
	...
	testExecName, err := vtsTestExecName(ctx, a, isARCVM(s))
	if err != nil {
...
// isARCVM returs true if the test software dependencies include "android_vm".
func isARCVM(s *testing.State) bool {
	isARCVM := false
