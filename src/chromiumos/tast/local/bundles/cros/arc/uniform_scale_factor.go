...
func UniformScaleFactor(ctx context.Context, s *testing.State) {
	...
	if err := perappdensity.VerifyPixelCount(ctx, cr, a, color.Black, squarePixelCount, viewAct); err != nil {
	...
	if err := perappdensity.VerifyPixelCount(ctx, cr, a, color.RGBA{255, 0, 0, 255}, squarePixelCount, secondAct); err != nil {
