...
func InputOverlayEditor(ctx context.Context, s *testing.State) {
	inputoverlay.SetupTestApp(ctx, s, func(params inputoverlay.TestParams) error {
		...
		if err := uiauto.Combine("Mappings changed to illegal keys",
			...
			uda.WaitUntilExists(uidetection.Word("Unsupported")),
			// Change mapping of "w" to "w".
			kb.TypeAction("w"),
			// Verify illegal mapping.
			uda.WaitUntilExists(uidetection.Word("Duplicated")),
		...
		if err := uiauto.Combine("Mapping unbound",
			...
			inputoverlay.TapOverlayButton(kb, "m", &params, inputoverlay.BotTap),
		)(ctx); err != nil {
...
func closeAndReopen(params *inputoverlay.TestParams) action.Action {
	return func(ctx context.Context) error {
		...
		act, err := inputoverlay.RelaunchActivity(params)
		if err != nil {
