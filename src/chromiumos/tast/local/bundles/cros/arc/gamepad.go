...
func Gamepad(ctx context.Context, s *testing.State) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			...
			return errors.Errorf("unexpected number of gamepad devices: got: %v; want 1", len(devices))
		}
	...
	if err := gp.Button(ctx, input.BTN_EAST); err != nil {
		s.Fatal("Failed to press button: ", err)
	}
	if err := gp.Button(ctx, input.BTN_SOUTH); err != nil {
		s.Fatal("Failed to press button: ", err)
	}
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if actualEvents, err = getKeyEvents(ctx, d); err != nil {
			...
			return errors.Errorf("unexpected number of gamepad events: got: %d; want: %d", len(actualEvents), len(expectedEvents))
		}
		return nil
	}, nil); err != nil {
