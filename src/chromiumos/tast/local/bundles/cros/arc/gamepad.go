...
type inputDevice struct {
	...
	SupportedKeyCodes map[int]bool `json:"supported_key_codes"`
}
...
func verifyGamepadDeviceInfo(s *testing.State, gp *input.GamepadEventWriter, d *inputDevice) {
	...
	supportedKeyCodeMapping := map[ui.KeyCode]bool{
		ui.KEYCODE_BUTTON_A:      true,  // KEYCODE_BUTTON_A
		ui.KEYCODE_BUTTON_B:      true,  // KEYCODE_BUTTON_B
		ui.KEYCODE_BUTTON_START:  true,  // KEYCODE_BUTTON_START
		ui.KEYCODE_BUTTON_SELECT: true,  // KEYCODE_BUTTON_SELECT
		ui.KEYCODE_BUTTON_MODE:   true,  // KEYCODE_BUTTON_MODE
		ui.KEYCODE_A:             false, // KEYCODE_A
		ui.KEYCODE_B:             false, // KEYCODE_B
	}
	...
	for keyCode, isSupported := range d.SupportedKeyCodes {
		expectSupport, ok := supportedKeyCodeMapping[ui.KeyCode(keyCode)]
		if !ok {
			s.Errorf("Keycode mapping for %d not found", keyCode)
		} else if isSupported != expectSupport {
			s.Errorf("Keycode support does not match for %d: got %t; want %t", keyCode, isSupported, expectSupport)
		}
	}
	return
}
