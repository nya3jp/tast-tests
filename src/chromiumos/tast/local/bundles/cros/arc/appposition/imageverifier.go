...
func newShadowedColorPredicate(expectedColor color.Color) (shadowedColorPredicate, error) {
	...
	if g > 0 {
		if component != "?" {
			return shadowedColorPredicate{}, errors.Errorf("%#v has more then 1 components", expectedColor)
		}
		component = "G"
		value = g
	}
...
// VerifyContent checks wheather the given area of the screenshot is the content of the app.
func VerifyContent(s *testing.State, img image.Image, contentBounds coords.Rect) {
	...
	if err != nil {
		s.Error("Failed to make shadowed color predicate: ", err)
	} else {
		verifyRegion(s, img, contentBounds, predicate)
	}
}

// VerifyShadow checks wheather the given areas of the screenshot is the shadow of the window of the app.
...
// VerifyCaption checks wheather the given area of the given screenshot is the caption of the window of the app.
