...
func verifyRegion(s *testing.State, img image.Image, rect coords.Rect, pred ColorPredicate) {
...
TODO
    ...
    assert (rect.bottom <= image_height
            and rect.right <= image_width), ('Region %s exceeds image '
                                             + 'size %s') % (rect, image.size)
        ...
        for x in xrange(rect.left, rect.right):
            ...
            if not predicate(color):
                raise error.TestFail(
                    'Pixel color at (%d, %d) is not valid: %s. Expected: %s' % (
                     x, y, color, predicate.expectedColor))
*/
}

/*
TODO
class ShadowedColorPredicate:
    ...
    black.
    """
    def __init__(self, expectedColor):
...
func VerifyContent(s *testing.State, img image.Image, contentBounds coords.Rect) {
    ...
    // TODO
    // verifyRegion(s, img, contentBounds, ShadowedColorPredicate(contentColor))
}


func VerifyShadow(s *testing.State, img image.Image, shadowBounds coords.Rect) {
/*
TODO
    for bound in shadowBounds:
        logging.info('Verifying shadow color in %s.', bound)
        verifyRegion(s, 
            img, bound, ShadowedColorPredicate(getShadowedBackgroundColor()))
*/
}
...
func (pred CaptionColorPredicate) call(c color.Color) bool {
	r, g, b, _ := c.RGBA()
	return r > 0 && g > 0 && b > 0 && max(r, max(g, b)) - min(r, min(g, b)) < captionColorComponentThreshold
}

func (pred CaptionColorPredicate) expectedColor() string {
