...
func (predicate shadowedColorPredicate) call(c color.Color) bool {
	const tolerance = colorErrorTolerance * 0x0101
	...
	r2, g2, b2, _ := predicate.expectedColor.RGBA()
	if (r2 > tolerance) && !(0 < r1 && int64(r1) <= int64(r2)+tolerance) {
		return false
	}
	if (g2 > tolerance) && !(0 < g1 && int64(g1) <= int64(g2)+tolerance) {
		return false
	}
	if (b2 > tolerance) && !(0 < b1 && int64(b1) <= int64(b2)+tolerance) {
		return false
	}
	return true
}
