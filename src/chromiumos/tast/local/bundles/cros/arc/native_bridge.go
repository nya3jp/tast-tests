...
func init() {
	testing.AddTest(&testing.Test{
		Func:         NativeBridge,
		Desc:         "Checks whether native bridge is properly setup for ARCVM",
		Contacts:     []string{"youkichihosoi@chromium.org", "arcvm-eng@google.com"},
		Data:         []string{"hello_world_arm", "hello_world_arm64"},
		SoftwareDeps: []string{"android_vm_r", "chrome", "native_bridge"},
...
func NativeBridge(ctx context.Context, s *testing.State) {
	...
	// Check whether binfmt_misc is successfully unmounted.
	...
	// Check whether the property for native bridge library is properly set.
	...
	if nb != "libhoudini.so" && nb != "libndk_translation.so" {
		s.Fatalf("Failure: the property %q is set to %q", nativeBridgeProp, nb)
	}

	// Check whether an ARM 32-bit executable can be run successfully.
	if err := pushAndExecute(ctx, a, s.DataPath("hello_world_arm")); err != nil {
		s.Fatal("Failed to run ARM executable: ", err)
	}

	// Check whether 64-bit native bridge is supported.
	// TODO(youkichihosoi): update the condition once a dedicated property for 64-bit support is implemented.
	...
	if abi == "x86_64,arm64-v8a" {
		// Check whether an ARM 64-bit executable can be run successfully.
