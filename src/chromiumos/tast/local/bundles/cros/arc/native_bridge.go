...
const (
	...
	// Names of the binarty translation libraries.
	...
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			Val: nativeBridgeConfig{
				...
				chromeExtraArgs:          []string{"--enable-arcvm", "--disable-features=" + nativeBridgeExperimentFeature},
			},
			...
			Val: nativeBridgeConfig{
				...
				chromeExtraArgs:          []string{"--enable-arcvm", "--enable-features=" + nativeBridgeExperimentFeature},
			},
			ExtraData:         []string{armExec},
...
func NativeBridge(ctx context.Context, s *testing.State) {
	cr, err := chrome.New(ctx, chrome.ARCEnabled(),
		chrome.ExtraArgs(s.Param().(nativeBridgeConfig).chromeExtraArgs...))
	if err != nil {
	...
	nbExpected := s.Param().(nativeBridgeConfig).binaryTranslationLibrary
	if nb != nbExpected {
	...
	for _, exec := range s.Param().(nativeBridgeConfig).armExecutables {
