...
type densityChange struct {
	...
	keySequence string
	// The expected dpi after performing the current action.
	densityDpi int
}
...
func performAndConfirmDensityChange(ctx context.Context, cr *chrome.Chrome, ew *input.KeyboardEventWriter, a *arc.ARC, test densityChange) error {
	...
	if err := ew.Accel(ctx, test.keySequence); err != nil {
		return errors.Wrapf(err, "could not %q scale factor", test.keySequence)
	}
	if err := checkBlackPixels(ctx, cr, test.densityDpi); err != nil {
		return errors.Wrap(err, "could not check number of black pixels")
	}
	return nil
}

// checkBlackPixels checks that number of black pixels is equal to dpi*dpi.
func checkBlackPixels(ctx context.Context, cr *chrome.Chrome, dpi int) error {
	wantPixelCount := dpi * dpi
	// Need to wait for relayout to complete, before grabbing new screenshot.
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		if gotPixelCount, err := rectBounds(ctx, cr); err != nil {
			return err
		} else if gotPixelCount != wantPixelCount {
...
// rectBounds grabs a screenshot, and returns the number of black pixels.
// -1 is returned as the pixel count if there was an error in grabbing the screenshot.
func rectBounds(ctx context.Context, cr *chrome.Chrome) (int, error) {
...
func PerAppDensity(ctx context.Context, s *testing.State) {
	const (
		...
		initialDpi   = 150
		increasedDpi = initialDpi * 1.1
		decreasedDpi = initialDpi * 0.9
	)
	...
	if err := ash.WaitForARCAppWindowState(ctx, cr, packageName, arc.WindowStateFullscreen); err != nil {
		s.Fatal("Failed to wait for the activity to be Maximized: ", err)
	}
	if err := checkBlackPixels(ctx, cr, initialDpi); err != nil {
