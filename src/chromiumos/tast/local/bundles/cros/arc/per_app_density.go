...
func performAndConfirmDensityChange(ctx context.Context, cr *chrome.Chrome, ew *input.KeyboardEventWriter, a *arc.ARC, name string, keySequence string, blackPixelCount float64) error {
	...
	if err := checkBlackPixels(ctx, cr, int(blackPixelCount)); err != nil {
...
func checkBlackPixels(ctx context.Context, cr *chrome.Chrome, wantPixelCount int) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		testing.ContextLog(ctx, "gotPixelCount: ", gotPixelCount)
		if gotPixelCount != wantPixelCount {
...
func PerAppDensity(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Could not get display density: ", err)
	}
	expectedInitialPixelCount := (physicalDisplayDensity * rectWidth) * (physicalDisplayDensity * rectWidth)
