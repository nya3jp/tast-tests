...
func performAndConfirmDensityChange(ctx context.Context, cr *chrome.Chrome, ew *input.KeyboardEventWriter, a *arc.ARC, name string, keySequence string, blackPixels float64) error {
	...
	if err := checkBlackPixels(ctx, cr, blackPixels); err != nil {
...
func checkBlackPixels(ctx context.Context, cr *chrome.Chrome, wantPixelCount float64) error {
	const delta = 0.01
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if gotPixelCount := screenshot.CountPixels(img, color.Black); math.Abs(float64(gotPixelCount)-wantPixelCount) > delta {
			return errors.Errorf("wrong number of black pixels, got: %d, want: %f", gotPixelCount, wantPixelCount)
		}
		return nil
...
func PerAppDensity(ctx context.Context, s *testing.State) {
	...
	expectedInitialPixelCount := (displayDensity * rectWidth) * (displayDensity * rectWidth)
	if err := checkBlackPixels(ctx, cr, expectedInitialPixelCount); err != nil {
	...
	for _, test := range []densityChange{
		...
		if err := performAndConfirmDensityChange(ctx, cr, ew, a, test.name, test.keySequence, test.blackPixelCount); err != nil {
