...
func performAndConfirmDensityChange(ctx context.Context, cr *chrome.Chrome, ew *input.KeyboardEventWriter, a *arc.ARC, name string, keySequence string, blackPixels int) error {
...
func checkBlackPixels(ctx context.Context, cr *chrome.Chrome, wantPixelCount int) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		diff := float64((wantPixelCount - gotPixelCount) / wantPixelCount)
		if diff > 0.01 {
