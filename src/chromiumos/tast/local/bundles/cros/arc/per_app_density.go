...
func performAndConfirmDensityChange(ctx context.Context, cr *chrome.Chrome, ew *input.KeyboardEventWriter, a *arc.ARC, test densityChange) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		if err := checkRectBounds(ctx, cr, test.densityDpi); err != nil {
			return err
		}
		return nil
	}, &testing.PollOptions{Timeout: 10 * time.Second}); err != nil {
		return errors.Wrap(err, "timed out waiting for updated scrren after changing density")
	}
	return nil
}

// checkRectBounds confirms that the square drawn on screen is the expected size.
// The size of the square is specified by densityDpi.
func checkRectBounds(ctx context.Context, cr *chrome.Chrome, densityDpi int) error {
	// The size of the drawn square.
	wantPixelCount := densityDpi * densityDpi
	if img, err := screenshot.GrabScreenshot(ctx, cr); err != nil {
		return err
	} else if gotPixelCount := screenshot.CountPixels(img, color.Black); gotPixelCount != wantPixelCount {
...
func PerAppDensity(ctx context.Context, s *testing.State) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		if err := checkRectBounds(ctx, cr, initialDpi); err != nil {
			return err
		}
		return nil
	}, &testing.PollOptions{Timeout: 10 * time.Second}); err != nil {
		s.Fatal("Timed out waiting for updated scrren after changing density: ", err)
	}
