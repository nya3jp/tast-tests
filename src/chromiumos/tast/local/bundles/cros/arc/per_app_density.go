...
func PerAppDensity(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Failed to get displaymode: ", err)
	}
	...
	widthHeightRatio := float64(dispMode.Width) / float64(dispMode.Height)
	expectedInitialPixelCount := (widthHeightRatio * rectWidth) * (widthHeightRatio * rectWidth)
