...
func performAndConfirmDensityChange(ctx context.Context, cr *chrome.Chrome, ew *input.KeyboardEventWriter, a *arc.ARC, test densityChange) error {
	testing.ContextLogf(ctx, "%s density using key %q", test.name, test.keySequence)
...
// checkBlackPixels grabs a screenshots and checks that number of black pixels is equal to dpi*dpi.
