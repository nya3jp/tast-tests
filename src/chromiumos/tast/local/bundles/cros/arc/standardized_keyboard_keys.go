...
func (eck *keyboardKeyPress) Press(ctx context.Context, kbd *input.KeyboardEventWriter) (bool, error) {
	// Load in the top row layout.
	topRow, err := input.KeyboardTopRowLayout(ctx, kbd)
	if err != nil {
		return true, errors.Wrap(err, "failed to load the top-row layout")
	}
...
func runStandardizedKeyboardKeysTest(ctx context.Context, s *testing.State, testParameters standardizedtestutil.TestFuncParams) {
	...
	for _, curTestKey := range allTestKeys {
		...
		keySkipped, err := curTestKey.keyPress.Press(ctx, kbd)
		if err != nil {
