...
func RunTest(ctx context.Context, s *testing.State, activities []TestActivity, f func(context.Context, *arc.ARC, *chrome.Conn, *chrome.TestConn, *input.KeyboardEventWriter, TestActivity) error) {
	fullCtx := ctx
	...
	for _, activity := range activities {
		s.Run(ctx, activity.Name, func(ctx context.Context, s *testing.State) {
			...
			if err := f(ctx, a, cvconn, tconn, ew, activity); err != nil {
