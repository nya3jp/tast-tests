...
func waitForSpokenFeedbackReady(ctx context.Context, cr *chrome.Chrome, a *arc.ARC) (*accessibility.ChromeVoxConn, error) {
	...
	cvconn, err := accessibility.NewChromeVoxExtConn(ctx, cr)
	if err != nil {
...
func RunTest(ctx context.Context, s *testing.State, activities []TestActivity, f func(context.Context, *accessibility.ChromeVoxConn, *chrome.TestConn, TestActivity) error) {
	...
	if err := accessibility.SetFeatureEnabled(ctx, tconn, accessibility.SpokenFeedback, true); err != nil {
		s.Fatal("Failed to enable spoken feedback: ", err)
	}
	defer func() {
		if err := accessibility.SetFeatureEnabled(fullCtx, tconn, accessibility.SpokenFeedback, false); err != nil {
	...
	for _, activity := range activities {
		s.Run(ctx, activity.Name, func(ctx context.Context, s *testing.State) {
			...
			defer faillog.DumpUITreeOnErrorToFile(ctx, s.OutDir(), s.HasError, &chrome.TestConn{Conn: cvconn.ChromeConn()}, "ui_tree"+activity.Name+".txt")
