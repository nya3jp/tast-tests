...
func waitForSpokenFeedbackReady(ctx context.Context, cr *chrome.Chrome, a *arc.ARC) (*chrome.Conn, error) {
	...
	cvconn, err := accessibilityutils.WaitForSpokenFeedbackReady(ctx, cr)
	return cvconn, err
}
...
func RunTest(ctx context.Context, s *testing.State, activities []TestActivity, f func(context.Context, *chrome.Conn, *chrome.TestConn, TestActivity) error) {
	...
	if err := accessibilityutils.SetFeatureEnabled(ctx, tconn, accessibilityutils.SpokenFeedback, true); err != nil {
		s.Fatal("Failed to enable spoken feedback: ", err)
	}
	defer func() {
		if err := accessibilityutils.SetFeatureEnabled(fullCtx, tconn, accessibilityutils.SpokenFeedback, false); err != nil {
	...
	for _, activity := range activities {
		s.Run(ctx, activity.Name, func(ctx context.Context, s *testing.State) {
			...
			if err := func() error {
				if err = accessibilityutils.WaitForFocusedNode(ctx, cvconn, tconn, &ui.FindParams{
					Name: activity.Title,
