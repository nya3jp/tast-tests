...
// WaitFocusedNode polls until the properties of the focused node matches expectedNode.
// Returns an error after 30 seconds.
func WaitForFocusedNode(ctx context.Context, chromeVoxConn *chrome.Conn, expectedNode AutomationNode) error {
	// Wait for focusClassName to receive focus.
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		node, err := GetFocusedNode(ctx, chromeVoxConn)
		if err != nil {
			return err
		} else if strings.TrimSpace(node.ClassName) != expectedNode.ClassName {
			return errors.Errorf("%q does not have focus, %q has focus instead", node.ClassName, expectedNode.ClassName)
		}

		if diff := cmp.Diff(node, expectedNode, cmpopts.EquateEmpty()); diff != "" {
			return errors.Errorf("node was not as expected. Got, want:%s", diff)
		}
		return nil
	}, &testing.PollOptions{Timeout: 30 * time.Second}); err != nil {
