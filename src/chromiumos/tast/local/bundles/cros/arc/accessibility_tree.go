...
// dumpArcAXNodeParams writes the given arcAXNodeParams to thye file specified by filepath.
func dumpArcAXNodeParams(ctx context.Context, tree arcAXNodeParams, filepath string) error {
	f, err := os.Create(filepath)
	if err != nil {
		return err
	}
	...
	testFunc := func(ctx context.Context, cvconn *chrome.Conn, tconn *chrome.TestConn, currentActivity accessibility.TestActivity) error {
		...
		if err != nil || !matchFound {
			nestedErr := func() error {
				...
				if err := dumpArcAXNodeParams(ctx, currentActivityTree, wantFilePath); err != nil {
