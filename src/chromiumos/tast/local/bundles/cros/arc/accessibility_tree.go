...
var axTreeTestActivities = []string{accessibility.MainActivity, accessibility.EditTextActivity}
...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:         []string{"accessibility_tree_expected_MainActivity.json", "accessibility_tree_expected_EditTextActivity.json"},
	...
	const script = `
		new Promise((resolve, reject) => {
			chrome.automation.getDesktop((root) => {
				let instance = LogStore.getInstance();
				instance.clearLog();
...
func AccessibilityTree(ctx context.Context, s *testing.State) {
	accessibility.RunTest(ctx, s, axTreeTestActivities, func(ctx context.Context, a *arc.ARC, cvconn *chrome.Conn, tconn *chrome.TestConn, ew *input.KeyboardEventWriter, activityName string) error {
		...
		outFileName := axTreeActualTreeFilePrefix + activityName + ".json"
		outFilePath := filepath.Join(s.OutDir(), outFileName)

		// Need to poll until the correct tree can be retrieved.
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			...
			if appRoot == nil || !ok {
				...
				return errors.Errorf("failed to get Android root from accessibility tree, wrote the entire tree to %q", outFileName)
			}
			return nil
		}, &testing.PollOptions{Timeout: 30 * time.Second}); err != nil {
			s.Fatal("Timed out waiting for desktop tree: ", err)
		}

		if diff := cmp.Diff(appRoot, expected, cmpopts.EquateEmpty()); diff != "" {
			diffFileName := axTreeDiffFilePrefix + activityName + ".txt"
			...
			return errors.Errorf("accessibility tree did not match (see diff:%s, actual:%s)", diffFileName, outFileName)
		}
		return nil
	})
}
