...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Checks Chrome accessibility tree for ARC application is correct",
...
// readTree reads a tree from a input file, specified by treePath.
...
func compareTrees(wantTree, gotTree, outputFilePath string) error {
	...
	for i, wantLine := range wantTreeBody {
		if matched, err := regexp.MatchString(regexp.QuoteMeta(wantLine)+".*", gotTreeRemoved[i]); err != nil {
			return err
		} else if !matched {
	...
	if len(diff) > 0 {
		if err := ioutil.WriteFile(outputFilePath, []byte(strings.Join(diff, "\n")), 0644); err != nil {
			return errors.Wrapf(err, "failed to write to %q: %v", outputFilePath)
		}
...
func AccessibilityTree(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Failed to start Chrome with accessibility: ", err)
	}
	defer cr.Close(ctx)
