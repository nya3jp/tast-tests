...
// Error indicates an internal failure, such as conencting to Chrome or invoking the JavaScript.
func matchTree(ctx context.Context, gotRoot *ui.Node, actualRoot arcAXNodeParams) (bool, error) {
	// Check the root node.
	if found, err := gotRoot.Matches(ctx, actualRoot.FindParams); err != nil {
		return false, err
	} else if !found {
		return false, errors.Errorf("could not find node %q", actualRoot.FindParams)
	}
...
func AccessibilityTree(ctx context.Context, s *testing.State) {
	...
	trees := make(map[string]arcAXNodeParams)
