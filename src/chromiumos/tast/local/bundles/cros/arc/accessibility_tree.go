...
// findParams constructs arca11y.FindParams from the given axTreeNode.
func (n *axTreeNode) findParams() arca11y.FindParams {
	return arca11y.FindParams{Name: n.Name, Role: n.Role, Attributes: n.Attributes}
}
...
func matchTree(ctx context.Context, actualRoot *arca11y.Node, expectedRoot *axTreeNode) (bool, error) {
	// Check the root node.
...
func AccessibilityTree(ctx context.Context, s *testing.State) {
	...
	testFunc := func(ctx context.Context, cvconn *a11y.ChromeVoxConn, tconn *chrome.TestConn, currentActivity arca11y.TestActivity) error {
		...
		if appRoot, err = arca11y.FindWithTimeout(ctx, tconn, arca11y.FindParams{Name: currentActivity.Title, Role: role.Application}, 10*time.Second); err != nil {
