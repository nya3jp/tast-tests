...
func compareSubtrees(ctx context.Context, extracted []string, subtree []string, outputFilePath string) error {
	...
	for i, wantLine := range extracted {
		testing.ContextLog(ctx, "extracted: "+strings.Trim(string(extracted[i]), "\n")+", wantLine:"+strings.Trim(string(wantLine), "\n"))
...
// wantTree and gotTree, want to see if wantTree is contained inside of gotTree
func containsSubtree(ctx context.Context, tree, subtree, outputFilePath string) error {
	...
	return compareSubtrees(ctx, strings.Split(tree, "\n")[1:], extracted, outputFilePath)
}

// extractSubtree, checks if the tree specified by wantTreeHeader exists,
...
func extractSubtree(gotTree, wantTreeHeader string) ([]string, bool) {
	...
	// Otherwise return the matching component
...
func checkAccessibilityTree(ctx context.Context, chromeVoxConn *chrome.Conn, wantFilePath, outputFilePath string) error {
	...
	if err := containsSubtree(ctx, wantTree, gotTree, outputFilePath); err != nil {
