...
func getDesktopTree(ctx context.Context, cvconn *chrome.Conn) (*ui.Node, error) {
	var root ui.Node
	...
	//testing.ContextLogf(ctx, "rootNode: %q", root)
	return &root, err
}
...
func AccessibilityTree(ctx context.Context, s *testing.State) {
	...
	testFunc := func(ctx context.Context, cvconn *chrome.Conn, tconn *chrome.TestConn, currentActivity accessibility.TestActivity) error {
		...
		// Recursively look into children of appRoot.
		...
		if nodeNotFound, err := checkTreeRecursive(ctx, appRoot, currentActivityTree.Children); err != nil {
			...
			if err := dumpTree(&nodeNotFound, diffFilePath); err != nil {
				return errors.Wrap(err, "accessibility tree did not match; failed to dump the actual tree")
			}
			// Also dump the node that we could not find.
