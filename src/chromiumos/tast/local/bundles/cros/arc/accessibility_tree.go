...
func AccessibilityTree(ctx context.Context, s *testing.State) {
	...
	if appRoot == nil || !ok {
		...
		s.Fatalf("Failed to get Android application root from accessibility tree, wrote the entire tree to %q", filepath.Base(outFilePath))
	}
	...
	if diff := cmp.Diff(appRoot, expected, cmpopts.EquateEmpty()); diff != "" {
		...
		s.Fatalf("Accessibility tree did not match (see %s)", filepath.Base(outFilePath))
	}
}
