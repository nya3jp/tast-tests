...
var streamInfoRex = regexp.MustCompile("(.*):(.*)")

func newStreamInfo(s string) (*StreamInfo, error) {
	data := streamInfoRex.FindAllStringSubmatch(s, -1)
...
// pollSomeStreams returns error if it fails to detect any active streams.
func (t *ARCAudioTast) pollSomeStreams(ctx context.Context) ([]StreamInfo, error) {
	...
	err := testing.Poll(ctx, hasStreams, &testing.PollOptions{Timeout: hasStreamsTimeout})
	return streams, err
}

// pollNoStreams returns error if it fails to wait for all active streams to stop.
func (t *ARCAudioTast) pollNoStreams(ctx context.Context) error {
	noStreams := func(ctx context.Context) error {
	...
	return testing.Poll(ctx, noStreams, &testing.PollOptions{Timeout: noStreamsTimeout})
}
...
func (t *ARCAudioTast) startPollStreamWorker(ctx context.Context) (<-chan []StreamInfo, <-chan error) {
	resCh := make(chan []StreamInfo, 1)
	errCh := make(chan error, 1)
	go func() {
		streams, err := t.pollSomeStreams(ctx)
		errCh <- err
		resCh <- streams
	}()
	return resCh, errCh
...
func (t *ARCAudioTast) RunAppAndPollStream(ctx context.Context, apkPath string, param TestParameters) ([]StreamInfo, error) {
	...
	if err := t.pollNoStreams(ctx); err != nil {
	...
	resCh, errCh := t.startPollStreamWorker(ctx)
	...
	if err := <-errCh; err != nil {
		// Returns errCh, if errCh is not a noStreamError
		var e *noStreamError
		if !errors.As(err, &e) {
			return streams, err
		}
