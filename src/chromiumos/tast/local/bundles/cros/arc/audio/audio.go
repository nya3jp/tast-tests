...
func newStreamInfo(s string) (*StreamInfo, error) {
	...
	keys := []string{Direction, Effects, FrameRate, NumChannels}
	...
	frameRate, err := strconv.ParseInt(res[FrameRate], 10, 32)
	if err != nil {
		return nil, errors.Errorf("failed to parse StreamInfo::frame_rate: %s", res[FrameRate])
	}
	numChannels, err := strconv.ParseInt(res[NumChannels], 10, 8)
	if err != nil {
		return nil, errors.Errorf("failed to parse StreamInfo::num_channels: %s", res[NumChannels])
	}
