...
func wmRV21(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device) error {
	...
	// Get final display orientation.
	fDO, err := display.GetOrientation(ctx, tconn)
	if err != nil {
		return errors.Wrap(err, "failed to get final display orientation")
	}
	// Display should be landscape.
	if fDO.Type != display.OrientationLandscapePrimary {
		return errors.Errorf("invalid fianl display orientation; got: %q, want: landscape-primary", tDO.Type)
	}
	return nil
}
