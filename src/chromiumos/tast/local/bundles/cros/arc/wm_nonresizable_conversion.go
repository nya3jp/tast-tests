...
func wmNV21(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device) error {
	...
	// Compare activity bounds to make sure it covers primary display work area.
	...
	testing.Poll(ctx, func(ctx context.Context) error {
		...
		if newDO.Type != display.OrientationPortraitPrimary {
			return errors.Errorf("invalid display orientation, got: %q, want: portrait-primary", newDO.Type)
		}
		return nil
	...
	testing.Poll(ctx, func(ctx context.Context) error {
		...
		if newDO.Type != display.OrientationLandscapePrimary {
			return errors.Errorf("invalid display orientation, got: %q, want: landscape-primary", newDO.Type)
		}
		return nil
	}, &testing.PollOptions{Timeout: 5 * time.Second})
