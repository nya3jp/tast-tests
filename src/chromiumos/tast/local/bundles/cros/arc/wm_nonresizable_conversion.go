...
func WMNonresizableConversion(ctx context.Context, s *testing.State) {
	cleanup, err := wm.SetupAndRunTestCases(ctx, s, []wm.TestCase{
		wm.TestCase{
	...
	if err != nil {
		s.Fatal("Failed to setup and run tests: ", err)
	}
	defer cleanup()
}
...
func wmNV19(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device) error {
	...
	// Disable tablet mode.
	if _, err := ash.EnsureTabletModeEnabled(ctx, tconn, false); err != nil {
		return errors.Wrap(err, "failed to ensure if tablet mode is disabled")
	}
