...
func wmNV19(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *ui.Device) error {
	...
	// Store activity's original winodw info to be compared with after tablet mode disabled.
	windowInfoBeforeTableMode, err := ash.GetARCAppWindowInfo(ctx, tconn, wm.Pkg24)
	if err != nil {
		return nil
	}

	// Enable tablet mode, activity will go to Maximized state.
	...
	// Store activity's winodw info when tablet mode is enabled to make sure it is in maximized mode.
	...
	if err := wm.CheckLandscapeMaximizeWindowInTabletMode(ctx, tconn, windowInfoAtTableMode.TargetBounds); err != nil {
		return err
	}
