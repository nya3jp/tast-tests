...
// createImage creates a file for VFat image. The file contains size bytes of 0.
func createImage(size int64) (string, error) {
	f, err := ioutil.TempFile("", "vfat.img")
	if err != nil {
...
func attachLoopDevice(ctx context.Context, path string) (string, error) {
	if err := testexec.CommandContext(ctx, "losetup", "-f", path).Run(testexec.DumpLogOnError); err != nil {
		return "", errors.Wrap(err, "failed to find unused loop device")
	}

	b, err := testexec.CommandContext(ctx, "losetup", "-j", path).Output(testexec.DumpLogOnError)
	if err != nil {
...
func RunTest(ctx context.Context, s *testing.State, a *arc.ARC, testFile string) {
	const (
		imageSize = 65536 * 1024
		diskName  = "MyDisk"
	)
	expected, err := ioutil.ReadFile(s.DataPath(testFile))
	if err != nil {
		s.Fatalf("Failed to read data at %s: %v", testFile, err)
	}

	// Set up a filesystem image.
	image, err := createImage(imageSize)
	if err != nil {
