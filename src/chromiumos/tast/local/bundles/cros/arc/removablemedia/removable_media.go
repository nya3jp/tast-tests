...
func RunTest(ctx context.Context, s *testing.State, a *arc.ARC, testFile string) {
	...
	verifyReadFileViaVolumeProvider := func(uri, dumpPath string) error {
	...
	if err := verifyReadFileViaVolumeProvider(legacyURI, filepath.Join(s.OutDir(), testFile+"_legacy")); err != nil {
	...
	if err := verifyReadFileViaVolumeProvider(uri, filepath.Join(s.OutDir(), testFile)); err != nil {
