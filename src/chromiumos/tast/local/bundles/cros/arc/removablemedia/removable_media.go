...
func checkReadFileViaVolumeProvider(ctx context.Context, s *testing.State, a *arc.ARC, uri string, expected []byte, dumpPath string) error {
...
func RunTest(ctx context.Context, s *testing.State, a *arc.ARC, testFile string) {
	...
	if err := checkReadFileViaVolumeProvider(ctx, s, a, legacyURI, expected, filepath.Join(s.OutDir(), testFile+"_legacy")); err != nil {
	...
	if err := checkReadFileViaVolumeProvider(ctx, s, a, uri, expected, filepath.Join(s.OutDir(), testFile)); err != nil {
