...
func attachLoopDevice(ctx context.Context, path string) (string, error) {
	if err := testexec.CommandContext(ctx, "losetup", "-f", path, "--show").Run(testexec.DumpLogOnError); err != nil {
	...
	if err != nil {
		return "", errors.Wrap(err, "failed to list attached loop device(s)")
	}
