...
type params struct {
	ARCVM bool // Whether ARCVM is run
}
...
func getRootPID(isARCVM bool) (int, error) {
	if isARCVM {
		return 1, nil
	}
...
func readCPUSetInfo(ctx context.Context, path string, isARCVM bool) ([]byte, error) {
	...
	return testexec.CommandContext(ctx, "android-sh", "-c", fmt.Sprintf("cat %s", path)).Output(testexec.DumpLogOnError)
}

func testCPUSet(ctx context.Context, s *testing.State, a *arc.ARC) {
	s.Log("Running testCPUSet")

	initPID, err := getRootPID(s.Param().(params).ARCVM)
	if err != nil {
	...
	for _, t := range types {
		...
		out, err := readCPUSetInfo(ctx, path, s.Param().(params).ARCVM)
