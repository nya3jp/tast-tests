...
func readFinskyPrefs(ctx context.Context, a *arc.ARC, user string, virtioBlkDataEnabled bool) ([]byte, error) {
...
func waitForDailyHygieneDone(ctx context.Context, a *arc.ARC, user string, virtioBlkDataEnabled bool) (bool, error) {
	...
	return ok, testing.Poll(ctx, func(ctx context.Context) error {
		out, err := readFinskyPrefs(ctx, a, user, virtioBlkDataEnabled)
		if err != nil {
...
func PlayStorePersistent(ctx context.Context, s *testing.State) {
	...
	ok, err := waitForDailyHygieneDone(ctx, a, cr.NormalizedUser(), virtioBlkDataEnabled)
	if err != nil {
		if out, rerr := readFinskyPrefs(ctx, a, cr.NormalizedUser(), virtioBlkDataEnabled); rerr != nil {
