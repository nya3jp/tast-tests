...
func PlayAutoInstall(ctx context.Context, s *testing.State) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		// On ARCVM virtio-blk /data enabled devices, we mount and unmount the disk image every time we access Android's /data
		// to ensure that the changes from the Android side is reflected to the host side.
