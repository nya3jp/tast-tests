...
func PlayAutoInstall(ctx context.Context, s *testing.State) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		if _, err := os.Stat(paiListUnderHome); err != nil {
			if os.IsNotExist(err) {
				return errors.Errorf("paiLis %q is not created yet", paiListUnderHome)
			}
	...
	for _, doc := range strings.Split(string(data), "\n") {
		// Mark that app was not verified.
		if doc != "" {
			paiDocs[doc] = false
		}
	}
	...
	// Define required PAI list. Some boards might have extended set, however following must
	// exist on any board.
	requiredPaiDocs := []string{
	...
	// Verify that all required app from the minimal set are scheduled for installation.
	for _, requiredDoc := range requiredPaiDocs {
		if _, ok := paiDocs[requiredDoc]; ok {
			s.Logf("Required app %q is verified in the list", requiredDoc)
			paiDocs[requiredDoc] = true
		} else {
			s.Errorf("Required app %q was not found in the list. Server configuration might be outdated", requiredDoc)
		}
	}

	// Print leftover portion as board extra customization.
	for doc, verified := range paiDocs {
		if !verified {
			s.Logf("Found app %q outside of required list", doc)
