...
func IPv6Connectivity(ctx context.Context, s *testing.State) {
	...
	out, err := arc.BootstrapCommand(ctx, "/system/bin/ip", "-6", "addr", "show", "scope", "global").Output(testexec.DumpLogOnError)
	if err != nil {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		return arc.BootstrapCommand(ctx, "/system/bin/ping6", "-c1", "-w1", "2001:4860:4860::8888").Run()
	}, &testing.PollOptions{Timeout: pingTimeout}); err != nil {
		s.Errorf("Cannot ping 2001:4860:4860::8888: %s", err)
	}

	// Verify connectivity to IPv6-only host name
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		return arc.BootstrapCommand(ctx, "/system/bin/ping6", "-c1", "-w1", "ipv6.google.com").Run()
	}, &testing.PollOptions{Timeout: pingTimeout}); err != nil {
		s.Errorf("Cannot ping ipv6.google.com: %s", err)
	}
}
