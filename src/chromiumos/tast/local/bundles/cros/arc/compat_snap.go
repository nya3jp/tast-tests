...
func checkCompatSnappedWindowState(ctx context.Context, tconn *chrome.TestConn, d *ui.Device, act *arc.Activity, primary bool, stableWidth int) error {
...
func testUnsnapByDragging(ctx context.Context, tconn *chrome.TestConn, pc pointer.Context, d *ui.Device, act *arc.Activity, stableWidth int) error {
...
func testSnapFromOverview(ctx context.Context, tconn *chrome.TestConn, pc pointer.Context, d *ui.Device, act *arc.Activity, primary bool, stableWidth int) error {
	info, err := display.GetPrimaryInfo(ctx, tconn)
	if err != nil {
		return errors.Wrap(err, "failed to get the primary display info")
	}
	...
	if err := checkCompatSnappedWindowState(ctx, tconn, d, act, primary, stableWidth); err != nil {
...
func testSnapByDragToSnap(ctx context.Context, tconn *chrome.TestConn, pc pointer.Context, d *ui.Device, act *arc.Activity, primary bool, stableWidth int) error {
	info, err := display.GetPrimaryInfo(ctx, tconn)
	if err != nil {
		return errors.Wrap(err, "failed to get the primary display info")
	}
...
func testSnapViaKeyboardShortcut(ctx context.Context, tconn *chrome.TestConn, d *ui.Device, act *arc.Activity, primary bool, stableWidth int) error {
...
func CompatSnap(ctx context.Context, s *testing.State) {
	...
	for _, primary := range []bool{false, true} {
		...
		if err := testSnapFromOverview(ctx, tconn, pc, d, act, primary, stableWidth); err != nil {
			s.Fatalf("Failed to snap window from overview (primary=%t): %v", primary, err)
		}
		if err := testUnsnapByDragging(ctx, tconn, pc, d, act, stableWidth); err != nil {
		...
		if err := testSnapByDragToSnap(ctx, tconn, pc, d, act, primary, stableWidth); err != nil {
		...
		if err := testSnapViaKeyboardShortcut(ctx, tconn, d, act, primary, stableWidth); err != nil {
