...
func downloadFileWithApp(ctx context.Context, cr *chrome.Chrome, a *arc.ARC, localServerPort int, sourcePath, targetPath string) (retErr error) {
	...
	return downloadFileWithAppFromLocalAndroidPort(ctx, cr, a, tconn, androidPort, sourcePath, targetPath)
}

// downloadFileWithAppFromLocalAndroidPort downloads a file from sourcePath in
// Chrome OS to targetPath in Android with an Android app using the specified
// local port on the Android side.
func downloadFileWithAppFromLocalAndroidPort(ctx context.Context, cr *chrome.Chrome, a *arc.ARC, tconn *chrome.TestConn, androidPort int, sourcePath, targetPath string) (retErr error) {
	const (
