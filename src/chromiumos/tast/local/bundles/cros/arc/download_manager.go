...
// It first sets up reverse port forwarding to connect the local server with
// an Android local port, and then lets the app download the file from the
// connected Android port.
...
// downloadFileWithAppFromLocalAndroidPort downloads a file from sourcePath in
// Chrome OS to targetPath in Android with an Android app via a specified
// Android local port. It first installs the app, starts its MainActivity with
// an explicit intent to trigger a file download, and returns when the download
// is completed.
