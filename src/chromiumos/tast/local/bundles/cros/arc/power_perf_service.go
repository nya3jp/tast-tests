...
func (c *PowerPerfService) PowerSetup(ctx context.Context, request *arcpb.PowerSetupRequest) (*emptypb.Empty, error) {
	...
	if request.ArcDoze {
		if err := a.Command(ctx, "dumpsys", "deviceidle", "enable").Run(testexec.DumpLogOnError); err != nil {
			return nil, errors.Wrap(err, "failed to enable doze mode")
		}
		if err := a.Command(ctx, "dumpsys", "deviceidle", "force-idle").Run(testexec.DumpLogOnError); err != nil {
			return nil, errors.Wrap(err, "failed to enter doze mode")
		}
		output, err := a.Command(ctx, "dumpsys", "deviceidle").Output(testexec.DumpLogOnError)
		if err != nil {
			return nil, errors.Wrap(err, "failed to confirm doze mode")
		}
		testing.ContextLog(ctx, "Doze mode status: ", string(output))
		cleanup = append(cleanup, func(ctx context.Context) {
			if err := a.Command(ctx, "dumpsys", "deviceidle", "unforce").Run(testexec.DumpLogOnError); err != nil {
				testing.ContextLog(ctx, "Failed to leave doze mode: ", err)
			}
		})
	}
