...
func AuthPerf(ctx context.Context, s *testing.State) {
	...
	for len(playStoreShownTimes) < successBootCount {
		...
		tosTimes = append(tosTimes, v.tosTime)
...
func readResultProp(ctx context.Context, a *arc.ARC, propName string) (float64, error) {
	out, err := a.Command(ctx, "/system/bin/getprop", propName).Output(testexec.DumpLogOnError)
	if err != nil {
...
func bootARC(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn) (measuredValues, error) {
	...
	// Calculate
	//   * TOS(Terms Of Services) time as a difference preStartTime and enabledTime.
	...
	output, err := a.Command(ctx, "stat", "-c", "%z", "/proc/1").CombinedOutput(testexec.DumpLogOnError)
	if err != nil {
	...
	if err != nil {
		tInit, err = time.ParseInLocation("2006-01-02 15:04:05.999999999", timeStr, time.Now().Location())
	}
	if err != nil {
...
func dumpLogcat(ctx context.Context, s *testing.State, filePath string) error {
	...
	defer logFile.Close()
	// It is safe to use android-sh to access logcat
