...
func bootARC(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *chrome.TestConn) (measuredValues, error) {
	...
	s.Log("Waiting for ARC is stopped")
...
// waitForARCStopped waits for ARC is stopped.
func waitForARCStopped(ctx context.Context) error {
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		exist, err := arc.InitExists()
		if err != nil {
		...
		return errors.Wrap(err, "failed to wait for ARC is stopped")
	}
	return nil
}
