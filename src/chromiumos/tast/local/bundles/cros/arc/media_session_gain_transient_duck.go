...
func MediaSessionGainTransientDuck(ctx context.Context, s *testing.State) {
	...
	arcmediasession.RunTest(ctx, s, func(a *arc.ARC, d *ui.Device, sr *httptest.Server, cr *chrome.Chrome) {
		...
		must(arcmediasession.WaitForAndroidAudioFocusGain(ctx, d, arcmediasession.AudioFocusGainTransientMayDuck))
		...
		must(arcmediasession.AbandonAudioFocusInAndroid(ctx, d))
