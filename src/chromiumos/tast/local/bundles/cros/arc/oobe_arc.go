...
func OobeArc(ctx context.Context, s *testing.State) {
	...
	if err := ui.LeftClick(nodewith.NameRegex(regexp.MustCompile(
		"Accept and continue|Got it")).Role(role.Button))(ctx); err != nil {
		s.Fatal("Failed to click Accept and continue/ Got it Button : ", err)
	}
