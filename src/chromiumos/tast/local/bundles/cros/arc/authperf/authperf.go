...
func RunTest(ctx context.Context, s *testing.State, username string, password string, resultSuffix string) {
	...
	for {
		...
		s.Logf("Running iteration #%d out of %d",
			len(playStoreShownTimes)+1, successBootCount)

		v, err := bootARC(ctx, cr, tconn)
		if err != nil {
			errorCount++
	...
	// resultForLog is an array for generating comma separated results that can
	// be easily pasted into Google sheets.
	var resultForLog []string

	version, err := arc.ChromeOSVersion()
	if err != nil {
		s.Log("Can't get ChromeOS version: ", err)
	}
...
func bootARC(ctx context.Context, cr *chrome.Chrome, tconn *chrome.Conn) (measuredValues, error) {
	v := measuredValues{}
