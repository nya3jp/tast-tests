...
func testDebugfsTracefs(ctx context.Context, s *testing.State, arc []sysutil.MountInfo) {
	...
	if numTracing != 1 {
		s.Errorf("Unexpected debugfs/tracefs mount points: got %d; want %d", numTracing, 1)
	}
...
func testCPUSet(ctx context.Context, s *testing.State, a *arc.ARC) {
	...
	if maxCPUs > 4 {
		// ARC supports at most 4 cores. See also init.cheets.rc.
		maxCPUs = 4
	}

	allCPUs := fmt.Sprintf("0-%d", maxCPUs-1)
	for _, t := range types {
		...
		if t == "foreground" || t == "top-app" {
			...
			if val == allCPUs {
				s.Errorf("Unexpected CPU setting for %s: got %s", path, val)
			}
		}
	}
}
...
func RunTest(ctx context.Context, s *testing.State) {
	...
	testNoARCSharedLeak(ctx, s, arc, append(append(append(global, adbd...), sdcard...), obb...))
