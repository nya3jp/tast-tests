...
func isHeterogeneousCores(s *testing.State) (bool, error) {
	...
	re := regexp.MustCompile(`^CPU part\s+:\s+(0x[0-9a-f]+)$`)
	...
	for _, line := range lines {
		...
		if matches != nil {
			...
			if lastCPUPart != cpuPart {
				s.Log("Detected heterogeneous cores")
				return true, nil
			}
		}
	}
	if lastCPUPart == "" {
		return false, errors.New("failed to read cpuinfo. No CPU part information found")
	}
	return false, nil
}
...
func testCPUSet(ctx context.Context, s *testing.State, a *arc.ARC) {
	...
	cores := map[string]int{"foreground": 0, "background": 0, "system-background": 0, "top-app": 0, "restricted": 0}
	for t := range cores {
		...
		cores[t] = len(cpusInUse)
	}

	heterogeneousCores, err := isHeterogeneousCores(s)
	if err != nil {
	...
	if heterogeneousCores {
		...
		if cores["background"] == 1 && cores["system-background"] == 1 && cores["restricted"] == 1 {
