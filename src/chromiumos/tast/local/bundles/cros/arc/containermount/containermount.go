...
func isHeterogeneousCores() (bool, error) {
	...
	// big.LITTLE, it has two differnt CPU parts (e.g. 0xd03 and 0xd09).
	re := regexp.MustCompile(`^CPU part[ ]+: (0x[0-9a-f]+)$`)
...
func testCPUSet(ctx context.Context, s *testing.State, a *arc.ARC) {
	...
	heterogeneousCores, err := isHeterogeneousCores()
	if err != nil {
