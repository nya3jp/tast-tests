...
func testCPUSet(ctx context.Context, s *testing.State, a *arc.ARC) {
	...
	for _, t := range types {
		...
		if t == "foreground" || t == "top-app" {
			...
			if val != allCPUs {
				s.Errorf("Unexpected CPU setting for %q: got %q; want %s", path, val, allCPUs)
			}
		} else {
			// Other processes should not.
			if val == allCPUs {
				s.Errorf("Unexpected CPU setting for %q: got %q", path, val)
			}
		}
	}
}
...
func testSDCard(ctx context.Context, s *testing.State, sdcard []sysutil.MountInfo) {
	...
	for _, m := range sdcard {
		if m.Master > 0 {
			s.Errorf("Unexpected SD card slave mount at %s", m.MountPath)
			continue
		}
		if m.Shared == 0 {
			continue
		...
		if !re.MatchString(m.MountPath) {
			s.Errorf("Unexpected SD card shared mount at %s", m.MountPath)
		}
	}
}
