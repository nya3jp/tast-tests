...
const (
	...
	// cpuSharesPreArcBoot is the value for cpu.shares before ARC is done booting
	cpuSharesArcPreBoot = 32
	// cpuSharesArcBackground is the value for cpu.shares when ARC has nothing in the foreground
	cpuSharesArcBackground = 64
	// cpuSharesArcForegroun is the value of cpu.shares when ARC is in the foreground
...
func Cgroups(ctx context.Context, s *testing.State) {
	...
	// Check shares before ARC starts
	share, err := getCPUCgroupShares(ctx)
	if err != nil {
		s.Fatal("Failed to get ARC CPU shares value: ", err)
	}
	if share != cpuSharesArcPreBoot {
		s.Fatal("Unexpected ARC CPU shares value pre-boot:  ", share)
	}
	...
	// Check shares after ARC is done booting but nothing is active
	share, err = getCPUCgroupShares(ctx)
	if err != nil {
		s.Fatal("Failed to get ARC CPU shares value: ", err)
	}
	if share != cpuSharesArcBackground {
		s.Fatal("Unexpected ARC CPU shares value for background:  ", share)
	}
