...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Chrome's shortcut can work on an Android app",
...
func KeyboardShortcut(ctx context.Context, s *testing.State) {
	...
	intlID := "_comp_ime_jkghodnilhceideoidjikpgommlajknkxkb:us:intl:eng"
	if err := tconn.Eval(ctx,
		fmt.Sprintf(`chrome.languageSettingsPrivate.addInputMethod(%q);`, intlID), nil); err != nil {
		s.Fatal("Failed to enable US International keyboard: ", err)
	}

	s.Log("Activating US keyboard")
	defaultID := "_comp_ime_jkghodnilhceideoidjikpgommlajknkxkb:us::eng"
	if err := tconn.Eval(ctx,
		fmt.Sprintf(`chrome.inputMethodPrivate.setCurrentInputMethod(%q);`, defaultID), nil); err != nil {
		s.Fatal("Failed to activate US keyboard: ", err)
	}

	var beforeImeID string
	...
	if beforeImeID != defaultID {
		s.Fatal("Failed to activate US keybpard: ", err)
	}
	...
	// Press Ctrl-Space
	kb.Accel(ctx, "Ctrl+Space")

	var afterImeID string
	if err := tconn.EvalPromise(ctx,
		`new Promise(function(resolve, reject) {
		  chrome.inputMethodPrivate.getCurrentInputMethod(function(id) {
		    resolve(id);
