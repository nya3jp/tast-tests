...
func RestoreData(ctx context.Context, s *testing.State) {
	...
	if err := testexec.CommandContext(ctx,
		...
		"/opt/google/containers/android/rootfs/android-data/data").Run(testexec.DumpLogOnError); err != nil {
...
func restoreDataInitialBoot(ctx context.Context, credPool string) (chrome.Creds, error) {
	...
	// Let devive setup itself for some time. There is no clear trigger to wait.
	testing.ContextLog(ctx, "Waiting for the device setup")
	if err := testing.Sleep(ctx, 3*time.Minute); err != nil {
		return chrome.Creds{}, errors.Wrap(err, "failed to wait for the device setup")
	}
...
func restoreDataRegularBoot(ctx context.Context, testDir string, creds chrome.Creds, dataRestoreExpected bool) error {
	...
	if metric.TotalCount() != 1 {
		return errors.Wrapf(err, "histogram %s expected to set only once", statusMetricName)
	}
	...
	if metric.Sum != expectation {
		return errors.Wrapf(err, "histogram %s is expected to be %d but set to %d", statusMetricName, expectation, metric.Sum)
	}

	metric, err = metrics.WaitForHistogram(ctx, tconn, durationMetricName, 10*time.Second)
	if err != nil {
		if !dataRestoreExpected {
			// Not expected to set if data restoration did not take a place.
			return nil
		}
		return errors.Wrapf(err, "failed to get %s histogram", durationMetricName)
	}
	if !dataRestoreExpected {
		return errors.Errorf("%s  histogram is set but not expected", durationMetricName)
	}
