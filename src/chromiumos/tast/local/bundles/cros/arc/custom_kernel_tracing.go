...
func CustomKernelTracing(ctx context.Context, s *testing.State) {
	...
	if mode == 0 {
		...
		err := tracingManualLoading(ctx, s.OutDir())
		if err != nil {
			s.Fatal("Failed to do manual testing session: ", err)
		}
	}
...
func tracingArcOn(ctx context.Context, cmd string) error {
	...
	if isVMEnabled {
		if err := testexec.CommandContext(ctx, "/usr/sbin/android-sh", "-c", "echo "+cmd+" >/sys/kernel/tracing/tracing_arc_on").Run(testexec.DumpLogOnError); err != nil {
...
func tracingStopAndPrintResult(ctx context.Context, testDir string) error {
	...
	out := []byte("")
	if isVMEnabled {
		out, err = testexec.CommandContext(ctx, "/usr/sbin/android-sh", "-c", "cat /sys/kernel/tracing/tracing_arc_read").Output(testexec.DumpLogOnError)
		if err != nil {
...
func tracingGameLoading(ctx context.Context, testDir string, creds chrome.Creds) error {
	...
	if isVMEnabled {
		if err := testexec.CommandContext(ctx, "/usr/sbin/android-sh", "-c", "echo 0 >/sys/kernel/tracing/tracing_arc_on").Run(testexec.DumpLogOnError); err != nil {
...
func waitAnyKeyIsPressed() error {
	const root = "/dev/input/by-path"
	files, err := ioutil.ReadDir(root)
	if err != nil {
		return errors.Wrap(err, "failed to enum input devices")
	}

	for _, file := range files {
		if !strings.HasSuffix(file.Name(), "event-kbd") || file.IsDir() {
			continue
		}
		...
		_, err = f.Read(b)
		if err != nil {
			return errors.Wrap(err, "failed to read keyboard device")
		}

		return nil
	}

	return errors.New("no keyboard input device was found")
}

func tracingManualLoading(ctx context.Context, testDir string) error {
	a, err := arc.New(ctx, testDir)
	if err != nil {
		return errors.Wrap(err, "failed to connect to ARC")
	}
	...
	testing.ContextLog(ctx, "Press any key on device when you are ready")
	err = waitAnyKeyIsPressed()
	if err != nil {
		return errors.Wrap(err, "failed to wait user input")
	}
	...
	testing.ContextLog(ctx, "Press any key on device when you are done")
	err = waitAnyKeyIsPressed()
	if err != nil {
		return errors.Wrap(err, "failed to wait user input")
	}

	tracingTime := time.Now().Sub(startTime)

	testing.ContextLog(ctx, "Tracing time: ", tracingTime.Seconds())
