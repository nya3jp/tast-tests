...
// NewVR returns a voiceRecorder.
func NewVR(ctx context.Context, kb *input.KeyboardEventWriter, tconn *chrome.TestConn, a *arc.ARC) (*VoiceRecorder, error) {
...
// SetFileLocation sets location path from default to downloads.
func (vr *VoiceRecorder) SetFileLocation(ctx context.Context) error {
...
// RecordSound clicks on record button to record sound and gets the name of this audio file.
func (vr *VoiceRecorder) RecordSound(ctx context.Context) (string, error) {
...
// PlayCurrentRecord plays the latest record.
func (vr *VoiceRecorder) PlayCurrentRecord(ctx context.Context, tconn *chrome.TestConn, name string) error {
	...
	fileName := vr.app.D.Object(ui.ID(fileNameID))
	if err := fileName.WaitForExists(ctx, defaultUITimeout); err != nil {
		return errors.Wrap(err, "failed to find the file name")
	}

	fnText, err := fileName.GetText(ctx)
	if err != nil {
...
// DeleteAudioFile deletes the audio file created by RecordSound method.
...
func (vr *VoiceRecorder) DeleteAudioFile(name string) error {
	return os.Remove(filepath.Join(filesapp.DownloadPath, "Recorders", name))
}
