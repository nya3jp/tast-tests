...
func sendMdns(hostname, port, ifname string, IPv6 bool) error {
	...
	if IPv6 {
		src = &net.UDPAddr{IP: net.ParseIP("::"), Port: p}
		dst = &net.UDPAddr{IP: net.ParseIP("ff02::fb"), Port: 5353}
	} else {
		src = &net.UDPAddr{IP: net.IPv4(0, 0, 0, 0), Port: p}
		dst = &net.UDPAddr{IP: net.IPv4(224, 0, 0, 251), Port: 5353}
	}

	if err := sendMulticast(b.Bytes(), src, dst, ifname, IPv6); err != nil {
		return err
	}
	return nil
}
...
// sendMulticast take data |b| and send it from |src| to |dst| with interface |ifname|.
// This function intentionally set multicast loopback to true.
func sendMulticast(b []byte, src, dst *net.UDPAddr, ifname string, IPv6 bool) error {
	c, err := net.ListenUDP("udp", src)
	if err != nil {
		return errors.Errorf("failed to create UDP socket %s", err)
	}
	defer c.Close()
	...
	if IPv6 {
		...
		iface, err := net.InterfaceByName(ifname)
		if err != nil {
			return errors.Errorf("failed to get interface by name %s ", err)
		}
		if err := pc.SetMulticastInterface(iface); err != nil {
			return errors.Errorf("failed to set multicast interface %s ", err)
		}
