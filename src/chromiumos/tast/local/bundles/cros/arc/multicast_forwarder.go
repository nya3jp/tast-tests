...
func MulticastForwarder(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < retry; i++ {
		...
		var mdnsDst, ssdpDst *net.UDPAddr
...
func sendMDNS(hostname, port, ifname string, dst *net.UDPAddr) error {
	...
	if err := sendMulticast(b.Bytes(), p, dst, ifname); err != nil {
		return err
	}
	return nil
}
...
func sendMulticast(b []byte, port int, dst *net.UDPAddr, ifname string) error {
	var d int
	...
	if _, err := c.WriteTo(b, dst); err != nil {
		return errors.Errorf("failed to send data %s", err)
	}
	return nil
}
