...
func isPlayStoreOpen(ctx context.Context, s *testing.State, d *ui.Device, tconn *chrome.TestConn) {
	if err := ash.WaitForApp(ctx, tconn, apps.PlayStore.ID); err != nil {
		s.Error("Play Store failed to open.")
	}

	toc := d.Object(ui.ID("com.android.vending:id/play_card"))
	if err := toc.WaitForExists(ctx, 10*time.Second); err != nil {
		s.Error("Timed waiting for Play Store table of contents.")
	}
}

// Opts into Play Store, starts up UI automator and ARC.
func testSetup(ctx context.Context, s *testing.State) (cr *chrome.Chrome, a *arc.ARC, d *ui.Device, tconn *chrome.TestConn) {
