...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:         []string{"app_sanity_hello-world.apk"},
		Pre:          arc.Booted(),
	})
}
...
func AppSanity(ctx context.Context, s *testing.State) {
	const (
		...
		// FIXME update URL https://codelabs.developers.google.com/codelabs/android-training-hello-world
		apk = "app_sanity_hello-world.apk"
	...
	err = testing.Poll(ctx, func(ctx context.Context) error {
		...
		if bounds.Width <= 0 || bounds.Height <= 0 {
			return errors.Errorf("bounds should be positive but was %dx%d", bounds.Width, bounds.Height)
		}
		return nil
	}, nil)
	if err != nil {
