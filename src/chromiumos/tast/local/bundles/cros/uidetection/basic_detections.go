...
// expectError returns an error if the error returned by an action doesn't match the expectation.
...
func BasicDetections(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine("verify detections",
		...
		expectError(ud.Exists(uidetection.TextBlock([]string{"Add", "shortcut"}).Nth(2)), uidetection.ErrMultipleMatch),
	)(ctx); err != nil {
