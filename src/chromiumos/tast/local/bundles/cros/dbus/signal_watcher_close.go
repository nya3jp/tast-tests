...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:     "Verifies dbusutil.SignalWatcher can properly close without deadlock when the number of signals exceeds channel buffer",
...
func SignalWatcherClose(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatalf("Failed to create signal emitter bus, err=%s", err.Error())
	}
	defer emitter.Close()
	receiver, err := setupPrivateSystemBus()
	if err != nil {
		s.Fatalf("Failed to create receiver bus, err=%s", err.Error())
	}
	...
	if err != nil {
		s.Fatalf("Failed to create reference signal watcher, err=%s", err.Error())
	}
	defer refWatcher.Close(ctx)
	watcher, err := dbusutil.NewSignalWatcher(ctx, receiver, match)
	if err != nil {
		s.Fatalf("Failed to create signal watcher, err=%s", err.Error())
	}
	...
	for i := 0; i < sigCount; i++ {
		...
		case <-ctx.Done():
			s.Fatal("Failed to receive all sent signals in time")
		}
	}

	// Try close the watcher. As it may hang, run it in goroutine so that we can
	// rescue it in main thread.
