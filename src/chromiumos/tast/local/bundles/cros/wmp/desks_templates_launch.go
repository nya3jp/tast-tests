...
func DesksTemplatesLaunch(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine(
		...
		s.Fatal("Failed to launch a saved template of kTemplate type: ", err)
	}
	...
	if err := uiauto.Combine(
		...
		s.Fatal("Failed to launch a saved desk of kSaveAndRecall type: ", err)
	}
	...
	if savedDeskViewInfo, err := ash.FindDeskTemplates(ctx, ac); err == nil {
		if len(savedDeskViewInfo) != 1 {
			s.Fatalf("Found inconsistent number of saved template(s): got %v, want 1", len(savedDeskViewInfo))
		}
	} else {
		s.Fatal("Failed to find saved templates: ", err)
	}
}
