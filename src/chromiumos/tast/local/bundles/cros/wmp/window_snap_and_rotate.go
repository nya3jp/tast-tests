...
func WindowSnapAndRotate(ctx context.Context, s *testing.State) {
	...
	if !windows[0].IsActive {
		s.Fatalf("The first window %d is not active", windows[0].ID)
	}

	if windows[0].State != ash.WindowStateNormal {
		s.Fatalf("The first window %d is not normal (state %q)", windows[0].ID, windows[0].State)
	}
	...
	if portrait {
		...
		verifyState(ctx, s, tconn, windows[0].ID, ash.WindowStateMaximized)
		...
		verifyState(ctx, s, tconn, windows[0].ID, ash.WindowStateLeftSnapped)

		// Activate the second window to make sure it is not hidden behind |windows[0]| and tests
		// drag to snap bottom.
		windows[1].ActivateWindow(ctx, tconn)
		...
		verifyState(ctx, s, tconn, windows[1].ID, ash.WindowStateRightSnapped)
...
func verifyState(ctx context.Context, s *testing.State, tconn *chrome.TestConn,
	windowID int, wantState ash.WindowStateType) {
	window, err := ash.GetWindow(ctx, tconn, windowID)
	if err != nil {
		s.Fatalf("Failed to obtain window(id=%d): %v", windowID, err)
	}
	if window.State != wantState {
		s.Fatalf("Unexpected window(id=%d) state = got %v, want %v",
...
func dragWindowTo(ctx context.Context, tconn *chrome.TestConn,
	...
	if err := ash.WaitWindowFinishAnimating(ctx, tconn, windowID); err != nil {
		return errors.Wrap(err, "failed to wait for window animation")
	}
