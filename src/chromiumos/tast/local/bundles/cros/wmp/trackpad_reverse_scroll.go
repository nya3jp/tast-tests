...
func swipeToDesk(ctx context.Context, tconn *chrome.TestConn, tpw *input.TrackpadEventWriter, swipeDirection trackpad.SwipeDirection, deskContainerName string) error {
	...
	if err := trackpad.Swipe(ctx, tconn, tpw, swipeDirection, 4); err != nil {
		return errors.Wrap(err, "failed to swipe right with 4 fingers")
	}
...
func TrackpadReverseScroll(ctx context.Context, s *testing.State) {
	...
	if err := trackpad.TurnOffReverseScroll(ctx, tconn); err != nil {
		s.Fatal("Failed to turn off trackpad reverse scroll")
	}
	// -------------- Outside Overview Mode ---------------------
	// 1. Swiping down with 3 fingers won't enter overview.
	// 2. Consecutively swiping down twice with 3 fingers will trigger a system
	//    toast (only for non reverse scrolling).
	// 3. Swiping up with 3 fingers will enter overview.

	// Swiping down twice with 3 fingers will trigger a system toast.
	...
	// 2. Consecutively swiping up twice with 3 fingers will trigger a system
	//    toast (only for non reverse scrolling).
	// 3. Swiping down with 3 fingers will exit overview.

	// Swiping up twice with 3 fingers will trigger a system toast.
	...
	// 1. Swiping left will not switch desk.
	// 2. Consecutively swiping left twice with 4 fingers will trigger a system
	//    toast (only for non reverse scrolling).
	// 3. Swiping right with 4 fingers will switch to the right desk.

	// Swiping left twice with 4 fingers will trigger a system toast.
	...
	// Swiping right with 4 fingers will switch to the right desk..
	if err := swipeToDesk(ctx, tconn, tpw, trackpad.RightSwipe, "Desk_Container_B"); err != nil {
		s.Fatal("Failed to swipe right to switch to the right desk: ", err)
	}
	//-------------------------------------------------------

	// ------------ On The Rightmost Desk ---------------------
	// 1. Swiping right will not switch desk.
	// 2. Consecutively swiping right twice with 4 fingers will trigger a system
	//    toast (only for non reverse scrolling).
	// 3. Swiping left with 4 fingers will switch to the left desk.

	// Swiping right twice with 4 fingers will trigger a system toast.
	...
	// Swiping left with 4 fingers will switch to the left desk..
	if err := swipeToDesk(ctx, tconn, tpw, trackpad.LeftSwipe, "Desk_Container_A"); err != nil {
		s.Fatal("Failed to swipe left to switch to the left desk: ", err)
	}
	//--------------------------------------------------------------------
	...
	if err := trackpad.TurnOnReverseScroll(ctx, tconn); err != nil {
		s.Fatal("Failed to turn on trackpad reverse scroll: ", err)
	}
	// -------------- Outside Overview Mode ---------------------
	// Swiping up with 3 fingers will enter Overview.
	if err := swipeToEnterOverview(ctx, tconn, tpw); err != nil {
	...
	// ------------ On The Leftmost Desk ----------------------
	// Swiping left with 4 fingers will switch to the right desk..
	if err := swipeToDesk(ctx, tconn, tpw, trackpad.LeftSwipe, "Desk_Container_B"); err != nil {
		s.Fatal("Failed to swipe left to switch to the right desk: ", err)
	}
	// ------------ On The Rightmost Desk ---------------------
	// Swiping right with 4 fingers will switch to the left desk..
	if err := swipeToDesk(ctx, tconn, tpw, trackpad.RightSwipe, "Desk_Container_A"); err != nil {
		s.Fatal("Failed to swipe right to switch to the left desk: ", err)
	}
}
