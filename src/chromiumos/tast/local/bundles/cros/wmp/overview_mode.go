...
func OverviewMode(ctx context.Context, s *testing.State) {
	...
	if err := ash.ForEachWindow(ctx, tconn, func(w *ash.Window) error {
		if wmputils.IsBrowserWindow(w, bt) {
	...
	go func() {
		testing.Poll(ctx, func(ctx context.Context) error {
			...
			for _, window := range ws {
				if wmputils.IsBrowserWindow(window, bt) && !window.IsAnimating {
	...
	if err := ash.SetOverviewModeAndWaitWithTimeout(ctx, tconn, true, 10*time.Second); err != nil {
		s.Fatal("Failed to enter into the overview mode: ", err)
	}
	defer ash.SetOverviewModeAndWaitWithTimeout(cleanupCtx, tconn, false, 10*time.Second)
	...
	if wmputils.IsBrowserWindow(ws[0], bt) {
