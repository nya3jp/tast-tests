...
func WindowCaptionButton(ctx context.Context, s *testing.State) {
	...
	if err := ash.WaitForCondition(ctx, tconn, func(w *ash.Window) bool {
		...
		s.Fatalf("Unexpected Chrome window state: got %s, expected %s", bw.State, ash.WindowStateMinimized)
	}
	...
	if err := ash.WaitForCondition(ctx, tconn, func(w *ash.Window) bool {
		...
		s.Fatalf("Unexpected Chrome window state: got %s, expected %s", bw.State, ash.WindowStateNormal)
	}
	...
	if err := ash.WaitForCondition(ctx, tconn, func(w *ash.Window) bool {
		...
		s.Fatalf("Unexpected Chrome window state: got %s, expected %s", bw.State, ash.WindowStateMaximized)
	}
	...
	if len(ws) != 1 {
		s.Fatalf("Unexpected number of open windows; got %s, expected %s", len(ws), 1)
	}
	if ws[0].ID != bw.ID {
		s.Fatalf("Unexpected open window; got %s, expected Chrome window", ws[0].Title)
	}
}
