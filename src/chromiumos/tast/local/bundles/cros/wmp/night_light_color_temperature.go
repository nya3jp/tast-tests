...
func NightLightColorTemperature(ctx context.Context, s *testing.State) {
	...
	if coolerCTM == defaultCTM {
		s.Fatal("Dragging slider to cooler did not change the night light color")
	}
	...
	if coolerCTM == warmerCTM || defaultCTM == warmerCTM {
		s.Fatal("Dragging slider to warmer did not change the night light color")
	}
...
func modetestCTM(ctx context.Context) (string, error) {
	...
	// Start to find "CTM" line.
	find := findCTM
	ctm := ""
	for _, line := range strings.Split(string(output), "\n") {
		if find == findCTM {
			// If found "CTM:" line, continue to find "value:" line.
			...
			// If encounter "GAMMA_LUT" line, break when we get a non-empty CTM,
