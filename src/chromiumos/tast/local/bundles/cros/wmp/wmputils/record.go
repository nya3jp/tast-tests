...
// EnsureRecordModeActivated makes sure that the recording mode is activated.
func EnsureRecordModeActivated(tconn *chrome.TestConn, activated bool) uiauto.Action {
	return func(ctx context.Context) error {
		return testing.Poll(ctx, func(ctx context.Context) error {
			...
			if err := condition.function(ctx); err != nil {
				if err := kb.AccelAction("sysrq")(ctx); err != nil {
					return errors.Wrap(err, "failed to press sysrq")
				}
