...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Checks that we can drag and snap camera preview",
		Contacts: []string{
...
func CaptureSelfieCamDragToSnap(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine(
		...
		dragCameraPreviewTo(tconn, coords.NewPoint(leftToTheCenter, upToTheCenter), captureSurfaceTopLeft),
		dragCameraPreviewTo(tconn, coords.NewPoint(rightToTheCenter, upToTheCenter), captureSurfaceTopRight),
		dragCameraPreviewTo(tconn, coords.NewPoint(leftToTheCenter, belowToTheCenter), captureSurfaceBottomLeft),
		dragCameraPreviewTo(tconn, coords.NewPoint(rightToTheCenter, belowToTheCenter), captureSurfaceBottomRight),
	)(ctx); err != nil {
		s.Fatal("Failed to drag to snap camera preview: ", err)
	}
}

func dragCameraPreviewTo(tconn *chrome.TestConn, targetPoint coords.Point, targetRect coords.Rect) uiauto.Action {
