...
// ScreenshotWhileRecording verifies that takeing screenshot while doing screen recording works.
func ScreenshotWhileRecording(ctx context.Context, s *testing.State) {
	...
	// For verifying the screenshot and screen recording later, delete all files first.
	...
	testing.ContextLog(ctx, "Launch 'Screen capture' and start recording")
	if err := wmp.StartFullScreenRecording(tconn)(ctx); err != nil {
		s.Fatal("Failed to start screen recording: ", err)
	}
	testing.ContextLog(ctx, "Start screenshot while recording is in progress")
	if err := wmp.LaunchScreenCapture(ctx, tconn); err != nil {
	...
	testing.ContextLog(ctx, "End screen recording now")
	...
	testing.ContextLog(ctx, "Check the existence and the size of the screenshot and recording")
	if err := wmp.CheckScreenshot(ctx, tconn, downloadsPath); err != nil {
		s.Fatal("Failed to verify the screenshot: ", err)
	}
	testing.ContextLog(ctx, "Check the existence and the size of the screen recording")
