...
func VirtualDesksShortcuts(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine(
		"Use keyboard shortcut to create a new desk",
	...
	if err := uiauto.Combine(
		"Use keyboard shortcut to switch to the desk on the left",
	...
	if err := uiauto.Combine(
		"Use keyboard shortcut to switch to the desk on the right",
		kb.AccelAction("Search+]"),
		ac.WaitForLocation(deskFinder("Desk_Container_B")),
	)(ctx); err != nil {
	...
	// We then move the currently active window between desks. We are on desk 2 when this sequence starts.
	...
	if !findBrowserWindow(ctx, s, tconn).OnActiveDesk {
		s.Fatal("Browser not on desk 2")
	}

	if err := kb.Accel(ctx, "Search+Shift+["); err != nil {
		s.Fatal("Failed to move browser to desk 1: ", err)
	}
	...
	if findBrowserWindow(ctx, s, tconn).OnActiveDesk {
		s.Fatal("Browser unexpectedly on desk 2")
	}
	...
	ash.WaitWindowFinishAnimating(ctx, tconn, browserWindowID)
	if findBrowserWindow(ctx, s, tconn).OnActiveDesk {
		s.Fatal("Browser unexpectedly on desk 1")
	}
	...
	if err := uiauto.Combine(
		"Use keyboard shortcut to reorder desks",
		kb.AccelAction("Tab"),
		kb.AccelAction("Tab"),
	...
	if err := uiauto.Combine(
		"Use keyboard shortcut to remove a desk",
		kb.AccelAction("Search+Shift+-"),
		ac.WaitForLocation(deskFinder("Desk_Container_A")),
	)(ctx); err != nil {
