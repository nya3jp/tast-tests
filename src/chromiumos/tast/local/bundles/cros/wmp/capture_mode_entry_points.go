...
func CaptureModeEntryPoints(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine(
		...
		kb.AccelAction("Ctrl+Shift+"+layout.SelectTask),
		ac.WaitUntilExists(captureModeBarView),
		ac.WaitUntilExists(partialCaptureModeLabel),
		kb.AccelAction("esc"),
	)(ctx); err != nil {
	...
	// Ctrl + F5 accelerator skips entering capture mode and instead
	...
	if err := uiauto.Combine(
		...
		kb.AccelAction("Ctrl+"+layout.SelectTask),
	...
	// TODO(b/253039033): Devices reserve "Ctrl+Alt+(F2,F3,F4)" to go
	// to command line. On some devices with a certain layout, the
	// shortcut here is "Ctrl+Alt+F4". Figure a way to handle this
	// double mapping.
	fullCaptureModeLabel := nodewith.Name("Click anywhere to capture full screen")
	if err := uiauto.Combine(
		"enter window capture mode using accelerator",
		kb.AccelAction("Ctrl+Alt+"+layout.SelectTask),
		ac.WaitUntilExists(captureModeBarView),
