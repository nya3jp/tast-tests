...
func WindowCycle(ctx context.Context, s *testing.State) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		s.Fatal("Failed to wait for the window to be open, err: ", err)
	}
	defer func() {
		// Clean up windows if no error occurs. Otherwise, leave them unchanged to take a screenshot for troubleshooting.
		if !s.HasError() {
