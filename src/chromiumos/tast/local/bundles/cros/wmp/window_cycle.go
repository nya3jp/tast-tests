...
func WindowCycle(ctx context.Context, s *testing.State) {
	...
	if err := verifyWindowsForCycleMenu(ctx, tconn, ac, windowCycleView, windows); err != nil {
...
func verifyWindowsForCycleMenu(ctx context.Context, tconn *chrome.TestConn, ac *uiauto.Context, cycleMenu *nodewith.Finder, windows []*ash.Window) error {
	...
	for _, direction := range []string{"forward", "backward"} {
		...
		for i := 0; i < numWindows+1; i++ {
			if err := openCycleWindowAndPressTabNTimes(ctx, ac, cycleMenu, direction, numWindows, i); err != nil {
...
func openCycleWindowAndPressTabNTimes(ctx context.Context, ac *uiauto.Context, cycleMenu *nodewith.Finder, direction string, numWindows, n int) error {
	// Get the keyboard
	keyboard, err := input.Keyboard(ctx)
	if err != nil {
