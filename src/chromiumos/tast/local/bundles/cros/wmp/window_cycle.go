...
func WindowCycle(ctx context.Context, s *testing.State) {
	...
	verifyWindowsForCycleMenu(ctx, s, tconn, ac, cycleMenu, keyboard, windows)

	// 2. Tests that when there are 8 desks and each desk has at least one browser/app, we can cycle all the apps/browsers that are opened in all desks when the default status of the alt-tab window is "All desks".
	...
	// 4. Tests that when we traversal all of the desks and enter the alt-tab window, the "Current desk" shoud be enabled based on the test 3.
	...
	// 6. Delete all desks and press alt-tab keys. All the active apps and browsers should be shown without any option at the top of the alt-tab window.
