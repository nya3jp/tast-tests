...
func TabletModeBasicWindowControl(ctx context.Context, s *testing.State) {
	...
	//addressBarNode := nodewith.Role(role.MenuItem).Name("Always show full URLs")
	...
	if err := ui.WithTimeout(5 * time.Second).WaitUntilExists(addressBarNode.Onscreen())(ctx); err != nil {
