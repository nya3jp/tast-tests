...
func ZeroStateDesksBar(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine(
		...
		// The desk name view of the default desk mini view shoudl be focused.
	...
	wmputils.FindDeskMiniViews(ctx, ac, 1)
	...
	deskMiniViewsInfo, err := wmputils.FindDeskMiniViews(ctx, ac, 2)
	if err != nil {
	...
	wmputils.FindDeskMiniViews(ctx, ac, 2)
}
