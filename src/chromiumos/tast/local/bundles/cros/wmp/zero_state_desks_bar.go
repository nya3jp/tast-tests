...
func ZeroStateDesksBar(ctx context.Context, s *testing.State) {
	...
	if _, err := ash.FindDeskMiniViews(ctx, ac, 1); err != nil {
	...
	if (*newDeskButtonInExpandedDesksBarLoc).Width != secondDeskMiniViewLoc.Width {
		s.Fatalf("Expected width of the expanded new desk button is %, got %d instead", secondDeskMiniViewLoc.Width, (*newDeskButtonInExpandedDesksBarLoc).Width)
	}
	if (*newDeskButtonInExpandedDesksBarLoc).Height != secondDeskMiniViewLoc.Height {
		s.Fatalf("Expected height of the expanded new desk button is %, got %d instead", secondDeskMiniViewLoc.Height, (*newDeskButtonInExpandedDesksBarLoc).Height)
	}
	...
	if _, err := ash.FindDeskMiniViews(ctx, ac, 2); err != nil {
		s.Fatal("Failed to find desks: ", err)
	}
}
