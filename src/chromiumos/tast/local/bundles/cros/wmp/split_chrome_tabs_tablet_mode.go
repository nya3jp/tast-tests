...
func SplitChromeTabsTabletMode(ctx context.Context, s *testing.State) {
	...
	// Drag the first tab in the tab strip and snap it to the right. Add sleep
	...
	// Drag the first tab in the left window and snap it to the left. Add slee
	// to long press the tab thumbnail to be able to grab it.
	...
	// Ensure the tab has been dragged to a new window and snapped left.
	ws, err = ash.GetAllWindows(ctx, tconn)
	if err != nil {
		s.Fatal("Failed to obtain the window list: ", err)
	}
