...
func moveToAndCloseWindowsOnDesk2(ctx context.Context, tconn *chrome.TestConn, ac *uiauto.Context) error {
	...
	if info.ActiveDeskIndex != 2 {
		return errors.Wrapf(nil, "Active desk should be Desk 3; while current desk is Desk %d", info.ActiveDeskIndex+1)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to find all windows on Desk 3")
	}
	numWindows := 3
	if len(ws) != numWindows {
		return errors.Wrapf(nil, "unexpected number of windows found; want: %d, got: %d", numWindows, len(ws))
	}
	...
	if len(ws) != numWindows {
		return errors.Wrapf(nil, "unexpected number of windows found; want: 3, got %d", len(ws))
	}
