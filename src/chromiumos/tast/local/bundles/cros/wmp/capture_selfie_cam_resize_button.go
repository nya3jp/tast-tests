...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Verifies the visibility change and resize behavior of the resize button",
		Contacts: []string{
...
func CaptureSelfieCamResizeButton(ctx context.Context, s *testing.State) {
	...
	outsidePoint := coords.NewPoint(cameraPreviewLocation.TopLeft().X-1, cameraPreviewLocation.TopLeft().Y-1)
	if err := uiauto.Combine(
