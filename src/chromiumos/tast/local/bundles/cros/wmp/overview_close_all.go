...
func OverviewCloseAll(ctx context.Context, s *testing.State) {
	...
	cr, err := chrome.New(ctx,
		...
		chrome.EnableFeatures("DesksTemplates", "DesksCloseAll"),
		// chrome.ARCSupported(),
	...
	// Close desk and windows by mouse.
	...
	if err := uiauto.Combine(
		...
		ac.LeftClick(zeroAddDeskButton),
	...
	// Activate the second desk and launch app windows on it.
	...
	// Get the locations of the second desk mini view.
	desk2DeskMiniViewLoc, err := ac.Location(ctx, desk2DeskMiniView)
	if err != nil {
		s.Fatal(err, "failed to get the mini view location of desk %s:", desk2Name)
	}

	// Move the mouse to second desk and hover.
	if err := mouse.Move(tconn, desk2DeskMiniViewLoc.CenterPoint(), 100*time.Millisecond)(ctx); err != nil {
		s.Fatal(err, "failed to hover at the second desk mini view")
	}
	...
	if err := uiauto.Combine(
		"Close a desk and windows on it",
		ac.LeftClick(closeAllButton),
	)(ctx); err != nil {
	...
	// There should still be 0 visible windows since all windows are closed.
	windowCount, err := ash.CountVisibleWindows(ctx, tconn)
	if err != nil {
		s.Fatal("Failed to count visible windows: ", err)
	}
	if windowCount != 0 {
		s.Fatalf("Expected 0 visible windows, got %v instead", windowCount)
	}
}
