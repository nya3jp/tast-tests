...
func OverviewCloseAll(ctx context.Context, s *testing.State) {
	...
	// If we are in lacros-chrome, then browserfixt.SetUp has already opened a blank browser window in the first desk. In that case, we want to move the already-existing browser window over to the second desk with a keyboard shortcut and wait for the window to finish moving.
	if bt == browser.TypeLacros {
		if err := kb.Accel(ctx, "Search+Shift+]"); err != nil {
			s.Fatal("Failed to move lacros browser window to second desk: ", err)
		}
	...
	// Opens PlayStore, Chrome and Files. As mentioned above, if we are in lacros-chrome we will already have a chrome window, so if that is the case then we can skip opening another browser window.
	...
	// We give windows 1 second after the toast goes away for them to close down before we force close them, so we need to wait for a second.
