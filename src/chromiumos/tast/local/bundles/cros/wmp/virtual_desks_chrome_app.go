...
func VirtualDesksChromeApp(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Failed to check window size: ", err)
	}
	...
	if err != nil {
		s.Fatal("Failed to get the active desk index: ", err)
	}
	...
	if len(currWs) != len(ws) {
		s.Fatalf("Unexpected number of windows found; wanted %v, got %v", ws, currWs)
	}
