...
func VirtualDesksChromeApp(ctx context.Context, s *testing.State) {
	...
	// TODO(shidi): Need autotest API to simulate PWA apps.
	...
	// Wait until switch desk animation ends.
	testing.Sleep(ctx, 2*time.Second)

	// Check no new window are created even after clicked Youtube button from desk 2.
	// TODO(shidi): Need autotest api to check which desk is the current active desk.
