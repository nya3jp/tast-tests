...
func checkDriveResults(ctx context.Context, tconn *chrome.TestConn, cont *vm.Container) error {
	...
	if shared, err := s.GetSharedFolders(ctx); err != nil {
		...
		return errors.Wrapf(err, "failed to verify shared folders list, got %s, want %s", shared, want)
	}
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		if list, err := cont.GetFileList(ctx, sharedfolders.MountPath); err != nil {
			return err
		} else if err := listset.CheckListsMatch(list, "fonts", sharedfolders.MountFolderGoogleDrive); err != nil {
			return err
		}

		if list, err := cont.GetFileList(ctx, sharedfolders.MountPathGoogleDrive); err != nil {
			return err
		} else if err := listset.CheckListsMatch(list, sharedfolders.MountFolderMyDrive); err != nil {
			return err
		}
		return nil
