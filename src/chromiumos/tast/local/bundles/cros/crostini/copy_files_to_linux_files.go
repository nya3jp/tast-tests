...
func CopyFilesToLinuxFiles(ctx context.Context, s *testing.State) {
	...
	if !reflect.DeepEqual(testFiles, fileList) {
		s.Fatalf("File lists in Linux files and home directory in container do not equal: files in Linux files are %s, files in container are %s", strings.Join(testFiles, " "), strings.Join(fileList, " "))
	}
}
