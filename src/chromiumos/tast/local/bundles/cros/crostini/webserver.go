...
func Webserver(ctx context.Context, s *testing.State) {
	...
	checkNavigation := func(url string) error {
		if err = conn.Navigate(ctx, url); err != nil {
			return fmt.Errorf("navigating to %q failed: %v", url, err)
		}
		var actual string
		if err = conn.Eval(ctx, "document.documentElement.innerText", &actual); err != nil {
			return fmt.Errorf("failed to get page content for %s: %v", url, err)
		}
		if !strings.HasPrefix(actual, expectedWebContent) {
			return fmt.Errorf("unexpected page content for %s; got %q, want %q", url, actual, expectedWebContent)
		}
	...
	testing.ContextLog(ctx, "Waiting for python webservers to start up")
	for _, sockaddr := range sockaddrs {
	...
	for url, _ := range checkURLs {
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			if err := checkNavigation(url); err != nil {
				return err
			}
			return nil
