...
func PackageInstallUninstall(ctx context.Context, s *testing.State) {
	...
	crostini.TransferToContainerOrDie(ctx, s, cont, s.DataPath("package.deb"), filePath)
	defer crostini.RemoveContainerFileOrDie(ctx, s, cont, filePath)
	...
	if waitStatus, ok := testexec.GetWaitStatus(err); !ok {
		...
		s.Fatal("Cros-tast-tests still installed")
