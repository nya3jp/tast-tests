...
func checkShareResults(ctx context.Context, tconn *chrome.TestConn, cont *vm.Container) error {
	...
	sharedFoldersList, err := settings.GetLinuxSharedFolders(ctx, tconn, settings.SettingsMSF)
	if err != nil {
		return errors.Wrap(err, "failed to find the shared folders list")
	}
	if err := util.CheckListsMatch(sharedFoldersList, sharedfolders.MyFiles); err != nil {
