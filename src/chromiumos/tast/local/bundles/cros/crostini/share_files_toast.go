...
func ShareFilesToast(ctx context.Context, s *testing.State) {
	...
	shareFolders := &linuxfiles.SharedFolders{Folders: make(map[string]struct{}, 1)}
	// Clean up shared folders in the end.
	defer shareFolders.UnshareAll(cleanupCtx, tconn)

	// Share My files and click Manage in the toast nofication.
	if err := shareFolders.ShareMyfiles(ctx, tconn, filesApp, linuxfiles.MyFilesMsg, true, true); err != nil {
	...
	if err := util.CheckTwoListsEqual(shareFoldersList, linuxfiles.MyFiles); err != nil {
		s.Fatal("Failed to verify shared folders list: ", err)
	}

	// Check the file list in the containers.
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		list, err := cont.GetFileList(ctx, linuxfiles.MountPath)
		if err != nil {
			return err
		}
		if err := util.CheckTwoListsEqual(list, "fonts", linuxfiles.MountFolderMyFiles); err != nil {
			return err
		}
		return nil
	...
	if err := shareFolders.Unshare(ctx, tconn, linuxfiles.MyFiles); err != nil {
	...
	// if err := testing.Poll(ctx, func(ctx context.Context) error {
	// 	if err := util.CheckTwoListsEqual(ctx, linuxfiles.MountPath, "fonts"); err != nil {
	// 		return err
	// 	}
	// 	return nil
	// }, &testing.PollOptions{Timeout: 5 * time.Second}); err != nil {
