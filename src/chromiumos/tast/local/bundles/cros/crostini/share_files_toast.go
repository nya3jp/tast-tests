...
func ShareFilesToast(ctx context.Context, s *testing.State) {
	...
	defer func() {
		if err := sharedFolders.UnshareAll(cleanupCtx, tconn); err != nil {
	...
	if err := util.CheckListsMatch(shareFoldersList, sharedfolders.MyFiles); err != nil {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err := util.CheckListsMatch(list, "fonts", sharedfolders.MountFolderMyFiles); err != nil {
			return err
		}
		return nil
	...
	// Unshare folders. This is part of the test, different from clean up in line 72.
	if err := sharedFolders.Unshare(ctx, tconn, sharedfolders.MyFiles); err != nil {
		s.Fatal("Failed to delete shared folder My files: ", err)
	}
	if err := sharedFolders.CheckNoSharedFolders(ctx, tconn); err != nil {
		s.Fatal("Failed to verify shared folder list after unshare My files: ", err)
	}
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err := util.CheckListsMatch(list, "fonts"); err != nil {
			return err
		}
		return nil
	}, &testing.PollOptions{Timeout: 5 * time.Second}); err != nil {
