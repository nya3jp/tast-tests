...
func LifecyclePerf(ctx context.Context, s *testing.State) {
	...
	alloc, err := memory.NewCrostiniAnonAlloc(ctx, cont, 0.67)
	if err != nil {
		s.Fatal("Failed to create CrostiniAnonAlloc: ", err)
	}
	if err := alloc.Allocate(ctx, 0); err != nil {
		s.Fatal("Allocate failed: ", err)
	}
	...
	for i := 0; i < numProc; i++ {
		tasks = append(tasks, memoryuser.NewCrostiniLifecycleTask(cont, allocMiB*memory.MiB, 0.67, limit))
	}
