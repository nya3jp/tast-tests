...
func ShareMovies(ctx context.Context, s *testing.State) {
	...
	// According to local test results, it does not matter even if these two calls fail.
	// Therefore, just log the error if any.
	if err := optin.Perform(ctx, cr, tconn); err != nil {
		s.Log("Failed to optin to Play Store: ", err)
	}
	if err := optin.WaitForPlayStoreShown(ctx, tconn); err != nil {
		s.Log("Failed to wait for Play Store: ", err)
	}
