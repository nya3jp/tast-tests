...
func ResizeCancel(ctx context.Context, s *testing.State) {
	...
	originalContDS, err := cont.GetDiskSize(ctx)
	if err != nil {
		s.Fatal("Failed to get the disk size of Crostini: ", err)
	}
	...
	newContDS, err := cont.GetDiskSize(ctx)
	if err != nil {
		s.Fatal("Failed to get disk size of Crostini after cancelling resizing: ", err)
	}
