...
func shareDeviceNode(ctx context.Context, s *testing.State, pre crostini.PreData, path string, dev int) {
	if err := syscall.Mknod(path, 0o600, dev); err != nil {
		s.Fatal("Failed to create dev node: ", err)
	}
	defer os.Remove(path)

	if path, err := pre.Container.VM.ShareDownloadsPath(ctx, "dev_node", false); err == nil {
		...
		s.Error("Unexpected error when sharing a path to a non-regular file")
	}
}
...
func ShareInvalidPaths(ctx context.Context, s *testing.State) {
	...
	for _, dev := range devs {
		shareDeviceNode(ctx, s, pre, devPath, dev)
	}
}
