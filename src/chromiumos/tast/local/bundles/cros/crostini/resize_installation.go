...
func ResizeInstallation(ctx context.Context, s *testing.State) {
	...
	// Install Crostini.
	resuleDiskSize, err := ui.InstallCrostini(ctx, tconn, iOptions)
	if err != nil {
	...
	if err := verifyDiskSize(ctx, tconn, cont, resuleDiskSize); err != nil {
...
func verifyDiskSize(ctx context.Context, tconn *chrome.TestConn, cont *vm.Container, size uint64) error {
	...
	resultSize, err := settings.ParseDiskSize(sizeOnSettings)
	if err != nil {
		return errors.Wrap(err, "failed to get result disk size in bytes")
	}
	if size != resultSize {
		return errors.Errorf("failed to verify the disk size on the Settings app, got %v, want %v", resultSize, size)
	}
