...
func AppVscode(ctx context.Context, s *testing.State) {
	...
	uiauto.RecordScreen(ctx, s, tconn, func() {
		...
		if err := testCreateFileWithVSCode(ctx, terminalApp, keyboard, tconn, cont, d, s.RequiredVar(uidetection.KeyType), s.RequiredVar(uidetection.Key), s.RequiredVar(uidetection.Server)); err != nil {
...
func testCreateFileWithVSCode(ctx context.Context, terminalApp *terminalapp.TerminalApp, keyboard *input.KeyboardEventWriter, tconn *chrome.TestConn, cont *vm.Container, d screenshot.Differ, keyType, key, server string) error {
	const (
		testNewFile   = "new.go"
	...
	uda := uidetection.New(tconn, keyType, key, server)
