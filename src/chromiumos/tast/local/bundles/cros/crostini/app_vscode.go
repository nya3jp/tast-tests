...
func init() {
	testing.AddTest(&testing.Test{
		...
		VarDeps:      append(screenshot.ScreenDiffVarDeps, uidetection.UIDetectionVars...),
...
func AppVscode(ctx context.Context, s *testing.State) {
	...
	uiauto.RecordScreen(ctx, s, tconn, func() {
		...
		// TODO(b/209326914): Stabilize Screendiff for Crostini.Vscode test.
		defer func() {
			if err := d.GetFailedDiffs(); err != nil {
				s.Log("screen diffs failed: ", err)
			}
		}()
		...
		if err := testCreateFileWithVSCode(ctx, s, terminalApp, keyboard, tconn, cont, d); err != nil {
...
func testCreateFileWithVSCode(ctx context.Context, s *testing.State, terminalApp *terminalapp.TerminalApp, keyboard *input.KeyboardEventWriter, tconn *chrome.TestConn, cont *vm.Container, d screenshot.Differ) error {
	const (
		testNewFile   = "new.go"
	...
	uda := uidetection.New(tconn, s.RequiredVar(uidetection.KeyType), s.RequiredVar(uidetection.Key), s.RequiredVar(uidetection.Server))
