...
func testCopyFilesToLinuxfiles(ctx context.Context, filesApp *filesapp.FilesApp, keyboard *input.KeyboardEventWriter, cont *vm.Container) error {
	...
	if err := cont.CheckFileExists(ctx, testfiles); err != nil {
		return err
	}
	return nil
}
...
func testDeleteFileFromLinuxFiles(ctx context.Context, tconn *chrome.TestConn, filesApp *filesapp.FilesApp, cont *vm.Container, filename string) error {
	...
	if err := filesApp.SelectContextMenu(ctx, filename, filesapp.Delete); err != nil {
		errors.Wrapf(err, "failed to delete %s in Linux files", filename)
		return err
	}
	...
	if err := delete.LeftClick(ctx); err != nil {
		errors.Wrapf(err, "failed to click button Delete on file %s ", filename)
		return err
	}
	...
	if err := cont.CheckFileDoesNotExist(ctx, []string{filename}); err != nil {
...
func testRenameFileFromLinuxfiles(ctx context.Context, filesApp *filesapp.FilesApp, cont *vm.Container, keyboard *input.KeyboardEventWriter, filename, newFileName string) error {
	...
	if err := cont.CheckFileExists(ctx, []string{newFileName}); err != nil {
		return err
	}
	return nil
}
