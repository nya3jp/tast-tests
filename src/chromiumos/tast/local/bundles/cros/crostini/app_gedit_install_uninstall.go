...
func AppGeditInstallUninstall(ctx context.Context, s *testing.State) {
	...
	// Check container size before Gedit install.
	sizeBytesBeforeGedit, err := getContainerSizeInBytes(ctx)
	if err != nil {
		s.Fatal("Failed to get container size before installing Gedit: ", err)
	}
	...
	// Check container size after Gedit install.
	sizeBytesAfterGedit, err := getContainerSizeInBytes(ctx)
	if err != nil {
		s.Fatal("Failed to get container size after installing Gedit: ", err)
	}
	if err := assertSizeUnchanged(ctx, sizeBytesBeforeGedit, sizeBytesAfterGedit); err != nil {
		s.Fatal("Container size has changed unexpectedly after installing gedit: ", err)
	}
	...
	sizeBytesAfterGeditRemoved, err := getContainerSizeInBytes(ctx)
	if err != nil {
		s.Fatal("Failed to get container size after removing Gedit: ", err)
	}
	if err := assertSizeUnchanged(ctx, sizeBytesBeforeGedit, sizeBytesAfterGeditRemoved); err != nil {
		s.Fatal("Container size has changed unexpectedly after removing gedit: ", err)
	}
...
func getContainerSizeInBytes(ctx context.Context) (int, error) {
	...
	testing.ContextLog(ctx, "Here is the result: ", string(cmdOut))
	...
	if matches[0] != matches[2] {
		return 0, errors.Errorf("expected termina container sizes to match, but got two values - %s and %s", matches[0], matches[2])
	}
...
func assertSizeUnchanged(ctx context.Context, before, after int) error {
	...
	if beforeSizeGB != afterSizeGB {
		return errors.Errorf("Termina container of unexpected size- want %fGB but got %fGB", beforeSizeGB, afterSizeGB)
	}
