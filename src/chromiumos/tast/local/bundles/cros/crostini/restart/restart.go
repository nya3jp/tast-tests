...
// back up again the required number of times, ensureing that `uptime` is
// correct each time.
func RunTest(ctx context.Context, s *testing.State, cont *vm.Container, numRestarts int) {
	startupTime := getUptime(ctx, s, cont)
	...
	for i := 0; i < numRestarts; i++ {
		...
		// Compare start times.
		newStartupTime := getUptime(ctx, s, cont)
...
func getUptime(ctx context.Context, s *testing.State, cont *vm.Container) time.Time {
