...
var (
	usersStartWithLetter = []string{"a", "tester00", "whoisthis", "myuser"}

	usersStartWithUnderscore = []string{"_", "_tast", "_9", "__"}

	usersContainHyphens = []string{"_-", "t-t", "_8-", "you-_", "gogo-20-21_", "sad-_-"}

	usersContainReservedUser = []string{"daemonvsdaemon", "binbin", "ssys", "synccnys",
...
func CheckValidUsers(ctx context.Context, s *testing.State) {
	...
	m := map[string][]string{
		"users_start_with_letter":     usersStartWithLetter,
		"users_start_with_underscore": usersStartWithUnderscore,
		"users_contain_reserved_user": usersContainReservedUser,
		"long_users":                  longUsers,
	}

	for k, users := range m {
		des := fmt.Sprintf("test_%s", k)
		s.Run(ctx, des, func(ctx context.Context, s *testing.State) {
			...
			if err := checkUsersAndInstall(ctx, tconn, cr, kb, users); err != nil {
				s.Fatal("Failed to check user sand install crostini: ", err)
			}
		})
	}
