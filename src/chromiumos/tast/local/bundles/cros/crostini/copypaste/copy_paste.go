...
func RunTest(ctx context.Context, s *testing.State, tconn *chrome.Conn, cont *vm.Container, keyboard *input.KeyboardEventWriter, copy *CopyConfig, paste *PasteConfig) {
	...
	output, err := crostini.RunWindowedApp(ctx, tconn, cont, keyboard, 5*time.Second, func(ctx context.Context, c <-chan []byte) error {
		return tconn.EvalPromise(ctx, "copy_promise", nil)
	}, copyAppletTitle, append(copy.cmdArgs, copiedData))
	if err != nil {
	...
	}, pasteAppletTitle, paste.cmdArgs)
	if err != nil {
