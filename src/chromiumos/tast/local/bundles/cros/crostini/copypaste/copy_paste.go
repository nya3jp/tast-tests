...
var WaylandCopyConfig = &CopyConfig{
	gdkBackend: "wayland",
	cmdArgs:    []string{"GDK_BACKEND=wayland", "python3", copyAppletDest},
	data:       utf8Data,
}
...
var X11CopyConfig = &CopyConfig{
	gdkBackend: "x11",
	cmdArgs:    []string{"GDK_BACKEND=x11", "python3", copyAppletDest},
	data:       utf8Data,
}
...
var WaylandPasteConfig = &PasteConfig{
	gdkBackend:   "wayland",
	cmdArgs:      []string{"GDK_BACKEND=wayland", "python3", pasteAppletDest},
...
var X11PasteConfig = &PasteConfig{
	gdkBackend:   "x11",
	cmdArgs:      []string{"GDK_BACKEND=x11", "python3", pasteAppletDest},
	expectedData: utf8Data,
}
...
func RunTest(ctx context.Context, s *testing.State, tconn *chrome.Conn, cont *vm.Container, copy *CopyConfig, paste *PasteConfig) {
	...
	func() {
		...
		args := append(copy.cmdArgs, copy.data)
	...
	// TODO before submitting: Clear the clipboard at the end of the test, or verify that reseting chrome already does this, otherwise we risk one test bleeding into another
}
