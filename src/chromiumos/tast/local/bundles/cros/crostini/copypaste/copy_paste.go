...
const utf8PlainText = "text/plain;charset=utf-8"

const utf8Data = "Some data that gets copied"
...
func RunTest(ctx context.Context, s *testing.State, tconn *chrome.Conn, cont *vm.Container, copy *CopyConfig, paste *PasteConfig) {
	...
	func(timeout time.Duration, args ...string) {
		...
		keyboard.Type(ctx, " ")
	}(time.Second*5, copy.AppPath, copy.MimeType, copy.Data)
