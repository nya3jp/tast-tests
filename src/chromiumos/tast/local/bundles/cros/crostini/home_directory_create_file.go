...
func HomeDirectoryCreateFile(ctx context.Context, s *testing.State) {
	...
	defer uiauto.ScreenRecorderStopSaveRelease(ctx, screenRecorder, filepath.Join(s.OutDir(), "homeDirectoryCreateFile.webm"))
...
func testCreateFileFromContainer(ctx context.Context, tconn *chrome.TestConn, filesApp *filesapp.FilesApp, cont *vm.Container) error {
	...
	refresh := nodewith.Name("Refresh").Role(role.Button).Ancestor(filesapp.WindowFinder(filesapp.ChromeApp))
