...
func CommandCd(ctx context.Context, s *testing.State) {
	...
	// Check the output of command cd.
	content, err := cont.ReadFile(ctx, fmt.Sprintf("/home/%s/%s/%s", userName, folderName, outputFile))
	if err != nil {
		s.Fatal("Failed to cat the result file: ", err)
	}
	if content != fmt.Sprintf("/home/%s/%s", userName, folderName) {
		s.Fatal("Cd failed to take user into the newly created folder, it goes to: ", content)
	}
}
