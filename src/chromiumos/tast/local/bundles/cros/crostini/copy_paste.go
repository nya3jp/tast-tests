...
copy_promise = new Promise((resolve, reject) => {
	...
	output, err := crostini.RunWindowedApp(ctx, tconn, cont, pre.Keyboard, 5*time.Second, func(ctx context.Context) error {
	...
	output, err = crostini.RunWindowedApp(ctx, tconn, cont, pre.Keyboard, 5*time.Second, nil, false, pasteAppletTitle, param.Paste.cmdArgs)
	if err != nil {
