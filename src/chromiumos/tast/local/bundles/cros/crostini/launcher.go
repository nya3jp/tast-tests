...
func Launcher(ctx context.Context, s *testing.State) {
	...
	checkIconAbsence(ctx, s, ownerID, conf.launcherID)
	defer checkIconAbsence(ctx, s, ownerID, conf.launcherID)
	...
	if conf.installType == local {
		...
		defer crostini.RemoveContainerFileOrDie(ctx, s, cont, iconPath)
		crostini.TransferToContainerOrDie(ctx, s, cont, s.DataPath("launcher_"+conf.desktopFile), desktopPath)
		defer crostini.RemoveContainerFileOrDie(ctx, s, cont, desktopPath)
	} else {
	...
	checkIconExistence(ctx, s, ownerID, conf.launcherID)
...
func checkIconExistence(ctx context.Context, s *testing.State, ownerID, appID string) {
	...
	if err != nil {
		s.Errorf("Failed checking icons exist in %v: %v", iconDir, err)
	} else {
		s.Logf("Icon exists for %q", appID)
	}
}

// checkIconAbsence is the opposite of checkIconExistence, in that it polls until it can't find the icon.
func checkIconAbsence(ctx context.Context, s *testing.State, ownerID, appID string) {
	iconDir := filepath.Join("/home/user", ownerID, "crostini.icons", appID)
	err := testing.Poll(ctx, func(ctx context.Context) error {
		fileInfo, err := os.Stat(iconDir)
		if err != nil {
	...
	if err != nil {
		s.Errorf("Failed checking icons don't exist in %v: %v", iconDir, err)
	} else {
		s.Logf("Icon does not exist for %q", appID)
	}
}
