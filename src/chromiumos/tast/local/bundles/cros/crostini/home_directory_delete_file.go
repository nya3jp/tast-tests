...
func HomeDirectoryDeleteFile(ctx context.Context, s *testing.State) {
	...
	s.Run(ctx, "Rename from Linux files", func(ctx context.Context, s *testing.State) {
		// Rename if from Linux files and check it get renamed in container.
	...
	s.Run(ctx, "Rename from container", func(ctx context.Context, s *testing.State) {
		// Rename if from container and check it get renamed in Linux files.
...
func testDeleteFileFromLinuxFiles(ctx context.Context, filesApp *filesapp.FilesApp, cont *vm.Container, fileName string) error {
	...
	// Delete the file in Linux file and it should also be deleted from container.
...
func testDeleteFileFromContainer(ctx context.Context, filesApp *filesapp.FilesApp, cont *vm.Container, fileName string) error {
	...
	// Open "Linux files".
	if err := filesApp.OpenDir(ctx, "Linux files", "Files - Linux files"); err != nil {
		return errors.Wrap(err, "failed to open Linux files after creating files inside container")
	}
