...
func AppFirefoxInstall(ctx context.Context, s *testing.State) {
	...
	if err := transferToContainer(ctx, cont, s.DataPath(firefoxPackage), firefoxPath); err != nil {
		s.Fatal("Failed to transfer Firefox package to the container: ", err)
	}
	defer func(ctx context.Context) {
		if err := removeFromContainer(ctx, cont, firefoxPath); err != nil {
...
func transferToContainer(ctx context.Context, cont *vm.Container, source, destination string) error {
	if err := crostini.TransferToContainer(ctx, cont, source, destination); err != nil {
		return errors.Wrapf(err, "failed to transfer %s to the container: ", source)
	}

	return nil
}

func removeFromContainer(ctx context.Context, cont *vm.Container, filePath string) error {
	if err := crostini.RemoveContainerFile(ctx, cont, filePath); err != nil {
		return errors.Wrapf(err, "failed to remove %s from the container: ", filePath)
	}
