...
func CommandVim(ctx context.Context, s *testing.State) {
	...
	cmd := cont.Command(ctx, "cat", testFile)
	result, err := cmd.Output()
	if err != nil {
		s.Fatal("Failed to cat the result file: ", err)
	}
	if strings.TrimRight(string(result), "\r\n") != testString {
		s.Fatalf("The content of the file is not %s, but is %s", testString, string(result))
	}
}
...
func installVimInContainer(ctx context.Context, cont *vm.Container) error {
	...
	if err := cont.Command(ctx, "sudo", "apt", "install", "vim").Run(testexec.DumpLogOnError); err != nil {
	...
	cmd := cont.Command(ctx, "vim", "--version")
	result, err := cmd.Output()
	if err != nil {
		return errors.Wrap(err, "failed to run command vim --version in container")
	}
	if strings.Contains(string(result), "No such file or directory") {
		return errors.Errorf("Vim was not successfully installed in container, running vim --version got: %q", string(result))
	}
	return nil
}
