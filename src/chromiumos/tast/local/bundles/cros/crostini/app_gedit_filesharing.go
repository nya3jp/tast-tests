...
func AppGeditFilesharing(ctx context.Context, s *testing.State) {
	...
	// Run the tests that test sharing behavior prior to shutting down Crostini.
	checkFilesharingBeforeRestart(
		ctx, cont, tconn, ui, ud, filesApp, keyboard, geditWindow, filesAppShelfButton, tmpFilename, tmpFileContents, tmpFileCrostiniMountPath, defaultTextEditorContextMenuItem)
	...
	// Run remaining tests.
	if err = checkFilesharingWorksAfterRestart(
		ctx, cont, tconn, filesApp, keyboard, ui, ud, geditWindow, tmpFilename, tmpFileContents, tmpFileCrostiniMountPath, geditContextMenuItem); err != nil {
		s.Fatal("Failed tests cases after restart of Crostini: ", err)
	}
}
...
func checkFilesharingBeforeRestart(
	...
	geditWindow,
	filesAppShelfButton *nodewith.Finder,
	tmpFilename,
	tmpFileContents,
	tmpFileCrostiniMountPath,
	defaultTextEditorContextMenuItem string) error {
	...
	if _, err := bringWindowToForeground(ctx, tconn, "Files - Downloads"); err != nil {
...
func bringWindowToForeground(ctx context.Context, tconn *chrome.TestConn, windowTitle string) (*ash.Window, error) {
	predicate := func(w *ash.Window) bool {
		return w.Title == windowTitle
	}

	w, err := ash.FindWindow(ctx, tconn, predicate)
	if err != nil {
