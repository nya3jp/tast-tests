...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:     "Ran a test to make sure linux does not have access to drive on chrome using a pre-built crostini image",
		Contacts: []string{"jinrong@chromium.org"},
...
func NoaccessToDrive(ctx context.Context, s *testing.State) {
	...
	if err := filesApp.DeleteAFileOrFolder(shortCtx, newfolder); err != nil {
		s.Error("Failed to delete the new folder: ", err)
	}
}
