...
func getOffsets(ctx context.Context, filepath string, smallPattern, bigPattern []byte) (_, _ []int64, resultError error) {
	...
	defer func() {
		if err := file.Close(); err != nil && resultError == nil {
			resultError = err
		}
	}()
...
func writeAtSync(filepath string, b []byte, offsets []int64) (resultError error) {
...
func testOverwriteAtOffsets(ctx context.Context, tconn *chrome.TestConn, userName string, offsets []int64, container *vm.Container, diskPath, backupPath, outDir string) (resultError error) {
	...
	defer func() {
		if err := signalWatcher.Close(ctx); err != nil && resultError == nil {
			resultError = err
		}
	}()
	...
	defer func() {
		if err := container.VM.Stop(ctx); err != nil && resultError == nil {
			resultError = err
		}
	}()
