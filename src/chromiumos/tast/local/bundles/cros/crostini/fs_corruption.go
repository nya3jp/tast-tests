...
func scanBuffer(baseOffset int64, existingOffsets []int64, buffer, pattern []byte) []int64 {
	for {
		...
		// possible some of the offsets found here have
		// already been added to the result and we
		// shouldn't duplicate them.
		if len(existingOffsets) == 0 || existingOffsets[len(existingOffsets)-1] < newOffset {
			existingOffsets = append(existingOffsets, newOffset)
		}
...
func getOffsets(ctx context.Context, filepath string, smallPattern, bigPattern []byte) ([]int64, []int64, error) {
	...
	for i = 0; i < size; i += oneMiB {
		...
		smallOffsets = scanBuffer(i, smallOffsets, buffer, smallPattern)
		bigOffsets = scanBuffer(i, bigOffsets, buffer, bigPattern)
	}
...
func testOverwriteAtOffsets(ctx context.Context, tconn *chrome.TestConn, userName string, offsets []int64, container *vm.Container, diskPath, backupPath, outDir string) error {
	...
	if terminalapp, _ := terminalapp.Launch(ctx, tconn, userName); terminalapp != nil {
		// Make sure the UI node in terminalapp is released.
		defer terminalapp.Root.Release(ctx)
	}
