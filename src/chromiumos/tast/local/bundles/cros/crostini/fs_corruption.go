...
func testOverwriteAtOffsets(ctx context.Context, tconn *chrome.TestConn, userName string, offsets []int64, container *vm.Container, diskPath, backupPath, outDir string) (resultError error) {
	...
	terminal, _ := terminalapp.Launch(ctx, tconn, userName)

	// Stop the VM before returning from this function by a direct
	// call to concierge since we want to ensure it isn't running
	// even when there may be an error in the startup process.
	defer func() {
	...
	if terminal != nil {
		...
		// terminalapp.Launch will only return a non-error
		// value if the terminal launched and connected to the
		// VM correctly. However, if an error occurs the
		// terminal window the will remain open, and we won't
		// have a terminal object to call Close on.
		//
		// To avoid leaving these windows around, close them
		// here using the apps interface.
