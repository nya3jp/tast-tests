...
func ResizeBackupRestore(ctx context.Context, s *testing.State) {
	...
	if err = cont.WriteFile(ctx, existingFile, existingFileStr); err != nil {
		s.Fatal("Failed to write file "+existingFile, err)
	}
	...
	if err != nil {
		s.Fatal("Failed to read checksum output file "+checksumPreFile+": ", err)
	}
	s.Log("Pre checksum\n" + checksumPreStr)
	...
	defer func() { faillog.DumpUITreeAndScreenshot(cleanupCtx, tconn, "resize_backup_restore", err) }()
	...
	if err = keyboard.TypeAction(backupFileBaseName)(ctx); err != nil {
		s.Fatal("Failed to enter backup name "+backupFileBaseName+": ", err)
	}
	...
	if err = cont.RemoveAll(ctx, existingFile); err != nil {
		s.Fatal("Failed to remove "+existingFile+" from container:", err)
	}
