...
func DragDrop(ctx context.Context, s *testing.State) {
	...
	if err = dragFromFilesApp(ctx, s, files, dirDragFromFilesapp, "['file:///mnt/chromeos/MyFiles/filesappdir']"); err != nil {
		s.Fatalf("Failed to drag %s from FilesApp to crostini: %v", dirDragFromFilesapp, err)
	}
	if err = dragFromFilesApp(ctx, s, files, fileDragFromFilesapp, "['file:///mnt/chromeos/MyFiles/filesapp.txt']"); err != nil {
	...
	if err = dragFromCrostini(ctx, s, files, dirDragFromCrostini, false); err != nil {
		s.Fatalf("Failed to drag %s from crostini to FilesApp: %v", dirDragFromCrostini, err)
	}
	if err = dragFromCrostini(ctx, s, files, fileDragFromCrostini, true); err != nil {
...
func dragFromFilesApp(ctx context.Context, s *testing.State, files *filesapp.FilesApp, path, expected string) error {
	pre := s.PreValue().(crostini.PreData)
	tconn := pre.TestAPIConn
	cont := pre.Container
	kb := pre.Keyboard
...
func dragFromCrostini(ctx context.Context, s *testing.State, files *filesapp.FilesApp, path string, isFile bool) error {
	pre := s.PreValue().(crostini.PreData)
	tconn := pre.TestAPIConn
	cont := pre.Container
