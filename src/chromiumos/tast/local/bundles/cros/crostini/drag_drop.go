...
func init() {
	...
	testing.AddTest(&testing.Test{
		Func:         DragDrop,
		Desc:         "Drag from FilesApp and drop in crostini applet",
...
func DragDrop(ctx context.Context, s *testing.State) {
	...
	// Setup the test file.
	testFileLocation := filepath.Join(filesapp.MyFilesPath, testFileName)
	if err := ioutil.WriteFile(testFileLocation, []byte(testFileContent), 0644); err != nil {
	...
	if err = files.DragAndDropFile(ctx, tconn, testFileName, dropPoint); err != nil {
	...
	contPath := filepath.Join(sharedfolders.MountPathMyFiles, testFileName)
	if err = cont.CheckFileContent(ctx, contPath, testFileContent); err != nil {
		s.Fatalf("Drag and drop file %s not shared with VM: %v", testFileName, err)
	}
}
