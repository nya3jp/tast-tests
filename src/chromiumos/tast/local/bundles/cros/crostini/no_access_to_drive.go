...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:     "Run a test to make sure crostini does not have access to drive on chrome using a pre-built crostini image",
...
func NoAccessToDrive(ctx context.Context, s *testing.State) {
	...
	newFolder := fmt.Sprintf("NoAccessToDrive_%d", rand.Intn(10000000000))
	...
	if len(fileList) != 0 {
		s.Fatal("Home directory in the container is not empty after creating a folder in Drive in Files app: got ", fileList)
	}
