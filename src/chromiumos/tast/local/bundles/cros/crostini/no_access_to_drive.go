...
func NoAccessToDrive(ctx context.Context, s *testing.State) {
	...
	newfolder := fmt.Sprintf("New%d", rand.Intn(10000000000))
	s.Log("The new folder name is ", newfolder)
	// Create a new folder in Drive
	if err := createFolderInDrive(shortCtx, filesApp, keyboard, newfolder); err != nil {
		s.Fatal("Failed to create a folder in Drive: ", err)
	}

	defer filesApp.DeleteFileOrFolder(shortCtx, newfolder)
	...
	if fileList != "" {
		s.Fatal("Home directory in container is not empty after creating a folder in Drive in Chrome: got ", fileList)
	}
...
func createFolderInDrive(shortCtx context.Context, filesApp *filesapp.FilesApp, keyboard *input.KeyboardEventWriter, newfolder string) error {
	// Open Google Drive
