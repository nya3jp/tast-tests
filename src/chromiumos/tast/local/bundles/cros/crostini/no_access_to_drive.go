...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:     "Run a test to make sure crostini does not have access to GoogleDrive using a pre-built crostini image",
...
func NoAccessToDrive(ctx context.Context, s *testing.State) {
	...
	if len(fileList) != 0 {
		s.Fatalf("Home directory in the container is not empty after creating a folder in Drive in Files app: got %q", fileList)
	}
