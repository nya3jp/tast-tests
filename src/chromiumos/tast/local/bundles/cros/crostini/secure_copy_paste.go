...
func SecureCopyPaste(ctx context.Context, s *testing.State) {
	...
	var blockerWindowMaximised = false
	for _, w := range ws {
		if strings.Contains(w.Title, "/secure_blocker.html") {
			ash.SetWindowState(ctx, tconn, w.ID, ash.WMEventMaximize)
			blockerWindowMaximised = true
			break
		}
	}
	if !blockerWindowMaximised {
		s.Fatal("Failed to find the secure_blocker window to maximise")
	}
