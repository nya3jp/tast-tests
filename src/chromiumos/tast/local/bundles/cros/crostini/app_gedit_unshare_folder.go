...
func AppGeditUnshareFolder(ctx context.Context, s *testing.State) {
	...
	if err = geditAshWindow.CloseWindow(ctx, tconn); err != nil {
		// Since the folder is no longer shared, the "ctrl+S" keyboar action
		...
		//  The absence of such a dialog indicates that
		// the "ctrl+S" above succeeded. If "ctrl+s" succeeds, it means the unshare
		// operation failed, which means this test fails.
