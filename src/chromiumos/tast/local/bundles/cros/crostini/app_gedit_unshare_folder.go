...
func AppGeditUnshareFolder(ctx context.Context, s *testing.State) {
	...
	// linuxSettings, err := settings.OpenLinuxSettings(ctx, tconn, cr) // TODO(is this redundant? Can we open shared folders directly?)
	// defer linuxSettings.Close(ctx)
	// if err != nil {
	// 	s.Fatal("Failed to open Linux setttings page: ", err)
	// }

	// We need to open the Manage Shared Folders settings page before we can unshare.
	...
	folders, err := linuxSettings.GetSharedFolders(ctx)
	if err != nil {
		s.Fatal("Failed to retrieve list of shared folders: ", err)
	}
	if len(folders) != 0 {
		s.Fatal("Failed to unshare folders as expected zero folders shared with Linux, but got non-zero value: ", err)
	}
	...
	// is expected fail because  a "save file as" dialog window remains open over the Gedit window.
	// The absense of such a dialog indicates that the test fails.
