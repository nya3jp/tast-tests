...
func InputLatency(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < numPings; i++ {
		...
		if rtt, err := pingGuest(); err != nil {
			...
			badPings++
			if badPings >= maxBadPings {
				s.Fatalf("Too many (%v) failed pings", maxBadPings)
			}
		} else {
			sumRTT += rtt
		}
	}
	avgRTT := sumRTT / time.Duration(numPings-badPings)
	...
	for i := 0; i < numKeystrokes; i++ {
		...
		if err != nil {
			...
			badKeystrokes++
			if badKeystrokes >= maxBadPings {
				s.Fatalf("Too many (%v) failed input latency measurements", maxBadKeystrokes)
			}
			continue
		}
	...
	avgKeyLatency := sumKeyLatency / time.Duration(numKeystrokes-badKeystrokes)
...
func startInputLatencyServer(ctx context.Context, cont *vm.Container, socketServerFile, socketServerLog fileMapping,
	...
	socketServerArgs := []string{"xterm", "-e", fmt.Sprintf("/usr/bin/python3 %v >%v 2>&1", socketServerFile.guestPath, socketServerLog.guestPath)}
