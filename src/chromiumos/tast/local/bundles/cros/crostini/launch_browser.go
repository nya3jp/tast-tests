...
func LaunchBrowser(ctx context.Context, s *testing.State) {
	...
	checkLaunch := func(urlTarget string, command ...string) {
		...
		conn, _, closeBrowser, err := browserfixt.SetUpWithURL(ctx, s.FixtValue().(crostini.FixtureData).ParentFixtV, s.Param().(browser.Type), urlTarget)
		if err != nil {
			s.Fatal("Didn't see crosh renderer: ", err)
		} else {
			conn.Close()
		}
		closeBrowser(ctx)
