...
func MousePerf(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < numDeltas; i++ {
		...
		distances = append(distances, (outputParsed[next]-outputParsed[cur])+(outputParsed[next+1]-outputParsed[cur+1]))
...
func processStats(samples []float64) stats {
	...
	return stats{
		...
		LowerBound:        lb,
		Average:           sum / n,
		StandardDeviation: math.Sqrt((sumSquared - sum*sum/n) / (n - 1)),
	}
}
