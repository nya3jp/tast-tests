...
func RoutineSection(ctx context.Context, s *testing.State) {
	...
	if err := ui.WithTimeout(time.Minute).WaitUntilExists(
		diagnosticsapp.DxProgressBadge.Ancestor(diagnosticsapp.DxRootNode).First())(ctx); err != nil {
		s.Fatal("Could not verify test routine has started: ", err)
	}

	if err := ui.WithTimeout(5 * time.Minute).WaitUntilExists(
		diagnosticsapp.DxPassedBadge.Ancestor(diagnosticsapp.DxRootNode).First())(ctx); err != nil {
	...
	if err := ui.WithTimeout(20 * time.Second).WaitUntilExists(
		diagnosticsapp.DxCancelledBadge.Ancestor(diagnosticsapp.DxRootNode).First())(ctx); err != nil {
