...
func attemptToSaveSessionLog(ctx context.Context, tconn *chrome.TestConn, s *testing.State) {
	...
	if err != nil {
		// Return early since session log was saved and the file picker is now closed.
		return
	}
...
func SessionLog(ctx context.Context, s *testing.State) {
	...
	// Try save action multiple times since the "Save" button occasionally fails
	// to respond to the "Enter" key press.
	for i := 0; i < 2; i++ {
		attemptToSaveSessionLog(ctx, tconn, s)
	}
