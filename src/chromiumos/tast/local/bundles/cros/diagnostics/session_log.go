...
func clickSaveButton(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if err := ui.ImmediateLeftClick(saveButton)(ctx); err != nil {
		return err
	}
...
func SessionLog(ctx context.Context, s *testing.State) {
	...
	if err := ui.WithTimeout(20 * time.Second).WaitUntilExists(sessionLogButton)(ctx); err != nil {
		s.Fatal("Failed to find the session log button: ", err)
	}

	// If needed, scroll down to make the session log visible.
	if err := ui.MakeVisible(sessionLogButton)(ctx); err != nil {
		s.Fatal("Failed to locate session log within the screen bounds: ", err)
	}

	// Click session log button.
	pollOpt := testing.PollOptions{Interval: time.Second, Timeout: 20 * time.Second}
	if err := ui.WithPollOpts(pollOpt).LeftClick(sessionLogButton)(ctx); err != nil {
