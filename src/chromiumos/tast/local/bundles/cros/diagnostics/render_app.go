...
func RenderApp(ctx context.Context, s *testing.State) {
	...
	if err := ui.WithTimeout(20 * time.Second).WaitUntilExists(
		diagnosticsapp.DxCPUChart.Ancestor(diagnosticsapp.DxRootNode).First())(ctx); err != nil {
		s.Fatal("Failed to find CPU chart: ", err)
	}

	// Verify test routine button is rendered
	if err := ui.WithTimeout(20 * time.Second).WaitUntilExists(
		diagnosticsapp.DxCPUTestButton.Ancestor(diagnosticsapp.DxRootNode).First())(ctx); err != nil {
		s.Fatal("Failed to find cpu routine button: ", err)
	}

	if err := ui.WithTimeout(20 * time.Second).WaitUntilExists(
		diagnosticsapp.DxMemoryTestButton.Ancestor(diagnosticsapp.DxRootNode).First())(ctx); err != nil {
	...
	if err := ui.WithTimeout(20 * time.Second).WaitUntilExists(
		diagnosticsapp.DxLogButton.Ancestor(diagnosticsapp.DxRootNode).First())(ctx); err != nil {
