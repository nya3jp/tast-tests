...
func RenderApp(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Diagnostics app did not appear in shelf after launch: ", err)
	}

	dxRootnode, err := diagnosticsapp.DiagnosticsRootNode(ctx, tconn)
	if err != nil {
		s.Fatal("Failed to find diagnostics app: ", err)
	}
