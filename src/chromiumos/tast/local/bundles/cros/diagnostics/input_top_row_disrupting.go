...
func InputTopRowDisrupting(ctx context.Context, s *testing.State) {
	...
	checkMuted := func(muted bool) action.Action {
		return func(ctx context.Context) error {
			...
			if newMuted != muted {
				return errors.Errorf("muted is %t, want %t", newMuted, muted)
			}
			return nil
		}
	}
	...
	if app == "Chrome" {
		...
		// Assigne the expected window state based on current state.
		...
		if err := uiauto.Combine("verify top row keys for browser",
			...
			checkWindowState("Chrome - Google", winState), // Check new window state.
			bChecker.set(),
	...
	if app == "Refresh" {
		...
		winState, err := getWindowState("Chrome - Google")
		if err != nil {
			s.Fatal("Failed to get window state: ", err)
		}
		// Assigne the expected window state based on current state.
		...
		startTime := time.Now()
		if err := uiauto.Combine("verify top refresh key for browser",
			func(ctx context.Context) error {
				if err := webutil.WaitForQuiescence(ctx, conn, 10*time.Second); err != nil {
					return errors.Wrap(err, "failed to wait for page load")
				}
				testing.ContextLog(ctx, "WaitForQuiescence time before refresh: ", time.Now().Sub(startTime))
				// Ensure browser goes back to Google.
				_, err := getWindowState("Chrome - Google")
				return err
			},
			kb.AccelAction(topRow.BrowserRefresh),
			func(ctx context.Context) error {
				if err := webutil.WaitForQuiescence(ctx, conn, 10*time.Second); err != nil {
					return errors.Wrap(err, "failed to wait for page load")
				}
				testing.ContextLog(ctx, "WaitForQuiescence time after refresh: ", time.Now().Sub(startTime))
				// Ensure browser goes back to Google.
				_, err := getWindowState("Chrome - Google")
				return err
			},
