...
// UnzipTestFiles extracts test data files to the target directory and returns an array containing the files' base names.
...
// CrosTestFiles prepares test files to send from a CrOS device. The test files will be staged in nearbyshare.SendDir,
...
func CrosTestFiles(ctx context.Context, zipPath string) ([]string, error) {
	...
	if err := os.Mkdir(targetPath, os.FileMode(int(0711))); err != nil {
		if os.IsExist(err) {
			testing.ContextLogf(ctx, "Skipping creating %v, path already exists", targetPath)
		} else {
