...
function () {
  return {
    ...
    // page's iron-list, where share target info is exposed to the page.
    getShareTarget(name) {
      shareTargets = document.querySelector("nearby-share-app").shadowRoot
        .querySelector("nearby-discovery-page")
        .shadowRoot.querySelectorAll("nearby-device");
      target = null;
      shareTargets.forEach(element => {
        t = element.__data.shareTarget;
        if (t.name == name) {
          target = t;
        }
      });
