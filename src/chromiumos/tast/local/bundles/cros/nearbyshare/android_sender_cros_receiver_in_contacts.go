...
func AndroidSenderCrosReceiverInContacts(ctx context.Context, s *testing.State) {
	...
	// TODO(crbug/1171010): this test assumes the Android device is signed in as user "crosnearbytest@gmail.com". Explicitly sign in when Android account management is added.
	rooted := true
	...
	if err := nearbyshare.AwaitReceivingCompleteNotification(ctx, tconn, androidDisplayName, 10*time.Second); err != nil {
