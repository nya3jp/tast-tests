...
// CrosToAndroidFiles tests file sharing with a CrOS device as sender and Android device as receiver.
func CrosToAndroidFiles(ctx context.Context, s *testing.State) {
	...
	// Set up the connected Android device. Don't override GMS Core flags or perform settings changes that require root access if specified in the runtime vars.
	rooted := true
	...
	testDataZip := s.DataPath(s.Param().(string))
	filenames, err := nearbytestutils.CrosTestFiles(ctx, testDataZip)
	if err != nil {
