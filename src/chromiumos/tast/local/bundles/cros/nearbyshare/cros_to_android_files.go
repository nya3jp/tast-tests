...
func CrosToAndroidFiles(ctx context.Context, s *testing.State) {
	...
	androidDevice, err := nearbysetup.AndroidSetup(ctx, s.DataPath(nearbysnippet.ZipName), rooted, nearbysetup.DefaultScreenTimeout,
		nearbysnippet.DataUsageOffline, nearbysnippet.VisibilityAllContacts, androidDisplayName,
	)
	if err != nil {
	...
	filenames, err := nearbytestutils.CrosTestFiles(ctx, s.DataPath(s.Param().(string)))
	if err != nil {
	...
	if err := nearbytestutils.FileHashComparison(ctx, filenames, nearbyshare.SendDir, nearbysnippet.DownloadDir, androidDevice); err != nil {
