...
// UIHighVisReceive tests that we can receive a file from an out-of-contacts Android device in high-visibility mode, using the UI to drive CrOS Nearby.
func UIHighVisReceive(ctx context.Context, s *testing.State) {
	// cr, err := chrome.New(
	// 	ctx,
