...
// The returned function slice contains cleanup functions that should be called whether or not an error is returned to ensure all resources are freed.
func New(ctx context.Context, d *adb.Device, apkZipPath string, overrideGMS bool) (*AndroidNearbyDevice, []func(ctx context.Context) error, error) {
	...
	if err != nil {
		return a, cleanup, errors.Wrap(err, "failed to create temp dir")
	}
	defer os.RemoveAll(tempDir)
	if err := testexec.CommandContext(ctx, "unzip", apkZipPath, ApkName, "-d", tempDir).Run(testexec.DumpLogOnError); err != nil {
		return a, cleanup, errors.Wrapf(err, "failed to unzip %v from %v", ApkName, apkZipPath)
	}
	...
	if err := a.device.Install(ctx, filepath.Join(tempDir, ApkName), adb.InstallOptionGrantPermissions); err != nil {
		return a, cleanup, errors.Wrap(err, "failed to install Nearby Snippet APK on the device")
	}
	...
	if err := a.LaunchSnippet(ctx); err != nil {
		return a, cleanup, errors.Wrap(err, "failed to launch snippet server on device")
	}
	cleanup = append(cleanup, a.StopSnippet)

	// Forward the Nearby Snippet's listening port to the host.
	if err := a.ForwardPort(ctx); err != nil {
		return a, cleanup, err
	}
	cleanup = append(cleanup, a.ReleasePort)

	// Create a TCP connection to the Nearby Snippet.
	if err := a.TCPConn(ctx); err != nil {
		return a, cleanup, err
	}
	cleanup = append(cleanup, a.CloseTCPConn)
