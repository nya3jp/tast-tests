			...
			if param.wantChecked == checked.True {
				...
				for _, address := range addressValues {
					...
					if err := conn.Eval(ctx, "document.getElementById('"+address.htmlFieldID+"').value", &valueFromHTML); err != nil {
						s.Fatalf("Failed to complete the JS test for htmlFieldID=%d. Error: %e", address.htmlFieldID, err)
					}
					if valueFromHTML != address.fieldValue {
						s.Fatalf("Address was not set properly. Actual value %d doesnt match with expected %e", valueFromHTML, address.fieldValue)
					}
				}
			}
		})
