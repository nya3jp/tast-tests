...
func SystemTimezone(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			expectedRestriction := ui.RestrictionNone
			if timezoneSet {
				expectedRestriction = ui.RestrictionDisabled
			}
			expectedSelection := "Set automatically"
			if timezoneSet {
				expectedSelection = "Choose from list"
			}
