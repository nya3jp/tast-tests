...
const (
	backgroundJS = "screen_shooter_extension/background.js"
	contentJS    = "screen_shooter_extension/content.js"
	manifestJSON = "screen_shooter_extension/manifest.json"
)

var extensionFiles = []string{backgroundJS, contentJS, manifestJSON}
...
func DisableScreenshotsExtension(ctx context.Context, s *testing.State) {
	...
	s.Logf("FakeDMS starting in %s", tmpdir)
	...
	for _, tc := range []struct {
		...
		if title != tc.wantTitle {
			s.Fatalf("Unexpected title: get %q; want %q", title, tc.wantTitle)
		}
	}
}
