...
func (c *PolicyService) EnrollUsingChrome(ctx context.Context, req *ppb.EnrollUsingChromeRequest) (*empty.Empty, error) {
	...
	if req.SkipLogin {
		opts = append(opts, chrome.Enroll(chrome.Creds{User: user, Pass: "test0000", GAIAID: "gaia-id"}))
	} else {
		opts = append(opts, chrome.EnrollLogin(chrome.Creds{User: user, Pass: "test0000", GAIAID: "gaia-id"}))
	}
