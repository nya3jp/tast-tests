...
func (c *PolicyService) GAIAEnrollUsingChrome(ctx context.Context, req *ppb.GAIAEnrollUsingChromeRequest) (*empty.Empty, error) {
	testing.ContextLogf(ctx, "Enrolling using Chrome with username %s", string(req.Username))
	testing.ContextLogf(ctx, "Enrolling using Chrome with dmserver %s", string(req.DmserverURL))
