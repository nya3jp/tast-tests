...
func (c *PolicyService) VerifyVisibleNotification(ctx context.Context, req *ppb.VerifyVisibleNotificationRequest) (*empty.Empty, error) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		for _, notification := range notifications {
			testing.ContextLogf(ctx, "Waiting for notification with id %s %s", req.NotificationId, notification.ID)
		...
		return nil, errors.Wrap(err, "failed to verify notification")
	}
...
func (c *PolicyService) EvalExpressionInChromeURL(ctx context.Context, req *ppb.EvalExpressionInChromeUrlRequest) (*empty.Empty, error) {
	...
	if req.Url == "" {
		return nil, errors.New("URL is empty in the request object")
	}
	if req.Expression == "" {
		return nil, errors.New("expression is empty in the request object")
	}
