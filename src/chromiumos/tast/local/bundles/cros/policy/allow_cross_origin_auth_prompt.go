...
// tmpHTMLPage returns html template with specified port.
...
func tmpHTMLPage(port string) string {
...
func AllowCrossOriginAuthPrompt(ctx context.Context, s *testing.State) {
	...
	// Extract the port number generated from the auth server.
	u, err := url.Parse(authServer.URL)
	_, port, err := net.SplitHostPort(u.Host)

	// Create a server that will serve html page with 3rd party image (http://localhost:port).
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/html")
		io.WriteString(w, tmpHTMLPage(port))
	}))
	defer server.Close()
