...
func init() {
	testing.AddTest(&testing.Test{
		...
		Pre:          pre.User,
		Data:         []string{"allow_cross_origin_auth_prompt.jpeg", "allow_cross_origin_auth_prompt_index.html"},
	})
}
...
func AllowCrossOriginAuthPrompt(ctx context.Context, s *testing.State) {
	...
	// Load the image to serve.
	img, err := os.Open(s.DataPath("allow_cross_origin_auth_prompt.jpeg"))
	if err != nil {
		s.Error("Failed to load test image: ", err)
	}
	defer img.Close()
	...
	if err != nil {
		s.Error("Failed to create a listener with port 8080: ", err)
	}
	...
	authServer.Listener.Close()
	authServer.Listener = l
	authServer.Start()
