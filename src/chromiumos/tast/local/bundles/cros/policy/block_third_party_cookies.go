...
func BlockThirdPartyCookies(ctx context.Context, s *testing.State) {
	...
	// These certificate are signed with the same CA key, but specify 127.0.0.1 instead of localhost
	...
	if err != nil {
		s.Fatal("Could read html template file")
	}
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			ui := uiauto.New(tconn)
			localhostButton := nodewith.Name("localhost").Role(role.Button)
			ipButton := nodewith.Name("127.0.0.1").Role(role.Button)
