...
func TranslateEnabled(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			isTranslate, err := ui.Exists(ctx, tconn, ui.FindParams{
				Role: ui.RoleTypeButton,
				Name: "Translate this page",
			})
			if err != nil {
				s.Fatal("Error during checking for UI Compontent to translate. Error: ", err)
			}

			if isTranslate != param.wantTranslate {
				s.Errorf("Wrong visibility for translated gadget: got %t; want %t", isTranslate, param.wantTranslate)
			}
		})
	}
}
