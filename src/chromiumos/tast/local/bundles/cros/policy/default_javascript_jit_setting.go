...
func DefaultJavascriptJitSetting(ctx context.Context, s *testing.State) {
	...
	for _, tc := range []struct {
		...
		s.Run(ctx, tc.name, func(ctx context.Context, s *testing.State) {
			...
			jitAllowed := false
			// We need this error to let Poll execute until the condition is true or ultimately fail after the polling timeout.
			jitEvalFalseErr := errors.New("isJitEnabled() evaluated to false")
			if err := testing.Poll(ctx, func(ctx context.Context) error {
				if err := conn.Eval(ctx, "isJitEnabled()", &jitAllowed); err != nil {
					return testing.PollBreak(err)
				}
				if !jitAllowed {
					return jitEvalFalseErr
				}
				return nil
			}, &testing.PollOptions{Interval: 10 * time.Millisecond, Timeout: 1 * time.Second}); err != nil {
				if !errors.Is(err, jitEvalFalseErr) {
