...
func AllowDinosaurEasterEgg(ctx context.Context, s *testing.State) {
	checkAllowDinosaurEasterEgg := func(ctx context.Context, s *testing.State, value *policy.AllowDinosaurEasterEgg) {
		...
		if isBlocked && !(value.Stat == policy.StatusUnset) && value.Val {
			s.Error("Incorrect behavior: Dinosaur game was blocked")
		}
		// False or Unset: game is blocked.
		if !isBlocked && ((value.Stat == policy.StatusUnset) || !value.Val) {
			s.Error("Incorrect behavior: Dinosaur game was not blocked")
		}
	}
