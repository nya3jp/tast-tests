...
func FullscreenAllowed(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if err := kb.Accel(ctx, "F3"); err != nil {
			...
			fullscreenEnabled := isFullScreen == true
			if fullscreenEnabled != param.wantFullscreenEnabled {
				s.Errorf("Unexpected button checked state: got %v, want %v", fullscreenEnabled, param.wantFullscreenEnabled)
			}
