...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Behavior of NotificationsAllowedForUrls policy",
		Contacts: []string{
		...
		Pre:          pre.User,
		Data:         []string{"notification_test_page.html"},
	})
}
...
func NotificationsAllowedForUrls(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		name                 string
		notification_allowed bool            // whether the notifications are allowed or not.
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			conn, err := cr.NewConn(ctx, server.URL + "/notification_test_page.html")
			if err != nil {
			...
			if notification_allowed != param.notification_allowed {
				s.Errorf("Unexpected outcome: got %v; want %v", notification_allowed, param.notification_allowed)
			}
		})
	}
}
