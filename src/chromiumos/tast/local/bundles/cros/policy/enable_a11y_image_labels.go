...
func EnableA11yImageLabels(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			// Make sure we display the webpage and the image on screen before attempting to select it.
			ui := uiauto.New(tconn)
			...
			// We only expect the image labels to work if the policy is set to true,
			// in the other two cases, we should get a message describing how to get
			// missing image descriptions.
			var expectation a11y.SpeechExpectation
			if param.value.Val == true {
				// The image description may not load fast enough for the test, so we
				// might get a message that the description is being fetched. This is
				// still a success, as we would not even send that request if the policy
				// was not enabled.
				expectation = a11y.NewRegexExpectation("(Appears to be|Getting description).*")
			} else {
				expectation = a11y.NewRegexExpectation(".*missing image.*")
			}

			// Test that we get the expected spoken feedback.
			if err := a11y.PressKeysAndConsumeExpectations(ctx, sm,
				[]string{"Search+Right"},
				[]a11y.SpeechExpectation{expectation},
			); err != nil {
