...
func PrintingBackgroundGraphicsDefault(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		name                               string
		wantPrintBackgroundGraphicsChecked checked.Checked
		policy                             *policy.PrintingBackgroundGraphicsDefault
	}{
		{
			name:                               "enabled",
			wantPrintBackgroundGraphicsChecked: checked.True,
			policy:                             &policy.PrintingBackgroundGraphicsDefault{Val: "enabled"},
		},
		{
			name:                               "disabled",
			wantPrintBackgroundGraphicsChecked: checked.False,
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if err := uiauto.Combine("open print preview",
				...
				ui.WaitUntilExists(backgroundGraphicsCheckbox),
			)(ctx); err != nil {
			...
			if nodeInfo.Checked != param.wantPrintBackgroundGraphicsChecked {
				s.Errorf("Unexpected state of the 'Background graphics' checkbox: got %s; want %s", nodeInfo.Checked, param.wantPrintBackgroundGraphicsChecked)
			}
		})
	}
}
