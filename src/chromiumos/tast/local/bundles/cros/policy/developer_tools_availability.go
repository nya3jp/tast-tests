...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Behavior of the DeveloperToolsAvailability policy, check whether developer tools can be open on chrome:// page",
		Contacts: []string{
		...
		s.Run(ctx, tc.name, func(ctx context.Context, s *testing.State) {
			...
			for _, keys := range []string{
				...
				s.Run(ctx, keys, func(ctx context.Context, s *testing.State) {
					...
					if err := ui.WaitUntilExists(ctx, tconn, ui.FindParams{Name: "User Action", Role: ui.RoleTypeColumnHeader}, 5*time.Second); err != nil {
					...
					case false:
						...
						if exists, err := ui.Exists(ctx, tconn, ui.FindParams{Name: "Elements", Role: ui.RoleTypeTab}); err != nil {
							s.Fatal("Failed to check whether DevTools are available: ", err)
						} else if exists {
							s.Fatal("Unexpected DevTools availability: get allowed; want disallowed")
						}
					case true:
						if err := ui.WaitUntilExists(ctx, tconn, ui.FindParams{Name: "Elements", Role: ui.RoleTypeTab}, 5*time.Second); err != nil {
							s.Fatal("Failed to wait for DevTools: ", err)
						}
					}
				})
			}
