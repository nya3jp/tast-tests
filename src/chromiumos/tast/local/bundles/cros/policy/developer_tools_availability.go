...
func DeveloperToolsAvailability(ctx context.Context, s *testing.State) {
	...
	for _, tc := range []struct {
		...
		s.Run(ctx, tc.name, func(ctx context.Context, s *testing.State) {
			...
			// The current hotkey order eventually leaves DevTools open. We have
			// to go through all hotkeys to verify that none of them can open
			// DevTools if it isn't allowed by policy.
