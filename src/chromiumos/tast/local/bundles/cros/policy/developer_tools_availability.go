...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Behavior of the DeveloperToolsAvailability policy, check whether developer tools can be open on chrome://user-actions page",
		Contacts: []string{
		...
		s.Run(ctx, tc.name, func(ctx context.Context, s *testing.State) {
			...
			for _, keys := range []string{
				...
				s.Run(ctx, keys, func(ctx context.Context, s *testing.State) {
					...
					userActionParams := ui.FindParams{Name: "User Action", Role: ui.RoleTypeColumnHeader}
					if err := ui.WaitUntilExists(ctx, tconn, userActionParams, 5*time.Second); err != nil {
