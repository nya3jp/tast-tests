...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Behavior of DefaultPopupsSetting policy, checking if popups won't be blocked after setting the policy",
		Contacts: []string{
...
func DefaultPopupsSetting(ctx context.Context, s *testing.State) {
	...
	policies := []policy.Policy{
		&policy.DefaultPopupsSetting{
			Val: 1, // Popups allowed (0: popups are blocked, default behavior)
		},
	}
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		return errors.New("failed to find a popup window")
	}, nil); err != nil {
