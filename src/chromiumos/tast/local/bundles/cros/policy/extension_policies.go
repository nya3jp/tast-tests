...
const (
	extID                   = "dbbinhebhbmlbjnjpeiledcefofbelcl"
...
func ExtensionPolicies(ctx context.Context, s *testing.State) {
	...
	pb.ExtensionPM[extID] = json.RawMessage(fmt.Sprintf(extensionPolicyTemplate, defaultProxyServerURL))
	...
	mgs, cr, err := mgs.New(
		...
		mgs.AddPublicAccountPolicies(mgs.MgsAccountID, []policy.Policy{
			&policy.ExtensionInstallForcelist{Val: []string{extID}},
		}),
	)
	if err != nil {
	...
	pb.ExtensionPM[extID] = json.RawMessage(fmt.Sprintf(extensionPolicyTemplate, updatedProxyServerURL))
