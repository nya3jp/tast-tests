...
func init() {
	testing.AddTest(&testing.Test{
		Func: ShowHomeButton,
		Desc: "Behavior of ShowHomeButton policy, check if a home button is shown based on the value of the policy",
		Contacts: []string{
...
func ShowHomeButton(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if _, err := ui.FindWithTimeout(ctx, tconn, params, 30*time.Second); err != nil && !errors.Is(err, ui.ErrNodeDoesNotExist) {
				s.Fatal("Failed to find home button: ", err)
			} else if b := !errors.Is(err, ui.ErrNodeDoesNotExist); param.wantButton != b {
				s.Errorf("Unexpected existence of home button: got %t; want %t", b, param.wantButton)
			}
		})
	}
}
