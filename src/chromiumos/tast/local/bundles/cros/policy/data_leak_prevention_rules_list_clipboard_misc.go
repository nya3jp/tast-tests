...
func checkExtensionAccess(ctx context.Context, tconn *chrome.TestConn, url string) error {
	got, err := pasteContent(tconn, "text/plain")(ctx)
	if err != nil {
...
func rightClickShelfbox(ctx context.Context, tconn *chrome.TestConn, name string) error {
	...
	if name == "Example" {
		if err := uiauto.Combine("Select shelf",
			ui.LeftClick(searchNode))(ctx); err != nil {
			return errors.Wrap(err, "failed to select shelf: ")
		}
	}
