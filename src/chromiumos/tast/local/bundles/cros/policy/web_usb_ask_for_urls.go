...
func init() {
	testing.AddTest(&testing.Test{
		Func:         WebUSBAskForUrls,
		LacrosStatus: testing.LacrosVariantExists,
		Desc:         "Behavior of WebUsbAskForUrls policy, checking that allowed URLs request for access to a USB device",
		Contacts: []string{
...
// WebUSBAskForUrls tests the WebUsbAskForUrls policy.
func WebUSBAskForUrls(ctx context.Context, s *testing.State) {
	...
	ui := uiauto.New(tconn)
	// Set the default USB policy to not allow any site to request access to USB devices via the WebUSB API.
	defaultWebUSBGuardSetting := &policy.DefaultWebUsbGuardSetting{Val: 2}

	for _, param := range []struct {
		name        string
		expectedAsk bool // expectedAsk states whether a dialog to ask for permission should appear or not.
		policy      *policy.WebUsbAskForUrls
	}{
		{
			name:        "include_url",
			expectedAsk: true,
			policy:      &policy.WebUsbAskForUrls{Val: []string{server.URL + "/web_usb_blocked.html"}},
		},
		{
			name:        "exclude_url",
			expectedAsk: false,
			policy:      &policy.WebUsbAskForUrls{Val: []string{"https://my_corp_site.com/conference.html"}},
		},
		{
			name:        "unset",
			expectedAsk: false,
			policy:      &policy.WebUsbAskForUrls{Stat: policy.StatusUnset},
		},
	} {
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			// Update policies.
			if err := policyutil.ServeAndVerify(ctx, fdms, cr, []policy.Policy{param.policy, defaultWebUSBGuardSetting}); err != nil {
