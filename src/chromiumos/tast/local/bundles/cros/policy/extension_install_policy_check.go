...
func isInstallationAllowed(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome) (bool, error) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		infos, _ := ui.NodesInfo(ctx, blockedfinder)
		for _, info := range infos {
			testing.ContextLogf(ctx, "!!!!!!!!!!!!!!!%s", info.Location)
		}
		infom, _ := ui.NodesInfo(ctx, addfinder)
		for _, info := range infom {
			testing.ContextLogf(ctx, "!!!!!!!!!!!!!!!%s", info.Value)
		}
