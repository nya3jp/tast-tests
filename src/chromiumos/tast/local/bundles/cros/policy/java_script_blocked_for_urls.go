...
func JavaScriptBlockedForUrls(ctx context.Context, s *testing.State) {
	...
	for _, tc := range []struct {
		...
		{
			name:  "allow_multiple",
			value: &policy.JavaScriptBlockedForUrls{Val: []string{"http://www.bing.com", "https://ww.yahoo.com"}},
			allow: true,
		},
		{
			name:  "block_multiple",
			value: &policy.JavaScriptBlockedForUrls{Val: []string{"http://www.bing.com", "https://ww.yahoo.com", url}},
			allow: false,
		},
	} {
		s.Run(ctx, tc.name, func(ctx context.Context, s *testing.State) {
			...
			if err := conn.Eval(ctx, "jsAllowed", &allowed); err != nil && tc.allow {
				s.Error("Failed to read jsAllowed although JavaScript should be allowed: ", err)
			} else {
				if allowed != tc.allow {
					s.Errorf("Expected jsAllowed = %v, got jsAllowed = %v", tc.allow, allowed)
				}
			}
		})
	}
