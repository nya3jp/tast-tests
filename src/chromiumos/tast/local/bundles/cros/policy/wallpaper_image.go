...
func WallpaperImage(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		name                    string
		photoContainerClassName string                 // photoContainerClassName is to check if the photo container is clickable.
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if err := wallpaper.OpenWallpaperFromSettings(ctx, cr, ui); err != nil {
				s.Error("Failed to open the wallpaper subpage in the personalization settings: ", err)
			}
