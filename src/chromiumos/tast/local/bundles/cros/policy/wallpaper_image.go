	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if param.wantImageCheck {
				...
				expectedPercent := 90
				// The background color in tablets is a bit different and close to (165, 13, 14).
			...
			if param.wantImageCheck {
				...
				expectedPercent := 90
				// The background color in tablets locked screens is a bit different and close to (83, 32, 31).
				if tablet {
