...
func DeviceTargetVersionPrefix(ctx context.Context, s *testing.State) {
	...
	if err := servePolicyAndCheckUpdates(ctx, fdms, cr, dbusConn, false); err != nil {
		s.Fatal("Failed to serve policy and check for updates: ", err)
	}

	// Update policies with target version.
	if err := servePolicyAndCheckUpdates(ctx, fdms, cr, dbusConn, true); err != nil {
		s.Fatal("Failed to serve policy and check for updates: ", err)
	}
}
...
func servePolicyAndCheckUpdates(ctx context.Context, fdms *fakedms.FakeDMS, cr *chrome.Chrome, dbusConn *dbus.Conn, setTargetVersion bool) error {
	// Update policies with target version.
	var newPolicy []policy.Policy
	if setTargetVersion {
		newPolicy = append(newPolicy, &policy.DeviceTargetVersionPrefix{Val: targetVersionInPolicy})
	}
	testing.ContextLog(ctx, "Serving new policy, with target version: ", setTargetVersion)
	if err := policyutil.ServeAndVerify(ctx, fdms, cr, newPolicy); err != nil {
	...
	testing.ContextLog(ctx, "Checking for updates. Expecting update: ", !setTargetVersion)
