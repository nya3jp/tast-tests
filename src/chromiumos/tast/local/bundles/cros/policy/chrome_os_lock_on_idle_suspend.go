...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Behavior of ChromeOsLockOnIdleSuspend policy",
		Contacts: []string{
...
func ChromeOsLockOnIdleSuspend(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		policyVal       *policy.ChromeOsLockOnIdleSuspend // crosLockOnIdleSuspendPolicy is the value of the ChromeOsLockOnIdleSuspend policy.
	}{
		{
			name:            "forced",
			wantLockDevice:  true,
			...
			policyVal:       &policy.ChromeOsLockOnIdleSuspend{Val: true},
		},
		{
			name:            "disabled",
			...
			policyVal:       &policy.ChromeOsLockOnIdleSuspend{Val: false},
		},
		{
			name:            "unset",
			wantLockDevice:  false,
			...
			wantChecked:     checked.False,
			policyVal:       &policy.ChromeOsLockOnIdleSuspend{Stat: policy.StatusUnset},
		},
	} {
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			// TODO(crbug.com/1264764): check if the policy works correctly when the user is idle and the device suspends.
			...
			defer func() {
				...
				if err := emitter.EmitInputEvent(cleanupCtx, &pmpb.InputEvent{Type: &eventType}); err != nil {
				...
				if err := kb.Type(cleanupCtx, fixtures.Password+"\n"); err != nil {
					s.Fatal("Typing the password failed: ", err)
				}
				if st, err := lockscreen.WaitState(cleanupCtx, tconn, func(st lockscreen.State) bool { return !st.Locked }, authTimeout); err != nil {
