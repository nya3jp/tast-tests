...
func DefaultSearchProviderKeyword(ctx context.Context, s *testing.State) {
	const (
		...
		testKeyword      = "tranquillity" // keyword that triggers the search engine.
		testSearchTerm   = "abc"          // test search term value.
	)
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			testSearchEngineTriggeredByKeyword := strings.Contains(location, fmt.Sprintf("%s/search?q=%s", testSearchEngine, testSearchTerm))
			if param.enabled != testSearchEngineTriggeredByKeyword {
				s.Fatalf("Unexpected usage of the search engine keyword: got %t; want %t", testSearchEngineTriggeredByKeyword, param.enabled)
			}
		})
	}
}
