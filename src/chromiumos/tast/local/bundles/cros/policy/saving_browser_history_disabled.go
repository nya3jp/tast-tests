...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Behavior of SavingBrowserHistoryDisabled policy, check if a browsing history is shown based on the value of the policy",
		Contacts: []string{
...
func SavingBrowserHistoryDisabled(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		name     string
		disabled bool                                 // enabled is the expected enabled state of the virtual keyboard.
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if noHist != param.disabled {
				s.Errorf("Unexpected existence of browser history found: got %t; want %t", noHist, param.disabled)
			}
		})
	}
}
