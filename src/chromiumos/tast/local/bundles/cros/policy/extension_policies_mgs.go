...
const (
	enterpriseTextExtensionID   = "dbbinhebhbmlbjnjpeiledcefofbelcl"
	enterpriseTextExtensionName = "Enterprise Verified Access Test Bed"
...
func ExtensionPoliciesMGS(ctx context.Context, s *testing.State) {
	...
	pb.ExtensionPM[enterpriseTextExtensionID] = json.RawMessage(
	...
	mgs, cr, err := mgs.New(
		...
		mgs.AddPublicAccountPolicies(mgs.MgsAccountID, []policy.Policy{
			&policy.ExtensionInstallForcelist{Val: []string{enterpriseTextExtensionID}},
		}),
	)
	if err != nil {
	...
	ui := uiauto.New(tconn)
	extensionButtonFinder := nodewith.Name(enterpriseTextExtensionName).Role(role.Button)
