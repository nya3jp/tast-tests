...
func AllowWakeLocks(ctx context.Context, s *testing.State) {
	...
	// Set idle warning window to show after 5 seconds, and logout after 30 seconds.
	...
	for _, param := range []struct {
	...
	}{
		{
			...
			policies: []policy.Policy{
				&policy.PowerManagementIdleSettings{Val: &policy.PowerManagementIdleSettingsValue{
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if param.wantIdleWindow {
				...
				if err := ui.WithTimeout(10 * time.Second).WaitUntilExists(idleWindow.First())(ctx); err != nil {
					s.Fatal("Failed to find the idle window for logout: ", err)
				}
			} else {
