...
func AudioOutputAllowed(ctx context.Context, s *testing.State) {
	...
	mutedFindParams := nodewith.Name("Toggle Volume. Volume is muted.").Role(role.ToggleButton)

	unmutedFindParams := nodewith.Name("Toggle Volume. Volume is on, toggling will mute audio.").Role(role.ToggleButton)
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if param.expectDisabled {
				ui := uiauto.New(tconn).WithTimeout(1 * time.Second)
				if err := ui.LeftClick(mutedFindParams)(ctx); err != nil {
					s.Fatal("Failed to click the audio toggle: ", err)
				}

				// Check if device is still muted.
				if err := policyutil.UiautoVerifyNotExists(ctx, tconn, unmutedFindParams, 2*time.Second); err != nil {
