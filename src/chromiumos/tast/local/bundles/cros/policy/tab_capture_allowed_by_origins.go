	...
	for _, tc := range []struct {
		...
		s.Run(ctx, tc.name, func(ctx context.Context, s *testing.State) {
			...
			if tc.wantCaptureBlocked {
				...
				if err := ui.WaitUntilExists(nodewith.Name("Share a Chromium tab").Role(role.Window).ClassName("DesktopMediaPickerDialogView"))(ctx); err != nil {
					if err := ui.WaitUntilExists(nodewith.Name("Share a Chrome tab").Role(role.Window).ClassName("DesktopMediaPickerDialogView"))(ctx); err != nil {
