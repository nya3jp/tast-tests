...
func LacrosSecondaryProfilesAllowed(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			newProfileOrGuestAllowed := param.value.Stat != policy.StatusUnset && param.value.Val
			...
			if newProfileEnabled != newProfileOrGuestAllowed {
				s.Fatalf("Unexpected new profile behavior: got %t; want %t", newProfileEnabled, newProfileOrGuestAllowed)
			}
			...
			if guestProfileEnabled != newProfileOrGuestAllowed {
				s.Fatalf("Unexpected guest profile behavior: got %t; want %t", guestProfileEnabled, newProfileOrGuestAllowed)
			}
		})
	}
}
