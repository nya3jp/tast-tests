...
func RequiredClientCertificate(ctx context.Context, s *testing.State) {
	...
	if browserType == browser.TypeLacros {
		lacrosOpts, err := getLacrosOptsForAsh()
		if err != nil {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			pb.AddPolicies(append(extraPolicies, param.policy))
			...
			if browserType == browser.TypeLacros {
				...
				func() {
					...
					l, err := lacros.LaunchFromShelf(ctx, tconn, "/run/lacros")
					if err != nil {
...
func getLacrosOptsForAsh() ([]chrome.Option, error) {
