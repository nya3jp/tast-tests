...
func canAddOpenedPageAsBookmark(ctx context.Context, tconn *chrome.TestConn) (bool, error) {
	...
	visibility of the bookmark star icon is related to bookmark functionality being enabled.
	*/
...
func canRemoveBookmark(ctx context.Context, tconn *chrome.TestConn, bookmarkName string) (bool, error) {
	...
	*/
	...
	if err := browser.RemoveBookmark(ctx, tconn); err != nil {
...
func canOpenInitiallyAddedBookmark(ctx context.Context, tconn *chrome.TestConn) (bool, error) {
	...
	addressBefore, err := browser.GetAddressBarText(ctx, tconn)
	if err != nil {
		return false, errors.Wrap(err, "problem getting address bar value")
	}

	if err := bookmarksbar.OpenBookmark(ctx, tconn, "Google Maps"); err != nil {
		return false, errors.Wrap(err, "failed to open Google Maps bookmark")
	}

	addressAfter, err := browser.GetAddressBarText(ctx, tconn)
	if err != nil {
		return false, errors.Wrap(err, "problem getting address bar value")
	}
