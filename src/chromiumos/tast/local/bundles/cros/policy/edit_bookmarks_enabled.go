...
func EditBookmarksEnabled(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if err := browser.ShowBookmarksBar(ctx, tconn); err != nil {
			...
			if allowed, err := browser.IsBookmarkStarIconVisible(ctx, tconn); err != nil {
				s.Fatalf(errorMessage, testStep, err)
			} else if allowed != param.wantAllowed {
				s.Fatalf(assertionFailureErrorMessage, testStep, allowed, param.wantAllowed)
			}
			...
			if allowed, err := canOpenInitiallyAddedBookmark(ctx, tconn); err != nil {
				...
				s.Fatalf(assertionFailureErrorMessage, testStep, allowed, true)
			}
		})
	}
}
...
func canAddBookmark(ctx context.Context, tconn *chrome.TestConn) (bool, error) {
	...
	bookmarksCountBefore, err := browser.GetVisibleBookmarksCount(ctx, tconn)
	if err != nil {
		return false, errors.Wrap(err, "could not get count of bookmarks")
	}

	if err := browser.BookmarkCurrentTab(ctx, tconn); err != nil {
	...
	bookmarksCountAfter, err := browser.GetVisibleBookmarksCount(ctx, tconn)
	if err != nil {
		return false, errors.Wrap(err, "could not get count of bookmarks")
	}
