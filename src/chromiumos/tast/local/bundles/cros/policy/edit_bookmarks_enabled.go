...
func EditBookmarksEnabled(ctx context.Context, s *testing.State) {
	...
	if allowed, err := addInitialBookmark(ctx, tconn, cr); err != nil {
		s.Fatalf("Encountered error when adding initial bookmark, Err: %s", err)
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if allowed, err := browser.IsBookmarkStarIconVisible(ctx, tconn); err != nil {
				...
				s.Fatalf(assertionFailureErrorMessage, testDomain, allowed, param.wantAllowed)
			}

			testDomain = "adding bookmark"
			if allowed, err := canAddOpenedPageAsBookmark(ctx, tconn); err != nil {
				s.Fatalf(errorMessage, testDomain, err)
			} else if allowed != param.wantAllowed {
				s.Fatalf(assertionFailureErrorMessage, testDomain, allowed, param.wantAllowed)
			}
			...
			if allowed, err := canOpenInitiallyAddedBookmark(ctx, tconn); err != nil {
				...
				s.Fatalf(assertionFailureErrorMessage, testDomain, allowed, true)
			}
		})
	}
}
