...
// timeoutForPolling is the timeout used when polling for UI elements.
const timeoutForPolling = 15 * time.Second
...
func canAddOpenedPageAsBookmark(ctx context.Context, tconn *chrome.TestConn) (bool, error) {
	...
	*/
	if canSeeBookmarkIcon, err := canSeeBookmarkIcon(ctx, tconn); err != nil {
