...
func EditBookmarksEnabled(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		},
		{
			...
			urlToBookmark:   "", // Not going to be used for this case
			bookmarkNewName: "", // Not going to be used for this case
			wantAllowed:     false,
		},
	} {
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if allowed, err := browser.IsBookmarkStarIconVisible(ctx, tconn); err != nil {
				...
				s.Fatalf("Unexpected result for checking visibility of bookmark icon; got %t, want %t", allowed, param.wantAllowed)
			}
			...
			if allowed, err := canAddBookmark(ctx, tconn, keyboard); err != nil {
				...
				s.Fatalf("Unexpected result for adding bookmark; got %t, want %t", allowed, param.wantAllowed)
			}
			...
			if allowed, err := canRenameBookmark(ctx, tconn, keyboard, param.bookmarkNewName); err != nil {
				...
				s.Fatalf("Unexpected result for editing bookmark; got %t, want %t", allowed, param.wantAllowed)
			}
			...
			if allowed, err := canRemoveBookmark(ctx, tconn, keyboard, param.bookmarkNewName); err != nil {
				...
				s.Fatalf("Unexpected result for removing bookmark; got %t, want %t", allowed, param.wantAllowed)
			}
			...
			if allowed, err := canOpenInitiallyAddedBookmark(ctx, tconn); err != nil {
				...
				s.Fatalf("Unexpected result for opening existing bookmark; got %t, want true", allowed)
			}
		})
	}
}
...
// Returns true if bookmark's name was changes, otherwise returns false.
