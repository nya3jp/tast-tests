...
import (
	...
	bw "chromiumos/tast/local/chrome/ui/browser"
...
func EditBookmarksEnabled(ctx context.Context, s *testing.State) {
	...
	if allowed, err := addInitialBookmark(ctx, tconn, cr); err != nil {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if err := bw.ShowBookmarksBar(ctx, tconn); err != nil {
			...
			if allowed, err := bw.IsBookmarkStarIconVisible(ctx, tconn); err != nil {
				s.Fatalf("Encountered error when executing test logic for [%s], Err: %s", testStep, err)
			} else if allowed != param.wantAllowed {
				s.Fatalf("Unexpected result of %s; got %t, want %t", testStep, allowed, param.wantAllowed)
			}

			testStep = "adding bookmark"
			if allowed, err := canAddBookmark(ctx, tconn); err != nil {
				s.Fatalf("Encountered error when executing test logic for [%s], Err: %s", testStep, err)
			} else if allowed != param.wantAllowed {
				s.Fatalf("Unexpected result of %s; got %t, want %t", testStep, allowed, param.wantAllowed)
			}
			...
			if allowed, err := canRenameBookmark(ctx, tconn, param.bookmarkNewName); err != nil {
				s.Fatalf("Encountered error when executing test logic for [%s], Err: %s", testStep, err)
			} else if allowed != param.wantAllowed {
				s.Fatalf("Unexpected result of %s; got %t, want %t", testStep, allowed, param.wantAllowed)
			}
			...
			if allowed, err := canRemoveBookmark(ctx, tconn, param.bookmarkNewName); err != nil {
				s.Fatalf("Encountered error when executing test logic for [%s], Err: %s", testStep, err)
			} else if allowed != param.wantAllowed {
				s.Fatalf("Unexpected result of %s; got %t, want %t", testStep, allowed, param.wantAllowed)
			}
			...
			if allowed, err := canOpenInitiallyAddedBookmark(ctx, tconn); err != nil {
				...
				s.Fatalf("Unexpected result of %s; got %t, want true", testStep, allowed)
			}
		})
	}
}
...
func addInitialBookmark(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome) (bool, error) {
	...
	return canAddBookmark(ctx, tconn)
}
...
func canAddBookmark(ctx context.Context, tconn *chrome.TestConn) (bool, error) {
	...
	if canSeeBookmarkIcon, err := bw.IsBookmarkStarIconVisible(ctx, tconn); err != nil {
		return false, errors.Wrap(err, "could not check if bookmark icon was visible")
	} else if canSeeBookmarkIcon != true {
		return false, nil
	}

	bookmarksCountBefore, err := bw.GetVisibleBookmarksCount(ctx, tconn)
	if err != nil {
		return false, errors.Wrap(err, "could not get count of bookmarks")
	}

	if err := bw.BookmarkCurrentTab(ctx, tconn); err != nil {
	...
	for i := 0; i < 2; i++ {
		if err := bw.ToggleBookmarksBar(ctx); err != nil {
			return false, errors.Wrap(err, "unable to toggle bookmark bar")
		}
	}

	bookmarksCountAfter, err := bw.GetVisibleBookmarksCount(ctx, tconn)
	if err != nil {
		return false, errors.Wrap(err, "could not get count of bookmarks")
	}
...
func canRenameBookmark(ctx context.Context, tconn *chrome.TestConn, newName string) (bool, error) {
	...
	if err := bw.RenameBookmarkForCurrentTab(ctx, tconn, newName); err != nil {
		return false, errors.Wrap(err, "could not rename the current tab bookmark")
	}

	bookmarkName, err := bw.GetCurrentTabBookmarkName(ctx, tconn)
	if err != nil {
...
func canRemoveBookmark(ctx context.Context, tconn *chrome.TestConn, bookmarkName string) (bool, error) {
	...
	if err := bw.RemoveBookmarkForCurrentTab(ctx, tconn); err != nil {
	...
	visibleAfterRemoving, err := bw.IsBookmarkVisible(ctx, tconn, bookmarkName)
	if err != nil {
...
func canOpenInitiallyAddedBookmark(ctx context.Context, tconn *chrome.TestConn) (bool, error) {
	if err := bw.OpenBookmark(ctx, tconn, nameOfInitialBookmark); err != nil {
		return false, errors.Wrapf(err, "failed to open %s bookmark", nameOfInitialBookmark)
	}

	addressBarText, err := bw.GetAddressBarText(ctx, tconn)
	if err != nil {
