...
func IncognitoModeAvailability(ctx context.Context, s *testing.State) {
	...
	checkIncognitoModeAvailability := func(ctx context.Context, s *testing.State, value *policy.IncognitoModeAvailability) {
		...
		triggerAccelerator := func(accelerator string) error {
			expr := fmt.Sprintf("(async () => {let accelerator = %s; await tast.promisify(chrome.autotestPrivate.activateAccelerator)(accelerator); accelerator.pressed = false; await tast.promisify(chrome.autotestPrivate.activateAccelerator)(accelerator)})()", accelerator)
		...
		if len(windows) != expectedWindows {
			s.Errorf("%d windows are open; expected %d", len(windows), expectedWindows)
		}

		// TODO: check if windows are created as expected
		// Force enable is a Windows feature
	}
	...
	for _, param := range []struct {
		...
		{
			name:  "available",
			value: &policy.IncognitoModeAvailability{Val: 0},
		},
		{
			name:  "disabled",
			value: &policy.IncognitoModeAvailability{Val: 1},
		},
		{
			name:  "forced",
			value: &policy.IncognitoModeAvailability{Val: 2},
		},
	} {
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			checkIncognitoModeAvailability(ctx, s, param.value)
		})
	}
}
