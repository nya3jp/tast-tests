...
func AutoclickEnabled(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			var c coords.Point
			c.X = stNode.Location.Left + stNode.Location.Width/2
			c.Y = stNode.Location.Top + stNode.Location.Height/2
			if err := mouse.Move(ctx, tconn, c, 0); err != nil {
				s.Fatal("Failed to move the mouse: ", err)
			}

			// Check if a click occured by checking whether the Sign out button is visible or not.
