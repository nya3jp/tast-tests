...
func DefaultSearchProviderSearchURL(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		policies []policy.Policy // policies is the value of DefaultSearchProviderEnabled and DefaultSearchProviderSearchURL policies.
	}{
		{
			name:    "set",
			...
			policies: []policy.Policy{&policy.DefaultSearchProviderEnabled{Val: true}, &policy.DefaultSearchProviderSearchURL{Val: fmt.Sprintf("https://%s/search?q={searchTerms}", fakeURL)}},
		},
		{
			name:     "unset",
			wantURL:  "google.com",
			policies: []policy.Policy{&policy.DefaultSearchProviderEnabled{Val: true}, &policy.DefaultSearchProviderSearchURL{Stat: policy.StatusUnset}},
		},
		{
			name:     "unset",
			wantURL:  searchTerm,
			policies: []policy.Policy{&policy.DefaultSearchProviderEnabled{Val: false}, &policy.DefaultSearchProviderSearchURL{Val: fmt.Sprintf("https://%s/search?q={searchTerms}", fakeURL)}},
		},
		{
			name:     "unset",
			wantURL:  searchTerm,
			policies: []policy.Policy{&policy.DefaultSearchProviderEnabled{Val: false}, &policy.DefaultSearchProviderSearchURL{Stat: policy.StatusUnset}},
		},
	} {
