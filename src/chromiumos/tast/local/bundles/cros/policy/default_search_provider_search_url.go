...
func DefaultSearchProviderSearchURL(ctx context.Context, s *testing.State) {
	const (
		googleURL  = "google.com" // googleURL is the default search engine.
	...
	for _, param := range []struct {
	...
	}{
		{
			...
			value: &policy.DefaultSearchProviderSearchURL{Val: fmt.Sprintf("https://%s/search?q={searchTerms}", fakeURL)},
		},
		{
			name:    "unset",
			wantURL: googleURL,
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if !strings.Contains(addressBar.Value, param.wantURL) {
				s.Fatalf("Unexpected search engine used: got %s; want %s", addressBar.Value, param.wantURL)
			}
		})
	}
}
