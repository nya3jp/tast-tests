...
func DefaultGeolocationSetting(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		},
		{
			name:           "allow",
			nodeName:       "Ask before accessing (recommended)",
			code:           "new Promise(function(resolve, reject) {navigator.geolocation.getCurrentPosition(resolve, reject);}).then((position) => {return position.coords.latitude;}).catch((err) => {(err.message)});",
			wantAsk:        false,
			wantRestricted: true,
			wantChecked:    ui.CheckedStateTrue,
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			lat := 91.0
			if err := conn.Eval(ctx, param.code, &lat); err != nil {
				s.Fatal("Failed to request current position: ", err)
			}

			// Check if lat is a valid value
			if lat < -90 || lat > 90 {
				s.Error("Failed to get valid geolocation")
			}

			// Check whether a dialog to ask for permission opens.
			if _, err = ui.FindWithTimeout(ctx, tconn, ui.FindParams{
				...
				s.Fatal("Finding wants to node failed: ", err)
			} else {
				d := !errors.Is(err, ui.ErrNodeDoesNotExist)
				if param.wantAsk != d {
