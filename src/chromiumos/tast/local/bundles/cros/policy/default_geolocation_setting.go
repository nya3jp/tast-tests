	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			go func() {
				...
				params := ui.FindParams{Role: ui.RoleTypeButton, Name: "Allow"}
				node, err := ui.FindWithTimeout(ctx, tconn, params, 15*time.Second)
				if err != nil && !errors.Is(err, ui.ErrNodeDoesNotExist) {
					s.Fatal("Finding allow button node failed: ", err)
				} else if b := !errors.Is(err, ui.ErrNodeDoesNotExist); param.wantAsk != b {
					s.Errorf("Unexpected existence of dialog to ask for permission: got %t; want %t", b, param.wantAsk)
				}
			...
			code := `new Promise(function(resolve, reject) {
				...
				.then((position) => {return 0;})
				.catch((err) => {return err.code;});`

			// Try to access the geolocation.
			var ec int
			if err := conn.Eval(ctx, code, &ec); err != nil {
