...
func DefaultSearchProviderEnabled(ctx context.Context, s *testing.State) {
	const (
		...
		noConnectionErr     = "chrome-error://chromewebdata/"
	)
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if strings.Contains(location, noConnectionErr) {
			...
			searchEngineURLIsInOmnibox := strings.Contains(location, defaultSearchEngine)

			if param.enabled != searchEngineURLIsInOmnibox {
				s.Fatal("Unexpected usage of search engine: got %t; want %t", searchEngineURLIsInOmnibox, param.enabled)
			}
		})
	}
}
