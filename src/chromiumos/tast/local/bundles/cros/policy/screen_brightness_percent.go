...
func ScreenBrightnessPercent(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			nodeS, err := ui.FindWithTimeout(ctx, tconn, paramsS, 15*time.Second)
			if err != nil {
				s.Fatal("Failed to find Status tray: ", err)
			}
			defer nodeS.Release(ctx)

			if nodeS.Value != param.expectedBrightness {
				s.Errorf("Unexpected brightness set: got %d; want %d", nodeS.Value, param.expectedBrightness)
			}
