...
func ScreenBrightnessPercent(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		name               string
		expectedBrightness string                          // expectedBrightness is the expected brightness value.
		...
		{
			name:               "unset",
			expectedBrightness: "47%", // In the unset case the last set brightness is expected.
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if nodeBS.Value != param.expectedBrightness {
				s.Errorf("Unexpected brightness set: got %s; want %s", nodeBS.Value, param.expectedBrightness)
			}
