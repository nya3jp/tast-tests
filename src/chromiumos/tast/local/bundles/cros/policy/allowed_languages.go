...
func AllowedLanguages(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			// Find and clilck on Add languages button to select the preffered languages from the popup dialog.
			if err := ui.StableFindAndClick(ctx, tconn, ui.FindParams{
			...
			if (param.minLangs > len(nodeSlice)) || (len(nodeSlice) > param.maxLangs) {
				s.Errorf("The number of preferred languages doesn't match: got %t; want at least %t and at most %t", len(nodeSlice), param.minLangs, param.maxLangs)
			}
		})
	}
}
