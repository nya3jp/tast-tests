...
func VideoCaptureAllowedUrls(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
	...
	}{
		{
			name:        "include_url",
			expectedAsk: false,
			policy:      &policy.VideoCaptureAllowedUrls{Val: []string{"http://127.0.0.1:*/video_capture_allowed.html"}},
		},
		{
			name:        "exclude_url",
			expectedAsk: true,
			policy:      &policy.VideoCaptureAllowedUrls{Val: []string{"http://192.168.*.*:*/video_capture_allowed.html"}},
		},
		{
			name:        "unset",
			expectedAsk: true,
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if param.expectedAsk {
				if err := ui.WithTimeout(10 * time.Second).WaitUntilExists(allowButton.First())(ctx); err != nil {
					s.Fatal("Failed to find the video capture prompt dialog: ", err)
				}
			} else {
				if err := ui.EnsureGoneFor(allowButton, 10*time.Second)(ctx); err != nil {
					s.Fatal("Failed to make sure no video capture prompt dialog shows: ", err)
				}
			}
		})
	}
