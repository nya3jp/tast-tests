...
func BookmarkBarEnabled(ctx context.Context, s *testing.State) {
	...
	lacrosPath, _ := s.Var(lacrosfixt.LacrosDeployedBinary)
	br, closeBrowser, err := browserfixt.SetUpPersistent(ctx, s.FixtValue(), s.Param().(browser.Type), lacrosPath)
	if err != nil {
	...
	// XXX Before sending keyboard events we should wait until the browser
	// window appears. How to do this for Ash?
	//const policiesPageTitle = "Policies"
	//if err := lacros.WaitForLacrosWindow(ctx, tconn, policiesPageTitle); err != nil {
