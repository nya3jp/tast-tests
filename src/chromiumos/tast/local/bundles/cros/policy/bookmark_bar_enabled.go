...
func BookmarkBarEnabled(ctx context.Context, s *testing.State) {
	...
	// Each test case contains a different bookmarkURL to add because irrespective
	// of whether the bookmark bar is shown or not, the URL is added to the list of
	// bookmarks so it cannot be added again in the subsequent tests.
	for _, param := range []struct {
		name           string                     // name is the subtest name.
		wantBookmarbar bool                       // wantBookmarbar is the expected existence of the bookmark bar.
		bookmarkURL    string                     // bookmarkURL is the URL to bookmark.
		bookmarkString string                     // bookmarkString is the expected text for the bookmark on the bookmark bar.
		...
		},
		{
			name:           "disabled",
			wantBookmarbar: false,
			bookmarkURL:    "chrome://version",
			bookmarkString: "About Version",
			policy:         &policy.BookmarkBarEnabled{Val: false},
		},
		{
			name:           "enabled",
			wantBookmarbar: true,
			bookmarkURL:    "chrome://policy",
			bookmarkString: "Policies",
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			conn, err := cr.NewConn(ctx, param.bookmarkURL)
			if err != nil {
			...
			conn2, err := cr.NewConn(ctx, "chrome://history")
			if err != nil {
				s.Fatal("Failed to connect to chrome: ", err)
			}
			defer conn2.Close()
