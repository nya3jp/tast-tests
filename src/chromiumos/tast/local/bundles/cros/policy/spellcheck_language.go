...
func verifySpellcheckForLanguage(ctx context.Context, s *testing.State, cr *chrome.Chrome, tconn *browser.TestConn, language spellcheckLanguageWordTestCase) error {
	...
	{
		// In the following block we try to access "chrome://os-settings/osLanguages/input".
		// But it cannot be opened using apps.LaunchOSSettings(ctx, cr, "chrome://os-settings/osLanguages/input").
		// Instead, we navigate through "chrome://os-settings/osLanguages", then click on Input and Keyboards link.
		...
		if err := ui.LeftClick(nodewith.Name("Inputs and keyboards English (US)").Role(role.Link))(ctx); err != nil {
			return errors.Wrap(err, "could not find the password field")
		}
