...
func checkTime(boxes statusBoxesMap, boxNames []string, key, reStr string, s *testing.State) {
	re, err := regexp.Compile(reStr)
	if err != nil {
		s.Fatalf("Failed to compile regexp `%s`: %s", reStr, err)
	}
	...
	for _, boxName := range boxNames {
		...
		if didMatch := re.Match([]byte(status[key])); !didMatch {
