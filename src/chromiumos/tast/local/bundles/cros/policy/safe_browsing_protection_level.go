	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if err := testing.Poll(ctx, func(ctx context.Context) error {
				...
				if err := conn.Eval(ctx, `document.querySelector("body > settings-ui").shadowRoot.querySelector("#main").shadowRoot.querySelector("settings-basic-page").shadowRoot.querySelector("#basicPage > settings-section.expanded > settings-privacy-page").shadowRoot.querySelector("#security > settings-security-page").shadowRoot.querySelector("#safeBrowsingStandard").disabled`, &isRestricted); err != nil {
				...
				if isRestricted != param.wantRestricted {
					return errors.Errorf("failed to verify restriction behavior: got %t; want %t", isRestricted, param.wantRestricted)
				}
				return nil
			}, &testing.PollOptions{
			...
			if err := testing.Poll(ctx, func(ctx context.Context) error {
				...
				if err := conn.Eval(ctx, `document.querySelector("body > settings-ui").shadowRoot.querySelector("#main").shadowRoot.querySelector("settings-basic-page").shadowRoot.querySelector("#basicPage > settings-section.expanded > settings-privacy-page").shadowRoot.querySelector("#security > settings-security-page").shadowRoot.querySelector("#safeBrowsingRadioGroup").selected`, &selctedOption); err != nil {
				...
				if selctedOption != param.wantSelected {
					return errors.Errorf("failed to verify the selected browsing safety level option behavior: got %t; want %t", selctedOption, param.wantSelected)
				}
				return nil
			}, &testing.PollOptions{
				Timeout:  30 * time.Second,
