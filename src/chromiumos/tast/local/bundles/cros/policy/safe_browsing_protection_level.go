		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if err := testing.Poll(ctx, func(ctx context.Context) error {
				...
				if isRestricted != param.wantRestricted {
					return errors.Errorf("failed to verify restriction behavior: got %q; want %q", isRestricted, param.wantRestricted)
				}
				return nil
			}, &testing.PollOptions{
			...
			if err := testing.Poll(ctx, func(ctx context.Context) error {
				...
				if selctedOption != param.wantSelected {
					return errors.Errorf("failed to verify the selected browsing safety level option behavior: got %q; want %q", selctedOption, param.wantSelected)
				}
				return nil
			}, &testing.PollOptions{
				Timeout:  30 * time.Second,
