...
func init() {
	testing.AddTest(&testing.Test{
		...
		Pre:          pre.User,
		Data:         []string{"default_download_directory.html", "default_download_directory.json"},
	})
}
...
func AllowDeletingBrowserHistory(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
				...
				`new Promise(function(resolve, reject) {
					chrome.settingsPrivate.getAllPrefs(function(root) {
						resolve(root.find(obj => {
							return obj.key == "browser.clear_data.browsing_history"
						}))
					})
				})`, &checkbox); err != nil {
			...
			if param.restricted {
				if v1, v2 := checkbox["controlledBy"], checkbox["enforcement"]; v1 != nil || v2 != nil {
					s.Error("Unexpected key in checkbox properties (controlledBy or enforcement)")
				}
				if val := checkbox["value"]; val != true {
					s.Errorf("Unexpected value checkbox, %v instead of true", val)
				}
			} else {
				if checkbox["controlledBy"] != "USER_POLICY" || checkbox["enforcement"] != "ENFORCED" || checkbox["value"] != false {
					s.Error("Clear Browsing history checkbox is not restricted")
				}
			}
