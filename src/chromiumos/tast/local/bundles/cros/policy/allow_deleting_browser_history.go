		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			for _, cb := range []struct {
				...
				if checked, err := cbNode.Attribute(ctx, "checked"); err != nil {
					...
					s.Fatalf("The checked attribute of the %s checkbox is not a string: %v", cb.name, checkedStr)
				} else if checkedStr != strconv.FormatBool(param.wantChecked) {
					s.Errorf("Unexpected %s checkbox checked state in the %s tab: got %s; want %t", cb.name, cb.tab, checkedStr, param.wantChecked)
				}
				...
				if restriction, err := cbNode.Attribute(ctx, "restriction"); err != nil {
					...
					s.Fatalf("The restriction attribute of the %s checkbox is not a string: %v", cb.name, restriction)
