...
func AllowDeletingBrowserHistory(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			// settingValueAndRestriction checks for a Chrome setting with the corresponding key,
			// and returns with its value and whether it is restrincted.
			// Works only for settings with bool type values.
			settingValueAndRestriction := func(key string) (bool, bool) {
				...
				if !ok {
					s.Fatal("Unexpected value type in JS response")
				}

				// In case of no restriction, the enforcement key is not present.
				if _, ok := checkbox["enforcement"]; !ok {
					return value == true, false
				}

				enforcement, ok := checkbox["enforcement"].(string)
				if !ok {
					s.Fatal("Unexpected enforcement type in JS response")
				}
