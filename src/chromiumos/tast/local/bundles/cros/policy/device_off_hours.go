...
func DeviceOffHours(ctx context.Context, s *testing.State) {
	...
	var alwaysOff = []*RefWeeklyTimeIntervals{
		{
	...
	var neverOff = []*RefWeeklyTimeIntervals{
		{
			Start: &RefWeeklyTime{DayOfWeek: 1, Time: 0},
			End:   &RefWeeklyTime{DayOfWeek: 1, Time: 0},
		},
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			// Check the state of the controlled policy.
			...
			if ok != !param.active {
				s.Errorf("Invalid policy state: expected %t; got %t", param.active, ok)
			}
		})
	}
}
