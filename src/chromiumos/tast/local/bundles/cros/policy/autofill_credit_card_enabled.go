		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if param.wantChecked == checked.True {
				...
				for _, creditCardField := range creditCardFields {
					if err := ui.LeftClick(nodewith.Role(role.TextField).Name(creditCardField.fieldName))(ctx); err != nil {
				...
				// 		s.Fatalf("Failed to complete the JS test for htmlFieldID=%d. Error: %e", creditCardField.htmlFieldID, err)
				// 	}
				// 	if valueFromHTML != creditCardField.fieldValue {
				// 		s.Fatalf("Credit card field was not set properly. Actual value %d doesnt match with expected %e", valueFromHTML, creditCardField.fieldValue)
				// 	}
				// }
			}
		})
