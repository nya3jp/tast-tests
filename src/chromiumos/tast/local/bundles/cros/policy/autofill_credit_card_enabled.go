...
const certFileName = "autofill_credit_card_enabled_certificate.pem"
const keyFileName = "autofill_credit_card_enabled_key.pem"
const caCertFileName = "autofill_credit_card_enabled_cacert.pem"
...
func AutofillCreditCardEnabled(ctx context.Context, s *testing.State) {
	...
	server, err := newLocalHTTPSTestServer(http.FileServer(s.DataFileSystem()), s.DataPath(certFileName), s.DataPath(keyFileName))
	if err != nil {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if param.wantChecked == checked.True {
				...
				if err := configureChromeToAcceptCertificate(ctx, cr, br, tconn, s.DataPath(caCertFileName)); err != nil {
