...
func AutofillCreditCardEnabled(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		name            string
		wantRestriction restriction.Restriction           // wantRestriction is the wanted restriction state of the toggle button for the "Save and fill payment methods" option.
		wantChecked     checked.Checked                   // wantChecked is the wanted checked state of the toggle button for the "Save and fill payment methods" option.
		policy          *policy.AutofillCreditCardEnabled // policy is the value of the policy.
	}{
		{
			name:            "unset",
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if err := policyutil.SettingsPage(ctx, cr, br, "payments").
				SelectNode(ctx, nodewith.
					NameStartingWith("Save and fill payment").
