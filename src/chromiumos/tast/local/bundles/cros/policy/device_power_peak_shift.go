...
// is plugged into AC. Even after the "end_time" period, DUT runs on AC till "charge_start_time" but
...
func DevicePowerPeakShift(ctx context.Context, s *testing.State) {
	...
	pxy, err := servo.NewProxy(ctx, s.RequiredVar("servo"), s.DataPath("testing_rsa"), "/root/.ssh")
	if err != nil {
