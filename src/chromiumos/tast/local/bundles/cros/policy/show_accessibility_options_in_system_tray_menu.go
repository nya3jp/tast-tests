...
func ShowAccessibilityOptionsInSystemTrayMenu(ctx context.Context, s *testing.State) {
	...
	// TODO: Remove it if the test is not informational anymore.
	...
	// Close system tray if it was left open by a previous test.
	if err := kb.Accel(ctx, "Esc"); err != nil {
		s.Fatal("Failed to press Esc to close system tray: ", err)
	}
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			// Open an empty window. Opening the Settings page here does not work with UI automation.
			conn, err := cr.NewConn(ctx, "")
			if err != nil {
				s.Fatal("Failed to open a new page: ", err)
			}
			defer conn.Close()
