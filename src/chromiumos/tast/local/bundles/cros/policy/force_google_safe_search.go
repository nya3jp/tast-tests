...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Behavior of the ForceGoogleSafeSearch policy",
		Contacts: []string{
...
func ForceGoogleSafeSearch(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if err := policyutil.ServeAndVerify(ctx, fdms, cr, []policy.Policy{param.value, &policy.DefaultSearchProviderEnabled{Stat: policy.StatusUnset}}); err != nil {
			...
			isSafe := strings.Contains(url, "&safe=active")
			if param.wantSafe != isSafe {
