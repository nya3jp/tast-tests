...
func init() {
	testing.AddTest(&testing.Test{
		Func:         JavascriptJITBlockedForSites,
		LacrosStatus: testing.LacrosVariantExists,
		Desc:         "Check that the JavascriptJITBlockedForSites policy blocks the JIT compiler for the specified URLs",
		Contacts: []string{
...
func JavascriptJITBlockedForSites(ctx context.Context, s *testing.State) {
	...
	for _, tc := range []struct {
		name  string
		value *policy.JavaScriptJitBlockedForSites
		allow bool
	}{
		{
			name:  "set_site_match_pattern",
			value: &policy.JavaScriptJitBlockedForSites{Val: []string{url}},
			allow: false,
		},
		{
			name:  "set_site_unmatch_pattern",
			value: &policy.JavaScriptJitBlockedForSites{Val: []string{"https://my_website.com/*"}},
			allow: true,
		},
		{
			name:  "unset",
			value: &policy.JavaScriptJitBlockedForSites{Stat: policy.StatusUnset},
			allow: true,
		},
	} {
