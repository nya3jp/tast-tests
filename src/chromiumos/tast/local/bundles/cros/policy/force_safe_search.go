...
func ForceSafeSearch(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			isGoogleSafe := safesearch.IsGoogleSafeSearchEnabled(ctx, s, br)
			if isGoogleSafe != param.wantGoogleSafe {
				s.Errorf("Unexpected Google safe search behavior; got %t, want %t", isGoogleSafe, param.wantGoogleSafe)
			}
