...
func PrintingEnabled(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			printingPossible, err := testParams.TestFunc(ctx, tconn)
			if err != nil {
				s.Fatal("Failed to run test body: ", err)
			}

			if printingPossible != param.printingAllowed {
				s.Errorf("Unexpected printing restriction. Expected printing allowed: %t, got: %t", param.printingAllowed, printingPossible)
			}
		})
	}
}
