...
func updatePolicyBlob(fdms *fakedms.FakeDMS, value *policy.DeviceEphemeralUsersEnabled) error {
...
func DeviceEphemeralUsersEnabled(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			if err := updatePolicyBlob(fdms, param.value); err != nil {
				s.Fatal("Failed to write policies to FakeDMS: ", err)
			}
			defer updatePolicyBlob(fdms, &policy.DeviceEphemeralUsersEnabled{Stat: policy.StatusUnset})
			opts := []chrome.Option{
