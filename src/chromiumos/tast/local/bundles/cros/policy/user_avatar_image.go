...
func UserAvatarImage(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if err := policyutil.Verify(ctx, tconn, []policy.Policy{param.value}); err != nil {
				s.Fatal("Failed to verify UserAvatarImage policy value: ", err)
			}
			...
			// Close OS settings app window.
			if err := ui.WithTimeout(timeout).LeftClick(nodewith.Name("Close").Role(role.Button))(ctx); err != nil {
				s.Fatal("Failed to close the OS settings app window: ", err)
			}

			// Open the OS settings changePicture page again.
			// This is done because even when the policy is set, the user can still click on an avatar image and the user image preview will be changed. However, the setting is not applied after closing the window.
			// By closing and opening the page again, we check if the user image preview was changed or not.
			conn, err = apps.LaunchOSSettings(ctx, cr, "chrome://os-settings/changePicture")
			if err != nil {
				s.Fatal("Failed to open the changePicture page: ", err)
			}

			if err := ui.WithTimeout(timeout).WaitUntilExists(userImagePreviewNode)(ctx); err != nil {
				s.Fatal("Failed to wait until user image preview exists: ", err)
			}
			...
			if param.shouldMatchPolicyImage {
				...
				if prcnt < 95 {
					s.Fatalf("User image preview doesn't match the policy-provided image: Red Pixels Percentage: %d", prcnt)
				}
			} else {
				...
				if sim < 95 {
					s.Fatalf("User cannot change device account image: Similarity percentage: %d", sim)
				}
			}
		})
	}
