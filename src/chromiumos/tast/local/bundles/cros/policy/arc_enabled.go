	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if exists, err := ui.Exists(ctx, tconn, ui.FindParams{ClassName: "AppListView"}); err != nil || !exists {
				...
				err = ui.WaitUntilExists(ctx, tconn, ui.FindParams{ClassName: "AppListView"}, 10*time.Second)
				if err != nil {
			...
			err = ui.WaitUntilExists(ctx, tconn, ui.FindParams{
			...
			if enabled := !errors.Is(err, ui.ErrNodeDoesNotExist); enabled != param.wantEnabled {
