...
type popupsSettingTestTable struct {
	...
	expectedTitle string          // expectedTitle is the expected title of the window after test is run with policies applied.
		...
		s.Run(ctx, tc.name, func(ctx context.Context, s *testing.State) {
			...
			if err := testing.Poll(ctx, func(ctx context.Context) error {
				...
				for _, window := range windows {
					if strings.Contains(window.Title, tc.expectedTitle) {
						return nil
					}
				}
				...
				s.Errorf("Failed to verify test %s, expected behavior: %s", tc.name, tc.expectedTitle)
			}
		})
	}
}
