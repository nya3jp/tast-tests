...
func PopupsForURLCheck(ctx context.Context, s *testing.State) {
	...
	defer server.Close()
	defaultPopupsURL := server.URL + "/default_popups_setting_index.html"
	...
	for _, param := range []struct {
	...
	}{
		{
			...
			policies: []policy.Policy{
				&policy.PopupsBlockedForUrls{Val: []string{defaultPopupsURL}},
		...
		},
		{
			...
			policies: []policy.Policy{
				...
				&policy.PopupsAllowedForUrls{Val: []string{defaultPopupsURL}},
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			conn, err := cr.NewConn(ctx, defaultPopupsURL)
			if err != nil {
