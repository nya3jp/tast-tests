...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Behavior of RequiredClientCertificateFor* policies, check if a certificate is issued when the policy is set",
		Contacts: []string{
...
func RequiredClientCertificateFor(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		slot   int           // slot is the used slot for the certificate
	}{
		{
			name: "Device",
