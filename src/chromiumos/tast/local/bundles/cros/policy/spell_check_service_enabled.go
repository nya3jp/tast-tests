...
func SpellCheckServiceEnabled(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		name            string
		wantRestriction restriction.Restriction          // wantRestriction is the wanted restriction state of the toggle button for the "Spell check" option.
		wantChecked     checked.Checked                  // wantChecked is the wanted checked state of the toggle button for the "Spell check" option.
		policy          *policy.SpellCheckServiceEnabled // The value of the policy.
	}{
		{
			name:            "unset",
