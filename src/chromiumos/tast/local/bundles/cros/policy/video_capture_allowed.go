...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Behavior of VideoCaptureAllowed policy, checking the location site settings after setting the policy",
		Contacts: []string{
...
func VideoCaptureAllowed(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		name          string
		expectedBlock bool // wantAsk states whether a dialog to ask for permission should appear or not.
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if blocked != param.expectedBlock {
				s.Errorf("Unexpected block of video capture: want %q got %q", blocked, param.expectedBlock)
			}
		})
	}
}
