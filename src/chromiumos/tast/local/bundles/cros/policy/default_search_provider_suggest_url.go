...
func DefaultSearchProviderSuggestURL(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
	...
	}{
		{
			...
			wantCalls:     []string{"/suggest?q=", "/suggest?q=a", "/suggest?q=ab", "/suggest?q=abc"},
		},
		{
			name:          "suggestUnset",
			searchPolicy:  &policy.DefaultSearchProviderSearchURL{Val: fmt.Sprintf("%s/search?q={searchTerms}", srv.URL)},
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			// Wait for keyboard input to be processed, possibly resulting in calls to srv.
			if err := ui.WaitForLocation(addressBarNode)(ctx); err != nil {
			...
			if !reflect.DeepEqual(result, param.wantCalls) {
				s.Fatalf("Unexpected calls to server - called URIs: %s; want %s", result, param.wantCalls)
			}
		})
	}
}
