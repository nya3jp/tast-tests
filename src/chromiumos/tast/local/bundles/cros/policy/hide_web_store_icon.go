...
func HideWebStoreIcon(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if err := kb.Accel(ctx, "Shift+Search"); err != nil {
				s.Fatal("Failed to type Shift+Search: ", err)
			}

			// Wait for Expanded Application list view
			if err := ui.WaitUntilExists(ctx, tconn, ui.FindParams{ClassName: "AppsGridView"}, 15*time.Second); err != nil {
				s.Fatal("Waiting for Expanded Application list view failed: ", err)
			}
