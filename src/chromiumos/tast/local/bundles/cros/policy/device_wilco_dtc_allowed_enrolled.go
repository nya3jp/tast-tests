	...
	for _, tc := range []struct {
		...
		s.Run(ctx, tc.name, func(ctx context.Context, s *testing.State) {
			...
			if supportdRunning != tc.wantRunning {
				s.Fatalf("Unexpected wilco_dtc_supportd running status: got %t; want %t", supportdRunning, tc.wantRunning)
			}
			...
			if vmRunning != tc.wantRunning {
				s.Fatalf("Unexpected wilco_dtc VM running status: got %t; want %t", vmRunning, tc.wantRunning)
			}
		})
	}
}
