...
func DeviceMinimumVersionNotifications(ctx context.Context, s *testing.State) {
	...
	// Check update required end-of-life notification is visible.
	...
	if err != nil {
		s.Fatal("Failed to find update required notification")
	}

	// Check update required end-of-life banner is visible on the Chrome management page.
