...
func CookiesAllowedForUrls(ctx context.Context, s *testing.State) {
	...
	for _, tc := range []struct {
		...
		s.Run(ctx, tc.name, func(ctx context.Context, s *testing.State) {
			...
			allowed := false
			// Evaluating is going to fail if Cookies is not allowed.
			if err := conn.Eval(ctx, "cookiesAllowed", &allowed); err != nil && tc.allow {
				s.Fatal("Failed to read cookiesAllowed although cookies should be allowed: ", err)
			} else {
				if allowed != tc.allow {
