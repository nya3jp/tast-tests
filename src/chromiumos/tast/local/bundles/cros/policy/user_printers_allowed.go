...
func UserPrintersAllowed(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			// Click on the "Add printer" button until the "Add a printer
			// manually" dialog appears. This way we check if it is possible to
			// add new printers as the restriction state of the button is not
			// visible in the ui tree. DoDefaultUntil cannot be used as it will
			...
			if dialogExists != param.printersAllowed {
				s.Errorf("Unexpected printer dialog existence; want: %t, got: %t", param.printersAllowed, dialogExists)
			}
		})
	}
}
