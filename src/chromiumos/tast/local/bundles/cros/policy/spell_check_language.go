...
type misspelledWordTestCase struct {
	correct    string // correct is the word to test the spell check suggestion.
	misspelled string // misspelled is the same word with an intentional typo.
}
...
func spellCheckWithSuggestionEnabled(ctx context.Context, tconn *browser.TestConn, br *browser.Browser, words []misspelledWordTestCase) error {
	...
	for _, w := range words {
		...
		if err := kb.Type(ctx, w.misspelled+" "); err != nil {
