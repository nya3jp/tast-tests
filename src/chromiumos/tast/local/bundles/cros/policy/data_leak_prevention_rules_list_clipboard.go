...
func DataLeakPreventionRulesListClipboard(ctx context.Context, s *testing.State) {
	...
	message := "Pasting from www.example.com to this location is blocked by administrator policy"
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			// _, err := param.pasteFunc(ctx)
			// if err != nil && param.wantAllowed {
			// 	s.Fatal("Failed to paste")
			// }

			// if err == nil && !param.wantAllowed {
			// 	s.Fatal("Failed to prevent from pasting ", got)
			// }

			if _, err := ash.WaitForNotification(ctx, tconn, 15*time.Second, ash.WaitIDContains("ClipboardDlpBubble")); err != nil {
				s.Fatalf("Failed to wait notification with title %q: %v", message, err)
			}

			// var ret []string
			// if err := tconn.Call(ctx, &ret, "tast.promisify(chrome.autotestPrivate.getVisibleWidgets)"); err != nil {
			// 	s.Fatal("failed to call getVisibleNotifications", err)
			// }
