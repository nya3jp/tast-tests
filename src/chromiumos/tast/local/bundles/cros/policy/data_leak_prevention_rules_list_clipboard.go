...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Test behavior of DataLeakPreventionRulesList policy with clipboard blocked restriction",
		Contacts: []string{
...
func DataLeakPreventionRulesListClipboard(ctx context.Context, s *testing.State) {
	...
	policyDLP := []policy.Policy{&policy.DataLeakPreventionRulesList{
		Val: []*policy.DataLeakPreventionRulesListValue{
			{
				Name:        "Disable Clipboard in confidential content",
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if !param.wantAllowed {
				...
				message := "Pasting from " + param.url + " to this location is blocked by administrator policy"
				bubbleMessage := nodewith.Name(message).Role(role.StaticText).Ancestor(bubbleClass)

				if err := uiauto.Combine("Bubble ",
					ui.WaitUntilExists(bubbleView),
					ui.WaitUntilExists(bubbleButton),
					ui.WaitUntilExists(bubbleClass),
					ui.WaitUntilExists(bubbleMessage))(ctx); err != nil {
