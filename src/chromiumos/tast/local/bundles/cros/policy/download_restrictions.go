...
func DownloadRestrictions(ctx context.Context, s *testing.State) {
	...
	dd, err := os.Open(filesapp.DownloadPath)
	if err != nil {
		s.Fatal("Failed to open Downloads directory")
	}
	defer dd.Close()
	files, err := dd.Readdirnames(-1)
	if err != nil {
		s.Fatal("Failed to get files from Downloads directory")
	}
	for _, file := range files {
		if err = os.RemoveAll(filepath.Join(filesapp.DownloadPath, file)); err != nil {
			s.Fatal("Failed to remove file: ", file)
		}
	}
