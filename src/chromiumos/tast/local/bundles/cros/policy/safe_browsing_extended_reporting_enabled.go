...
func SafeBrowsingExtendedReportingEnabled(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		name            string
		wantRestriction restriction.Restriction                      // wantRestriction is the wanted restriction state of the toggle button.
		wantChecked     checked.Checked                              // wantChecked is the wanted checked state of the toggle button.
		policy          *policy.SafeBrowsingExtendedReportingEnabled // policy is the value of the policy.
	}{
		{
			name:            "unset",
