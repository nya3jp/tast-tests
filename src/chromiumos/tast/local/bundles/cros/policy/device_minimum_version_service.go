...
func (c *DeviceMinimumVersionService) TestUpdateRequiredScreenIsVisible(ctx context.Context, req *empty.Empty) (*empty.Empty, error) {
	...
	if err := oobeConn.Exec(ctx, "document.querySelector('update-required-card').$$('oobe-text-button').click()"); err != nil {
		return &empty.Empty{}, errors.Wrap(err, "failed to click update required screen update now button")
	}
	if err := oobeConn.WaitForExprFailOnErr(ctx, "!document.querySelector('update-required-card').$$('checking-downloading-update[hidden]')"); err != nil {
		return &empty.Empty{}, errors.Wrap(err, "failed to wait for the updating screen to be visible")
	}
