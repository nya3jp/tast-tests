...
func Printers(ctx context.Context, s *testing.State) {
	...
	if err := uiauto.Combine("configure additional print options",
		// TODO(crbug/1290846): Replace `DoDefault` with `LeftClick`
		ui.DoDefault(nodewith.Role(role.PopUpButton).Name("Color")),
		ui.DoDefault(nodewith.Role(role.ListBoxOption).Name("Black and white")),
		// Configure more settings
		ui.DoDefault(nodewith.Role(role.Button).Name("More settings")),
		ui.DoDefault(nodewith.Role(role.PopUpButton).Name("Paper size")),
		ui.DoDefault(nodewith.Role(role.ListBoxOption).Name("A5")),
		// Setting 'Pages per sheet' will disable the 'Margins' option, thus set
		// 'Margins' first.
		ui.DoDefault(nodewith.Role(role.PopUpButton).Name("Margins")),
		ui.DoDefault(nodewith.Role(role.ListBoxOption).Name("Minimum")),
		ui.DoDefault(nodewith.Role(role.PopUpButton).Name("Pages per sheet")),
		ui.DoDefault(nodewith.Role(role.ListBoxOption).Name("2")),
		...
		ui.DoDefault(nodewith.Role(role.PopUpButton).Name("Scale")),
		ui.DoDefault(nodewith.Role(role.ListBoxOption).Name("Custom")),
		ui.DoDefault(nodewith.Role(role.CheckBox).Name("Background graphics")),
	)(ctx); err != nil {
