...
func ExtensionPolicy(ctx context.Context, s *testing.State) {
	...
	type ExtensionPolicies struct {
		...
		SensitiveStringPolicy string `json:"VisibleStringPolicy"`
		VisibleStringPolicy   string `json:"VisibleStringPolicy"`
	}

	var providedPolicy ExtensionPolicies
	if err := json.Unmarshal(b, &providedPolicy); err != nil {
		s.Fatal("Failed to parse provided policies")
	}
	...
	if err := json.Unmarshal(*policies.Extension[extID], &receivedPolicy); err != nil {
	...
	if receivedPolicy != expectedPolicy {
		s.Errorf("Extension policy dump missmatch: want %v; got %v", receivedPolicy, expectedPolicy)
	}
	...
	if err := conn.Call(ctx, &val,
		...
		[]string{"SensitiveStringPolicy", "SensitiveDictPolicy"}); err != nil {
	...
	if extensionPolicy != expectedPolicy {
		s.Errorf("Extension policy missmatch: want %v; got %v", extensionPolicy, expectedPolicy)
	}
}
