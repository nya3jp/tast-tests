...
import (
	...
	"chromiumos/tast/local/bundles/cros/policy/util"
...
// Cleanup is a helper that calls util.ResetChromeAndPolicies
func (h *UserPoliciesHelper) Cleanup(ctx context.Context) error {
	ctx, st := timing.Start(ctx, "user_policy_cleanup")
	defer st.End()

	return util.ResetChromeAndPolicies(ctx, h.FakeDMS, h.Chrome)
}

// UpdatePolicyBlob is a helper that calls util.UpdatePolicyBlob
func (h *UserPoliciesHelper) UpdatePolicyBlob(ctx context.Context, pb *fakedms.PolicyBlob) error {
	return util.UpdatePolicyBlob(ctx, h.FakeDMS, h.Chrome, pb)
}

// UpdatePolicies is a helper that calls util.UpdatePolicies
func (h *UserPoliciesHelper) UpdatePolicies(ctx context.Context, ps []policy.Policy) error {
	return util.UpdatePolicies(ctx, h.FakeDMS, h.Chrome, ps)
}
