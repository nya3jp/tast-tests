...
func canInstallExtension(ctx context.Context, tconn *chrome.TestConn, br *browser.Browser, url string) (bool, error) {
	...
	if err := policyutil.EnsureMaximized(ctx, tconn); err != nil {
	...
	// Install extension.
	if err := uiauto.Combine("Wait and Left Click on Agree button",
		uia.WaitUntilExists(addButton),
		uia.LeftClick(addButton),
	)(ctx); err != nil {
