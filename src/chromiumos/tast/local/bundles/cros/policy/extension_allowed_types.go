...
func canInstallExtension(ctx context.Context, tconn *chrome.TestConn, br *browser.Browser, url string) (bool, error) {
	...
	// Ensure the browser is in fullscreen.
	if err := policyutil.EnsureFullscreenWindow(ctx, tconn); err != nil {
		return false, errors.Wrap(err, "failed to enter fullscreen window")
	}
