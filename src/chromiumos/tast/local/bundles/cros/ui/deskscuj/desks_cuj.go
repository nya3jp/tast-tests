...
func Run(ctx context.Context, s *testing.State) {
	...
	// If we want to have the full test run in 10 minutes, then we want
	// to have each workflow run in 10/3 minutes.
		...
		for _, deskSwitcher := range deskSwitchers {
			...
			for endTime := time.Now().Add(deskSwitchingDuration); time.Now().Before(endTime); {
				...
				if activeDesk != expectedActiveDesk {
					return errors.Errorf("incorrect active desk: desk %d is active, expected %d to be active", activeDesk, expectedActiveDesk)
				}
