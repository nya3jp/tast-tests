...
func (conf *ZoomConference) VideoAudioControl(ctx context.Context) error {
	...
	toggleVideo := func(ctx context.Context) error {
		...
		if err := ui.Retry(retryTimes, uiauto.Combine("switch camera",
			uiauto.IfSuccessThen(ui.Exists(nowCameraButton), ui.DoDefault(nowCameraButton)),
	...
	toggleAudio := func(ctx context.Context) error {
		...
		if err := ui.Retry(retryTimes, uiauto.Combine("switch microphone",
			uiauto.IfSuccessThen(ui.Exists(nowAudioButton), ui.DoDefault(nowAudioButton)),
			ui.WaitUntilGone(nowAudioButton),
		))(ctx); err != nil {
			...
			if err := ui.Exists(settingsButton)(ctx); err == nil {
				actions = append(actions,
					uiauto.NamedAction("click settings button",
						ui.WithTimeout(longUITimeout).RetryUntil(ui.DoDefault(settingsButton), ui.WaitUntilExists(backgroundTab)),
					))
			} else {
