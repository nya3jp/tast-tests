...
func (conf *ZoomConference) Join(ctx context.Context, room string) error {
	...
	videoLocation, err := ui.Location(ctx, startVideoButton)
	if err != nil {
		return errors.Wrap(err, "failed to get the location of the video button")
	}
	// If kept idle, the zoom UI buttons may become invisible.
	// Use mouse move to make the buttons reappear.
	if err := uiauto.Combine("Start video",
		mouse.Move(conf.tconn, videoLocation.CenterPoint(), 500*time.Millisecond),
...
func (conf *ZoomConference) ChangeLayout(ctx context.Context) error {
	...
	if err := uiauto.Combine("Click 'View'",
		kb.AccelAction("Tab"), // For showing view button
	...
	viewLocation, err := ui.Location(ctx, viewButton)
	if err != nil {
		return errors.Wrap(err, "failed to get the location of the view button")
	}
	...
	for _, mode := range []string{speaker, gallery} {
		...
		if err := uiauto.Combine("Change layout to '"+mode+"'",
			mouse.Move(conf.tconn, viewLocation.CenterPoint(), 500*time.Millisecond), // Use mouse move in order to make the button reappear.
			ui.ImmediateLeftClick(viewButton),
...
func (conf *ZoomConference) PresentSlide(ctx context.Context) error {
	...
	shareScreen := func(ctx context.Context) error {
		...
		return uiauto.Combine("Share Screen",
			...
			kb.AccelAction("Tab"),
			ui.WithInterval(time.Second).LeftClickUntil(shareScreenButton, ui.Exists(presentWindow)),
