...
func (conf *ZoomConference) VideoAudioControl(ctx context.Context) error {
	...
	for _, f := range []func(ctx context.Context) error{
		...
		// After toggle, give it 5 seconds for viewing before applying next one.
...
func (conf *ZoomConference) switchToChromeTab(ctx context.Context, tabName string) error {
	...
	if conf.tabletMode {
		...
		return ui.LeftClick(nodewith.NameContaining("toggle tab strip").Role(role.Button).First())(ctx)
	}
