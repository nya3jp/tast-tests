...
func GetGoogleMeetConfig(ctx context.Context, s *testing.ServiceState, roomSize int) (GoogleMeetConfig, error) {
	...
	bondEnabledStr, ok := s.Var("spera.bond_enabled")
	if ok && bondEnabledStr == "true" {
		bondEnabled = true
	} else {
		bondEnabled = false
	}
	bondCreds, ok := s.Var("spera.bond_credentials")
	if !ok || len(bondCreds) < 1 {
		if bondEnabled {
			return GoogleMeetConfig{}, errors.New("bond API is enabled via spera.bond_enabled but spera.bond_credentials is not set")
		}
		bondCreds = ""
	}
