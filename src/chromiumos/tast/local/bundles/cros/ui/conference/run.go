...
func Run(ctx context.Context, cr *chrome.Chrome, conf Conference, prepare Prepare, tier, outDir string, tabletMode, extendedDisplay bool) (retErr error) {
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		if tier == "plus" || tier == "premium" {
			if err := newGoogleSlides(ctx, cr, extendedDisplay); err != nil {
				return err
			}
			if extendedDisplay {
