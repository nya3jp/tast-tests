...
func (conf *zoomConference) Join(ctx context.Context, room string) error {
	...
	if err := conference.WaitUIByName(ctx, tconn, "SIGN IN", time.Second*3); err == nil {
		...
		if err := conference.ClickUIByName(ctx, tconn, conf.account, time.Second*3); err == nil {
			if err := conference.WaitUIByName(ctx, tconn, "Profile picture", time.Minute); err != nil {
