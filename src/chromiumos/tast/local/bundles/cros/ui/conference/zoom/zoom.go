...
// Join implements Conference.Join interface and joins a conference room.
func (conf *zoomConference) Join(ctx context.Context, room string) error {
	...
	if err := testing.Sleep(ctx, time.Second*8); err != nil {
		return errors.Wrap(err, "failed to sleep")
	}
	testing.ContextLog(ctx, "Click 'Join Audio by Computer' button")
	if err := conference.ClickUIByName(ctx, tconn, "Join Audio by Computer", timeout); err != nil {
		return err
	}
...
// AdmitParticipant implements Conference.AdmitParticipant interface and administrates participants.
func (conf *zoomConference) AdmitParticipant(ctx context.Context) error {
	const timeout = time.Second * 15
	...
	if err := conference.ClickUIByName(ctx, tconn, "Admit", timeout*2); err != nil {
		return err
	}
...
// VideoAudioControl implements Conference.VideoAudioControl interface and controls video and audio.
...
// VideoAudioControl implements Conference.SwitchTabs interface and switches between apps.
...
// ChangeLayout implements Conference.ChangeLayout interface and changes the layout.
...
// BackgroundBlurring implements Conference.BackgroundBlurring interface and blurs the background.
func (conf *zoomConference) BackgroundBlurring(ctx context.Context) error {
	//zoom website didn't support background change
	return nil
}

// PresentSlide implements Conference.PresentSlide interface and presents slide.
...
// StopPresenting implements Conference.StopPresenting interface and stops presentation.
...
// End implements Conference.End interface and ends the conference.
...
func NewZoomConference(cr *chrome.Chrome) *zoomConference {
	return &zoomConference{cr: cr}
}
