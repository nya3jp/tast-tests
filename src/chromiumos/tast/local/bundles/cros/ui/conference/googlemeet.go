	...
	enterAccount := func(ctx context.Context) error {
		...
		if err := ui.WaitUntilExists(emailContent)(ctx); err != nil {
			...
			actions = append(actions,
				...
				// And some DUT models require more time to find the email field, so increase the timeout here.
				uiauto.NamedAction("click email field", ui.WithTimeout(30*time.Second).LeftClickUntil(emailField, ui.Exists(emailField.Focused()))),
