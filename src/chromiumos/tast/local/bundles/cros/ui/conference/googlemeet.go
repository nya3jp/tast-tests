...
func (conf *GoogleMeetConference) Join(ctx context.Context, room string, toBlur bool) error {
		...
		for _, step := range []struct {
			...
			if err := ui.WithTimeout(shortUITimeout).WaitUntilExists(step.finder)(ctx); err == nil {
				...
				if err := uiauto.NamedAction(step.name,
					ui.RetryUntil(ui.DoDefault(step.button), ui.WithTimeout(shortUITimeout).WaitUntilGone(step.finder)))(ctx); err != nil {
					return err
				}
			} else {
	...
	switchUser := func(ctx context.Context) error {
		...
		if err := uiauto.Combine("switch account",
			ui.RetryUntil(ui.DoDefault(switchAccount), ui.WithTimeout(shortUITimeout).WaitUntilGone(switchAccount)),
...
func (conf *GoogleMeetConference) VideoAudioControl(ctx context.Context) error {
	...
	toggleVideo := func(ctx context.Context) error {
		...
		if err := ui.Retry(retryTimes, uiauto.Combine("switch camera",
			uiauto.IfSuccessThen(ui.Exists(cameraButton), ui.DoDefault(cameraButton)),
			ui.WaitUntilGone(cameraButton),
		))(ctx); err != nil {
	...
	toggleAudio := func(ctx context.Context) error {
		...
		if err := ui.Retry(retryTimes, uiauto.Combine("switch microphone",
			uiauto.IfSuccessThen(ui.Exists(microphoneButton), ui.DoDefault(microphoneButton)),
			ui.WaitUntilGone(microphoneButton),
		))(ctx); err != nil {
