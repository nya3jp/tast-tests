...
func (conf *GoogleMeetConference) Join(ctx context.Context, room string) error {
	...
	checkParticipantsNum := func(ctx context.Context) error {
		...
		if participantNumber == 1 {
			return ParticipantError(errors.Wrapf(err, "there are no other participants in the conference room %s; meeting participant number got %v; want %v", room, num, roomSize))
		}
		switch roomSize {
		case ClassRoomSize:
			if participantNumber < roomSize {
				return ParticipantError(errors.Wrapf(err, "room url %s; meeting participant number got %v; want at least %v", room, num, roomSize))
			}
		case SmallRoomSize, LargeRoomSize:
			if participantNumber != roomSize && participantNumber != roomSize+1 {
				return ParticipantError(errors.Wrapf(err, "room url %s; meeting participant number got %v; want %v ~ %v", room, num, roomSize, roomSize+1))
			}
		case TwoRoomSize:
			if participantNumber != roomSize {
				return ParticipantError(errors.Wrapf(err, "room url %s; meeting participant number got %v; want %v", room, num, roomSize))
			}
		}
