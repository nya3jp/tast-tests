...
func (conf *googleMeetConference) BackgroundBlurring(ctx context.Context) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err := conference.ClickUIByName(ctx, tconn, "More options", time.Second); err != nil {
			return err
		}

		if err := conference.ClickUIByName(ctx, tconn, "Change background", time.Second); err != nil {
			return err
		}

		if err := conference.ClickUIByName(ctx, tconn, "Blur your background", time.Second); err != nil {
			return err
		}

		if err := conference.ClickUIByName(ctx, tconn, "Close", time.Second); err != nil {
			return err
		}
		return nil
...
func NewGoogleMeetConference(cr *chrome.Chrome) *googleMeetConference {
	return &googleMeetConference{cr: cr}
}
