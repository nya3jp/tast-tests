...
func ShowAndHidePassword(ctx context.Context, tconn *chrome.TestConn, username, pwdValue string, pin bool) error {
	...
	if passwordField != pwdValue {
		return errors.New("PIN / Password revealed after clicking the Show password button is not matching with the user entered values")
	}

	// Verify that the PIN / Password field is in hidden state after clicking Hide password button.
...
func ShowPassword(ctx context.Context, tconn *chrome.TestConn, username string) (string, error) {
	...
	testing.ContextLog(ctx, "PIN / Password revealed after clicking Show password button is: ", passwordField.Value)
...
func WaitForPasswordHidden(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		flag, e := ui.Exists(ctx, tconn, lockscreen.HidePasswordBtnParams)
		if e != nil {
