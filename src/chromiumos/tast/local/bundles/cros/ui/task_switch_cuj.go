...
func TaskSwitchCUJ(ctx context.Context, s *testing.State) {
	...
	// Preparations of ARC apps; gmail, calendar, and youtube.
	...
	// Installing android apps for the everyday works: Gmail and Google Calendar.
	...
	for _, pkgName := range []string{gmailPackageName, calendarPackageName} {
		...
		// Goes back until the search icon appears.
	...
	// Closing the play-store app window through ctrl-w.
	...
	// Setting up the cuj.Recorder: this test will measures the combinations of
	...
	// launching arc apps from the app launcher. each app might have its own
	// splash screen, so here also has the code of skipping them.
	for _, app := range []struct {
		...
		{"gmail", gmailPackageName, func() error {
			...
			gotIt := d.Object(arcui.TextMatches("GOT IT"))
			...
			// is fine.
			pleaseAdd := d.Object(arcui.TextMatches("Please add at least one email address"))
			if err := pleaseAdd.WaitForExists(ctx, timeout); err == nil {
				// the message appears, but the email address should appear behind.
				// Simply clicking the 'OK' button after a few seconds should work.
				if err := testing.Sleep(ctx, timeout); err != nil {
					return errors.Wrap(err, "failed to wait for the email address appearing")
				}
				okButton := d.Object(arcui.ClassName("android.widget.Button"), arcui.Text("OK"))
				// The OK button should appear along with the "please add" message, so
			...
			takeMe := d.Object(arcui.TextMatches("TAKE ME TO GMAIL"))
			if err := takeMe.WaitForExists(ctx, timeout); err != nil {
				// After clicking "got it", it should show "take me to gmail".
			...
			// After clicking 'take me to gmail', it might show a series of dialogs
			// to finalize the setup. Simply clicking 'ok' buttons on them.
			for i := 0; i < customPanelMaxCount; i++ {
				dialog := d.Object(arcui.ID(dialogID))
				if err := dialog.WaitForExists(ctx, timeout); err != nil {
					// No dialogs found. finishing.
					return nil
				}
				dismiss := d.Object(arcui.ID(dismissID))
		...
		{"calendar", calendarPackageName, func() error {
			...
			gotIt := d.Object(arcui.TextMatches("GOT IT|Got it"))
			nextArrow := d.Object(arcui.ID(nextArrowID))
			rightArrow := d.Object(arcui.ID(rightArrowID))
			gotItFound := false
			...
			// skipping the permission dialogs.
			permissionAllow := d.Object(arcui.ID(permissionAllowID))
		...
		if err = recorder.Run(ctx, cr, func() error {
			// Launching app window from launcher; search key -> type the search
			// query -> hit enter to pick up the first result, and wait for the new
			// window to appear, and then conduct the logic of skipping the splash
			// screen.
			...
			// 	return errors.Wrap(err, "failed to wait for the launcher state")
			// }
			...
			numWindows++
			// Waiting some time to stabilize the result of launcher animations.
	...
	for _, url := range []string{
		...
		if !tabletMode {
			// Turn the browser window into the normal state in clamshell mode.
	...
	for i := 0; i < numWindows; i++ {
		if err := recorder.Run(ctx, cr, func() error {
			...
			// Use the arrow key to select the next focused window.
			...
			// Note that there is 'new desks' button in clamshell mode which takes
