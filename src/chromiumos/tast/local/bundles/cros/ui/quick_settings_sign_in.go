...
func QuickSettingsSignIn(ctx context.Context, s *testing.State) {
	...
	// Get the Microphone gain Slider params from Audio Settings subpage
	initial, err := quicksettings.SliderValue(ctx, tconn, quicksettings.SliderTypeMicGain)
	if err != nil {
		s.Fatal("Failed to get the SliderValue in Audio Settings Page: ", err)
	}
	s.Log("Mic gain slider exist with the slider value : ", initial)
