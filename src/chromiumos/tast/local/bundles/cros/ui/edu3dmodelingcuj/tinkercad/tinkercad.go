...
// The editor window is the design editing area of TinkerCAD.
func (tc *TinkerCad) GetEditorWindowRect(ctx context.Context) error {
...
func (tc *TinkerCad) rotate(ctx context.Context) error {
	udOps := tc.ud.WithOptions(uidetection.Retries(3)).WithScreenshotStrategy(uidetection.ImmediateScreenshot).WithTimeout(time.Minute)
	rotateIcon := uidetection.CustomIcon(tc.rotateIconPath, uidetection.MinConfidence(0.6))
	rotateRect, err := udOps.Location(ctx, rotateIcon)
	if err != nil {
