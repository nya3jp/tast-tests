...
func Run(ctx context.Context, cr *chrome.Chrome, isTablet bool, outDir, sampleDesignURL, rotateIconPath string) (retErr error) {
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		if err := tinkerCad.Open(ctx, cr); err != nil {
			return errors.Wrapf(err, "failed to open TinkerCAD URL %s", cuj.TinkerCadSignInURL)
		}
		...
		if err := uiauto.NamedCombine("3D modeling on TinkerCAD",
			// Wait for quiescence and get editor window rectangular before running the scenario.
			tinkerCad.WaitForQuiescence(),
			tinkerCad.GetEditorWindowRect(),
			...
			kb.AccelAction("fullscreen"),
			// Export the design.
			tinkerCad.Export(ctx, downloadsPath, designName),
			// Switch between tabs.
