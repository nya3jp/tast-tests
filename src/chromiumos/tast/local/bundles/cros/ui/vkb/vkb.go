...
// UIConn returns a shared connection to the virtual keyboard HTML page,
...
// extension is loaded or ctx's deadline is reached. The caller should not close
// the returned connection; it will be closed automatically by Close.
...
new Promise((resolve, reject) => {
	const key = document.querySelector('[aria-label="%s"]');
