...
func (s *GoogleMeetService) MeetScenario(ctx context.Context, req *pb.MeetScenarioRequest) (*empty.Empty, error) {
	...
	if err != nil {
		return &empty.Empty{}, errors.Wrap(err, "failed to connect to the test API connection")
	}
	defer tconn.Close()
	...
	gmcli := googlemeet.NewGoogleMeetConference(cr)
	if err := conference.MeetConference(ctx, tconn, gmcli, prepare, nil, req.PerfLevel, outDir); err != nil {
