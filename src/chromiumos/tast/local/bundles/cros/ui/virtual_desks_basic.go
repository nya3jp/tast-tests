...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Checks that virtual desks correctly",
		Contacts: []string{
...
func VirtualDesksBasic(ctx context.Context, s *testing.State) {
	...
	firstDeskLoc, secondDeskLoc := deskMiniViewsInfo[0].Location, deskMiniViewsInfo[1].Location
	if err := pc.Drag(
		firstDeskLoc.CenterPoint(),
		pc.DragTo(secondDeskLoc.CenterPoint(), 3*time.Second))(ctx); err != nil {
	...
	if err := pc.Drag(
		...
		pc.DragTo(firstDeskLoc.CenterPoint(), 3*time.Second))(ctx); err != nil {
	...
	if err := ash.ForEachWindow(ctx, tconn, func(w *ash.Window) error {
		testing.ContextLog(ctx, w.OnActiveDesk)
		...
		s.Fatal("Failed to verify that Files app is in new desk: ", err)
	}
	...
	// Verifies that there are only 1 desk.
	_, err = findDeskMiniViews(ac, ctx, 1)
	if err != nil {
		s.Fatal("Failed to find desks: ", err)
	}
...
// findDeskMiniViews returns a list of DeskMiniView nodes and verifies
// the number of nodes.
