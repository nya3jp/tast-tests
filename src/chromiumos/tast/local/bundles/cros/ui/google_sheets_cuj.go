...
func GoogleSheetsCUJ(ctx context.Context, s *testing.State) {
	...
	configs := []cuj.MetricConfig{cuj.NewCustomMetricConfig(
		"Graphics.Smoothness.PercentDroppedFrames.MainThread.TouchScroll", "percent", perf.SmallerIsBetter,
		[]int64{50, 20})}
	configs = append(configs, cuj.NewCustomMetricConfig(
		"Event.Latency.ScrollUpdate.Touch.TimeToHandled2", "ms", perf.SmallerIsBetter,
		[]int64{100, 250}))
