...
func GoogleSheetsCUJ(ctx context.Context, s *testing.State) {
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		if params.scroll == scrollTypeSwipe {
			s.Log("Using scrollbar")
			if err := cuj.SwipeDownFor(ctx, tpw, tw, 500*time.Millisecond, scrollTimeout); err != nil {
				return errors.Wrap(err, "failed to scroll with scrollbar")
			}
		} else if params.scroll == scrollTypeDownKey {
			s.Log("Using down arrow")
			if err := cuj.PressKeyFor(ctx, kw, "Down", 500*time.Millisecond, scrollTimeout); err != nil {
