...
func GoogleSheetsCUJ(ctx context.Context, s *testing.State) {
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		for _, scroller := range []struct {
			...
			if err := ui.WithInterval(2*time.Second).WithTimeout(5*time.Second).WaitUntilNoEvent(nodewith.Root(), event.LocationChanged)(ctx); err != nil {
				s.Log("Failed to wait for scroll animations to complete: ", err)
			}
