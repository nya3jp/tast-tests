...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:         []string{"test.zip", "misc_zip_files.zip", "many_small_files.zip"},
		Pre:          chrome.LoggedIn(),
...
func FilesAppZipPerf(ctx context.Context, s *testing.State) {
	...
	for _, zipBaseName := range []string{"test", "misc_zip_files", "many_small_files"} {
		...
		duration = testExtractingZipFile(ctx, s, zipBaseName, files, ew)
		...
		duration = testZippingFiles(ctx, tconn, s, files, ew)
...
func testExtractingZipFile(ctx context.Context, s *testing.State, zipBaseName string, files *filesapp.FilesApp, ew *input.KeyboardEventWriter) float64 {
...
func testZippingFiles(ctx context.Context, tconn *chrome.TestConn, s *testing.State, files *filesapp.FilesApp, ew *input.KeyboardEventWriter) float64 {
	// Get the Files App listBox.
	filesBox, err := files.Root.DescendantWithTimeout(ctx, ui.FindParams{Role: ui.RoleTypeListBox}, 15*time.Second)
	if err != nil {
		s.Fatal("Failed getting filesBox: ", err)
