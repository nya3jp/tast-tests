...
func init() {
	testing.AddService(&testing.Service{
		Register: func(srv *grpc.Server, s *testing.ServiceState) {
			pb.RegisterChromeServiceServer(srv, &ChromeService{})
		},
...
func (svc *ChromeService) New(ctx context.Context, req *pb.NewRequest) (*empty.Empty, error) {
	common.SharedObjectsForServiceSingleton.ChromeMutex.Lock()
	defer common.SharedObjectsForServiceSingleton.ChromeMutex.Unlock()
	...
	common.SharedObjectsForServiceSingleton.Chrome = cr
...
func (svc *ChromeService) Close(ctx context.Context, req *empty.Empty) (*empty.Empty, error) {
	...
	if common.SharedObjectsForServiceSingleton.Chrome == nil {
		return nil, errors.New("Chrome not available")
	}

	err := common.SharedObjectsForServiceSingleton.Chrome.Close(ctx)
	if err != nil {
		testing.ContextLog(ctx, "Failed to close Chrome: ", err)
	}

	common.SharedObjectsForServiceSingleton.Chrome = nil
