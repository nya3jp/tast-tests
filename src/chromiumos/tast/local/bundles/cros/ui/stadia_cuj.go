...
func StadiaCUJ(ctx context.Context, s *testing.State) {
	...
	// Wait for the start stage of the game to show up
	...
	if err := recorder.Run(ctx, tconn, func() error {
		...
		// s.Log("press space")
		// if err := kb.Accel(ctx, "Space"); err != nil {
		// 	return errors.Wrap(err, "failed to enter the arcade")
		// }
		// testing.Sleep(ctx, time.Second)
		// s.Log("press down")
		// if err := kb.Accel(ctx, "Down"); err != nil {
		// 	return errors.Wrap(err, "failed to choose 'the walled garden' game")
		// }
		// testing.Sleep(ctx, time.Second)
		// s.Log("press space")
		// if err := kb.Accel(ctx, "Space"); err != nil {
		// 	return errors.Wrap(err, "failed to start the game")
		// }
		// all, _ := webview.Descendants(ctx, ui.FindParams{})
		// for _, child := range all {
		// 	s.Logf("Name = %s, ClassName = %s, Role = %s, State = %s, Location = %s, Value = %s",
		// 		child.Name, child.ClassName, child.Role, child.State, child.Location, child.Value)
		// }
		return nil
	}); err != nil {
