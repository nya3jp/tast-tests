...
func StadiaCUJ(ctx context.Context, s *testing.State) {
	...
	for _, sadName := range []string{"Tabs.SadTab.CrashCreated",
		"Tabs.SadTab.OomCreated", "Tabs.SadTab.KillCreated.OOM",
		"Tabs.SadTab.KillCreated"} {
		h, err := metrics.GetHistogram(ctx, tconn, sadName)
		if err != nil {
			s.Fatal("Sad tab name is not correct: ", err)
		}
		if h.TotalCount() != 0 {
			s.Fatal("Tab renderer crashed and sad tab showed up:", err)
		}
	}
