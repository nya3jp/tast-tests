...
func StadiaCUJ(ctx context.Context, s *testing.State) {
	...
	sadHistNames := []string{"Tabs.SadTab.CrashCreated", "Tabs.SadTab.OomCreated",
		"Tabs.SadTab.KillCreated.OOM", "Tabs.SadTab.KillCreated"}
	sadBefore, err := metrics.GetHistograms(ctx, tconn, sadHistNames)
	if err != nil {
	...
	// TODO(yichenz): use signal from Stadia games instead.
	...
	sadAfter, err := metrics.GetHistograms(ctx, tconn, sadHistNames)
	if err != nil {
		s.Fatal("Failed to get histograms: ", err)
	}
	sadDiffs, err := metrics.DiffHistograms(sadBefore, sadAfter)
	if err != nil {
	...
	for _, h := range sadDiffs {
		if h.Sum != 0 {
			s.Fatal("Tab renderer crashed. Sad tab showed up")
		}
	}
