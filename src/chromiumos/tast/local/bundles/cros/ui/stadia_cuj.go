...
func StadiaCUJ(ctx context.Context, s *testing.State) {
	...
	sadTabMetrics := []string{"Tabs.SadTab.CrashCreated", "Tabs.SadTab.OomCreated",
		"Tabs.SadTab.KillCreated.OOM", "Tabs.SadTab.KillCreated"}
	for _, sadTabMetric := range sadTabMetrics {
		configs = append(configs, cuj.NewCustomMetricConfig(
			sadTabMetric, "tabs", perf.SmallerIsBetter, []int64{1, 1}))
	}
	...
	// Check if any tab renderer crashed
	for _, sadTabMetric := range sadTabMetrics {
		tc, err := recorder.GetMetricTotalCount(sadTabMetric)
		if err != nil {
			s.Fatal("Crash metrics is not recorded.")
		}
		if tc != 0 {
