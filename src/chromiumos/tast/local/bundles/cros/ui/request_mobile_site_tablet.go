...
func RequestMobileSiteTablet(ctx context.Context, s *testing.State) {
	credentials := map[userType]chrome.Creds{
		child:  {User: s.RequiredVar("family.unicornEmail"), Pass: s.RequiredVar("family.unicornPassword"), ParentUser: s.RequiredVar("family.parentEmail"), ParentPass: s.RequiredVar("family.parentPassword")},
		normal: {User: s.RequiredVar("family.parentEmail"), Pass: s.RequiredVar("family.parentPassword")},
	}
...
// then revisit the website to verify if "request mobile site" is still on.
