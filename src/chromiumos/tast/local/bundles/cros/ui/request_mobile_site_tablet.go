...
func RequestMobileSiteTablet(ctx context.Context, s *testing.State) {
	...
	for usr, creds := range accounts {
		...
		if !s.Run(ctx, fmt.Sprintf("request mobile site for %s", usr), f) {
			s.Errorf("Failed to run complete test for %s", usr)
		}
	}
}
...
func mobileSiteTest(ctx context.Context, cr *chrome.Chrome, res *mobileTestResources, websiteName, url string) (retErr error) {
	...
	defer func(ctx context.Context) {
		faillog.DumpUITreeWithScreenshotOnError(ctx, res.outDir, func() bool { return retErr != nil }, cr, fmt.Sprintf("ui_dump_%s", res.usr))
...
func ensureLeftOffSettingOn(ctx context.Context, cr *chrome.Chrome, res *mobileTestResources) (retErr error) {
	cleanupCtx := ctx
	ctx, cancel := ctxutil.Shorten(ctx, 10*time.Second)
	defer cancel()
	...
	defer func(ctx context.Context) {
		faillog.DumpUITreeWithScreenshotOnError(ctx, res.outDir, func() bool { return retErr != nil }, cr, fmt.Sprintf("ui_dump_left_off_%s", res.usr))
		conn.CloseTarget(ctx)
		conn.Close()
	}(cleanupCtx)
