	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		for _, app := range appsList {
			if err := apps.Launch(ctx, tconn, app.ID); err != nil {
				errors.Wrapf(err, "failed to open %s", app.Name)
			}
			if err := ash.WaitForApp(ctx, tconn, app.ID, time.Minute); err != nil {
				errors.Wrapf(err, "%s did not appear in shelf after launch", app.Name)
			}
		}

		if err := ac.WaitForLocation(nodewith.Root())(ctx); err != nil {
			errors.Wrap(err, "failed to wait for app launch events to be completed")
		}

		// Enters overview mode.
		if err := ash.SetOverviewModeAndWait(ctx, tconn, true); err != nil {
			errors.Wrap(err, "failed to set overview mode")
		}

		if err := ac.WaitForLocation(nodewith.Root())(ctx); err != nil {
			errors.Wrap(err, "failed to wait for overview animation to be completed")
		}
		...
		if err := uiauto.Combine(
			...
			errors.Wrap(err, "failed to save a desk template")
		}

		// Exits overview mode.
		if err = ash.SetOverviewModeAndWait(ctx, tconn, false); err != nil {
			errors.Wrap(err, "failed to exit overview mode")
		}

		// Close all existing windows.
		ws, err := ash.GetAllWindows(ctx, tconn)
		if err != nil {
			errors.Wrap(err, "failed to get all open windows")
		}
		for _, w := range ws {
			if err := w.CloseWindow(ctx, tconn); err != nil {
				errors.Wrapf(err, "failed to close window (%+v)", w)
			}
		}
		...
		if err := uiauto.Combine(
			...
			errors.Wrap(err, "failed to show saved desks templates")
		}

		// Confirm there is one desk template.
		deskTemplatesInfo, err := ash.FindDeskTemplates(ctx, ac)
		if err != nil {
			errors.Wrap(err, "failed to find desk templates")
		}
		if len(deskTemplatesInfo) != 1 {
			errors.Errorf("got %v desk template(s), there should be one desk template", len(deskTemplatesInfo))
		}
		...
		if err := uiauto.Combine(
			...
			errors.Wrap(err, "failed to launch a desk template")
		}
		...
		if len(ws) != len(appsList) {
			errors.Errorf("got %v window(s), should have %v windows", len(ws), len(appsList))
		}
