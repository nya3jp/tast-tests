...
func init() {
	testing.AddTest(&testing.Test{
		...
		Vars: []string{
			"ui.cuj_mode", // Optional. Use "tablet" or "clamshell" to force the mode. Other values will be be taken as "clamshell".
...
// Known issues: b:187165216 (on certain models)
func ExtendedDisplayCUJ(ctx context.Context, s *testing.State) {
	...
	// The external display is not able to control by touch event,
	// so we'll treat all UI action as clamshell ways even on tablet mode.
