...
func TabHoverCardAnimationPerf(ctx context.Context, s *testing.State) {
	...
	for i := 0; i < 2; i++ {
		if i == 0 {
			if conn, br, closeBrowser, err = browserfixt.SetUpWithURL(ctx, cr, bt, ui.PerftestURL); err == nil {
	...
	if err != nil {
		s.Fatalf("Failed to connect to test API for %v browser: %v", bt, err)
	}
	// Note that `br` and `bTconn` should be passed to perfutil since the metrics are recorded in primary browser.
