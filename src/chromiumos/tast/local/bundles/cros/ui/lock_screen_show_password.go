...
// LockScreenShowPassword tests the functionality of Show password button to view the password and Hide password button to hide it on lockscreen.
func LockScreenShowPassword(ctx context.Context, s *testing.State) {
	...
	if err := showpassword.ShowAndHidePassword(ctx, tconn, username, password, false); err != nil {
		s.Fatal("Show/Hide password button failed to work for lockscreen password field: ", err)
	}
}
