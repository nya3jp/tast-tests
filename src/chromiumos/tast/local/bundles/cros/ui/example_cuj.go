...
func ExampleCUJ(ctx context.Context, s *testing.State) {
	...
	// Access Chrome from the fixture, as long as the fixture supports
	// the HasChrome interface.
	...
	// us click with the mouse on the device.
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		// Lacros, by default, starts out with a New Tab window. Thus,
		// after opening about:blank, there will be 2 tabs open. Close
		// the New Tab window to make Ash/Lacros variants equivalent.
		...
		for endTime := time.Now().Add(testDuration); time.Now().Before(endTime); {
			...
			for _, direction := range []string{"Down", "Up"} {
				// Scroll 50 mouse wheel ticks, with a 50 milliseconds
	...
	// report the metrics, and pv.Save to save the metrics to the test
	// out directory.
	pv := perf.NewValues()
