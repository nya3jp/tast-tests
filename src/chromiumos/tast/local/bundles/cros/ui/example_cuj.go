...
func ExampleCUJ(ctx context.Context, s *testing.State) {
	...
	if err != nil {
		s.Fatal("Failed to open the browser")
	}
	...
	if err != nil {
		s.Fatal("Falied to connect to browser test API connection")
	}
	...
	// Set up the recorder. This example recorder setup consists of
	// the following steps, with only #1 being required to use the
	// recorder:
	// 1. Creating a new recorder with cujrecorder.NewRecorder.
	// 2. Enabling tracing.
	// 3. Adding the pre-existing list of metrics to the recorder
	// with recorder.AddCommonMetrics.
	// 4. Adding the screen recorder to the recorder to capture a
	// video of the test, only if the "record" flag is passed as a
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		// this test, we repeat the following action until 10
		// minutes have passed.
		for endTime := time.Now().Add(testDuration); time.Now().Before(endTime); {
			...
			// Click on the search window on the Chromium
			// home page. Once the expected search popup
			// appears, type a query in the text box. Press
			// ESC to exit the search window. These series of
			// actions on Clamshell devices generate mouse
			// move/press/release and key press metrics.
			...
			// canned series of actions that can generate
			// ADF metrics. In particular, it toggles the
			// system tray, and drags a window around in
			// overview mode.
