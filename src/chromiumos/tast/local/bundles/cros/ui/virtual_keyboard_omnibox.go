...
func VirtualKeyboardOmnibox(s *testing.State) {
	...
	clickOmnibox := func(ctx context.Context, tconn *chrome.Conn) error {
	...
	cr, err := chrome.New(s.Context(), chrome.ExtraArgs([]string{"--enable-virtual-keyboard", "--enable-experimental-extension-apis"}))
	if err != nil {
