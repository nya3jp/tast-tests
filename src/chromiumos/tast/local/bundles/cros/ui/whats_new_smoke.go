...
func WhatsNewSmoke(ctx context.Context, s *testing.State) {
	...
	// Wait for Settings and What's New to be available in the list of all Chrome apps.
	// Without this step, sometimes Settings will not launch and What's New will launch
	// as a Chrome window instead of a PWA.
	for _, app := range []apps.App{apps.Settings, apps.WhatsNew} {
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			...
			for _, capp := range capps {
				if capp.AppID == app.ID {
					return nil
				}
			}
			return errors.New("App not yet found in available Chrome apps")
		}, nil); err != nil {
			s.Fatalf("Unable to find the %v app in the available Chrome apps: %v", app.Name, err)
		}
	}
	...
	// Particulary in this case, the apostrophe in What’s is not actually the normal
	// apostrophe character, but sintead the "right single quotation mark" character (&rsquo;).
