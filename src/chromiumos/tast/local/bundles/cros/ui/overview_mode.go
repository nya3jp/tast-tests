...
func OverviewMode(ctx context.Context, s *testing.State) {
	...
	// Chrome (maximized window) should open in animation when entering overview mode.
	go func() {
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			...
			for _, window := range ws {
				if strings.Contains(window.Title, "Chrome") && !window.IsAnimating {
					return errors.New("chrome window is not animating")
				}
				if strings.Contains(window.Title, "Files") && window.IsAnimating {
					return errors.New("files window is animating")
				}
			}
			return nil
		}, &testing.PollOptions{Timeout: 5 * time.Second}); err != nil {
			s.Fatal("Maximized and(or) normal windows didn't open in the overview as expected: ", err)
		}
	}()
	...
	// Click the close button in overview should close the window.
	chromeOverviewItemView := nodewith.NameRegex(regexp.MustCompile("Chrome(.*?)")).ClassName("OverviewItemView")
	closeChromeButton := nodewith.Name("Close").Ancestor(chromeOverviewItemView)
