...
func init() {
	testing.AddTest(&testing.Test{
		Func:         TCPBS4CrXPRT2,
		Desc:         "Execute Chrome extension CrXPRT 2 to do benchmark and acquire that score",
		...
		HardwareDeps: hwdep.D(
			...
			hwdep.Model("barla", "bluebird", "eve", "krane", "liara", "maple", "pyke"),
		),
...
// TCPBS4CrXPRT2 execute Chrome extension CrXPRT 2 to do benchmark and acquire that score
func TCPBS4CrXPRT2(ctx context.Context, s *testing.State) {
	const (
		extName     = "CrXPRT 2"
	...
	// it is possible that the whole page is blank, if connect to app right after it launch/appear
	...
	// setting the device name, CrXPRT 2 demand the device name be set before execution
	if shown, err := isElementShown(ctx, conn, inputFieldDivID); err != nil {
		...
		deviceName := s.RequiredVar("ui.cuj_username")
...
// installChromeExt install the Chrome app by given Chrome extension URL.
func installChromeExt(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, url string) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		return errors.New("crxprt-2 still installing")
	}, &pollOpt); err != nil {
		return errors.Wrap(err, "failed to install crxprt-2")
	}
	return nil
