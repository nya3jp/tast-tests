...
func WindowArrangementCUJ(ctx context.Context, s *testing.State) {
	...
	if !tabletMode {
		...
		f = func(ctx context.Context) error {
			...
			// Snap one of the window to the left from the overview mode and the
			// another window will still be in the overview mode.
			...
			if err := pc.Press(ctx, w.OverviewInfo.Bounds.CenterPoint()); err != nil {
