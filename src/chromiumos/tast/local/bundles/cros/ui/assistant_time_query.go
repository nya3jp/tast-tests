...
func AssistantTimeQuery(ctx context.Context, s *testing.State) {
	...
	results, err := parseTimeNearNow(html, now)
	if err != nil {
...
func parseTimeNearNow(html string, now time.Time) ([]time.Time, error) {
	re := regexp.MustCompile(`(\d{1,2})(:\d\d)? ([AaPp][Mm])?`)
	matches := re.FindStringSubmatch(html)
	if matches == nil {
		return nil, errors.Errorf("the HTML string doesn't contain any well-formatted time results: %v", html)
	}
...
// interpretTimeNearNow chooses the interpretation closest to the now time because the Assistant time
// result only contains HH:MM.
