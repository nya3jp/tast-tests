...
func parseTimeNearNow(ctx context.Context, html string, now time.Time) ([]time.Time, error) {
	...
	// Logs the HTML string for debugging if we failed to find any well-formatted time results.
	if matches == nil {
		outDir, ok := testing.ContextOutDir(ctx)
		if !ok {
			testing.ContextLog(ctx, "Failed to write log message. Could not get ContextOutDir()")
		}
		const logName = "time_query_html_response.txt"
		if err := ioutil.WriteFile(filepath.Join(outDir, logName), []byte(html), 0644); err != nil {
			testing.ContextLogf(ctx, "Failed to write log to %s: %v", filepath.Join(outDir, logName), err)
		}
		return nil, errors.Errorf("HTML response(logged in %s) doesn't contain any well-formatted time results", logName)
	}
