...
func MeetMultiTaskingCUJ(ctx context.Context, s *testing.State) {
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		s.Logf("Scrolling down the Google Sheets file for %d minutes", int(docsTimeout.Minutes()))
		...
		if err != nil {
			s.Fatal("Failed to open the google slides website: ", err)
		}
		defer slidesConn.Close()
		defer slidesConn.CloseTarget(closeCtx)

		// Go through the Slides deck.
		s.Logf("Going through the Google Slides file for %d minutes", int(slidesTimeout.Minutes()))
		...
		if err != nil {
			s.Fatal("Failed to open the Gmail website: ", err)
		}
		...
		s.Logf("Scrolling down the Gmail inbox for %d minutes", int(gmailTimeout.Minutes()))
		for end.Sub(time.Now()).Seconds() > 0 {
			// Double swipe from the middle buttom to the middle top of the touchpad.
			var startX, startY, endX, endY input.TouchCoord
			startX, startY, endX, endY = tpw.Width()/2, 1, tpw.Width()/2, tpw.Height()-1
