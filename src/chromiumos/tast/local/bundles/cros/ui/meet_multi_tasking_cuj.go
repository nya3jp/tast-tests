...
func MeetMultiTaskingCUJ(ctx context.Context, s *testing.State) {
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		twoFingerScrollDown := func(duration time.Duration) error {
			...
			testing.ContextLogf(ctx, "Scrolling down the Google Sheets file for %d seconds", int(duration.Seconds()))
		...
		s.Logf("Scrolling down the Google Docs file for %d seconds", int(docsScrollTimeout.Seconds()))
		...
		if err := docsConn.Eval(ctx, "parseInt(document.getElementsByClassName('navigation-widget-content')[0].scrollTop)", &scrollTop); err != nil {
			return errors.Wrap(err, "failed to get the number of pixels that the scrollbar is scrolled vertically")
		}
		if scrollTop == 0 {
			return errors.New("Docs file is not getting scrolled")
		}
		...
		s.Logf("Going through the Google Slides file for %d seconds", int(slidesScrollTimeout.Seconds()))
		end := time.Now().Add(slidesScrollTimeout)
		for end.Sub(time.Now()).Seconds() > 0 {
			if err := uiauto.Combine(
		...
		if scrollTop == 0 {
			return errors.New("Slides file is not getting scrolled")
		}
		...
		// 3. Multi-tasking with Gmail by opening the inbox and scolling down.
		...
		s.Logf("Scrolling down the Gmail inbox for %d seconds", int(gmailScrollTimeout.Seconds()))
