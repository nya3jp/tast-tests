...
func Run(ctx context.Context, outDir string, cr *chrome.Chrome, tabletMode bool) error {
	...
	uiHandler, err := getUIHandler(ctx, tconn, tabletMode)
	if err != nil {
		return err
	}
	...
	if err := recorder.Run(ctx, func(ctx context.Context) (retErr error) {
		...
		defer func(ctx context.Context) {
			cleanup(ctx, tconn, cr, w, outDir, retErr)
		}(cleanupCtx)
		...
		defer func(ctx context.Context) {
			docCleanup(ctx, tconn, cr, uiHandler, outDir, retErr)
		}(cleanupCtx)
...
func getUIHandler(ctx context.Context, tconn *chrome.TestConn, tabletMode bool) (uiHandler cuj.UIActionHandler, err error) {
	if tabletMode {
		if uiHandler, err = cuj.NewTabletActionHandler(ctx, tconn); err != nil {
			return uiHandler, errors.Wrap(err, "failed to create tablet action handler")
		}
	} else {
		if uiHandler, err = cuj.NewClamshellActionHandler(ctx, tconn); err != nil {
			return uiHandler, errors.Wrap(err, "failed to create clamshell action handler")
		}
	}
	return uiHandler, nil
}

func cleanup(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome, w *wevideo.WeVideo, outDir string, retErr error) {
...
func docCleanup(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome, uiHandler cuj.UIActionHandler, outDir string, retErr error) {
