...
func videoEditingScenario(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome, kb *input.KeyboardEventWriter,
	uiHdl cuj.UIActionHandler, tabletMode bool, outDir string) (retErr error) {
	cleanupCtx := ctx
	ctx, cancel := ctxutil.Shorten(ctx, 15*time.Second)
	defer cancel()
	...
	defer func(ctx context.Context) {
		docCleanup(ctx, tconn, cr, uiHdl, outDir, retErr)
	}(cleanupCtx)

	return uiauto.Combine("run the video editing scenario",
...
func docCleanup(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome, uiHdl cuj.UIActionHandler, outDir string, retErr error) {
