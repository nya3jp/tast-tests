...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc: "Verify youtube streaming and audio routing through onboard speaker",
		Contacts: []string{
...
func YoutubeAudioStress(ctx context.Context, s *testing.State) {
	...
	var (
		...
		videoSource       = videocuj.VideoSrc{
			"https://www.youtube.com/watch?v=g3PWYH1P7Oo",
			"12 Hours Of Tropical Coral Reef Fishes At Monterey Bay Aquarium | Littoral Relaxocean - YouTube",
			"1080p60",
		}
	)
	...
	// isVideoPlaying verifies whether youtube video is playing or not.
	isVideoPlaying := func() error {
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			var playerSate int
			if err := ytbWeb.GetYtWebConn().Eval(ctx, `document.getElementById('movie_player').getPlayerState()`, &playerSate); err != nil {
	...
	if err := testing.Poll(ctx, func(c context.Context) error {
		...
		if timeNow < endTime {
			s.Logf("Audio is routing to %s, test remaining time: %f/%f sec", expectedAudioNode, timeNow, endTime)
			return errors.New("audio is routing")
		}
		return nil
	}, &testing.PollOptions{Interval: 2 * time.Minute, Timeout: time.Duration(duration.minutes) * time.Minute}); err != nil {
