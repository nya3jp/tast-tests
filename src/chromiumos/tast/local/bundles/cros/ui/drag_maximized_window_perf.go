...
func DragMaximizedWindowPerf(ctx context.Context, s *testing.State) {
	...
	const numPoints = 5
	var points [numPoints]coords.Point

	// Caption center.
	points[0] = coords.NewPoint(window.BoundsInRoot.Width/2, 10)
	// Points in some select parts of the display (diamond shape).
	points[1] = coords.NewPoint(10, height/2)
	points[2] = coords.NewPoint(width/2, height-10)
	points[3] = coords.NewPoint(width-10, height/2)
	...
	hists, err := metrics.Run(ctx, tconn, func() error {
		...
		if err := ash.MouseMove(ctx, tconn, points[0], 10*time.Millisecond); err != nil {
			return errors.Wrap(err, "failed to move to caption")
		}
		if err := ash.MousePress(ctx, tconn, ash.LeftButton); err != nil {
