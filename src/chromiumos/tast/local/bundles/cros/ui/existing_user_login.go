...
func ExistingUserLogin(ctx context.Context, s *testing.State) {
	...
	if err := login.WaitForStatus(ctx, tLoginConn, login.ReadyForPassword, true, 30*time.Second); err != nil {
		s.Fatal("Failed waiting for the login screen to be ready for password entry: ", err)
	}

	// TODO(crbug/1109381): the password field isn't actually ready just yet when WaitForStatus returns.
	...
	if err := login.WaitForPasswordField(ctx, tLoginConn, username, 5*time.Second); err != nil {
	...
	if err := login.WaitForStatus(ctx, tLoginConn, login.LoggedIn, true, 30*time.Second); err != nil {
		s.Fatal("Failed waiting for the login screen to be ready for password entry: ", err)
	}
