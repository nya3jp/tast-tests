...
import (
	...
	"chromiumos/tast/local/chrome/ui/login"
...
func ExistingUserLogin(ctx context.Context, s *testing.State) {
	...
	if st, err := login.WaitStatus(ctx, tLoginConn, func(st login.Status) bool { return st.Ready }, 30*time.Second); err != nil {
		s.Fatalf("Failed waiting for the login screen to be ready for password entry: %v (last status: %+v)", err, st)
	}

	// TODO(crbug/1109381): the password field isn't actually ready just yet when WaitStatus returns.
	...
	if err := login.WaitForPasswordField(ctx, tLoginConn, 5*time.Second); err != nil {
	...
	if st, err := login.WaitStatus(ctx, tLoginConn, func(st login.Status) bool { return st.LoggedIn }, 30*time.Second); err != nil {
		s.Fatalf("Failed waiting for login status to change: %v (last status: %+v)", err, st)
	}
