...
func NotificationCentreSmoke(ctx context.Context, s *testing.State) {
	...
	// Depending on DUT state when test is run, the notificaiton could be focused and will not autodismiss.
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			// Notificaiton dismissed
			return nil
		}
		// Notification stil exists. Check if it has focus
