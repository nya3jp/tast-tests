...
type notificationCloseTestType string

const (
	clearOneAtATime notificationCloseTestType = "ClearOneAtATime" // In test, clear notifications one at a time.
	clearAll        notificationCloseTestType = "ClearAll"        // In test, clear all notifications at once
)
...
func NotificationClosePerf(ctx context.Context, s *testing.State) {
	...
	var shouldClearOneAtATime = s.Param().(notificationCloseTestType) == clearOneAtATime
	...
	// Create 12 notifications (3 groups of 4 different notifications), close them all via either the ClearAll button or one at a time, and record performance metrics.
