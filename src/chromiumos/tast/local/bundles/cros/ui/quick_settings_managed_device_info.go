...
func QuickSettingsManagedDeviceInfo(ctx context.Context, s *testing.State) {
	...
	if !strings.Contains(managedBtn.Name, "managedchrome.com") && !strings.Contains(managedBtn.Name, "enterprise managed") {
		s.Fatalf("Managed info string: %s, expected containing management domain name or enterprise managed indication", managedBtn.Name)
	}
	...
	if conn, err := cr.NewConnForTarget(ctx, chrome.MatchTargetURL("chrome://management/")); err != nil {
		s.Fatal("Management page does not open: ", err)
	} else {
		conn.Close()
	}
}
