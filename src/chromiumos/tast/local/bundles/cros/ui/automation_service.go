...
func (svc *AutomationService) Info(ctx context.Context, req *pb.InfoRequest) (*pb.InfoResponse, error) {
	...
	pbNodeInfo, _ := toNodeInfoPB(nodeInfo)
...
func (svc *AutomationService) MouseClickAtLocation(ctx context.Context, req *pb.MouseClickAtLocationRequest) (*empty.Empty, error) {
	...
	//uiauto.clickType is private. Passing an integer stored in an int variable does not compile.
	switch req.ClickType {
	case pb.ClickType_LEFT_CLICK:
		err = ui.MouseClickAtLocation(0, loc)(ctx)
	case pb.ClickType_RIGHT_CLICK:
		err = ui.MouseClickAtLocation(1, loc)(ctx)
	case pb.ClickType_DOUBLE_CLICK:
...
func toClickType(ct pb.ClickType_ClickType) (clickType, error) {
	switch ct {
	case pb.ClickType_LEFT_CLICK:
		return leftClick, nil
	case pb.ClickType_RIGHT_CLICK:
		return rightClick, nil
	case pb.ClickType_DOUBLE_CLICK:
		return doubleClick, nil
	default:
		return 0, errors.New("unknown clicktype")
	}
}

// func toPoint (ct pb.ClickType_ClickType) clickType
...
func toRolePB(r role.Role) pb.Role_Role {
	enumVal := pb.Role_Role_value[toConstCase(string(r))]
	r2 := pb.Role_Role(enumVal)
	return r2
}
...
func toCheckedPB(c checked.Checked) pb.Checked_Checked {
	switch c {
	case checked.True:
		return pb.Checked_TRUE
	case checked.False:
		return pb.Checked_FALSE
	case checked.Mixed:
		return pb.Checked_MIXED
	default:
		return pb.Checked_UNSPECIFIED
	}
}

func toRestrictionPB(r restriction.Restriction) pb.Restriction_Restriction {
	switch r {
	case restriction.Disabled:
		return pb.Restriction_DISABLED
	case restriction.ReadOnly:
		return pb.Restriction_READ_ONLY
	case restriction.None:
		return pb.Restriction_NONE
	default:
		return pb.Restriction_UNSPECIFIED
	}
}
...
func toRole(input pb.Role_Role) (role.Role, error) {
