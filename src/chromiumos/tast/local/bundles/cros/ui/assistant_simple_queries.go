...
func testAssistantSimpleMathQuery(ctx context.Context, s *testing.State, tconn *chrome.TestConn) {
	...
	if !strings.Contains(html, "-999.9") {
		// Log the HTML string to file for debugging.
		if err := ioutil.WriteFile(filepath.Join(s.OutDir(), "math_query_html_response.txt"), []byte(html), 0644); err != nil {
			s.Errorf("Failed to write output to %s: %v", filepath.Join(s.OutDir(), "math_html.txt"), err)
		}
