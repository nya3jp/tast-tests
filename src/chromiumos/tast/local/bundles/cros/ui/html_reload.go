...
func HTMLReload(ctx context.Context, s *testing.State) {
	...
	for video, url := range map[string]string{
		...
		dataPath, err := setupImportedFiles(video, url)
		if err != nil {
...
// setupImportedFiles imported given files to `Downloads` folder.
func setupImportedFiles(name, source string) (string, error) {
