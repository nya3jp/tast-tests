...
func scrollAlongOneDirectionUntilEnd(ctx context.Context, tconn *chrome.TestConn, d directionType) error {
	var scrollTime int
...
func runShelfScrollAnimation(ctx context.Context, tconn *chrome.Conn) error {
	...
	if err := scrollAlongOneDirectionUntilEnd(ctx, tconn, right); err != nil {
		return err
	}

	if err := scrollAlongOneDirectionUntilEnd(ctx, tconn, left); err != nil {
		return err
	}
