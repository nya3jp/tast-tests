...
func DMICRecord(ctx context.Context, s *testing.State) {
	...
	// Record the audio for 30 Seconds using rec command.
	cmd := fmt.Sprintf("rec -r %d -c %d %s trim 0 30", audioRate, audioChannel, filepath.Join(filesapp.DownloadPath, recWavFileName))
	...
	defer func() {
		files.Close(ctx)
		os.Remove(filepath.Join(filesapp.DownloadPath, recWavFileName))
