...
func (svc *ConnService) NewConnForTarget(ctx context.Context, req *pb.NewConnForTargetRequest) (*pb.NewConnForTargetResponse, error) {
	...
	return &pb.NewConnForTargetResponse{Id: id}, nil
}
...
func (svc *ConnService) getConnByID(id string) (*chrome.Conn, error) {
	...
	if !ok {
		return nil, errors.Errorf("no conn exists for id = %s", id)
	}
