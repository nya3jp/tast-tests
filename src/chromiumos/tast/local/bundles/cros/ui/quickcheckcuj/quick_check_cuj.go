...
func Run(ctx context.Context, s *testing.State, cr *chrome.Chrome, pauseMode PauseMode, tabletMode bool) *perf.Values {
	...
	if pauseMode == Suspend {
		...
		// replace ui_action_handler's Test API connection with new one
		uiActionHandler.ResetTestConnection(tconn)
	...
	go func() {
		...
		for scanner.Scan() {
			...
			if u == 1 {
				...
				testing.ContextLogf(ctx, "Failed to parser unexpected time unit in command line message %q", ss[0])
				return
			}
