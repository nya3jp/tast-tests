...
// Network connection might took much longer time. Use a longer polling time.
var networkPollOpt = testing.PollOptions{Timeout: time.Minute, Interval: time.Second}
...
func prepareWiFi(ctx context.Context, s *testing.State, tconn *chrome.TestConn) error {
	ssid := s.RequiredVar("ui.QuickCheckCUJ2_wifissid")
	wpwd := s.RequiredVar("ui.QuickCheckCUJ2_wifipassword")
	...
	// Ensure the wi-fi is enabled so that the page can be navigate to Wi-Fi page
	...
	if err := ui.IfSuccessThen(ui.WaitUntilExists(wifiBtn), ui.LeftClick(wifiBtn))(ctx); err != nil {
		return errors.Wrap(err, "failed to navigate to Wi-Fi button")
	}
	...
	if err := ui.IfSuccessThen(ui.WaitUntilExists(wifiApNode), ui.LeftClick(wifiApNode))(ctx); err != nil {
		return errors.Wrapf(err, "failed to find Wi-Fi AP (SSID: %s)", ssid)
	}
	...
	}, &networkPollOpt); err != nil {
		return errors.Wrapf(err, "failed to connect to Wi-Fi (SSID: %s)", ssid)
	}
...
func Run(ctx context.Context, s *testing.State, cr *chrome.Chrome, pauseMode PauseMode, tabletMode bool) *perf.Values {
	...
	if err = recorder.Run(runCtx, func(ctx context.Context) error {
		...
		if err = ui.WithPollOpts(networkPollOpt).WaitUntilExists(connectedNodeFinder)(ctx); err != nil {
			return errors.Wrap(err, "failed to wait for Wi-Fi be connected")
		}
		s.Log("Wi-Fi is connected")
