...
func Run(ctx context.Context, s *testing.State, cr *chrome.Chrome, pauseMode PauseMode, tabletMode bool) *perf.Values {
	...
	if pauseMode == Suspend {
		// suspendSeconds is the actual sleep time. Time used to suspending the system is not included.
		// Tast runner might time out if DUT is unaccessible for more than 60 seconds. So keep this
		// sleep time short.
		suspendSeconds := 15
		earliestResumeEndTime := time.Now().Add(time.Duration(suspendSeconds) * time.Second)

		if err := suspendAndResume(ctx, cr, suspendSeconds); err != nil {
			s.Fatal("Failed to suspend the DUT: ", err)
		}

		wakeupDuration, err := getWakeupDuration(ctx, earliestResumeEndTime)
		if err != nil {
			s.Fatal("Failed to get wakeup time: ", err)
		}
...
func suspendAndResume(ctx context.Context, cr *chrome.Chrome, suspendSeconds int) error {
	...
	testing.ContextLogf(ctx, "Start a DUT suspend of %d seconds: %s", suspendSeconds, cmd.Args)
...
// getWakeupDuration gets the wakeup duration from last_resume_timings file.
...
func getWakeupDuration(ctx context.Context, earliestModTime time.Time) (float64, error) {
	const (
