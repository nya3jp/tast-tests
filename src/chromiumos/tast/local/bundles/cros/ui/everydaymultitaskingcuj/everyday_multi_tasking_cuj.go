...
func Run(ctx context.Context, s *testing.State, musicPlayer string, tabletMode, openBluetooth bool) {
	...
	// TODO: (jane.yang@cienet.com) Don't read ui.bt_devicename. In stead,
	// disable all BT devices when openBluetooth is false.
	deviceName := s.RequiredVar("ui.bt_devicename")
	isBtConnected, err := bluetooth.IsConnected(ctx, tconn, deviceName)
	if err != nil {
		s.Fatal("Failed to check if bluetooth is connected: ", err)
	}

	if openBluetooth && !isBtConnected {
