...
func NewRunParams(cr *chrome.Chrome, a *arc.ARC, tier cuj.Tier, ccaScriptPaths []string, outDir, appName, account string, tabletMode bool) *RunParams {
	return &RunParams{cr: cr, a: a, tier: tier, ccaScriptPaths: ccaScriptPaths, outDir: outDir, appName: appName, account: account, tabletMode: tabletMode}
}
...
func Run(ctx context.Context, params *RunParams) (retErr error) {
	cleanupCtx := ctx
	ctx, cancel := ctxutil.Shorten(ctx, 10*time.Second)
	defer cancel()

	tconn, err := params.cr.TestAPIConn(ctx)
	if err != nil {
	...
	d, err := params.a.NewUIDevice(ctx)
	if err != nil {
	...
	if params.appName == SpotifyAppName {
		appSpotify = NewSpotify(kb, params.a, d, params.account, uiTimeout)

		testing.ContextLog(ctx, "Check and install ", spotifyPackageName)
		if err := appSpotify.Install(ctx, tconn, 3*time.Minute); err != nil {
			return errors.Wrap(err, "failed to install Spotify")
		}
	}

	testing.ContextLog(ctx, "Start to get browser start time")
	browserStartTime, err := cuj.GetBrowserStartTime(ctx, params.cr, tconn, params.tabletMode)
	if err != nil {
	...
	defer func(ctx context.Context) {
		faillog.SaveScreenshotOnError(ctx, params.cr, params.outDir, func() bool { return retErr != nil })
		...
		params.a.DumpUIHierarchyOnError(ctx, params.outDir, func() bool { return retErr != nil })
		cuj.CloseBrowserTabs(ctx, tconn)
		if appSpotify != nil {
			appSpotify.Close(ctx, tconn)
		}
		if params.appName == YoutubeMusicAppName {
			leaveButton := nodewith.Name("Leave").Role(role.Button)
			if err := uiHandler.Click(leaveButton)(ctx); err != nil {
				testing.ContextLog(ctx, "Leave button did not display")
			}
		}
	}(faillogCtx)
	...
	if params.appName == SpotifyAppName {
		if err = recorder.Run(ctx, func(ctx context.Context) error {
			t, err := appSpotify.Launch(ctx, tconn, time.Minute)
			if err != nil {
			...
			return testing.Sleep(ctx, uiTimeout)
		}); err != nil {
			return errors.Wrap(err, "failed to launch Spotify")
		}
	}

	resources := &runResources{tconn: tconn, kb: kb, topRow: topRow, ui: ui, vh: vh, uiHandler: uiHandler, recorder: recorder}

	if err := openAndSwitchTabs(ctx, params, resources); err != nil {
		return errors.Wrap(err, "failed to open and switch chrome tabs")
	}

	if err := switchWindows(ctx, params, resources); err != nil {
		return errors.Wrap(err, "failed to switch windows")
	}

	testing.ContextLog(ctx, "Take photo and video")
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		return takePhotoAndVideo(ctx, params.cr, params.ccaScriptPaths, params.outDir)
	}); err != nil {
...
func openAndSwitchTabs(ctx context.Context, params *RunParams, resources *runResources) error {
	const (
	...
	openBrowserWithTabs := func(urlList []string) error {
		for idx, url := range urlList {
			conn, err := resources.uiHandler.NewChromeTab(ctx, params.cr, url, idx == 0)
			if err != nil {
	...
	switchAllBrowserTabs := func(ctx context.Context) error {
		...
		ws, err := ash.GetAllWindows(ctx, resources.tconn)
		if err != nil {
		...
		for range ws {
			...
			w, err := ash.GetActiveWindow(ctx, resources.tconn)
			if err != nil {
...
func switchWindows(ctx context.Context, params *RunParams, resources *runResources) error {
