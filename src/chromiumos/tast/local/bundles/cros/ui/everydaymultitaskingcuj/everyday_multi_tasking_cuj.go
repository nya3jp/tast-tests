	...
	if tabletMode {
		...
		subtest2 = subtest{
			...
			func(ctx context.Context, s *testing.State, i int) error {
				...
				if isPopup {
					menusNode := nodewith.ClassName("MenuItemView")
					if err := ui.WaitUntilExists(menusNode.First())(ctx); err != nil {
						return errors.Wrap(err, "expected to see menu items, but not seen")
					}
					menus, err := ui.NodesInfo(ctx, menusNode)
					if err != nil {
						return errors.Wrap(err, "can't find the menu items")
					}
					targetMenus := make([]uiauto.NodeInfo, 0, len(menus))
