...
func openAndSwitchTabs(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, params *RunParams, resources *runResources) error {
	...
	firstWindowURLList := []string{cuj.Gmail, cuj.GoogleCalendar, cuj.YoutubeMusic, cuj.Hulu, cuj.GoogleNews}
	secondWindowURLList := []string{cuj.GoogleNews, cuj.Cnn, cuj.Wikipedia, cuj.GoogleNews, cuj.Cnn}
	thirdWindowURLList := []string{cuj.GoogleNews, cuj.Cnn, cuj.Wikipedia, cuj.Reddit, cuj.Cnn}
	...
	openBrowserWithTabs := func(urlList []string) error {
		for idx, url := range urlList {
			...
			if params.appName == YoutubeMusicAppName && url == cuj.YoutubeMusic {
