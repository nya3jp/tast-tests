...
func Run(ctx context.Context, s *testing.State, cr *chrome.Chrome, a *arc.ARC, musicPlayer string, tabletMode, openBluetooth bool) {
	const (
	...
	if err = recorder.Run(ctx, func(ctx context.Context) error {
		...
		if _, err := gmail.New(ctx, tconn, kw, d); err != nil {
	...
	for _, app := range []struct {
		...
		{"Calendar", calendarPackageName, func(ctx context.Context) error {
			...
			// Click "next" button until no "next" is found.
			for {
				s.Log("Find and click next button")
			...
			// check and click "Got it" button.
			gotIt := d.Object(ui.Text("Got it"))
			if err := waitAndClickObject(ctx, gotIt, "Got it", timeout); err != nil {
				s.Log(`"Got it" button not found`)
			}

			// Click on "allow" button to allow access of user's contacts.
			allowButton := d.Object(ui.ClassName(AndroidButtonClassName), ui.Text(allowButtonText))
			// Double the timeout value to wait longer.
			if err := allowButton.WaitForExists(ctx, timeout*2); err != nil {
				s.Log("Allow Button doesn't exists: ", err)
				// Assume splash procedure has completed and return.
				return nil
			}
		...
		if err = recorder.Run(ctx, func(ctx context.Context) error {
			if contains(packages, app.packageName) {
				...
				if err := launcher.SearchAndLaunch(tconn, kw, app.query)(ctx); err != nil {
