...
func ChromeCrashLoop(ctx context.Context, s *testing.State) {
	...
	// Ensure success file isn't left over from previous test. Ignoring errors
	// since it doesn't matter if the file was present before or not.
	os.Remove(testModeSuccessfulFile)
	// Clean up at the end, too
	defer os.Remove(testModeSuccessfulFile)

	// Check that we could remove the file.
	if _, err := os.Stat(testModeSuccessfulFile); err == nil {
		s.Error(testModeSuccessfulFile, " still exists")
	} else if !errors.Is(err, os.ErrNotExist) {
		s.Error("Could not stat ", testModeSuccessfulFile, ": ", err)
	}
