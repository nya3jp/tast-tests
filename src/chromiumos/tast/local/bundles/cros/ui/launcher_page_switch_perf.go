...
func LauncherPageSwitchPerf(ctx context.Context, s *testing.State) {
	...
	runner.RunMultiple(ctx, s.Run, "click", perfutil.RunAndWaitAll(tconn, action.Combine(
	...
	runner.RunMultiple(ctx, s.Run, "drag", perfutil.RunAndWaitAll(tconn, action.Combine(
		"launcher page drag",
		// Drag-up operation.
