...
func LauncherPageSwitchPerf(ctx context.Context, s *testing.State) {
	...
	runner.RunMultiple(ctx, "click", uiperf.Run(s.Run, perfutil.RunAndWaitAll(tconn, action.Combine(
	...
	runner.RunMultiple(ctx, "drag", uiperf.Run(s.Run, perfutil.RunAndWaitAll(tconn, action.Combine(
		"launcher page drag",
		// Drag-up operation.
