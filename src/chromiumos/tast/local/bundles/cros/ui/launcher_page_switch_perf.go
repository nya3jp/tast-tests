...
func LauncherPageSwitchPerf(ctx context.Context, s *testing.State) {
	...
	uiperf.RunMultiple(ctx, runner, s.Run, "click", perfutil.RunAndWaitAll(tconn, action.Combine(
	...
	uiperf.RunMultiple(ctx, runner, s.Run, "drag", perfutil.RunAndWaitAll(tconn, action.Combine(
		"launcher page drag",
		// Drag-up operation.
