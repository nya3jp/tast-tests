	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err := conn.Eval(ctx,
			`(function() {
				...
				var b = Array.from(document.querySelectorAll('div')).find(e => e.textContent === "Play").getBoundingClientRect();
		...
		testing.ContextLogf(ctx, "Play button is on top (clickable)? : %s", atTop)
		if !atTop {
