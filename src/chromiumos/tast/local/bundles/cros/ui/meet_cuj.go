...
type meetTest struct {
	...
	present bool           // Whether it is presenting or not.
...
func MeetCUJ(ctx context.Context, s *testing.State) {
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		if err := webview.WaitUntilDescendantExists(ctx, ui.FindParams{Name: "More options", Role: ui.RoleTypeStaticText}, timeout); err != nil {
			s.Fatal("Failed to focus on the More options buttton: ", err)
		}
		if err = kw.Accel(ctx, "Enter"); err != nil {
