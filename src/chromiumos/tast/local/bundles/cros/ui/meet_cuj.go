...
func MeetCUJ(ctx context.Context, s *testing.State) {
	...
	// opened, to prevent visual oddities on some devices.
	// Additionally, on some devices, the display zoom is not
	// enough to show all of the participants, so we also zoom out
	// on the browser.
	...
	if zoomInfo.Name != "75%" {
		s.Fatalf("Failed to zoom window to 75%%, instead zoomed to %s", zoomInfo.Name)
	}
	testing.ContextLog(ctx, "Zoomed browser window to 75%")
