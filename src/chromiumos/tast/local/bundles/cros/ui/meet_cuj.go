...
func reportWebRTCInternals(pv *perf.Values, dump []byte, numBots int, present bool) error {
	...
	for connID, peerConn := range webRTC.PeerConnections {
		...
		// Unlike byDirection["Outbound"], |outboundStreamIDs| will include every outbound stream, even if the
		// stream has none of the statistics in |unit|.
		type void struct{}
		var member void
		outboundStreamIDs := make(map[string]void)
			...
			case "contentType":
				...
				for _, value := range statistic.Values {
					if value != screenshareContentType {
						return errors.Errorf("%s expected all content types for %q to be %q, got %v", errPrefix, fullName, screenshareContentType, statistic.Values)
					}
				}
			case "[codec]":
				if len(statistic.Values) == 0 {
		...
		if isScreenshareConn && (numStreamsWithContentType != expectedNumStreamsWithContentType) {
