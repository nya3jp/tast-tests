...
func reportWebRTCInternals(pv *perf.Values, dump []byte, numBots int, present bool) error {
	...
	for connID, peerConn := range webRTC.PeerConnections {
		...
		totalStreams := len(streamIDs)
		if (isScreenshareConn && (numStreamsWithContentType != totalStreams)) || (!isScreenshareConn && (numStreamsWithContentType != 0)) {
			return errors.Errorf("%s expected %d streams to have the stat contentType, got %d streams", errPrefix, totalStreams, numStreamsWithContentType)
		}
