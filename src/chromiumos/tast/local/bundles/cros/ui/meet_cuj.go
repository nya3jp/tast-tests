...
func MeetCUJ(ctx context.Context, s *testing.State) {
	...
	if !codeOk {
		...
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			botList, numOfFailures, err := bc.AddBots(sctx, meetingCode, addBotsCount, meetTimeout+15*time.Minute, meet.botsOptions...)
			if err != nil {
				return testing.PollBreak(errors.Wrapf(err, "failed to create %d bots", addBotsCount))
			}
			s.Logf("%d bots started, %d bots failed", len(botList), numOfFailures)
