...
func MeetCUJ(ctx context.Context, s *testing.State) {
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		if participantCount != meet.num+1 {
			return errors.Errorf("expected %d participants, got %d", meet.num+1, participantCount)
		}
		testing.ContextLogf(ctx, "%d participants in the meeting")
