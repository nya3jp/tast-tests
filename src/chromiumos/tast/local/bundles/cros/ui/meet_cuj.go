...
func MeetCUJ(ctx context.Context, s *testing.State) {
	const (
		...
		defaultDocsURL = "https://docs.google.com/document/d/1qEq8IkPwjdPOG5MoOkAbtyvgHbBJavEkMoB7Qo0GTxw/edit?usp=sharing"
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		if err := ui.WaitUntilExists(nodewith.Name(shareMessage).Ancestor(webview))(ctx); err == nil {
			...
			if err := uiauto.Combine(
				...
				ui.LeftClick(nodewith.Name("Close").Role(role.Button).Ancestor(webview)),
