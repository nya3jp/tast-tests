...
func MeetCUJ(ctx context.Context, s *testing.State) {
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		defer cancel()
		// Add 5 minutes to the bot duration, to ensure that the bots stay long enough
		// for the test to detect the video codecs used for encoding and decoding.
		if !codeOk {
			if _, err := bc.AddBots(sctx, meetingCode, meet.num, meetTimeout+5*time.Minute, meet.botsOptions...); err != nil {
	...
	if webrtcInternals, err := cs.NewConn(ctx, "chrome://webrtc-internals"); err != nil {
		...
		if err := ui.WithTimeout(time.Minute).WaitUntilExists(nodewith.NameContaining("VideoStream").First())(ctx); err != nil {
