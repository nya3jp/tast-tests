...
// SimulateKeyPress presses down the specified key repeatedly with a delay between each key press.
// This action is repeated until the amount of time specified by the duration parameter has passed.
func SimulateKeyPress(ctx context.Context, kw *input.KeyboardEventWriter, key string, delay, duration time.Duration) error {
