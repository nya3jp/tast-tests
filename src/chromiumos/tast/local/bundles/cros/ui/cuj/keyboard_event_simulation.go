...
// PressKeyFor presses down the specified key repeatedly with a delay between each key press.
// This action is repeated until the amount of time specified by the duration parameter has passed.
func PressKeyFor(ctx context.Context, kw *input.KeyboardEventWriter, key string, delay, duration time.Duration) error {
