...
func NewTPSValidationHelper(ctx context.Context) *TPSValidationHelper {
	// Infinite loop of calculating fibonacci number. This can produce
	// extra CPU/memory load for TPS validation tests.
	script := `a=0; b=1; while true; do echo "$a"; fn=$((a + b)); a=$b; b=$fn; done`
