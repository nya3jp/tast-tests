...
// PreKeepData is the struct returned by the preconditions.
type PreKeepData struct {
	Chrome    *chrome.Chrome
	ARC       *arc.ARC
	LoginTime int64
}

type loginKeepStateImpl struct {
	name            string
...
func (p *loginKeepStateImpl) String() string {
	return p.name
}

func (p *loginKeepStateImpl) Timeout() time.Duration {
	return p.timeout
}

func (p *loginKeepStateImpl) Prepare(ctx context.Context, s *testing.PreState) interface{} {
	...
	if p.cr != nil && p.arc != nil {
		...
		return PreKeepData{Chrome: p.cr, ARC: p.arc}
	}
	...
	func() {
		...
		start := time.Now()
		p.cr, err = chrome.New(ctx, chrome.GAIALogin(),
	...
	return PreKeepData{Chrome: p.cr, ARC: p.arc, LoginTime: p.loginTime}
}

func (p *loginKeepStateImpl) runningPackages(ctx context.Context) (map[string]struct{}, error) {
...
func (p *loginKeepStateImpl) resetState(ctx context.Context) error {
	...
	// Unlike ARC.loginKeepStateImpl, this does not uninstall apps. This is because we
...
func (p *loginKeepStateImpl) Close(ctx context.Context, s *testing.PreState) {
...
func (p *loginKeepStateImpl) closeInternal(ctx context.Context) {
	chrome.Unlock()
...
var loginKeepState = &loginKeepStateImpl{name: "login_keepstate", timeout: chrome.LoginTimeout + optin.OptinTimeout + arc.BootTimeout + 10*time.Second}

// LoginKeepState returns a precondition of chrome logged in to the "CUJ"
// test user and activates ARC.
func LoginKeepState() testing.Precondition {
	return loginKeepState
}
