...
func cpuUtilization(prev, time cpu.TimesStat) float64 {
	...
	if prevBusy > newBusy || prevTotal >= newTotal {
		return 0
	}
...
func NewCPUUsageSource(name string, perCPU bool) *CPUUsageSource {
	...
	return &CPUUsageSource{name: name, perCPU: perCPU, prevStats: map[string]cpu.TimesStat{}, maxFreqReported: map[string]bool{}}
}
...
func (s *CPUUsageSource) Start(ctx context.Context) error {
	times, err := cpu.TimesWithContext(ctx, s.perCPU)
	if err != nil {
		return err
	}
...
func (s *CPUUsageSource) Snapshot(ctx context.Context, values *perf.Values) error {
	...
	if s.perCPU {
		...
		values.Append(perf.Metric{
			...
			Unit:      "percent",
			Direction: perf.SmallerIsBetter,
		}, cpuUtilization(s.prevStats[times[0].CPU], times[0]))
		s.prevStats[times[0].CPU] = times[0]
	}
	return nil
}
