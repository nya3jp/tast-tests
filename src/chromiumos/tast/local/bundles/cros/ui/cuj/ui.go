...
func GetBrowserStartTime(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn,
	...
	bTconn := tconn
	if lFixtVal != nil {
		l, err = lacros.Connect(ctx, lFixtVal.LacrosPath(), lacros.UserDataDir)
		if err != nil {
...
// This is useful for lacros-Chrome.
func KeepNewTab(ctx context.Context, tconn *chrome.TestConn) error {
	...
	// Create a new tab and close all other tabs
	if err := tconn.Eval(ctx, `(async () => {
