...
// It will only look for the icon locate on built-in display.
func appIconFinder(appName string, appOtherPossibleNames ...string) *nodewith.Finder {
	builtInDisplay := nodewith.ClassName("RootWindow-0").Role(role.Window)
	finder := nodewith.ClassName("ash/ShelfAppButton").Ancestor(builtInDisplay)
