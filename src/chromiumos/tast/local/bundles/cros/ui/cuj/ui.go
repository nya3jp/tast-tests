...
func GetBrowserStartTime(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, tabletMode bool) (time.Duration, error) {
	...
	if appName == "Chrome" {
		// Chrome shown as "Google Chrome" in UI.
		appName = "Google Chrome"
	}
	if tabletMode {
		testing.ContextLogf(ctx, "Launch %q from hotseat", appName)
		startTime, launchErr = LaunchAppFromHotseat(ctx, tconn, appName)
	} else {
		testing.ContextLogf(ctx, "Launch %q from shelf", appName)
		startTime, launchErr = LaunchAppFromShelf(ctx, tconn, appName)
	}

	if launchErr != nil {
		testing.ContextLogf(ctx, "Failed to open %q", appName)
		return browserStartTime, errors.Wrapf(launchErr, "failed to open %q", appName)
	}
