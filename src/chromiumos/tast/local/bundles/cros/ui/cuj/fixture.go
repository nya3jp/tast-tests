...
const (
	// CPUCooleDownTimeout is the time to wait for cpu cool down.
	CPUCooleDownTimeout = 10 * time.Minute
	// CPUIdleTimeout is the time to wait for cpu utilization to go down.j
	// It value should match waitIdleCPUTimeout in cpu/idle.go
	...
	CPUStablizationTimeout = CPUCooleDownTimeout + CPUIdleTimeout
...
func CPUCoolDownConfig() cpu.CoolDownConfig {
	...
	cdConfig.PollTimeout = CPUCooleDownTimeout
	return cdConfig
}
