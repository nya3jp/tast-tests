...
func init() {
	...
	testing.AddFixture(&testing.Fixture{
		...
		Impl:            &loggedInToCUJUserFixture{isLacros: true},
		Parent:          "prepareForCUJ",
		SetUpTimeout:    chrome.GAIALoginTimeout + optin.OptinTimeout + arc.BootTimeout + 2*time.Minute,
		ResetTimeout:    resetTimeout,
		TearDownTimeout: resetTimeout,
	...
	testing.AddFixture(&testing.Fixture{
		...
		Impl:            &loggedInToCUJUserFixture{keepState: true, webUITabStrip: true, isLacros: true},
		SetUpTimeout:    chrome.GAIALoginTimeout + optin.OptinTimeout + arc.BootTimeout + 2*time.Minute,
		ResetTimeout:    resetTimeout,
		TearDownTimeout: resetTimeout,
		Vars: []string{
...
type loggedInToCUJUserFixture struct {
	...
	webUITabStrip bool
	// isLacros describes whether this fixture should enable lacros.
	isLacros bool
}
...
func (f *loggedInToCUJUserFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	func() {
		...
		var err error
		if f.isLacros {
			opts, err = lacrosfixt.NewConfigFromState(s, lacrosfixt.Mode(lacros.LacrosPrimary),
