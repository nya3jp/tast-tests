...
func NewScreenRecorder(ctx context.Context, tconn *chrome.TestConn) (*ScreenRecorder, error) {
	expr := `(function() { window.mediaRecorder = null; window.stream = null;})()`
	...
	// Choose to record the entire desktop/screen with audio.
	...
	audioButton, err := mediaview.DescendantWithTimeout(ctx, ui.FindParams{Name: "Share audio", Role: ui.RoleTypeCheckBox}, timeout)
	if err != nil {
		return nil, errors.Wrap(err, "failed to find the audio checkbox")
	}
	defer audioButton.Release(ctx)
	if err := audioButton.FocusAndWait(ctx, timeout); err != nil {
		return nil, errors.Wrap(err, "failed to focus on the audio checkbox")
	}
	if err := audioButton.LeftClick(ctx); err != nil {
		return nil, errors.Wrap(err, "failed to click the audio checkbox")
	}
...
// Save saves the last stored encoded string into a file 'fileName' under
