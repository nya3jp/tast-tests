...
func NewScreenRecorder(ctx context.Context, tconn *chrome.TestConn) (*ScreenRecorder, error) {
	expr := `(function() {
		let videoRecorder = {
			...
			streamPromise: null,
			request: function() {
				videoRecorder.streamPromise = navigator.mediaDevices.getDisplayMedia({
					audio: false,
					video: {
						cursor: "always"
					}
			...
			start: function() {
				...
				Promise.resolve(videoRecorder.streamPromise).then(
					stream => {
						videoRecorder.recorder = new MediaRecorder(stream, {mimeType: 'video/webm;codecs=vp9'});
						videoRecorder.recorder.ondataavailable = function(e) {
							videoRecorder.chunks.push(e.data);
						};
						videoRecorder.recorder.start();
					}
				);
			},
			stop: function() {
				return new Promise((resolve, reject) => {
					videoRecorder.recorder.onstop = function() {
...
func (r *ScreenRecorder) Stop(ctx context.Context, tconn *chrome.TestConn) error {
	...
	var result string
	if err := r.videoRecorder.Call(ctx, &result, `async function() {var result = await this.stop(); return result;}`); err != nil {
...
// the directory /tmp/tast/results/latest/tests/[testname]/
func (r *ScreenRecorder) Save(ctx context.Context, tconn *chrome.TestConn, filepath string) error {
	if err := ioutil.WriteFile(filepath, []byte(r.result), 0644); err != nil {
		return errors.Wrapf(err, "failed to dump encoded string to %s", filepath)
	}
	return nil
}
