...
func SimulateMouseMovement(ctx context.Context, tconn *chrome.TestConn, duration time.Duration) error {
	...
	for endTime := time.Now().Add(duration); time.Now().Before(endTime); {
		// Move mouse to the center of the screen.
		if err := mouse.Move(tconn, center, 0)(ctx); err != nil {
			return errors.Wrap(err, "failed to move mouse")
		}

		// Start doing a spiral
		spiralEndTime := time.Now().Add(singleSpiralDuration)
		spiralStartTime := time.Now()
		for time.Now().Before(spiralEndTime) && time.Now().Before(endTime) {
			timeSinceSpiralStart := time.Now().Sub(spiralStartTime)
