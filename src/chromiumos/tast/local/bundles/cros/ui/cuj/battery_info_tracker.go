	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				if err != nil {
					t.collectingErr = errors.Wrap(err, "failed to read system power")
					return
				}
				tNew := time.Now()
...
func (t *BatteryInfoTracker) Stop(ctx context.Context) error {
	...
	// Stop engery collecting go routine.
	close(t.collecting)
	if t.collectingErr != nil {
		return t.collectingErr
	}
