...
func NewRecorder(configs ...MetricConfig) (*Recorder, error) {
	...
	for _, config := range configs {
		...
		r.configs[config.HistogramName] = config
		if len(config.JankCriteria) != 2 {
			switch config.Category {
			case CategorySmoothness:
				copy(r.configs[config.HistogramName].JankCriteria, smoothnessJankCriteria)
			case CategoryLatency:
				copy(r.configs[config.HistogramName].JankCriteria, latencyJankCriteria)
			default:
