...
func (r *Recorder) Run(ctx context.Context, f func(ctx context.Context) error) (err error) {
	...
	if r.traceDir != "" {
		if err := r.cr.StartTracing(ctx,
			...
			err = errors.Wrap(err, "failed to start tracing")
		}
		defer func() {
			tr, err := r.cr.StopTracing(closeCtx)
			if err != nil {
				err = errors.Wrap(err, "failed to stop tracing")
				return
			}
			if tr == nil || len(tr.Packet) == 0 {
				err = errors.New("no trace data is collected")
				return
			}
			filename := "trace.data.gz"
			if err := chrome.SaveTraceToFile(closeCtx, tr, filepath.Join(r.traceDir, filename)); err != nil {
				err = errors.Wrap(err, "failed to save trace to file")
				return
			}
			return
		}()
