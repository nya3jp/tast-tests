...
// RunUntil conducts the test scenario f repeatedly until the total
// duration of metrics collection (including any that was already done
// before this call to RunUntil) meets or exceeds a given minimum.
func (r *Recorder) RunUntil(ctx context.Context, f func(ctx context.Context) error, minimumTotalDuration time.Duration) error {
