...
//   function.
// * Cleanup function that should be called to stop tracing and clean up after
//   start.
// * Error
func (r *Recorder) StartRecording(ctx context.Context) (runCtx context.Context, cleanup func() error, e error) {
	if !r.startedAtTm.IsZero() {
		return nil, nil, errors.New("start requested on the started recorder")
	}
	if len(r.mr) != 0 {
		return nil, nil, errors.New("start requested but there are already metric recorders initialized")
	}
	...
	defer func() {
		...
		err := cancel()
		if e == nil && err != nil {
			e = errors.Wrap(err, "failed to cleanup after StartRecording")
		}
	}()
	...
	if r.traceDir != "" {
		if err := r.cr.StartTracing(ctx,
			...
			return nil, nil, errors.Wrap(err, "failed to start tracing")
		}
	...
	for tconn, names := range r.names {
		...
		if err != nil {
			return nil, nil, errors.Wrap(err, "failed to start metrics recorder")
		}
	}
	...
	// Cancel local cleanup and pass cleanup to the caller.
	cleanup = cancel
	cancel = nil
...
//
// In:
// * context returned from the StartRecording()
// * cleanup function returned from the StartRecording()
//
// Out:
// * Error
func (r *Recorder) StopRecording(runCtx context.Context, cleanup func() error) (e error) {
	defer func() {
		err := cleanup()
		if e == nil && err != nil {
	...
	for tconn, r := range r.mr {
		h, err := r.Histogram(runCtx, tconn)
		if err != nil {
...
// different runners to accomodate them.
//
// This functino also serves as an example for test developers on how to
...
func (r *Recorder) Run(ctx context.Context, f func(ctx context.Context) error) (e error) {
	...
	var err error
	if runCtx, cleanup, err = r.StartRecording(ctx); err != nil {
		return err
	}
	defer func() {
		if cleanup == nil {
		...
		err := cleanup()
		if e == nil && err != nil {
			e = errors.Wrap(err, "failed to cleanup in Run()")
		}
	}()
	if err := f(runCtx); err != nil {
		return err
	}
	// Stop Recording will take care of the cleanup.
	passedCleanup := cleanup
	cleanup = nil
	if err := r.StopRecording(runCtx, passedCleanup); err != nil {
		return err
	}
	return nil
}
