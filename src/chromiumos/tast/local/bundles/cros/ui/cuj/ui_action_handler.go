...
// It shows the hot seat, clicks the app icon to to show the window menu, and then select one of the windows based on
// the given menuItemFinder. If menuItemFinder is nil, which is used when there is only one window for the app, it
// will just tap the app icon to do the switch.
func (t *TabletActionHandler) switchToWindowThroughHotseat(ctx context.Context, appName string, menuItemFinder *nodewith.Finder) error {
	...
	waitMenuItemView := t.ui.WithTimeout(10 * time.Second).WaitUntilExists(nodewith.ClassName("MenuItemView").First())
	...
	if err := testing.Poll(ctx, func(c context.Context) error {
		return uiauto.Combine("click app icon and show MenuItemView", clickIcon, waitMenuItemView)(ctx)
