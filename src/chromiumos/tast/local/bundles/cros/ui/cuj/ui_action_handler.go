...
func NewTabletActionHandler(ctx context.Context, tconn *chrome.TestConn) (*TabletActionHandler, error) {
	...
	tew := tc.TouchscreenEventWriter()
	stew, err := tew.NewSingleTouchWriter()
	if err != nil {
		return nil, errors.Wrap(err, "failed to create SingleTouchWriter")
	}
...
func (t *TabletActionHandler) ScrollChromePage() []func(ctx context.Context, conn *chrome.Conn) error {
	var (
		x      = t.displayBounds.Width / 2
		ystart = t.displayBounds.Height / 4 * 3       // 75% of screen height
		yend   = t.displayBounds.Height / 4           // 25% of screen height
