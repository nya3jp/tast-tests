...
func NearbyShareMojoSend(ctx context.Context, s *testing.State) {
	const (
		...
		phoneName = "SENDER-9C041FQC200408"
	)
	...
	if err := nearbyshare.SetNearbySettings(
		ctx, s, nsSettingsConn, "RECEIVER-172.16.243.132:hatch", 1, 2); err != nil {
		s.Fatal("Failed to set Nearby settings: ", err)
	}

	// Setup the test image.
	const (
		previewImageFile = "image.jpg"
	)
	...
	nearbyConn, err := cr.NewConn(ctx, "chrome://nearby/share?file=/home/chronos/user/MyFiles/Downloads/image.jpg")
	if err != nil {
	...
	// Manually receive at Android. Wait until detected.
	s.Log("Start sending now")
	...
	if err := nearbyDiscoveryTester.Call(ctx, &confirmSuccess, `function() {return this.confirmationManager.value != null}`); err != nil {
