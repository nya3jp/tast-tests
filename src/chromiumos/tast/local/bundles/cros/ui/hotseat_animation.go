...
func dragFromBottomOfScreenToShowOverview(ctx context.Context, tsw *input.TouchscreenEventWriter, stw *input.SingleTouchEventWriter, s *testing.State) error {
	startX := tsw.Width() / 2
	startY := tsw.Height() - 1
	...
	// Wait with the swipe paused so the overview mode gesture is recognized.
	const overviewSwipePauseDuration = time.Second
	if err := testing.Sleep(ctx, overviewSwipePauseDuration); err != nil {
...
func HotseatAnimation(ctx context.Context, s *testing.State) {
	...
	histograms, err := metrics.Run(ctx, cr, func() error {
		...
		if err := dragFromBottomOfScreenToShowOverview(ctx, tsw, stw, s); err != nil {
