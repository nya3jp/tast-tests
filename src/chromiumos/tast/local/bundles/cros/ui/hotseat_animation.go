...
func HotseatAnimation(ctx context.Context, s *testing.State) {
	...
	uiperf.Run(ctx, runner, s.Run, "WindowCreation", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	uiperf.Run(ctx, runner, s.Run, "", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
