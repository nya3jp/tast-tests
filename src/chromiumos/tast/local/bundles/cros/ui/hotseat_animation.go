...
func HotseatAnimation(ctx context.Context, s *testing.State) {
	...
	runner.RunMultiple(ctx, uiperf.Run(s.Run), "WindowCreation", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	runner.RunMultiple(ctx, uiperf.Run(s.Run), "", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
