...
func HotseatAnimation(ctx context.Context, s *testing.State) {
	...
	runner.RunMultiple(ctx, perfutil.SubtestRun(s.Run), "WindowCreation", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	runner.RunMultiple(ctx, perfutil.SubtestRun(s.Run), "", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
