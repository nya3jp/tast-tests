...
func showOverview(ctx context.Context, tsw *input.TouchscreenEventWriter, stw *input.SingleTouchEventWriter, c *chrome.Conn) error {
	startX := tsw.Width() / 2
	startY := tsw.Height() - 1
	...
	if err := ash.WaitForOverviewToFinishAnimating(ctx, c); err != nil {
