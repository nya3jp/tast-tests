...
type animationTest struct {
	waitForCPU bool // Whether to wait a constant time, or wait for the cpu to idle.
}
...
func wait(ctx context.Context, s *testing.State) error {
	shouldwaitForCPUIdle := s.Param().(animationTest).waitForCPU
	if shouldwaitForCPUIdle {
...
func HotseatAnimation(ctx context.Context, s *testing.State) {
	...
	histograms, err := metrics.Run(ctx, cr, func() error {
		...
		// After overview has triggered, wait a bit for things to calm down.
		wait(ctx, s)
		...
		// After opening the launcher, wait for things to calm down.
		wait(ctx, s)
		...
		pressX = input.TouchCoord(tsw.Width() / 3)
		pressY = input.TouchCoord(tsw.Height() / 3)
		...
		// After going to overview, wait a bit to calm down.
		wait(ctx, s)
		...
		// After going to home launcher, wait for things to calm down
		wait(ctx, s)
		return nil
	},
