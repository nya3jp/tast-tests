...
func createAndMinmizeAWindow(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome) error {
	const errorMsg = "failed to create and minmize a window"
...
// HotseatAnimation mesures the performance of hotseat background bounds animation.
func HotseatAnimation(ctx context.Context, s *testing.State) {
	...
	if err := createAndMinmizeAWindow(ctx, tconn, cr); err != nil {
