...
type overflowShelfTest struct {
	enableOverflowShelf bool // Whether overflow shelf should be tested.
}
	...
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			Val: overflowShelfTest{
				enableOverflowShelf: false,
			},
		}, {
			Name: "overflow_shelf",
			Val: overflowShelfTest{
				enableOverflowShelf: true,
			},
		}},
	})
}
...
func HotseatAnimation(ctx context.Context, s *testing.State) {
	...
	overflowShelf := s.Param().(overflowShelfTest)
	if overflowShelf.enableOverflowShelf {
		if err := ash.EnterOverflowShelf(ctx, tconn); err != nil {
			return errors.Wrap(err, "failed to enter overflow shelf")
		}
	}
