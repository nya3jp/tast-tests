...
func enterHomeLauncherFromOverview(ctx context.Context, tconn *chrome.TestConn, stw *input.SingleTouchEventWriter, tsw *input.TouchscreenEventWriter) error {
...
func enterInAppModeFromOverview(ctx context.Context, tconn *chrome.TestConn, stw *input.SingleTouchEventWriter, tsw *input.TouchscreenEventWriter) error {
	const errorMsg = "failed to enter in-app mode from overview"
...
func enterHomeLauncherFromInAppMode(ctx context.Context, tconn *chrome.TestConn, stw *input.SingleTouchEventWriter, tsw *input.TouchscreenEventWriter) error {
	const errorMsg = "failed to enter home launcher from in-app mode"
...
func HotseatAnimation(ctx context.Context, s *testing.State) {
	const perfTestErrorMsg = "Failed to collect data for hotseat bounds animation: "

	cr := s.PreValue().(*chrome.Chrome)

	tconn, err := cr.TestAPIConn(ctx)
	if err != nil {
		s.Fatal(perfTestErrorMsg, err)
	}
	...
	histogramGroup1, err := metrics.Run(ctx, tconn, func() error {
		const errorMsg = "failed to collect metrics data from hiding hotseat by window creation"
	...
	histogramGroup2, err := metrics.Run(ctx, tconn, func() error {
		const errorMsg = "failed to collect metrics data from entering/exiting overview"
	...
	histogramGroup3, err := metrics.Run(ctx, tconn, func() error {
		const errorMsg = "failed to collect metrics data from hiding hotseat by window activation"
	...
	for _, h := range append(append(histogramGroup1, histogramGroup2...), histogramGroup3...) {
		mean, err := h.Mean()
		if err != nil {
			s.Fatalf("%sFailed to get mean for histogram %s: %v", perfTestErrorMsg, h.Name, err)
		}
	...
	if err := pv.Save(s.OutDir()); err != nil {
		s.Fatalf("%sFailed saving perf data: %v", perfTestErrorMsg, err)
	}
}
