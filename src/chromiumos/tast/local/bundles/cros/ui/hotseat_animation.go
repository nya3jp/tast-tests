...
func dragFromBottomOfScreenToShowOverview(ctx context.Context, tsw *TouchscreenEventWriter, stw *SingleTouchEventWriter, s *testing.State) {
	startX := tsw.Width() / 2
	startY := tsw.Height() - 1
...
func HotseatAnimation(ctx context.Context, s *testing.State) {
	...
	histograms, err := metrics.Run(ctx, cr, func() error {
		for i := 1; i < 10; i++ {
			...
			dragFromBottomOfScreenToShowOverview(tsw, stw, ctx, s)
