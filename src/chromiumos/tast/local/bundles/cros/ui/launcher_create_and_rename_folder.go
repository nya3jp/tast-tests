...
func LauncherCreateAndRenameFolder(ctx context.Context, s *testing.State) {
	...
	if err := icons[0].FocusAndWait(ctx, 2*time.Second); err != nil {
		s.Fatalf("Failed to focus on icon %q: %v", icons[0].Name, err)
	}
...
// We need this function because  existing function pointer.Drag does not hold the icon long
// enough to create the effect of creating new folder.
