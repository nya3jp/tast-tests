...
func LauncherCreateAndRenameFolder(ctx context.Context, s *testing.State) {
	...
	// On the other hand, clamshell mode's home screen does not have any applications icons and useres
	// have to expand the launder to see applications icons.
	// Therefore, the following code wait for the icons to go away when changing from tablet mode
	// to clamshell mode.
...
func dragIcon(ctx context.Context, tconn *chrome.TestConn, srcIcon, targetIcon *ui.Node, tabletMode bool) error {
	...
	// The following statement makes sure the icon was move to the top of another one
	...
	// When an icon is dragged and dropped another icon, the two icons will
