...
func openAndCloseLauncher(ctx context.Context, tconn *chrome.TestConn,
	kb *input.KeyboardEventWriter, ui *uiauto.Context) error {
...
func BubbleLauncherAnimationPerf(ctx context.Context, s *testing.State) {
	...
	runner.RunMultiple(ctx, s, name,
		perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
			return openAndCloseLauncher(ctx, tconn, kb, ui)
