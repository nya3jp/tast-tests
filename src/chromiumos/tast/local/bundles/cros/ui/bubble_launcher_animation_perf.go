...
func BubbleLauncherAnimationPerf(ctx context.Context, s *testing.State) {
	...
	opts = append(opts, chrome.EnableFeatures("ProductivityLauncher", "ProductivityLauncherAnimation", "LauncherAppSort"))
	if bt == browser.TypeLacros {
		// Lacros should keep alive to be able to get the browser object without opening any window for this test.
		lacrosOpts, err := lacrosfixt.NewConfig(lacrosfixt.KeepAlive(true)).Opts()
		if err != nil {
	...
	// Connect to the running browser process to get the browser instance (br) and pass it over to perfutil for starting/stoping tracing.
	...
	if err != nil {
		s.Fatal("Failed to open a new browser: ", err)
	}
	defer conn.Close()
