...
// Gmail provides controls of Gmail application
type Gmail struct {
}
...
// New creates a Gmail instance and open Gmail application at start.
// If the application be executed first time, it would skip all of initialization steps.
func New(ctx context.Context, tconn *chrome.TestConn, d *ui.Device, tabletMode bool) (*Gmail, error) {
	gmail := &Gmail{}
	if err := apps.LaunchBySearch(ctx, tconn, gmailAppName, gmailPackageName, tabletMode); err != nil {
...
// Send sends an email through Gmail application to specific receiver.
func (*Gmail) Send(ctx context.Context, d *ui.Device, receiver, subject, content string) error {
	const (
