...
func ChromePIPRoundedCornersUnderlay(ctx context.Context, s *testing.State) {
	...
	// We want to check that if the PIP video is promoted to
	// overlay, then it uses the underlay overlay strategy. It may
	// take a moment to be promoted to overlay, and we should wait
	// to see what overlay strategy is used then. On the other
	// hand, there is no guarantee that it will be promoted to
	// overlay. Also, sometimes the "SingleOnTop" overlay strategy
	// is used on a few frames, but then after a moment, the
	// "Underlay" overlay strategy is used consistently. Because
	// of the unpredictability, we cannot detect when a steady
	// state has been reached. We just have to wait some time.
	if err := testing.Sleep(ctx, 5*time.Second); err != nil {
		s.Fatal("Failed to wait 5 seconds: ", err)
	}
