...
func UnlockPerf(ctx context.Context, s *testing.State) {
	...
	for _, windows := range []int{2, 8} {
		conns, err := ash.CreateWindows(ctx, tconn, cr, url, windows-currentWindows)
		if err != nil {
		...
		for _, inTabletMode := range []bool{false, true} {
			...
			r.RunMultiple(ctx, s, fmt.Sprintf("%s-%dwindows", suffix, currentWindows), perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
				...
				if st, err := lockscreen.WaitState(ctx, tconn, func(st lockscreen.State) bool { return !st.Locked || st.ReadyForPassword }, badAuthTimeout); err != nil {
