...
func LoginPerf(ctx context.Context, s *testing.State) {
	...
	cujPerfValuesList := perf.NewValuesList()
	...
	for _, windows := range []int{2, 8} {
		for _, arcMode := range arcmodes {
			...
			for _, inTabletMode := range []bool{false, true} {
				...
				cujRunPerfValuesList := perf.NewValuesList()
				r.RunMultiple(ctx, s,
					...
					func(ctx context.Context) ([]*metrics.Histogram, error) {
						...
						for _, metricConfig := range [][]cujrecorder.MetricConfig{cujrecorder.AshCommonMetricConfigs(), cujrecorder.BrowserCommonMetricConfigs(), cujrecorder.AnyChromeCommonMetricConfigs()} {
							if err := cujRecorder.AddCollectedMetrics(tLoginConn, metricConfig...); err != nil {
						...
						cujRunPerfValuesList.AppendWithSuffix(fmt.Sprintf("%s.%s.%dwindows", suffix, arcMode, currentWindows), pv)
				...
				cujPerfValuesList.AppendValuesList(cujRunPerfValuesList)
			}
		}
	}
	...
	if err := cujPerfValuesList.Save(s.OutDir()); err != nil {
