...
func loginPerfDoLogin(ctx context.Context, cr *chrome.Chrome, credentials chrome.Creds, browserType browser.Type) (l *lacros.Lacros, retErr error) {
	...
	if browserType == browser.TypeLacros {
		...
		// lacros.Connect() fails if DevTools connection file was already created, but Chrome does not accept connections. Retry for some time.
		for i:=0; i < 100; i++ {
			l, err = lacros.Connect(ctx, tconn)
			if err != nil {
				err = errors.Wrap(err, "failed to connec to lacros-chrome")
			} else {
				break
			}
			testing.ContextLogf(ctx, "loginPerfDoLogin: Sleeping for 10 milliseconds... ")
...
func loginPerfCreateWindows(ctx context.Context, cr *chrome.Chrome, l *lacros.Lacros, url string, n int) error {
	...
	if l != nil {
		for i := 0; i < n; i++ {
			testing.ContextLogf(ctx, "=> l.NewConn(...)")
		...
		testing.ContextLogf(ctx, "=> ash.CreateWindows(...)")
...
func countVisibleWindows(ctx context.Context, cr *chrome.Chrome) (int, error) {
	...
	testing.ContextLogf(ctx, "countVisibleWindows: connecting to Ash browser")
...
func initializeLoginPerfTest(ctx context.Context, s *testing.State, browserType browser.Type, lacrosConfig *lacrosfixt.Config) (chrome.Creds, error) {
	...
	if arc.Supported() {
		...
		s.Log("Initialize: let session fully initialize. Sleeping for 4 seconds... ")
	...
	for _, windows := range []int{2, 8} {
		for _, arcMode := range arcmodes {
			...
			for _, inTabletMode := range []bool{false/*, true*/} {
				...
				allHistograms := []string{ensureWorkVisibleHistogram, ensureWorkVisibleLowResHistogram, allBrowserWindowsCreated, allBrowserWindowsShown, allBrowserWindowsPresented}
				...
				r.RunMultiple(ctx, s,
					...
					func(ctx context.Context) ([]*metrics.Histogram, error) {
						...
						cujFunc := func(ctx context.Context) error {
							...
							if visible != currentWindows && visible != currentWindows+1 {
								...
								testing.ContextLogf(ctx, "#################3 CUJ FUNCTION: Sleeping for 300 seconds... ")
								testing.Sleep(ctx, 3000*time.Second)
							}
					...
					func(ctx context.Context, pv *perfutil.Values, hists []*metrics.Histogram) error {
						...
						for _, hist := range hists {
							...
							} else if hist.Name == ensureWorkVisibleHistogram || hist.Name == ensureWorkVisibleLowResHistogram || hist.Name == allBrowserWindowsCreated || hist.Name == allBrowserWindowsShown || hist.Name == allBrowserWindowsPresented{
