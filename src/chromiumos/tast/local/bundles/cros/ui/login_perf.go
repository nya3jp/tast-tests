	...
	for _, windows := range []int{2, 8} {
		for _, arcMode := range arcmodes {
			...
			for _, inTabletMode := range []bool{false, true} {
				...
				allHistograms := []string{ensureWorkVisibleHistogram, ensureWorkVisibleLowResHistogram, allBrowserWindowsCreated, allBrowserWindowsShown, allBrowserWindowsPresented, allShelfIconsLoaded, ShelfLoginAnimationEnd}
				allHistograms = append(allHistograms, heuristicsHistograms...)

				s.Logf("Starting test: '%dwindows%s'", currentWindows, suffix)
				r.RunMultiple(ctx, s,
					...
					func(ctx context.Context, pv *perfutil.Values, hists []*metrics.Histogram) error {
						...
						for _, hist := range hists {
							...
							} else if hist.Name == ensureWorkVisibleHistogram || hist.Name == ensureWorkVisibleLowResHistogram || hist.Name == allBrowserWindowsCreated || hist.Name == allBrowserWindowsShown || hist.Name == allBrowserWindowsPresented || hist.Name == allShelfIconsLoaded || hist.Name == ShelfLoginAnimationEnd {
