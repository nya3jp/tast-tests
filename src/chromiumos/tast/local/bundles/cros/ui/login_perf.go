...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{{
			...
			// Only run on a subset of devices since it downloads
			// from omaha and it will not use our lab's caching
			// mechanisms. We don't want to overload our lab.
			ExtraHardwareDeps: hwdep.D(hwdep.Model("kled", "enguarde", "samus", "sparky", "phaser")),
			Val: loginPerfTestParam{
...
func loginPerfCreateWindows(ctx context.Context, cr *chrome.Chrome, l *lacros.Lacros, url string, n int) error {
	if l != nil {
		for i := 0; i < n; i++ {
			...
			if err != nil {
				return errors.Wrapf(err, "(%d) failed to connect to the %v restore URL", i, url)
			}
			defer conn.Close()
		}
	} else {
...
func initializeLoginPerfTest(ctx context.Context, browserType browser.Type, lacrosConfig *lacrosfixt.Config, loginPool string) (chrome.Creds, error) {
	...
	if arc.Supported() {
		testing.ContextLog(ctx, "Initialize: let session fully initialize. Sleeping for 40 seconds... ")
		if err := testing.Sleep(ctx, 40*time.Second); err != nil {
				...
				r.RunMultiple(ctx, testName,
					...
					func(ctx context.Context, pv *perfutil.Values, hists []*metrics.Histogram) error {
						...
						for _, hist := range hists {
							...
							switch {
							case heuristicsHistogramsMap[hist.Name]:
								storeHeuristicsHistograms(ctx, pv, []*metrics.Histogram{hist})
							case hist.Name == ensureWorkVisibleHistogram:
								reportMaxHistogramValue(ctx, pv, hist, "microsecond", valueName)
							case hist.Name == ensureWorkVisibleLowResHistogram:
								reportMaxHistogramValue(ctx, pv, hist, "millisecond", valueName)
							case hist.Name == allBrowserWindowsCreated:
								reportMaxHistogramValue(ctx, pv, hist, "millisecond", valueName)
							case hist.Name == allBrowserWindowsShown:
								reportMaxHistogramValue(ctx, pv, hist, "millisecond", valueName)
							case hist.Name == allBrowserWindowsPresented:
								reportMaxHistogramValue(ctx, pv, hist, "millisecond", valueName)
							case hist.Name == allShelfIconsLoaded:
								reportMaxHistogramValue(ctx, pv, hist, "millisecond", valueName)
							case hist.Name == shelfLoginAnimationEnd:
								reportMaxHistogramValue(ctx, pv, hist, "millisecond", valueName)
							case hist.Name == ashTastBootTimeLogin2:
								reportMaxHistogramValue(ctx, pv, hist, "millisecond", valueName)
							default:
