...
func VP9VideoDecoder(ctx context.Context, s *testing.State) {
	...
	if err := cui.RightClick(fullScreenButton)(ctx); err != nil {
		s.Fatal("Failed to right click: ", err)
	}

	statsForNerdsItem := nodewith.Name("Stats for nerds").Role(role.MenuItem)
	if err := cui.WaitUntilExists(statsForNerdsItem)(ctx); err != nil {
		s.Fatal("Failed to wait Stats for nerds element: ", err)
	}

	if err := cui.LeftClick(statsForNerdsItem)(ctx); err != nil {
		s.Fatal("Failed to left click Stats for nerds element: ", err)
	}

	codecsVp9Text := nodewith.NameStartingWith(" vp09").Role(role.StaticText)
	if err := cui.WaitUntilExists(codecsVp9Text)(ctx); err != nil {
		s.Fatal("Failed to wait for VP9 codecs element: ", err)
	}

	selectedResolutionText := nodewith.NameStartingWith(" 3840x2160@60").Role(role.StaticText)
	if err := cui.WaitUntilExists(selectedResolutionText)(ctx); err != nil {
		s.Fatal("Failed to wait for selected resolution 2160p60: ", err)
	}
}
