...
func QuickCheckCUJ(ctx context.Context, s *testing.State) {
	const (
		...
		readingDurationTop           = 10 * time.Second
		readingDurationMiddle        = 25 * time.Second
		readingDurationBottom        = 5 * time.Second
		scrollingDurationTopToMiddle = 3 * time.Second
		scrollingDurationMiddleToEnd = 2 * time.Second
	)
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		if err := testing.Sleep(ctx, readingDurationTop); err != nil {
		...
		if err := mouse.Drag(tconn, scrollThumbDragStart, scrollThumbDragMiddle, scrollingDurationTopToMiddle)(ctx); err != nil {
		...
		if err := testing.Sleep(ctx, readingDurationMiddle); err != nil {
			return errors.Wrap(err, "failed to sleep (middle scroll position)")
		}

		s.Log("Scrolling the email thread (middle to bottom)")
		if err := mouse.Drag(tconn, scrollThumbDragMiddle, scrollThumbDragEnd, scrollingDurationMiddleToEnd)(ctx); err != nil {
		...
		if err := testing.Sleep(ctx, readingDurationBottom); err != nil {
