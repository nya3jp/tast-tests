...
func QuickCheckCUJ(ctx context.Context, s *testing.State) {
	...
	cs, tconn, chromeCleanup, err := func(ctx context.Context) (ash.ConnSource, *chrome.TestConn, cleanupFunc, error) {
		...
		if ct == lacros.ChromeTypeChromeOS {
			...
			if err != nil {
				return nil, nil, nil, errors.Wrap(err, "failed to initialize test")
			}
