...
func DesktopControl(ctx context.Context, s *testing.State) {
	...
	r.RunMultiple(ctx, "launcher", uiperf.Run(s.Run, perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	r.RunMultiple(ctx, "quick-settings", uiperf.Run(s.Run, perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
