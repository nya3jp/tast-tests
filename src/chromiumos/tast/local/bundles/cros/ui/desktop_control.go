...
func DesktopControl(ctx context.Context, s *testing.State) {
	...
	r.RunMultiple(ctx, perfutil.SubtestRun(s.Run), "launcher", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	r.RunMultiple(ctx, perfutil.SubtestRun(s.Run), "quick-settings", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
