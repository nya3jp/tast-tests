...
func DesktopControl(ctx context.Context, s *testing.State) {
	...
	r.RunMultiple(ctx, s.Run, "launcher", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	r.RunMultiple(ctx, s.Run, "quick-settings", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
