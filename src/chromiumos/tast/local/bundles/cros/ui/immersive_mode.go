...
func ImmersiveMode(ctx context.Context, s *testing.State) {
	...
	if len(ws) != 1 {
		s.Fatalf("Expected 1 window, got %d window", len(ws))
	}
	if ws[0].State != ash.WindowStateFullscreen {
	...
	// Swipe up from the buttom edge to the center.
	tc, err := touch.New(ctx, tconn)
	if err != nil {
		s.Fatal("Failed to get the touch context: ", err)
	}
	screenButtomPt := info.Bounds.BottomCenter().Sub(coords.Point{0, 1})
	screenCenterPt := info.Bounds.CenterPoint()
	if err := uiauto.Combine(
		"swipe up from the buttom edge, and check if the launcher appears",
		tc.Swipe(screenButtomPt, tc.SwipeTo(screenCenterPt, time.Second)),
