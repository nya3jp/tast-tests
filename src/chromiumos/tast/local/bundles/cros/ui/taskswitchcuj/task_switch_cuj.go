...
func Run(ctx context.Context, s *testing.State) {
	...
	defer browser.CloseAllTabs(ctx, bTconn)
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		if testParam.Tablet {
			...
			subtests = append(subtests, *switchTaskByHotseat)
		} else {
			subtests = append(subtests, initializeSwitchTaskByAltTab(ctx, kw, numWindows))
		}

		for _, subtest := range subtests {
			s.Log(subtest.description)
			cycles := 0
			for endTime := time.Now().Add(subtestDuration); time.Now().Before(endTime); {
				if err := subtest.run(ctx); err != nil {
				...
				// of time to keep consistency in task switches between different
				// devices.
			...
			// successfully finish. Continue to the next subtest if
