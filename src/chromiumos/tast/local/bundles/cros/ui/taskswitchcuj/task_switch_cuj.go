...
func Run(ctx context.Context, s *testing.State) {
	...
	if tabletMode {
		...
		// Tablet mode overview scrolls horizontally, limiting the number of windows
		// visible on the screen at a given time. Thus, after showing overview mode,
		// horizontally scroll the window to the right, so the least recently used
		...
		openAppList = kw.AccelAction("search")
	}
	defer pc.Close()
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		if numAppWindows, err = launchPackages(ctx, tconn, kw, ac, packages, openAppList); err != nil {
		...
		for _, subtest := range subtests {
			...
			for endTime := time.Now().Add(subtestDuration); time.Now().Before(endTime); {
				if err := action.Combine(
					...
					// Try to scroll down and up by pressing the down and up arrow key.
					// This gives us some input latency metrics while delaying between
					// each task switch. This also helps increase memory pressure, because
