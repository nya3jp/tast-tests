...
// 5. Google Slides -- large slide deck for RAM pressure.
...
func openChromeTabs(ctx context.Context, tconn, bTconn *chrome.TestConn, cs ash.ConnSource, isLacros, tabletMode bool, pv *perf.Values) (func(ctx context.Context) error, func(ctx context.Context) error, int, error) {
	...
	if manyTabs, err = cuj.NewTabs(ctx, cs, false, numExtraWebsites); err != nil {
	...
	if taskSwitchTabs, err = cuj.NewTabsByURLs(ctx, cs, true, simpleWebsites); err != nil {
	...
	if speedometerTab, err = cuj.NewTabByURL(ctx, cs, true, "https://browserbench.org/Speedometer2.0/"); err != nil {
