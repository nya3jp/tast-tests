...
// launchPackages launches each package in |packages|. This returns the
// number of packages that were opened.
func launchPackages(ctx context.Context, tconn *chrome.TestConn, kw *input.KeyboardEventWriter, ac *uiauto.Context, packages []packageInfo) (int, error) {
	...
	for _, pkg := range packages {
		...
		testing.ContextLog(ctx, "Skipping the splash screen for ", pkg.query)
		...
		if err := ac.WithInterval(2*time.Second).WaitUntilNoEvent(nodewith.Root(), event.LocationChanged)(ctx); err != nil {
			return 0, errors.Wrap(err, "failed to wait for windows to stabilize")
		}
	}
