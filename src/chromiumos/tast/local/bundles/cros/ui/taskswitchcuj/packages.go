...
// packageInfo represents information needed to launch and open an ARC
// application.
...
func getPackages(ctx context.Context, tconn *chrome.TestConn, d *ui.Device) []packageInfo {
	return []packageInfo{
		...
		{name: playStorePackageName, query: "play store app", skipSplash: func(ctx context.Context) error {
			return nil
		}},
	}
}

// installPackages installs all packages in |packages|.
...
// launchPackages launches all packages in |packages| by calling
// |openAppList|, searching for the package name, and clicking
// on the first result.
func launchPackages(ctx context.Context, tconn *chrome.TestConn, kw *input.KeyboardEventWriter, ac *uiauto.Context, packages []packageInfo, openAppList func(ctx context.Context) error) (int, error) {
	// Determine how many windows are present before launching packages.
	var initialNumWindows int
	var ws []*ash.Window
	var err error
	...
	for _, pkg := range packages {
		...
		if err := action.Combine(
			...
			kw.TypeAction(pkg.query),
			// Assumes the first search result tile is the app. Not searching
			// by name because launcher could append a suffix to the app
			// name. E.g. Gmail app has "Gmail, Installed app" as its name. The
			// suffix is for chromevox and could change.
			ac.WithTimeout(15*time.Second).LeftClick(nodewith.HasClass("SearchResultView").First()),
...
func skipGmailSplash(ctx context.Context, tconn *chrome.TestConn, d *ui.Device) error {
	// actionTimeout is how long we should wait for each individual
	// action taken to skip the splash screen.
