...
func ResizeWindow(ctx context.Context, s *testing.State) {
	...
	for _, app := range appList {
		f := func(ctx context.Context, s *testing.State) {
			...
			for i := 0; i < int(windowBoundEnd); i++ {
				if err := testUtil.resizeWindow(app.getWindowFinder(), windowBound(i))(ctx); err != nil {
...
func (t *resizeWindowUtil) resizeWindow(f *nodewith.Finder, dragBound windowBound) uiauto.Action {
	return func(ctx context.Context) error {
		...
		start, end := t.dragDetail(dragBound, windowInfoBefore)
...
func (t *resizeWindowUtil) dragDetail(dragBound windowBound, nodeToResize *uiauto.NodeInfo) (sourcePt, endPt coords.Point) {
