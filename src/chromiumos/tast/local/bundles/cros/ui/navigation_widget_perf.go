...
func NavigationWidgetPerf(ctx context.Context, s *testing.State) {
	...
	// Histograms can only be recorded in tablet mode.
	...
	// Fetch histograms from activating a window to hide UI components. It usually brings better animation smoothness data than window creation.
	histograms, err = metrics.Run(ctx, tconn, func() error {
