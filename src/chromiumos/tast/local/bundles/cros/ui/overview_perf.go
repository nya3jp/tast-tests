...
func init() {
	testing.AddTest(&testing.Test{
		...
		// This test uses contents from external sources, including
		// blob.html and its dependencies (.js files), which are under
		// Google copyright. For license information, please see
		// README.txt in blob.zip.
...
func OverviewPerf(ctx context.Context, s *testing.State) {
	...
	tempDir, err := ioutil.TempDir("", "blob")
	if err != nil {
	...
	if err := perfutil.Unzip(s.DataPath("blob/blob.zip"), tempDir); err != nil {
		s.Fatal("Failed to decompress test contents: ", err)
	}
