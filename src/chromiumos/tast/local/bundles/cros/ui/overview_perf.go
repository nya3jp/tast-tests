...
func OverviewPerf(ctx context.Context, s *testing.State) {
	...
	for i, windows := range []int{2, 3, 4, 8} {
		...
		for _, test := range []struct {
			...
			if err := doTestCase(
				ctx, s, tconn, runner, fullDescription, windowsDescription, test.tablet, test.overviewWindowState, false, /*splitview*/
		...
		if windows == 2 {
			if err := doTestCase(
				ctx, s, tconn, runner, "SplitView-2windows", windowsDescription, true /*tablet*/, ash.WindowStateMaximized, true, /*splitview*/
				...
				s.Fatalf("Test case \"SplitView-2windows\" failed: %s", err)
			}
			continue
		}
		...
		for _, test := range []struct {
			...
			if err := doTestCase(
				ctx, s, tconn, runner, fullDescription, windowsDescription, true /*tablet*/, test.overviewWindowState, true, /*splitview*/
				"Ash.Overview.AnimationSmoothness.Enter.SplitView",
