...
func OverviewPerf(ctx context.Context, s *testing.State) {
	...
	// isBlank returns true if the window title suggests a Chrome
	// browser window displaying about:blank. Some test cases snap
	// such a window to measure overview smoothness in split view.
	...
	for _, overviewWindows := range []int{2, 8} {
		for _, test := range []struct {
			...
			// animationWindows. If not, then this will generate:
			...
			runner.RunMultiple(ctx, s, fmt.Sprintf("%s-%s", test.histogramSuffix, extraDescription), perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
