...
func SettingsToggleAdvanced(ctx context.Context, s *testing.State) {
	...
	// We need to call focus() on the button to get it to come into view so it will be clickable.
	// The focus event does not complete instantly, so we use an EventWatcher to make sure focus()
	// has finished.
	ew, err := ui.NewWatcher(ctx, advBtn, ui.EventTypeFocus)
	if err != nil {
		s.Fatal("Failed to create focus event watcher: ", err)
	}
	defer ew.Release(ctx)

	if err := advBtn.Focus(ctx); err != nil {
		s.Fatal("Failed to call focus() on the Advanced button: ", err)
	}

	if _, err := ew.WaitForEvent(ctx, 5*time.Second); err != nil {
		s.Fatal("Failed to wait for the focus event on the Advanced button: ", err)
	}
