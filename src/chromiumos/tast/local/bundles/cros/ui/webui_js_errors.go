...
func checkJavaScriptError(ctx, cleanupCtx context.Context, crashDir, outDir, expectedErrorMessage string,
	...
	if !strings.Contains(string(metaContents), expectedErrorMessagePrefix+expectedErrorMessage) {
		...
		return errors.Errorf("didn't find expected meta var: `%s`. Leaving for debugging: %s", expectedErrorMessagePrefix+expectedErrorMessage, metas[0])
	}
...
func checkLoggedError(ctx, cleanupCtx context.Context, conn *chrome.Conn, crashDir, outDir string) error {
	...
	if err := conn.DispatchKeyEvent(ctx, &keyEventArgs); err != nil {
		return errors.Wrap(err, "could not dispatch key event")
	}
...
func checkUncaughtExceptionError(ctx, cleanupCtx context.Context, conn *chrome.Conn, crashDir, outDir string) error {
...
func checkUnhandledPromiseRejectionError(ctx, cleanupCtx context.Context, conn *chrome.Conn, crashDir, outDir string) error {
...
func WebUIJSErrors(ctx context.Context, s *testing.State) {
	...
	if err := checkLoggedError(ctx, cleanupCtx, conn, crashDir, s.OutDir()); err != nil {
		s.Error("checkLoggedError failed: ", err)
	}
	if err := checkUncaughtExceptionError(ctx, cleanupCtx, conn, crashDir, s.OutDir()); err != nil {
		s.Error("checkUncaughtExceptionError failed: ", err)
	}
	if err := checkUnhandledPromiseRejectionError(ctx, cleanupCtx, conn, crashDir, s.OutDir()); err != nil {
