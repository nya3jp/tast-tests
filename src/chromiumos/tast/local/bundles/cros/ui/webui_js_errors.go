...
func checkJavaScriptError(ctx context.Context, crashDir, outDir, expectedErrorMessage string,
	...
	defer crash.RemoveAllFiles(ctx, crashFileMap)
	...
	if !strings.Contains(string(metaContents), expectedErrorMessage) {
		crash.MoveFilesToOut(ctx, outDir, metas[0])
		return errors.Errorf("didn't find expected meta var: `%s`. Leaving for debugging: %s", expectedErrorMessage, metas[0])
	}
	if !strings.Contains(string(metaContents), expectedType) {
		crash.MoveFilesToOut(ctx, outDir, metas[0])
		return errors.Errorf("didn't find expected meta var: `%s`. Leaving for debugging: %s", expectedType, metas[0])
	}
	expectedPayload := expectedPayloadPrefix + filepath.Base(stacks[0])
	if !strings.Contains(string(metaContents), expectedPayload) {
		crash.MoveFilesToOut(ctx, outDir, metas[0])
		return errors.Errorf("didn't find expected meta payload: `%s`. Leaving for debugging: %s", expectedPayload, metas[0])
	}
	expectedLogUpload := expectedLogPrefix + filepath.Base(logs[0])
	if !strings.Contains(string(metaContents), expectedLogUpload) {
		crash.MoveFilesToOut(ctx, outDir, metas[0])
		return errors.Errorf("didn't find expected meta log upload: `%s`. Leaving for debugging: %s", expectedLogUpload, metas[0])
	}
	...
	for _, expectedStackEntry := range expectedStackEntries {
		...
		if location == -1 {
			...
			return errors.Errorf("didn't find expected text in stack: %s. Leaving for debugging: %s", expectedStackEntry, stacks[0])
		}
		if location < previousLocation {
			return errors.Errorf("stack is out of order; %s too early. Leaving for debugging: %s", expectedStackEntry, stacks[0])
		}
...
func checkPageLoadError(ctx context.Context, crashDir, outDir string) error {
	...
	const (
		...
		expectedErrorMessage = "upload_var_error_message=WebUI JS Error: printing error on page load"
	)
	if err := checkJavaScriptError(ctx, crashDir, outDir, expectedErrorMessage, []string{expectedFunction1, expectedFunction2}); err != nil {
		return errors.Wrap(err, "page load error")
	}

	return nil
}

func checkLoggedError(ctx context.Context, conn *chrome.Conn, crashDir, outDir string) error {
	...
	modifiers := 1 // Alt
	unmodifiedText := "l"
	keyEventArgs := input.DispatchKeyEventArgs{Type: "char", Modifiers: &modifiers, UnmodifiedText: &unmodifiedText}
	conn.DispatchKeyEvent(ctx, &keyEventArgs)

	const (
		expectedFunction1    = "logsErrorFromButtonClickInner"
		expectedFunction2    = "logsErrorFromButtonClickHandler"
		expectedErrorMessage = "upload_var_error_message=WebUI JS Error: printing error on button click"
	)

	if err := checkJavaScriptError(ctx, crashDir, outDir, expectedErrorMessage, []string{expectedFunction1, expectedFunction2}); err != nil {
		return errors.Wrap(err, "logged error")
	}

	return nil
}

func checkUncaughtExceptionError(ctx context.Context, conn *chrome.Conn, crashDir, outDir string) error {
	...
	const (
		...
		expectedErrorMessage = "upload_var_error_message=Uncaught Error: WebUI JS Error: exception button clicked"
	)

	if err := checkJavaScriptError(ctx, crashDir, outDir, expectedErrorMessage, []string{expectedFunction1, expectedFunction2}); err != nil {
		return errors.Wrap(err, "uncaught exception")
	}

	return nil
}

func checkUnhandledPromiseRejectionError(ctx context.Context, conn *chrome.Conn, crashDir, outDir string) error {
	...
	const (
		...
		expectedErrorMessage = "upload_var_error_message=Uncaught (in promise) WebUI JS Error: The rejector always rejects!"
	)

	if err := checkJavaScriptError(ctx, crashDir, outDir, expectedErrorMessage, []string{expectedStack}); err != nil {
		return errors.Wrap(err, "uncaught exception")
	}
...
func WebUIJSErrors(ctx context.Context, s *testing.State) {
	...
	if err := checkPageLoadError(ctx, crashDir, s.OutDir()); err != nil {
		s.Error("checkPageLoadError failed: ", err)
	}
	if err := checkLoggedError(ctx, conn, crashDir, s.OutDir()); err != nil {
		s.Error("checkLoggedError failed: ", err)
	}
	if err := checkUncaughtExceptionError(ctx, conn, crashDir, s.OutDir()); err != nil {
		s.Error("checkUncaughtExceptionError failed: ", err)
	}
	if err := checkUnhandledPromiseRejectionError(ctx, conn, crashDir, s.OutDir()); err != nil {
