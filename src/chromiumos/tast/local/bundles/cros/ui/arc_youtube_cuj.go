...
func ArcYoutubeCUJ(ctx context.Context, s *testing.State) {
	...
	if err := recorder.Run(ctx, func(ctx context.Context) error {
		...
		// Wait for the ARC YouTube app to idle, so that we know it is ready for our next step.
		...
		// Put EPlRnpgY2bY in the search box, because we want: https://www.youtube.com/watch?v=EPlRnpgY2bY
		if err := d.Object(
			ui.Text("Search YouTube"),
			ui.ClassName("android.widget.EditText"),
			ui.PackageName("com.google.android.youtube"),
		).SetText(ctx, "EPlRnpgY2bY"); err != nil {
		...
		if err := d.Object(
			...
			ui.DescriptionMatches("Top 100 Free Stock Videos 4K Rview and Download in Pixabay 12 2018 - 41 minutes - Go to channel - WORLD NATURE .+ - play video"),
			ui.PackageName("com.google.android.youtube"),
		).Click(ctx); err != nil {
