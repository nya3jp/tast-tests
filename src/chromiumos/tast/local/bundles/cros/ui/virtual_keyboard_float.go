...
func VirtualKeyboardFloat(ctx context.Context, s *testing.State) {
	...
	err = vkb.SwitchToFloatLayout(ctx, kconn)
	if err != nil {
	...
	if !newDragPoint.Equals(destinationPoint) {
		s.Errorf("Failed to drag float VK. Expected position: %s. Actual position: %s", destinationPoint.String(), newDragPoint.String())
	}
	...
	// New position after resizing can be not precisely verified. Simply check new drag point moves in the direction.
	if resizeTopLeftHandler.X <= newResizeTopLeftHandler.X || resizeTopLeftHandler.Y <= newResizeTopLeftHandler.Y {
		s.Errorf("Failed to resize float VK. Top left resize handle old position: %s. New position: %s", resizeTopLeftHandler.String(), newResizeTopLeftHandler.String())
	}
}
