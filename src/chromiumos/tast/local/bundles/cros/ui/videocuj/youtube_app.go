...
func openAndPlayYoutubeApp(ctx context.Context, s *testing.State, tconn *chrome.TestConn, a *arc.ARC, d *androidui.Device, video Video, index int) (*arc.Activity, error) {
	const (
	...
	switchQuality := func(resolution string) error {
		...
		if err := testing.Poll(ctx, func(context.Context) error {
			...
			if err := moreBtn.WaitForExists(ctx, uiWaitTime); err != nil {
...
func enterYoutubeAppFullscreen(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, d *androidui.Device) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err := fsBtn.WaitForExists(ctx, uiWaitTime); err != nil {
