...
func Run(ctx context.Context, s *testing.State, cr *chrome.Chrome, a *arc.ARC, appName string, tabletMode bool, tier cuj.Tier, extendedDisplay bool) {
	...
	// The playback settings is bound with netflix profile.
	playbackSettings := "Basic"

	if tier == cuj.Plus {
		videoSrc = plusVideoSrc
		playbackSettings = "Plus"
	}
	...
	runScenario := func(ctx context.Context, index int) {
		if err = recorder.Run(ctx, func(ctx context.Context) error {
			...
			switch appName {
			case YoutubeWeb:
				videoApp = NewYtWeb(cr, tconn, kb, videoSrc[index], extendedDisplay, ui, uiHandler)
				repeatTimes = len(videoSrc)
			case YoutubeApp:
				videoApp = NewYtApp(tconn, kb, a, d, videoSrc[index], index)
				repeatTimes = len(videoSrc)
			case NetflixWeb:
				nfUsername := s.RequiredVar("ui.netflix_username")
				nfPassword := s.RequiredVar("ui.netflix_password")

				videoApp = NewNtWeb(cr, tconn, kb, playbackSettings, extendedDisplay, ui, uiHandler, nfUsername, nfPassword)
			}
...
func clearNotificationPrompts(ctx context.Context, ui *uiauto.Context, uiHdl cuj.UIActionHandler, prompts ...string) {
	for _, name := range prompts {
		...
		if err := uiauto.Combine("click '"+name+"' prompt",
			ui.WithTimeout(3*time.Second).WaitUntilExists(tartgetPrompt),
			// Immediately clicking the target button sometimes doesn't work. Sleep 2 seconds.
			ui.Sleep(2*time.Second),
			uiHdl.Click(tartgetPrompt),
		)(ctx); err != nil {
			testing.ContextLog(ctx, "There's no '"+name+"' prompt")
		}
	}
}
