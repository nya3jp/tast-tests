...
func (y *YtWeb) OpenAndPlayVideo(ctx context.Context) (err error) {
	...
	if err := uiauto.Combine("click 'Skip Ads' button",
		y.ui.WithTimeout(10*time.Second).WaitForLocation(adsButton),
	...
	switchQuality := func(resolution string) error {
		...
		coords, err := y.ui.Location(ctx, settings)
		if err != nil {
			return err
		}
		...
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			if err := uiauto.Combine("show the setting panel and click it",
				...
				y.ui.WithTimeout(time.Second).WaitUntilExists(settings),
				y.ui.WaitForLocation(settings),
...
func clearNotificationPrompts(ctx context.Context, ui *uiauto.Context, uiHdl cuj.UIActionHandler, prompts ...string) {
	for _, name := range prompts {
		...
		ui.WithTimeout(15*time.Second).IfSuccessThen(
