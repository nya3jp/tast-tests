...
func (y *YtWeb) PerformFrameDropsTest(ctx context.Context) error {
	...
	if err := y.ytConn.EvalPromiseDeprecated(ctx, `document.querySelector('#movie_player video').webkitDecodedFrameCount`, &decodedFrameCount); err != nil {
		return errors.Wrap(err, "failed to get decoded framecount")
	}
	if err := y.ytConn.EvalPromiseDeprecated(ctx, `document.querySelector('#movie_player video').webkitDroppedFrameCount`, &droppedFrameCount); err != nil {
