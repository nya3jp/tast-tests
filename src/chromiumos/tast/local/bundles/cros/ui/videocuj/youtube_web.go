...
func (y *YtWeb) OpenAndPlayVideo(ctx context.Context) (err error) {
	...
	if y.extendedDisplay {
		infos, err := display.GetInfo(ctx, y.tconn)
		if err != nil {
			return errors.Wrap(err, "failed to get display info")
		}
		if len(infos) != 2 {
			return errors.Errorf("want 2 displays, got %d", len(infos))
		}
		// Root window on extended display.
		targetWin = nodewith.ClassName("RootWindow-1").Role(role.Window)
		if err := y.ui.Exists(nodewith.Name("YouTube Video Player").Ancestor(targetWin))(ctx); err != nil {
