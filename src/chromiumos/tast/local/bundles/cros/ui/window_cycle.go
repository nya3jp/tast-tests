...
func WindowCycle(ctx context.Context, s *testing.State) {
	...
	for _, direction := range []string{"forward", "backward"} {
		...
		for i := 0; i < 4; i++ {
			func() {
				// Ensure the cycle window is closed.
				cycleWindowParams := ui.FindParams{ClassName: "WindowCycleList (Alt+Tab)"}
				if err := ui.WaitUntilGone(ctx, tconn, cycleWindowParams, 5*time.Second); err != nil {
					s.Fatal("Alt+Tab cycle menu is still open: ", err)
				}
				...
				cycleWindow, err := ui.FindWithTimeout(ctx, tconn, cycleWindowParams, 5*time.Second)
				if err != nil {
