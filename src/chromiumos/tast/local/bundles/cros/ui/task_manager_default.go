...
// TaskManagerDefault verifies "Task Manager" default UI and the basic functionality of "End process" button.
func TaskManagerDefault(ctx context.Context, s *testing.State) {
...
func (f *terminateProcess) verify(ctx context.Context) error {
	...
	url := ""
	if tab, ok := p.(*taskmanager.ChromeTab); ok {
		url = tab.URL
	} else {
	...
	expr := `async () => {
			const tabs = await tast.promisify(chrome.tabs.query)({url: "%s", status: "%s"});
			if (tabs.length !== 1) {
			  throw new Error("unexpected number of tabs: got " + tabs.length)
			}
		  }`

	// There are 3 types of TabStatus: "unloaded", "loading" and "complete",
	// see: https://developer.chrome.com/docs/extensions/reference/tabs/#type-TabStatus
	var (
		tabComplete = fmt.Sprintf(expr, url, "complete")
		tabUnloaded = fmt.Sprintf(expr, url, "unloaded")
	)

	if err := f.tconn.Call(ctx, nil, tabComplete); err != nil {
		return errors.Wrap(err, "failed to ensure the tab status is complete")
	}
	...
	if err := f.tconn.Call(ctx, nil, tabUnloaded); err != nil {
		return errors.Wrap(err, "failed to verify the tab is crashed")
	}
