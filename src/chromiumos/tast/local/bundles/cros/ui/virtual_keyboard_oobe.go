...
func VirtualKeyboardOOBE(ctx context.Context, s *testing.State) {
	...
	if err = insertFieldWithVK(ctx, kconn, gaiaConn, "input", []string{"t", "e", "s", "t", "@", "g", "m", "a", "i", "l", ".", "c", "o", "m"}, "test@gmail.com"); err != nil {
		s.Fatal("Failed to input identifierId with vk in user login: ", err)
	}
}

func insertFieldWithVK(ctx context.Context, kconn, gaiaConn *chrome.Conn, cssSelector string, keys []string, expectedValue string) error {
	...
	getViewPortSize := func() (coords.Size, error) {
		var vpSize coords.Size
		if err := gaiaConn.Eval(ctx, `(function() {
	...
	fullScreenSize, err := getViewPortSize()
	if err != nil {
	...
	testing.ContextLog(ctx, "touch window size: ", touchWidth, touchHeight)
	...
	// For unknown reason, single touch does not trigger virtual keyboard. It requires double touch to show virtual keyboard.
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		newViewPort, err := getViewPortSize()
