...
func init() {
	testing.AddTest(&testing.Test{
		Func:         TCPBS2PCMark,
		Desc:         "Execute Android app PCMark to do benchmark and acquire that score",
...
// TCPBS2PCMark execute Android app PCMark to do benchmark and acquire that score
func TCPBS2PCMark(ctx context.Context, s *testing.State) {
	const (
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		// check app finished launch by check button existance
	...
	if !runBtnAvailable && installBtnAvaiable {
		...
		if err := testing.Poll(ctx, func(context.Context) error {
			// RUN button will shown after the installation finished
