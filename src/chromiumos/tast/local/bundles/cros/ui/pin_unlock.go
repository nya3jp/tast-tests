...
func PINUnlock(ctx context.Context, s *testing.State) {
	...
	if s.Param().(bool) {
		extraArgs = chrome.ExtraArgs("--enable-features=QuickUnlockPinAutosubmit")
	}

	cr, err := chrome.New(ctx, chrome.Auth(username, password, gaiaID), extraArgs)
	if err != nil {
	...
	if err := ossettings.EnablePINUnlock(ctx, settingsConn, password, PIN, s.Param().(bool)); err != nil {
