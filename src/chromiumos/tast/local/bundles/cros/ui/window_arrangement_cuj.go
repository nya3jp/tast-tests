...
func WindowArrangementCUJ(ctx context.Context, s *testing.State) {
	...
	if !tabletMode {
		...
		f = func(ctx context.Context) error {
			...
			tabList, err := ui.FindWithTimeout(ctx, tconn, ui.FindParams{Role: ui.RoleTypeTabList, ClassName: "TabStrip"}, timeout)
			if err != nil {
				return errors.Wrap(err, "failed to find the tab list")
			}
			defer tabList.Release(ctx)
			tabs, err := tabList.Children(ctx)
			if err != nil {
				return errors.Wrap(err, "failed to find children nodes of the tab list")
			}
			defer tabs.Release(ctx)
			if len(tabs) != 3 {
				return errors.Errorf("failed to get the second tab, expected 2 tabs, got %s", len(tabs)-1)
			}
			tab2 := tabs[1]
			testing.ContextLog(ctx, "Snapping the second tab to the right")
