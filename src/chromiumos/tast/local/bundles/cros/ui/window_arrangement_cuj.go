...
func WindowArrangementCUJ(ctx context.Context, s *testing.State) {
	...
	if !tabletMode {
		...
		f = func(ctx context.Context) error {
			...
			tabList, err := ui.FindWithTimeout(ctx, tconn, ui.FindParams{Role: ui.RoleTypeTabList}, timeout)
			if err != nil {
				return errors.Wrap(err, "failed to find the tab list")
			}
			defer tabList.Release(ctx)
			tabListSlice, err := tabList.Children(ctx)
			if err != nil {
				return errors.Wrap(err, "failed to find children nodes of tab list")
			}
			defer tabListSlice.Release(ctx)
			if len(tabListSlice) < 1 {
				return errors.New("failed to find tab list container because tab list doesn't have any children")
			}
			tabListContainer := tabListSlice[0]
			tabs, err := tabListContainer.Children(ctx)
			if err != nil {
				return errors.Wrap(err, "failed to find children nodes of tab list container")
			}
			defer tabs.Release(ctx)
			if len(tabs) != 2 {
				return errors.Errorf("failed to get the second tab, expected 2 tabs, got %s", len(tabs))
			}
			tab2 := tabs[1]
			testing.ContextLog(ctx, "Snapping the second tab to the right")
