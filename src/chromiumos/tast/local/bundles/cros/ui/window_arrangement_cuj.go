...
func WindowArrangementCUJ(ctx context.Context, s *testing.State) {
	...
	connNoPiP, err := cr.NewConn(ctx,
		"https://www.youtube.com/watch?v=yn3HwLBphW8&absolute_experiments="+
			s.RequiredVar("ui.ytExperiments"))
	if err != nil {
		s.Fatal("Failed to open the first youtube tab: ", err)
	}
	defer connNoPiP.Close()
	connPiP, err := cr.NewConn(ctx,
		"https://www.youtube.com/watch?v=yn3HwLBphW8&absolute_experiments="+
			s.RequiredVar("ui.ytExperiments"))
	if err != nil {
	...
	if !tabletMode {
		...
		f = func(ctx context.Context) error {
			...
			// Split view resizing. This is just dragging the divider between two windows, it doesn't work to
			// collect Ash.SplitViewResize.PresentationTime.ClamshellMode.SingleWindow.
			// if err := mouse.Move(ctx, tconn, splitViewDragPoints[0], 0); err != nil {
			// 	return errors.Wrap(err, "failed to move mouse to the divider")
			// }
			// divider, err := ui.FindWithTimeout(ctx, tconn, ui.FindParams{Role: ui.RoleTypeWindow, ClassName: "MultiWindowResizeController"}, timeout)
			// if err != nil {
			// 	return errors.Wrap(err, "failed to find the divider")
			// }
			// // Drag divider
			// // if err := testing.Sleep(ctx, 2*time.Second); err != nil {
			// // 	return errors.Wrap(err, "failed to wait for location-change events to be completed")
			// // }
			// if err := pc.Press(ctx, divider.Location.CenterPoint()); err != nil {
			// 	return errors.Wrap(err, "failed to start divider drag")
			// }
			// testing.ContextLog(ctx, "Dragging the divider")
			// if err := pc.Move(ctx, divider.Location.CenterPoint(), splitViewDragPoints[1], duration); err != nil {
			...
			// }
			// if err := pc.Move(ctx, splitViewDragPoints[2], divider.Location.CenterPoint(), duration); err != nil {
