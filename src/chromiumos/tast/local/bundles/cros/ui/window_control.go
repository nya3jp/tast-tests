...
func WindowControl(ctx context.Context, s *testing.State) {
	...
	r.RunMultiple(ctx, "window-state", uiperf.Run(s.Run, perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	r.RunMultiple(ctx, "drag-maximized-window", uiperf.Run(s.Run, perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	defer kw.Close()
	r.RunMultiple(ctx, "alt-tab", uiperf.Run(s.Run, perfutil.RunAndWaitAll(tconn, func(ctx context.Context) (err error) {
		pressed := false
		defer func() {
			if pressed {
	...
	r.RunMultiple(ctx, "overview", uiperf.Run(s.Run, perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	r.RunMultiple(ctx, "resize", uiperf.Run(s.Run, perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
		var w *ash.Window
