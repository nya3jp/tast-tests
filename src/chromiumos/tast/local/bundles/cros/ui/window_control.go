...
func WindowControl(ctx context.Context, s *testing.State) {
	expects := map[string]float64{
		// Skipping Ash.Window.AnimationSmoothness.Snap: https://crbug.com/1023586
		...
		"Ash.Overview.AnimationSmoothness.Enter.SingleClamshellMode": 30,
		"Ash.Overview.AnimationSmoothness.Exit.SingleClamshellMode":  30,
	}
	...
	// Test: state transition. Minimize -> Unminimize -> Maximize -> Unmaximize.
	...
	states = append(states, ash.WindowStateLeftSnapped, ash.WindowStateNormal, ash.WindowStateRightSnapped, states[1])
	wmTypes := map[ash.WindowStateType]ash.WMEventType{
		...
		ash.WindowStateLeftSnapped:  ash.WMEventSnapLeft,
		ash.WindowStateRightSnapped: ash.WMEventSnapRight,
	}
	r.RunMultiple(ctx, s, "window-state", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
		for i, newState := range states {
			if gotState, err := ash.SetWindowState(ctx, tconn, ws[0].ID, wmTypes[newState]); err != nil {
	...
	for _, err := range r.Values().Check(ctx, expects) {
