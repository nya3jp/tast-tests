...
func WindowControl(ctx context.Context, s *testing.State) {
	...
	uiperf.Run(ctx, r, s.Run, "window-state", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	uiperf.Run(ctx, r, s.Run, "drag-maximized-window", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	defer kw.Close()
	uiperf.Run(ctx, r, s.Run, "alt-tab", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) (err error) {
		pressed := false
		defer func() {
			if pressed {
	...
	uiperf.Run(ctx, r, s.Run, "overview", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	uiperf.Run(ctx, r, s.Run, "resize", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
		var w *ash.Window
