...
func WindowControl(ctx context.Context, s *testing.State) {
	...
	r.RunMultiple(ctx, s.Run, "window-state", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	r.RunMultiple(ctx, s.Run, "drag-maximized-window", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	defer kw.Close()
	r.RunMultiple(ctx, s.Run, "alt-tab", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) (err error) {
		pressed := false
		defer func() {
			if pressed {
	...
	r.RunMultiple(ctx, s.Run, "overview", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	r.RunMultiple(ctx, s.Run, "resize", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
		var w *ash.Window
