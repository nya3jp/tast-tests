...
const (
	// uiStartStopTimeout is the maximum amount of time we'd expect to wait for
	// session manager to stop and restart.
	uiStartStopTimeout = 80 * time.Second
	...
	chromeCrashEarlyCleanupTimeout = uiStartStopTimeout
...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Checks that if Chrome crashes before crashpad is initialized",
		Contacts:     []string{"iby@chromium.org", "cros-telemetry@google.com"},
		Attr:         []string{"group:mainline", "informational"},
		SoftwareDeps: []string{"chrome", "crashpad"},
		Timeout:      uiStartStopTimeout + chromeCrashEarlyCleanupTimeout + time.Minute,
	})
}
...
func ChromeCrashEarly(ctx context.Context, s *testing.State) {
	...
	defer func() {
		if err = upstart.StopJob(cleanupCtx, "ui"); err != nil {
			s.Log("Failed to stop ui job during cleanup: ", err)
		}
		if err = upstart.EnsureJobRunning(cleanupCtx, "ui"); err != nil {
			s.Log("Failed to restart ui job during cleanup: ", err)
		}
	}()
	...
	// /home/chronos/crash/.
	if files, err := crash.WaitForCrashFiles(ctx, []string{crash.LocalCrashDir}, regexes, crash.MetaString(earlyCrashSignature)); err != nil {
