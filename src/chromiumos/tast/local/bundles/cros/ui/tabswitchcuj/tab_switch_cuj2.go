...
func (tab *chromeTab) searchElementWithPatternAndClick(ctx context.Context, pattern string) error {
	...
	// 500 ms is refer a short pause but enough for allow CDP return and object release.
	jsClickAfterTimeout := `setTimeout(() => { ele.click(); }, 500);`
	...
	if tab.link.webName == googleNews {
		...
		// Which the target content is a <div> with certain class name,
		// but apparently the class name is share with other elements, and there's not other filed to use to match them.
		// For the list of elements with certain class name,
		// we found that at the tail of this list is actually visible and it is what we're looking for,
		// therefore, here use last one and the second last one as the target element.
		pos := 1 // the last one
		...
		script = fmt.Sprintf(`() => {
			...
			%s
			return true;
		}`, pos, jsClickAfterTimeout)
	} else {
		// The link might combine with parameter or token, therefore, better find the element by
		// matching with pattern (CSS selector, not regular expression).
		script = fmt.Sprintf(`(pattern) => {
	...
	// Redirection is not instantly, use poll to detect is redirected.
...
func (tab *chromeTab) clickAnchor(ctx context.Context, timeout time.Duration) error {
	...
	// The web page might still loading at this point, this could cause fail to find a anchor,
	// therefore, use poll here to ensure the anchor is found and click (no need to wait for entire page is loaded)
	ts := time.Now()
	if err := testing.Poll(ctx, func(ctx context.Context) error {
...
func Run2(ctx context.Context, s *testing.State, cr *chrome.Chrome, caseLevel Level, isTablet bool) {
	...
	// To make debug easier if something goes worng, take screen shot before tabs are closed on failure
