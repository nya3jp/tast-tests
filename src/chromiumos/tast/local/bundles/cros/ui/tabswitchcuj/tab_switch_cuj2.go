...
// webPageInfo records a Chrome page's informations, including the current browsing page
// and url links (in patterns) for page rediretion.
...
func generatePageInfo(level Level, web webType, url string, patterns []string) *webPageInfo {
	if len(patterns) < 2 {
...
func (tab *chromeTab) searchElementWithPatternAndClick(ctx context.Context, pattern string) error {
	...
	// regular expression).
	//
	// TODO(crbug.com/1193417): remove setTimeout()
	...
	testing.ContextLogf(ctx, "HTML element clicked [%s], page is redirect to: %q", pattern, tab.pageInfo.url)
...
func (tab *chromeTab) clickAnchor(ctx context.Context, timeout time.Duration) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		if err := tab.searchElementWithPatternAndClick(ctx, pattern); err != nil {
			testing.ContextLogf(ctx, "Click anchor failed, retry, elapsed: %s, error: %v", time.Since(ts), err)
...
// chromeWindow corredponding to a window of Chrome browser,
// it holds multiple chromeTab within this window
type chromeWindow struct {
	tabs []*chromeTab
}

// generateTabSwitchTargets sets all web targets according to the input Level.
func generateTabSwitchTargets(caseLevel Level) []*chromeWindow {
	allTargest := []*webPageInfo{

		generatePageInfo(Basic, wikipedia, "https://en.wikipedia.org/wiki/Main_Page", []string{`/Main_Page`, `/Wikipedia:Contents`}),
		generatePageInfo(Basic, wikipedia, "https://en.wikipedia.org/wiki/Portal:Current_events", []string{`/Portal:Current_events`, `/Special:Random`}),
		generatePageInfo(Basic, wikipedia, "https://en.wikipedia.org/wiki/Wikipedia:About", []string{`/Wikipedia:About`, `/Wikipedia:Contact_us`}),
		generatePageInfo(Plus, wikipedia, "https://en.wikipedia.org/wiki/Help:Contents", []string{`/Help:Contents`, `/Help:Introduction`}),
		generatePageInfo(Plus, wikipedia, "https://en.wikipedia.org/wiki/Wikipedia:Community_portal", []string{`/Wikipedia:Community_portal`, `/Special:RecentChanges`}),
		generatePageInfo(Premium, wikipedia, "https://en.wikipedia.org/wiki/COVID-19_pandemic", []string{`/COVID-19_pandemic`, `/Coronavirus_disease_2019`}),

		generatePageInfo(Basic, reddit, "https://www.reddit.com/r/wallstreetbets", []string{`/r/wallstreetbets/hot/`, `/r/wallstreetbets/new/`}),
		generatePageInfo(Basic, reddit, "https://www.reddit.com/r/technews", []string{`/r/technews/hot/`, `/r/technews/new/`}),
		generatePageInfo(Basic, reddit, "https://www.reddit.com/r/olympics", []string{`/r/olympics/hot/`, `/r/olympics/new/`}),
		generatePageInfo(Plus, reddit, "https://www.reddit.com/r/programming", []string{`/r/programming/hot/`, `/r/programming/new/`}),
		generatePageInfo(Plus, reddit, "https://www.reddit.com/r/apple", []string{`/r/apple/hot/`, `/r/apple/new/`}),
		generatePageInfo(Premium, reddit, "https://www.reddit.com/r/brooklynninenine", []string{`/r/brooklynninenine/hot/`, `/r/brooklynninenine/new/`}),

		generatePageInfo(Basic, medium, "https://medium.com/topic/business", []string{`/topic/business`, `/topic/money`}),
		generatePageInfo(Basic, medium, "https://medium.com/topic/startups", []string{`/topic/startups`, `/topic/leadership`}),
		generatePageInfo(Plus, medium, "https://medium.com/topic/work", []string{`/topic/work`, `/topic/freelancing`}),
		generatePageInfo(Premium, medium, "https://medium.com/topic/software-engineering", []string{`/software-engineering`, `/topic/programming`}),
		generatePageInfo(Premium, medium, "https://medium.com/topic/artificial-intelligence", []string{`/artificial-intelligence`, `/topic/technology`}),

		generatePageInfo(Basic, googleNews, "https://news.google.com/topics/CAAqJggKIiBDQkFTRWdvSUwyMHZNRGRqTVhZU0FtVnVHZ0pWVXlnQVAB", // topics: Technology
		...
		generatePageInfo(Basic, googleNews, "https://news.google.com/topics/CAAqJggKIiBDQkFTRWdvSUwyMHZNREpxYW5RU0FtVnVHZ0pWVXlnQVAB", // topics: Entertainment
		...
		generatePageInfo(Plus, googleNews, "https://news.google.com/topics/CAAqJggKIiBDQkFTRWdvSUwyMHZNRFp1ZEdvU0FtVnVHZ0pWVXlnQVAB", // topics: Sports
		...
		generatePageInfo(Premium, googleNews, "https://news.google.com/topics/CAAqJggKIiBDQkFTRWdvSUwyMHZNRFp0Y1RjU0FtVnVHZ0pWVXlnQVAB", // topics: Science
		...
		generatePageInfo(Premium, googleNews, "https://news.google.com/topstories", // top stories
			[]string{`/topstories`,
				`./topics/CAAqJggKIiBDQkFTRWdvSUwyMHZNRFZxYUdjU0FtVnVHZ0pWVXlnQVAB`}), // Headlines

		generatePageInfo(Plus, cnn, "https://edition.cnn.com/world", []string{`/world`, `/africa`}),
		generatePageInfo(Plus, cnn, "https://edition.cnn.com/americas", []string{`/americas`, `/asia`}),
		generatePageInfo(Plus, cnn, "https://edition.cnn.com/australia", []string{`/australia`, `/china`}),
		generatePageInfo(Premium, cnn, "https://edition.cnn.com/europe", []string{`/europe`, `/india`}),
		generatePageInfo(Premium, cnn, "https://edition.cnn.com/middle-east", []string{`/middle-east`, `/uk`}),

		generatePageInfo(Plus, espn, "https://www.espn.com/nfl/", []string{`/nfl/scoreboard`, `/nfl/schedule`}),
		generatePageInfo(Plus, espn, "https://www.espn.com/nba/", []string{`/nba/scoreboard`, `/nba/schedule`}),
		generatePageInfo(Plus, espn, "https://www.espn.com/mens-college-basketball/", []string{`/mens-college-basketball/scoreboard`, `/mens-college-basketball/schedule`}),
		generatePageInfo(Premium, espn, "https://www.espn.com/tennis/", []string{`/tennis/dailyResults`, `/tennis/schedule`}),
		generatePageInfo(Premium, espn, "https://www.espn.com/soccer/", []string{`/soccer/scoreboard`, `/soccer/schedule`}),

		generatePageInfo(Plus, hulu, "https://www.hulu.com/hub/movies", []string{`/hub/movies`, `/hub/originals`}),

		generatePageInfo(Plus, pinterest, "https://www.pinterest.com/ideas/", []string{`/ideas/`, `/ideas/holidays/910319220330/`}),

		generatePageInfo(Premium, youtube, "https://www.youtube.com", []string{`/`, `/feed/trending`}),

		generatePageInfo(Premium, netflix, "https://www.netflix.com", []string{`netflix.com`, `help.netflix.com/legal/termsofuse`}),
	}
	...
	for i := range windows {
		...
		for j := range tabs {
			for idx < len(allTargest) {
				if allTargest[idx].level <= caseLevel {
					tabs[j] = &chromeTab{conn: nil, pageInfo: allTargest[idx]}
				} else {
					// skip
				}
				idx++
...
func Run2(ctx context.Context, s *testing.State, cr *chrome.Chrome, caseLevel Level, isTablet bool) {
	...
	windows := generateTabSwitchTargets(caseLevel)

	var tsAction uiActionHandler
	if isTablet {
		if tsAction, err = newTabletActionHandler(ctx, tconn); err != nil {
			s.Fatal("Failed to create tablet action handler: ", err)
		}
	} else {
		if tsAction, err = newClamshellActionHandler(ctx, tconn); err != nil {
...
func openAllWindowsAndTabs(ctx context.Context, cr *chrome.Chrome, targets *[]*chromeWindow, tsAction uiActionHandler, caseLevel Level) (err error) {
	windows := (*targets)
	for idxWindow, window := range windows {
		for idxTab, tab := range window.tabs {
			...
			// There's a empty Chrome page been opened in advance to measure the launch time of Chrome browser
			// therefore, different procedure for the very first window and tab is needed
			if idxWindow == 0 && idxTab == 0 {
				...
				if tab.conn, err = tsAction.newTab(ctx, cr, tab.pageInfo.url, idxTab == 0); err != nil {
...
func tabSwitchAction(ctx context.Context, cr *chrome.Chrome, targets *[]*chromeWindow, tsAction uiActionHandler, caseLevel Level) error {
	windows := (*targets)
