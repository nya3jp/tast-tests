...
func testBody(ctx context.Context, s *testing.State, test *tabSwitchVariables) error {
	...
	if err != nil {
		return errors.Wrap("failed to create a mouse")
	}
	defer mw.Close()
	...
	for _, data := range test.webPages {
		...
		tabIcons := nodewith.HasClass("TabIcon")
		...
		for time.Now().Before(endTime) {
			tabToClick := tabIcons.Nth(currentTab)
			...
			for _, direction := range []sim.ScrollDirection{sim.Down, sim.Up} {
				if err := sim.RepeatMouseScroll(ctx, mw, direction, 50*time.Millisecond, 20); err != nil {
					return errors.Wrapf(err, "failed to scroll %v in between tab switches", direction)
				}
			}
