...
func TabSwitchCUJ(ctx context.Context, s *testing.State) {
	...
	recorder, err := cuj.NewRecorder(cuj.MetricConfig{
		Name:         "MPArch.RWH_TabSwitchPaintDuration",
		Unit:         "ms",
	...
	for _, data := range []struct {
		...
		const tabSwitchTimeout time.Duration = 20 * time.Second
	...
	if err = recorder.Record(s.OutDir()); err != nil {
		s.Error("Failed to report: ", err)
	}
}
