...
func (app *MicrosoftWebOffice) showTabList(ctx context.Context) error {
	...
	return uiauto.Combine("show tab list",
		app.uiHdl.Click(toggle),
		app.ui.WaitUntilExists(tcFinder),
	)(ctx)
}
...
func (app *MicrosoftWebOffice) maybeCloseOneDriveTab(tabName string) action.Action {
	nameFound := tabName
	tab := nodewith.Name(tabName).Role(role.Tab)
	oneDrive := nodewith.Name(oneDriveTab).Role(role.Tab)
	return func(ctx context.Context) error {
		if app.tabletMode {
			if err := app.showTabList(ctx); err != nil {
				return err
			}
		}
		if err := app.ui.WaitUntilExists(tab)(ctx); err != nil {
			if !strings.Contains(err.Error(), nodewith.ErrNotFound) {
				return err
			}
			nameFound = oneDriveTab
			found, err := app.ui.IsNodeFound(ctx, oneDrive)
			if err != nil {
				return err
			}
			if !found {
		...
		return uiauto.NamedAction(fmt.Sprintf("close the %q tab", nameFound), app.closeTab(nameFound))(ctx)
	}
}
