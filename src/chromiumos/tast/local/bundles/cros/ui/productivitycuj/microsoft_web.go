...
func (app *MicrosoftWebOffice) VoiceToTextTesting(ctx context.Context, expectedText string, playAudio action.Action) error {
	...
	wordWebArea := nodewith.Name("Word").Role(role.RootWebArea)
	paragraph := nodewith.Role(role.Paragraph).HasClass("Paragraph").Ancestor(wordWebArea)
...
// maybeRemoveSheet checks if the spreadsheet already exists with the name created in the test and clean them up.
func (app *MicrosoftWebOffice) maybeRemoveSheet() uiauto.Action {
