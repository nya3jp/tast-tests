...
func (app *GoogleDocs) CreateDocument(ctx context.Context) error {
	_, err := app.cr.NewConn(ctx, cuj.GoogleDocs)
	if err != nil {
		return errors.Wrapf(err, "failed to open URL: %s", cuj.GoogleDocs)
	}
...
func (app *GoogleDocs) CreateSlides(ctx context.Context) error {
	_, err := app.cr.NewConn(ctx, cuj.GoogleSlides)
	if err != nil {
		return errors.Wrapf(err, "failed to open URL: %s", cuj.GoogleSlides)
	}
...
func (app *GoogleDocs) CreateSpreadsheet(ctx context.Context) (string, error) {
	conn, err := app.cr.NewConn(ctx, cuj.GoogleSheets)
	if err != nil {
		return "", errors.Wrapf(err, "failed to open URL: %s", cuj.GoogleSheets)
	}
	defer conn.Close()
...
func (app *GoogleDocs) OpenSpreadsheet(ctx context.Context, filename string) error {
	...
	_, err := app.cr.NewConn(ctx, cuj.GoogleSheets)
	if err != nil {
		return errors.Wrapf(err, "failed to open URL: %s", cuj.GoogleSheets)
	}
