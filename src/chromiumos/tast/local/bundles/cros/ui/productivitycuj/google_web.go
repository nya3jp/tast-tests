...
func (app *GoogleDocs) selectCell(cell string) action.Action {
	...
	return uiauto.NamedAction(fmt.Sprintf("to select cell (%s)", cell),
...
func (app *GoogleDocs) getCellValue(ctx context.Context, cell string) (clipData string, err error) {
	...
	testing.ContextLogf(ctx, "Getting cell (%s) value: %s", cell, clipData)
...
func (app *GoogleDocs) editCellValue(ctx context.Context, cell, value string) error {
	testing.ContextLogf(ctx, "Writing cell (%s) value: %s", cell, value)

	return uiauto.Combine(fmt.Sprintf("write cell (%s) value", cell),
		app.selectCell(cell),
