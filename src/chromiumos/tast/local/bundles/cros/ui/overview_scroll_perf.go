...
func OverviewScrollPerf(ctx context.Context, s *testing.State) {
	...
	pv := perfutil.RunMultiple(ctx, s, cr, perfutil.RunAndWaitAll(tconn, func() error {
		...
		if err := stw.Swipe(ctx, tsew.Width()-10, 10, tsew.Width()/2-10, 10, 500*time.Millisecond); err != nil {
		...
		if err := stw.Swipe(ctx, tsew.Width()/2-10, 10, tsew.Width()-10, 10, 500*time.Millisecond); err != nil {
			return errors.Wrap(err, "failed to execute a swipe gesture")
		}
		if err := stw.End(); err != nil {
			return errors.Wrap(err, "failed to finish the swipe gesture")
