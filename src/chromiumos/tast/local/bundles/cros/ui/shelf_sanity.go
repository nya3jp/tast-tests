...
// ShelfSanity verifies that we can launch, switch between, and close apps from the shelf.
func ShelfSanity(ctx context.Context, s *testing.State) {
	...
	// TODO(kyleshima): find a better way to determine if the icons have stopped moving/loading
	testing.Sleep(ctx, time.Second)
	chromeBtn, err := ui.Find(ctx, tconn, ui.FindParams{ClassName: "ash/ShelfAppButton", Name: "Google Chrome"})
	if err != nil {
		s.Fatal("Failed to find Chrome shelf button: ", err)
	}
	defer chromeBtn.Release(ctx)

	filesBtn, err := ui.Find(ctx, tconn, ui.FindParams{ClassName: "ash/ShelfAppButton", Name: "Files"})
	if err != nil {
		s.Fatal("Failed to find Chrome shelf button: ", err)
	}
	defer filesBtn.Release(ctx)

	chromeInfo := appInfo{chromeBtn, apps.Chrome.ID, "Chrome - New Tab", "Chrome"}
	filesInfo := appInfo{filesBtn, apps.Files.ID, "Files - My files", "Files"}
	...
	for i := 0; i < 2; i++ {
		for _, app := range checkApps {
			...
			if err := ash.WaitForWindow(ctx, tconn, func(w *ash.Window) bool {
	...
	for _, app := range checkApps {
		...
		// TODO(kyleshima): find a better clickability indicator
		testing.Sleep(ctx, time.Second)
