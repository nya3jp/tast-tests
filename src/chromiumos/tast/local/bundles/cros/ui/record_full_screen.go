...
func RecordFullScreen(ctx context.Context, s *testing.State) {
	...
	// Checks there is a screen record video file stored in Download folder.
	files, err := filesapp.Launch(ctx, tconn)
	if err != nil {
		s.Fatal("Launching the Files App failed: ", err)
	}
	screenRecordFile := nodewith.Role(role.StaticText).NameRegex(regexp.MustCompile("Screen recording(.*?).webm")).First()
	if err := uiauto.Combine(
		"checks screen record is stored",
		files.OpenDownloads(),
		files.WaitUntilExists(screenRecordFile),
	)(ctx); err != nil {
		s.Fatal("Failed to check if the screen record is stored: ", err)
	}
}
