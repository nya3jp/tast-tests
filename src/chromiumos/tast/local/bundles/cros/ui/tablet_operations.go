...
func TabletOperations(ctx context.Context, s *testing.State) {
	...
	defer conn.Close()
	if conn, err := br.NewConn(ctx, url, browser.WithNewWindow()); err != nil {
	...
	r.RunMultiple(ctx, perfutil.SubtestRun(s.Run), "tablet-mode", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) error {
	...
	r.RunMultiple(ctx, perfutil.SubtestRun(s.Run), "hotseat-revealing", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) (err error) {
		defer func() {
			if err != nil {
		...
		button := nodewith.ClassName("AppListItemView").NameRegex(regexp.MustCompile("^" + browserApp.Name + "$")).First()
	...
	r.RunMultiple(ctx, perfutil.SubtestRun(s.Run), "overview-splitview", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) (err error) {
