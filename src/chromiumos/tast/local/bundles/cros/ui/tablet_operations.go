...
func TabletOperations(ctx context.Context, s *testing.State) {
	...
	// TODO: Check if an extra blank window popped up via SetUp needs to be closed for lacros-chrome or not.
	...
	r.RunMultiple(ctx, s, "hotseat-revealing", perfutil.RunAndWaitAll(tconn, func(ctx context.Context) (err error) {
		...
		// TODO: no window is in normal state: context deadline exceeded; last error follows: no window satisfies the condition
		if err := ash.WaitForCondition(ctx, tconn, func(w *ash.Window) bool {
			// s.Logf("hyungtaekim: w.State: %v, w: %#v", w.State, w)
