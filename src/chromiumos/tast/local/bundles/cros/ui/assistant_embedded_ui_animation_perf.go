...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Measures the smoothness of the embedded Assistant UI animations",
...
// The only possible state change for app list is between peeking and closed when we open/close Assistant UI
// with the accelerator.
func toggleAssistantUiOpenOrClose(ctx context.Context, tconn *chrome.Conn, open bool) error {
	// closed->peeking.
	if err := assistant.ToggleUi(ctx, tconn, true); err != nil {
...
func AssistantEmbeddedUiAnimationPerf(ctx context.Context, s *testing.State) {
	...
	for openedWindows := 0; openedWindows <= maxNumOfWindows; openedWindows++ {
		...
		histograms, err := metrics.Run(ctx, cr,
			func() error {
				if err := toggleAssistantUiOpenOrClose(ctx, tconn, true); err != nil {
					return err
				}
				return nil
			},
