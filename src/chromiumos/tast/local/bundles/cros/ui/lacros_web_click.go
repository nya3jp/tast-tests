...
func LacrosWebClick(ctx context.Context, s *testing.State) {
	...
	_, _, cs, err := lacros.Setup(ctx, s.FixtValue(), browser.TypeLacros)
	if err != nil {
	...
	testing.ContextLog(ctx, "Maximize browser window")
	// Maximize browser window to ensure a consistent state.
