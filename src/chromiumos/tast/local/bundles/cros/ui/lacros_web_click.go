...
func LacrosWebClick(ctx context.Context, s *testing.State) {
	...
	testing.ContextLog(ctx, "Location of shuffle button box: ", shuffleBtnInfo.Location)
	...
	if err := ui.WaitUntilExists(pauseButton)(ctx); err != nil {
		s.Fatal("Music is not played after click Shuffle button - probabley the button is not clicked")
	}
}
