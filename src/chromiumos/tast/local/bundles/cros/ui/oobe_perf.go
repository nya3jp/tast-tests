...
func OobePerf(ctx context.Context, s *testing.State) {
	...
	r.RunMultiple(ctx, s, "OobePerf",
		func(ctx context.Context) ([]*metrics.Histogram, error) {
			// Load OOBE Welcome Screen (first OOBE screen). Test extentions is required to fetch histograms.
			var cr *chrome.Chrome
			var err error
			if cr, err = chrome.New(ctx, chrome.NoLogin(), chrome.LoadSigninProfileExtension(s.RequiredVar("ui.signinProfileTestExtensionManifestKey"))); err != nil {
				s.Fatal("Failed to start Chrome: ", err)
			}
			defer cr.Close(ctx)

			var tLoginConn *chrome.TestConn
			if tLoginConn, err = cr.SigninProfileTestAPIConn(ctx); err != nil {
