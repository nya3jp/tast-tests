...
func VideoCUJ(ctx context.Context, s *testing.State) {
	...
	tapYtElem := func(sel string) error {
		...
		if err := ac.WaitForLocation(ytWeb)(ctx); err != nil {
			return errors.Wrap(err, "failed to get YouTube WebContentsViewAura")
		}

		info, err := ac.Info(ctx, ytWeb)
		if err != nil {
			return errors.Wrap(err, "failed to get YouTube WebContentsViewAura info")
		}

		tapPoint := bounds.CenterPoint().Add(info.Location.TopLeft())
