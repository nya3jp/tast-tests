...
func runLauncherAnimation(ctx context.Context, tconn *chrome.Conn, s *testing.State, kb *input.KeyboardEventWriter, at launcherAnimationType) {
	var err error
	...
	if err = ash.TriggerLauncherStateChangeAndWait(ctx, tconn, trigger, firstState); err != nil {
...
func LauncherAnimationPerf(ctx context.Context, s *testing.State) {
	...
	for _, windows := range []int{0, 2} {
		...
		for _, at := range []launcherAnimationType{animationTypePeeking, animationTypeHalf, animationTypeFullscreenSearch, animationTypeFullscreenAllApps} {
			...
			runLauncherAnimation(ctx, tconn, s, kb, at)
