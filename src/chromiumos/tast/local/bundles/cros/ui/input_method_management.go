...
func InputMethodManagement(ctx context.Context, s *testing.State) {
	...
	if err := ossettings.LaunchAtInputsSettingsPage(ctx, tconn, cr); err != nil {
	...
	if err := imesettings.SearchInputMethod(ctx, tconn, searchKeyword, inputMethodName); err != nil {
	...
	if err := ime.WaitForInstalledInputMethod(ctx, tconn, inputMethdCode, 10*time.Second); err != nil {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			return testing.PollBreak(errors.Wrap(err, "failed to get input methods"))
		}
		for _, inputMethod := range installedInputMethods {
			if inputMethod.ID == inputMethdCode {
				return errors.Wrapf(err, "%s is not removed", inputMethdCode)
			}
		}
		return nil
	}, &testing.PollOptions{Timeout: 10 * time.Second, Interval: 1 * time.Second}); err != nil {
		s.Fatal("Failed to wait for input method removed via Chrome API: ", err)
	}
}
