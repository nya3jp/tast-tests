...
func GoogleSlidesCUJ(ctx context.Context, s *testing.State) {
	...
	if err := recorder.Run(ctx, func(ctx context.Context) (retErr error) {
		...
		// Keep track of the number of iterations we run the scroll
		// down cycle for. This allows us to control how often we want
		// to run Ash UI interactions to ensure collection of
		...
		for endTime := time.Now().Add(slidesScrollTimeout); time.Now().Before(endTime); {
			...
			// Every 5 scroll iterations, interact with the Ash UI.
			if i%5 == 0 {
