...
// MashLogin checks that chrome --enable-features=Mash starts and at least one
// mash service is running.
func MashLogin(s *testing.State) {
	...
	if !found {
		s.Fatal("Could not find chrome with mash services, command lines", cmds)
	}
}
