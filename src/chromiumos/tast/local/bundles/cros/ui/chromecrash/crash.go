...
func getNonBrowserProcess(ctx context.Context, ptype ProcessType) (pid int, err error) {
	...
	if err != nil {
		return -1, errors.Wrapf(err, "error looking for Chrome %s", ptype)
	}
	if len(processes) == 0 {
		return -1, errors.Errorf("no Chrome %s's found", ptype)
	}
...
func killNonBrowser(ctx context.Context, ptype ProcessType, dirs, oldFiles []string) error {
	...
	err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		testing.ContextLogf(ctx, "Sending SIGSEGV to target Chrome %s pid %d", ptype, toKill)
