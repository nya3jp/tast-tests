...
func (cfType CrashFileType) String() string {
	return [...]string{"MetaFile", "BreakpadDmp"}[cfType]
}
...
func waitForBreakpadDmpFile(ctx context.Context, pid int, dirs, oldFiles []string) error {
	...
	err := testing.Poll(ctx, func(c context.Context) error {
		files, err := localCrash.WaitForCrashFiles(ctx, dirs, oldFiles, []string{fileName})
		if err != nil {
		...
		for _, fileName := range files {
			if found, err := localCrash.IsBreakpadDmpFileForPid(fileName, pid); err != nil {
