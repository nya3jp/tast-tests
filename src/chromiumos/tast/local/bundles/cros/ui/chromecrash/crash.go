...
func killNonBrowser(ctx context.Context, ptype ProcessType, dirs, oldFiles []string) error {
	...
	err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		createTimeSeconds := createTimeMS / millisecondsPerSecond
		createTimeRemainderNS := (createTimeMS % millisecondsPerSecond) * nanosecondsPerMillisecond
		createTime := time.Unix(createTimeSeconds, createTimeRemainderNS)
