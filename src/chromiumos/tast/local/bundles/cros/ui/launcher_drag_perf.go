...
func LauncherDragPerf(ctx context.Context, s *testing.State) {
	...
	// TODO(mukai): fix this after crrev.com/c/2090666 is merged.
	root, err := chromeui.Root(ctx, tconn)
	if err != nil {
		s.Fatal("Failed to access to the desktop: ", err)
	}
	appIcons, err := root.Descendants(ctx, chromeui.FindParams{ClassName: "ash/ShelfAppButton"})
	if err != nil {
