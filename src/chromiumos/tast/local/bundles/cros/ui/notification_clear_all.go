...
// NotificationClearAll tests that several notifications can be dismissed with the 'Clear all' button.
func NotificationClearAll(ctx context.Context, s *testing.State) {
	...
	const n = 10
	s.Log("Creating notifications")
	for i := 0; i < n; i++ {
		title := fmt.Sprintf("TestNotification%v", i)
	...
	r, err := regexp.Compile("TestNotification")
	if err != nil {
