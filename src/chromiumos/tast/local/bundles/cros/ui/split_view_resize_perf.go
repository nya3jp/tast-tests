	...
	if tabletMode {
		...
		testCases = TestCases{
			{"SingleWindow", 2, func() error {
				...
				if err := stw.LongPressAt(ctx, wCenterX, wCenterY); err != nil {
					return errors.Wrap(err, "failed to long-press to select the target window")
				}
				deskMiniViewBounds, err := ash.GetDeskMiniViewBounds(ctx, tconn, 0, 1)
				if err != nil {
					return errors.Wrap(err, "failed to get desk mini-view bounds")
				}
				dCenterX, dCenterY := tcc.ConvertLocation(deskMiniViewBounds.CenterPoint())
