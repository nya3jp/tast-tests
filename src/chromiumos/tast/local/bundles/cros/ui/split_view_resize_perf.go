	...
	if tabletMode {
		testCases = testCaseSlice{
			...
			{"MultiWindow", 8, func() error {
				...
				id1 := w.ID
				if err := pointerController.Press(ctx, w.OverviewInfo.Bounds.CenterPoint()); err != nil {
		...
		testCases = testCaseSlice{
			{"SingleWindow", 2, func() error {
				...
				start := w.OverviewInfo.Bounds.CenterPoint()
				end := deskMiniViews[1].Location.CenterPoint()
				if err := pointerController.Press(ctx, start); err != nil {
					return errors.Wrap(err, "failed to start window drag from overview grid to desk mini-view")
				}
				if err := pointerController.Move(ctx, start, end, time.Second); err != nil {
					return errors.Wrap(err, "failed during window drag from overview grid to desk mini-view")
				}
				if err := pointerController.Release(ctx); err != nil {
					return errors.Wrap(err, "failed to end window drag from overview grid to desk mini-view")
				}
