...
func SplitViewResizePerf(ctx context.Context, s *testing.State) {
	...
	getWindowCenterInOverview := func(ctx context.Context) (w *ash.Window, centerX, centerY input.TouchCoord, err error) {
		...
		if err != nil {
			return nil, 0, 0, errors.Wrap(err, "failed to find the window in the overview")
		}
		return w, centerX, centerY, nil
	}
	...
	for _, testCase := range []struct {
		...
		{"MultiWindow", 8, func() error {
			...
			// point of its bounds.
			w, centerX, centerY, err := getWindowCenterInOverview(ctx)
			if err != nil {
				return err
			}
			id1 := w.ID
		...
		s.Run(ctx, testCase.name, func(ctx context.Context, s *testing.State) {
			...
			// achieve ths split-view state. Dragging starts from the center of the
			// target window to detach the brwoser window and ends at the middle-left
			// of the screen to cause the left-snap. Note that
			// ash.SetWindowState(SnapLeft) is not available -- it's not yet working
			// well on the tablet mode.
