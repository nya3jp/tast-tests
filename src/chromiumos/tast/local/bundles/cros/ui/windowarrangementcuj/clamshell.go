...
func multiresize(ctx context.Context, tconn *chrome.TestConn, ui *uiauto.Context, pc pointer.Context, duration time.Duration, dragPoints ...coords.Point) error {
	...
	if err := drag(ctx, tconn, pc, duration, multiresizeDragPoints...); err != nil {
...
func RunClamShell(ctx, closeCtx context.Context, tconn *chrome.TestConn, ui *uiauto.Context, pc pointer.Context, d *androidui.Device, act *arc.Activity, withTestVideo arc.ActivityStartOption) (retErr error) {
	...
	if err := drag(ctx, tconn, pc, duration, upperLeftPt, middlePt); err != nil {
	...
	if err := drag(ctx, tconn, pc, duration, tabStripGapPt, middlePt); err != nil {
	...
	if err := drag(ctx, tconn, pc, duration, splitViewDragPoints...); err != nil {
		return errors.Wrap(err, dividerDragError)
	}
