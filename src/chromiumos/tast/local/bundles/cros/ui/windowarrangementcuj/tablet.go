...
func RunTablet(ctx context.Context, tconn *chrome.TestConn, ui *uiauto.Context, pc pointer.Context, act *arc.Activity) error {
	...
	// divider width is 8, half of that is 4, and so there are 4 DIPs between the drag point and the
	// ARC window. That means the ARC window has width 342 at a drag point 346 away from the right end.
	// So we avoid it with a drag point 347 away from the right end.
	// TODO(https://crbug.com/1297297): Remove this when the bug is fixed.
	splitViewDragPoints[2].X -= 347
