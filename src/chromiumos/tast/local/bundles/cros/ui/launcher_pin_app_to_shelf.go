...
func LauncherPinAppToShelf(ctx context.Context, s *testing.State) {
	...
	if err := launcher.OpenExpandedView(ctx, tconn, tsew, stw); err != nil {
	...
	pinApps(ctx, s, tconn, tc, []apps.App{app1, app2, app3})

	// Launch another app that is not pinned.
	if err := launcher.LaunchApp(ctx, tconn, tsew, stw, app4); err != nil {
	...
	// Launch another app that is not pinned.
	if err := launcher.LaunchApp(ctx, tconn, tsew, stw, app5); err != nil {
...
func pinApps(ctx context.Context, s *testing.State, tconn *chrome.TestConn, tc *pointer.TouchController, apps []apps.App) {
	var tsew *input.TouchscreenEventWriter
	var stw *input.SingleTouchEventWriter
	if tc != nil {
		tsew = tc.Touchscreen()
	...
	for _, app := range apps {
		if err := launcher.PinAppToShelf(ctx, tconn, tsew, stw, app); err != nil {
