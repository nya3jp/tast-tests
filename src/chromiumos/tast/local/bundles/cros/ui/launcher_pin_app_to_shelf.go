...
func LauncherPinAppToShelf(ctx context.Context, s *testing.State) {
	...
	if err := ui.WaitForLocationChangeCompleted(ctx, tconn); err != nil {
		s.Fatal("Failed to wait for location change to be completed: ", err)
	}
	...
	if tabletMode {
		// If it is in tablet mode, open the home screen first.
		// Otherwise, we cannot click another app icon.
		tsew := tc.Touchscreen()
		if err := ash.DragToShowHomescreen(ctx, tsew.Width(), tsew.Height(), stw, tconn); err != nil {
			s.Fatal("Failed to drag from bottom of the screen to show home screen: ", err)
		}
	} else if err := launcher.OpenExpandedView(ctx, tconn); err != nil {
