...
func LauncherPinAppToShelf(ctx context.Context, s *testing.State) {
	...
	if tabletMode {
		err = ash.PinAppFromHotseat(ctx, tconn, stw, tcc, app5.Name)
	} else {
	...
	if tabletMode {
		err = ash.LaunchAppFromHotseat(ctx, tconn, stw, tcc, app1.Name, app1.ID)
	} else {
...
func pinApps(ctx context.Context, tconn *chrome.TestConn, apps []apps.App) error {
	...
	if err != nil {
		return errors.New("cannot get location for buttons")
	}
...
func rightmostButton(ctx context.Context, tconn *chrome.TestConn, appName string) error {
	...
	appButtons, err := ui.FindAll(ctx, tconn, params)
	if err != nil {
		return errors.Wrap(err, "failed to get all buttons on shell")
	}
	defer appButtons.Release(ctx)
	...
	if rightMostButton.Name != appName {
		return errors.Errorf("%q is not the rightmost button on the shelf", appName)
	}
	return nil
}
