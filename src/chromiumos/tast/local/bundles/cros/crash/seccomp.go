...
func Seccomp(ctx context.Context, s *testing.State) {
	...
	cmd := testexec.CommandContext(ctx, "/sbin/minijail0", "-f", pidFile.Name(), "-S", "/dev/null", "/bin/true")
	...
	matches, err := filepath.Glob(fmt.Sprintf("/var/spool/crash/coreutils.*.%d.*", pid))
	if err != nil {
		s.Fatal("Failed to find crash files: ", err)
	}
	if len(matches) == 0 {
		s.Fatalf("Failed to find crash files for pid: %d", pid)
	}
	...
	for _, match := range matches {
		if strings.HasSuffix(match, ".proclog") {
			...
			if err = mustContain(match, "SECCOMP_POLICY_PATH=/dev/null"); err != nil {
				...
				// Do not clean up the file in this case so it can be inspected.
				continue
			}
		}
		if err = os.Remove(match); err != nil {
			s.Errorf("Failed to clean up %q: %v", match, err)
		}
	}
	if !found {
