...
func SenderRateLimit(ctx context.Context, s *testing.State) {
	...
	for {
		...
		rs, err := sender.ListRecords()
		if err != nil {
			s.Fatal("Failed to get upload records: ", err)
		}
		if len(rs) != runs {
			s.Fatalf("Upload records are not correctly counted: got %d, want %d", len(rs), runs)
		}
	...
	// Change the timestamp of one upload record to 25 hours ago.
	rs, err := sender.ListRecords()
	if err != nil {
		s.Fatal("Failed to get upload records: ", err)
	}
	if len(rs) == 0 {
		s.Fatal("No upload record found")
	}
	fn := filepath.Join(sender.RecordDir, rs[0].Name())
	ts := time.Now().Add(-25 * time.Hour)
	if err := os.Chtimes(fn, ts, ts); err != nil {
		s.Fatal("Failed to change the timestamp of an upload record file: ", err)
	}

	// Attempt crash_sender again. It should succeed this time.
	s.Logf("Iteration #%d (after modifying upload record timestamp)", runs)
	...
	if !got[0].Success {
		s.Error("crash_sender still fails to upload a crash dump after modifying upload record timestamp")
	}
}
