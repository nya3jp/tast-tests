...
func AuthFailure(ctx context.Context, s *testing.State) {
	...
	// Restart tcsd first to cleanup previous auth failure
	...
	files, err := crash.WaitForCrashFiles(ctx, []string{crash.SystemCrashDir}, expectedAuthFailureRegexes, crash.Timeout(30*time.Second))
	if err == nil {
