...
func KernelIwlwifiError(ctx context.Context, s *testing.State) {
	...
	duOutput, err := testexec.CommandContext(ctx, "du", "-a", "/mnt/stateful_partition").Output()
	if err != nil {
		s.Error("Failed to run the command du -a /mnt/stateful_partition: ", err)
	}
	content = content + "\n\n Output of the command du -a /mnt/stateful_partition at the beginning of the test::\n" + string(duOutput)
