...
func AccountAccessWebsite(ctx context.Context, s *testing.State) {
	...
	// Update policies - changed to Refresh rather to verify as verify yielded
	// error or policy mismatch.
	...
	if err := ui.WithTimeout(10 * time.Second).WaitUntilExists(nodewith.NameStartingWith(username).Role(role.StaticText))(ctx); err != nil {
