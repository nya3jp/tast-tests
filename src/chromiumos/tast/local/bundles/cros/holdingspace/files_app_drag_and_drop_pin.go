...
func FilesAppDragAndDropPin(ctx context.Context, s *testing.State) {
	...
	// Call MarkTimeOfFirstAdd to cause holding space tray to show, so that we
	// can drag to it.
	var wasSet bool
	if err := tconn.Call(ctx, &wasSet,
		"tast.promisify(chrome.autotestPrivate.holdingSpaceMarkTimeOfFirstAdd)",
	); err != nil {
		s.Fatalf("Failed to show holding space tray: %s", err)
	} else if !wasSet {
		s.Error("TimeOfFirstAdd pref was unexpectedly already set")
	}
