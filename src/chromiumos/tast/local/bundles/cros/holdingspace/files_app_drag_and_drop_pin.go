...
func FilesAppDragAndDropPin(ctx context.Context, s *testing.State) {
	...
	if err := testDragAndDrop(ctx, tconn, uia, kb, fsapp, myFilesPath, param); err != nil {
		s.Fatal("Fail to perform parameterized testing: ", err)
	}

}

func testDragAndDrop(ctx context.Context, tconn *chrome.TestConn, uia *uiauto.Context, kb *input.KeyboardEventWriter, fsapp *filesapp.FilesApp, myFilesPath string, param int) error {
	...
	for i := 0; i < param; i++ {
		...
		if err := ioutil.WriteFile(testFilePath, []byte("Per aspera, ad astra"), 0644); err != nil {
			errors.Errorf("creating file %q failed: %s", testFilePath, err)
		}
		...
		func(ctx context.Context) error {
			for _, fileName := range listFiles {
				if err := uia.WaitUntilExists(holdingspace.FindPinnedFileChip().Name(fileName))(ctx); err != nil {
					errors.Errorf("failed to verify that %s exists in holding space: %s", fileName, err)
				}
			}
			return nil
		},
	)(ctx); err != nil {
		errors.Errorf("failed to pin %d items %v from Files App to Holding Space by drag and drop: %s", param, listFiles, err)
	}
	...
	if err := uiauto.Combine(fmt.Sprintf("Copy %d file(s) from Holding Space to Files App by drag and drop", param),
		func(ctx context.Context) error {
			...
			if err = mouse.Drag(tconn, lastFileLocation.CenterPoint(), fsappLocation.CenterPoint(), time.Second)(ctx); err != nil {
				errors.Errorf("failed to drag and drop %d pinned files(s) %v from Holding Space to Files App: %s", param, listFiles, err)
			}
			return err
		},
		uia.Gone(holdingspace.FindChip()),
	)(ctx); err != nil {
		errors.Errorf("failed to copy %d file(s) from Holding Space to Files App by drag and drop", param)
	}

	// Remove the copied file(s).
	for _, fileName := range listFiles {
		os.Remove(filepath.Join(myFilesPath, fmt.Sprintf("%s (1).txt", fileName[:len(fileName)-len(filepath.Ext(fileName))])))
	}
