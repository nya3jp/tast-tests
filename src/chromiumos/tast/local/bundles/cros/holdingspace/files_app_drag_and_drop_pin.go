...
// dragging and dropping single/multiple files to/from the Files app as well as dragging and
// dropping single/multiple files from Holding Space to Chrome Browser.
func FilesAppDragAndDropPin(ctx context.Context, s *testing.State) {
	...
	// Open a file from Holding Space to Chrome browser by drag and drop.
	uia.LeftClick(tray)(ctx)
	mouse.Drag(tconn, fileLocation1.CenterPoint(), chromeLocation.CenterPoint(), time.Second)(ctx)
	err = uia.Gone(holdingspace.FindChip())(ctx)
	if err != nil {
		s.Fatal("Failed to automatically close Holding Space by dragging item out of Holding Space: ", err)
	}
	err = uia.WaitUntilExists(nodewith.Role(role.Tab).Name(testFile1))(ctx)
	if err != nil {
		s.Fatalf("Failed to open file %s in Chrome browser: %s", testFile1, err)
	}

	// Open multiple files from Holding Space in Chrome browser by drag and drop.
	uia.LeftClick(tray)(ctx)
	if err := kb.AccelPress(ctx, "Ctrl"); err != nil {
		s.Fatal("Failed to press Ctrl: ", err)
	}
	...
	err = mouse.Drag(tconn, fileLocation3.CenterPoint(), chromeLocation.CenterPoint(), time.Second)(ctx)
	if err != nil {
		s.Fatalf("Failed to drag and drop multiple pinned files %v from Holding Space to Chrome Browser: %s", listFiles, err)
	}
	uia.Gone(holdingspace.FindChip())(ctx)
	if err != nil {
		s.Fatal("Failed to automatically close Holding Space by dragging multiple items out of Holding Space: ", err)
	}
	err = uia.WaitUntilExists(nodewith.Role(role.Tab).Name(testFile2))(ctx)
	if err != nil {
		s.Fatalf("Failed to open file %s in Chrome browser: %s", testFile2, err)
	}
	err = uia.WaitUntilExists(nodewith.Role(role.Tab).Name(testFile3))(ctx)
	if err != nil {
		s.Fatalf("Failed to open file %s in Chrome browser: %s", testFile3, err)
	}
