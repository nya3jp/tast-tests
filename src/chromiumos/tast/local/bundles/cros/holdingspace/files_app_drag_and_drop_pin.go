...
func FilesAppDragAndDropPin(ctx context.Context, s *testing.State) {
	...
	if param == 1 {
		err = testSingleFileDragAndDrop(ctx, tconn, uia, kb, fsapp, myFilesPath, listFiles)
	} else if param == 2 {
		err = testMultipleFilesDragAndDrop(ctx, tconn, uia, kb, fsapp, myFilesPath, listFiles)
	} else {
