...
func FilesAppContextMenuPinAndUnpin(ctx context.Context, s *testing.State) {
	...
	if err := uia.IfSuccessThen(
		uia.WaitUntilExists(maxBtn),
		uia.LeftClick(maxBtn),
	)(ctx); err != nil {
...
func verifyTipInModes(ctx context.Context, tconn *chrome.TestConn, isTablet bool) error {
	...
	if err := uiauto.New(tconn).WaitUntilExists(nodewith.Role(role.StaticText).Name("Create a shortcut for your files"))(ctx); err != nil {
...
func createTarget(targetPath string, isFile bool) error {
	if exists, err := isTargetExists(targetPath); err != nil {
...
// isTargetExists checks if the target is in the specific file path.
func isTargetExists(targetPattern string) (bool, error) {
	files, err := filepath.Glob(targetPattern)
	if err != nil {
		return false, errors.Wrapf(err, "the pattern %q is malformed", targetPattern)
	}
