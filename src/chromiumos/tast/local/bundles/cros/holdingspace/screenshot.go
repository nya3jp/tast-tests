...
func Screenshot(ctx context.Context, s *testing.State) {
	...
	if len(screenshots) != 0 || err != nil {
		s.Fatal("Failed to verify no screenshots exist: ", err)
	}

	ui := uiauto.New(tconn)

	if err := uiauto.Combine("Ensure tray is absent and capture screenshot",
		...
		s.Fatal("Failed to ensure tray is absent and capture screenshot: ", err)
	}
	...
	if err := uiauto.Combine("Pin and unpin screenshot",
		...
		s.Error("Failed to pin and unpin screenshot: ", err)
	}
}
