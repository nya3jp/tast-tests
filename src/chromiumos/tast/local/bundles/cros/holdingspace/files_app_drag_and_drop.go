...
func FilesAppDragAndDrop(ctx context.Context, s *testing.State) {
	...
	fsappLocation, err := uia.Location(ctx, nodewith.Role(role.ListBox).Ancestor(nodewith.HasClass("files-ng").First()))
	if err != nil {
	...
	if err := uiauto.Combine(fmt.Sprintf("Copy %d file(s) from Holding Space to Files App by drag and drop", param),
		...
		func(ctx context.Context) error {
			for _, fileName := range listFiles {
				var copiedFileName = fmt.Sprintf("%s (1)%s", fileName[:len(fileName)-len(filepath.Ext(fileName))], filepath.Ext(fileName))
				defer os.Remove(filepath.Join(copiedFileName))
				// if err := fsapp.SelectFile(copiedFileName)(ctx); err != nil {
				// 	s.Fatalf("Failed to select copied file %q: %s", copiedFileName, err)
				// }
