	...
	server := httptest.NewServer(http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			...
			if redirect := r.URL.Query().Get("redirect"); redirect == "true" {
				fmt.Fprintf(w, fmt.Sprintf("<meta http-equiv='refresh' content='1; url=/?file=%s' />", downloadFileName))
				return
			}
...
func (c *holdingspaceChip) finder(fileName string) *nodewith.Finder {
	return c.chipFinder.Name(c.chipName(fileName))
}

func (c *holdingspaceChip) chipName(fileName string) string {
	switch c.chipType {
	case downloading:
