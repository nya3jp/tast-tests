...
// verifyLaunchFiles verifies that the specified files are launched in new browser tabs.
func verifyLaunchFiles(res *downloadResource, fileNames []string) uiauto.Action {
	return func(ctx context.Context) error {
		var browserRoot, tabNode *nodewith.Finder

		switch res.browserType {
		case browser.TypeAsh:
			browserRoot = nodewith.Ancestor(nodewith.Role(role.Window).HasClass("BrowserFrame"))
			tabNode = browserRoot.HasClass("Tab").Role(role.Tab)
		case browser.TypeLacros:
			classNameRegexp := regexp.MustCompile(`^ExoShellSurface(-\d+)?$`)
			browserRoot = nodewith.Ancestor(nodewith.Name("Text").Role(role.Window).ClassNameRegex(classNameRegexp))
			tabNode = browserRoot.HasClass("filename").Role(role.GenericContainer)
		default:
			return errors.New("unsupported browser type")
		}

		for _, file := range fileNames {
			if err := res.ui.WaitUntilExists(tabNode.NameContaining(file))(ctx); err != nil {
