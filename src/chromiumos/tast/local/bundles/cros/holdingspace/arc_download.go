...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Verifies that ARC downloads are successfully shown in holding space",
		Contacts: []string{
...
func ArcDownload(ctx context.Context, s *testing.State) {
	...
	defer func() {
		if s.HasError() {
			if err := arc.Command(ctx, "uiautomator", "dump").Run(testexec.DumpLogOnError); err != nil {
	...
	if err := playstore.InstallApp(ctx, arc, uid, packageName, &playstore.Options{InstallationTimeout: 180 * time.Second}); err != nil {
	...
	if err := arc.Command(ctx, "am", "start", "-a", "android.intent.action.VIEW", "-p", packageName, "-d", dlURL).Run(); err != nil {
	...
	acceptButton := uid.Object(ui.ClassName("android.widget.Button"), ui.TextMatches("(?i)"+"Accept.+"))
	...
	noThanksButton := uid.Object(ui.ClassName("android.widget.Button"), ui.TextMatches("(?i)"+"No thanks"))
	...
	// file system.
	continueButton := uid.Object(ui.ClassName("android.widget.Button"), ui.TextMatches("(?i)"+"Continue"))
	...
	allowButton := uid.Object(ui.ClassName("android.widget.Button"), ui.TextMatches("(?i)"+"Allow"))
