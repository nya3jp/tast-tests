...
func ArcDownload(ctx context.Context, s *testing.State) {
	...
	// TODO(dmblack): Document.
	ui := uiauto.New(tconn)
	if err := uiauto.Combine("Click through consents",
		ui.LeftClick(nodewith.Name("Got it").Role(role.Button)),
		clickNodeWithResourceID(d, pkgName+":id/terms_accept"),
		clickNodeWithResourceID(d, pkgName+":id/negative_button"),
		clickNodeWithResourceID(d, pkgName+":id/ack_button"),
	)(ctx); err != nil {
		s.Fatal("Failed to click through consents: ", err)
	}
}

// clickNodeWithResourceID TODO(dmblack): Document.
func clickNodeWithResourceID(d *androidui.device, resourceID string) uiauto.Action {
	return func(ctx context.Context) error {
		return d.Object(androidui.ResourceID(resourceID)).Click(ctx)
	}
}
