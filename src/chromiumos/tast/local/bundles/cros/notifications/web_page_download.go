...
func init() {
	testing.AddTest(&testing.Test{
		...
		Timeout:      2*time.Minute + 2*downloadTimeout, // Each test will download a file twice.
...
func (bf *bigFileTest) verifyNotificationAutoHide(res *webPageDownloadTestResource) uiauto.Action {
	...
	return cancelDownloading(res.ui, bf.fileName)
}
...
func (bf *bigFileTest) verifyNotificationAndDismiss(res *webPageDownloadTestResource) uiauto.Action {
	return func(ctx context.Context) error {
		if !bf.progressNotificationEnabled {
			...
			if _, err := ash.WaitForNotification(ctx, res.tconn, downloadTimeout, ash.WaitTitle(bf.expectedNotification)); err != nil {
				return errors.Wrapf(err, "failed to wait for notification within %v", downloadTimeout)
			}
		}

		// This test won't examine if the notification can be dismissed because a big file is unlikely to finish downloading.
		// Terminate the download process by canceling it to proceed other tests.
		return cancelDownloading(res.ui, bf.fileName)(ctx)
	}
}
...
func cancelDownloading(ui *uiauto.Context, fileName string) uiauto.Action {
	downloadSection := holdingspace.FindDownloadChip().Name(fmt.Sprintf("Downloading %s", fileName))
	// Pause/Resume button and close button in the downloadSection are identical on the UI tree.
	// Close button is the second node.
	...
	return uiauto.Combine("cancel downloading",
		ui.LeftClick(holdingspace.FindTray().Role(role.Button)),
		// Hover on the downloadSection to make the button appears.
		ui.MouseMoveTo(downloadSection, 0),
		ui.LeftClick(closeBtn),
		ui.WaitUntilGone(downloadSection),
	)
}
