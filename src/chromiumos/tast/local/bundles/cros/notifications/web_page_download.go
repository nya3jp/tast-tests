...
const (
	...
	maliciousFileName  = "download_test_file_dangerous_20221103.exe"
	suspiciousFileName = "download_test_file_suspicious_20221103.crx"
	...
	closedNotification          = "Notification close"
...
func init() {
	testing.AddTest(&testing.Test{
		...
		Params: []testing.Param{
			{
				Name: "big_file",
				Val:  []webPageDownloadTest{newBigFileTest(bigFileName, "Downloading "+bigFileName)},
			}, {
				Name: "malicious_file",
				Val:  []webPageDownloadTest{newMaliciousFileTest("Dangerous download blocked")},
			}, {
				Name: "suspicious_file",
				Val:  []webPageDownloadTest{newSuspiciousFileTest("Confirm download")},
			}, {
				Name: "image_file",
				Val: []webPageDownloadTest{
...
func WebPageDownload(ctx context.Context, s *testing.State) {
	...
	source := map[string]string{
		bigFileName:        path.Join("ui", bigFileName),
		jpgFileName:        path.Join("arc", jpgFileName),
		pngFileName:        path.Join("apps", pngFileName),
		mp4FileName:        path.Join("video", mp4FileName),
		webmFileName:       path.Join("video", webmFileName),
		mp3FileName:        path.Join("audio", mp3FileName),
		wavFileName:        path.Join("audio", wavFileName),
		maliciousFileName:  path.Join("filemanager", maliciousFileName),
		suspiciousFileName: path.Join("filemanager", suspiciousFileName),
	}
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		for name, path := range source {
			fullURL, err := url.JoinPath(regularFileURLPrefix, path)
			if err != nil {
				s.Fatal("Failed to join path: ", err)
			}
			if _, err := fmt.Fprintf(w, `<a href=%s>%s</a><br>`, fullURL, name); err != nil {
				s.Fatal("Failed to format string: ")
			}
		}
	}))
	defer server.Close()
	...
	for _, test := range s.Param().([]webPageDownloadTest) {
		func() {
...
func (p *webPageDownloadTestSample) verifyNotificationAutoHide(res *webPageDownloadTestResource) uiauto.Action {
	return func(ctx context.Context) error {
		...
		if goneDuration > 8*time.Second {
			return errors.Errorf("failed to verify notification autohide, want:6s, get:%s", goneDuration)
		}
		return nil
	}
}
...
func (p *webPageDownloadTestSample) verifyNotificationAndDismiss(res *webPageDownloadTestResource) uiauto.Action {
	return func(ctx context.Context) error {
		...
		if err := res.ui.LeftClick(nodewith.Name(closedNotification).Role(role.Button))(ctx); err != nil {
...
func newBigFileTest(fileName, expectedNotification string) *bigFileTest {
	return &bigFileTest{
...
func (bf *bigFileTest) verifyNotificationAutoHide(res *webPageDownloadTestResource) uiauto.Action {
	...
	return uiauto.Combine("cancel downloading and verify the notification doesn't exist",
		cancelDownloading(res, bf.fileName),
		// The notification should be gone immediately after download is canceled.
		res.ui.WithTimeout(shortTimeout).WaitUntilGone(nodewith.Name(bf.expectedNotification)),
	)
}
...
func (mf *maliciousFileTest) downloadSample(res *webPageDownloadTestResource) uiauto.Action {
	// Only download the malicious file by directly left-click on the link will show the expected notification.
	// Therefore, an implementation, different with "(*webPageDownloadTestSample).downloadSample" is needed here.
	return res.ui.LeftClick(nodewith.Name(mf.fileName).Role(role.Link))
}
...
func (sf *suspiciousFileTest) downloadSample(res *webPageDownloadTestResource) uiauto.Action {
	// Only download the suspicious file by directly left-click on the link will show the expected notification.
	// Therefore, an implementation, different with "(*webPageDownloadTestSample).downloadSample" is needed here.
	return res.ui.LeftClick(nodewith.Name(sf.fileName).Role(role.Link))
}
...
func cancelDownloading(res *webPageDownloadTestResource, file string) uiauto.Action {
	return func(ctx context.Context) (retErr error) {
		...
		return uiauto.Combine("cancel downloading",
			...
			res.ui.LeftClick(download.NameStartingWith(fmt.Sprintf("Remove %s", file)).Role(role.Cell)),
