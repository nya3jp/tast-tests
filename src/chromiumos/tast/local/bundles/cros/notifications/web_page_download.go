...
func (bf *bigFile) verify(res *webPageDownloadTestResource) action.Action {
	if !bf.expectedProgressShown {
		return uiauto.Combine("ensure notification doesn't appear",
			res.ui.EnsureGoneFor(nodewith.Name(bf.expectedNotification).HasClass("Label"), 10*time.Second),
			res.kb.AccelAction("Ctrl+j"),
		)
	}
	return func(ctx context.Context) error {
