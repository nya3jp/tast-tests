...
func Printer(ctx context.Context, s *testing.State) {
	...
	s.Log("Validates that a printer can be installed.")
	...
	s.Log("Verifies the error for a failure in lpadmin.")
	if result, err := d.CupsAddManuallyConfiguredPrinter(
	...
	s.Log("Validates that malformed PPDs are rejected.")
	...
	s.Log("Attempts to add an unreachable autoconfigured printer.")
