...
func PacketCapture(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		// It'll be set to false if the packet capture operation will terminate itself due to an error or
		// reaching maximum file size limit.
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			for i := 0; i < param.numberOfCaptures; i++ {
				fileName := fmt.Sprintf("test%d.pcap", i)
				// Create output file for packet capture operation.
				of, err := ioutil.TempFile("", fileName)
				if err != nil {
			...
			// A notification must be shown after packet captures starts successfully.
