...
func PacketCapture(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if _, err = ash.WaitForNotification(ctx, tconn, timeout, ash.WaitIDContains(notificationID)); err != nil {
			...
			if ash.WaitUntilNotificationGone(ctx, tconn, timeout, ash.WaitIDContains(notificationID)) != nil {
				s.Error("Notification isn't gone after stopping packet captures")
			}
