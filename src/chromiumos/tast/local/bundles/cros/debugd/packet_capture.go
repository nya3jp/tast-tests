...
func PacketCapture(ctx context.Context, s *testing.State) {
	...
	for _, param := range []struct {
		...
		s.Run(ctx, param.name, func(ctx context.Context, s *testing.State) {
			...
			if !checkPacketCaptureNotification(ctx, tconn) {
...
// checkPacketCaptureNotification returns true when the notification is visible.
func checkPacketCaptureNotification(ctx context.Context, tconn *chrome.TestConn) bool {
