...
func DrmTraceTool(ctx context.Context, s *testing.State) {
	...
	var testCategories uint32 = drmTraceCategoryCORE | drmTraceCategoryKMS
	testSetCategories(ctx, s, obj, testCategories, testCategories)

	var expectedDefaultCategories uint32 = drmTraceCategoryDRIVER | drmTraceCategoryKMS | drmTraceCategoryDP
	testSetCategories(ctx, s, obj, uint32(0), expectedDefaultCategories)
...
func testAnnotate(ctx context.Context, s *testing.State, obj dbus.BusObject, log string) {
	...
	bytes, err := ioutil.ReadFile(traceContentsPath)
	if err != nil {
...
func testSetCategories(ctx context.Context, s *testing.State, obj dbus.BusObject, category, expected uint32) {
	...
	bytes, err := ioutil.ReadFile(traceMaskPath)
	if err != nil {
...
func getTraceBufferSize(s *testing.State) (int, error) {
	var size int
	sizeStr, err := readFileToString(s, bufferSizePath)
	if err == nil {
