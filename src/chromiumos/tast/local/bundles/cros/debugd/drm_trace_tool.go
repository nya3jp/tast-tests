...
// getTraceMaskAndSize reads the drm_trace mask and per-cpu buffer size from sysfs.
...
// setTraceMaskAndSize sets the DRMTrace categories and mask using the DRMTraceSet* D-Bus methods on the org.chromium.debugd interface.
