...
import (
	...
	"chromiumos/tast/local/bundles/cros/bluetooth/util"
...
func ToggleBluetoothFromQuickSettings(ctx context.Context, s *testing.State) {
	...
	if err := quicksettings.Hide(ctx, tconn); err != nil {
		s.Fatal("Failed to close the Quick Settings: ", err)
	}

	if err := quicksettings.ShowWithRetry(ctx, tconn, 5*time.Second); err != nil {
		s.Fatal("Failed to open the Quick Settings: ", err)
	}
	...
	if err := uiauto.Combine("Click the Quick Settings collapse button",
		ui.WaitUntilExists(quicksettings.CollapseButton),
		ui.LeftClick(quicksettings.CollapseButton),
	)(ctx); err != nil {
	...
	for i := 0; i < iterations; i++ {
		...
		if err := ui.LeftClick(util.FeaturePodIconButton)(ctx); err != nil {
