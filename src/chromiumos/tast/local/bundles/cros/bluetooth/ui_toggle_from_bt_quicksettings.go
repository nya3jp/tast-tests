...
func UIToggleFromBTQuicksettings(ctx context.Context, s *testing.State) {
	...
	bluetoothTurnOffButton := nodewith.NameContaining("Toggle Bluetooth. Bluetooth is on").HasClass("IconButton").Role(role.ToggleButton)
	// Bluetooth button in the quick setting menu, when Bluetooth is off.
	bluetoothTurnOnButton := nodewith.NameContaining("Toggle Bluetooth. Bluetooth is off").HasClass("IconButton").Role(role.ToggleButton)
