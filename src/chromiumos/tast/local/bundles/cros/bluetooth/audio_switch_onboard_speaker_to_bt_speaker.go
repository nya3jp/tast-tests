...
func init() {
	testing.AddTest(&testing.Test{
		Func:         AudioSwitchOnboardSpeakerToBTSpeaker,
...
// AudioSwitchOnboardSpeakerToBTSpeaker performs audio playback on internal-speaker
// before connecting bluetooth device and audio switching to bluetooth device once
// bluetooth device is connected to DUT.
func AudioSwitchOnboardSpeakerToBTSpeaker(ctx context.Context, s *testing.State) {
	...
	if len(adapters) == 0 {
		s.Fatal("Failed to find bluetooth adapters")
	}
	adapter := adapters[0]
