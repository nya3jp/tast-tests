...
type enableDisableTestParams struct {
	iterValue int
}
...
func EnableDisableBluetoothWithAudioPlay(ctx context.Context, s *testing.State) {
	...
	testOpt := s.Param().(enableDisableTestParams)
	...
	devName, err := audioPlayback(ctx, tconn)
	if err != nil {
	...
	for i := 1; i <= testOpt.iterValue; i++ {
		ui := uiauto.New(tconn)
		testing.ContextLogf(ctx, "Iteration %d/%d", i, testOpt.iterValue)
		...
		if err := ui.WaitForLocation(btDeviceNode)(ctx); err != nil {
			s.Fatalf("Failed to find %q in scanned BT devices list: %v", btHeadset, err)
		}

		if err := ui.LeftClick(btDeviceNode)(ctx); err != nil {
			s.Fatalf("Failed to left click and connect %q: %v", btHeadset, err)
		}
...
func audioPlayback(ctx context.Context, tconn *chrome.TestConn) (string, error) {
