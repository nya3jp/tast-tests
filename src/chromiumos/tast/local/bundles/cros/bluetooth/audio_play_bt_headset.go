...
// AudioPlayBtHeadset plays audio file over BT Headset.
func AudioPlayBtHeadset(ctx context.Context, s *testing.State) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		return nil
	}, &testing.PollOptions{Timeout: 35 * time.Second, Interval: 250 * time.Millisecond}); err != nil {
...
func audioPlay(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if err := testing.Sleep(ctx, 5*time.Second); err != nil {
		testing.ContextLog(ctx, "Error while waiting during sample time: ", err)
	}
