...
// OpenBluetoothSettingsFromQuickSettings tests that a user can succesfully
...
func OpenBluetoothSettingsFromQuickSettings(ctx context.Context, s *testing.State) {
	...
	if err := util.ShowDetailedView(ctx, tconn); err != nil {
	...
	if err := uiauto.Combine("Click the Bluetooth Settings button",
		ui.WaitUntilExists(util.DetailedViewSettingsButton),
		ui.LeftClick(util.DetailedViewSettingsButton),
	)(ctx); err != nil {
	...
	matcher := chrome.MatchTargetURL(util.BluetoothSubPageURL)
