...
func AddEpsonPrinter(ctx context.Context, s *testing.State) {
	...
	updater, err := chrome.NewComponentUpdater(ctx)
	if err != nil {
		s.Fatal("Failed to connect ComponentUpdaterService: ", err)
	}
	_, err = updater.LoadComponent(ctx, componentName, true /* mount */)
	if err != nil {
		s.Fatal("Failed to update epson-inkjet-printer-escpr: ", err)
	}
