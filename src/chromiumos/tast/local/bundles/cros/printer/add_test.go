...
func TestAddParams(t *testing.T) {
	...
	// TODO: promote printer.add* tests by merging codeWithProxy with code after b/240307209 is fixed
	codeWithProxy := genparams.Template(t, `{{ range . }} {
        Name: {{ .Name | fmt }},
        Val: &ippprint.Params{
                PPDFile:      {{ .PPDFile | fmt }},
                PrintFile:    {{ .PrintFile | fmt }},
	...
	genparams.Ensure(t, "proxy_add.go", codeWithProxy)
}
