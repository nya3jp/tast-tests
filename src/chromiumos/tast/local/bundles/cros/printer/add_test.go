...
// testS adds non-informational parametrized tests (one proxy, one regular)
// that use "to_print.pdf" for printing and file size instead of a golden file.
func testS(name, ppdFile string, expectedSize int, options ...string) base {
	return base{PrintFile: "to_print.pdf", Name: name, PPDFile: ppdFile, ExpectedSize: expectedSize, Options: options}
}

// iTestS adds informational parametrized tests (one proxy, one regular)
// that use "to_print.pdf" for printing and file size instead of a golden file.
func iTestS(name, ppdFile string, expectedSize int, options ...string) base {
...
func TestAddParams(t *testing.T) {
	code := genparams.Template(t, `{{ range . }} {
		...
		iTestS("hp_ljcolor", "printer_add_hp_ljcolor.ppd.gz", 154284),
