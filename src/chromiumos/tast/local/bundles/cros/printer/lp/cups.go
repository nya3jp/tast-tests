...
func PrinterStateMessage(ctx context.Context, printerName string) (string, error) {
	...
	testing.ContextLog(ctx, "lpstat output:\n"+string(out))
	return "", errors.New("failed to get the printer-state-message")
}

// PrinterNotResponding returns whether |printerName| is unreachable by
// evaluating its printer-state-message.
func PrinterNotResponding(ctx context.Context, printerName string) (bool, error) {
	const printerUnreachableStateMessage = "The printer is not responding."
	if stateMessage, err := PrinterStateMessage(ctx, printerName); err != nil {
		return false, err
	} else if strings.Contains(stateMessage, printerUnreachableStateMessage) {
		return true, nil
	}
