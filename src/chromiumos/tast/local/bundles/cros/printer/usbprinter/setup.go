...
func loadPrinterIDs(path string) (string, string, error) {
	...
	if err != nil {
		return "", "", errors.Wrapf(err, "failed to load contents of %s", path)
	}
	...
	dev, ok := m["device_descriptor"]
	if !ok {
		return "", "", errors.New(path + " does not contain device_descriptors")
	}

	devm := dev.(map[string]interface{})
	vid, ok1 := devm["idVendor"]
	pid, ok2 := devm["idProduct"]
	if !ok1 || !ok2 {
		return "", "", errors.New(path + " does not contain idVendor and idProduct")
	}

	vidStr := fmt.Sprintf("%04x", int(vid.(float64)))
	pidStr := fmt.Sprintf("%04x", int(pid.(float64)))
	return vidStr, pidStr, nil
}
...
func Start(ctx context.Context, descriptorsPath string,
