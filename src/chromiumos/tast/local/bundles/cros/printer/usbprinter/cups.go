...
// getPrinterName runs the lpstat command to search for a configured printer
// which has the an address that matches the lpstatMatcher format and has the
// same vid/pid as in devInfo. Return the name of the matching printer if found.
func getPrinterName(ctx context.Context, devInfo DevInfo) (name string, err error) {
	...
	for _, line := range lines {
		...
		// If we did not match on each of the sub-expressions then skip the line.
