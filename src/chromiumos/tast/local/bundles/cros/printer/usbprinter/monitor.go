...
func waitEvent(ctx context.Context, action, vid, pid string) error {
	...
	go func() {
		...
		for {
			...
			if text == "\n" {
				...
				// the chunk. Check the contents of the chunk to see if matched the
				// expected event.
				chunk := sb.String()
	...
	case <-ctx.Done():
		return errors.Wrap(ctx.Err(), "failed to get result")
	}
}
...
func waitLaunch(ctx context.Context, r io.Reader) error {
	...
	for sc.Scan() {
		...
		case <-time.After(2 * time.Second):
			return errors.New("timed out before receiving event")
		case <-ctx.Done():
			return errors.Wrap(ctx.Err(), "failed to start printer")
		default:
		}
