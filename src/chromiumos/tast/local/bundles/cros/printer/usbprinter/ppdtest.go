...
// RunPPDTest configures an IPP-over-USB printer using the virtual USB printer
// configured using the given descriptors and attributes. Once the printer has
// been automatically configured by CUPS the attributes of the generated PPD
// file are checked against the provided ppdAttributes map. If there are
// any differences in values between the generated PPD and ppdAttributes for the
// same key then the test will
func RunPPDTest(ctx context.Context, s *testing.State, descriptors, attributes string, ppdAttributes map[string]string) {
