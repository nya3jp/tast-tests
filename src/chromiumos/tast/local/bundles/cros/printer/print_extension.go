...
func init() {
	testing.AddTest(&testing.Test{
		...
		Contacts: []string{"batrapranav@google.com", "cros-peripherals@google.com"},
		Attr: []string{
			"group:mainline",
			"informational",
			"group:paper-io",
...
func PrintExtension(ctx context.Context, s *testing.State) {
	...
	if err := tconn.Call(ctx, nil, "chrome.autotestPrivate.updatePrinter", map[string]string{"printerName": printerID, "printerId": printerID, "printerUri": "socket://127.0.0.1/", "printerPpd": ppdFilePath}); err != nil {
	...
	if len(info.Capabilities.Printer) != 10 {
		s.Errorf("Unexpected printer capabilities: found %d elements", len(info.Capabilities.Printer))
	}
	...
	if len(request) != 168211 {
		s.Error("Unexpected print data size: ", len(request))
	}
