...
func PrintSettings(ctx context.Context, s *testing.State) {
	const (
	...
	if err := printpreview.SetDropdown(ctx, tconn, "Layout", "Landscape"); err != nil {
		s.Fatal("Failed to set layout dropdown: ", err)
	}

	if err := printpreview.SetPaperSize(ctx, tconn, "A4"); err != nil {
		s.Fatal("Failed to set paper size: ", err)
	}

	if err := printpreview.SetMargins(ctx, tconn, "Minimum"); err != nil {
		s.Fatal("Failed to set margins: ", err)
	}

	if err := printpreview.SetPagesPerSheet(ctx, tconn, "2"); err != nil {
		s.Fatal("Failed to set pages per sheet: ", err)
	}

	if err := printpreview.SetScaleDefault(ctx, tconn); err != nil {
		s.Fatal("Failed to set scale to default: ", err)
	}

	if err := printpreview.SetCheckboxState(ctx, tconn, "Headers and footers", false); err != nil {
		s.Fatal("Failed to set checkbox state: ", err)
	}

	if err := printpreview.SetScaleCustom(ctx, tconn, 71); err != nil {
		s.Fatal("Failed to set scale to custom: ", err)
	}
	...
	if err := printpreview.SetAdvancedSetting(ctx, tconn, "Job sheets", "Standard"); err != nil {
		s.Fatal("Failed to set advanced setting: ", err)
	}

	if err := printpreview.SetAdvancedSetting(ctx, tconn, "Page delivery", "same-order"); err != nil {
		s.Fatal("Failed to set advanced setting: ", err)
	}

	/*if err := printpreview.SetAdvancedSetting(ctx, tconn, "", "this should fail"); err != nil {
		s.Fatal("Failed to set advanced setting: ", err)
	}*/
