...
// return nil.
func printJobTimedOut(ctx context.Context, printerName, job, outFile string) error {
	// Check that |printerName| is reported as not responding.
	if printerReachable, err := lp.PrinterResponding(ctx, printerName, outFile); err != nil {
		return err
	} else if printerReachable {
		return errors.New("printer is unexpectedly reachable")
	}
...
func IPPPrinterConnectTimeout(ctx context.Context, s *testing.State) {
	...
	const uri = "ipp://localhost/printers/" + printerName
	...
	outFile := filepath.Join(s.OutDir(), "lpstat_output.txt")
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		return printJobTimedOut(ctx, printerName, job, outFile)
