...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:         []string{"ipp_no_copies.json"},
		Pre:          chrome.LoggedIn(),
	})
}

func PPDNoCopies(ctx context.Context, s *testing.State) {
	const descriptors = "/usr/local/etc/virtual-usb-printer/ippusb_printer.json"
	usbprinter.RunPPDTest(ctx, s, descriptors, s.DataPath("ipp_no_copies.json"), map[string]string{
