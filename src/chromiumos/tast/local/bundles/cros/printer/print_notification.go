...
func PrintNotification(ctx context.Context, s *testing.State) {
	const (
	...
	if !containsPrinterName(nearbyPrinters, "DavieV Virtual USB Printer (USB)") {
		s.Fatal("Virtual USB Printer not found in nearby printers, but is expected to be found")
	}

	// Press the save printer button
	err = clickSavePrinterButton(ctx, connect, "DavieV Virtual USB Printer (USB)")
	...
	if !containsPrinterName(savedPrinters, "DavieV Virtual USB Printer (USB)") {
		s.Fatal("Virtual USB Printer not found in saved printers, but is expected to be found")
	}
