...
func Run(ctx context.Context, s *testing.State, p *Params) {
	...
	diff, err := diff.Diff(cleanPSContents(string(expect)), cleanPSContents(string(request)))
	if err != nil {
	...
	if diff != "" {
		...
		s.Errorf("Printer output differs from expected: diff saved to %q (-want +got), output to %q", p.OutDiffFile, p.ExpectedFile)
	}
}
