...
func Print(ctx context.Context, s *testing.State) {
	...
	if _, err := ossettings.LaunchAtPageURL(ctx, tconn, cr, settingsPage, uiauto.Exists(entryFinder)); err != nil {
		s.Fatal("Failed to launch Settings page: ", err)
	}
	if err := uiauto.LeftClick(entryFinder)(ctx); err != nil {
	...
	if err := uiauto.LeftClick(savePrinterButton)(ctx); err != nil {
	...
	if err := uiauto.WithTimeout(time.Minute).WaitUntilExists(editPrinterButton)(ctx); err != nil {
	...
	if err := kb.Accel(ctx, "Ctrl+P"); err != nil {
		s.Fatal("Failed to type print preview shortcut: ", err)
	}
	...
	s.Log("Launch Print Management app and verify print job")
	pmRootnode, err := printmanagementapp.Launch(ctx, tconn)
	if err != nil {
		s.Fatal("Failed to launch Print Management app: ", err)
	}
	defer pmRootnode.Release(ctx)

	// Verify the History section of Print Management app is visible.
	if err := pmRootnode.WaitUntilDescendantExists(ctx, printmanagementapp.PrintManagementHistoryLabel, 20*time.Second); err != nil {
		s.Fatal("Failed to find History label: ", err)
	}
	// Verify at least one print job entry exists.
	if err := pmRootnode.WaitUntilDescendantExists(ctx, printmanagementapp.PrintManagementPrintJobEntry, 20*time.Second); err != nil {
		s.Fatal("Failed to find print job entry: ", err)
	}
}
