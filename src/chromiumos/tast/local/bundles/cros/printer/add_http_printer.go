...
func AddHTTPPrinter(ctx context.Context, s *testing.State) {
	run(ctx, s, httpTestPpdFile)
}

// run downloads the PPD and tries to install the printer using the dbus method.
func run(ctx context.Context, s *testing.State, ppdFile string) {
