...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:         []string{"printer_add_generic_printer_GenericPostScript.ppd.gz", "to_print.pdf", "printer_add_generic_printer_golden.ps"},
	})
}
...
func AddGenericPrinter(ctx context.Context, s *testing.State) {
	...
	if diff != "" {
		path := filepath.Join(s.OutDir(), "diff.txt")
		if err := ioutil.WriteFile(path, []byte(diff), 0644); err != nil {
			s.Error("Failed to dump diff: ", err)
		}
		s.Error("Read request has diff from the golden file, dumped at: ", path)
	}
}
