...
var classDrivers = map[string]string{
	"07": "skip",        // Printer.  Skip verification because the test deliberately changes it.
	"08": "usb-storage", // Storage
	"ff": "none",        // Vendor
}

func verifyPrinterInterfaces(ctx context.Context, dev usbprinter.DevInfo) error {
	devices, err := usbutil.ExpectedDevices(ctx)
	if err != nil {
	...
	for _, d := range devices {
		...
		for _, i := range d.Interfaces {
			...
			if expectedDriver == "skip" {
				continue
			}

			if i.Driver == nil {
				if expectedDriver != "none" {
...
func MultiFunctionPrinter(ctx context.Context, s *testing.State) {
	...
	usbprintertests.RunPrintJob(ctx, s, usbprintertests.PrintJobSetup{
