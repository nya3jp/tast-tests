...
// Package proxyippprint implements printing with IPP options.
package proxyippprint
...
func Run(ctx context.Context, s *testing.State, p *Params) {
	proxylpprint.RunWithOptions(ctx, s, p.PpdFile, p.PrintFile, p.ExpectedFile, p.OutDiffFile, optionsToString(p.Options))
}
