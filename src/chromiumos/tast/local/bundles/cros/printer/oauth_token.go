...
func init() {
	testing.AddTest(&testing.Test{
		...
		Data:         []string{"oauth_token_data.txt"},
...
func OAuthToken(ctx context.Context, s *testing.State) {
	...
	if files, err := fs.Glob(os.DirFS(tmpDir), "*"); err != nil {
		s.Fatal("Unable to read HTTP header values: ", err)
	} else {
		for _, file := range files {
			fileToRead := filepath.Join(tmpDir, file)
			if data, err := ioutil.ReadFile(fileToRead); err != nil {
				s.Fatal("Unable to read HTTP header file ", file, ": ", err)
			} else {
				stringData := string(data)
				if !(strings.Contains(stringData,
					"Authorization Bearer "+oauthTokenString+"\n")) {
