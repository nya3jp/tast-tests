...
func (s *Service) HidPreconnectedTouchscreenOnly(ctx context.Context, e *empty.Empty) (*empty.Empty, error) {
	...
	if err := oobeHelper.IsHidDetectionMouseNotDetected(ctx, oobeConn, tconn); err != nil {
		return &empty.Empty{}, errors.New("failed to detect that no mouse was detected")
	}

	// Uncomment this when the servo is working
