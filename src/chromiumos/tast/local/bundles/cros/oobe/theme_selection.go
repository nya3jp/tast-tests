...
// getExpectedTextColor executes JS code on the Theme Selection screen page to fetch current page header text color.
func getExpectedTextColor(ctx context.Context, oobeConn *chrome.Conn, darkMode bool) (string, error) {
...
// getNameOfSelectedTheme queries OobeApi to fetch selected value of a theme selection radio button.
// This call might return non-string value in case there is no selected value. We raise an error if it happens
// as we expect one of the theme modes to be always selected
func getNameOfSelectedTheme(ctx context.Context, oobeConn *chrome.Conn) (string, error) {
...
// waitForColorUpdate polls Theme Selection screen until new color schema is applied to the UI.
// It should break polling and return an error if we fail to execute JS calls, otherwise it waits until
// expected colors are present on the screen.
func waitForColorUpdate(ctx context.Context, oobeConn *chrome.Conn, darkMode bool) error {
	...
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		expectedColor, err := getExpectedTextColor(ctx, oobeConn, darkMode)
		if err != nil {
			// Break polling if we failed to query JS on the screen.
			return testing.PollBreak(errors.Wrap(err, "failed to get color of the text header"))
		}

		err = compareColorStrings(headerTextColor, expectedColor)
		return err
	}, pollOpts)
}

// compareColorStrings tries to unify color strings in the rgb(x,y,z) format by removing whitespaces from them
// and then compare given values. JS calls might return fetched properties with a different amount of whitespaces
// inside them, so we want to mitigate this risk here.
func compareColorStrings(headerColor, expectedColor string) error {
	headerColor = strings.ReplaceAll(headerColor, " ", "")
	expectedColor = strings.ReplaceAll(expectedColor, " ", "")
	if headerColor != expectedColor {
		return errors.Errorf("header color (%s) doesn't match expected color (%s)", headerColor, expectedColor)
	}
	return nil
}
...
func ThemeSelection(ctx context.Context, s *testing.State) {
	...
	selectedTheme, err := getNameOfSelectedTheme(ctx, oobeConn)
	if err != nil {
	...
	// We can combine testing for light and dark modes as they execute similar steps
	for _, theme := range []string{lightTheme, darkTheme} {
		...
		err = waitForColorUpdate(ctx, oobeConn, theme == darkTheme /*darkMode*/)
		if err != nil {
