...
func init() {
	testing.AddTest(&testing.Test{
		...
		Desc:         "Checks that HID screen is skipped on all form factors other than Chromebase, Chromebox and Chromebit",
		Contacts: []string{
...
func HidScreenSkipped(ctx context.Context, s *testing.State) {
	cr, err := chrome.New(ctx, chrome.DontDisableHIDScreenOnOobe(), chrome.NoLogin())
	if err != nil {
