...
type smokeEndToEndParams struct {
	isHpsEnabled bool
}
		...
		Params: []testing.Param{
			{
				...
				Val: smokeEndToEndParams{
					isHpsEnabled: false,
				},
			},
			{
				Name:    "hps",
				Fixture: "chromeLoggedInWithOobeDeferredLoginAndHpsScreenEnabled",
				Val: smokeEndToEndParams{
					isHpsEnabled: true,
				},
...
func SmokeEndToEnd(ctx context.Context, s *testing.State) {
	...
	testParams := s.Param().(smokeEndToEndParams)
	...
	if testParams.isHpsEnabled {
		s.Log("Wainting for the smart privacy protection screen")
