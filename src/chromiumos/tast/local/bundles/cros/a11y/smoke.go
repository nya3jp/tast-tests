...
func init() {
	testing.AddTest(&testing.Test{
		...
		Contacts: []string{
			"chromeos-sw-engprod@google.com", // Test author
...
func Smoke(ctx context.Context, s *testing.State) {
	...
	// var ltconn *chrome.TestConn

	// Abstract interfaces between chromeos-chrome and lacros-chrome
	...
	if ct == lacros.ChromeTypeChromeOS {
		...
		app = apps.App{apps.Chrome.ID, apps.Chrome.Name}
	}
	if ct == lacros.ChromeTypeLacros {
		...
		s.Log("artifactPath: " + artifactPath)
		...
		app = apps.App{apps.Lacros.ID, apps.Lacros.Name}
	...
	if ct == lacros.ChromeTypeChromeOS {
		topLevelWindow = nodewith.Role(role.Window).ClassName("BrowserFrame")
	} else {
	...
	rootWebArea := nodewith.Role("rootWebArea").Ancestor(topLevelWindow)
	...
	closeButton := nodewith.ClassName("FrameCaptionButton").Name("Close").Role(role.Button)
