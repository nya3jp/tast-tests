...
func SpokenFeedback(ctx context.Context, s *testing.State) {
	...
	if err := a11y.PressKeysAndConsumeExpectations(ctx, sm, []string{jumpToLauncher}, []a11y.SpeechExpectation{a11y.NewStringExpectation("Launcher"), a11y.NewStringExpectation("Button"), a11y.NewStringExpectation("Shelf"), a11y.NewStringExpectation("Tool bar"), a11y.NewStringExpectation("Press Search plus Space to activate")}); err != nil {
		s.Error("Error when pressing keys and expecting speech: ", err)
	}
	if err := a11y.PressKeysAndConsumeExpectations(ctx, sm, []string{jumpToStatusTray}, []a11y.SpeechExpectation{a11y.NewRegexExpectation("Quick Settings*"), a11y.NewRegexExpectation("Press search plus left to access the notification center*"), a11y.NewStringExpectation("window")}); err != nil {
		s.Error("Error when pressing keys and expecting speech: ", err)
	}
}
