...
func SpokenFeedback(ctx context.Context, s *testing.State) {
	...
	if err := a11y.PressKeysAndExpectSpeech(ctx, tts, "Alt+Shift+L", []string{"Launcher", "Button", "Shelf", "Tool bar", "Press Search plus Space to activate"}); err != nil {
	...
	if err := tts.VerifyUtterances(ctx); err != nil {
		s.Fatal("Failed to verify utterances: ", err)
	}
}
