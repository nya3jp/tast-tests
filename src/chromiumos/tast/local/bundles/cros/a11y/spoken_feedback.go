...
func SpokenFeedback(ctx context.Context, s *testing.State) {
	...
	err = cvconn.DoCommand(ctx, "showOptionsPage")
	if err != nil {
		s.Fatal("Failed to perform the showOptionsPage ChromeVox command: ", err)
	}
	err = tts.ExpectSpeech(ctx, []string{"tab created", "ChromeVox Options"})
	if err != nil {
	...
	err = ew.Accel(ctx, "Alt+Shift+L")
	if err != nil {
		s.Fatal("Error when pressing Alt+Shift+L: ")
	}
	err = tts.ExpectSpeech(ctx, []string{"Launcher", "Button", "Shelf", "Tool bar"})
	if err != nil {
		s.Fatal("Failed to verify speech from the Google TTS engine: ", err)
	}

	// Press Alt+Shift+S, which moves ChromeVox focus to the shelf.
	err = ew.Accel(ctx, "Alt+Shift+S")
	if err != nil {
		s.Fatal("Error when pressing Alt+Shift+S: ")
	}
	err = tts.ExpectSpeech(ctx, []string{"Quick Settings, Press search plus left to access the notification center., window"})
	if err != nil {
		s.Fatal("Failed to verify speech from the Google TTS engine: ", err)
	}
}
