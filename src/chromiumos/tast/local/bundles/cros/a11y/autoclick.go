...
func Autoclick(ctx context.Context, s *testing.State) {
	...
	moveMouseToStableLocation := func(node *ui.Node, name string) {
		if err := node.WaitLocationStable(ctx, &testing.PollOptions{Interval: 1 * time.Second, Timeout: 10 * time.Second}); err != nil {
			s.Fatalf("Failed to wait for %s to have a stable location", name)
		}
		...
		if err := mouse.Move(ctx, tconn, node.Location.CenterPoint(), 0); err != nil {
			s.Fatalf("Failed to move the mouse to %s", name)
		}
	}
	...
	moveMouseToStableLocation(scroll, "autoclick scroll button")
	...
	moveMouseToStableLocation(leftClick, "autoclick left click button")
	...
	moveMouseToStableLocation(yesButton, "autoclick confirmation dialog yes button")
