...
func Autoclick(ctx context.Context, s *testing.State) {
	...
	defer func() {
		...
		yes, err := ui.FindWithTimeout(ctx, tconn, ui.FindParams{
			Name:      "Yes",
		...
		defer yes.Release(ctx)

		if err := mouse.Move(ctx, tconn, yes.Location.CenterPoint(), 0); err != nil {
	...
	// Autoclicks is in left click mode once the scroll view disappears.
