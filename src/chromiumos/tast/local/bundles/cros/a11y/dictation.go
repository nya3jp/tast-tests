...
func Dictation(ctx context.Context, s *testing.State) {
	...
	firstCandidate := nodewith.Attribute("value", "hello").Role(role.TextField).HasClass("myTextArea").Onscreen()
	secondCandidate := nodewith.Attribute("value", "Hello").Role(role.TextField).HasClass("myTextArea").Onscreen()
	firstErr := uiauto.Combine("Verify text input",
		ui.WaitUntilExists(firstCandidate),
	)(ctx)
	secondErr := uiauto.Combine("Verify text input",
		ui.WaitUntilExists(secondCandidate),
	)(ctx)
	if firstErr != nil && secondErr != nil {
		s.Fatal("Failed to verify text input: ", firstErr, secondErr)
	}
}
