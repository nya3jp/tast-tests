...
func init() {
	testing.AddTest(&testing.Test{
		Func:         Scan,
		Desc:         "Tests that a scan can be performed using the DocumentScan API",
...
func Scan(ctx context.Context, s *testing.State) {
	...
	params = ui.FindParams{Role: ui.RoleTypeTextField}
	scanCompleteText, err := ui.FindWithTimeout(ctx, tconn, params, 10*time.Second)
	if err != nil {
		s.Fatal("Failed to find scan complete node: ", err)
	}
	defer scanCompleteText.Release(cleanupCtx)
	...
	params = ui.FindParams{Role: ui.RoleTypeImage}
	imageNode, err := ui.FindWithTimeout(ctx, tconn, params, 5*time.Second)
	if err != nil {
		s.Fatal("Failed to find scanned image: ", err)
	}
	defer imageNode.Release(cleanupCtx)
