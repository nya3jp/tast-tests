...
func ScrollUpAndDown(ctx context.Context, s *testing.State) {
	...
	triView := nodewith.ClassName("TriView").Ancestor(calendarView).Nth(0)
	closeButtonContainer := nodewith.ClassName("View").Ancestor(triView).Nth(0)
	closeButton := nodewith.ClassName("IconButton").Ancestor(closeButtonContainer).Nth(0)
	if err := ui.LeftClick(closeButton)(ctx); err != nil {
		s.Fatal("Failed to click the close button in calendar view bubble: ", err)
	}
}
