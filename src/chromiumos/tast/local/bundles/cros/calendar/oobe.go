...
func Oobe(ctx context.Context, s *testing.State) {
	...
	// Here a small amount (5) of pixel is added to the top of the scroll view each time in the loop to click on the Monday cells.
	...
	for i := 0; i < findCellTimes; i++ {
		s.Logf("Moving towards to the Monday cells (iteration %d of %d)", i+1, findCellTimes)
		cellPositionY += 5
		...
		if err := mouse.Click(tconn, firstMondayDateCellPt, mouse.LeftButton)(ctx); err != nil {
			s.Fatal("Failed to click a Monay date cell: ", err)
		}
		if found, err := ui.IsNodeFound(ctx, eventCloseButtonContentView); err != nil {
			...
			s.Fatal("Should not open event list after clicking on a Monay date cell: ", err)
			break
		}
	}
