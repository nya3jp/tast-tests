...
func RobloxACUITILogin(ctx context.Context, s *testing.State) {
	const (
	...
	testutil.PerformTest(ctx, s, appPkgName, appActivity, func(params testutil.TestParams) error {
		...
		if err := uiauto.Combine("Confirm launch",
			uda.WithTimeout(time.Minute*3).WaitUntilExists(uidetection.Word("Privacy")),
		)(ctx); err != nil {
