...
// given heuristics.
func GetCoords(ctx context.Context, tconn *chrome.TestConn, bounds coords.Rect, widthHeuristic, heightHeuristic float64) (coords.Point, error) {
	...
	return coords.Point{int(float64(bounds.Width) * relativeWidth), int(float64(bounds.Height) * relativeHeight)}, nil
}
