...
func AmongusGpuBenchmark(ctx context.Context, s *testing.State) {
	...
	// Data will include sloth picture, settings button picture, and "x" button picture.
	...
	testutil.PerformTest(ctx, s, appPkgName, appActivity, func(params testutil.TestParams) error {
		...
		if err := uiauto.Combine("Load GPU Benchmark",
			...
			action.IfSuccessThen(
				uda.WithTimeout(time.Second*45).WaitUntilExists(uidetection.TextBlock([]string{"Accept"})),
				uda.WithScreenshotStrategy(uidetection.ImmediateScreenshot).Tap(uidetection.TextBlock([]string{"Accept"})),
			),

			action.IfSuccessThen(
				uda.WithTimeout(time.Second*60).WaitUntilExists(uidetection.TextBlock([]string{"Understand"})),
				uda.Tap(uidetection.TextBlock([]string{"Understand"})),
			),

			action.Sleep(time.Second*30),
			action.IfSuccessThen(
				uda.WithScreenshotStrategy(uidetection.ImmediateScreenshot).WaitUntilExists(uidetection.CustomIcon(s.DataPath("amongus-dob-ok-button.png"))),
				uda.WithScreenshotStrategy(uidetection.ImmediateScreenshot).Tap(uidetection.Word("OK").Nth(1)),
			),

			action.Sleep(time.Second*30),
			action.IfSuccessThen(
				uda.WithScreenshotStrategy(uidetection.ImmediateScreenshot).WaitUntilExists(uidetection.TextBlock([]string{"Offline"})),
				uda.WithScreenshotStrategy(uidetection.ImmediateScreenshot).Tap(uidetection.TextBlock([]string{"Offline"})),
			),

			// Google Play Games may pop-up after this.
			action.IfSuccessThen(
				uda.WithScreenshotStrategy(uidetection.ImmediateScreenshot).WaitUntilExists(uidetection.Word("CANCEL")),
				uda.WithScreenshotStrategy(uidetection.ImmediateScreenshot).Tap(uidetection.Word("CANCEL")),
			),

			// Identify and click "x" button to close announcements pop-up.
			action.IfSuccessThen(
				uda.WithScreenshotStrategy(uidetection.ImmediateScreenshot).WaitUntilExists(uidetection.CustomIcon(s.DataPath("amongus-announcements-close-button.png"))),
				uda.WithScreenshotStrategy(uidetection.ImmediateScreenshot).Tap(uidetection.CustomIcon(s.DataPath("amongus-announcements-close-button.png"))),
			),

			// Identify and click "Local".
			action.Sleep(waitForActiveInputTime),
			uda.WithScreenshotStrategy(uidetection.ImmediateScreenshot).Tap(uidetection.Word("LOCAL")),

			// Identify and click "Create Game".
			action.Sleep(waitForActiveInputTime),
			uda.WithScreenshotStrategy(uidetection.ImmediateScreenshot).Tap(uidetection.TextBlock([]string{"Create", "Game"})),

			// Poll created game loaded (wait until settings button appears).
			uda.WithScreenshotStrategy(uidetection.ImmediateScreenshot).WaitUntilExists(uidetection.CustomIcon(s.DataPath("amongus-in-game-settings-button.png"))),
		)(ctx); err != nil {
