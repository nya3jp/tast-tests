...
func RobloxRoyaleHighDiviniaPark(ctx context.Context, s *testing.State) {
	...
	// Royale High has loud music that might disturb my co-workers. This is not
	// important for the actual test.
	...
	testutil.PerformRobloxTest(ctx, s, username, password, "Royale High", "roblox-search-royale-high-game-icon.png", func(params testutil.RobloxTestParams) error {
		...
		uda := params.Uda.WithScreenshotStrategy(uidetection.ImmediateScreenshot)
		...
		// Collect FPS metrics for a minute.
		...
		if err := testutil.Benchmark(ctx, params.TestParams, gameBenchmarkTime, perfValues); err != nil {
			return errors.Wrap(err, "failed to benchmark Divinia Park")
		}
		...
		for i := 1; i <= idleMinutes; i++ {
			...
			if err := params.Kbd.TypeKey(ctx, input.KEY_SPACE); err != nil {
