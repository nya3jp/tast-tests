...
// MTBF007PlayYoutube4K play 4K/2K youtube video.
func MTBF007PlayYoutube4K(ctx context.Context, s *testing.State) {
	...
	var framedrops int
	framedrops, err = youtube.GetFrameDropsFromStatsForNerd(ctx, conn)
	if err != nil {
		s.Error(mtbferrors.New(mtbferrors.VideoGetFrmDrop, err))
	}
	if framedrops != 0 {
		s.Error(mtbferrors.New(mtbferrors.VideoFrmDrop, err, framedrops))
	}
