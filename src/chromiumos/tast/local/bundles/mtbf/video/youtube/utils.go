...
func PlayVideo(ctx context.Context, conn *chrome.Conn) error {
	if err := tryToSkipAds(ctx, conn); err != nil {
		return err
	}
...
// OpenStatsForNerd clicks on an option in context menu of VideoPlayer to display statistic information dialog
...
func OpenAndPlayVideo(ctx context.Context, cr *chrome.Chrome, url string) (*chrome.Conn, error) {
	...
	if err := PlayVideo(ctx, conn); err != nil {
		return nil, mtbferrors.New(mtbferrors.ChromeExeJs, err)
	}
