...
func JoinRtcRoom(ctx context.Context, cr *chrome.Chrome, roomName string) (*chrome.Conn, error) {
	...
	if err := conn.EvalPromise(ctx, expr, nil); err != nil {
		...
		return nil, mtbferrors.New(mtbferrors.ChromeExeJs, err, expr)
	}
	...
	if err := dom.WaitForDocumentReady(ctx, conn); err != nil {
		conn.CloseTarget(ctx)
		return nil, mtbferrors.New(mtbferrors.VideoDocLoad, err)
	}
	...
	if roomSelfOnly(ctx, conn) {
		conn.Close()
		conn.CloseTarget(ctx)
		return nil, mtbferrors.New(mtbferrors.VideoRoomEmpty, nil)
	}

	if roomFull(ctx, conn) {
		conn.Close()
		conn.CloseTarget(ctx)
		return nil, mtbferrors.New(mtbferrors.VideoRoomFull, nil)
	}
