...
func downloadFile(ctx context.Context, s *testing.State, filePath string, url string) error {
...
func getDownloadStatusGap(ctx context.Context, s *testing.State, outputLen int) int {
	statusGap := -1
	statusGapStr, ok := s.Var("tc58.wifi.dl.status.gap")
	var err error
	...
	if ok {
		...
		if err != nil {
			...
			s.Log(mtbferrors.New(mtbferrors.OSVarRead, err, "tc58.wifi.dl.status.gap"))
		}
	} else {
		s.Log("tc58.wifi.dl.status.gap is not set. Use default value")
	}
