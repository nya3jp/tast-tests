...
func init() {
	testing.AddTest(&testing.Test{
		...
		Vars: []string{
			"wifi.802.11ac.wave2.ssid",
			"wifi.802.11ac.wave2.pwd",
			"dut.id",
...
func MTBF068WifiConnect(ctx context.Context, s *testing.State) {
	...
	wifiSsid := common.GetVar(ctx, s, "wifi.802.11ac.wave2.ssid")
	wifiPwd := common.GetVar(ctx, s, "wifi.802.11ac.wave2.pwd")
