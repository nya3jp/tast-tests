...
// GetConnection get connection from existed CCA instance
func GetConnection(ctx context.Context, cr *chrome.Chrome, scriptPaths []string, outDir string) (*App, error) {
	tconn, err := cr.TestAPIConn(ctx)
	if err != nil {
		return nil, err
	}
	var windowURL string
	if mtbferr := tconn.EvalPromise(ctx, `CCAReady`, &windowURL); mtbferr != nil {
		testing.ContextLog(ctx, mtbferr)
		return nil, err
	}

	testing.ContextLogf(ctx, "windowURL =%s", windowURL)
	// The expected windowURL is returned as:
	//		views/main.html
	// Or:
	//		views/main.html?...
...
// CheckElementExist returns an error if whether an element with given selector exists is not expected.
func (a *App) CheckElementExist(ctx context.Context, selector string, expected bool) error {
	var actual bool
	err := a.conn.Eval(ctx, selector, &actual)
	if err != nil {
		return err
	} else if actual != expected {
		return errors.Errorf("unexpected an element with given selector existence: got %v, want %v", actual, expected)
	}
	return nil
}
...
func (a *App) GoToGallery(ctx context.Context) error {
	testing.ContextLog(ctx, "Click on gallery button")
	if err := a.ClickWithSelector(ctx, "#gallery-enter"); err != nil {
		return err
	}
	return nil
}
