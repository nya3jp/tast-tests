...
// MTBF020CaptureUnittests runs Chrome's capture_unittests.
func MTBF020CaptureUnittests(ctx context.Context, s *testing.State) {
	...
	if upstart.JobExists(shortCtx, "cros-camera") {
		...
		if err := upstart.EnsureJobRunning(shortCtx, "cros-camera"); err != nil {
			s.Fatal(mtbferrors.New(mtbferrors.CmrStart, err))
		}
	}
	...
	if err := os.MkdirAll(dataDir, 0755); err != nil {
		s.Fatal(mtbferrors.New(mtbferrors.VideoCreate, err, dataDir))
	}
	if err := fsutil.CopyFile(s.DataPath(testFile),
		filepath.Join(dataDir, testFile)); err != nil {
		s.Fatal(mtbferrors.New(mtbferrors.VideoCopy2, err, testFile))
	}
