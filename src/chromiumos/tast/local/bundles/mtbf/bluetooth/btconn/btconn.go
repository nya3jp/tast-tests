...
// 	.shadowRoot.querySelector("#enableBluetooth").click()`

const entableBtJs = btDom + `
...
//const clickBtDevice = btDeviceJs + `.click();`
...
// SwitchOn enable ChromeOS bluetooth function
func (c *BtConn) SwitchOn() error {
	c.s.Log("switchOn - entableBtJs: ", entableBtJs)
	...
	if err := c.cdpConn.Exec(c.ctx, entableBtJs); err != nil {
...
// SwitchOff disable ChromeOS bluetooth function
func (c *BtConn) SwitchOff() error {
	c.s.Log("SwitchOff - entableBtJs: ", entableBtJs)
	btStatus, err := c.getBtSettingStatus()
...
func (c *BtConn) getBtSettingStatus() (string, error) {
	...
	if err := conn.WaitForExprWithTimeout(ctx, btStatusDom+" != null", jsTimeout); err != nil {
		// c.s.Fatal("Failed to check Wifi enabled.", err)
