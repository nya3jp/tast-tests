...
func TestReport(t *testing.T) {
	...
	if !reflect.DeepEqual(report, expected) {
		t.Fatalf("Unexpected parsed result: got %v; want %v", report, expected)
	}

	expectedFailures := []string{"MathTest.Addition"}
	failures := report.Failures()
	if !reflect.DeepEqual(failures, expectedFailures) {
		t.Errorf("Unexpected failures: got %v; want %v", failures, expectedFailures)
	}
}
