...
// New creates a new fixture that can launch Lacros chrome with the given setup mode and
// Chrome options.
func New(mode SetupMode, fOpt chrome.OptionsCallback) testing.FixtureImpl {
	return &fixtureImpl{
		mode: mode,
		fOpt: fOpt,
	}
...
func init() {
	...
	testing.AddFixture(&testing.Fixture{
		...
		Impl: New(PreExist, func(ctx context.Context, s *testing.FixtState) ([]chrome.Option, error) {
			return nil, nil
		}),
	...
	testing.AddFixture(&testing.Fixture{
		...
		Contacts: []string{"hidehiko@chromium.org", "edcourtney@chromium.org"},
		Impl: New(Omaha, func(ctx context.Context, s *testing.FixtState) ([]chrome.Option, error) {
	...
	testing.AddFixture(&testing.Fixture{
		...
		Impl: New(Rootfs, func(ctx context.Context, s *testing.FixtState) ([]chrome.Option, error) {
			return []chrome.Option{
				chrome.EnableFeatures("LacrosSupport"),
