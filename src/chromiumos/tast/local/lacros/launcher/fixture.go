...
func init() {
	...
	// testing.AddFixture(&testing.Fixture{
	// 	Name:     "lacrosLoggedInStartedFromScript",
	// 	Desc:     "Lacros Chrome from one of the sources selected with the vars -lacrosSelection and -lacrosDeployedBinary: (1) stateful, (2) rootfs and (3) external data source.",
	// 	Contacts: []string{"hyungtaekim@chromium.org", "lacros-team@google.com"},
	// 	Impl: NewStartedByData(Runtime, func(ctx context.Context, s *testing.FixtState) ([]chrome.Option, error) {
	// 		// Do not add a Chrome option for "LacrosSupport" to launch Lacros from the python script
	// 		return nil, nil
	// 	}),
	// 	SetUpTimeout:    chrome.LoginTimeout + 7*time.Minute,
	// 	ResetTimeout:    chrome.ResetTimeout,
...
func (f *fixtureImpl) checkVars(ctx context.Context, s *testing.FixtState, opts ...chrome.Option) (SetupMode, []chrome.Option) {
	...
	case PreExist:
		...
		// "chrome.EnableFeatures("LacrosSupport")" should be used for launching from the Shelf UI, but not for the python launcher script.
