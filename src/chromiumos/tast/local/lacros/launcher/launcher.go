...
func LaunchLacrosChrome(ctx context.Context, p PreData) (*LacrosChrome, error) {
	...
	l.cmd = testexec.CommandContext(ctx, "/usr/local/bin/python3",
		append([]string{"/usr/local/bin/mojo_connection_lacros_launcher.py", "-s", mojoSocketPath, p.LacrosPath + "/chrome"}, args...)...)
