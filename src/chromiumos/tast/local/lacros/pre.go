...
func (p *preImpl) LaunchChrome(ctx context.Context, s *testing.State) interface{} {
	...
	if err := c.PrepareExtensions(ctx); err != nil {
		...
		mountCmd = testexec.CommandContext(ctx, binary_loc + "/chrome", fields...)
		...
		var devsess *cdputil.Session // DevTools session
		var err error
		if devsess, err = cdputil.NewSession(ctx, debuggingPortPath); err != nil {
			return err
		}
		_, err = devsess.CreateTarget(ctx, "about:blank")

		s.Log("asdf3-------------------------: ", devsess.DebugAddrPort)
		// mountCmd.Cmd.Process.Release()
		time.Sleep(9 * 1000000000)
		s.Log("asdf4-------------------------")

		// mountCmd = testexec.CommandContext(ctx, "disown")
		// if err := mountCmd.Run(); err != nil {
		// 	mountCmd.DumpLog(ctx)
		// 	return errors.Wrap(err, "failed to mount component")
		// }
