...
type preImpl struct {
	...
	devsess      *cdputil.Session // Debuging session for linux-chrome
...
func (p *preImpl) LaunchChrome(ctx context.Context, s *testing.State) interface{} {
	c := &chrome.Chrome{}
	if err := c.PrepareExtensions(ctx); err != nil {
		...
		fields := strings.Fields("--ozone-platform=wayland --no-sandbox --no-first-run --user-data-dir=" + binary_loc + "/user_data --lang=en-US --breakpad-dump-location=" + binary_loc )
		fields = append(fields, "--whitelisted-extension-id=" + c.TestExtID())
		fields = append(fields, "--load-extension="+strings.Join(c.ExtDirs(), ","))
		fields = append(fields, "--remote-debugging-port=0")
		fields = append(fields, "about:blank")



		p.cmd = testexec.CommandContext(ctx, binary_loc + "/chrome", fields...)
		    p.cmd.Cmd.Env = os.Environ()
		    p.cmd.Cmd.Env = append(p.cmd.Cmd.Env, "XDG_RUNTIME_DIR=/run/chrome")
		    p.cmd.Cmd.Env = append(p.cmd.Cmd.Env, "LD_LIBRARY_PATH=" + binary_loc)
		if err := p.cmd.Cmd.Start(); err != nil {
			p.cmd.DumpLog(ctx)
			return errors.Wrap(err, "failed to mount component")
		}

		debuggingPortPath := binary_loc + "/user_data/DevToolsActivePort"
		var err error
		if p.devsess, err = cdputil.NewSession(ctx, debuggingPortPath); err != nil {
			return err
		}
		return nil
}
...
func (p *preImpl) cleanUp(ctx context.Context, s *testing.State) {
	...
	if p.cmd != nil {
		if err := p.cmd.Cmd.Process.Kill(); err != nil {
			s.Error("Failed to kill linux-chrome: ", err)
		}
		p.cmd = nil
	}
}
...
func (p *preImpl) buildPreData(ctx context.Context, s *testing.State) PreData {
	...
	return PreData{p.cr, p.tconn, p.devsess}
}
