...
var noCleanup = func(context.Context) error { return nil }
...
func SetupPerfTest(ctx context.Context, tconn *chrome.TestConn, name string) (CleanupCallback, error) {
	...
	if err := sup.Check(ctx); err != nil {
		return cleanup, errors.Wrap(err, "failed to setup GpuCUJ power test environment")
	}

	if err := quicksettings.ToggleSetting(ctx, tconn, quicksettings.SettingPodDoNotDisturb, true); err != nil {
		return cleanup, errors.Wrap(err, "failed to disable notifications")
	}
...
func SetupCrosTestWithPage(ctx context.Context, pd launcher.PreData, url string) (*chrome.Conn, CleanupCallback, error) {
	...
	if err != nil {
		return nil, noCleanup, errors.Wrap(err, "failed to open new tab")
	}
...
func SetupLacrosTestWithPage(ctx context.Context, pd launcher.PreData, url string) (
	*chrome.Conn, *chrome.TestConn, *launcher.LacrosChrome, CleanupCallback, error) {
	...
	if err != nil {
		return nil, nil, nil, noCleanup, errors.Wrap(err, "failed to launch lacros-chrome")
	}
	...
	if err != nil {
		return nil, nil, nil, cleanup, errors.Wrap(err, "failed to connect to test API")
	}
	...
	if err != nil {
		return nil, nil, nil, cleanup, errors.Wrap(err, "failed to open new tab")
	}
	cleanup = CombineCleanup(ctx, cleanup, func(ctx context.Context) error {
		conn.CloseTarget(ctx)
		conn.Close()
	...
	if err := CloseAboutBlank(ctx, l.Devsess); err != nil {
		return nil, nil, nil, cleanup, errors.Wrap(err, "failed to close about:blank tab")
	}
