...
// StartSludge will start the upstart process wilco_dtc and wait until the VM is
...
func StartSludge(ctx context.Context, startProcesses bool) error {
	...
	server, err := NewStartupListenerServer(wilcoVMStartupPort)
	defer server.Stop()
	if err != nil {
	...
	if err := server.WaitReady(ctx); err != nil {
		return errors.Wrap(err, "timedout waiting for server to start")
	}

	return nil
}

// StopSludge will stop the upstart process wilco_dtc
...
// StartWilcoSupportDaemon will start the upstart process wilco_dtc_supportd.
...
// StopWilcoSupportDaemon will stop the upstart process wilco_dtc_supportd.
