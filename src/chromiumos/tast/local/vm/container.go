...
// One must listen on cicerone Dbus signals to know the creation is done.
...
func NewContainerCreationWatcher(ctx context.Context, cont *Container) (*ContainerCreationWatcher, error) {
	// Listens on any cicerone signals.
	watcher, err := dbusutil.NewSignalWatcherForSystemBus(ctx, ciceroneDbusMatchSpec(""))
	if err != nil {
		return nil, err
	}
...
// WaitForDownloadingBegin waits for the downloading phase of the container to begin.
func (c *ContainerCreationWatcher) WaitForDownloadingBegin(ctx context.Context) error {
...
// WaitForDownloadingComplete waits for the downloading phase of the container to complete.
func (c *ContainerCreationWatcher) WaitForDownloadingComplete(ctx context.Context) error {
	spec := ciceroneDbusMatchSpec("LxdContainerDownloading")
	sigResult := &cpb.LxdContainerDownloadingSignal{}
	for {
		if err := waitForDbusSignal(ctx, c.watcher, &spec, sigResult); err != nil {
		...
		if c.isWatchingContainer(sigResult.VmName, sigResult.ContainerName, sigResult.OwnerId) {
			if sigResult.DownloadProgress == 100 {
				return nil
			}
		}
	}
