...
func (c *Container) WriteFile(ctx context.Context, filePath, fileContent string) error {
	cmd := c.Command(ctx, "sh", "-c", fmt.Sprintf("echo -n %s > %s", shutil.Escape(fileContent), filePath))
	if err := cmd.Run(); err != nil {
		cmd.DumpLog(ctx)
...
func (c *Container) RemovePath(ctx context.Context, path string) error {
	return c.Command(ctx, "sh", "-c", shutil.EscapeSlice([]string{"sudo", "rm", "-rf", path})).Run(testexec.DumpLogOnError)
}
