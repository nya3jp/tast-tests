...
func CreateDefaultVM(ctx context.Context, dir, user string, t ContainerType, artifactPath string) (*VM, error) {
	...
	if err != nil {
		return nil, errors.Wrap(StopConcierge(ctx), err.Error())
	}
	...
	if t == Tarball {
		if err := vmInstance.ShareDownloadsPath(ctx, "crostini", false); err != nil {
			return nil, errors.Wrap(err, errors.Wrap(StopConcierge(ctx), "failed to share container image with VM").Error())
		}
	}
