...
func DownloadStagingContainer(ctx context.Context, debianVersion ContainerArchType) (string, error) {
	...
	pathRe, err := regexp.Compile(fmt.Sprintf("images/debian/%s/%s/default/[^\\/]*/([^\\/\"]*)", debianVersionString, componentArch))
	if err != nil {
		return "", errors.Wrap(err, "unable to compile staging container path regexp")
	}

	allPaths := pathRe.FindAllStringSubmatch(string(imagesJSON.String()), -1)
