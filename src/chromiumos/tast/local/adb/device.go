...
type Device struct {
	// Serial is used to distinguish the specific device.
	Serial string
...
func Devices(ctx context.Context) ([]*Device, error) {
	...
	for _, line := range strings.Split(string(output), "\n") {
		...
		if len(fields) < 2 {
			testing.ContextLogf(ctx, "'adb devices -l' ran into unexpected line: %q", line)
			continue
		}
		...
		for _, field := range fields[2:] {
			...
			} else if strings.HasPrefix(field, "usb:") {
				device.USB = strings.TrimPrefix(field, "usb:")
			}
		}
...
// Command runs an ADB command on the specified device.
...
func (d *Device) State(ctx context.Context) (State, error) {
	out, err := d.Command(ctx, "get-state").CombinedOutput()
	if err != nil {
		if out != nil && strings.Contains(string(out), "device offline") {
			return StateOffline, nil
		}
		return StateUnknown, errors.Wrap(err, "failed to get device state")
	}
