...
func (di *devInfo) isKeyboard() bool {
	...
	return di.path != "" && di.hasBit(evGroup, uint16(EV_KEY)) &&
		di.hasBit(keyGroup, uint16(KEY_BRIGHTNESSDOWN)) && di.hasBit(keyGroup, uint16(KEY_Q)) &&
		di.hasBit(keyGroup, uint16(KEY_SPACE)) && di.hasBit(keyGroup, uint16(KEY_LEFTSHIFT)) && di.hasBit(keyGroup, uint16(KEY_RIGHTSHIFT))
}
