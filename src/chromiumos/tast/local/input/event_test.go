...
func TestEventWriterSuccess(t *testing.T) {
	...
	kw := KeyboardEventWriter{
		RawEventWriter: RawEventWriter{&b, func() time.Time { return now }},
		fast:           true}

	if err := kw.Event(EV_KEY, KEY_A, 1); err != nil {
		t.Error("Writing key down failed: ", err)
	}
	if err := kw.Sync(); err != nil {
		t.Error("Writing first sync failed: ", err)
	}
	if err := kw.Event(EV_KEY, KEY_A, 0); err != nil {
...
func TestEventWriterWriteError(t *testing.T) {
	...
	kw := KeyboardEventWriter{
		RawEventWriter: RawEventWriter{&b, time.Now},
		fast:           true}
	defer kw.Close()
