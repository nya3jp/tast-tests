...
func TestEventWriterSuccess(t *testing.T) {
	b := testBuffer{}
	now := time.Unix(1, 0)
	ew := KeyboardEventWriter{
		rw:   &RawEventWriter{&b, func() time.Time { return now }},
		fast: true}

	if err := ew.rw.Event(EV_KEY, KEY_A, 1); err != nil {
		t.Error("Writing key down failed: ", err)
	}
	if err := ew.rw.Sync(); err != nil {
		t.Error("Writing first sync failed: ", err)
	}
	if err := ew.rw.Event(EV_KEY, KEY_A, 0); err != nil {
...
func TestEventWriterTouch(t *testing.T) {
	...
	b := testBuffer{}
	now := time.Unix(5, 0)
	ew := MultiTouchEventWriter{
		rw:          &RawEventWriter{&b, func() time.Time { return now }},
		nextTouchID: touchID,
		touchInfoX:  EventAbsInfo{0, 0, 1000, 0, 0, 0},
		touchInfoY:  EventAbsInfo{0, 0, 1000, 0, 0, 0}}
	...
	tw.absPressure = p
	tw.touchMajor = tMajor
	tw.touchMinor = tMinor
	tw.MoveTo(x, y)
	tw.End()
