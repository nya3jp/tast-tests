...
// PressButton presses
func (gw *GamepadEventWriter) PressButton(ctx context.Context, code EventCode) error {
	if err := gw.rw.Event(EV_KEY, code, 1); err != nil {
		return err
	}
	return gw.rw.Sync()
}

// ReleaseButton releases
func (gw *GamepadEventWriter) ReleaseButton(ctx context.Context, code EventCode) error {
	if err := gw.rw.Event(EV_KEY, code, 0); err != nil {
		return err
	}
	return gw.rw.Sync()
}
...
// MoveAxis moves
func (gw *GamepadEventWriter) MoveAxis(ctx context.Context, code EventCode, val int32) error {
	if err := gw.rw.Event(EV_ABS, code, val); err != nil {
		return err
	}
	return gw.rw.Sync()
}
