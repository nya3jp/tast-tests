...
func (mw *TouchscreenEventWriter) NewMultiTouchWriter(numberOfTouches int) (*TouchEventWriter, error) {
	if numberOfTouches < 1 || numberOfTouches > mw.maxTouches {
		return nil, errors.Errorf("touch count %d not in range [1, %d]", numberOfTouches, mw.maxTouches)
	}
...
func (mw *TouchscreenEventWriter) Width() TouchCoord {
	return mw.width
}

// Height returns the height of the touchscreen device, in touchscreen coordinates.
func (mw *TouchscreenEventWriter) Height() TouchCoord {
	return mw.height
}
...
func (stw *SingleTouchEventWriter) Move(x, y TouchCoord) error {
	if x >= stw.mw.width || y >= stw.mw.height {
		return errors.Errorf("coordinates (%d, %d) outside valid bounds (%d, %d)",
			x, y, stw.mw.width, stw.mw.height)
	}
...
func (tw *TouchEventWriter) initTouchState(numberOfTouches int) {
	...
	defaultPressure := int32(tw.mw.maxPressure / 4)

	tw.touches = make([]TouchState, numberOfTouches)
	...
	for i := 0; i < numberOfTouches; i++ {
		...
		tw.mw.nextTouchID = (tw.mw.nextTouchID + 1) % int32(tw.mw.maxTrackingID)
	}
}
