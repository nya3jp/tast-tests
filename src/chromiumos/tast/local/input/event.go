...
// Multi touch can be achieved by using multiple TouchEventWriter instances.
...
// TouchAt generates a touch event at x and y touchscreen coordinates. Calling
// the same TouchEventWriter multiple times generates a continuos movement since
// the event has the same multitouch Id. x and y should be in touchscreen
// coordinates, not pixel coordiantes.
...
// End sends the "End of life" touch event to the device. The TouchEventWriter
// can be reused. If so, it will generate a new event with the same old
// multitouch Id. Regarless of how TouchEventWriter is used, this method should
// be called after using it, possibly with the "defer" statement.
