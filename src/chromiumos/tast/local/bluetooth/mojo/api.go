...
const (
	// Unavailable meants device does not have access to Bluetooth.
	Unavailable BluetoothSystemState = 0
	// Disabled means Bluetooth is turned off.
	Disabled BluetoothSystemState = 1
	// Disabling means Bluetooth is in the process of turning off.
	Disabling BluetoothSystemState = 2
	...
	Enabling BluetoothSystemState = 4

	// CannotModifyBluetooth means Bluetooth cannot be turned on/off, and devices cannot be connected. E.g.,
	// the current session may belong to a secondary user, or the screen is locked
	CannotModifyBluetooth BluetoothModificationState = 0

	// CanModifyBluetooth means Bluetooth settings can be modified as part of the current session.
	CanModifyBluetooth BluetoothModificationState = 1
)
...
func PollForBluetoothSystemState(ctx context.Context, m chrome.JSObject, exp BluetoothSystemState) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		prop, err := getSystemProperties(ctx, m)
		if err != nil {
