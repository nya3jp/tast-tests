...
// Devices creates an Device for each of the remote bluetooth devices in the system.
...
func DisconnectAllDevices(ctx context.Context) error {
	...
	for _, d := range devices {
		...
		if err != nil {
			return errors.Wrap(err, "failed to get connected property")
		}

		paired, err := d.Paired(ctx)
		if err != nil {
			return errors.Wrap(err, "failed to get paired property")
		}
...
// Disconnect disconnects the remote device.
