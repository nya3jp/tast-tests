...
// Package snapshot provides functions that store/restore the snapshot(i.e. the content) of important files during integration test. It also provides a system-level (i.e. set up by tast framework) snapshot manager for test items to use.
package snapshot
