...
func Get(ctx context.Context, path string, prop string) (string, error) {
	...
	// An exit code of 1 simply means that the value was not present in the
	// model.yaml, and cros_config returns an empty string.
	if !ok || (status.ExitStatus() != 0 && status.ExitStatus() != 1) {
		return "", err
	}
