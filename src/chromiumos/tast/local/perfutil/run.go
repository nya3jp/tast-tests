...
// RunFunc is the function to be used for RunMultiple that run subtests.
type RunFunc func(context.Context, string, *Runner)
...
func NewRunner(br *browser.Browser) *Runner {
	return &Runner{br: br, pv: NewValues(), Runs: DefaultRuns, RunTracing: (br != nil), Scenario: nil, Store: nil}
}
...
// RunSubtest is the callback that calls scenario and store functions. This function is to be
// called by tests or external helper functions.
func (r *Runner) RunSubtest(ctx context.Context) error {
	hists, err := r.Scenario(ctx)
	if err != nil {
		return errors.Wrap(err, "failed to run the test scenario")
	}

	if !r.br.IsTracing() {
		if err := r.Store(ctx, r.pv, hists); err != nil {
...
func (r *Runner) RunMultiple(ctx context.Context, run RunFunc, name string, scenario ScenarioFunc, store StoreFunc) error {
	r.Scenario = scenario
	r.Store = store
	...
	for i := 0; i < r.Runs; i++ {
		run(ctx, fmt.Sprintf("%s-%d", runPrefix, i), r)
	}
	if !r.RunTracing {
		return nil
	}
	...
	run(ctx, fmt.Sprintf("%s-tracing", runPrefix), r)
