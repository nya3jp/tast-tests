...
// CheckDevice sends an HTTP request for url through the ippusb_bridge socket that
// matches devInfo.  Returns nil if a response is received regardless of the HTTP
// status code or body contents.
func CheckDevice(ctx context.Context, devInfo usbprinter.DevInfo, url string) error {
	socket := SocketPath(devInfo)
