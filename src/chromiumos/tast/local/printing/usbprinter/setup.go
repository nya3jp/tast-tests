...
// UdevEventOnStop controls whether or not `PrinterInstance.Stop()`
// returns an error absent a udev event upon stoppage of
// `virtual-usb-printer`.
type UdevEventOnStop int

const (
	// RequireUdevEvent causes `PrinterInstance.Stop()` to
	// expect a udev event on printer stoppage.
	RequireUdevEvent UdevEventOnStop = iota

	// IgnoreUdev allows `PrinterInstance.Stop()` to ignore
	// udev altogether, opting only to terminate and wait for
	// the `virtual-usb-printer` process.
	IgnoreUdev
)
...
//	been removed _and_
// *	`expectUdevEvent` is `RequireUdevEvent`.
//
// This method is idempotent.
func (p *PrinterInstance) Stop(ctx context.Context, expectUdevEvent UdevEventOnStop) error {
	if p.cmd == nil {
		return nil
	}
	defer func() {
	...
	if expectUdevEvent == RequireUdevEvent {
