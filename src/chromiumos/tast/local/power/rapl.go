...
func (rapl *RAPLValues) ReportPerfMetrics(perfValues *perf.Values, prefix string) {
	for name, value := range rapl.joules {
...
func (r *RAPLSnapshot) diffWithCurrentRAPL(resetStart bool) (*RAPLValues, error) {
	...
	ret := newRAPLValues()
	ret.durationInSecond = endTime.Sub(r.startTime).Seconds()

	for _, name := range zoneNames {
		ret.joules[name] = diffJoules(r.start.joules[name], end.joules[name], r.maxJoules)
	}
	if resetStart {
...
func readRAPLValues(dirsToParse []string) (*RAPLValues, time.Time, error) {
	...
	isZoneExpected := make(map[string]bool)
	for _, z := range zoneNames {
		isZoneExpected[z] = true
	}
	rapl := newRAPLValues()
