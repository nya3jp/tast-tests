...
func NewRAPLSnapshot() (*RAPLSnapshot, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrapf(err, "failed to stat %q directory", powercapDir)
	}
...
func diffJoules(start, end, max float64) float64 {
	// Catching a theoretical-possible overflow.
	if start > end {
...
func readRAPLFile(filepath string) (string, error) {
	...
	l, err := rd.ReadString('\n')
	if err != nil {
		return "", errors.Wrapf(err, "failed to read string from file %q", filepath)
	}
