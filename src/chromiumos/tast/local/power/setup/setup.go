...
const (
	// DoNotChangeNightLight indicates that Night Light should be left in the same state.
	DoNotChangeNightLight NightLightMode = iota
...
const (
	// DisableBluetooth indicates that bluetooth should be disabled.
	DisableBluetooth = iota
...
func PowerTest(ctx context.Context, c *chrome.TestConn, options PowerTestOptions) (CleanupCallback, error) {
	return Nested(ctx, "power test", func(s *Setup) error {
		...
		if options.Bluetooth == DisableBluetooth {
			s.Add(DisableBluetoothAdapters(ctx))
		}
