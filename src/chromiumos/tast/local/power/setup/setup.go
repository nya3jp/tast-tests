...
func New() (*Setup, CleanupCallback) {
	...
	return s, func(ctx context.Context) error {
		...
		cleanedUp = true
		s.cleanUp(ctx, &failed)
		if failed {
			return errors.New("some cleanup items failed, see logs")
		}
		return nil
	}
}
...
// Whether any failures occurred is returned via reference so that it can be
// updated by deferred calls, guaranteeing that each callback is run.
