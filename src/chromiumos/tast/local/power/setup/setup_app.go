...
func InstallApp(ctx context.Context, a *arc.ARC, apkDataPath string, pkg string) Result {
	if err := a.Install(ctx, apkDataPath); err != nil {
		return ResultFailed(errors.Wrapf(err, "failed to install apk %q", apkDataPath))
	}
...
func StartActivity(ctx context.Context, a *arc.ARC, pkg string, activityName string) Result {
	activity, err := arc.NewActivity(a, pkg, activityName)
	if err != nil {
		return ResultFailed(errors.Wrapf(err, "failed to create activity %q in package %q", activityName, pkg))
	}
	if err := activity.Start(ctx); err != nil {
		return ResultFailed(errors.Wrapf(err, "failed to start activity %q in package %q", activityName, pkg))
	}
