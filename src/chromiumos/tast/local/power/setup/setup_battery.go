...
// level.
func SetBatteryDischarge(ctx context.Context, lowBatteryMargin float64) Result {
	low, err := ectool.LowBatteryShutdownPercent(ctx)
	if err != nil {
		return ResultFailed(err)
	}
	b, err := ectool.NewBatteryState(ctx)
	if err != nil {
		return ResultFailed(err)
	}
	if (low + lowBatteryMargin) >= b.ChargePercent() {
		return ResultFailed(errors.Errorf("battery percent %.2f is too low to start discharging", b.ChargePercent()))
	}
	if b.Discharging() {
	...
	testing.ContextLog(ctx, "Set battery to discharge")

	return ResultSucceeded(func(ctx context.Context) error {
		if err := setChargecontrolNormal(ctx); err != nil {
			return err
		}
		testing.ContextLog(ctx, "Reset battery discharge to normal")
		return nil
	})
}
