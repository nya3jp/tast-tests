...
func readFirstLine(ctx context.Context, filePath string) (string, error) {
	var result string
	fetchResult := func(ctx context.Context) error {
	...
	err := action.IfFailThen(
		action.Retry(4, fetchResult, 0),
		action.RetryWithExponentialBackoff(6, fetchResult, 15*time.Second, 2),
	)(ctx)
	return result, err
}
