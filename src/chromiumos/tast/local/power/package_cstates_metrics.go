...
// getIntelCPUUarch gets the name of the x86 microarchitecture.
func getIntelCPUUarch() (string, error) {
	...
	family := int64(0)
	familyRegex := regexp.MustCompile(`^cpu family\s+:\s+([0-9]+)$`)
	model := int64(0)
	modelRegex := regexp.MustCompile(`^model\s+:\s+([0-9]+)$`)
	for _, line := range lines {
		matches := familyRegex.FindAllStringSubmatch(strings.TrimSpace(line), -1)
		if matches != nil {
		...
		matches = modelRegex.FindAllStringSubmatch(strings.TrimSpace(line), -1)
		if matches != nil {
	...
	if family == 0 || model == 0 {
		return "", errors.New("failed to find cpu model/family")
	}

	return intelUarchTable[fmt.Sprintf("%02X_%02X", family, model)], nil
}

// getPackageStates gets a map of the package C-states to msr addresses.
func getPackageStates() (map[string]int64, error) {
...
// PackageCStatesMetrics records the package C-states of the DUT
