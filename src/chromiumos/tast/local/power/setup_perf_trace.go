...
func PerfTrace(ctx context.Context, dir string, extraArgs []string, chain CleanupChain) (CleanupChain, error) {
	...
	if err := perf.Start(); err != nil {
		return nil, errors.Wrap(err, "failed to start perf trace")
	}
