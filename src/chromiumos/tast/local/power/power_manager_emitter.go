...
func emitEvent(ctx context.Context, msg proto.Message, eventName string) error {
	...
	args := []string{"-u", "power", "--", "dbus-send", "--system", "--type=signal", dbusPath, dbusInterface + "." + eventName, data}
	testing.ContextLog(ctx, "Running command sudo ", args)
	if err := testexec.CommandContext(ctx, "sudo", args...).Run(testexec.DumpLogOnError); err != nil {
		return errors.Wrapf(err, "unable to run sudo %s", args)
	}
