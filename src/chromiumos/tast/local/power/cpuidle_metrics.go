...
// getCpuidleStateFiles returns the files containing the cpuidle residency information.
func getCpuidleStateFiles() (map[string][]string, int, error) {
	...
	for _, cpuInfo := range cpuInfos {
		match, err := regexp.MatchString(`^cpu\d+$`, cpuInfo.Name())
		if err != nil {
...
func (cs *CpuidleStateMetrics) Setup(ctx context.Context, prefix string) error {
	cpuidleFiles, numCpus, err := getCpuidleStateFiles()
	if err != nil {
