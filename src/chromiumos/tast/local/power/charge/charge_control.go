...
func EnsureBatteryWithinRange(ctx context.Context, cr *chrome.Chrome, s *servo.Servo, minPercentage, maxPercentage float64) error {
	if minPercentage < 0.0 || maxPercentage > 100.0 {
		return errors.New("invalid percentage detected, it should be within [0.0, 100.0]")
	}
...
func charge(ctx context.Context, displayPercentage float64) error {
	...
	if err != nil {
		return errors.Wrap(err, "failed to create PowerManager object to interact with powerd")
	}
...
// drain drains the device battery to the specified display percentage by rendering a resource heavy WebGL graphics.
func drain(ctx context.Context, cr *chrome.Chrome, displayPercentage float64) error {
	...
	pm, err := power.NewPowerManager(ctx)
	if err != nil {
		return errors.Wrap(err, "failed to create power manager object to interact with powerd")
	}
	brightness, err := pm.GetScreenBrightnessPercent(ctx)
