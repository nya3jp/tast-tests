...
// EnsureBatteryWithinRange ensures the device battery within the specified minPercentage
// & maxPercentage. FYI, powerd returns battery_percent & battery_display_percent and for
// all the operations the latter one has been taken into consideration.
// It queries the current battery display percentage of DUT and initiate charging or draining as required.
func EnsureBatteryWithinRange(ctx context.Context, cr *chrome.Chrome, s *servo.Servo, minPercentage, maxPercentage int) error {
	cleanupCtx := ctx
	...
	if p := int(status.BatteryDisplayPercent); p > minPercentage && p < maxPercentage {
		return nil
	}
	...
	if int(status.BatteryDisplayPercent) < minPercentage { // charging
		...
		if err := charge(ctx, float64(minPercentage)); err != nil {
			return err
		}
	} else { // discharging
		...
		if err := drain(ctx, cr, float64(maxPercentage)); err != nil {
			return err
		}
	}
...
// charge charges the device to the specified displayPercentage.
func charge(ctx context.Context, displayPercentage float64) error {
	...
	brightness, err := pm.CurrentBrightness(ctx)
	if err := pm.SetBrightness(ctx, 10); err != nil {
		return err
	}

	defer func(ctx context.Context) {
		if err := pm.SetBrightness(ctx, brightness); err != nil {
	...
	if err := testing.Poll(ctx, func(context.Context) error {
		...
		if status.BatteryDisplayPercent < displayPercentage {
			return errors.Errorf("still charging to %.2f%% from %.2f%%", displayPercentage, status.BatteryDisplayPercent)
		}
		return nil
	}, &testing.PollOptions{
...
// drain drains the device battery to the specified displayPercentage by rendering a resource heavy WebGL graphics.
func drain(ctx context.Context, cr *chrome.Chrome, displayPercentage float64) error {
	...
	brightness, err := pm.CurrentBrightness(ctx)
	if err := pm.SetBrightness(ctx, 100); err != nil {
		return err
	}
...
// servo role & DUT reflects the status.
func powerSourceStatus(ctx context.Context, isCharge bool) error {
	return testing.Poll(ctx, func(context.Context) error {
		status, err := power.GetStatus(ctx)
		if err != nil {
			return testing.PollBreak(errors.Wrap(err, "failed to obtain DUT power status"))
		}
		if isCharge && !status.LinePowerConnected {
			return errors.New("battery charging requires conntected to an active power source")
		}
		if !isCharge && status.LinePowerConnected {
