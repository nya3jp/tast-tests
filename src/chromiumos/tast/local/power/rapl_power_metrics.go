...
var metricNames = [...]string{
	"core", "dram", "package0", "psys", "uncore",
}
...
func (r *RAPLPowerMetrics) Setup(_ context.Context, prefix string) error {
	...
	for _, name := range metricNames {
		r.metrics[name] = perf.Metric{Name: prefix + name, Unit: "W",
...
func (r *RAPLPowerMetrics) Snapshot(_ context.Context, values *perf.Values) error {
	...
	for _, name := range metricNames {
		...
		values.Append(r.metrics[name], energy.joules[name]/energy.durationInSecond)
	}
	return nil
}
