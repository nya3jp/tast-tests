...
// SetCapsLockState changes the caps lock state.
func (c *Rgbkbd) SetCapsLockState(ctx context.Context, enabled bool) error {
	if err := c.call(ctx, "SetCapsLockState", enabled).Err; err != nil {
		return errors.Wrap(err, "failed to call method SetCapsLockState")
	}
	return nil
}

// ResetRgbkbdState restores rgbkbd's initial state by resetting the keyboard
// to the default background color and disabling caps lock.
func ResetRgbkbdState(ctx context.Context, rgbkbd *Rgbkbd) error {
	// Reset caps lock state.
	err := rgbkbd.SetCapsLockState(ctx, false)
	if err != nil {
