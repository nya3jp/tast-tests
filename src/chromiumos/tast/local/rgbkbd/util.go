...
func AreLastLogLinesEqual(expected []string, actual string) (bool, string) {
	// Note: We add "1" to our length of expected to account for the log entry
	// written after we make a call to SetStaticBackgroundColor to restore
	// our default state.
	linesToCheck := getLastNLines(actual, len(expected)+1)
