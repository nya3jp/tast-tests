...
func writeErrOutputToLog(outDir, log string) error {
	if err := ioutil.WriteFile(filepath.Join(outDir, "capslock_rgb_state_updates.txt"),
...
func logContents(expected, actual []string) string {
...
func LastLogLinesMatch(expected []string, actual, outDir string) (bool, error) {
	...
	if !matches {
		err := writeErrOutputToLog(outDir, logContents(expected, linesToCheck))
		if err != nil {
			return false, errors.New("unable to write to log file: ", err)
		}
		return matches, errors.New("unexpected logs, find more details in capslock_rgb_state_updates.txt")
	}
	return matches, nil
}
