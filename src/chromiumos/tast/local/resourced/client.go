...
// Client wraps DBUS calls to make requests to the Resource Manager (resourced).
type Client struct {
	obj *dbusutil.DBusObject
}

// GameMode returns the result of the GetGameMode DBUS method.
...
// AvailableMemoryKB returns the result of the GetAvailableMemoryKB DBUS method.
...
// GetForegroundAvailableMemoryKB DBUS method.
...
// MemoryMarginsKB returns the result of the GetMemoryMarginsKB DBUS method.
...
// ChromePressureSignal holds values created from the MemoryPressureChrome DBUS
// signal.
...
// ChromePressureWatcher converts the MemoryPressureChrome DBUS signal to a
...
// Close stops the MemoryPressureChrome DBUS signal, and closes Signals, the
// channel of ChromePressureSignal.
...
// NewChromePressureWatcher starts listening for MemoryPressureChrome DBUS
...
func (c *Client) NewChromePressureWatcher(ctx context.Context) (*ChromePressureWatcher, error) {
	...
	// Map DBUS signals into ChromePressureSignal.
...
// NewClient makes a new DBUS wrapper object for communicating with Resource
// Manager.
