...
func deviceNodes(ctx context.Context, d *UHIDDevice) error {
	...
	d.HidrawNodes, err = hidrawNodes(ctx, devicePath)
	if err != nil {
		return err
	}
...
func devicePath(infoString string) (string, error) {
	...
	if devicePath == "" {
		return "", errors.New("No device was created corresponding to this information")
	}
	return devicesDirectory + devicePath + "/", nil
}

// hidrawNodes returns the hidrawNodes that exist under <path>/hidraw.
// Because the hidraw directory takes some time to be createad we poll
// for it.
...
// directory in path.
func ID(path string) int {
	ID, err := strconv.ParseInt(filepath.Ext(path)[1:], 16, 0)
	if err != nil {
		return -1 // the given path did not represent a device
	}
	return int(ID)
}
