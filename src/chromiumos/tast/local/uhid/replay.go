...
func NewDeviceFromRecording(ctx context.Context, file *os.File) (*Device, error) {
	...
	for i := 1; scanner.Scan(); line, i = scanner.Text(), i+1 {
		if len(line) < 4 {
			return nil, errors.Errorf("line is empty (line: %d)", i)
		}
		if strings.HasPrefix(line, "D: ") {
			return nil, errors.Errorf("multi device recordings are not supported (line: %d)", i)
		}
		...
		case "I: ":
			var err error
			if dd.bus, dd.vendorID, dd.productID, err = parseInfo(data); err != nil {
				return nil, errors.Wrapf(err, "invalid recording file (line: %d)", i)
			}
		case "P: ":
			copy(dd.phys[:], data)
		case "R: ":
		...
		if err := scanner.Err(); err != nil {
			return nil, errors.Wrapf(err, "failure to parse file (line: %d)", i)
		}
	}
...
func (d *Device) Replay(ctx context.Context, file *os.File) error {
	...
	for i := 1; scanner.Scan(); line, i = scanner.Text(), i+1 {
		if strings.HasPrefix(line, "D: ") {
			return errors.Errorf("multi device recordings are not supported (line: %d)", i)
		}
		if len(line) < 15 {
		...
		if strings.HasPrefix(line, "E: ") {
			...
			if nextTimestamp, err = parseTime(line); err != nil {
				return errors.Wrapf(err, "invalid recording file (line: %d)", i)
			}
...
func parseInfo(line string) (ddBus uint16, ddVendorID, ddProductID uint32, err error) {
	var bus uint64
	var vendorID uint64
	...
	if len(info) == 0 {
		return 0, 0, 0, errors.New("info line does not comply with format <bus> <vendorID> <productID>")
	}
	if len(info) != 4 {
		return 0, 0, 0, errors.Errorf("unexpected number of elements in info line: got %d, want: 3", len(info)-1)
	}
