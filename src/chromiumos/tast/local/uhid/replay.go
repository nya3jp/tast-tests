...
func NewDeviceFromRecording(ctx context.Context, file *os.File) (*Device, error) {
	...
	for i := 1; scanner.Scan(); line, i = scanner.Text(), i+1 {
		if strings.HasPrefix(line, "D: ") {
			...
			if len(line) < 4 {
				return nil, errors.Errorf("%s line is empty. Line: %d", "name", i)
			}
			copy(data.name[:], line[3:])
		} else if strings.HasPrefix(line, "I: ") {
			if len(line) < 4 {
				return nil, errors.Errorf("%s line is empty. Line: %d", "info", i)
			}
			...
			if len(line) < 4 {
				return nil, errors.Errorf("%s line is empty. Line: %d", "phys", i)
			}
			copy(data.phys[:], line[3:])
		} else if strings.HasPrefix(line, "R: ") {
			if len(line) < 4 {
				return nil, errors.Errorf("%s line is empty. Line: %d", "descriptor", i)
			}
