...
func parseInfo(ctx context.Context, line string) (ddBus uint16, ddVendorID, ddProductID uint32, err error) {
	...
	if len(info) == 0 {
		testing.ContextLogf(ctx, "info line does not comply with format: got %+q wanted %+q", line, regex.String())
		return 0, 0, 0, errors.Errorf("info line does not comply with format: got %+q wanted %+q", line, regex.String())
	}
...
func parseTime(ctx context.Context, line string) (time.Duration, error) {
	...
	if len(times) == 0 {
		testing.ContextLogf(ctx, "event line does not comply with format: got %+q wanted %+q", line, regex.String())
		return 0, errors.Errorf("event line does not comply with format: got %+q wanted %+q", line, regex.String())
	}
