...
func parseInfo(dd *DeviceData, line string) error {
	...
	if len(info) != 3 {
		return errors.Errorf("missing parameters in info line. got %d, wanted 3", len(info))
	}
...
// both sides.
func parseDescriptor(dd *DeviceData, line string) error {
	...
	if size != uint64(len(descriptor[1:])) {
		return errors.Errorf("specified descriptor length does not match actual length; got %d, want %d", len(descriptor[1:]), size)
	}
