...
const (
	...
	// encountered.
	StatusOk ReadStatus = 1
	// StatusNoEvent signals that no event was read.
	StatusNoEvent = 2
	// StatusError signals than an error ocurred while raeding.
	StatusError = 3
)
...
func (d *Device) Dispatch(ctx context.Context) (ReadStatus, error) {
	if d == nil || d.file == nil {
		return StatusError, errors.New("device has not been initialized")
	}
	...
	case <-done:
		if err != nil {
			return StatusError, errors.Wrap(err, "failed reading uhid event")
		}
		...
		if err = binary.Read(reader, binary.LittleEndian, &requestType); err != nil {
			return StatusError, errors.Wrap(err, "failed parsing uhid file data into request")
		}
