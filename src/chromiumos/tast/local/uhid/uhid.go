...
const (
	...
	// NoEventError is returned when there were no bytes to be read from
	// the file in the given device. This can be a non fatal error
	// depending on your use case.
	NoEventError = "No event could be read from file"
	...
	fieldTooLongError    = "device %s too long: got %d wanted %d or shorter"
...
func NewDevice(name, descriptor []byte) (*Device, error) {
	if len(name) > 128 {
		return nil, errors.Errorf(fieldTooLongError, "name", len(name), 128)
	}
	if len(descriptor) > hidMaxDescriptorSize {
		return nil, errors.Errorf(fieldTooLongError, "descriptor", len(descriptor), hidMaxDescriptorSize)
	}
	d := Device{}
...
func (d *Device) NewKernelDevice(ctx context.Context) error {
	if d == nil || d.Data.Name == [128]byte{} || d.Data.Descriptor == [hidMaxDescriptorSize]byte{} {
...
func (d *Device) Dispatch(ctx context.Context) error {
	...
	case <-done:
		if err != nil {
			return errors.Wrap(err, "failed reading uhid event")
		}
		...
		if err = binary.Read(reader, binary.LittleEndian, &requestType); err != nil {
			return errors.Wrap(err, "failed parsing uhid file data into request")
		}
		return d.processEvent(ctx, buf, requestType)
	case <-ctx.Done():
		return errors.New(NoEventError)
	}
}
