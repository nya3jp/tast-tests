...
type Refiller struct {
	dir string
	// The name of the temporary file, or empty if there isn't one.
	filename string
}
...
func NewRefiller(dir string) Refiller {
	return Refiller{dir: dir, filename: ""}
}

// Close removes the temporary file if there is one. You can call it as many
// times as you want, and you probably want to also call it at the end of your
// tast.
func (r *Refiller) Close() error {
	if r.filename != "" {
		if err := os.Remove(r.filename); err != nil {
			return errors.Wrapf(err, "Unable to remove the temporary file %v/%v", r.dir, r.filename)
		}
		r.filename = ""
	}
	return nil
...
// RefillUntil removes the temporary file previously created, and then fill the disk. See
// `FillUntil()`.
...
func (r *Refiller) refillImpl(fillFunc func(string, uint64) (string, error), bytes uint64) error {
	...
	filename, err := fillFunc(r.dir, bytes)
	if err != nil {
		return err
	}
