...
func (server *DHCPTestServer) loopBody() {
	...
	packet, err := NewDHCPPacket(string(buffer[:n]))
	if err != nil {
		return
	}
	if !packet.isValid() {
	...
	if responseCode&RESPONSE_POP_HANDLER > 0 {
		server.handlingRules = server.handlingRules[1:]
	}

	if responseCode&RESPONSE_HAVE_RESPONSE > 0 {
	...
	if responseCode&RESPONSE_TEST_FAILED > 0 {
		server.testErrorChan <- errors.New("handling rule rejected packet")
		server.endTestUnsafe(false)
		return
	}

	if responseCode&RESPONSE_TEST_SUCCEEDED > 0 {
