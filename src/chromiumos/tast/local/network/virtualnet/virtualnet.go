...
type EnvOptions struct {
	...
	EnableDNS bool
	// DNSServers specify the DNS servers to be advertised by the router.
	DNSServers []string
...
func CreateRouterEnv(ctx context.Context, m *shill.Manager, pool *subnet.Pool, opts EnvOptions) (*shill.Service, *env.Env, error) {
	...
	if opts.EnableDHCP {
		...
		dnsmasq := dnsmasq.New(opts.EnableDHCP, opts.EnableDNS, v4Subnet, opts.DNSServers, opts.ResolvedHost, opts.ResolveHostToIP)
