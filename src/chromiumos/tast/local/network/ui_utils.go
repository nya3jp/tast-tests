...
// GetArcConnectivityAppResult waits for the network request started by calling `RunArcConnectivityApp` to finish,
// reads HTTP response code and stops the app and closes the capp. In case of success it returns the response code.
// If the app fails to connect to the url, it will return the network error message. If the test fails, it returns an error.
