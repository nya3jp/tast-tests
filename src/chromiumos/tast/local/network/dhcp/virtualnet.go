...
// StartTestServerInEnv start the DHCP test server in env with rules. The
// returned function should be used to stop the server and verify the rules.
func RunTestWithEnv(ctx context.Context, env *env.Env, rules []HandlingRule, test TestFunc) (testErr, svrErr error) {
	const (
		serverPort = 67
		clientPort = 68
	)
