...
func (d *DHCPHandlingRule) IsOurMessageType(packet DHCPPacket) bool {
	if packet.messageType() == d.messageType {
		return true
	}
	d.EmitWarning(fmt.Sprintf("Packet's message type was %s, not %s.", packet.messageType().name, d.messageType.name))
	return false
}
...
func (d *RespondToDiscoveryRule) Respond(queryPacket DHCPPacket) (*DHCPPacket, error) {
	...
	transactionID, ok := queryPacket.transactionID().(uint32)
	if !ok {
		return nil, errors.New("transaction ID isn't uint32")
	}
	clientHWAddress, ok := queryPacket.clientHWAddress().(string)
	if !ok {
		return nil, errors.New("client HW Address isn't string")
	}
