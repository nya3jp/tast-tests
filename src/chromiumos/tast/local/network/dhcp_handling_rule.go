...
func (d *dhcpHandlingRule) respond(queryPacket *dhcpPacket) (*dhcpPacket, error) {
	...
	sendNAK := d.ruleType == rejectAndRespondToRequest && ((d.respCounter == 0 && d.NAKFirst) || (d.respCounter != 0 && !d.NAKFirst))
	...
	} else if d.ruleType == rejectRequest || sendNAK {
	...
	if d.ruleType == respondToDiscovery ||
		d.ruleType == respondToRequest ||
		d.ruleType == respondToPostT2Request ||
		(d.ruleType == rejectAndRespondToRequest && !sendNAK) {
