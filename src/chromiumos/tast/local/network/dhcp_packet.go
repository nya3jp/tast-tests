...
func (o classlessStaticRoutesOption) pack(value interface{}) ([]byte, error) {
	...
	if !ok {
		return nil, errors.New("expected nx3 2-d interface{} slice")
	}
	var byteStr string
	for _, route := range routeList {
		prefixSize, ok := route[0].(uint8)
		if !ok {
			return nil, errors.New("invalid prefix size")
		}
		...
		destinationAddressBytes, err := IPToBytes(destinationAddress)
		if err != nil {
			return nil, err
		}
		byteStr += string(destinationAddressBytes)[:destinationAddressCount]
		routerAddressBytes, err := IPToBytes(routerAddress)
		if err != nil {
			return nil, err
		}
