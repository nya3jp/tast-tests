...
func (o IntOption) Pack(value interface{}) (string, error) {
	...
	if !ok {
		return "", errors.New("expected uint32")
	}
	buf := make([]byte, 4)
...
func (o ClasslessStaticRoutesOption) Unpack(byteStr string) (interface{}, error) {
	...
	for offset < len(byteStr) {
		...
		destinationAddress += strings.Repeat("\x00", 4-destinationAddressCount)
		routerAddress, err := Bytes2IP(byteStr[destinationAddressEnd:entryEnd])
		if err != nil {
			return nil, err
		}
		routeList = append(routeList, [3]interface{}{prefixSize, destinationAddress, routerAddress})
		offset = entryEnd
	}
...
func (o DomainListOption) readDomainName(byteStr string, offset int) (int, []string, error) {
	...
	for {
		...
		if maybePartLen == 0 {
			...
			parts = append(parts[offset : offset+partLen])
			offset += partLen
		}
	}
}
...
var (
	...
	FIELD_CLIENT_HWADDR    = IPAddressField{Field{"chaddr", 28, 16}}
	...
	FIELD_MAGIC_COOKIE       = IntField{Field{"magic_cookie", 236, 16}}
)
...
var (
	...
	OPTION_VALUE_PARAMETER_REQUEST_LIST_DFEFAULT = []int{
...
func getDHCPOptionByNumber(number int) *OptionInterface {
...
func CreateDiscoveryPacket(macAddr string) (*DHCPPacket, error) {
	macAddr += strings.Repeat(string(OPTION_PAD), 12-len(macAddr))
...
func NewDHCPPacket(byteStr string) (*DHCPPacket, error) {
	...
	for offset < len(byteStr) && int(byteStr[offset]) != OPTION_END {
		dataType := int(byteStr[offset])
		offset++
...
func (d *DHCPPacket) clientHWAddress() interface{} {
	return d.fields[FIELD_CLIENT_HWADDR]
}
...
func (d *DHCPPacket) messageType() MessageType {
	typeNum, ok := d.options[OPTION_DHCP_MESSAGE_TYPE]
	if !ok {
		return MESSAGE_TYPE_UNKNOWN
	}
	if typeNum > 0 && typeNum < len(MESSAGE_TYPE_BY_NUM) {
		return MESSAGE_TYPE_BY_NUM[typeNum]
	}
	return MESSAGE_TYPE_UNKNOWN
}

func (d *DHCPPacket) transactionID() interface{} {
	return d.fields[FIELD_TRANSACTION_ID]
}
...
func (d *DHCPPacket) toBinaryString() (string, error) {
	...
	for _, option := range DHCP_PACKET_OPTIONS {
		...
		data = append(data, string(option.number())+string(len(serializedValue)))
		offset += 2
		data = append(data, serializedValue)
		offset += len(serializedValue)
	}
	data = append(data, string(OPTION_END))
	offset += 1
	for offset < DHCP_MIN_PACKET_SIZE {
		data = append(data, string(OPTION_PAD))
		offset++
	}
