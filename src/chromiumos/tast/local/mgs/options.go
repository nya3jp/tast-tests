...
func DefaultAccount() Option {
	return func(cfg *MutableConfig) error {
		cfg.MGSDefaultAccount = &accountsConfiguration
		return nil
	}
}
...
// ExtraPolicies adds extra policies to be applied with MGS app.
...
// AutoLaunch sets MGS app ID to be started after Chrome restart. When used
// then defer cleaning and refreshing policies policyutil.ServeAndRefresh(ctx,
// fdms, cr, []policy.Policy{}). Otherwise with next Chrome restart MGS will
// auto start.
