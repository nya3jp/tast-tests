...
const (
	// StartTimeout is th etimeout of NewDevice.
...
func NewDevice(ctx context.Context, a *arc.ARC) (*Device, error) {
	...
	if err := s.waitServer(ictx); err != nil {
		s.Close()
		return nil, fmt.Errorf("UI Automator server did not bring up: %v", err)
	}
...
// waitServer waits for UI Automator server to bring up.
...
// Close releases resources associated to d.
...
func (d *Device) call(ctx context.Context, method string, out interface{}, params ...interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("failed %s: failed initializing request: %v", method, err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed %s: failed marshaling request: %v", method, err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed %s: %v", method, err)
	}
	defer res.Body.Close()
	if res.StatusCode != 200 {
		return fmt.Errorf("failed %s: got status %d", method, res.StatusCode)
	}

	resBody, err := ioutil.ReadAll(res.Body)
	if err != nil {
		return fmt.Errorf("failed %s: failed reading response: %v", method, err)
	}
	...
	if err := json.Unmarshal(resBody, &resData); err != nil {
		return fmt.Errorf("failed %s: failed unmarshaling response: %v", method, err)
	}

	if resData.Error != nil {
		return fmt.Errorf("failed %s: %s", method, resData.Error.Message)
	}
	...
	if len(resData.Result) == 0 {
		return fmt.Errorf("failed %s: no result", method)
	}
	if err := json.Unmarshal([]byte(resData.Result), out); err != nil {
		...
		return fmt.Errorf("failed %s: failed unmarshaling result: %v", method, err)
	}
	return nil
}
