...
// InstallApp creates a setup.Action that installs an APK.
func InstallApp(ctx context.Context, a *ARC, apkDataPath string, pkg string) setup.Action {
...
// StartActivity creates a setup.Action that starts an activity.
func StartActivity(ctx context.Context, a *ARC, pkg string, activity string) setup.Action {
...
// Cleanup re-enabled SELinux enforcement.
...
// DisableSELinux creates a setup.Action that disables SELinux
// enfrocement. If notOnVM is true, SELinux is not disabled when running ARCVM.
func DisableSELinux(ctx context.Context, notOnVM bool) setup.Action {
