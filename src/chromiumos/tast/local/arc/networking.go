...
func (a *ARC) UnblockOutbound(ctx context.Context) error {
	...
	if err := testexec.CommandContext(ctx, "/sbin/ip6tables", "-w", "-t", "filter", "-I", "FORWARD", "-i", "arc+", "-j", "ACCEPT").Run(testexec.DumpLogOnError); err != nil {
		return err
	}
	return testexec.CommandContext(ctx, "/sbin/iptables", "-w", "-t", "filter", "-I", "FORWARD", "-i", "arc+", "-j", "ACCEPT").Run(testexec.DumpLogOnError)
}
