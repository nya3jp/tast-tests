...
func (a *ARC) BlockOutbound(ctx context.Context) error {
	...
	if err := BootstrapCommand(ctx, "/system/bin/iptables", "-w", "-I", "OUTPUT", "-p", "tcp", "-s", IPAddr, "--sport", strconv.Itoa(ADBPort), "-j", "ACCEPT").Run(testexec.DumpLogOnError); err != nil {
		return err
	}
	if err := BootstrapCommand(ctx, "/system/bin/iptables", "-w", "-I", "OUTPUT", "-p", "tcp", "-s", IPAddr, "--sport", strconv.Itoa(ui.Port), "-j", "ACCEPT").Run(testexec.DumpLogOnError); err != nil {
		return err
	}
...
func (a *ARC) UnblockOutbound(ctx context.Context) error {
	...
	if err := BootstrapCommand(ctx, "/system/bin/iptables", "-w", "-D", "OUTPUT", "-p", "tcp", "-s", IPAddr, "--sport", strconv.Itoa(ADBPort), "-j", "ACCEPT").Run(testexec.DumpLogOnError); err != nil {
		return err
	}
