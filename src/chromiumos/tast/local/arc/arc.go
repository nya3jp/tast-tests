...
func startPstore(ctx context.Context, w io.Writer) (*testexec.Cmd, error) {
	// Wait for crosvm to make the .pstore file by polling vm_pstore_dump.
	cmd := testexec.CommandContext(ctx, "/bin/sh", "-c", "while ! /usr/bin/vm_pstore_dump; do sleep 0.1; done")
	if err := cmd.Run(testexec.DumpLogOnError); err != nil {
	...
	// so use context.Background instead of ctx to make sure it isn't killed prematurely.
	cmd = testexec.CommandContext(context.Background(), "/usr/bin/vm_pstore_dump", "--follow") // NOLINT: process may need to persist across multiple tests
	cmd.Stdout = w
	if err := cmd.Start(); err != nil {
		return nil, err
	}
