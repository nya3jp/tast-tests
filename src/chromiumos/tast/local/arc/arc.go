...
func (a *ARC) Close() error {
	...
	// TODO(kimiyuki) How can we combine two errors?
	if logcatErr != nil {
		return logcatErr
	} else if pstoreErr != nil {
		return pstoreErr
	} else {
		return nil
	}
...
func startPstore(ctx context.Context, w io.Writer) (*testexec.Cmd, error) {
	// Wait for logd to start by polling logcat.
	cmd := testexec.CommandContext(ctx, "/bin/sh", "-c", "while ! /build/rootfs/opt/google/vms/android/vendor/bin/vm_pstore_dump; do sleep 0.1; done")
	if err := cmd.Run(testexec.DumpLogOnError); err != nil {
		return nil, errors.Wrap(err, "vm_pstore_dump failed")
	}

	// The vm_pstore_dump process may need to span multiple tests if we're being used by a precondition,
	// so use context.Background instead of ctx to make sure it isn't killed prematurely.
	cmd = testexec.CommandContext(context.Background(), "/build/rootfs/opt/google/vms/android/vendor/bin/vm_pstore_dump", "--follow") // NOLINT: process may need to persist across multiple tests
	cmd.Stdout = w
	if err := cmd.Start(); err != nil {
		return nil, err
	}
