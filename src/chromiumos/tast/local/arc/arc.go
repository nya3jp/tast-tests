...
// arcGuest retrieve the current ARC guest Chrome OS is running (container / vm).
...
func arcGuest() (guestMode, error) {
	...
	// Find and return Chrome OS ARC guest type and error.
	if s["--enable-arcvm"] {
		return vm, nil
	} else if s["--arc-start-mode=always-start"] || s["--arc-start-mode=always-start-with-no-play-store"] {
