...
func withRetry(action func(context.Context, *chrome.Conn) error, maxAttempts int) func(context.Context, *chrome.Conn) error {
	return func(ctx context.Context, conn *chrome.Conn) error {
		...
		for {
			...
			testing.ContextLogf(ctx, "Attempt %d, failed to perform an action: %v", attempts, err)
			attempts++
...
func FindOptInExtensionPageAndAcceptTerms(ctx context.Context, cr *chrome.Chrome, maxAttempts int, wait bool) error {
	...
	if err := withRetry(waitForTerms, maxAttempts)(ctx, conn); err != nil {
	...
	if wait {
		return withRetry(waitForOptin, maxAttempts)(ctx, conn)
	}
