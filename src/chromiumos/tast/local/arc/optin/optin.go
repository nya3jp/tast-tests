...
func PerformAndClose(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn) error {
	...
	if err := WaitForPlayStoreShown(ctx, tconn); err != nil {
		// When we get here, play store is propably not shown, or it failed to be detected.
		// Just log the message and continue.
		testing.ContextLog(ctx, "Play store window is not detected")
	}

	// Skip checking the error response, and continue to call WaitForAppClosed().
