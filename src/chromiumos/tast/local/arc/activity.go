...
func (ac *Activity) MoveWindow(ctx context.Context, tconn *chrome.TestConn, t time.Duration, toBounds, fromBounds coords.Rect) error {
	...
	switch sdkVer {
	case SDKP:
		if err := ac.MoveWindowP(ctx, coords.NewPoint(toBounds.Left, toBounds.Top), t); err != nil {
			return errors.Wrap(err, "could not move window")
		}
		return nil
	case SDKR:
		if err := moveWindowR(ctx, tconn, t, toBounds, fromBounds); err != nil {
			return errors.Wrap(err, "could not move window")
		}
		return nil
	default:
...
func moveWindowR(ctx context.Context, tconn *chrome.TestConn, t time.Duration, toBounds, fromBounds coords.Rect) error {
...
func (ac *Activity) ResizeWindow(ctx context.Context, tconn *chrome.TestConn, border BorderType, to coords.Point, t time.Duration) error {
	...
	case SDKP:
		...
		return nil
	case SDKR:
		if err := resizeWindowR(ctx, ac, tconn, BorderTopLeft, coords.NewPoint(0, 0), time.Second); err != nil {
			return errors.Wrap(err, "could not resize window")
		}
		return nil
	default:
...
func resizeWindowR(ctx context.Context, ac *Activity, tconn *chrome.TestConn, border BorderType, to coords.Point, t time.Duration) error {
	// Default value: center of window.
	bounds, err := ac.WindowBounds(ctx)
	src := bounds.CenterPoint()
	...
	if err != nil {
		errors.Wrap(err, "could not get app window state")
	}
	borderOffset := borderOffsetForNormal
...
func (ac *Activity) SetWindowState(ctx context.Context, tconn *chrome.TestConn, pkgName string, state WindowState) error {
	...
	switch sdkVer {
	case SDKP:
		if err := setWindowStateP(ctx, ac, state); err != nil {
			return err
		}
	case SDKR:
...
// Supported states: WindowStateNormal, WindowStateMaximized, WindowStateFullscreen, WindowStateMinimized
func setWindowStateP(ctx context.Context, ac *Activity, state WindowState) error {
	t, err := ac.getTaskInfo(ctx)
	if err != nil {
		return errors.Wrap(err, "could not get task info")
	}
...
func dragWithPause(ctx context.Context, tconn *chrome.TestConn, from, to coords.Point, t time.Duration) error {
	if err := mouse.Move(ctx, tconn, from, 0); err != nil {
		return err
	}
	...
	if err := mouse.Move(ctx, tconn, to, t); err != nil {
		return err
	}
