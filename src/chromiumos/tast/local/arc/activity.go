...
// like fullscreen, maximized or minimized states.
...
func (ac *Activity) taskID(ctx context.Context) (int, error) {
	...
	if len(groups) != 2 {
		testing.ContextLog(ctx, "Failed to find taskID: ", groups[0])
...
func (ac *Activity) generateTouches(ctx context.Context, from, to Point, numTouches int) error {
	...
	for i := 0; i < numTouches; i++ {
		...
		stw.Move(input.TouchCoord(x*pixelToTuxelScaleX), input.TouchCoord(y*pixelToTuxelScaleY))
