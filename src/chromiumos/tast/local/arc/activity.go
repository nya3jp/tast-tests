...
import (
	...
	pb "android.com/frameworks/base/core/proto/android/server"
				...
				for _, a := range t.GetAppWindowTokens() {
					if a.GetName() == name {
						for _, w := range a.GetWindowToken().GetWindows() {
							// Return the first window whose name is not "Splash Screen".
							// A "Splash Screen" window might be added to the AppWindow. See:
							// http://cs/pi-arc-dev/frameworks/base/services/core/java/com/android/server/policy/PhoneWindowManager.java?l=3080&rcl=453999da27de2a0792c2bfa332ab4aa437e3e1f6
