...
type taskInfo struct {
	...
	idle bool
	// isResizable represents whether the activity is user-resizable or not.
	isResizable bool
}
...
// IsResizable return the window resizability.
func (ac *Activity) IsResizable(ctx context.Context) (bool, error) {
	task, err := ac.getTaskInfo(ctx)
	if err != nil {
		return false, errors.Wrap(err, "could not get task info")
	}
	return task.isResizable, nil
}
...
func (ac *Activity) getTasksInfo(ctx context.Context) (tasks []taskInfo, err error) {
	...
	for _, groups := range matches {
		...
		t.isResizable, err = strconv.ParseBool(groups[10])
		if err != nil {
			return nil, err
		}
