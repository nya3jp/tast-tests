...
const (
	borderOffset   = 3
	captionHeight  = 64
	touchFrequency = 15 * time.Millisecond
)
...
func (ac *Activity) Resize(border uint, toX, toY int, t time.Duration) error {
	...
	pixelToTouchFactorX := float64(tsw.Width()) / float64(ac.displayWidth)
	pixelToTouchFactorY := float64(tsw.Height()) / float64(ac.displayHeight)
	...
	testing.ContextLogf(ac.ctx, "---> Resizing from (%d,%d) to (%d,%d) with numTouches=%d\n",
		from.x, from.y, toX, toY, numTouches)
	...
	stw.Move(input.TouchCoord(float64(toX)*pixelToTouchFactorX),
		input.TouchCoord(float64(toY)*pixelToTouchFactorY))
	sleep(ac.ctx, touchFrequency)
...
func coordsForBorder(border uint, bounds Rect) vertex {
	...
	if border&BorderTop != 0 {
		coords.y = bounds.Top - captionHeight - borderOffset
	} else if border&BorderBottom != 0 {
		coords.y = bounds.Bottom + borderOffset
	}

	// Left & Right are exclusive
	if border&BorderLeft != 0 {
		coords.x = bounds.Left - borderOffset
	} else if border&BorderRight != 0 {
		coords.x = bounds.Right + borderOffset
	}
	return coords
}
