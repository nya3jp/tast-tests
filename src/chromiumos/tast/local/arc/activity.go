...
// WindowStateToString returns the string representation of WindowState
func WindowStateToString(windowState WindowState) string {
	switch windowState {
	...
	default:
		return fmt.Sprintf("Uknown window state: %d", windowState)
	}
}
...
// GetWindowState returns the window state as a string
func (ac *Activity) GetWindowState(ctx context.Context) (windowState WindowState, err error) {
	task, err := ac.getTaskInfo(ctx)
	if err != nil {
		return -1, errors.Wrap(err, "could not get task info")
	}
