...
// This is the size that activity thinks it has, although the physical size could be smaller.
// See: WindowFrame
...
// For example, WindowFrame does not include the caption size, since the caption is drawn by Chrome.
// And does not include the shelf size when it is in "show always" mode and the activity is
// in fullscreen or maximized modes.
// See: WindowBounds
func (ac *Activity) WindowFrame(ctx context.Context) (Rect, error) {
	...
	regStr := `(?m)` + // Enable multiline.
		...
		`^\s*mFrame=\[(\d+),(\d+)\]\[(\d+),(\d+)\]` // ...until we match mFrame=
	re := regexp.MustCompile(regStr)
	groups := re.FindStringSubmatch(string(output))
	if len(groups) != 5 {
		return Rect{}, errors.New("failed to parse dumpsys output; activity not running perhaps?")
