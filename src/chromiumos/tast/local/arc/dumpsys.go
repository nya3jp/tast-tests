	...
	traverse = func(wc windowContainer) {
		if t, ok := wc.(*server.TaskProto); ok && t != nil {
			...
			ti := TaskInfo{
				...
				// windowState: WindowState(t.GetWindowContainer().GetConfigurationContainer().GetFullConfiguration().GetWindowConfiguration().GetWindowingMode()),
...
func (a *ARC) dumpsysWaylandR(ctx context.Context) (tasks []TaskInfo, err error) {
	...
	re := regexp.MustCompile(`(?m)` + // Enable multiline.
		...
		`\(task\s+(?P<taskID>\d+).+\n` + // Grab the taskID. (Group idx 1)
		`.*window state:\s+(?P<windowState>[A-Z_]+)`) // Grab window state. (Group idx 2)
	taskIDIndex := 1
	windowStateIndex := 2
