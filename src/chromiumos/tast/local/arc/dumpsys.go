...
func (a *ARC) dumpsysActivityActivitiesN(ctx context.Context) (tasks []TaskInfo, err error) {
	...
	cmd := a.Command(ctx, "dumpsys", "activity", "activities")
	out, err := cmd.Output()
	if err != nil {
...
func (a *ARC) dumpsysActivityActivitiesQ(ctx context.Context) (tasks []TaskInfo, err error) {
	...
	if err := proto.Unmarshal(output, am); err != nil {
		...
		testing.ContextLogf(ctx, "Base64-encoded protobuf message: %q", encoded)
	...
	for _, d := range super.GetDisplays() {
		for _, stack := range d.GetStacks() {
			for _, t := range stack.GetTasks() {
				...
				// Any value different than 0 (RESIZE_MODE_UNRESIZEABLE) means resizable. Defined in ActivityInfo.java. See:
				...
				// Windowing mode constants taken from WindowConfiguration.java:
				...
				// PIP mode does not exist, but WINDOWING_MODE_PINNED is an acceptable replacement.
				// Minimized mode does not exist, but if the top-most activity is "STOPPED" we can assume it is minimized.
				...
				val, ok := ws[wm]
				if !ok {
					return nil, errors.Errorf("unsupported window state value: %q", ws)
				}
				ti.windowState = val

				// TODO(crbug.com/XXXX): Protobuf output does not providle "idle" information. Find a replacement.
				ti.idle = false
