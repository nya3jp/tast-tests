	...
	for _, d := range super.GetDisplays() {
		for _, stack := range d.GetStacks() {
			for _, t := range stack.GetTasks() {
				...
				// PIP mode does not exist, but WINDOWING_MODE_PINNED is an acceptable substitute for PIP.
				// TODO(crbug.com/1005422) Minimized and Maximized modes are not supported. Find a replacement.
