...
func (a *ARC) dumpsysActivityActivitiesQ(ctx context.Context) (tasks []TaskInfo, err error) {
	...
	if err := proto.Unmarshal(output, am); err != nil {
		...
		if ok {
			if f, err := ioutil.TempFile(dir, "activity-activities-protobuf-message-*.bin"); err == nil {
				defer f.Close()
				if _, err := f.Write(output); err == nil {
					testing.ContextLogf(ctx, "Protobuf messsage saved in test out directory. Filename: %q", path.Base(f.Name()))
				}
			}
		}
