...
// IsObjectExists waits and checks the UI object's existence.
// It returns true if found otherwise false.
func IsObjectExists(ctx context.Context, obj *ui.Object, timeout time.Duration) (bool, error) {
	if err := obj.WaitForExists(ctx, timeout); err != nil {
		if ui.IsTimeout(err) {
			return false, nil
		}
...
func ClickAnyFromObjectPool(ctx context.Context, pool map[*ui.Object]string, timeout time.Duration) error {
	...
	for btn, description := range pool {
		if exist, err := IsObjectExists(ctx, btn, timeout); err != nil {
