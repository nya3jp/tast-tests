...
// MountVirtioBlkDataDiskImageReadOnlyIfUsed first checks if ARCVM virtio-blk /data is used on the device, and
// if that is the case, mounts the virtio-blk disk image on the host's /home/root/<hash>/android-data/data as read-only.
func MountVirtioBlkDataDiskImageReadOnlyIfUsed(ctx context.Context, a *ARC, user string) (func(context.Context), error) {
	...
	if !virtioBlkDataEnabled {
		// If ARCVM virtio-blk /data is not enabled, Android's /data directory is already available at
		// the host's /home/root/<hash>/android-data/data.
		return func(context.Context) {}, nil
	}

	// Before mounting the virtio-blk disk image, run sync on the Android side to ensure that the disk image up-to-date.
