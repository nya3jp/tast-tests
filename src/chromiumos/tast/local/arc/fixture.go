...
// 1. Opt into PlayStore by default.
// 2. No content sync to avoid noise during power/performance measurements.
	...
	return &bootedFixture{
		...
		fOpt: func(ctx context.Context, s *testing.FixtState) ([]chrome.Option, error) {
			...
			return append(opts,
				chrome.ARCSupported(),
				chrome.ExtraArgs(DisableSyncFlags()...),
				chrome.ExtraArgs("--disable-features=ArcResizeLock"), // TODO: find an alternative solution for ARC mobile layout issue.
			), nil
		},
	}
}
