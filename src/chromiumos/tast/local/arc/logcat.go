...
// WaitForExpInLogcatChannel returns a channel to which the result of WaitForExpInLogcat will be written.
func WaitForExpInLogcatChannel(ctx context.Context, a *ARC, exp *regexp.Regexp, timeout time.Duration) chan error {
	c := make(chan error)
	go func() {
		c <- WaitForExpInLogcat(ctx, a, exp, timeout)
	}()
