...
func restartADBForContainer(ctx context.Context) error {
	// Restart adbd to load the newly installed public key.
	testing.ContextLog(ctx, "Restarting ADB Daemon")
	if err := restartADBDaemon(ctx); err != nil {
		return errors.Wrap(err, "failed to restart adbd")
	}

	return setUpADBAuth(ctx)
}

func restartADBForVM(ctx context.Context) error {
	return setUpADBAuth(ctx)
}
