...
`
	...
	// Though it is not referenced by tast-side code, testPublicKey is pre-installed
	// in ARC on test image (See go/tast-adb-key).
...
// restartADBLocalServerWithKey restarts local ADB server with testPrivateKeyPath
// so it can set up ADB auth with ARC.
func restartADBLocalServerWithKey(ctx context.Context) error {
