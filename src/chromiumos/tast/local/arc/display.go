...
func (d *Display) Size(ctx context.Context) (s Size, err error) {
	...
	if err != nil {
		cmd.DumpLog(ctx)
		return Size{-1, -1}, errors.Wrap(err, "failed to execute 'dumpsys window displays'")
	}
	...
	groups := re.FindStringSubmatch(string(output))
	if len(groups) != 3 {
		return Size{-1, -1}, errors.New("failed to parse dumpsys output")
	}

	width, err := strconv.Atoi(groups[1])
	if err != nil {
		return Size{-1, -1}, errors.Wrap(err, "could not parse bounds")
	}
...
// It always return the coordinates in this order: {W, H}.
func (d *Display) stableSize(ctx context.Context) (s Size, err error) {
	cmd := d.a.Command(ctx, "dumpsys", "display")
	output, err := cmd.Output()
	if err != nil {
		cmd.DumpLog(ctx)
		return Size{-1, -1}, errors.Wrap(err, "failed to execute 'dumpsys display'")
	}
