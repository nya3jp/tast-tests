...
// BootedInTouchView returns a precondition similar to Booted(). The only difference from Booted() is
// that Chrome is launched in touch_view mode in this precondition.
func BootedInTouchView() testing.Precondition { return bootedInTouchViewPre }

var bootedInTouchViewPre = &preImpl{
	name:      "arc_booted_in_touch_view",
	timeout:   resetTimeout + chrome.LoginTimeout + BootTimeout,
