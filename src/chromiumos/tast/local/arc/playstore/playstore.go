...
func installOrUpdate(ctx context.Context, a *arc.ARC, d *ui.Device, pkgName string, tryLimit int, op operation) error {
	...
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err := accountSetUpText.WaitForExists(ctx, shortUITimeout); err == nil {
			...
			info8, err := accountSetUpText.GetClassName(ctx)
			if err == nil {
				testing.ContextLogf(ctx, "Completing account setup GetClassName(ctx) : %v ", info8)
			}
			testing.ContextLog(ctx, "Completing account setup GetClassName(ctx): ", info8)
