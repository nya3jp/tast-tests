...
// UpdateApp updates an installed application via Play Store if an update is available. If the application is
// not installed yet, it will install it.
// It will wait for the app to finish installing/updating before returning.
func UpdateApp(ctx context.Context, a *arc.ARC, d *ui.Device, pkgName string, tryLimit int) error {
	installed, err := a.PackageInstalled(ctx, pkgName)
	if err != nil {
		return err
	}
