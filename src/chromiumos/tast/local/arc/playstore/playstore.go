...
func InstallApp(ctx context.Context, a *arc.ARC, d *ui.Device, pkgName string, tryLimit int) error {
	...
	if installed, err := verifyPkgInstalled(ctx, a, pkgName); err != nil {
		return err
	} else if installed {
		return nil
	}
...
func verifyPkgInstalled(ctx context.Context, a *arc.ARC, pkg string) (bool, error) {
	pkgs, err := a.InstalledPackages(ctx)
	if err != nil {
		return false, errors.Wrap(err, "failed to list packages")
	}
