...
func saveAsAndCompare(t *testing.T, p *Values, goldenPath string, format string, fileName string) {
	t.Helper()
...
func savePerfFormat(t *testing.T, format string, fileName string) {
	var (
		metric1  = Metric{Name: "metric1", Unit: "unit1", Direction: SmallerIsBetter}
		metric2  = Metric{Name: "metric2", Unit: "unit2", Direction: SmallerIsBetter, Multiple: true}
		metric3a = Metric{Name: "metric3", Variant: "a", Unit: "unit3a", Direction: SmallerIsBetter}
	...
	saveAsAndCompare(t, p, "testdata/TestSave.json", format, fileName)
}

func TestSaveAsCrosbolt(t *testing.T) {
	savePerfFormat(t, "crosbolt", "results-chart.json")
}

func TestSaveAsChromeperf(t *testing.T) {
	savePerfFormat(t, "chromeperf", "perf_results.json")
}
