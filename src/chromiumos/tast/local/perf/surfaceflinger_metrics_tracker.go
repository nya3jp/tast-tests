...
func (f *SurfaceFlingerMetrics) Stop(ctx context.Context) (fps, latency float64, err error) {
	if f.collecting == nil {
		return -1, -1, errors.New("SurfaceFlinger stop failed since it was never started")
	}
...
func (f *SurfaceFlingerMetrics) recentFrames(ctx context.Context) error {
	...
	*/
	...
	for _, line := range lines {
		...
		// there are various situations in which an entry could be "0" (go/entries-is-zero).
...
func (f *SurfaceFlingerMetrics) calculateMetrics() (fps, latency float64, err error) {
	...
	if numFrames < 2 {
		return -1, -1, errors.Errorf("Only %v frames recorded", numFrames)
	}
	// Calculate FPS.
