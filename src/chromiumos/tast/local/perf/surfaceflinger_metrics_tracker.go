...
func (f *SurfaceFlingerMetrics) getRecentFrames(ctx context.Context) error {
	...
	for _, line := range lines {
		...
		if draw, err = strconv.Atoi(entries[0]); err != nil {
			return errors.Wrap(err, "failed to parse \"draw\" timestamp")
		}
		if vsync, err = strconv.Atoi(entries[1]); err != nil {
			return errors.Wrap(err, "failed to parse \"vsync\" timestamp")
		}
		if submit, err = strconv.Atoi(entries[2]); err != nil {
			return errors.Wrap(err, "failed to parse \"submit\" timestamp")
		}
