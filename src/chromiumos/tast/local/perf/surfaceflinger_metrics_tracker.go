...
func (f *FrameLatencyStats) calculateMetrics() (float64, float64) {
	...
	total := len(f.frames) - 1
	if total > 0 {
		// Calculate FPS.
		dt := float64(f.frames[total].vsync-f.frames[0].vsync) / 1000000000.0
		fps := float64(total) / dt

		// Calculate average latency.
		var latencies = 0.0
		for _, f := range f.frames {
			latencies += float64(f.submit-f.draw) / 1000000000.0
		}
		avgLatency := latencies / float64(total+1)
