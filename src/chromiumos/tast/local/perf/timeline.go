...
// CaptureWhile captures metrics as long as `condition` returns true. The capture interval can be set as a parameter of `NewTimeline`. This method blocks until `condition` returns false.
...
// CaptureTimePeriod captures metrics for `duration` seconds. he capture interval can be set as a parameter of `NewTimeline`. This method blocks for at most `duration` seconds.
func (t *Timeline) CaptureTimePeriod(ctx context.Context, v *Values, duration time.Duration) error {
	...
	return t.CaptureWhile(ctx, v, func() bool {
		...
		return iterations < numIterations+1
	})
}
