...
func ResetTpmAndSystemStates(ctx context.Context) error {
	...
	// separately.
	defer func() {
		retErr = ensureDaemonsStarted(ctx, JobsToRestart)
	}()
	daemonsToStop := reverseStringSlice(JobsToRestart)
	if retErr = stopDaemons(ctx, daemonsToStop); retErr != nil {
		return retErr
	}
	...
	defer func() {
		retErr = ensureDaemonsStarted(ctx, trunksd)
	}()
	if retErr = stopDaemons(ctx, trunksd); retErr != nil {
		return retErr
	}
...
func ensureDaemonsStarted(ctx context.Context, daemons[] string) error {
	for _, daemon := range daemons {
		if err := upstart.EnsureJobRunning(ctx, daemon); err != nil {
			return errors.Wrapf(err, "Failed to start %s.", daemon)
		}
	}
...
func stopDaemons(ctx context.Context, daemons[] string) error {
	for _, daemon := range daemons {
		if err := upstart.StopJob(ctx, daemon); err != nil {
			return errors.Wrapf(err, "Failed to stop %s.", daemon)
		}
	}
