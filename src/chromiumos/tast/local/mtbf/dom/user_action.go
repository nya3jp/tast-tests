...
// GetElementReadyState returns media element ready state.
...
func GetElementReadyState(ctx context.Context, conn *chrome.Conn, selector string) (readyState int, err error) {
	err = conn.Eval(ctx, Query(selector)+".readyState", &readyState)
	return
}

// GetElementNetworkState returns medial element network state.
...
func GetElementNetworkState(ctx context.Context, conn *chrome.Conn, selector string) (networkState int, err error) {
