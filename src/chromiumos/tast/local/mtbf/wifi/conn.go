...
const wifiEnableElemJsOuside = wifiDom +
...
func (c *Conn) clickWifi() error {
	testing.ContextLog(c.ctx, "clickWifi() - wifiEnableElemJsOuside: ", wifiEnableElemJsOuside)
	// c.cdpConn.Eval(c.ctx, wifiEnableElemJsOuside+" != null ", &outside)
	err := c.cdpConn.WaitForExprWithTimeout(c.ctx, wifiEnableElemJsOuside+" != null ", 1*time.Second)
	...
	if outside {
		wifiEnableJs = wifiEnableElemJsOuside + ".click()"
	} else {
