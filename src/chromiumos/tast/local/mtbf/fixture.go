...
func CloseExistingAndLeftOffTabs(ctx context.Context, cr *chrome.Chrome) error {
	...
	for {
		tabsCnt, err := findExistingTabs(ctx, tconn)
		if err != nil {
			return err
		}
		if tabsCnt > 0 {
...
func findExistingTabs(ctx context.Context, tconn *chrome.TestConn) (int, error) {
