...
// ValidateWallpaperName checks that a text node exists inside the wallpaper app with the given name.
// Requires the wallpaper app to be open.
func ValidateWallpaperName(ui *uiauto.Context, name string) uiauto.Action {
	windowNode := nodewith.NameContaining("Wallpaper").Role(role.Window).First()
	wallpaperNameNode := nodewith.NameStartingWith(fmt.Sprintf("Currently set %v", name)).Role(role.Heading).Ancestor(windowNode)
