...
func (d *differ) initialize() error {
	...
	if strings.HasPrefix(release[lsbrelease.BuildType], "Continuous Builder") {
		...
		d.goldArgs = []string{
			...
			"--passfail"}
		d.triageURL = fmt.Sprintf("https://%s-gold.skia.org/search?jobid=%s", goldInstance, builderMatch[1])

		// Note: This will falsely pick up local builds that have been flashed with an offical build.
		...
		d.goldArgs = []string{"--passfail", "--git_hash", "f1d65adb1d7cd922f4677d0f9406a4083f5fdcbc"}
	}
	...
	if !d.config.SkipDpiNormalization {
		for _, zoomFactor := range info.AvailableDisplayZoomFactors {
			if math.Abs(zoomFactor*displayMode.DeviceScaleFactor-1) < 0.01 && zoomFactor != info.DisplayZoomFactor {
				...
				d.reset = append(d.reset, func() error {
...
// Collect all your diff results at the end with GetFailedDiffs() or DieOnFailedDiffs()
func (d *differ) DiffWithOptions(basename string, finder *nodewith.Finder, options DiffTestOptions) uiauto.Action {
	return func(_ context.Context) error {
		// Since elements of config are parameters that affect rendering, different configs should always have distinct diffs.
		name := d.state.TestName() + "." + basename
		...
		if err := d.runGoldCommand("imgtest", append([]string{"add",
			...
			d.failedTests = append(d.failedTests, basename)
			// In local dev, seeing the error early will help debug quicker.
			if d.testMode == local {
				d.state.Logf("Failed local test %s: %v", basename, err)
			}
		}
