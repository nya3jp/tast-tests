...
func (d *differ) capture(screenshotName string, finder *nodewith.Finder) (string, error) {
	ctx := d.ctx
	...
	node, err := uiauto.New(tconn).WithTimeout(time.Second*2).Info(d.ctx, finder)
	if err != nil {
		return "", err
	}
