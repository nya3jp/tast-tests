...
func (d *differ) capture(screenshotName string, finder *nodewith.Finder, options *DiffTestOptions) ([]string, error) {
	...
	windowBounds := window.BoundsInRoot
	...
	if windowState == ash.WindowStateNormal {
		if window.CanResize && (d.config.WindowWidthDP == 0 || d.config.WindowHeightDP == 0) {
			return testArgs, errors.Errorf("please add WindowWidthDP: %d, WindowHeightDP: %d to your screendiff config", windowBounds.Width, windowBounds.Height)
		}
		// Ensure it always goes to the top-left corner of the screen. This should prevent misalignment issues.
		requestedBounds := coords.Rect{Width: windowBounds.Width, Height: windowBounds.Height}
		if window.CanResize {
			requestedBounds = coords.Rect{Width: d.config.WindowWidthDP, Height: d.config.WindowHeightDP}
		}
		var displayID string
		windowBounds, displayID, err = ash.SetWindowBounds(d.ctx, d.tconn, window.ID, requestedBounds, window.DisplayID)
		if err != nil {
			return testArgs, err
	...
	isAtTopLeft := windowBounds.Top == 0 && windowBounds.Left == 0
	windowBounds = windowBounds.WithInset(d.config.WindowBorderWidthDP, d.config.WindowBorderWidthDP)
	...
	boundsPx := coords.ConvertBoundsFromDPToPX(location.Intersection(windowBounds), d.uiScale)
	...
	testArgs = append(testArgs,
		...
		"--add-test-optional-key", fmt.Sprintf("window_size:%dx%d", windowBounds.Width, windowBounds.Height),
