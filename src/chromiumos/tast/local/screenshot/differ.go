...
func (d *differ) initialize() error {
	...
	infos, err := display.GetInfo(d.ctx, tconn)
	if err != nil {
		return err
	}
	// Normalizing DPI on multiple displays with different DeviceScaleFactors won't work.
	if len(infos) != 1 {
		return errors.New("screenshot testing only supports a single display device")
	}
	info := infos[0]
	...
	// This should reduce the number of screenshots that haave to be approved.
	for _, zoomFactor := range info.AvailableDisplayZoomFactors {
		if math.Abs(zoomFactor*displayMode.DeviceScaleFactor-1) < 0.01 && zoomFactor != info.DisplayZoomFactor {
			...
			if err := display.SetDisplayProperties(d.ctx, tconn, info.ID, display.DisplayProperties{DisplayZoomFactor: &zoomFactor}); err != nil {
				return err
			}
		}
	}
