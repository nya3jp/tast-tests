...
func (d *differ) initialize() error {
	...
	if strings.HasPrefix(release[lsbrelease.BuildType], "Continuous Builder") {
		...
		d.triageURL = "Please look at the comment by the gold bot on the CL for a link to approve."
		...
		d.goldArgs = append(baseArgs, []string{"--passfail", "--git_hash", "f1d65adb1d7cd922f4677d0f9406a4083f5fdcbc"}...)
	}
	...
	return func(_ context.Context) error {
		...
		if err := d.runGoldCommand("imgtest", append([]string{"add",
			...
			if d.testMode == local {
				...
				d.state.Logf("Failed test on CQ %s: %v", basename, err)
			}
		}
