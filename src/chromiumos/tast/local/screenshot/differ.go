...
func (d *differ) initialize() error {
	...
	params := map[string]string{
		...
		// Fuzzy matcher is a bit wierd. Instead of "no more than <max different pixels> with difference of more than <delta>",
...
func (d *differ) capture(screenshotName string, finder *nodewith.Finder, options *DiffTestOptions) ([]string, error) {
	...
	if _, err := os.Stat(dir); err == nil {
		return testArgs, errors.Errorf("screenshot has already been taken for %s", screenshotName)
	}
