...
func CaptureChrome(ctx context.Context, cr *chrome.Chrome, path string) error {
	...
	return captureInternal(ctx, path, func(code string, png *string) error {
		return tconn.EvalPromise(ctx, code, png)
	})
}
...
func CaptureCDP(ctx context.Context, conn *cdputil.Conn, path string) error {
	return captureInternal(ctx, path, func(code string, png *string) error {
		_, _, err := conn.Eval(ctx, code, true /* awaitPromise */, png)
		return err
	})
}

func captureInternal(ctx context.Context, path string, eval func(code string, png *string) error) error {
