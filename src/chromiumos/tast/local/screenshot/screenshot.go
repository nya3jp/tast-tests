...
// Reporter is an interface that should match testing.State.
type Reporter interface {
	// ReportScreenshot reports a screenshot to a diff tester such as Skia Gold.
	ReportScreenshot(name string, screenshot *image.RGBA, keyValueMap map[string]string) error
}

// DiffTest takes a screenshot of the ui element described by `params` and sends it off for automated diff testing.
func DiffTest(ctx context.Context, s Reporter, cr *chrome.Chrome, testName string, params ui.FindParams) error {
	return DiffTestWithOptions(ctx, s, cr, testName, DiffTestOptions{Params: params})
}

// DiffTestWithOptions takes a screenshot of a particular ui element and sends it off for automated diff testing.
func DiffTestWithOptions(ctx context.Context, s Reporter, cr *chrome.Chrome, testName string, options DiffTestOptions) error {
	...
	return s.ReportScreenshot(testName, cropped, map[string]string{
