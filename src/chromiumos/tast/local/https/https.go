...
func ConfigureChromeToAcceptCertificate(ctx context.Context, configuration ServerConfiguration, chrome *chrome.Chrome, targetBrowser *browser.Browser, testConnection *chrome.TestConn) error {
	...
	if certExists, err := CheckIfCertificateExists(ctx, chrome, targetBrowser, testConnection, configuration.CaCertName, configuration.CaCertAuthName); err != nil {
		return err
	} else if certExists {
		return nil
	}
	...
	if certExists2, err := CheckIfCertificateExists(ctx, chrome, targetBrowser, testConnection, configuration.CaCertName, configuration.CaCertAuthName); err != nil {
		return err
	} else if !certExists2 {
