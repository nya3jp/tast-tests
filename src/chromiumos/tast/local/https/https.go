...
type ServerConfiguration struct {
	...
	caCommonName              string // If not set, sets to caCommonNameDefault
	caCertificateOrganization string // If not set, sets to caCertificateOrganizationDefault
}

// CaCommonName getter returns the set value or default in case not set.
func (config ServerConfiguration) CaCommonName() string {
	if config.caCommonName == "" {
		return caCommonNameDefault
	}
	return config.caCommonName
}

// SetCaCommonName setter in case not using library provided certificate.
func (config *ServerConfiguration) SetCaCommonName(caCommonName string) {
	config.caCommonName = caCommonName
}

// CaCertificateOrganization getter returns the set value or default in case not set.
func (config ServerConfiguration) CaCertificateOrganization() string {
	if config.caCertificateOrganization == "" {
		return caCertificateOrganizationDefault
	}
	return config.caCertificateOrganization
}

// SetCaCertificateOrganization setter in case not using library provided certificate.
func (config *ServerConfiguration) SetCaCertificateOrganization(caCertificateOrganization string) {
	config.caCertificateOrganization = caCertificateOrganization
}
...
func ConfigureChromeToAcceptCertificate(ctx context.Context, config ServerConfiguration, cr *chrome.Chrome, br *browser.Browser, tconn *chrome.TestConn) error {
	...
	if certExists, err := CertificateExists(ctx, cr, br, tconn, config.CaCommonName(), config.CaCertificateOrganization()); err != nil {
	...
	if certExists, err := CertificateExists(ctx, cr, br, tconn, config.CaCommonName(), config.CaCertificateOrganization()); err != nil {
		...
		return errors.Errorf("failed to add certificate with name %q", config.CaCommonName())
	}
