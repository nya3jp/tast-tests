...
func setCertificateDefaults(configuration ServerConfiguration) ServerConfiguration {
	if configuration.CaCommonName == "" {
		configuration.CaCommonName = caCommonNameDefault
	}
	if configuration.CaCertificateOrganization == "" {
		configuration.CaCertificateOrganization = caCertificateOrganizationDefault
	}
...
func ConfigureChromeToAcceptCertificate(ctx context.Context, configuration ServerConfiguration, chrome *chrome.Chrome, targetBrowser *browser.Browser, testConnection *chrome.TestConn) error {
	configuration = setCertificateDefaults(configuration)

	// Check if the cert already exists -> No need to add again.
	if certExists, err := CheckIfCertificateExists(ctx, chrome, targetBrowser, testConnection, configuration.CaCommonName, configuration.CaCertificateOrganization); err != nil {
		return err
	} else if certExists {
		return nil
	}
	...
	if certExists2, err := CheckIfCertificateExists(ctx, chrome, targetBrowser, testConnection, configuration.CaCommonName, configuration.CaCertificateOrganization); err != nil {
		return err
	} else if !certExists2 {
