...
func init() {
	...
	sort.Slice(sortedGBBFlags, func(i, j int) bool {
		return int(sortedGBBFlags[i]) < int(sortedGBBFlags[j])
	})
}
...
func NewImage(ctx context.Context) (*Image, error) {
	...
	_, err = testexec.CommandContext(ctx, "flashrom", "-p", "host", "-r", tmpFile.Name()).Output(testexec.DumpLogOnError)
	if err != nil {
...
// GetGBBFlags returns the list of clared and list of set flags.
func (i *Image) GetGBBFlags() ([]pb.GBBFlag, []pb.GBBFlag, error) {
	var gbb uint32
	err := i.readSectionData(GBBImageSection, gbbHeaderOffset, 4, &gbb)
	if err != nil {
		return nil, nil, err
	}
...
func calcGBBFlags(mask uint32) []pb.GBBFlag {
	var res []pb.GBBFlag
	// use soreted flags to return a deterministic order of flags.
...
func (i *Image) readSectionData(sec ImageSection, off, sz uint, out interface{}) error {
	...
	if len(i.Data) < int(end) {
		return errors.Errorf("Data length too short: %v (<=%v)", len(i.Data), end)
	}
	b := i.Data[beg:end]
	r := bytes.NewReader(b)
