...
func OpenAmbientSubpage(ui *uiauto.Context) uiauto.Action {
	return uiauto.Combine("open Ambient Subpage",
		...
		ui.WaitUntilExists(nodewith.Role(role.Button).HasClass("breadcrumb").Name("Screensaver")))
}
...
// isAmbientModeEnabled checks whether ambient mode is on.
func isAmbientModeEnabled(ctx context.Context, ui *uiauto.Context) (bool, error) {
...
func EnableAmbientMode(ctx context.Context, ui *uiauto.Context) error {
	if ambientMode, err := isAmbientModeEnabled(ctx, ui); err != nil {
