...
// "media string" in ambient mode. Taken from the basic youtube cuj test.
...
func waitForAmbientStart(tconn *chrome.TestConn, ui *uiauto.Context, timeout time.Duration) uiauto.Action {
	return func(ctx context.Context) error {
		...
		// .First() is needed because there are 2 actually media string "nodes"
		// present in the UI tree during slideshow mode. The second node is not
		// visible in the background and is a product of how slideshow mode is
		// implemented. Without .First() though, this condition fails saying there
		// are multiple matching nodes.
		return uiauto.Combine("validate ambient screen and media string",
			ui.Exists(nodewith.ClassName("LockScreenAmbientModeContainer").Role(role.Window)),
			ui.Exists(nodewith.NameContaining(TestVideoSrc.Title).First()),
		)(ctx)
	}
}
