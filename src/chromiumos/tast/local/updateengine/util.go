...
func IsFeatureEnabled(ctx context.Context, feature Feature) (bool, error) {
	f, err := getFeatureMapping(ctx, feature)
	if err != nil {
		return false, errors.Wrap(err, "failed to get feature mapping")
	}
	testing.ContextLog(ctx, "is feature enabled: ", f)

	buf, err := testexec.CommandContext(ctx, ClientBin, "--is_feature_enabled="+f).Output(testexec.DumpLogOnError)
	if err != nil {
		return false, errors.Wrap(err, "failed to toggle feature")
	}
...
func ToggleFeature(ctx context.Context, feature Feature, enable bool) error {
	f, err := getFeatureMapping(ctx, feature)
	if err != nil {
		return errors.Wrap(err, "failed to get feature mapping")
	}
	testing.ContextLog(ctx, "toggle feature: ", f, " to ", enable)

	var arg string
	if enable {
		arg = "--enable_feature=" + f
	} else {
		arg = "--disable_feature=" + f
	}
