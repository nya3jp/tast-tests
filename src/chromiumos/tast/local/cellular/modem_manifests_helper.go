...
func GetModemFirmwareDevice(ctx context.Context) (*mfwd.Device, error) {
	...
	if dutVariant == "" {
		testing.ContextLog(ctx, "No device with empty variant found in manifest, defaulting to first entry")
...
func GetModemFirmwareHelperEntry(ctx context.Context) (*mfwd.HelperEntry, error) {
	...
	if dutVariant == "" {
		testing.ContextLog(ctx, "No helper entry with empty variant found in manifest, defaulting to first entry")
...
func RestartModemWithHelper(ctx context.Context) error {
	...
	if err := modemfwd.WaitForDevice(ctx, device.DeviceId); err != nil {
		return errors.Wrap(err, "failed to wait for device")
	}
