...
func (h *Helper) ResetShill(ctx context.Context) []error {
	...
	// Disable Wifi to avoid log polution
...
func SetServiceProvidersExclusiveOverride(ctx context.Context, sourceFile string) error {
	input, err := ioutil.ReadFile(sourceFile)
	if err != nil {
		return errors.Wrapf(err, "failed to read %q", sourceFile)
	}
	fName := "/usr/share/shill/serviceproviders-exclusive-override.pbf"
	err = ioutil.WriteFile(fName, input, 0644)
	if err != nil {
		return errors.Wrapf(err, "failed to write %q", fName)
	}
	return nil
}
