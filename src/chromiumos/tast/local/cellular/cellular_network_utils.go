...
// VerifyArcIPConnectivity verifies the ip connectivity from ARC via cellular interface.
func VerifyArcIPConnectivity(ctx context.Context, cmd func(context.Context, string, ...string) *testexec.Cmd, ipType string) error {
	if ipType == "ipv4" || ipType == "ipv4v6" {
		testing.ContextLog(ctx, "Verify IPv4 connectivity")
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			if err := cmd(ctx, "/system/bin/ping", "-c1", "-w1", googleDotComIPv4).Run(); err != nil {
				return errors.Wrap(err, "failed ipv4 ping test in ARC")
			}
			return nil
		}, &testing.PollOptions{Timeout: pingTimeout}); err != nil {
			return errors.Wrap(err, "failed ipv4 ping test in ARC")
	...
	if ipType == "ipv6" || ipType == "ipv4v6" {
		testing.ContextLog(ctx, "Verify IPv6 connectivity")
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			if err := cmd(ctx, "/system/bin/ping6", "-c1", "-w1", googleDotComIPv6).Run(); err != nil {
				return errors.Wrap(err, "failed ipv6 ping test in ARC")
			}
			return nil
		}, &testing.PollOptions{Timeout: pingTimeout}); err != nil {
			return errors.Wrap(err, "failed ipv6 ping test in ARC")
