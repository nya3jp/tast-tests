...
func (f *eSimMojoFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	const url = "chrome://os-settings/internet"
	...
	f.cr = cr
	f.manager = &ESimManager{Crconn: conn, JS: &js}

	euiccs, err := f.manager.GetAvailableEuicc(ctx)
	if err != nil {
	...
	euiccProperties, err := euicc.GetProperties(ctx)
	if err != nil {
