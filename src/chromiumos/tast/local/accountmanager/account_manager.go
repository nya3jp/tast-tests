...
func CheckIsAccountPresentInArcAction(tconn *chrome.TestConn, d *androidui.Device, accountName string, expectedVal bool) action.Action {
	return func(ctx context.Context) error {
		...
		if err := testing.Poll(ctx, func(ctx context.Context) error {
			...
			// We can return early if account is expected to be present in ARC and it's already present.
			if expectedVal && accountExists {
				return nil
			}
			// If account expected to NOT be present in ARC - keep polling until timeout to make sure that it doesn't appear.
