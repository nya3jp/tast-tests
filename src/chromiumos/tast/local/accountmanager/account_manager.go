...
func CheckIsAccountPresentInARCAction(tconn *chrome.TestConn, d *androidui.Device, accountName string, expectedPresentInARC bool) action.Action {
	return func(ctx context.Context) error {
		if err := OpenARCAccounts(ctx, tconn, d); err != nil {
			return err
		}
		if err := CheckIsAccountPresentInARC(ctx, tconn, d, accountName, expectedPresentInARC); err != nil {
			return err
		}
		return nil
	}
...
func CheckIsAccountPresentInARC(ctx context.Context, tconn *chrome.TestConn, d *androidui.Device, accountName string, expectedPresentInARC bool) error {
	...
	accountExists := false
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		if err := scrollLayout.WaitForExists(ctx, time.Second); err == nil {
	...
	}, &testing.PollOptions{Timeout: arcAccountCheckTimeout, Interval: time.Second}); err != nil {
...
// OpenARCAccounts opens ARC account list page in ARC Settings.
func OpenARCAccounts(ctx context.Context, tconn *chrome.TestConn, d *androidui.Device) error {
