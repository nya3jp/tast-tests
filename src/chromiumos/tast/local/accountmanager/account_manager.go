...
// Add adds an account in-session. Account addition dialog should be already open.
func Add(ctx context.Context, tconn *chrome.TestConn, email, password string) error {
	// Set up keyboard.
...
// IsPresentInArc returns `true` if account is present in ARC Settings > Accounts.
func IsPresentInArc(ctx context.Context, tconn *chrome.TestConn, a *arc.ARC, accountName string) (bool, error) {
	const (
