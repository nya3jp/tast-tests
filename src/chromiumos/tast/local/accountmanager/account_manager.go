...
func AddAccount(ctx context.Context, tconn *chrome.TestConn, email, password string) error {
	...
	// After the iframe loads, the ui tree may not get updated. In this case only retry (which hides and then shows the iframe again) is required.
	if err := ui.Retry(2, func(ctx context.Context) error {
		...
		// Click 'Back' and then 'Next' to refresh the ui tree.
