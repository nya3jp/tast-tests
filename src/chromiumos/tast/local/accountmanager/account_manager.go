...
func TestCleanup(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome, brType browser.Type) error {
	...
	settings, err := ossettings.LaunchAtPageURL(ctx, tconn, cr, "accountManager", ui.Exists(nodewith.Name("Add Google Account").Role(role.Button)))
	if err != nil {
		return errors.Wrap(err, "failed to launch Account Manager page")
	}
	// Close settings window using OSSettings.Close API - Window.CloseWindown
	// API call fails often with http://go/paste/5693331754450944.
