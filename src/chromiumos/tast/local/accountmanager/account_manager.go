...
func AddAccount(ctx context.Context, tconn *chrome.TestConn, email, password string) error {
	...
	if err := uiauto.Combine("Enter email add password",
		// Enter the User Name.
		kb.TypeAction(email+"\n"),
		...
		return errors.Wrap(err, "failed enter email and password")
	}
...
func CheckIsAccountPresentInArcAction(tconn *chrome.TestConn, d *androidui.Device, accountName string, expectedVal bool) action.Action {
	return func(ctx context.Context) error {
		...
		if expectedVal {
			// Find account with long timeout.
			if err := account.WaitForExists(ctx, LongUITimeout); err != nil {
				return errors.Wrap(err, "failed to find account")
			}
		} else {
			if err := account.WaitUntilGone(ctx, DefaultUITimeout); err != nil {
				return errors.Wrap(err, "failed to wait until account is gone")
			}
		}
		return nil
	}
