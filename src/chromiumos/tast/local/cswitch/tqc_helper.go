...
func performHttpRequest(req *http.Request) (*http.Response, error) {
	...
	if err != nil {
		return resp, errors.Wrap(err, "failed to send post request for creating a session")
	}
	return resp, nil
...
func commandStatus(ctx context.Context, command_id string, domain string) (map[string]string, error) {
	...
	resp, err := performHttpRequest(req)
	if err != nil {
		return nil, errors.Wrap(err, "failed to send post request for creating a session")
	}
	defer resp.Body.Close()
...
func waitForCommandComplete(ctx context.Context, commandId string, domain string) error {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		resp, err := commandStatus(ctx, commandId, domain)
		if err != nil {
