...
func validatePartition(p *disk.PartitionStat) error {
	...
	case "ext4":
		if !devRegexp.MatchString(p.Device) || devLoopRegexp.MatchString(p.Device) {
			return fmt.Errorf("device path for the ext4 filesystem didn't match with /dev/* (except /dev/loop\\d+): %s", p.Device)
		}
	case "ecryptfs":
		if !shadowRegexp.MatchString(p.Device) {
			return fmt.Errorf("device path for ecryptfs didn't match with /home/.shadow/*/vault: %s", p.Device)
		}
	}
	return nil
}
...
func WaitForUserMount(ctx context.Context, user string) error {
	...
	testing.ContextLog(ctx, "Waiting for cryptohome for user: ", user)
