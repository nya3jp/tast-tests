...
func (c *RecoveryTestTool) Validate(ctx context.Context) error {
	...
	var err error
	if recoverySecretCreatedData, err = ioutil.ReadFile(c.getFullFilePath(recoverySecretCreatedFile)); err != nil {
		return errors.Wrapf(err, "could not read the created recovery secret file (%s)", recoverySecretCreatedFile)
	}
	if recoverySecretDecrypedData, err = ioutil.ReadFile(c.getFullFilePath(recoverySecretDecryptedFile)); err != nil {
