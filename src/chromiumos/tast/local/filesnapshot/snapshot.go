...
func (snapshot *Snapshot) save(filename string) error {
	...
	snapshot.table[filename] = snapshotValue{content, stat.Mode(), int(rawStat.Uid), int(rawStat.Gid)}
	return nil
}

// restore restores the snapshot of "filename". Note that the snapshot entry is not removed after the operation.
func (snapshot *Snapshot) restore(filename string) error {
	val, ok := snapshot.table[filename]
	if !ok {
...
func (snapshot *Snapshot) Stash(filename string) error {
...
func (snapshot *Snapshot) Pop(filename string) error {
	if err := snapshot.restore(filename); err != nil {
		return errors.Wrap(err, "failed to restore from snapshot")
	}
	delete(snapshot.table, filename)
	return nil
}
