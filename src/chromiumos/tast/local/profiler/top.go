...
func (t *top) end() (PerfOutput, error) {
	...
	if errClose := t.out.Close(); errClose != nil {
		return PerfOutputNull(), errors.Wrap(errClose, "failed closing output file")
	}
	if errTop != nil {
		return PerfOutputNull(), errors.Wrap(errTop, "failed waiting for the command 'top' to exit")
	}
	if errAwk != nil {
		return PerfOutputNull(), errors.Wrap(errAwk, "failed waiting for the command 'awk' to exit")
	}
