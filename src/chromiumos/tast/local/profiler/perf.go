...
func PerfStatOpts(pid int) *PerfOpts {
	return &PerfOpts{t: perfStat, pid: pid}
}
...
func (p *perf) handleStat() (Output, error) {
	...
	if err != nil {
		return OutputNull(), errors.Wrap(err, "failed parse stat file")
	}

	return Output{Props: map[string]interface{}{"cyclesPerSecond": cyclesPerSecond}}, nil
}

func (p *perf) handleOutput() (Output, error) {
