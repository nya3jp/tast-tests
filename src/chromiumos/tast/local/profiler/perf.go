...
type perfRecordOpts struct {
	...
	sampling PerfRecordSamplingType
}
...
func PerfRecordOpts(recordEvent string, samplePeriod int, samplingMode PerfRecordSamplingType) *PerfOpts {
	return &PerfOpts{record: &perfRecordOpts{event: recordEvent, period: samplePeriod, sampling: samplingMode}}
}
...
func getCmd(ctx context.Context, outDir string, opts *PerfOpts) (*testexec.Cmd, error) {
	...
	if opts.record != nil {
		...
		if opts.record.period > 0 {
			perfArgs = append(perfArgs, "-c", strconv.Itoa(opts.record.period))
		}
		switch opts.record.sampling {
