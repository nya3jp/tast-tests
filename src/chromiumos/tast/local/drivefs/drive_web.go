...
// RefreshFileList refreshes Recent and Trash files from Drive Web
func (d *DriveWeb) RefreshFileList(ctx context.Context) error {
	recentFiles, err := waitForFileThenGetFilesList(ctx, d.cr, Recent, "")
	if err != nil {
		return errors.Wrap(err, "failed getting drive web recent files")
	}
	d.recentFiles = recentFiles

	trashedFiles, err := waitForFileThenGetFilesList(ctx, d.cr, Trash, "")
	if err != nil {
		return errors.Wrap(err, "failed getting drive web trashed files")
	}
	d.trashedFiles = trashedFiles
