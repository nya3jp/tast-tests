...
// Requires that the supplied *chrome.Chrome was logged in using a GAIA login.
func WaitForDriveFs(ctx context.Context, cr *chrome.Chrome) (string, error) {
	...
	testing.ContextLog(ctx, "Logged in as: ", normUser)
	...
	testing.ContextLog(ctx, "Drivefs fully started")
