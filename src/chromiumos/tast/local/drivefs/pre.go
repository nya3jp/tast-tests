...
type PreData struct {
	...
	// Helper wrapper to find Recently and Trashed files in Google Drive Web
	DriveWeb *DriveWeb
}
...
func (p *preImpl) Prepare(ctx context.Context, s *testing.PreState) interface{} {
	...
	p.drive = &DriveWeb{p.cr, p.tconn, []string{}, []string{}}
...
func (p *preImpl) buildPreData(ctx context.Context, s *testing.PreState) PreData {
	...
	return PreData{p.cr, p.mountPath, p.tconn, p.drive}
}
