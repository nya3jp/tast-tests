...
func copyLogs(ctx context.Context, oldInfo os.FileInfo, outDir string) error {
	...
	if os.SameFile(info, oldInfo) {
		...
		prevLog := varLogMessages + ".1"

		sfp, err := os.Open(prevLog)
		if err != nil {
			_, _ = io.Copy(df, sf)

			return errors.Wrapf(err, "failed to read log: failed to open %s", prevLog)
		}
		defer sfp.Close()
		...
		if _, err = sfp.Seek(oldInfo.Size(), 0); err != nil {
			...
			return errors.Wrapf(err, "failed to read log: failed to seek %s", prevLog)
		}
		...
		if _, err = io.Copy(df, sfp); err != nil {
			...
			return errors.Wrapf(err, "failed to write log: failed to copy previous %s", prevLog)
		}

		// Copy current log
		if _, err = io.Copy(df, sf); err != nil {
			return errors.Wrapf(err, "failed to write log: failed to copy current %s", prevLog)
		}
	}
