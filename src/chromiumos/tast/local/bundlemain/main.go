...
func preTestRun(ctx context.Context, s *testing.State) func(ctx context.Context, s *testing.State) {
	...
	if freeSpaceBefore/mib < lowSpaceThreshold {
		s.Logf("Low disk space before starting test: %d bytes available", freeSpaceBefore)
	}
	...
	return func(ctx context.Context, s *testing.State) {
		...
		if freeSpaceBefore < freeSpaceAfter {
			...
			spaceUsage = (freeSpaceBefore - freeSpaceAfter) / mib
		}

		if spaceUsage > spaceUsageThreshold {
			s.Logf("Missing cleanup: test filled %d MiB of the stateful partition", spaceUsage)
		}
	}
}
