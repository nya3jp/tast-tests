...
func (bw *browserWatcher) check() bool {
	...
	if bw.initialPID != -1 {
		...
		if bw.sessionManagerPID == -1 {
			bw.mutex.Lock()
			defer bw.mutex.Unlock()
			bw.browserErr = errors.New("session manager PID is not known even when the browser proces is running")
			return false
		}
	...
	if err != nil {
		bw.mutex.Lock()
		defer bw.mutex.Unlock()
		bw.browserErr = errors.Wrapf(err, "bwoser process %d existed; Chrome probably crashed", bw.initialPID)
		return false
	}
	ppid, err := proc.Ppid()
	if err != nil {
