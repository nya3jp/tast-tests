...
func (bw *browserWatcher) check() bool {
	...
	if bw.initialPID != -1 {
		...
		if bw.sessionManagerPID == -1 {
			bw.mutex.Lock()
			defer bw.mutex.Unlock()
			bw.browserErr = errors.New("session manager PID is not known even when the browser process is running")
			return false
		}
	...
	proc, err := process.NewProcess(bw.initialPID)
	if err != nil {
		bw.mutex.Lock()
		defer bw.mutex.Unlock()
		bw.browserErr = errors.Wrapf(err, "browser process %d existed; Chrome probably crashed", bw.initialPID)
		return false
	}
	ppid, err := proc.Ppid()
	if err != nil {
