...
func CreateTestNotification(ctx context.Context, tconn *chrome.TestConn, notificationType, title, message string) (string, error) {
	var id string
	if err := tconn.Call(ctx, &id, `async (notificationType, title, message, iconUrl) => tast.promisify(chrome.notifications.create)({type: notificationType, title: title, message: message, iconUrl: iconUrl})`, notificationType, title, message, "data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw=="); err != nil {
