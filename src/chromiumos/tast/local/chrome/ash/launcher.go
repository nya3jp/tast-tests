...
func prepareFakeApp(baseDir, appName, iconDir string, iconFileMap map[int]string, manifestExtras string) (string, error) {
	...
	const bgPg = `chrome.app.runtime.onLaunched.addListener(console.log);`
	...
	if err := ioutil.WriteFile(filepath.Join(extDir, "bg.js"), []byte(bgPg), 0644); err != nil {
		return "", errors.Wrapf(err, "failed to prepare bg.js for %s", appName)
	}

	if err := ioutil.WriteFile(filepath.Join(extDir, "manifest.json"), []byte(fmt.Sprintf(manifestTmpl, appName, iconJSON, manifestExtras)), 0644); err != nil {
