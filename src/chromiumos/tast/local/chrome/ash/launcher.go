...
// prepareFakeAppIcon creates icon images in different scales based on the given icon data. These images are stored in a directory created under `baseDir`.
...
// PrepareDefaultFakeApps creates directories for `num` fake apps (hosted apps) under
// the directory of baseDir and returns their path names. Each fake app uses a generated name. The intermediate
...
// PrepareFakeAppsWithNames is similar with PrepareDefaultFakeApps but with the given app names.
