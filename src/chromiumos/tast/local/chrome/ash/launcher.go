...
// generateDefaultFakeAppNames generates default names for fake apps.
func generateDefaultFakeAppNames(numFakeApps int) []string {
	fakeAppNames := make([]string, numFakeApps)
	for idx := 0; idx < numFakeApps; idx++ {
		fakeAppNames[idx] = fmt.Sprintf("fake_%d", idx)
	}
	return fakeAppNames
}
...
func GetPrepareFakeAppsOptions(numFakeApps int) ([]chrome.Option, string, error) {
	...
	dirs, err := PrepareDefaultFakeApps(extDirBase, generateDefaultFakeAppNames(numFakeApps), false)
	if err != nil {
...
// GetPrepareFakeAppsWithNamesOptions is similar with GetPrepareFakeAppsOptions,
// with a difference that GetPrepareFakeAppsWithNamesOptions allows the caller
// to specify app names.
func GetPrepareFakeAppsWithNamesOptions(names []string) ([]chrome.Option, string, error) {
	extDirBase, err := ioutil.TempDir("", "")
	if err != nil {
		return nil, extDirBase, errors.Wrap(err, "failed to create tempdir")
	}

	dirs, err := PrepareDefaultFakeApps(extDirBase, names, true)
	if err != nil {
		return nil, extDirBase, errors.Wrap(err, "failed to create fake apps")
	}
...
// GetPrepareFakeAppsWithIconDataOptions is similar with GetPrepareFakeAppsWithNamesOptions,
// with a difference that GetPrepareFakeAppsWithIconDataOptions allows the caller
// to specify app names and icon data.
func GetPrepareFakeAppsWithIconDataOptions(names []string, iconData [][]byte) ([]chrome.Option, string, error) {
...
func prepareFakeAppIcon(baseDir, imageDir string, iconData []byte) (string, map[int]string, error) {
	iconDir := filepath.Join(baseDir, imageDir)
	...
	for _, siz := range []int{32, 48, 64, 96, 128, 192} {
		...
		if err := saveImageAsPng(iconFileFullPath, imgToSave); err != nil {
			return "", nil, errors.Wrapf(err, "failed to save the icon file to %s", iconFileFullPath)
		}
...
// the directory of baseDir and returns their path names. Fake apps' names
// are specified by the parameter. `hasIcon` specifies whether a default icon
// should be used. The intermediate data may remain even when an error is
// returned. It is the caller's responsibility to clean up the contents under
// the baseDir. This also may update the ownership of baseDir.
