...
// PrepareFakeAppsWithNames is similar with PrepareDefaultFakeApps but app names
// are specified.
func PrepareFakeAppsWithNames(baseDir string, names []string) ([]string, error) {
	if err := extension.ChownContentsToChrome(baseDir); err != nil {
		return nil, errors.Wrapf(err, "failed to change ownership of %s", baseDir)
	}
	...
	for _, name := range names {
		dir, err := prepareIndividualFakeApp(baseDir, name, iconDir, iconFiles)
		if err != nil {
			return nil, errors.Wrapf(err, "failed to prepare data for %s", name)
		}
		dirs = append(dirs, dir)
	}
