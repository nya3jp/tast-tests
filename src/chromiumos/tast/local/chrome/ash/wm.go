...
// WaitForOverviewToFinishAnimating waits until overview is shown or hidden completely. Returns immediately if overview mode state matches |overview_state|.
func WaitForOverviewToFinishAnimating(ctx context.Context, tConn *chrome.Conn, overviewState OverviewState) error {
	expr := fmt.Sprintf(
		`tast.promisify(chrome.autotestPrivate.waitForOverviewState)('%s')`, overviewState)
