...
// allowing an error within the given margin.
...
// expectedValue and margin are expected to be in pixels, but the window bounds GetARCAppWindowInfo returns in DP, so dsf is used to convert it to PX.
...
// WaitForNewBoundsBorder waits until Chrome animation finishes completely and check the position of an edge of a window with the given package name.
...
func WaitForNewBoundsBorder(ctx context.Context, tconn *chrome.Conn, expectedValue int, border BorderType, dsf float64, pkgName string) error {
	return WaitForNewBoundsBorderWithMargin(ctx, tconn, expectedValue, border, dsf, 0, pkgName)
}

// WaitForNewBoundsWithMargin waits until Chrome animation finishes completely and check the bounds of a window with the given package name.
// allowing an error within the given margin.
// expectedValue and margin are expected to be in pixels, but the window bounds GetARCAppWindowInfo returns in DP, so dsf is used to convert it to PX.
func WaitForNewBoundsWithMargin(ctx context.Context, tconn *chrome.Conn, expectedBounds Rect, dsf float64, margin int, pkgName string) error {
	error := WaitForNewBoundsBorderWithMargin(ctx, tconn, expectedBounds.Left, Left, dsf, margin, pkgName)
	if error != nil {
		return errors.Wrap(error, "WaitForNewBoundsWithMargin failed with border type left")
	}

	error = WaitForNewBoundsBorderWithMargin(ctx, tconn, expectedBounds.Top, Top, dsf, margin, pkgName)
	if error != nil {
		return errors.Wrap(error, "WaitForNewBoundsWithMargin failed with border type top")
	}

	error = WaitForNewBoundsBorderWithMargin(ctx, tconn, expectedBounds.Left+expectedBounds.Width, Right, dsf, margin, pkgName)
	if error != nil {
		return errors.Wrap(error, "WaitForNewBoundsWithMargin failed with border type right")
	}

	error = WaitForNewBoundsBorderWithMargin(ctx, tconn, expectedBounds.Top+expectedBounds.Height, Bottom, dsf, margin, pkgName)
	if error != nil {
		return errors.Wrap(error, "WaitForNewBoundsWithMargin failed with border type bottom")
	}

	return nil
}

// WaitForNewBounds waits until Chrome animation finishes completely and check the bounds of a window with the given package name.
// expectedValue is expected to be in pixels, but the window bounds GetARCAppWindowInfo returns in DP, so dsf is used to convert it to PX.
func WaitForNewBounds(ctx context.Context, tconn *chrome.Conn, expectedBounds Rect, dsf float64, pkgName string) error {
	return WaitForNewBoundsWithMargin(ctx, tconn, expectedBounds, dsf, 0, pkgName)
}
