...
func GetARCAppWindowInfo(ctx context.Context, c *chrome.Conn, pkgName string) (Window, error) {
	...
	var requestWinInfo Window
	for _, window := range windows {
		if window.ARCPackageName == pkgName {
			requestWinInfo = *window
		}
	}
	if requestWinInfo == (Window{}) {
		return Window{}, errors.Errorf("failed to find the window for %q", pkgName)
	}
	return requestWinInfo, nil
}
