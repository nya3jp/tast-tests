...
// GetARCAppWindowsInfo queries into Ash and returns the ARC windows info.
func GetARCAppWindowsInfo(ctx context.Context, tconn *chrome.TestConn, pkgName string) ([]*Window, error) {
	return FindWindows(ctx, tconn, func(window *Window) bool {
		return window.ARCPackageName == pkgName
	})
}
...
// GetARCAppWindowStates gets the Chrome side window states of the ARC app windows with pkgName.
func GetARCAppWindowStates(ctx context.Context, tconn *chrome.TestConn, pkgName string) (windowStates []WindowStateType, err error) {
	windows, err := GetARCAppWindowsInfo(ctx, tconn, pkgName)
	if err != nil {
...
// FindWindows returns the Chrome windows with which the given predicate returns true.
func FindWindows(ctx context.Context, tconn *chrome.TestConn, predicate func(*Window) bool) (matchingWindows []*Window, err error) {
	windows, err := GetAllWindows(ctx, tconn)
	if err != nil {
		return nil, errors.Wrap(err, "failed to get all windows")
	}
