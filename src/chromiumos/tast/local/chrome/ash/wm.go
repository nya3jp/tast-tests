...
// GetWindow returns the Chrome window with which the given predicate returns true.
// If there are multiple, this returns the first found window.
func GetWindow(ctx context.Context, c *chrome.Conn, predicate func(*Window) bool) (*Window, error) {
	windows, err := GetAllWindows(ctx, c)
	if err != nil {
