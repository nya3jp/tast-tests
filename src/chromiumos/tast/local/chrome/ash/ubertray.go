...
type ToggleableQuickSetting int

// Enum for toggleable quick settings.
const (
	Bluetooth ToggleableQuickSetting = iota
	DoNotDisturb
	NightLight
)
...
var settingNames = map[ToggleableQuickSetting]string{
	Bluetooth:    "Toggle Bluetooth. Bluetooth is",
	DoNotDisturb: "Toggle Do not disturb. Do not disturb is",
	NightLight:   "Toggle Night Light. Night Light is",
}
...
func getQuickSettingPod(ctx context.Context, tconn *chrome.TestConn, setting ToggleableQuickSetting) (*ui.Node, error) {
	r, err := regexp.Compile(settingNames[setting])
	if err != nil {
