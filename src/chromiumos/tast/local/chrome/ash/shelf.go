...
// GetWaitUntilShelfIconAnimationFinishAction returns an action to wait for the shelf icon animation to finish.
func GetWaitUntilShelfIconAnimationFinishAction(tconn *chrome.TestConn) func(ctx context.Context) error {
	return func(ctx context.Context) error {
		return testing.Poll(ctx, func(ctx context.Context) error {
...
// GetShelfAppBoundsFromNames returns the screen bounds of the apps specified by appNames.
func GetShelfAppBoundsFromNames(ctx context.Context, tconn *chrome.TestConn, ui *uiauto.Context, appNames []string) ([]*coords.Rect, error) {
	...
	for index, appName := range appNames {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "failed to get the screen bounds of %s", appName)
		}
...
// VerifyShelfAppBounds verifies that the screen bounds of shelf apps specified by `appNames` satisfy the following conditions:
...
func VerifyShelfAppBounds(ctx context.Context, tconn *chrome.TestConn, ui *uiauto.Context, appNames []string, isHorizontal bool) error {
	...
	boundsArray, err := GetShelfAppBoundsFromNames(ctx, tconn, ui, appNames)
	if err != nil {
