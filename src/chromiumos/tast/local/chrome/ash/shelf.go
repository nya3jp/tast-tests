...
// WaitForHotseatToEnterCorrectAutoHideState waits for the hotseat to reach the expected autohide state.
func WaitForHotseatToEnterCorrectAutoHideState(ctx context.Context, tc *chrome.TestConn, AutoHideState bool) error {
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if hotseatInfo.IsAutoHidden != AutoHideState {
			return errors.Errorf("got hotseat (IsAutoHidden, animating) = (%v, %v); want (%v, false)", hotseatInfo.IsAutoHidden, hotseatInfo.IsAnimating, AutoHideState)
		}
...
// SwipeHotseatAndWaitForCompletion swipes the hotseat and changes the state between hidden to extended. The function does not end until the hotseat animation completes.
