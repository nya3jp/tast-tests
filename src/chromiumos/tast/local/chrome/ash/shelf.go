...
// GetPinnedAppIds returns the list of pinned app ids.
...
// ResetShelfPinState resets the shelf app pin states to default. This function should be called before any change in shelf pin states.
func ResetShelfPinState(ctx context.Context, tconn *chrome.TestConn) (func(ctx context.Context) error, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to reset shelf pin state")
	}
	...
	return func(ctx context.Context) error {
		...
		// Indicates the apps which were pinned intially but are unpinned now. We are going to pin these apps.
		...
		if err := PinApps(ctx, tconn, toPinApps); err != nil {
			return err
		}

		if err := UnpinApps(ctx, tconn, toUnpinApps); err != nil {
			return err
		}
...
// PinApps pins the shelf icon for the apps specified by appIDs.
...
// UnpinApps unpins the shelf icon for the apps specified by appIDs.
