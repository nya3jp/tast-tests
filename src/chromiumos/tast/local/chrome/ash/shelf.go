...
// EnterShelfOverflowWithFakeApps uses fake apps to enter overflow.
...
// makes the shelf unexpectedly not overflow.
func EnterShelfOverflow(ctx context.Context, tconn *chrome.TestConn, underRTL bool, appsToPin []*ChromeApp) error {
	...
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		if sum > len(appsToPin) {
			return testing.PollBreak(errors.Errorf("need more than %d apps to cause overflow", len(appsToPin)))
		}
