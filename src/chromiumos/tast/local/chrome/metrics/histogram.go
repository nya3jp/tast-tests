...
// Chrome's chromeos::ExternalMetrics services scraps the file and adds the
...
func clearHistogramTransferFileByName(fileName string) error {
	// We don't really want to create the file if it doesn't exist, but we also
	// don't want to return errors if it doesn't exist and there's no easy way to
	// distinguish file-didn't-exist errors from other errors.
