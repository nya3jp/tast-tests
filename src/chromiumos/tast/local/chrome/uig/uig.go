...
type Action struct {
	...
	do func(ctx context.Context, tconn *chrome.TestConn, parent *nodeRef) (*nodeRef, error)
...
// refFromNode creates a nodeRef from a *ui.Node.  The reference count is set to 1
// so a single release() call will release the underlying ui.Node.
func refFromNode(node *ui.Node) *nodeRef {
	return &nodeRef{refs: 1, node: node}
}

// copy creates a "copy" of the ui.Node pointer. Internally it simply increments the reference counter.
func (r *nodeRef) copy() {
	r.refs++
}
...
func Find(params ui.FindParams) *Action {
	return ParentOrRoot().Find(params).WithNamef("uig.Find(%v)", params)
}
...
func FindWithTimeout(params ui.FindParams, timeout time.Duration) *Action {
	return ParentOrRoot().FindWithTimeout(params, timeout).WithNamef("uig.FindWithTimeout(%v, %v)", params, timeout)
}
...
func WaitUntilDescendantExists(params ui.FindParams, timeout time.Duration) *Action {
	return ParentOrRoot().WaitUntilDescendantExists(params, timeout).WithNamef("uig.WaitUntilDescendantExists(%v, %v)", params, timeout)
}
...
func WaitUntilDescendantGone(params ui.FindParams, timeout time.Duration) *Action {
	return ParentOrRoot().WaitUntilDescendantGone(params, timeout).WithNamef("uig.WaitUntilDescendantGone(%v, %v)", params, timeout)
}
...
// The caller is responsible for calling Release() on the returned *nodeRef.
