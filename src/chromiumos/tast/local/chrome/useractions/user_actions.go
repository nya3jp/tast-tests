...
func (ar *actionResult) stringArray() ([]string, error) {
	...
	return []string{ar.actionName, ar.testName, fmt.Sprintf("%s", attrStr), fmt.Sprintf("%s", strings.Join(ar.tags, ", ")), fmt.Sprintf("%d", int64(ar.duration/time.Millisecond)), fmt.Sprintf("%s", strconv.FormatBool(ar.pass)), fmt.Sprintf("%s", errMessage)}, nil
}
