...
func StartSendFiles(ctx context.Context, cr *chrome.Chrome, senderJS string, filepaths []string) (*SendSurface, error) {
	...
	sendSurface := &SendSurface{conn: sendConn, js: sender}

	// Start discovery.
	if err := sendSurface.js.Call(ctx, nil, `async function() {await this.startDiscovery()}`); err != nil {
		return nil, errors.Wrap(err, "failed to start discovery")
	}

	// Check the result to ensure discovery was started successfully.
	const prop = "startDiscoveryRes"
	var res startDiscoveryResult
	if err := sendSurface.waitForProp(ctx, prop, 10*time.Second); err != nil {
		return nil, err
	}
	if err := sendSurface.js.Call(ctx, &res, `function(prop) {return this[prop]}`, prop); err != nil {
