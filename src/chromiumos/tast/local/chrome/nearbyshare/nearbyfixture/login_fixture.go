...
func init() {
	...
	// Basic login fixtures for general CrOS<->Android sharing. The Android account for these fixtures uses the prod version of Nearby Share.
	...
	testing.AddFixture(&testing.Fixture{
		Name: "nearbyShareGAIALoginModulefood",
		Desc: "CrOS login with GAIA and Nearby Share flags enabled",
		Contacts: []string{
			"chromeos-sw-engprod@google.com",
		},
		Parent: "nearbyShareAndroidSetupModulefood",
		Impl:   NewNearbyShareLogin(false, false, false, defaultMediums),
		Vars: []string{
			defaultCrOSUsername,
			defaultCrOSPassword,
	...
	testing.AddFixture(&testing.Fixture{
		Name: "nearbyShareGAIALoginAndroidAccountModulefood",
		Desc: "CrOS login with Android nearby share account and Nearby Share enabled",
		Contacts: []string{
			"chromeos-sw-engprod@google.com",
		},
...
func (f *nearbyShareLoginFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	if userOk && passOk {
		...
		case modulefood:
			crosUsername = s.RequiredVar(modulefoodAndroidUsername)
			crosPassword = s.RequiredVar(modulefoodAndroidPassword)
		case dev:
