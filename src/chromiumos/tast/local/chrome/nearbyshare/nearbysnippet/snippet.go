...
// Sets the AndroidNearbyDevice's transferCallback, which is needed when awaiting follow-up SnippetEvents when calling eventWaitAndGet.
func (a *AndroidNearbyDevice) SendFile(ctx context.Context, senderName, receiverName, shareFileName string, mimetype nearbyshare.MimeType, turnaroundTime time.Duration) error {
	// Reset the transferCallback between shares.
	a.transferCallback = ""
