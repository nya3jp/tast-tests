...
func CrOSSetup(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome, dataUsage DataUsage, visibility Visibility, deviceName string) error {
	...
	var settingsModule chrome.JSObject
	if err := nearbyConn.Call(ctx, &settingsModule, `async function() {return await import('./shared/nearby_share_settings.m.js')}`); err != nil {
		return errors.Wrap(err, "failed to import nearby_share_settings.m.js")
	}

	if err := settingsModule.Call(ctx, nil, `function() {this.getNearbyShareSettings().setEnabled(true)}`); err != nil {
		return errors.Wrap(err, "failed to enable Nearby Share from OS settings")
	}

	if err := settingsModule.Call(ctx, nil, `function(dataUsage) {this.getNearbyShareSettings().setDataUsage(dataUsage)}`, dataUsage); err != nil {
		return errors.Wrapf(err, "failed to call setDataUsage with value %v", dataUsage)
	}

	if err := settingsModule.Call(ctx, nil, `function(visibility) {this.getNearbyShareSettings().setVisibility(visibility)}`, visibility); err != nil {
		return errors.Wrapf(err, "failed to call setVisibility with value %v", visibility)
	}

	if deviceName != "" {
		var res DeviceNameValidationResult
		if err := settingsModule.Call(ctx, &res, `async function(name) {
			r = await this.getNearbyShareSettings().setDeviceName(name);
			return r.result;
		}`, deviceName); err != nil {
