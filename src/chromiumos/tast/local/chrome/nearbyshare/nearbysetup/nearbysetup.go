...
func AndroidSetup(ctx context.Context, accountUtilZipPath, username, password string, loggedIn bool, apkZipPath string, rooted bool, screenOff time.Duration, dataUsage nearbysnippet.DataUsage, visibility nearbysnippet.Visibility, name string) (*nearbysnippet.AndroidNearbyDevice, error) {
	...
	if !loggedIn {
		...
		if err := testDevice.Install(ctx, filepath.Join(tempDir, nearbysnippet.AccountUtilApk)); err != nil {
		...
		testing.ContextLog(ctx, "Removing Nearby GAIA user from the Android device")
		removeAccountCmd := testDevice.ShellCommand(ctx, "am", "instrument", "-w",
			"-e", "account", username, "com.google.android.tradefed.account/.RemoveAccount",
		)
		if out, err := removeAccountCmd.Output(); err != nil {
			return nil, errors.Wrap(err, "failed to run remove account command")
		} else if !strings.Contains(string(out), "INSTRUMENTATION_RESULT: result=SUCCESS") {
			return nil, errors.Errorf("failed to remove account from the device (%v)", string(out))
		}
