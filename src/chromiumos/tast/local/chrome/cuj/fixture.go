...
// localTestVar is a runtime variable that specifies whether to skip
// waiting for the CPU to cooldown and idle, which speeds up overall
// test runtime. This variable is expicitly made for local testing,
// when performance data does not matter.
var localTestVar = testing.RegisterVarString(
	"cuj.localTest",
	"",
	"A variable signifying whether or not to skip certain startup procedures for local testing",
)
