...
// GetCurrentBrowserTab gets the active tab on the last focused
// Chrome window. |tconn| is the browser test connection.
func GetCurrentBrowserTab(ctx context.Context, tconn *chrome.TestConn) (*Tab, error) {
	var tabs []Tab
	if err := tconn.Eval(ctx, `(async () => {
		return await tast.promisify(chrome.tabs.query)({active:true, currentWindow: true});
	})()`, &tabs); err != nil {
		return nil, errors.Wrap(err, "failed to get the currently focused browser tab")
	}

	if len(tabs) == 0 {
		return nil, errors.New("no currently focused browser tab was found")
	} else if len(tabs) > 1 {
		return nil, errors.New("found multiple currently active browser tabs")
	}
	return &tabs[0], nil
}

// GetBrowserTabByTitle gets a single tab that has a tab title that matches |title|.
// If there are multiple matches or 0 matches, it returns an error. |tconn| is the
...
func GetBrowserTabByTitle(ctx context.Context, tconn *chrome.TestConn, title string) (*Tab, error) {
	...
	if err := tconn.Eval(ctx, fmt.Sprintf(`(async () => {
		...
		return nil, errors.Wrapf(err, "failed to find tab with title %q", title)
	}
...
// CloseBrowserTabByTitle finds a tab with title |title|, and closes that tab. If
// there are multiple tabs with that title, it returns an error. |tconn| is the
// browser test connection.
