...
// WaitForElementBeingVisiable waits for an element to be visible
func WaitForElementBeingVisiable(ctx context.Context, conn *chrome.Conn, selector string) error {
...
func WaitAndClick(ctx context.Context, conn *chrome.Conn, selector string) (err error) {
	if err = WaitForElementBeingVisiable(ctx, conn, selector); err != nil {
		return
	}
