...
// will respect that. In that case, that same path must be given as the
// lacrosPath parameter. Otherwise, lacrosPath must be the empty string, in
// which case the path is taken from the fixture.
func SetUpPersistent(ctx context.Context, f interface{}, bt browser.Type, lacrosPath string) (*browser.Browser, func(ctx context.Context), error) {
	cr := f.(chrome.HasChrome).Chrome()
	switch bt {
	case browser.TypeAsh:
		return cr.Browser(), func(context.Context) {}, nil
	case browser.TypeLacros:
		...
		if lacrosPath == "" {
			lacrosPath = f.LacrosPath()
		}
		l, err := lacros.Connect(ctx, lacrosPath, lacros.UserDataDir)
		if err != nil {
