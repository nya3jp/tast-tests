...
// SetUp returns a Browser instance for a given browser type and an ash-chrome
// instance that has already been created at the call site.
...
// This is useful when tests call chrome.New for restarting ash-chrome and want to tell it to launch Lacros browser.
// It also returns a closure to be called in order to close the browser instance.
...
// Or, consider keeping Lacros alive by passing lacrosfixt.KeepAlive to chrome.New then
// call browserfixt.Connect to get a Browser instance with no window open.
