...
func SetUp(ctx context.Context, f chrome.HasChrome, bt browser.Type) (*browser.Browser, func(ctx context.Context), error) {
	cr := f.Chrome()
	switch bt {
...
// closure.
func SetUpWithURL(ctx context.Context, f chrome.HasChrome, bt browser.Type, url string) (*chrome.Conn, *browser.Browser, func(ctx context.Context), error) {
	cr := f.Chrome()
	switch bt {
	case browser.TypeAsh:
