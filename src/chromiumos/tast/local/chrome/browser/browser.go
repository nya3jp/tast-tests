...
// Close calls a closer function registered from a caller which is either lacros-chrome or ash-chrome.
func (b *Browser) Close(ctx context.Context) error {
	// TODO: Make this thread safe or leave it up to underlying closer.
	if b.closer != nil {
		testing.ContextLogf(ctx, "Closing browser. closer: %#v", b.closer)
		return b.closer(ctx)
	}
	testing.ContextLog(ctx, "No closer found skipping Close")
	return nil
}
