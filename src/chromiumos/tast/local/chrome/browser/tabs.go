...
func Tabs(ctx context.Context, tconn *TestConn) ([]Tab, error) {
	var tabs []Tab
...
// CloseTabsByID closes the browser tabs corresponding to the given IDs.
...
// CloseTabs closes all browser tabs.
func CloseTabs(ctx context.Context, tconn *TestConn) error {
...
// CloseTabsKeepNewTab closes all browser tabs except one showing the NewTab page.
...
func CloseTabsKeepNewTab(ctx context.Context, tconn *TestConn) error {
	tabs, err := Tabs(ctx, tconn)
	if err != nil {
