...
func init() {
	testing.AddFixture(&testing.Fixture{
		...
		Impl: NewMTPFixture("arc.mtpUser", "arc.mtpPassword", chrome.ARCEnabled(), chrome.ExtraArgs(arc.DisableSyncFlags()...)),
		Vars: []string{
			"arc.mtpUser",
			"arc.mtpPassword",
		},
...
func (f *mtpFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	// adb  requires wait for restart as root for first run after new build flash.
