...
var scanButtonFinder *nodewith.Finder = nodewith.Name("Scan").Role(role.Button).FinalAncestor(WindowFinder)

var doneButtonFinder *nodewith.Finder = nodewith.Name("Done").Role(role.Button).FinalAncestor(WindowFinder)
...
func Launch(ctx context.Context, tconn *chrome.TestConn) (*ScanApp, error) {
	...
	if err := ui.WithTimeout(time.Minute).WaitUntilExists(scanButtonFinder)(ctx); err != nil {
		return nil, err
	}

	return &ScanApp{tconn: tconn, ui: ui}, nil
}
...
func (s *ScanApp) Close(ctx context.Context) error {
	...
	return s.ui.WithTimeout(time.Minute).WaitUntilGone(WindowFinder)(ctx)
}
...
func (s *ScanApp) ClickMoreSettings() uiauto.Action {
	return s.ui.LeftClick(nodewith.Name("More settings").Role(role.Button).FinalAncestor(WindowFinder))
}
...
func (s *ScanApp) SetScanSettings(settings ScanSettings) uiauto.Action {
	...
	for _, dropdown := range []struct {
		...
		dropdownFinder := nodewith.Name(string(dropdown.name)).ClassName("md-select").FinalAncestor(WindowFinder)
		dropdownOptionFinder := nodewith.Name(dropdown.option).Role(role.ListBoxOption).FinalAncestor(WindowFinder)
		steps = append(steps, s.ui.LeftClick(dropdownFinder), s.ui.LeftClick(dropdownOptionFinder))
	}
...
func (s *ScanApp) Scan() uiauto.Action {
	return uiauto.Combine("Scan",
		s.ui.LeftClick(scanButtonFinder),
		// Wait until the done button is displayed to verify the scan completed
		// successfully.
		s.ui.WaitUntilExists(doneButtonFinder),
	)
}
...
func (s *ScanApp) ClickDone() uiauto.Action {
	return s.ui.LeftClick(doneButtonFinder)
}
