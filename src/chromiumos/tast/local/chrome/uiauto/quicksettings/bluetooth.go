...
var BluetoothDetailedViewPairNewDeviceButton = nodewith.ClassName("TopShortcutButton").NameContaining("Pair new device").Ancestor(bluetoothDetailedView)

// BluetoothDetailedViewSettingsButton is the Settings button child within the
// detailed Bluetooth view.
var BluetoothDetailedViewSettingsButton = nodewith.ClassName("TopShortcutButton").NameContaining("Bluetooth settings").Ancestor(bluetoothDetailedView)
...
func NavigateToBluetoothDetailedView(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if err := uiauto.Combine("Click the Bluetooth feature pod label",
		ui.LeftClick(PodLabelButton(SettingPodBluetooth)),
