...
// Collapse will attempt to collapse an existing Quick Settings tray if it
// exists, or will open the Quick Settings and collapse the opened tray.
func Collapse(ctx context.Context, tconn *chrome.TestConn) error {
	...
	checkIfCollapsed := func(ctx context.Context) error {
		if currentBounds, err := Rect(ctx, tconn); err != nil {
			...
			return errors.New("the Quick Settings tray is still collapsing")
		}
		return nil
	}
	...
	if err := testing.Poll(ctx, checkIfCollapsed, &testing.PollOptions{Interval: 500 * time.Millisecond, Timeout: uiTimeout}); err != nil {
		return errors.Wrap(err, "the Quick Settings tray did not collapse")
	}
	return nil
}
