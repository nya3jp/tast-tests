...
func OpenAudioSettings(ctx context.Context, tconn *chrome.TestConn) error {
	...
	exist, err = ui.IsNodeFound(ctx, ExpandButton)
	if err != nil {
		return errors.Wrap(err, "failed to check expand menu button")
	}
	if exist {
		if err := ui.WithTimeout(uiTimeout).WaitUntilExists(ExpandButton)(ctx); err != nil {
			return err
		}

		if err := ui.WithTimeout(uiTimeout).LeftClickUntil(ExpandButton, ui.WithTimeout(1*time.Second).WaitUntilExists(audioSettingsBtn))(ctx); err != nil {
