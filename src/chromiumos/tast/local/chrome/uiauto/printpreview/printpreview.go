...
func WaitForPrintPreview(tconn *chrome.TestConn) uiauto.Action {
	...
	return uiauto.Combine("wait for Print Preview to finish loading",
		uiauto.NamedAction("wait for Print Preivew to appear", ui.WithTimeout(10*time.Second).WaitUntilExists(destinationButton)),
		...
		uiauto.IfSuccessThen(ui.WithTimeout(1*time.Second).WaitUntilExists(loadingPreviewText), emptyAction),
