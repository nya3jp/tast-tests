...
// EmojiSugestionsOption represents the option name of Emoji suggestions toggle option.
const EmojiSugestionsOption = "Emoji suggestions"
...
func SetEmojiSuggestions(uc *useractions.UserContext, isEnabled bool) *useractions.UserAction {
	...
	action := func(ctx context.Context) error {
		...
		return uiauto.Combine("toggle setting and close page",
			settings.SetToggleOption(uc.Chrome(), EmojiSugestionsOption, isEnabled),
			settings.Close,
		)(ctx)
	}
