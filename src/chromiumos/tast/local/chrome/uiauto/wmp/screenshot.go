...
// CaptureScreenRecording captures screen recording according to the argument passed in, fullscreen, partial or window.
// Note currently only fullScreenRecording type is supported.
func CaptureScreenRecording(tconn *chrome.TestConn, sst screenRecordingType) action.Action {
	return func(ctx context.Context) error {
		if err := ensureInScreenCaptureMode(ctx, tconn); err != nil {
			return errors.Wrap(err, "failed to verify the ui of toolbar")
		}
		...
		case FullScreenRecording:
			if err := StartFullScreenRecording(tconn)(ctx); err != nil {
				return err
			}
		default:
			return errors.New("unknown screen recording type")
		}

		if err := EndScreenRecording(tconn)(ctx); err != nil {
			return errors.Wrap(err, "failed to check the screen recording taken")
		}
