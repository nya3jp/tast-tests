...
func Launch(ctx context.Context, tconn *chrome.TestConn) (*RMAApp, error) {
	...
	r := app(ctx, tconn, nodewith.Name(apps.ShimlessRMA.Name).Role(role.Window))
...
// App returns an instance of the Shimless RMA app test handler that references
// an existing Shimless RMA app dialog.
// An error is returned if the dialog cannot be found.
func App(ctx context.Context, tconn *chrome.TestConn) (*RMAApp, error) {
	r := app(ctx, tconn, nodewith.HasClass("ShimlessRmaDialogView").Role(role.Window))
	// Find the main Shimless RMA window
	if err := r.ui.WithTimeout(waitUITimeout).WaitUntilExists(r.rootNode)(ctx); err != nil {
		return nil, err
	}
...
// app returns an instance of the Shimless RMA app test handler.
func app(ctx context.Context, tconn *chrome.TestConn, rootNode *nodewith.Finder) *RMAApp {
	return &RMAApp{tconn: tconn, ui: uiauto.New(tconn), rootNode: rootNode}
}
