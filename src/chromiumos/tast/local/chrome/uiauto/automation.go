...
func (ac *Context) WaitUntilCheckedState(finder *nodewith.Finder, expectedState bool) Action {
	return func(ctx context.Context) error {
		return testing.Poll(ctx, func(ctx context.Context) error {
			...
			isNodeChecked := (nodeInfo.Checked == checked.True)
			if isNodeChecked != expectedState {
				return errors.Wrapf(err, "%v is disabled", nodeInfo.Name)
			}
			return nil
		}, &ac.pollOpts)
...
func (ac *Context) MousePress(finder *nodewith.Finder) Action {
	return func(ctx context.Context) error {
		loc, err := ac.Location(ctx, finder)
		if err != nil {
			return err
...
func (ac *Context) MouseRelease() Action {
	return mouse.Release(ac.tconn, mouse.LeftButton)
}
...
// DoDefaultUntil returns a function that calls doDefault() JS method until the condition returns no error.
// It will try to call doDefault() once before it checks the condition.
// This is useful for situations where there is no indication of whether the node is ready to receive clicks.
// It uses the polling options from the Context.
func (ac *Context) DoDefaultUntil(finder *nodewith.Finder, condition func(context.Context) error) Action {
	return func(ctx context.Context) error {
...
func IfSuccessThenWithLog(preFunc, fn Action) Action {
	return action.IfSuccessThenWithLog(preFunc, fn)
}
