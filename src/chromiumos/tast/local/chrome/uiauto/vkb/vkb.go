...
// TapKeyIgnoringCase returns an action simulating a mouse click event on the middle of the specified key via touch event.
...
func (vkbCtx *VirtualKeyboardContext) tapKeyFunc(keyName string, ignoreCase bool) uiauto.Action {
	...
	if ignoreCase {
		keyFinder = KeyFinder.NameRegex(regexp.MustCompile("(?i)" + keyName))
	}
	...
	return uiauto.NamedAction(
		...
		func(ctx context.Context) error {
			for _, key := range keys {
				vkbCtx.tapKeyFunc(key, ignoreCase)
