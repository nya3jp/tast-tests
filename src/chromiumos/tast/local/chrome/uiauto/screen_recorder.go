...
func NewScreenRecorder(ctx context.Context, tconn *chrome.TestConn) (*ScreenRecorder, error) {
	...
	if err := Run(ctx, "to start screen recorder through ui", ui.LeftClick(entireDesktopButton), ui.LeftClick(shareButton)); err != nil {
		return nil, err
	}
...
func (r *ScreenRecorder) Start(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if err := Run(ctx, "to close notification and wait for it to disappear", ui.WithInterval(1*time.Second).LeftClick(closeNotificationButton), ui.WaitUntilGone(messagePopupAlert)); err != nil {
		return err
	}
	return nil
}
