...
func DragIconToIcon(tconn *chrome.TestConn, ui *uiauto.Context, srcIndex, destIndex int) func(ctx context.Context) error {
...
func RemoveIconFromFolder(tconn *chrome.TestConn) uiauto.Action {
	return func(ctx context.Context) error {
		...
		// Check that the folder has closed.
		folderViewInfo, err := ui.Info(ctx, folderView)
		if err != nil {
			return errors.Wrap(err, "failed to get the folderViewInfo")
		}
		if folderViewInfo.State["invisible"] != true {
			return errors.Wrap(err, "failed to close the folder during item drag")
		}
...
// GetFolderSize finds an existing folder and returns the number of apps inside.
func GetFolderSize(ctx context.Context, tconn *chrome.TestConn) (int, error) {
	ui := uiauto.New(tconn)
	src := nodewith.Name("Folder Unnamed").ClassName(ExpandedItemsClass)
	folderLocation, err := ui.Location(ctx, src)
	if err != nil {
		return 0, errors.Wrap(err, "failed to get location for the folder")
	}
