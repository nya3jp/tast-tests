...
// items in the app list directly - for example, while testing launcehr search.
...
//	}
func SetUpLauncherTest(ctx context.Context, tconn *chrome.TestConn, tabletMode, productivityLauncher, stabilizeAppCount bool) (func(ctx context.Context) error, error) {
	...
	if stabilizeAppCount {
		if err := WaitForStableNumberOfApps(ctx, tconn); err != nil {
			stabilizeErr = errors.Wrap(err, "failed to wait for item count in app list to stabilize")
		}
	}
