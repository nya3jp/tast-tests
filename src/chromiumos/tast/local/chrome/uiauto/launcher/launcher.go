...
// whether productivity launcher is enabled, which is infered from the curren app list search UI state.
...
func Search(tconn *chrome.TestConn, kb *input.KeyboardEventWriter, query string) uiauto.Action {
	return func(ctx context.Context) error {
		...
		ui := uiauto.New(tconn)
		if err := ui.LeftClick(nodewith.ClassName("SearchBoxView"))(ctx); err != nil {
