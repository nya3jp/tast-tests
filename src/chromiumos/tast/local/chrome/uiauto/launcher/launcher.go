...
// FirstNonRecentAppItem returnes the first app list item view shown in the current app list UI that is not in the recent apps container.
func FirstNonRecentAppItem(ctx context.Context, tconn *chrome.TestConn) (int, error) {
	...
	// The recent apps container only exists if the productivity launcher feature is enabled.
