...
func TriggerAppListSortAndWait(ctx context.Context, ui *uiauto.Context, sortType SortType, item *nodewith.Finder) error {
	...
	case ColorSort:
		contextMenuName = "Color"
	}
	SortContextMenuItem := nodewith.Name(contextMenuName).ClassName("MenuItemView")
...
// GetItemIndicesByName returns the view indices of the items specified by app names in the given apps grid.
func GetItemIndicesByName(ctx context.Context, ui *uiauto.Context, appNames []string, appsGrid *nodewith.Finder) ([]int, error) {
	...
	for viewIndex, item := range appListItems {
		...
		if viewIndices[nameIndex] != defaultIndex {
			return viewIndices, errors.Wrapf(err, "multiple items have the same name as %v", item.Name)
		}
