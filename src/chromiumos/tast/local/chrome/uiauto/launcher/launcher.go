...
func FirstNonRecentAppItem(ctx context.Context, tconn *chrome.TestConn) (int, error) {
	...
	// If the recent apps container is not present (which will be the case if productivity launcher is disabled), all app list items
	// are non-recent app items, so return the index of the first one.
...
// folderOpensOnCreation indicates whether the folder view exected to get opened after creation (with no extra user input). This happens if
...
// paginatedAppList indicates whether the top level apps grid supports app pages. If the grid does not support pages (for example, bubble
// launcher is a "single-page" scrollable container), the method will not attempt to move the folder to the next page.
