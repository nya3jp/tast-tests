...
func IsItemOnCurrentPage(ctx context.Context, tconn *chrome.TestConn, item *nodewith.Finder) (bool, error) {
	...
	// Handle the case where the item is only partially within bounds clipped by the apps grid view (so it's onscreen).
	var gridLocation *coords.Rect
	if err := ui.Exists(nodewith.ClassName("AppsGridView"))(ctx); err == nil {
		gridLocation, err = ui.Location(ctx, nodewith.ClassName("AppsGridView"))
		if err != nil {
			return false, errors.Wrap(err, "failed to apps grid location")
		}
	} else {
		gridLocation, err = ui.Location(ctx, nodewith.ClassName("ScrollableAppsGridView"))
		if err != nil {
			return false, errors.Wrap(err, "failed to get scrollable apps grid location")
		}
	}
