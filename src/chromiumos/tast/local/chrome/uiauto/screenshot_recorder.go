...
type ScreenshotRecorder struct {
	...
	// stopackc is the chanel for the background task to tell the
	...
	go func() {
		...
		for !done {
			...
			case <-time.After(r.interval):
				...
				// All of the images that should be taken at regular
				// intervals have already been taken, because we should
				// take maxImages-1 images + the last image at the end
				// of the test. Compress the images here to be safe,
				// because it take a while for the test to finish. This
				// ensures that if something catastrophic were to
				// happen while waiting for the test to complete, the
				// images will still be properly zipped.
...
// <number of screenshots>-<number of seconds since start>-<optional suffix>.jpg
// i.e. 1-10.jpg, 2-20-end.jpg
