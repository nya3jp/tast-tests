...
const zipName = "test-screenshots"
...
type ScreenshotRecorder struct {
	...
	// lastErr is the most recent error that occured in the recorder.
	lastErr error
	...
	// allowed to take, not including the end image.  Setting this
	// value is important to ensure tests that are hanging for much
	// longer than expected don't take unexpected number of screenshots.
	maxImages int
	...
	go func() {
		...
		for !done {
			...
			case <-time.After(r.interval):
				...
				// Since the max images have been taken, compress now.
				if err := r.compress(ctx); err != nil {
					testing.ContextLog(ctx, "screenshot_recorder: Failed to compress the screenshots: ", err)
				}
			case <-r.stopc:
...
// Stop stops the screenshot recorder. It takes one last picture that
// does not count towards the |r.maxImages| count. It also compresses
// any uncompressed screenshots into a zip file. This function can only
// be called after |Start| has been called.
...
// zip file |zipName|. Any successive calls to this function will
// update the original zip file to include any new screenshots taken
...
// <iteration>-<number of seconds since start>-<optional suffix>.jpg
// i.e. 1-10.jpg, 1-20-end.jpg
func (r *ScreenshotRecorder) captureDisplay(ctx context.Context, fileSuffix string) error {
	...
	path := filepath.Join(r.outDir, fmt.Sprintf("%d-%d%s.jpg", r.numImages+1, secondsSinceStart, fileSuffix))
