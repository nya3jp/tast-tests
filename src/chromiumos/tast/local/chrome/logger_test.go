...
func verifyLog(t *testing.T, m *logMaster, exp string) {
...
func TestLogger(t *testing.T) {
	...
	w1 := m.newLogWorker("foo", "fooURL", ev1)
	w2 := m.newLogWorker("bar", "barURL", ev2)
...
func TestLogger_Empty(t *testing.T) {
	...
	w := m.newLogWorker("targetID", "initURL", ev)
...
func TestLogger_ClearOnSave(t *testing.T) {
	...
	ev := newFakeConsoleAPICalledClient()
	w := m.newLogWorker("foo", "fooURL", ev)
