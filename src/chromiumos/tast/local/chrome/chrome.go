...
func (c *Chrome) testAPIConnFor(ctx context.Context, extConn **Conn, extID string) (*TestConn, error) {
	...
	if err := waitTestAPIReady(ctx, *extConn); err != nil {
		return nil, err
	}
...
// waitTestAPIReady waits until test APIs are ready on conn.
func waitTestAPIReady(ctx context.Context, conn *Conn) error {
	...
	if err := conn.Exec(ctx, "chrome.autotestPrivate.initializeEvents()"); err != nil {
		return errors.Wrap(err, "failed to initialize test API events")
	}
	return nil
}
...
func logIn(ctx context.Context, cfg *config, sess *driver.Session) (*driver.Session, error) {
	...
	case fakeLogin, gaiaLogin:
		...
		conn, err := sess.NewConnForTarget(ctx, driver.MatchTargetURL(ExtensionBackgroundPageURL(TestExtensionID)))
		if err != nil {
			return sess, err
		}
		defer conn.Close()
		if err := waitTestAPIReady(ctx, conn); err != nil {
			return sess, err
		}
		if err := conn.Eval(ctx, "tast.promisify(chrome.autotestPrivate.removeAllNotifications)()", nil); err != nil {
