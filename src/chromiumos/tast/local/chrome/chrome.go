...
func (c *Chrome) enterpriseEnrollTargets(ctx context.Context) ([]*target.Info, error) {
	...
	for _, target := range targets {
		...
		manageddomain := q["manageddomain"]

		if len(clientID) > 0 && len(manageddomain) > 0 {
			if strings.Contains(clientID[0], "apps.googleusercontent.com") &&
				strings.Contains(manageddomain[0], c.userDomain()) {
