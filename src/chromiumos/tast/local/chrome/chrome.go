...
// AddOnExtension returns an option that can be passed to New to add an extra extension
func AddOnExtension(s *testing.State, filesNames []string) option {
	return func(c *Chrome) {
		c.sExts = s
...
func (c *Chrome) writeExtensions() error {
	...
	if c.testExtID, err = writeTestExtension(
		filepath.Join(c.extsDir, "tast_api_extension")); err != nil {
		return err
	}
	if c.sExts != nil {
		if c.addOnTestExtID, err = writeAddOnTestExtension(filepath.Join(c.extsDir, "tast_addon_api_extension"), c.sExts, c.addOnExtsFileNames); err != nil {
			return err
		}
	}
...
func (c *Chrome) restartChromeForTesting(ctx context.Context) (port int, err error) {
	...
	//if c.sExts != nil {
	//	args = append(args, "--whitelisted-extension-id="+c.testExtID+","+c.addOnTestExtID)
	//} else {
	//	args = append(args, "--whitelisted-extension-id="+c.testExtID)
	//}
	switch c.arcMode {
	case arcDisabled:
