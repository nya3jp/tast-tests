...
func New(ctx context.Context, opts ...Option) (c *Chrome, retErr error) {
	...
	extArgs := exts.ChromeArgs() // for ash-chrome
	var lacrosExtArgs []string
	if len(cfg.LacrosExtraExtDirs()) != 0 {
		...
		lacrosExtArgs = lacrosExts.LacrosExtArgs() // for lacros-chrome
	}
	testing.ContextLog(ctx, "debug: extArgs: ", extArgs)
	testing.ContextLog(ctx, "debug: lacrosExtArgs: ", lacrosExtArgs)
