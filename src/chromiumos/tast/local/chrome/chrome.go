...
// TestExtConn returns a shared connection to the test API extension's
// background page (which can be used to access various API). The connection is
// lazily created, and this function will block until the extension is loaded or
// ctx's deadline is reached.
func (c *Chrome) TestExtConn(ctx context.Context) (*Conn, error) {
	if c.testExtConn != nil {
