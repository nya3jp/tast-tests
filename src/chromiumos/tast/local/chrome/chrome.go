...
func insertGAIAField(ctx context.Context, gaiaConn *Conn, inputID, nextID, value string) error {
	...
	for _, id := range []string{inputID, nextID} {
		if err := gaiaConn.WaitForExpr(ctx, fmt.Sprintf(
			"document.getElementById(%[1]q) != null && !document.getElementById(%[1]q).hidden", id)); err != nil {
		...
		(function() {
			...
			for (const button of buttons) {
				if ((button.textContent.indexOf(%[1]q) !== -1) ||
						(button.textContent.indexOf(%[2]q) !== -1)) {
					button.click();
					return;
				}
			}
			throw new Error('email not found; want %[1]q or %[2]q');
		})();`, normalizedParentUser, obfuscatedParentUser)
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		return gaiaConn.Exec(ctx, clickParentQuery)
	}, loginPollOpts); err != nil {
	...
	clickYesQuery := `
		(function() {
			const buttons = document.querySelectorAll('[role="button"]');
				...
				for (const button of buttons) {
					if (button.textContent.indexOf('Yes') !== -1) {
						button.click();
						return;
					}
				}
