...
const (
	...
	unicornLogin                  // real network-based login with Unicorn accounts
...
// UnicornAuth returns an Option that can be passed to New to configure the unicorn login credentials used by Chrome.
// Please do not check in real credentials to public repositories when using this in conjunction with UnicornLogin.
func UnicornAuth(childUser, childPass, parentUser, parentPass string) Option {
	return func(c *Chrome) {
		c.childUser = childUser
		c.childPass = childPass
...
// UnicornLogin returns an Option that can be passed to New to perform a real Unicorn-based login rather
// than the default fake login.
func UnicornLogin() Option {
	return func(c *Chrome) { c.loginMode = unicornLogin }
}
...
type Chrome struct {
	...
	childUser, childPass   string // unicorn child login credentials
	keepState              bool
	loginMode              loginMode
	region                 string
...
func New(ctx context.Context, opts ...Option) (*Chrome, error) {
	...
	} else if c.loginMode == unicornLogin {
		var err error
		if c.normalizedUser, err = session.NormalizeEmail(c.childUser, true); err != nil {
			return nil, errors.Wrapf(err, "failed to normalize email %q", c.user)
		}
	} else {
	...
	switch c.loginMode {
	case fakeLogin, gaiaLogin, unicornLogin:
...
func (c *Chrome) restartChromeForTesting(ctx context.Context) error {
	...
	if c.loginMode != gaiaLogin && c.loginMode != unicornLogin {
...
func (c *Chrome) logIn(ctx context.Context) error {
	...
	case unicornLogin:
		if err = c.performUnicornLogin(ctx, conn); err != nil {
			return err
		}
	}
...
func (c *Chrome) performGAIALogin(ctx context.Context, oobeConn *Conn) error {
	...
	gaiaConn, err := c.gaiaConn(ctx, oobeConn)
	if err != nil {
		return err
	}
	defer gaiaConn.Close()

	if err := insertGAIACredentials(ctx, gaiaConn, c.user, c.pass); err != nil {
		return err
	}
...
// gaiaConn creates a connection to the GAIA webview.
func (c *Chrome) gaiaConn(ctx context.Context, oobeConn *Conn) (*Conn, error) {
	...
	if err != nil {
		return nil, errors.Wrap(c.chromeErr(err), "failed to connect to GAIA webview")
	}
	return gaiaConn, nil
}

// insertGAIACredentials fills in the username and password fields of the GAIA login form.
// In the process, the form is submitted.
func insertGAIACredentials(ctx context.Context, gaiaConn *Conn, user string, pass string) error {
	testing.ContextLog(ctx, "Performing GAIA login")
	if err := insertGAIAField(ctx, gaiaConn, "identifierId", "identifierNext", user); err != nil {
		return errors.Wrap(err, "failed to fill username field")
	}
	if err := insertGAIAField(ctx, gaiaConn, "password", "passwordNext", pass); err != nil {
...
func insertGAIAField(ctx context.Context, gaiaConn *Conn, inputID string, nextID string, value string) error {
	// Ensure that the elements exist.
	if err := gaiaConn.WaitForExpr(ctx, fmt.Sprintf("document.getElementById(%q)", inputID)); err != nil {
		return errors.Wrapf(err, "failed to wait for %q element", inputID)
	}
	if err := gaiaConn.WaitForExpr(ctx, fmt.Sprintf("document.getElementById(%q)", nextID)); err != nil {
		return errors.Wrapf(err, "failed to wait for %q element", nextID)
	}
...
// performUnicornLogin waits for and interacts with the Unicorn webview to perform login.
// This function is heavily based on NavigateUnicornaLogin() in Catapult's
// telemetry/telemetry/internal/backends/chrome/oobe.py.
func (c *Chrome) performUnicornLogin(ctx context.Context, oobeConn *Conn) error {
	gaiaConn, err := c.gaiaConn(ctx, oobeConn)
	if err != nil {
		return err
	}
	...
	if err := insertGAIACredentials(ctx, gaiaConn, c.childUser, c.childPass); err != nil {
		return err
	}
	...
	obfuscatedParentUser := obfuscateUnicornEmail(normalizedParentUser)

	testing.ContextLogf(ctx, "Clicking parent email: %q or %q", normalizedParentUser, obfuscatedParentUser)
	clickParentQuery := fmt.Sprintf(`
			(function() {
				...
				return false;
			})();`, normalizedParentUser, obfuscatedParentUser)
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		var clicked bool
		if err := gaiaConn.Eval(ctx, clickParentQuery, &clicked); err != nil {
			return err
		}
		if !clicked {
		...
		return errors.Wrapf(c.chromeErr(err), "Parent email(%q, %q) not found", normalizedParentUser, obfuscatedParentUser)
	}
	...
	(function() {
		...
		if (buttons == null)
			return false;
		for (var i=0; i < buttons.length; ++i) {
			if (buttons[i].textContent.indexOf('Yes') != -1) {
				buttons[i].click();
				return true;
			}
		}
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		var clicked bool
		if err := gaiaConn.Eval(ctx, clickYesQuery, &clicked); err != nil {
			return err
		}
		if !clicked {
			return errors.New("buttons not found")
		}
		return nil
	}, loginPollOpts); err != nil {
		return errors.Wrap(c.chromeErr(err), "Yes button not found")
	}

	return nil
}

// obfuscateUnicornEmail creates an obfuscated version of an email.
// This function is based on _UnicornObfuscated() in Catapult's
// telemetry/telemetry/internal/backends/chrome/oobe.py.
func obfuscateUnicornEmail(email string) string {
	obfuscated := ""
	// Split at the email domain.
	separator := strings.Index(email, "@")

	// Obfuscate characters up to, but excluding, the one before the @.
	for i := 0; i < separator-1; i++ {
		// The 5th and 6th characters(index 4 and 5) are not converted to stars.
		if i == 4 || i == 5 {
			obfuscated += email[i : i+1]
		} else {
			obfuscated += "*"
		}
	}

	// Add the domain and character before the @ back.
	obfuscated += email[separator-1:]
	return obfuscated
}
