...
func makeTarget(t *devtool.Target) *Target {
...
func (c *Chrome) NewConnForTarget(ctx context.Context, tm TargetMatcher) (*Conn, error) {
	...
	f := func(t *devtool.Target) bool { return true }

	var all, matched []*devtool.Target
	if err := poll(ctx, func() error {
		var err error
		all, err = c.getDevtoolTargets(ctx, f)
		if err != nil {
	...
	if len(matched) != 1 {
		...
		for _, t := range all {
			testing.ContextLogf(ctx, "  %+v", makeTarget(t))
		}
