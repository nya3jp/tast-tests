...
func (c *Chrome) performGAIALogin(ctx context.Context, oobeConn *Conn) error {
	...
	testing.ContextLog(ctx, "Waiting for 60 sec...")
	time.Sleep(60 * time.Second)

	testing.ContextLog(ctx, "Enter local password")
	if err := insertOOBEField(ctx, oobeConn, "passwordInput", "test0000"); err != nil {
		return errors.Wrap(err, "failed to fill passwordInput field")
	}
	if err := insertOOBEField(ctx, oobeConn, "confirmPasswordInput", "test0000"); err != nil {
