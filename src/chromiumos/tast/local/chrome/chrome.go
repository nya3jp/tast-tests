...
func New(ctx context.Context, opts ...Option) (c *Chrome, retErr error) {
	...
	exts, err := extension.PrepareExtensions(extsDir, cfg.ExtraExtDirs, cfg.SigninExtKey, guestModeLogin)
	if err != nil {
...
// ResetSession re-creates a chrome session to the browser.
// After the session is reset, test connection needs to be regenerated by the caller by calling
// chrome.TestAPIConn().
func (c *Chrome) ResetSession(ctx context.Context) error {
	...
	sess, err := driver.NewSession(ctx, cdputil.DebuggingPortPath, cdputil.WaitPort, c.agg)
	if err != nil {
		testing.ContextLog(ctx, "Chrome cannot create a new session: ", err)
		return err
	}
