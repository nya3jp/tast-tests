...
func (c *Chrome) performGAIALogin(ctx context.Context, oobeConn *Conn) error {
	...
	isOOBESignin := func(t *target.Info) bool {
		return strings.HasPrefix(t.URL, "chrome://oobe/gaia-signin")
	}
	if targets, err := c.devsess.FindTargets(ctx, isOOBESignin); err != nil {
		return err
		...
		testing.ContextLog(ctx, "Found chrome://oobe/gaia-signin, force opening GAIA webview")
