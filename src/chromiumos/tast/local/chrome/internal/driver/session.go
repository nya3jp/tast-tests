...
func NewSession(ctx context.Context, execPath, debuggingPortPath string, portWait cdputil.PortWaitOption, agg *jslog.Aggregator) (cr *Session, retErr error) {
	...
	watcher, err := browserwatcher.NewWatcher(ctx, execPath)
	if err != nil {
		return nil, err
	}
...
func (s *Session) TestAPIConn(ctx context.Context, initializeAutotestPrivateEvents bool) (*TestConn, error) {
	return s.testAPIConnFor(ctx, &s.testExtConn, extension.TestExtensionID, initializeAutotestPrivateEvents)
}
...
// extID.
func (s *Session) testAPIConnFor(ctx context.Context, extConn **Conn, extID string, initializeAutotestPrivateEvents bool) (*TestConn, error) {
	if *extConn != nil {
	...
	if initializeAutotestPrivateEvents {
...
// Note: StopTracing should be called even if StartSystemTracing returns an error.
// Sometimes, the request to start tracing reaches the browser process, but there
// is a timeout while waiting for the reply.
func (s *Session) StartSystemTracing(ctx context.Context, perfettoConfig []byte) error {
	// Note: even when StartSystemTracing fails, it might be due to the case that the
	// StartSystemTracing request is successfully sent to the browser and tracing
	// collection has started, but the context deadline is exceeded before Tast
	// receives the reply.  Therefore, tracingStarted flag is marked beforehand.
	s.tracingStarted = true
