...
// LoginTimeout returns a custom timeout for login. If 0, use chrome.LoginTimeout.
func (c *Config) LoginTimeout() time.Duration {
	return time.Duration(c.m.LoginTimeout) * time.Second
}
...
type MutableConfig struct {
	...
	LoginTimeout                    int       `reuse_match:"false"`
