...
func waitForSingleGAIAWebView(ctx context.Context, sess *driver.Session, targetMatcher cdputil.TargetMatcher, pollOpt *testing.PollOptions) (*driver.Target, error) {
...
func performGAIALogin(ctx context.Context, cfg *config.Config, sess *driver.Session, oobeConn *driver.Conn) error {
	...
	target, err := waitForSingleGAIAWebView(ctx, sess, MatchSignInGAIAWebView(ctx, sess), pollOpts)
	if err != nil {
