...
func submitGAIAEnrollmentSignIn(ctx context.Context, oobeConn *driver.Conn, creds config.Creds, sess *driver.Session) error {
	if err := oobeConn.WaitForExprFailOnErr(ctx, "OobeAPI.screens.EnterpriseEnrollmentScreen.signInScreen.isReadyForTesting()"); err != nil {
...
// number of time, subjected to time out.
func performGAIAEnrollmentSignIn(ctx context.Context, oobeConn *driver.Conn, creds config.Creds, sess *driver.Session) error {
	...
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err := oobeConn.WaitForExprFailOnErr(ctx,
			"OobeAPI.screens.EnterpriseEnrollmentScreen.successScreen.isReadyForTesting()"); err == nil {
		...
		if err := oobeConn.Eval(ctx, "OobeAPI.screens.EnterpriseEnrollmentScreen.errorScreen.isReadyForTesting()", &isOnErrorStep); err != nil {
