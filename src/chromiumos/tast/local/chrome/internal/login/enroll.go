...
// failures are recoverable. The bypass these one-off failures, automation functions will re-attempt
// enterprise enrollment until consistent failure.
const maxGAIAEnterpriseEnrollmentRetries = 3

// maxGAIAEnterpriseEnrollmentTimeout is the maximum amount of time to wait for enterprise enrollment
// to succeed.
const maxGAIAEnterpriseEnrollmentTimeout = 3 * time.Minute
...
func waitForEnrollmentLoginScreen(ctx context.Context, cfg *config.Config, sess *driver.Session) error {
	...
	if err := oobeConn.WaitForExprWithTimeout(ctx,
		"OobeAPI.screens.GaiaScreen.isReady()", 10*time.Second); err != nil {
...
// performGAIAEnrollment will enterprise enroll the test device using the OOBE screen.
...
func performGAIAEnrollmentSignIn(ctx context.Context, oobeConn *driver.Conn, creds config.Creds, sess *driver.Session) error {
	...
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		retries--
		if retries <= 0 {
			return testing.PollBreak(errors.Wrap(err, "Exhausted retries"))
		}
		...
		return errors.New("hit temporary enrollment error")
	}, &testing.PollOptions{Timeout: maxGAIAEnterpriseEnrollmentTimeout})
}
