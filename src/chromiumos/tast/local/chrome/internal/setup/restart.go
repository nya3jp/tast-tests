...
func RestartChromeForTesting(ctx context.Context, cfg *config.Config, exts *extension.Files) error {
	...
	// Lacros features and additional args used to launch lacros-chrome should be delimited by '####' and
	// passed in from ash-chrome as a single argument with --lacros-chrome-additional-args.
	...
	if as := cfg.LacrosExtraArgs(); len(args) != 0 {
		// TODO: Check if the as has --enable-features and append it to the list above.
