...
func SetupPerfTest(ctx context.Context, tconn *chrome.TestConn, name string) (retCleanup CleanupCallback, retErr error) {
	...
	sup.Add(setup.PowerTest(ctx, tconn, setup.NoBatteryDischarge, setup.PowerTestOptions{
		Wifi: setup.DoNotChangeWifiInterfaces, NightLight: setup.DisableNightLight}))
