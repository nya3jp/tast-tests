...
// ConnectToLacros connects to a running lacros instance (e.g launched by the UI) and returns a Lacros object that can be used to interact with it.
func ConnectToLacros(ctx context.Context, lacrosPath, userDataDir string) (l *Lacros, retErr error) {
...
func LaunchFromShelf(ctx context.Context, tconn *chrome.TestConn, lacrosPath string) (*Lacros, error) {
	...
	l, err := ConnectToLacros(ctx, lacrosPath, UserDataDir)
	if err != nil {
...
// LaunchLacros launches a fresh instance of lacros-chrome.
func LaunchLacros(ctx context.Context, f lacrosfixt.FixtValue) (*Lacros, error) {
	return LaunchLacrosWithURL(ctx, f, chrome.BlankURL)
}

// LaunchLacrosWithURL launches a fresh instance of lacros-chrome having the given url.
func LaunchLacrosWithURL(ctx context.Context, f lacrosfixt.FixtValue, url string) (*Lacros, error) {
	succeeded := false
	defer lacrosfaillog.SaveIf(ctx, f, func() bool { return !succeeded })
	...
	l, err := ConnectToLacros(ctx, f.LacrosPath(), userDataDir)
	if err != nil {
		return nil, errors.Wrap(err, "failed to connect to debugging port")
	}
