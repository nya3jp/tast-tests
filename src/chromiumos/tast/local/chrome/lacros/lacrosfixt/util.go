...
// StateMixin is a private interface that contains common interfaces to allow both testing.FixtState and testing.State to be passed into ParseVars.
...
type SetupVars struct {
	...
	dataArtifactPath string // exists for 'External' setup mode
	mode             SetupMode
}

// ParseVars reads the vars needed to set up Lacros per setup mode.
// s is a StateMixin that allows both testing.FixtState and testing.State to be passed in.
...
// EnsureLacrosReadyForLaunch waits for the lacros binary to prepared and ready for use given the setup mode.
// It returns the lacros executable path. (eg, used to force quit the running process)
func EnsureLacrosReadyForLaunch(ctx context.Context, vars SetupVars) (string, error) {
