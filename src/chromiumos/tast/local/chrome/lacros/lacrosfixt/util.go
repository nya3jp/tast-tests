...
// StateMixin is an interface that contains common interfaces between testing.FixtState and testing.State.
// This is useful for the util functions that need to be exposed to fixture and/or test and take testing.FixtState or testing.State as an arg accordingly.
type StateMixin interface {
	Var(name string) (val string, ok bool)
	DataPath(p string) string
}

// SetupVars holds runtime vars or other variables needed to set up Lacros.
// This will be passed in to DefaultOpts to return common chrome options for Lacros, and to WaitForReady to prepare Lacros for use by either fixture or test.
type SetupVars struct {
	deployed         bool
	deployedPath     string // executable path
	dataArtifactPath string // retrieved from s.DataPath(dataArtifact) just for 'External' setup mode
	mode             SetupMode
}

// ParseVars reads the vars needed to set up Lacros per setup mode.
// s could be either *testing.FixtState for vars declared in fixture or *testing.State for the ones in test.
func ParseVars(s StateMixin, mode SetupMode) SetupVars {
	...
	return SetupVars{
		...
		dataArtifactPath: dataArtifactPath,
		mode:             mode}
}

// DefaultOpts returns common chrome options for Lacros given the vars and setup mode passed in.
func DefaultOpts(vars SetupVars, opts ...chrome.Option) ([]chrome.Option, error) {
	...
	if vars.deployed || vars.mode == External {
...
func WaitForReady(ctx context.Context, vars SetupVars) (string, error) {
	testing.ContextLogf(ctx, "Waiting for Lacros ready with vars: %+v", vars)
	if vars.deployed {
	...
	if vars.mode == Omaha || vars.mode == Rootfs {
	...
	switch vars.mode {
	case External:
		if err := prepareLacrosBinary(ctx, vars.dataArtifactPath); err != nil {
	...
	case Rootfs:
		...
		lacrosPath = filepath.Dir(matches[0])
	default:
		return "", errors.New("Unrecognized mode: " + string(vars.mode))
	}
