...
func (f *fixtImpl) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	// Read Vars
	vars := ParseVars(s, f.mode)

	// Set chrome options for Lacros from multiple sources: current fixture, parent fixture and common default.
	...
	defaultOpts, err := DefaultOpts(vars, opts...)
	if err != nil {
	...
	lacrosPath, err := EnsureLacrosReadyForLaunch(ctx, vars)
	if err != nil {
