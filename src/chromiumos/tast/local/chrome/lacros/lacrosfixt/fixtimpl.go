...
func init() {
	...
	testing.AddFixture(&testing.Fixture{
		...
		Desc:     "Lacros Chrome loading 100 fake apps with app sorting disabled but keepalive enabled",
		Contacts: []string{"jamescook@chromium.org", "hyungtaekim@chromium.org"},
		Impl: chrome.NewLoggedInFixture(func(ctx context.Context, s *testing.FixtState) ([]chrome.Option, error) {
			return NewConfig(
				KeepAlive(true), // Opening fake apps from the Launcher depend on appservice that needs KeepAlive on.
				ChromeOptions(chrome.DisableFeatures("LauncherAppSort"))).Opts()
		}),
		Parent:          "install100LacrosApps",
