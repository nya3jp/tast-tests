...
func SaveRecordIf(ctx context.Context, tconn *chrome.TestConn, hasRecord bool, hasError func() bool) {
	if hasRecord && hasError() {
		out, ok := testing.ContextOutDir(ctx)
		if !ok {
			testing.ContextLog(ctx, "OutDir not found")
			return
...
func StartRecord(ctx context.Context, tconn *chrome.TestConn) bool {
