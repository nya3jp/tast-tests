...
func (l *Lacros) Close(ctx context.Context) error {
	...
	if outDir, ok := testing.ContextOutDir(ctx); ok {
		...
		files, err := filepath.Glob(filepath.Join(lacros.UserDataDir, "lacros.log*"))
		if err == nil {
			for _, file := range files {
				filename := filepath.Base(file)
				if err := fsutil.CopyFile(file, filepath.Join(dstDir, filename)); err != nil {
					testing.ContextLogf(ctx, "Failed to save %s to %s: %v", filename, dstDir, err)
				} else {
					testing.ContextLogf(ctx, "%s saved to %s", filename, dstDir)
				}
			}
		}
	} else {
