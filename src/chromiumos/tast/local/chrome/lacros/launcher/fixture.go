...
func (f *fixtImpl) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	if f.mode == Rootfs {
		opts = append(opts, chrome.EnableFeatures("LacrosSupport", "ForceProfileMigrationCompletion"), chrome.ExtraArgs("--lacros-selection=rootfs"))
	} else if f.mode == Omaha {
		opts = append(opts, chrome.EnableFeatures("LacrosSupport", "ForceProfileMigrationCompletion"), chrome.ExtraArgs("--lacros-selection=stateful"))
	}
