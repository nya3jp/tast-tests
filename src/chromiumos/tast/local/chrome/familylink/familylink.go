...
// If `verify` is set to true - the account addition will be verified ("School
// account added" message in the dialog + confirming that new account was added
// in OS Settings).
...
// and attempts to add a secondary account for a Family Link (FL) primary user.
...
func AddEduSecondaryAccountWithOneParent(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn,
	parentUser, parentPass, secondUser, secondPass string, verify bool) error {
