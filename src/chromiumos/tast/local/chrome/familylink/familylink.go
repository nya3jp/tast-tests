...
func VerifyUserSignedIntoBrowserAsChild(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, bt browser.Type, email string) error {
	...
	for i := range nodes {
		row := rows.Nth(i)
		// If this is the email row, check value matches user email.
		accountCell := nodewith.Role(role.LayoutTableCell).NameContaining("Account").Ancestor(row)
		emailValueCell := nodewith.Role(role.LayoutTableCell).NameContaining(strings.ToLower(email)).Ancestor(row)
		if err := ui.Exists(accountCell)(ctx); err == nil {
			foundEmail = true
			if valueErr := ui.Exists(emailValueCell)(ctx); valueErr != nil {
				return errors.Wrap(valueErr, "user with email "+email+" is not logged into browser")
			}
		}
		// If this is the "is child" row, check value is true.
		childCell := nodewith.Role(role.LayoutTableCell).NameContaining("Child").Ancestor(row)
		trueCell := nodewith.Role(role.LayoutTableCell).NameContaining("true").Ancestor(row)
