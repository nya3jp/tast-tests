...
// AddEduSecondaryAccountForFamilyLink goes through the Edu
// Coexistence in-session flow and adds an EDU secondary account for a
// Family Link (FL) primary user. FL users can only add EDU secondary
// accounts.
...
func AddEduSecondaryAccountForFamilyLink(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, ui *uiauto.Context,
	...
	if err := kb.Type(ctx, parentPass+"\n"); err != nil {
		errors.Wrap(err, "failed to type parent password")
	}
