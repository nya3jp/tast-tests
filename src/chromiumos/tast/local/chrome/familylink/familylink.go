...
func maybeSelectParentFromDropdown(ctx context.Context, tconn *chrome.TestConn, parentEmail string) error {
	...
	parentAccessDialog := nodewith.Name("Parent Access").Role(role.Dialog)
	selectedParentEmail := nodewith.Ancestor(parentAccessDialog).Name(parentEmail).Role(role.StaticText)
	// Check if the parentEmail is already selected.
	if err := ui.Gone(selectedParentEmail)(ctx); err != nil {
		return nil
	}
