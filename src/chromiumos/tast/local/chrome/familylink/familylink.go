...
// and adds an EDU secondary account for a Family Link (FL) primary
// user. FL users can only add EDU secondary accounts.
...
func AddEduSecondaryAccount(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn,
	...
	if err := NavigateEduCoexistenceFlow(ctx, tconn, parentPass, eduUser, eduPass); err != nil {
