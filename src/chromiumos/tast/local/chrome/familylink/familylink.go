...
// AddEduSecondaryAccountForFamilyLink opens the Edu Coexistence
// in-session flow and adds an EDU secondary account for a Family Link
// (FL) primary user. FL users can only add EDU secondary accounts.
// Precondition: The current logged in user must be FL (such as Geller
// or Unicorn).
func AddEduSecondaryAccountForFamilyLink(ctx context.Context, cr *chrome.Chrome, tconn *chrome.TestConn, ui *uiauto.Context,
	parentFirstName, parentLastName, parentUser, parentPass,
	eduUser, eduPass string) error {
	...
	if err := NavigateEduCoexistenceFlow(ctx, ui, parentPass, eduUser, eduPass); err != nil {
...
// Precondition: The current logged in user must be FL (such as Geller
// or Unicorn).
func NavigateEduCoexistenceFlow(ctx context.Context, ui *uiauto.Context, parentPass,
	eduUser, eduPass string) error {
