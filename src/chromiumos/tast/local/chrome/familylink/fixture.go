...
// NewFamilyLinkFixtureNonOwner creates a new implementation of the Family Link
// fixture for a regular user device owner and Family Link non-owner.
...
// NewFamilyLinkFixtureUnicornPool creates a new implementation of the Family Link fixture with a pool of Unicorn accounts.
func NewFamilyLinkFixtureUnicornPool(parentUser, parentPassword, childPool string, opts ...chrome.Option) testing.FixtureImpl {
	return &familyLinkFixture{
		opts:           opts,
		parentUser:     parentUser,
		parentPassword: parentPassword,
...
func init() {
	testing.AddFixture(&testing.Fixture{
		...
		Impl:     NewFamilyLinkFixtureUnicornPool("family.unicornAccountPoolParentEmail", "family.unicornAccountPoolParentPassword", "family.unicornAccountPool"),
		Vars: []string{
	...
	testing.AddFixture(&testing.Fixture{
		...
		Contacts: []string{"tobyhuang@chromium.org", "cros-families-eng+test@google.com"},
		Impl:     NewFamilyLinkFixture("family.parentEmail", "family.parentPassword", "family.gellerEmail", "family.gellerPassword"),
		Vars: []string{
			"family.parentEmail",
	...
	testing.AddFixture(&testing.Fixture{
		...
		// TODO(b/250089099): We can't use NewFamilyLinkFixtureUnicornPool
		// here because the PersistentFamilyLink parent fixture hard-codes
		// the persistent policy user. Figure out a way to pass the selected
		// user to the parent fixture.
		Impl: NewFamilyLinkFixture("family.parentEmail", "family.parentPassword", "family.unicornEmail", "family.unicornPassword"),
		Vars: []string{
			"family.parentEmail",
			"family.parentPassword",
			"family.unicornEmail",
...
func (f *familyLinkFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	if len(f.childPool) > 0 {
		...
		creds.ParentUser = parentUser
		creds.ParentPass = parentPass
		childCreds = &creds
