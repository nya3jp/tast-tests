...
func init() {
	...
	testing.AddFixture(&testing.Fixture{
		...
		Vars: []string{
			// TODO(crbug/1186078): add these vars to familylink.yaml.
			"geller.parentUser",
	...
	testing.AddFixture(&testing.Fixture{
		Name: "familyLinkRegularArcLogin",
		Desc: "Non-supervised Family Link user login with Regular account and ARC support",
...
func (f *familyLinkFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	parentPassword := s.RequiredVar(f.parentPassword)
	if len(f.childUser) > 0 && len(f.childPassword) > 0 {
		childUser := s.RequiredVar(f.childUser)
		childPassword := s.RequiredVar(f.childPassword)
		f.opts = append(f.opts, chrome.Auth(childUser, childPassword, "gaia-id"), chrome.ParentAuth(parentUser, parentPassword))
	} else {
		f.opts = append(f.opts, chrome.Auth(parentUser, parentPassword, "gaia_id"))
	}
