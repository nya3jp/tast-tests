...
func (f *familyLinkFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	if isFLPolicyTest {
		if err := policyutil.ServeBlobAndRefresh(ctx, fdms, cr, fakedms.NewPolicyBlob()); err != nil {
...
func (f *familyLinkFixture) Reset(ctx context.Context) error {
	...
	if f.fdms != nil {
		...
		defer cancel()
		if err := policyutil.ServeBlobAndRefresh(ctx, f.fdms, f.cr, fakedms.NewPolicyBlob()); err != nil {
