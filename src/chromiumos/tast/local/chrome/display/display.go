...
func SetDisplayRotationSync(ctx context.Context, c *chrome.Conn, dispID string, rot RotationAngle) error {
	...
	expr := fmt.Sprintf(
		`tast.promisify(chrome.autotestPrivate.waitForDisplayRotation)(%q, %q).then(function(success) {
		    if (!success)
		      return Promise.reject(new Error("failed to wait for display rotation"));
		})`, dispID, rot)
