...
func TestSelectedMode(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.name, func(t *testing.T) {
			mode, err := c.info.GetSelectedMode()
			if c.expected == nil {
				if err == nil {
					t.Errorf("Expected to fail, but successfully got %+v", mode)
				}
			} else {
				if err != nil {
					t.Error("Failed to get the selected info: ", err)
				}
				if !reflect.DeepEqual(c.expected, mode) {
					t.Errorf("Unexpected mode is selected: want %+v, got %+v", c.expected, mode)
				}
			}
		})
	}
...
func TestEffectiveDeviceScaleFactor(t *testing.T) {
	for _, c := range []struct {
		...
		t.Run(c.name, func(t *testing.T) {
			dsf, err := c.info.GetEffectiveDeviceScaleFactor()
			if c.shouldSuccess {
				if err != nil {
					t.Fatal("Failed to get DSF: ", err)
				}
				if c.expectedDSF != dsf {
					t.Errorf("Incorrect DSF: want %f got %f", c.expectedDSF, dsf)
				}
			} else {
				if err == nil {
					t.Errorf("Expected to fail, but successfully got DSF %f", dsf)
				}
			}
		})
	}
