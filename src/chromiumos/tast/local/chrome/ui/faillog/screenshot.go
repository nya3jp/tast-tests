...
func SaveScreenshotOnError(ctx context.Context, cr *chrome.Chrome, outDir string, hasError func() bool, tconn *chrome.TestConn) {
	saveScreenshotToFileOnError(ctx, cr, outDir, hasError, tconn, screenshotFileName)
}

// saveScreenshotToFileOnError checks the given hasError function and takes screenshot into a file 'screetshotFile' when the test fails.
// It does nothing when the test succeeds.
func saveScreenshotToFileOnError(ctx context.Context, cr *chrome.Chrome, outDir string, hasError func() bool, tconn *chrome.TestConn, fileName string) {
	if !hasError() {
		return
	}
	...
	screetshotFile := filepath.Join(dir, screenshotFileName)
