...
func Show(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if err != nil {
		...
		testing.ContextLog(ctx, "bookmarks bar is already visible")
		return nil
	}

	if err := Toggle(ctx); err != nil {
...
func Hide(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if err != nil {
		...
		testing.ContextLog(ctx, "bookmarks bar is already not visible")
		return nil
	}
...
func Toggle(ctx context.Context) error {
	...
	testing.ContextLog(ctx, "Execute ctrl+shift+b")
