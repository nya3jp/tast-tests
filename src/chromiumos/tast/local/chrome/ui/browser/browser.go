...
// OpenBookmarkDialog opens bookmark menu for the current tab and waits for its
// element to exist.
func OpenBookmarkDialog(ctx context.Context, tconn *chrome.TestConn) error {
...
func BookmarkCurrentTab(ctx context.Context, tconn *chrome.TestConn) error {
	if err := OpenBookmarkDialog(ctx, tconn); err != nil {
		return err
	}
