...
// OpenBookmarkDialogAndWaitForIt opens bookmark menu for the current tab.
func OpenBookmarkDialogAndWaitForIt(ctx context.Context, tconn *chrome.TestConn) error {
...
func AddCurrentTabAsBookmark(ctx context.Context, tconn *chrome.TestConn) error {
	if err := OpenBookmarkDialogAndWaitForIt(ctx, tconn); err != nil {
		return err
	}
