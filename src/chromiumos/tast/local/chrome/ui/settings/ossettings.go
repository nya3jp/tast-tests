...
func LaunchAtPage(ctx context.Context, tconn *chrome.TestConn, subpage ui.FindParams) error {
	...
	// If needed, open the main menu. This may not be present depending on the
	// width of the screen, so don't error out if not needed.
	ui.FindAndClick(ctx, tconn, ui.FindParams{
		Name: "Main menu",
