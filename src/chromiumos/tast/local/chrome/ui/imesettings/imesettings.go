...
// SearchInputMethod searchs input method by typing keyboard into searchbox.
// SearchInputMethod also waits for expected IME displayed on screen.
func SearchInputMethod(ctx context.Context, tconn *chrome.TestConn, searchKeyword, inputMethodName string) error {
	...
	if err := keyboard.Type(ctx, searchKeyword); err != nil {
...
func SelectInputMethod(ctx context.Context, tconn *chrome.TestConn, inputMethodName string) error {
	...
	inputMethodNode, err := ossettings.DescendantNode(ctx, tconn, inputMethodOptionParams)
	if err != nil {
...
func ClickAddButtonToConfirm(ctx context.Context, tconn *chrome.TestConn) error {
	...
	addButton, err := ossettings.DescendantNode(ctx, tconn, addButtonParams)
	if err != nil {
