...
func SetNearbySettings(ctx context.Context, s *testing.State, nsSettingsConn *chrome.Conn,
	...
	if err := nsSettings.Call(ctx, nil, `function(vis){this.setEnabled(true);}`); err != nil {
		s.Fatal("Failed to enable the Nearby Share feature: ", err)
	}
	var isNearbyEnabled bool
	if err := nsSettings.Call(ctx, &isNearbyEnabled, `async function() {
			var obj = await this.getEnabled();
			return obj.enabled;
		}`); err != nil {
		s.Fatal("Failed to get the Nearby Share feature state: ", err)
	}
	if !isNearbyEnabled {
		s.Fatal("Failed to enable the Nearby Share feature.")
	}
	s.Log("Successfully enable the Nearby Share feature")

	return nil
}

// WaitAndGetEvent waits the specific event within timeout.
