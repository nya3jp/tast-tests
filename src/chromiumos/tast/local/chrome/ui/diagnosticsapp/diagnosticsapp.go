...
// Package diagnosticsapp contains common functions used in the app.
...
// type cardParams string

// func (t cardParams) uiFindParams() ui.FindParams {
...
// const (
// 	BatteryCard = cardParams("batteryStatusCard")
// )
...
func WaitForApp(ctx context.Context, tconn *chrome.TestConn) error {
	diagnosticsRootNode, err := DiagnotsicsRootNode(ctx, tconn)
	if err != nil {
		return errors.Wrap(err, "failed to find diagnostics app")
	}
	// Find session log button to verify app rendering.
	if _, err := diagnosticsRootNode.DescendantWithTimeout(ctx, diagnosticsLogButton, 20*time.Second); err != nil {
