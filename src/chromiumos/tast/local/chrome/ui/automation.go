...
// It continously check the element location, until 3 consecutive check results the same.
func (n *Node) WaitForPositioned(ctx context.Context) error {
	...
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		} else if !locations[len(locations)-1].Equals(locations[len(locations)-2]) || !locations[len(locations)-2].Equals(locations[len(locations)-3]) {
