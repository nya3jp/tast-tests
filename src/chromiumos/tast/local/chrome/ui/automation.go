...
func (n *Node) FindSelectedRadioButton(ctx context.Context) (*Node, error) {
	...
	rbexists, err := n.DescendantExists(ctx, FindParams{Role: RoleTypeRadioButton})
	if err != nil {
	...
	srbexists, err := n.DescendantExists(ctx, FindParams{
		Role:       RoleTypeRadioButton,
		Attributes: map[string]interface{}{"checked": CheckedStateTrue},
	})
	if err != nil {
		return nil, errors.Wrap(err, "couldn't find selected radio button")
	}
	if srbexists == false {
		return nil, ErrNoActiveRadioButton
	}

	sb, err := n.Descendant(ctx, FindParams{
		Role:       RoleTypeRadioButton,
		Attributes: map[string]interface{}{"checked": CheckedStateTrue},
	})
	if err != nil {
		return nil, errors.Wrap(err, "couldn't find selected radio buttons")
	}
