...
// Node intentionally leaves out many properties. If they become needed, add them to the Node struct and to the Init function.
// As defined in chromium/src/extensions/common/api/automation.idl
type Node struct {
...
func (n *Node) Descendant(ctx context.Context, params FindParams) (*Node, error) {
	...
	if err := n.object.Call(ctx, &node.object, fmt.Sprintf("function(){return this.find(%s)}", paramsBytes)); err != nil {
		return nil, err
	}
...
func (n *Node) Descendants(ctx context.Context, params FindParams) ([]*Node, error) {
	...
	for i := 0; i < len; i++ {
		...
		if err := nodeList.Call(ctx, &node.object, "function(i){return this[i]}", i); err != nil {
			return []*Node{}, err
		}
...
func Root(ctx context.Context, conn *chrome.Conn) (*Node, error) {
	...
	err := conn.EvalPromise(ctx, "tast.promisify(chrome.automation.getDesktop)()", &root.object)
	if err != nil {
		return nil, err
	}
