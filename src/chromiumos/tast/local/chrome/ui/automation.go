...
// target node visible.
func (n *Node) MakeVisible(ctx context.Context, timeout time.Duration) error {
	testing.ContextLogf(ctx, "original position = %s", n.Location.String())
	...
	if err := n.WaitLocationStable(ctx, &testing.PollOptions{Timeout: timeout}); err != nil {
		return errors.Wrap(err, "failed to wait for node to be positioned")
	}
	testing.ContextLogf(ctx, "after scroll position = %s", n.Location.String())
	return nil
}
