...
func PasswordFieldFinder(ctx context.Context, tconn *chrome.TestConn, username string) (*nodewith.Finder, error) {
	var finder *nodewith.Finder
	r, err := regexp.Compile(fmt.Sprintf("Password for %v", username))
	if err != nil {
		return finder, errors.Wrap(err, "failed to compile regexp for name attribute")
	}
	finder = nodewith.Role(role.TextField).Attribute("name", r)
	return finder, nil
}
...
func EnterPIN(ctx context.Context, tconn *chrome.TestConn, PIN string) error {
	...
	for i, d := range PIN {
		...
		if err := uiauto.Combine("click the button",
			ui.WithTimeout(uiTimeout).WaitUntilExists(button),
			ui.LeftClick(button),
		)(ctx); err != nil {
...
func SubmitPIN(ctx context.Context, tconn *chrome.TestConn) error {
	...
	return uiauto.Combine("click the submit button",
		ui.WithTimeout(uiTimeout).WaitUntilExists(submitButton),
		ui.LeftClick(submitButton))(ctx)
}
