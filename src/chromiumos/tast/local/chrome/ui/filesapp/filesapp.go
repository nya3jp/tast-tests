...
func (f *FilesApp) Close(ctx context.Context) {
	...
	root.WaitForDescendantToDisappear(ctx, rootFindParams, time.Minute)
}
...
func (f *FilesApp) OpenDownloads(ctx context.Context) error {
	...
	return f.Root.WaitForDescendantToAppear(ctx, params, 15*time.Second)
}
...
func (f *FilesApp) WaitForFile(ctx context.Context, filename string, timeout time.Duration) error {
	...
	return filesBox.WaitForDescendantToAppear(ctx, params, 15*time.Second)
}
