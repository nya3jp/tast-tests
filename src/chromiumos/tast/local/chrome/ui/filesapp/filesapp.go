...
// SelectDirectoryContextMenuItem right clicks one of the directories shown in the navigation tree and select one of the context menu items.
// An error is returned if dir is not found or right click fails.
func (f *FilesApp) SelectDirectoryContextMenuItem(ctx context.Context, dirName, memnuItem string) error {
	// Select dirName in the directory tree.
	params := ui.FindParams{
		Name: dirName,
		Role: ui.RoleTypeTreeItem,
