...
// restoreVisibility returns a function that will restore the Ubertray to its original state (shown or hidden).
func restoreVisibility(ctx context.Context, tconn *chrome.TestConn) (func(ctx context.Context) error, error) {
...
func QuickSettingEnabled(ctx context.Context, tconn *chrome.TestConn, setting QuickSetting) (bool, error) {
	cleanup, err := restoreVisibility(ctx, tconn)
	if err != nil {
		return false, err
	}
	defer cleanup(ctx)
