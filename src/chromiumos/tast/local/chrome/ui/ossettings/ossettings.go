...
const (
	inputsSubPageURL    = "osLanguages/input"
	languagesSubPageURL = "osLanguages/languages"
)
...
func LaunchAtInputsSettingsPage(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome) error {
	condition := func(ctx context.Context) (bool, error) {
		return DescendantNodeExists(ctx, tconn, ui.FindParams{Role: ui.RoleTypeHeading, Name: "Inputs"})
	}
	return LaunchAtPageURL(ctx, tconn, cr, inputsSubPageURL, condition)
}
