...
func Rect(ctx context.Context, tconn *chrome.TestConn) (coords.Rect, error) {
	...
	for i := range results {
		if exists, err := ui.IsNodeFound(ctx, quickSettingsFinder.Ancestor(bubbleFrameView.Nth(i))); err == nil && exists {
...
func Shown(ctx context.Context, tconn *chrome.TestConn) (bool, error) {
	ui := uiauto.New(tconn)
	return ui.IsNodeFound(ctx, quickSettingsFinder)
}
...
func ShowWithRetry(ctx context.Context, tconn *chrome.TestConn, timeout time.Duration) error {
	...
	f := func(ctx context.Context) error {
		return ui.Exists(quickSettingsFinder)(ctx)
	}
	if err := ui.WithPollOpts(testing.PollOptions{Timeout: timeout, Interval: time.Second}).LeftClickUntil(statusArea, f)(ctx); err != nil {
		return errors.Wrap(err, "quick settings not shown")
	}
	return nil
}

// PodIconButton generates ui.FindParams for the specified quick setting pod.
func PodIconButton(setting SettingPod) (*nodewith.Finder, error) {
	...
	if setting == SettingPodNetwork {
		return nodewith.ClassName("NetworkFeaturePodButton"), nil
	}
	...
	if err != nil {
		return &nodewith.Finder{}, errors.Wrapf(err, "failed to compile regexp for %v pod icon name attribute", setting)
	}
	podButton := nodewith.ClassName("FeaturePodIconButton").Attribute("name", r)

	return podButton, nil
}

// findPodButton finds the UI node corresponding to the specified quick setting pod icon button.
func findPodButton(ctx context.Context, tconn *chrome.TestConn, setting SettingPod) (*nodewith.Finder, error) {
	podButton, err := PodIconButton(setting)
	if err != nil {
		return nil, err
	}

	ui := uiauto.New(tconn)
	if err := ui.WithTimeout(uiTimeout).WaitUntilExists(podButton)(ctx); err != nil {
...
func OpenSettingsApp(ctx context.Context, tconn *chrome.TestConn) error {
	...
	ui := uiauto.New(tconn)
	if err := ui.WithTimeout(uiTimeout).WaitUntilExists(SettingsBtn)(ctx); err != nil {
	...
	condition := func(ctx context.Context) error {
		err := ui.Gone(SettingsBtn)(ctx)
		return err
	}
	opts := testing.PollOptions{Timeout: 10 * time.Second, Interval: 500 * time.Millisecond}
	if err := ui.WithPollOpts(opts).LeftClickUntil(SettingsBtn, condition)(ctx); err != nil {
...
func LockScreen(ctx context.Context, tconn *chrome.TestConn) error {
	...
	ui := uiauto.New(tconn)
	if err := ui.WithTimeout(uiTimeout).WaitUntilExists(LockBtn)(ctx); err != nil {
		return errors.Wrap(err, "failed to find lock button")
	}

	if err := ui.LeftClick(LockBtn)(ctx); err != nil {
...
func OpenAudioSettings(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if exist {
		...
		if err := ui.WithPollOpts(testing.PollOptions{Timeout: uiTimeout}).LeftClickUntil(expandMenuBtn, func(ctx context.Context) error {
			if err := ui.WithTimeout(1 * time.Second).WaitUntilExists(audioSettingsBtn)(ctx); err != nil {
				return err
			}
			return nil
		})(ctx); err != nil {
	...
	if err := ui.WithTimeout(uiTimeout).WaitUntilExists(audioSettingsBtn)(ctx); err != nil {
	...
	if err := ui.WithPollOpts(testing.PollOptions{Timeout: uiTimeout}).LeftClickUntil(audioSettingsBtn, func(ctx context.Context) error {
		if err := ui.WithTimeout(1 * time.Second).WaitUntilExists(audioDetailedView)(ctx); err != nil {
			return err
		}
		return nil
	})(ctx); err != nil {
...
func focusSlider(ctx context.Context, tconn *chrome.TestConn, kb *input.KeyboardEventWriter, slider SliderType) error {
	...
	if info.State[state.Focused] == true {
		return nil
	}
...
func micToggleButton(ctx context.Context, tconn *chrome.TestConn) (*nodewith.Finder, error) {
	if err := OpenAudioSettings(ctx, tconn); err != nil {
		return nil, err
	}
	ui := uiauto.New(tconn)
	if err := ui.WithTimeout(uiTimeout).WaitUntilExists(MicToggle)(ctx); err != nil {
		return nil, errors.Wrap(err, "failed to find the mic toggle button")
	}
	return MicToggle, nil
}
...
func CommonElements(ctx context.Context, tconn *chrome.TestConn, hasBattery, isLockedScreen bool) (map[string]*nodewith.Finder, error) {
	...
	getNodes := map[string]*nodewith.Finder{
		"Shutdown button":   ShutdownBtn,
		"Collapse button":   CollapseBtn,
	...
	if isLockedScreen {
		...
		accessibility, err := PodIconButton(SettingPodAccessibility)
		if err != nil {
		...
		for _, settingPod := range featuredPods {
			podParams, err := PodIconButton(settingPod)
			if err != nil {
