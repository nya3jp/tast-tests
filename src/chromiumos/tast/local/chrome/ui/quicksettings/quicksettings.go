...
func PodIconButton(setting SettingPod) (*nodewith.Finder, error) {
	...
	return nodewith.ClassName("FeaturePodIconButton").Attribute("name", r), nil
}
...
func OpenSettingsApp(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if err := ui.WithPollOpts(opts).LeftClickUntil(SettingsBtn, ui.Gone(SettingsBtn))(ctx); err != nil {
...
func LockScreen(ctx context.Context, tconn *chrome.TestConn) error {
	...
	ui := uiauto.New(tconn)
	if err := ui.WithTimeout(uiTimeout).LeftClick(LockBtn)(ctx); err != nil {
