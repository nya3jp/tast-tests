...
func changeSlider(ctx context.Context, tconn *chrome.TestConn, kb *input.KeyboardEventWriter, slider SliderType, increase bool) error {
	...
	// The polling below will time out in these cases. Instead we will return the value after a short delay,
	// to account for the possibility of the slider having some unexpected movement.
	if (initial == 0 && !increase) || (initial == 100 && increase) {
		testing.Sleep(ctx, time.Second)
		if v, err := SliderValue(ctx, tconn, slider); err != nil {
			return err
		} else if v != initial {
			return errors.Errorf("slider value changed unexpectedly; expected %v, got %v", initial, v)
		}
		return nil
	}
