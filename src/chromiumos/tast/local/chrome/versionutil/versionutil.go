...
// ListReleaseResponse is imported from google.installer.versionhistory.v1.ListReleasesResponse
...
func HTTPGetVersionHistory(ctx context.Context) ([]Release, error) {
	...
	releaseResp := ListReleaseResponse{}
	...
	return releaseResp.Releases, nil
}

// AshVersion returns a string of Ash version. eg. "100.0.1000.0"
...
// ChromeOSVersion returns a string of OS version. eg. "12345.0.0"
...
// Compatible compares Ash and Lacros versions and returns whether it is compatible with each other based on the version skew policy.
func Compatible(lacros, ash string) (bool, error) {
	...
	if l[0] == a[0] {
		...
		// Examples:
		//	ash 100.0.1000.0, lacros 100.0.2000.0 => compatible when lacros' build number (2000) >= ash (1000)
		//	ash 100.0.1000.9, lacros 100.0.1000.1 => compatible when lacros' build number == ash regardless of the branch number (9, 1)
		//	ash 100.0.1000.0, lacros 100.0.0999.0 => incompatible when lacros' build number (999) < ash (1000)
		return l[1] > a[1] || (l[1] == a[1] && l[2] >= a[2]), nil
	} else if l[0] > a[0] && l[0] <= a[0]+maxMajorVersionSkew {
		// In the milestone skews, compare the milestone number (M) in M.x.xxxx.x
		// Examples:
		//	ash 100.x.xxxx.x, lacros 102.x.xxxx.x => compatible when lacros' milestone (102) > ash (100) within [0,2] of milestone skews
		//	ash 100.x.xxxx.x, lacros 103.x.xxxx.x => incompatible when lacros' milestone (103) > ash (100) not within [0,2] of milestone skews
		//	ash 100.x.xxxx.x, lacros  99.x.xxxx.x => incompatible when lacros' milestone (99) < ash (100)
		return true, nil
	}
	return false, nil
}
