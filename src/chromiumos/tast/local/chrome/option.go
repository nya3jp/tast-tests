...
// RTReportingServer returns an option that can be passed to New to tell the device
// to report Chrome real-time events to the Reporting Server at the given url. By
// default real-time event reporting is not enabled, and when enabled, Chrome reports
// event to the Prod Reporting Server at chromereporting-pa.googleapis.com
func RTReportingServer(url string) Option {
	return func(cfg *config.MutableConfig) error {
		cfg.PolicyEnabled = true
		cfg.RtReportingAddr = url
		return nil
	}
}
