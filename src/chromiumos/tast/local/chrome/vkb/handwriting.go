...
func findHandwritingCanvas(ctx context.Context, tconn *chrome.TestConn) (*ui.Node, error) {
	...
	// Interval of 500ms was used as using a faster interval removed the first stroke while drawing the handwriting.
	opts := testing.PollOptions{Timeout: 2 * time.Second, Interval: 500 * time.Millisecond}
...
func newStrokeGroup(file io.Reader) (*strokeGroup, error) {
...
func scaleHandwritingData(strokeContainer *strokeGroup, canvasLoc coords.Rect) error {
...
func drawHandwriting(ctx context.Context, tconn *chrome.TestConn, strokeContainer *strokeGroup) error {
	...
	for _, currentStroke := range strokeContainer.strokes {
		for i, currentPoint := range currentStroke.points {
			...
			if err := mouse.Move(ctx, tconn, currentPoint.toCoordsPoint(), 30*time.Millisecond); err != nil {
...
func DrawHandwritingFromFile(ctx context.Context, tconn *chrome.TestConn, filePath string) error {
	...
	strokeContainer, err := newStrokeGroup(file)
	if err != nil {
	...
	if err := scaleHandwritingData(strokeContainer, canvasLoc); err != nil {
