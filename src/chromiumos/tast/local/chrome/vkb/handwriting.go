...
// extractStroke transforms the SVG path data from string to a stroke.
func extractStroke(path *path) *stroke {
	s := &stroke{}
...
func newStrokeGroup(svgFile *svg, n int) (*strokeGroup, error) {
	...
	for _, path := range paths {
		...
		extractedStroke := extractStroke(&path)
...
func DrawHandwritingFromFile(ctx context.Context, tconn *chrome.TestConn, filePath string) error {
	...
	sg, err := newStrokeGroup(svgFile, numPointsPerStroke)
	if err != nil {
		return errors.Wrap(err, "failed to read data from file")
	}
