...
func NewContext(c *chrome.Chrome, tconn *chrome.TestConn) *VirtualKeyboardContext {
...
func (vkbCtx *VirtualKeyboardContext) UIConn(ctx context.Context) (*chrome.Conn, error) {
	...
	return vkbCtx.c.NewConnForTarget(ctx, f)
}
...
func (vkbCtx *VirtualKeyboardContext) IsShown(ctx context.Context) bool {
	return uiauto.New(vkbCtx.tconn).Exists(vkRootFinder)(ctx) == nil
}

// IsKeyShown immediately checks whether the given key is shown.
func (vkbCtx *VirtualKeyboardContext) IsKeyShown(ctx context.Context, keyName string) bool {
	return uiauto.New(vkbCtx.tconn).Exists(KeyFinder(keyName))(ctx) == nil
}
...
func (vkbCtx *VirtualKeyboardContext) TapKeyAction(keyName string) uiauto.Action {
	return func(ctx context.Context) error {
		keyLocation, err := vkbCtx.uiAutoCtx.Location(ctx, KeyFinder(keyName))
		if err != nil {
...
// EnableA11yVirtualKeyboardAction returns an action enabling or disabling
// accessibility mode of the virtual keyboard.
// When disabled, the tablet non-a11y virtual keyboard will be used.
func (vkbCtx *VirtualKeyboardContext) EnableA11yVirtualKeyboardAction(enabled bool) uiauto.Action {
	return func(ctx context.Context) error {
