...
func EnableLanguage(ctx context.Context, tconn *chrome.TestConn, lang string) error {
	return tconn.Eval(ctx, fmt.Sprint(`chrome.languageSettingsPrivate.enableLanguage(%q)`, lang), nil)
}
...
func AddInputMethod(ctx context.Context, tconn *chrome.TestConn, imeID string) error {
	return tconn.Eval(ctx, fmt.Sprintf(`chrome.languageSettingsPrivate.addInputMethod(%q)`, ImePrefix+imeID), nil)
}
