...
func (vkbCtx *VirtualKeyboardContext) IsShown(ctx context.Context) bool {
	return vkbCtx.ui.Exists(vkRootFinder)(ctx) == nil
}
...
func (vkbCtx *VirtualKeyboardContext) IsKeyShown(ctx context.Context, keyName string) bool {
	return vkbCtx.ui.Exists(KeyFinder.Name(keyName))(ctx) == nil
}
...
// ClickUntilVKShownAction returns an action retrying left clicks the node until the vk is shown with no error.
