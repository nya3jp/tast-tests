...
// WaitForLocationed waits for the virtual keyboard to appear and locationed.
func WaitForLocationed(ctx context.Context, tconn *chrome.TestConn) error {
...
func ClickUntilVKShown(ctx context.Context, tconn *chrome.TestConn, node *ui.Node) error {
	...
	return WaitForLocationed(ctx, tconn)
}
...
func WaitForVKReady(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome) error {
	if err := WaitForLocationed(ctx, tconn); err != nil {
		return err
	}
