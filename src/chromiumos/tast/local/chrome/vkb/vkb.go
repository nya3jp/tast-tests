...
func WaitForVKReady(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome) error {
	if err := uiauto.Run(ctx, WaitLocationStableAction(tconn), WaitForDecoderEnabledAction(cr, true)); err != nil {
		return errors.Wrap(err, "failed to wait for virtual keyboard to be ready")
	}
	return nil
}
...
// ShowVirtualKeyboardAction is the same as ShowVirtualKeyboard, but generates
