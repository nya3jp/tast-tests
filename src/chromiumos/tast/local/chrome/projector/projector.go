...
// microphone and sets up a fake one if necessary.
func SetUpProjectorApp(ctx context.Context, tconn *chrome.TestConn) (func(ctx context.Context), error) {
	...
	// Setup CRAS Aloop for audio test. Set up a fake microphone
	// so the test may proceed.
	cleanup, err := voice.EnableAloop(ctx, tconn)
	if err != nil {
		return cleanup, errors.Wrap(err, "failed to enable aloop")
	}
