...
// DownloadRecentPhoto downloads the first photo shown in Phone Hub's Recent Photos section to Tote.
func DownloadRecentPhoto(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if err := uiauto.Combine("download the first photo displayed in the Recent Photos section",
		ui.WaitUntilExists(mostRecentPhotoThumbnail),
