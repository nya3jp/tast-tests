...
// enableTimeout is the default timeout for Phone Hub UI automation related to enabling Phone Hub.
// It is longer than the default uiauto timeout to account for any connection latency between devices.
const enableTimeout time.Duration = 30 * time.Second
...
const (
	...
	featureCheckedJS               = `.checked_`
	toggleFeatureJS                = `.toggleFeature()`
...
func Enable(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome) error {
	...
	_, err = ossettings.LaunchAtPageURL(ctx, tconn, cr, "multidevice/features", func(context.Context) error { return nil })
	if err != nil {
	...
	if !enabled {
		if err := settingsConn.Eval(ctx, phoneHubToggleJS+toggleFeatureJS, nil); err != nil {
	...
	ui := uiauto.New(tconn).WithTimeout(enableTimeout)
...
func Show(ctx context.Context, tconn *chrome.TestConn) error {
	// Use the longer timeout here in case Phone Hub is being opened for the first time and is still connecting.
	ui := uiauto.New(tconn).WithTimeout(enableTimeout)
