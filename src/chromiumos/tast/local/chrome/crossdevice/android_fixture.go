...
func OverrideFeatureFlags(ctx context.Context, adbDevice *adb.Device, feature Feature) error {
	...
	case PhoneHub:
		...
		return nil
	case SmartLock:
		// These flags need to be overridden to ensure Nearby doesn't tear down the GATT Connection during SmartLock.
		if err := adbDevice.OverridePhenotypeFlag(ctx, "com.google.android.gms.nearby", "ConnectionsFeature__connections_allow_control_ble_gatt_connection_in_advertising_option", "true", "boolean"); err != nil {
			return errors.Wrap(err, "failed to override required flag for SmartLock")
		}
		if err := adbDevice.OverridePhenotypeFlag(ctx, "com.google.android.gms.nearby", "SharingFeature__sharing_enable_self_share_background_advertising", "true", "boolean"); err != nil {
			return errors.Wrap(err, "failed to override required flag for SmartLock")
		}
		return nil
	default:
