...
func (c *AndroidDevice) EnableChromeSync(ctx context.Context) error {
	...
	// Accepting the ToS automatically starts the chrome sync opt-in flow, starting with account selection.
	...
	if err := acceptBtn.WaitForExists(ctx, 3*time.Second); err != nil {
		...
		// return errors.Wrap(err, "failed to find account selection button for Chrome Sync")
	} else if err := acceptBtn.Click(ctx); err != nil {
		testing.ContextLog(ctx, "Failed to select account for Chrome Sync: ", err)
