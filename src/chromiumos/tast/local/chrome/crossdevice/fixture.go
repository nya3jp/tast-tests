...
func (f *crossdeviceFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	f.opts = append(f.opts, chrome.LoadSigninProfileExtension(s.RequiredVar("ui.signinProfileTestExtensionManifestKey")))
	...
	if _, err := ash.WaitForNotification(ctx, tconn, 30*time.Second, ash.WaitTitleContains("Connected to")); err != nil {
		s.Fatal("Did not receive notification that Chromebok and Phone are paired")
	}
