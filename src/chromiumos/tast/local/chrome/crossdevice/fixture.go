...
func (f *crossdeviceFixture) PreTest(ctx context.Context, s *testing.FixtTestState) {
	...
	if err := uiauto.StartRecordFromKB(s.TestContext(), f.tconn, f.kb); err != nil {
...
func (f *crossdeviceFixture) PostTest(ctx context.Context, s *testing.FixtTestState) {
	...
	if err := uiauto.StopRecordFromKBAndSaveOnError(ctx, f.tconn, s.HasError, s.OutDir()); err != nil {
		s.Fatal("Failed to save CrOS screen recording: ", err)
	}
}
