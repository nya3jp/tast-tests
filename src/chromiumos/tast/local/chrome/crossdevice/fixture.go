...
func init() {
	const (
		// These are the default GAIA credentials that will be used to sign in on CrOS and Android for Cross Device tests.
...
func (f *crossdeviceFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	crosUsername := s.RequiredVar("crossdevice.username")
	crosPassword := s.RequiredVar("crossdevice.password")
	...
	const pairTimeout time.Duration = 30 * time.Second
	if err := androidDevice.Pair(ctx, pairTimeout); err != nil {
