...
func init() {
	...
	testing.AddFixture(&testing.Fixture{
		Name:     "chromeLoggedInWithOobeDeferredLoginAndHpsScreenEnabled",
		Desc:     "Create chrome instance with the HPS screen enabled that will start on the Welcome screen and login when needed using a GAIA pool account",
		Contacts: []string{"bohdanty@google.com, cros-oobe@google.com"},
		Vars:     []string{"ui.gaiaPoolDefault", "ui.signinProfileTestExtensionManifestKey"},
		Impl: NewLoggedInFixture(func(ctx context.Context, s *testing.FixtState) ([]Option, error) {
			return []Option{
