...
// field is used for devices that technically support e.g. 4K HW decoding, but
// don't have the autocaps labels set because these devices are so slow that
// running 4K tests would be a huge drain on lab resources. See
...
// that a capability is detected if expected and viceversa, is not detected if
// not expected, unless it's marked as optional, case in which it doesn't care.
// Capabilities statically marked as "disable" are not verified either.
//
