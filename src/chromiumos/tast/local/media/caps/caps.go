...
func VerifyCapabilities(ctx context.Context, e ErrorReporter, avtestLabelToCapability map[string]Capability) error {
	...
	for _, c := range avtestLabelToCapability {
		...
		if !ok {
			...
			e.Error("static capabilities don't include %q", c.Name)
			continue
		}
		...
		case autocaps.Yes:
			if !wasDetected {
				e.Error("%q statically set but not detected", c.Name)
			}
		case autocaps.No:
			if wasDetected && !c.Optional {
				e.Error("%q detected but not statically set and not optional", c.Name)
			}
		}
	}
	return nil
