...
func ChromeAlternateVideoDecoder() testing.Precondition {
	alternateVideoDecoderOnce.Do(func() {
		if isLegacyVideoDecoderEnabled() {
...
// isLegacyVideoDecoderEnabled returns true if the Legacy, VDA-based Video Decoder is enabled.
// TODO(b/141652665): Query the USE flags to figure out if Legacy video decoder is in use.
func isLegacyVideoDecoderEnabled() bool {
	return true
}
