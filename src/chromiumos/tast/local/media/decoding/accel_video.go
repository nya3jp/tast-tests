...
// 1. Which Chrome stack implementation to run agains (e.g. VDA, VD etc)
// 2. Whether the output of the decoder should a linear buffer (false by
...
func RunAccelVideoPerfTest(ctx context.Context, outDir, filename string, parameters TestParams) error {
	...
	var tests = []struct {
		...
		{Capped, "MeasureCappedPerformance", "", parseCappedPerfMetrics},
		{Uncapped, "MeasureUncappedPerformance", "single_decdder", parseUncappedPerfMetrics},
		{UncappedConcurrent, "MeasureUncappedPerformance_TenConcurrentDecoders", "ten_concurrent_decoders", parseUncappedPerfMetrics},
	}
	...
	for _, test := range tests {
		...
		if err := test.parseFunc(JSON, p, test.metricName); err != nil {
