...
func RunAccelVideoTestToTestVectors(ctx context.Context, outDir string, filenames []string) error {
	vl, err := logging.NewVideoLogger()
	if err != nil {
		return errors.Wrap(err, "failed to set values for verbose logging")
	}
	...
	for _, filename := range filenames {
		args := generateCmdArgs(outDir, filename, VD)
		args = append(args, "--gtest_filter=VideoDecoderTest.FlushAtEndOfStream")
		const exec = "video_decode_accelerator_tests"
		if _, _ = gtest.New(
			filepath.Join(chrome.BinTestDir, exec),
			gtest.Logfile(filepath.Join(outDir, exec+".log")),
