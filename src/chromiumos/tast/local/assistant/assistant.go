...
// when it received a query. Currently we only defined fields which are using in tests.
...
// by sending a query, e.g. query text, mic status, and query response. Currently we only
// defined fields which are using in tests.
