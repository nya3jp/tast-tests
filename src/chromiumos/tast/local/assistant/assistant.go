...
// SetBetterOnboardingEnabled enables/disables the Assistant onboarding feature by controlling the number
// of sessions where onboarding screen has shown. Note that true pref value will *not* be restored later, so
// tests that need this feature must explicitly enable it during setup. It is recommended to disable
// Better Onboarding for Assistant performance tests that are not explicitly testing the Better
// Onboarding feature.
