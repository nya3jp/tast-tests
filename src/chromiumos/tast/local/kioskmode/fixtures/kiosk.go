...
func (k *kioskFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	if err != nil {
		// TODO: how to clean clean policies here?
		s.Fatal("Failed to create Chrome in kiosk mode: ", err)
	}

	proc, err := ashproc.Root()
	if err != nil {
		policyutil.ServeAndRefresh(ctx, k.fdms, k.cr, []policy.Policy{k.localAccounts})
		cr.Close(ctx)
...
func (k *kioskFixture) TearDown(ctx context.Context, s *testing.FixtState) {
	...
	// When applying empty policies slice Chrome crashes. Hence just local
	...
	//  them. In an alternative way kiosk account will stay loaded.
