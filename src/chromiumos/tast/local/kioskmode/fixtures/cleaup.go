...
func (k *kiosk) TearDown(ctx context.Context, s *testing.FixtState) {
	...
	if err != nil {
		s.Log("Failed to start Chrome: ", err)
	}
	defer cr.Close(ctx)

	if err := policyutil.ServeAndRefresh(ctx, k.fakeDMS, cr, []policy.Policy{}); err != nil {
		s.Log("Could not serve and refresh policies: ", err)
	}
}

func (k *kiosk) Reset(ctx context.Context) error {
	testing.ContextLog(ctx, "Kiosk clean up: Starting Chrome to clean policies")
	cr, err := chrome.New(
		ctx,
		chrome.FakeLogin(chrome.Creds{User: username, Pass: password}),
		chrome.DMSPolicy(k.fakeDMS.URL),
	...
	if err != nil {
		return errors.Wrap(err, "could not start Chrome to clean the policies. Next test could be affected by kioskmode.AutoLaunch() option")
	}
	defer cr.Close(ctx)

	if err := policyutil.ServeAndRefresh(ctx, k.fakeDMS, cr, []policy.Policy{}); err != nil {
		return errors.Wrap(err, "could not serve and refresh policies. Next test could be affected by kioskmode.AutoLaunch() option")
	}
	return nil
}
