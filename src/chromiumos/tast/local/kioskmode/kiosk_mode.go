...
func (k *Kiosk) Close(ctx context.Context) error {
	...
	if err := policyutil.ServeAndRefresh(ctx, k.fdms, k.cr, []policy.Policy{k.localAccounts}); err != nil {
		...
		return errors.Wrap(err, "could not serve and refresh policies")
	}
	if err := k.cr.Close(ctx); err != nil {
		return errors.Wrap(err, "could not close Chrome while closing Kiosk session")
	}
	return nil
}
...
func New(ctx context.Context, fdms *fakedms.FakeDMS, opts ...Option) (*Kiosk, *chrome.Chrome, error) {
	...
	if cfg.m.AutoLaunch {
		...
		if err != nil {
			// Cleanup. Login with Chrome and serve an empty policies slice.
			func(ctx context.Context, username, password string, fdms *fakedms.FakeDMS) {
				cr, err := chrome.New(
					ctx,
				...
				if err != nil {
					testing.ContextLog(ctx, "Kiosk mode: Failed to start Chrome for cleanup: ", err)
				}
				defer cr.Close(ctx)

				if err := policyutil.ServeAndRefresh(ctx, fdms, cr, []policy.Policy{}); err != nil {
					testing.ContextLog(ctx, "Kiosk mode: Failed to serve policies while performing cleanup: ", err)
				}
			}(ctx, fixtures.Username, fixtures.Password, fdms)
