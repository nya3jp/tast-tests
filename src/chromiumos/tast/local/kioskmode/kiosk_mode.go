...
func (k *Kiosk) CancelKioskLaunch(ctx context.Context, opts ...chrome.Option) (*chrome.Chrome, error) {
	const disableChromeStartFile = "/run/disable_chrome_restart"
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed to connect to session manager")
	}

	if err := sm.DisableUIRespawn(ctx); err != nil {
		return nil, errors.Wrap(err, "failed to disable UI respawn")
	}
	...
	// session.
	_, err = os.Create(disableChromeStartFile)
	if err != nil {
		return nil, errors.Wrap(err, "failed to create Chrome flag file")
	}
	defer func(ctx context.Context) {
		if err := os.RemoveAll(disableChromeStartFile); err != nil && !os.IsNotExist(err) {
	...
	// started.
	if err := os.RemoveAll(disableChromeStartFile); err != nil {
