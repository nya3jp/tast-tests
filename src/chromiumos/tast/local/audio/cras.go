...
func (c *Cras) SetActiveNodeByType(ctx context.Context, nodeType string) error {
	...
	waitForNodeExist := func(ctx context.Context) error {
		n, err := c.GetNodeByType(ctx, nodeType)
		node = n
		return err
	}
	if err := testing.Poll(ctx, waitForNodeExist, &testing.PollOptions{Timeout: time.Second}); err != nil {
	...
	waitForNodeActive := func(ctx context.Context) error {
		n, err := c.GetNodeByType(ctx, nodeType)
		if err != nil {
			return err
		}
	...
	if err := testing.Poll(ctx, waitForNodeActive, &testing.PollOptions{Timeout: time.Second}); err != nil {
