...
func WaitForDevice(ctx context.Context, streamType StreamType) error {
	...
	// the internal speaker is removed, too. For this case, we should send a
	// user activity ping to powerd to light up the display.
	powerd, err := power.NewPowerManager(ctx)
	if err != nil {
		return err
	}
	call := powerd.HandleUserActivity(ctx, 0)
	if call.Err != nil {
		return call.Err
	}
