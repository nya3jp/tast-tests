...
func CheckAudioStreamAtSelectedDevice(ctx context.Context, deviceName, deviceType string) error {
	if deviceType == "BLUETOOTH" {
		output, err := testexec.CommandContext(ctx, "sh", "-c", outputDeviceCmd).Output()
		if err != nil {
			errors.Wrapf(err, "failed to execute %q", outputDeviceCmd)
		}
	...
	if cardNum == "" || devNum == "" {
		return errors.New("failed to find Audio device name")
	}

	output, err := testexec.CommandContext(ctx, "sh", "-c", fmt.Sprintf(audioDeviceStatusCmd, cardNum, devNum)).Output()
	if err != nil {
		return errors.Wrapf(err, "failed to execute %q", outputDeviceCmd)
	}
	testing.ContextLogf(ctx, "Selected output device status is %s", output)
	if strings.Contains(string(output), "RUNNING") {
		testing.ContextLog(ctx, "Audio is routing through expected node")
	} else if strings.Contains(string(output), "closed") {
		return errors.New("Audio is not routing through expected audio node")
	} else {
		return errors.New("Audio routing error! Device may be preparing")
	}
	return nil
}
