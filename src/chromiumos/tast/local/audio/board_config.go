...
func LoadBoardConfig(ctx context.Context) (*BoardConfig, error) {
	...
	if err != nil {
		if crosconfig.IsNotFound(err) {
			// No cras config, return empty config.
			return &BoardConfig{}, nil
		}
		return nil, errors.Wrap(err, "cannot get cras config dir")
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			// no board config, return empty config
			return &BoardConfig{}, nil
		}
		return nil, err
	}

	return parseBoardConfig(configBytes)
}
