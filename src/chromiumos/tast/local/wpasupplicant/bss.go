...
const (
	dbusInterfaceBSS          = "fi.w1.wpa_supplicant1.BSS"
	dbusInterfaceBSSPropBSSID = "BSSID"
)
...
func NewBSS(ctx context.Context, path dbus.ObjectPath) (*BSS, error) {
	d, err := NewDBusObject(ctx, path, dbusInterfaceBSS)
	if err != nil {
		return nil, err
	}
...
func (b *BSS) BSSID(ctx context.Context) ([]byte, error) {
	var bssid []byte
	if err := b.dbus.Get(ctx, dbusInterfaceBSSPropBSSID, &bssid); err != nil {
		return nil, err
	}
	return bssid, nil
}
