...
func (m *Manager) ConnectToWifiNetwork(ctx context.Context, props map[string]interface{}) (bool, error) {
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		return false, errors.Wrap(err, "couldn't connect to ap")
	}
	return true, nil
}
...
// that adheres to the requested properties.
func (m *Manager) DisconnectFromWifiNetwork(ctx context.Context, props map[string]interface{}) (bool, error) {
	var servicePath dbus.ObjectPath
	var p map[string]interface{}
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		path, err := m.FindMatchingService(ctx, props)
