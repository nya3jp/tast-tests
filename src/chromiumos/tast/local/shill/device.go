...
func (d *Device) updateProperty(prop DeviceProperty, value interface{}) {
	d.props[prop] = value
}
...
// Wait waits for "PropertyChanged" signal and updates appropriate device property value.
func (d *DevicePropertyWatcher) Wait(ctx context.Context) (DeviceProperty, interface{}, error) {
	select {
	case sig := <-d.watcher.Signals:
		...
		if prop, ok := sig.Body[0].(string); !ok {
			...
			d.device.updateProperty(DeviceProperty(prop), foundVal)
			return DeviceProperty(prop), foundVal, nil
		}
	case <-ctx.Done():
...
func (d *DevicePropertyWatcher) WaitForExpectedChanges(ctx context.Context, expectedChanges map[DeviceProperty]interface{}) error {
	for {
		prop, foundVal, err := d.Wait(ctx)
		if err != nil {
			return errors.Wrapf(err, "Didn't receive expected PropertyChanged signals %v: ", expectedChanges)
		}

		if val, ok := expectedChanges[DeviceProperty(prop)]; !ok {
			continue
...
// CreatePropertyChangedWatcher returns a SignalWatcher to observe the
// "PropertyChanged" signal.
func (d *Device) CreatePropertyChangedWatcher(ctx context.Context) (*DevicePropertyWatcher, error) {
	...
	return &DevicePropertyWatcher{device: d, watcher: watcher}, err
}
