...
func NewDevice(ctx context.Context, path dbus.ObjectPath) (*Device, error) {
	...
	if _, err = m.GetProps(ctx); err != nil {
		return nil, err
	}
	return m, nil
}
...
// Note that it is called in NewDevice(). Users can call again to reflash properties.
...
// GetStringProp returns (props[key], nil) if |key| exists and its value type is string.
