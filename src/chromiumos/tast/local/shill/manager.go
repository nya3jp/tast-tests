...
// ServicesByTechnology returns list of Devices and their Properties snapshots of the specified technology.
func (m *Manager) ServicesByTechnology(ctx context.Context, technology Technology) ([]*Service, []*dbusutil.Properties, error) {
	...
	devs, err := m.Services(ctx)
	if err != nil {
		return nil, nil, err
	}

	for _, service := range devs {
