...
// It skips a device if it encounters DBusErrorUnknownObject when getting the device's properties.
// Note for callback that wish to stop traversing, e.g. the target is found, the callback can emit an
// expected error then deal with it on the caller side, cf. DeviceByName().
...
func (m *Manager) DeviceByName(ctx context.Context, iface string) (*Device, error) {
	...
	if err := m.TraverseDevices(ctx, matcher); err != nil && err != shortcut {
		return nil, err
	}
	if ret == nil {
