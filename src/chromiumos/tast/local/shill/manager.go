...
func (m *managerProps) GetDBusPathArray(key string) ([]dbus.ObjectPath, error) {
	value, ok := (*m)[key]
	if !ok {
	...
	if !ok {
		return nil, errors.Errorf("can not convert value to []dbus.ObjectPath: %s", value)
	}
	return arr, nil
}
...
func (m *Manager) GetProfiles(ctx context.Context) ([]dbus.ObjectPath, error) {
	...
	return props.GetDBusPathArray("Profiles")
}
...
func (m *Manager) GetDevices(ctx context.Context) ([]dbus.ObjectPath, error) {
	...
	return props.GetDBusPathArray("Devices")
}
