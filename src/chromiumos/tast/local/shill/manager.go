...
const (
	TechnologyCellular Technology = techtype.Cellular
	TechnologyEthernet Technology = techtype.Ethernet
	TechnologyPPPoE    Technology = techtype.PPPoE
	TechnologyVPN      Technology = techtype.VPN
	TechnologyWifi     Technology = techtype.Wifi
)
...
func (m *Manager) ProfilePaths(ctx context.Context) ([]dbus.ObjectPath, error) {
	...
	return p.GetObjectPaths(mgrprop.Profiles)
}
...
func (m *Manager) ActiveProfile(ctx context.Context) (*Profile, error) {
	...
	path, err := props.GetObjectPath(mgrprop.ActiveProfile)
	if err != nil {
		return nil, err
	}
...
func (m *Manager) Devices(ctx context.Context) ([]*Device, error) {
	...
	paths, err := p.GetObjectPaths(mgrprop.Devices)
	if err != nil {
		return nil, err
	}
...
func (m *Manager) DevicesByTechnology(ctx context.Context, technology Technology) ([]*Device, []*Properties, error) {
	...
	for _, dev := range devs {
		...
		if devType, err := p.GetString(devprop.Type); err != nil {
...
func (m *Manager) DeviceByName(ctx context.Context, iface string) (*Device, error) {
	...
	for _, dev := range devs {
		...
		if devIface, err := p.GetString(devprop.Interface); err != nil {
