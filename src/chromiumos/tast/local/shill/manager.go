...
// If it is enabled, it disables the technology and defers re-enabling the technology,
// returning the shortened context. Otherwise the original context is returned.
func (m *Manager) DisableTechnologyForTesting(ctx context.Context, technology Technology) (func(ctx context.Context), error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "Unable to determine enabled state for %v: ", technology)
	}
	if !enabled {
		return nil, nil
	}
	...
	return func(ctx context.Context) {
		...
		// Additional delay for primary connection to complete.
