...
func NewManager(ctx context.Context) (*Manager, error) {
	...
	dbusObj := &DBusObject{Interface: dbusManagerInterface, Object: obj, Conn: conn}
...
func (m *Manager) String() string {
	return string(m.dbusObject.Object.Path())
}
...
func (m *Manager) FindMatchingService(ctx context.Context, propsToMatch map[string]interface{}) (dbus.ObjectPath, error) {
	return m.findMatchingServiceInner(ctx, propsToMatch, false)
}

// FindMatchingAnyService returns any service including not visible with matching properties.
func (m *Manager) FindMatchingAnyService(ctx context.Context, propsToMatch map[string]interface{}) (dbus.ObjectPath, error) {
	return m.findMatchingServiceInner(ctx, propsToMatch, true)
}
...
func (m *Manager) findMatchingServiceInner(ctx context.Context, propsToMatch map[string]interface{}, complete bool) (dbus.ObjectPath, error) {
	...
	for _, path := range servicePaths {
		...
		testing.ContextLogf(ctx, "Got service property %s", path)

		match := true
		for key, val1 := range propsToMatch {
...
func (m *Manager) WaitForServiceProperties(ctx context.Context, props map[string]interface{}, timeout time.Duration) error {
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err != nil {
			testing.ContextLogf(ctx, "Error FindMatchingService %q", err)
		}
		return err
