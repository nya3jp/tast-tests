...
// PropertyHolder accesses the dbus interfaces which support GetProperties, SetProperty methods and PropertyChanged signal.
type PropertyHolder struct {
	dbusObject *DBusObject
}

// NewPropertyHolder connects to dbus service on the given path and interface, and creates PropertyHolder object with that
// connection.
func NewPropertyHolder(ctx context.Context, service, iface string, path dbus.ObjectPath) (PropertyHolder, error) {
	conn, obj, err := dbusutil.Connect(ctx, service, path)
	if err != nil {
...
// Properties calls GetProperties on the interface and returns the result.
func (h *PropertyHolder) Properties(ctx context.Context) (*Properties, error) {
