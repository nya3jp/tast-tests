...
func NewProperties(ctx context.Context, dbus *DBus) (*Properties, error) {
	var m map[string]interface{}
	if err := call(ctx, dbus.Object, dbus.Interface, "GetProperties").Store(&m); err != nil {
		return nil, errors.Wrap(err, "failed getting properties")
	}
	return &Properties{dbus: dbus, m: m}, nil
}
...
// Get returns property value and flag whether the property exist.
func (p *Properties) Get(prop string) (interface{}, bool) {
	value, ok := p.m[prop]
	return value, ok
}

// GetString returns string property value, flag whether the property exist and error if property value is not string.
func (p *Properties) GetString(prop string) (string, bool, error) {
	if value, ok := p.m[prop]; !ok {
		return "", false, nil
	} else if str, ok := value.(string); !ok {
		return "", true, errors.Errorf("property is not a string: %q", value)
	} else {
		return str, true, nil
	}
}
...
func (p *PropertiesWatcher) Wait(ctx context.Context) (string, interface{}, error) {
	...
	case sig := <-p.watcher.Signals:
		...
		if prop, ok := sig.Body[0].(string); !ok {
			...
			p.props.set(prop, foundVal)
			return prop, foundVal, nil
		}
	case <-ctx.Done():
