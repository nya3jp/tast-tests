...
const (
	// UserVar is the variable that holds the GAIA username.
	UserVar = "arc.PlayBillingUser"
	// PassVar is the variable that holds the GAIA password.
...
// SetUpUIAutomator sets up UIAutomator on the ARC device and waits for the
// intent helper system to be ready.
func SetUpUIAutomator(ctx context.Context, arcDevice *arc.ARC) (*ui.Device, error) {
	uiAutomator, err := arcDevice.NewUIDevice(ctx)
	if err != nil {
		if err := arcDevice.Close(ctx); err != nil {
			testing.ContextLog(ctx, "Failed to close ARC device: ", err)
		}
...
func NewTestPWA(ctx context.Context, cr *chrome.Chrome, pwaDir string) (pwa *TestPWA, retErr error) {
	cleanupCtx := ctx
