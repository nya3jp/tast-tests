...
func (f *playBillingFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	apkPath := filepath.Join(builtLocalDataPath, apk)
	if _, err := os.Stat(builtLocalDataPath); os.IsNotExist(err) {
		apkPath = filepath.Join(prebuiltLocalDataPath, apk)
	} else if err != nil {
		s.Fatal("Failed to check if built local data path exists: ", err)
	}

	// Install the test APK.
	if err := arcDevice.Install(ctx, apkPath); err != nil {
	...
	for _, name := range pwaFiles {
		// TODO(b/187793648): Replace with s.DataPath(name) when data is supported in Fixtures.
		dataFilePath := filepath.Join(builtLocalDataPath, name)
		if _, err := os.Stat(builtLocalDataPath); os.IsNotExist(err) {
			dataFilePath = filepath.Join(prebuiltLocalDataPath, name)
		} else if err != nil {
			s.Fatal("Failed to check if built local data path exists: ", err)
		}
