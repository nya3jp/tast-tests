...
func (ta *TestApp) CheckPresenceOfArcObject(ctx context.Context, uiAutomator *ui.Device, objectType, objectText string) error {
	object := uiAutomator.Object(ui.ClassName(objectType), ui.TextMatches(objectText), ui.Enabled(true))
	return object.WaitForExists(ctx, 10*time.Second)
}
...
func (ta *TestApp) BuySku(ctx context.Context, uiAutomator *ui.Device) error {
	return ta.ClickButtonOnArcPaymentOverlay(ctx, uiAutomator, "Button", "Buy")
}
