...
// waitForStableElementByJs Waits until location of the element defiend by jsExpr is stable.
...
// ClickElementByJs Clicks the element defiend by jsExpr.
// A separate function to emulate a click is needed, because there are restrictions,
// which don't allow to invoke billing actions via js interactions.
func ClickElementByJs(conn *chrome.Conn, jsExpr string) action.Action {
	return func(ctx context.Context) error {
...
// AlreadyOwnErrorClose closes you already own window.
