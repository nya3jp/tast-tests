...
func Verify(ctx context.Context, tconn *chrome.Conn, ps []Policy) error {
	var ms []*mismatch
	newM := func(a *DUTPolicy, e Policy, problem string) *mismatch {
		return &mismatch{Act: a, Exp: e, Err: errors.New(problem)}
	}
	...
	for _, expected := range ps {
		...
		if !ok {
			...
			ms = append(ms, newM(nil, expected, "policy was not set on DUT"))
			continue
		}
		if expected.Status() == StatusUnset {
			// Policy is set when it should be unset.
			ms = append(ms, newM(actual, expected,
				"policy should not have been set on DUT"))
			continue
		}

		// Flag any set policies with an error value, e.g. schema violations.
		if actual.Error != "" {
			ms = append(ms, newM(actual, expected, "policy error:"+actual.Error))
			continue
		}
		...
		case StatusSet, StatusSetRecommended:
			if actual.Source != dutSourceCloud {
				ms = append(ms, newM(actual, expected,
					fmt.Sprintf("saw a source of %s, not %s", actual.Source, dutSourceCloud)))
			}
		case StatusDefault:
		...
		case StatusSet, StatusDefault:
			if actual.Level != dutLevelMandatory {
				ms = append(ms, newM(actual, expected,
					fmt.Sprintf("saw a level of %s, not %s", actual.Level, dutLevelMandatory)))
			}
		case StatusSetRecommended:
			if actual.Level != dutLevelRecommended {
				ms = append(ms, newM(actual, expected,
					fmt.Sprintf("saw a level of %s, not %s", actual.Level, dutLevelRecommended)))
			}
		}
		...
		if (expected.Scope() == ScopeUser && actual.Scope != dutScopeUser) ||
			...
			ms = append(ms, newM(actual, expected,
				fmt.Sprintf("saw scope of %s, not %s", actual.Scope, expected.Scope())))
		}

		// Compare policy value.
		actualValue, err := expected.UnmarshalAs(actual.ValueJSON)
		if err != nil {
			ms = append(ms, newM(actual, expected, fmt.Sprintf("value unmarshal error: %v", err)))
			continue
		}
		if !expected.Equal(actualValue) {
			ms = append(ms, newM(actual, expected,
				fmt.Sprintf("actual value did not match expected")))
		}
	}
