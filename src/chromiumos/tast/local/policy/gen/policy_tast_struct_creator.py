...
HEADER = """// Copyright {} The Chromium OS Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.
...
# A double "{{" is to produce a single "{" in the formatted Go code.
INTERFACE_TEMPLATE = (
    """
///////////////////////////////////////////////////////////////////////////////
...
func (p *{self.name}) Field() string     {{ return "{self.field}" }}
...
"""
    'func (p *{self.name}) UnmarshalAs(m json.RawMessage) '
    '(interface{{}}, error) {{{self.unmarshal}}}'
    """
func (p *{self.name}) Equal(iface interface{{}}) bool {{{self.eq}}}
""")
...
# Default values for the contents of function code in INTERFACE_TEMPLATE.
...
def parse_object(name, schema, refs, references_only):
  ...
  for key in props:
    ...
    key_type, key_code = parse_schema(key_name, props[key], refs,
                                      references_only)
    members += new_property(key, key_type)
    code += key_code
  ...
  code = new_struct(name, members) + code
...
class Policy(object):
  """Class representing a single policy, used with INTERFACE_TEMPLATE."""
  def __init__(self, p, device_field_lookup):
    ...
    # field: If this is a device policy, what is the field name.
    self.scope, self.field = self.get_scope_and_field(p, device_field_lookup)
  ...
  def get_scope_and_field(self, p, lookup):
    ...
    if p.get('device_only', False):
      if self.name in lookup:
        return 'ScopeDevice', lookup[self.name]
      else:
        raise_key_error(self.name, 'device fieldnames')
    else:
      return 'ScopeUser', ''

  def get_additional_info(self, p):
  ...
  def generate_code(self, refs):
    ...
    self.code = INTERFACE_TEMPLATE.format(self=self)
...
def write_code(output_path, policies_by_id, schema_ids):
  ...
  with open(output_path, 'w') as fh:
    ...
    fh.write(HEADER)
    sorted_ids = policies_by_id.keys()
    sorted_ids.sort()
    for i in sorted_ids:
...
def main():
  ...
  parser.add_argument('--id', dest='id', default=None, type=int,
                      help='Optional id of a single policy to print out.')
  parser.add_argument('--out', dest='out', default=None, type=str,
                      help=('Optional directory for the file output. '
                            'If none specified, the local directory is used.'))
  ...
  with open(args.policy_path) as f:
    pt_contents = eval(f.read(), {}) # pylint: disable=eval-used

  device_field_lookup = pt_contents['device_policy_proto_map']
  # Save legacy policy values.  Note: this implementation won't work with any
  # "one-to-many" legacy policies.
  ...
  for pt in pt_contents['policy_definitions']:
    if 'type' in pt and pt['type'] == 'group':
      continue
    try:
  ...
  # Write all valid policies to file or print out a single policy.
  if args.id:
    print(policies[args.id].code)
  else:
    if args.out:
      output_path = os.path.join(args.out, OUTPUT_FILENAME)
    else:
      output_path = OUTPUT_FILENAME
    write_code(output_path, policies_by_id, schema_ids)
