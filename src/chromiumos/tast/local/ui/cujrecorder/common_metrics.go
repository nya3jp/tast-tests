...
var (
	// BrowsingFocusedMetrics contains the metrics that are most useful for measuring the web browsing performance.
	// These metrics can be collected from Lacros-Chrome browser only if Lacros is enabled.
	BrowsingFocusedMetrics = []MetricConfig{
		// Responsiveness.
	...
	// SystemOnlyMetrics contains metrics that are only used to measure the CrOS system performance.
	// These metrics should be collected from Ash-Chrome only if Lacros is enabled.
	SystemOnlyMetrics = []MetricConfig{
		// Media Quality.
	...
	// SystemFocusedMetrics contains the metrics that are most useful to measure the CrOS non web browsing related operations.
	// These metrics can be collected from Ash-Chrome only if Lacros is enabled.
	SystemFocusedMetrics = []MetricConfig{
		// Responsiveness.
	...
	// BrowsingAndSystemMetrics contains the metrics that are both important for the user browsing experience and other CrOS operations.
	// These metrics can be collect from both Ash-Chrome and Lacros-Chrome when Lacros is enabled.
	BrowsingAndSystemMetrics = []MetricConfig{
		// Responsiveness.
...
func MetricConfigs() []MetricConfig {
	...
	configs = append(configs, BrowsingFocusedMetrics...)
	configs = append(configs, SystemOnlyMetrics...)
	configs = append(configs, SystemFocusedMetrics...)
	configs = append(configs, BrowsingAndSystemMetrics...)
	return configs
}

// MetricCollectOption indicates how does the test want to collect metrics from
// the ash-Chrome and lacros-Chrome.
type MetricCollectOption int

const (
	// MetricsCollectFullSystem indicates the test wants to collect as many metrics for the full system as possible.
	// All metrics except the Ash only ones will be collected from both ash-Chrome and lacros-Chrome.
	MetricsCollectFullSystem MetricCollectOption = iota
	// MetricsCollectBrowsingSeperated indicates the test wants to collect web browsing releated
	// metrics seperately from lacros-Browser only, and collect CrOS system related metrics
	// from ash-Browser only. There are still some commom metrics which could be collected
	// from both.
	MetricsCollectBrowsingSeperated
)

// BrowserBasedMetricConfigs returns the metric configs that are seperated as
// ash-Chrome only configs, lacros-Chrome only configs, and common configs for both Ash and Lacros browsers.
func BrowserBasedMetricConfigs(option MetricCollectOption) (ashConfigs, lacrosConfigs, commonConfigs []MetricConfig) {
	if option == MetricsCollectFullSystem {
		ashConfigs = append(ashConfigs, SystemOnlyMetrics...)
		// Put all other metrics into common configs so they can be collected from all browsers.
		commonConfigs = append(commonConfigs, BrowsingFocusedMetrics...)
		commonConfigs = append(commonConfigs, SystemFocusedMetrics...)
		commonConfigs = append(commonConfigs, BrowsingAndSystemMetrics...)
	} else if option == MetricsCollectBrowsingSeperated {
		ashConfigs = append(ashConfigs, SystemOnlyMetrics...)
		ashConfigs = append(ashConfigs, SystemFocusedMetrics...)
		lacrosConfigs = append(lacrosConfigs, BrowsingFocusedMetrics...)
		commonConfigs = append(commonConfigs, BrowsingAndSystemMetrics...)
	}
	return ashConfigs, lacrosConfigs, commonConfigs
}
