...
// connection.
func (r *Recorder) AddCommonMetrics(tconn, bTconn *chrome.TestConn, bt browser.Type) error {
	...
	if err := r.AddCollectedMetrics(tconn, brower.TypeAsh, AnyChromeCommonMetricConfigs()...); err != nil {
		return errors.Wrap(err, "failed to add Ash AnyChrome common metrics")
	}
	if bt == browser.TypeLacros {
		if err := r.AddCollectedMetrics(bTconn, brower.TypeLacros, AnyChromeCommonMetricConfigs()...); err != nil {
