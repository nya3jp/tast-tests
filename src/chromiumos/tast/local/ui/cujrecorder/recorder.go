...
type Recorder struct {
	...
	browsers map[browser.Type]*chrome.TestConn
...
func (r *Recorder) AddCollectedMetrics(tconn *chrome.TestConn, bt browser.Type, configs ...MetricConfig) error {
	...
	r.browsers[bt] = tconn // Keep the latest tconn instance for the given browser type.
...
func NewRecorder(ctx context.Context, cr *chrome.Chrome, a *arc.ARC, options RecorderOptions) (*Recorder, error) {
	...
	r.browsers = make(map[browser.Type]*chrome.TestConn)
...
func (r *Recorder) startRecording(ctx context.Context) (runCtx context.Context, e error) {
	...
	for bt, names := range r.names {
		var err error
		r.mr[bt], err = metrics.StartRecorder(ctx, r.browsers[bt], names...)
		if err != nil {
...
func (r *Recorder) Record(ctx context.Context, pv *perf.Values) error {
	...
	var allRecords = make(map[string]*record) // Combined records from all browsers.
