...
type MetricConfig struct {
	...
	// of test scenarios: BootTimes.* and ShutodwnTime.*. This way the
...
// NewBootAndShutdownCustomMetricConfig creates a new CustomMetricConfig with
...
func (r *Recorder) stopRecording(ctx, runCtx context.Context) (e error) {
	...
	for bt, rr := range r.mr {
		...
		testing.ContextLogf(ctx, "The following out-of-test metrics are collected from %q: %v", bt+"-Chrome", histsWithSamples(bootAndShutdownHistograms))
