...
func NewRecorder(ctx context.Context, cr *chrome.Chrome, a *arc.ARC, options RecorderOptions, configs ...MetricConfig) (*Recorder, error) {
	...
	var dischargeThreshold = defaultDischargeThreshold
	if options.DischargeThreshold != 0.0 {
	...
	powerTestOptions := setup.PowerTestOptions{
		...
		Battery:    setup.TryBatteryDischarge(&batteryDischargeErr, dischargeThreshold),
	...
	if batteryDischargeErr != nil && options.FailOnDischargeErr && !power.IsNoBatteryError(batteryDischargeErr) {
