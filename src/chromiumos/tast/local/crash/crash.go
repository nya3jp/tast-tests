...
func setUpCrashTestWithDirectories(inProgDir, sysCrashDir, sysCrashStash, userCrashDir, userCrashStash string) error {
	...
	if err := moveAllCrashesTo(userCrashDir, userCrashStash); err != nil && !os.IsNotExist(err) {
		if cleanErr := cleanUpStashDir(sysCrashStash, sysCrashDir); cleanErr != nil {
			return errors.Wrapf(err, "error while cleaning up: %v", cleanErr)
		}
		return err
	}
	...
	if err := os.MkdirAll(inProgDir, 0755); err != nil {
		if cleanErr := cleanUpStashDir(sysCrashStash, sysCrashDir); cleanErr != nil {
			err = errors.Wrapf(err, "error while cleaning up: %v", cleanErr)
		}
		if cleanErr := cleanUpStashDir(userCrashStash, userCrashDir); cleanErr != nil {
			err = errors.Wrapf(err, "error while cleaning up: %v", cleanErr)
		}
