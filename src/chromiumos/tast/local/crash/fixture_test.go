...
func TestSetUpAndTearDownCrashTest(t *gotesting.T) {
	...
	}, &testing.PollOptions{Timeout: time.Minute, Interval: 5 * time.Millisecond}); err != nil {
...
func TestSetUpAndTearDownReboot(t *gotesting.T) {
	tmpDir := testutil.TempDir(t)
	defer os.RemoveAll(tmpDir)
...
func TestSetUpAndTearDownCrashTestWithOldStash(t *gotesting.T) {
	tmpDir := testutil.TempDir(t)
	defer os.RemoveAll(tmpDir)
...
func TestMoveAllCrashesTo(t *gotesting.T) {
	tmpDir := testutil.TempDir(t)
	defer os.RemoveAll(tmpDir)
...
func TestFilterIn(t *gotesting.T) {
	tmpDir := testutil.TempDir(t)
	defer os.RemoveAll(tmpDir)
