...
func TestSetUpAndTearDownCrashTest(t *testing.T) {
	...
	go func() {
		defer close(done)
		// We don't use ioutil.ReadAll because it blocks, causing the test to *always* take 30s.
		// This means we won't necessarily get the full stderr, if anything is logged after the
		// sleep starts.
