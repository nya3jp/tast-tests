...
func CreatePerUserConsent(ctx context.Context, enable bool) error {
	...
	for _, d := range dirs {
		f := filepath.Join(d, "consent-enabled")
		contents := "0"
		if enable {
			contents = "1"
		}
	...
	if err := ioutil.WriteFile("/run/metrics/use-per-user-consent", []byte{}, 0644); err != nil {
...
func RemovePerUserConsent(ctx context.Context) error {
	...
	if err := os.Remove("/run/metrics/use-per-user-consent"); err != nil && !os.IsNotExist(err) {
