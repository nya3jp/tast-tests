...
func TestDeleteAllCores(t *testing.T) {
	...
	initFiles := []string{"a.core", "a.dmp", "a.txt", "b.core", "b.dmp", "b.jpg"}
	...
	func() {
		...
		reporterRunning := func() (bool, error) {
			// Cancel the context so that deleteAllCores returns after observing exactly once that
			// crash_reporter is running.
			cancel()
			return true, nil
		}

		if err := deleteAllCores(ctx, []string{dir1, dir2}, reporterRunning); err == nil {
			t.Error("deleteAllCores succeeded unexpectedly while crash_reporter is running")
		}

		got := filesIn(dir2)
		if diff := cmp.Diff(got, initFiles); diff != "" {
			t.Error("Files mismatch after failed deleteAllCores (-got +want):\n", diff)
		}
	}()
	...
	if err := deleteAllCores(ctx, []string{dir1, dir2}, reporterRunning); err != nil {
		t.Error("deleteAllCores failed when crash_reporter is not running: ", err)
	}

	got := filesIn(dir2)
	want := []string{"a.dmp", "a.txt", "b.dmp", "b.jpg", "c.core"}
	if diff := cmp.Diff(got, want); diff != "" {
		t.Error("Files mismatch after successful deleteAllCores (-got +want):\n", diff)
	}
}
