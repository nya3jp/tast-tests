...
// checkNonExistant checks that all specified files do not exist.
func checkNonExistant(files ...string) error {
	for _, f := range files {
		if _, err := os.Stat(f); err == nil {
...
func TestSetUpAndTearDownCrashTest(t *testing.T) {
	...
	if err := checkNonExistant(file, sysStashDir, userStashDir); err != nil {
		t.Errorf("checkNonExistant(%s, %s, %s): %v", file, sysStashDir, userStashDir, err)
	}
}
...
func TestSetUpAndTearDownCrashTestWithOldStash(t *testing.T) {
	...
	if err := checkNonExistant(sysStashDir, userStashDir); err != nil {
		t.Errorf("checkNonExistant(%s, %s): %v", sysStashDir, userStashDir, err)
	}
}
