...
func (oe OptionsExpectation) matches(utteranceData UtteranceData) error {
	err := errors.New("Utterance data does not match expected")
	hasError := false
	if oe.expectation != utteranceData.Utterance {
		err = errors.Wrap(errors.Errorf("expected utterance: %s does not match utterance: %s", oe.expectation, utteranceData.Utterance), "utterance error message")
		hasError = true
	}

	if oe.expectedOptions.Lang != utteranceData.Options.Lang {
		err = errors.Wrap(errors.Errorf("expected lang: %s does not match lang: %s", oe.expectedOptions.Lang, utteranceData.Options.Lang), "lang error message")
		hasError = true
	}

	if oe.expectedOptions.Pitch != utteranceData.Options.Pitch {
		err = errors.Wrap(errors.Errorf("expected pitch: %s does not match pitch: %s", oe.expectedOptions.Pitch, utteranceData.Options.Pitch), "pitch error message")
		hasError = true
	}

	if oe.expectedOptions.Rate != utteranceData.Options.Rate {
		err = errors.Wrap(errors.Errorf("expected rate: %s does not match rate: %s", oe.expectedOptions.Rate, utteranceData.Options.Rate), "rate error message")
		hasError = true
	}
