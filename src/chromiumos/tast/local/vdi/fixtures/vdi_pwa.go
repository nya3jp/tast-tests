...
func init() {
	testing.AddFixture(&testing.Fixture{
		...
		Impl: &pwaFixtureState{
			vdiConnector:   &citrix.PWAConnector{},
...
func (v *pwaFixtureState) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	v.vdiConnector.Init(s, tconn, detector)
...
func (v *pwaFixtureState) Reset(ctx context.Context) error {
	...
	// In order to reset the state of the fixture is is enough to close the
	// window and load the VDI website again. No login is required.
	...
	if err := w.CloseWindow(ctx, tconn); err != nil {
		return errors.Wrap(err, "failed toc lose window")
	}
	...
	// Login is not required as the session persists.
