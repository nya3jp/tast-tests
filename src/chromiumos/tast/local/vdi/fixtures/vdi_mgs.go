...
type mgsFixtureState struct {
	...
	useTape bool
	// tapeAccountManager is sued for cleaning up Tape.
...
func (v *mgsFixtureState) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	if v.useTape {
		// Create an account manager and lease a vdi test account for 30 minutes.
		accHelper, acc, err := tape.NewGenericAccountManager(ctx, []byte(s.RequiredVar(tape.ServiceAccountVar)), tape.WithTimeout(1800), tape.WithPoolID(tape.Citrix))
		if err != nil {
