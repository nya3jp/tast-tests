...
func (v *mgsFixtureState) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	if err != nil {
		s.Fatal(err, "failed preparing Chrome to start with given mgs configuration")
	}
	...
	s.Log("Mgs: Wait for apps to be installed before launching")
	...
	testing.ContextLog(ctx, "Mgs: Starting VDI app")
