...
func (c *Connector) Login(ctx context.Context, k *input.KeyboardEventWriter, cfg *apps.VDILoginConfig) error {
	...
	if err := c.detector.WaitUntilExists(uidetection.CustomIcon(c.state.DataPath(VmwareData[SplashscreenAddBtn])))(ctx); err != nil {
	...
	testing.ContextLog(ctx, "Vmware: enter server url")
	...
	testing.ContextLog(ctx, "Vmware: enter username and password")
...
// VMware. Before calling make sure main Vmware screen is visible by calling
...
func (c *Connector) ResetSearch(ctx context.Context, k *input.KeyboardEventWriter) error {
	testing.ContextLog(ctx, "Vmware: clean search")
