...
func (c *Connector) Login(ctx context.Context, k *input.KeyboardEventWriter, cfg *apps.VDILoginConfig) error {
	...
	if err := c.detector.WaitUntilExists(uidetection.CustomIcon(c.state.DataPath(UIFragments[SplashscreenServerURLTbx])))(ctx); err != nil {
...
// Citrix. Call ResetSearch() to clean the search state.
