...
const hwclockDateFormat = "02 Jan 2006 15:04"
...
const hwclockOutputFormat = "2006-01-02 15:04:05.000000Z07:00"
const hwclockTimeout = 2 * time.Second

// RTC represents a real time clock that is accessed with the "hwclock" command
type RTC struct {
	// DevName indicates which RTC to use. For example DevName="rtc1" means that /dev/rtc1 will be use.
	DevName string
...
// Set the RTC using the "hwclock" command. This only changes the
// external hardware clock, it does not change the OS/system time.
func (rtc RTC) Write(ctx context.Context, t time.Time) error {
	...
	args = append(args, fmt.Sprintf("--date=%s", dateString))

	ctx, cancel := context.WithTimeout(ctx, hwclockTimeout)
	defer cancel()
	cmd := testexec.CommandContext(ctx, "hwclock", args...)
	return cmd.Run(testexec.DumpLogOnError)
}

// Read the RTC using the "hwclock" command.
func (rtc RTC) Read(ctx context.Context) (time.Time, error) {
	...
	defer cancel()
	cmd := testexec.CommandContext(ctx, "hwclock", args...)
	bytes, err := cmd.CombinedOutput(testexec.DumpLogOnError)
	if err != nil {
...
func (rtc RTC) hwclockArgs() []string {
	args := []string{}
	args = append(args, fmt.Sprintf("--rtc=/dev/%s", rtc.DevName))
	if rtc.LocalTime {
