...
func ResetChrome(ctx context.Context, fdms *fakedms.FakeDMS, cr *chrome.Chrome) error {
	if err := ResetChromeWithBlob(ctx, fdms, cr, fakedms.NewPolicyBlob()); err != nil {
...
// ResetChromeWithBlob resets chrome and removes all policies previously served by the FakeDMS with PolicyBlob.
