...
func InstallPwaAppByPolicy(ctx context.Context, tconn *chrome.TestConn, cr *chrome.Chrome, fdms *fakedms.FakeDMS, root http.FileSystem) (string, string, func(ctx context.Context) error, error) {
	...
	var id string
	var err error
	// TODO(b/254067263): Ensure the policy is refreshed for app installation particularly on Lacros when ServeAndVerify returns. Remove this polling once the flake is addressed.
