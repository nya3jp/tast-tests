...
type FixtData struct {
	...
	// Private helper functions used to implement ResetChrome.
	reset     func(ctx context.Context) error
	setChrome func(cr *chrome.Chrome)
}
...
func (f *FixtData) ResetChrome(ctx context.Context) error {
	...
	if err := f.reset(resetCtx); err != nil {
...
func (p *policyChromeFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	return &FixtData{p.fdms, p.cr, func(ctx context.Context) error {
		return p.Reset(ctx)
	}, func(cr *chrome.Chrome) {
		p.cr = cr
	}}
}
