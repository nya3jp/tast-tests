...
type Cmd struct {
	...
	*exec.Cmd
	...
	// watchdogStop is notified in Wait to ask the watchdog goroutine to exit
	// without killing the process.
	watchdogStop chan bool

	// watchdogDone is notified when the watchdog goroutine is finished.
...
func (c *Cmd) Start() error {
	...
	c.watchdogStop = make(chan bool, 1)
	c.watchdogDone = make(chan bool, 1)
