...
// WaitForPciDevice polls for the presence of the PCI device with ID |VID:DID|
// Note: This expects both the Vendor ID an Device ID to be provided and will return
// an error for wildcard or partial IDs such as "*:*", or "ffff:".
func WaitForPciDevice(ctx context.Context, pciID string, maxWaitTime time.Duration) error {
	...
	ids := strings.Split(pciID, ":")
	if len(ids) != 2 {
		return errors.Errorf("unsupported PCI ID format, got %q, want %q", pciID, "<Vendor ID>:<Device ID>")
	}
	if _, err := hex.DecodeString(ids[0]); err != nil {
		return errors.Errorf("unsupported Vendor ID format in PCI ID: %q", pciID)
	}
	if _, err := hex.DecodeString(ids[1]); err != nil {
		return errors.Errorf("unsupported Device ID format in PCI ID: %q", pciID)
	}

	// Check whether PCI device presented as |VID:DID| exists in host
