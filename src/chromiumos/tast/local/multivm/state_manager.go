...
type GaiaVars struct {
	...
	GaiaIDVar string // the secret variable for the GAIA ID
}
...
func (s *StateManager) Activate(ctx context.Context, st StateManagerTestingState) (errRet error) {
	...
	if err := func() error {
		...
		if s.crOptions.gaia != nil {
			username := st.RequiredVar(s.crOptions.gaia.UserVar)
			password := st.RequiredVar(s.crOptions.gaia.PassVar)
			gaiaID := st.RequiredVar(s.crOptions.gaia.GaiaIDVar)
			opts = append(opts, chrome.GAIALogin(), chrome.Auth(username, password, gaiaID))
		}
		if s.useARC != nil {
			if s.crOptions.gaia != nil {
	...
	if s.useARC != nil {
		if err := func() error {
			...
			if err := s.arcState.Activate(ctx, st.OutDir()); err != nil {
				return errors.Wrap(err, "failed to activate ARC state")
			}
			arc.Lock()
			return nil
		}(); err != nil {
	...
	if s.useCrostini != nil {
		if err := func() error {
			...
			// TODO: report disk size?
			iOptions := &cui.InstallationOptions{
				UserName:      s.cr.User(),
				Mode:          s.useCrostini.mode,
				MinDiskSize:   s.useCrostini.minDiskSize,
				DebianVersion: s.useCrostini.debianVersion,
			}
			if s.useCrostini.mode == cui.Artifact {
				iOptions.ImageArtifactPath = st.DataPath(crostini.ImageArtifact)
			}
			if err := cui.InstallCrostini(ctx, s.tconn, iOptions); err != nil {
...
func (s *StateManager) CheckAndReset(ctx context.Context, st StateManagerTestingState) error {
	...
	// ARC.
	if s.arcState.Active() {
		if err := s.arcState.CheckAndReset(ctx, st.OutDir()); err != nil {
			return errors.Wrap(err, "failed checking or resetting ARC")
		}
	}
	...
	if s.crostini != nil {
		...
		// TODO: (cwd) Reset crostini state between tests?
	}
	return nil
}
...
func (s *StateManager) Deactivate(ctx context.Context) (errRet error) {
	...
	if s.arcState.Active() {
		defer func() {
			arc.Unlock()
			if err := s.arcState.Deactivate(ctx); err != nil {
				if errRet == nil {
					errRet = errors.Wrap(err, "failed to deactivate ARC")
				} else {
					testing.ContextLog(ctx, "Failed to deactivate ARC: ", err)
				}
			}
		}()
	}
