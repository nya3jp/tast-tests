...
// VMOptions to ensure the methods can only be called after a call to Activate.
...
type StateManager struct {
	...
	vmOptions map[string]VMOptions
...
func NewStateManager(crOptions ChromeOptions, vms ...VMOptions) StateManager {
	vmOpts := make(map[string]VMOptions)
	for _, vmOpt := range vms {
		vmOpts[vmOpt.Name()] = vmOpt
	}
...
// VMs returns the active VMs as map, keyed by a VM-defined name.
...
func (s *StateManager) Activate(ctx context.Context, st StateManagerTestingState) (errRet error) {
	...
	if err := func() error {
		...
		if _, ok := s.vmOptions[ARCName]; ok {
			opts = append(opts, chrome.ARCEnabled())
		} else {
			opts = append(opts, chrome.ARCDisabled())
		}
		if _, ok := s.vmOptions[CrostiniName]; ok {
	...
	for k, v := range s.vmOptions {
		if err := func() error {
			...
			return errors.Wrapf(err, "activating %s", k)
		}
	}
...
func (s *StateManager) CheckAndReset(ctx context.Context, st StateManagerTestingState) error {
	...
	for k, v := range s.vms {
		if err := v.CheckAndReset(ctx, st); err != nil {
			return errors.Wrapf(err, "checking and resetting %s", k)
		}
	}
