...
type PreData struct {
	...
	ARC *arc.ARC
	// Available if useCrostini procided to preImpl.
	Crostini *vm.Container
}

func (p *preImpl) String() string { return p.name }
func (p *preImpl) Timeout() time.Duration {
	timeout := p.crOptions.timeout
	if p.useARC != nil {
		timeout += p.useARC.timeout
	}
	if p.useCrostini != nil {
		timeout += p.useCrostini.timeout
	}
	return timeout
}
...
func (p *preImpl) Prepare(ctx context.Context, s *testing.PreState) interface{} {
	if p.Active() {
		if err := p.CheckAndReset(ctx, s); err != nil {
	...
	if err := p.Activate(ctx, s); err != nil {
