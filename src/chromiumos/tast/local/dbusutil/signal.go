...
func (sw *SignalWatcher) Close(ctx context.Context) error {
	...
	// To stop watching signals from sw.conn, i.e. sw.conn.RemoveSignal(
	// sw.allSigs), we first need to consume sw.Signals to avoid the case
	// that sw.Signals and sw.allSigs are full and sw.allSigs block conn
	// dispatching goroutines which hold RLock of its channel slice. To
	// avoid deadlock, the signal consuming goroutine must be run before
	// RemoveSignal(sw.allSigs) is called, or else RemoveSignal may be
	// blocked while acquiring Lock of channel slice.
