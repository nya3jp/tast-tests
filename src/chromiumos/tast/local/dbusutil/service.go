...
func SystemBusPrivateWithAuth(ctx context.Context, uid uint32) (*dbus.Conn, error) {
	...
	// Workaround for b/206462481: the syscall.Setreuid attempts to change the EUID on all threads and hangs.
	// Instead, we change the EUID on current OS thread only (via syscall.RawSyscall) and ensure that this
	// same thread is used to communicate with DBus by calling runtime.LockOSThread.
