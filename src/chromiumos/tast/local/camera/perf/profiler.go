...
func Start(ctx context.Context, sDur, mDur time.Duration, outPrefix, outdir string) *ProfilerContext {
	...
	pctx.tasks = []*task{
		...
		{name: "PerfRecord", run: func() (*perf.Values, error) {
			return runProfiler(&pctx, "perf", profiler.Perf(profiler.PerfRecordOpts()))
		}},
		{name: "Top", run: func() (*perf.Values, error) {
			return runProfiler(&pctx, "top", profiler.Top(nil))
		}},
	}
...
func runProfiler(pctx *ProfilerContext, name string, prof profiler.Profiler) (*perf.Values, error) {
	testing.Sleep(pctx.Ctx, pctx.StabilizeDuration)
	rp, err := profiler.Start(pctx.Ctx, pctx.OutDir, prof)
	if err != nil {
		return nil, errors.Wrapf(err, "failed to start %s profiler", name)
	}
	if err := testing.Sleep(pctx.Ctx, pctx.MeasureDuration); err != nil {
