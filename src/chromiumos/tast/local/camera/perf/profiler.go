...
// CPU and GPU usage, power, perf events and top. The profilers availabe are:
...
// is not given.
func Start(ctx context.Context, sDur, mDur time.Duration, outPrefix, outdir string, profilers []string) *ProfilerContext {
	...
	allTaskfunc := map[string]taskfunc{
	...
	if profilers == nil {
		...
		pctx.tasks = []*task{
			{name: CPU, run: allTaskfunc[CPU]},
			{name: GPU, run: allTaskfunc[GPU]},
			{name: Top, run: allTaskfunc[Top]},
		}
	} else {
		for _, p := range profilers {
			if _, exists := allTaskfunc[p]; !exists {
				testing.ContextLog(ctx, "Unknown profiler: ", p)
				return nil
			}
			pctx.tasks = append(pctx.tasks, &task{name: p, run: allTaskfunc[p]})
		}
	}
