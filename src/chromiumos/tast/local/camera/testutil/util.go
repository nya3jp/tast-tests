...
func CloseApp(ctx context.Context, cr *chrome.Chrome, appConn *chrome.Conn) error {
	...
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		if err := tconn.Call(ctx, &isOpen, `tast.promisify(chrome.autotestPrivate.isSystemWebAppOpen)`, apps.Camera.ID); err != nil {
			return testing.PollBreak(errors.Wrap(err, "failed to check if camera app is open via AutoTestPrivate API"))
		}
		if isOpen {
			return errors.New("failed to close app within time. Unexpected result returned from AutoTestPrivate API")
		}
		return nil
