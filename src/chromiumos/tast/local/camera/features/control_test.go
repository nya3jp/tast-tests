...
const (
  ...
  "redrix4es": {
    "feature_set": [ {
      ...
      "type": "face_detection",
      "config_file_path": "/etc/camera/face_detection_config_redrix4es.json"
    } ]
  }
}`
	model            = "redrix"
...
func TestNewModelConfigFromJSON(t *testing.T) {
	...
	if err := modelConf.FeatureConfig(hdrnet, featureConf, []byte(fakeHDRnetConfig)); err != nil {
		t.Errorf("Failed to get feature config for %q: %s", hdrnet, err)
	}
	for k, v := range map[string]interface{}{
		...
		if featureConf[k] != v {
			t.Errorf("Unexpected %s: got %s , want %s", k, featureConf[k], v)
		}
	}
}
...
func TestFeatureConfigFilePath(t *testing.T) {
	...
	if p, err := modelConf.FeatureConfigFilePath(hdrnet); p != expPath || err != nil {
		t.Errorf("Unexpected %q config path: got %v, want %v", hdrnet, p, expPath)
	}
