...
func (a *App) ToggleOption(ctx context.Context, option Option) (bool, error) {
	...
	if err := a.Click(ctx, option.UIComponent); err != nil {
		return false, err
	}
...
func (a *App) setEnableGifRecording(ctx context.Context, enabled bool) error {
	...
	if err := a.setEnableOption(ctx, showGifRecordingOption, enabled); err != nil {
		if isUINotExist(err) {
			// No need to enable it after formally launched.
			// TODO(b/191950622): Remove the old enbling logic after formally launched.
		} else {
			return errors.Wrap(err, "failed to enable gif recording")
		}
	}
