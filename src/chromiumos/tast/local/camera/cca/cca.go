...
// enableMultiStreamRecordingOption toggles the multi-stream video recording option, which is one of the expert mode options, on if it is currently off.
func (a *App) enableMultiStreamRecordingOption(ctx context.Context) error {
	return a.enableOption(ctx, "enable-multistream-recording", "#expert-enable-multistream-recording")
}

// EnableMultiStreamRecording enables recording videos with multiple streams via expert mode.
func (a *App) EnableMultiStreamRecording(ctx context.Context) error {
	...
	if err := a.enableMultiStreamRecordingOption(ctx); err != nil {
