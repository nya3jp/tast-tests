...
func (a *App) RecordGif(ctx context.Context, save bool) (os.FileInfo, error) {
	...
	if err := a.Click(ctx, GifRecordingOption); err != nil {
		if !isUINotExist(err) {
			return nil, errors.Wrap(err, "failed to click gif recording option")
		}
		// The legacy UI hide the enabling of gif recording in expert mode.
		// Toggle the option in expert mode and try again.
		...
		if err := a.Click(ctx, GifRecordingOption); err != nil {
			return nil, errors.Wrap(err, "failed to click gif recording option after toggling enabling option")
		}
	}
