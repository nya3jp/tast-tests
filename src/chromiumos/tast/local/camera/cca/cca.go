...
func (a *App) ConfirmResult(ctx context.Context, isConfirmed bool, mode Mode) error {
	...
	if isConfirmed {
		selector, err := a.resolveUISelector(ctx, ConfirmResultButton)
		if err != nil {
			return errors.Wrap(err, "failed to get the selector for confirm button")
		}
		...
		expr := fmt.Sprintf("setTimeout(() => Tast.click(%q), 0)", selector)
