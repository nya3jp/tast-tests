...
func (s *Selector) location() (*Location, error) {
	...
	if len(s.boundingBoxes) == 0 {
		return nil, errors.New("no matching elements found")
	}
	if s.first {
		return s.boundingBoxes[0], nil
	}
	if len(s.boundingBoxes) < s.nth {
		return nil, errors.Errorf("find %d elements, but want the %d-th one", len(s.boundingBoxes), s.nth)
	}
	return s.boundingBoxes[s.nth-1], nil
}
