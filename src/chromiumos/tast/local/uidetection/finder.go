...
func (f *Finder) pxBuilder(rect *coords.Rect) *Finder {
...
// WithinUid ensures that the element returned must be within the element returned by the uid finder on the screen.
func (f *Finder) WithinUid(other *Finder) *Finder { // NOLINT: uid is not a UID, but a package name, so it shouldn't be caps.
	return f.newConstraint(func(ctx context.Context, uda *Context, scaleFactor float64) (*coords.Rect, error) {
...
// WithinUiauto ensures that the element returned must be above the element returned by the uiauto finder on the screen.
func (f *Finder) WithinUiauto(other *nodewith.Finder) *Finder {
	return f.newConstraint(func(ctx context.Context, uda *Context, scaleFactor float64) (*coords.Rect, error) {
...
func (f *Finder) dpBuilder(dp int, direction directionFn) *Finder {
	return f.newConstraint(func(ctx context.Context, uda *Context, scaleFactor float64) (*coords.Rect, error) {
		return direction(int(float64(dp) / scaleFactor)), nil
	})
}

func (f *Finder) uidBuilder(other *Finder, direction directionFn, side func(Location) int) *Finder {
	return f.newConstraint(func(ctx context.Context, uda *Context, scaleFactor float64) (*coords.Rect, error) {
		loc, err := other.locationPx(ctx, uda, scaleFactor)
		if err != nil {
			return nil, err
...
func (f *Finder) uiautoBuilder(other *nodewith.Finder, direction directionFn, side func(coords.Rect) int) *Finder {
	return f.newConstraint(func(ctx context.Context, uda *Context, scaleFactor float64) (*coords.Rect, error) {
		loc, err := uiauto.New(uda.tconn).WithPollOpts(uda.pollOpts).Location(ctx, other)
		if err != nil {
			return nil, err
...
func (f *Finder) AboveDp(dp int) *Finder {
	return f.dpBuilder(dp, above)
}

// AboveUid ensures that the element returned must be above the element returned by the uid finder on the screen.
func (f *Finder) AboveUid(other *Finder) *Finder { // NOLINT: uid is not a UID, but a package name, so it shouldn't be caps.
	return f.uidBuilder(other, above, func(l Location) int { return l.Top })
}

// AboveUiauto ensures that the element returned must be above the element returned by the uiauto finder on the screen.
func (f *Finder) AboveUiauto(other *nodewith.Finder) *Finder {
	return f.uiautoBuilder(other, above, func(r coords.Rect) int { return r.Top })
}

// BelowPx ensures that the element returned must be below px pixels on the screen.
func (f *Finder) BelowPx(px int) *Finder {
	return f.pxBuilder(below(px))
}
...
func (f *Finder) BelowDp(dp int) *Finder {
	return f.dpBuilder(dp, below)
}

// BelowUid ensures that the element returned must be below the element returned by the uid finder on the screen.
func (f *Finder) BelowUid(other *Finder) *Finder { // NOLINT: uid is not a UID, but a package name, so it shouldn't be caps.
	return f.uidBuilder(other, below, func(l Location) int { return l.Bottom() })
}

// BelowUiauto ensures that the element returned must be below the element returned by the uiauto finder on the screen.
func (f *Finder) BelowUiauto(other *nodewith.Finder) *Finder {
	return f.uiautoBuilder(other, below, func(r coords.Rect) int { return r.Bottom() })
}

// LeftOfPx ensures that the element returned must be left of px pixels on the screen.
func (f *Finder) LeftOfPx(px int) *Finder {
	return f.pxBuilder(leftOf(px))
}
...
func (f *Finder) LeftOfDp(dp int) *Finder {
	return f.dpBuilder(dp, leftOf)
}

// LeftOfUid ensures that the element returned must be left of the element returned by the uid finder on the screen.
func (f *Finder) LeftOfUid(other *Finder) *Finder { // NOLINT: uid is not a UID, but a package name, so it shouldn't be caps.
	return f.uidBuilder(other, leftOf, func(l Location) int { return l.Left })
}

// LeftOfUiauto ensures that the element returned must be left of the element returned by the uiauto finder on the screen.
func (f *Finder) LeftOfUiauto(other *nodewith.Finder) *Finder {
	return f.uiautoBuilder(other, leftOf, func(r coords.Rect) int { return r.Left })
}
...
func (f *Finder) RightOfDp(dp int) *Finder {
	return f.dpBuilder(dp, rightOf)
}

// RightOfUid ensures that the element returned must be right of the element returned by the uid finder on the screen.
func (f *Finder) RightOfUid(other *Finder) *Finder { // NOLINT: uid is not a UID, but a package name, so it shouldn't be caps.
	return f.uidBuilder(other, rightOf, func(l Location) int { return l.Right() })
}

// RightOfUiauto ensures that the element returned must be right of the element returned by the uiauto finder on the screen.
func (f *Finder) RightOfUiauto(other *nodewith.Finder) *Finder {
	return f.uiautoBuilder(other, rightOf, func(r coords.Rect) int { return r.Right() })
}

// locationPx resolves the UI detection request and stores the bounding boxes of the matching element.
func (f *Finder) locationPx(ctx context.Context, uda *Context, scaleFactor float64) (*Location, error) {
	...
	case numMatches == 1:
		if f.nth > 0 {
			return nil, errors.Errorf("%s: find only one element, but want the %d-th one", ErrNthNotFound, f.nth)
		}
		return &locations[0], nil
	default: // case numMatches > 1.
		if f.nth < 0 {
			return nil, errors.Errorf("%s: found %d elementf. If it is expected, consider using First() or Nth()", ErrMultipleMatch, numMatches)
		}
		if f.nth > numMatches-1 {
			return nil, errors.Errorf("%s: find %d elements, but want the %d-th one", ErrNthNotFound, numMatches, f.nth)
		}
