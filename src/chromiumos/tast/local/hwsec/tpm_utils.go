...
//
// There might be multiple errors happening in this function, in which case only the last error
// will be returned, and all but the last error will be logged. It's caller's responsibility to log
// the returned error, if any.
func ResetTPMAndSystemStates(ctx context.Context) (lastErr error) {
	// Makes sure this is a TPM 2.0 device.
	version, lastErr := GetTPMVersion(ctx)
	if lastErr != nil {
		return errors.Wrap(lastErr, "failed to get TPM version")
	} else if version != "2.0" {
	...
	hasSysKey, lastErr := hasSystemKey(ctx)
	if lastErr != nil {
		return errors.Wrap(lastErr, "failed to check system key in NVRAM")
	}

	// Checks if we have system key backup.
	hasSysKeyBackup, lastErr := hasSystemKeyBackup()
	if lastErr != nil {
		return errors.Wrap(lastErr, "failed to check the system key backup file")
	}
	...
	// Logs the last error, if any, and replaces it with the new error, if not nil.
	// This is to make sure none of the errors is silently suppressed.
	logAndUpdateLastErr := func(newErr error) {
		if newErr == nil {
			return
		}

		if lastErr != nil {
			testing.ContextLogf(ctx, "%v", lastErr)
		}
	...
	defer func() {
		...
		logAndUpdateLastErr(err)

		err = cryptohome.CheckService(ctx)
		logAndUpdateLastErr(err)
	}()
	daemonsToStop := reverseStringSlice(jobsToRestart)
	if lastErr = stopJobs(ctx, daemonsToStop); lastErr != nil {
		return lastErr
	}

	// Actually clears the TPM.
	if lastErr = testexec.CommandContext(ctx, "tpm_softclear").Run(); lastErr != nil {
		return lastErr
	}
	...
	if lastErr = stopJobs(ctx, trunksd); lastErr != nil {
		return lastErr
	}

	if hasSysKey {
		lastErr = restoreSystemKey(ctx)
