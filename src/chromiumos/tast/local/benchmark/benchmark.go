...
const (
	// Console sets the output formatting to human-readable texts. This is the default used by Google Benchmark.
	Console Format = iota
...
type Benchmark struct {
	...
	// filter is a string pattern as defined by the Google Benchmark for specifying the sub-benchmark(s) to run.
	filter string

	// resultFormat specifies the output formatting of the benchmark result written to stdout.
	resultFormat Format

	// outFile is an optional file path for storing another copy of the benchmark result in addition to the one written to stdout.
	outFile string

	// outResultFormat specifies the output formatting of the benchmark result writted to outFile.
	outResultFormat Format

	// extraArgs is a list of strings specifying the extra arguments that will be fed to the benchmark binary.
	extraArgs []string
}
...
// ResultFormat sets the output formatting for stdout.
func ResultFormat(format Format) option {
	return func(b *Benchmark) { b.resultFormat = format }
}
...
func (b *Benchmark) Args() ([]string, error) {
	...
	switch b.resultFormat {
	case Console:
		args = append(args, "--benchmark_format=console")
	case JSON:
		args = append(args, "--benchmark_format=json")
	case CSV:
		args = append(args, "--benchmark_format=csv")
	}
...
func (b *Benchmark) Run(ctx context.Context) ([]byte, error) {
	args, err := b.Args()
	if err != nil {
		return nil, err
	}
...
// UnmarshalJSONBytes unmarshals the given byte array as JSON payload and returns the BenchmarkResult instance.
func UnmarshalJSONBytes(bytes []byte) Result {
	var ret Result
