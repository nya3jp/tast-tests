...
// This is useful for investigating hang processes.
package minidump
...
// MatchByName specified processes by process names.
...
// MatchByName specified processes by PIDs.
...
// ctx is used only for logging. Its deadline is ignored.
func SaveWithoutCrash(ctx context.Context, dir string, matchers ...Matcher) {
	...
	for _, p := range procs {
		...
		testing.ContextLogf(ctx, "Saving minidump: %s (%d)", name, p.Pid)
		path := filepath.Join(dir, fmt.Sprintf("%s.%d.dmp", name, p.Pid))
		saveWithoutCrash(ctx, p.Pid, path)
	}
}

// freeze finds processes matching matches and sends SIGSTOP to them.
...
// unfreeze sends SIGCONT to processes.
