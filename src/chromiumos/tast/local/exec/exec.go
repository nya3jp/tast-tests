...
// Package exec is a wrapper of the standard os/exec package optimized for
...
// Usage
//
//  cmd := exec.CommandContext(ctx, "some", "external", "command")
...
type Cmd struct {
	...
	*exec.Cmd
	...
	// watchdogStopCh is closed in Wait to ask the watchdog goroutine to exit
	// without killing the process.
	watchdogStopCh chan struct{}

	// watchdogDoneCh is closed when the watchdog goroutine is finished.
...
// OutputCombined runs an external command, waits for its completion and
// returns stdout/stderr output of the command.
//
// See os/exec package for details.
func (c *Cmd) OutputCombined() ([]byte, error) {
	if c.Stdout != nil {
		return nil, errStdoutSet
	}
	if c.Stderr != nil {
...
func (c *Cmd) Wait() error {
	...
	// Ask the watchdog goroutine to exit.
