...
func SwitchSlot(ctx context.Context) (uint32, error) {
	...
	modemManufacturer, err := props.GetString(mmconst.ModemPropertyManufacturer)
	if err != nil {
		return math.MaxUint32, errors.Wrap(err, "missing manufacturer property")
	}
	setSlot := 1
	if modemManufacturer == mmconst.ModemQualcomm {
		if primary == 1 {
			setSlot = 2
		}
	} else if modemManufacturer == mmconst.ModemFibocom {
		setSlot = 0
		if primary == 0 {
			setSlot = 1
		}
