...
// EnsureConnectState checks modem property disconnected
func EnsureConnectState(ctx context.Context, modem *Modem, simpleModem *Modem, expectedConnected bool) error {
	// poll for expected modem state
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if (!expectedConnected && isConnected) || (expectedConnected && !isConnected) {
			return errors.New("still not in expected stated")
		}
		return nil
	}, &testing.PollOptions{
