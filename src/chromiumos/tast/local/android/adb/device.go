...
func (d *Device) GMSCoreVersion(ctx context.Context) (int, error) {
	versionInfo, err := d.ShellCommand(ctx, "dumpsys", "package", "com.google.android.gms", "|", "grep", "versionCode").Output(testexec.DumpLogOnError)
	if err != nil {
		return 0, err
	}
...
func (d *Device) GoogleAccount(ctx context.Context) (string, error) {
	accountInfo, err := d.ShellCommand(ctx, "dumpsys", "account", "all", "|", "grep", "Account").Output(testexec.DumpLogOnError)
	if err != nil {
		return "", err
	}
