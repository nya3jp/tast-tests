...
// Forward forwards the ADB device local port specified to a host port and returns that host port.
func (d *Device) Forward(ctx context.Context, localPort int) (int, error) {
	out, err := d.Command(ctx, "forward", "tcp:0", fmt.Sprintf("tcp:%d", localPort)).Output(testexec.DumpLogOnError)
	if err != nil {
		return -1, err
	}
...
// RemoveForward removes the forwarding from an ADB device local port to the specified host port.
func (d *Device) RemoveForward(ctx context.Context, hostPort int) error {
