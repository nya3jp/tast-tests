...
// HashFile computes the sha256sum of the file within the Android download directory.
func (s *AndroidNearbyDevice) HashFile(ctx context.Context, filename string) (string, error) {
	hashCmd := s.device.ShellCommand(ctx, "sha256sum", downloadDir+filename)
	res, err := hashCmd.Output()
	if err != nil {
...
func (s *AndroidNearbyDevice) SetupDevice(dataUsage DataUsage, visibility Visibility, name string) error {
...
func (s *AndroidNearbyDevice) ReceiveFile(senderName, receiverName string, turnaroundTime time.Duration) (string, error) {
...
func (s *AndroidNearbyDevice) EventWaitAndGet(callbackID string, eventName SnippetEvent, timeout time.Duration) error {
...
func (s *AndroidNearbyDevice) AcceptTheSharing(token string) error {
...
func (s *AndroidNearbyDevice) CancelReceivingFile() error {
