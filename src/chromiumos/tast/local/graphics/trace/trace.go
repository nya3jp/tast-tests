...
func runTrace2(ctx context.Context, cont *vm.Container, entry *TestEntryConfig) (*perf.Values, error) {
    ...
    Multiple: true,
	}, getValues(testResult.Values, func(r ReplayResult) float64 { return float64(r.DurationInSeconds); })...)
	perfValues.Set(perf.Metric{
		Name:      entry.Name,
		Variant:   "frames",
		Unit:      "frame",
		...
		Multiple: true,
	}, getValues(testResult.Values, func(r ReplayResult) float64 { return float64(r.TotalFrames); })...)
	perfValues.Set(perf.Metric{
		Name:      entry.Name,
		Variant:   "fps",
		Unit:      "fps",
		Direction: perf.BiggerIsBetter,
		Multiple: true,
	}, getValues(testResult.Values, func(r ReplayResult) float64 { return float64(r.AverageFps); })...)
	testing.ContextLog(ctx, fmt.Sprintf(">>>%v", *perfValues))
