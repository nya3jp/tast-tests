...
func RunTest(ctx context.Context, s *testing.State, cont *vm.Container, traces map[string]string) {
	...
	for traceFile, traceName := range traces {
		...
		defer st.End()
		output, err := runTrace(shortCtx, cont, s.DataPath(traceFile), traceName)
		if err != nil {
			s.Fatal("Failed running trace: ", err)
		}
		frames, duration, fps, err := parseResult(output)
		if err != nil {
			s.Fatal("Failed to parse trace replay output: ", err)
		}
...
func runTrace(ctx context.Context, cont *vm.Container, traceFile, traceName string) (string, error) {
	...
	if err != nil {
		cmd.DumpLog(ctx)
		return "", errors.Wrap(err, "failed to replay apitrace")
	}

	outDir, ok := testing.ContextOutDir(ctx)
	if !ok {
		return "", errors.New("failed to get OutDir for writing trace result")
	}
	...
	if err := ioutil.WriteFile(file, traceOut, 0644); err != nil {
		return "", errors.Wrap(err, "error writing tracing output")
	}
	return string(traceOut), nil
}
...
// parseResult parses the output of apitrace and return the statistic.
func parseResult(output string) (uint64, float64, float64, error) {
	re := regexp.MustCompile(`Rendered (\d+) frames in (\d*\.?\d*) secs, average of (\d*\.?\d*) fps`)
	match := re.FindStringSubmatch(output)
	if match == nil {
		return 0, 0, 0, errors.New("result line can't be located")
	}

	frames, err := strconv.ParseUint(match[1], 10, 64)
	if err != nil {
		return 0, 0, 0, errors.Wrapf(err, "failed to parse frames %q", match[1])
	}
	duration, err := strconv.ParseFloat(match[2], 64)
	if err != nil {
		return 0, 0, 0, errors.Wrapf(err, "failed to parse duration %q", match[2])
	}
	fps, err := strconv.ParseFloat(match[3], 64)
	if err != nil {
		return 0, 0, 0, errors.Wrapf(err, "failed to parse fps %q", match[3])
	}
	return frames, duration, fps, nil
}
