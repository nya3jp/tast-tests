...
// FileInfo is used to specifiy trace replay test storage artifact info
...
func (server *fileServer) ServeHTTP(wr http.ResponseWriter, req *http.Request) {
	testing.ContextLog(server.Context, "[Proxy Server] Serving request: ", req.URL.RawQuery)
	query, err := url.ParseQuery(req.URL.RawQuery)
	if err != nil {
		testing.ContextLogf(server.Context, "[Proxy Server] Error: Unable to parse request query <%s>: %s", req.URL.RawQuery, err.Error())
		wr.WriteHeader(http.StatusBadRequest)
		return
	}

	gsURLStr := query["d"][0]
	if validateGsURL(gsURLStr, server.Entries) != true {
		testing.ContextLogf(server.Context, "[Proxy Server] Error: Unable to find gs url <%s> in the test artifacts list", gsURLStr)
		wr.WriteHeader(http.StatusUnauthorized)
		return
	}

	testing.ContextLog(server.Context, "[Proxy Server] Downloading: ", gsURLStr)
	gsURL, err := url.Parse(gsURLStr)
	if err != nil {
		testing.ContextLogf(server.Context, "[Proxy Server] Error: Unable to parse gs url <%s>: %s", gsURLStr, err.Error())
		wr.WriteHeader(http.StatusBadRequest)
		return
	}

	r, err := server.CloudStorage.Open(server.Context, gsURL.String())
	if err != nil {
		testing.ContextLogf(server.Context, "[Proxy Server] Error: Unable to open <%v>: %v", gsURL, err)
	...
	if err != nil {
		testing.ContextLog(server.Context, "[Proxy Server] Error: io.Copy() failed: ", err)
	}
	testing.ContextLogf(server.Context, "[Proxy Server] Request served successfully. %d byte(s) copied", copied)
}
...
func startFileServer(ctx context.Context, addr string, cloudStorage *testing.CloudStorage, entries []TestEntryConfig) *http.Server {
	handler := &fileServer{Context: ctx, CloudStorage: cloudStorage, Entries: entries}
...
func replayTraceEntry(ctx context.Context, cont *vm.Container, entry *TestEntryConfig, perfValues *perf.Values) error {
	...
	if err := json.Unmarshal(replayOutput, &testResult); err != nil {
		return errors.Wrapf(err, "Unable to parse: %s", string(replayOutput))
	}
