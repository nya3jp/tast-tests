...
const (
	...
	hostInfoSystemBoardKey   = "CHROMEOS_RELEASE_BOARD"
	hostInfoSystemVersionKey = "CHROMEOS_RELEASE_VERSION"
...
// ProxyServerInfo struct is used as a container for a proxy server information
...
type TestSettings struct {
	SelectionAttributes []string `json:"SelectionAttributes"`
...
func getHostInfo() (*map[string]string, error) {
	var rex = regexp.MustCompile("(.*)=(.*)")
	file, err := os.Open(lsbReleaseFile)
	if err != nil {
		return nil, errors.Wrapf(err, "unable to open  <%s>", lsbReleaseFile)
	}
	defer file.Close()
	...
	for scanner.Scan() {
		data := rex.FindStringSubmatch(scanner.Text())
		k := data[1]
		hostInfo[k] = data[2]
	}

	if err := scanner.Err(); err != nil {
		return nil, errors.Wrapf(err, "unable to read <%s>", lsbReleaseFile)
	}
...
func RunTraceReplayTest(ctx context.Context, resultDir string, cloudStorage *testing.CloudStorage, cont *vm.Container, bundle *TestBundleConfig) error {
	...
	bundle.Host.SystemBoard = (*hostInfo)[hostInfoSystemBoardKey]
	bundle.Host.SystemVersion = (*hostInfo)[hostInfoSystemVersionKey]
