...
// serveLogRequest logs the content tramitted via http request via tast testing.ContextLog.
func (s *fileServer) serveLogRequest(ctx context.Context, wr http.ResponseWriter, msg string) error {
	s.log(ctx, msg)
	wr.WriteHeader(http.StatusOK)
	return nil
}
...
func (s *fileServer) ServeHTTP(wr http.ResponseWriter, req *http.Request) {
	...
	for key, val := range query {
		...
		case "download":
			...
			break
		case "log":
			if err := s.serveLogRequest(ctx, wr, val[0]); err != nil {
				s.log(ctx, "serveLogRequest failed: ", err.Error())
			}
			break
		default:
