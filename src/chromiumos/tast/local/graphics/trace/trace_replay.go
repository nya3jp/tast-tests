...
func (server *FileServer)ServeHTTP(wr http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		server.state.Fatalf("[Proxy Server] Unable to parse request query <%s>: %s", req.URL.RawQuery, err.Error())
	}

	gsUrlStr := query["d"][0]
	if validateGsUrl(gsUrlStr, server.entries) != true {
		server.state.Fatalf("[Proxy Server] Unable to find gs url <%s> in the test artifacts list", gsUrlStr)
	}

	server.state.Log("[Proxy Server] Downloading: ", gsUrlStr);
	gsUrl, err := url.Parse(gsUrlStr)
	if err != nil {
		server.state.Fatalf("[Proxy Server] Unable to parse gs url <%s>: %s", gsUrlStr, err.Error())
	}
...
func startFileServer(ctx context.Context, state *testing.State, addr string, entries []TestEntryConfig) *http.Server {
	handler := &FileServer{ ctx: ctx, state: state, entries: entries }
...
func RunTraceReplayTest(ctx context.Context, state *testing.State, cont *vm.Container, entries []TestEntryConfig) {
	...
	server := startFileServer(ctx, state, serverAddr, entries)
	defer func() {
