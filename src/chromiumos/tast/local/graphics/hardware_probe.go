...
func runAndGrepRegex(ctx context.Context, args []string, regexStr string) ([]string, error) {
	...
	matches = re.FindAllStringSubmatch(string(out))
...
// GPUFamily is similar to GPUFamily but instead returns the first GPU it found.
func GPUFamily(ctx context.Context) (string, error) {
	gpuFamilies, err := GPUFamilies(ctx)
	if err != nil {
		return "", err
	}
	return gpuFamilies[0], nil
}
