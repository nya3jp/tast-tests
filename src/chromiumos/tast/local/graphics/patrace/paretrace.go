...
func RunTrace(ctx context.Context, s *testing.State, apkName string, traceFile string) {
...
func waitForResult(ctx context.Context, a *arc.ARC, resultPath string) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		return nil
	}, &testing.PollOptions{Interval: 5 * time.Second}) // Also stops when the test timeouts
}
