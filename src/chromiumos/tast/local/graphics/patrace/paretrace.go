...
func RunTrace(ctx context.Context, s *testing.State, apkFile, traceFile string) {
	...
	if err := startRetrace(ctx, a, s.DataPath(apkFile), tracePath, resultPath); err != nil {
		s.Fatal("Cannot start retrace: ", err)
	}

	if err := waitForResult(ctx, a, resultPath); err != nil {
		s.Fatal("waitForResult failed: ", err)
	}
...
// startRetrace launches retrace of patrace. apkPath, tracePath and resultPath
// specify the android path to eglretrace-release.apk, the trace file and the
// output result file
func startRetrace(ctx context.Context, a *arc.ARC, apkPath, tracePath, resultPath string) error {
	const (
...
func waitForResult(ctx context.Context, a *arc.ARC, resultPath string) error {
