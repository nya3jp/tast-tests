...
func glesVersion(ctx context.Context) (major int, minor int, err error) {
	...
	if len(matches) != 1 {
		return 0, 0, fmt.Errorf(
			"%d OpenGL version strings found in wflinfo output:\n%s",
			len(matches), string(out))
	}
	if major, err = strconv.Atoi(matches[0][1]); err != nil {
		return 0, 0, fmt.Errorf("could not parse major version from '%s': %v",
			matches[0][0], err)
	}
	if minor, err = strconv.Atoi(matches[0][2]); err != nil {
		return 0, 0, fmt.Errorf("could not parse minor version from '%s': %v",
			matches[0][0], err)
	}
