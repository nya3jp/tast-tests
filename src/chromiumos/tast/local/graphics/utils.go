...
const (
	modesetConnectorPattern = `^(\d+)\s+\d+\s+(connected|disconnected)\s+(\S+)\s+\d+x\d+\s+\d+\s+\d+`

	// Group names match the drmModeModeInfo struct
	modesetModePattern = `\s+(?P<name>.+)\s+(?P<vrefresh>\d+)\s+(?P<hdisplay>\d+)\s+(?P<hsync_start>\d+)\s+(?P<hsync_end>\d+)\s+(?P<htotal>\d+)\s+(?P<vdisplay>\d+)\s+(?P<vsync_start>\d+)\s+(?P<vsync_end>\d+)\s+(?P<vtotal>\d+)\s+(?P<clock>\d+)\s+flags:.+type: preferred`
)
...
// GetModetestConnectors retrieves a list of connectors using modetest.
func GetModetestConnectors(ctx context.Context) ([]Connector, error) {
	...
	connectRe := regexp.MustCompile(modesetConnectorPattern)
	modeRe := regexp.MustCompile(modesetModePattern)
	for _, line := range strings.Split(string(output), "\n") {
		matches := connectRe.FindStringSubmatch(line)
		if matches != nil {
			...
			matches = modeRe.FindStringSubmatch(line)
			if matches != nil {
				var size [2]int
				var err error
				for i, name := range modeRe.SubexpNames() {
					if name == "hdisplay" {
...
// GetNumberOfOutputsConnected parses the output of modetest to determine the number of connected displays.
// And returns the number of connected displays.
func GetNumberOfOutputsConnected(ctx context.Context) (int, error) {
	connectors, err := GetModetestConnectors(ctx)
	if err != nil {
		return 0, err
	}
	connected := 0
