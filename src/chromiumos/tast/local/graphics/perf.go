...
func MeasureDramBandwidth(ctx context.Context, duration time.Duration, p *perf.Values) error {
	...
	if err != nil {
		// Failure to execute mtk_dram_tool might just indicate we
		// aren't running on a supported platform. Log a warning and
		// continue as usual.
		testing.ContextLog(ctx, "mtk_dram_tool failed to run, possibly unsupported platform")
		return nil
	}
