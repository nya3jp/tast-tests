...
// TimedRunGTest runs t for measurement seconds. It is expected that t would run
// freely for much longer than said mesasurement seconds. The function will
// return error if t fails to start, or finishes/dies before measurement.
func TimedRunGTest(ctx context.Context, measurement time.Duration, t *gtest.GTest) (retErr error) {
	...
	testing.ContextLog(ctx, "Running GTest for ", measurement.Round(time.Second))
