...
// monitoring, the returned counter map will be nil. If the reference TSC (Time
// Stamp Counter) is not available, it returns an error, because the TSC is
// needed.
func collectPackagePerformanceCounters(ctx context.Context, interval time.Duration) (counters map[string]int64, err error) {
	...
	// Limit the collection to the first core ("-C 1") otherwise the results would
