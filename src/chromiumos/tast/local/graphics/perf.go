...
func collectPerformanceCounters(ctx context.Context, interval time.Duration, outDir string) (counters map[string]time.Duration, megaPeriods int64, err error) {
	...
	for _, perfCounter := range perfCounters {
		if _, err := os.Stat(perfCounter.filePath); err == nil {
			...
			testing.ContextLogf(ctx, "Could not find %s perf event file (error: %s)", perfCounter.filePath, err.Error())
		}
	}
	...
	for _, line := range strings.Split(perfOutput.String(), "\n") {
		for name, r := range regexps {
			...
			if err != nil {
				const perfStatOutputFile = "perf_stat_output.log"
				_ = ioutil.WriteFile(filepath.Join(outDir, perfStatOutputFile), perfOutput.Bytes(), 0644)
				return nil, 0, errors.Wrapf(err, "error parsing perf output, see %s if present", perfStatOutputFile)
			}
		}
	}
...
func parseAndReportCounter(ctx context.Context, counters map[string]time.Duration, counterName string, p *perf.Values) {
	if counter, ok := counters[counterName]; ok && counter.Seconds() != 0 {
		...
		reportMetric(counterName, "percent", usage, perf.SmallerIsBetter, p)
	}
}

// MeasureGPUCounters measures GPU usage for a period of time t into p.
func MeasureGPUCounters(ctx context.Context, t time.Duration, outDir string, p *perf.Values) error {
	testing.ContextLog(ctx, "Measuring GPU usage for ", t)
	counters, megaPeriods, err := collectPerformanceCounters(ctx, t, outDir)
	if err != nil {
