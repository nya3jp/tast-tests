...
func collectAMDBusyCounter(ctx context.Context, interval time.Duration) (counters map[string]time.Duration, megaPeriods int64, err error) {
	...
	accuBusy := float64(0)
	const samplePeriod = 500 * time.Millisecond
	...
	for i := 0; i < numSamples; i++ {
		...
		// Divide busy by hundred to remove the percentage.
		accuBusy += float64(busy) / 100.0
	...
	counters = make(map[string]time.Duration)
	counters["rcs"] = time.Duration(accuBusy * float64(time.Second))
