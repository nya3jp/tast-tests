...
// elapsed. If the hardware/kernel doesn't provide this type of event
// monitoring, or the reference TSC (Time Stamp Counter) is not available, the
// returned counter map will be nil.
func collectPackagePerformanceCounters(ctx context.Context, interval time.Duration) (counters map[string]int64, err error) {
	...
	for _, perfCounter := range perfCounters {
		if _, err := os.Stat(perfCounter.filePath); err == nil {
			...
			return nil, errors.Wrap(err, "could not find a necessary event")
		} else {
			testing.ContextLogf(ctx, "Could not find %s perf event file (error: %v)", perfCounter.filePath, err)
		}
	}
	...
	cmd := testexec.CommandContext(ctx,
		"/usr/bin/perf", "stat", "-C", "1", "-e", strings.Join(eventsToCollect, ","), "--", "sleep",
		strconv.FormatInt(int64(interval/time.Second), 10))
	_, stderr, err := cmd.SeparatedOutput()
	if err != nil {
...
// package was in a given state, with larger numbers corresponding to deeper
// states. The total elapsed cycles is available under the first CPU TSC (Time
// Stamp Counter) register. The "active " state, which would be c0, is the
...
func MeasurePackageCStateCounters(ctx context.Context, t time.Duration, p *perf.Values) error {
	...
	for name, value := range counters {
		...
		reportMetric(name, "percent", cStatePercent, perf.LargerIsBetter, p)
	}
