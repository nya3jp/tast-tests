...
func (f *gpuWatchDogFixture) checkHangs(ctx context.Context, reader *syslog.Reader) error {
	for {
		...
		if len(matches) > 0 {
			return errors.Errorf("detected a gpu hang during test: %s", e.Line)
		}
	}
	return nil
}
...
func (f *gpuWatchDogFixture) PreTest(ctx context.Context, s *testing.FixtTestState) {
	...
	// syslog.NewReader reports syslog message written after it is started for GPU hangs detection.
