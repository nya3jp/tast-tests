...
type Backend interface {
	...
	// Round implementesthe platform-specific graphic- or codec- rounding.
	Round(value int) int
...
func CompareGraphicsMemoryBeforeAfter(ctx context.Context, payload func(), backend Backend, width, height int) (err error) {
	...
	if before != after {
		return errors.Wrapf(err, "graphic objects before=%d, after=%d do not coincide! delta=%d", before, after, after-before)
	}
	testing.ContextLogf(ctx, "graphic objects before=%d, after=%d, OK", before, after)
	return nil
}
