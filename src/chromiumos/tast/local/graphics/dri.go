...
func GetBackend() (backend Backend, err error) {
	adrenoBackend := AdrenoBackend{}
	i915Backend := I915Backend{}
	// In the future we might want to support systems with both GPUs; for the time
	// being just fail.
	if adrenoBackend.SupportsFramebufferInfo() && i915Backend.SupportsFramebufferInfo() {
		return nil, errors.New("wystems with both i915 and adreno backends are not supported")
	}
	if i915Backend.SupportsFramebufferInfo() {
		return i915Backend, nil
	}
	if adrenoBackend.SupportsFramebufferInfo() {
