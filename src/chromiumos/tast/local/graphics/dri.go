...
type Backend interface {
	...
	// ReadFramebufferCount tries to retrieve the amount of framebuffers of width
...
// SupportsFramebufferInfo returns true if the debug file for graphics
// memory allocation readings is available.
...
func (g I915Backend) ReadFramebufferCount(ctx context.Context, width, height int) (framebuffers int, err error) {
	...
	for _, line := range lines {
		...
		_, err := fmt.Sscanf(line, "user size: %d x %d", &fbWidth, &fbHeight)
		if err != nil {
			continue
		}
...
// before and after running the payload function, using the backend.
func CompareGraphicsMemoryBeforeAfter(ctx context.Context, payload func(), backend Backend, width, height int) (err error) {
	...
	if before, err = readStableObjectCount(ctx, backend, width, height); err != nil {
		return errors.Wrap(err, "failed to get the i915 framebuffer object count")
	}

	payload()

	if after, err = readStableObjectCount(ctx, backend, width, height); err != nil {
		return errors.Wrap(err, "failed to get the i915 framebuffer object count")
	}
...
func readStableObjectCount(ctx context.Context, backend Backend, width, height int) (objectCount int, err error) {
	...
	// TODO()mcasas) width and height need to be rounded up to closest multiple of 16.
