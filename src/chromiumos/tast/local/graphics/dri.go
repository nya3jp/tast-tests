...
// SupportsI915FramebufferInfo returns true if the debug file for graphics
...
// before and after running the payload.
func CompareGraphicsMemoryBeforeAfter(ctx context.Context, payload func()) (err error) {
	...
	if after, err = readStableI915ObjectCount(ctx); err != nil || before == 0 {
		return errors.Wrap(err, "failed to get the i915 framebuffer object count")
	}
