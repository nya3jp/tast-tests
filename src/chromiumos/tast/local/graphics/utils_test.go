...
func TestExtractOpenGLVersion(t *testing.T) {
	for _, tc := range []struct {
		...
		{"TypicalOutput", "Waffle platform: null\n" +
			"Waffle api: gles2\n" +
		...
		{"DuplicateVersionString", "Waffle platform: null\n" +
			"Waffle api: gles2\n" +
			"OpenGL vendor string: Intel Open Source Technology Center\n" +
		...
		{"NoVersionString", "Waffle platform: null\n" +
			"Waffle api: gles2\n" +
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// This is to show a readable message depending on whether or not an
			// error is expected.
			eerr := "err = <nil>"
			if tc.err {
				eerr = "err != <nil>"
			...
			if (aerr != nil && !tc.err) ||
				(aerr == nil && tc.err) ||
				(amajor != tc.major || aminor != tc.minor) {
				t.Errorf("extractOpenGLVersion on %q = (%d, %d), err = %v; want (%d, %d), %v",
					tc.wflout, amajor, aminor, aerr, tc.major, tc.minor, eerr)
			}
		})
	}
}
