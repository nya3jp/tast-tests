...
func TestParseUIUseFlags(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err = ioutil.WriteFile(f, []byte(tc.conf), 0600)
...
OpenGL vendor string: Intel Open Source Technology Center`, 0, 0, true},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr && aerr == nil {
				t.Errorf("extractOpenGLVersion(%q) unexpectedly succeeded", tc.wflout)
				return
			} else if !tc.wantErr && aerr != nil {
				t.Errorf("extractOpenGLVersion(%q) unexpectedly failed: %v", tc.wflout, aerr)
				return
			}

			if amajor != tc.major || aminor != tc.minor {
				t.Errorf("extractOpenGLVersion(%q) = [%v, %v]; want [%v, %v]", tc.wflout, amajor, aminor, tc.major, tc.minor)
			}
		})
	}
}
