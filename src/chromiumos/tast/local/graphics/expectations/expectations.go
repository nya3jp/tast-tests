...
// overridedBoard can be specified to override the board identity.
// This can be used for manual testing and for early bringup.
...
// overridedChipset can be specified to override the GPU chipset identity.
// This can be used for manual testing and for early bringup.
...
func getDeviceModel(ctx context.Context) (string, error) {
	...
	if err != nil {
		...
		hwid := string(out)
		// hwid is in the form of: <MODEL>-AAAA ADA-ADA-ADA-ADA-ADA
		// where A is upper case alpabetic, and D is a digit.
		upperCaseModel, _, found := strings.Cut(hwid, "-")
		if !found {
...
func generateTestExpectationsFilename(ctx context.Context, testExpectationDirectory string, theFileType fileType) (string, error) {
	...
	return fmt.Sprintf("%s/%s-%s.%s", testExpectationDirectory, theFileType, identifier, expectationsFileExtension), err
}
