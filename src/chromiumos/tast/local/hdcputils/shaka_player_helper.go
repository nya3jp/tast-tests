...
var (
	...
	rdpPollOpts = &testing.PollOptions{Interval: time.Second, Timeout: 15 * time.Second}
)
...
// SelectVideoRobustness select specified video robustness name in the drop down list.
...
// VerifyVideoPlayWithDuration verifies whether video is playing or not, it returns error if any player error or video current time is
...
func (s *ShakaPlayer) VerifyVideoPlayWithDuration(ctx context.Context, duration int, cras *audio.Cras, expectedAudioNode string) error {
	curTimeBefore, err := s.GetVideoCurrentTime(ctx)
	if err != nil {
		return errors.Wrap(err, "failed to get video current time")
	}
	timeBefore := strings.Split(strings.Split(curTimeBefore, " ")[0], ":")
	...
	totalSecBefore := m1*60 + s1

	// Verify video play for the provided duration.
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		// Check if any player error found for the specified video duration.
		if err := s.CheckPlayerError(ctx); err != nil {
			return testing.PollBreak(errors.Wrap(err, "video player error found"))
		}
		curTimeAfter, err := s.GetVideoCurrentTime(ctx)
		if err != nil {
			return errors.Wrap(err, "failed to get video current time")
		}
		timeAfter := strings.Split(strings.Split(curTimeAfter, " ")[0], ":")
		m2, err := strconv.Atoi(strings.TrimSpace(timeAfter[0]))
		if err != nil {
			return errors.Wrap(err, "failed to convert string to integer")
		}
		s2, err := strconv.Atoi(strings.TrimSpace(timeAfter[1]))
		if err != nil {
			return errors.Wrap(err, "failed to convert string to integer")
		}
		totalSecAfter := m2*60 + s2
		...
		if diffSec < duration {
			return errors.Errorf("failed to play video untill specified duration %d sec", duration)
		}
		return nil
...
func (s *ShakaPlayer) FullScreenEntryExit(ctx context.Context, iteration int) error {
	...
	for i := 1; i <= iteration; i++ {
		...
		// Play video in default screen for atleast 1 seconds.
		testing.Sleep(ctx, 1500*time.Millisecond)
	}
	return nil
}
...
// ShowVideoSeekbar hover mouse on video seek bar to get updated current time.
...
func VerifyFirstRunningDevice(ctx context.Context, cras *audio.Cras, expectedAudioNode string) error {
	return testing.Poll(ctx, func(ctx context.Context) error {
		...
		if deviceName := expectedAudioNode; deviceName != devName {
