...
// RemoveContainerFileOrDie removes a file from the container's file system.
// This is useful for cleaning up files placed in the container by
// TransferToContainerOrDie().
func RemoveContainerFileOrDie(ctx context.Context, s *testing.State, cont *vm.Container, containerPath string) {
	if err := cont.Command(ctx, "sh", "-c", shutil.EscapeSlice([]string{"sudo", "rm", containerPath})).Run(testexec.DumpLogOnError); err != nil {
		s.Fatalf("Failed to remove %q: %v", containerPath, err)
	}
}
