...
// UpdatePerf records changes in the Recording's Chrome metrics, storing the
// bucket mid-points to a perf.Values
func (r *Recording) UpdatePerf(ctx context.Context, tconn *chrome.TestConn) (*perf.Values, error) {
	if r.recorder == nil {
		return nil, errors.New("There is no recorder for perf metrics")
	}
