...
// StartedArtifactStretchARCEnabledGaia is similar to StartedByArtifactStrech, but will
...
func (p *preImpl) Prepare(ctx context.Context, s *testing.PreState) interface{} {
	...
	if p.arcEnabled {
		opts = []chrome.Option{chrome.ARCEnabled(), chrome.ExtraArgs(arc.DisableSyncFlags()...), chrome.ExtraArgs("--vmodule=crostini*=1")}
	}
