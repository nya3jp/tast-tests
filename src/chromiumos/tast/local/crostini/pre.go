...
func (p *preImpl) Prepare(ctx context.Context, s *testing.PreState) interface{} {
	...
	if keepState && vm.TerminaImageExists() {
	...
	if keepState && vm.TerminaImageExists() {
		s.Log("keepState attempting to start the existing VM and container")
		if err := p.fastDebugStart(ctx, s.OutDir()); err != nil {
			...
			if err := vm.DeleteImages(); err != nil {
				s.Fatalf("keepState failed and could not reset. You must manually delete %s to reset: %v", vm.TerminaImage, err)
			}
			s.Fatal("keepState failed. Try again, cryptohome will be cleared on the next run to reset to a good state")
		}
	} else {
...
// keepState returns whether the precondition should keep state from the previous
// test execution and try to recycle the VM.
func keepState(s *testing.PreState) bool {
	val, ok := s.Var("keepState")
