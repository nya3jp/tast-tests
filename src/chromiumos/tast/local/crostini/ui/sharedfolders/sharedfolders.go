...
// CheckNoSharedFolder checks there is no folder listed in the Manage shared folder page.
func (sf *SharedFolders) CheckNoSharedFolder(ctx context.Context, tconn *chrome.TestConn) error {
	s, err := settings.Open(ctx, tconn, settings.SettingsLinux, settings.ManageShareFolders)
	if err != nil || s == nil {
		return errors.Wrap(err, "failed to find Manage shared folders")
	}
...
func (sf *SharedFolders) UnshareAll(ctx context.Context, tconn *chrome.TestConn) error {
	...
	if err := sf.CheckNoSharedFolder(ctx, tconn); err != nil {
