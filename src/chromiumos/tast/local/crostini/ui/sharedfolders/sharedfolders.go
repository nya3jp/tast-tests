...
func (sf *SharedFolders) Unshare(ctx context.Context, tconn *chrome.TestConn, folders ...string) error {
	s, err := settings.Open(ctx, tconn, settings.PageNameLinux, settings.ManageSharedFolders)
	if err != nil {
...
func (sf *SharedFolders) CheckNoSharedFolders(ctx context.Context, tconn *chrome.TestConn, cont *vm.Container) error {
	sharedFoldersList, err := settings.GetLinuxSharedFolders(ctx, tconn, settings.PageNameLinux, settings.ManageSharedFolders)
	if err != nil {
