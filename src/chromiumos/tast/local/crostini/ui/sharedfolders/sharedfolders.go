...
// DismissShareConfirmDialog checks the content in the folder sharing confirmation dialog.
// OK is clicked if clickOK is true, otherwise Cancel is clicked.
func DismissShareConfirmDialog(ctx context.Context, tconn *chrome.TestConn, msg string, clickOK bool) error {
...
// DismissShareToast checks the content of the sharing toast and clicks Manage if clickManage is true.
func DismissShareToast(ctx context.Context, tconn *chrome.TestConn, clickManage bool) error {
	// Make sure that there is no sharing toast notification.
	toast, err := findShareToastNotification(ctx, tconn)
	if err != nil {
...
func (sf *SharedFolders) ShareMyfiles(ctx context.Context, tconn *chrome.TestConn, filesApp *filesapp.FilesApp, msg string, clickOK, clickManage bool) error {
	...
	if err := DismissShareConfirmDialog(ctx, tconn, msg, clickOK); err != nil {
		return errors.Wrap(err, "failed to check confirmation dialog")
	}

	if clickOK {
		// Check the toast notification.
		if err := DismissShareToast(ctx, tconn, clickManage); err != nil {
			return errors.Wrap(err, "failed to check toast notification")
		}
	}
