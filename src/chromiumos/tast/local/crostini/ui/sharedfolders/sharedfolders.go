...
func (sf *SharedFolders) CheckNoSharedFolders(ctx context.Context, tconn *chrome.TestConn, cont *vm.Container) error {
	sharedFoldersList, err := settings.GetLinuxSharedFolders(ctx, tconn, settings.SettingsLinux, settings.ManageShareFolders)
	if err != nil {
		return errors.Wrap(err, "failed to find the shared folders list")
	}
	if len(sharedFoldersList) != 0 {
		return errors.Errorf("failed to verify the shared folders list: want[], got %s", sharedFoldersList)
	}
	...
	if err := testing.Poll(ctx, func(ctx context.Context) error {
		...
		if len(list) != 1 || list[0] != "fonts" {
			return errors.Errorf("failed to verify the folders in /mnt/chromeos, want [fonts], got %q", list)
		}
		return nil
...
func (sf *SharedFolders) UnshareAll(ctx context.Context, tconn *chrome.TestConn, cont *vm.Container) error {
	...
	if len(sharedFoldersList) > 0 {
		if err := sf.Unshare(ctx, tconn, sharedFoldersList...); err != nil {
			return errors.Wrap(err, "failed to unshare all shared folders")
		}
	}

	if err := sf.CheckNoSharedFolders(ctx, tconn, cont); err != nil {
