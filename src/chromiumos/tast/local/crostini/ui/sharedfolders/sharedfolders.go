...
func (sf *SharedFolders) checkConfirmatioDialog(msg string) uiauto.Action {
	...
	return uiauto.Combine("to check content of the confirmation dialog",
		sf.ui.WaitUntilExists(ShareConfirmDialog.Self),
...
func (sf *SharedFolders) checkToastNotification() uiauto.Action {
	return uiauto.Combine("check content of the toast notification",
		sf.ui.WaitUntilExists(ShareToastNotification.Self),
...
func (sf *SharedFolders) Unshare(tconn *chrome.TestConn, cr *chrome.Chrome, folders ...string) uiauto.Action {
	return func(ctx context.Context) error {
...
func (sf *SharedFolders) CheckNoSharedFolders(tconn *chrome.TestConn, cont *vm.Container, cr *chrome.Chrome) uiauto.Action {
	return func(ctx context.Context) error {
		s, err := settings.OpenLinuxSettings(ctx, tconn, cr, settings.ManageSharedFolders)
		if err != nil {
			return errors.Wrap(err, "failed to find Manage shared folders")
...
func (sf *SharedFolders) UnshareAll(tconn *chrome.TestConn, cont *vm.Container, cr *chrome.Chrome) uiauto.Action {
	return func(ctx context.Context) error {
		s, err := settings.OpenLinuxSettings(ctx, tconn, cr, settings.ManageSharedFolders)
		if err != nil {
