...
func (s *Settings) UnshareFolder(ctx context.Context, folder string) error {
	...
	if err := ui2.ActionOnCondition(
		ui2.LeftClick(okButton),
		ui2.WithTimeout(5*time.Second).WaitUntilExists(unshareFailDlg))(ctx); err != nil {
		return errors.Wrap(err, "failed to click OK on Unshare failed dialog")
	}

	if err := ui2.ActionOnCondition(
		ui2.WaitUntilGone(folderButton),
		ui2.Exists(sharedFoldersList))(ctx); err != nil {
