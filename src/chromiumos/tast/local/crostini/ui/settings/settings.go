...
func (s *Settings) ClickRemove() uiauto.Action {
	return s.ui.WithInterval(time.Second).LeftClickUntil(removeLinuxButton, s.ui.WithTimeout(shortUITimeout).WaitUntilExists(RemoveConfirmDialog.Self))
}
...
func (s *Settings) Remove() uiauto.Action {
	return uiauto.Combine("remove Linux",
		s.ClickRemove(),
		s.ui.WithInterval(time.Second).LeftClickUntil(RemoveConfirmDialog.Delete, s.ui.WithTimeout(shortUITimeout).WaitUntilExists(RemoveLinuxAlert)),
