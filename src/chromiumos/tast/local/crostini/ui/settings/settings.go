...
var (
	...
	developersButton      = ui.FindParams{Name: "Developers", Role: ui.RoleTypeButton}
...
func navigateToDevelopers(ctx context.Context, tconn *chrome.TestConn) (err error) {
	...
	if err = uig.Do(ctx, tconn, uig.Retry(2, uig.FindWithTimeout(linuxBetaButton, uiTimeout).FocusAndWait(uiTimeout).LeftClick())); err != nil {
		...
		return uig.Do(ctx, tconn, uig.Retry(2, uig.FindWithTimeout(developersButton, uiTimeout).FocusAndWait(uiTimeout).LeftClick()))
	}
	return err
}
