...
// If the target disk size is smaller, set direction to "left", otherwise, set it to "right".
// The method will return if it reaches the target or the end of the slider.
// The real size might not be exactly equal to the target because the incremental of each move is 0.2GB.
// So when moving right, if the size is equal or bigger than the target size, it reaches the target.
// When moving left, if the size is equal or less than the target size, it reaches the target.
func ChangeDiskSize(ctx context.Context, tconn *chrome.TestConn, kb *input.KeyboardEventWriter, slider *uig.Action, direction string, targetDiskSize uint64) (uint64, error) {
	...
	for {
		...
		if (direction == "right" && size >= targetDiskSize) || (direction == "left" && size <= targetDiskSize) {
			return size, nil
		}
