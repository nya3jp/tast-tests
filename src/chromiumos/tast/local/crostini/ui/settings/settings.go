...
// ChangeDiskSize changes the disk size to targetDiskSize through JS code.
// If the target size is not a valid value in the disk size ticks, the closest
// larger valid size will be used.
// It throws an error if the target size is not in the valid range.
...
func (s *Settings) VerifyResizeResults(ctx context.Context, cont *vm.Container, sizeOnSlider string, size uint64) error {
	...
	// Allow some gap. When the size >= 100 GB, the size on text label will be
	// rounded up. E.g., if the size is 104.5 GB, the label shows 105 GB.
	// TODO(b/251693057): Remove the gap when the values on the labels match.
	if math.Abs(float64(bytesOnSetting-bytesOnSlider)) > SizeGB {
