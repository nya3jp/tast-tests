...
var (
	...
	BackupFileWindow      = nodewith.Name("Backup").Role(role.Window).ClassName("ExtensionViewViews").Onscreen()
	...
	RestoreFileWindow     = nodewith.Name("Restore").Role(role.Window).ClassName("ExtensionViewViews").Onscreen()
...
func (s *Settings) ResizeDisk(ctx context.Context, kb *input.KeyboardEventWriter, targetSize uint64) error {
	...
	if _, err := ChangeDiskSize(ctx, s.tconn, kb, ResizeDiskDialog.Slider, targetSize); err != nil {
