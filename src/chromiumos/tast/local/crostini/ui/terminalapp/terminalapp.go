...
func LaunchSSH(ctx context.Context, tconn *chrome.TestConn, sshArgs, password string) (*TerminalApp, error) {
	// Launch the Terminal App.
	if err := apps.Launch(ctx, tconn, apps.Terminal.ID); err != nil {
		return nil, errors.Wrap(err, "failed to launch the Terminal App through package apps")
	}
	...
	// Toggle a11y to get xterm.js to render a11y tree.
	setA11y := func(enabled bool) uiauto.Action {
		return func(ctx context.Context) error {
			return tconn.Call(ctx, nil, `tast.promisify(chrome.autotestPrivate.setAllowedPref)`, "settings.accessibility", enabled)
		}
	}
