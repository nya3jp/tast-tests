...
func (ta *TerminalApp) waitForPrompt(ctx context.Context, userName string) error {
	waitForPrompt := uig.FindWithTimeout(ui.FindParams{Role: ui.RoleTypeRootWebArea, Name: userName + "@penguin: ~"}, 30*time.Second).
...
// clickMenuItemOnShelf shuts down crostini by right clicking on the terminal app shelf icon.
func (ta *TerminalApp) clickMenuItemOnShelf(ctx context.Context, itemName string) error {
	...
	).WithNamef("TerminalApp.clickMenuItemOnShelf()")
...
func (ta *TerminalApp) RestartCrostini(ctx context.Context, keyboard *input.KeyboardEventWriter, cont *vm.Container, userName string) error {
	if err := ta.clickMenuItemOnShelf(ctx, "Shut down Linux (Beta)"); err != nil {
	...
	if err := ta.clickMenuItemOnShelf(ctx, "Close"); err != nil {
