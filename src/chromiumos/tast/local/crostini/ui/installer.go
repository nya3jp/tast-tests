...
func (p *Settings) OpenInstaller(ctx context.Context) (*Installer, error) {
	...
	return &Installer{p.tconn}, uig.Do(ctx, p.tconn,
		uig.Steps(
			...
			uig.FindWithTimeout(ui.FindParams{Role: ui.RoleTypeButton, Name: "Next"}, 10*time.Second).LeftClick()).WithNamef("OpenInstaller()"))
}
...
func (p *Installer) SetDiskSize(ctx context.Context, minDiskSize uint64) error {
	window := uig.FindWithTimeout(ui.FindParams{Role: ui.RoleTypeRootWebArea, Name: "Set up Linux (Beta) on your Chromebook"}, 10*time.Second)
	slider := window.FindWithTimeout(ui.FindParams{Role: ui.RoleTypeSlider}, 10*time.Second)

	if err := uig.Do(ctx, p.tconn, slider.FocusAndWait(3*time.Second)); err != nil {
		return errors.Wrap(err, "error in SetDiskSize()")
	}

	getSize := func() (string, error) {
		node, err := uig.GetNode(ctx, p.tconn, slider.FindWithTimeout(ui.FindParams{Role: ui.RoleTypeStaticText}, 10*time.Second))
		if err != nil {
...
func (p *Installer) Install(ctx context.Context) error {
	return uig.Do(ctx, p.tconn,
		uig.Steps(
			...
			uig.WaitUntilDescendantGone(ui.FindParams{Role: ui.RoleTypeButton, Name: "Cancel"}, 2*time.Minute)).WithNamef("Install()"))
}
