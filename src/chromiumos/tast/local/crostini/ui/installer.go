...
func (p *Installer) SetDiskSize(ctx context.Context, minDiskSize uint64, IsSoftMinimum bool) (uint64, error) {
	...
	if err := uiauto.Run(ctx, "to click radio custom and display slider",
		ui.LeftClick(customStaticText),
		ui.FocusAndWait(slider)); err != nil {
		return 0, err
	}
...
func (p *Installer) Install(ctx context.Context) error {
	...
	if err := uiauto.Run(ctx, "to click install and wait it to finish", ui.LeftClick(installButton), ui.WithTimeout(8*time.Minute).WaitUntilGone(installWindow)); err != nil {
