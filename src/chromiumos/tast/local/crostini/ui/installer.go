...
func (p *Installer) SetDiskSize(ctx context.Context, minDiskSize uint64) error {
	...
	for {
		...
		if size == lastSize {
			return errors.Errorf("error in SetDiskSize: could not set disk size to larger than %s, largest disk size available is %s (%s)", minDiskSize, sizeStr, size)
		}
		lastSize = size
...
func (p *Installer) Install(ctx context.Context) error {
	return uig.Do(ctx, p.tconn,
		uig.Steps(
			...
			uig.WaitUntilDescendantGone(ui.FindParams{Role: ui.RoleTypeButton, Name: "Cancel"}, 5*time.Minute)).WithNamef("Install()"))
}
