...
func (p *Installer) Install(ctx context.Context) error {
	...
	if err := uiauto.Run(ctx, "to click install and wait for finish", ui.LeftClick(installButton), ui.WithTimeout(8*time.Minute).WaitUntilGone(installWindow)); err != nil {
