...
// CloseApplication closes the application with the given ID.
...
// ExitIfShown conditionally will exit the app with the given ID if it is
// currently running (and otherwise does nothing).
func ExitIfShown(ctx context.Context, tconn *chrome.Conn, appID string) error {
	...
	return CloseApplication(ctx, tconn, appID)
}
