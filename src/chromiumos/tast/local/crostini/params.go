...
type Param struct {
	...
	// Add this for backward-compatibility when changing default container for
	// Crostini Apps testing from Buster to Bullseye.
	SilenceBuster bool
...
func MakeTestParamsFromList(t genparams.TestingT, baseCases []Param) string {
	...
	for _, testCase := range baseCases {
		...
		iterate := func(i iterator, bt browser.Type) {
			...
			name := testCase.Name
			if (i.debianVersion == vm.DebianBullseye && !testCase.MinimalSet) || (i.debianVersion == vm.DebianBuster && !testCase.SilenceBuster) {
