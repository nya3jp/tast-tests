	...
	for _, testCase := range baseCases {
		...
		for _, i := range it {
			...
			if i.vmMode == "component" {
				extraData = []string{getVMArtifact(i.arch)}
			}
			...
			if testCase.SelfManagedInstall {
				...
				precondition = fmt.Sprintf("crostini.StartedBy%sWithGaiaLogin%s()", strings.Title(i.vmMode), strings.Title(string(i.debianVersion)))
			} else {
