...
func RunCrostiniPostTest(ctx context.Context, p PreData) {
	...
	trySaveVMLogs(ctx, p.vmLogReader, dir, p.Chrome.NormalizedUser())
}
...
func trySaveVMLogs(ctx context.Context, vmLogReader *vm.LogReader, dir, user string) {
	if err := vmLogReader.TrySaveLogs(ctx, dir); err != nil {
