...
// container.
func RunCrostiniPostTest(ctx context.Context, s *testing.State) {
	dir, ok := testing.ContextOutDir(ctx)
	if !ok || dir == "" {
		testing.ContextLog(ctx, "Failed to get name of directory")
		return
	}
	trySaveContainerLogs(ctx, dir, s)
}
...
func trySaveContainerLogs(ctx context.Context, dir string, s *testing.State) {
	if s.PreValue() == nil {
		testing.ContextLog(ctx, "No precondition, is this a Crostini test?")
		return
	}
	cont := s.PreValue().(PreData).Container
	if cont == nil {
	...
	if cursor != "" {
		arg = append(cursorArgs, "--cursor")
		args = append(cursorArgs, cursor)
	}
