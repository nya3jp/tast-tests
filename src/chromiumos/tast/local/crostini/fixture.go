...
// crostiniFixture is a fixture to set up Crostini.
...
// FixtureData is the data SetUp returns.
...
func (f *crostiniFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	// Check container.
	// If there is no container, continue set up.
	// If it exists but does not work, fail fixture and clean up.
	// If it exists and works, return FixtureData.
	if f.cont != nil {
		if err := BasicCommandWorks(ctx, f.cont); err != nil {
			s.Log("Failed to check basic commands in the existing container: ", err)
			f.cont = nil
			f.close(ctx, s)
...
func (f *crostiniFixture) Reset(ctx context.Context) error {
	// Check container.
	// If there is no container or it does not work, return error.
	// If it exists and works, return nil.
	if f.cont == nil {
