...
type FixtureData struct {
	LacrosfixtV lacrosfixt.FixtValue
	CR          *chrome.Chrome
	Tconn       *chrome.TestConn
	Cont        *vm.Container
	KB          *input.KeyboardEventWriter
...
// Chrome return the chrome instance in FixtureData.
func (fd *FixtureData) Chrome() *chrome.Chrome {
	return fd.CR
}

// TestAPIConn return the TestConn instance in FixtureData.
func (fd *FixtureData) TestAPIConn() *chrome.TestConn {
	return fd.Tconn
}

// Mode return the SetupMode instance in FixtureData.
func (fd *FixtureData) Mode() lacrosfixt.SetupMode {
	if fd.LacrosfixtV != nil {
		return fd.LacrosfixtV.Mode()
	}
	return lacrosfixt.External
}

// LacrosPath return the Lacros path in FixtureData.
func (fd *FixtureData) LacrosPath() string {
	if fd.LacrosfixtV != nil {
		return fd.LacrosfixtV.LacrosPath()
	}
	return ""
}

// UserTmpDir return the user temp dir in FixtureData.
func (fd *FixtureData) UserTmpDir() string {
	if fd.LacrosfixtV != nil {
		return fd.LacrosfixtV.UserTmpDir()
	}
	return ""
}
...
func (f *crostiniFixture) SetUp(ctx context.Context, s *testing.FixtState) interface{} {
	...
	fd := FixtureData{nil, f.cr, f.tconn, f.cont, f.kb, f.postData}
