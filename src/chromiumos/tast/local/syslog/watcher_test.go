...
func TestHasMessageAddToFileNoUnsuccesfulReads(t *testing.T) {
	...
	const filename = "/tmp/TestHasMessageAddToFileNoUnsuccesfulReads"
	file, err := os.Create(filename)
	if err != nil {
		t.Fatalf("os.Create(%s) failed: %v", filename, err)
	}
...
func TestSplitAcrossWrites(t *testing.T) {
	const filename = "/tmp/TestSplitAcrossWrites"
	file, err := os.Create(filename)
	if err != nil {
		t.Fatalf("os.Create(%s) failed: %v", filename, err)
	}
...
func TestWaitForMessageMessageExistsBefore(t *testing.T) {
	const filename = "/tmp/TestWaitForMessageMessageExistsBefore"
	file, err := os.Create(filename)
	if err != nil {
		t.Fatalf("os.Create(%s) failed: %v", filename, err)
	}
...
func TestWaitForMessageMessageAddedDuring(t *testing.T) {
	const filename = "/tmp/TestWaitForMessageMessageAddedDuring"
	file, err := os.Create(filename)
	if err != nil {
		t.Fatalf("os.Create(%s) failed: %v", filename, err)
	}
	...
	go func() {
		...
		testing.Sleep(ctx.Background(), time.Second)
...
func TestWaitForMessageFails(t *testing.T) {
	const filename = "/tmp/TestWaitForMessageFails"
	file, err := os.Create(filename)
	if err != nil {
		t.Fatalf("os.Create(%s) failed: %v", filename, err)
	}
