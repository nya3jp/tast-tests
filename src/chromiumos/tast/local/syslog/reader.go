...
func NewReader(opts ...Option) (r *Reader, retErr error) {
	...
	// Seek to the last byte in the file if the file is not empty.
	// It is possible this will position us in the middle of a log message
	// (if syslogd is in the middle of writing something out), so we set
	// skipNext to true to discard the next line.
	// We don't seek to the end of the file so that Reader can handle all
	// log messages written after its construction.
