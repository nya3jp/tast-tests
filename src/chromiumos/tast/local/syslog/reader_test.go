...
const (
	...
	fakeLine4 = "2019-12-10T11:17:31.123456+09:00 CRIT foo[1234]: oops\n"
	fakeLines = fakeLine1 + fakeLine2 + fakeLine3 + fakeLine4
)
...
var (
	...
	fakeEntry4 = &Entry{
		Timestamp: time.Date(2019, 12, 10, 11, 17, 31, 123456000, jst),
		Severity:  "CRIT",
		Tag:       "foo[1234]",
		Program:   "foo",
		PID:       1234,
	...
	fakeEntries = []*Entry{fakeEntry1, fakeEntry2, fakeEntry3, fakeEntry4}
)
...
func TestReaderRead(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name:   "WriteAligned",
			writes: []string{fakeLine1, fakeLine2 + fakeLine3, fakeLine4},
			want:   fakeEntries,
		},
		{
			name:   "WriteUnaligned",
			writes: []string{fakeLine1[:10], fakeLine1[10:] + fakeLine2 + fakeLine3[:20], fakeLine3[20:] + fakeLine4},
			want:   fakeEntries,
		},
		{
		...
		// Option tests:
		{
			...
			writes: []string{fakeLines},
			want:   []*Entry{fakeEntry1, fakeEntry3, fakeEntry4},
		},
	} {
