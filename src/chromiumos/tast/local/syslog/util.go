...
// CollectLogs collects |target| log shards between timing of calling this
// function and the returned function.
func CollectLogs(target string) (func(context.Context, string) error, error) {
	// Store the current log state.
	oldInfo, err := os.Stat(target)
	if err != nil {
		return nil, errors.Wrapf(err, "filed to stat %s: %v", target, err)
	}
	return func(ctx context.Context, outDir string) error {
		dp := filepath.Join(outDir, filepath.Base(target))
		...
		sf, err := os.Open(target)
		if err != nil {
			return errors.Wrapf(err, "failed to read log: failed to open %s", target)
		}
		defer sf.Close()

		info, err := sf.Stat()
		if err != nil {
			return errors.Wrapf(err, "failed reading log position: failed to stat %s", target)
		}
		...
		if os.SameFile(info, oldInfo) {
			...
			if _, err = sf.Seek(oldInfo.Size(), 0); err != nil {
				return errors.Wrapf(err, "failed to read log: failed to seek %s", target)
			}

			if _, err = io.Copy(df, sf); err != nil {
				return errors.Wrapf(err, "failed to write log: failed to copy %s", target)
			}
		} else {
