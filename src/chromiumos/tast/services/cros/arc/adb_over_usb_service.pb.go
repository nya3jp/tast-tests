...
type EnableUDCResponse struct {
	// Whether or not the requested value is updated succesfully. If set true, UDC request executed successfully, and needs a device reboot. If set false, means no-ops or error occurred.
...
type ADBOverUSBServiceClient interface {
	// Set UDC Enabled to enable or disable USB Device Controller (UDC). Return true if the requested value updated succesfully. Otherwise return false.
