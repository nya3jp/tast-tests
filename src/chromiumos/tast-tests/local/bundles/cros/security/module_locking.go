...
func ModuleLocking(ctx context.Context, s *testing.State) {
	...
	if loaded, err := loadViaInitModule(modulePath); err != nil {
		s.Fatal("Failed to handle old-style init_module syscall test: ", err)
	} else if loaded {
...
func loadViaInitModule(modulePath string) (bool, error) {
	var r io.ReadCloser
	r, err := os.Open(modulePath)
	if err != nil {
		return false, errors.Wrapf(err, "failed to open module %q", modulePath)
	}
	defer r.Close()
	...
	return unix.InitModule(buf, "") == nil, nil
}
