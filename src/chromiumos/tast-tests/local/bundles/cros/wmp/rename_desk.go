...
func RenameDesk(ctx context.Context, s *testing.State) {
	...
	// Test 5: Duplicated name is allowed.
	if err := ash.CreateNewDesk(ctx, tconn); err != nil {
...
func checkDeskName(ctx context.Context, tconn *chrome.TestConn, id int, expectedName string) error {
	...
	if deskNum == 0 && id == 0 {
		...
		if defaultDeskInfo.Name != fullExpectedName {
			return errors.Errorf("unexpected name for desk %d: want %s; got %s", id, fullExpectedName, defaultDeskInfo.Name)
		}
	...
	if deskName != fullExpectedName {
		return errors.Errorf("unexpected name for desk %d: want %s; got %s", id, fullExpectedName, deskName)
	}
