...
import (
	...
	commoncrash "chromiumos/tast-tests/common/crash"
	"chromiumos/tast-tests/common/testexec"
	"chromiumos/tast/ctxutil"
	"chromiumos/tast/errors"
	"chromiumos/tast-tests/local/bundles/cros/crash/crash"
	"chromiumos/tast-tests/local/chrome"
	localcrash "chromiumos/tast-tests/local/crash"
	"chromiumos/tast-tests/local/syslog"
...
func testCoreFileRemovedInProduction(ctx context.Context, cr *chrome.Chrome, s *testing.State) {
	...
	reader, err := syslog.NewReader(ctx, syslog.Program(syslog.CrashReporter))
	if err != nil {
...
func checkFilterCrasher(ctx context.Context, shouldReceive bool) error {
	...
	if shouldReceive {
		...
		expected = fmt.Sprintf("Ignoring crash invocation '--user=%d:11:0:0:%s'", cmd.Process.Pid, crasherBasename)
	}
...
func testCrashBlocking(ctx context.Context, c *chrome.Chrome, s *testing.State) {
	...
	if err := localcrash.EnableCrashBlocking(ctx, filepath.Base(crash.CrasherPath)); err != nil {
		s.Fatal("Failed to enable crash blocking: ", err)
	}
	if err := checkFilterCrasher(ctx, false); err != nil {
		s.Errorf("testCrashBlocking failed for filter=%q: %v", filepath.Base(crash.CrasherPath), err)
	}

	if err := localcrash.DisableCrashBlocking(); err != nil {
		s.Fatal("Failed to disable crash blocking: ", err)
	}
	if err := checkFilterCrasher(ctx, true); err != nil {
