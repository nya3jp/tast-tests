...
func LoopDeviceBehaviour(ctx context.Context, s *testing.State) {
	...
	// TODO(b/233553312): This is an anti-pattern, but there is no other way
	// presently to constraint the test only to certain kernel versions. Fix
	// it when a kernel version test constraint is available.
	if ver, _, err := sysutil.KernelVersionAndArch(); err != nil {
		s.Fatal("Failed to get kernel version: ", err)
	} else if !ver.IsOrLater(5, 4) {
		s.Log("Only 5.4+ has modified loop device behaviour")
		return
	}
