...
`,
			expected: &Result{
				...
				Received:   5,
				Loss:       0,
				AvgLatency: (1159 + 1237 + 2047 + 2301 + 1369) * time.Microsecond / 5,
				ResponderIPs: []string{
					"192.168.87.20",
					"192.168.87.20",
					"192.168.87.20",
...
`,
			expected: &Result{
				...
				Received:   3,
				Loss:       40,
				AvgLatency: (1392 + 1678 + 2138) * time.Microsecond / 3,
				ResponderIPs: []string{
					"192.168.87.20",
					"192.168.87.20",
					"192.168.87.20"},
		...
		if !cmp.Equal(res, testcase.expected, cmp.Comparer(func(x, y float64) bool {
