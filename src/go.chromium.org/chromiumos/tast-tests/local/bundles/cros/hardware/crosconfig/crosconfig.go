...
var runGet = func(ctx context.Context, path, prop string) (string, error) {
	return crosconfig.Get(ctx, path, prop)
}
...
func CheckHardwareProperty(ctx context.Context, prop HardwareProperty) (bool, error) {
	output, err := runGet(ctx, "/hardware-properties", string(prop))
	if err != nil {
