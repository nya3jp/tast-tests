...
import (
	...
	"go.chromium.org/chromiumos/tast-tests/local/bundles/cros/vm/audioutils"
	"go.chromium.org/chromiumos/tast-tests/local/bundles/cros/vm/dlc"
...
func AudioLoopbackLatency(ctx context.Context, s *testing.State) {
	...
	for _, bufferSize := range bufferSizes {
		...
		// Let's improve loopback-latency and remove tiny hacks like this
		numValidLoops := len(measuredLatencies)
		if len(reportedLatencies) < numValidLoops {
			numValidLoops = len(reportedLatencies)
		}
		...
		perfValues.Set(
			...
			}, float64(loop-numValidLoops))

		if numValidLoops != loop {
			s.Logf("Requested %d loops, got %d for bufferSize=%s", loop, numValidLoops, bufferSize)
			continue
		}
