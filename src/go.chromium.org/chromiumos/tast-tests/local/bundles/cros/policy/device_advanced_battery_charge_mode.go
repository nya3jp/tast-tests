...
import (
	...
	"go.chromium.org/chromiumos/tast-tests/local/power/charge"
	"go.chromium.org/chromiumos/tast-tests/local/rtc"
	"go.chromium.org/chromiumos/tast-tests/local/wilco"
...
func DeviceAdvancedBatteryChargeMode(ctx context.Context, s *testing.State) {
	...
	// Advanced Battery Charge config is a time dependent policy, depends on DUT internal RTC.
	// For stable testing a mock-time needs to be applied.
	wRTC := wilco.RTC{
