...
import (
	...
	"go.chromium.org/chromiumos/tast-tests/local/audio"
...
func SetAudioNode(ctx context.Context, audioNode string) (string, error) {
	...
	cras, err := audio.NewCras(ctx)
	if err != nil {
		return deviceName, errors.Wrap(err, "failed to create Cras object")
	}
	deviceName, deviceType, err = cras.SelectedOutputDevice(ctx)
	if err != nil {
		return deviceName, errors.Wrap(err, "failed to get the selected audio device")
	}

	if deviceType != audioNode {
		testing.ContextLogf(ctx, "%s is not selected, selecting again", audioNode)
		if err := cras.SetActiveNodeByType(ctx, audioNode); err != nil {
			return deviceName, errors.Wrapf(err, "failed to select active device %s", audioNode)
		}
		deviceName, deviceType, err = cras.SelectedOutputDevice(ctx)
		if err != nil {
			return deviceName, errors.Wrap(err, "failed to get the selected audio device")
		...
		if deviceType != audioNode {
			return deviceName, errors.Errorf("failed to select the active device: got %q; want %q", deviceType, audioNode)
		}
	}
