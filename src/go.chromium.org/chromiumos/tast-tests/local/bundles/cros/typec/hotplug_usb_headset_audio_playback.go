...
import (
	...
	"go.chromium.org/chromiumos/tast-tests/local/cswitch"
...
func usbHeadsetEvents(ctx context.Context, usbKeyFile, usbType string) error {
	...
	re := regexp.MustCompile(fmt.Sprintf(`(?i)/dev/input/event([0-9]+):.*(%s).*`, regexp.QuoteMeta(usbType)))
