...
import (
	...
	"go.chromium.org/chromiumos/tast-tests/local/chrome/uiauto/filepicker/vars"
...
func App(ctx context.Context, tconn *chrome.TestConn, appID string) (*FilesApp, error) {
	if appID == vars.FilePickerPseudoAppID {
		...
		caller.Check(2, []string{"go.chromium.org/chromiumos/tast-tests/local/chrome/uiauto/filepicker"})
	}
