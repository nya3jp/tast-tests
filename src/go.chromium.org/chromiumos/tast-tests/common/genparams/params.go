...
func Ensure(t TestingT, file, params string) {
	...
	// Ensure that go.chromium.org/chromiumos/tast/testing is imported without alias.
	for _, im := range root.Imports {
		path, err := strconv.Unquote(im.Path.Value)
		if err != nil {
			continue
		}
		if path == "go.chromium.org/chromiumos/tast/testing" {
			if im.Name != nil {
				t.Fatalf("go.chromium.org/chromiumos/tast/testing must be imported without alias")
			}
		}
	}
