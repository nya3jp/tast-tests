...
TAST_GENERATE_UPDATE=1 ~/trunk/src/platform/tast/tools/go.sh test -count=1 go.chromium.org/chromiumos/tast-tests/common/genparams`,
	}
...
import mytesting "go.chromium.org/chromiumos/tast/testing"
...
`
	errors, _ := callEnsure(t, oldCode, testParams)
	wantErrors := []string{
		"go.chromium.org/chromiumos/tast/testing must be imported without alias",
	}
	if diff := cmp.Diff(errors, wantErrors); diff != "" {
		t.Errorf("Errors mismatch (-got +want):\n%s", diff)
	}
