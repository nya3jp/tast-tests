...
import (
	...
	"go.chromium.org/chromiumos/tast-tests/common/network/cmd"
...
func (r *Runner) Arping(ctx context.Context, targetIP, iface string, ops ...Option) (*Result, error) {
	conf := &config{timeout: -1, count: 10}
	for _, op := range ops {
		op(conf)
	}
	if conf.timeout < 0 {
...
// logArpingResult saves the output of arping to OutDir with random generated name.
