...
func (f *FlashProtect) UnmarshalerEctool(data []byte) error {
	dataStr := string(data)
	reActive := regexp.MustCompile(`Flash protect flags\:\s+(0x[[:xdigit:]]+)`)
	reValid := regexp.MustCompile(`Valid flags\:\s+(0x[[:xdigit:]]+)`)
	reWriteable := regexp.MustCompile(`Writable flags\:\s+(0x[[:xdigit:]]+)`)

	var fp FlashProtect
	var flags FlashProtectFlags

	result := reActive.FindStringSubmatch(dataStr)
	...
	result = reWriteable.FindStringSubmatch(dataStr)
	if result == nil || len(result) != 2 {
