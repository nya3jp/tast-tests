...
def main():
  ...
  url = 'gs://{prefix}/{test_dir}/{data_file}'.format(
      prefix=_GCP_PREFIX, test_dir=args.test_dir, data_file=args.data_file)
  ...
  if args.upload:
    ...
    ret = subprocess.run(['gsutil', 'mv', '-n', args.data_file, url],
                         errors=True)
    if ret.returncode != 0:
      print('Failed to upload file: ', ret.stderr)
