...
parser.add_argument("--fast-build-tast", action="store_const", dest="tast_binary", default="tast", const="~/go/bin/tast", help="Use the version of tast built by fast_build.sh")
parser.add_argument("--debug", action="store_const", dest="debug", const=True, help="Run tast test and wait for a debugger to attach")
parser.add_argument("--no-debug", action="store_const", dest="debug", const=False, help="Run tast without waiting for a debugger to attach")
